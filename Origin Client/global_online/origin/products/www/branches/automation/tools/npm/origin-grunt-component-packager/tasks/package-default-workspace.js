...
module.exports = function(grunt, options) {
     ...
     */
    function writeApplicationData(source, directiveGroup, crxNodeName, componentsDefaultValues, invalidDefaults, skipDefault, overwriteExistingDefaultValue, fixtureFilterPaths) {
        generateApps(source, directiveGroup, crxNodeName);
        generateDesign(source, directiveGroup, crxNodeName);
        generateContent(source, directiveGroup, crxNodeName, componentsDefaultValues, invalidDefaults, skipDefault, overwriteExistingDefaultValue, fixtureFilterPaths);
    }
   ...
   /**
     ...
     * @param {string} crxnodeName the directive node name eg. origin
     */
    function generateContent(source, directiveGroup, crxNodeName, componentsDefaultValues, invalidDefaults, skipDefault, overwriteExistingDefaultValue, fixtureFilterPaths) {
        var pageCrxNode = options.defaultsAppsCrxPath + '/components/pages/' + directiveGroup + '/' + crxNodeName;
        ...
        grunt.file.write(webContentPath + '/' + source.info.directiveName + '/.content.xml', defaultsAppsBuilder.buildWebContentXml(
            ...
            directiveGroup,
            source,
            componentsDefaultValues,
            invalidDefaults,
            skipDefault,
            overwriteExistingDefaultValue,
            fixtureFilterPaths));
    }
