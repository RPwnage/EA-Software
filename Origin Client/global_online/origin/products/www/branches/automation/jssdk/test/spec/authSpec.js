    ...
    it('Origin.auth.login()', function(done) {
        ...
        if (Origin.client.isEmbeddedBrowser()) {
            ...
            var authSuccess = function() {
                ...
                expect(Origin.auth.isLoggedIn()).toEqual(true, 'Origin.auth.isLoggedIn() should return true');
                ...
                expect(Origin.user.userStatus()).toEqual('', 'Origin.user.userStatus() should have empty userStatus');
                ...
                expect(Origin.user.tfaSignup()).toEqual(false, 'Origin.user.tfaSignup() should return false if the user has no signed up fpr TFA');
            ...
            var authSuccess = function() {
                ...
                expect(Origin.user.personaId()).toEqual(1000101795502, 'Origin.user.personaId() should return correct personaId');
                expect(Origin.user.userPid()).toEqual('jssdkfunctionaltest', 'Origin.user.userPid() should return correct userPid');
                expect(Origin.user.userStatus()).toEqual('ACTIVE', 'Origin.user.userStatus() should return correct userStatus');
                expect(Origin.user.underAge()).toEqual(false, 'Origin.user.underAge() should return false');
                expect(Origin.user.dob()).toEqual('1980-01-01', 'Origin.user.dob() should return correct date of birth');
                expect(Origin.user.tfaSignup()).toEqual(false, 'Origin.user.tfaSignup() should return false if the user has no signed up fpr TFA');
    ...
    it('Origin.auth.logout()', function(done) {
        ...
        if (Origin.client.isEmbeddedBrowser()) {
            ...
            var authLoggedout = function() {
                ...
                expect(Origin.auth.isLoggedIn()).toEqual(false, 'Origin.auth.isLoggedIn() should return false');
                asyncTicker.clear(done);
