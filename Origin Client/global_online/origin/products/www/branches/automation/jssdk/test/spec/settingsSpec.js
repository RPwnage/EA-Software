describe('Origin Settings API', function() {
    ...
    it('Origin.settings.email.sendEmailVerification()', function(done) {
        var asyncTicker = new AsyncTicker();
        asyncTicker.tick(1000, done, 'sendEmailVerification did not complete in the allotted time');
        Origin.settings.email.sendEmailVerification().then(function(result) {
            var expected = {
                confirm: true
            }
            expect(result).toEqual(expected)
    ...
    it('Origin.settings.email.sendOptinEmailSetting()', function(done) {
        var asyncTicker = new AsyncTicker();
        asyncTicker.tick(1000, done, 'sendOptinEmailSetting did not complete in the allotted time');
        Origin.settings.email.sendOptinEmailSetting().then(function(result) {
            var expected = {
                confirm: true
            }
            expect(result).toEqual(expected)
