describe('Origin Atom API', function() {
    ...
    it('Origin.atom.atomFriendsForUser() First Page', function(done) {

        var asyncTicker = new AsyncTicker();

        asyncTicker.tick(1000, done, 'Expect onResolve() to be executed');

        Origin.atom.atomFriendsForUser('1000120382991', 1).then(function(result){
	...
	it('Origin.atom.atomFriendsForUser() Non-existent Page', function(done) {

        var asyncTicker = new AsyncTicker();

        asyncTicker.tick(1000, done, 'Expect onResolve() to be executed');

        Origin.atom.atomFriendsForUser('1000120382991', 2).then(function(result){
	...
	it('Origin.atom.atomFriendCountForUser()', function(done) {
		
		var asyncTicker = new AsyncTicker();
		asyncTicker.tick(1000, done, 'Expected atomFriendCountForUser() to be resolved');
		
		Origin.atom.atomFriendCountForUser('1000120382991').then(function(result) {
			expect(result).toEqual('3', 'Expected to get a friend count of 3');
			asyncTicker.clear(done);
		}).catch(function(error) {
			expect().toFail(error.message);
			asyncTicker.clear(done);
