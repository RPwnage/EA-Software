...
(function(){
    ...
    var MAX_RETRIES = 3,
        RETRY_INTERVAL = 200,
        CAROUSEL_SLIDER_SELECTOR = '.origin-store-carousel-carousel-slidearea',
        CAROUSEL_HEADER_SELECTOR = '.origin-store-carousel-carousel-header';
    ...
    function OriginStoreCarouselCarouselCtrl($scope, UtilFactory) {
        ...
        $scope.animate = function(slideNumber){
            var tilesInView = $scope.carouselState.totalTiles - slideNumber * $scope.carouselState.maxTilesInView,
                carouselWidth = $scope.carouselState.carouselWidth,
                emptySpace = Math.max(0, carouselWidth - tilesInView * $scope.carouselState.tileWidth),
                distance;

            // first slide is always 0px offset from left
            if(slideNumber === 0) {
                distance = 0;
            } else {
                distance = slideNumber * $scope.carouselState.maxTilesInView * $scope.carouselState.tileWidth - emptySpace;
            }
    ...
    function originStoreCarouselCarousel(AppCommFactory, UtilFactory, $compile, $timeout) {

        function originStoreCarouselCarouselLink(scope, elem, attrs, ctrl) {
            var initializeTimeoutHandle,
                finishedTimeoutHandle,
                setupTimeoutHandle,
                setViolatorHeightTimeout;

            function onCarouselSetup(event) {
                event.stopPropagation();
                setup();
            }

            function onCarouselFinished(event) {
                event.stopPropagation();
                finishedLoading();
            }
            /**
            ...
            function initialize(){
                ...
                elem.find(CAROUSEL_HEADER_SELECTOR).append(controls);
                ...
                elem.on('carousel:setup', onCarouselSetup);

                elem.on('carousel:finished', onCarouselFinished);
            ...
            /**
             * Using the same retry mechanism as the carousel
             */
            function checkViolatorHeight(){
                var tileContainer = elem.find(CAROUSEL_SLIDER_SELECTOR),
                    productTiles = tileContainer.find('li:visible'),
                    $carouselContainer = elem.find('.origin-store-carousel-product-carousel-container'),
                    tries=0;

                function startViolatorTimer(){
                    setViolatorHeightTimeout = $timeout(setViolatorHeight, RETRY_INTERVAL, false);
                }

                function setViolatorHeight(){
                    var $violator,
                        $violatorHeight;

                    if ($carouselContainer) {
                        $violator = productTiles.find('.origin-storegametile-violator');
                        if ($violator.length && $violator.get(0)) {
                            $violatorHeight = $violator.get(0).offsetHeight;
                            if ($violatorHeight) {
                                var carouselHeight = $carouselContainer.height() + $violatorHeight;
                                $carouselContainer.css('min-height', carouselHeight + 'px');
                                $timeout.cancel(setViolatorHeightTimeout);
                            }
                        } else {
             ...
             */
            function setup() {
                var carousel = elem.find(CAROUSEL_HEADER_SELECTOR),
                    tileContanier = elem.find(CAROUSEL_SLIDER_SELECTOR),
                    productTiles = tileContanier.find('li:visible'),
                ...
                if(productTiles.length > 0 || angular.isDefined(scope.carouselState.totalTiles) && scope.carouselState.totalTiles === 0){
                    finishedLoading();
                }
                ...
                scope.carouselState.carouselWidth = carouselWidth;
                ...
                if(scope.carouselState.totalTiles > 0 && scope.carouselState.maxTilesInView > 0) {
                    ...
                    if(scope.retries < MAX_RETRIES){
                        setupTimeoutHandle = $timeout(setup, RETRY_INTERVAL); // This shouldn't really be here. The modal isn't drawing in the correct time
                    }
            ...
            function finishedLoading() {
                finishedTimeoutHandle = $timeout(function() {
            ...
            function onDestroy() {
                // unbind from swipe events only on touch-enabled devices
                if (UtilFactory.isTouchEnabled()) {
                    elem.off('swipeleft', ctrl.goToNext);
                    elem.off('swiperight', ctrl.goToPrev);
                }

                elem.off('carousel:setup', onCarouselSetup);
                elem.off('carousel:finished', onCarouselFinished);
                AppCommFactory.events.off('carousel:resetup', resetCarousel);
                $(window).off('resize', resize);
                $timeout.cancel(initializeTimeoutHandle);
                $timeout.cancel(finishedTimeoutHandle);
                $timeout.cancel(setupTimeoutHandle);
                $timeout.cancel(setViolatorHeightTimeout);
            }

            scope.$on('$destroy', onDestroy);

            initializeTimeoutHandle = $timeout(initialize, 0);
