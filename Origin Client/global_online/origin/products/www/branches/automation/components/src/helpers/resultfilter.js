...
(function () {
    ...
    function ResultFilterFactory(ObjectHelperFactory) {
        ...
        function getInstance(filters) {
            ...
            /**
             * Add filter object to the internal data model filters reference by key filterName
             * @param {mixed} filter -  filter object containing properties and values
             *    For example:
             *        {filterType: NORMAL_FILTER, value: false}
             *
             * @param {mixed} filterName - filter option name
             *    For example:
             *      -- bundle is a filter option under category gameType
             * @return {void}
             */
            function addFilter(filter, filterName) {
                filters[filterName] = createFilterObject(filter);
            }
            ...
            function hasCountAndVisible(filterObject) {
               return filterObject.count > 0 && filterObject.visible;
            }
            ...
            function setFilterAttribute(filterName, attr, value) {
                ...
                if (filterObject) {
                    var assignable = _.isObject(value) ? value[attr] : value;
                    filterObject[attr] = _.isNumber(assignable) ? parseInt(assignable, 10) : assignable;
                }
            }
            ...
            function setFilterVisibility(filterName, visibility) {
                setFilterAttribute(filterName, 'visible',  visibility);
            }
            ...
            function isFilterSelectableAndVisible() {
                return length(filter(hasCountAndVisible, filters)) > 0;
            }
            ...
            /**
             * Returns values for all the default filters that can be applied as a Javascript object.
             * @return {object}
             */
            function getDefaultFilterValues() {
                return map(getProperty('default'), filter(isApplicable, filters));
            }
            ...
            return {
                ...
                isFilterSelectableAndVisible: isFilterSelectableAndVisible,
