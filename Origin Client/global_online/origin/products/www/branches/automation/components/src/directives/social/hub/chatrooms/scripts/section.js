...
(function () {
    ...
    * The controller
	*/
    function OriginSocialHubChatroomsSectionCtrl($scope, $filter, SocialDataFactory, LocalStorageFactory) {
        $scope.chatrooms = [];
        ...
        this.toggleSectionVisible = function() {
            if ($scope.$viewportElement.is(':hidden')) {                
                self.openSection();
                LocalStorageFactory.delete('origin-social-hub-section-CHATROOMSLIST-'+$scope.sectionName+'-closed-'+Origin.user.originId(), true);               
            } else {
                self.closeSection();                    
                LocalStorageFactory.set('origin-social-hub-section-CHATROOMSLIST-'+$scope.sectionName+'-closed-'+Origin.user.originId(), 'true', true);              
            }
            $scope.$digest();
        };
    ...
    angular.module('origin-components')
        ...
        .directive('originSocialHubChatroomsSection', function(ComponentsConfigFactory, LocalStorageFactory) {
            ...
            return {
                ...
                link: function(scope, element, attrs, ctrl) {
                    ...
                    if (LocalStorageFactory.get('origin-social-hub-section-CHATROOMSLIST-'+scope.filter+'-closed-'+Origin.user.originId(), true)) {
                        ctrl.closeSection();
                    }
                } // link
            };
