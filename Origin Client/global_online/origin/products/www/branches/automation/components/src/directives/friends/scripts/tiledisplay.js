...
(function() {
    ...
    function OriginFriendsTiledisplayCtrl($scope, DialogFactory) {
        ...
        this.displayFriends = function() {
            ...
            $scope.showFriendTiles = $scope.friends.length > 0;
            $scope.$emit('friendsTileDisplayVisible', $scope.showFriendTiles);
        };

        $scope.$watchCollection('friendslist', updateFriends);
        this.friends = [];
    ...
    function originFriendsTiledisplay(ComponentsConfigFactory, $window) {
        ...
        function originFriendsTiledisplayLink(scope, element, attrs, ctrl) {
            ...
            function determineMaxFriends() {
                var avatarList = element.find('.origin-avatarlist'),
                    listWidth = avatarList.length > 0 ? avatarList[0].offsetWidth : 0;
                ctrl.maxFriends = Math.floor(listWidth / AVATAR_WIDTH) - 1;
            }
