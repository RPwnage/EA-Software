    ...
    function GamesNonOriginFactory($timeout, ComponentsLogFactory, ComponentsConfigFactory, ObjectHelperFactory, DialogFactory) {
        ...
        function logError(message) {
            return function (error) {
                ComponentsLogFactory.error('GamesNonOriginFactory: ' + message, error);
            };
        }

        /**
         * Refresh nogs and the game library
         */
        function refreshNogsAndGameLibrary() {
            refreshNogs().then(function() {
                myEvents.fire('GamesNonOriginFactory:nogsRefreshed');
            });
        }
        ...
        function handleNogRefresh(updatedGames) {
            var updateNogs = _.map(updatedGames, updateNog);
            return Promise.all(updateNogs);
        }
        ...
        function updateCustomBoxArt(offerId) {
            return function (boxArt) {
                ...
                if (games.hasOwnProperty(offerId)) {
                    ...
                    gameData.packArtLarge = encodedBoxArt[0];
                    gameData.packArtMedium = encodedBoxArt[0];
                    gameData.packArtSmall = encodedBoxArt[0];
        ...
        Origin.events.on(Origin.events.CLIENT_GAMES_NOGUPDATED, refreshNogsAndGameLibrary);
