...
(function(){
    ...
    */
    function originStoreMediaCarouselVideoitem(YoutubeFactory, ComponentsConfigFactory) {
        ...
        * The directive link
        */
        function originStoreMediaCarouselVideoitemLink(scope) {
            scope.src = '';
            var watcherUnbind = scope.$watch('videoId', function(videoId){
                if (videoId){
                    watcherUnbind();
                    scope.src = YoutubeFactory.getThumbnailImage(videoId);
                }
            });
        }
