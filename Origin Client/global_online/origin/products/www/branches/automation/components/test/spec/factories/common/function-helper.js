...
describe('Function Helper Factory', function() {
    ...
    describe('not - negate the result of a function', function(){
        ...
        var stubTruthy = function() {
            return 'a';
        };

        var stubFalsy = function() {
            return;
        };

        it('Will negate a simple function with no arguments', function() {
            expect(functionHelper.not(stubTrue)()).toBe(false);
        });

        it('Will negate a function with multiple arguments', function() {
            expect(functionHelper.not(isIdentical)(1,1)).toBe(false);
        });

        it('Will negate a function with multiple arguments', function() {
            expect(functionHelper.not(isIdentical)(2,1)).toBe(true);
        });

        it('Will negate a simple truthy function', function() {
            expect(functionHelper.not(stubTruthy)()).toBe(false);
        });

        it('Will negate a simple falsy function', function() {
            expect(functionHelper.not(falsy)()).toBe(true);
        });
    })
});