...
(function() {
    ...
    function ObjectHelperFactory($filter) {
        ...
        function filter(filterFunction, object) {
            var inputType = _.isArray(object) ? 'array' : 'object',
        ...
        /**
         * Filter a collection [Object[]] without type coercion to an object
         *
         * @param {function} filterFunction - function to apply to each property
         * @return {Function}
         */
        function filterCollectionBy(filterFunction) {
            return _.partial(_.filter, _, filterFunction);
        }
        ...
        /**
         * Handle an empty/falsy and hydrated/truthy object using a callback for each case
         * @param {Function} hydratedFunc the callback to run if the collection is truthy and not empty
         * @return {Array} hydratedFuncArgs the hydrated func's args
         * @param {Function} emptyFunc the callback to run if the collection is falsy or empty
         * @return {Array} emptyFuncArgs the empty func's args
         */
        function maybe(hydratedFunc, hydratedFuncArgs, emptyFunc, emptyFuncArgs) {
            /**
             * Inspect the contents of data and react with the correct callback
             * @param  {mixed} data the data to analyze
             * @return {mixed} the applied function
             */
            return function(data) {
                if((!data || _.isEmpty(data)) && _.isFunction(emptyFunc)) {
                    return emptyFunc.apply(undefined, emptyFuncArgs)(data);
                } else if (_.isFunction(hydratedFunc)) {
                    return hydratedFunc.apply(undefined, hydratedFuncArgs)(data);
                } else {
                    return data;
                }
            };
        ...
        /**
         * Merge array pairs with into single object with pairs combined to their common keys.
         *     example: [['a','1'], ['a','2'], ['b','3']] -> {'a':['1','2'], 'b':['3']}
         * @param  {Array} pairs Sets of key/value pairs as nested arrays
         * @return {object}
         */
        function mergeToObject(pairs) {
            var result = {};
        ...
        /**
         * Returns the result of Promise.all
         *
         * @param {Array or Object|Promise} data - array or object of Promises to resolve
         * @return {Array} resolved promise array
         */
        function fetchPromises(data) {
            return Promise.all(toArray(data));
        }
        ...
        function hasTruth(list) {
            ...
            return (toArray(list).indexOf(true) > -1);
        }

        /**
         * Cast output to boolean
         * @param  {mixed} data the information to cast
         * @return {boolean}
         */
        ...
        /**
         * The every() mapper for asserting all values are true
         * @param  {mixed} value the object to analyze
         * @return {Boolean}
         */
        function testForTruthiness(value) {
            return toBoolean(value);
        }

        /**
         * Given a promise.all chain of bool/mixed values, ensure all responses are truthy
         * @param  {Array} data the data to analyze
         * @return {Boolean}
         */
        function assertAllValuesTruthy(data) {
            if(_.isArray(data)) {
                return data.every(testForTruthiness);
            } else if(_.isObject(data)) {
                return _.toArray(data).every(testForTruthiness);
            }
        ...
        return {
            ...
            filter: filter,
            filterBy: filterBy,
            filterCollectionBy: filterCollectionBy,
            where: where,
            length: length,
            forEach: forEach,
            ...
            hasTruth: hasTruth,
            toBoolean: toBoolean,
            assertAllValuesTruthy: assertAllValuesTruthy
        };
    }
