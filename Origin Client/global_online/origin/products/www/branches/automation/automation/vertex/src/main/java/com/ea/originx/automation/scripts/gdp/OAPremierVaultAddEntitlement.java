...
/**
 * Test a premier vault entitlement can be added to library through origin
 * premier subscription
 *
 * @author cdeaconu
 */
public class OAPremierVaultAddEntitlement extends EAXVxTestTemplate{
    
    @TestRail(caseId = 3486254)
    @Test(groups = {"gdp", "full_regression"})
    public void testPremierVaultGameEntitlement(ITestContext context) throws Exception {

        final OriginClient client = OriginClientFactory.create(context);
        final UserAccount userAccount = AccountManagerHelper.registerNewThrowAwayAccountThroughREST();
        final EntitlementInfo entitlement = EntitlementInfoHelper.getEntitlementInfoForId(EntitlementId.GDP_PREMIER_VAULT_ENTITLEMENT);
        ...
        logFlowPoint("Navigate to GDP of a game that has an edition in the 'Premier Vault' and is not owned."); // 3
