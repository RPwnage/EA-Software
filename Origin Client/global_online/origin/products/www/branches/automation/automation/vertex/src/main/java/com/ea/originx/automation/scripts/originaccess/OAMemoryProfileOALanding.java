 ...
 * and the 'Origin Access' landing page in order to expose memory leaks
 *
 * @author jdickens
 */

public class OAMemoryProfileOALanding extends EAXVxTestTemplate {

    @Test(groups = {"originaccess", "memory_profile"})
    public void testMemoryProfileOALanding(ITestContext context) throws Exception {
        ...
        for (int i = 0; i < 15; i++) {
            logFlowPoint("Navigate to the 'Application Settings' page and then the 'Origin Access' landing page, and perform garbage collection"); // 2 - 16
        }
        ...
        boolean isOriginAccessPageReached = false;
        ...
        for (int i = 0; i < 15; i++) {
            ...
            // Select the 'Origin Access' page from the nav bar
            navBar.clickOriginAccessLink();
            isOriginAccessPageReached = originAccessPage.verifyPageReached();

            // Perform garbage collection
            isGarbageCollectionSuccessful = garbageCollect(driver, context);

            if (isAppSettingsPageReached && isOriginAccessPageReached && isGarbageCollectionSuccessful) {
                logPass("Successfully navigated to the 'Application Settings' page followed by the 'Origin Access' landing page "
                        + "and then perform garbage collection");
            } else {
                logFail("Failed to navigate to the 'Application Settings' page followed by the 'Origin Access' landing page "
