 ...
 * and the 'Browse Games' page in order to expose memory leaks
 *
 * @author jdickens
 */

public class OAMemoryProfileBrowseGames extends EAXVxTestTemplate {

    @Test(groups = {"browsegames", "memory_profile"})
    public void testPerformanceBrowseGames(ITestContext context) throws Exception {
        ...
        for (int i = 0; i < 15; i++) {
            logFlowPoint("Navigate to the 'Application Settings' page and then the 'Browse Games' page, and perform garbage collection"); // 2 - 16
        }
        ...
        boolean isBrowseGamesPageReached = false;
        ...
        for (int i = 0; i < 15; i++) {
            ...
            // Select the 'Browse Games' page from the nav bar
            navBar.gotoBrowseGamesPage();
            isBrowseGamesPageReached = browseGamesPage.verifyBrowseGamesPageReached();

            // Collect the garbage
            isGarbageCollectionSuccessful = garbageCollect(driver, context);

            if (isAppSettingsPageReached && isBrowseGamesPageReached && isGarbageCollectionSuccessful) {
                logPass("Successfully navigated to the 'Application Settings' page followed by the 'Browse Games' page "
                        + "and perform garbage collection");
            } else {
                logFail("Failed to navigate to the 'Application Settings' page followed by the 'Browse Games' page "
