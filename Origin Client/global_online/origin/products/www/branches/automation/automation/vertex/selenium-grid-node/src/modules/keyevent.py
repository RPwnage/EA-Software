...
"""
Module for Key Event operation interface
"""
...
class Type:
    ## POST method to generate keyboard event to simulate user input.
    #
    # @param delay (optional) -- interval between key event in millisecond. Example: <URL>?delay=10
    # @param POST message body -- a series key to be generated.
    # @result 'ok' if succeed
    def POST(self):
        try:
            try:
                delay = float(web.input().delay)
            except:
                delay = 50
            for event in keyboard_stream(web.data()):
                SendInput(event)
...
class Multikey:
    ## GET method to simulate multiple keys press and release.
    #
    # @param combo (mandatory) -- multiple key codes seperate by '+' sign. Example (CONTROL+SHIFT): <URL>?combo=17+70
    # @param delay (optional) -- interval between key press and release in millisecond. Example: <URL>?delay=1500
    # @result 'ok' if succeed
    def GET(self):
        try:
            # in case of a plus sign represents space, convert all plus sign to space then split by space
            keys = web.input().combo.replace('+', ' ').split()
            # Simulates key(s) press
            for key in keys:
                SendInput(Keyboard(int(key), KEYEVENTF_EXTENDEDKEY))
            # holding
            try:
                delay = float(web.input().delay)
                time.sleep(delay / 1000)
                ...
                pass
            # Simulates key(s) release
            for key in reversed(keys):
                SendInput(Keyboard(int(key), KEYEVENTF_EXTENDEDKEY | KEYEVENTF_KEYUP))
            return 'ok'
        except Exception as e:
            print(e)
            return web.internalerror()
...
keyeventop = web.application(urls, locals())