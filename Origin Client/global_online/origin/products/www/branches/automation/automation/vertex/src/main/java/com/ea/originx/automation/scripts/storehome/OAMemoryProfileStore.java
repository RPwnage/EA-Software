 ...
 * and the 'Store' page in order to expose memory leaks
 *
 * @author jdickens
 */

public class OAMemoryProfileStore extends EAXVxTestTemplate {

    @Test(groups = {"storehome", "memory_profile"})
    public void testMemoryProfileStore(ITestContext context) throws Exception {
        ...
        for (int i = 0; i < 15; i++) {
            logFlowPoint("Navigate to the 'Application Settings' page and then the 'Store' page, and finally perform garbage collection"); // 2 - 16
        }
        ...
        boolean isStorePageReached = false;
        ...
        for (int i = 0; i < 15; i++) {
            ...
            // Select the 'Store' page from the nav bar
            navBar.gotoStorePage();
            isStorePageReached = storePage.verifyStorePageReached();

            // Perform garbage collection
            isGarbageCollectionSuccessful = garbageCollect(driver, context);

            if (isAppSettingsPageReached && isStorePageReached && isGarbageCollectionSuccessful) {
                logPass("Successfully navigated to the 'Application Settings' page followed by the 'Store' page "
                        + "and then performed garbage collection");
            } else {
                logFail("Failed to navigate to the 'Application Settings' page followed by the 'Store' page "
