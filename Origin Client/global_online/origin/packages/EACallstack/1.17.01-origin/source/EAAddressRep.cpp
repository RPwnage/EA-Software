...
namespace ARLocal
{
    ...
    bool FileNamesMatch(const char* pFirst, const char* pSecond)
    {
        ...
        EA::IO::Path::PathString8 sFirst  = EA::IO::Path::GetFileName(pFirst);
        EA::IO::Path::PathString8 sSecond = EA::IO::Path::GetFileName(pSecond);
    ...
    mFileOffset(0)
{
    ...
    for(size_t i = 0; i < sizeof(mAddressRep) / sizeof(mAddressRep[0]); i++)
    {
...
{
    if(!pModuleName)
        return GetModuleBaseAddressByHandle(0);
    ...
    *EA::IO::Path::GetFileExtension(pModuleFileName) = 0;
        ...
        if(hPSAPI)
        {
            ...
            DWORD        cbNeeded = 0;
            char         pBaseName[MAX_PATH]; pBaseName[0] = 0;
            char         pFullName[MAX_PATH]; pFullName[0] = 0;
            MODULEINFO   mi;
            ...
            if(pEnumProcessModules(hProcess, hModuleBuffer, sizeof(hModuleBuffer), &cbNeeded))
            {
                ...
                for(size_t i = 0; i < moduleCount; i++)
                {
                    ...
                    pGetModuleBaseName   (hProcess, hModuleBuffer[i], pBaseName, MAX_PATH);
                    pGetModuleFileNameEx (hProcess, hModuleBuffer[i], pFullName, MAX_PATH);
...
EACALLSTACK_API uint64_t GetModuleBaseAddressByHandle(ModuleHandle moduleHandle)
{
    ...
    #if EA_WINAPI_FAMILY_PARTITION(EA_WINAPI_PARTITION_DESKTOP) // LoadLibrary and GetModuleInformation are supported with desktop libraries only.
            ...
            if(pEnumProcessModules(hProcess, hModuleBuffer, sizeof(hModuleBuffer), &cbNeeded))
            {
                ...
                if(pModuleInfoArray)
                {
                    ...
                    for(size_t i = 0; i < moduleCount; i++)
                    {
                        ...
                        if(result)
                        {
                            char16_t path16[EA::IO::kMaxPathLength]; path16[0] = 0;
                            char8_t  path8[EA::IO::kMaxPathLength];

                            GetModuleFileNameExW(hProcess, hModuleBuffer[i], path16, EA::IO::kMaxPathLength);
                            EA::StdC::Strlcpy(path8, path16, IO::kMaxPathLength);
            ...
            if((buffer[0] == 0x7f) && (buffer[1] ==  'E') && (buffer[2] ==  'L') && (buffer[3] == 'F'))
            {
                ...
                if(!bFileIsGHSElf)
                {
                  ...
                  //const bool b64BitELF        = (buffer[EI_CLASS] == ELFCLASS64);
                    ...
                    memcpy(&elfProcessor, buffer + 18, sizeof(uint16_t));
                                            ...
                                            bool bDelayLoad, EA::Allocator::ICoreAllocator* pCoreAllocator)
{
    ...
    IAddressRepLookup* const pARL = MakeAddressRepLookupFromDatabase(pDatabaseFilePath, mpCoreAllocator);

    if(pARL)
    {
        if(baseAddress) // We might have a problem if we encounter a platform whereby 0x00000000 is a valid base addresss.
            pARL->SetBaseAddress(baseAddress);
...
bool AddressRepLookupSet::RemoveDatabaseFile(const wchar_t* pDatabaseFilePath)
{
    for(AddressRepLookupList::iterator it = mAddressRepLookupList.begin(); it != mAddressRepLookupList.end(); ++it)
    {
        ...
        if(EA::StdC::Stricmp(pDatabaseFilePath, pARL->GetDatabasePath()) == 0)
        {
...
void AddressRepLookupSet::AutoDatabaseAdd()
{
    if(!mbLookedForDBFiles)
    {
        ...
        wchar_t dbPath[EA::IO::kMaxPathLength];
        ...
        // Under GCC, the application file (ELF file) also contains the debug information.
            ...
            if(IO::File::Exists(dbPath))
                AddDatabaseFile(dbPath, 0, true, mpCoreAllocator);
        #endif

        wchar_t* const pExtension = IO::Path::GetFileExtension(dbPath);

        // See if <app>.map exists.
        wcscpy(pExtension, L".map");
        if(IO::File::Exists(dbPath))
            AddDatabaseFile(dbPath, 0, true, mpCoreAllocator);
        
        // See if <app>.pdb exists.
        wcscpy(pExtension, L".pdb");
        if(IO::File::Exists(dbPath))
            AddDatabaseFile(dbPath, 0, true, mpCoreAllocator);
    }
}
...
int AddressRepCache::GetAddressRep(int addressRepTypeFlags, uint64_t address, FixedString* pRepArray, int* pIntValueArray)
{
    ...
    memset(pStrResults, 0, sizeof(pStrResults));
                                        ...
                                        const char* pRep, size_t nRepLength, int intValue)
{
    ...
    // in which case we do a strlen to tell its length.
    if((ptrdiff_t)nRepLength < 0)
        nRepLength = strlen(pRep);
