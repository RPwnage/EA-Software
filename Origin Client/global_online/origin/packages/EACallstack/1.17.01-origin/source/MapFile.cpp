...
#include <EACallstack/internal/CoreAllocatorStack.h>
...
int MapFileMSVC::GetAddressRep(int addressRepTypeFlags, uint64_t nAddress, FixedString* pRepArray, int* pIntValueArray)
{
    ...
    if(mBaseAddress == 0) // If the user hasn't set the base address, then we have no choice but to assume it's equal to the map file default base address.
        mBaseAddress = mMapFileBaseAddress;
    ...
    else
    {
        ...
        nMaxAddress += (mBaseAddress - mMapFileBaseAddress);
    }

    if((nAddress < mBaseAddress) || (nAddress > (mBaseAddress + nMaxAddress)))
        return 0;
    ...
    // be relocated and the nAddress value will be in the relocated place. mBaseAddress refers to the
    ...
    const uint64_t nMapFileAddress = (nAddress + (mMapFileBaseAddress - mBaseAddress));
...
int MapFileMSVC::GetAddressRep(int addressRepTypeFlags, const void* pAddress, FixedString* pRepArray, int* pIntValueArray)
{
...
bool MapFileMSVC::FindRVAToFilePosArray(uint64_t nAddress, Symbol& s)
{
    ...
    while((lineSize = IO::ReadLine(&mFileBufferLowMemoryUsage, line, kLineSize)) < IO::kSizeTypeDone) // While there are more lines...
    {
        ...
        else
        {
            unsigned seg;
            unsigned offset;
            char     symbolMangled[2048]; // If you change this 2048, change the sscanf limit below.
            uint64_t rva;
            ...
            if(1 == EA::StdC::Sscanf(line, "%*x:%*x %*s %I64x", &rva))
            {
                if(rva > nAddress)  // If we have passed the line that refers to the symbol, then the previous line is the symbol line.
                {
                    ...
                    if(4 == EA::StdC::Sscanf(line, "%x:%x %2047s %I64x", &seg, &offset, symbolMangled, &rva))
                    {
...
bool MapFileMSVC::Load()
{
    ...
    if(!mbLoadAttempted)
    {
        ...
        if(mFile.Open()) 
        {
                ...
                filePosition = mFileBuffer.GetPosition(EA::IO::kPositionTypeBegin)) 
            {
                ...
                FixedString linebuf(line, line + lineSize);         // To consider: Make this more efficient by using eastl::fixed_substring here instead of EA::Callstack::FixedString.
                ...
                if(linebuf.compare(0, 12, "Line numbers") == 0)
                {
                    ...
                    if(!mbLowMemoryUsage) // We don't read line numbers in low memory usage mode.
                    {
                        ...
                        {
                            FixedString filename(&linebuf[12 + (unsigned)pos1], (unsigned)(pos2 - pos1));
                            eastl_size_t i(filename.find_last_of("/\\:"));

                            if(i != FixedString::npos)
                ...
                if(!linebuf.empty())
                {
                    ...
                    switch(mPhase)
                    {
                        case kPhaseCapturingGroups:
                        {
                            ...
                            char  className[32]; // This will be something like "CODE" or "DATA". // If you change this 32, change the %31s sscanf limit below.
                            Group g;

                            if(4 == EA::StdC::Sscanf(linebuf.c_str(), "%x:%x %xH %*s %31s", (unsigned*)&g.mSegment, (unsigned*)&g.mOffset, (unsigned*)&g.mLength, className))
                            {
                        ...
                        case kPhaseCapturingSymbols:
                        {
                            ...
                            if(4 == EA::StdC::Sscanf(linebuf.c_str(), "%x:%x %2047s %I64x", &seg, &offset, symbolMangled, &rva))
                            {
    ...
    if(bSuccess)
    {
        ...
        // we wet the map file base address based on the current module base address. In this case there's no reason for
        // the user to call SetBaseAddress, and even if the user did then the only practical base address the user could
        // specify would be the same one we auto-detect below.
        if((mBaseAddress == 0) && !mbOfflineLookup) 
        {
            char8_t pPath8[IO::kMaxPathLength];
...
MapFileGCC3::MapFileGCC3(EA::Allocator::ICoreAllocator* pCoreAllocator)
  : mpCoreAllocator(pCoreAllocator ? pCoreAllocator : EA::Callstack::GetAllocator()),
    ...
    mStackAllocator(NULL, 65536, EA::Callstack::StackAllocatorCoreAllocate, EA::Callstack::StackAllocatorCoreFree, mpCoreAllocator),
    mBaseAddress(0),
    mFileBufferSize(4096)
{
}
...
void MapFileGCC3::SetAllocator(EA::Allocator::ICoreAllocator* pCoreAllocator)
{
    ...
    mStackAllocator.Init(NULL,  0, EA::Callstack::StackAllocatorCoreAllocate, EA::Callstack::StackAllocatorCoreFree, pCoreAllocator);
}
...
bool MapFileGCC3::Load()
{
            ...
            while(IO::ReadLine(&streamBuffer, line, kLineSize) < IO::kSizeTypeDone) // While there are more lines...
            {
                ...
                else if(bReadLines)
                {
                    ...
                    if(address && symbolLength) // If we have valid data...
                    {
                        ...
                        char* const pSymbol = (char*)mStackAllocator.Malloc(symbolLength + 1);
                        EA::StdC::Strcpy(pSymbol, symbol);

                        const EntryPair entryPair = { address, pSymbol };

                        EntryPairArray::iterator it = 
                            eastl::lower_bound(mEntryPairArray.begin(), mEntryPairArray.end(), entryPair, EntryPairCompare());

                        mEntryPairArray.insert(it, entryPair); // Much of the time, this will be an append, because the entries are largely sorted already.
                    }
                }
            }
        }
    ...
    {
        if((mBaseAddress == 0) && !mbOfflineLookup) // If the base address hasn't been set...
        {
            char8_t pPath8[IO::kMaxPathLength];
            EA::StdC::Strlcpy(pPath8, msMapFilePath.c_str(), IO::kMaxPathLength);
                    ...
                    it != itEnd; ++it)
            {
                ...
                if (EA::StdC::Strstr(it->mpSymbol, gpBookmarkFunctionSymbol) != 0)
                {
...
bool MapFileGCC3::Shutdown()
{
    ...
    mStackAllocator.FreeObjects(NULL);
    mBaseAddress = 0;
...
int MapFileGCC3::GetAddressRep(int addressRepTypeFlags, uint64_t nAddress, FixedString* pRepArray, int* pIntValueArray)
{
    ...
    if(addressRepTypeFlags & kARTFlagFunctionOffset)
    {
        uint64_t  nAddressRelative = (nAddress - mBaseAddress);
        EntryPair entryPair = { nAddressRelative, NULL };
        ...
        if(it != mEntryPairArray.begin())
        {
            ...
            if(abs(addressOffset) < 65536) // Do a sanity check.
            {
                ...
                UnmangleSymbol(entryPair.mpSymbol, buffer, sizeof(buffer), kCompilerTypeGCC);
...
int MapFileGCC3::GetAddressRep(int addressRepTypeFlags, const void* pAddress, FixedString* pRepArray, int* pIntValueArray)
{
    return GetAddressRep(addressRepTypeFlags, (uint64_t)(uintptr_t)pAddress, pRepArray, pIntValueArray);
}
        ...
        if(fs.Open()) 
        {
            ...
            while(IO::ReadLine(&streamBuffer, line, kLineSize) < IO::kSizeTypeDone) // While there are more lines...
            {
                ...
                else if(bReadLines)
                {
                    ...
                    // The line with "[  0]" seems to signify the end of the addresses
                    if( EA::StdC::Strstr(line, "[  0]" ) != NULL)
                    {
                        bReadLines = false;
                        break;
                    }                    
...
int MapFileApple::GetAddressRep(int addressRepTypeFlags, const void* pAddress, FixedString* pRepArray, int* pIntValueArray)
{
    return GetAddressRep(addressRepTypeFlags, (uint64_t)(uintptr_t)pAddress, pRepArray, pIntValueArray);
}
...
int MapFileSN::GetAddressRep(int addressRepTypeFlags, uint64_t nAddress, FixedString* pRepArray, int* pIntValueArray)
{
    ...
    if(addressRepTypeFlags & kARTFlagFunctionOffset)
    {
        if(nAddress >= mBaseAddress) // Possibly we can also check to see that the address is less than some ceiling address. But the code below will take care of that.
        {
            uint64_t  nAddressRelative = (nAddress - mBaseAddress);
            EntryPair entryPair = { nAddressRelative, NULL };
            CAString8 symbol;
            ...
            if(mbLowMemoryUsage)
            {
                ...
                if((nAddressRelative >= pAddressPosEnd->mAddress) && !mbSymbolEndFound) // If this address is beyond where we've yet read in the file and if there's anything else left to read in the file...
                {
                    FillAddressToFilePosArray(nAddressRelative);
                    pAddressPosEnd = &mAddressToFilePosArray.back();
                }

                if(nAddressRelative <= (pAddressPosEnd->mAddress + 65536)) // If this address is somewhere within the map file...
                {
                    if(FindAddressToFilePosArray(nAddressRelative, symbol))
                        entryPair.mpSymbol = symbol.c_str(); // We'll demangle below.
            ...
            if(entryPair.mpSymbol) // If the above succeeded...
            {
                const int addressOffset = (int)(unsigned)(nAddressRelative - entryPair.mAddress);
                ...
                {
                    char buffer[512]; // The symbol may in fact already be demangled, so UnmangleSymbol may just do a strcpy.
                    UnmangleSymbol(entryPair.mpSymbol, buffer, sizeof(buffer), kCompilerTypeSN);
...
int MapFileSN::GetAddressRep(int addressRepTypeFlags, const void* pAddress, FixedString* pRepArray, int* pIntValueArray)
{
    return GetAddressRep(addressRepTypeFlags, (uint64_t)(uintptr_t)pAddress, pRepArray, pIntValueArray);
}
...
///////////////////////////////////////////////////////////////////////////////
// A CodeWarrior .map file looks like the text below. 
// We want to read the .text entries.
// There are sections other than shown below. You can tell the start of any
// new section because its first header line has no indentation.
//
///////////////////////////////////////////////////////////////////////////////
// .init section layout
...
//   ---------------------------------
//   00000000 000134 80004000 000001c0  1 .init 	Runtime.PPCEABI.H.a __mem.o
//   00000000 000050 80004000 000001c0  4 memcpy 	Runtime.PPCEABI.H.a __mem.o
//   00000050 0000b4 80004050 00000210  4 __fill_mem 	Runtime.PPCEABI.H.a __mem.o
//   00000104 000030 80004104 000002c4  4 memset 	Runtime.PPCEABI.H.a __mem.o
//   00000134 000054 80004134 000002f4  1 .init 	TRK_Hollywood_Revolution.a C:\products\RVL\runtime_libs\debugger\embedded\MetroTRK\Portable\mem_TRK.o
// 
// .text section layout
//   Starting        Virtual  File
//   address  Size   address  offset
//   ---------------------------------
//   00000000 000400 80006b60 00002d20  1 .text 	TestUTFXml.cpp.obj 
//   00000000 0001f8 80006b60 00002d20  4 SetupTestFileList__Fv 	TestUTFXml.cpp.obj 
//   000001f8 000208 80006d58 00002f18  4 main 	TestUTFXml.cpp.obj 
//   00000400 000430 80006f60 00003120  1 .text 	TestUTFXml.cpp.obj 
//   00000400 000060 80006f60 00003120  4 __nw__FUl 	TestUTFXml.cpp.obj 
//   UNUSED   000050 ........ ........    __nw__FUlRQ23std9nothrow_t TestUTFXml.cpp.obj 
//   UNUSED   000050 ........ ........    __nw__FUlRCQ23std9nothrow_t TestUTFXml.cpp.obj 
// 
// .data section layout
//   Starting        Virtual  File
//   address  Size   address  offset
//   ---------------------------------
//   00000000 000394 800a5300 000a14c0  1 .data 	TestUTFXml.cpp.obj 
//   00000000 000000 800a5300 000a14c0    ...data.0 (entry of .data) 	TestUTFXml.cpp.obj 
//   00000000 00001a 800a5300 000a14c0  4 @7278 	TestUTFXml.cpp.obj 
//   00000328 000029 800a5628 000a17e8  4 @STRING@__vc__Q25eastl60basic_string<w,Q25eastl34fixed_vector_allocator<2,96,2,0,1>>FUl 	TestUTFXml.cpp.obj 
//   00000388 00000c 800a5688 000a1848  4 @STRING@allocate__Q25eastl9allocatorFUli 	TestUTFXml.cpp.obj 
//   00000394 000004 800a5698 000a1858  8 *fill*
//   000003f0 00002c 800a56f0 000a18b0  4 __vt__14ContentHandler 	TestXmlCallbackReader.cpp.obj 
// 
// .sdata section layout
//   Starting        Virtual  File
//   address  Size   address  offset
//   ---------------------------------
//   00000000 000042 800ed420 000c6100  1 .sdata 	TestUTFXml.cpp.obj 
//   UNUSED   000002 ........ ........    @4456 TestUTFXml.cpp.obj 
//   UNUSED   000002 ........ ........    @4457 TestUTFXml.cpp.obj 
//   00000000 000004 800ed420 000c6100  4 @4458 	TestUTFXml.cpp.obj 
//   UNUSED   000004 ........ ........    @4459 TestUTFXml.cpp.obj 
//   00000004 000006 800ed424 000c6104  4 @7622 	TestUTFXml.cpp.obj 
//   0000000c 000006 800ed42c 000c610c  4 @7734 	TestUTFXml.cpp.obj 
//   00000014 000006 800ed434 000c6114  4 @7735 	TestUTFXml.cpp.obj 
// 
// .sdata2 section layout
//   Starting        Virtual  File
//   address  Size   address  offset
//   ---------------------------------
//   00000000 000004 800ee2a0 000c69c0  1 .sdata2 	TestUTFXml.cpp.obj 
//   UNUSED   000002 ........ ........    kLineEndUnix8__Q22EA2IO TestUTFXml.cpp.obj 
//   UNUSED   000004 ........ ........    kLineEndUnix16__Q22EA2IO TestUTFXml.cpp.obj 
//   UNUSED   000003 ........ ........    kLineEndWindows8__Q22EA2IO TestUTFXml.cpp.obj 
//   UNUSED   000006 ........ ........    kLineEndWindows16__Q22EA2IO TestUTFXml.cpp.obj 
///////////////////////////////////////////////////////////////////////////////
...
// MapFileCodeWarrior
//
MapFileCodeWarrior::MapFileCodeWarrior(EA::Allocator::ICoreAllocator* pCoreAllocator)
  : mpCoreAllocator(pCoreAllocator ? pCoreAllocator : EA::Callstack::GetAllocator()),
    mbLoadAttempted(false),
    mbLowMemoryUsage(true), // Low memory mode by default for this class, as in this mode it can get file name information.
    mbOfflineLookup(false),
    mbSymbolEndFound(false),
    mbMainGameBaseAddress(true),
    msMapFilePath(),
    mEntryPairArray(EA::Callstack::EASTLCoreAllocator(EACALLSTACK_ALLOC_PREFIX "EACallstack/MapFileCodeWarrior", mpCoreAllocator)),
    mStackAllocator(NULL, 65536, EA::Callstack::StackAllocatorCoreAllocate, EA::Callstack::StackAllocatorCoreFree, mpCoreAllocator),
    mBaseAddress(0),
    mMainGameMapFileAddress(0),
    mFileBufferSize(4096),
    mFile(),
    mFileBuffer(IO::StreamBuffer::kBufferSizeUnspecified, IO::StreamBuffer::kBufferSizeUnspecified, NULL, mpCoreAllocator),
    mFileLowMemoryUsage(),
    mFileBufferLowMemoryUsage(IO::StreamBuffer::kBufferSizeUnspecified, IO::StreamBuffer::kBufferSizeUnspecified, NULL, mpCoreAllocator),
    mAddressToFilePosArray(EA::Callstack::EASTLCoreAllocator(EACALLSTACK_ALLOC_PREFIX "EACallstack/MapFileCodeWarrior", mpCoreAllocator)),
    mFileSize(0),
    mLastLookupLinePosition(0),
    mSymbolsPerLowMemSample(0) // Will be initialized in Load.
{
    mFile.AddRef(); // This prevents it from being released away by anybody.
    mFileLowMemoryUsage.AddRef();
...
// ~MapFileCodeWarrior
//
MapFileCodeWarrior::~MapFileCodeWarrior()
{
    Shutdown();
}
...
// SetAllocator
//
void MapFileCodeWarrior::SetAllocator(EA::Allocator::ICoreAllocator* pCoreAllocator)
{
    mpCoreAllocator = pCoreAllocator;
...
// Init
//
bool MapFileCodeWarrior::Init(const wchar_t* pMapFilePath, bool bDelayLoad)
{
    bool bResult = true;
...
// Since the symbols are in order by address this can be used to tell us 
// roughly where a symbol can later be found in the file in the FindAddressToFilePosArray function.
bool MapFileCodeWarrior::FillAddressToFilePosArray(uint64_t nAddressRelative)
{
    bool bAddressFound   = false;
    bool bReadEntireFile = (mBaseAddress != 0); // If the base address is non-zero, then we have a DLL (Wii RSO). In that case we need to read the entire file because we need to know if there's a MainGame function somewhere in it.

    // Assert that mAddressToFilePosArray doesn't contain nAddressRelative (otherwise we wouldn't be calling this function).
    EA_ASSERT(!mAddressToFilePosArray.empty() && (nAddressRelative >= mAddressToFilePosArray.back().mAddress));

    // We assume that mFileBuffer is positioned at the line after the last location we read a mAddressToFilePosArray entry.
    while(!mbSymbolEndFound && (bReadEntireFile || !bAddressFound))
    {
        IO::off_type filePosPrev    = mFileBuffer.GetPosition(EA::IO::kPositionTypeBegin);
        uint64_t     symbolAddress  = UINT64_C(0xffffffffffffffff);
        ...
        // We always append at least one new mAddressToFilePosArray entry.
        for(uint32_t i = 0; !mbSymbolEndFound && (i < mSymbolsPerLowMemSample); i++)
        {
            //  00000000 00015c 800068e0 00002aa0  1 .text 	EAStdCTest.cpp.obj 
            //  00000000 000030 800068e0 00002aa0 16 __nw__FUlPCciUiPCci 	EAStdCTest.cpp.obj 
            //  00000030 000030 80006910 00002ad0 16 __nwa__FUlPCciUiPCci 	EAStdCTest.cpp.obj 
            //  UNUSED   000030 ........ ........    __nwa__FUlUlUlPCciUiPCci EAStdCTest.cpp.obj 
            //  00000060 000030 80006940 00002b00 16 __nw__FUlUlUlPCciUiPCci 	EAStdCTest.cpp.obj 
            //  00000090 0000cc 80006970 00002b30 16 main 	EAStdCTest.cpp.obj 
            //  0000015c 000004 80006a40 00002c00 16 *fill*
            //  00000160 00016c 80006a40 00002c00  1 .text 	EATest.cpp.obj 
            //  00000160 000044 80006a40 00002c00 16 Report__Q22EA8UnitTestFPCc 	EATest.cpp.obj 
            //  UNUSED   0000e8 ........ ........    stristr__Q22EA8UnitTestFPCcPCc EATest.cpp.obj 
            //  UNUSED   000074 ........ ........    IsWhiteSpace__Q22EA8UnitTestFc EATest.cpp.obj 
            //  UNUSED   000014 ........ ........    __ct__Q32EA8UnitTest11CommandLineFiPPc EATest.cpp.obj 
            //  . . .
            //
            //
            //.sbss section layout
            ...
            if(lineSize < IO::kSizeTypeDone)  // If the line was read without error and isn't at the end of the file...
                fieldCount = EA::StdC::Sscanf(line, "  %*s %x %I64x", &symbolSize, &symbolAddress);

            // We currently define the end of the File section to be the first line 
            // that doesn't begin with "  cccccccc cccccc cccccccc cccccccc".
            if((symbolAddress == 0) || (fieldCount != 2)) // If we had a problem reading the line...
            {
                if(line[0] == ' ') // If we have a line like "  UNUSED   000030 ........ ........"...
                {
                    // We skip this line; pretend it wasn't read, for all practical purposes.
                    --i;
                }
                else // If the line is bad or is beyond the File section...
                {
                    mbSymbolEndFound = true;
                    symbolAddress    = lastSymbolEnd; // Set it to this value so mAddressToFilePosArray below has a valid final entry.
            ...
            {
                lastSymbolEnd = symbolAddress + symbolSize; // We use this to tell what the address of the end of the symbols is if we reach the end of the file.

                // If this is a WiiBootLoader RSO, then the users specify the base address via the address of the MainGame function instead of 
                // via the address of the first function in the .text section. If we find such a MainGame function then we use that as the 'base address.'
                if((lineSize > 50) && (mMainGameMapFileAddress == 0))
                {
                    if(EA::StdC::Strstr(line, " MainGame") && EA::StdC::Strstr(line, "main.cpp"))
                    {
                        mMainGameMapFileAddress = symbolAddress;

                        if(mbMainGameBaseAddress && mBaseAddress)
                        {
                            nAddressRelative += mBaseAddress;               // Convert it back to the absolute form.
                            mBaseAddress     -= mMainGameMapFileAddress;    // Set base address to what the user really meant it to be.
                            nAddressRelative -= mBaseAddress;               // Now get our real relative address.
                        }
                    }
                }
            }
        ...
        AddressToFilePos addressPos = { (AddressSize)symbolAddress, (AddressSize)filePosPrev };
        mAddressToFilePosArray.push_back(addressPos);

        bAddressFound = (nAddressRelative < symbolAddress);
    }

    // Due to the MainGame handling above, if bReadEntireFile == true then we 
    // need to the delay the actual check until after reading the entire file.
    if(bReadEntireFile)
    {
        AddressToFilePos addressPos = { (AddressSize)nAddressRelative, 0 };
        AddressToFilePosArray::iterator it = eastl::upper_bound(mAddressToFilePosArray.begin(), mAddressToFilePosArray.end(), addressPos, AddressToFilePosCompare());

        if(it != mAddressToFilePosArray.begin())
            --it;

        if(addressPos.mAddress >= mAddressToFilePosArray[0].mAddress)
            bAddressFound = true;
    }
...
// starts reading symbol lines from it till it gets to the one that corresponds
// to nAddressRelative.
bool MapFileCodeWarrior::FindAddressToFilePosArray(uint64_t nAddressRelative, CAString8& s, AddressSize& result)
{
    bool bResult = false;
    AddressToFilePos addressPos = { (AddressSize)nAddressRelative, 0 };

    // Find the first symbol line in the file which might correspond to nAddressRelative.
    // We will start with this line and read successive lines until we find the 
    // one that corresponds to our symbol. This function should not be being called
    // unless we have found that nAddressRelative is represented with lines we have read
    // into mAddressToFilePosArray.
    EA_ASSERT(!mAddressToFilePosArray.empty());
    AddressToFilePosArray::iterator it = eastl::upper_bound(mAddressToFilePosArray.begin(), mAddressToFilePosArray.end(), addressPos, AddressToFilePosCompare());
    ...
    if(nAddressRelative < mAddressToFilePosArray[0].mAddress)
        return false;

    addressPos = *it; // Start at this line.
    mFileBufferLowMemoryUsage.SetPosition((IO::off_type)addressPos.mFilePos, IO::kPositionTypeBegin);

    // Read lines until we get to the first line that is beyond nAddressRelative.
    const size_t  kLineSize = 1200;
    char          line[kLineSize];
    IO::size_type lineSize;
    uint64_t      filePosPrev    = addressPos.mFilePos;
    uint64_t      filePosCurrent = filePosPrev;
    ...
    while((lineSize = IO::ReadLine(&mFileBufferLowMemoryUsage, line, kLineSize)) < IO::kSizeTypeDone) // While there are more lines...
    {
        ...
        if((lineSize >= 40) && EA::StdC::Isxdigit(line[2]) && !EA::StdC::Strstr(line, " .text")) // If this is a valid line and not an "UNUSED" line and not a .text line (which is a summary of the following lines) ...
        {
            int      fieldCount;
            uint64_t symbolAddress = 0;
            uint32_t symbolSize = 0;
            char     symbolMangled[1024]; symbolMangled[0] = 0;// If you change this buffer size, change the sscanf limit below.

            fieldCount = EA::StdC::Sscanf(line, "%*s %x %I64x %*s %*s %1023s", &symbolSize, &symbolAddress, symbolMangled);

            if((fieldCount == 3) && (nAddressRelative < (symbolAddress + symbolSize))) // If this appears to be the symbol of interest...
            {
                // We leave the symbol mangled here; we'll unmangle it in GetAddressRep.
                s.get_allocator().set_allocator(mpCoreAllocator);
                s = symbolMangled;
                result = (AddressSize)symbolAddress;
                mLastLookupLinePosition = filePosCurrent;
                bResult = true;
                break;
            }
...
bool MapFileCodeWarrior::Load()
{
    ...
    if(!mbLoadAttempted)
    {
        ...
        if(mFile.Open()) 
        {
            ...
            if(mbLowMemoryUsage)
            {
                ...
                if(mSymbolsPerLowMemSample < 1)
                    mSymbolsPerLowMemSample = 1;
                mAddressToFilePosArray.reserve(256);
            }
            else
            {
                // We'll almost certainly need a few hundred entries, so reserve some up-front.
            ...
            // Read the map file data here.
            // We read each line from the file until we get to the ".text section layout" line. At that point each 
            // line in this section is a function entry. See the example .map file.
            // We stop when we see the end of the .text section due to the beginning of a new section.
            // We add to our mEntryPairArray from each symbol line. In mbLowMemoryUsage mode, we don't read all 
            // the lines, but rather we take note of where in the file the .text section begins and then exit;
            // we'll read the lines later on demand.
            ...
            for(bool bContinueReading = true; 
                ...
                filePosition = mFileBuffer.GetPosition(EA::IO::kPositionTypeBegin)) 
            {
                if((lineSize >= 1) && (EA::StdC::Strstr(line, "SYMBOL NOT FOUND") == NULL)) // Wii RSO files have lines of "SYMBOL NOT FOUND" in them.
                {
                    if(EA::StdC::Strstr(line, ".text section layout") == line)
                        bReadLines = true;
                    else if(line[0] != ' ') // Else if we have a section but it isn't .text ...
                        bReadLines = false;
                    else if(bReadLines && (lineSize >= 40) && Isxdigit(line[2]) && Isxdigit(line[11]) && Isxdigit(line[18]) && Isxdigit(line[27]))
                    {
                        uint64_t address, size, virtualAddress;

                        // We have a line that is like this:
                        //     "  00000500 000098 80008580 00004740 16 Close__Q32EA2IO9StreamCppFv 	TestEAStreamCpp.cpp.obj"

                        const int fieldCount = EA::StdC::Sscanf(&line[2], "%I64x %I64x %I64x", &address, &size, &virtualAddress);

                        if(fieldCount == 3) // This will be true except for the first few lines after the start of the .text section, because these lines have header strings in them like "address  Size   address  offset"
                        {
                            if(mbLowMemoryUsage)
                            {
                                // In low memory mode, we stop loading right here, after the first symbol.
                                ...
                                AddressToFilePos entry = { (AddressSize)virtualAddress, (AddressSize)filePosition };
                                mAddressToFilePosArray.push_back(entry);
                            ...
                            {
                                // We look for a line like so:
                                //     "  00449c90 000050 00449c90 00449cd0  4 MainGame 	main.cpp.obj "
                                // which indicates that we are using WiiBootLoader and the .map file is a conglomeration of the 
                                // executable (loader) and the main game RSO. The problem with this kind of map file is that it
                                // puts the RSO addresses (beginning with MainGame) right after the executable addresses yet 
                                // these RSO addresses have no useful meaning, as they are set as if they were in the executable
                                // .text segment when in fact they are not and will never be. So the .map file is lying to us 
                                // and what we really want is the original .map file associated with the ELF that generated the RSO.
                                // With this map file, we have a situation whereby we really have two map (or more) map files, each
                                // with a different base address. For the general case, the user will simply need to provide the 
                                // original map files

                                if((lineSize > 50) && (mMainGameMapFileAddress == 0))
                                {
                                    if(EA::StdC::Strstr(line, "MainGame") && EA::StdC::Strstr(line, "main.cpp"))
                                        mMainGameMapFileAddress = virtualAddress;
                                }

                                char   symbol[256];
                                size_t symbolLength;
                                char*  pSpace = EA::StdC::Strchr(line + 39, ' '); // Find the space between the function name and the file name.

                                if(pSpace)
                                    *pSpace = 0;

                                symbolLength = EA::StdC::Strlcpy(symbol, line + 39, sizeof(symbol));
                                if(symbolLength >= sizeof(symbol))
                                    symbolLength = sizeof(symbol) - 1;

                                if(symbolLength && (EA::StdC::Strstr(symbol, ".text") == 0)) // If we have valid data and this isn't one of those lines that simply announces the beginning of a new source file...
                                {
                                    // We leave symbols mangled, as that typically uses less space. We'll demangle in GetAddressRep.
                                    char* const pSymbol = (char*)mStackAllocator.Malloc(symbolLength + 1);
                                    EA::StdC::Strcpy(pSymbol, symbol);

                                    const EntryPair entryPair = { (AddressSize)virtualAddress, pSymbol };
    ...
    if(bSuccess)
    {
        if(mBaseAddress == 0) // If the base address hasn't been set...
        {
            if(mbOfflineLookup)
            {
                // There's nothing to do here at this point. If this is a DLL or Wii RSO then the user is going to
                // need to call SetBaseAddress manually, at which point we can possibly subract mMainGameMapFileAddress from it.
            }
            else
            {
                char8_t pPath8[IO::kMaxPathLength];
        ...
        else
        {
            if(mbOfflineLookup)
            {
                // If this is a Wii RSO and there was an EA WiiBootLoader MainGame function found, 
                // then we assume that the user-specified base address wasn't the runtime address 
                // of the .text section, but was the runtime address of the MainGame function.
                if(mbMainGameBaseAddress)
                    mBaseAddress -= mMainGameMapFileAddress;
            }
        }
    }
...
// Shutdown
//
bool MapFileCodeWarrior::Shutdown()
{
    // Clear our member data.
    mbLoadAttempted = false;
    mEntryPairArray.set_capacity(0);
...
// AsInterface
//
void* MapFileCodeWarrior::AsInterface(int id)
{
    if(id == MapFileCodeWarrior::kTypeId)
        return static_cast<MapFileCodeWarrior*>(this); // The cast isn't necessary, but makes the intent more clear.
    return NULL;
}
...
// SetBaseAddress
//
void MapFileCodeWarrior::SetBaseAddress(uint64_t baseAddress)
{
    mBaseAddress = baseAddress;
...
// GetDatabasePath
//
const wchar_t* MapFileCodeWarrior::GetDatabasePath() const
{
    return msMapFilePath.c_str();
}
...
// SetOption
//
void MapFileCodeWarrior::SetOption(int option, int value)
{
    if(option == kOptionLowMemoryUsage)
    {
        EA_ASSERT(!mbLoadAttempted); // Must set this option before loading anything or doing any lookups.
    ...
    //    mbUseMemoryFile = (value != 0);
        ...
        mbOfflineLookup = (value != 0);
    else if(option == kOptionMainGameBaseAddress)
        mbMainGameBaseAddress = (value != 0);
}
...
// GetAddressRep
//
int MapFileCodeWarrior::GetAddressRep(int addressRepTypeFlags, uint64_t nAddress, FixedString* pRepArray, int* pIntValueArray)
{
    int returnFlags = 0;
    ...
    // We need to make the following check here, as the code below doesn't account for it.
    // We don't need to make an upper-bound check, as the code below does account for it.
    if((nAddress < mBaseAddress) && !mbLowMemoryUsage) // Don't do this early exit in the caes of mbLowMemoryUsage, due to the MainGame problem.
        return 0;

    if(addressRepTypeFlags & kARTFlagFileLine)
       addressRepTypeFlags |= kARTFlagFunctionOffset;

    // CodeWarrior .map files can give you only function/offset and file name (not path and line) information.
    // You need to read the DWARF information from a .elf file to get file path/line information.
    if(addressRepTypeFlags & kARTFlagFunctionOffset)
    {
        // Usually mBaseAddress is zero, which means that the virtual addresses in the map file are
        // the same as they are when the app is running on the hardware. For RSOs (Wii DLLs) the 
        // base address should not be zero, but should be a value like 0x83008000. 

        uint64_t  nAddressRelative = (nAddress - mBaseAddress);
        EntryPair entryPair = { (AddressSize)nAddressRelative, NULL };
        CAString8 symbol;
        ...
        if(mbLowMemoryUsage)
        {
            ...
            if((nAddressRelative >= pAddressPosEnd->mAddress) && !mbSymbolEndFound) // If this address is beyond where we've yet read in the file and if there's anything else left to read in the file...
            {
                ...
                // It's possible mBaseAddress might have changed (due to the MainGame problem), so we re-evaluate.
                nAddressRelative = (nAddress - mBaseAddress);
            }

            if(nAddressRelative < pAddressPosEnd->mAddress) // If this address is somewhere within the map file...
            {
                if(FindAddressToFilePosArray(nAddressRelative, symbol, entryPair.mAddress))
                    entryPair.mpSymbol = symbol.c_str();    // We'll demangle below.
            } // Else the address is not present in the map file.
        }
        else
        ...
        if(entryPair.mpSymbol) // If the above succeeded...
        {
            ...
            if(abs(addressOffset) < 100000) // Do a sanity check.
            {
                char buffer[512];
                UnmangleSymbol(entryPair.mpSymbol, buffer, sizeof(buffer), kCompilerTypeCodeWarrior);
    ...
    if(addressRepTypeFlags & kARTFlagFileLine)
    {
        if(returnFlags & kARTFlagFunctionOffset) // If something was found above...
        {
            if(mbLowMemoryUsage)
            {
                // We have a line that is like one of the following:
                //     "  00000500 000098 80008580 00004740 16 Close__Q32EA2IO9StreamCppFv 	TestEAStreamCpp.cpp.obj"
                //     "  00749370 000044 00749370 007493b0 16 IsIndexed__Q310RenderCore7Texture7IndexedFPCQ310RenderCore6Memory5Blocki 	libRenderCorez.a rcTextureIndexed.cpp.obj"
                //     "  0003691c 000028 0003691c 0003695c  4 OnFlush__Q25Juice12FileIOPluginFv 	JuiceConsole_FileIO.a"
                // As a result, the file name will be either the 7th or 8th string, and we can't tell which until we read it and see.
                // Also, sometimes the file name is absent and we just have a library name. Or the library name is absent and we just have a file name.

                char          line[1024]; line[0] = 0;
                IO::size_type lineSize;
				EA_UNUSED(lineSize);
                int           fieldCount;
                char          fileName1[256]; fileName1[0] = 0;   // If you change this size, change the format size below.
                char          fileName2[256]; fileName2[0] = 0;   // If you change this size, change the format size below.

                mFileBufferLowMemoryUsage.SetPosition((EA::IO::off_type)mLastLookupLinePosition, EA::IO::kPositionTypeBegin);   // This should always succeed.
                lineSize   = IO::ReadLine(&mFileBufferLowMemoryUsage, line, sizeof(line));                                      // This should always succeed.
                fieldCount = EA::StdC::Sscanf(&line[2], "%*s %*s %*s %*s %*s %*s %255s %255s", fileName1, fileName2);           // This should always return at least 1.

                if((fieldCount >= 1) && fileName1[0])
                {
                    pRepArray[kARTFileLine]      = (fieldCount == 1) ? fileName1 : fileName2;
                    pIntValueArray[kARTFileLine] = 0;
                    returnFlags                 |= kARTFlagFileLine;

                    // Do some possible cleanup of the file name.
                    if(pRepArray[kARTFileLine].rfind(".obj") == (pRepArray[kARTFileLine].size() - 4))
                        pRepArray[kARTFileLine].resize(pRepArray[kARTFileLine].size() - 4);

                    if(pRepArray[kARTFileLine].rfind(".o") == (pRepArray[kARTFileLine].size() - 2))
                        pRepArray[kARTFileLine].resize(pRepArray[kARTFileLine].size() - 2);

                    if(pRepArray[kARTFileLine].rfind(".a") == (pRepArray[kARTFileLine].size() - 2))
                    {
                        pRepArray[kARTFileLine].resize(pRepArray[kARTFileLine].size() - 2);
                        pRepArray[kARTFileLine] += " library";
                    }
                }
            }
            // We don't have a way to do this in regular mode, because we don't save line locations in regular mode.
        }
    }

    //if(returnFlags == 0) // If the lookup failed...
    //{
        // It's not clear if this is useful to enable.
        //if(!mbMainGameBaseAddress && mMainGameMapFileAddress) // If the MainGame option was not enabled but there was in fact a "MainGame" found in the map file...
        //{
        //    // We temporarily enable mbMainGameBaseAddress and recursively call ourselves again. It will probably work this time.
        //    mbMainGameBaseAddress = true;
        //    returnFlags = GetAddressRep(addressRepTypeFlags, pAddress, pRepArray, pIntValueArray);
        //    mbMainGameBaseAddress = false;
        //}
    //}
...
int MapFileCodeWarrior::GetAddressRep(int addressRepTypeFlags, const void* pAddress, FixedString* pRepArray, int* pIntValueArray)
{
    return GetAddressRep(addressRepTypeFlags, (uint64_t)(uintptr_t)pAddress, pRepArray, pIntValueArray);
}

#endif // EACALLSTACK_CODEWARRIOR_MAPFILE_ENABLED
...
///////////////////////////////////////////////////////////////////////////////
// A Green Hills .map file looks like the text below, aside from the leading
// "// " on each line below as a C++ comment identifier. 
// We want to read the .text entries from the "Global Symbols" section. Things
// are a little complicated by the fact that the map files may have symbols 
// that are supported alphabetically (link with -Ma) instead of by address (link with -Mn).
// We can tell this because of the text "Global Symbols (sorted alphabetically)" 
// vs. "Global Symbols (sorted numerically)". In the case of alphabetically sorted
// symbols, .text entries and other section entries are mixed together, whereas
// for numerically sorted symbols all .text entries are grouped together.
//
///////////////////////////////////////////////////////////////////////////////
// Link Date:	Mon Apr 23 14:48:16 2012
// Host OS:	Windows 7 Service Pack 1
// Version:	ELXR 5.3 (c) Green Hills Software   Build: Jan 20 2012
// 
// 
// Load Map Mon Apr 23 14:48:16 2012
// Image Summary
// 
//   Section              Base      Size(hex)    Size(dec)  SecOffs
//   .syscall             02000000  00000008            8   0000180
//   .text                02000020  000d0620       853536   00001a0
//   .fexports            c0000000  00000000            0   00d07c0
//   .dexports            c0000000  00000000            0   00d07c0
//   .sdata2              10000000  00000000            0   00d07c0
//   .rodata              10000000  00022870       141424   00d07c0
//   .data                10022880  000048a4        18596   00f3040
//   .bss                 10027140  0001d380       119680   00f7900
//   .ipc_smem            100444c0  000883c0       558016   00f7900
//   .sdabase             100cc880  00000000            0   00f7900
//   .sdata               100cc880  00000000            0   00f7900
//   .sbss                100cc880  00000000            0   00f7900
//   .appended            020d0640  00000000            0   00f7900
//   .ghs.uvfd.rout       00000000  00001bb0         7088   00f7900
//   .ghs.uvfd.vtbl       00000000  000002b0          688   00f94b0
//   .rela.text           00000000  00022230       139824   00f9760
//   .rela.rodata         00000000  00001c80         7296   011b990
//   .rela.data           00000000  000019ec         6636   011d610
//   .rela.bss            00000000  00000000            0   011effc
//   .rela.ghsinfo        00000000  00000510         1296   011effc
//   .rela.ghs.uvfd.rout  00000000  00000408         1032   011f50c
//   .rela.ghs.uvfd.vtbl  00000000  00000024           36   011f914
//   .fimport_coreinit    020d0640  000013b8         5048   011f938
//   .dimport_coreinit    020d19f8  00000038           56   0120cf0
//   .rela.ipc_smem       00000000  00000000            0   0120d28
//   .fimport_coreuser.rpl 020d1a30  00000038           56   0120d28
//   .fimport_nsysccr.rpl 020d1a68  00000348          840   0120d60
//   .ghsinfo             00000000  00000494         1172   01210a8
// . . .
// Load Map Mon Apr 23 14:48:16 2012
// Module Summary
// 
//   Origin+Size    Section          Module
// 02000020+001e44  .text            main.cpp.obj
// 00000000+000ac8  .rela.text       main.cpp.obj
// 10000000+0005ad  .rodata          main.cpp.obj
// 00000000+000090  .rela.rodata     main.cpp.obj
// 10022880+00002c  .data            main.cpp.obj
// . . .
// Load Map Mon Apr 23 14:48:16 2012
// Global Symbols (sorted alphabetically)
// 
//  .text            020a4264+000040 AR_Assert__Q2_2EA9AllocatorFbPc
//  .text            020a42a4+0000d4 AR_PtrToString__Q2_2EA9AllocatorFPCvPciPPc
//  .text            020a4378+0000d4 AR_SizeToString__Q2_2EA9AllocatorFUiPciPPc
// . . .
//  .text            020ce068+000080 write
//  .text            020c75d4+000120 xdecout..trg.2Fppc.2Fobjs.2Flibs.2Flibansi.2Fccllout.
//  .text            020c75a4+000030 xxdecout..trg.2Fppc.2Fobjs.2Flibs.2Flibansi.2Fccllout.
// 
///////////////////////////////////////////////////////////////////////////////
...
// MapFileGreenHills
//
MapFileGreenHills::MapFileGreenHills(EA::Allocator::ICoreAllocator* pCoreAllocator)
  : mpCoreAllocator(pCoreAllocator ? pCoreAllocator : EA::Callstack::GetAllocator()),
    mbMapFileAlphabetic(false), // False until found to be true.
    mbLoadAttempted(false),
    mbLowMemoryUsage(false),
    mbLowMemoryUsageEnabled(false),
    mbOfflineLookup(false),
    mbSymbolEndFound(false),
    msMapFilePath(),
  //mLineBuffer,
    mEntryPairArray(EA::Callstack::EASTLCoreAllocator(EACALLSTACK_ALLOC_PREFIX "EACallstack/MapFileGreenHills", mpCoreAllocator)),
    mStackAllocator(NULL, 65536, EA::Callstack::StackAllocatorCoreAllocate, EA::Callstack::StackAllocatorCoreFree, mpCoreAllocator),
    mBaseAddress(0),
    mFileBufferSize(4096),
    mFile(),
    ...
    mFileLowMemoryUsage(),
    mFileBufferLowMemoryUsage(IO::StreamBuffer::kBufferSizeUnspecified, IO::StreamBuffer::kBufferSizeUnspecified, NULL, mpCoreAllocator),
    mAddressToFilePosArray(EA::Callstack::EASTLCoreAllocator(EACALLSTACK_ALLOC_PREFIX "EACallstack/MapFileGreenHills", mpCoreAllocator)),
    mFileSize(0),
    mLastLookupLinePosition(0),
    mSymbolsPerLowMemSample(0), // Will be initialized in Load.
    mLastSymbolEnd(0)
...
// ~MapFileGreenHills
//
MapFileGreenHills::~MapFileGreenHills()
{
    Shutdown();
}
...
// SetAllocator
//
void MapFileGreenHills::SetAllocator(EA::Allocator::ICoreAllocator* pCoreAllocator)
{
    mpCoreAllocator = pCoreAllocator;
...
// Init
//
bool MapFileGreenHills::Init(const wchar_t* pMapFilePath, bool bDelayLoad)
{
    bool bResult = true;
...
bool MapFileGreenHills::Load()
{
    ...
    if(!mbLoadAttempted)
    {
        ...
        if(mFile.Open()) 
        {
            ...
            if(mbLowMemoryUsageEnabled)
            {
                mSymbolsPerLowMemSample = (uint32_t)(mFileSize / 40000); // This number is qualitative. Used only for the case of mbLowMemoryUsageEnabled.
                if(mSymbolsPerLowMemSample < 1)
                    mSymbolsPerLowMemSample = 1;
                mAddressToFilePosArray.reserve(256);
            }
            ...
            // Read the map file data here.
            // We read each line from the file until we get to a line beginning with "Global Symbols (sorted numerically)" or
            // "Global Symbols (sorted alphabetically). 
            // From that point on we pay attention to all lines that begin with " .text" until the end
            // of the file (alphabetic symbol sorting) or until the first non-.text line (numeric sorting).
            // We add to our mEntryPairArray from each symbol line. In LowMemoryUsage mode, we don't read all 
            // the lines, but rather we take note of where in the file the .text section begins and then exit;
            // we'll read the lines later on demand.

            bool              bReadLines = false;       // This is a tiny state machine variable that is used to tell if we are in the global symbols section.
            bool              bTextEntryFound = false;  // True if we've found the first .text entry in the global symbols.
            IO::size_type     lineSize;
            IO::off_type      filePosition = 0;         // Denotes the file position for the beginning of the current line.

            for(bool bContinueReading = true; 
                bContinueReading && (lineSize = IO::ReadLine(&mFileBuffer, mLineBuffer, EAArrayCount(mLineBuffer))) < IO::kSizeTypeDone; // While we should continue reading and while there are more lines...
                filePosition = mFileBuffer.GetPosition(EA::IO::kPositionTypeBegin)) 
            {
                if(lineSize > 0) // If the line is not empty...
                {
                    if(!bReadLines && EA::StdC::Strstr(mLineBuffer, "Global Symbols") == mLineBuffer) // If the line begins with "Global Symbols" ...
                    {
                        bReadLines = true;
                        mbMapFileAlphabetic = (EA::StdC::Strstr(mLineBuffer, "alpha") != NULL); // If it's this: "Global Symbols (sorted alphabetically)"
                        if(mbMapFileAlphabetic)
                            mbLowMemoryUsageEnabled = false; // Regardless of whether the user requested it, we cannot do it. Our iterative low memory approach can't work if the symbols are alphabetically sorted instead of sorted by address.
                    }
                    else if(bTextEntryFound && !mbMapFileAlphabetic && (EA::StdC::Strstr(mLineBuffer, " .text ") != mLineBuffer)) // If it looks like we are done with all .text entries in the global symbols section...
                    {
                        bReadLines = false;
                        bContinueReading = false;
                    }
                    ...
                    {
                        if(EA::StdC::Strstr(mLineBuffer, " .text ") == mLineBuffer) // If the line is a .text line...
                        {
                            bTextEntryFound = true;

                            // We have a line that is like any of these:
                            //     " .text            02000f50+000020 FloatEqual__Q2_2EA9InputTestFfT1"
                            //     " .text            02000ecc+000008 TestAccelerationDueToGravity__Q2_2EA9InputTestFPQ3_2EA5Input8InputManPQ3_2EA5Input10Controller..C.3A.2FProjects.2FEAOS.2FCafe.2Fbuild.2FInputMan.2Fdev.2Dcafe.2Fcafe.2Dghs.2Ddev.2Dopt.2Fbuild.2Ftest.2FInputManTest.2Ftest.2Fsource.2FTestAccelerometer.2Ecpp."
                            //     " .fimport_coreinit 020d0688+000000 DK_BlockClose"
                            //     " .syscall         02000000+000008 __dotsyscall"
                            //     " .text            02073ec4+000024 __CPR174____ne__tm__105_Q4_2EA2IO17DirectoryIterator5EntryPQJ15J2IOJ23JJ42JRQJ15J2IOJ23JJ42J__5eastlFRCQ2_J120J30ListIterator__tm__10_Z1ZZ2ZZ3ZT1_b..C.3A.2FProjects.2FEAOS.2FCafe.2Fbuild.2FEAIO.2Fdev.2Dcafe.2Fcafe.2Dghs.2Ddev.2Ddebug.2Fbuild.2FEAIO.2FBB_runtime.2EEAIO.2Ecpp.2EFdY7UnAa0njOcwkT7xSw."
                            // If !mbMapFileAlphabetic then only .text entries will be found.

                            uint64_t address, size;
                            int fieldCount = EA::StdC::Sscanf(&mLineBuffer[7], "%I64x+%I64x", &address, &size); // Read the (e.g.) 02000f50+000020 part which is after the " .text " section

                            if(fieldCount == 2) // If the address+size fields could be read (it appears this will always be true)...
                            {
                                if(mbLowMemoryUsageEnabled)
                                {
                                    // In low memory mode, we stop loading right here, after the first symbol.
                                    ...
                                    AddressToFilePos entry = { (AddressSize)address, (AddressSize)filePosition };
                                    mAddressToFilePosArray.push_back(entry);
                                ...
                                {
                                    // Note that there is no case here for which we set bContinueReading = false. If the map file
                                    // is alphabetic then we need to always read until the end of the file. If the map file is 
                                    // sorted by address then we would set bContinueReading above.

                                    char* pPlus   = EA::StdC::Strchr(&mLineBuffer[7], '+');
                                    char* pSymbol = pPlus;
                                    while(*pSymbol && ((pSymbol - pPlus) < 8))
                                       ++pSymbol;

                                    if(*pSymbol) // This should always be true...
                                    {
                                        // We leave symbols mangled, as that typically uses less space. We'll demangle in GetAddressRep.
                                        const size_t symbolLength   = (size_t)(mLineBuffer + lineSize - pSymbol);
                                        char* const  pSymbolStorage = (char*)mStackAllocator.Malloc(symbolLength + 1);
                                        EA::StdC::Strcpy(pSymbolStorage, pSymbol);

                                        // We don't use the 'size' variable, as we assume all symbols are packed together. 
                                        // This allows us to save space in our data structures here. In practice is 
                                        // rarely if ever makes a difference to the user.
                                        const EntryPair entryPair = { (AddressSize)address, pSymbolStorage };
    ...
    if(bSuccess)
    {
        if(mBaseAddress == 0) // If the base address hasn't been set...
        {
            if(mbOfflineLookup)
            {
                // There's nothing to do here at this point. If this is a DLL then the user is going to need to call SetBaseAddress manually.
            }
            else
            {
                char8_t pPath8[IO::kMaxPathLength];
...
// Shutdown
//
bool MapFileGreenHills::Shutdown()
{
    // Clear our member data.
    mbLoadAttempted = false;
    mEntryPairArray.set_capacity(0);
...
// AsInterface
//
void* MapFileGreenHills::AsInterface(int id)
{
    if(id == MapFileGreenHills::kTypeId)
        return static_cast<MapFileGreenHills*>(this); // The cast isn't necessary, but makes the intent more clear.
    return NULL;
}
...
// SetBaseAddress
//
void MapFileGreenHills::SetBaseAddress(uint64_t baseAddress)
{
    mBaseAddress = baseAddress;
}
...
// GetDatabasePath
//
const wchar_t* MapFileGreenHills::GetDatabasePath() const
{
    return msMapFilePath.c_str();
}
...
// SetOption
//
void MapFileGreenHills::SetOption(int option, int value)
{
    if(option == kOptionLowMemoryUsage)
    {
        EA_ASSERT(!mbLoadAttempted);        // Must set this option before loading anything or doing any lookups.
        mbLowMemoryUsage = (value != 0);
        if(!mbMapFileAlphabetic)    // Our iterative low memory approach can't work if the symbols are alphabetically sorted instead of sorted by address.
            mbLowMemoryUsageEnabled = mbLowMemoryUsage;
        // If the .map file later turns out to be alphabetically sorted, then we will set mbMapFileAlphabetic = false.
    }
    else if(option == kOptionFileBufferSize)
        mFileBufferSize = (uint32_t)value;
    // else if(option == kOptionCopyFileToMemory) // Not currently supported.
...
// This function is used when mbLowMemoryUsageEnabled == true.
// This function fills in mAddressToFilePosArray until nAddress is found.
// Thus it builds a cache of info (mAddressToFilePosArray) about where symbols are. 
// It doesn't read every symbol line into a mAddressToFilePosArray entry, 
// but rather reads and stores one every N of them in order to save memory.
// Since the symbols are in order by address this can be used to tell us 
// roughly where a symbol can later be found in the file in the FindAddressToFilePosArray function.
bool MapFileGreenHills::FillAddressToFilePosArray(uint64_t nAddressRelative)
{
    bool bAddressFound   = false;
    bool bReadEntireFile = (mBaseAddress != 0); // If the base address is non-zero, then we have a DLL.
    ...
    while(!mbSymbolEndFound && (bReadEntireFile || !bAddressFound))
    {
        ...
        for(uint32_t i = 0; !mbSymbolEndFound && (i < mSymbolsPerLowMemSample); i++)
        {
            // We have a line that is like any of these (and possibly others):
            //     " .text            02000f50+000020 FloatEqual__Q2_2EA9InputTestFfT1"
            //     " .fimport_coreinit 020d0688+000000 DK_BlockClose"
            //     " .syscall         02000000+000008 __dotsyscall"
            ...
            symbolAddress = 0;
            filePosPrev   = mFileBuffer.GetPosition(EA::IO::kPositionTypeBegin);
            lineSize      = IO::ReadLine(&mFileBuffer, mLineBuffer, EAArrayCount(mLineBuffer));

            if(lineSize < IO::kSizeTypeDone)  // If the line was read without error and isn't at the end of the file...
                fieldCount = EA::StdC::Sscanf(&mLineBuffer[1], "%32s %I64x+%x", entryType, &symbolAddress, &symbolSize); // Read the (e.g.) " .text" and the (e.g.) 02000f50+000020 part

            // We currently define the end of the .text section to be the first line 
            // that doesn't begin with " .text". mbMapFileAlphabetic is guaranteed to be
            // false because we disallow this low memory mode when it is true. So all .text
            // entries are together because the map file is sorted by increasing address.
            if((fieldCount == 3) && (EA::StdC::Strstr(entryType, ".text") == entryType)) // If this is a valid .text line...
                mLastSymbolEnd = symbolAddress + symbolSize; // We use this to tell what the address of the end of the symbols is if we reach the end of the file.
            else
            {
                // Else the line was bad or is beyond the .text section...
                mbSymbolEndFound = true;
                symbolAddress    = mLastSymbolEnd; // Set it to this value so mAddressToFilePosArray below has a valid final entry.
    ...
    // Due to the possible DLL bookmark handling mentioned above, we may need 
    // to the delay the actual check until after reading the entire file.
    if(bReadEntireFile)
    {
        AddressToFilePos addressPos = { (AddressSize)nAddressRelative, 0 };
        AddressToFilePosArray::iterator it = eastl::upper_bound(mAddressToFilePosArray.begin(), mAddressToFilePosArray.end(), addressPos, AddressToFilePosCompare());
...
// starts reading symbol lines from it till it gets to the one that corresponds
// to nAddressRelative.
bool MapFileGreenHills::FindAddressToFilePosArray(uint64_t nAddressRelative, CAString8& s, AddressSize& result)
{
    bool bResult = false;
    AddressToFilePos addressPos = { (AddressSize)nAddressRelative, 0 };
    ...
    while((IO::ReadLine(&mFileBufferLowMemoryUsage, mLineBuffer, EAArrayCount(mLineBuffer))) < IO::kSizeTypeDone) // While there are more lines...
    {
        // .text            02061a2c+00000c GetDefaultReportFunction__Q2_2EA8UnitTestFv
        // .text            02061a38+00000c WriteToEnsureFunctionCalled__Q2_2EA8UnitTestFv
        // .text            02061a44+000018 PlatformStartup__Q2_2EA8UnitTestFv
        // .text            02061a5c+000004 PlatformShutdown__Q2_2EA8UnitTestFi
        // .text            02061a60+000140 ThreadSleepRandom__Q2_2EA8UnitTestFUiT1b
        // .bss             02062000+000000 _multiArgs

        if(EA::StdC::Strstr(mLineBuffer, " .text") == mLineBuffer) // If this is a .text line...
        {
            int      fieldCount;
            uint64_t symbolAddress = 0;
            uint32_t symbolSize = 0;

            fieldCount = EA::StdC::Sscanf(mLineBuffer, " %*s %I64x+%x", &symbolAddress, &symbolSize);

            if((fieldCount == 2) && (nAddressRelative < (symbolAddress + symbolSize))) // If this appears to be the symbol we are looking for...
            {
                char* pPlus   = EA::StdC::Strchr(&mLineBuffer[7], '+');
                char* pSymbol = pPlus;
                while(*pSymbol && ((pSymbol - pPlus) < 8))
                ...
                s.get_allocator().set_allocator(mpCoreAllocator); // To consider: Find a way to not do this for every symbol lookup.
                mLastLookupLinePosition = filePosCurrent;
                result = (AddressSize)symbolAddress;

                if(*pSymbol) // This should always be true for a valid .map file 
                    s = pSymbol; // We leave the symbol mangled here; we'll unmangle it in GetAddressRep.
                else
                    s = "?";
...
// GetAddressRep
//
int MapFileGreenHills::GetAddressRep(int addressRepTypeFlags, uint64_t nAddress, FixedString* pRepArray, int* pIntValueArray)
{
    int returnFlags = 0;
    ...
    // We need to make the following check here, as the code below doesn't account for it.
    // We don't need to make an upper-bound check, as the code below does account for it.
    if((nAddress < mBaseAddress) && !mbLowMemoryUsageEnabled) // Don't do this early exit in the case of mbLowMemoryUsageEnabled, as the base address of the map file might not be determined yet, in the case of DLLs that are relocatable in memory and we use some 'bookmark' function within the map file to determine the true runtime base address.
        return 0;
    ...
    {
        // Usually mBaseAddress is zero, which means that the virtual addresses in the map file are
        // the same as they are when the app is running on the hardware. For WiiU DLLs the 
        // base address should not be zero, but should be a value like 0x83008000. 
        ...
        if(mbLowMemoryUsageEnabled)
        {
            ...
            if((nAddressRelative >= pAddressPosEnd->mAddress) && !mbSymbolEndFound) // If this address is beyond where we've yet read in the file and if there's anything else left to read in the file...
            {
                ...
                // It's possible mBaseAddress might have changed (if we end up revising this code to handle "MainGame" like we do with MapFileCodeWarrior), so we re-evaluate.
                nAddressRelative = (nAddress - mBaseAddress);
            }
        ...
        if(entryPair.mpSymbol) // If the above succeeded...
        {
            ...
            {
                char buffer[512];
                UnmangleSymbol(entryPair.mpSymbol, buffer, sizeof(buffer), kCompilerTypeGreenHills);
    ...
    //if(addressRepTypeFlags & kARTFlagFileLine) // If the user wants file/line information...
    //{
        // It's actually possible for us to come up with some very basic file information for the given address,
        // as the .map file has a section in it like the following. We could keep a table of all the lines below
        // which have a .text section and be able to convert an address back to a .cpp file name. 
        //
        //Module Summary
        //
        //  Origin+Size    Section          Module
        //02000020+000eac  .text            main.cpp.obj
        //1000c560+00002c  .data            main.cpp.obj
        //02000ecc+000084  .text            TestAccelerometer.cpp.obj
        //10000584+000068  .rodata          TestAccelerometer.cpp.obj
        //02000f50+001a78  .text            TestController.cpp.obj
        //100005f0+0003bc  .rodata          TestController.cpp.obj
        //020029c8+000310  .text            TestInputMan.cpp.obj
        //1000c58c+000001  .data            TestInputMan.cpp.obj
        //02002cd8+000c68  .text            TestKeyboard.cpp.obj
        //00001170+000618  .rela.text       TestKeyboard.cpp.obj
...
int MapFileGreenHills::GetAddressRep(int addressRepTypeFlags, const void* pAddress, FixedString* pRepArray, int* pIntValueArray)
{
    return GetAddressRep(addressRepTypeFlags, (uint64_t)(uintptr_t)pAddress, pRepArray, pIntValueArray);
}


#endif // EACALLSTACK_GREENHILLS_MAPFILE_ENABLED
