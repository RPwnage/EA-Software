...
///////////////////////////////////////////////////////////////////////////////
    ...
    #define EACALLSTACK_VERSION   "1.20.00"
    #define EACALLSTACK_VERSION_N  12000
#endif
...
//
        ...
        #define EACALLSTACK_MODULE_INFO_SUPPORTED EA_XBDM_ENABLED
    #elif defined(EA_PLATFORM_MICROSOFT) || defined(EA_PLATFORM_SONY) || defined(EA_PLATFORM_APPLE)
...
//
// Defined as 0 or 1, with 1 being the default on platforms that benefit from it.
// If enabled, the CodeWarrior symbol name demangling code is enabled.
// CodeWarrior is a C/C++ compiler independent of other well-known compilers.
//
#ifndef EACALLSTACK_CODEWARRIOR_DEMANGLE_ENABLED
    #if defined(EA_PLATFORM_DESKTOP) || defined(__MWERKS__)
        #define EACALLSTACK_CODEWARRIOR_DEMANGLE_ENABLED 1
    #else
        #define EACALLSTACK_CODEWARRIOR_DEMANGLE_ENABLED 0
    #endif
#endif
...
///////////////////////////////////////////////////////////////////////////////
// EACALLSTACK_GREENHILLS_DEMANGLE_ENABLED
//
// Defined as 0 or 1, with 1 being the default on platforms that benefit from it.
// If enabled, the Green Hills compiler symbol name demangling code is enabled.
// Green Hills is a C/C++ compiler which uses the EDG front-end but its own
// debug symbol format and name mangling. 
//
#ifndef EACALLSTACK_GREENHILLS_DEMANGLE_ENABLED
    #if defined(EA_PLATFORM_WINDOWS) // Currently we need to use their demangling applet (decode.exe) or dll (demangle_ghs.so/demangle_gnu.so (yes these are DLLs)).
        #define EACALLSTACK_GREENHILLS_DEMANGLE_ENABLED 1 // You currently need to have the demangle_ghs.so file in the same directory as the .exe in order for this to work.
    #else
        #define EACALLSTACK_GREENHILLS_DEMANGLE_ENABLED 0
    #endif
#endif
...
//
    ...
    #if defined(EA_PLATFORM_PS3) || defined(__SNC__) || (defined(__clang__) && defined(EA_PLATFORM_SONY)) || defined(EA_PLATFORM_DESKTOP)
...
//
// Defined as 0 or 1, with 1 being the default on platforms that benefit from it.
// If enabled, the CodeWarrior .map file address/symbol lookup is enabled.
//
#ifndef EACALLSTACK_CODEWARRIOR_MAPFILE_ENABLED
    #if defined(__MWERKS__) || defined(EA_PLATFORM_DESKTOP)
        #define EACALLSTACK_CODEWARRIOR_MAPFILE_ENABLED 1
    #else
        #define EACALLSTACK_CODEWARRIOR_MAPFILE_ENABLED 0
    #endif
#endif
...
//
// Defined as 0 or 1, with 1 being the default on platforms that benefit from it.
// If enabled, the Green Hills .map file address/symbol lookup is enabled.
// Green Hills is a compiler vendor which has its own proprietary .map file format.
//
#ifndef EACALLSTACK_GREENHILLS_MAPFILE_ENABLED
    #if defined(__ghs__) || defined(EA_PLATFORM_DESKTOP)
        #define EACALLSTACK_GREENHILLS_MAPFILE_ENABLED 1
    #else
        #define EACALLSTACK_GREENHILLS_MAPFILE_ENABLED 0
    #endif
#endif
...
//
// Defined as 0 or 1, with 1 being the default on platforms that benefit from it.
// If enabled, the Green Hills executable file address/symbol lookup is enabled.
// Green Hills is a compiler vendor which has its own proprietary binary debug
// format which is an alternative to elf/dwarf debug info.
// Until we know the formal name of Green Hills debug format, we call it GHSFILE.
//
#ifndef EACALLSTACK_GREENHILLS_GHSFILE_ENABLED
    #if defined(EA_PLATFORM_DESKTOP) // Currently the only means we have to read these proprietary files is by using the Green Hills' gaddr2line.exe utility from a Windows machine.
        #define EACALLSTACK_GREENHILLS_GHSFILE_ENABLED 1
    #else
        #define EACALLSTACK_GREENHILLS_GHSFILE_ENABLED 0
    #endif
#endif
...
//
    ...
    #if defined(EA_PLATFORM_WINDOWS) || defined(__GNUC__) || defined(__SNC__) || defined(__MWERKS__) || defined(__clang__)
...
//
    ...
    #define EA_CALLSTACK_GETCALLSTACK_SUPPORTED 0
#elif (defined(EA_PLATFORM_PLAYBOOK) || defined(__QNX__)) && defined(EA_DEBUG)
    #define EA_CALLSTACK_GETCALLSTACK_SUPPORTED 1
...
// 
    ...
    #define EACALLSTACK_ENABLE_ELF_SPAWN 0
#endif
