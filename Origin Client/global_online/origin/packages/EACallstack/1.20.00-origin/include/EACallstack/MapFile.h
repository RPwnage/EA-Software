    ...
    namespace Callstack
    {
            ...
            protected:
                struct EntryPair
                {
                    ...
                    const char* mpSymbol; // Pointer to a string within the space of mStackAllocator.
                };
            ...
            protected:
                ...
                EA::Allocator::StackAllocator  mStackAllocator;  /// This is a big linear buffer which will have the symbol strings one after another.
            ...
            ///
            /// You need to link your app with the -Map linker argument in order to generate a GCC .map file.
            /// At least for the Wii platform, SetBaseAddress will need to be called for RSOs (Wii DLLs) but not ELFs (main executables).
            ///
            class EACALLSTACK_API MapFileCodeWarrior : public IAddressRepLookup
            {
            public:
                /// kTypeId
                /// Defines what type of IAddressRepLookup this is. This is used in conjunction
                /// with the AsInterface function to safely dynamically cast a base class 
                /// such as IAddressRepLookup to a subclass such as MapFileCodeWarrior.
                static const int kTypeId = 0x0685ba42; // This is just some unique value.

                /// OptionCW
                /// Additional options beyond the base set.
                enum OptionCW
                {
                    kOptionMainGameBaseAddress = 101    // Value is 0 or 1; default is 1 (but has no effect if MainGame is absent). If enabled, then when the user calls SetBaseAddress, the user is passing the address of the (WiiBootLoader) MainGame function and not the first function in the .text section. EA WiiBootLoader games do this because they don't have a more formal way of using the reporting of the .text section.
                };

                MapFileCodeWarrior(EA::Allocator::ICoreAllocator* pCoreAllocator = NULL);
               ~MapFileCodeWarrior();
            ...
            protected:
                typedef uint32_t AddressSize;

                struct EntryPair
                {
                    AddressSize mAddress; // Might want to make mAddress 64 bits so that we can read .map files from both 32 and 64 bit platforms.
                    const char* mpSymbol; // Pointer to a string within the space of mStackAllocator.
                };
                ...
                struct AddressToFilePos
                {
                    AddressSize mAddress;  // We don't store a whole lot of these so this isn't as important as it might first seem.
                    AddressSize mFilePos;  // Position in the file where this line begins.
                };
                ...
                bool Load();
                bool FillAddressToFilePosArray(uint64_t nAddress, uint64_t baseAddressToUse);              // Used when mbLowMemoryUsage == true.
                bool FindAddressToFilePosArray(uint64_t nAddress, CAString8& symbol, AddressSize& result); // Used when mbLowMemoryUsage == true.
            ...
            protected:
                ...
                bool                           mbOfflineLookup;             /// True if we are to do offline address lookups, instead of lookups for the current app's addresses. Affects how we try to auto-determine module base addresses, which is important for relocatable DLLs on all platforms.
                bool                           mbSymbolEndFound;            /// True if during load we have encountered the end of .text symbols. Used for the case of mbLowMemoryUsage = true.
                bool                           mbMainGameBaseAddress;       /// True if the user is specifying the address of the "MainGame" function as the base address of the module as opposed to specifying the base address of the first function in the .text section as the base address of the module. This could be true when using the EA WiiBootLoader build.
                FixedStringW                   msMapFilePath;               /// 
                EntryPairArray                 mEntryPairArray;             /// This holds pointers into mStackAllocator's space.
                EA::Allocator::StackAllocator  mStackAllocator;             /// This is a big linear buffer which will have the symbol strings one after another.
                uint64_t                       mBaseAddress;                /// The load address of the module.
                uint64_t                       mMainGameMapFileAddress;     /// Used for rso .map files generated by WiiBootLoader. Users of WiiBootLoader track the location of the MainGame function in order to tell where the base address of the rso is, instead of just keeping track of where the start of the .text segment really is.
                uint32_t                       mFileBufferSize;             /// Size for file buffering.
                IO::FileStream                 mFile;                       /// Used for reading the file. In low memory mode, it is read periodically as we need to see more of the symbol lines.
                IO::StreamBuffer               mFileBuffer;                 /// Used for reading the file. In low memory mode, it is read periodically as we need to see more of the symbol lines.
                IO::FileStream                 mFileLowMemoryUsage;         /// Used in low memory mode to read lines dynamically, independently of mFile/mFileBuffer.
                ...
                AddressToFilePosArray          mAddressToFilePosArray;      /// Used when mbLowMemoryUsage is true.
                uint64_t                       mFileSize;                   /// Size of map file.
                uint64_t                       mLastLookupLinePosition;     /// Where the beginning of the line is for the last successful lookup. This helps us do file name lookups efficiently. See the source code.
                uint32_t                       mSymbolsPerLowMemSample;     /// When mbLowMemoryUsage is true, we read only every N lines from the .text symbols section. That way we use less memory, but have to read as many as N lines from the file on every lookup instead of just reading our (potentially large) data structures. See mRVAToFilePosArray.
            ...
            /// MapFileGreenHills
            ///
            /// You need to link your app with the -map=<name> linker argument in order to generate a Green Hills .map file.
            /// The -Mn argument causes the map file entries to be sorted by ascending address instead of alphabetic name.
            ///
            class EACALLSTACK_API MapFileGreenHills : public IAddressRepLookup
            {
            public:
                /// kTypeId
                /// Defines what type of IAddressRepLookup this is. This is used in conjunction
                /// with the AsInterface function to safely dynamically cast a base class 
                /// such as IAddressRepLookup to a subclass such as MapFileGreenHills.
                static const int kTypeId = 0x0685b99a; // This is just some unique value.

                MapFileGreenHills(EA::Allocator::ICoreAllocator* pCoreAllocator = NULL);
               ~MapFileGreenHills();
            ...
            protected:
                EA::Allocator::ICoreAllocator* mpCoreAllocator;             /// 
                bool                           mbMapFileAlphabetic;         /// If true then the .map file entries were written by the linker in alphabetic order instead of address order. This prevents us from supporting mbLowMemoryUsage, at least as it was intended to be.
                bool                           mbLoadAttempted;             /// True if we have tried to load the map file.
                bool                           mbLowMemoryUsage;            /// True if the user wants us to read the file iteratively instead of compiling it into our data structures.
                bool                           mbLowMemoryUsageEnabled;     /// True if mbLowMemoryUsage is true (i.e. the user requested it) and if it's technically possible (which is so if the map file is sorted by address instead of symbol name).
                bool                           mbOfflineLookup;             /// True if we are to do offline address lookups, instead of lookups for the current app's addresses. Affects how we try to auto-determine module base addresses, which is important for relocatable DLLs on all platforms.
                bool                           mbSymbolEndFound;            /// True if during load we have encountered the end of .text symbols. Used for the case of mbLowMemoryUsage = true.
                FixedStringW                   msMapFilePath;               /// 
                char                           mLineBuffer[2200];           /// This needs to be a little bigger than mSymbolNameBuffer.
                EntryPairArray                 mEntryPairArray;             /// This holds pointers into mStackAllocator's space.
                EA::Allocator::StackAllocator  mStackAllocator;             /// This is a big linear buffer which will have the symbol strings one after another.
                uint64_t                       mBaseAddress;                /// The load address of the module.
                uint32_t                       mFileBufferSize;             /// Size for file buffering.
                ...
                uint64_t                       mLastLookupLinePosition;     /// Where the beginning of the line is for the last successful lookup. This helps us do file name lookups efficiently. See the source code.
                uint32_t                       mSymbolsPerLowMemSample;     /// When mbLowMemoryUsage is true, we read only every N lines from the .text symbols section. That way we use less memory, but have to read as many as N lines from the file on every lookup instead of just reading our (potentially large) data structures. See mRVAToFilePosArray.
                uint64_t                       mLastSymbolEnd;              /// When mbLowMemoryUsage is true, we keep track of what the apparent end of the symbols is.
            }; // MapFileGreenHills
