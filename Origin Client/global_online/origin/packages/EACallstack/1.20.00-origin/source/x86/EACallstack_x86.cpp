...
//
// As of this writing, this file is experimental. However, if you find 
// it useful and would like to test it, feel free to do so and comment
// on its usefulness and correctness. 
//
///////////////////////////////////////////////////////////////////////////////
...
#if defined(EA_PLATFORM_LINUX) // Others?
    #include <dlfcn.h>
#endif
...
EACALLSTACK_API size_t GetCallstack(void* pReturnAddressArray[], size_t nReturnAddressArrayCapacity, const CallstackContext* pContext)
{
    ...
    // For Linux and Apple platforms we have separate source files which have more extensive support than this.
...
EACALLSTACK_API size_t GetModuleFromAddress(const void* address, char* pModuleName, size_t moduleNameCapacity)
{
        ...
        int     result = dladdr((void*)address, &dlInfo);

        if((result == 0) && dlInfo.dli_fname)
...
EACALLSTACK_API bool GetCallstackContext(CallstackContext& context, intptr_t threadId)
{
        ...
        if(dwThreadIdCurrent == dwThreadIdArg)
        {
            ...
            #ifndef __MWERKS__ // The Metrowerks assembler is broken.
                __asm{
