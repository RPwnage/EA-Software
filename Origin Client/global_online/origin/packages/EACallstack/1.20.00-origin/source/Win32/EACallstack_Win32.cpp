...
#if defined(EA_PLATFORM_WIN32) // The following only works on Win32 and not Win64.
...
EACALLSTACK_API void GetCallstackContext(CallstackContext& context, const Context* pContext)
{
        ...
        EA_COMPILETIME_ASSERT(offsetof(EA::Callstack::Context, Eip) == offsetof(CONTEXT, Eip));
        EA_COMPILETIME_ASSERT(offsetof(EA::Callstack::Context, SegSs) == offsetof(CONTEXT, SegSs));
    #endif
...
EACALLSTACK_API bool GetCallstackContext(CallstackContext& context, intptr_t threadId)
{
    ...
    const DWORD sysThreadId        = EA::Callstack::GetThreadIdFromThreadHandle(threadId);
