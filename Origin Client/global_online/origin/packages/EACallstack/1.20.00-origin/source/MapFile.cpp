...
int MapFileMSVC::GetAddressRep(int addressRepTypeFlags, uint64_t nAddress, FixedString* pRepArray, int* pIntValueArray)
{
    ...
    if(mbLowMemoryUsage) // If we haven't read the entire .map file ahead of time but instead are reading it incrementally...
    {
        ...
        // So we use a rule of thumb which in practice will yield no false failures for 99% of apps (100% on some platforms).
            ...
            nMaxAddress = baseAddressToUse +  75000000;    //  ~75 MB
...
// Since the symbols are in order by address this can be used to tell us 
// roughly where a symbol can later be found in the file in the FindAddressToFilePosArray function.
bool MapFileCodeWarrior::FillAddressToFilePosArray(uint64_t nAddressRelative, uint64_t baseAddressToUse)
{
    bool bAddressFound   = false;
    bool bReadEntireFile = (baseAddressToUse != 0); // If the base address is non-zero, then we have a DLL (Wii RSO). In that case we need to read the entire file because we need to know if there's a MainGame function somewhere in it.
            ...
            else
            {
                ...
                if((lineSize > 50) && (mMainGameMapFileAddress == 0))
                {
                    if(EA::StdC::Strstr(line, " MainGame") && EA::StdC::Strstr(line, "main.cpp"))
                    {
                        ...
                        if(mbMainGameBaseAddress && baseAddressToUse)
                        {
                            nAddressRelative += baseAddressToUse;           // Convert it back to the absolute form.
                            baseAddressToUse -= mMainGameMapFileAddress;    // Set base address to what the user really meant it to be.
                            nAddressRelative -= baseAddressToUse;           // Now get our real relative address.
                        }
                    }
                }
            }
...
bool MapFileCodeWarrior::FindAddressToFilePosArray(uint64_t nAddressRelative, CAString8& s, AddressSize& result)
{
    ...
    while((lineSize = IO::ReadLine(&mFileBufferLowMemoryUsage, line, kLineSize)) < IO::kSizeTypeDone) // While there are more lines...
    {
        ...
        if((lineSize >= 40) && EA::StdC::Isxdigit(line[2]) && !EA::StdC::Strstr(line, " .text")) // If this is a valid line and not an "UNUSED" line and not a .text line (which is a summary of the following lines) ...
        {
            ...
            uint32_t symbolSize = 0;
            char     symbolMangled[1024+1]; symbolMangled[0] = 0;// If you change this buffer size, change the sscanf limit below.

            fieldCount = EA::StdC::Sscanf(line, "%*s %x %I64x %*s %*s %1024s", &symbolSize, &symbolAddress, symbolMangled);
...
// GetBaseAddress
//
uint64_t MapFileCodeWarrior::GetBaseAddress() const
{
    return mBaseAddress;
}
...
int MapFileCodeWarrior::GetAddressRep(int addressRepTypeFlags, uint64_t nAddress, FixedString* pRepArray, int* pIntValueArray)
{
    ...
    // We need to make the following check here, as the code below doesn't account for it.
    // We don't need to make an upper-bound check, as the code below does account for it.
    if((nAddress < baseAddressToUse) && !mbLowMemoryUsage) // Don't do this early exit in the caes of mbLowMemoryUsage, due to the MainGame problem.
        return 0;
    ...
    if(addressRepTypeFlags & kARTFlagFunctionOffset)
    {
        // Usually mBaseAddress (baseAddressToUse) is zero, which means that the virtual addresses in the map file are
        ...
        if(mbLowMemoryUsage)
        {
            ...
            if((nAddressRelative >= pAddressPosEnd->mAddress) && !mbSymbolEndFound) // If this address is beyond where we've yet read in the file and if there's anything else left to read in the file...
            {
                FillAddressToFilePosArray(nAddressRelative, baseAddressToUse);
                pAddressPosEnd = &mAddressToFilePosArray.back();

                // It's possible mBaseAddress (baseAddressToUse) might have changed (due to the MainGame problem), so we re-evaluate.
                nAddressRelative = (nAddress - baseAddressToUse);
            }
    ...
    if(addressRepTypeFlags & kARTFlagFileLine)
    {
        if(returnFlags & kARTFlagFunctionOffset) // If something was found above...
        {
            if(mbLowMemoryUsage)
            {
				...
				EA_UNUSED(lineSize);
                int           fieldCount;
                char          fileName1[256 + 1]; fileName1[0] = 0;   // If you change this size, change the format size below.
                char          fileName2[256 + 1]; fileName2[0] = 0;   // If you change this size, change the format size below.
                ...
                fieldCount = EA::StdC::Sscanf(&line[2], "%*s %*s %*s %*s %*s %*s %256s %256s", fileName1, fileName2);           // This should always return at least 1.
...
// GetBaseAddress
//
uint64_t MapFileGreenHills::GetBaseAddress() const
{
    return mBaseAddress;
}
...
// Since the symbols are in order by address this can be used to tell us 
// roughly where a symbol can later be found in the file in the FindAddressToFilePosArray function.
bool MapFileGreenHills::FillAddressToFilePosArray(uint64_t nAddressRelative, uint64_t baseAddressToUse)
{
    bool bAddressFound   = false;
    bool bReadEntireFile = (baseAddressToUse != 0); // If the base address is non-zero, then we have a DLL.
    ...
    if(addressRepTypeFlags & kARTFlagFunctionOffset)
    {
        ...
        if(mbLowMemoryUsageEnabled)
        {
            ...
            if((nAddressRelative >= pAddressPosEnd->mAddress) && !mbSymbolEndFound) // If this address is beyond where we've yet read in the file and if there's anything else left to read in the file...
            {
                ...
                // It's possible mBaseAddress (baseAddressToUse) might have changed (if we end up revising this code to handle "MainGame" like we do with MapFileCodeWarrior), so we re-evaluate.
                nAddressRelative = (nAddress - baseAddressToUse);
            }
