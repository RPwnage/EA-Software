...
int TestDasm()
{
    ...
    // Test DetectCurrentProcessor.
    EA::Dasm::Processor currentProcessor = EA::Dasm::kProcessorNone;
    currentProcessor = EA::Dasm::DetectCurrentProcessor(); // GCC gets confused, so we re-assign like this.
    (void)currentProcessor;
    EATEST_VERIFY(currentProcessor != EA::Dasm::kProcessorNone);
        ...
        for(eastl::list<eastl::wstring>::iterator it = gTestFileList.begin(); it != gTestFileList.end(); ++it)
        {
            ...
            if(sFilePath.find(EA_WCHAR(".dasm")) < sFilePath.size()) // For each .dasm file...
            {
                ...
                switch((int)processor) // Cast to int to avoid compiler warnings about non-handled enums.
                {
                    ...
                    // To do: Make cases for other platforms.
                }
            }
        }
    }
...

    {
        ...
        // If the following fails, are you running on x86 but don't have DasmX86Dll.dll present?
        // Disabled because it sometimes makes mistakes.
        // EATEST_VERIFY((processor == currentProcessor) && EA::StdC::Strcmp("Are you running on x86 but don't have DasmX86Dll.dll present?", "") != 0);
    ...
    {
        // Do the random instruction pattern test for x86.
    }
