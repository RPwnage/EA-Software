...
///////////////////////////////////////////////////////////////////////////////
// TestDemangleCodeWarrior
//
static int TestDemangleCodeWarrior()
{
    ...
    int nErrorCount = 0;
        ...
        len = UnmangleSymbol("CharStringUninitializedCopy<c>__5eastlFPCcPCcPc_Pc", buffer, sizeof(buffer), kCompilerTypeCodeWarrior);
        // Currently fails: EATEST_VERIFY((len == strlen("eastl::CharStringUninitializedCopy<char>(const char*,const char*,char*)")) && (strcmp(buffer, "eastl::CharStringUninitializedCopy<char>(const char*,const char*,char*)") == 0));

        len = UnmangleSymbol("RegisterInterfacesCB__Fv", buffer, sizeof(buffer), kCompilerTypeCodeWarrior);
        EATEST_VERIFY((len == strlen("RegisterInterfacesCB(void)")) && (strcmp(buffer, "RegisterInterfacesCB(void)") == 0));

        len = UnmangleSymbol("CB_GetExternalVariableFunc__FPCc", buffer, sizeof(buffer), kCompilerTypeCodeWarrior);
        EATEST_VERIFY((len == strlen("CB_GetExternalVariableFunc(const char*)")) && (strcmp(buffer, "CB_GetExternalVariableFunc(const char*)") == 0));

        len = UnmangleSymbol("Init__11AIPExternalFRCQ211AIPExternal12AIPInterface", buffer, sizeof(buffer), kCompilerTypeCodeWarrior);
        EATEST_VERIFY((len == strlen("AIPExternal::Init(const AIPExternal::AIPInterface&)")) && (strcmp(buffer, "AIPExternal::Init(const AIPExternal::AIPInterface&)") == 0));

        len = UnmangleSymbol("__ct__16MainMenuHandlersFv", buffer, sizeof(buffer), kCompilerTypeCodeWarrior);
        EATEST_VERIFY((len == strlen("MainMenuHandlers::MainMenuHandlers(void)")) && (strcmp(buffer, "MainMenuHandlers::MainMenuHandlers(void)") == 0));
    #endif
...
// TestDemangleGCC
//
// Applies to SN/PS3/EDG as well as general GCC.
//
...
// TestDemangleMSVC
//
static int TestDemangleMSVC()
{
    int nErrorCount = 0;

    #if EACALLSTACK_MSVC_DEMANGLE_ENABLED
        // Currently EACallstack implements MSVC demangling by using the built-in 
        // VC++ demangling function. We don't currently test this demangling because
        // we assume that the Microsoft version works. We don't support demangling
        // VC++ symbols on other platforms, but most people wouldn't care about 
        // that anyway.
    #endif
...
///////////////////////////////////////////////////////////////////////////////
// TestDemangleGreenHills
//
static int TestDemangleGreenHills()
{
    int nErrorCount = 0;
...
int TestSymbolUtil()
{
    ...
    nErrorCount += TestDemangleCodeWarrior();
    nErrorCount += TestDemangleGCC();
    nErrorCount += TestDemangleMSVC();
    nErrorCount += TestDemangleGreenHills();
