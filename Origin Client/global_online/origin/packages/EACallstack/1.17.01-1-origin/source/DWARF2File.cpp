...
//
// This a DWARF2 reader designed for gleaning basic symbol information such
// as the function name, line number, and source file information from the
// DWARF2 symbol data stored in an ELF file.
//
// The canonical DWARF2 format specification can be found here:
...
// DWARF2File
//
DWARF2File::DWARF2File(EA::Allocator::ICoreAllocator* pCoreAllocator)
...
// SetAllocator
//
void DWARF2File::SetAllocator(EA::Allocator::ICoreAllocator* pCoreAllocator)
{
...
// Init
//
bool DWARF2File::Init(const wchar_t* pDWARF2FilePath, bool /*bDelayLoad*/)
{
    if(!mbInitialized)
    {
        if(pDWARF2FilePath)
        {
            msDwarfFilePath = pDWARF2FilePath;
            mFileStream.SetPath(pDWARF2FilePath);
            ...
            if(mFileStream.Open(IO::kAccessFlagRead, IO::kCDOpenExisting, IO::FileStream::kShareRead))
            {
                ...
                if(mELFFile.Init(mpStream))
                {
                    ...
                    // Gather the position information of the DWARF2 data from the ELF
                    // section headers that DWARF2 defines.
                    ...
                    {
                        mbRelocated      = textShdr.sh_addr == 0;
                        mBaseAddress     = textShdr.sh_addr;    // Relocatable libraries (e.g. Unix .so, PlayStation3 .prx) should usually (always?) have a sh_addr of 0.
    ...
    if(mbInitialized)
    {
        ...
        if((mBaseAddress == 0) && !mbOfflineLookup) // If the base address hasn't been set...
        {
            ...
            EA::StdC::Strlcpy(pPath8, pDWARF2FilePath, IO::kMaxPathLength);
...
// AsInterface
//
void* DWARF2File::AsInterface(int id)
{
    if(id == DWARF2File::kTypeId)
        return static_cast<DWARF2File*>(this); // The cast isn't necessary, but makes the intent more clear.
    return NULL;
}
...
// SetBaseAddress
//
void DWARF2File::SetBaseAddress(uint64_t baseAddress)
{
...
// GetDatabasePath
//
const wchar_t* DWARF2File::GetDatabasePath() const
{
...
// SetOption
//
void DWARF2File::SetOption(int option, int value)
{
...
// GetAddressRep
//
int DWARF2File::GetAddressRep(int addressRepTypeFlags, uint64_t nAddress, FixedString* pRepArray, int* pIntValueArray)
{
    int returnFlags = 0;
    ...
    if(mbInitialized)
    {
        ...
        if((nAddress < mBaseAddress) || (nAddress >= (mBaseAddress + mTextSegmentSize)))
        {
            #if defined(EA_PLATFORM_WINDOWS) && EACALLSTACK_ENABLE_ELF_SPAWN
        ...
        if(mbRelocated)
            ...
            if(FindAbbrevSectionEntry(nAddress, &abbrevSectionEntry)) // This used to be here but it doesn't seem to be correct: (&& abbrevSectionEntry.mnStmtList)
            {
                ...
                if(addressRepTypeFlags & (1 << kARTFunctionOffset))
                {
                    ...
                    if(GetFunctionName(&abbrevSectionEntry, &pRepArray[kARTFunctionOffset]))
                    {
                        char   buffer[256];
...
// GetAddressRep
//
int DWARF2File::GetAddressRep(int addressRepTypeFlags, const void* pAddress, FixedString* pRepArray, int* pIntValueArray)
{
...
// GetFunctionName
//
bool DWARF2File::GetFunctionName(const AbbrevSectionEntry* pAbbrevSectionEntry, FixedString* pFunctionNameOut)
{
    ...
    if(pAbbrevSectionEntry->HasAnyAttribute(ATTR_NAME))
    {
        ...
        if(mpStream->SetPosition(pos))
        {
            ...
            do
            {
                if(!IO::ReadUint8(mpStream, nTemp8))
                    return false;
                if(nTemp8)
                    *pFunctionNameOut += (int8_t)nTemp8; // Append to the string.
            }
            while(nTemp8);
...
// GetSourceFilePath
//
bool DWARF2File::GetSourceFilePath(const FileLineInfo* pFileLineInfo, FixedString* pSourceFilePathOut)
{
...
// #define DWARF2_DEBUG
// #define DWARF2_DEBUG_2

#if defined(DWARF2_DEBUG_2)
...
//
bool DWARF2File::FindAbbrevSectionEntry(uint64_t address, AbbrevSectionEntry* pAbbrevSectionEntryOut)
{
...
//
int DWARF2File::GetAddressRepSpawn(int addressRepTypeFlags, uint64_t nAddress, FixedString* pRepArray, int* pIntValueArray)
{
    int returnFlags = 0;
        ...
        enum Addr2LineType
        {
            ...
            kAddr2LineTypeSPU,      // PS3 SPU addr2line.exe path
            kAddr2LineTypePPU,      // PS3 PPU addr2line.exe path
            kAddr2LineTypeCount
        };

        wchar_t elfReaderCabFileDir [EA::IO::kMaxPathLength]; elfReaderCabFileDir[0] = 0;
        wchar_t elfReaderCabFilePath[EA::IO::kMaxPathLength]; elfReaderCabFilePath[0] = 0;
        wchar_t elfReaderFilePath[kAddr2LineTypeCount][EA::IO::kMaxPathLength]; elfReaderFilePath[0][0] = 0; elfReaderFilePath[1][0] = 0; elfReaderFilePath[2][0] = 0;
        ...
        // See if the user has PS3 tools installed on their system. Nearly all PS3 programmers do.
        const char* pPS3RootPath = getenv("SCE_PS3_ROOT");

        if(pPS3RootPath)
        {
            // PPU
            length = EA::StdC::Strlcpy(elfReaderFilePath[kAddr2LineTypePPU], pPS3RootPath, EA::IO::kMaxPathLength);
            eastl::replace(elfReaderFilePath[kAddr2LineTypePPU], elfReaderFilePath[kAddr2LineTypePPU] + length, (wchar_t)'/', (wchar_t)'\\');
            if(elfReaderFilePath[kAddr2LineTypePPU][length] != '\\')
                elfReaderFilePath[kAddr2LineTypePPU][length++] = '\\';
            EA::StdC::Strlcat(elfReaderFilePath[kAddr2LineTypePPU], L"host-win32\\ppu\\bin\\ppu-lv2-addr2line.exe", EA::IO::kMaxPathLength);

            // SPU
            length = EA::StdC::Strlcpy(elfReaderFilePath[kAddr2LineTypeSPU], pPS3RootPath, EA::IO::kMaxPathLength);
            eastl::replace(elfReaderFilePath[kAddr2LineTypeSPU], elfReaderFilePath[kAddr2LineTypeSPU] + length, (wchar_t)'/', (wchar_t)'\\');
            if(elfReaderFilePath[kAddr2LineTypeSPU][length] != '\\')
                elfReaderFilePath[kAddr2LineTypeSPU][length++] = '\\';
            EA::StdC::Strlcat(elfReaderFilePath[kAddr2LineTypeSPU], L"host-win32\\spu\\bin\\spu-lv2-addr2line.exe", EA::IO::kMaxPathLength);
        }
            ...
            if(cabFile.Open(EA::IO::kAccessFlagWrite, EA::IO::kCDCreateAlways))
            {
                ...
                if(bCabFileOK)
                {
                    ...
                    if(cabReader.Init())
                    {
                        ...
                        char8_t elfReaderCabFilePath8[EA::IO::kMaxPathLength];
                        char8_t elfReaderCabFileDir8[EA::IO::kMaxPathLength];
        ...
        if(runner.Init())
        {
            ...
            for(int i = 0; i < kAddr2LineTypeCount; i++)
            {
                runner.SetTargetProcessPath(elfReaderFilePath[i]);
                ...
                bool elfReadSuccessful    = (sOutput.find("error: ") == EA::Callstack::CAString8::npos) && (sOutput.find(": Bad value") == EA::Callstack::CAString8::npos);

                if((result == 0) && fileFormatRecognized && elfReadSuccessful)
                {
                    // Process the output
...
///////////////////////////////////////////////////////////////////////////////
// DWARF2SectionReader::DWARF2SectionReader
//
DWARF2SectionReader::DWARF2SectionReader()
    : IO::MemoryStream(),
      mpDWARF2(NULL),
...
///////////////////////////////////////////////////////////////////////////////
// DWARF2SectionReader::ReadULEB128
//
...
//
// See the DWARF2 specification section 7.6.
//
bool DWARF2SectionReader::ReadULEB128(uint64_t* pnULEB128Out)
{
    ...
    #if defined(EA_DEBUG) || (EA_PLATFORM_WORD_SIZE >= 8)
    uint64_t nResult = 0;
    #else
    uint32_t nResult = 0; // For 32 bit ELFs we don't need 64 bit data, and VC++ is slow at working with 64 bit integers on x86.
    #endif
    ...
    do
    {
        ...
        nResult  = nResult | ((nTemp8 & 0x7F) << nShift);
        nShift  += 7;
...
///////////////////////////////////////////////////////////////////////////////
// DWARF2SectionReader::ReadSLEB128
//
...
// See the DWARF2 specification section 7.6.
//
bool DWARF2SectionReader::ReadSLEB128(int64_t* pnSLEB128Out)
{
    // This function is a major bottleneck on large .elf files. We gain 
    // significant speedups (~3 faster lookups) by doing direct access to 
    // the read buffer and by avoiding some checking in release builds.
...
//
#if defined(EA_DEBUG)
    bool DWARF2SectionReader::ReadULEB128(AbbrevTag* pAbbrevTag)
    {
        uint64_t nValue64;
...
//
#if defined(EA_DEBUG)
    bool DWARF2SectionReader::ReadULEB128(DwarfForm* pDwarfForm)
    {
        uint64_t nValue64;
...
//
#if defined(EA_DEBUG)
    bool DWARF2SectionReader::ReadULEB128(DwarfAttribute* pDwarfAttribute)
    {
        uint64_t nValue64;
...
// CompUnitReader::Init
//
bool CompUnitReader::Init(DWARF2File* pDWARF2)
{
    mpDWARF2 = pDWARF2;

    // Read the aranges section into memory.
    int nArangesSectionStart = mpDWARF2->mnDebugArangesStart;
    int nArangesSectionEnd   = mpDWARF2->mnDebugArangesEnd;

    mnStreamMemSize = (uint32_t)(unsigned)(nArangesSectionEnd - nArangesSectionStart);
    mpStreamMem     = (char8_t*)mpDWARF2->mpCoreAllocator->Alloc(mnStreamMemSize, EACALLSTACK_ALLOC_PREFIX "EACallstack/DWARF2File", 0);

    if(mpStreamMem)
    {
        const IO::off_type pos = (IO::off_type)(int32_t)pDWARF2->mELFOffset + (IO::off_type)nArangesSectionStart;

        if(pDWARF2->mpStream->SetPosition(pos) &&
           pDWARF2->mpStream->Read(mpStreamMem, mnStreamMemSize) == mnStreamMemSize)
        {
            SetData(mpStreamMem, mnStreamMemSize, true, true, mpDWARF2->mpCoreAllocator); // Have the stream take over the pointer. It will free it on stream destruction.
            return true;
        }
    }
...
bool CompUnitReader::FindUnitContainingAddress(uint64_t address, CompUnitInfo* pCompUnitInfoOut)
{
    ...
    // the specified address. See section 7.20 of the DWARF2 specification.
    bool bFound = false;
    ...
    if(SetPosition(0))
    {
        ...
        while(!bFound && (GetPosition() < streamSize) && (++outerLoopCount < 100000000))
        {
            ...
            if(!IO::ReadUint32(this, artHeader.mnLength, mpDWARF2->mEndian)) // In 64 bit DWARF3 format (not necessarily meaning a 64 bit platform), this is 0xffffffff, followed by a uint64_t which is the real length.
                return false;
            if(!IO::ReadUint16(this, artHeader.mnVersion, mpDWARF2->mEndian))
                return false;
            if(artHeader.mnVersion < 2) // If not DWARF version 2 or 3 (also this provides something of a sanity check)...
                return false;
            if(!IO::ReadUint32(this, artHeader.mnInfoOffset, mpDWARF2->mEndian))
                return false;
            ...
            while((nTupleOffset < (artHeader.mnLength + 4)) && (++innerLoopCount < 100000000)) // 4-byte length field not included in size
            {
                ...
                {
                    case 4:
                        if(!IO::ReadUint32(this, nAddressStart32, mpDWARF2->mEndian))
                            return false;
                        if(!IO::ReadUint32(this, nAddressLength32, mpDWARF2->mEndian))
                            return false;
                    ...
                    case 8:
                        if(!IO::ReadUint64(this, nAddressStart64, mpDWARF2->mEndian))
                            return false;
                        if(!IO::ReadUint64(this, nAddressLength64, mpDWARF2->mEndian))
                            return false;
                        break;
        ...
        if(bFound)
        {
            // Read the header. See section 7.5.1 of the DWARF2 standard.
            if(!mpDWARF2->mpStream->SetPosition((int32_t)(mpDWARF2->mELFOffset + mpDWARF2->mnDebugInfoStart + artHeader.mnInfoOffset)))
                return false;
            if(!IO::ReadUint32(mpDWARF2->mpStream, pCompUnitInfoOut->mnLength, mpDWARF2->mEndian))
                return false;
            if(!IO::ReadUint16(mpDWARF2->mpStream, pCompUnitInfoOut->mnVersion, mpDWARF2->mEndian))
                return false;
            if(pCompUnitInfoOut->mnVersion < 2) // Verify DWARF version 2 or later. This check also acts as a sanity check.
                return false;
            if(!IO::ReadUint32(mpDWARF2->mpStream, pCompUnitInfoOut->mnAbbrevOffset, mpDWARF2->mEndian))
                return false;
            if(!IO::ReadUint8(mpDWARF2->mpStream, pCompUnitInfoOut->mnPointerSize))
                return false;

            pCompUnitInfoOut->mnPositionInInfoSection   = mpDWARF2->mnDebugInfoStart   + artHeader.mnInfoOffset;
            pCompUnitInfoOut->mnPositionInAbbrevSection = mpDWARF2->mnDebugAbbrevStart + pCompUnitInfoOut->mnAbbrevOffset;
        }
    }
...
//
bool InfoSectionReader::Init(DWARF2File* pDWARF2)
{
    mpDWARF2 = pDWARF2;

    // Read the portion of the compilation unit info section that contains data related to 
    // the compilation unit containing our target address. See the DWARF2 Standard, sec 6.1.2.
    const int nInfoSectionStart = (int)(mpDWARF2->mCompUnitInfo.mnPositionInInfoSection + 11);      // Skip 11 byte header (size of the CompUnitInfo struct data) 
    const int nInfoSectionEnd   = (int)(nInfoSectionStart + mpDWARF2->mCompUnitInfo.mnLength + 4);  // Length field (4 bytes) not included in size
    ...
    if(mpStreamMem)
    {
        const IO::off_type pos = (IO::off_type)pDWARF2->mELFOffset + (IO::off_type)nInfoSectionStart;
...
///////////////////////////////////////////////////////////////////////////////
// InfoSectionReader::ReadAddressForm
//
bool InfoSectionReader::ReadAddressForm(uint64_t /*nFormType*/, uint64_t* pnFormOut)
{
    if(mpDWARF2->mCompUnitInfo.mnPointerSize == 4)
    {
        uint32_t value;
        const bool result = ReadUint32(this, value, mpDWARF2->mEndian);
        *pnFormOut = value;
        return result;
    }

    return IO::ReadUint64(this, *pnFormOut, mpDWARF2->mEndian);
}
...
bool InfoSectionReader::ReadConstantForm(uint64_t nFormType, uint64_t* pnFormOut)
{
    ...
    switch(nFormType)
    {
        ...
        case DW_FORM_DATA2:
            if(!IO::ReadUint16(this, nTemp16, mpDWARF2->mEndian))
                return false;
            *pnFormOut = nTemp16;
            break;

        case DW_FORM_DATA4:
            if(!IO::ReadUint32(this, nTemp32, mpDWARF2->mEndian))
                return false;
            *pnFormOut = nTemp32;
            break;

        case DW_FORM_DATA8:
            if(!IO::ReadUint64(this, nTemp64U, mpDWARF2->mEndian))
                return false;
            *pnFormOut = nTemp64U;
            break;
...
bool InfoSectionReader::ReadReferenceForm(uint64_t nFormType, uint64_t* pnFormOut)
{
    if(nFormType == DW_FORM_REF_ADDR)
    {
        ...
        {
            uint32_t value;
            const bool result = IO::ReadUint32(this, value, mpDWARF2->mEndian);
            *pnFormOut = value;
            return result;
        }
        else // (pointer size == 8)
    ...
    else
    {
        ...
        *pnFormOut = mpDWARF2->mCompUnitInfo.mnPositionInInfoSection;
        ...
        switch(nFormType)
        {
            ...
            case DW_FORM_REF8:
                if(!IO::ReadUint64(this, nTemp64, mpDWARF2->mEndian))
                    return false;
                break;
...
bool InfoSectionReader::ReadStringForm(uint64_t nFormType, uint64_t* pnFormOut)
{
    if(nFormType == DW_FORM_STRING)
    {
        ...
        *pnFormOut = (uint64_t)(int64_t)GetPosition() + mpDWARF2->mCompUnitInfo.mnPositionInInfoSection + 11;
    ...
    else if(nFormType == DW_FORM_STRP)
    {
        ...
        // Return the string's offset from the beginning of the file.
        *pnFormOut  = (unsigned)mpDWARF2->mnDebugStrStart;
        *pnFormOut += nTemp32;
...
bool InfoSectionReader::SkipForm(uint64_t nFormType)
{
    ...
    {
        case DW_FORM_ADDR:
            if(!SetPosition(mpDWARF2->mCompUnitInfo.mnPointerSize, IO::kPositionTypeCurrent))
                return false;
            break;
...
//
bool AbbrevSectionReader::Init(DWARF2File* pDWARF2, InfoSectionReader* pInfoSectionReader, bool bCacheEntries, EA::Allocator::ICoreAllocator* /*pAllocator*/)
{
    mpDWARF2            = pDWARF2;
    ...
    // assert(mpDWARF2->mnDebugAbbrevStart                      <= mpDWARF2->mCompUnitInfo.mnPositionInAbbrevSection);
    // assert(mpDWARF2->mCompUnitInfo.mnPositionInAbbrevSection <  mpDWARF2->mnDebugAbbrevEnd);
    int nAbbrevSectionStart = (int)mpDWARF2->mCompUnitInfo.mnPositionInAbbrevSection;
    int nAbbrevSectionEnd   = (int)mpDWARF2->mnDebugAbbrevEnd;
    ...
    if(mpStreamMem)
    {
        const IO::off_type pos = (IO::off_type)pDWARF2->mELFOffset + (IO::off_type)nAbbrevSectionStart;
...
#if defined(DWARF2_DEBUG)
    void TraceAbbrevSectionEntry(uint64_t nTempName, uint64_t /*nTempForm*/, DWARF2File* pFile, const AbbrevSectionEntry* pAbbrevSectionEntry)
    {
        FixedString fs;
...
bool AbbrevSectionReader::FindEntryByCode(uint64_t nAbbrevCode, AbbrevSectionEntry* pAbbrevSectionEntryOut)
{
    ...
    // See section 7.5.3 of the DWARF2 Standard.
...
bool AbbrevSectionReader::ReadAbbreviationSectionEntry(AbbrevSectionEntry* pAbbrevSectionEntryOut)
{
    ...
    while(nTempName && nTempForm) // Read all the attributes of the current entry (AbbrevTag).
    {
        ...
        else
        {
            ...
            #if defined(DWARF2_DEBUG)
                TraceAbbrevSectionEntry(nTempName, nTempForm, mpDWARF2, pAbbrevSectionEntryOut);
            #endif
...
//
bool LineSectionReader::Init(DWARF2File* pDWARF2, AbbrevSectionEntry* pAbbrevSectionEntry)
{
    mpDWARF2 = pDWARF2;
    ...
    // compilation unit.
    const int nLineSectionStart = mpDWARF2->mnDebugLineStart + (int)(unsigned)pAbbrevSectionEntry->mnStmtList;

    // Determining the length of the line program requires loading the length
    // field from the program prologue.
    const IO::off_type pos = (IO::off_type)pDWARF2->mELFOffset + (IO::off_type)nLineSectionStart;

    if(!pDWARF2->mpStream->SetPosition(pos))
        return false;
    if(!IO::ReadUint32(pDWARF2->mpStream, mPrologue.mnTotalLength, mpDWARF2->mEndian))
        return false;
    ...
    if(mpStreamMem)
    {
        if(pDWARF2->mpStream->Read(mpStreamMem, mnStreamMemSize) == mnStreamMemSize)
        {
            // Set the buffer as our memory stream.
            SetData(mpStreamMem, mnStreamMemSize, true, true, mpDWARF2->mpCoreAllocator); // Have the stream take over the pointer. It will free it on stream destruction.
            return true;
...
bool LineSectionReader::FindFileLineInfoForAddress(uint64_t address, FileLineInfo* pFileLineInfoOut)
{
    ...
    if(!IO::ReadUint16(this, mPrologue.mnVersion, mpDWARF2->mEndian))
        return false;
    if(mPrologue.mnVersion != 2)
        return false;
    if(!IO::ReadUint32(this, mPrologue.mnPrologueLength, mpDWARF2->mEndian))
        return false;
    ...
    for(unsigned int i = 0; i < nFileIndex; ++i)
    {
        ...
        do
        {
            if(!IO::ReadUint8(this, nTemp8))
                return false;
            if(nTemp8)
                pFileLineInfoOut->msSourceFileName += (int8_t)nTemp8;
        }
        while(nTemp8);
    ...
    for(unsigned int i = 0; i < nDirectoryIndex; ++i)
    {
        ...
        do
        {
            if(!IO::ReadUint8(this, nTemp8))
                return false;
            if(nTemp8)
                pFileLineInfoOut->msSourceFileDirectory += (int8_t)nTemp8;
        }
        while(nTemp8);
    }
    ...
    while(GetPosition() < streamSize)
    {
        ...
        switch(nOpcode)
        {
            ...
            case DW_LNS_FIXED_ADVANCE_PC:
                if(!IO::ReadUint16(this, nOperandU16, mpDWARF2->mEndian))
                    return false;
...
bool LineSectionReader::ExecuteSpecialOpcode(uint64_t address, uint8_t nSpecialOpcode)
{
    ...
    {
        // See DWARF2 specification section 6.2.5.1, p. 54.
        nSpecialOpcode        = (uint8_t)(unsigned)(nSpecialOpcode - mPrologue.mnOpcodeBase);
        mNextState.mnLine    += mPrologue.mnLineBase + (nSpecialOpcode % mPrologue.mnLineRange);
        mNextState.mnAddress += nSpecialOpcode / mPrologue.mnLineRange * mPrologue.mnMinimumInstructionLength;
