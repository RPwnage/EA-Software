...
// GetCallstackHash
//
EACALLSTACK_API callstack_hash_t GetCallstackHash(const void* callstack[], size_t entryCount)
{
    ...
    for(size_t i = 0; i < entryCount; i++)
    {
            ...
            h = (callstack_hash_t)((h << 4) ^ (uintptr_t)callstack[i]);
        #else
            h = (callstack_hash_t)((h << 4) ^ ((uintptr_t)callstack[i] >> 2));  // '>> 2' because the addresses will have the two lower bits as 0 on RISC processors.
        #endif
    }
