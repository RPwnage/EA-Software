...
//#include <cvconst.h> // This header defines enum SymTagEnum, etc. but this header doesn't exist in VC++, at least up till VS2008. It's present in the DIA SDK.
    ...
    { 
        ...
        FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, dwError, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR) &pError, 0, NULL);

        strncpy(buffer, pError, capacity);
        buffer[capacity-1] = 0;
...
bool PDBFileMS::Init(const wchar_t* pPDBFilePath, bool /*bDelayLoad*/)
{
    ...
    const uint64_t baseAddressGuesses[3] = { 0x82000000, 0x00400000, 0x0000000140000000i64 }; // XBox 360, Win32, Win64 defaults.
    ...
    if(mBaseAddress == 0) // If the base address hasn't been set...
    {
        ...
        if(mBaseAddress == 0) // We don't test mbOfflineLookup because we have no choice but to provide a non-zero base address and unless we guess something then we will fail SymLoadModule64 below.
        {
    ...
    dwLoadResult = ::SymLoadModule64(mhSymbols, NULL, pPath8, NULL,
                                     mBaseAddress,    // Non-zero value required when loading a .pdb file.
    ...
    if(dwLoadResult) // If SymLoadModule64 succeeded...
    {
        ...
        if(bGuessBaseAddress)
        {
            for(int i = 1; (i < 2) && dwLoadResult; i++)
            {
                ...
                SymUnloadModule64(mhSymbols, mBaseAddress);
    ...
    else
    {
        ...
        if(dwError == ERROR_SUCCESS)
            mBaseAddress = 1; // Something non-zero.
        else
        {
            #if defined(EA_DEBUG) && defined(EA_ASSERT_ENABLED) && EA_WINAPI_FAMILY_PARTITION(EA_WINAPI_PARTITION_DESKTOP)
                char buffer[256];
...
int PDBFileMS::GetAddressRep(int addressRepTypeFlags, uint64_t nAddress, FixedString* pRepArray, int* pIntValueArray)
{
    ...
    if(addressRepTypeFlags & kARTFlagFunctionOffset)
    {
        ...
        if(result && ((symbolInfo.mSymbolInfo.Tag == 0) || (symbolInfo.mSymbolInfo.Tag == 5))) // 5 == SymTagFunction
        {
            pRepArray[kARTFunctionOffset]      = symbolInfo.mSymbolInfo.Name;
...
int PDBFileMS::GetAddressRep(int addressRepTypeFlags, const void* pAddress, FixedString* pRepArray, int* pIntValueArray)
{
...
static inline void LE(uint16_t* p16, uint32_t n)
{
    #if defined(EA_SYSTEM_BIG_ENDIAN)
        for(uint16_t* p = p16; n; n--, p++)
            *p = (uint16_t) ((*p >> 8) | (*p << 8));
    #else
        (void)p16; (void)n;
    #endif
...
PDBFileCustom::SymbolInfo::SymbolInfo()
  : mAddressOffset(0),
    mpSymbolText(NULL)
{
}
...
PDBFileCustom::SymbolInfo::SymbolInfo(uint32_t a, char* p)
  : mAddressOffset(a),
    mpSymbolText(p)
{
}
            ...
            if(addressRepTypeFlags & kARTFlagFunctionOffset)
            {
                if(mbBuildSymbolInfoArray) // If we have pre-stored symbol information...
                {
                    ...
                    if(it != mSymbolInfoArray.end())
                    {
                        ...
                        s.resize((eastl_size_t)UnmangleSymbol(si.mpSymbolText, (char*)s.data(), s.size(), kCompilerTypeMSVC));
...
int PDBFileCustom::GetAddressRep(int addressRepTypeFlags, const void* pAddress, FixedString* pRepArray, int* pIntValueArray)
{
    return GetAddressRep(addressRepTypeFlags, (uint64_t)(uintptr_t)pAddress, pRepArray, pIntValueArray);
}
                    ...
                    if(bReturnValue)
                    {
                        ...
                        if(bReturnValue)
                        {
                            ...
                            if(bReturnValue)
                            {
                                ...
                                // It usually begins at section #7 and its information is repeated up to as much as section #10.
                                // We loop until we find the first instance of it. Since we don't know the actual format of the
                                // the section entry system in detail, we do this based on empirical observation of the file format.
                                pBlockIndexArray = (mpTOC->mFileSizeArray + mpTOC->mFileCount);

                                for(uint32_t i = 0, iLast = PDB::kFileTypeSectionDir; i <= iLast; i++)
                                {
                                    nBlockCount       = (mpTOC->mFileSizeArray[i] + mHeader.mBlockSize - 1) / mHeader.mBlockSize; // Round up mFileSizeArray[i] to block size.
                                    pBlockIndexArray += nBlockCount;
                                ...
                                EA_ASSERT(bReturnValue); // If this fails then we didn't find our .text section above. But it should always be present. Maybe we need to loop through more entries.
                            }
                        }
                    }
                }
...
bool PDBFileCustom::IterateSymbols()
{
    ...
    SymbolBuffer        symbolBuffer(EA::Callstack::EASTLCoreAllocator(EACALLSTACK_ALLOC_PREFIX "EACallstack/PDBFileCustom", mpCoreAllocator));
    AddressOffsetArray  addressOffsetArray;   // Holds temporary strings for the current best-match symbol.
                ...
                case PDB::kPublicFunc2V3:
                {
                    ...
                    if(symbolCurrent.mPublicV3.mSymType == 2) // We usually have either 0 (data) or 2 (function)
                    {
                        ...
                        if(mbBuildSymbolInfoArray) // If we are to build the symbol information into a data structure...
                        {
                            ...
                            const size_t nStrlen = strlen(pSymbolCurrent->mPublicV3.mName);
                            SymbolInfo   symbolInfoCurrent(symbolCurrent.mPublicV3.mOffset, (char*)mStackAllocator.Malloc(nStrlen + 1));

                            if(symbolInfoCurrent.mpSymbolText)
                            {
                                strcpy(symbolInfoCurrent.mpSymbolText, pSymbolCurrent->mPublicV3.mName);
                                mSymbolInfoArray.push_back(symbolInfoCurrent);
                            }
                        }
    ...
    if(mbBuildSymbolInfoArray)
    {
        ...
        if(!mSymbolInfoArray.empty())
        {
            ...
            SymbolInfo  symbolInfo(symbolInfoLast.mAddressOffset + 8192, symbolInfoLast.mpSymbolText);
            mSymbolInfoArray.push_back(symbolInfo);
        }
