...
{
    sprintf(dd.mOperation, "(invalid) 0x%08x", (unsigned)in); // e.g. "invalid 0x12345678"
    dd.mOperands[0] = 0;
...
void WriteImmediateOperands(DasmData& dd, ppc_word in, int uimm, int instructionType)
{
    ...
    {
        case 0: // kOFHex
            sprintf(s, "r%u,r%u,%d", (unsigned)GetD(in), (unsigned)GetA(in), (int)i); // e.g. "r4,r6,3256"
            break;

        case 1: // kOFHex
            sprintf(s, "r%u,r%u,%d", (unsigned)GetA(in), (unsigned)GetD(in), (int)i); // e.g. "r6,r4,3256"
            break;

        case 2: // kOFHex
            sprintf(s, "r%u,%d", (unsigned)GetA(in), (int)i); // e.g. "r4,3256"
            break;

        case 3: // kOFHex
            sprintf(s, "r%u,%d", (unsigned)GetD(in), (int)i); // e.g. "r4,3256"
            break;
...
{
    s += strlen(s);
    sprintf(s, "r%u,r%u", (unsigned)GetA(in), (unsigned)GetB(in)); // e.g. "r2,r9"
}
...
char* Write_rDrArB(char* s, ppc_word in, int mask)
{
    ...
    {
        if(mask & 4)
            s += sprintf(s, "r%u,", (unsigned)GetD(in));
        if(mask & 2)
            s += sprintf(s, "r%u,", (unsigned)GetA(in));
        if(mask & 1)
            s += sprintf(s, "r%u,", (unsigned)GetB(in));
        --s;
    }
...
char* Write_fDrArB(char* s, ppc_word in, int mask)
{
    ...
    {
        if(mask & 4)
            s += sprintf(s, "f%u,", (unsigned)GetD(in));
        if(mask & 2)
            s += sprintf(s, "r%u,", (unsigned)GetA(in));
        if(mask & 1)
            s += sprintf(s, "r%u,", (unsigned)GetB(in));
...
char* Write_vDvAvB(char* s, ppc_word in, int mask)
{
    ...
    {
        if(mask & 4)
            s += sprintf(s, "v%u,", (unsigned)GetD(in));
        ...
        if(mask & 2)
        {
            if(mask & 16)  // A = SIMM (signed immediate value)
            {
                ...
                s += sprintf(s, "%d,", (int)((a > 15) ? (a - 32) : a)); // kOFHex
            }
            else if(mask & 8)  // A = UIMM (unsigned immediate value)
                s += sprintf(s, "%u,", (unsigned)GetA(in)); // kOFHex
            else
                s += sprintf(s, "v%u,", (unsigned)GetA(in));
        }

        if(mask & 1)
            s += sprintf(s, "v%u,", (unsigned)GetB(in));
...
void WriteTrapi(DasmData& dd, ppc_word in, InstructionFlags flag)
{
    ...
    if(kStrTrapCondition[index])
    {
        ...
        sprintf(dd.mOperation, "t%c%s", flag & kIF64Bit ? 'd' : 'w', kStrTrapCondition[index]); // e.g. "twgti"
...
void WriteTrap(DasmData& dd, ppc_word in, unsigned char dmode)
{
    ...
    if(kStrTrapCondition[to])
    {
        ...
        sprintf(dd.mOperation, "t%c%s", dmode ? 'd' : 'w', kStrTrapCondition[to]); // e.g. "tdeq", "tdlt"
...
void WriteCmpi(DasmData& dd, ppc_word in, int uimm)
{
    ...
    if(i < 2)
    {
        ...
        sprintf(dd.mOperation, "%si", kStrCompare[(uimm * 2) + i]); // e.g. "cmpdi", "cmpwi"

        i = GetCRD(in);

        if(i)
            sprintf(dd.mOperands, "cr%c,", (char)('0' + i)); // e.g.  "cr3,"
...
void WriteCmp(DasmData& dd, ppc_word in)
{
    ...
    if(i < 2)
    {
        ...
        if(i)
            sprintf(dd.mOperands, "cr%c,", (char)('0' + i));
...
void WriteAddi(DasmData& dd, ppc_word in, const char* pSuffix)
{
    ...
    {
        sprintf(dd.mOperation, "l%s", pSuffix);  // e.g. "li", "lis"
        WriteImmediateOperands(dd, in, 0, 3);
    }
    else
    {
        sprintf(dd.mOperation, "%s%s", (in & 0x00008000) ? "sub" : "add", pSuffix); // e.g. "subic", "addis"
...
void WriteBranch(DasmData& dd, ppc_word in, const char* bname, int aform, int displacement)
{
    ...
    if(bo & 4)
    {
        ...
        if(bo & 16)
        {
            // branch always 
            if(GetOpcode(in) != 16)
                sprintf(dd.mOperation, "b%s%s", bname, pSuffix);
            else
            {
                sprintf(dd.mOperation, "bc%s", pSuffix); // e.g. "bcla", "bcl"
                sprintf(dd.mOperands, "%u,%u", (unsigned)bo, (unsigned)bi);
            }
        }
        else 
        {
            // branch conditional 
            sprintf(dd.mOperation,"b%s%s%s%c", kStrCondition[((bo & 8) >> 1) + (bi & 3)], bname, pSuffix, y);

            if(bi >= 4)
                sprintf(dd.mOperands, "cr%d", (unsigned)bi >> 2);
        }
    }
    else
    {
        // CTR is decremented and checked 
        sprintf(dd.mOperation, "bd%s%s%s%c", kStrDecrementType[bo >> 1], bname, pSuffix, y);

        if((bo & 16) == 0)
            sprintf(dd.mOperands, "%u", (unsigned)bi);
    }
}
...
void WriteBranchConditional(DasmData& dd, ppc_word in)
{
    ...
    if(in & 2)  // If branching to an absolute address...
        sprintf(s, "0x%08x", (unsigned)d);
    else
    {
        if(dd.mInstructionAddress > (uint64_t)UINT32_MAX)
            sprintf(s, "0x%016" PRIx64, (uint64_t)(dd.mInstructionAddress + d));
        else
            sprintf(s, "0x%08x", (unsigned)(dd.mInstructionAddress + d));
    }
...
void WriteBli(DasmData& dd, ppc_word in)
{
    ...
    sprintf(dd.mOperation, "b%s", kStrBranchDetail[in & 3]);  // e.g. "bl", "ba", "bla"

    if(in & 2) // If branching to an absolute address...
        sprintf(dd.mOperands, "0x%08x", (unsigned)d);
    else
    {
        if(dd.mInstructionAddress > (uint64_t)UINT32_MAX)
            sprintf(dd.mOperands, "0x%016" PRIx64, (uint64_t)(dd.mInstructionAddress + d));
        else
            sprintf(dd.mOperands, "0x%08x", (unsigned)(dd.mInstructionAddress + d));
    }
...
void WriteMcrf(DasmData& dd, ppc_word in, char c)
{
    if((in & 0x0063f801) == 0)
    {
        sprintf(dd.mOperation, "mcrf%c", c); // e.g. "mcrf", "mcrfs"
        sprintf(dd.mOperands, "cr%u,cr%u", (unsigned)GetCRD(in), (unsigned)GetCRA(in));
    }
    else
        WriteInvalidInstruction(dd, in);
}
...
void WriteCROperation(DasmData& dd, ppc_word in, const char* n1, const char* n2)
{
    ...
    if((in & 1) == 0)
    {
        sprintf(dd.mOperation, "cr%s", ((cra == crb) && n2) ? n2 : n1); // e.g. "crnand", "crnor", "crnot"

        if((cra == crb) && n2) // kOFMnemonics
            sprintf(dd.mOperands, "%u,%u", (unsigned)crd, (unsigned)cra); // e.g. "crnand cr2,cr3"
        else
            sprintf(dd.mOperands, "%u,%u,%u", (unsigned)crd, (unsigned)cra, (unsigned)crb); // e.g. "crnand cr2,cr3,cr4"
    }
    else
        WriteInvalidInstruction(dd, in);
}
...
void WriteRlw(DasmData& dd, ppc_word in, const char* name, int bImmediateOrRegister)
{
    ...
    sprintf(dd.mOperation, "rlw%s%c", name, (in & 1) ? '.' : '\0'); // e.g. "rlwimi", "rlwinm."
    sprintf(dd.mOperands, "r%u,r%u,%s%u,%u,%u", (unsigned)a, (unsigned)s, kStrImmediateOrRegister[bImmediateOrRegister & 1], (unsigned)bsh, (unsigned)mb, (unsigned)me);
}
...
void WriteRld(DasmData& dd, ppc_word in, const char* name, int bImmediateOrRegister)
{
    ...
    dd.mInstructionFlags |= kIF64Bit;
    sprintf(dd.mOperation, "rld%s%c", name, (in & 1) ? '.' : '\0'); // e.g. "rldimi", "rldinm."
    sprintf(dd.mOperands, "r%u,r%u,%s%u,%u", (unsigned)a, (unsigned)s, kStrImmediateOrRegister[bImmediateOrRegister & 1], (unsigned)bsh, (unsigned)m);
}
...
void WriteThreeRegisterOperation(DasmData& dd, ppc_word in, const char* name, int mask, int smode, int chkoe, int32_t chkrc, unsigned char dmode)
{
    ...
    else
    {
        ...
        sprintf(dd.mOperation, "%s%s%s", name, kStrOverflow[chkoe && (in & kMaskOE)], kStrConditionRegisterModified[(chkrc < 0) && (in & 1)]);
...
void WriteMtcr(DasmData& dd, ppc_word in)
{
    ...
    {
        sprintf(dd.mOperation, "mtcr%c", (crm == 0xff) ? '\0' : 'f'); // e.g. "mtcr", "mtcrf"

        if(crm != 0xff) // kOFMnemonics
            oper += sprintf(oper, "0x%02x,", (unsigned)crm); // Might be useful to spell out all the fields. kOFHelp

        sprintf(oper, "r%u", (unsigned)s);
    }
}
...
void WriteMsr(DasmData& dd, ppc_word in,int smode)
{
    ...
    else
    {
        ...
        sprintf(dd.mOperation, "m%csr", smode ? 't' : 'f'); 

        if(smode)
            sprintf(dd.mOperands, "%u,r%u", (unsigned)sr, (unsigned)s); // Might be useful to spell out all the fields. kOFHelp
        else
            sprintf(dd.mOperands, "r%u,%u", (unsigned)s, (unsigned)sr);
    }
}
...
void WriteMspr(DasmData& dd, ppc_word in, int smode)
{
    ...
    else
    {
        ...
        sprintf(dd.mOperation, "m%c%s", smode ? 't': 'f', x);

        if(fmt) 
        {
            if(smode)
                sprintf(dd.mOperands,"%u,r%u", (unsigned)spr, (unsigned)d);
            else
                sprintf(dd.mOperands,"r%u,%u", (unsigned)d, (unsigned)spr);
        }
        else
            sprintf(dd.mOperands,"r%u", (unsigned)d);
    }
}
...
void WriteMtb(DasmData& dd, ppc_word in)
{
    ...
    else
    {
        ...
        s += sprintf(dd.mOperands, "r%u", (unsigned)destRegister);
        ...
        switch (timeBaseRegister)
        {
            ...
            default:
                lh = '\0';
                dd.mInstructionFlags |= kIFSupervisor;
                sprintf(s, ",%u", (unsigned)timeBaseRegister); // e.g. mOperands => "r7,270"
                break;
        }

        sprintf(dd.mOperation, "mftb%c", lh); // e.g. "mftbl"
    }
}
...
void WriteSradi(DasmData& dd, ppc_word in)
{
    ...
    dd.mInstructionFlags |= kIF64Bit;
    sprintf(dd.mOperation, "sradi%c", (in & 1) ? '.' : '\0');                     // e.g. "sradi."
    sprintf(dd.mOperands, "r%u,r%u,%u", (unsigned)a, (unsigned)s, (unsigned)bsh); // e.g. "r8,r7,4"
}
...
void WriteLdst(DasmData& dd, ppc_word in, const char* name, char reg, unsigned char dmode)
{
    ...
    sprintf(dd.mOperands, "%c%d,%d(r%u)", reg, (unsigned)s, (unsigned)d, (unsigned)a); // e.g. "r3,(16)r24",  "f4,(16)r24"    // kOFHex
}
...
void WriteFdacb(DasmData& dd, ppc_word in, const char* name, int mask, unsigned char dmode)
{
    ...
    dd.mInstructionFlags |= dmode;
    sprintf(dd.mOperation, "f%s%s", name, kStrConditionRegisterModified[in & 1]);

    s += sprintf(s, "f%u,", (unsigned)GetD(in));      // fD

    if(mask & 4)
        s += sprintf(s, "f%u,", (unsigned)GetA(in));   // fA
    else
        err |= GetA(in);

    if(mask & 1)
        s += sprintf(s, "f%u,", (unsigned)GetRotationB(in)); // fC
    else if((mask & 8) == 0)
        err |= GetRotationB(in);

    if(mask & 2)
        s += sprintf(s, "f%u,", (unsigned)GetB(in));  // fB
    else if(GetB(in))
        err |= GetB(in);
...
void WriteFcmp(DasmData& dd, ppc_word in, char c)
{
    ...
    {
        sprintf(dd.mOperation, "fcmp%c", c);
        sprintf(dd.mOperands, "cr%u,f%u,f%u", (unsigned)GetCRD(in), (unsigned)GetA(in), (unsigned)GetB(in));
    }
}
...
void WriteMtfsb(DasmData& dd, ppc_word in, int n)
{
    ...
    {
        sprintf(dd.mOperation, "mtfsb%d%s", n, kStrConditionRegisterModified[in & 1]); // e.g "mtfsb0", "mtfsb1."
        sprintf(dd.mOperands, "%u", (unsigned)GetD(in));
    }
}
...
{
    sprintf(dd.mOperation, "%s%s", name, kStrConditionRegisterModified[chkrc && (in & kMaskAltivec)]);
...
void WriteVdabc(DasmData& dd, ppc_word in)
{
    ...
    {
        sprintf(dd.mOperation, "%s", kStrAltivec[nameidx]);
        char* s = Write_vDvAvB(dd.mOperands, in, 7);
        sprintf(s, (nameidx == 12) ? ",%u" : ",v%u", (unsigned)GetRotationB(in));
    }
    else
        WriteInvalidInstruction(dd, in);
}
...
void WriteVldst(DasmData& dd, ppc_word in, const char* name)
{
    ...
    else
    {
        ...
        strcpy(dd.mOperation, name);
        sprintf(dd.mOperands, "v%d,r%d,r%d", (unsigned)GetD(in), (unsigned)GetA(in), (unsigned)GetB(in));
    }
}
...
void WriteAltivecDataStream(DasmData& dd, ppc_word in, const char* name)
{
    ...
    else
    {
        ...
        if(GetExtendedOpcode(in) == 822)  // dss, dssall
        {
            ...
            sprintf(dd.mOperation, "d%s%s", name, (in & kMaskDestination) ? "all" : "");
        }
        else
        {
            sprintf(dd.mOperation, "d%s%c", name, (in & kMaskDestination) ? 't' : '\0');
            s += sprintf(s, "r%u,r%u,", (unsigned)GetA(in), (unsigned)GetB(in));
        }

        sprintf(s, "%u", (unsigned)GetAltivecStream(in));
        dd.mInstructionFlags |= kIFAltivec;
    }
}
            ...
            switch (GetExtendedOpcode(in))
            {
                ...
                case 512:
                    ...
                    else 
                    {
                        ...
                        sprintf(dd.mOperands,"cr%u", (unsigned)GetCRD(in));
                    }
                    break;
            ...
            else
            {
                switch (GetExtendedOpcode(in))
                {
                    ...
                    case 134:
                        if((in & 0x006f0800) == 0)
                        {
                            sprintf(dd.mOperation,"mtfsfi%s", kStrConditionRegisterModified[in & 1]);
                            sprintf(dd.mOperands,"%u,%u", (unsigned)GetCRD(in), (unsigned)(in & 0x0000f000) >> 12);
                        }
                        else
                            WriteInvalidInstruction(dd, in);
                        break;
                    ...
                    case 583:
                        ...
                        {
                            sprintf(dd.mOperation, "mffs%s", kStrConditionRegisterModified[in & 1]);
                            sprintf(dd.mOperands, "f%u", (unsigned)GetD(in));
                        }
                        else
                            WriteInvalidInstruction(dd, in);
                        break;
                    ...
                    case 711:
                        if((in & 0x02010000) == 0)
                        {
                            sprintf(dd.mOperation, "mtfsf%s", kStrConditionRegisterModified[in & 1]);
                            sprintf(dd.mOperands, "%u,f%u", (unsigned)(in & 0x01fe0000) >> 17, (unsigned)GetB(in));
                        }
                        else
                            WriteInvalidInstruction(dd, in);
                        break;
                                       ...
                                       DasmData& dd, uint32_t optionFlags, uint64_t dataAddress)
{
    ...
    if(dd.mInstructionAddress > (uint64_t)UINT32_MAX)
        sprintf(dd.mAddress, "%016" PRIx64, (uint64_t)dd.mInstructionAddress);
    else
    ...
    EA::StdC::Sprintf(dd.mInstruction, "%08x", *pData32);
