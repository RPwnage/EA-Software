...
{
    eastl_size_t pos(sPath.find_last_of(L"\\/"));
...
{
    const eastl_size_t pos(sPath.find_last_of(L"\\/"));
...
void Win32Runner::ExceptionOccurred(const void* pExceptionInformation)
{
    ...
    if(mbPrintOutput)
    {
        ...
        printf(" Address  = 0x%08x\n", (DWORD)(UINT_PTR)pExceptionRecord->ExceptionAddress);
...
int Win32Runner::LocalRun()
{
    ...
    if(bResult)
    {
        ...
        bResult = CreateProcessW(msTargetProcessPath.c_str(),       // lpApplicationName - if 0 the first arg of the cmdline is used and the path is searched.
                                 ...
                                 sTargetDirectory.c_str(),          // lpCurrentDirectory
        ...
        if(bResult)
        {
            ...
            while(mbProcessIsRunning)
            {
                ...
                if(GetRunnerTime() >= min(mResponseTimeoutAbsolute, mUnilateralTimeoutAbsolute)) // If we have a timeout...
                    break;
...
int Win32Runner::RunToCompletionWithOutput(const char8_t* pCommandString, int64_t responseTimeoutSeconds, int64_t unilateralTimeoutSeconds, EA::Callstack::CAString8& sOutput)
{
    std::basic_string<char8_t>  s8(pCommandString);
    std::basic_string<char16_t> s16(EA::StdC::ConvertString<std::basic_string<char8_t>, std::basic_string<char16_t> >(s8));
