...
// TODO: Remove old callstack implementation code by June, 2014.
    ...
    namespace EA
    {
        namespace Callstack
        {
            ...
            using ::EA::Thread::GetStackLimit;
                ...
                using ::EA::Thread::GetCallstackSPU;
            #endif
    ...
    namespace EA
    {
        namespace Callstack
        {
            ...
            EACALLSTACK_API size_t GetCallstack(void* callstack[], size_t maxDepth, const CallstackContext* pContext = NULL);
                ...
                /// GetCallstackSPU
                ///
                /// Returns the callstack of the SPU identified by spuThreadId.
                /// The returned values are addresses on the SPU.
                /// This function executes on the PPU (main PS3 CPU).
                ///
                EACALLSTACK_API size_t GetCallstackSPU(void* callstack[], size_t maxDepth, intptr_t spuThreadId);
            #endif
            ...
            /// just an integer (Wii, Playstation3) or opaque identifier (e.g. pthread).
            ...
            #elif defined(EA_PLATFORM_PSP2)  // To consider: This may well work for all SN-based compilers. If so then try to coalesce them all here.

                EACALLSTACK_API EA_NO_INLINE void GetInstructionPointer(void*& p);

                #define EAGetInstructionPointer(p) { p = reinterpret_cast<void*>(__builtin_current_pc()); }
            ...
            #elif (defined(__MWERKS__) || defined(EA_COMPILER_GREEN_HILLS)) && defined(EA_PROCESSOR_POWERPC) // This covers EA_PLATFORM_GAMECUBE, EA_PLATFORM_REVOLUTION and EA_PLATFORM_CAFE
            ...
            #elif defined(__GNUC__) || defined(EA_COMPILER_CLANG) // This covers EA_PLATFORM_PS3, EA_PLATFORM_UNIX, EA_PLATFORM_OSX, EA_PLATFORM_PLAYSTATION2, EA_PLATFORM_GAMECUBE
            ...
            EACALLSTACK_API void* GetStackCurrent();
                ...
                #define EASetStackBase()  \
                    ::EA::Callstack::SetStackBottom(__GETREG(1));

            #elif defined(EA_PLATFORM_CTR)          // Nintendo 3DS
            ...
            #elif defined(__GNUC__) // This covers EA_PLATFORM_PS3, EA_PLATFORM_UNIX, EA_PLATFORM_OSX, EA_PLATFORM_PLAYSTATION2, EA_PLATFORM_GAMECUBE
            ...
            struct CallstackContextARM : public CallstackContextBase
            {
                ...
                struct RSOModuleInfo
                {
                    FixedString8     mModuleFileName;    // Usually a file name or path. May be useful if this is the .map file name or path, though this is usually the same as the file name.
                    RSOObjectHeader* mpRSOObjectHeader;  // The original RSOObjectHeader.
                    uint32_t         mModuleTextOffset;  // Address of the beginning of the code in memory (the .text segment base address). This is an address, but for symbol lookup purposes it is an offset from zero.
                    uint32_t         mModuleTextSize;    // Size of the code (as opposed to data)
                };

                typedef eastl::fixed_vector<RSOModuleInfo, 2, true, EA::Callstack::EASTLCoreAllocator> RSOModuleInfoArray;

                // RSOModuleInfoManager
                //
                // This class is not thread-safe. However, in practice it can usually be used in a thread-safe manner
                // without synchronization if the data is constant over the life of the application.
                //
                // Example usage:
                //     // Typical RSO loading code:
                //     DVDOpen(moduleFileName, &fileInfo);
                ...
                //
                //     // In this case we immediately trace the information.
                //     RSOModuleInfo info;
                //     RSOModuleInfoManager::ReadRSOModuleInfo(moduleFileName, pRSOObjectHeaderr, info);
                //     DebugPrint("RSO Module Load: %s %u %u\n", info.mModuleFileName.c_str(), info.mModuleOffset, info.mModuleSize);    
                //
                //     // In this case we save the information for later retrieval.
                //     gRSOModuleInfoManager.AddRSOModule(moduleFileName, pRSOObjectHeader);
                //
                class RSOModuleInfoManager
                {
                public:
                    RSOModuleInfoArray mRSOModuleInfoArray;

                    // The user can set a global RSOModuleInfoManager pointer so that other parts of the app can find it.
                    // If the user doesn't set this pointer then it remains NULL.
                    // This is useful only if the AddRSOModule, etc. class functions are used.
                    static RSOModuleInfoManager* sRSOModuleInfoManager;

                    // Returns the mRSOModuleInfoArray array index of the newly added module, which will be the last index in the array.
                    // The pModuleFileName argument can be either the file name or the file path.
                    const RSOModuleInfo* AddRSOModule(const char* pModuleFileName, RSOObjectHeader* pRSOObjectHeader);

                    // Remove a module by object header.
                    void RemoveRSOModule(RSOObjectHeader* pRSOObjectHeader);

                    // Given an module name, object header, or pCodeAddress (whichever is non-NULL), return a pointer to the associated RSOModuleInfo.
                    // The pModuleFileName argument can be either the file name or the file path.
                    const RSOModuleInfo* GetRSOModuleInfo(const char* pModuleFileName, const RSOObjectHeader* pRSOObjectHeader, const void* pCodeAddress) const;

                    // This is a base function that converts an RSOObjectHeader to RSOModuleInfo.
                    // Gets the RSOModuleInfo for an RSOObjectHeader. The pModuleFileName argument isn't required, as it will just be copied into moduleInfo.
                    // The pModuleFileName argument can be either the file name or the file path.
                    static void ReadRSOModuleInfo(const char* pModuleFileName, RSOObjectHeader* pRSOObjectHeader, RSOModuleInfo& moduleInfo);
                };
    ...
    namespace EA
    {
        namespace Callstack
        {
            ...
            #ifdef EA_PLATFORM_CTR // Nintendo 3DS
                #define SET_CTR_CALLSTACK_BOTTOM EASetStackBase
                inline void SetCallstackBottom(uintptr_t p){ SetStackBase(p); }
            #endif
        }
    }
