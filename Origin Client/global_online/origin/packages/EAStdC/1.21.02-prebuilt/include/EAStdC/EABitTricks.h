...
namespace StdC
{
	...
	inline int GetSign(int32_t x){
		return (int)((x >> 31) | ((uint32_t)-x >> 31));
	}

	inline int GetSign(int64_t x){
		return (int)((x >> 63) | ((uint64_t)-x >> 63));
	}
	...
	///
	/// Note: This function has been moved to EABase and exists here only for backwards compatibility.
	///
	/// Example usage:
	///     int array[75];
	///     size_t arrayCount = EAArrayCount(array); // The count is 75.
	///
	#ifndef EAArrayCount
	...
	/// even if the given usage happens to work. Workaround is to suppress this warning.
	#ifndef EAOffsetOf

		#undef EA_STDC_USE_CUSTOM_OFFSETOF

		#if defined(EA_COMPILER_CLANG)
			#pragma clang diagnostic ignored "-Winvalid-offsetof"
		#elif  defined(EA_COMPILER_SN)
			// SN does not produce a warning when using offsetof on non POD types
			// but builtin offsetof fails on non const input: offsetof(Type, x[j]). Use workaround. This means static_assert can't be used.
			#define EA_STDC_USE_CUSTOM_OFFSETOF 1
		#elif  defined(EA_PLATFORM_PS3) || defined(EA_PLATFORM_PS3_SPU)
			// PS3 gcc and sn does not produce warning when using offsetof on non POD types. 
			// but builtin offsetof fails on non const input: offsetof(Type, x[j]). Use workaround. This means static_assert can't be used.
			#define EA_STDC_USE_CUSTOM_OFFSETOF 1
		...
		#if defined(EA_STDC_USE_CUSTOM_OFFSETOF)
			#define EAOffsetOf(s, m) ((size_t)(((uintptr_t)&reinterpret_cast<const volatile char&>((((s*)65536)->m))) - 65536))
		#else
			#define EAOffsetOf (int16_t)offsetof
		#endif
