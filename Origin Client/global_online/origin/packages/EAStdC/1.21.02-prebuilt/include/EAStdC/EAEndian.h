...
// EAEndian.h
//
// Copyright (c) 2007-2009, Electronic Arts. All Rights Reserved.
...
namespace EA
{
	namespace StdC
	{
		...
		// PowerPC / SNC
		#elif defined(EA_PROCESSOR_POWERPC) && defined(__SNC__)
			inline uint16_t Swizzle(uint16_t x)
			{
				return (uint16_t)__builtin_lhbrx(&x, 0);
			}
			inline int16_t Swizzle(int16_t x)
				{ return (int16_t)Swizzle((uint16_t)x); }
			#define EA_SWIZZLE_16_DEFINED
			...
			inline uint32_t Swizzle(uint32_t x)
			{
				return (uint32_t)__builtin_lwbrx(&x, 0);
			}
			inline int32_t Swizzle(int32_t x)
				{ return (int32_t)Swizzle((uint32_t)x); }
			#define EA_SWIZZLE_32_DEFINED
		...
		// GNUC/PowerPC
		#elif defined(EA_PROCESSOR_POWERPC) && defined(EA_COMPILER_GNUC) && !defined(__SNC__)
			inline uint16_t Swizzle(uint16_t x)
			{
				uint16_t temp;
