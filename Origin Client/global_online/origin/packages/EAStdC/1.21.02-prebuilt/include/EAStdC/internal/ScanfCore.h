...
enum Modifier
{
	...
	kModifierMax_t,      // Use intmax_t argument. Specified by 't' in front of d, i, o, u, x, or X.
	kModifierSize_t,     // Use size_t argument. Specified by 't' in front of d, i, o, u, x, or X.
...
struct CharBitmap
{
	...
	int Get16(int c) const // If c >= 256, we return whatever the first bit is, since it will be equal to what bits 256 - 65536 are meant to be.
		{ if(c < 256) return (int)mBits[(uint8_t)(unsigned)c >> 5] & (1 << (c & 31)); else return (int)(mBits[0] & 0x00000001); }

	int Get32(int c) const // If c >= 256, we return whatever the first bit is, since it will be equal to what bits 256 - 2^32 are meant to be.
		{ if(c < 256) return (int)mBits[(uint8_t)(unsigned)c >> 5] & (1 << (c & 31)); else return (int)(mBits[0] & 0x00000001); }

	void Set8(int c)
		{ mBits[(uint8_t)(unsigned)c >> 5] |= (1 << (c & 31)); }

	void Set16(char16_t c)
		{ if((uint16_t)c < 256) mBits[(uint8_t)(uint16_t)c >> 5] |= (1 << (c & 31)); }

	void Set32(char32_t c)
...
// ReadUint64
//
// Convert an ascii string to a long 32-bit value. The base used for the 
// calculations is supplied by the user. The base must be 0 or in the range [2,36].
// If a base of 0 is supplied, the ascii string must be examined to determine 
// the base of the number:
//    (a) First char = '0' (zero) and second char = 'x' or 'X', use base 16.
//    (b) First char = '0' (zero), use base 8
//    (c) First char in range '1' - '9', use base 10.
//
uint64_t ReadUint64_8(ReadFunction8 pReadFunction8, void* pContext,
					  uint64_t nMaxValue, int nBase, int nMaxFieldWidth, 
					  int& nReadCount, int& bNegative, int& bIntegerOverflow);

uint64_t ReadUint64_16(ReadFunction16 pReadFunction16, void* pContext,
					   uint64_t nMaxValue, int nBase, int nMaxFieldWidth, 
					   int& nReadCount, int& bNegative, int& bIntegerOverflow);

uint64_t ReadUint64_32(ReadFunction32 pReadFunction32, void* pContext,
					   uint64_t nMaxValue, int nBase, int nMaxFieldWidth, 
					   int& nReadCount, int& bNegative, int& bIntegerOverflow);
...
// ReadReadDouble
//
// Convert an ascii string to a 64 bit double value. 
//
double ReadDouble8(ReadFunction8 pReadFunction8, void* pContext,
				   int nMaxFieldWidth, int cDecimalPoint, int& nReadCount, int& bOverflow);

double ReadDouble16(ReadFunction16 pReadFunction16, void* pContext,
					int nMaxFieldWidth, int cDecimalPoint, int& nReadCount, int& bOverflow);

double ReadDouble32(ReadFunction32 pReadFunction32, void* pContext,
					int nMaxFieldWidth, int cDecimalPoint, int& nReadCount, int& bOverflow);
...
// ReadFormat8
//
const char8_t*  ReadFormat8 (const char8_t*  pFormat, ScanfLocal::FormatData* pFormatData);
const char16_t* ReadFormat16(const char16_t* pFormat, ScanfLocal::FormatData* pFormatData);
const char32_t* ReadFormat32(const char32_t* pFormat, ScanfLocal::FormatData* pFormatData);
