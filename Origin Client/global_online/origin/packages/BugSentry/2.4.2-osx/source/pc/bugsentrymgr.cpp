        ...
        */
        ...
        void BugSentryMgr::ReportCrash(void* exceptionPointers, unsigned int exceptionCode, const char* contextData, const char* categoryId)
        {
            if (mState == STATE_IDLE)
            {
                ...
                if (mConfig.mUseHeuristicStackWalk)
                {
                    Utils::GetCallstackHeuristic(exceptionPointers, &crashData.mStackAddresses[0], CrashData::MAX_STACK_ADDRESSES, &crashData.mStackAddressesCount);
                }
                else
                {
