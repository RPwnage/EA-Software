...
namespace EA
{
    namespace BugSentry
    {
        ...
        /*! BugSentryMgr::BugSentryMgr
            ...
            \param      imageInfoRev: The BugSentry ImageInfo structure for generating the screenshot.

        */
        /******************************************************************************/
        BugSentryMgr::BugSentryMgr(BugSentryConfig* config, ImageInfo* imageInfoRev) : BugSentryMgrBase(config)
        {
            // Save off image information
            if (imageInfoRev)
            {
                mImageInfoRev = *imageInfoRev;
            }
        ...
        /******************************************************************************/
        /*! BugSentryMgr::SetupFramebuffer

            \brief      Sets up variables for accessing the Wii front buffer

            \param       - imageInfo: The ImageInfo struct in CrashData
            \return      - none.
        */
        /******************************************************************************/
        void BugSentryMgr::SetupFramebuffer(ImageInfo* imageInfo)
        {
            if (imageInfo)
            {
                // ANSI copy
        ...
        */
        ...
        void BugSentryMgr::ReportCrash(void* exceptionPointers, unsigned int exceptionCode, const char* contextData, const char* categoryId)
        {
            if (mState == STATE_IDLE)
            {
                ...
                // Setup for accessing the front buffer
                SetupFramebuffer(&(mReporter.mBugData->mImageInfo));

                // Generate a callstack
                Utils::GetCallstack(exceptionPointers, &crashData.mStackAddresses[0], CrashData::MAX_STACK_ADDRESSES, &crashData.mStackAddressesCount, mConfig.mRsoBaseAddress);
