...
static int TestStreamBuffer()
{
    ...
    #if !defined(EA_PLATFORM_REVOLUTION) && !defined(EA_PLATFORM_GAMECUBE) // These platforms have standard file systems that are read-only.
        EATEST_VERIFY(bResult);
    #endif
...
static int TestStreamAdapter()
{
    ...
    typedef eastl::fixed_string<char8_t,  100, true> String8;
    typedef eastl::fixed_string<char16_t, 100, true> String16;
...
static int TestStreamStringAdapters()
{
    ...
    // Test string stream adapters (8-bit)
    {
        ...
        const char8_t*  pString1 = "Past is prologue";
        const size_t    nString1 = EA::StdC::Strlen(pString1);
        const char8_t*  pString2 = "The best way around is always through.";
        ...
        memoryStream.SetPosition(0);
        char8_t buffer[128];
        EATEST_VERIFY(ReadString(&memoryStream, (char8_t *)NULL, 0) == nString1);
...
static int TestStreamLineAdapters()
{
...

    {
        ...
        memoryStream.SetOption(MemoryStream::kOptionResizeEnabled, true);
        const char8_t*  pString1 = "When, in the course of human events, it becomes necessary";
        const char8_t*  pString2 = "for one people to dissolve the political bands which have";
        const char8_t*  pString3 = "connected them with another, and to assume,";
        const char8_t*  pString4 = "among the Powers of the earth,";
...
static int TestStreamEncoding()
{
    ...
    {   // Standalone functions
        /*
        size_t ReadEncoded(IStream* pSource, char8_t* pDestText, size_type nDestReadCapacity, EA::Text::Encoding srcEncoding, EA::Text::Encoding destEncoding);
        size_t ReadEncoded(IStream* pSource, char16_t* pDestText, size_type nDestReadCapacity, EA::Text::Encoding destEncoding);
        size_t ReadLineEncoded(IStream* pSource, char8_t* pLine, size_type nLineCapacity, EA::Text::Encoding srcEncoding, EA::Text::Encoding destEncoding);
        size_t ReadLineEncoded(IStream* pSource, char16_t* pLine, size_type nLineCapacity, EA::Text::Encoding srcEncoding);
        bool   WriteEncoded(IStream* pDestination, const char8_t* pSourceText, size_type nSourceWriteCount, EA::Text::Encoding srcEncoding, EA::Text::Encoding destEncoding);
        ...
        bool   WriteLineEncoded(IStream* pDestination, const char8_t* pSourceLine, size_type nSourceLineLength, EA::Text::Encoding srcEncoding, EA::Text::Encoding destEncoding, LineEnd lineEndToUse = kLineEndAuto);
