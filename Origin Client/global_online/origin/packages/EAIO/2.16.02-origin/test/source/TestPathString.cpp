...
#define VERIFY_STR_EQ(a, b) EATEST_VERIFY(EA::StdC::Strcmp(a, b) == 0)
...
static bool TestFindComponentFwd(const char8_t* pPath, const char8_t* pExpectedResult) 
{
    const char8_t* const pResult = EA::IO::Path::FindComponentFwd(pPath);

    return EA::StdC::Strcmp(pResult, pExpectedResult) == 0;
}


static bool TestFindComponentRvs(const char8_t* pPath, const char8_t* pExpectedResult) 
{
    const char8_t* const pResult = EA::IO::Path::FindComponentRvs(pPath);
...
int TestPathString()
{
    EA::UnitTest::Report("TestPathString\n");
...

    {
		...
		#elif defined(EA_PLATFORM_PSP2) // mix of unix and ms, supports drives but we do not support "\\" ( platform does )
			EATEST_VERIFY(TestFindComponentFwd(EA_CHAR16("app0:/abc/def"), EA_CHAR16("abc/def")));
        #else
        ...
        EATEST_VERIFY(TestFindComponentFwd("abc", ""));
            ...
            // Possibly unix-specific stuff.
		#elif defined(EA_PLATFORM_PSP2) // mix of unix and ms, supports drives but we do not support "\\" ( platform does )
			EATEST_VERIFY(TestFindComponentFwd("app0:/abc/def", "abc/def"));
        #else
...

    {
        ...
        EATEST_VERIFY(TestFindComponentRvs(EA_CHAR16("abc"), EA_CHAR16("abc")));
            ...
            // Possibly unix-specific stuff.
        #elif defined(EA_PLATFORM_PSP2) // mix of unix and ms, supports drives but we do not support "\\" ( platform does )
			EATEST_VERIFY(TestFindComponentRvs(EA_CHAR16("app0:/abc/def"), EA_CHAR16("def")));
			EATEST_VERIFY(TestFindComponentRvs(EA_CHAR16("app0:/abc"), EA_CHAR16("abc")));
			EATEST_VERIFY(TestFindComponentRvs(EA_CHAR16("app0:/"), EA_CHAR16("app0:/")));
		#else
        ...
        EATEST_VERIFY(TestFindComponentRvs("abc", "abc"));
            ...
            // Possibly unix-specific stuff.
        #elif defined(EA_PLATFORM_PSP2) // mix of unix and ms, supports drives but we do not support "\\" ( platform does )
			EATEST_VERIFY(TestFindComponentRvs("app0:/abc/def", "def"));
			EATEST_VERIFY(TestFindComponentRvs("app0:/abc", "abc"));
			EATEST_VERIFY(TestFindComponentRvs("app0:/", "app0:/"));
        #else
...

    {
        ...
        EATEST_VERIFY(TestGetStart(EA_CHAR16("drive:a/b/c"), 0,   EA_CHAR16("drive:a/b/c")));
        #if defined(EA_FILE_PATH_SEPARATOR_TYPE_UNIX) && EA_FILE_PATH_SEPARATOR_TYPE_UNIX && !defined(EA_PLATFORM_PSP2)
            // Unix (and PS3) filesystems don't support the concept of drives.
...

    {
        ...
        #if defined(EA_FILE_PATH_SEPARATOR_TYPE_UNIX) && EA_FILE_PATH_SEPARATOR_TYPE_UNIX && !defined(EA_PLATFORM_PSP2)
            // Unix (and PS3) filesystems don't support the concept of drives.
            // PSP2 is however a mix of ms(has concept of drive) and unix (has unix path separators)
    ...
    {
        #if defined(EA_PLATFORM_PSP2) // mix of unix and ms, supports drives but we do not support "\\" ( platform does )
        ...
        #elif defined(EA_FILE_PATH_SEPARATOR_TYPE_UNIX) && EA_FILE_PATH_SEPARATOR_TYPE_UNIX
            // Unix (and PS3) filesystems don't support the concept of drives.
    ...
    */
...

    {
        ...
        // PathString8& Canonicalize(PathString8& path, char8_t separator = EA_FILE_PATH_SEPARATOR_8);
    }
...

    {
        ...
        #if defined(EA_FILE_PATH_SEPARATOR_TYPE_WINDOWS) && EA_FILE_PATH_SEPARATOR_TYPE_WINDOWS && !defined(EA_PLATFORM_PSP2)
...

    {
        ...
        const char8_t* pSourceTargetResult[7][3] = 
        { 
...

    {
        ...
        struct Results2
        {
            const char8_t* pPathIn8;
            const char8_t* pPathOut8[2];    // [0] Microsoft, [1] Unix.
        };
