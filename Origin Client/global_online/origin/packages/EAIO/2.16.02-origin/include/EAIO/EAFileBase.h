...
namespace EA
{
    namespace IO
    {
        ...
        #else
            #if defined(EA_PLATFORM_PLAYSTATION2) // PS2 typedefs both off_t and ssize_t (as opposed 
                typedef signed int off_type;      // to size_t) as 64 bit, but size_t as just 32 bit.
            #elif defined(__ghs)                  // Green Hills compiler.
                typedef intptr_t off_type;
            #elif defined(__APPLE__) && (EA_PLATFORM_PTR_SIZE == 4)
        ...
        enum FileSystem
        {
            ...
            #elif EAIO_USE_UNIX_IO || defined(EA_PLATFORM_PS3)
            ...
            #elif defined(EA_PLATFORM_PLAYSTATION2) || defined(EA_PLATFORM_PLAYSTATION2_IOP) || defined(EA_PLATFORM_GAMECUBE) || defined(EA_PLATFORM_REVOLUTION)
                kFileSystemDefault = kFileSystemFAT32  /// Used to refer to whatever the default least-common demoninator file system for the platform is.
            #else
                kFileSystemDefault = kFileSystemFAT32  /// Used to refer to whatever the default least-common demoninator file system for the platform is.
            #endif
        ...
        ///
            ...
            #if (defined(EA_PLATFORM_UNIX) && !defined(EA_PLATFORM_OSX)) || defined(EA_PLATFORM_PS3) || defined(EA_PLATFORM_PSP2)
        ...
        //
            ...
            inline bool IsFilePathSeparator(int c)
                { return (c == EA_FILE_PATH_SEPARATOR_8) || (c == EA_FILE_PATH_DRIVE_SEPARATOR_8); }
        ...
        //
            ...
            const int              kMaxDirectoryLength               = 256;      // As for this writing (2/2005) the Xenon supports only 42 characters per file name.
        ...
        #elif defined(EA_PLATFORM_GAMECUBE) || defined(EA_PLATFORM_PS3) || defined(EA_PLATFORM_PSP2) || defined(EA_PLATFORM_REVOLUTION)
          #if defined(EA_PLATFORM_PS3) || defined(EA_PLATFORM_PSP2)
            const int              kMaxPathLength                    = 1024; 
          #else
            const int              kMaxPathLength                    =  256; 
          #endif
            const int              kMaxDriveLength                   =    8; // GameCube doesn't support the concept of drives. But we set this to a nominal value so that code that declares arrays of this can at least compile.
            ...
            const int              kMaxDirectoryLength               =  1024;  // Change to match EAStdC define
          #else
            const int              kMaxDirectoryLength               =  255;  // 
          #endif
            const int              kMaxFileNameLength                =  255;  // 
        ...
        ///
        typedef char8_t  FilePathBuffer8[EA::IO::kMaxPathLength];
...
namespace EA
{
    namespace IO
    {
        ...
        const char32_t kLineEndWindows32[3] = { '\r', '\n', 0 };
        ...
        #elif defined(EA_PLATFORM_GAMECUBE) || defined(EA_PLATFORM_PS3) || defined(EA_PLATFORM_REVOLUTION)
            const char             kFilePathSeparator8               = '/';
            const char*      const kFilePathSeparatorString8         = "/";
            const char             kFilePathSeparatorAlt8            = '/';
            const char*      const kFilePathSeparatorAltString8      = "/";
            const char             kFilePathDriveSeparator8          = '\0'; // GameCube doesn't support the concept of drives.
		...
		#elif defined(EA_PLATFORM_PSP2) // not quite unix, not quite ms - uses unix style for most things but includes the concept of drives "host:", "app:", etc
			const char             kFilePathSeparator8               = '/';
            const char*      const kFilePathSeparatorString8         = "/";
            const char             kFilePathSeparatorAlt8            = '/';
            const char*      const kFilePathSeparatorAltString8      = "/";
            ...
            const char16_t         kFilePathSeparatorAlt16           =           '/';
            const char16_t*  const kFilePathSeparatorAltString16     = EA_CHAR16("/");
			const char             kFilePathDriveSeparator16          = ':';
			const char*      const kFilePathDriveSeparatorString16    = ":";
            ...
            const char32_t         kFilePathSeparatorAlt32           =   '/';
            const char32_t*  const kFilePathSeparatorAltString32     = kFilePathSeparatorString32;
			const char             kFilePathDriveSeparator32          = ':';
			const char*      const kFilePathDriveSeparatorString32    = ":";
            #else
            const char16_t         kFilePathSeparator16              =   '/';
            const char16_t         kFilePathSeparatorString16[]      = { '/', 0 };
            const char16_t         kFilePathSeparatorAlt16           =   '/';
			...
			const char             kFilePathDriveSeparator32         = ':';
			const char*      const kFilePathDriveSeparatorString32   = EA_CHAR32(":");
            #endif
