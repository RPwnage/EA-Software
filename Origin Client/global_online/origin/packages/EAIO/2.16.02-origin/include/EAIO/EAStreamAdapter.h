...
namespace EA
{
    ...
    namespace IO
    {
        ...
        public:
            ...
            size_t ReadString32(char32_t* string, uint32_t capacity);
            ...
            void WriteString32(const char32_t* string, uint32_t length);
...
{
    const uint32_t stringLength = static_cast<uint32_t>(string.length());   // This will truncate size_t on 64 bit platforms.
...
{
    return WriteUint32(pIS, string.length(), endianDestination)
        && WriteUint32(pIS, (const uint32_t*)string.data(), string.length(), endianDestination);
}
