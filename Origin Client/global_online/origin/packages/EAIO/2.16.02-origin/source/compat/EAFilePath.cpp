...
//#include <Foundation/EATextEncoding.h>
#include <string.h>
#include <ctype.h>
...
static char16_t EAIOTolower16(char16_t c)
{
    if(c < 128)
...
/*
static char16_t Toupper16(char16_t c)
{
    if(c < 128)
...
static bool EAIOIsalpha16(char16_t c)
{
    if(c < 128)
...
/*
static char8_t* EAIOStrncpy8(char8_t* pDestination, const char8_t* pSource, size_t n)
{
...
static char16_t* EAIOStrncpy16(char16_t* pDestination, const char16_t* pSource, size_t n)
{
...
static char16_t* EAIOStrstr(const char16_t* pString, const char16_t* pSubString)
{
...
static int EAIOStrcmp16(const char16_t* pString1, const char16_t* pString2)
{
    char16_t c1, c2;
...
static int EAIOStricmp16(const char16_t* pString1, const char16_t* pString2)
{
    char16_t c1, c2;

    while((c1 = EAIOTolower16(*pString1++)) == (c2 = EAIOTolower16(*pString2++)))
    {
        if(c1 == 0)
            return 0;
    }
...
static char16_t* EAIOStrlwr16(char16_t* pString)
{
...
bool FilePath::RemoveLastDirectory()
{
    ...
    if (dir != dirEnd)
    {
        ...
        if (keepEnd[-1] == EA_FILE_PATH_SEPARATOR_16)
            --keepEnd;
        ...
        while(keepEnd > dir)
        {
            ...
            if (*keepEnd == EA_FILE_PATH_SEPARATOR_16)
            {
...
int FilePath::CompareSubstrings(const char16_t *pStart1, const char16_t *pEnd1, const char16_t *pStart2, const char16_t *pEnd2, FileSystem fileSystemType)
{
    ...
    else
    {
        do
        {
            c1 = EAIOTolower16(*pStart1++);
            c2 = EAIOTolower16(*pStart2++);
...
size_t FilePath::SafeSubstringCopy(const char16_t* begin, const char16_t* end, char16_t* dst, size_t capacity)
{
    ...
    {
        memcpy(dst, begin, nRequiredStrlen * sizeof(char16_t));
...
bool FilePath::SafeReplace(char16_t* replaceBegin, char16_t* replaceEnd, const char16_t* srcBegin, const char16_t* srcEnd, char16_t prefixChar, char16_t suffixChar)
{
    ...
    memmove(replaceBegin + srcLen + extraLen, replaceEnd, (tailLen+1) * sizeof(char16_t));

    if (prefixChar)
    {
        replaceBegin[0] = prefixChar;
        memcpy(replaceBegin+1, srcBegin, srcLen * sizeof(char16_t));
    }
    else
        memcpy(replaceBegin, srcBegin, srcLen * sizeof(char16_t));
...
// GetHasTrailingPathSeparator
//
EAIO_API bool GetHasTrailingPathSeparator(const char8_t* pDirectory, size_t nDirectoryLength)
{
    if(nDirectoryLength == kLengthNull)
        nDirectoryLength = EA::StdC::Strlen(pDirectory);
    return (nDirectoryLength && IsFilePathSeparator(pDirectory[nDirectoryLength - 1]));
...
// RemoveTrailingPathSeparator
//
EAIO_API bool RemoveTrailingPathSeparator(char8_t* pDirectory, size_t nDirectoryLength)
{
    if(nDirectoryLength == kLengthNull)
        nDirectoryLength = EA::StdC::Strlen(pDirectory);
...
EAIO_API void SplitPathPtrs(char8_t* pPath, char8_t*& pDirStart, char8_t*& pFileStart, char8_t*& pExtStart)
{
    // To do.
    // Users should use the PathString module instead.
    pDirStart = pFileStart = pExtStart = pPath;
}


EAIO_API void SplitPathPtrs(const char8_t* pPath, const char8_t*& pDirStart, const char8_t*& pFileStart, const char8_t*& pExtStart)
{
    // To do.
    // Users should use the PathString module instead.
    pDirStart = pFileStart = pExtStart = pPath;
...
EAIO_API void SplitPath(const char16_t* pPath, char16_t* pDrive, char16_t* pDirectory, char16_t* pFileName, char16_t* pExtension, FileSystem /*fileSystemType*/)
{
    ...
    const size_t driveLen = LOCAL_MIN((size_t)kMaxDriveLength     - 1, (size_t)(pDirStart - pPath));
    const size_t dirLen   = LOCAL_MIN((size_t)kMaxDirectoryLength - 1, (size_t)(pFileNameStart - pDirStart));
    const size_t nameLen  = LOCAL_MIN((size_t)kMaxFileNameLength  - 1, (size_t)(pExtStart - pFileNameStart));
    const size_t extLen   = LOCAL_MIN((size_t)kMaxExtensionLength - 1, EA::StdC::Strlen(pExtStart));

    if (pDrive)
    {
        EAIOStrncpy16(pDrive, pPath, driveLen);
        pDrive[driveLen] = 0;
    }

    if (pDirectory) 
    {
        EAIOStrncpy16(pDirectory, pDirStart, dirLen);
        pDirectory[dirLen] = 0;
    }

    if (pFileName)
    {
        EAIOStrncpy16(pFileName, pFileNameStart, nameLen);
        pFileName[nameLen] = 0;
    }

    if (pExtension)
    {
        EAIOStrncpy16(pExtension, pExtStart, extLen);
        pExtension[extLen] = 0;
    }
}
        ...
        while(((intptr_t)nPosition >= 0) && (nDirectoryNamesCount > (nDirectoryIndex + 1)))
        {
            ...
            if(bIsFilePathSeparator || (nPosition == 0))
            {
                if(bIsFilePathSeparator)
                {
                    ...
                    if(pDirectoryNames)
                        EAIOStrncpy16(pDirectoryNames[nDirectoryIndex], pDirectory + nPosition + 1, nCharCount);
                ...
                else
                {
                    ...
                    if(pDirectoryNames)
                        EAIOStrncpy16(pDirectoryNames[nDirectoryIndex], pDirectory, nCharCount);
...
{
    char16_t pDirectoryNames[2][kMaxPathLength] = { { 0 } };
...
EAIO_API char16_t* FullPath(char16_t* pPathResult, const char16_t* pPath, size_t /*nMaxLength*/, FileSystem fileSystemType)
{
    ...
    if(((unsigned)fileSystemType - (unsigned)kFileSystemFAT) <= (unsigned)(kFileSystemNTFS - kFileSystemFAT)) // If Windows-based
    {
        ...
        else // Else conventional drive-based path.
        {
            ...
            if(pSource[0] && pSource[1] && (pSource[1] == EA_FILE_PATH_DRIVE_SEPARATOR_16) && (((unsigned)(EAIOTolower16(pSource[0]) - 'a')) <= (unsigned)('z' - 'a')))
            {
                nDriveLetter = EAIOTolower16(pSource[0]);
            ...
            else // else path is relative. Input path is of form "..\.." or
            {
                ...
                // if (nDrive >= 0) // Note: This line has been disabled because it prevents UNC paths from working. It is not clear why this line exists, but perhaps there was a good reason.
                    if (_wgetdcwd( nDrive + 1, pBaseDirectory, kBasePathLength ) == NULL)
...
//    (Xenon)       D:\Temp\SomeFile.txt
//    (Xenon)       GAME:\Temp\SomeFile.txt
//    (PS3 / Unix)  /Temp/SomeFile.txt
//    (PS3 / Unix)  /Temp/                                  // This comment here to appease GCC regarding trailing \ chars.
//    (PS3 / Unix)  /Temp/*.txt
//    (PS3 / Unix)  /Temp\..\Temp\                          // This comment here to appease GCC regarding trailing \ chars.
//
// The following are not rooted paths:
//    (Win32)       Temp\SomeFile.txt
//    (Win32)       ..\Temp\SomeFile.txt
//    (Xenon)       Temp\SomeFile.txt
//    (Xenon)       ..\Temp\SomeFile.txt
//    (PS3 / Unix)  Temp/SomeFile.txt
//    (PS3 / Unix)  ../Temp/SomeFile.txt
//
EAIO_API bool IsPathRooted(const char16_t* pPath, FileSystem fileSystemType)
{
    if((unsigned)(fileSystemType - kFileSystemFAT) <= (unsigned)(kFileSystemNTFS - kFileSystemFAT)) // If Microsoft-derived...
    {
        ...
        if(EAIOIsalpha16(pPath[0]) && EAIOStrstr(pPath, pDrivePattern))  // Test for "C:\" or "GAME:\", for example.
            return true;
...
//    (Win32)       ..\Temp\SomeFile.txt
//    (Xenon)       Temp\SomeFile.txt
//    (Xenon)       D:\Temp\..\Temp\                    // This comment here to appease GCC regarding trailing \ chars.
//    (Xenon)       \\SomeServer\Temp\../Temp/*.txt
//    (Xenon)       ..\Temp\SomeFile.txt
//    (PS3 / Unix)  Temp/SomeFile.txt
//    (PS3 / Unix)  /Temp\..\Temp\                      // This comment here to appease GCC regarding trailing \ chars.
//    (PS3 / Unix)  ../Temp/SomeFile.txt
...
EAIO_API bool IsFullPath(const char16_t* pPath, FileSystem fileSystemType)
{
    if(((unsigned)(fileSystemType - kFileSystemFAT) <= (unsigned)(kFileSystemNTFS - kFileSystemFAT)) || // If Windows-derived...
       (fileSystemType == kFileSystemUDFX))  // For Xbox360
    {
        const char16_t pDrivePattern[3] = { ':',  '\\', 0 }; // Drives must have '\' after the colon and not '/'.

        if((EAIOIsalpha16(pPath[0]) && EAIOStrstr(pPath, pDrivePattern)) ||  // Test for "C:\" or "GAME:\", for example.
           ((pPath[0] == '\\') && (pPath[1] == '\\')))                   // Test for a UNC path...
        {
            // Look for relative path specifiers
            const char16_t pattern1[3] = { '/',  '.', 0 };    // "/." also covers "/.."
            const char16_t pattern2[3] = { '\\', '.', 0 };    // "\." also covers "\.."

            if(!EAIOStrstr(pPath, pattern1) && !EAIOStrstr(pPath, pattern2))
                return true;
        }
    }
    else
    {
        if(pPath[0] == EA_FILE_PATH_SEPARATOR_16) // If the path begins with a root directory...
        {
            const char16_t pattern[3] = { '/', '.', 0 };  // "/." also covers "/.."

            if(!EAIOStrstr(pPath, pattern))
                return true;
        }
    }
...
{
    const int kMaxLength = kMaxPathLength; // Possibly make kMaxPathLength a caller-specified value.

    if(IsPathRooted(pPath))
    {
        // Just copy the string
        if(EA::StdC::Strlen(pPath) < (kMaxLength - 1))
        {
    ...
    return FullPath(pPathResult, pPath, pBaseDirectory, fileSystemType); // OK if pBaseDirectory is NULL.
}
...
EAIO_API int ComparePaths(const char16_t* pPath1, const char16_t* pPath2, FileSystem fileSystemType)
{
    ...
    if(!IsFileSystemCaseSensitive(fileSystemType))
    {
        EAIOStrlwr16(path1Temp);
        EAIOStrlwr16(path2Temp);
    }

    return EAIOStrcmp16(path1Temp, path2Temp);
}
...
EAIO_API FileSystem GetFileSystem(const char16_t* pPath)
{
        ...
        if(GetVolumeInformationW(pDrive, pVolumeName, nVolumeNameSize, 
                                  NULL, &nMaximumComponentLength, &nFileSystemFlags, 
                                  pFileSystemType, nFileSystemTypeSize))
        {
            if(EAIOStricmp16(pVolumeName, EA_CHAR16("NTFS")) == 0)
                return kFileSystemNTFS;

            if(EAIOStricmp16(pVolumeName, EA_CHAR16("FAT32")) == 0)
        ...
        EA::StdC::Strlcpy(pDirectory8, pPath, kMaxPathLength);

        // Convert "vol:\abc\def\ghi" to "vol:\"
        char8_t* p;
        for(p = pDirectory8; *p && ((*p == ':') || !IsFilePathSeparator(*p)); ++p)
            { } // Do nothing.
        ...
        *p   = 0;
        ...
        BOOL  result = GetVolumeInformation(pDirectory8, NULL, 0, &dwSerialNumber, 
                                            &dwMaxComponentLength, &dwFileSystemFlags, pFileSystemName8, 32);
        EA_ASSERT(result);
        if(result)
        {
            if(pFileSystemName8[0] == 'F') // FATX
                return kFileSystemFATX;
            else 
                return kFileSystemUDFX;
        }
        ...
        while(*pDirectory)
        {
            if(*pSubdirectory)
            {
                ...
                else
                {
                    ...
                    {
                        if(EAIOTolower16(*pSubdirectory) != EAIOTolower16(*pDirectory))
                        {  
                            bReturnValue = false;
                            break; // We have a mismatch between the directories.
                        }
