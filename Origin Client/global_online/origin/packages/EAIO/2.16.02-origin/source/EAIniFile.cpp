...
EA::IO::StringW EAStringSprintf(const wchar_t* pFormat, ...)
{
    va_list arguments;
    va_start(arguments, pFormat);
    
    EA::IO::StringW result;

    // Solution #1:
    // We avoid this because it creates a dependency onthe user providing an EASTL string Vsnprintf:
    // result.sprintf_va_list(pFormat, arguments);

    #if (EASTDC_VERSION_N >= 10600)
        // Solution #2:
        // This is most efficiently as it appends directly to the string as it goes.
        EA::StdC::StringVcprintf(result, pFormat, arguments);
    #else
        // Solution #3:
        int requiredStrlen = EA::StdC::Vsnprintf(NULL, 0, pFormat, arguments);
        result.resize((eastl_size_t)requiredStrlen);
        EA::StdC::Vsnprintf(&result[0], (size_t)(requiredStrlen + 1), pFormat, arguments);
    #endif
...
// IniFile
//
IniFile::IniFile(const char8_t* pPath, int optionFlags, EA::Allocator::ICoreAllocator* pCoreAllocator)
{
...
// SetPath
//
bool IniFile::SetPath(const char8_t* pPath)
{
...
// ReadEntry
//
int IniFile::ReadEntry(const char8_t* pSection, const char8_t* pKey, String8& sValue)
{
    eastl::fixed_string<wchar_t, 32, true> sSection16;
    eastl::fixed_string<wchar_t, 32, true> sKey16;
    StringW sValue16;
    size_t   n;

    n = EA::StdC::Strlcpy((wchar_t*)NULL, pSection, 0);
    sSection16.resize((eastl_size_t)n);
    EA::StdC::Strlcpy(&sSection16[0], pSection, sSection16.length() + 1);

    n = EA::StdC::Strlcpy((wchar_t*)NULL, pKey, 0);
    sKey16.resize((eastl_size_t)n);
    EA::StdC::Strlcpy(&sKey16[0], pKey, sKey16.length() + 1);

    int result = ReadEntry(sSection16.c_str(), sKey16.c_str(), sValue16);

    if(result >= 0)
    {
        n = EA::StdC::Strlcpy((char8_t*)NULL, sValue16.c_str(), 0, sValue16.length());
        sValue.resize((eastl_size_t)n);
        EA::StdC::Strlcpy(&sValue[0], sValue16.c_str(), sValue.length() + 1, sValue16.length());
        result = (int)n; 
    }
...
// ReadEntryFormatted
//
int IniFile::ReadEntryFormatted(const char8_t* pSection, const char8_t* pKey, const char8_t* pValueFormat, ...)
{
    int converted = 0;
    ...
    if(mpStream && pSection && pSection[0] && pKey && pKey[0]) // If there is a place to write the value...
    {
        ...
        if(!(mpStream->GetAccessFlags() & IO::kAccessFlagWrite)) // If the stream isn't currently open for writing...
        {
            ...
            if(!File::Exists(mPath)) // If we have a file path (as opposed to a user-supplied stream) and the ini file doesn't exist (yet)...
            {
                ...
                {
                    sToWrite1 = EAStringSprintf(kSectionFormat, pSection, kTextFileNewLineString);
                    sToWrite2 = EAStringSprintf(kKeyFormat, pKey, pValue, kTextFileNewLineString);
        ...
        if(mpStream->GetState() == IO::kStateSuccess || Open(IO::kAccessFlagReadWrite))
        {
            ...
            if(it == mSectionPositionMap.end()) // If the section doesn't exist already...
            {
                ...
                {
                    wchar_t nLastCharInFile(0);
                    
                    mpStream->SetPosition(-off_type(sizeof(wchar_t)), kPositionTypeCurrent);
                    
                    if(mpStream->Read(&nLastCharInFile, sizeof(wchar_t)) == sizeof(wchar_t))
                ...
                if(mpStream->GetSize() > 0)
                {
                    if(!bFileHasNewlineAtEndAlready)
                        sToWrite1 = EAStringSprintf(kValueFormat, kTextFileNewLineString);
                }
                            ...
                            if(sValueCurrent.length())
                            {
                                if(sValueCurrent[0] != ';')  // Skip comment lines
                                {
                                    if(sValueCurrent[0] == '[') // If we run into the next header...
                                    {
                                        ...
                                        wchar_t* const pData        = (wchar_t*)pAllocator->Alloc((size_t)nBytesToSave * sizeof(wchar_t), EAIO_ALLOC_PREFIX "EAIniFile", 0);
...
// WriteEntry
//
bool IniFile::WriteEntry(const char8_t* pSection, const char8_t* pKey, const char8_t* pValue)
{
    eastl::fixed_string<wchar_t, 32, true> sSection16;
    eastl::fixed_string<wchar_t, 32, true> sKey16;
    eastl::fixed_string<wchar_t, 32, true> sValue16;
    size_t n;
    ...
    n = EA::StdC::Strlcpy((wchar_t*)NULL, pValue, 0);
    sValue16.resize((eastl_size_t)n);
    EA::StdC::Strlcpy(&sValue16[0], pValue, sValue16.length() + 1);

    return WriteEntry(sSection16.c_str(), sKey16.c_str(), sValue16.c_str());
}
...
{
    IFLocal::StackArray<wchar_t, 256> buffer;

    va_list arguments;
    va_start(arguments, pFormat);

    size_t size_needed = EA::StdC::Vsnprintf(buffer, buffer.GetSize(), pFormat, arguments);

    if(size_needed > buffer.GetSize())
    {
        buffer.Resize(size_needed);
        EA::StdC::Vsnprintf(buffer, buffer.GetSize(), pFormat, arguments);
    }
...
// WriteEntryFormatted
//
bool IniFile::WriteEntryFormatted(const char8_t* pSection, const char8_t* pKey, const char8_t* pFormat, ...)
{
    IFLocal::StackArray<char8_t, 256> buffer;
...
//
bool IniFile::GetFileLine16To16(StringW& sLine)
{
    using namespace EA::IO;
    using namespace IFLocal;
    ...
    {
        for(StringW::size_type i(0), iEnd(sLine.length()); i < iEnd; i++)
...
bool IniFile::ConvertAndWriteStream(const wchar_t* pText16, size_t count)
{
    using namespace IFLocal;
    ...
    else if(mnEncodingSrc == kEncodingUTF8) // else if the IniFile is 8 bit...
    {
        ...
        conversionBufferSize = EA::StdC::Strlcpy(conversionBuffer, pText16, conversionBufferSize, count);
