...
#include EA_ASSERT_HEADER
    ...
    #if(__SDKVERNUMERIC__ < 2008073003L)
...
//
// Apple (and Sun?) seems to be the only Unix variant we use which 
// supports mkstemps. GCC under Linux supports only mkstemp as of GCC 4.3.
//
...
namespace
{
        ...
        time_t EAIOTime()
        {
                ...
                time_t nTime = (time_t)(nBusClock64 / 147456000);
            #elif defined(EA_PLATFORM_GAMECUBE) || defined(EA_PLATFORM_REVOLUTION)
                time_t nTime = (time_t)(OSGetTime() / OS_TIMER_CLOCK);
            #elif defined(EA_PLATFORM_CTR)
                time_t nTime = (time_t)nn::os::Tick::GetSystemCurrent().ToTimeSpan().GetSeconds();
...
EAIO_API bool File::Create(const char16_t* pPath, bool bTruncate)
{
    if(!pPath || !(*pPath))
		...
		HANDLE hFile = INVALID_HANDLE_VALUE;
            ...
            hFile = CreateFileW(pPath, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, 
    ...
    #elif(defined(EA_PLATFORM_GAMECUBE) || defined(EA_PLATFORM_REVOLUTION)) && defined(DVDETH) // DVDETH is a nintendo define which you can use to enable DVD emulation over ethernet.
        ...
        if(n < (size_t)kMaxPathLength)
        {
            ...
            memset(&dvdFileInfo, 0, sizeof(dvdFileInfo));
            const BOOL bResult = DVDCreate(path8, &dvdFileInfo); // The DVDFileInfo parameter's usage is not documented.
...
EAIO_API bool File::Create(const char8_t* pPath, bool bTruncate)
{
        ...
        if(pPath && pPath[0])
        {
            ...
            // entity but in write mode with no ability for us to read the file.
                ...
                // The GCC version of Airplay's open function truncates long paths
                // and succeeds here. This causes a failure in the unit tests.
                // I think enforcing a consistent path length across all platforms
                // makes more sense, so we'll do this check ourselves.
                if(strlen(pPath) >= (size_t)kMaxPathLength)
                    return false;
            #endif
...
EAIO_API bool File::Exists(const char16_t* pPath)
{
    if(pPath && *pPath)
    {
			    ...
			    if (GetFileAttributesExW(pPath, GetFileExInfoStandard, &fileAttr) == FALSE)
				    return false;
            ...
            if(n < (size_t)kMaxPathLength) // If the string could be fully copied...
            {
                const int32_t result = DVDConvertPathToEntrynum(path8);
            ...
            if(n < (size_t)kMaxPathLength) // If the string could be fully copied... (Question: what's the max path lenght of appbundle file paths?
            {
                ...
                {
                    AutoJNIEnv autoEnv;
                    bool result = AssetManagerJNI::Exists(autoEnv, path8 + APPBUNDLE_PATH_PREFIX_LENGTH);
...
EAIO_API bool File::Exists(const char8_t* pPath)
{
            ...
            if(pPath && pPath[0])
            {
                    ...
                    {
                        AutoJNIEnv autoEnv;
                        bool result = AssetManagerJNI::Exists(autoEnv, pPath + APPBUNDLE_PATH_PREFIX_LENGTH);
...
EAIO_API bool File::Remove(const char16_t* pPath)
{
    if(!pPath || !(*pPath))
        ...
        DVDFileInfo dvdFileInfo;
        memset(&dvdFileInfo, 0, sizeof(dvdFileInfo));
        const BOOL bResult = DVDRemove(path8, &dvdFileInfo); // The DVDFileInfo parameter's usage is not documented.
...
{
    #if defined(EA_PLATFORM_WINDOWS) // For Windows we need to use wchar_t paths.
...
{
    #if (EAIO_USE_UNIX_IO && defined(EA_HAVE_SYS_STAT_H)) || defined(EA_PLATFORM_PS3) || defined(EA_PLATFORM_XENON)
...
EAIO_API size_type File::GetSize(const char16_t* pPath)
{
    size_type nReturnValue = kSizeTypeError;
            ...
            #else
		        const HANDLE hFindFile = FindFirstFileExW(pPath, FindExInfoBasic, &win32FindDataW, FindExSearchNameMatch, NULL, 0);
            #endif
        ...
        if(path8[0] && DVDOpen(path8, &dvdFileInfo)) // Wii panics if path is empty.
        {
            nReturnValue = (size_type)dvdFileInfo.length;
            DVDClose(&dvdFileInfo);
        }
...
EAIO_API bool File::SetTime(const char16_t* pPath, int nFileTimeTypeFlags, time_t nTime)
{
    ...
    #if defined(EA_PLATFORM_WINDOWS) || (EAIO_USE_UNIX_IO && EAIO_UTIME_PRESENT) || defined(EA_PLATFORM_XENON) || defined(EA_PLATFORM_PS3)
...
EAIO_API bool InitializeFileSystem(bool bInitialize)
{
        ...
        if(bInitialize)
        {
            ...
            return ((result == 0) && IsVolumeAvailable("/app_home/"));
        }
        else
        {
...
EAIO_API bool MakeTempPathName(char16_t* pPath, const char16_t* pDirectory, const char16_t* pFileName, const char16_t* pExtension, uint32_t nDestPathCapacity)
{
        ...
        const char16_t XXXXX[] = { 'X', 'X', 'X', 'X', 'X', 0};
        ...
        nLength +=        EA::StdC::Strlcpy(path8 + nLength, XXXXX,              kMaxPathLength - nLength, EA::StdC::Strlen(XXXXX));
...
EAIO_API bool MakeTempPathName(char8_t* pPath, const char8_t* pDirectory, const char8_t* pFileName, const char8_t* pExtension, uint32_t nDestPathCapacity)
{
        ...
        if(snprintf(pPath, nDestPathCapacity, "%s%sXXXXX%s", pDirectory, pFileName, pExtension) >= (int)nDestPathCapacity)
            return false;
    ...
    if(gpTempDirectory[0])
    {
        ...
        return (int)(unsigned)EA::StdC::Strlcpy(pDirectory, gpTempDirectory, nPathCapacity);
            ...
            if(dwResult) // If succeeded...
            {
                ...
                return (int)(unsigned)EA::StdC::Strlcpy(pDirectory, pPath16, kMaxDirectoryLength);
            }
            ...
            uint32_t nLength = (uint32_t)EA::StdC::Strlcpy(pDirectory, tempfolder->Data(), nPathCapacity);
            nLength = (uint32_t)EA::StdC::Strlcat(pDirectory, "\\", nPathCapacity); // must have trailing slash
        ...
        // It is recommended that the user use SetTempDirectory on PS3.
        const size_t nLength = Directory::GetCurrentWorkingDirectory(pDirectory, nPathCapacity);
        ...
        size_t nLength = strlen(AssetManagerJNI::GetTempRoot());
        ...
        bool addedSeperator = EA::IO::Path::EnsureTrailingSeparator(pDirectory,nPathCapacity);
        const size_t nLength = EAIOStrlen8(pDirectory);
        EAIOStrlcpy8(pDirectory + nLength, "tmp/", nPathCapacity - nLength);  // We don't have a strlcat to use, so we use strlcpy.
        Directory::EnsureExists(pDirectory);
...
EAIO_API uint64_t GetDriveFreeSpace(const char16_t* pPath)
{
        ...
        *p0   = 0;
        ...
        // Marmalade (formerly known as Airplay) doesn't support this.
...
EAIO_API int GetDriveName(const char16_t* pPath, char16_t* pName)
{
    EA_ASSERT(*pPath); // Assert that the input is non-empty.
        ...
        *p   = 0;
    ...
    #elif defined(EA_PLATFORM_PS3) || EAIO_USE_UNIX_IO
        ...
        // This might work on Unix and Wii systems.
        EA_UNUSED(pPath);
        pName[0] = 0;
        return -1;
    #endif
...
EAIO_API int GetDriveSerialNumber(const char16_t* pPath, char16_t* pSerialNumber)
{
        ...
        if(GetVolumePathNameW((const WCHAR*)pPath, root, kMaxPathLength))
        {
            ...
            {
                wsprintfW(pSerialNumber, EA_CHAR16("%04X-%04X"), dwSerialNumber >> 16, dwSerialNumber & 0xffff);
                return 10;
            }
        }
...
EAIO_API DriveType GetDriveTypeValue(const char16_t* pPath)
{
    ...
    #elif defined(EA_PLATFORM_REVOLUTION) || defined(EA_PLATFORM_GAMECUBE)
        return kDriveTypeDVD; // Consider that if we supported Nand then we kDriveTypeFixed, and if we supported Hio2 then we have kDriveTypeRemote.
...
EAIO_API size_t GetDriveInfo(DriveInfo16* pDriveInfoArray, size_t nDriveInfoArrayCapacity)
{
        ...
        char16_t     pDriveStringArray[kArraySize];
        ...
        char16_t* pCurrent = pDriveStringArray;
        char16_t* pEnd     = pDriveStringArray + nStringLength;
        ...
        while(pCurrent != pEnd) // Walk through (e.g.) "c:\<nul>d:\<nul>e:\<nul><nul>"
        {
            if(*pCurrent) // If we have a non-null character (and thus the beginning of a new drive)...
            {
                ...
                if(GetVolumeInformationW((LPCWSTR)pCurrent, NULL, 0, NULL, NULL, &fsFlags, NULL, 0))
                {
                    nCount++;
                    ...
                    if(pDriveInfoArray && (nCount <= nDriveInfoArrayCapacity))  // If there is more capacity in the user-provided array...
                    {
                        ...
                        pDriveInfoArray->mName.assign(pCurrent);    // pCurrent => "c:\<nul>d:\<nul>e:\<nul><nul>", mName => "c:\"

                        // Find out the drive type
                        const uint32_t nDriveType = GetDriveTypeW(pDriveInfoArray->mName.c_str());
...
EAIO_API bool Directory::Exists(const char16_t* pDirectory)
{
    if(pDirectory && *pDirectory)
    {
			    ...
			    WIN32_FILE_ATTRIBUTE_DATA fileAttr;
			    if (GetFileAttributesExW(pDirectory, GetFileExInfoStandard, &fileAttr) == FALSE)
			    {
				    return false;
			    }
			    return ((fileAttr.dwFileAttributes != INVALID_FILE_ATTRIBUTES) && ((fileAttr.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0));
            ...
            if(n < (size_t)kMaxPathLength) // If the string could be fully copied...
            {
                const int32_t result = DVDConvertPathToEntrynum(directory8);
            ...
            if(n < (size_t)kMaxPathLength) // If the string could be fully copied... (Question: what's the max path length of appbundle file paths?
            {
                if(strstr(directory8, APPBUNDLE_PATH_PREFIX) == directory8) // If the path begins with the APPBUNDLE_PATH_PREFIX prefix..
                {
                    ...
                    Path::PathStringW            directoryW;
                        ...
                        directoryW = pDirectory;                                                 // Make a copy. e.g. "/abc/def"
                    #else
            ...
            if (n < (size_t)kMaxPathLength)
            {
                ...
                strcpy(path, directory8);
                int length = strlen(directory8);
...
EAIO_API bool Directory::Exists(const char8_t* pDirectory)
{
            ...
            if(pDirectory && *pDirectory)
            {
                ...
                #if defined(EA_PLATFORM_ANDROID)
                    if(strstr(pDirectory, APPBUNDLE_PATH_PREFIX) == pDirectory) // If the path begins with the APPBUNDLE_PATH_PREFIX prefix..
                    {
...
namespace
{
    bool CreateDirectoryInternal(const char16_t* pDirectory)
    {
        ...
        #elif EAIO_USE_UNIX_IO || defined(EA_PLATFORM_XENON)  || defined(EA_PLATFORM_PS3)
...
EAIO_API bool Directory::Remove(const char32_t* pDirectory, bool bAllowRecursiveRemoval)
{
    Path::PathString8 path8;
    ConvertPath(path8, pDirectory);
    return Directory::Remove(path8.c_str(), bAllowRecursiveRemoval);
}
...
EAIO_API bool Directory::Rename(const char16_t* pDirectoryOld, const char16_t* pDirectoryNew)
{
        ...
        //return (cellFsRename(const char *from, const char *to) == CELL_FS_SUCCEEDED);
...
EAIO_API int Directory::GetCurrentWorkingDirectory(char16_t* pDirectory, uint32_t nPathCapacity)
{
    ...
    // #elif defined(EA_PLATFORM_GAMECUBE) || defined(EA_PLATFORM_REVOLUTION)
        ...
        // The system API has no way of telling you what it thinks the
        // current directory is. So we let our default code below do it.
        ...
        if(!gbCurrentWorkingDirectoryInitialized) // If we haven't set the default value of the current working directory...
        {
            ...
            #elif defined(EA_PLATFORM_PS3)
                EA::StdC::Strlcpy(gpCurrentWorkingDirectory, EA_CHAR16("/app_home/"), kTempDirectoryLength);
...
EAIO_API int Directory::GetCurrentWorkingDirectory(char8_t* pDirectory, uint32_t nPathCapacity)
{
        ...
        {
            Path::EnsureTrailingSeparator(pDirectory, nPathCapacity);
            return (int)strlen(pDirectory); // No StdC
        }
...
EAIO_API bool Directory::SetCurrentWorkingDirectory(const char16_t* pDirectory)
{
        ...
        if((nDirectoryLength + 2) < (sizeof(gpCurrentWorkingDirectory) / sizeof(gpCurrentWorkingDirectory[0])))
        {
            ...
            if(pDirectory[0])
            {
                ...
                // Echo the change to the DVD system.
                char8_t directory8[kMaxPathLength];
                EA::StdC::Strlcpy(directory8, pDirectory, kMaxPathLength);

                DVDChangeDir(directory8);
            #endif
    ...
    pDirectory[0] = 0;
        ...
        switch(specialDirectory)
        {
            case kSpecialDirectoryTemp:
                success = (GetTempPathW(nPathCapacity, pDirectory) != 0);
                if(success)
                    Path::EnsureTrailingSeparator(pDirectory, nPathCapacity);
                break;

            case kSpecialDirectoryOperatingSystem:
                success = (GetWindowsDirectoryW(pDirectory, nPathCapacity) != 0);
                if(success)
                    Path::EnsureTrailingSeparator(pDirectory, nPathCapacity);
                break;
            ...
            case kSpecialDirectoryCurrentApplication:
            {
                ...
                if(GetModuleFileNameW(GetModuleHandle(NULL), filename, MAX_PATH))
                {
                    ...
                    // GetFullPathNameW memsets pDirectory with nPathCapacity chars.
                    if(GetFullPathNameW(filename, nPathCapacity, pDirectory, &pFilePart))
                    {
                        *pFilePart = 0;
                        success = true;
                    }
            ...
            default:
            {
                ...
                if(success)
                {
                    const size_t nLength = wcslen(pDirectory);
        ...
        switch (specialDirectory)
        {
            ...
            case kSpecialDirectoryTemp:
                if(EA::IO::Path::GetHasTrailingSeparator(tmp, EA::StdC::Strlen(tmp)))
                    ...
                    return snprintf(pDirectory, nPathCapacity, "%s/", tmp);
            ...
            case kSpecialDirectoryOperatingSystemTrash:
                ...
                return snprintf(pDirectory, nPathCapacity, "%s/.Trash/", home);
            ...
            case kSpecialDirectoryUserDesktop:
                return snprintf(pDirectory, nPathCapacity, "%s/Desktop/", home);

            case kSpecialDirectoryCommonDesktop:
                return snprintf(pDirectory, nPathCapacity, "%s/Desktop/", shared);

            case kSpecialDirectoryUserApplicationData:
                return snprintf(pDirectory, nPathCapacity, "%s/Library/", home);

            case kSpecialDirectoryCommonApplicationData:
                return snprintf(pDirectory, nPathCapacity, "%s/Library/", shared);

            case kSpecialDirectoryUserDocuments:
                return snprintf(pDirectory, nPathCapacity, "%s/Documents/", home);

            case kSpecialDirectoryCommonDocuments:
                return snprintf(pDirectory, nPathCapacity, "%s/Documents/", shared);

            case kSpecialDirectoryUserMusic:
                return snprintf(pDirectory, nPathCapacity, "%s/Music/", home);

            case kSpecialDirectoryCommonMusic:
                return snprintf(pDirectory, nPathCapacity, "%s/Music/", shared);

            case kSpecialDirectoryUserProgramsShortcuts:    // shortcuts is a windows-ism, assume they mean the apps directory
                return snprintf(pDirectory, nPathCapacity, "%s/Applications/", home);
