...
template<typename T>
static bool EnsureTrailingSeparatorImpl(T *pDirName, size_t nMaxPermittedLength)
{
    // To consider: should we add to an empty path?
    const size_t n = EA::StdC::Strlen(pDirName);

    if (!EA::IO::Path::GetHasTrailingSeparator(pDirName, (uint32_t)n))
    {
        if (n + 2 <= nMaxPermittedLength)
        {
            pDirName[n] = PathCharTraits<T>::PathSeparator;
            pDirName[n + 1] = 0;
...
{
    return EnsureTrailingSeparatorImpl(pDirName, nMaxPermittedLength);
}
...
EAIO_API bool EnsureTrailingSeparator(char32_t* pDirName, size_t nMaxPermittedLength)
{
    return EnsureTrailingSeparatorImpl(pDirName, nMaxPermittedLength);
}
...
EAIO_API void StripTrailingSeparator(char32_t* pPath, size_t nLength)
{
    ...
    EAIO_API bool EnsureTrailingSeparator(wchar_t* pDirName, size_t nMaxPermittedLength)
    {
        #if (EA_WCHAR_SIZE == 2)
            return EnsureTrailingSeparator(reinterpret_cast<char16_t *>(pDirName), nMaxPermittedLength);
        #else
            return EnsureTrailingSeparator(reinterpret_cast<char32_t *>(pDirName), nMaxPermittedLength);
        #endif
    }
