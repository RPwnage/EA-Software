...
namespace EA
{
	...
	namespace IO
	{
		...
		public:
			...
			/// A newly constructed instance has a reference count of 0.
			int AddRef();

			/// Reference count decrement. Returns new reference count.
			/// If the reference count becomes <= 0, this class calls 'delete this', which 
			/// goes through the allocator set by SetReleaseAllocator if specified.
			int Release();
			...
			void SetReleaseAllocator(Allocator* pReleaseAllocator);
			Allocator* GetReleaseAllocator() const;
...
inline void EA::IO::MemoryStream::SetReleaseAllocator(Allocator* pReleaseAllocator)
{
	mpReleaseAllocator = pReleaseAllocator;
}

inline EA::IO::MemoryStream::Allocator* EA::IO::MemoryStream::GetReleaseAllocator() const
{
