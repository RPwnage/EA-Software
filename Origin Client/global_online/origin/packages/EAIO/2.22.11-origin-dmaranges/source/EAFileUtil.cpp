...
#include EA_ASSERT_HEADER
	...
	#include <utime.h>        // Some versions may require <sys/utime.h>
		...
		namespace EA { namespace IO {
			int appleGetTempDirectory(char8_t*  pDirectory, uint32_t nPathCapacity);
			...
			int appleGetAppName(char8_t*  pAppName, uint32_t nPathCapacity);
...
namespace
{
		...
		time_t EAIOTime()
		{
				...
				time_t nTime = (time_t)(nTimeBase / sys_time_get_timebase_frequency());
			#elif defined(EA_PLATFORM_XENON)
				time_t nTime = (time_t)__mftb();
			#elif defined(EA_PLATFORM_PSP2)
				time_t nTime = (time_t)(sceKernelGetProcessTimeWide() / 1000000);
			#else
...
//
	...
	char8_t gGameDataTitle[CELL_GAME_SYSP_TITLE_SIZE];      // e.g. "Game X"
	char8_t gGameDataTitleId[CELL_GAME_SYSP_TITLEID_SIZE];  // e.g. "GameX-28931"
	char8_t gGameDataVersion[CELL_GAME_SYSP_VERSION_SIZE];  // e.g. "1.0"
	char8_t gGameDataDirName[CELL_GAME_DIRNAME_SIZE];       // e.g. "GameX-28931Data"
	char8_t gGameDataDirectory[CELL_GAME_PATH_MAX + 1];     // e.g. "/dev_hdd0/game/<TITLE_ID>". This is a cached string for the GetPS3GameDataDirectory function below.


	/// SetPS3GameDataNames
	///
	/// Specifies application-specific strings needed by GameData functionality.
	///
	/// The name used for pDirName must be different between Disc-based games and
	...
	/// 
	/// Example usage:
	///     SetPS3GameDataNames("Game X", "GameX-28931", "1.0", "GameX-28931Data");
	///
	EAIO_API void SetPS3GameDataNames(const char8_t* pTitle, const char8_t* pTitleId, const char8_t* pVersion, const char8_t* pDirName)
	{
		EA::StdC::Strlcpy(gGameDataTitle,   pTitle,   EAArrayCount(gGameDataTitle));
		EA::StdC::Strlcpy(gGameDataTitleId, pTitleId, EAArrayCount(gGameDataTitleId));
		EA::StdC::Strlcpy(gGameDataVersion, pVersion, EAArrayCount(gGameDataVersion));
		EA::StdC::Strlcpy(gGameDataDirName, pDirName, EAArrayCount(gGameDataDirName));
	}

	EAIO_API void GetPS3GameDataNames(char8_t* pTitle, char8_t* pTitleId, char8_t* pVersion, char8_t* pDirName)
	{
		EA::StdC::Strlcpy(pTitle,   gGameDataTitle,   CELL_GAME_SYSP_TITLE_SIZE);
		EA::StdC::Strlcpy(pTitleId, gGameDataTitleId, CELL_GAME_SYSP_TITLEID_SIZE);
		EA::StdC::Strlcpy(pVersion, gGameDataVersion, CELL_GAME_SYSP_VERSION_SIZE);
		EA::StdC::Strlcpy(pDirName, gGameDataDirName, CELL_GAME_PATH_MAX);
	}


	EAIO_API size_t GetPS3GameDataDirectory(char8_t* pDirectory)
	{
		if(gGameDataDirectory[0]) // If it was already read...
		{
			EA_COMPILETIME_ASSERT(EAArrayCount(gGameDataDirectory) <= (size_t)kMaxPathLength); // This assertion guarantees that we can use pDirectory below as we do.
			return EA::StdC::Strlcpy(pDirectory, gGameDataDirectory, kMaxPathLength);
		}

		size_t nStrlen = 0;
		pDirectory[0] = 0;

		if(gGameDataTitle[0] && gGameDataTitleId[0]) // Need to check these here because cellGameCreateGameData below doesn't do so itself, yet needs to.
		{
			...
			if(result == CELL_OK)
			{
				...
				result = cellGameDataCheck(CELL_GAME_GAMETYPE_GAMEDATA, gGameDataDirName, &size);

				if(result == CELL_GAME_RET_NONE) // If the gameData doesn't already exist...
				{
					CellGameSetInitParams params;

					memset(&params, 0, sizeof(params));
					EA::StdC::Strcpy(params.title,   gGameDataTitle);
					EA::StdC::Strcpy(params.titleId, gGameDataTitleId);
					EA::StdC::Strcpy(params.version, gGameDataVersion); 

					EA_COMPILETIME_ASSERT(CELL_GAME_PATH_MAX < kMaxPathLength); // This assertion guarantees that we can use pDirectory below as we do.
					result = cellGameCreateGameData(&params, contentInfoPath, gGameDataDirectory); // We don't use the resulting gGameDataDirectory, as it's not a final value. 
				}

				if(result == CELL_GAME_RET_OK)
				{
					EA_COMPILETIME_ASSERT(CELL_GAME_PATH_MAX < kMaxPathLength); // This assertion guarantees that we can use pDirectory below as we do.
					result = cellGameContentPermit(contentInfoPath, gGameDataDirectory);

					if(result == CELL_GAME_RET_OK)
					{
						nStrlen = EA::StdC::Strlen(gGameDataDirectory);

						if((nStrlen > 0) && (gGameDataDirectory[nStrlen - 1] != '/')) // Append '/' if not already.
						{
							gGameDataDirectory[nStrlen++] = '/';
							gGameDataDirectory[nStrlen]   = '\0';
						}

						EA::StdC::Strlcpy(pDirectory, gGameDataDirectory, kMaxPathLength);
					}
				}
				else
				{
					// We must *always* followup a call to cellGameDataCheck with a call to cellGameContentPermit.
					cellGameContentPermit(contentInfoPath, gGameDataDirectory);
				}
			}
		}
...
//
	...
	EAIO_API int PS4GetTemporaryDataMount(char8_t* pDirectory, size_t nDirectoryPathCapacity, bool registerAsEAIOTempDirectory)
	{
	...
	if(!pPath || !(*pPath))
		...
		HANDLE hFile = INVALID_HANDLE_VALUE;
			...
			if(n < (size_t)kMaxPathLength)
			{
				...
				// for the existence of the file befoconst HANDLE re trying to create it.
				hFile = ::CreateFileA(path8, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, 
					NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
			}

		#elif EA_WINAPI_FAMILY_PARTITION(EA_WINAPI_PARTITION_DESKTOP) || (EA_WINAPI_FAMILY_PARTITION(EA_WINAPI_FAMILY_TV_TITLE) && _MSC_VER >= 1900)
		...
		EA_UNUSED(bTruncate); // To do: Implement bTruncate.
		...
		if(n < (size_t)kMaxPathLength)
		{
			...
			// In this case we will probably want to change this code to try to test
			// for the existence of the file before trying to create it.
			CellFsErrno result = cellFsOpen(path8, CELL_FS_O_RDWR | CELL_FS_O_CREAT, &fd, NULL, 0);
			...
			if(result == CELL_FS_SUCCEEDED)
			{
				...
				// Note that as of this writing (12/2005, Sony SDK v050), that unless we    
				// do this seek and write, the Sony file system throws our newly created 
				// file away (!). This will be reported to Sony, needless to say.
				result = cellFsLseek(fd, 0, CELL_FS_SEEK_SET, &pos);
				result = cellFsWrite(fd, &b, 1, &pos);
		...
		char8_t path8[kMaxPathLength];
		size_t n = StrlcpyUTF16ToUTF8(path8, kMaxPathLength, pPath);

		if(n < (size_t)kMaxPathLength)
		{
			SceUID fileHandle = sceIoOpen(path8, SCE_O_WRONLY | SCE_O_CREAT | (bTruncate ? SCE_O_TRUNC : 0), SCE_STM_RWU);
...
EAIO_API bool File::Exists(const char16_t* pPath)
{
	if(pPath && *pPath)
	{
			...
			{
				CellFsStat fsStat;
				const CellFsErrno result = cellFsStat(path8, &fsStat);

				if(result == CELL_FS_SUCCEEDED)
					return (fsStat.st_mode & CELL_FS_S_IFREG) != 0;
			}
			...
			if(n < (size_t)kMaxPathLength) // If the string could be fully copied...
			{
				...
				int result = sceIoGetstat(path8, &stat);
...
EAIO_API bool File::PatternExists(const char8_t* pPathPattern)
{
	...
	#if(EA_WCHAR_SIZE == 1) // if wchar_t == char8_t
		return PatternExistsW(pPathPattern);
	#else
		EA::IO::Path::PathStringW patternW;
		ConvertPath(patternW, pPathPattern);
...
EAIO_API bool File::Remove(const char16_t* pPath)
{
	if(!pPath || !(*pPath))
		...
		const CellFsErrno result = cellFsUnlink(path8);

		return (result == CELL_FS_SUCCEEDED);
...
EAIO_API bool File::Move(const char16_t* pPathSource, const char16_t* pPathDestination, bool bOverwriteIfPresent)
{
	...
	if(!pPathSource || !(*pPathSource) || !pPathDestination || !(*pPathDestination))
		...
		if(bOverwriteIfPresent || !Exists(pPathDestination))
		{
			// It is not clear from the documentation if the cellFsRename function 
			// can do a move between directories, but it seems likely that it cannot
			// do so. So we do a file copy if the destination directory differs from
			...
			if(bSameDirectory)
			{
				...
				const CellFsErrno result = cellFsRename(srcPath8, dstPath8);
				return (result == CELL_FS_SUCCEEDED);
			}
		...
		char8_t srcPath8[kMaxPathLength];
		size_t n = StrlcpyUTF16ToUTF8(srcPath8, kMaxPathLength, pPathSource);

		char8_t destPath8[kMaxPathLength];
		size_t m = StrlcpyUTF16ToUTF8(destPath8, kMaxPathLength, pPathDestination);

		if (n < (size_t)kMaxPathLength && m < (size_t)kMaxPathLength)
		{
			if (!bOverwriteIfPresent && Exists(destPath8))
			{
				return false;
			}

			int renameResult = sceIoRename(srcPath8,destPath8);

			if (renameResult == SCE_OK)
			{
				return true;
			}
			...
			// If we failed, it might be because we tried to move between two different filesystems.
			// In which case we must manually copy.
			if(File::Copy(srcPath8, destPath8, bOverwriteIfPresent))
			{
				File::Remove(srcPath8);
				return true;
			}
...
EAIO_API bool File::Stat(const char16_t* pPath, StatData& statData)
{
	// For Windows and some other supporting Microsoft platforms we need to use wchar_t paths.
		...
		#if EA_WINAPI_FAMILY_PARTITION(EA_WINAPI_PARTITION_DESKTOP) || (EA_WINAPI_FAMILY_PARTITION(EA_WINAPI_FAMILY_TV_TITLE) && _MSC_VER >= 1900)
...
EAIO_API bool File::Stat(const char8_t* pPath, StatData& statData)
{
	#if defined(EA_PLATFORM_XENON)
		WIN32_FIND_DATAA win32FindData;
		HANDLE           hFindFile = FindFirstFileA(pPath, &win32FindData);
		...
		return false;
	...
	#elif (EAIO_USE_UNIX_IO && defined(EA_HAVE_SYS_STAT_H)) || defined(EA_PLATFORM_KETTLE) || defined(EA_PLATFORM_PS3)
		#if defined(EA_PLATFORM_KETTLE)
			...
			CellFsStat tempStat;
			const CellFsErrno result = cellFsStat(pPath, &tempStat);
			EA_COMPILETIME_ASSERT(CELL_FS_SUCCEEDED == 0); // The code below depends on this.
	...
	#elif defined(EA_PLATFORM_MICROSOFT) && !defined(EA_PLATFORM_XENON)
...
EAIO_API size_type File::GetSize(const char16_t* pPath)
{
	size_type nReturnValue = kSizeTypeError;
		...
		if(n < (size_t)kMaxPathLength)
		{
			SceIoStat stat;
			memset(&stat,0,sizeof(SceIoStat));
			const int result = sceIoGetstat(path8, &stat);

			if(result == SCE_OK)
			{
				return (size_type) stat.st_size;
			}
...
EAIO_API int File::GetAttributes(const char16_t* pPath)
{
	int nAttributes = 0;
		...
		// The PS3 doesn't help you much in the way of file attributes, 
		// but there is enough for us to do some basic testing. If we can tell
		...
		// a local HD is usually (always?) writable. cellFsMountInformation can 
		// in fact give us some of this information.
		...
		else
		{
			...
			// However, this doesn't work if the file is exclusively opened by another entity.

			char8_t path8[kMaxPathLength];
			EA::StdC::Strlcpy(path8, pPath, kMaxPathLength);

			int nFileHandle;
			CellFsErrno result = cellFsOpen(path8, CELL_FS_O_RDWR, &nFileHandle, NULL, 0);
			...
			{
				result = cellFsOpen(path8, CELL_FS_O_RDONLY, &nFileHandle, NULL, 0);
				...
				{
					result = cellFsOpen(path8, CELL_FS_O_WRONLY, &nFileHandle, NULL, 0);
			...
			if(result == CELL_FS_SUCCEEDED) // If the file was left open by any of the above calls...
		...
		if(n < (size_t)kMaxPathLength)
		{
			...
			if (result == SCE_OK)
			{
				if (stat.st_mode & SCE_FREAD)
					nAttributes |= kAttributeReadable;

				if (stat.st_mode & SCE_FWRITE)
					nAttributes |= kAttributeWritable;
...
EAIO_API bool File::SetAttributes(const char16_t* pPath, int nAttributeMask, bool bEnable)
{
	if(!pPath || !pPath[0])
		...
		if(n < (size_t)kMaxPathLength)
		{
			...
			const int getStatResult = sceIoGetstat(path8, &stat);

			if (getStatResult == SCE_OK)
			{
				SceIoMode newMode = 0;

				if (nAttributeMask & kAttributeReadable)
				{
					stat.st_mode |= SCE_FREAD;
				}
				if (nAttributeMask & kAttributeWritable)
				{
					stat.st_mode |= SCE_FWRITE;
				}
				...
				const int result = sceIoChstat(path8,&stat,SCE_CST_MODE);
...
EAIO_API time_t File::GetTime(const char16_t* pPath, FileTimeType timeType)
{
		...
		HANDLE hFindFile = INVALID_HANDLE_VALUE;
			...
			char8_t path8[kMaxPathLength];
			int requiredStrlen = EA::StdC::Strlcpy(path8, pPath, kMaxPathLength);

			if(requiredStrlen < EAArrayCount(path8))
				hFindFile = FindFirstFileA(path8, &win32FindData);
		...
		{
			if(timeType == kFileTimeTypeCreation)
				return fsStat.st_ctime;
			else if(timeType == kFileTimeTypeLastModification)
				return fsStat.st_mtime;
			else if(timeType == kFileTimeTypeLastAccess)
				return fsStat.st_atime;
		}
		...
		if (n < (size_t)kMaxPathLength)
		{
			...
			if(result == SCE_OK)
			{
				SceDateTime targetTime;

				if(timeType == kFileTimeTypeCreation)
				{
					targetTime = stat.st_ctime;
				}
				else if(timeType == kFileTimeTypeLastModification)
				{
					targetTime = stat.st_mtime;
				}
				else if(timeType == kFileTimeTypeLastAccess)
				{
					targetTime = stat.st_atime;
				}

				tm posixTime;
				memset(&posixTime,0,sizeof(tm));

				//We need to adjust the months to range from 0 - 11
				//and years must be the number of years since 1900
				posixTime.tm_year = targetTime.year - 1900;
				posixTime.tm_mon = targetTime.month - 1;
				posixTime.tm_mday = targetTime.day;
				posixTime.tm_hour = targetTime.hour;
				posixTime.tm_min = targetTime.minute;
				posixTime.tm_sec = targetTime.second;
...
EAIO_API bool File::SetTime(const char16_t* pPath, int nFileTimeTypeFlags, time_t nTime)
{
	using namespace EA::IO::Path;
	...
	#if defined(EA_PLATFORM_MICROSOFT) || (EAIO_USE_UNIX_IO && EAIO_UTIME_PRESENT) || defined(EA_PLATFORM_PS3)

		#if defined(EA_PLATFORM_MICROSOFT)
			_utimbuf timbuf;
		#elif defined(EA_PLATFORM_PS3)
			CellFsUtimbuf timbuf;
		#elif EAIO_USE_UNIX_IO
			 utimbuf timbuf;
		#endif
		...
		#elif defined(EA_PLATFORM_PS3)
			return (cellFsUtime(path8.c_str(), &timbuf) == CELL_FS_SUCCEEDED);
		#elif EAIO_USE_UNIX_IO
		...
		if (n < (size_t)kMaxPathLength)
		{
			...
			// warning, not re-entrant (no _r version exists for the psp2sdk as of 9.25)
			posixTime = *(localtime(&nTime));

			SceDateTime dateTime;

			memset(&dateTime,0,sizeof(SceDateTime));

			dateTime.year = posixTime.tm_year;
			dateTime.month = posixTime.tm_mon;
			dateTime.day = posixTime.tm_mday;
			dateTime.hour = posixTime.tm_hour;
			dateTime.minute = posixTime.tm_min;
			dateTime.second = posixTime.tm_sec;

			if(nFileTimeTypeFlags | kFileTimeTypeCreation)
			{
				stat.st_ctime = dateTime;
				flags |= SCE_CST_CT;
			}
			
			if(nFileTimeTypeFlags == kFileTimeTypeLastModification)
			{
				stat.st_mtime = dateTime;
				flags |= SCE_CST_AT;
			}
			
			if(nFileTimeTypeFlags == kFileTimeTypeLastAccess)
			{
				stat.st_atime = dateTime;
				flags |= SCE_CST_MT;
			}

			const int result = sceIoChstat(path8, &stat, flags);
													 ...
													 uint32_t nDestCapacity)
{
	...
	// This function currently doesn't have a pathway for non-UTF8 char8_t strings.
...
EAIO_API bool File::CreateAlias(const char8_t* pDestinationPath,
								const char8_t* pShortcutPath, 
								const char8_t* pShortcutDescription,
								const char8_t* pShortcutArguments)
{
	// This function currently doesn't have a pathway for non-UTF8 char8_t strings.
	// As it currently stands, that doesn't matter much because the implementation
	// currently is implemented only for Windows, with which we require all file 
...
EAIO_API bool IsVolumeAvailable(const char8_t* pPath, int timeoutMS)
{
		...
		uint64_t   nTimeBase; SYS_TIMEBASE_GET(nTimeBase);
		uint64_t   freqMS    = sys_time_get_timebase_frequency() / 1000;
		uint64_t   timeMSEnd = (nTimeBase / freqMS) + timeoutMS;
		uint64_t   timeMS;
		CellFsStat fsStat;

		do {
			const int result = cellFsStat(pPath, &fsStat);
			...
			SYS_TIMEBASE_GET(nTimeBase);
			timeMS = nTimeBase / freqMS;
		}
		while(timeMS < timeMSEnd);
...
EAIO_API bool InitializeFileSystem(bool bInitialize)
{
		...
		if(bInitialize)
		{
			const int result = cellSysmoduleLoadModule(CELL_SYSMODULE_FS);
			return ((result == 0) && IsVolumeAvailable("/app_home/", 0)); // For IsVolumeAvailable, it's irrelevant whether an /app_home/ directory actually exists or is reachable.
		}
		else
		{
...
{
	char8_t path8[kMaxPathLength];      path8[0] = 0;
	char8_t directory8[kMaxPathLength]; directory8[0] = 0;
	char8_t fileName8[kMaxPathLength];  fileName8[0] = 0;
	char8_t extension8[kMaxPathLength]; extension8[0] = 0;
...
EAIO_API bool MakeTempPathName(char16_t* pPath, const char16_t* pDirectory, const char16_t* pFileName, const char16_t* pExtension, uint32_t nDestPathCapacity)
{
		...
		char8_t fileName8[kMaxPathLength];
		char8_t extension8[kMaxPathLength];
		char8_t *pFileName8 = NULL, *pDirectory8 = NULL, *pExtension8 = NULL;
...
EAIO_API bool MakeTempPathName(char8_t* pPath, const char8_t* pDirectory, const char8_t* pFileName, const char8_t* pExtension, uint32_t nDestPathCapacity)
{
   #if EAIO_MKSTEMPS_AVAILABLE
		...
		if(pPath)
		{
			...
			for(size_t i = 0; i < 100000; i++, nRand = EAIORand())
			{
				...
				// Xenon has a bug whereby CreateFile(CREATE_NEW) mistakenly always fails, 
				// yet the file is created anyways, when drive compression is enabled via XFileEnableTransparentDecompression.
				// Until that bug is fixed, we assume that __mftb returns a random enough number
				// and that if the file exists then it was created by the Open call above. 
				// The problem case is whereby somehow __mftb returns a number that by rare coincidence
				// is equal to an existing temp file name. 
...
{
	char8_t pPathDest8[EA::IO::kMaxPathLength];
...
EAIO_API int GetTempDirectory(char8_t* pDirectory, uint32_t nPathCapacity)
{
	// The PS3 doesn't actually allow a generic temp directory the user can write to.
	// You need to use the "game data" functionality of the PS3 to dynamically create
	// scratch drive space on the PS3. You would then call our SetTempDirectory function
	...
	if(gpTempDirectory[0])
	{
		...
		return (int)(unsigned)EA::StdC::Strlcpy(pDirectory, gpTempDirectory, nPathCapacity);
			...
			// With the Xenon (a.k.a. XBox 360), a user hard drive is optional.
			// As of the 4/2005 Xenon SDK, there is no way to detect such a hard drive,
			// so for the time being we simply return a default value.
			// It is recommended that the user use our SetTempDirectory on Xenon.
			uint32_t nLength = (uint32_t)EA::StdC::Strlcpy(pDirectory, "GAME:\\temp\\", nPathCapacity);
		...
		// It is recommended that the user use SetTempDirectory on PS3, as otherwise the 
		// code below executes and gives the user a directory that might not be ideal.

		char8_t dirPath[kMaxPathLength];

		if(EA::IO::GetPS3GameDataDirectory(dirPath))
			EA::StdC::Strlcat(dirPath, "tmp/", kMaxPathLength); // This is guaranteed to succeed.
		else
			Directory::GetCurrentWorkingDirectory(dirPath, kMaxPathLength);

		size_t nRequiredStrlen = EA::StdC::Strlcpy(pDirectory, dirPath, nPathCapacity);

		if(nRequiredStrlen < nPathCapacity)
		{
			Directory::EnsureExists(pDirectory);
...
EAIO_API uint64_t GetDriveFreeSpace(const char16_t* pPath)
{
		...
		*p0   = 0;
			...
			char8_t path8[kMaxPathLength];
			EA::StdC::Strlcpy(path8, pPathResult, kMaxPathLength);
		...
		uint64_t freeBlockCount = 0;

		// If you use "/" as a directory then you will get an error back, as that doesn't 
		// refer to a location that there is a unique drive mounted. It's a root for multiple drives.
		const CellFsErrno result = cellFsGetFreeSize(path8, &blockSize, &freeBlockCount);

		if(result == CELL_FS_SUCCEEDED)
			return blockSize * freeBlockCount;
		// Fall through
...
EAIO_API uint64_t GetDriveFreeSpace(const char8_t* pPath)
{
		...
		// If you use "/" as a directory then you will get an error back, as that doesn't 
		// refer to a location that there is a unique drive mounted. It's a root for multiple drives.
		const CellFsErrno result = cellFsGetFreeSize(pPath, &blockSize, &freeBlockCount);
...
EAIO_API int GetDriveName(const char16_t* pPath, char16_t* pName)
{
	EA_ASSERT(*pPath); // Assert that the input is non-empty.
		...
		*p   = 0;
		...
		if(result)
			return EA::StdC::Strlcpy(pName, pVolumeName8, kMaxVolumeNameLength);

		pName[0] = 0;
		return -1;

	#elif defined(EA_PLATFORM_PS3) || EAIO_USE_UNIX_IO || defined(EA_PLATFORM_KETTLE)
...
EAIO_API int GetDriveSerialNumber(const char16_t* pPath, char16_t* pSerialNumber)
{
		...
		*p   = 0;
		...
		EA::StdC::U32toa((unsigned)dwSerialNumber, buffer, 10);
		return EA::StdC::Strlcpy(pSerialNumber, buffer, kMaxVolumeSerialNumberLength);
...
EAIO_API DriveType GetDriveTypeValue(const char16_t* pPath)
{
		...
		*p   = 0;
		...
		const size_t numDrives = GetDriveInfo((DriveInfo8*)NULL,0);
		...
		*p = '\0';
		...
		DriveInfo8* infoBuffer = new DriveInfo8[sizeof(DriveInfo8) * numDrives];

		GetDriveInfo(infoBuffer,numDrives);

		DriveType driveType = kDriveTypeUnknown;

		for (size_t i = 0; i < numDrives && driveType == kDriveTypeUnknown; ++i)
		{
			if (strcmp(infoBuffer[i].mName.c_str(),path8) == 0)
			{
				driveType = infoBuffer[i].mType;
			}
		}
...
EAIO_API size_t GetDriveInfo(DriveInfo8* pDriveInfoArray, size_t nDriveInfoArrayCapacity)
{
		...
		if(nDriveInfoArrayCapacity > 0)
		{
			// We currently enumerate only a single drive, the GAME: drive, 
			// which is identical to the d: drive. 
			pDriveInfoArray[0].mName.assign("GAME:");

			DWORD   dwMaxComponentLength;
			DWORD   dwFileSystemFlags;
			char8_t pFileSystemName8[32];
			BOOL    result = GetVolumeInformation("D:\\", NULL, 0, NULL, &dwMaxComponentLength, 
												   &dwFileSystemFlags, pFileSystemName8, 32);
			EA_ASSERT(result); EA_UNUSED(result);

			if(pFileSystemName8[0] == 'F') // FATX
				pDriveInfoArray[0].mType = kDriveTypeFixed;
			else 
				pDriveInfoArray[0].mType = kDriveTypeDVD; // UDFX (DVD drive)
		}
		...
		// The PS3 uses a Unix-like file tree, upon which you add branches
		// via mounting devices. As such, drives or volumes exist but aren't
		// exposed by the file path system. The PS3 SDK does not expose a 
		// method to enumerate the volumes that are mounted onto the file system.
		// However, there are standard mount points which are known to exist.

		if(count++ < nDriveInfoArrayCapacity)
		{
			// The mount point for a file system on builtin BDVD drive
			pDriveInfoArray[0].mName.assign("/dev_bdvd");
			pDriveInfoArray[0].mType = kDriveTypeDVD;
		}

		if(count++ < nDriveInfoArrayCapacity)
		{
			// The mount point for a file system on builtin HDD
			pDriveInfoArray[1].mName.assign("/dev_hdd0");
			pDriveInfoArray[1].mType = kDriveTypeFixed;
		}
			...
			if(count++ < nDriveInfoArrayCapacity)
			{
				...
				pDriveInfoArray[2].mName.assign("/dev_ms");
				pDriveInfoArray[2].mType = kDriveTypeMemoryCard;
			}

			if(count++ < nDriveInfoArrayCapacity)
			{
				// The mount point for a file system on a remote host PC
				pDriveInfoArray[3].mName.assign("/host_root");
				pDriveInfoArray[3].mType = kDriveTypeFixed;
			}
		#endif
		...
		// At the moment, I have no idea if there is a way of enumerating the set of drives availiable on the system.
		// That being said, I can enumerate the ones that I know will exist.  The question is should I also try
		// to enumerate the logical drives such as 'savedata0:', or 'tty0:', etc...
		static const size_t numDrives = 2;
		static DriveInfo8 knownDrives[numDrives] =
		{
			DriveInfo8("host0:",kDriveTypeRemote),
			DriveInfo8("disc0:",kDriveTypeDVD),
		};
		...
		{
			if (Directory::Exists(knownDrives[i].mName.c_str()))
			{
				//if this drive will fit into the output array
				if (drivesFound < nDriveInfoArrayCapacity)
				{
					pDriveInfoArray[drivesFound].mName = knownDrives[i].mName;
					pDriveInfoArray[drivesFound].mType = knownDrives[i].mType;  
				}
		...
		// Note, the other platforms are doing it wrong: according to the doc, the return value should be the
		// total number of drives found on the system, regardless of whether they fit into the array
		return drivesFound;
		...
		char8_t convertBuffer[kMaxPathLength];
...
EAIO_API bool Directory::Exists(const char16_t* pDirectory)
{
	if(pDirectory && *pDirectory)
	{
			...
			{
				CellFsStat fsStat;
				const CellFsErrno result = cellFsStat(directory8, &fsStat);

				if(result == CELL_FS_SUCCEEDED)
					return (fsStat.st_mode & CELL_FS_S_IFDIR) != 0;
			}
			...
			char8_t directory8[kMaxPathLength];
			size_t n = StrlcpyUTF16ToUTF8(directory8, kMaxPathLength, pDirectory);

			if (n < (size_t)kMaxPathLength)
			{
				// The SDK doesn't like / or \\ at the end, so we have to strip it out
				// or it will give an error.
				char8_t path[kMaxPathLength];
				EA::StdC::Strcpy(path, directory8);
				int length = EA::StdC::Strlen(directory8);
				if ((path[length -1] == '\\') || (path[length -1] == '/'))
					path[length -1] = 0;

				const SceUID dirHandle = sceIoDopen(path);

				bool exists = dirHandle >= 0;

				sceIoDclose(dirHandle);
...
namespace
{
	bool CreateDirectoryInternal(const char16_t* pDirectory)
	{
		...
		// it is currently called only from from Create and Create is taking care of them.
		...
		#elif EAIO_USE_UNIX_IO || defined(EA_PLATFORM_XENON)  || defined(EA_PLATFORM_PS3) || defined(EA_PLATFORM_KETTLE)
			...
			if(pDirectory[0]) // Our code below requires at least one char in the string.
			{
				...
				Path::StripTrailingSeparator(&directory8[0], nLength);
					...
					#endif
				#elif defined(EA_PLATFORM_PS3)
					const CellFsErrno result = cellFsMkdir(directory8.c_str(), (CellFsMode)-1);
					return ((result == CELL_FS_SUCCEEDED) || (result == CELL_FS_EEXIST));
				#elif defined(EA_PLATFORM_KETTLE)
		...
		#elif defined(EA_PLATFORM_PSP2)
			char8_t path8[kMaxPathLength];
			size_t n = StrlcpyUTF16ToUTF8(path8,kMaxPathLength,pDirectory);

			if(n < (size_t)kMaxPathLength)
			{
				int result = sceIoMkdir(path8,SCE_STM_RWU);
				return (result == SCE_OK);
			}
...
EAIO_API bool Directory::Remove(const char16_t* pDirectory, bool bAllowRecursiveRemoval)
{
	...
	else // Non-recursive 
	{
			...
			#elif defined(EA_PLATFORM_PS3)
				return (cellFsRmdir(directory8.c_str()) == CELL_FS_SUCCEEDED);
			#elif defined(EA_PLATFORM_KETTLE)
			...
			Path::PathString8 directory8;
			uint32_t n = ConvertPathUTF16ToUTF8(directory8, pDirectory);

			if(n < (size_t)kMaxPathLength)
			{
				const int result = sceIoRmdir(directory8.c_str());
...
EAIO_API bool Move(const char8_t* pDirectorySource, const char8_t* pDirectoryDestination, bool bOverwriteIfPresent)
{
...
EAIO_API bool Directory::Rename(const char16_t* pDirectoryOld, const char16_t* pDirectoryNew)
{
	...
	#elif defined(EA_PLATFORM_PS3)
		char8_t pDirectoryOld8[kMaxDirectoryLength];
		char8_t pDirectoryNew8[kMaxDirectoryLength];

		if((EA::StdC::Strlcpy(pDirectoryOld8, pDirectoryOld, EAArrayCount(pDirectoryOld8)) < (int)EAArrayCount(pDirectoryOld8)) &&
		   (EA::StdC::Strlcpy(pDirectoryNew8, pDirectoryNew, EAArrayCount(pDirectoryNew8)) < (int)EAArrayCount(pDirectoryNew8))) // If the names weren't too long...
		{
			Path::StripTrailingSeparator(pDirectoryOld8);
			Path::StripTrailingSeparator(pDirectoryNew8);

			return (cellFsRename(pDirectoryOld8, pDirectoryNew8) == CELL_FS_SUCCEEDED);
		}
		...
		const int result = sceIoRename(pPathSource8,pPathDestination8);

		return result == SCE_OK;

	#elif defined(EA_PLATFORM_KETTLE)
		char8_t pathSource8[kMaxDirectoryLength];
		size_t n = EA::StdC::Strlcpy(pathSource8, pDirectoryOld, kMaxDirectoryLength);
		if (n < (size_t)kMaxDirectoryLength)
			return false;

		char8_t pathDestination8[kMaxDirectoryLength];
...
EAIO_API bool Directory::Copy(const char8_t* pDirectorySource, const char8_t* pDirectoryDestination, bool bRecursive, bool bOverwriteIfAlreadyPresent)
{
	// This function currently requires UTF8 path strings.
...
EAIO_API bool Directory::SetAttributes(const char8_t* pPath, int nAttributeMask, bool bEnable, bool bRecursive, bool bIncludeBaseDirectory)
{
	// This function currently requires UTF8 path strings.
	#if (EA_WCHAR_SIZE == 1)
		return SetAttributesW(pPath, nAttributeMask, bEnable, bRecursive, bIncludeBaseDirectory);
...
EAIO_API int Directory::GetCurrentWorkingDirectory(char16_t* pDirectory, uint32_t nPathCapacity)
{
		...
		if(pResult)
		{
			size_t nStrlen = EA::StdC::Strlcpy(pDirectory, (char8_t*)pPath8, kMaxDirectoryLength - 2, kLengthNull); // -2 so we have space for possible trailing '\' and '\0'.
		...
		if(!gbCurrentWorkingDirectoryInitialized) // If we haven't set the default value of the current working directory...
		{
			...
			gbCurrentWorkingDirectoryInitialized = true;
			...
			#if defined(EA_PLATFORM_XENON)
				// D:\ is the same as GAME:\, according to Xenon documentation.
			 ...
			 #elif defined(EA_PLATFORM_MICROSOFT) || defined(EA_PLATFORM_PS3)
			...
			#elif defined(EA_PLATFORM_PSP2)
				EAIOStrlcpy16(gpCurrentWorkingDirectory, L"host0:./", kTempDirectoryLength);
			#elif defined(EA_PLATFORM_KETTLE)
...
EAIO_API int Directory::GetCurrentWorkingDirectory(char8_t* pDirectory, uint32_t nPathCapacity)
{
	#if EAIO_USE_UNIX_IO // Special case for Unix because Unix paths aren't necessarily UTF8.
...
EAIO_API bool Directory::IsDirectoryEmpty(const char8_t* pDirectory, int nDirectoryEntryFlags, bool bRecursive)
{
	// This function currently requires UTF8 path strings.
	#if (EA_WCHAR_SIZE == 1)
		return IsDirectoryEmptyW(pDirectory, nDirectoryEntryFlags, bRecursive);
								 ...
								 bool bEnsureDirectoryExistence, uint32_t nPathCapacity)
{
	pDirectory[0] = 0;
		...
		switch(specialDirectory)
		{
			...
			{
				char8_t buffer[EA::IO::kMaxDirectoryLength];
		...
		switch(specialDirectory)
		{
			...
			case kSpecialDirectoryTemp:
				#if _XDK_VER >= 9516 // on or after the JulyXDK
					success = ((uint32_t)EA::StdC::Strlcpy(pDirectory, "T:\\", nPathCapacity) < nPathCapacity);
				#else
					success = (EA::StdC::Strlcpy(pDirectory, Windows::Xbox::Storage::LocalStorage::TemporaryFolder->Path->Data(), nPathCapacity) > 0);
					if(success)
						EA::StdC::Strlcat(pDirectory, "\\", nPathCapacity); // must have trailing slash
				#endif
				break;
	...
	#elif EAIO_USE_UNIX_IO || defined(EA_PLATFORM_APPLE) || defined(EA_PLATFORM_KETTLE)
...
EAIO_API int GetSpecialDirectory(SpecialDirectory specialDirectory, char8_t* pDirectory, 
								 bool bEnsureDirectoryExistence, uint32_t nPathCapacity)
{
	#if EAIO_USE_UNIX_IO || defined(EA_PLATFORM_APPLE)
		...
		switch(specialDirectory)
		{
			case kSpecialDirectoryDevScratch: // /hostapp/ is not usable in retail titles.
			...
			case kSpecialDirectoryUserApplicationData:
			case kSpecialDirectoryCommonApplicationData: // Sony states that /data/ is supported by development kits only and not retail titles.
...
bool IsFileNameStringValid(const char8_t* pName, FileSystem fileSystemType)
{
	char16_t name16[kMaxPathLength];
	EA::StdC::Strlcpy(name16, pName, kMaxPathLength);
	return IsFileNameStringValid(name16, fileSystemType); 
...
bool IsDirectoryNameStringValid(const char8_t* pName, FileSystem fileSystemType)
{
...
bool IsFilePathStringValid(const char8_t* pPath, FileSystem fileSystemType)
{
	char16_t path16[kMaxPathLength];
	EA::StdC::Strlcpy(path16, pPath, kMaxPathLength);
	return IsFilePathStringValid(path16, fileSystemType);
