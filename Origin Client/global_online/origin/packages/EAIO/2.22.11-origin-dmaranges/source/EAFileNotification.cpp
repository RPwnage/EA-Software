...
///////////////////////////////////////////////////////////////////////////////
// ReleaseHandlers
// This function cleans up the handlers and must be called before stopping 
// the thread
//
void FileChangeNotification::ReleaseHandlers()
{
    // Close the shutdown event. mhEvents[0] is the algorithm-specific
    // event created by the thread itself, so we don't close it here.
    if (mhEvents[1])
...
bool FileChangeNotification::Stop()
{
	...
	if(mhWaitingThread)
	{
		...
		SetEvent(mhEvents[1]);
		WaitForSingleObject(mhWaitingThread, 100);
...
static int GetCurrentFCNTime()
{
		...
		return (int)(nTimeBase / (sys_time_get_timebase_frequency() / 1000));
	#elif EAIO_USE_UNIX_IO
		return (int)(clock() * INT64_C(1000) / CLOCKS_PER_SEC);
	#elif defined(EA_PLATFORM_PSP2)
		return  static_cast<int>(sceKernelGetProcessTimeWide() / 1000);
