...
EA::IO::MemoryStream::MemoryStream(const MemoryStream& memoryStream)
  : IStream(),
    ...
    mpReleaseAllocator(memoryStream.mpReleaseAllocator),
...
int EA::IO::MemoryStream::Release()
{
	const int rc = (int)((mnRefCount--) - 1); // Must use postdec for AtomicInt32. This is because we act on the resulting value, and if two threads simultaneously call Release with a starting refcount of 2, they could both see the new value as 0, whereas we want only the last one to see it as 0.

	if(rc)
		return rc;

	mnRefCount = 1; // Prevent double destroy if AddRef/Release pair occurs during destruction

	if(mpReleaseAllocator)
	{
		this->~MemoryStream();
		mpReleaseAllocator->Free(this, sizeof(*this));
	}
	else
