...
{
	auto sSectionW = EA::StdC::Strlcpy<StringW, char8_t>(pSection);
	auto sKeyW = EA::StdC::Strlcpy<StringW, char8_t>(pKey);
...
bool IniFile::WriteEntry(const char8_t* pSection, const char8_t* pKey, const char8_t* pValue)
{
	auto sSectionW = EA::StdC::Strlcpy<StringW, char8_t>(pSection);
	auto sKeyW = EA::StdC::Strlcpy<StringW, char8_t>(pKey);
	auto sValueW = EA::StdC::Strlcpy<StringW, char8_t>(pValue);
...
bool IniFile::WriteEntryFormatted(const char8_t* pSection, const char8_t* pKey, const char8_t* pFormat, ...)
{
	...
	eastl::fixed_string<char8_t, 32, true, Allocator::EAIOEASTLCoreAllocator> value;
	EA::StdC::StringVcprintf(value, pFormat, arguments);
	bool bResult = WriteEntry(pSection, pKey, value.c_str());
...
bool IniFile::ConvertAndWriteStream(const wchar_t* pTextW, size_t count)
{
	...
	if(mpStream && pTextW)
	{
		if(mnEncodingSrc == kEncodingUTF8) // else if the IniFile is 8 bit...
		{
			...
			return mpStream->Write(conversionBuffer, count * sizeof(char8_t));
		}
