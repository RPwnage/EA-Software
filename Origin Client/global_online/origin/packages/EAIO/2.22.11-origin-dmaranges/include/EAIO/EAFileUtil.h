...
namespace EA
{
	namespace IO
	{
		...
		namespace File
		{
			...
			/// File::Stat
			EAIO_API bool Stat(const char8_t*  pPath, StatData& statData);
			...
			/// 
			EAIO_API ResolveAliasResult ResolveAlias(const char8_t*  pPathSource, char8_t*  pPathDestination, uint32_t nDestPathCapacity = kMaxPathLength);
		...
		namespace Directory
		{
			...
			// EAIO_API bool Move(const char8_t*  pDirectorySource, const char8_t*  pDirectoryDestination, bool bOverwriteIfPresent = true);
			...
			EAIO_API int GetCurrentWorkingDirectory(char8_t*  pDirectory, uint32_t nPathCapacity = kMaxPathLength);
		...
		/// a directory path name that ends in a path separator. This is by design as it simplifies
		/// the specification of and manipulation of paths.
		EAIO_API bool MakeTempPathName(char8_t*  pDestPath, const char8_t*  pDirectory = NULL, const char8_t*  pFileName = NULL, const char8_t*  pExtension = NULL, uint32_t nDestPathCapacity = kMaxPathLength);
		...
		/// on some platforms.
		EAIO_API int GetTempDirectory(char8_t*  pDirectory, uint32_t nPathCapacity = kMaxDirectoryLength);
		...
		/// on platforms such as PS3 and Unix which use mounted volumes.
		...
		struct EAIO_API DriveInfo8
		{
			...
			typedef eastl::fixed_string<char8_t, 20, true, Allocator::EAIOEASTLCoreAllocator> DriveInfoName;
		...
		/// a directory path name that ends in a path separator. This is by design as it simplifies
		/// the specification of and manipulation of paths.
		EAIO_API int GetSpecialDirectory(SpecialDirectory specialDirectory, char8_t*  pDirectory, bool bEnsureDirectoryExistence = false, uint32_t nPathCapacity = kMaxDirectoryLength);
		...
		///////////////////////////////////////////////////////////////////////
			...
			/// If you will be calling GetTempDirectory or GetCurrentWorkingDirectory
			/// then you need to call this function before-hand. You can avoid calling
			/// this function if you manually set the temp and working directories via
			/// SetTempDirectory and SetCurrentWorkingDirectory.
			///
			...
			/// 
			/// Example usage:
			///     SetPS3GameDataNames("Game X", "GameX-28931", "1.0", "GameX28931Data");
			///
			...
			// The minimum sizes for each of these buffers is defined by the PS3 SDK respectively: 
			//     CELL_GAME_SYSP_TITLE_SIZE, CELL_GAME_SYSP_TITLEID_SIZE, CELL_GAME_SYSP_VERSION_SIZE, CELL_GAME_PATH_MAX 
			...
			///
			/// Example usage:
			///     char8_t tempPath[EA::IO::kMaxDirectoryLength];
