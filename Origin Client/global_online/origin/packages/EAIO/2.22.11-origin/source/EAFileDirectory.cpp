...
#undef ENTRYLIST_NAME
		  ...
		  #endif
			{
				...
				do // Run a loop so we can filter out any "." and ".." entries that might be present and entries that don't match the input pattern.
				{
					   ...
					   (strcmp(dirEntry->d_name, "..") != 0))
					{
					  ...
					  #endif
						{
							// The following char8_t to wchar_t Strlcpy can return -1 if d_name is not valid UTF8. Unix file systems
	...
	EAIO_API EntryFindData* EntryFindFirst(const wchar_t* pDirectory, const wchar_t* pFilterPattern, EntryFindData* pEntryFindData)
	{
		...
		EA_ASSERT(pDirectory);
		if(pDirectory[0] || (!pFilterPattern || pFilterPattern[0]))
		{
			Path::PathStringW directoryTempW;
			directoryTempW += pDirectory;
			StripTrailingSeparator(directoryTempW);

			Path::PathString8 directory8;
			// Measure how many UTF-8 chars we'll need (EASTL factors-in a hidden + 1 for NULL terminator)
			size_t nCharsNeeded = EA::StdC::Strlcpy((char8_t*)NULL, directoryTempW.c_str(), 0);
			directory8.resize(nCharsNeeded);
			EA::StdC::Strlcpy(&directory8[0], directoryTempW.c_str(), nCharsNeeded + 1);
			...
			if(result == CELL_FS_SUCCEEDED)
			{
				...
				EA::StdC::Strlcpy(pEntryFindData->mDirectoryPath, directoryTempW.c_str(), kMaxPathLength);
				FilterEntries(pEntryFindData, pFilterPattern);
		...
		while((result == CELL_FS_SUCCEEDED) && (n != 0))
		{
			...
			if(!pEntryFindData->mEntryFilterPattern[0] || FnMatch(pEntryFindData->mEntryFilterPattern, pEntryFindData->mName, kFNMCaseFold))
			{
				...
				else
				{
					...
					Path::PathStringW fullPathW(pEntryFindData->mDirectoryPath);
					Join(fullPathW, pEntryFindData->mName);
					
					// Convert to UTF8 and test for existence
					// Measure how many UTF-8 chars we'll need (EASTL factors-in a hidden + 1 for NULL terminator)
					size_t nCharsNeeded = EA::StdC::Strlcpy((char8_t*)NULL, fullPathW.c_str(), 0);
					
					Path::PathString8 fullPath8;
					fullPath8.resize(nCharsNeeded);
					EA::StdC::Strlcpy(&fullPath8[0], fullPathW.c_str(), nCharsNeeded + 1);
	...
	struct EntryFindDataKettle 
	{
		void Init(int dirDescr, int bufferSize)
		{
			...
			mBuffer = static_cast<char*>(GetAllocator()->Alloc(mBufferSize, EAIO_ALLOC_PREFIX "EAFileDirectory/EntryFindData/EntryBuffer", 0, 0));
		}
	...
	EAIO_API EntryFindData* EntryFindFirst(const wchar_t* pDirectoryPath, const wchar_t* pFilterPattern, EntryFindData* pEntryFindData)
	{
		...
		EA_COMPILETIME_ASSERT(sizeof(EntryFindDataKettle) <= sizeof(pEntryFindData->mPlatformData));
		...
		if (pDirectoryPath[0] || (!pFilterPattern || pFilterPattern[0]))
		{
			...
			EntryFindDataKettle *entryDataKettle = reinterpret_cast<EntryFindDataKettle*>(pEntryFindData->mPlatformData);
