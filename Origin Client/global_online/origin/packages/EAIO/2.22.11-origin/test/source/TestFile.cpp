...
#include <limits.h>
	...
	#define TIME_TEST_SKIP_HOST_FILE_SYS L"/app_home/"
...
static int TestFileUtil()
{
		...
		{
			{
				...
				// time_t File::GetTime(const char8_t* pPath, FileTimeType timeType);
				// bool   File::SetTime(const char16_t* pPath, int nFileTimeTypeFlags, time_t nTime);
				// bool   File::SetTime(const char8_t* pPath, int nFileTimeTypeFlags, time_t nTime);
				// bool   File::Stat(const char8_t*  pPath, StatData& statData);
...

	{
		...
		{
			char8_t root[] = "/";
			const char8_t* result = EA::IO::Path::GetPathComponentEnd(root, root + 1, 1);
...
static int TestFileDirectory()
{
...

	{
		...
		{
			timeval tv;
			EA::StdC::GetTimeOfDay(&tv, NULL, true);    // We use EAStdC time functionality instead of built-in standard C time because the time function is broken on some platforms (e.g. PS3).
			if(gbVerboseOutput)
