...
namespace EA
{
	namespace IO
	{
		...
		public:
			...
			FilePath(const char8_t*  pPath);
			FilePath(const char16_t* pPath);
			FilePath(const char8_t*  pDrive, const char8_t*  pDirectory, const char8_t*  pFileName, const char8_t*  pExtension);
			...
			//
			size_t GetDrive                (char8_t*  buf, size_t capacity = kMaxDriveLength    ) const;
			size_t GetDrive                (char16_t* buf, size_t capacity = kMaxDriveLength    ) const;
			size_t GetDirectory            (char8_t*  buf, size_t capacity = kMaxDirectoryLength) const;
			size_t GetDirectory            (char16_t* buf, size_t capacity = kMaxDirectoryLength) const;
			size_t GetFileName             (char8_t*  buf, size_t capacity = kMaxFileNameLength ) const;
			size_t GetFileName             (char16_t* buf, size_t capacity = kMaxFileNameLength ) const;
			size_t GetExtension            (char8_t*  buf, size_t capacity = kMaxExtensionLength) const;
			size_t GetExtension            (char16_t* buf, size_t capacity = kMaxExtensionLength) const;

			size_t GetPath                 (char8_t*  buf, size_t capacity = kMaxPathLength) const;
			size_t GetPath                 (char16_t* buf, size_t capacity = kMaxPathLength) const;
			size_t GetDriveAndDirectory    (char8_t*  buf, size_t capacity = kMaxPathLength) const;
			size_t GetDriveAndDirectory    (char16_t* buf, size_t capacity = kMaxPathLength) const;
			size_t GetFileNameAndExtension (char8_t*  buf, size_t capacity = kMaxPathLength) const;
			...
			bool   AppendDirectory(const char8_t* );   //< Adds a directory component to the path; filename and extension are preserved. Returns false if there is not enough room.
			...
			bool   SetDrive                  (const char8_t*  pDrive,                bool bInputIsPath = false);  // This function has no effect on non-Win32 file systems.
			bool   SetDrive                  (const char16_t* pDrive,                bool bInputIsPath = false);  // This function has no effect on non-Win32 file systems.
			bool   SetDirectory              (const char8_t*  pDirectory,            bool bInputIsPath = false);
			bool   SetDirectory              (const char16_t* pDirectory,            bool bInputIsPath = false);
			bool   SetFileName               (const char8_t*  pFileName,             bool bInputIsPath = false);
			bool   SetFileName               (const char16_t* pFileName,             bool bInputIsPath = false);
			bool   SetExtension              (const char8_t*  pExtension,            bool bInputIsPath = false);
			bool   SetExtension              (const char16_t* pExtension,            bool bInputIsPath = false);
			bool   SetDriveAndDirectory      (const char8_t*  pDriveAndDirectory,    bool bInputIsPath = false);
			bool   SetDriveAndDirectory      (const char16_t* pDriveAndDirectory,    bool bInputIsPath = false);
			bool   SetFileNameAndExtension   (const char8_t*  pFileNameAndExtension, bool bInputIsPath = false);
			bool   SetFileNameAndExtension   (const char16_t* pFileNameAndExtension, bool bInputIsPath = false);
			bool   SetPath                   (const char8_t*  pPath);
			bool   SetPath                   (const char16_t* pPath);
			bool   SetPath                   (const char8_t*  pDrive, const char8_t*  pDirectoryOnly, const char8_t*  pFileNameOnly, const char8_t*  pExtension);
		...
		protected:
			...
			bool ReplacePathComponent(PathPos start, PathPos end, const char8_t*  src, bool bSrcIsPath);
		...
		EAIO_API bool GetHasTrailingPathSeparator(const char8_t*  pDirectory, size_t nDirectoryLength = kLengthNull);
		...
		///
		EAIO_API bool RemoveTrailingPathSeparator(char8_t*  pDirectory, size_t nDirectoryLength = kLengthNull);
		...
		inline const char8_t* GetFileName(const char8_t* pPath)
		{
			const char8_t* pDirStart;
			const char8_t* pFileStart = NULL;
			const char8_t* pExtStart;
		...
		/// GetFileName
		/// Return a pointer to the filename part of the path
		inline char8_t* GetFileName(char8_t* pPath)
		{
			char8_t* pDirStart;
			char8_t* pFileStart = NULL;
			char8_t* pExtStart;
		...
		inline const char8_t* GetFileExtension(const char8_t* pPath)
		{
			const char8_t* pDirStart;
			const char8_t* pFileStart;
			const char8_t* pExtStart = NULL;
		...
		/// GetFileExtension
		/// Return a pointer to the file extension part of the path
		inline char8_t* GetFileExtension(char8_t* pPath)
		{
			char8_t* pDirStart;
			char8_t* pFileStart;
			char8_t* pExtStart = NULL;
