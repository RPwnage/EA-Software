...
//Origin notes - we had to branch this package because when compiled the allocate() would use the EASTL_DLL code path
//And the deallocate would use the non-EASTL_DLL code path, resulting in a crash.
