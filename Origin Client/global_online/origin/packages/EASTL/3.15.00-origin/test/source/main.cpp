...
{
	size_t memory = EA_MEBIBYTE(512);
	nn::Result result = nn::os::SetMemoryHeapSize(memory);
	NN_ASSERT(result.IsSuccess());

	// Allocate and initialize the malloc heap
	uintptr_t address;
	result = nn::os::AllocateMemoryBlock(&address, memory);
	NN_ASSERT(result.IsSuccess());
	nn::init::InitializeAllocator(reinterpret_cast<void*>(address), memory);
}
#endif
