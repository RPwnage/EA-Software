    ...
    class VC_Common_PostprocessOptions : AbstractModuleProcessorTask
    {
        ...
        private void ProcessDependents(CcCompiler cc)
        {
            if (Module.IsKindOf(Model.Module.Managed))
            {
                ...
                var fuAssemblies = Project.GetFileSet(PropGroupName("forceusing-assemblies"));
                if (fuAssemblies != null)
                {
                    foreach (var assembly in fuAssemblies.FileItems.Select(item => item.Path))
                    {
        ...
        private void CreateResourceTool(Module_Native module)
        {
            ...
            if (module.Link != null)
            {
                ...
                if (resourcefiles != null && resourcefiles.FileItems.Count > 0)
                {
                    BuildTool rc = null;
                    BuildTool resx = null;
                    var inputDependencies = resourcefiles.SafeClone();
                    ...
                    if (rcfiles.Count() > 0 && Properties.Contains("eaconfig.PlatformSDK.dir"))
                    {
                        foreach (var resourcefile in rcfiles)
                        {
                            ...
                            if (Convert.ToInt32(Properties.GetPropertyOrDefault("package.WindowsSDK.MajorVersion", "0")) >= 8)
                            {
                            ...
                            inputDependencies.Exclude(resourcefile.Path);
                    ...
                    if (module.IsKindOf(Model.Module.Managed))
                    {
                        ...
                        if (resxfiles.Count() > 0)
                        {
                            ...
                            foreach (var file in resxfiles)
                            {
                                ...
                                inputDependencies.Exclude(file.Path);
                            }
                    ...
                    if (rc != null)
                    {
                        rc.InputDependencies.AppendWithBaseDir(inputDependencies);
                    }
                    if (resx != null)
                    {
                        resx.InputDependencies.AppendWithBaseDir(inputDependencies);
                    }
                }
            }
