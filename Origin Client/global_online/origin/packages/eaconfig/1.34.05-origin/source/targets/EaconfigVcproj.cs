...
namespace EA.Eaconfig
{
    ...
    public class EaconfigVcproj : EaconfigVisualStudioBase
    {
        ...
        private string GetPreBuildStep(ProjBuildData BaseBuildData)
        {
            ...
            {
                // Loop over the parent NAnt command line and append user defined properties.
                StringBuilder sbUserProperties = new StringBuilder();
                string[] arrCommandLine = Environment.CommandLine.Split(' ');				

                foreach (string c in arrCommandLine)
                {
                    if (c.StartsWith("-D:"))
                    {
                        if (c.StartsWith("-D:config") ||
                            c.StartsWith("-D:eaconfig.build.group") ||
                            c.StartsWith("-D:build.buildtype") ||
                            c.StartsWith("-D:build.buildtype.base") ||
                            c.StartsWith("-D:groupname") ||
                            c.StartsWith("-D:build.module") ||
                            c.StartsWith("-D:build.outputname"))
                        {
                            continue;
                        }

                        sbUserProperties.AppendFormat("{0} ", c);
                    }
                }								
            ...
            {                				
				...
				foreach(string c in arrCommandLine)
				{
					if(c.StartsWith("-D:"))
					{
                        if (c.StartsWith("-D:config") ||
                            ...
                            c.StartsWith("-D:build.buildtype") ||
                            c.StartsWith("-D:build.buildtype.base") ||
                            c.StartsWith("-D:groupname -D:config") ||
                            c.StartsWith("-D:build.module") ||
                            c.StartsWith("-D:build.outputname"))
                        {
                            continue;
				...
				string expandedProperties = Project.ExpandProperties("${nant.location}/nant.exe " + targetName + 
                    " -D:eaconfig.build.group=${eaconfig.build.group} -D:build.buildtype=${build.buildtype}"+
                    " -D:build.buildtype.base=${build.buildtype.base} -D:groupname=${groupname} -D:config=${config}"+
                    " -D:build.module=${build.module}" +
                    " -D:build.outputname=${build.outputname}" +
				...
				preBuildStep.AppendLine(expandedProperties);
            }
        ...
        private void SetupProperties(string outputbinary, string finalPath, string ccoutputpath)
        {
            ...
            BuildData.BuildProperties["appxmanifestoptionset"] = PropGroupName("appxmanifestoptions");
