        ...
        private void SetDefaultBuildStep(ConditionalPropertyElement step, BuildTargetElement targetProperty)
        {
            if (String.IsNullOrEmpty(step.Value))
            {
                if (targetProperty != null && !String.IsNullOrEmpty(targetProperty.TargetName))
                {
                    ...
                    string command = Project.ExpandProperties("${nant.location}/nant.exe " + targetProperty.TargetName + " -D:config=${config} -buildfile:${nant.project.buildfile} -masterconfigfile:${nant.project.masterconfigfile} -buildroot:${nant.project.buildroot} " + sbUserProperties.ToString());
