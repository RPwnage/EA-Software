...
//
	...
	#define EATHREAD_VERSION   "1.32.04"
	#define EATHREAD_VERSION_N  13204
...
///////////////////////////////////////////////////////////////////////////////
// EA_PROCESSOR_ARM64 and EA_PROCESSOR_ARM32
//
// Defined as 1 or undefined.
// Implements support for the definitions of EA_PROCESSOR_ARM64 and EA_PROCESSOR_ARM32 
// when using a version of EABase prior to their additions.
//
#if (EABASE_VERSION_N < 20501)
	#if !defined(EA_PROCESSOR_ARM64) && (defined(__aarch64__) || defined(__AARCH64))
		#define EA_PROCESSOR_ARM64 1
	#elif !defined(EA_PROCESSOR_ARM32) && defined(__arm__)
...
//
	...
	#if (defined(EA_PLATFORM_UNIX) && !defined(EA_PLATFORM_SONY)) || defined(EA_PLATFORM_IPHONE) || defined(EA_PLATFORM_OSX) || defined(EA_PLATFORM_KETTLE) || defined(EA_PLATFORM_NX)
...
//
	...
	#if EABASE_VERSION_N >= 20034
		#define EAT_COMPILETIME_ASSERT(expression) static_assert(expression, EA_STRINGIFY(expression))
	#else
		#ifdef EA_COMPILER_GNUC
			#define EAT_COMPILETIME_ASSERT(expression) extern int dummy__[bool(expression) ? 1 : -1] __attribute__ ((unused));
		#else
			#define EAT_COMPILETIME_ASSERT(expression) extern int dummy__[bool(expression) ? 1 : -1];
		#endif
	#endif
#endif
