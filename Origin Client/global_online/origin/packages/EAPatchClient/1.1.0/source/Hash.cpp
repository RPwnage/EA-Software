...
#include <string.h>
#include <DirtySDK/crypt/cryptmd5.h>  // from DirtySDK 
...
void MD5::Begin()
{
    EA_COMPILETIME_ASSERT((sizeof(mWorkData) >= sizeof(CryptMD5T)) && EA_ALIGN_OF(uint64_t) >= EA_ALIGN_OF(CryptMD5T));
    CryptMD5Init(eastl::union_cast<CryptMD5T*>(mWorkData));
}
...
void MD5::AddData(const void* data, size_t dataLength)
{
    ...
    CryptMD5Update(eastl::union_cast<CryptMD5T*>(mWorkData), data, (int32_t)dataLength);
}

void MD5::End(HashValueMD5& hashValue)
{
    CryptMD5Final(eastl::union_cast<CryptMD5T*>(mWorkData), hashValue.mData, EAArrayCount(hashValue.mData));
}
