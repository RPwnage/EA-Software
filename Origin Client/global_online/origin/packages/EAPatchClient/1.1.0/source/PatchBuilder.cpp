                                          ...
                                          GenerateBuiltFileInfoEventCallback* pEventCallback, intptr_t userContext)
{
    ...
    #if (EAPATCH_DEBUG >= 1)
        uint32_t skippedBlocks = 0; //Count the amount of blocks skipped by repeated hash mismatches
    #endif
    if(reader.Open(bfi.mLocalFilePath.c_str(), (size_t)diffData.mBlockSize))
    {
        ...
        const uint32_t kMaxRetries = 20; // Amount of checksum passes followed by hash misses that we allow in a row
        ...
        while(!bDone)
        {
            ...
            #endif
            const bool checksumMatch = (it != bhwpSet.end());

            if(checksumMatch) //If there are no checksum matches. there's no point in calculating a hash
            {
            ...
            //If the checksum matched but no hash did, count a miss
            if (checksumMatch && !bMatchFound)
                ...
                {
                    ++skippedBlocks; //Keep track of skipped blocks for debugging purposes
                }
            #endif
    ...
    #if (EAPATCH_DEBUG >= 1)
        EAPATCH_TRACE_FORMATTED("PatchBuilder::GenerateBuiltFileInfo: Finished file, skipped %d blocks out of %d.\n", skippedBlocks, bfi.mLocalFileSize/diffData.mBlockSize);
    #endif
