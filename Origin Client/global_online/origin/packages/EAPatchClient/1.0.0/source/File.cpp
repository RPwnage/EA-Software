...
bool FileChecksumReader::GoToFirstByte(uint32_t& checksum, bool& bDone)
{
    if(mbSuccess && !mbDone)
    {
...
bool FileChecksumReader::GoToNextByte(uint32_t& checksum, bool& bDone)
{
    if(mbSuccess && !mbDone)
    {
        ...
        uint64_t readSizeResult = 0;
        size_t   prevBufferReadPosition = (mBufferReadPosition >= mDiffBlockSize) ? (mBufferReadPosition - mDiffBlockSize) : ((mBufferSize + mBufferReadPosition) - mDiffBlockSize);

        // Read the next byte. This File class is already buffered, so this won't be hitting the disk slowly every time.
        if(mFile.Read(&mBuffer[mBufferReadPosition], 1, readSizeResult, false))
        {
            if(readSizeResult) // If there was another byte in the file to read...
            {
            ...
            // Else there are no more bytes and we are done.
