            ...
            for(BuiltFileSpanList::iterator it2 = it1; it2 != mBuiltFileSpanList.end(); )
            {
              ...
              //const uint64_t bfs2StartPos = bfs2.mDestStartPos;
                ...
                if(destEndPostion <= bfs2EndPos) // If this is where we stop...
                {
                    ...
                    for (auto node = mBuiltFileSpanList.begin(); node != mBuiltFileSpanList.end();)
                    {
                        const auto nextNode = eastl::next(node);

                        if (nextNode != mBuiltFileSpanList.end()) // If there is another one after this one in the range we are looking at.
                        {

                            if (node->mSourceFileType == nextNode->mSourceFileType) // If these are both the same source...
                            {
                                if ((node->mSourceStartPos + node->mCount) == nextNode->mSourceStartPos) // If the current and next node are contiguous sources.
                                {
                                    node->mCount += nextNode->mCount;
                                    mBuiltFileSpanList.erase(nextNode);
                                    continue;
                                }
                            } // Else nothing to do
                        }
                        eastl::advance(node, 1);
                    }
...
BuiltFileSpanList::iterator BuiltFileInfo::FindNextRemoteSpanInRange(BuiltFileSpanList::iterator start, int minLocalSpanLength)
{
    // Make sure we are not operating on the end of the list
    if (start != end(mBuiltFileSpanList))
    {
        // The first span to test should be a remote span
        if (start->mSourceFileType == kFileTypeRemote || start == begin(mBuiltFileSpanList))
        {
            // The end of the span passed in unless the start span is local
            uint64_t startPos = start->mDestStartPos + (start->mSourceFileType == kFileTypeRemote ? start->mCount : 0);

            // Go to the first span to test
            start++;

            // iterate through the remaining spans to find a remote span
            for (BuiltFileSpanList::iterator i = start; i != end(mBuiltFileSpanList); i++)
            {
                // Is this a remote span
                if (i->mSourceFileType == kFileTypeRemote)
                {
                    // Is the distance to this remote span smaller than the minimal local span length?
                    if (i->mDestStartPos - startPos < minLocalSpanLength)
                    {
                        // Yes, return the span we found.
                        return i;
                    }
                    else
                    {
                        // No, return end.
                        return end(mBuiltFileSpanList);
                    }
                }
            }
        }
...
BuiltFileSpanList::iterator BuiltFileInfo::MergeIntoRemoteSpan(BuiltFileSpanList::iterator start, BuiltFileSpanList::iterator finish)
{
    if (start == end(mBuiltFileSpanList))
        return start;

    // The last span if there is any should be a remote span
    if (finish != end(mBuiltFileSpanList) && finish->mSourceFileType != kFileTypeRemote)
        return end(mBuiltFileSpanList);

    // The first span needs to be remote, unless it is the first span in the list.
    if (start == begin(mBuiltFileSpanList) && start->mSourceFileType == kFileTypeLocal)
    {
        start->mSourceFileType = kFileTypeRemote;

        mLocalSpanCount -= start->mCount;
        mRemoteSpanCount += start->mCount;
    }


    for (BuiltFileSpanList::iterator i = start; i != finish; i++)
    {
        BuiltFileSpanList::iterator iSpanToMerge = i;
        iSpanToMerge++;

        if (iSpanToMerge != end(mBuiltFileSpanList))
        {
            // Switch the local span to be remote.
            if (iSpanToMerge->mSourceFileType == kFileTypeLocal)
            {
                mLocalSpanCount -= iSpanToMerge->mCount;
                mRemoteSpanCount += iSpanToMerge->mCount;
            }

            // Add the length to the merged span.
            start->mCount += iSpanToMerge->mCount;
        }
    }

    BuiltFileSpanList::iterator iFirstSpanToErase = start;
    iFirstSpanToErase++;

    if (finish != end(mBuiltFileSpanList))
        finish++;

    // erase all the merged spans.
    start = mBuiltFileSpanList.erase(iFirstSpanToErase, finish);
    start--;

    return start;
}

void BuiltFileInfo::RemoveSmallLocalSpans(int minLocalSpanLength)
{
    for (BuiltFileSpanList::iterator i = mBuiltFileSpanList.begin(); i != mBuiltFileSpanList.end();)
    {
        BuiltFileSpanList::iterator nextRemoteSpan = FindNextRemoteSpanInRange(i, minLocalSpanLength);

        if (nextRemoteSpan != end(mBuiltFileSpanList))
        {
            i = MergeIntoRemoteSpan(i, nextRemoteSpan);
        }
        else
        {
            i++;
...
bool BuiltFileInfoSerializer::Serialize(BuiltFileInfo& bfi, const char8_t* pFilePath)
{
    EAPATCH_ASSERT(EA::StdC::Strcmp(EA::IO::Path::GetFileExtension(pFilePath), kPatchBFIFileNameExtension) == 0);

    if(mbSuccess && bfi.ShouldWrite())
    {
        File file;
...
void PatchBuilder::GenerateBuiltFileInfoJob(JobArgs* args)
{
    auto diffData = args->mDiffData;
    auto* bfi = args->mBfi;
    auto pEventCallback = args->mEventCallback;

    eastl::vector<LocalSegmentInfo> results;
    ...
    if (reader.Open(bfi->mLocalFilePath.c_str(), (size_t)diffData.mBlockSize))
    {
        ...
        constexpr uint32_t kMaxRetries = 20; // Amount of checksum passes followed by hash misses that we allow in a row
        uint32_t       retryCount = 0;
        uint64_t       lastByteProgress = 0;

        reader.GoToFirstByte(checksum, bDone, args->mStartOffset);

        while (!bDone && reader.GetBlockPosition() <= args->mEndOffset)
        {
            bool bMatchFound = false;
            auto blockVector = args->mBlockInfoMap.find(checksum);

            const bool checksumMatch = (blockVector != args->mBlockInfoMap.end());
            ...
            if (checksumMatch)
            {
                ...
                for (const auto& block : blockVector->second)
                {
                    if (0 == memcmp(localHashValueBrief.mData, block.hash, sizeof(uint8_t) * 8)) // If the MD5-based hash matches, then this block is 99.99999+% certain to match the patch block.
                    {
                        ...
                        const uint64_t patchPosition = block.position;

                        // https://developer.origin.com/browse/EAXPRODUCT-70703 Apply changes made for EADesktop in Origin
                        // https://developer.origin.com/browse/EAXPRODUCT-70178 Fix diff patching assert when local segment selected that goes beyond end of local file
                        const uint64_t maxRemotePatchSize = ((patchPosition + diffData.mBlockSize) < diffData.mFileSize) ? diffData.mBlockSize : (diffData.mFileSize - patchPosition);
                        // When using this info later to build the destination files in PatchBuilder::BuildDestinationFiles via FileSpanCopier::CopySpan, can only copy bytes from the local
                        // file that actually exist. There is a possibility we've read the last block in localFile, it is smaller than blockSize, and the 0's that get initialized for the hash
                        // end up matching the patch file. In this situation, we need to limit the patchSize to only what the local file can provide.
                        const uint64_t maxLocalPatchSize = ((localPosition + diffData.mBlockSize) < bfi->mLocalFileSize) ? diffData.mBlockSize : (bfi->mLocalFileSize - localPosition);
                        const uint64_t patchSize = (maxRemotePatchSize <= maxLocalPatchSize) ? maxRemotePatchSize : maxLocalPatchSize;
...
#endif
                        results.push_back({ localPosition, patchPosition, patchSize });
            ...
            if (checksumMatch && !bMatchFound)
                ++retryCount;

            if (bMatchFound || retryCount > kMaxRetries)
            {
            ...
            // Every N bytes (e.g. ~100MB), generate a notification message about the progress of this file.
            uint64_t byteProgress = 0;

            if (reader.GetBlockPosition() > args->mStartOffset)
                byteProgress = reader.GetBlockPosition() - args->mStartOffset;

            if ((byteProgress > 0) && ((byteProgress - lastByteProgress) >= 100 * 1024 * 1024))
            {
                if (pEventCallback)
                {
                    EA::Thread::AutoFutex lock(args->mFutext);
                    pEventCallback->GenerateBuiltFileInfoProgress(diffData, *bfi, args->mError, args->mUserContext, byteProgress - lastByteProgress);
                }
    ...
    {
        EA::Thread::AutoFutex lock(args->mFutext);
        bfi->mLocalFilePath.clear();
    }

    {
        EA::Thread::AutoFutex lock(args->mResultFlushFutex);
        for (const auto& info : results) //There could be some repeated results but we just overwrite them if found
        {
            bfi->InsertLocalSegment(info.localPos, info.patchPos, info.patchSize, diffData.mFileSize);
        }
    }

}

EA::Thread::Futex PatchBuilder::JobArgs::mFutext;
EA::Thread::Futex PatchBuilder::JobArgs::mResultFlushFutex;

struct runnable : public EA::Thread::IRunnable
{
    runnable(const PatchBuilder::JobArgs& args) : mArgs(args) {}
    PatchBuilder::JobArgs mArgs;
    intptr_t Run(void* pContext = NULL)
    {
        auto args = reinterpret_cast<PatchBuilder::JobArgs*>(pContext);
        PatchBuilder::GenerateBuiltFileInfoJob(args);
        return 0;
    }
};
...
bool PatchBuilder::GenerateBuiltFileInfo(const DiffData& diffData, BuiltFileInfo& bfi, Error& error,
                                             GenerateBuiltFileInfoEventCallback* pEventCallback, intptr_t userContext, EA::Thread::ThreadPool* thPool)
{
    ...
    auto blockInfoMap = BlockInfoMap();

    for (eastl_size_t i = 0, iEnd = diffData.mBlockHashArray.size(); i != iEnd; i++)
    {
        const BlockHash&            blockHash = diffData.mBlockHashArray[i];
        BlockInfo block;
        memcpy(block.hash, blockHash.mHashValue.mData, sizeof(uint8_t) * 8);
        block.position = i * diffData.mBlockSize;
        blockInfoMap[blockHash.mChecksum].push_back(std::move(block));
    ...
    //If this checks passes we can assume that the thread reader is going to work fine
    //Unless the user deletes the files while the patching is working
    FileChecksumReader reader;
    if (reader.Open(bfi.mLocalFilePath.c_str(), (size_t)diffData.mBlockSize))
    {
    ...
    EA::Thread::ThreadPool* pool = nullptr;
    bool cleanUp = false;
    auto maxJobCount = 4;
    ...
    {
        pool = thPool;
        maxJobCount = pool->GetThreadCount();
    }
    else
    {
        EA::Thread::ThreadPoolParameters params;
        params.mnInitialCount = maxJobCount;
        params.mDefaultThreadParameters.mpName = "PatchBuilderWorker";
        pool = new EA::Thread::ThreadPool(&params);
        cleanUp = true;
    }

    eastl::vector<runnable> jobs;
    jobs.reserve(maxJobCount);
    const auto jobSize = (diffData.mFileSize / maxJobCount);

    uint64_t end = -1;
    for (auto i = 0; i < maxJobCount; ++i)
    {
        const uint64_t start = (end == -1) ? 0 : end - diffData.mBlockSize; // we want to get the overlap too

        end = i == (maxJobCount - 1) ? ULLONG_MAX : (jobSize * (i + 1)) - ((jobSize * (i + 1)) % diffData.mBlockSize);

        auto arg = JobArgs{ diffData, &bfi, error, userContext, pEventCallback, start, end, blockInfoMap};
        jobs.emplace_back(std::move(arg));
        pool->Begin(&jobs[i], &jobs[i].mArgs, nullptr, true);
    }

    if (EA::Thread::ThreadPool::kResultOK != pool->WaitForJobCompletion())
    {
        EAPATCH_ASSERT_MESSAGE(false, "WaitForJobCompletition Failed");
        return false;
    }
    ...
    EAPatchErrorId eid;
    ErrorClass e;
    error.GetErrorId(e, sid, eid);

    if (e == kECNone)
    {
        bfi.SetSpanCounts(); // This calculates the value of variables such as bfi.mRemoteSpanCount.

        const int MIN_LOCAL_SPAN_SIZE = 65536;

        bfi.RemoveSmallLocalSpans(MIN_LOCAL_SPAN_SIZE);
