...
void PatchBuilder::GenerateBuiltFileInfoJob(JobArgs* args)
{
    ...
    if (reader.Open(bfi->mLocalFilePath.c_str(), (size_t)diffData.mBlockSize))
    {
        ...
        if (pEventCallback)
        {
            //(TODO)As compromise for now we notify how much the worker thread did
            EA::Thread::AutoFutex lock(args->mFutext);
            pEventCallback->GenerateBuiltFileInfoProgress(diffData, *bfi, args->mError, args->mUserContext, reader.GetBlockPosition() - args->mStartOffset);
        }
