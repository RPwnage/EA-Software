...
namespace EA
{
    namespace Patch
    {
        ...
        /// 
        const size_t kDiffBlockShiftMin = 8; // 256 bytes
        const size_t kDiffBlockSizeMin = 1 << kDiffBlockShiftMin; 

        const size_t kDiffBlockShiftMax = 20; // 1M bytes
        const size_t kDiffBlockSizeMax = 1<<kDiffBlockShiftMax; 

        constexpr size_t kHelperBufferSize = 65536 / 2;
        ...
        ///
        const size_t kDiffBlockShiftDefault = 13; // 8192 bytes
        const size_t kDiffBlockSizeDefault   = 1<<kDiffBlockShiftDefault;
        ...
        struct HashValueMD5Brief
        {
            ...
            HashValueMD5Brief();
            explicit HashValueMD5Brief(const uint8_t* data) { memcpy(mData, data, sizeof(uint8_t) * 8); };
            void Clear();
        };
