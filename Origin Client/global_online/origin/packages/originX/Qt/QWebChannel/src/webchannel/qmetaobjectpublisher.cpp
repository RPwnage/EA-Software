...
QJsonObject QMetaObjectPublisher::classInfoForObject(const QObject *object)
{
    ...
    for (int i = 0; i < metaObject->propertyCount(); ++i) {
        ...
        QJsonValue retjson = QJsonValue::fromVariant(prop.read(object));
        if(prop.type() == QMetaType::QVariant)
        {
            QVariant propObj = object->property(propertyName.toLocal8Bit());
                retjson = wrapResult(propObj);
        }

        propertyInfo.append(signalInfo);
        propertyInfo.append(retjson);
...
{   if(result.type() == QVariant::List)
    {
        QVariantList oldList = result.toList();
        QJsonArray newList;
         for (QVariantList::iterator i = oldList.begin(); i != oldList.end(); ++i)
            newList.append(wrapResult(*i));
