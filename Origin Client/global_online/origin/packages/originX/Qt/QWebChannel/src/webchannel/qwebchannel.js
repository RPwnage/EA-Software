...
function QObject(name, data, webChannel)
{
    ...
    {
        if (response === null || !response["__QObject*__"]
    ...
    this.propertyUpdate = function(signals, propertyMap)
    {
        ...
        for (var propertyIndex in propertyMap) {
            ...
            object.__propertyCache__[propertyIndex] = unwrapQObject(propertyValue);
        }
    ...
    {
        if(signalArgs instanceof Array)
        {
            var arrayLength = signalArgs.length;
            for (var i = 0; i < arrayLength; i++) {
                signalArgs[i] = unwrapQObject(signalArgs[i]);
            }
        }
    ...
    function addMethod(methodData)
    {
        ...
        object[methodName] = function() {
            ...
            if ( (response !== undefined) && callback ) {
                var unwrappedResponse = unwrapQObject(response);
                if(unwrappedResponse instanceof Array) {
                    var arrayLength = unwrappedResponse.length;
                    for (var i = 0; i < arrayLength; i++) {
                        unwrappedResponse[i] = unwrapQObject(unwrappedResponse[i]);
                    }
                    (callback).call(context, unwrappedResponse);
                }
            });
        };
    }
    ...
    function bindGetterSetter(propertyInfo)
    {
        ...
        object.__propertyCache__[propertyIndex] = unwrapQObject(propertyInfo[3]);
        ...
        object.__defineSetter__(propertyName, function(value) {
            ...
            object.__propertyCache__[propertyIndex] = unwrapQObject(value);
