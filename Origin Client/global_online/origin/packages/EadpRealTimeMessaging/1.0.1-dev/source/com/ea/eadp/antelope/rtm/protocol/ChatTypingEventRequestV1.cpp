// GENERATED CODE (Source: chat_protocol.proto) - DO NOT EDIT DIRECTLY.
// Copyright(C) 2016-2019 Electronic Arts, Inc. All rights reserved.

#include <com/ea/eadp/antelope/rtm/protocol/ChatTypingEventRequestV1.h>
#include <eadp/foundation/internal/ProtobufWireFormat.h>
#include <eadp/foundation/internal/Utils.h>
#include <eadp/foundation/Hub.h>

namespace com
{

namespace ea
{

namespace eadp
{

namespace antelope
{

namespace rtm
{

namespace protocol
{

ChatTypingEventRequestV1::ChatTypingEventRequestV1(const ::eadp::foundation::Allocator& allocator)
: ::eadp::foundation::Internal::ProtobufMessage(allocator.make<::eadp::foundation::String>("com.ea.eadp.antelope.rtm.protocol.ChatTypingEventRequestV1"))
, m_allocator_(allocator)
, m_cachedByteSize_(0)
, m_channelId(m_allocator_.emptyString())
, m_event(1)
, m_customTypingEvent(m_allocator_.emptyString())
{
}

void ChatTypingEventRequestV1::clear()
{
    m_cachedByteSize_ = 0;
    m_channelId.clear();
    m_event = 1;
    m_customTypingEvent.clear();
}

size_t ChatTypingEventRequestV1::getByteSize()
{
    size_t total_size = 0;
    if (!m_channelId.empty())
    {
        total_size += 1 + ::eadp::foundation::Internal::ProtobufWireFormat::getStringSize(m_channelId);
    }
    if (m_event != 1)
    {
    total_size += 1 + ::eadp::foundation::Internal::ProtobufWireFormat::getEnumSize(m_event);
    }
    if (!m_customTypingEvent.empty())
    {
        total_size += 1 + ::eadp::foundation::Internal::ProtobufWireFormat::getStringSize(m_customTypingEvent);
    }
    m_cachedByteSize_ = total_size;
    return total_size;
}

uint8_t* ChatTypingEventRequestV1::onSerialize(uint8_t* target) const
{
    if (!getChannelId().empty())
    {
        target = ::eadp::foundation::Internal::ProtobufWireFormat::writeString(1, getChannelId(), target);
    }
    if (m_event != 1)
    {
        target = ::eadp::foundation::Internal::ProtobufWireFormat::writeEnum(2, static_cast<int32_t>(getEvent()), target);
    }
    if (!getCustomTypingEvent().empty())
    {
        target = ::eadp::foundation::Internal::ProtobufWireFormat::writeString(3, getCustomTypingEvent(), target);
    }
    return target;
}

bool ChatTypingEventRequestV1::onDeserialize(::eadp::foundation::Internal::ProtobufInputStream* input)
{
    clear();
    uint32_t tag;
    for (;;)
    {
        tag = input->readTag();
        if (tag > 127) goto handle_unusual;
        switch (::eadp::foundation::Internal::ProtobufWireFormat::getTagFieldNumber(tag))
        {
            case 1:
            {
                if (tag == 10)
                {
                    if (!input->readString(&m_channelId))
                    {
                        return false;
                    }
                } else {
                    goto handle_unusual;
                }
                if (input->expectTag(16)) goto parse_event;
                break;
            }

            case 2:
            {
                if (tag == 16)
                {
                    parse_event:
                    int value;
                    if (!input->readEnum(&value))
                    {
                        return false;
                    }
                    setEvent(static_cast<com::ea::eadp::antelope::rtm::protocol::TypingEventType>(value));
                } else {
                    goto handle_unusual;
                }
                if (input->expectTag(26)) goto parse_customTypingEvent;
                break;
            }

            case 3:
            {
                if (tag == 26)
                {
                    parse_customTypingEvent:
                    if (!input->readString(&m_customTypingEvent))
                    {
                        return false;
                    }
                } else {
                    goto handle_unusual;
                }
                if (input->expectAtEnd()) return true;
                break;
            }

            default: {
            handle_unusual:
                if (tag == 0)
                {
                    return true;
                }
                if(!input->skipField(tag)) return false;
                break;
            }
        }
    }
}
::eadp::foundation::String ChatTypingEventRequestV1::toString(const ::eadp::foundation::String& indent) const
{
    ::eadp::foundation::String result = m_allocator_.make<::eadp::foundation::String>();
    result.append(indent);
    result.append("{\n");
    if (!m_channelId.empty())
    {
        ::eadp::foundation::Internal::Utils::appendSprintf(result, "%s  channelId: \"%s\",\n", indent.c_str(), m_channelId.c_str());
    }
    if (m_event != 1)
    {
        ::eadp::foundation::Internal::Utils::appendSprintf(result, "%s  event: %d,\n", indent.c_str(), m_event);
    }
    if (!m_customTypingEvent.empty())
    {
        ::eadp::foundation::Internal::Utils::appendSprintf(result, "%s  customTypingEvent: \"%s\",\n", indent.c_str(), m_customTypingEvent.c_str());
    }
    result.append(indent);
    result.append("}");
    return result;
}

const ::eadp::foundation::String& ChatTypingEventRequestV1::getChannelId() const
{
    return m_channelId;
}

void ChatTypingEventRequestV1::setChannelId(const ::eadp::foundation::String& value)
{
    m_channelId = value;
}

void ChatTypingEventRequestV1::setChannelId(const char8_t* value)
{
    m_channelId = m_allocator_.make<::eadp::foundation::String>(value);
}


com::ea::eadp::antelope::rtm::protocol::TypingEventType ChatTypingEventRequestV1::getEvent() const
{
    return static_cast<com::ea::eadp::antelope::rtm::protocol::TypingEventType>(m_event);
}
void ChatTypingEventRequestV1::setEvent(com::ea::eadp::antelope::rtm::protocol::TypingEventType value)
{
    m_event = static_cast<int>(value);
}

const ::eadp::foundation::String& ChatTypingEventRequestV1::getCustomTypingEvent() const
{
    return m_customTypingEvent;
}

void ChatTypingEventRequestV1::setCustomTypingEvent(const ::eadp::foundation::String& value)
{
    m_customTypingEvent = value;
}

void ChatTypingEventRequestV1::setCustomTypingEvent(const char8_t* value)
{
    m_customTypingEvent = m_allocator_.make<::eadp::foundation::String>(value);
}


}
}
}
}
}
}
