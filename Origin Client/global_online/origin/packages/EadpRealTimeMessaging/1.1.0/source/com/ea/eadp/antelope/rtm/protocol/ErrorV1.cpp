...
size_t ErrorV1::getByteSize()
{
    ...
    if (hasLoginError() && m_body.m_loginError)
    {
        total_size += 1 + ::eadp::foundation::Internal::ProtobufWireFormat::getMessageSize(*m_body.m_loginError);
    }
...
uint8_t* ErrorV1::onSerialize(uint8_t* target) const
{
    ...
    if (hasLoginError() && m_body.m_loginError)
    {
        target = ::eadp::foundation::Internal::ProtobufWireFormat::writeMessage(7, *m_body.m_loginError, target);
    }
    return target;
}
    ...
    for (;;)
    {
        ...
        switch (::eadp::foundation::Internal::ProtobufWireFormat::getTagFieldNumber(tag))
        {
            ...
            case 6:
            {
                ...
                if (input->expectTag(58)) goto parse_loginError;
                break;
            }
            ...
            case 7:
            {
                if (tag == 58)
                {
                    parse_loginError:
                    clearBody();
                    if (!input->readMessage(getLoginError().get()))
                    {
                        return false;
                    }
                } else {
...
::eadp::foundation::String ErrorV1::toString(const ::eadp::foundation::String& indent) const
{
    ...
    if (hasLoginError() && m_body.m_loginError)
    {
        ::eadp::foundation::Internal::Utils::appendSprintf(result, "%s  loginError: %s,\n", indent.c_str(), m_body.m_loginError->toString(indent + "  ").c_str());
    }
    result.append(indent);
    result.append("}");
    return result;
...
::eadp::foundation::SharedPtr<com::ea::eadp::antelope::rtm::protocol::LoginErrorV1> ErrorV1::getLoginError()
{
    if (!hasLoginError())
    {
        setLoginError(m_allocator_.makeShared<com::ea::eadp::antelope::rtm::protocol::LoginErrorV1>(m_allocator_));
    }
    return m_body.m_loginError;
}

bool ErrorV1::hasLoginError() const
{
    return m_bodyCase == BodyCase::kLoginError;
}

void ErrorV1::setLoginError(::eadp::foundation::SharedPtr<com::ea::eadp::antelope::rtm::protocol::LoginErrorV1> value)
{
    clearBody();
    new(&m_body.m_loginError) ::eadp::foundation::SharedPtr<com::ea::eadp::antelope::rtm::protocol::LoginErrorV1>(value);
    m_bodyCase = BodyCase::kLoginError;
}

void ErrorV1::releaseLoginError()
{
    if (hasLoginError())
    {
        m_body.m_loginError.reset();
        m_bodyCase = BodyCase::kNotSet;
    }
}
