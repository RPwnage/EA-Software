...
, m_forceDisconnectSessionKey(allocator.emptyString())
, m_reconnectSessionKey(allocator.emptyString())
{
}
...
void ConnectionServiceImpl::connect(const String& accessToken, const String& clientVersion, ConnectCallback connectCallback, DisconnectCallback disconnectCallback, bool forceDisconnect)
{
    auto connectJob = m_hub->getAllocator().makeUnique<ConnectionJob>(m_hub.get(), shared_from_this(), ConnectionJob::Type::CONNECT, accessToken, clientVersion, connectCallback, nullptr, disconnectCallback, nullptr, forceDisconnect, m_hub->getAllocator().emptyString(), m_hub->getAllocator().emptyString());
...
void ConnectionServiceImpl::connect(UniquePtr<ConnectionService::MultiConnectionRequest> request)
{
    auto connectJob = m_hub->getAllocator().makeUnique<ConnectionJob>(m_hub.get()
                                                                      ...
                                                                      , request->m_forceDisconnectSessionKey
                                                                      , m_hub->getAllocator().emptyString()); // not a reconnect, so pass an empty string
...
void ConnectionServiceImpl::reconnect(const String& accessToken, const String& clientVersion, ConnectCallback connectCallback, DisconnectCallback disconnectCallback)
{
    auto reconnectJob = m_hub->getAllocator().makeUnique<ConnectionJob>(m_hub.get(), shared_from_this(), ConnectionJob::Type::RECONNECT, accessToken, clientVersion, connectCallback, nullptr, disconnectCallback, nullptr, true, m_hub->getAllocator().emptyString(), m_hub->getAllocator().emptyString());
...
void ConnectionServiceImpl::reconnect(UniquePtr<ConnectionService::MultiConnectionRequest> request)
{
    auto reconnectJob = m_hub->getAllocator().makeUnique<ConnectionJob>(m_hub.get()
                                                                        ...
                                                                        , request->m_forceDisconnect
                                                                        , request->m_forceDisconnectSessionKey
                                                                        , request->m_reconnectSessionKey);

    m_hub->addJob(EADPSDK_MOVE(reconnectJob));
}

void ConnectionServiceImpl::getSessions(GetSessionsCallback callback)
{
...
{
    auto disconnectJob = m_hub->getAllocator().makeUnique<ConnectionJob>(m_hub.get(), shared_from_this());
    m_hub->addJob(EADPSDK_MOVE(disconnectJob));
}

void ConnectionServiceImpl::cleanupSession(const String& sessionKey, CleanupSessionCallback callback)
{
