...
public:
     ...
     * This callback is invoked if the session limit is reached with code RealTimeMessagingError::Code::SESSION_LIMIT_REACHED.
     *
     * See RealTimeMessagingError for RTM related error codes.
     */
    ...
    /*!
     * @brief SessionRequestCallback is triggered for multi-sessions when a user logs in or logs out.
     *
     * @param sessionResponse The currently connected sessions.
     * @param error Error object
     */
    using SessionRequestCallback = eadp::foundation::CallbackT<void(eadp::foundation::SharedPtr<com::ea::eadp::antelope::rtm::protocol::SessionResponseV1> sessionResponse, const eadp::foundation::ErrorPtr& error) > ;
    
    /*!
     * @brief SessionCleanupCallback is triggered when the session cleanup request is completed.
     *
     * @param error Error object
     */
    using SessionCleanupCallback = eadp::foundation::CallbackT<void(const eadp::foundation::ErrorPtr& error) >;
    ...
    struct EADPREALTIMEMESSAGING_API MultiConnectionRequest
    {
        ...
        bool m_forceDisconnect;                                 /*!< If True, disconnect the session associated with the sessionKey parameter. If false, the connect may fail with error code SESSION_LIMIT_REACHED. */
        eadp::foundation::String m_sessionKey;                  /*!<  The sessionKey to disconnect if forceDisconnect is true. */
    };
     ...
     */
    virtual void connect(eadp::foundation::String accessToken, eadp::foundation::String clientVersion, ConnectCallback connectCallback, DisconnectCallback disconnectCallback, bool forceDisconnect) = 0;

    /*!
     * @brief Starts a connected with backend server using the multiple-sessions supported API (LoginRequestV3)
     *
     ...
     * @param disconnectCallback Triggered when there is a disconnection
     */
    virtual void reconnect(eadp::foundation::String accessToken, eadp::foundation::String clientVersion, ConnectCallback connectCallback, DisconnectCallback disconnectCallback) = 0;
     ...
     */
    virtual void getSessions(SessionRequestCallback callback) = 0;
     ...
     * Should only be called when connected.
     *
     * @param sessionKey The session identifier for the session to log out.
     */
    virtual void cleanupSession(const eadp::foundation::String& sessionKey, SessionCleanupCallback callback) = 0;
