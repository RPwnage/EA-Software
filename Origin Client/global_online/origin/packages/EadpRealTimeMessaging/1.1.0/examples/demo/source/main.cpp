...
public:
    RTMServiceIntegrator(const FoundationIntegrator& foundation)
        ...
        , m_currentSessionKey(foundation.getHub()->getAllocator().emptyString())
        , m_isDone(false)
    {
...
private:
    ...
    String m_currentSessionKey; // populated after connection
    bool m_isDone;
};
...
void RTMServiceIntegrator::onConnect(eadp::foundation::SharedPtr<com::ea::eadp::antelope::rtm::protocol::LoginV3Response> loginResponse, eadp::foundation::SharedPtr<com::ea::eadp::antelope::rtm::protocol::LoginErrorV1> loginError, const ErrorPtr& error)
{
    if (error)
    {
        if (loginError)
        {
            std::cout << "[Connection Service] SESSION_LIMIT_EXCEEDED." << std::endl;
        }
    ...
    m_currentSessionKey = loginResponse->getSessionKey();
...
void RTMServiceIntegrator::getConnectedSessions()
{
    ...
    m_connectionService->getSessions(eadp::realtimemessaging::ConnectionService::GetSessionsCallback(getSessionsCallback, m_persistenceObject, RTM_GROUP_ID));
}
...
void RTMServiceIntegrator::disconnectRTM()
{
    ...
    // the disconnect callback will set m_Done = true and end the demo
}
