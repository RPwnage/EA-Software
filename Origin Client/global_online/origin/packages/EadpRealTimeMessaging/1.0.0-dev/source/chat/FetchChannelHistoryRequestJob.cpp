...
using FetchChannelHistoryResponse = ResponseT<const String&,
    ...
    const ErrorPtr&>;
constexpr char kFetchChannelHistoryResponseJobName[] = "FetchChannelHistoryResponseJob";
...
void FetchChannelHistoryRequestJob::setupOutgoingCommunication(const eadp::foundation::String& requestId)
{
    ...
    if (m_channelId.empty())
    {
        ...
        EADPSDK_LOGE(getHub(), "FetchChannelHistoryRequest", message);
        auto error = FoundationError::create(getHub()->getAllocator(), FoundationError::Code::INVALID_ARGUMENT, message);

        FetchChannelHistoryResponse::post(getHub(), kFetchChannelHistoryResponseJobName, m_callback, m_channelId, nullptr, error);
        return;
    }
...
void FetchChannelHistoryRequestJob::onComplete(SharedPtr<CommunicationWrapper> communicationWrapper)
{
    if (communicationWrapper->hasSocialCommunication())
    {
        ...
        if (communication->hasHistoryResponse())
        {
            if (communication->getHistoryResponse()->getSuccess())
            {
                EADPSDK_LOGV(getHub(), "FetchChannelHistoryRequest", "Fetch Channel History successful");
                FetchChannelHistoryResponse::post(getHub(), kFetchChannelHistoryResponseJobName, m_callback, m_channelId, getHub()->getAllocator().makeUnique<Vector<SharedPtr<protocol::ChannelMessage>>>(communication->getHistoryResponse()->getMessages()), nullptr);
            }
            else
            {
                String message = getHub()->getAllocator().make<String>("Error received from server in response to Fetch Channel History request.");
                if (communication->getHistoryResponse()->getErrorCode().length() > 0)
                {
                    eadp::foundation::Internal::Utils::appendSprintf(message, "\nErrorCode: %s", communication->getHistoryResponse()->getErrorCode().c_str());
                }
                if (communication->getHistoryResponse()->getErrorMessage().length() > 0)
                {
                    eadp::foundation::Internal::Utils::appendSprintf(message, "\nReason: %s", communication->getHistoryResponse()->getErrorMessage().c_str());
                }

                EADPSDK_LOGE(getHub(), "FetchChannelHistoryRequest", message);

                auto error = RealTimeMessagingError::create(getHub()->getAllocator(), communication->getHistoryResponse()->getErrorCode(), message);
