...
constexpr char kPresenceUpdateJobName[] = "PresenceUpdateJob";

PresenceUpdateJob::PresenceUpdateJob(IHub* hub, SharedPtr<eadp::realtimemessaging::Internal::RTMService> rtmService, SharedPtr<rtm::protocol::PresenceUpdateV1> request, PresenceService::ErrorCallback errorCallback)
...
void PresenceUpdateJob::onSendError(const ErrorPtr& error)
{
    PresenceResponse::post(getHub(), kPresenceUpdateJobName, m_errorCallback, error);
}

void PresenceUpdateJob::setupOutgoingCommunication()
{
    ...
    v1->setPresenceUpdate(m_request);
