...
public:
    ...
    void initializeRTM(const String& accessToken);
    void initializeChat(); 
...
void RTMServiceIntegrator::initializeRTM(const String& accessToken)
{
    ...
    m_connectionService->connect(accessToken,
...
int main(int argc, char *argv[])
{
    ...
    // EADP SDK does not have any MacOS authentication mechanisms. The access
    // token must be provided manually.
    if (argc < 2)
    {
        std::cout << " No access token provided. Ending MacOS demo.";
        return 0;
    }
    
    const char* accessToken = argv[1];
    rtmIntegrator.initializeRTM(accessToken);
