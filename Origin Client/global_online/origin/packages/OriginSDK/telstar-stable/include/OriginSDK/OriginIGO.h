...
/// \ingroup profile
/// \brief Log out of a profile.
///
/// This will log out the secondary user.
/// \param userIndex The user to log out. Valid values are [1..3]
/// \param [in] callback The callback to be called when the IGO is shown. Can be NULL.
/// \param [in] pContext a user-defined context to be used in the callback
/// \return returns ORIGIN_SUCCESS. 
...
/// &nbsp;&nbsp;&nbsp;&nbsp;\ref ORIGIN_IGO_BROWSER_APP&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Creates a browser container for a single page app. No URL, no tabs, full screen.<br/>
...
/// \ingroup igo
/// \brief Bring up the up-sell UI
/// As this API is data driven, do not hard code anything that requires a client patch to fix.
/// \param [in] user The local user. Use OriginGetDefaultUser().
/// \param [in] pType The up-sell type to show, e.g. 'WEB', 'STORE', 'ADDONS' etc. 
/// \param [in] pUri The item to show. The URI can be a masterTitleId for upselling to a (next) game, or it can be a store path like: 'origin-access/refer-a-friend' to upsell to the friend referral program.
/// \param [in] pParams An optional list of modifications to the item to show.
/// \param [in] callback The callback to be called when the IGO is shown. Can be NULL.
/// \param [in] pContext a user-defined context to be used in the callback. Can be NULL.
