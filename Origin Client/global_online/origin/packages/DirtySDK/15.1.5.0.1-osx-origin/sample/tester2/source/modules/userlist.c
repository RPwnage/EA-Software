...
static void _CmdUlaCb(UserListApiRefT *pRef, UserListApiReturnTypeE eResponseType, UserListApiEventDataT *pUserApiEventData, void *pUserData)
{
    ...
    if (eResponseType == TYPE_USER_DATA)
    {
        ...
        {
#if defined(DIRTYCODE_PS4)
            DirtyUserToNativeUser(&AcountId, sizeof(AcountId), &pUserApiEventData->UserData.DirtyUser);
            ZPrintf("Friend: online id: %llu, gamertag %s\n", AcountId, pUserApiEventData->UserData.ExtendedUserData.Profile.strGamertag);
#elif defined (DIRTYCODE_XBOXONE)
...
#if defined(DIRTYCODE_PS4)
        AcountId = ds_strtoull(argv[3], NULL, 10);
        DirtyUserFromNativeUser(&User, &AcountId);
#elif defined (DIRTYCODE_XBOXONE)
