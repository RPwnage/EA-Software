...
static void _ChaCha20Block(uint8_t *pOutput, const uint32_t *pInput)
{
    ...
    for (iBlock = 0; iBlock < 16; iBlock += 1)
    {
        aBlock[iBlock] = CHACHA_PLUS(aBlock[iBlock], pInput[iBlock]);
    }

    // write output
    for (iBlock = 0; iBlock < 16; iBlock += 1)
    {
        CHACHA_WRWORD(pOutput + (4*iBlock), aBlock[iBlock]);
    }
}
...
/*!
    ...
    \Description
        Encrypt input data in place and generate authentication tag
...
/*!
    ...
    \Description
        Decrypt input data in place, generate authentication tag, and verify
...
int32_t CryptChaChaDecrypt(CryptChaChaT *pChaCha, uint8_t *pBuffer, int32_t iLength, const uint8_t *pInitVec, int32_t iIvLen, const uint8_t *pAddData, int32_t iAddLen, const uint8_t *pTag, int32_t iTagLen)
{
    ...
    return(!memcmp(pTag, aTag, iTagLen) ? iLength : -1);
}
