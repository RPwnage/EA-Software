...
static int32_t _NetConnQueryNpUser(NetConnRefT *pRef)
{
    ...
    for (iLocalUserIndex = 0; iLocalUserIndex < NETCONN_MAXLOCALUSERS; ++iLocalUserIndex)
    {
        if (pRef->aUsers[iLocalUserIndex].NpAvailability.bDone == TRUE && pRef->aUsers[iLocalUserIndex].NpAvailability.iResult == SCE_OK)
        {
            // return the first
...
static int32_t _NetConnStartQueryNpUser(NetConnRefT *pRef)
{
    ...
    for (iLocalUserIndex = 0; iLocalUserIndex < NETCONN_MAXLOCALUSERS; ++iLocalUserIndex)
    {
        ...
        if (iRequest > 0)
        {
            ...
            if (!((userId = NetConnStatus('suid', iLocalUserIndex, NULL, 0)) < 0))
            {
                if ((iResult = sceNpCheckNpAvailabilityA(iRequest, userId)) == SCE_OK)
                {
                    ...
                    pRef->aUsers[iLocalUserIndex].NpAvailability.iRequestId = iRequest;
                    pRef->aUsers[iLocalUserIndex].NpAvailability.iResult = 0;
                    pRef->aUsers[iLocalUserIndex].NpAvailability.bDone = FALSE;
                    bOneStarted = true;
                }
                else
                {
                    pRef->aUsers[iLocalUserIndex].NpAvailability.iRequestId = 0;
                    pRef->aUsers[iLocalUserIndex].NpAvailability.iResult = iResult;
                    pRef->aUsers[iLocalUserIndex].NpAvailability.bDone = TRUE;
        ...
        {
            pRef->aUsers[iLocalUserIndex].NpAvailability.iRequestId = 0;
            pRef->aUsers[iLocalUserIndex].NpAvailability.iResult = iRequest;
            pRef->aUsers[iLocalUserIndex].NpAvailability.bDone = TRUE;
            NetPrintf(("netconnps4: local user at index %d failed to create a NpRequest\n", iLocalUserIndex));
        }
    }
...
static void _NetConnQueryNpUserIdle(NetConnRefT *pRef)
{
    ...
    for (iLocalUserIndex = 0; iLocalUserIndex < NETCONN_MAXLOCALUSERS; ++iLocalUserIndex)
    {
        if(pRef->aUsers[iLocalUserIndex].NpAvailability.bDone == TRUE)
        {
            continue;
        }

        iReturn = sceNpPollAsync(pRef->aUsers[iLocalUserIndex].NpAvailability.iRequestId, &iResult);
        ...
        else if (iReturn == SCE_NP_POLL_ASYNC_RET_FINISHED)
        {
            ...
            pRef->aUsers[iLocalUserIndex].NpAvailability.bDone = TRUE;
            pRef->aUsers[iLocalUserIndex].NpAvailability.iResult = iResult;
            sceNpDeleteRequest(pRef->aUsers[iLocalUserIndex].NpAvailability.iRequestId);
        }
        else
        {
            ...
            pRef->aUsers[iLocalUserIndex].NpAvailability.bDone = TRUE;
            pRef->aUsers[iLocalUserIndex].NpAvailability.iResult = iReturn;
            sceNpDeleteRequest(pRef->aUsers[iLocalUserIndex].NpAvailability.iRequestId);
        }
    }
...
int32_t NetConnDisconnect(void)
{
    ...
    for (iLocalUserIndex = 0; iLocalUserIndex < NETCONN_MAXLOCALUSERS; ++iLocalUserIndex)
    {
        ...
        // npavailability
        if (pRef->aUsers[iLocalUserIndex].NpAvailability.iRequestId != 0)
        {
            sceNpDeleteRequest(pRef->aUsers[iLocalUserIndex].NpAvailability.iRequestId);
        }
        pRef->aUsers[iLocalUserIndex].NpAvailability.iRequestId = 0;
...
int32_t NetConnStatus(int32_t iKind, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    if (iKind == 'chav')
    {
        ...
        {
            if (pRef->aUsers[iData].NpAvailability.bDone == TRUE)
            {
