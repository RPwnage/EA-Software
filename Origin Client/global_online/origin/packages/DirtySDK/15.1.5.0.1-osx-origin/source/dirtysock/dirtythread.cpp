...
typedef struct DirtyThreadT
{
    ...
    int32_t iVerbosity;                     //!< logging verbosity
} DirtyThreadT;
...
/*F**************************************************************************************/
/*!
    \Function _DirtyThreadLogEvent

    \Description
        Helper to log the enter/exit events for a thread

    \Input *pThread - thread state
    \Input *pEvent  - string representation of the event

    \Version 09/28/2017 (eesponda)
*/
/***************************************************************************************F*/
static void _DirtyThreadLogEvent(DirtyThreadT *pThread, const char *pEvent)
{
    #if DIRTYCODE_LOGGING
    const char *pName = EA::Thread::GetThreadName();
    const char *pThreadId = EAThreadThreadIdToString(EA::Thread::GetThreadId());
    const char *pSysThreadId = EAThreadSysThreadIdToString(EA::Thread::GetSysThreadId());

    if (pThread->pUserData != NULL)
    {
        NetPrintfVerbose((pThread->iVerbosity, 0, "dirtythread: [%p] %s %s (thid=%s, sysid=%s)\n", pThread->pUserData, pName,
            pEvent, pThreadId, pSysThreadId));
    }
    else
    {
        NetPrintfVerbose((pThread->iVerbosity, 0, "dirtythread: %s %s (thid=%s, sysid=%s)\n", pName, pEvent,
            pThreadId, pSysThreadId));
    }
    #endif
}
...
static intptr_t _DirtyThreadWrapper(void *pUserData)
{
    ...
    _DirtyThreadLogEvent(pThread, "thread started");
    pThread->pFunction(pThread->pUserData);
    _DirtyThreadLogEvent(pThread, "thread exited");

    // clean up and return
    DirtyMemFree(pThread, DIRTYTHREAD_MEMID, pThread->iMemGroup, pThread->pMemGroupUserData);

    /* there is a bug in eathread that can cause thread end to put things in a bad state
       this later causes asserts in eathread. given this we don't call the function unless
       we have the fix in place. */
    #if EATHREAD_VERSION_N > 13204
...
int32_t DirtyThreadCreate(DirtyRunnableFunctionT *pFunction, void *pUserData, const DirtyThreadConfigT *pConfig)
{
    ...
    pThread->iVerbosity = pConfig->iVerbosity;
