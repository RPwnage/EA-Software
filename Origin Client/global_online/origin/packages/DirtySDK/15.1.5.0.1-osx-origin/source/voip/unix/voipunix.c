...
void VoipSetLocalUser(VoipRefT *pVoip, int32_t iLocalUserIndex, uint32_t bRegister)
{
    if (bRegister)
    {
        if (VOIP_NullUser(&pVoip->Common.Connectionlist.LocalUsers[0]))
        {
            ...
            NetPrintf(("voipunix: registering a local user %s\n", voipUser.strUserId));
            VOIP_CopyUser(&pVoip->Common.Connectionlist.LocalUsers[0], &voipUser);
        }
        else
        {
            NetPrintf(("voipunix: warning -- ignoring attempt to register local user because user %s is already registered\n", pVoip->Common.Connectionlist.LocalUsers[0].strUserId));
        }
    }
    else
    {
        if (!VOIP_NullUser(&pVoip->Common.Connectionlist.LocalUsers[0]))
        {
            ...
            NetPrintf(("voipunix: unregistering local user %s\n", pVoip->Common.Connectionlist.LocalUsers[0].strUserId));
...
void VoipActivateLocalUser(VoipRefT *pVoip, int32_t iLocalUserIndex, uint8_t bActivate)
{
    if (!VOIP_NullUser(&pVoip->Common.Connectionlist.LocalUsers[0]))
    {
        if (bActivate == TRUE)
        {
            NetPrintf(("voipunix: promoting user %s to participating state\n", &pVoip->Common.Connectionlist.LocalUsers[0].strUserId));
        ...
        {
            NetPrintf(("voipunix: demoting user %s from participating state\n", &pVoip->Common.Connectionlist.LocalUsers[0].strUserId));
    ...
    {
        NetPrintf(("voipunix: VoipActivateLocalUser() failed because no local user registered\n"));
    }
}
