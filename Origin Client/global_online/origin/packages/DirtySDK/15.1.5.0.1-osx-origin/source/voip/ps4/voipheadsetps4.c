...
typedef struct
{
    ...
    uint32_t uPcmPortId;            //!< id of the IN_PCMDEVICE port associated with this local user
    ...
    int8_t iNarrating;              //!< narration status; zero=inactive, one=narrating, negative=doneTRUE if narration is in progress, else FALSE
    ...
    uint8_t aAudioData[8*1024];     //!< buffer up to 4k samples (~256ms at 16khz)
} PS4LocalVoipUserT;
...
/*!
    ...
    \Input *pLocalUser  - local user we're creating transcription for
...
*/
/********************************************************************************F*/
static void _VoipHeadsetTranscribeCheckCreate(VoipHeadsetRefT *pHeadset, PS4LocalVoipUserT *pLocalUser)
{
    ...
    if ((pLocalUser->pTranscribeRef = VoipTranscribeCreate(0)) != NULL)
    {
...
static int32_t _VoipHeadsetNarrateCb(VoipNarrateRefT *pNarrateRef, int32_t iUserIndex, const int16_t *pSamples, int32_t iSize, void *pUserData)
{
    ...
    else if (iSize == VOIPNARRATE_STREAM_START)
    {
        ...
        _VoipHeadsetTranscribeCheckCreate(pHeadset, pLocalUser);
...
/*!
    ...
    \Input *pLocalUser  - local user info
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetPcmPortCreate(PS4LocalVoipUserT *pLocalUser)
{
    ...
    sceVoicePortParam.pcmaudio.bufSize = 2*1024;

    if ((iResult = sceVoiceCreatePort(&pLocalUser->uPcmPortId, &sceVoicePortParam)) == SCE_OK)
    {
        NetPrintf(("voipheadsetps4: created IN_PCMAUDIO port %d for user index %d (user ID = %d/0x%08x)\n", pLocalUser->uPcmPortId, pLocalUser->iLocalUserIndex, pLocalUser->sceUserId, pLocalUser->sceUserId));
    }
    else
    {
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetIsHeadsetDetected(uint32_t uInPortId, uint8_t *pHeadsetDetected)
{
    int32_t iResult;

    if ((iResult = sceAudioInGetSilentState(uInPortId)) >= 0)
    {
        // consider headset detected if data is available, or if the device reports as available but muted
        *pHeadsetDetected = ((iResult == 0) || (iResult == SCE_AUDIO_IN_SILENT_STATE_USER_SETTING)) ? TRUE : FALSE;
        iResult = 0;
    }
    else
    {
    ...
    if (pVoipUser)
    {
        ...
        if (!VOIP_SameUser(pVoipUser, &pLocalUser->user))
        {
            NetPrintf(("voipheadsetps4: deleting local talker %s failed because it does not match user at index %d (%s)\n",
                pVoipUser->strUserId, iLocalUserIndex, pLocalUser->user.strUserId));
            return(-1);
        }
    }
...
static int32_t _VoipHeadsetAddLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, VoipUserT *pVoipUser)
{
    ...
    {
        NetPrintf(("voipheadsetps4: can't add NULL local voip user at local user index %d\n", pVoipUser->strUserId, iLocalUserIndex));
        return(-1);
    }
    VOIP_CopyUser(&pHeadset->aLocalUsers[iLocalUserIndex].user, pVoipUser);
    ...
    // create pcm port that we will push mic audio into
    _VoipHeadsetPcmPortCreate(pLocalUser);
...
{
    uint8_t aRawAudioBuffer[VOIPHEADSET_FRAMESIZE], aEncAudioBuffer[128];
    int32_t iLocalUserIndex, iFrameBytes, iFrameBytesUsed;
    PS4LocalVoipUserT *pLocalUser;
    uint8_t bCameraMic;

    // see if a camera is connected
    ...
    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex += 1)
    {
        ...
        while ((iFrameBytes = iFrameBytesUsed = _VoipHeadsetReadAudioInput(pLocalUser, aRawAudioBuffer, VOIPHEADSET_FRAMESIZE)) > 0)
        {
            ...
            {
                // feed audio input to the encoder, unless we're doing text chat accessibility and not narrating
                if (!pHeadset->bTextChatAccessibility || (pLocalUser->iNarrating != 0))
                {
                    iFrameBytesUsed = _VoipHeadsetWriteToPort(pLocalUser->uPcmPortId, aRawAudioBuffer, iFrameBytes);
                }
                // transcribe audio if enabled. if we are doing text chat accessibility don't transcribe narrated audio
                if (pHeadset->bVoiceTranscriptionEnabled && (!pHeadset->bTextChatAccessibility || (pLocalUser->iNarrating == 0)))
                {
                    // the first time the TranscribeRef is used we will create it, then it will persist till headset destruction
                    _VoipHeadsetTranscribeCheckCreate(pHeadset, pLocalUser);

                    // submit data for transcription
                    if (pLocalUser->pTranscribeRef != NULL)
                    {
                        VoipTranscribeSubmit(pLocalUser->pTranscribeRef, aRawAudioBuffer, iFrameBytesUsed);
                    }
                }
            }
        ...
        while ((iFrameBytes = _VoipHeadsetIsLocalVoiceFrameReady(pLocalUser->uEncoderPortId)) > 0)
        {
            ...
            if (_VoipHeadsetReadFromPort(pHeadset->aLocalUsers[iLocalUserIndex].uEncoderPortId, aEncAudioBuffer, (uint32_t *)&iFrameBytes) == 0)
            {
                if (!pHeadset->bLoopback)
                {
                    int32_t iUserIndex;
                    // set user index
                    iUserIndex = bCameraMic ? VOIP_SHARED_USER_INDEX : iLocalUserIndex;
                    ...
                    */
                    if (pHeadset->bSharedPrivilege)
                    {
                        ...
                        pHeadset->pMicDataCb(aEncAudioBuffer, iFrameBytes, NULL, 0, iUserIndex, pHeadset->uSendSeq++, pHeadset->pCbUserData);
                    }
                }
                else
                {
                    ...
                    if (iDecoderIndex != VOIPHEADSET_INVALID_INDEX)
                    {
                        ...
                        _VoipHeadsetWriteToPort(pHeadset->pDecoders[iDecoderIndex].uDecoderPortId, aEncAudioBuffer, iFrameBytes);
        ...
        // if a camera is connected, data from all local users will be the same, so we need to process only one user
        if (bCameraMic)
        {
            break;
        }
...
void VoipHeadsetDestroy(VoipHeadsetRefT *pHeadset)
{
    ...
    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; ++iLocalUserIndex)
    {
        ...
        if (!VOIP_NullUser(&pLocalUser->user))
        {
            ...
            {
                _VoipHeadsetDeactivateLocalTalker(pHeadset, iLocalUserIndex, &pLocalUser->user);
            }
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'voic' - set voice config for a local user; iValue=local user index, pValue=pointer to VoipSynthesizedSpeechCfgT
...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'aloc')
    {
        return(iValue2 ? _VoipHeadsetActivateLocalTalker(pHeadset, iValue, (VoipUserT *)pValue) : _VoipHeadsetDeactivateLocalTalker(pHeadset, iValue, (VoipUserT *)pValue));
    }
    if (iControl == 'ebps')
    {
