    ...
    if ((iLocalUserIndex < VOIP_MAXLOCALUSERS) && (iLocalUserIndex >= 0))
    {
        ...
        if (bRegister)
        {
            if (VOIP_NullUser(&pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex]))
            {
                ...
                ds_snzprintf(voipUser.strUserId, sizeof(voipUser.strUserId), "%s", NetConnMAC());

                NetPrintf(("voippc: registering a local user %s at index %d\n", voipUser.strUserId, iLocalUserIndex));
                VOIP_CopyUser(&pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex], &voipUser);
            }
            else
            {
                NetPrintf(("voippc: warning -- ignoring attempt to register user index %d because user %s is already registered at that index\n",
    ...
    if ((iLocalUserIndex < NETCONN_MAXLOCALUSERS) && (iLocalUserIndex >= 0))
    {
        ...
        if (!VOIP_NullUser(&pConnectionlist->LocalUsers[iLocalUserIndex]))
        {
            if (bActivate == TRUE)
            {
                ...
                {
                    NetPrintf(("voippc: promoting local user %s to participating state.\n", &pConnectionlist->LocalUsers[iLocalUserIndex].strUserId));
                    ...
                    VoipHeadsetControl(pVoip->Common.pHeadset, 'aloc', iLocalUserIndex, TRUE, &pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex]);
                }
                else
                {
                    NetPrintf(("voippc: a user is already active, %s to ready state.\n", &pConnectionlist->LocalUsers[iLocalUserIndex].strUserId));
                }
                pVoip->aIsReadyToParticipate[iLocalUserIndex] = TRUE;
            }
            else
            {
                NetPrintf(("voippc: demoting local user %s from participating/ready state\n", &pConnectionlist->LocalUsers[iLocalUserIndex].strUserId));
                ...
                if (_VoipGetActiveParticipantIndex(pVoip) == VOIP_INVALID_LOCAL_USER_INDEX)
                {
                    ...
                    if (_VoipGetFirstReadyParticipantIndex(pVoip) != VOIP_INVALID_LOCAL_USER_INDEX)
                    {
                        NetPrintf(("voippc: promoting local user %s to participating state\n", &pConnectionlist->LocalUsers[iLocalUserIndex].strUserId));
                        pVoip->Common.Connectionlist.aIsParticipating[iLocalUserIndex] = TRUE;
                        // reapply playback muting config based on the channel configuration only if this is a participating user
                    ...
                    {
                        NetPrintf(("voippc: there are no active users.\n"));
                        VoipHeadsetControl(pVoip->Common.pHeadset, 'aloc', iLocalUserIndex, FALSE, &pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex]);
                    }
                }
            }
        }
