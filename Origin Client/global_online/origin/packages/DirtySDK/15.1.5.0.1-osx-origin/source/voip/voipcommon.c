...
static void _VoipCommonTextDataCb(const char *pStrUtf8, uint32_t uLocalUserIndex, void *pUserData)
{
    ...
    /* We also want locally generated transcribed text shown to its originator if that local user has
       ...
       ** Not working on XBoxOne **: Because transcribed text in xboxone is an "opaque data frame",
       it is not routed through _VoipCommonTextDataCb(), but through _VoipCommonOpaqueDataCb().
       Because we have no visibility on whether the opaque data is text, or voice or control,
       we cannot implement that specific feature.
    */
...
*/
/********************************************************************************F*/
static void _VoipCommonReceiveTextDataCb(const VoipUserT *pRemoteUser, const char *pStrUtf8, void *pUserData)
{
    ...
    for (iConnectionIndex = 0; iConnectionIndex < pVoipCommon->Connectionlist.iMaxConnections; iConnectionIndex++)
    {
        ...
        {
            if (VOIP_SameUser(pRemoteUser, &pConnection->RemoteUsers[iRemoteUserIndex]))
            {
                bFound = TRUE;
                break;
            }
...
VoipRefT *VoipCommonStartup(int32_t iMaxPeers, int32_t iVoipRefSize, VoipHeadsetStatusCbT *pStatusCb, int32_t iData)
{
    ...
    VoipHeadsetSetTranscribedTextReceivedCallback(pVoipCommon->pHeadset, &_VoipCommonReceiveTextDataCb, (void *)pVoipCommon);
#else
    VoipConnectionSetCallbacks(&pVoipCommon->Connectionlist, &VoipHeadsetReceiveVoiceDataCb, (void *)pVoipCommon->pHeadset,
...
int32_t VoipCommonControl(VoipCommonRefT *pVoipCommon, int32_t iControl, int32_t iValue, void *pValue)
{
    ...
    if (iControl == 'chan')
    {
        ...
        {
            #if (VOIP_SHARED_USER_INDEX != VOIP_INVALID_SHARED_USER_INDEX)
            {
                NetPrintf(("voipcommon: setting voip channels (0x%08x) for local shared user\n", iValue));
                pVoipCommon->Connectionlist.aChannels[iUserIndex] = iValue;
                return(0);
            }
            /* the shared user index is a concept on PS4 and XboxOne
               for shared audio input devices. other platforms don't
               have the same concept causing this code to potentially
               access outside the range of the aChannel buffer */
            #else
            {
                NetPrintf(("voipcommon: setting voip channels (0x%08x) for local shared user is not supported on this platform\n", iValue));
                return(-1);
            }
            #endif
        }
    ...
    if (iControl == 'vcid')
    {
        if (pVoipCommon->Connectionlist.pConnections[iValue].bIsLocalClientIdValid)
        {
            NetPrintf(("voipcommon: warning - local client id 0x%08x is being replaced with 0x%08x for conn id %d\n",
                pVoipCommon->Connectionlist.pConnections[iValue].uLocalClientId, *(int32_t*)pValue, iValue));
        }
        else
        {
            NetPrintf(("voipcommon: assigning local client id 0x%08x to conn id %d\n", *(int32_t*)pValue, iValue));
        }
        pVoipCommon->Connectionlist.pConnections[iValue].uLocalClientId = *(int32_t*)pValue;
        pVoipCommon->Connectionlist.pConnections[iValue].bIsLocalClientIdValid = TRUE;
        return(0);
    }
    if (iControl == 'conm')
    {
...
int32_t VoipCommonSelectChannel(VoipCommonRefT *pVoipCommon, int32_t iUserIndex, int32_t iChannel, VoipChanModeE eMode)
{
    ...
    else if (VOIP_SHARED_USER_INDEX == VOIP_INVALID_SHARED_USER_INDEX)
    {
