...
/*!
    ...
    \Input iConnId          - connection index (-1 if originator is local)
...
*/
/********************************************************************************F*/
static int32_t _VoipGroupManagerDisplayTranscribedTextCallback(int32_t iConnId, int32_t iUserIndex, const char *pStrUtf8, void *pUserData)
{
    VoipGroupManagerT *pManager = (VoipGroupManagerT *)pUserData;
    ...
    for (iGroup = 0, bDisplayed = FALSE; (iGroup < pManager->iMaxGroups) && !bDisplayed; iGroup++)
    {
        if ((pManager->aGroups[iGroup].bUsed) && (pManager->aGroups[iGroup].pDisplayTranscribedTextCb))
        {
            if (iConnId != -1)
            {
                ...
                if (iMappedConnId != VOIP_CONNID_NONE)
                {
                    bDisplayed = pManager->aGroups[iGroup].pDisplayTranscribedTextCb(iMappedConnId, iUserIndex, pStrUtf8, pManager->aGroups[iGroup].pDisplayTranscribedTextUserData);
                }
            }
            else
            {
...
static VoipGroupManagerT *_VoipGroupManagerCreate(int8_t iMaxGroups)
{
    if (_VoipGroupManager_pRef == NULL)
    {
        ...
        if (VoipGetRef() != NULL)
        {
            ...
            VoipSetDisplayTranscribedTextCallback(VoipGetRef(), _VoipGroupManagerDisplayTranscribedTextCallback, _VoipGroupManager_pRef);
        }
    }
...
static void _VoipGroupManagerShutdown(void)
{
    if (_VoipGroupManager_pRef != NULL)
    {
        ...
        if (VoipGetRef() != NULL)
        {
            ...
            VoipSetDisplayTranscribedTextCallback(VoipGetRef(), NULL, NULL);
        }
...
int32_t VoipGroupConnect(VoipGroupRefT *pVoipGroup, int32_t iConnId, uint32_t uAddress, uint32_t uManglePort, uint32_t uGamePort, uint32_t uClientId, uint32_t uSessionId, uint8_t bIsConnectivityHosted, uint32_t uLowLevelConnectivityId)
{
    ...
    if (!bMuted)
    {
        ...
        VoipControl(VoipGetRef(), 'uspk', pVoipGroup->bSilent ? 0x00000000 : _VoipGroupManager_pRef->uUserRecvMask, NULL);
        _VoipGroupManager_pRef->uUserSendMask |= (1 << iMappedLowLevelConnId);
        VoipControl(VoipGetRef(), 'umic', pVoipGroup->bSilent ? 0x00000000 : _VoipGroupManager_pRef->uUserSendMask, NULL);
    }
...
void VoipGroupDisconnect(VoipGroupRefT *pVoipGroup, int32_t iConnId)
{
    ...
    if (pVoipGroup->aConnections[iConnId].eConnState != VOIPGROUP_CONNSTATE_SUSPENDED)
    {
        ...
        if (iMappedLowLevelConnId != VOIP_CONNID_NONE)
        {
            ...
            if (!_VoipGroupLowLevelConnInUse(iMappedLowLevelConnId))
            {
                ...
                VoipDisconnect(VoipGetRef(), iMappedLowLevelConnId, TRUE);
            ...
            else
            {
                ...
                NetPrintf(("voipgroup: [%p] skipped call to VoipDisconnect() because low-level conn ref count has not reached 0\n", pVoipGroup));
            }
    ...
    {
        NetPrintf(("voipgroup: [%p] skipped call to VoipDisconnect() because high-level conn %d is suspended\n", pVoipGroup, iConnId));
    }
...
void VoipGroupSuspend(VoipGroupRefT *pVoipGroup, int32_t iConnId)
{
    ...
    if (iMappedLowLevelConnId != VOIP_CONNID_NONE)
    {
        ...
        if (_VoipGroupLowLevelConnInUse(iMappedLowLevelConnId))
        {
            ...
            */
            NetPrintf(("voipgroup: [%p] VoipGroupSuspend() skips call to VoipDisconnect() because low-level conn ref count has not reached 0\n", pVoipGroup));
        }
        else
        {
            /*
            ...
            */
            VoipDisconnect(VoipGetRef(), iMappedLowLevelConnId, FALSE);
...
int32_t VoipGroupMuteByConnId2(VoipGroupRefT *pVoipGroup, uint32_t iConnId, uint8_t bMute, uint32_t uMuteFlag)
{
    ...
    VoipControl(VoipGetRef(), 'uspk', pVoipGroup->bSilent ? 0x00000000 : pManager->uUserRecvMask, NULL);
    VoipControl(VoipGetRef(), 'umic', pVoipGroup->bSilent ? 0x00000000 : pManager->uUserSendMask, NULL);
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            ruid: returns the user id of the remote peer
