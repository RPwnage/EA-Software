...
#define VOIP_PACKET_VERSION     ('i')           //!< current packet version
...
static void _VoipConnectionConn(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, uint32_t uTick)
{
    ...
    #if VOIP_MAXLOCALUSERS != VOIP_MAXLOCALUSERS_EXTENDED
    ConnPacket.uNumLocalUsers |= 0x80; // set the high bit to signal extended list
    #endif
    ConnPacket.uNumLocalUsers |= (VOIP_MAXLOCALUSERS & 0x7f);
...
static void _VoipConnectionRecvSingle(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, struct sockaddr_in *pSendAddr, VoipPacketBufferT *pPacket, int32_t iSize)
{
    ...
    switch (uPktType)
    {
        ...
        case 'C':
        {
            ...
            pConnection->iMaxRemoteUsers = pConnPacket->uNumLocalUsers & 0x7f;
            pConnection->iMaxRemoteUsersExt = (pConnPacket->uNumLocalUsers & 0x80) == 0x80 ? pConnection->iMaxRemoteUsers + 1 : pConnection->iMaxRemoteUsers;
            ds_memcpy_s(pConnection->RemoteUsers, sizeof(pConnection->RemoteUsers), pConnPacket->LocalUsers, sizeof(*pConnPacket->LocalUsers) * pConnection->iMaxRemoteUsersExt);
        ...
        case 'M':
        {
            ...
            if (pConnection->bConnPktRecv)
            {
                ...
                /* if connection is not muted, then forward voice data to registered callback
                   xboxone: Never "mute" here, i.e. skip applying pConnectionlist->uRecvMask.
                ...
                #endif
                {
                    ...
                    {
                        pConnectionlist->pVoiceCb((VoipUserT *)&pConnection->RemoteUsers[0], iConnID, &pMicrPacket->MicrInfo, pRead, pConnectionlist->pVoiceUserData);
                    }
                }
...
void VoipConnectionSetSendMask(VoipConnectionlistT *pConnectionlist, uint32_t uSendMask)
{
    if (pConnectionlist->uSendMask != uSendMask)
    {
        ...
        {
            if ( (pConnectionlist->uSendMask & (1 << iConnIndex)) != (uSendMask & (1 << iConnIndex)) )
            {
                int32_t iLocalUserIndex;
                ...
                for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex++)
                {
                    if (!VOIP_NullUser(&pConnectionlist->LocalUsers[iLocalUserIndex]))
                    {
                        ...
                        for (iRemoteUserIndex = 0; iRemoteUserIndex < pConnectionlist->pConnections[iConnIndex].iMaxRemoteUsers; iRemoteUserIndex++)
                        {
                            ...
                            {
                                VoipCommonControl((VoipCommonRefT *)VoipGetRef(), (uSendMask & (1 << iConnIndex)) ? '+snd' : '-snd', iLocalUserIndex, pRemoteUser);
                            }
                        }
                    }
                }
...
void VoipConnectionSetRecvMask(VoipConnectionlistT *pConnectionlist, uint32_t uRecvMask)
{
    ...
    {
        /* With XBoxOne, per-connection inbound muting handling is rather deferred to
           VoipHeadsetXboxOne with usage of +rcv, -rcv. (We still alter
           pConnectionlist->uRecvMask but its contents is ignored when receiving over the voip
           connection.
        ...
        for (iConnIndex = 0; iConnIndex < pConnectionlist->iMaxConnections; iConnIndex++)
        {
            if ((pConnectionlist->uRecvMask & (1 << iConnIndex)) != (uRecvMask & (1 << iConnIndex)))
            {
                int32_t iLocalUserIndex;
                ...
                for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex++)
                {
                    if (!VOIP_NullUser(&pConnectionlist->LocalUsers[iLocalUserIndex]))
                    {
                        ...
                        for (iRemoteUserIndex = 0; iRemoteUserIndex < pConnectionlist->pConnections[iConnIndex].iMaxRemoteUsers; iRemoteUserIndex++)
                        {
                            ...
                            if (!VOIP_NullUser(pRemoteUser))
                            {
                                VoipCommonControl((VoipCommonRefT *)VoipGetRef(), (uRecvMask & (1 << iConnIndex)) ? '+rcv' : '-rcv', iLocalUserIndex, pRemoteUser);
                            }
                        }
                    }
                }
