...
#define VOIPNARRATE_SAMPLERATE             (16000)                                                     //!< sample rate; 16khz audio
...
static void _VoipNarrateInitialize(VoipNarrateRefT *pVoipNarrate)
{
    ...
    {
        NetPrintf(("voipnarratepc: failed to initialize COM library!\n"));
        VoipNarrateDestroy(pVoipNarrate);
        return;
    }

    // create our own IStream
    pVoipNarrate->pBaseStream = new (DirtyMemAlloc(sizeof(VoipNarrateVoiceStream), VOIP_MEMID, pVoipNarrate->iMemGroup, pVoipNarrate->pMemGroupUserData)) VoipNarrateVoiceStream();

    // create SpVoice interface
    if (CoCreateInstance(CLSID_SpVoice, NULL, CLSCTX_ALL, IID_ISpVoice, (void **)&pVoipNarrate->pVoice) != S_OK)
    {
        NetPrintf(("voipnarratepc: failed to create ISpVoice interface!\n"));
        CoUninitialize();
        VoipNarrateDestroy(pVoipNarrate);
        return;
    }
    ...
    if (CoCreateInstance(CLSID_SpStream, NULL, CLSCTX_ALL, IID_ISpStream, (void **)&pVoipNarrate->pVoiceStream) != S_OK)
    {
        NetPrintf(("voipnarratepc: failed to create ISpStream interface!\n"));
        pVoipNarrate->pVoice->Release();
        pVoipNarrate->pVoice = NULL;
        CoUninitialize();
        VoipNarrateDestroy(pVoipNarrate);
