...
/*!
    ...
    \Version 08/15/2018 (jbrookes)  Removed SLSv3 & RC4 ciphers
*/
/********************************************************************************H*/
...
#define SSL3_MSG_HELLO_RETRY_REQUEST    6   // hello retry (tls1.3)
...
{
    uint16_t uIdent;
    SignatureAlgorithmT SigAlg;
    uint8_t uVerifyScheme;
    uint8_t uCrvType;
} SignatureSchemeT;
...
typedef struct SecureStateT
{
    ...
    uint8_t bRecvProc;              //!< true if packet has been processed by _RecvPacket(), else false
...
{
    // curve 23: P-256
    { CRYPTCURVE_SECP256R1, "SECP256R1" },
    // curve 24: P-384
    { CRYPTCURVE_SECP384R1, "SECP384R1" },
    // curve 29: X25519
    { CRYPTCURVE_X25519,    "X25519"    },
    // curve 30: X448
    { CRYPTCURVE_X448,      "X448"      }
};
...
static const SignatureSchemeT _SSL3_SignatureSchemes[] =
{
    ...
    { SSL3_SIGSCHEME_RSA_PSS_PSS_SHA256,  { SSL3_HASHID_SHA256, SSL3_SIGALG_RSA   }, SSL3_SIGVERIFY_RSA_PSS,   ASN_OBJ_NONE            },
    { SSL3_SIGSCHEME_RSA_PSS_RSAE_SHA256, { SSL3_HASHID_SHA256, SSL3_SIGALG_RSA   }, SSL3_SIGVERIFY_RSA_PSS,   ASN_OBJ_NONE            },
    { SSL3_SIGSCHEME_RSA_PKCS1_SHA256,    { SSL3_HASHID_SHA256, SSL3_SIGALG_RSA   }, SSL3_SIGVERIFY_RSA_PKCS1, ASN_OBJ_NONE            },
    { SSL3_SIGSCHEME_ECDSA_SHA384,        { SSL3_HASHID_SHA384, SSL3_SIGALG_ECDSA }, SSL3_SIGVERIFY_NONE,      ASN_OBJ_ECDSA_SECP384R1 },
    { SSL3_SIGSCHEME_RSA_PSS_PSS_SHA384,  { SSL3_HASHID_SHA384, SSL3_SIGALG_RSA   }, SSL3_SIGVERIFY_RSA_PSS,   ASN_OBJ_NONE            },
    { SSL3_SIGSCHEME_RSA_PSS_RSAE_SHA384, { SSL3_HASHID_SHA384, SSL3_SIGALG_RSA   }, SSL3_SIGVERIFY_RSA_PSS,   ASN_OBJ_NONE            },
    { SSL3_SIGSCHEME_RSA_PKCS1_SHA384,    { SSL3_HASHID_SHA384, SSL3_SIGALG_RSA   }, SSL3_SIGVERIFY_RSA_PKCS1, ASN_OBJ_NONE            },
    { SSL3_SIGSCHEME_ECDSA_SHA512,        { SSL3_HASHID_SHA512, SSL3_SIGALG_ECDSA }, SSL3_SIGVERIFY_NONE,      ASN_OBJ_NONE            },
    { SSL3_SIGSCHEME_RSA_PSS_PSS_SHA512,  { SSL3_HASHID_SHA512, SSL3_SIGALG_RSA   }, SSL3_SIGVERIFY_RSA_PSS,   ASN_OBJ_NONE            },
    { SSL3_SIGSCHEME_RSA_PSS_RSAE_SHA512, { SSL3_HASHID_SHA512, SSL3_SIGALG_RSA   }, SSL3_SIGVERIFY_RSA_PSS,   ASN_OBJ_NONE            },
    { SSL3_SIGSCHEME_RSA_PKCS1_SHA512,    { SSL3_HASHID_SHA512, SSL3_SIGALG_RSA   }, SSL3_SIGVERIFY_RSA_PKCS1, ASN_OBJ_NONE            },
    { SSL3_SIGSCHEME_ECDSA_SHA1,          { SSL3_HASHID_SHA1,   SSL3_SIGALG_ECDSA }, SSL3_SIGVERIFY_NONE,      ASN_OBJ_NONE            },
    { SSL3_SIGSCHEME_RSA_PKCS1_SHA1,      { SSL3_HASHID_SHA1,   SSL3_SIGALG_RSA   }, SSL3_SIGVERIFY_RSA_PKCS1, ASN_OBJ_NONE            }
};
...
static ProtoSSLCACertT _ProtoSSL_CACerts[] =
{
    ...
    { { "US", "California", "Redwood City", "Electronic Arts, Inc.",
      ...
      SSL_CACERTFLAG_GOSCA, ASN_OBJ_RSA_PKCS_SHA1, 0, 
...
*/
/********************************************************************************F*/
static void _AsnParseString(const unsigned char *pData, int32_t iSize, char *pString, int32_t iLength)
{
...
static const uint8_t *_AsnParseIdent(const uint8_t *pData, int32_t iSize, ProtoSSLCertIdentT *pIdent)
{
    ...
    for (iObjType = 0, pDataEnd = pData+iSize; (pData = _AsnParseHeader(pData, pDataEnd, &iType, &iSize)) != NULL; )
    {
        ...
        if ((iType == ASN_TYPE_PRINTSTR+ASN_PRIMITIVE) || (iType == ASN_TYPE_UTF8STR+ASN_PRIMITIVE) || (iType == ASN_TYPE_T61+ASN_PRIMITIVE))
        {
            ...
            {
                _AsnParseString(pData, iSize, pIdent->strCountry, sizeof(pIdent->strCountry));
            }
            if (iObjType == ASN_OBJ_STATE)
            {
                _AsnParseString(pData, iSize, pIdent->strState, sizeof(pIdent->strState));
            }
            if (iObjType == ASN_OBJ_CITY)
            {
                _AsnParseString(pData, iSize, pIdent->strCity, sizeof(pIdent->strCity));
            }
            if (iObjType == ASN_OBJ_ORGANIZATION)
            {
                _AsnParseString(pData, iSize, pIdent->strOrg, sizeof(pIdent->strOrg));
            }
            ...
            {
                _AsnParseString(pData, iSize, pIdent->strCommon, sizeof(pIdent->strCommon));
            }
            iObjType = 0;
        }
        pData += iSize;
...
static const uint8_t *_AsnParseDate(const uint8_t *pData, int32_t iSize, char *pBuffer, int32_t iBufSize, uint64_t *pDateTime)
{
    ...
    _AsnParseString(pData, iSize, pBuffer, iBufSize);
    if (pDateTime != NULL)
    {
...
static int32_t _ProtoSSLVerifyCertificate(ProtoSSLRefT *pState, SecureStateT *pSecure, X509CertificateT *pCert, uint8_t bCertIsCA)
{
    ...
    else
    {
        ...
        for (pCACert = _ProtoSSL_CACerts; pCACert != NULL; pCACert = pCACert->pNext)
        {
            ...
            if ((_CertificateCompareIdent(&pCACert->Subject, &pCert->Issuer, (bCertIsCA||pCert->iCertIsCA)) != 0) || (pCACert->iKeyModSize != pCert->iSigSize))
            {
                continue;
            }
...
/*F********************************************************************************/
/*!
    \Function _ProtoSSLSplitDelimitedString
...
*/
/********************************************************************************F*/
static uint32_t _ProtoSSLSplitDelimitedString(const char *pSource, char cDelimiter, char *pDest, int32_t iDstSize, const char **pNewSource)
{
...
static const SignatureSchemeT *_ProtoSSLChooseSignatureScheme(SecureStateT *pSecure, CertificateDataT *pCertificate, const uint8_t *pData, uint32_t uSigSchemeLen, const uint8_t *pDataEnd)
{
    ...
    {
        // skip unsupported signature algorithms
        if ((pSigScheme = _ProtoSSLGetSignatureScheme(_SafeRead16(pData, pDataEnd))) == NULL)
        {
            continue;
        }
        // don't pick a signature algorithm we don't have a certificate for
        ...
        if (pSecure->uSslVersion >= PROTOSSL_VERSION_TLS1_3)
        {
            // don't allow pkcs signature algorithms
            ...
            if ((uCertSigAlg == SSL3_SIGALG_ECDSA) && (pCertificate->iCrvType != pSigScheme->uCrvType))
            {
                continue;
            }
        }
...
static int32_t _ProtoSSLHelloExtnWriteEllipticCurves(ProtoSSLRefT *pState, uint8_t *pBuffer, int32_t iBufLen)
{
    ...
    *pBuffer++ = (uint8_t)(uEllipticCurvesLength>>0);
...
static int32_t _ProtoSSLHelloExtnWriteKeyShare(ProtoSSLRefT *pState, uint8_t *pBuffer, int32_t iBufLen)
{
    ...
    // ref elliptic curve
    if (((pEllipticCurve = pSecure->pEllipticCurve) == NULL) && (pState->iCurveDflt != -1))
    {
        pEllipticCurve = &_SSL3_EllipticCurves[pState->iCurveDflt];
    }
...
static int32_t _ProtoSSLParseHelloExtensions(ProtoSSLRefT *pState, const uint8_t *pData, const uint8_t *pDataEnd, uint16_t uExtensionParse)
{
    ...
    for (iResult = 0; (pData < pDataEnd) && (iResult == 0); pData += uExtensionLength)
    {
        ...
        // parse supported_versions extension only if tls1.3 is enabled and we are explicitly parsing for it
        if ((pState->uSslVersion >= SSL3_TLS1_3) && (uExtensionType == SSL_EXTN_SUPPORTED_VERSIONS) && (uExtensionParse == uExtensionType))
        {
...
static int32_t _ProtoSSLSendClientHello(ProtoSSLRefT *pState)
{
    ...
    {
        pSecure->pEllipticCurve = &_SSL3_EllipticCurves[pState->iCurveDflt];
    }
    ...
    *pData++ = (uint8_t)(pSecure->uSslClientVersion>>0);
    ...
    if ((_SessionHistoryGetInfo(&SessHist, pState->strHost, SockaddrInGetPort(&pState->PeerAddr), NULL) != NULL) && pState->bSessionResumeEnabled)
    {
        ...
        *pData++ = (uint8_t)sizeof(pSessInfo->SessionId);
        ds_memcpy(pData, pSessInfo->SessionId, sizeof(pSessInfo->SessionId));
        pData += sizeof(pSessInfo->SessionId);
...
static int32_t _ProtoSSLRecvClientHello(ProtoSSLRefT *pState, const uint8_t *pData, int32_t iDataSize)
{
    ...
    pSecure->uSslClientVersion = pSecure->uSslVersion = _SafeRead16(pData, pDataEnd);
    pData += 2;
    ...
    // if tls1.3+ and no tls1.3 ciphers are enabled, add them
    if ((pSecure->uSslVersion >= PROTOSSL_VERSION_TLS1_3) && !(uEnabledCiphers & PROTOSSL_CIPHER_ALL_13))
    {
        uEnabledCiphers |= PROTOSSL_CIPHER_ALL_13;
    }
    ...
    if ((pSecure->uSslVersion >= PROTOSSL_VERSION_TLS1_3) && ((pSecure->pEllipticCurve == NULL) || (pSecure->uPubKeyLength == 0)))
    {
        ...
        pSecure->pEllipticCurve = (pState->iCurveDflt != -1) ? &_SSL3_EllipticCurves[pState->iCurveDflt] : &_SSL3_EllipticCurves[SSL3_CURVE_DEFAULT];
...
static int32_t _ProtoSSLRecvHelloRetryRequest(ProtoSSLRefT *pState, const uint8_t *pData, int32_t iDataSize)
{
    ...
    if (pSecure->bSentSessionId && memcmp(pSecure->SessionId, aSessionId, sizeof(pSecure->SessionId)))
    {
        /* as per https://tools.ietf.org/html/rfc8446#section-4.1.3, a client which  receives a legacy_session_id
        field that does not match what it sent in the ClientHello MUST abort the handshake with an "illegal_parameter" alert */
        _ProtoSSLSendAlert(pState, SSL3_ALERT_LEVEL_FATAL, SSL3_ALERT_DESC_ILLEGAL_PARAMETER);
        return(ST_FAIL_SETUP);
    }

    // as per https://tools.ietf.org/html/rfc8446#section-4.1.4, a client which receives a cipher suite that was not offered MUST abort the handshake
...
static int32_t _ProtoSSLRecvServerHello(ProtoSSLRefT *pState, const uint8_t *pData, int32_t iDataSize)
{
    ...
    // read session info
    if ((uSessionLen = _SafeRead8(pSessionId, pDataEnd)) == SSL_SESSID_SIZE)
    {
        _SafeReadBytes(aSessionId, sizeof(aSessionId), pSessionId+1, SSL_SESSID_SIZE, pDataEnd);
        #if DEBUG_RAW_DATA
    ...
    else if (pSecure->bSentSessionId && memcmp(pSecure->SessionId, aSessionId, sizeof(pSecure->SessionId)))
    {
        /* as per https://tools.ietf.org/html/rfc8446#section-4.1.3, a client which  receives a legacy_session_id
           field that does not match what it sent in the ClientHello MUST abort the handshake with an "illegal_parameter" alert */
        _ProtoSSLSendAlert(pState, SSL3_ALERT_LEVEL_FATAL, SSL3_ALERT_DESC_ILLEGAL_PARAMETER);
...
static int32_t _ProtoSSLRecvCertificate(ProtoSSLRefT *pState, const uint8_t *pData, int32_t iDataSize)
{
    ...
    if ((iCertSize = iSize1 = _SafeRead24(pData, pDataEnd)) <= 3)
    {
        ...
        if (!pState->bServer || (pState->bServer && (pState->iClientCertLevel == 2)))
        {
            /* as per http://tools.ietf.org/html/rfc5246#section-7.4.6, a client providing an empty certificate response to a certificate
    ...
    for (iVerify = -1, iCertHeight = 0; (iSize1 > 0) && (iVerify != 0); pData += iSize2, iSize1 -= iSize2)
    {
        ...
        #if DEBUG_VAL_CERT
        _DebugPrintCert(&pSecure->Cert, "parsed certificate");
        #endif  
...
static int32_t _ProtoSSLSendCertificateRequest(ProtoSSLRefT *pState)
{
    ...
    if (pSecure->uSslVersion < SSL3_TLS1_3)
    {
        ...
        {
            strBody[iBodySize++] = 0;
            strBody[iBodySize++] = 8*2;             // eight supported SignatureAndHashAlgorithms
            strBody[iBodySize++] = SSL3_HASHID_SHA256; strBody[iBodySize++] = SSL3_SIGALG_ECDSA;
            strBody[iBodySize++] = SSL3_HASHID_SHA256; strBody[iBodySize++] = SSL3_SIGALG_RSA;
            strBody[iBodySize++] = SSL3_HASHID_SHA384; strBody[iBodySize++] = SSL3_SIGALG_ECDSA;
            strBody[iBodySize++] = SSL3_HASHID_SHA384; strBody[iBodySize++] = SSL3_SIGALG_RSA;
            strBody[iBodySize++] = SSL3_HASHID_SHA512; strBody[iBodySize++] = SSL3_SIGALG_ECDSA;
            strBody[iBodySize++] = SSL3_HASHID_SHA512; strBody[iBodySize++] = SSL3_SIGALG_RSA;
            strBody[iBodySize++] = SSL3_HASHID_SHA1;   strBody[iBodySize++] = SSL3_SIGALG_ECDSA;
            strBody[iBodySize++] = SSL3_HASHID_SHA1;   strBody[iBodySize++] = SSL3_SIGALG_RSA;
        }
...
static int32_t _ProtoSSLRecvCertificateRequest(ProtoSSLRefT *pState, const uint8_t *pData, int32_t iDataSize)
{
    ...
    if (pSecure->uSslVersion <= SSL3_TLS1_2)
    {
        ...
        if (iType == iNumTypes)
        {
            NetPrintf(("protossl: no valid cert signing option found in CertificateRequest\n"));
...
static int32_t _ProtoSSLRecvHelloRequest(ProtoSSLRefT *pState, const uint8_t *pData, int32_t iDataSize)
{
    ...
    _ProtoSSLSendAlert(pState, 1, SSL3_ALERT_DESC_NO_RENEGOTIATION);
...
/*!
    ...
    \Output
        uint8_t *       - pointer to handshake packet start

    \Version 11/10/2005 (gschaefer)
*/
/********************************************************************************F*/
static const uint8_t *_ProtoSSLRecvHandshake(ProtoSSLRefT *pState, uint8_t uType)
{
    SecureStateT *pSecure = pState->pSecure;
    uint8_t *pRecv = pSecure->RecvData+pSecure->iRecvHshkProg;

    // make sure it's a handshake message, and that we got desired packet
    if ((pSecure->RecvHead[0] != SSL3_REC_HANDSHAKE) || (pRecv[0] != uType))
    {
        return(NULL);
    }
...
static uint32_t _ProtoSSLUpdateRecvHandshake(ProtoSSLRefT *pState, SecureStateT *pSecure)
{
    ...
    {
        NetPrintf(("protossl: received unexpected message %d when expecting a handshake\n", pSecure->RecvHead[0]));
        return(FALSE);
    }
    ...
    {
        if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_CLIENT_HELLO)) != NULL)
        {
            pState->iState = _ProtoSSLRecvClientHello(pState, pData, iDataSize);
        }
        else if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_SERVER_HELLO)) != NULL)
        {
            pState->iState = _ProtoSSLRecvServerHello(pState, pData, iDataSize);
        }
        else if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_ENCRYPTED_EXTENSIONS)) != NULL)
        {
            pState->iState = _ProtoSSLRecvEncryptedExtensions(pState, pData, iDataSize);
        }
        else if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_CERTIFICATE)) != NULL)
        {
            pState->iState = _ProtoSSLRecvCertificate(pState, pData, iDataSize);
        }
        else if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_CERT_REQ)) != NULL)
        {
            pState->iState = _ProtoSSLRecvCertificateRequest(pState, pData, iDataSize);
        }
        else if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_CERT_VERIFY)) != NULL)
        {
            pState->iState = _ProtoSSLRecvCertificateVerify(pState, pData, iDataSize);
        }
        else if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_CLIENT_KEY)) != NULL)
        {
            pState->iState = _ProtoSSLRecvClientKeyExchange(pState, pData, iDataSize);
        }
        else if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_SERVER_KEY)) != NULL)
        {
            pState->iState = _ProtoSSLRecvServerKeyExchange(pState, pData, iDataSize);
        }
        else if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_SERVER_DONE)) != NULL)
        {
    ...
    {
        if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_FINISHED)) != NULL)
        {
    ...
    else if (pState->iState == ST3_SECURE)
    {
        ...
        if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_HELLO_REQUEST)) != NULL)
        {
            pState->iState = _ProtoSSLRecvHelloRequest(pState, pData, iDataSize);
        }
        if ((pData = _ProtoSSLRecvHandshake(pState, SSL3_MSG_NEW_SESSION_TICKET)) != NULL)
        {
...
static void _ProtoSSLUpdateRecvPacket(ProtoSSLRefT *pState, SecureStateT *pSecure)
{
    ...
    {
        NetPrintf(("protossl: unhandled %s (%d) packet of type %d in state %d\n", _ProtoSSLRecvGetRecordTypeName(pSecure->RecvHead[0]), pSecure->RecvHead[0], pSecure->RecvData[pSecure->iRecvHshkProg], pState->iState));
        _ProtoSSLSendAlert(pState, SSL3_ALERT_LEVEL_FATAL, SSL3_ALERT_DESC_DECODE_ERROR);
        pState->iClosed = 1;
        _ProtoSSLRecvReset(pSecure);
        pState->iState = (pState->iState < ST3_SECURE) ? ST_FAIL_SETUP : ST_FAIL_SECURE;
...
int32_t ProtoSSLControl(ProtoSSLRefT *pState, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iSelect == 'alpn')
    {
        ...
        for (uProtocol = 0, uAlpnExtensionLength = 0; uProtocol < SSL_ALPN_MAX_PROTOCOLS; uProtocol += 1)
        {
            ...
            if ((pProtocol->uLength = (uint8_t)_ProtoSSLSplitDelimitedString(pSrc, ',', pProtocol->strName, sizeof(pProtocol->strName), &pSrc)) == 0)
            {
                break;
            }
    ...
    if (iSelect == 'crvd')
    {
        pState->iCurveDflt = (int8_t)DS_CLAMP(iValue, -1, SSL3_CURVE_MAX);
    ...
    if (((iSelect == 'gcph') || (iSelect == 'gver') || (iSelect == 'gvmn')) && (_ProtoSSL_pState != NULL))
    {
        if (iSelect == 'gcph')
        {
