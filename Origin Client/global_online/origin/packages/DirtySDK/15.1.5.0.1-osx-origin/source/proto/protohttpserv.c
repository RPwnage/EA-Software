...
static ProtoHttpServThreadT *_ProtoHttpServGetThreadFromId(ProtoHttpServRefT *pHttpServ, uint32_t uHttpThreadId)
{
    ...
    for (pHttpThread = pHttpServ->pThreadList; (pHttpThread != NULL) && (pHttpThread->uHttpThreadId != uHttpThreadId); pHttpThread = pHttpThread->pNext)
        ;
...
static int32_t _ProtoHttpServUpdateListen(ProtoHttpServRefT *pHttpServ)
{
    ...
    if (pHttpServ->pThreadList != NULL)
    {
        pHttpServ->pThreadList->pNext = pHttpThread;
    }
    else
    {
