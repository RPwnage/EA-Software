...
static int32_t _ProtoHttpRedirectCallback(int32_t iRequest, int32_t iStatusCode, int32_t *pMethod, const char *pLocation, void *pUserArg)
{
    ...
    int32_t iResult = 0;
    NetPrintfVerbose((pState->iVerbose, 2, "protohttpps4: [%p] processing redirect for request 0x%08x code %d\n", pState, iRequest, iStatusCode));
    ...
    if ((iResult = _ProtoHttpGetHeader(pState)) >= 0)
    {
        ...
        {
            // setup the callback for the main thread, do not call callback here as it can be inside from a worker
            pState->QueuedRedData.strRespHeader = pState->pSceRespHeader;
            pState->QueuedRedData.uRespHeaderLen = (uint32_t)strlen(pState->pSceRespHeader);
        }
        else
        {
            pState->QueuedRedData.strRespHeader = NULL;
...
void ProtoHttpUpdate(ProtoHttpRefT *pState)
{
    ...
    if (pState->eState == ST_REDIR)
    {
        ...
        if (pState->pReceiveHeaderCb != NULL)
        {
            pState->pReceiveHeaderCb(pState, pState->QueuedRedData.strRespHeader, pState->QueuedRedData.uRespHeaderLen, pState->pCallbackRef);
        }
        
        // setup
        if (ProtoHttpGetLocationHeader(pState, pState->QueuedRedData.strRespHeader, strLocation, sizeof(strLocation), NULL) != -1)
        {
