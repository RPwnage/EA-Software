...
static const char _ProtoSSL_GOS2013ServerModulus2048[] =
"-----BEGIN CERTIFICATE-----\n"
...
{
    { (const uint8_t *)_ProtoSSL_GOS2013ServerModulus2048, sizeof(_ProtoSSL_GOS2013ServerModulus2048) },
...
int32_t ProtoSSLControl(ProtoSSLRefT *pState, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iSelect == 'alpn')
    {
        ...
        {
            char strName[256];
            if (_ProtoSSLSplitDelimitedString(pSrc, ',', strName, sizeof(strName), &pSrc) == 0)
            {
                break;
            }
