...
static void _VoipNarrateStart(VoipNarrateRefT *pVoipNarrate, VoipNarrateRequestT *pRequest)
{
    ...
    refAsyncOp->Completed = ref new AsyncOperationCompletedHandler<SpeechSynthesisStream ^>(
        [pVoipNarrate, pRequest](IAsyncOperation<SpeechSynthesisStream ^> ^refAsyncOp, AsyncStatus eStatus)
    {
        switch (eStatus)
        {
        ...
        if (eStatus != AsyncStatus::Started)
        {
            refAsyncOp->Close();
        }
    });
...
static void _VoipNarrateProcessResult(VoipNarrateRefT *pVoipNarrate, VoipNarrateRequestT *pRequest)
{
    ...
    if (pVoipNarrate->iDataLen > 0)
    {
        ...
        {
            const int32_t iLength = DS_MIN(160*2, pVoipNarrate->iDataLen - pVoipNarrate->iDataOff);
            int32_t iRead;
    ...
    DataReaderLoadOperation ^refAsyncOp = pRequest->refReader->LoadAsync(!bStreamStart ? (pVoipNarrate->iDataMax) : (pVoipNarrate->iDataMax + sizeof(WaveHeaderT)));
    refAsyncOp->Completed = ref new AsyncOperationCompletedHandler<uint32_t>(
        [pVoipNarrate, pRequest, bStreamStart](IAsyncOperation<uint32_t> ^refAsyncOp, AsyncStatus eStatus)
    {
        bool bError = false;
