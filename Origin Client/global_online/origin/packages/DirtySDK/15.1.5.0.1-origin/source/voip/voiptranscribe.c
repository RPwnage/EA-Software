...
/*!
    ...
    \Notes
        ...
        Amazon Transcribe:
            ...
            HTTP2 interface: https://docs.aws.amazon.com/transcribe/latest/dg/API_streaming_StartStreamTranscription.html
                NOTE: Amazon public API documentation is as of this writing not fully correct;
                the streaming format is completely different than what is described and there are
                other minor changes.
    ...
    \Version 05/21/2019 (eax-play-support) Second Version
*/
/********************************************************************************H*/
...
static int32_t _VoipTranscribeSubmitFinish(VoipTranscribeRefT *pVoipTranscribe, VoipBufferT *pVoipBuffer)
{
    ...
    {
        NetPrintf(("voiptranscribe: discarding short audio segment with only %d samples\n", pVoipBuffer->iNumSamples));
    ...
    NetPrintfVerbose((pVoipTranscribe->iVerbose, 1, "voiptranscribe: submit finish\n"));
    ...
    // move to next recording buffer and set it up
    pVoipTranscribe->iRecBuffer = (pVoipTranscribe->iRecBuffer+1)%2;
    pVoipBuffer = &pVoipTranscribe->VoipBuffer[pVoipTranscribe->iRecBuffer];
    pVoipBuffer->bRecStarting = TRUE;
    pVoipBuffer->bRecFinished = FALSE;
    pVoipBuffer->bRecFull = FALSE;
    pVoipBuffer->bMinDiscard = TRUE;
...
static int32_t _VoipTranscribeBackoffCheck(VoipTranscribeRefT *pVoipTranscribe, VoipBufferT *pVoipBuffer)
{
    ...
    {
        NetPrintfVerbose((pVoipTranscribe->iVerbose, 0, "voiptranscribe: discarding audio segment with %d samples due to backoff\n", pVoipBuffer->iNumSamples));
        pVoipBuffer->iBufOff = 0;
        pVoipBuffer->iNumSamples = 0;
    }
...
static int32_t _VoipTranscribeSendFinish(VoipTranscribeRefT *pVoipTranscribe)
{
    ...
    {
        iResult = pTransport->Send(pTransport->pState, pTransport->iStreamId, NULL, PROTOHTTP_STREAM_END);
    }
    if (pTransport->eTransport == TRANSPORT_WEBSOCKETS)
    {
        ...
        //$$ todo - "request" is "send text" currently... this is awkward and confusing
...
static void _VoipTranscribeUpdateRecord(VoipTranscribeRefT *pVoipTranscribe, uint32_t uCurTick)
{
    ...
    // if we're currently receiving we need to wait until we're done
    if (pVoipTranscribe->eState == ST_RECV)
    {
        NetPrintfVerbose((pVoipTranscribe->iVerbose, 1, "voiptranscribe: waiting to finish submitting due to ongoing receive\n"));
        return;
    }
...
static int32_t _VoipTranscribeUpdateSend(VoipTranscribeRefT *pVoipTranscribe, VoipBufferT *pVoipBuffer)
{
    ...
    if ((pVoipBuffer->iBufInp == pVoipBuffer->iBufOff) && pVoipBuffer->bRecFinished)
    {
        ...
        iResult = _VoipTranscribeSendFinish(pVoipTranscribe);
...
{
    TransportT *pTransport = &pVoipTranscribe->Transport;
    int32_t iResult, iState=ST_RECV;
...
static uint32_t _VoipTranscribeConfig(VoipTranscribeRefT *pVoipTranscribe, uint32_t uProfile, const char *pUrl, const char *pCred)
{
    ...
    if (pVoipTranscribe->eProvider != VOIPTRANSCRIBE_PROVIDER_GOOGLE)
    {
        ...
        else if (pVoipTranscribe->eFormat == VOIPTRANSCRIBE_FORMAT_XFLOAT)
        {
            ds_snzprintf(pVoipTranscribe->strAudioFormat, sizeof(pVoipTranscribe->strAudioFormat), "audio/x-float-array; rate=%d; endianness=little-endian; channels=1", pVoipTranscribe->iAudioRate);
            pVoipTranscribe->bCompressed = FALSE;
        }
    }
    else
...
{ 
    ...
    {
        _VoipTranscribeUpdateRecv(pVoipTranscribe);
    }

    // update module in idle state
    if (pVoipTranscribe->eState == ST_IDLE)
    {
        ...
        NetPrintfVerbose((pVoipTranscribe->iVerbose, 1, "voiptranscribe: starting transcription request on buffer %d\n", pVoipBuffer->iBuffer));
