...
struct VoipHeadsetRefT
{
    ...
    int32_t iParticipatingUserIndex;    //!< local user is now in "participating" state
...
static int32_t _VoipHeadsetNarrateCb(VoipNarrateRefT *pNarrateRef, int32_t iUserIndex, const int16_t *pSamples, int32_t iSize, void *pUserData)
{
    ...
    const int8_t *buffer = (const int8_t *)pSamples;
    ...
    if (iSize > 0)
    {
        ...
        while (iRemain != 0)
        {
            ...
            ds_memcpy(pFrameBuffer, buffer + (iSize - iRemain), iCopySize);
            iRemain -= iCopySize;
            ...
            if ((iCopySize == VOIP_HEADSET_FRAMESIZE) || ((pHeadset->iNarrateWritePos + iCopySize) == VOIP_HEADSET_FRAMESIZE))
            {
                ...
                if ((iCompBytes = VoipCodecEncode(pData, (const int16_t*)(pHeadset->narrateBuffer), VOIP_HEADSET_FRAMESAMPLES, (!pHeadset->bTextChatAccessibility && pHeadset->bVoiceTranscriptionEnabled) ? pHeadset->pTranscribeRef : NULL)) > 0)
                {
                    ...
                    else
                    {
                        ...
                        pPacket->MicrInfo.uSeqn = ++pHeadset->uSendSeq;
    ...
    else
    {
        ...
        // disable/re-enable short audio discard during narration
        VoipTranscribeControl(pHeadset->pTranscribeRef, 'vdis', !pHeadset->bNarrating, 0, NULL);
    }
    return(0);
}
...
static void _VoipHeadsetProcessPlay(VoipHeadsetRefT *pHeadset)
{
    ...
    for (uPacket = 0; uPacket < uPackets; uPacket++)
    {
        ...
        if ((iCompBytes = VoipCodecEncode(voipMicrPacketT.aData, &pHeadset->pPlayerBuffer[VOIP_HEADSET_FRAMESAMPLES * pHeadset->uPlayerBufferFrameCurrent], VOIP_HEADSET_FRAMESAMPLES, pHeadset->pTranscribeRef)) > 0)
        {
            if (pHeadset->bLoopback == FALSE)
            {
                // send to mic data callback
                pHeadset->pMicDataCb(voipMicrPacketT.aData, iCompBytes,  NULL, 0, pHeadset->iParticipatingUserIndex, pHeadset->uSendSeq++, pHeadset->pCbUserData);
            }
            else
            {
                voipMicrPacketT.MicrInfo.uSubPacketSize = iCompBytes;
                voipMicrPacketT.MicrInfo.uSeqn = ++pHeadset->uSendSeq;
...
static void _VoipHeadsetProcessRecord(VoipHeadsetRefT *pHeadset)
{
    ...
    if (pMicrInfo->bActive == TRUE)
    {
        ...
        for (; pMicrInfo->WaveData[pMicrInfo->iCurWaveBuffer].WaveHdr.dwFlags & WHDR_DONE; )
        {
            ...
            {
                if ((iCompBytes = VoipCodecEncode(pData, (int16_t *)FrameData, VOIP_HEADSET_FRAMESAMPLES, pHeadset->bVoiceTranscriptionEnabled ? pHeadset->pTranscribeRef : NULL)) > 0)
                {
...
void VoipHeadsetReceiveVoiceDataCb(VoipUserT *pRemoteUsers, int32_t iConsoleId, VoipMicrInfoT *pMicrInfo, uint8_t *pPacketData, void *pUserData)
{
    ...
    for (uMicrPkt = 0; uMicrPkt < pMicrInfo->uNumSubPackets; uMicrPkt++)
    {
        ...
        VoipConduitReceiveVoiceData(pHeadset->pConduitRef, &pRemoteUsers[pMicrInfo->uUserIndex], pSubPacket, uSubPacketSize);
...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    if (iControl == 'aloc')
    {
        ...
        {
            pHeadset->iParticipatingUserIndex = VOIP_INVALID_LOCAL_USER_INDEX;
            NetPrintf(("voipheadsetpc: no users participating\n", iValue));
        }
        else
        {
