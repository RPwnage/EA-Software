...
static VoiceAppT _Voice_App = { NULL, NULL, (ZFileT)NULL, 0, 0, 0, 0, 0, 0, 0, 0 };
...
static void _VoiceCreate(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    // set event callback
    VoipSetEventCallback(pApp->pVoip, _VoiceEvntCallback, pApp);
...
static void _VoiceConnect(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    if (argc >= 3)
    {
        ...
        uRemoteClientId = (argc == 4) ? (uint32_t)strtol(argv[3], NULL, 16) : uRemoteAddr;
        ...
        pApp->iConnID = VoipConnect(pApp->pVoip, VOIP_CONNID_NONE, uRemoteAddr, uManglePort, uRemotePort, uRemoteClientId, 0);
    }
    else
    {
        #if !defined(DIRTYCODE_XBOXONE)
...
static void _VoiceControl(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    if (argc > 2)
    {
        ...
        if (iCmd == 'stot')
        {
            ...
            pValue = &iValue2;
            VoipSetDisplayTranscribedTextCallback(pApp->pVoip, _VoiceDisplayTranscribedTextCallback, pApp);
        }
        else if (argc > 4)
        {
            pValue = argv[4];
