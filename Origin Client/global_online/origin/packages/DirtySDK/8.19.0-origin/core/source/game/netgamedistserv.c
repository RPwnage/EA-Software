...
typedef struct NetGameDistServClientT
{
    ...
    NetGameDistStreamT *pGameStream;    //!< client stream ref (optional)
...
struct NetGameDistServT
{
    ...
    //! stream base (zero means no stream)
    int32_t iStreamBase;
    ...
    int32_t iHighWaterStreamUsed;
    int32_t iHighWaterStreamNeeded;
...
/*!
    ...
    \Description
        Destroy game stream and game dist refs.
...
static void _NetGameDistServDiscClient(NetGameDistServT *pDistServ, NetGameDistServClientT *pDistClient, int32_t iDistClient, int32_t iReason)
{
    ...
    else if (pDistClient->pGameStream != NULL || pDistClient->pGameDist != NULL)
    {
        NetPrintf(("netgamedistserv: warning -- client %d has gamestream and/or gamedist but is disconnected\n", iDistClient));
    }
    
    // destroy stream
    if (pDistClient->pGameStream != NULL)
    {
        NetPrintfVerbose((pDistServ->iVerbosity, 0, "netgamedistserv: deleting dist stream for client %s/%d\n", pDistClient->strName, iDistClient));
        NetGameDistDestroyStream(pDistClient->pGameDist, pDistClient->pGameStream);
        pDistClient->pGameStream = NULL;
    }
    // destroy dist
...
/*F********************************************************************************/
/*!
    \Function _NetGameDistServStreamRecv

    \Description
        Handle stream receives, and rebroadcast them to other clients.

    \Input *pStream     - stream
    \Input iSubChan     - stream subchannel (should always be zero for server)
    \Input iKind        - stream kind
    \Input *pBuffer     - stream packet data
    \Input iLength      - stream packet size

    \Version 02/05/2007 (jbrookes)
*/
/********************************************************************************F*/
static void _NetGameDistServStreamRecv(NetGameDistStreamT *pStream, int32_t iSubChan, int32_t iKind, void *pBuffer, int32_t iLength)
{
    NetGameDistServT *pDistServ = (NetGameDistServT *)pStream->pRefPtr;
    NetGameDistServClientT *pDistClient;
    int32_t iClient, iResult;
    
    NetPrintfVerbose((pDistServ->iVerbosity, 2, "netgamedistserv: received %d byte packet on stream '%c%c%c%c/%c%c%c%c/%d' from client %d\n", iLength,
        (uint8_t)(pStream->iIdent >> 24), (uint8_t)(pStream->iIdent >> 16), (uint8_t)(pStream->iIdent >> 8), (uint8_t)(pStream->iIdent),
        (uint8_t)(iKind >> 24), (uint8_t)(iKind >> 16), (uint8_t)(iKind >> 8), (uint8_t)(iKind),
        iSubChan, pStream->iRefNum));

    // broadcast stream packet to connected peers    
    for (iClient = 0; iClient < pDistServ->ClientList.iMaxClients; iClient++)
    {
        // ref client
        pDistClient = &pDistServ->ClientList.Clients[iClient];
        ...
        if (!pDistClient->bInitialized || (iClient == pStream->iRefNum))
        {
            continue;
        }
        
        // send to peer if they have a stream
        if (pDistClient->pGameStream != NULL)
        {
            if ((iResult = pDistClient->pGameStream->Send(pDistClient->pGameStream, pStream->iRefNum, iKind, pBuffer, iLength)) == iLength)
            {
                NetPrintfVerbose((pDistServ->iVerbosity, 2, "netgamedistserv: broadcast %d byte stream packet to client %d (kind='%c%c%c%c'/%d)\n", iLength, iClient,
                    (uint8_t)(iKind>>24), (uint8_t)(iKind>>16), (uint8_t)(iKind>>8), (uint8_t)(iKind), pStream->iRefNum));
            }
            else
            {
                NetPrintf(("netgamedistserv: stream send to client %d failed (err=%d)\n", iClient, iResult));
                _NetGameDistServDiscClient(pDistServ, pDistClient, iClient, GMDIST_STREAM_FAILURE);
            }
        }
        else if (pDistClient->bInitialized && !pDistClient->bDisconnected)
        {
            NetPrintf(("netgamedistserv: unable to send stream packet to client %d (no ref)\n", iClient));
            _NetGameDistServDiscClient(pDistServ, pDistClient, iClient, GMDIST_NO_STREAM);
        }
    }
}
...
static void _NetGameDistUpdateMulti(NetGameDistServT *pDistServ)
{
    ...
    for (iClient = 0; iClient < pDistServ->ClientList.iMaxClients; iClient++)
    {
        ...
        if (pDistClient->pGameDist != NULL)
        {
            ...
            // enable sparse multi-packet with meta information
            NetGameDistControl(pDistClient->pGameDist, 'meta', 1, NULL);

            // set mask to all players
            NetGameDistControl(pDistClient->pGameDist, 'metm', uMask, NULL);

            // set multi-packet version to 1
            NetGameDistControl(pDistClient->pGameDist, 'metv', pDistServ->uMultiPacketVersion, NULL);
        }
    }
}
...
/*!
    ...
    \Description
        Prepare the inputs for sendign a multipacket
...
/*!
    ...
    \Input iStreamBase      - base stream kind
...
*/
/********************************************************************************F*/
NetGameDistServT *NetGameDistServCreate(int32_t iMaxClients, int32_t iStreamBase, int32_t iVerbosity)
{
    ...
    pDistServ->iStreamBase = iStreamBase;
...
int32_t NetGameDistServAddClient(NetGameDistServT *pDistServ, int32_t iClient, NetGameLinkRefT *pLinkRef, const char *pClientName)
{
    ...
    memset(pDistClient, 0, sizeof(*pDistClient));
    strnzcpy(pDistClient->strName, pClientName, sizeof(pDistClient->strName));
    ...
    // create a stream?
    if (pDistServ->iStreamBase != 0)
    {
        NetPrintfVerbose((pDistServ->iVerbosity, 0, "netgamedistserv: creating dist stream for client %s/%d\n", pClientName, iClient));
        pDistClient->pGameStream = NetGameDistCreateStream(pDistClient->pGameDist, 0, pDistServ->iStreamBase, 10240, 10240, 10240);
        pDistClient->pGameStream->Recv = (NetGameDistStreamRecvProc *)_NetGameDistServStreamRecv;
        pDistClient->pGameStream->pRefPtr = pDistServ;
        pDistClient->pGameStream->iRefNum = iClient;
    }
...
/*F********************************************************************************/
/*!
    \Function NetGameDistServStartGame

    \Description
        Starts the game, enabling controller passing.
...
{
    NetPrintf(("netgamedistserv: startgame\n"));

    // update dist multi configuration
    _NetGameDistUpdateMulti(pDistServ);

    pDistServ->bGameStarted = TRUE;
    _NetGameDistServFlowControl(pDistServ);

    // reset the CRC challenge mode
    pDistServ->uCRCResponseCountdown = 0;
    pDistServ->uCRCRemainingFrames = pDistServ->uCRCRate;
}
...
/*F********************************************************************************/
/*!
    \Function NetGameDistServStopGame

    \Description
        Stops the game, halting controller passing.
...
{
    NetPrintf(("netgamedistserv: stopgame\n"));

    pDistServ->bGameStarted = FALSE;
    // update dist multi configuration
    _NetGameDistUpdateMulti(pDistServ);
}
...
void NetGameDistServUpdate(NetGameDistServT *pDistServ)
{
    ...
    if ((pDistServ->uLastStatsUpdate == 0) || (NetTickDiff(uNow, pDistServ->uLastStatsUpdate) > 2000))
    {
        ...
        for (iClient = 0; iClient < pDistServ->ClientList.iMaxClients; iClient++)
        {
            ...
            if (pDistClient->bInitialized && !pDistClient->bDisconnected)
            {
                NetGameDistStatus(pDistServ->ClientList.Clients[iClient].pGameDist, 'stat', 0, &Stat, sizeof(NetGameLinkStatT));
                aStats[iClient].ping = SocketHtonl(Stat.ping);
                aStats[iClient].late = SocketHtonl(Stat.late);
                aStats[iClient].bps = SocketHtonl(Stat.bps);
                aStats[iClient].pps = SocketHtonl(Stat.pps);
            }
            else
            {
                memset(&aStats[iClient], 0, sizeof(aStats[0]));
            }
        }
    ...
    for (iClient = 0; iClient < pDistServ->ClientList.iMaxClients; iClient++)
    {
        ...
        if(pDistClient->bInitialized && !pDistClient->bDisconnected)
        {
            iRet = NetGameDistControl(pDistClient->pGameDist, 'hwsu', 0, NULL);
            if (iRet > pDistServ->iHighWaterStreamUsed)
            {
                pDistServ->iHighWaterStreamUsed = iRet;
                pDistServ->uHighWaterChanged = TRUE;
            }

            iRet = NetGameDistControl(pDistClient->pGameDist, 'hwsn', 0, NULL);
            if (iRet > pDistServ->iHighWaterStreamNeeded)
            {
                pDistServ->iHighWaterStreamNeeded = iRet;
                pDistServ->uHighWaterChanged = TRUE;
            }
    ...
    if (((uSendThreshold == 0) || (pDistServ->uNbFramesNoData < uSendThreshold)) && !noPeek)
    {
        ...
        for (iClient = 0; iClient < pDistServ->ClientList.iMaxClients; iClient++)
        {
            ...
            if (uNewFlow[iClient])
            {
                ...
                memcpy(pDistClient->aLocalInput, aLocalInput[iClient], sizeof(pDistClient->aLocalInput));
            }
        }
...
/*!
    ...
    \Input *pDistServ               - module state
    \Input pHighWaterStreamUsed     - pointer to set to the highwater stream usage
    \Input pHighWaterStreamNeeded   - pointer to set to the highwater stream need
...
*/
/********************************************************************************F*/
uint8_t NetGameDistServHighWaterChanged(NetGameDistServT *pDistServ, int32_t* pHighWaterStreamUsed, int32_t* pHighWaterStreamNeeded, int32_t* pHighWaterInputQueue, int32_t* pHighWaterOutputQueue)
{
#if MONITOR_HIGHWATER
    uint8_t ret = pDistServ->uHighWaterChanged;

    if (pHighWaterStreamUsed != NULL)
    {
        *pHighWaterStreamUsed = pDistServ->iHighWaterStreamUsed;
    }
    if (pHighWaterStreamNeeded != NULL)
    {
        *pHighWaterStreamNeeded = pDistServ->iHighWaterStreamNeeded;
    }
