...
*/
/********************************************************************************F*/
int32_t VoipCodecEncode(uint8_t *pOutput, const int16_t *pInput, int32_t iNumSamples)
{
    VoipCodecRegT *pRegEntry;
    int32_t iResult;
    ...
    #if VOIPCODEC_TIMER
    {
        VoipTimerT TimerA, TimerB;
        VoipTimer(&TimerA);
    #endif

    if (pRegEntry->pCodecRef->bVadEnabled)
    {
        // if VAD module detects silence, do not encode this frame
        if (_VoipCodecVadProcess16BitFrame(pRegEntry->pCodecRef, pInput, iNumSamples))
        {
            return(0);
        }
    }
    ...
    #if VOIPCODEC_TIMER
        VoipTimer(&TimerB);
        NetPrintf(("voipcodecencode: %d cycles\n", VoipTimerDiff(&TimerB, &TimerA)));
    }
    #endif
    return(iResult);
}
...
int32_t VoipCodecDecode(int32_t *pOutput, const uint8_t *pInput, int32_t iInputBytes, int32_t iChannel)
{
    ...
    #if VOIPCODEC_TIMER
        VoipTimer(&TimerB);
        NetPrintf(("voipcodecdecode: %d cycles\n", VoipTimerDiff(&TimerB, &TimerA)));
    }
    #endif
