...
SocketT *SocketAccept(SocketT *sock, struct sockaddr *addr, int32_t *addrlen)
{
    ...
    sock->iLastError = SOCKERR_INVALID;
    ...
    if (iInfo == 'stat')
    {
        ...
        if (pSocket->opened > 0)
        {
            ...
            if (select(1, &fdread, NULL, &fdexcept, &tv) > 0)
            {
                ...
                if (fdexcept.fd_count > 0)
                {
                    NetPrintf(("dirtynetwin: connection failure\n"));
                    pSocket->opened = -1;
                }
                else if (fdread.fd_count > 0)
                {
                    ...
                    if ((ioctlsocket(pSocket->socket, FIONREAD, &uAvailBytes) != 0) || (uAvailBytes == 0))
                    {
                        ...
                        NetPrintf(("dirtynetwin: connection closed (wsaerr=%d)\n", (uAvailBytes==0) ? WSAECONNRESET : WSAGetLastError()));
                        pSocket->opened = -1;
                    }
                }
            }
...
#if defined(DIRTYCODE_XENON) // xbox 360 specific functions
/*F*************************************************************************************/
/*!
    \Function    SocketLookupDone
...
int32_t SocketHost(struct sockaddr *host, int32_t hostlen, const struct sockaddr *dest, int32_t destlen)
{
    ...
    if (hostlen != destlen)
    {
        NetPrintf(("dirtynetwin: SocketHost() len mismatch\n"));
        return(-1);
    }
