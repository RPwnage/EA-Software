...
typedef struct HttpManagerHttpCmdT
{
    ...
    void *pWriteCbUserData;             //!< write callback user data
    ...
    uint32_t uBytesXfer;                //!< bytes transferred
...
static void _HttpManagerDisplayStats(HttpManagerRefT *pHttpManager)
{
    ...
    if (pHttpManager->HttpManagerStats.uNumTransactions > 0)
    {
        ...
        NetPrintf(("httpmanager: total bytes transferred: %d\n", pHttpManager->HttpManagerStats.uTransactionBytes));
...
{
    memset(&pHttpManager->HttpManagerStats, 0, sizeof(pHttpManager->HttpManagerStats));
}
...
static void _HttpManagerPipelineReset(HttpManagerRefT *pHttpManager, HttpManagerHttpRefT *pHttpRef, int32_t iHttpCmdFirst)
{
    ...
    for (iHttpCmd = iHttpCmdFirst ; iHttpCmd < pHttpRef->iTransactions; iHttpCmd += 1)
    {
        ...
        {
            NetPrintf(("httpmanager: reset handle %d for re-issue\n", pHttpCmd->iHttpHandle));
...
/*!
    ...
    \Input uDataLen         - total size of data
...
*/
/********************************************************************************F*/
static int32_t _HttpManagerCustomHeaderCb(ProtoHttpRefT *pState, char *pHeader, uint32_t uHeaderSize, const char *pData, uint32_t uDataLen, void *pUserRef)
{
    ...
    NetPrintf(("httpmanager: calling custom header callback with userref 0x%08x for handle %d\n", (uintptr_t)pHttpCmd->pCallbackRef, pHttpCmd->iHttpHandle));
    return(pHttpCmd->pHttpManager->pCustomHeaderCb(pState, pHeader, uHeaderSize, pData, uDataLen, pHttpCmd->pCallbackRef));
}
...
static int32_t _HttpManagerReceiveHeaderCb(ProtoHttpRefT *pState, const char *pHeader, uint32_t uHeaderSize, void *pUserRef)
{
   ...
   {
       NetPrintf(("httpmanager: HTTP %d result on handle %d\n", iResult, pHttpCmd->iHttpHandle));
    ...
    {
        NetPrintf(("httpmanager: piped commands lost due to premature close on handle %d\n", pHttpCmd->iHttpHandle));
        iHttpCmd = 1;
        ...
        {
            NetPrintf(("httpmanager; disabling pipelining without keep-alive\n"));
    ...
    if (pHttpManager->pReceiveHeaderCb != NULL)
    {
        NetPrintf(("httpmanager: calling receive header callback with userref 0x%08x for handle %d\n", (uintptr_t)pHttpCmd->pCallbackRef, pHttpCmd->iHttpHandle));
        return(pHttpManager->pReceiveHeaderCb(pState, pHeader, uHeaderSize, pHttpCmd->pCallbackRef));
    }
    else
    {
        return(0);
...
static void _HttpManagerWriteCbProcess(HttpManagerRefT *pHttpManager, HttpManagerHttpCmdT *pHttpCmd)
{
    ...
    {
        pHttpCmd->pWriteCb(pHttpCmd->pHttpRef->pProtoHttp, &WriteCbInfo, strTempRecv, iResult, pHttpCmd->pWriteCbUserData);
    }
    ...
    if ((iResult == PROTOHTTP_RECVDONE) || (iResult == PROTOHTTP_RECVHEAD) || (iResult == PROTOHTTP_RECVFAIL))
    {
        if (iResult != PROTOHTTP_RECVFAIL)
        {
            ...
            pHttpCmd->pWriteCb(pHttpCmd->pHttpRef->pProtoHttp, &WriteCbInfo, "", iResult, pHttpCmd->pWriteCbUserData);
        }
        else
        {
            pHttpCmd->pWriteCb(pHttpCmd->pHttpRef->pProtoHttp, &WriteCbInfo, "", PROTOHTTP_RECVFAIL, pHttpCmd->pWriteCbUserData);
        }
        pHttpCmd->pWriteCb = NULL;
        pHttpCmd->pWriteCbUserData = NULL;
    }
...
{
    int32_t iHdrLen = (pAppendHdr != NULL) ? (int32_t)strlen(pAppendHdr) : 0;
    char **ppAppendHdr;
...
/*!
    ...
    \Description
        Applies append header to apply for specified command.  The append header returned
        will be the command-specific header if set, else it will be the global append
        header if set.  If neither is set, it will be
                ...
                if ((pHttpPipeCmd->uState == HTTPMANAGER_CMDSTATE_WAIT) && ((pHttpPipeCmd->bKeepAlive == TRUE) || (pHttpManager->bPipeWithoutKeepAlive == TRUE)))
                {
                    if (_HttpManagerPipelineCheck(pHttpPipeCmd, pHttpCmd) && (pHttpRef->iTransactions < HTTPMANAGER_MAXREFQUEUE))
                    {
                        if (iHttpCmd == (pHttpRef->iTransactions - 1))
                        {
                            ...
                            NetPrintfVerbose((pHttpManager->iVerbose, 0, "httpmanager: queueing ref %2d count=%d pipe=1 url=%s\n", iHttpRef, pAllocRef->iTransactions, pHttpCmd->pUrl));
                            break;
                        }
                    }
                }
        ...
        for (iQueueDepth = 0; (pAllocRef == NULL) && (iQueueDepth < HTTPMANAGER_MAXREFQUEUE); iQueueDepth += 1)
        {
            ...
            for (iHttpRef = 0; iHttpRef < pHttpManager->iHttpNumRefs; iHttpRef += 1)
            {
                ...
                if ((pHttpRef->iTransactions == iQueueDepth) && (aKeepAliveState[iHttpRef] == TRUE))
                {
                    ...
                    pAllocRef = pHttpRef;
                    pHttpCmd->bKeepAlive = TRUE;
                    NetPrintfVerbose((pHttpManager->iVerbose, 0, "httpmanager: queueing ref %2d count=%d keep=1 url=%s\n", iHttpRef, pAllocRef->iTransactions, pHttpCmd->pUrl));
                    break;
                }
            }
            // did we find one?
            ...
            if (iBestMatchIdx >= 0)
            {
                ...
                pHttpCmd->bKeepAlive = FALSE;
                NetPrintfVerbose((pHttpManager->iVerbose, 0, "httpmanager: queueing ref %2d count=%d keep=0 url=%s\n", iBestMatchIdx, pAllocRef->iTransactions, pHttpCmd->pUrl));
                break;
            }
        }
    }
...
*/
/********************************************************************************F*/
static int32_t _HttpManagerRequestIssue(HttpManagerRefT *pHttpManager, HttpManagerHttpCmdT *pHttpCmd, const char *pData, int32_t iDataSize)
{
    int32_t iResult;

    // initiate the request
    if ((iResult = ProtoHttpRequest(pHttpCmd->pHttpRef->pProtoHttp, pHttpCmd->pUrl, pData, iDataSize, (ProtoHttpRequestTypeE)pHttpCmd->uRequestType)) < 0)
    {
        return(iResult);
    }

    NetPrintfVerbose((pHttpManager->iVerbose, 0, "httpmanager: started ref %2d url=%s\n", pHttpCmd->pHttpRef - pHttpManager->HttpRefs, pHttpCmd->pUrl));
...
static int32_t _HttpManagerRequestStart(HttpManagerRefT *pHttpManager, HttpManagerHttpCmdT *pHttpCmd, const char *pData, int32_t iDataSize)
{
    ...
    if ((iResult = _HttpManagerRequestIssue(pHttpManager, pHttpCmd, pData, iDataSize)) < 0)
    {
        ...
        if (iResult < 0)
        {
            NetPrintf(("httpmanager: ProtoHttpRequest() returned %d on handle %d url=%s\n", iResult, pHttpCmd->iHttpHandle, pHttpCmd->pUrl));
...
static void _HttpManagerRequestCheckCompletion(HttpManagerRefT *pHttpManager, HttpManagerHttpCmdT *pHttpCmd)
{
    ...
    else if ((iStatus == 1) && (ProtoHttpStatus(pHttpRef->pProtoHttp, 'body', NULL, 0) != (signed)pHttpCmd->uBytesXfer))
    {
    ...
    {
        NetPrintf(("httpmanager: could not get header size for stat tracking (err=%d)\n", iHeadSize));
    ...
    NetPrintf(("httpmanager: [%d] %s transaction %s (%d bytes in %dms, %.2fbps)\n", pHttpCmd->iHttpHandle, pHttpCmd->pUrl,
        (iStatus == 1) ? "complete" : "failed", pHttpCmd->uBytesXfer, NetTickDiff(pHttpCmd->uComplTick, pHttpCmd->uIssueTick),
        ((float)pHttpCmd->uBytesXfer * 1000.0f) / (float)(NetTickDiff(pHttpCmd->uComplTick, pHttpCmd->uIssueTick))));
}
...
static void _HttpManagerRequestDelCmdFromQueue(HttpManagerRefT *pHttpManager, HttpManagerHttpRefT *pHttpRef, HttpManagerHttpCmdT *pHttpCmd)
{
    ...
    {
        NetPrintf(("httpmanager: contracting transaction queue for ref %d\n", pHttpCmd->pHttpRef - pHttpManager->HttpRefs));
...
static void _HttpManagerCmdReleaseRef(HttpManagerRefT *pHttpManager, HttpManagerHttpCmdT *pHttpCmd)
{
    ...
    {
        NetPrintfVerbose((pHttpManager->iVerbose, 0, "httpmanager: warning -- no completion timestamp for handle %d being deleted (%dms since queued)\n",
            pHttpCmd->iHttpHandle, NetTickDiff(NetTick(), pHttpCmd->uQueueTick)));
        NetPrintfVerbose((pHttpManager->iVerbose, 0, "httpmanager:    url=%s\n", pHttpCmd->pUrl));
        NetPrintfVerbose((pHttpManager->iVerbose, 0, "httpmanager:    state=%d\n", pHttpCmd->uState));
        NetPrintfVerbose((pHttpManager->iVerbose, 0, "httpmanager:    data=%d\n", pHttpCmd->uBytesXfer));
        uFreeLatency = 0;
    }
...
static int32_t _HttpManagerPipeline(HttpManagerRefT *pHttpManager, HttpManagerHttpRefT *pHttpRef, HttpManagerHttpCmdT *pHttpCmd)
{
    ...
    {
        NetPrintfVerbose((pHttpManager->iVerbose, 0, "httpmanager: started ref %2d url=%s\n", pHttpRef - pHttpManager->HttpRefs, pHttpCmd->pUrl));
        ...
        for (iHttpCmd = 1; (iHttpCmd < pHttpRef->iTransactions) && (iHttpCmd < pHttpManager->iMaxPipedUrls); iHttpCmd += 1)
        {
            ...
            {
                NetPrintf(("httpmanager: could not pipe handle %d; request will be retried on a new ref\n", pHttpCmd2->iHttpHandle));
                break;
            }
            NetPrintfVerbose((pHttpManager->iVerbose, 0, "httpmanager: piped   ref %2d url=%s\n", pHttpRef - pHttpManager->HttpRefs, pHttpCmd2->pUrl));
    ...
    {
        // resized input buffer, let _HttpManagerRequest() re-issue it for us without pipelining
        return(0);
    }
    else
    {
...
/*!
    ...
    \Input *pWriteCbUserData- user data for write callback
...
*/
/********************************************************************************F*/
static int32_t _HttpManagerRequestCb(HttpManagerRefT *pHttpManager, int32_t iHandle, const char *pUrl, const char *pData, int32_t iDataSize, ProtoHttpRequestTypeE eRequestType, ProtoHttpWriteCbT *pWriteCb, void *pWriteCbUserData)
{
    HttpManagerHttpCmdT *pHttpCmd;
    ...
    pHttpCmd->pWriteCbUserData = pWriteCbUserData;
    ...
    if (pHttpManager->bCopyUrl)
    {
        ...
        strnzcpy((char *)pHttpCmd->pUrl, pUrl, iStrLen+1);
...
static int32_t _HttpManagerCreateRef(HttpManagerRefT *pHttpManager, int32_t iHttpRef)
{
    ...
    NetPrintf(("protohttpmanager: allocating http ref %d\n", iHttpRef));
...
static void _HttpManagerDestroyRef(HttpManagerRefT *pHttpManager, int32_t iHttpRef)
{
    ...
    for (iTransaction = 0; iTransaction < pHttpRef->iTransactions; iTransaction += 1)
    {
        ...
        {
            NetPrintf(("protohttpmanager: resize destroying ref %2d; handle %d is being terminated\n", iHttpRef, pHttpCmd->iHttpHandle));
    ...
    // destroy ref
    NetPrintf(("protohttpmanager: destroying http ref %d\n", iHttpRef));
    ProtoHttpDestroy(pHttpRef->pProtoHttp);
    // clear state
    memset(pHttpRef, 0, sizeof(*pHttpRef));
}
...
static int32_t _HttpManagerSizeRefPool(HttpManagerRefT *pHttpManager, int32_t iHttpNumRefs)
{
    ...
    {
        NetPrintf(("protohttpmanager: clamping 'pool' request to max %d refs\n", HTTPMANAGER_MAXREFS));
        iHttpNumRefs = HTTPMANAGER_MAXREFS;
    }
    else if (iHttpNumRefs < 1)
    {
        NetPrintf(("protohttpmanager: clamping 'pool' request to min of one ref\n"));
        iHttpNumRefs = 1;
    }
...
void HttpManagerDestroy(HttpManagerRefT *pHttpManager)
{
    ...
    for (iHttpCmd = 0; iHttpCmd < HTTPMANAGER_MAXCMDS; iHttpCmd += 1)
    {
        ...
        {
            NetPrintf(("httpmanager: warning; handle %d not cleaned up\n", pHttpCmd->iHttpHandle));
...
{
    return(_HttpManagerRequestCb(pHttpManager, iHandle, pUrl, NULL, 0, bHeadOnly ? PROTOHTTP_REQUESTTYPE_HEAD : PROTOHTTP_REQUESTTYPE_GET, NULL, NULL));
}
...
{
    return(_HttpManagerRequestCb(pHttpManager, iHandle, pUrl, pData, iDataSize, bDoPut ? PROTOHTTP_REQUESTTYPE_PUT : PROTOHTTP_REQUESTTYPE_POST, NULL, NULL));
}
...
*/
/********************************************************************************F*/
int32_t HttpManagerRequestCb(HttpManagerRefT *pHttpManager, int32_t iHandle, const char *pUrl, const char *pData, int32_t iDataSize, ProtoHttpRequestTypeE eRequestType, ProtoHttpWriteCbT *pWriteCb, void *pWriteCbUserData)
{
    return(_HttpManagerRequestCb(pHttpManager, iHandle, pUrl, pData, iDataSize, eRequestType, pWriteCb, pWriteCbUserData));
}
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'xhtp'      enable xhttp (xbox 360 only, global only) (default=disabled)
        \endverbatim
...
int32_t HttpManagerControl(HttpManagerRefT *pHttpManager, int32_t iHandle, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    {
        NetPrintf(("httpmanager: HttpManagerControl(%d, '%c%c%c%c') failed; unrecognized handle\n", iHandle,
    ...
    if (pHttpCmd != NULL)
    {
        ...
        if (iSelect == 'xhtp')
        {
            NetPrintf(("protohttpmanager: xhttp is a global setting and cannot be enabled for an individual http ref\n"));
            return(-1);
        }
        #endif
    ...
    else // global (not command-specific) selectors
    {
        ...
        if (iSelect == 'auto')
        {
            NetPrintf(("protohttpmanager: auto-update %s\n", iValue ? "enabled" : "disabled"));
        ...
        if (iSelect == 'copy')
        {
            NetPrintf(("protohttpmanager: urlcopy %s\n", iValue ? "enabled" : "disabled"));
        ...
        if (iSelect == 'maxp')
        {
            NetPrintf(("protohttpmanager: setting max pipeline depth to %d\n", iValue));
        ...
        if (iSelect == 'pipe')
        {
            NetPrintf(("protohttpmanager: pipelining %s\n", iValue ? "enabled" : "disabled"));
        ...
        if (iSelect == 'pwka')
        {
            NetPrintf(("protohttpmanager: pipelining without keep-alive %s\n", iValue ? "enabled" : "disabled"));
        ...
        if (iSelect == 'xhtp')
        {
            NetPrintf(("protohttpmanager: enabling xhttp\n"));
            // disable pipelining (not supported)
            pHttpManager->bPipelining = FALSE;
            // remember we've enabled it
            pHttpManager->bXhttpEnabled = TRUE;
            // intentional fall-through to pass down to protohttp refs
        }
        #endif
    ...
    // unhandled
    NetPrintf(("httpmanager: HttpManagerControl(%d, '%c%c%c%c') unhandled\n", iHandle,
        (uint8_t)(iSelect>>24), (uint8_t)(iSelect>>16), (uint8_t)(iSelect>>8), (uint8_t)(iSelect)));
    return(-1);
}
...
int32_t HttpManagerStatus(HttpManagerRefT *pHttpManager, int32_t iHandle, int32_t iSelect, void *pBuffer, int32_t iBufSize)
{
    ...
    if (iHandle > 0)
    {
        ...
        if ((pHttpCmd = _HttpManagerGetCmd(pHttpManager, iHandle)) == NULL)
        {
            NetPrintf(("httpmanager: HttpManagerStatus(%d, '%c%c%c%c') failed; unrecognized handle\n", iHandle,
                (uint8_t)(iSelect>>24), (uint8_t)(iSelect>>16), (uint8_t)(iSelect>>8), (uint8_t)(iSelect)));
            return(-1);
        }
        ...
        if (iSelect == 'urls')
        {
            strnzcpy(pBuffer, pHttpCmd->pUrl, iBufSize);
            return(0);
        }
        ...
        if (pHttpCmd->uState < HTTPMANAGER_CMDSTATE_ACTV)
        {
            ...
            NetPrintf(("httpmanager: HttpManagerStatus(%d, '%c%c%c%c') failed; handle not active or done (state=%d)\n", iHandle,
                (uint8_t)(iSelect >> 24), (uint8_t)(iSelect >> 16), (uint8_t)(iSelect >> 8), (uint8_t)(iSelect),
                pHttpCmd->uState));
            return(-1);
        }
    ...
    else
    {
        ...
        if (iSelect == 'stat')
        {
            if (pBuffer != NULL)
            {
                ...
                {
                    memcpy(pBuffer, &pHttpManager->HttpManagerStats, sizeof(pHttpManager->HttpManagerStats));
                    return(0);
                }
                else
                {
    ...
    // unhandled
    NetPrintf(("httpmanager: HttpManagerStatus(%d, '%c%c%c%c') unhandled\n", iHandle,
        (uint8_t)(iSelect>>24), (uint8_t)(iSelect>>16), (uint8_t)(iSelect>>8), (uint8_t)(iSelect)));
    return(-1);
}
...
void HttpManagerUpdate(HttpManagerRefT *pHttpManager)
{
    ...
    for (iHttpCmd = 0; iHttpCmd < HTTPMANAGER_MAXCMDS; iHttpCmd += 1)
    {
        ...
        if ((pHttpCmd->pUrl != NULL) && (pHttpCmd->uState == HTTPMANAGER_CMDSTATE_IDLE) && (pHttpCmd->pHttpRef == NULL))
        {
            ...
            NetPrintf(("httpmanager: deferred alloc for handle %d\n", pHttpCmd->iHttpHandle));
    ...
    for (iHttpRef = 0; iHttpRef < pHttpManager->iHttpNumRefs; iHttpRef += 1)
    {
        ...
        if (pHttpRef->uHttpState == HTTPMANAGER_REFSTATE_BUSY)
        {
            ...
            {
                NetPrintf(("httpmanager: warning -- ref %d last updated %dms previous (update)\n", iHttpRef, NetTickDiff(uCurTick, pHttpRef->uLastTick)));
            }
