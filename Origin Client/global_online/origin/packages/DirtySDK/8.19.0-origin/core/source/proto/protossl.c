...
#define SSL_SESSHIST_MAX    (4)                             // max number of previous sessions that will be tracked for possible later resumption
...
typedef struct SessionHistoryT
{
    ...
    uint8_t SessionId[32];          //!< session id used to identify session
} SessionHistoryT;
...
typedef struct SecureStateT
{
    ...
    uint8_t SessionId[32];          //!< session id
...
static SessionHistoryT *_SessionHistoryGet(ProtoSSLRefT *pState, struct sockaddr *pPeerAddr, const uint8_t *pSessionId)
{
...
static void _SessionHistoryAdd(ProtoSSLRefT *pState, struct sockaddr *pPeerAddr, const uint8_t *pSessionId, uint32_t iSessionLen, const uint8_t *pMasterSecret)
{
    SessionHistoryT *pSessHist;
...
static int32_t _ProtoSSLUpdateSendClientHello(ProtoSSLRefT *pState)
{
    ...
    *pData++ = (uint8_t)(SSL3_VERSION>>0);
    ...
    if ((pSessHist = _SessionHistoryGet(pState, &pState->PeerAddr, NULL)) != NULL)
    {
        #if DEBUG_RES_SESS
        NetPrintf(("protossl: setting session id for resume\n"));
        #if DEBUG_RAW_DATA
        NetPrintMem(pSessHist->SessionId, sizeof(pSessHist->SessionId), "ClientHello session id");
        #endif
        #endif
        // add sessionid to ClientHello
        *pData++ = (uint8_t)sizeof(pSessHist->SessionId);
        memcpy(pData, pSessHist->SessionId, sizeof(pSessHist->SessionId));
        pData += sizeof(pSessHist->SessionId);
    }
...
static int32_t _ProtoSSLUpdateRecvServerHello(ProtoSSLRefT *pState, const uint8_t *pData)
{
    ...
    {
        NetPrintf(("protossl: _ServerHello: version %d.%d unsupported\n", pData[0], pData[1]));
    ...
    if (*pData == sizeof(pState->SessionHistory[0].SessionId))
    {
        ...
        if ((pSessHist = _SessionHistoryGet(pState, NULL, pSecure->SessionId)) != NULL)
        {
            NetPrintf(("protossl: resuming previous session\n"));
            // copy session id and master secret
            memcpy(pSecure->MasterKey, pSessHist->MasterSecret, sizeof(pSecure->MasterKey));
            memcpy(pSecure->SessionId, pSessHist->SessionId, sizeof(pSecure->SessionId));
            // update timestamp
            pSessHist->uSessionTick = NetTick();
            // update state
    ...
    else
    {
        ...
        NetPrintf(("protossl: session cannot be resumed\n"));
...
static int32_t _ProtoSSLUpdateRecvServerCert(ProtoSSLRefT *pState, const uint8_t *pData)
{
    ...
    {
        NetPrintf(("protossl: bypassing certificate validation\n"));
        return(ST3_RECV_HELLO);
    }
...
static int32_t _ProtoSSLUpdateSendClientKey(ProtoSSLRefT *pState)
{
    ...
    // save session
    _SessionHistoryAdd(pState, &pState->PeerAddr, pSecure->SessionId, sizeof(pSecure->SessionId), pSecure->MasterKey);
...
static int32_t _ProtoSSLUpdateRecv(ProtoSSLRefT *pState, SecureStateT *pSecure)
{
    ...
    if (pSecure->iRecvSize == SSL_MIN_PACKET)
    {
        ...
        if ((pSecure->RecvData[1] != (SSL3_VERSION>>8)) || (pSecure->RecvData[2] != (SSL3_VERSION&255)))
        {
            // proper solution is to turn packet into alert to force shutdown; temp hack is to kill length bytes to avoid processing
