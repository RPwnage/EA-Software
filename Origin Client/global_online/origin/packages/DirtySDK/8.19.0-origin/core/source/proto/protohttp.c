...
static int32_t _ProtoHttpSetAppendHeader(ProtoHttpRefT *pState, const char *pAppendHdr)
{
    ...
    {
        NetPrintf(("protohttp: ignoring set of append header '%s' that is already set\n", pAppendHdr));
        return(0);
    }
...
static int32_t _ProtoHttpFormatRequest(ProtoHttpRefT *pState, const char *pUrl, const char *pData, int32_t iDataLen, ProtoHttpRequestTypeE eRequestType)
{
    ...
    {
        NetPrintf(("protohttp: using proxy server %s\n", pState->strProxy));
        _ProtoHttpParseUrl(pState->strProxy, strKind, sizeof(strKind), strHost, sizeof(strHost), &iPort, &iSecure, &bPortSpecified);
    }
...
/*!
    ...
    \Notes
        ...
        header, as protohttp appends that itself.
...
int32_t ProtoHttpControl(ProtoHttpRefT *pState, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iSelect == 'prxy')
    {
        NetPrintf(("protohttp: [0x%08x] setting %s as proxy server\n", (uintptr_t)pState, pValue));
        strnzcpy(pState->strProxy, pValue, sizeof(pState->strProxy));
    }
    if (iSelect == 'rmax')
    {
