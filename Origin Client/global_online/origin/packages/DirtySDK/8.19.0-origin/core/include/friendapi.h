...
typedef enum FriendApiEventsE
{
    ...
    FRIENDAPI_EVENT_BLOCKEDLIST_UPDATED,        //!< the list of blocked friends has changed   $$todo v9 - rename to FRIENDAPI_EVENT_BLOCKLIST_UPDATED
    FRIENDAPI_EVENT_MSG_RCVED,                  //!< message received
    FRIENDAPI_EVENT_MSG_ATTACH_RCVED,           //!< message attachment received
    FRIENDAPI_EVENT_MSG_ATTACH_RESULT,          //!< message attachment result
    FRIENDAPA_EVENT_MAX                         //!< number of events
} FriendApiEventsE;
...
/*
event data associated with different event types
*/

//! generic event data associated with FriendApi events
typedef struct FriendEventDataT
{
    uint8_t bPresence;                              //!< TRUE - online; FALSE - offline
    int32_t iMsgSize;                               //!< message size
    int8_t strMsgBuf[FRIENDAPI_EVENTMSG_MAXSIZE];   //!< event message
...
typedef void (FriendApiNotifyCbT) (FriendApiRefT *pFriendApiRef, FriendApiEventsE eventType, FriendEventDataT *pEventData, FriendDataT *pFriendData, void *pNotifyCbUserData);
...
// returns the list of friends in the user-provided buffers (also returns corresponding list of 1st-party friend data structures)
...
// returns the current version of the list of friends
