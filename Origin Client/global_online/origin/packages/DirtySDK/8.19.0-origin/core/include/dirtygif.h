...
{
    // image info
    uint8_t     *pImageData;    //!< pointer to start of gif bits
    uint8_t     *pImageEnd;     //!< pointer to end of gif bits
    uint32_t    uWidth;         //!< image width
    uint32_t    uHeight;        //!< image height
    
    // palette info
    uint8_t     *pColorTable;   //!< pointer to gif color table
...
int32_t DirtyGifParse(DirtyGifHdrT *pGifHdr, const uint8_t *pGifData, const unsigned char *pGifEnd);

// decode a GIF palette to 32bit color table
int32_t DirtyGifDecodePalette(DirtyGifHdrT *pGifHdr, uint8_t *pPalette, unsigned char *pPaletteEnd, unsigned char uAlpha);
