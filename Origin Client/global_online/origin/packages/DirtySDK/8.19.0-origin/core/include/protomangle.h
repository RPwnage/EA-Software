...
void ProtoMangleConnect(ProtoMangleRefT *pRef, int32_t iGamePort, const char *pSessID);

// connect to demangler server, using the given socket ref to issue UDP probes instead of creating one
void ProtoMangleConnectSocket(ProtoMangleRefT *pRef, intptr_t uSocketRef, const char *pSessID);
...
// encode XSESSION_INFO (Xenon only)
void ProtoMangleEncodeSession(char *pBuffer, int32_t iBufSize, const void *pSessionInfo);

// decode XSESSION_INFO (Xenon only)
void ProtoMangleDecodeSession(void *pSessionInfo, const char *pBuffer);
