...
TesterModulesT *TesterModulesCreate(void)
{
    ...
    ZMemSet(pState, 0, sizeof(TesterModulesT));
...
int32_t TesterModulesRegister(TesterModulesT *pState, const char *pCommand, ZCommand *pFunctionPtr)
{
    ...
    strnzcpy(pCmd->strName, pCommand, sizeof(pCmd->strName));
...
int32_t TesterModulesGetCommandName(TesterModulesT *pState, int32_t iCommandNum, char *pBuf, int32_t iBufSize)
{
    ...
    if (pCmd->strName[0] != 0)
    {
        strnzcpy(pBuf, pCmd->strName, iBufSize);
    }
...
int32_t TesterModulesDispatch(TesterModulesT *pState, const char *pCommandLine)
{
    ...
    for (pCommand = (char *)pCommandLine; (*pCommand != 0) && (*pCommand <= ' '); ++pCommand)
        ;

    // copy over strCommand and null-terminate
    strnzcpy(strCommand, pCommand, sizeof(strCommand));
    strnzcpy(strCommandLine, pCommand, sizeof(strCommandLine));

    // terminate command name
    for (pCommand = strCommand; (*pCommand != 0) && (*pCommand > ' '); ++pCommand)
        ;
    *pCommand = 0;
    ...
    for (uLoop = 0; uLoop < TESTERMODULES_NUMCOMMANDS_DEFAULT; uLoop++)
    {
        ...
        if( (uListNameSize > 0) && (uListNameSize == uCommandNameSize) &&
            (strncmp(strCommand, pListName, uCommandNameSize) == 0))
        {
            pCmd = &(pState->Commands[uLoop]);
            pEnv = ZCreate(NULL, strCommandLine);
    ...
    { //give the status of the actual command..
...
void TesterModulesHelp(TesterModulesT *pState, const char *pCommand)
{
    ...
    if(pCommand == NULL)
    {
        ...
        for(iLoop = 0; iLoop < TESTERMODULES_NUMCOMMANDS_DEFAULT; iLoop++)
        {
            ...
            {
                strnzcat(strLine, pState->Commands[iLoop].strName, sizeof(strLine));
                ...
                {
                    strnzcat(strLine, " ", sizeof(strLine));
                }
            }
        }
