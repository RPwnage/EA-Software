...
ZFileT ZFileOpen(const char *pFileName, uint32_t uFlags)
{
    ...
    ZMemSet(strMode, 0, sizeof(strMode));
...
int32_t ZFileRead(ZFileT iFileId, void *pData, int32_t iSize)
{
    ...
    iResult = fread(pData, 1, iSize, (FILE *)iFileId);
    iPos = ftell((FILE *)iFileId);

    if (feof((FILE *)iFileId))
    {
        return(iPos);
    }
    else if(ferror((FILE *)iFileId))
    {
        return(0);
    }
    else
...
int32_t ZFileWrite(ZFileT iFileId, void *pData, int32_t iSize)
{
    ...
    iResult = fwrite(pData, iSize, 1, (FILE *)iFileId);
    return(iResult);
}
...
int32_t ZFileMkdir(const char *pPathName)
{
    ...
    for (pPath = strPath; ; pPath += 1)
    {
        ...
        {
            int32_t iResult;
            if ((iResult = mkdir(strPath, S_IRWXU)) != 0)
            {
                if (errno == EEXIST)
                {
