...
static int32_t _TesterCommCheckInput(TesterCommT *pState)
{
    ...
    iInputDataSize = 0;
    pInputLoop = strtok(pInputData, "\r\n");
    while(pInputLoop)
    {
        iInputDataSize++;
        ZMemSet(&pState->LineData, 0, sizeof(pState->LineData));
        pState->LineData.iType = TagFieldGetNumber(TagFieldFind(pInputLoop, "TYPE"), 0);
        TagFieldGetString(TagFieldFind(pInputLoop, "TEXT"), pState->LineData.strBuffer, sizeof(pState->LineData.strBuffer)-1, "");
        // add to the back of the list
        // remember to add one for the terminator
        ZListPushBack(pState->pInputData, &pState->LineData);
        // get the next line
        pInputLoop = strtok(NULL, "\r\n");
    }
...
static int32_t _TesterCommCheckOutput(TesterCommT *pState)
{
...
#endif
    pFileBuf = (char *)ZMemAlloc(TESTERCOMM_FILEBUFFERSIZE);
    ZMemSet(pFileBuf, 0, TESTERCOMM_FILEBUFFERSIZE);
    ...
    while(ZListPeekFront(pState->pOutputData))
    {
        ...
        TagFieldClear(pState->strCommand, sizeof(pState->strCommand));
        TagFieldSetNumber(pState->strCommand, sizeof(pState->strCommand)-1, "TYPE", pState->LineData.iType);
        iResult = TagFieldSetString(pState->strCommand, sizeof(pState->strCommand)-1, "TEXT", pState->LineData.strBuffer);
        if (iResult < 0)
        {
        ...
        iLineLength = ds_strnzcat(pState->strCommand, "\n", sizeof(pState->strCommand));

        // see if we need to write the file first
        if (iLineLength >= (TESTERCOMM_FILEBUFFERSIZE-iWrittenSize))
        {
            // not enough room for the next buffer - write the file out
#if TESTERCOMM_FILEPROFILING
            iTime1 = ZTick();
#endif
            ZFileWrite(iTempFile, pFileBuf, iWrittenSize);
#if TESTERCOMM_FILEPROFILING
            iTime2 = ZTick();
        ...
        // now dump the data into the file buffer
        ZMemCpy((void *)&(pFileBuf[iWrittenSize]), pState->strCommand, iLineLength);
        iWrittenSize += iLineLength;
    }
...
static int32_t _TesterCommConnect(TesterCommT *pState, const char *pParams, uint32_t bIsHost)
{
    ...
    ZMemSet(strInputFile, 0, sizeof(strInputFile));
    ZMemSet(strOutputFile, 0, sizeof(strOutputFile));
    ZMemSet(strControlDir, 0, sizeof(strControlDir));
    TagFieldGetString(TagFieldFind(pParams, "INPUTFILE"),  strInputFile, sizeof(strInputFile), "");
    TagFieldGetString(TagFieldFind(pParams, "OUTPUTFILE"), strOutputFile, sizeof(strOutputFile), "");
    ...
    ZMemSet((void *)pInterfaceData->strControlDir,     0, sizeof(pInterfaceData->strControlDir));
    ZMemSet((void *)pInterfaceData->strTempPathFile,   0, sizeof(pInterfaceData->strTempPathFile));
    ZMemSet((void *)pInterfaceData->strInputPathFile,  0, sizeof(pInterfaceData->strInputPathFile));
    ZMemSet((void *)pInterfaceData->strOutputPathFile, 0, sizeof(pInterfaceData->strOutputPathFile));
...
static int32_t _TesterCommDisconnect(TesterCommT *pState)
{
    ...
    ZMemSet((void *)pData->strControlDir,     0, sizeof(pData->strControlDir));
    ZMemSet((void *)pData->strTempPathFile,   0, sizeof(pData->strTempPathFile));
    ZMemSet((void *)pData->strInputPathFile,  0, sizeof(pData->strInputPathFile));
    ZMemSet((void *)pData->strOutputPathFile, 0, sizeof(pData->strOutputPathFile));
...
void TesterCommAttachFile(TesterCommT *pState)
{
    ...
    ZMemSet(pState->pInterface->pData, 0, sizeof(TesterCommFileT));
}
