...
static char *_ds_sockaddrtostr(char *pBuffer, int32_t iBufSize, struct sockaddr *pSockAddr)
{
    ...
    {
        ds_snzprintf(pBuffer, iBufSize, "[%a]%s", SockaddrInGetAddr(pSockAddr), strPort);
    }
    if (pSockAddr->sa_family == AF_INET6)
    {
        if (SockaddrIn6IsIPv4(pSockAddr))
        {
            ds_snzprintf(pBuffer, iBufSize, "[%a]%s", SockaddrIn6GetAddr4(pSockAddr6), strPort);
        }
        else if (SockaddrIn6IsNAT64(pSockAddr))
        {
            ds_snzprintf(pBuffer, iBufSize, "[64:ff9b::%a]%s", SockaddrIn6GetAddr4(pSockAddr6), strPort);
        }
        else
        {
            uint16_t *pWords = (uint16_t *)pSockAddr6->sin6_addr.s6_addr;
            ds_snzprintf(pBuffer, iBufSize, "[%x:%x:%x:%x:%x:%x:%x:%x]%s",
