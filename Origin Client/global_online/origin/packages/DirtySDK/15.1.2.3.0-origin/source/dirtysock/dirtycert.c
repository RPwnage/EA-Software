...
#define DIRTYCERT_URL               "https://gosca.ea.com:44325/redirector"
...
struct DirtyCertRefT
{
    ...
    char strServiceName[128];       //!< service name used to identify requester (required)
...
{
#if DIRTYCERT_URL_ENCRYPTED
    static const uint8_t _DirtyCert_strKey[] =
    {
        0x6E,0x8D,0x61,0x52, 0x44,0x64,0x48,0xDF, 0x23,0xA7,0x0C,0x76, 0xAE,0xE2,0x9B,0xA0,
        0xE5,0xC5,0xED,0x47, 0x55,0x71,0x2B,0x70, 0x41,0x83,0x8D,0x28, 0x9B,0xE2,0x82,0x9D,
    };
    static const uint8_t _DirtyCert_strEnc[] =
    {
        0x01,0xA3,0xD8,0x39, 0xAF,0xCF,0x25,0xEE, 0x24,0xC3,0x1D,0x1D, 0x5F,0x78,0xDA,0x66,
        0xA3,0xAD,0x9C,0xD1, 0xD8,0x06,0xED,0x17, 0x9E,0x44,0xE0,0x0A, 0x1E,0x13,0xE8,0x56,
        0xAB,0xBB,0x24,0x7A, 0x1D,0x88,0x59,0x11, 0x8C,0x3E,0x4C,0x87, 0x53,0x0E,0x00,0x18,
        0x78,0x07,0xB7,0x33, 0xA9,0x5F,0x76,0x09, 0x59,0xEC,0x2B,0xA5, 0x0B,0x7D,0x46,0x48,
    };
    ds_memcpy_s(_DirtyCert_strRedirectorUrl, sizeof(_DirtyCert_strRedirectorUrl), _DirtyCert_strEnc, sizeof(_DirtyCert_strEnc));
    return(CryptArc4StringEncryptStatic(_DirtyCert_strRedirectorUrl, sizeof(_DirtyCert_strRedirectorUrl), _DirtyCert_strKey, sizeof(_DirtyCert_strKey), DIRTYCERT_URL));
#else
    return(0);
#endif
}
...
static int32_t _DirtyCertCARequestFree(DirtyCertRefT *pState, DirtyCertCARequestT *pRequest, int32_t iRequestId)
{
    ...
    else if (--pRequest->iRefCount == 0)
    {
        ...
        if (--pState->iCount == 0)
        {
            /*$$ TODO - This should be a 'close' not an 'abort'.  The end result on the socket (closing it)
              is the same, but the semantics imply that this is an error, where in fact we just want to
              terminate a keep-alive connection */
            ProtoHttpAbort(pState->pHttp); // this should be a 'close'
        }
        // success
        iResult = 1;
    }
...
int32_t DirtyCertControl(int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'prld')
    {
        NetPrintf(("dirtycert: %s CA preload\n", (iControl != 0) ? "enabling" : "disabling"));
        pState->bPreload = (iControl != 0) ? TRUE : FALSE;
        iResult = 0;
    }
    // set service name
    if (iControl == 'snam')
