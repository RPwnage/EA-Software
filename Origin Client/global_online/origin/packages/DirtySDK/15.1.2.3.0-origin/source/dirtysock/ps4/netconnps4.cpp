...
int32_t NetConnStartup(const char *pParams)
{
    ...
    if (pRef != NULL)
    {
        NetPrintf(("netconnps4: NetConnStartup() called with params='%s' while the module is already started\n", pParams));
    ...
    memset(strThreadCpuAffinity, 0, sizeof(strThreadCpuAffinity));
...
int32_t NetConnControl(int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue, void *pValue2)
{
    ...
    if (iControl == 'pctl')
    {
        return(_NetConnNpAsyncRequest(pRef, iValue, (NetConnAsyncRequestDataT*)&pRef->NpUsers.aParentalControls[iValue], _NetConnParentalControlsRequest));
    }
    
    // request refresh of PS+ membership
    if (iControl == 'pspl')
    {
        pRef->NpUsers.aPsPlus[iValue].Param.size = sizeof(pRef->NpUsers.aPsPlus[iValue].Param);
        pRef->NpUsers.aPsPlus[iValue].Param.userId = pRef->NpUsers.aSceUserServiceUserId[iValue];
        pRef->NpUsers.aPsPlus[iValue].Param.features = iValue2;
...
int32_t NetConnStatus(int32_t iKind, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    if ((iKind == 'npid') && (iBufSize >= (int32_t)sizeof(SceNpId)) && (iData < NETCONN_MAXLOCALUSERS))
    {
        if (iData >= 0)
        {
            ds_memcpy(pBuf, &pRef->NpUsers.aSceNpid[iData], sizeof(SceNpId));
            return(0);
        }
    ...
    if ((iKind == 'npst') && (iData < NETCONN_MAXLOCALUSERS))
    {
        if (iData >= 0)
        {
            return(pRef->NpUsers.aSceNpState[iData]);
        }
    ...
    if (iKind == 'soid')
    {
        ...
        {
            SceUserServiceUserId sceUserServiceUserId = pRef->NpUsers.aSceUserServiceUserId[iData];

            // ignore if user is not SIGNEDIN with PSN
            if (pRef->NpUsers.aSceNpState[iData] == SCE_NP_STATE_SIGNED_IN)
            {
                // check if the cache is dirty
                NetCritEnter(&pRef->Common.crit);
                if( pRef->aSceNpOnlineIdCache[iData].invalid == TRUE )
                {
                    //get a fresh value for onlineid
                    pRef->aSceNpOnlineIdCache[iData].result = _NetConnSaveErr(pRef, sceNpGetOnlineId(sceUserServiceUserId, &pRef->aSceNpOnlineIdCache[iData].onlineId));
                    pRef->aSceNpOnlineIdCache[iData].invalid = FALSE;
                }

                if(pRef->aSceNpOnlineIdCache[iData].result == SCE_OK)
                {
                    ds_memcpy(pBuf, &pRef->aSceNpOnlineIdCache[iData].onlineId, iBufSize);
                }
                else
                {
                    NetPrintf(("netconnps4: sceNpGetOnlineId(0x%08x) (idx=%d) failed with %s\n", sceUserServiceUserId, iData, DirtyErrGetName(pRef->aSceNpOnlineIdCache[iData].result)));
                }
                NetCritLeave(&pRef->Common.crit);

                return(pRef->aSceNpOnlineIdCache[iData].result);
            }
            NetPrintf(("netconnps4: NetConnStatus('soid') sceUserServiceUserId(0x%08x) (idx=%d) no op, user status(%d) not signed in\n", sceUserServiceUserId, iData, pRef->NpUsers.aSceNpState[iData]));
        }
        else
        {
    ...
    if (iKind == 'pctl')
    {
        ...
        {
            return(_NetConnReadAsyncResponse(pRef, iData, pBuf, iBufSize, (NetConnAsyncRequestDataT*)&pRef->NpUsers.aParentalControls[iData], _NetConnParentalControlsRequest, _NetConnParentalControlsResponse));
        }
    }

    if (iKind == 'pspl')
    {
        if (iData < 0 || iData >= NETCONN_MAXLOCALUSERS || pBuf == NULL || iBufSize != sizeof(pRef->NpUsers.aPsPlus[iData].Result.authorized))
        {
        ...
        else
        {
            ...
            if (iRet > 0)
            {
                NetPrintf(("netconnps4: NetConnStatus('pspl') PS+ membership request for user (%d) returned (%s).\n", iData, pRef->NpUsers.aPsPlus[iData].Result.authorized ? "true" : "false"));
                ds_memcpy(pBuf, &pRef->NpUsers.aPsPlus[iData].Result.authorized, sizeof(pRef->NpUsers.aPsPlus[iData].Result.authorized));
            }
            else if (iRet == -2)
            {
    ...
    if (iKind == 'mask')
    {
        ...
        for (iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; ++iIndex)
        {
            if (iIndex >= 0 && pRef->NpUsers.aSceNpState[iIndex] == SCE_NP_STATE_SIGNED_IN)
            {
                iUserMask |= (1 << iIndex);
            }
        }
    ...
    if (iKind == 'gtag')
    {
        ...
        if (pRef->NpUsers.aSceNpState[iData] == SCE_NP_STATE_SIGNED_IN)
        {
            ds_strnzcpy((char*)pBuf, pRef->NpUsers.aSceNpid[iData].handle.data, iBufSize);
            return(0);
        }
