...
typedef struct NetConnRefT
{
    ...
    NetConnStateE                   eNetConnStateBeforeSuspd;    //!< netconn state before suspend. This is use to resume to the correct state

    uint32_t                        uConnStatus;                 //!< connection status (surfaced to user)

    uint8_t                         bUpdateNetworkStatus;        //!< starts a netowrk status update
    uint32_t                        uNetworkStatusRetryCount;    //!< network status retry count
    uint32_t                        uNetworkStatusRetryTimer;    //!< netwrok status retry timer
    HANDLE                          hNetworkStatusEvent;         //!< event use to wake the network status update thread
    ...
    uint8_t                         bShowAccountPicker;          //!< show the accounter picker if there are no signed in user
    ...
    NetConnEvnStateE                eNetConnEnvState;                   //!< TRUE if Environment Determination is stated
    IAsyncOperation<TitleStorage::TitleStorageBlobResult^> ^PlatEnvAsyncOp;

    uint8_t                         bDelayConn;                  //!< delay connect due external cleanup
    uint8_t                         bProcessingCleanupList;      //!< TRUE if we are processing the cleanup list
...
/*!
    ...
    \Input *pRef             - pointer to net NetConn module ref
     ...
     \Output
        int32_t              -  0 for when we exit

    \Version 09/21/2015 (tcho)
*/
/********************************************************************************F*/
static int32_t _NetworkStatusThread(NetConnRefT *pRef)
{
    #if DIRTYCODE_LOGGING
    ...
    while (pRef->hNetworkStatusThread != 0)
    {
        ...
        if (WaitForSingleObject(pRef->hNetworkStatusEvent, INFINITE) == WAIT_OBJECT_0)
        {
            ...
            NetPrintf(("netconnxboxone: network status thread woke up, attempting to update network connectivity level (thid=%d)\n", GetCurrentThreadId()));
            ...
            if (bRetry == FALSE)
            {
                if (eCurrentConnectivityLevel != pRef->eConnectivityLevel)
                {
                    ...
                    #if DIRTYCODE_LOGGING
                    // display address
                    NetPrintf(("netconnxboxone: connectivity level %s (%d) addr=%a\n (thid=%d)\n", _strConnectivityLevel[(int)pRef->eConnectivityLevel], pRef->eConnectivityLevel, NetConnStatus('addr', 0, NULL, 0), GetCurrentThreadId()));
                    #endif
                }
...
static void _NetConnGetEnvironment(NetConnRefT *pRef)
{
    ...
    pRef->eNetConnEnvState = NetConnEvnStateE::ST_ENV_INPROGRESS;
    ...
    else if ((ds_stricmp(pRef->strSandboxId, NETCONNXBOXONE_CERT_SANDBOX) == 0) || (ds_stricmp(pRef->strSandboxId, NETCONNXBOXONE_INTERNAL_CERT_SANDBOX) == 0))
    {
    ...
    {
        pRef->eNetConnEnvState = NetConnEvnStateE::ST_ENV_COMPLETED;
        return;
    }
                ...
                case Windows::Foundation::AsyncStatus::Error:
                {
                    ...
                    if ((pRef->uEnvTimer == 0) || (NetTickDiff(NetTick(), pRef->uEnvTimer) < NETCONNXBOXONE_ENV_TIMEOUT))
                    {
                        ...
                        if ((pRef->eNetConnState == NetConnStateE::ST_RUNNING) && (pRef->uConnStatus >> 24 == '~') && (pRef->eNetConnRunState == NetConnRunStateE::ST_RUN_ENV))
                        {
                            ...
                            pRef->eNetConnEnvState = NetConnEvnStateE::ST_ENV_INVALID;
...
static void _NetConnRunTokenState(NetConnRefT * pRef)
{
    ...
    if (NetConnQuery(NULL, NULL, 0) >= 0)
    {
        ...
        {
            // transiton to running env state
...
{
    if (pRef->eNetConnEnvState == NetConnEvnStateE::ST_ENV_INVALID)
    {
    ...
    else if (pRef->eNetConnEnvState == NetConnEvnStateE::ST_ENV_COMPLETED)
    {
...
*/
/********************************************************************************F*/
static void _NetConnCleanupState(NetConnRefT * pRef)
{
    ...
    if (pRef->iExternalCleanupListCnt == 0)
    {
        if (pRef->bDelayConn)
        {
            // cleanup is done we can connect
            if (pRef->bNoLogin == TRUE)
            {
               NetPrintf(("netconnxboxone: skipping login and connect process because -nologin was used\n"));
               // transiton to running env state
               _NetConnMoveToNextRunState(pRef, NetConnRunStateE::ST_RUN_ONLINE);
               _NetConnUpdateConnStatus(pRef, '+onl');
            }
...
/*F********************************************************************************/
/*!
    \Function   _NetConnSuspendedState
...
static void _NetConnUpdate(void *pData, uint32_t uTick)
{
    ...
    switch(pRef->eNetConnState)
    {
        ...
        case NetConnStateE::ST_FAILED:
         ...
         default:
            // in theory we should never get here
            NetPrintf(("netconnxboxone: encountered an unknown state"));
            _NetConnMoveToNextState(pRef, NetConnStateE::ST_FAILED);
            _NetConnUpdateConnStatus(pRef, '-sta');
            break;
    }
...
int32_t NetConnStartup(const char *pParams)
{
    ...
    if (pRef != NULL)
    {
        NetPrintf(("netconnxboxone: NetConnStartup() called with params='%s' while the module is already started\n", pParams));
    ...
    if ((pRef->hNetworkStatusThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)&_NetworkStatusThread, (LPVOID)pRef, 0, (LPDWORD)&iPid)) != NULL)
    {
        // crank priority since we deal with real-time events
        if (SetThreadPriority(pRef->hNetworkStatusThread, THREAD_PRIORITY_NORMAL) == FALSE)
        {
            NetPrintf(("netconnxboxone: warning %d adjusting network status thread priority\n", GetLastError()));
        }

        // set the thread affinity based on startup params
        if ((pRef->iThreadCpuAffinity != 0) && (SetThreadAffinityMask(pRef->hNetworkStatusThread, pRef->iThreadCpuAffinity) == 0))
        {
            NetPrintf(("netconnxboxone: failed to set network status thread's cpu affinity (err=%d)\n", GetLastError()));
        }

        // we dont need thread handle any more
        CloseHandle(pRef->hNetworkStatusThread);
...
int32_t NetConnShutdown(uint32_t bStopEE)
{
    ...
    if (pRef->iExternalCleanupListCnt != 0 || pRef->PlatEnvAsyncOp != nullptr)
    {
        #if DIRTYCODE_LOGGING
