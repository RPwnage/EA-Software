...
{
    pConduitRef->pMixerRef = pMixerRef;
}
...
void VoipConduitReceiveVoiceData(VoipConduitRefT *pConduitRef, VoipUserT *pRemoteUser, const uint8_t *pData, int32_t iDataSize)
{
    ...
    // make sure we have a mixer ref
    if (pConduitRef->pMixerRef == NULL)
    {
        NetPrintf(("voipconduit: error; no mixer available\n"));
        return;
    }
    ...
    for (iConduit = 0; iConduit < pConduitRef->iNumConduits; iConduit++)
    {
        ...
        if (VOIP_SameUser(&pConduit->PacketUser, pRemoteUser))
        {
            ...
            if (VoipMixerAccumulate(pConduitRef->pMixerRef, (uint8_t *)pData, iDataSize, -1, iConduitMask, iConduit) < 0)
            {
                NetPrintfVerbose((pConduitRef->iVerbosity, 3, "voipconduit: [%c] discarding packet due to mixbuffer overflow\n", VOIP_ConduitName(iConduit)));
            }
            break;
        }
    }
    ...
    {
        NetPrintf(("voipconduit: could not find a conduit for voice packet from user '%s'\n", pRemoteUser->strUserId));
    }
    #endif
}
...
void VoipConduitRegisterUser(VoipConduitRefT *pConduitRef, VoipUserT *pRemoteUser, uint32_t bRegister)
{
    ...
    for (iConduit = 0; iConduit < pConduitRef->iNumConduits; iConduit++)
    {
        ...
        if (bRegister)
        {
            ...
            {
                NetPrintf(("voipconduit: [%c] registering user '%s'\n", VOIP_ConduitName(iConduit), pRemoteUser->strUserId));
        ...
        else
        {
            ...
            {
                NetPrintf(("voipconduit: [%c] unregistering user '%s'\n", VOIP_ConduitName(iConduit), pRemoteUser->strUserId));
    ...
    if (iConduit >= pConduitRef->iNumConduits)
    {
        NetPrintf(("voipconduit: could not %s user '%s'\n",
