...
*/
/********************************************************************************F*/
static void _VoipTunnelSuspendGame(VoipTunnelRefT *pVoipTunnel, VoipTunnelClientT *pClient)
{
    int32_t iGameClient;
...
static void _VoipTunnelUnsuspendGame(VoipTunnelRefT *pVoipTunnel, VoipTunnelClientT *pClient)
{
    ...
    memcpy(pClient->aClientIds, pSuspendInfo->aClientIds, sizeof(pClient->aClientIds));
    ...
    // clear out the data
    memset(pSuspendInfo, -1, sizeof(*pSuspendInfo));
...
int32_t VoipTunnelClientListAdd2(VoipTunnelRefT *pVoipTunnel, const VoipTunnelClientT *pClientInfo, VoipTunnelClientT **ppNewClient, int32_t iStartIndex)
{
    ...
    if ((pNewClient = VoipTunnelClientListMatchId(pVoipTunnel, pClientInfo->uClientId)) == NULL)
    {
        ...
        memset(pNewClient->aSuspendedData, -1, sizeof(pNewClient->aSuspendedData));
    }
    ...
    {
        _VoipTunnelSuspendGame(pVoipTunnel, pNewClient);
        ...
        memset(pNewClient->aClientIds, 0, sizeof(pNewClient->aClientIds)); // reset the ids so it can be updated later
...
void VoipTunnelClientListDel(VoipTunnelRefT *pVoipTunnel, VoipTunnelClientT *pClient, int32_t iGameIdx)
{
    ...
    if (pClient->iGameIdx != iGameIdx)
    {
        ...
        {
            NetPrintf(("voiptunnel: could not find game %d for client 0x%08x suspended data\n", iGameIdx, pClient->uClientId));
            return;
        }
    }
    ...
    {
        // only remove the gap if the not at the end
...
int32_t VoipTunnelStatus(VoipTunnelRefT *pVoipTunnel, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'nusr')
    {
        ...
        {
            NetPrintf(("voiptunnel: invalid game index passed to 'nusr' selector defaulting to -1\n"));
            iNumClients = -1;
        }
        return(iNumClients);
    }
