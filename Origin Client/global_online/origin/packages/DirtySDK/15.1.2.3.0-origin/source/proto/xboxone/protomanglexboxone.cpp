...
const char *_strStates[] =
{
    ...
    "PROTOMANGLE_STATE_MULTI_UNLOCK",
...
typedef enum ProtoMangleAssocStateE
{
    ...
    PROTOMANGLE_STATE_MULTI_UNLOCK,      //!< unlocking multiplayer capability is in progress
...
struct ProtoMangleRefT      //!< module state
{
    ...
    NetCritT MangleCrit;                        //!< critical section for thread-safety of async op cancellation
    ...
    uint8_t bMultiplayerSessionAllowed;         //!< TRUE if multiplayer session was successfully allowed
    uint8_t bSecureBypass;                      //!< TRUE if secure bypass is enabled by template, else FALSE
...
typedef struct ProtoMangleStateT
{
    ...
    int32_t iMapRefCount;                                                       //!< number of protomangle instances using _ProtoMangle_State.refRemoteHostRefCountMap
} ProtoMangleStateT;
...
*/
/********************************************************************************F*/
void _ProtoMangleIncomingAssociationEventHandler(ProtoMangleRefT *pRef, SecureDeviceAssociation ^refSecurityAssociation)
{
...
static int32_t _ProtoMangleAddIPv6ToIPv4Mapping(ProtoMangleRefT *pRef, ProtoMangleSecurityAssocT *pAssoc)
{
    ...
    */
    if ((iIpv4Addr = SocketControl(NULL, '+ip6', sizeof(pAssoc->remoteSocketAddress), &pAssoc->remoteSocketAddress, NULL)) < 0)
    {
...
static void _ProtoMangleUpdateIPv6ToIPv4Mapping(ProtoMangleRefT *pRef, ProtoMangleSecurityAssocT *pAssoc, SOCKADDR_STORAGE *pNewSocketAddress)
{
    ...
    */
    if ((iIpv4Addr = SocketControl(NULL, '~ip6', sizeof(*pNewSocketAddress), &pAssoc->remoteSocketAddress, pNewSocketAddress)) < 0)
    {
...
*/
/********************************************************************************F*/
void _ProtoMangleProcessIncomingSAs(ProtoMangleRefT *pRef)
{
                    ...
                    try
                    {
                        if (refSecurityAssociation->RemoteSecureDeviceAddress->Compare(pAssoc->refSecureDeviceAddr) == 0)
                        {
                            ...
                            if (bAddressObtained && memcmp(&pAssoc->remoteSocketAddress, &newRemoteSocketAddress, sizeof(newRemoteSocketAddress)) != 0)
                            {
                                ...
                                memcpy(&pAssoc->remoteSocketAddress, &newRemoteSocketAddress, sizeof(pAssoc->remoteSocketAddress));
                            }
    ...
    if (pAssoc->refAsyncAction == nullptr)
    {
        ...
        if (refSecurityAssociation != nullptr)
        {
            ...
            if (_ProtoMangleRemoteHostDelRef(pRef, pAssoc->refRemoteCanonicalName + L":" + pAssoc->refRemotePort) == 0)
            {
                ...
                {
                    NetPrintfVerbose((pRef->iVerbose, 1, "protomanglexboxone: [%p][%p] destruction of security association (state:%S) for %s %d is left to remote peer\n",
                        pRef, pAssoc, refSecurityAssociation->State.ToString()->Data(), (pAssoc->bInKillList?"killed clnt formerly at idx":"actv clnt at idx"), pAssoc->iClientIndex));
...
static void _ProtoMangleUpdateInit(ProtoMangleRefT *pRef, ProtoMangleSecurityAssocT *pAssoc)
{
    ...
    if (pKilledAssoc == NULL)
    {
        ...
        _ProtoMangleMoveToNextState(pRef, pAssoc, PROTOMANGLE_STATE_MULTI_UNLOCK);
    }
}
...
/*F********************************************************************************/
/*!
    \Function    _ProtoMangleUpdateMultiplayerUnlock

    \Description
        Asynchronous unlocking of multiplayer activity.
...
*/
/*******************************************************************************F*/
static void _ProtoMangleUpdateMultiplayerUnlock(ProtoMangleRefT *pRef, ProtoMangleSecurityAssocT *pAssoc)
{
    // verify if multiplayer session is already allowed
    if (!pRef->bMultiplayerSessionAllowed)
    {
        /*
        August 2013 - Info from MS
        Launch titles will not be required to explicitly allow / disallow multiplayer sessions on
        SecureDeviceAssociationTemplates in order to establish Security Associations between
        peers or between the console and Xbox Cloud Compute servers.
        The related APIs will be removed from the XDK.

        Reason for change: The implementation backing this requirement was being delivered late
        in the release. Given its timing and the potential for slowing down certification the 
        decision was made to defer the implementation of these APIs until after launch.

        Launch titles will not be required to call these APIs.

        $todo - fix this when MS makes it mandatory again
        */

        pRef->bMultiplayerSessionAllowed = TRUE;
    }
    else
    {
        NetPrintfVerbose((pRef->iVerbose, 1,"protomanglexboxone: [%p] multiplayer session already allowed\n", pRef));
    }
...
static void _ProtoMangleUpdateAssoc(ProtoMangleRefT *pRef, ProtoMangleSecurityAssocT *pAssoc)
{
    ...
    force cleanup of security association data space when:
        1 - it is still in init state or it has been moved to a termination state (done, failed, timedout), and
    ...
    */
    ...
    // waiting for completion of multiplayer unlock
    if (pAssoc->eMangleState == PROTOMANGLE_STATE_MULTI_UNLOCK)
    {
        _ProtoMangleUpdateMultiplayerUnlock(pRef, pAssoc);
    }
...
static int32_t _ProtoMangleDestroyInternal(void *pDestroyData)
{
    ...
    if (pRef->pSecurityAssocKillList == NULL)
    {
        ...
        _ProtoMangle_State.iMapRefCount -= 1;
        NetPrintfVerbose((pRef->iVerbose, 1, "protomanglexboxone: [%p] ref count on global remote host ref counting map decremented to %d\n", pRef, _ProtoMangle_State.iMapRefCount));
        if (_ProtoMangle_State.iMapRefCount == 0)
        {
            _ProtoMangle_State.refRemoteHostRefCountMap->Clear();
            _ProtoMangle_State.refRemoteHostRefCountMap = nullptr;
            NetPrintfVerbose((pRef->iVerbose, 1, "protomanglexboxone: [%p] global remote host ref counting map destroyed\n", pRef));
        }
...
ProtoMangleRefT *ProtoMangleCreate(const char *pServer, int32_t iPort, const char *pGameID, const char *pLKey)
{
    ...
    iSize = sizeof(*pRef) + sizeof(ProtoMangleClientT) * iMaxClients;
    ...
    {
        memset(&_ProtoMangle_State, 0, sizeof(_ProtoMangle_State));
        _ProtoMangle_bStateInitialized = TRUE;
        NetPrintfVerbose((pRef->iVerbose, 1, "protomanglexboxone: [%p] global remote host ref counting map created\n", pRef));
    }
    if (_ProtoMangle_State.iMapRefCount == 0)
    {
    ...
    _ProtoMangle_State.iMapRefCount += 1;
    NetPrintfVerbose((pRef->iVerbose, 1, "protomanglexboxone: [%p] ref count on global remote host ref counting map incremented to %d\n", pRef, _ProtoMangle_State.iMapRefCount));
    ...
    // check acceptor socket description for debug communications allowance
    if ((int32_t)(refTemplate->AcceptorSocketDescription->AllowedUsages & (Windows::Xbox::Networking::SecureDeviceSocketUsage::SendInsecure|Windows::Xbox::Networking::SecureDeviceSocketUsage::ReceiveInsecure)) != 0)
    {
        NetPrintfVerbose((pRef->iVerbose, 0, "protomanglexboxone: [%p] acceptor socket description '%S' allows debug communications; secure communications will be disabled\n",
            pRef, refTemplate->AcceptorSocketDescription->Name->ToString()->Data()));
        pRef->bSecureBypass = TRUE;
    }
    // check initiator socket description for debug communications allowance
    if ((int32_t)(refTemplate->InitiatorSocketDescription->AllowedUsages & (Windows::Xbox::Networking::SecureDeviceSocketUsage::SendInsecure|Windows::Xbox::Networking::SecureDeviceSocketUsage::ReceiveInsecure)) != 0)
    {
        NetPrintfVerbose((pRef->iVerbose, 0, "protomanglexboxone: [%p] initiator socket description '%S' allows debug communications; secure communications will be disabled\n",
            pRef, refTemplate->InitiatorSocketDescription->Name->ToString()->Data()));
        pRef->bSecureBypass = TRUE;
    }
...
void ProtoMangleConnect2(ProtoMangleRefT *pRef, int32_t iClientIndex, const char *pAddr, int32_t iAddrSize)
{
    ...
    memset(pRef->aClients[iClientIndex].pAssoc, 0, sizeof(ProtoMangleSecurityAssocT));
...
int32_t ProtoMangleComplete(ProtoMangleRefT *pRef, int32_t *pAddr, int32_t *pPort)
{
    ...
    switch (pRef->aClients[iClientIndex].pAssoc->eMangleState)
    {
        ...
        case PROTOMANGLE_STATE_MULTI_UNLOCK:
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            secu: true/false indication of whether security is enabled (based on template settings) for this ref
