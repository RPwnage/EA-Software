...
static void _ProtoHttpServResetState(ProtoHttpServRefT *pHttpServ, ProtoHttpServThreadT *pHttpThread)
{
    ...
    memset(&pHttpThread->ResponseInfo, 0, sizeof(pHttpThread->ResponseInfo));
...
static int32_t _ProtoHttpServParseHeader(ProtoHttpServRefT *pHttpServ, ProtoHttpServThreadT *pHttpThread)
{
    ...
    {
        if (!ds_strnicmp(_ProtoHttpServ_strRequestNames[iType], pHttpThread->RequestInfo.strHeader, (int32_t)strlen(_ProtoHttpServ_strRequestNames[iType])))
        {
            pHttpThread->RequestInfo.eRequestType = (ProtoHttpRequestTypeE)iType;
            break;
        }
    }
    ...
    // find url
    if ((pHdr = strchr(pHttpThread->RequestInfo.strHeader, '/')) == NULL)
    {
        // not a valid header
    ...
    ds_strsubzcpy(pHttpThread->RequestInfo.strUrl, sizeof(pHttpThread->RequestInfo.strUrl), pHdr, (int32_t)(pEnd-pHdr));
    _ProtoHttpServLogPrintfVerbose(pHttpServ, pHttpThread, 1, "url=%s\n", pHttpThread->RequestInfo.strUrl);

    /* if the beginning of the query string
       was found, set that to the beginning so we can
    ...
    if (*pEnd == '?')
    {
        ...
        {
            // not a valid header
            _ProtoHttpServLogPrintf(pHttpServ, pHttpThread, "could not find query string end\n");
            pHttpThread->ResponseInfo.eResponseCode = PROTOHTTP_RESPONSE_BADREQUEST;
            return(-3);
        }
        ds_strsubzcpy(pHttpThread->RequestInfo.strQuery, sizeof(pHttpThread->RequestInfo.strQuery), pHdr, (int32_t)(pEnd-pHdr));
    ...
    _ProtoHttpServLogPrintfVerbose(pHttpServ, pHttpThread, 0, "processing %s %s from %a:%d user-agent=%s\n", 
        _ProtoHttpServ_strRequestNames[iType], pHttpThread->RequestInfo.strUrl, 
...
static int32_t _ProtoHttpServUpdateSendBody(ProtoHttpServRefT *pHttpServ, ProtoHttpServThreadT *pHttpThread)
{
    ...
    for (bError = FALSE; pHttpThread->iContentSent < pHttpThread->ResponseInfo.iContentLength; )
    {
        ...
        if (pHttpThread->iBufOff == pHttpThread->iBufLen)
        {
            ...
            // read the data
            if ((pHttpThread->iBufLen = pHttpServ->pSendCb(&pHttpThread->ResponseInfo, pHttpThread->pBuffer+iReadOff, iReadMax, pHttpServ->pUserData)) > 0)
            {
...
void ProtoHttpServDestroy(ProtoHttpServRefT *pHttpServ)
{
    ...
    for (pHttpThread = pHttpServ->pThreadList; pHttpThread != NULL; pHttpThread = pHttpThreadNext)
    {
