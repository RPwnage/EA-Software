...
#define PROTOTUNNEL_PACKETRECVFAIL_OUTOFORDER   (0)    //!< out-of-order packet discard
...
/*!
    ...
    \Output
        int32_t         - PROTOTUNNEL_PACKETRECVFAIL_*, or number of sub-packets decoded
...
/*!
    ...
    \Output
        int32_t         - negative=PROTOTUNNEL_PACKETRECVFAIL_*, else success

    \Notes
        This function handles PROTOTUNNEL_PACKETRECVFAIL_OUTOFORDER, and either
        successfully recovers the out-of-order packet, or returns a VALIDATE
        failure.  This is the only function that processes the OUTOFORDER return
        code from _ProtoTunnelDecryptAndValidatePacket2().
...
static int32_t _ProtoTunnelDecryptAndValidatePacket(ProtoTunnelRefT *pProtoTunnel, ProtoTunnelT *pTunnel, uint32_t *pHeadOffset, uint8_t *pPacketData, int32_t iRecvLen, uint8_t bUpdateState)
{
    ...
    if ((iResult = _ProtoTunnelDecryptAndValidatePacket2(pProtoTunnel, pTunnel, pHeadOffset, pPacketData, iRecvLen, bUpdateState, TRUE)) == PROTOTUNNEL_PACKETRECVFAIL_OUTOFORDER)
    {
        ...
        if ((iResult = _ProtoTunnelDecryptAndValidatePacket2(pProtoTunnel, pTunnel, pHeadOffset, pPacketData, iRecvLen, bUpdateState, FALSE)) == PROTOTUNNEL_PACKETRECVFAIL_OUTOFORDER)
        {
...
static int32_t _ProtoTunnelRecvData(ProtoTunnelRefT *pProtoTunnel, uint16_t *pRemotePortList, int32_t iPortListSize, uint32_t *pHeadOffset, uint8_t *pPacketData, int32_t iRecvLen, struct sockaddr *pRecvAddr)
{
    ...
    if (iTunnel < pProtoTunnel->iMaxTunnels)
    {
        ...
        {
            NetPrintf(("prototunnel: [%p][%04d] attempting to rematch tunnel\n", pProtoTunnel, (pTunnel - pProtoTunnel->Tunnels)));
            /* attempt rematch; if rematch is successful we do not update iNumPackets, as we need the caller to call us again
               following a successful rematch.  if rematch fails here, we do not want to try again, so we set NOMATCH */
            if (!_ProtoTunnelMatchTunnel(pProtoTunnel, pTunnel, pPacketData, iRecvLen))
            {
...
static int32_t _ProtoTunnelRecv(ProtoTunnelRefT *pProtoTunnel, uint8_t *pPacketData, int32_t iRecvLen, struct sockaddr *pRecvAddr)
{
    ...
    if ((iNumPackets = _ProtoTunnelRecvData(pProtoTunnel, aRemotePortList, sizeof(aRemotePortList), &uHeadOffset, pPacketData, iRecvLen, pRecvAddr)) == PROTOTUNNEL_PACKETRECVFAIL_VALIDATE)
    {
...
int32_t ProtoTunnelValidatePacket(ProtoTunnelRefT *pProtoTunnel, ProtoTunnelT *pTunnel, uint8_t *pOutputData, const uint8_t *pPacketData, int32_t iPacketSize, const char *pKey)
{
    ...
    memcpy(&pTunnel->CryptRecvState, &CryptTemp, sizeof(CryptTemp));
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'bnds' - return the remote server port associated with the server tunnel socket
...
int32_t ProtoTunnelStatus(ProtoTunnelRefT *pProtoTunnel, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'bnds')
    {
        return(pProtoTunnel->uServerRemotePort);
    }
    if (iSelect == 'dpkt')
    {
