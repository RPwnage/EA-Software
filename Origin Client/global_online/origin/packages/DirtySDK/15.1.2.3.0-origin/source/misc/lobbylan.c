...
static int32_t _LobbyLanUnpackPlyrInfo(LobbyLanPlyrInfoT *pPlyrInfo, const char *pBuffer)
{
    ...
    ds_strnzcpy(pPlyrInfo->strName, pBuffer, sizeof(pPlyrInfo->strName));
    pBuffer += strlen(pPlyrInfo->strName)+1;
    ds_strnzcpy(pPlyrInfo->strNote, pBuffer, sizeof(pPlyrInfo->strNote));
    pBuffer += strlen(pPlyrInfo->strNote)+1;
    ds_strnzcpy(pPlyrInfo->Addr.strMachineAddr, pBuffer, sizeof(pPlyrInfo->Addr.strMachineAddr));
    pBuffer += strlen(pPlyrInfo->Addr.strMachineAddr)+1;
    return((int32_t)(pBuffer - pStart));
}
...
static int32_t _LobbyLanAddPlayer(LobbyLanRefT *pRef, LobbyLanPlyrInfoT *pPlyrInfo)
{
    ...
    ds_memcpy_s(&pRef->Game.pPlyrList[pRef->Game.GameInfo.uNumPlayers++], sizeof(pRef->Game.pPlyrList[0]), pPlyrInfo, sizeof(*pPlyrInfo));
    return(0);
}
...
static int32_t _LobbyLanJoinGame(LobbyLanRefT *pRef, LobbyLanGameInfoT *pGameInfo, const char *pPassword)
{
    ...
    // save game info
    ds_memcpy_s(&pRef->Game.GameInfo, sizeof(pRef->Game.GameInfo), pGameInfo, sizeof(*pGameInfo));
    ds_strnzcpy(pRef->Game.strPass, pPassword, sizeof(pRef->Game.strPass));
    ...
    ds_snzprintf(strConnect, sizeof(strConnect), "%a:%d:%d", pGameInfo->uHostAddr, pRef->iGamePort, pRef->iGamePort);
...
static void _LobbyLanAdvert(LobbyLanRefT *pRef, unsigned char bListen)
{
    ...
    {
        char strPort[16];
        ds_snzprintf(strPort, sizeof(strPort), ":%d", pRef->iGamePort);
...
static void _LobbyLanUpdateAdverts(LobbyLanRefT *pRef, char *pQueryBuf, uint32_t uNumAdverts)
{
    ...
    // save advertisement
    ds_strnzcpy(pRef->strQueryBuf, pQueryBuf, sizeof(pRef->strQueryBuf));
    ...
    for (uAdvert = 0, pQueryPtr = pQueryBuf; uAdvert < uNumAdverts; uAdvert++)
    {
        ...
        *pTempPtr = '\0';
        ...
        ds_strnzcpy(pGameInfo->strNote, pTempPtr, sizeof(pGameInfo->strNote));
    ...
    if (pConn->eState == ST_INIT)
    {
        ...
        if ((_LobbyLanGameLinkRecv(pRef, pConn->pLinkRef, FALSE, &Packet, &uPacketType)) > 0)
        {
            ...
            if (uPacketType == LOBBYLAN_CLNTMESG_JOIN)
            {
                ...
                {
                    ds_strnzcpy(strPass, (char *)&Packet.body.data[iPassOffset], sizeof(strPass));
                ...
                // save player info
                ds_memcpy_s(pPlyrInfo, sizeof(*pPlyrInfo), &PlyrInfo, sizeof(PlyrInfo));
    ...
    if (pConn->eState == ST_OPEN)
    {
        ...
        while (_LobbyLanGameLinkRecv(pRef, pConn->pLinkRef, FALSE, &Packet, &uPacketType) > 0)
        {
            if (uPacketType == LOBBYLAN_CLNTMESG_INFO)
            {
                ...
                ds_strnzcpy(pPlyrInfo->strNote, (char *)&Packet.body.data[0], sizeof(pPlyrInfo->strNote));
...
LobbyLanRefT *LobbyLanCreate2(const char *pSelfName, const DirtyAddrT *pSelfAddr, const char *pKind, uint32_t uMaxGames, CommAllConstructT *pConstruct)
{
    ...
    ds_strnzcpy(pRef->Self.strName, pSelfName, sizeof(pRef->Self.strName));
    ds_strnzcpy(pRef->Self.Addr.strMachineAddr, pSelfAddr->strMachineAddr, sizeof(pRef->Self.Addr));
    pRef->iAdvtFreq = 30;    
...
int32_t LobbyLanCreateGame(LobbyLanRefT *pRef, uint32_t uGameFlags, const char *pName, const char *pNote, const char *pPassword, uint32_t uMaxPlayers)
{
    ...
    ds_strnzcpy(pRef->Game.GameInfo.strName, pName, sizeof(pRef->Game.GameInfo.strName));
    ds_strnzcpy(pRef->Game.GameInfo.strNote, pNote, sizeof(pRef->Game.GameInfo.strNote));
    ds_strnzcpy(pRef->Game.strPass, pPassword, sizeof(pRef->Game.strPass));
...
{
    // save info
    ds_strnzcpy(pRef->Self.strNote, pNote, sizeof(pRef->Self.strNote));
    ...
    if (pRef->iHosting == 1)
    {
        ...
        // update info
        ds_memcpy_s(&pRef->Game.pPlyrList[0], sizeof(pRef->Game.pPlyrList[0]), &pRef->Self, sizeof(pRef->Self));
    ...
    else if (pRef->iHosting == 0)
    {
        ...
        ds_strnzcpy((char *)&Packet.body.data[0], pNote, LOBBYLAN_PLYRNOTE_LEN);
...
int32_t LobbyLanStatus(LobbyLanRefT *pRef, int32_t iSelect, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'self')
    {
        ...
        {
            ds_memcpy(pBuf, &pRef->Self, sizeof(pRef->Self));
            return(0);
        }
    }
