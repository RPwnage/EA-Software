...
#if !defined(DIRTYCODE_XBOXONE) && !defined(DIRTYCODE_PC) && !defined(DIRTYCODE_WINRT)
...
#if defined(DIRTYCODE_PS4) || defined(DIRTYCODE_WINRT)
...
#endif // DIRTYCODE_PS4 || DIRTYCODE_WINRT
...
#define SockaddrInSetPort(addr,val) { (addr)->sa_data[0] = (SA_DATA_TYPE)((val)>>8); (addr)->sa_data[1] = (SA_DATA_TYPE)(val); }
...
#define SockaddrInit6(addr,fam)     { memset(addr, 0, sizeof(*(addr))); (addr)->sin6_family = (fam); }
...
DIRTYCODE_API int32_t SockaddrCompare(struct sockaddr *addr1, struct sockaddr *addr2);

// set the address in text format in a sockaddr
DIRTYCODE_API int32_t SockaddrInSetAddrText(struct sockaddr *addr, const char *str);

// get the address in text format from sockaddr
DIRTYCODE_API char *SockaddrInGetAddrText(struct sockaddr *addr, char *str, int32_t len);

// convert int16_t from host to network byte order
DIRTYCODE_API uint16_t SocketHtons(uint16_t addr);

// convert int32_t from host to network byte order
DIRTYCODE_API uint32_t SocketHtonl(uint32_t addr);

// convert int16_t from network to host byte order
DIRTYCODE_API uint16_t SocketNtohs(uint16_t addr);

// convert int32_t from network to host byte order.
DIRTYCODE_API uint32_t SocketNtohl(uint32_t addr);

// convert 32-bit internet address into textual form
DIRTYCODE_API char *SocketInAddrGetText(uint32_t addr, char *str, int32_t len);

// convert textual internet address into 32-bit integer form
DIRTYCODE_API int32_t SocketInTextGetAddr(const char *addrtext);

// parse address:port combination
DIRTYCODE_API int32_t SockaddrInParse(struct sockaddr *addr, const char *parse);

// parse address:port combination into separate components
DIRTYCODE_API int32_t SockaddrInParse2(uint32_t *pAddr, int32_t *pPort, int32_t *pPort2, const char *parse);

// identify if specified sockaddr is an IPv4-mapped IPv6 address
DIRTYCODE_API uint8_t SockaddrIn6IsIPv4(const struct sockaddr *pSockAddr);

// identify if specified sockaddr is a NAT64 IPv6 address
DIRTYCODE_API uint8_t SockaddrIn6IsNAT64(const struct sockaddr *pSockAddr);

// identify if specified sockaddr is zero (unspecified)
DIRTYCODE_API uint8_t SockaddrIn6IsZero(const struct sockaddr *pSockAddr);
