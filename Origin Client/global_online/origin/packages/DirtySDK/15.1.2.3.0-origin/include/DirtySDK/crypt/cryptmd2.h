...
DIRTYCODE_API void CryptMD2Init(CryptMD2T *pContext);

// init the MD5 context (alternate version)
DIRTYCODE_API void CryptMD2Init2(CryptMD2T *pContext, int32_t iHashSize);

// add data to the MD2 context (hash the data).
DIRTYCODE_API void CryptMD2Update(CryptMD2T *pContext, const void *pBuffer, int32_t iLength);

// convert MD2 state into final output form
DIRTYCODE_API void CryptMD2Final(CryptMD2T *pContext, void *pBuffer, int32_t iLength);
