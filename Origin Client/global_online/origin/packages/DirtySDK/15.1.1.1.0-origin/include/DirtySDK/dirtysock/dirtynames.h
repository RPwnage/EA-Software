...
extern DIRTYCODE_API int32_t DirtyUsernameCompare(const char *pName1, const char *pName2);

// determine if pMatch is a substring of pSrc.  Ignore case and strip non-printable ascii characters.
extern DIRTYCODE_API int32_t DirtyUsernameSubstr(const char *pSrc, const char *pMatch);

// create the canonical form of the given name. Essentially lowercases the name and strip non-printable ascii characters.
extern DIRTYCODE_API int32_t DirtyUsernameCreateCanonical(const char *pName, char * pCanonical, size_t uLen);

// generate the hash code for the given name.  Ignore case and strip non-printable ascii characters.
extern DIRTYCODE_API uint32_t DirtyUsernameHash(const char *pName);

// create the canonical form of the given name.  Ignore case and strip non-printable ascii characters.
extern DIRTYCODE_API int32_t DirtyNameCreateCanonical(const char *pName, char * pCanonical, size_t uLen);
