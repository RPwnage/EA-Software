...
/*!
    ...
    \Description
        The Game module provides game-level peer networking.
...
//! dist stat proc
DIRTYCODE_API typedef int32_t (NetGameDistStatProc)(void *pLinkRef, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize);

//! dist send proc
DIRTYCODE_API typedef int32_t (NetGameDistSendProc)(void *pLinkRef, NetGamePacketT *pPkt, int32_t iLen);

//! dist recv proc
DIRTYCODE_API typedef int32_t (NetGameDistRecvProc)(void *pLinkRef, NetGamePacketT *pPkt, int32_t iLen, uint8_t bDist);

//! dist peek proc
DIRTYCODE_API typedef int32_t (NetGameDistPeekProc)(void *pLinkRef, NetGamePacketT **pPkt, uint32_t uMask);

//! dist drop proc
DIRTYCODE_API typedef int8_t (NetGameDistDropProc)(void *pLinkRef, void *pExisting, void *pIncoming, uint8_t uTypeExisting, uint8_t uTypeIncoming);

DIRTYCODE_API typedef int32_t (NetGameDistLinkCtrlProc)(void *ref, int32_t iKind, int32_t iData);
...
DIRTYCODE_API void NetGameDistSetServer(NetGameDistRefT *pRef, uint8_t uActAsServer);

// If using the "multi" versions, sets the index of the local user and the total number of players
DIRTYCODE_API void NetGameDistMultiSetup(NetGameDistRefT *pRef, int32_t iOurIndex, int32_t iTotPlrs );

// set whether to enable meta-information sending, the mask and the version number
DIRTYCODE_API void NetGameDistMetaSetup(NetGameDistRefT *pRef, uint8_t uSparse, uint32_t uMask, uint32_t uVersion);
...
DIRTYCODE_API int32_t NetGameDistInputLocalMulti(NetGameDistRefT *pRef, uint8_t* uTypes, void **pBuffer, int32_t *pLen, int32_t iDelta);
