...
#define DIRTYCODE_XENON             1
    ...
    #if defined (EA_PLATFORM_CAPILANO_XDK) && !defined (EA_PLATFORM_CAPILANO_ADK)
    #define DIRTYCODE_XBOXONE_ONLY     1
    #elif defined (EA_PLATFORM_CAPILANO_ADK) && !defined (EA_PLATFORM_CAPILANO_XDK)
    #define DIRTYCODE_XBOXONEADK_ONLY   1
    #endif
    ...
    #define DIRTYCODE_WINRT_ONLY     1
    #elif defined(WINAPI_FAMILY) && WINAPI_FAMILY == WINAPI_FAMILY_PHONE_APP
    #define DIRTYCODE_WINPRT_ONLY    1
    #else
...
#elif defined(DIRTYCODE_XBOXONE_ONLY)
#define DIRTYCODE_PLATNAME "XboxOne"
#define DIRTYCODE_PLATNAME_SHORT "xone"
#elif defined(DIRTYCODE_XBOXONE_ONLY)
#define DIRTYCODE_PLATNAME "XboxOneAdk"
#define DIRTYCODE_PLATNAME_SHORT "xoneadk"
...
#if defined(_WIN64) || defined (DIRTYCODE_XBOXONE) || defined(__LP64__) || defined(_LP64) || defined(_M_IA64) || defined(__ia64__) || defined(__arch64__) || defined(__mips64__) || defined(__64BIT__)
...
// c99 types for all platforms - check for defines for eabase compatibility
#if defined(DIRTYCODE_PC) || defined(DIRTYCODE_XENON) || defined(DIRTYCODE_WINRT) || defined(DIRTYCODE_XBOXONE)
        ...
        #if defined(DIRTYCODE_XBOXONE) || defined(_WIN64)
...
DIRTYCODE_API struct tm *ds_localtime(struct tm *tm, uint64_t elap);
DIRTYCODE_API struct tm *ds_secstotime(struct tm *tm, uint64_t elap);
DIRTYCODE_API uint64_t ds_timetosecs(const struct tm *tm);
DIRTYCODE_API struct tm *ds_plattimetotime(struct tm *tm, void *pPlatTime);
DIRTYCODE_API struct tm *ds_plattimetotimems(struct tm *tm, int32_t *iMsec);
DIRTYCODE_API char *ds_timetostr(const struct tm *tm, TimeToStringConversionTypeE eConvType, uint8_t bLocalTime, char *pStr, int32_t iBufSize);
