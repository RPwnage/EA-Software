...
DIRTYCODE_API char * HLBBudGetName(  HLBBudT  *pBuddy );

// Xenon aka Xbox360  ONLY --get XUID of buddy.
DIRTYCODE_API uint64_t HLBBudGetXenonXUID(  HLBBudT  *pBuddy );
...
DIRTYCODE_API uint32_t HLBBudGetGameInviteFlags( HLBBudT  *pBuddy  ) ;

// Does Buddy have a headset plugged in, talking or none? Returns VOIPState_Enum. Local;.
DIRTYCODE_API int32_t HLBBudGetVOIPState (  HLBBudT  *pBuddy);

// Does Buddy have my game and is ready to talk?
DIRTYCODE_API int32_t HLBBudCanVoiceChat (  HLBBudT  *pBuddy);

// Someone who sent me a message, may not be a real buddy. Local.
DIRTYCODE_API int32_t HLBBudIsTemporary(  HLBBudT *pBuddy );

// Do I ignore this person? Local.
DIRTYCODE_API int32_t HLBBudIsBlocked(  HLBBudT  *pBuddy );


// This is really a buddy, dual commited. Local.
DIRTYCODE_API int32_t HLBBudIsRealBuddy( HLBBudT  *pBuddy );

 //  Has this buddy, has invited me to be his buddy. Local.
DIRTYCODE_API int32_t HLBBudIsWannaBeMyBuddy( HLBBudT  *pBuddy );

 //  Have I invited this user to be my buddy. Local.
DIRTYCODE_API int32_t HLBBudIsIWannaBeHisBuddy( HLBBudT  *pBuddy );

 // Return Title played by this buddy, based on language set in Presence.
DIRTYCODE_API char * HLBBudGetTitle( HLBBudT  *pBuddy);

// Is this buddy one of these temp buddy kinds -- can be true for permanent buddies too.
DIRTYCODE_API int32_t HLBBudTempBuddyIs(  HLBBudT  *pBuddy, int32_t tempBuddyType );

// It this buddy a member of group that's been set up to send msg. See HLBListAddToGroup.
DIRTYCODE_API int32_t HLBBudIsInGroup(  HLBBudT  *pBuddy );

// Get buddy state  --if need support for states other than ones below..
DIRTYCODE_API int32_t HLBBudGetState(  HLBBudT  *pBuddy);

// Is buddy playing a  game? -- ie is he in passive state.
DIRTYCODE_API int32_t HLBBudIsPassive(  HLBBudT  *pBuddy);

// Is buddy Online and in chat mode?
DIRTYCODE_API int32_t HLBBudIsAvailableForChat (  HLBBudT  *pBuddy);

// Does buddy logged into same game? Local.
DIRTYCODE_API int32_t HLBBudIsSameProduct (  HLBBudT  *pBuddy);

// Can I join this buddy -- in Xbox, from presence.
DIRTYCODE_API int32_t HLBBudIsJoinable(  HLBBudT  *pBuddy);

//Join buddy in another game; He MUST be online,joinable in other title.
DIRTYCODE_API int32_t HLBBudJoinGame( HLBBudT *pBuddy, HLBOpCallbackT *cb , void * pContext );

// Is buddy available for my game -- must be online and not playing.
DIRTYCODE_API int32_t HLBBudCanPlayMyGame(  HLBBudT  *pBuddy);

// Is 'special' temp (i.e.admin)  buddy who WONT accept messages or invites etc.
DIRTYCODE_API int32_t HLBBudIsNoReplyBud(  HLBBudT  *pBuddy);

// Return presence, if not, make default one based on other's product.Local.
DIRTYCODE_API void HLBBudGetPresence( HLBBudT  *pBuddy , char * presence);

// Return any extra  presence string  other client may have sent -eg score of game buddy is playing.
DIRTYCODE_API void HLBBudGetPresenceExtra( HLBBudT  *pBuddy , char * strExtraPres, int32_t iStrSize );

//! Set or reset joinable status in presence;
DIRTYCODE_API int32_t HLBApiPresenceJoinable ( HLBApiRefT  * pApiRef, int32_t bOn , int32_t bSendImmediate);
//
...
DIRTYCODE_API BuddyApiMsgT *  HLBMsgListGetMsgByIndex(  HLBBudT  *pBuddy, int32_t index, int32_t read );

// Return  first unread, starting from startPos. If read is true, mark message as read.
DIRTYCODE_API BuddyApiMsgT *HLBMsgListGetFirstUnreadMsg (  HLBBudT  *pBuddy, int32_t startPos , int32_t read);

// Inject a message, as if from some buddy. Message callbacks etc will be called.

DIRTYCODE_API int32_t HLBMsgListMsgInject (  HLBApiRefT  * pApiRef, int32_t iKind, const char *pSenderName, const char *pMsgBody,
...
DIRTYCODE_API void HLBMsgListGetMsgText (HLBApiRefT  * pApiRef,BuddyApiMsgT *pMsg, char * strText, int32_t iLen);

// Get number of unread messages from this buddy. Local. ?todo do we need?
DIRTYCODE_API int32_t HLBMsgListGetUnreadCount( HLBBudT  *pBuddy  );

// Get total number of messages from this buddy. Local.
DIRTYCODE_API int32_t HLBMsgListGetTotalCount ( HLBBudT  *pBuddy  );

// Zap msg at index position. Local.
DIRTYCODE_API void HLBMsgListDelete(  HLBBudT *pBuddy, int32_t index );

// Delete all, or just read msgs from this buddy. Local. If bZapReadOnly is true will delete just read msgs.
DIRTYCODE_API void HLBMsgListDeleteAll(  HLBBudT  *pBuddy, int32_t bZapReadOnly   );
...
DIRTYCODE_API HLBBudT *HLBListGetBuddyByName ( HLBApiRefT *  pApiRef  , const char * name );

// Get a Buddy by index into the buddy list. Local.
DIRTYCODE_API HLBBudT  *HLBListGetBuddyByIndex ( HLBApiRefT  * pApiRef, int32_t index  );

// Given a buddy name, find the index into the list. Local.
DIRTYCODE_API int32_t HLBListGetIndexByName ( HLBApiRefT  * pApiRef, const char * name );

// Get a count of buddies by various flags (types).
DIRTYCODE_API int32_t HLBListGetBuddyCountByFlags ( HLBApiRefT  *pApiRef, uint32_t uTypeFlag);

// Return count of all  buddies in list; TODO -- add support for type?
DIRTYCODE_API int32_t HLBListGetBuddyCount ( HLBApiRefT  *pApiRef);

// Ignore this buddy. Async --will call OpCallback if registered.
DIRTYCODE_API int32_t HLBListBlockBuddy (   HLBApiRefT  *pApiRef, const char * name , HLBOpCallbackT *cb , void * pContext ) ;

// No longer Ignore this buddy. Async --will call OpCallback if registered.
DIRTYCODE_API int32_t HLBListUnBlockBuddy ( HLBApiRefT  *pApiRef,   const char * name,   HLBOpCallbackT *cb , void * pContext );

// Drastic, no callback function to nuke all invites, prior to say, Block a buddy.
DIRTYCODE_API int32_t HLBListCancelAllInvites ( HLBApiRefT  *pApiRef,  const char  *pName);

 // Request that this person  be my buddy. Async. Will be added to my IWannabeHisBuddy list.
DIRTYCODE_API int32_t HLBListInviteBuddy( HLBApiRefT  *pApiRef, const char * name , HLBOpCallbackT *cb , void * pContext );

// Respond to an invite
DIRTYCODE_API int32_t HLBReceiveMsgAttachment(HLBApiRefT *pHLBApi, void *pContext);

// Respond to an invite or change state of one i sent, --applies to wannabe buddy invites only
DIRTYCODE_API int32_t HLBListAnswerInvite  (  HLBApiRefT  *pApiRef,  const char * name ,
...
DIRTYCODE_API int32_t HLBListAnswerGameInvite  (  HLBApiRefT  *pApiRef,  const char * name ,
                                    /*HLBInviteAnswerE*/ int32_t iInviteAction,
                                    HLBOpCallbackT *cb , void * pContext );
// Cancel any game invites I sent -- also see HLBListAnswerGameInvite
DIRTYCODE_API int32_t HLBListCancelGameInvite  (  HLBApiRefT  *pApiRef, HLBOpCallbackT *cb , void * pContext );
...
DIRTYCODE_API int32_t HLBListGameInviteBuddy( HLBApiRefT  *pApiRef, const char * name , const char * pGameInviteState,
                           HLBOpCallbackT *cb , void * pContext );

// Ensure user is no longer a buddy of mine. Async.
DIRTYCODE_API int32_t HLBListUnMakeBuddy( HLBApiRefT  *pApiRef, const char * name , HLBOpCallbackT *cb , void * pContext );

// Flag /add as a tourney,challenge or other buddy. Sync, local call.
DIRTYCODE_API int32_t HLBListFlagTempBuddy (  HLBApiRefT  *pApiRef,  const char * name, int32_t tempBuddyType);

 // Unflag from temp buddy; . Sync, local call.
DIRTYCODE_API int32_t HLBListUnFlagTempBuddy (  HLBApiRefT  *pApiRef, const char * name,   int32_t tempBuddyType);

 // Explicitly delete this temp buddy. Will ignore request if buddy is permanent. Sync local call.
DIRTYCODE_API int32_t HLBListDeleteTempBuddy (  HLBApiRefT  *pApiRef, const char * name);

//sort function to be set if client wants custom sort of buddy list -- However API will have a reasonable default sort.
DIRTYCODE_API void HLBListSetSortFunction (HLBApiRefT  * pApiRef,  HLBRosterSortFunctionT   * sortCallbackFunction  );   // custom sort, will  have default if this is not set

// Sort list of buddies. Once sorted, list will be frozen until re-sorted,
DIRTYCODE_API int32_t  HLBListSort (HLBApiRefT  * pApiRef);

// Disable Sorting
DIRTYCODE_API void HLBListDisableSorting( HLBApiRefT  *  pApiRef );

// Has the  list of  buddies  or their presence changed ?  Local. HLB_NO_CHANGE if none.
DIRTYCODE_API int32_t  HLBListChanged(HLBApiRefT  * pApiRef);

// Has messages from any buddies changed? Local. Returns buddy index, or HLB_NO_CHANGE if none.
DIRTYCODE_API int32_t HLBListBuddyWithMsg(HLBApiRefT  * pApiRef);

// On closing screen, need to clear temp buddies w/o messages etc.
DIRTYCODE_API int32_t HLBListDeleteNoMsgTempBuddies (HLBApiRefT  * pApiRef );

// Can create a list of buddies, to send a message to.

// Add user to group to whom I will send a common msg.
DIRTYCODE_API int32_t HLBListAddToGroup ( HLBApiRefT  * pApiRef, const char * buddyName);

// Remove this buddy from group Iam going to send to.
DIRTYCODE_API int32_t HLBListRemoveFromGroup ( HLBApiRefT  * pApiRef, const char * buddyName);

// Clear group of users so I can re-use it. Try not to use -- more efficient to clear group when sending msg.
DIRTYCODE_API int32_t HLBListClearGroup ( HLBApiRefT  * pApiRef );

// Send message to group of users I created earlier. Reset group if clearGroup is set.
DIRTYCODE_API int32_t HLBListSendMsgToGroup (HLBApiRefT  * pApiRef,    int32_t msgType , const char * msg , int32_t bClearGroup,
                                    HLBOpCallbackT *cb , void * pContext);

// Send an actual chat message.
DIRTYCODE_API int32_t HLBListSendChatMsg(  HLBApiRefT  * pApiRef, const char * name, const char * body,
                                    HLBOpCallbackT *cb , void * pContext);
...
DIRTYCODE_API HLBApiRefT *HLBApiCreate(  const char *pProduct, const char *pRelease,
                           int32_t iRefMemGroup, void *pRefMemGroupUserData);

//Create the API, and override memory alloc/free functions.
DIRTYCODE_API HLBApiRefT *HLBApiCreate2( BuddyMemAllocT *pAlloc,  BuddyMemFreeT *pFree,
                           const char *pProduct, const char *pRelease,
                           int32_t iRefMemGroup, void *pRefMemGroupUserData);

// Set a callback to write out  debug messages within API. Local.
DIRTYCODE_API void HLBApiSetDebugFunction (  HLBApiRefT  * pApiRef, void *pDebugData, void (*pDebugProc)(void *pData, const char *pText));

// Application  may override default constants.-1 leaves it as default. Only valid before HLBApiConnect is called
DIRTYCODE_API void HLBApiOverrideConstants (HLBApiRefT  * pApiRef,
...
DIRTYCODE_API void HLBApiOverrideMaxMessagesPerBuddy (HLBApiRefT  * pApiRef, int32_t  max_msgs_per_perm_buddy);

//! Set to 1 if on XBL and NTSC cannot play each other, but, have same title id.
DIRTYCODE_API void HLBApiOverrideXblIsSameProductCheck (HLBApiRefT  * pApiRef, int32_t bDontUseXBLSameTitleIdCheck);

// Configure  and initialize any data structures in the API. Local.
DIRTYCODE_API int32_t HLBApiInitialize( HLBApiRefT  * pApiRef, const char * prodid , uint32_t uLocality );

// Connect to buddy server. Local. key can be lkey or user:password. Register connect c/b or poll to find state of connect.
DIRTYCODE_API int32_t HLBApiConnect( HLBApiRefT  * pApiRef, const char * pServer, int32_t port,  const char *key, const char *pRsrc   );

// What is state of connection to the server.  see HLBConnectState_Enum . Local. Can be used to poll state instead of callback.
DIRTYCODE_API  int32_t HLBApiGetConnectState ( HLBApiRefT  * pApiRef);

// Informs caller when connect, disconnect or a re-connect happens. Not neeeded if polling. Local.
DIRTYCODE_API int32_t HLBApiRegisterConnectCallback( HLBApiRefT  * pApiRef, HLBOpCallbackT * pMsgCallback, void * pAppData);

// Informs caller when Cross Game Invite activity happens. Not neeeded if polling. Local.
DIRTYCODE_API int32_t HLBApiRegisterMsgAttachmentCallback( HLBApiRefT  * pApiRef, HLBBudActionCallbackT * pActionCallback, void * pAppData);

// Informs caller when Cross Game Invite activity happens. Not neeeded if polling. Local.
DIRTYCODE_API int32_t HLBApiRegisterGameInviteCallback( HLBApiRefT  * pApiRef, HLBBudActionCallbackT * pActionCallback, void * pAppData);

//! Optional callback --Informe caller when buddy presence changes --state or text.
DIRTYCODE_API int32_t HLBApiRegisterBuddyPresenceCallback( HLBApiRefT * pApiRef, HLBPresenceCallbackT * pListCallback, void * pAppData);

// Disconnect  from server. Local.
DIRTYCODE_API void HLBApiDisconnect ( HLBApiRefT  * pApiRef) ;

// Destroy api and any memory held. Local.
DIRTYCODE_API void HLBApiDestroy( HLBApiRefT   *pState);

// Give API and lower levels some time to process. must be called periodically.  Local.
DIRTYCODE_API void HLBApiUpdate (HLBApiRefT  * pApiRef) ;

// Suspend buddy api, cleanup and put buddy in passive state --used when want to go into game. Local.
DIRTYCODE_API int32_t  HLBApiSuspend( HLBApiRefT  * pApiRef);

// Refresh roster  and put buddy in online state -- used  on return from game, or on startup. Async.
DIRTYCODE_API void *  HLBApiResume(HLBApiRefT  * pApiRef);

// state of last operation --Implies only one operation at  a time. See HLB_ERROR_XXX for status.
DIRTYCODE_API int32_t HLBApiGetLastOpStatus (HLBApiRefT  * pApiRef) ;

// cancel any pending operations -- will clear any operational state and call op callback, with status HLB_ERROR_CANCEL.
DIRTYCODE_API void HLBApiCancelOp (HLBApiRefT  * pApiRef) ;
...
DIRTYCODE_API int32_t HLBApiFindUsers( HLBApiRefT  * pApiRef, const char * name , HLBOpCallbackT * pOpCallback, void * pAppData) ;

// Return  user name from last FindUser operation. Local, Network. Null item not found.
DIRTYCODE_API BuddyApiUserT *HLBApiUserFound ( HLBApiRefT  * pApiRef , int32_t iItem);

// Inform caller on new message waiting. Not neeeded if polling. Local.
DIRTYCODE_API int32_t HLBApiRegisterNewMsgCallback( HLBApiRefT  * pApiRef, HLBMsgCallbackT  *pMsgCallback, void * pAppData );

// Inform caller on buddy list  or buddy presence change.  Not neeeded if polling. Local.
DIRTYCODE_API int32_t HLBApiRegisterBuddyChangeCallback( HLBApiRefT  * pApiRef, HLBOpCallbackT * pMsgCallback, void * pAppData);
...
DIRTYCODE_API int32_t HLBApiPresenceOffline (  HLBApiRefT  * pApiRef, int32_t bOnOff);

//Set presence with string for other games.
DIRTYCODE_API void HLBApiPresenceDiff (  HLBApiRefT  * pApiRef,  const char * globalPres);

//! Set presence my presence state and presence string for this product
DIRTYCODE_API void HLBApiPresenceSame (  HLBApiRefT  * pApiRef, int32_t iPresState, const char * strSamePres);

//Set "extra"  presence for scores etc, must call HLBApiPresenceSend to send.
DIRTYCODE_API void HLBApiPresenceExtra (  HLBApiRefT  * pApiRef,  const char * strExtraPres);

//Send presence msg, with string for this game and others.
DIRTYCODE_API int32_t HLBApiPresenceSend ( HLBApiRefT  * pApiRef,  /*HLBStatE*/int32_t iBuddyState , int32_t VOIPState, const char * gamePres,   int32_t waitMsecs);

// Send presence related to this user's voip i.e. headset status.
DIRTYCODE_API int32_t  HLBApiPresenceVOIPSend ( HLBApiRefT  * pApiRef,  /*HLBVOIPE*/ int32_t iVOIPState );

//Send previously set presence.
DIRTYCODE_API int32_t HLBApiPresenceSendSetPresence ( HLBApiRefT  * pApiRef);

// Set or clear my msg forwarding to email/sms etc.
DIRTYCODE_API int32_t HLBApiSetEmailForwarding( HLBApiRefT  * pApiRef,int32_t bEnable, const char * strEmail,
                                                            HLBOpCallbackT * pOpCallback, void *pCalldata );

DIRTYCODE_API int32_t HLBApiGetEmailForwarding( HLBApiRefT  * pApiRef, int32_t *bEnable, char * strEmail );

// Get my msg forwarding flags and email address etc if set.
DIRTYCODE_API int32_t HLBApiGetForwarding( HLBApiRefT  * pApiRef, unsigned *puForwardingFlags, char * strEmail );

//!Set if want api to translate utf8 chars to 8 bit characters in Presence, and Message Text.
DIRTYCODE_API void HLBApiSetUtf8TransTbl ( HLBApiRefT  * pApiRef, Utf8TransTblT *pTransTbl);

//!Get the "Official" title name for this game. pLang ignored for now--defaults to English.
DIRTYCODE_API int32_t HLBApiGetMyTitleName (HLBApiRefT  * pApiRef,  const char * pLang, const int32_t iBufSize, char * pTitleName);

//!Get the " Official" Xbox title name for a given title, based on my locale-language.
DIRTYCODE_API int32_t HLBApiGetTitleName (HLBApiRefT  * pApiRef, const uint32_t uTitleId,  const int32_t iBufSize, char * pTitleName);

// Set the SessionID that is passed to the game invitation --Xbox Only
DIRTYCODE_API int32_t HLBListSetGameInviteSessionID(HLBApiRefT *pApiRef, const char *sSessionID);


// Get the SessionID for a particular user --works on Xbox only -- for game invites
DIRTYCODE_API int32_t HLBListGetGameSessionID(HLBApiRefT *pApiRef, const char * name, char *sSessionID, const int32_t iBufSize);

// set the Enumerator index used only by xbox360
DIRTYCODE_API int32_t HLBApiSetUserIndex(HLBApiRefT *pApiRef, int32_t iEnumeratorIndex);

// Get the Enumerator index used only by xbox360
DIRTYCODE_API int32_t HLBApiGetUserIndex(HLBApiRefT *pApiRef);
