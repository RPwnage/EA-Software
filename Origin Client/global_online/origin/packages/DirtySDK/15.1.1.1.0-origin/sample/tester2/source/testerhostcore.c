...
static void _TesterHostCoreMsgCommand(TesterCommT *pState, const char *pMsg, void *pParam)
{
    ...
    TagFieldSetString(pState->strCommand, sizeof(pState->strCommand), "COMMAND", pMsg);
    TagFieldSetToken(pState->strCommand, sizeof(pState->strCommand), "STATUS", 'rcvd');
    TesterCommMessage(pState, TESTER_MSGTYPE_STATUS, pState->strCommand);

    // signal that we're executing the message
    TagFieldClear(pState->strCommand, sizeof(pState->strCommand));
    TagFieldSetString(pState->strCommand, sizeof(pState->strCommand), "COMMAND", pMsg);
    TagFieldSetToken(pState->strCommand, sizeof(pState->strCommand), "STATUS", 'exec');
    TesterCommMessage(pState, TESTER_MSGTYPE_STATUS, pState->strCommand);
    ...
    TagFieldClear(pState->strCommand, sizeof(pState->strCommand));
    TagFieldSetString(pState->strCommand, sizeof(pState->strCommand), "COMMAND", pMsg);
    TagFieldSetToken(pState->strCommand, sizeof(pState->strCommand), "STATUS", 'done');
    // include an error if present
    if (iResult)
    {
        TagFieldSetNumber(pState->strCommand, sizeof(pState->strCommand), "ERROR", iResult);
    }
    TesterCommMessage(pState, TESTER_MSGTYPE_STATUS, pState->strCommand);
...
static int32_t _TesterHostCorePrintf(void *pParm, const char *pText)
{
    ...
    #if (defined(DIRTYCODE_XBOXONE) || defined(DIRTYCODE_XBOXONEADK)) && 0
    {
...
int32_t TesterHostCoreStartSavingLog(TesterHostCoreT *pState, const char * strLogfileName)
{
    ...
    if (pState)
    {
        if (pState->zFile == ZFILE_INVALID)
        {
            ...
            memset(strCompleteLogfileName, 0, sizeof(strCompleteLogfileName));
