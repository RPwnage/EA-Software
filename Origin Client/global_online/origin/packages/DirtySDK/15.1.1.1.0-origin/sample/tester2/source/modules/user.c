...
static void _CmdUaCb(UserApiRefT *pRef, UserApiEventDataT *pUserApiEventData, void *pUserData)
{
    ...
    {
#if defined(DIRTYCODE_PS4)
        DirtyUserToNativeUser(&npId, sizeof(npId), &(pUserApiEventData->EventDetails.UserData.DirtyUser));
        ZPrintf("user: profile data \n\t id %s ", npId.handle.data);
#elif defined (DIRTYCODE_XBOXONE)
        DirtyUserToNativeUser(&uXuid, sizeof(uXuid), &(pUserApiEventData->EventDetails.UserData.DirtyUser));
        ZPrintf("user: profile data \n\t id %llu ", uXuid);
#endif
...
static void _CmdUaUpdateCb(UserApiRefT *pRef, UserApiNotifyTypeE eNotifyType, UserApiNotifyDataT *pData, void *pUserData)
{
    ...
    {
#if defined(DIRTYCODE_PS4)
        DirtyUserToNativeUser(&npId, sizeof(npId), &(pData->PresenceData.DirtyUser));
        ZPrintf("user: presence update for id %s\n", npId.handle.data);
#elif defined (DIRTYCODE_XBOXONE)
        DirtyUserToNativeUser(&uXuid, sizeof(uXuid), &(pData->PresenceData.DirtyUser));
    ...
    {
#if defined(DIRTYCODE_PS4)
        DirtyUserToNativeUser(&npId, sizeof(npId), &(pData->TitleData.DirtyUser));
        ZPrintf("user: title update for id %s\n", npId.handle.data);
#elif defined (DIRTYCODE_XBOXONE)
        DirtyUserToNativeUser(&uXuid, sizeof(uXuid), &(pData->TitleData.DirtyUser));
    ...
    {
#if defined(DIRTYCODE_PS4)
        DirtyUserToNativeUser(&npId, sizeof(npId), &(pData->RichPresenceData.DirtyUser));
        ZPrintf("user: rich presence update for id %s\n", npId.handle.data);
#elif defined (DIRTYCODE_XBOXONE)
        DirtyUserToNativeUser(&uXuid, sizeof(uXuid), &(pData->RichPresenceData.DirtyUser));
...
static void _UaGetProfiles(void *_pApp, int32_t argc, char **argv, unsigned char bHelp)
{
    ...
    {
#if defined(DIRTYCODE_PS4)
        ZPrintf("   usage: %s getprofiles [user index] [comma seperated user online ids]\n", argv[0]);
#elif defined (DIRTYCODE_XBOXONE)
        ZPrintf("   usage: %s getprofiles [user index] [comma seperated user xuids]\n", argv[0]);
#endif
    ...
    memset(aDirtyUsers, 0, sizeof(aDirtyUsers));
    ...
    {
#if defined(DIRTYCODE_PS4)
        ds_strnzcpy(sceNpId.handle.data, pch, SCE_NP_ONLINEID_MAX_LENGTH + 1);
        DirtyUserFromNativeUser(&aDirtyUsers[uNumUsers++], &sceNpId);
#elif defined (DIRTYCODE_XBOXONE)
        uXuid = _strtoi64(pch, NULL, 10);
        DirtyUserFromNativeUser(&aDirtyUsers[uNumUsers++], &uXuid);
#endif
        pch = strtok (NULL, ",");
    }
...
static void _UaGetProfile(void *_pApp, int32_t argc, char **argv, unsigned char bHelp)
{
    ...
    {
#if defined(DIRTYCODE_PS4)
        ZPrintf("   usage: %s getprofile [user index] [user online ids] [comma seperated flags in profile,presence,richpresence]\n", argv[0]);
#elif defined (DIRTYCODE_XBOXONE)
        ZPrintf("   usage: %s getprofile [user index] [user xuids] [comma seperated flags in profile,presence,richpresence]\n", argv[0]);
#endif
...
#if defined(DIRTYCODE_PS4)
    ds_strnzcpy(sceNpId.handle.data, argv[3], SCE_NP_ONLINEID_MAX_LENGTH + 1);
    DirtyUserFromNativeUser(&DirtyUser, &sceNpId);
#elif defined (DIRTYCODE_XBOXONE)
    pCh = argv[3];
    iBase = 10;
    if ((pCh[0] == '0') && (pCh[1] == 'x'))
    ...
    uXuid = ds_strtoll(pCh, NULL, iBase);
    DirtyUserFromNativeUser(&DirtyUser, &uXuid);
#endif
...
static void _UaRecentlyMet(void *_pApp, int32_t argc, char **argv, unsigned char bHelp)
{
...
#elif defined (DIRTYCODE_XBOXONE)
        ZPrintf("   %s recentlymet is not supported on XBox One\n", argv[0]);
#endif
        return;
    }
    ...
    iResult = UserApiPostRecentlyMetAsync(pApp->pUa, uUserIndex, &DirtyUser, &_CmdUaPostCb, NULL);
    if (iResult == USERAPI_ERROR_UNSUPPORTED)
    {
...
static void _UaSetRichPresence(void *_pApp, int32_t argc, char **argv, unsigned char bHelp)
{
...
#elif defined (DIRTYCODE_XBOXONE)
        ZPrintf("   usage: %s setrichpresence [user index] [rich presence identifier]\n", argv[0]);
#endif
        return;
    }
    ...
    memset(&Data, 0, sizeof(UserApiRichPresenceT));
