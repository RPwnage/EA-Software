...
/*!
    ...
    \Input *pStrBegin   - begin signature
    \Input *pStrEnd     - end signature
...
*/
/********************************************************************************F*/
static char *_HttpLoadCertificate(const char *pFilename, int32_t *pCertSize, const char *pStrBegin, const char *pStrEnd)
{
    ...
    {
        NetPrintf(("httpserv: could not pem end signature\n"));
        return(NULL);
    }
...
int32_t CmdHttp(ZContext *argz, int32_t argc, char *argv[])
{
    ...
    {
        if ((pRef->pClientCert = (char *)_HttpLoadCertificate(argv[2], &pRef->iCertSize, "-----BEGIN CERTIFICATE-----", "-----END CERTIFICATE-----")) == NULL)
        {
    ...
    {
        if ((pRef->pClientKey = (char *)_HttpLoadCertificate(argv[2], &pRef->iKeySize, "-----BEGIN RSA PRIVATE KEY-----", "-----END RSA PRIVATE KEY-----")) == NULL)
        {
