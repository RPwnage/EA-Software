...
static uint32_t __stdcall _TesterCommCmdProcessor(const char* pIncomingCommand)
{
    ...
    pState->LineData.iType = TagFieldGetNumber(TagFieldFind(pCommand, "TYPE"), 0);
    TagFieldGetString(TagFieldFind(pCommand, "TEXT"), pState->LineData.strBuffer, sizeof(pState->LineData.strBuffer)-1, "");
...
{
    TesterCommXenonT *pData;
    uint32_t uResponseSize = sizeof(pState->strResponse);
    ...
    while(ZListPeekFront(pState->pOutputData))
    {
        ...
        pLineOut = pState->strCommand;
        iLineOutSize = sizeof(pState->strCommand);
        ZMemSet(pState->strCommand, 0, sizeof(pState->strCommand));
        pLineOut = strcat(pLineOut, strPrefix);
        iLineOutSize -= strlen(pState->strCommand);
        iLineOutSize -= 1; // for NULL
        TagFieldSetNumber(pState->strCommand, iLineOutSize, "TYPE", pState->LineData.iType);
        TagFieldSetString(pState->strCommand, iLineOutSize, "TEXT", pState->LineData.strBuffer);
        iLineLength = strlen(pState->strCommand);
        pState->strCommand[iLineLength] = '\0';

        // send it as a command
        iResult = pData->XBDMLib.XBDMSendCommand(pData->pDMConnection, pState->strCommand, pState->strResponse, &uResponseSize);
        if (FAILED(iResult))
        {
            char strError[100];
            ...
            {
                ds_strnzcpy(strError, pState->strResponse, sizeof(strError));
            }
