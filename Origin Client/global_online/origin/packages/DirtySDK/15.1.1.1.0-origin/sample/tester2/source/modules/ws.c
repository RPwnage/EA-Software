...
static void _WSSend(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    if (!strncmp(argv[2], "-f", 2))
    {
        ...
        // try to open file
        if ((pFileData = ZFileLoad(argv[2]+2, &iFileSize, ZFILE_OPENFLAG_RDONLY|ZFILE_OPENFLAG_BINARY)) != NULL)
        {
            pMessage = pFileData;
            iMsgLen = iFileSize;
        }
    ...
    else if (!strncmp(argv[2], "-m", 2))
    {
        iMsgLen = (int32_t)strlen(argv[2]+2);
        if (iMsgLen > 0)
        {
            pMessage = ZMemAlloc(iMsgLen+1);
            ds_strnzcpy(pMessage, argv[2]+2, iMsgLen+1);
        }
        else
        {
