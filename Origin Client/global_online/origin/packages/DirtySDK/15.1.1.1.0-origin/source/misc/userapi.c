...
void _ClearContext(UserApiRefT *pRef, int32_t iUserIndex)
{
    ...
    if (pRef->pLookupUsers[iUserIndex])
    {
        DirtyMemFree(pRef->pLookupUsers[iUserIndex], USERAPI_MEMID, pRef->iMemGroup, pRef->pMemGroupUserData); 
        pRef->pLookupUsers[iUserIndex] = NULL;
    }
...
/*F*************************************************************************************************/
/*!
    \Function    UserApiRecentlyMetAsync
...
*/
/*************************************************************************************************F*/
int32_t UserApiPostRecentlyMetAsync(UserApiRefT *pRef, uint32_t uUserIndex, DirtyUserT *pPlayerMet, UserApiPostCallbackT *pCallback, void *pUserData)
{
    int32_t result = 0;
    if (pRef == NULL)
    {
    ...
    result = UserApiPlatRequestRecentlyMet(pRef, (int32_t)uUserIndex, pPlayerMet);
