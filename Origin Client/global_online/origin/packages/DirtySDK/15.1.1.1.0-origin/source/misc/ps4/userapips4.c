...
#define USERAPI_NPID_LIST_MAX_LEN               (100)
#define USERAPI_MAX_LOOKUPS_PER_REQUEST         (50)

#define USERAPI_WEBAPI_GROUP                    "userProfile"
#define USERAPI_WEBAPI_RECENTLY_MET_GROUP       "activityFeed"
#define USERAPI_PATH_PROFILE_FORMAT             "/v1/profiles?onlineId=%s&fields=@default,personalDetail&avatarSize=%c"
#define USERAPI_PATH_PRESENCE_FORMAT            "/v1/users/%s/presence?type=primary&presenceDetail=true"
#define USERAPI_PATH_RECENTLY_MET_FORMAT        "/v1/users/%s/feed"
#define USERAPI_PATH_RECENTLY_MET_POST_FORMAT   "{\"source\": {\"meta\": \"%s\", \"type\": \"ONLINE_ID\" }, \"storyType\": \"PLAYED_WITH\",\"targets\": [ {\"meta\": \"%s\", \"type\": \"TITLE_ID\" },{\"meta\": \"%s\", \"type\": \"ONLINE_ID\" }]}"
#define USERAPI_PATH_RICH_PRESENCE_FORMAT1      "/v1/users/%s/presence/gameStatus"
#define USERAPI_PATH_RICH_PRESENCE_FORMAT2      "/v1/users/%s/presence/gameData"
#define USERAPI_PATH_RICH_PRESENCE_POST_FORMAT1 "{\"gameStatus\": \"%s\"}"
#define USERAPI_PATH_RICH_PRESENCE_POST_FORMAT2 "{\"gameData\": \"%s\"}"

#define USERAPI_PATH_PROFILE_FORMAT_LEN_MAX     (128 + ((SCE_NP_ONLINEID_MAX_LENGTH+1) * USERAPI_MAX_LOOKUPS_PER_REQUEST))
#define USERAPI_PATH_RECENTLY_MET_LEN_MAX       (256)

#define USERAPI_HTTP_RESP_BODY_MAX_LEN          (1024 * USERAPI_MAX_LOOKUPS_PER_REQUEST)
...
{
    memset(ProfileData, 0, sizeof(UserApiProfileT));
    ...
    if (pRef->pPlatformData->ProfilePlatResponse.iStatusCode[uUserIndex] == 200)
    {
        ...
        for (int64_t i = 0; i < size; ++i)
        {
            ...
            // get GamerTag
            strPosGamerTag = JsonFind(uJasonList, "onlineId");
            if (strPosGamerTag)
            {
            ...
            // get NpId
            strPosGamerTag = JsonFind(uJasonList, "npId");
            if (strPosGamerTag)
            {
                char strNativeId[DIRTYUSER_NATIVEUSER_MAXSIZE];
                JsonGetString(strPosGamerTag, strNativeId, sizeof(strNativeId), "");
                if (sceNpWebApiUtilityParseNpId(strNativeId, &npid) < 0)
                {
                    NetPrintf(("userapips4: [%p] failed to parse http response, 'npId' attribute\n", pRef));
                    pRef->UserContextList[uUserIndex].iTotalErrors += iSmallSize;
                    return(USERAPI_ERROR_REQUEST_PARSE_FAILED);
                }

                DirtyUserFromNativeUser(&pUserData->DirtyUser, &npid);
            }
...
{
    memset(pPresenceData, 0, sizeof(UserApiPresenceT));
...
int32_t _UserApiProcessRichPresenceResponse(UserApiRefT *pRef, int32_t uUserIndex, UserApiRichPresenceT *pRichPresenceData, UserApiUserDataT *pUserData)
{
    // For PS4 rich presence data comes in the regular presence query
        memset(pRichPresenceData, 0, sizeof(UserApiRichPresenceT));
    ...
    if (pRef->pPlatformData->PresencePlatResponse.iStatusCode[uUserIndex] == 200)
    {
        ...
        // get game status
        pTmp = JsonFind(uJasonList, "presence.primaryInfo.gameStatus");
        if (pTmp)
        {
        ...
        // get game data
        pTmp = JsonFind(uJasonList, "presence.primaryInfo.gameData");
        if (pTmp)
        {
...
static void _UserListOnWebApiUpdate(int32_t userCtxId, int32_t callbackId, const char *pNpServiceName, SceNpServiceLabel npServiceLabel, const SceNpPeerAddress *pTo, const SceNpPeerAddress *pFrom, const SceNpWebApiPushEventDataType *pDataType, const char *pData, size_t dataLen, void *pUserArg)
{
    ...
    if (pUserArg != NULL)
    {
        ...
        memset(data, 0, sizeof(UserApiNotifyDataT));
        ...
        ds_strnzcpy(npid.handle.data, pFrom->onlineId.data, SCE_NP_ONLINEID_MAX_LENGTH + 1);
    ...
    if (pRef)
    {
        for (uint32_t i = 0; i < NETCONN_MAXLOCALUSERS; ++i)
        {
            ...
            if (pRef->bLookupUserAvailable[i] && pRef->iLookupUserIndex[i] > -1 && pRef->iLookupUserIndex[i] < pRef->iLookupUsersLength[i])
            {
                ...
                char strUserList[((SCE_NP_ONLINEID_MAX_LENGTH+1) * USERAPI_MAX_LOOKUPS_PER_REQUEST)];
                int32_t iLookups = 0;
                int32_t iWritten = 0;
                ...
                {
                    if (!(DirtyUserToNativeUser(&aNativeUser, sizeof(aNativeUser), ((pRef->pLookupUsers[i]) + pRef->iLookupUserIndex[i]))))
                    {
                    ...
                    {
                        iWritten += ds_snzprintf(strUserList + iWritten, sizeof(strUserList) - iWritten, "%s,", aNativeUser.handle.data);
                    }
...
UserApiPlatformDataT * UserApiPlatCreateData(UserApiRefT *pRef)
{
    ...
    {
        pPlatformData->ProfilePlatResponse.iDataLen[i]     = 0;
        pPlatformData->ProfilePlatResponse.pData[i]        = NULL;
        pPlatformData->ProfilePlatResponse.iStatusCode[i]  = 0;
        pPlatformData->ProfilePlatResponse.pUserData[i]    = NULL;

        pPlatformData->PresencePlatResponse.iDataLen[i]    = 0;
        pPlatformData->PresencePlatResponse.pData[i]       = NULL;
        pPlatformData->PresencePlatResponse.iStatusCode[i] = 0;
        pPlatformData->PresencePlatResponse.pUserData[i]   = NULL;

        pPlatformData->RmpPlatResponse.iDataLen[i]         = 0;
        pPlatformData->RmpPlatResponse.pData[i]            = NULL;
        pPlatformData->RmpPlatResponse.iStatusCode[i]      = 0;
        pPlatformData->RmpPlatResponse.pUserData[i]        = NULL;
    }
...
int32_t UserApiPlatDestroyData(UserApiRefT *pRef, UserApiPlatformDataT *pPlatformData)
{
    for (uint32_t iValue = 0; iValue < NETCONN_MAXLOCALUSERS; ++iValue)
    {
        ...
        if (pRef->pLookupUsers[iValue])
        {
            DirtyMemFree(pRef->pLookupUsers[iValue], USERAPI_MEMID, pRef->iMemGroup, pRef->pMemGroupUserData); 
            pRef->pLookupUsers[iValue] = NULL;
        }
...
int32_t UserApiPlatRequestProfile(UserApiRefT *pRef, uint32_t uUserIndex, DirtyUserT *pLookupUsers, int32_t iLookupUsersLength)
{
    ...
    pRef->pLookupUsers[uUserIndex] = (DirtyUserT*)DirtyMemAlloc((sizeof(DirtyUserT) * iLookupUsersLength), USERAPI_MEMID, pRef->iMemGroup, pRef->pMemGroupUserData); 

    if ((pRef->pLookupUsers) == NULL)
    {
        NetCritLeave(&pRef->crit);
    ...
    ds_memcpy(pRef->pLookupUsers[uUserIndex], pLookupUsers, (sizeof(DirtyUserT) * iLookupUsersLength));
    NetCritLeave(&pRef->crit);
    return(0);
}
...
int32_t UserApiPlatRequestPresence(UserApiRefT *pRef, uint32_t uUserIndex, DirtyUserT *pLookupUser)
{
    ...
    NetCritEnter(&pRef->crit);
    if (DirtyUserToNativeUser(&PlayerId, sizeof(PlayerId), pLookupUser))
    {
        ds_snzprintf(strPath, sizeof(strPath), USERAPI_PATH_PRESENCE_FORMAT, PlayerId.handle.data);
...
int32_t UserApiPlatRequestRecentlyMet(UserApiRefT *pRef, uint32_t uUserIndex, DirtyUserT *pPlayerMet)
{
    ...
    SceNpId RecentlyMetUser;
    SceNpId LocalUser;

    if (NetConnStatus('npid', uUserIndex, &LocalUser, sizeof(SceNpId)) < 0)
    {
    ...
    {
        memset(strTitleId, 0, sizeof(strTitleId));
        ...
        ds_snzprintf(strPath, sizeof(strPath), USERAPI_PATH_RECENTLY_MET_FORMAT, LocalUser.handle.data);
        ds_snzprintf(strPostData, sizeof(strPostData), USERAPI_PATH_RECENTLY_MET_POST_FORMAT, LocalUser.handle.data, strTitleId, (char*)&RecentlyMetUser.handle);
...
int32_t UserApiPlatRequestPostRichPresence(UserApiRefT *pRef, uint32_t uUserIndex, UserApiRichPresenceT *pData)
{
    ...
    ds_snzprintf(strPath, sizeof(strPath), USERAPI_PATH_RICH_PRESENCE_FORMAT1, LocalUser.handle.data);
...
int32_t UserApiPlatRegisterUpdateEvent(UserApiRefT *pRef, uint32_t uUserIndex, UserApiNotifyTypeE eType)
{
    ...
    case USERAPI_NOTIFY_PRESENCE_UPDATE:
        iReturn = DirtyWebApiAddPushEventListener(pRef->pPlatformData->pWebApi, "np:service:presence:onlineStatus", _UserListOnWebApiUpdate, pRef);
        break;

    case USERAPI_NOTIFY_TITLE_UPDATE:
        iReturn = DirtyWebApiAddPushEventListenerEx(pRef->pPlatformData->pWebApi, "np:service:presence:gameTitleInfo", "inGamePresence", SCE_NP_DEFAULT_SERVICE_LABEL, _UserListOnWebApiUpdate, pRef);
        break;

    case USERAPI_NOTIFY_RICH_PRESENCE_UPDATE:
        iReturn = DirtyWebApiAddPushEventListenerEx(pRef->pPlatformData->pWebApi, "np:service:presence:gameStatus", "inGamePresence", SCE_NP_DEFAULT_SERVICE_LABEL, _UserListOnWebApiUpdate, pRef);
        break;
    default:
