...
static QosApiRequestT *_QosApiCreateRequest(QosApiRefT *pQosApi)
{
    ...
    pQosApiRequest->pQosInfo->hResult = DirtyErrGetHResult(DIRTYAPI_QOS, QOSAPI_STATUS_INIT, TRUE);
...
/*!
    ...
    \Input *phResult    - ouput of new hResult

    \Output 
        uint32_t    - qosapi status flags of the request

    \Version 04/11/2008 (cadam)
*/
/********************************************************************************F*/
static uint32_t _QosApiConvertXNetQosInfoFlags(XNQOS *pXnQos, uint32_t *phResult)
{
    uint32_t uFlags = 0;
    *phResult = 0;
    ...
    if (pXnQos->axnqosinfo[0].bFlags & XNET_XNQOSINFO_TARGET_DISABLED)
    {
       ...
       *phResult = DirtyErrGetHResult(DIRTYAPI_QOS, QOSAPI_STATUS_XB_DISABLED, TRUE);
    }
    else
    {
        ...
        if (pXnQos->axnqosinfo[0].bFlags & XNET_XNQOSINFO_COMPLETE)
        {
            ...
            *phResult = DirtyErrGetHResult(DIRTYAPI_QOS, QOSAPI_STATUS_SUCCESS, FALSE);
        }
        else
        {
            *phResult = DirtyErrGetHResult(DIRTYAPI_QOS, QOSAPI_STATUS_PENDING, TRUE);
        }
    }
...
/*!
    ...
    \Output 
        QosApiNatTypeE - NAT type of the client
...
/*F********************************************************************************/
/*!
    \Function _QosApiUpdateQosInfo

    \Description
        Copies qos info from xbox structures to ours
...
static void _QosApiUpdate(void *pData, uint32_t uTick)
{
    ...
    if (pQosApi->eQosApiNatType == QOSAPI_NAT_PENDING)
    {
        ...
        if (pQosApi->eQosApiNatType != QOSAPI_NAT_PENDING)
        {
            ...
            QosInfo.hResult = DirtyErrGetHResult(DIRTYAPI_QOS, QOSAPI_STATUS_SUCCESS, FALSE);
    ...
    while (*ppRequest != NULL)
    {
        ...
        if ((*ppRequest)->iIssued == 0)
        {
            if ((*ppRequest)->eRequestType == TP_CLIENT)
            {
                ...
                if (dwResult == ERROR_SUCCESS)
                {
                    if ((*ppRequest)->pSessionLookupResults->dwSearchResults == 1)
                    {
                        ...
                        ds_memcpy_s(&(*ppRequest)->SessionInfo,
                               sizeof((*ppRequest)->SessionInfo), 
                               &(*ppRequest)->pSessionLookupResults->pResults[0].info,
                               sizeof((*ppRequest)->pSessionLookupResults->pResults[0].info));
                                          ...
                                          (*ppRequest)->uBitsPerSec, 0, NULL, &(*ppRequest)->pXnQos) != 0)
                        {
                            ...
                            (*ppRequest)->pQosInfo->hResult = DirtyErrGetHResult(DIRTYAPI_QOS, QOSAPI_STATUS_XB_PROBE_FAIL, TRUE);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        uFlags = QOSAPI_STATFL_FAILED;
                        (*ppRequest)->pQosInfo->hResult = DirtyErrGetHResult(DIRTYAPI_QOS, QOSAPI_STATUS_XB_SESSION_UNEXPECTED_NUM, TRUE);
                    }
                    (*ppRequest)->iIssued = 1;
                }
                else if (dwResult != ERROR_IO_INCOMPLETE)
                {
                    ...
                    uFlags = QOSAPI_STATFL_FAILED;
                    (*ppRequest)->pQosInfo->hResult = DirtyErrGetHResult(DIRTYAPI_QOS, QOSAPI_STATUS_XB_SESSION_LOOKUP_FAIL, TRUE);
                    (*ppRequest)->iIssued = 1;
                }
            }
            else
            {
                ...
                if (iHost == -1)
                {
                    ...
                    uFlags = QOSAPI_STATFL_FAILED;
                    (*ppRequest)->pQosInfo->hResult = DirtyErrGetHResult(DIRTYAPI_QOS, QOSAPI_STATUS_XB_UNKNOWN_HOST, TRUE);
                    (*ppRequest)->iIssued = 1;
                }
                else if (iHost != 0)
                {
                    ...
                    else
                    {
                        ...
                        uFlags = QOSAPI_STATFL_FAILED;
                        (*ppRequest)->pQosInfo->hResult = DirtyErrGetHResult(DIRTYAPI_QOS, QOSAPI_STATUS_XB_ADDR_MAP_FAIL, TRUE);
                    }
                    (*ppRequest)->iIssued = 1;
                }
            }
        ...
        if ((*ppRequest)->iIssued == 1)
        {
            ...
            {
                uFlags = _QosApiConvertXNetQosInfoFlags((*ppRequest)->pXnQos, &((*ppRequest)->pQosInfo->hResult));
            }
                ...
                ((NetTick() - (*ppRequest)->pQosInfo->uWhenRequested) > (*ppRequest)->uTimeout))
            {
                ...
                (*ppRequest)->pQosInfo->hResult = DirtyErrGetHResult(DIRTYAPI_QOS, QOSAPI_STATUS_OPERATION_TIMEOUT, TRUE);
            }
            ...
            if ((*ppRequest)->pQosInfo->uFlags != uFlags)
            {
                ...
                CBInfo.eQosApiNatType = pQosApi->eQosApiNatType;
                pQosApi->pCallback(pQosApi, &CBInfo, QOSAPI_CBTYPE_X360_STATUS, pQosApi->pUserData);
            }
        }
...
int32_t QosApiStatus(QosApiRefT *pQosApi, int32_t iSelect, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    {
        ds_memcpy(pBuf, &pQosApi->strSession, sizeof(pQosApi->strSession));
        return(0);
    }
    if ((iSelect == 'requ') && (pBuf != NULL) && (iBufSize >= (signed)sizeof(QosInfoT)))
    {
        ...
        for (ppQosApiRequest = &pQosApi->pRequestQueue; *ppQosApiRequest != NULL; ppQosApiRequest = &(*ppQosApiRequest)->pNext)
        {
            ...
            {
                ds_memcpy(pBuf, (*ppQosApiRequest)->pQosInfo, sizeof(QosInfoT));
                return(0);
            }
        }
        return(-2);
