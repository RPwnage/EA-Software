...
*/
/*************************************************************************************************F*/
PresenceTitleRecord^ _UserListApiGetPriorityTitle(IVectorView<PresenceTitleRecord^>^ presenceTitleRecordList)
{
...
*/
/*************************************************************************************************F*/
void _UserListApiProcessIsFriend(UserListApiRefT *pRef)
{
    UserListApiIsADataT   isAQueryData;
    UserListApiIsFriendQueueT *pTmpQueuePtr = pRef->pIsFriendRequestQueue;

    memset(&isAQueryData, 0, sizeof(UserListApiIsADataT));
...
void UserListApiUpdate(UserListApiRefT *pRef)
{
    ...
    while (pCurrent)
    {
        ...
        memset(&data,                0, sizeof(UserListApiEventDataT));
        memset(&isAQueryData,        0, sizeof(UserListApiIsADataT));
                        ...
                        if( refCurrentResultFriend != nullptr && refCurrentResultFriend->Items != nullptr )
                        {
                            ...
                            for( uint32_t index = 0; index < list->Size; index++ )
                            {
                                ...
                                if (refPersonProfile)
                                {
                                    ...
                                    Utf8EncodeFromUCS2 (pUserApiProfile->strAvatarUrl,   sizeof(pUserApiProfile->strAvatarUrl),   (uint16 *)refPersonProfile->DisplayPictureUri->AbsoluteUri->ToString()->Data());
                                ...
                                if (refPersonPresence)
                                {
                                    ...
                                    if (refPersonPresence->PresenceTitleRecords->Size > 0)
                                    {
                                        // Return data from the 1st device the user is online with XBox One with only
                                        PresenceTitleRecord^ refRecord = _UserListApiGetPriorityTitle(refPersonPresence->PresenceTitleRecords);
                                        uint32_t uCurrentTitleId = _wcstoui64(Windows::Xbox::Services::XboxLiveConfiguration::TitleId->Data(), NULL, 10);

                                        if (refRecord != nullptr)
                                        {
                                            Utf8EncodeFromUCS2 (pUserApiPresence->strTitleName,    sizeof(pUserApiPresence->strTitleName),    (uint16 *)refRecord->TitleName->ToString()->Data());
                                            ds_snzprintf(pUserApiPresence->strTitleId, sizeof(pUserApiPresence->strTitleId), "%u", refRecord->TitleId);
                                            ds_snzprintf(pUserApiPresence->strPlatform, sizeof(pUserApiPresence->strPlatform), "%i", refRecord->DeviceType);
                                            Utf8EncodeFromUCS2 (pUserApiRichPresence->strData,    sizeof(pUserApiRichPresence->strData),    (uint16 *)refRecord->Presence->ToString()->Data());

                                            if (uCurrentTitleId == refRecord->TitleId)
                                            {
                    ...
                    else
                    {
                        ...
                        if ((iStatusCode = ProtoHttpStatus(pRef->pHttp, 'done', NULL, 0)) > 0)
                        {
                            if ((iHttpStatus = ProtoHttpRecvAll(pRef->pHttp, pBuf, sizeof(pBuf))) > 0)
                            {
                                ...
                                int32_t iStatusCode = ProtoHttpStatus(pRef->pHttp, 'code', NULL, 0);
                            ...
                            else if (iHttpStatus == PROTOHTTP_RECVFAIL || iHttpStatus <= PROTOHTTP_MINBUFF)
                            {
                                pCurrent->bResponseProcessed = true;
...
int32_t UserListApiGetListAsync(UserListApiRefT *pRef, uint32_t uUserIndex, UserListApiTypeE eType, int32_t iLimit, int32_t iOffset, UserListApiFiltersT *filter, UserListApiCallbackT *pNotifyCb, uint32_t uTypeMask, void *pUserData)
{
    ...
    else
    {
        ...
        if (refUser->IsSignedIn && !refUser->IsGuest)
        {
            uint32_t currentId = 0;
            if (pRef->aXblContext[uUserIndex].refXblUserContext != nullptr)
            {
                currentId = pRef->aXblContext[uUserIndex].refXblUserContext->User->Id;
            }

            if (pRef->aXblContext[uUserIndex].refXblUserContext == nullptr || refUser->Id != currentId)
            {
                try
                {
