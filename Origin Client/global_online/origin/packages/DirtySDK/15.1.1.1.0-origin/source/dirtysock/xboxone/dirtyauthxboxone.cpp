...
*/
/********************************************************************************F*/
static DirtyAuthTokenT *_DirtyAuthNewToken(DirtyAuthRefT *pDirtyAuth, const char *pHost, int32_t iUserIndex)
{
...
int32_t DirtyAuthGetToken(int32_t iUserIndex, const char *pHost, uint8_t bForceNew)
{
    ...
    if ((pDirtyToken = _DirtyAuthNewToken(pDirtyAuth, pHost, iUserIndex)) == NULL)
    {
        NetCritLeave(&pDirtyAuth->TokenCrit);
    ...
    User ^user = nullptr;
    if (NetConnStatus('xusr', iUserIndex, &user, sizeof(user)) < 0)
    {
    ...
    try
    {
        pDirtyToken->asyncOperation = user->GetTokenAndSignatureAsync(pHttpMethod, pStrUrl, nullptr, strBody);
    }
    catch (Exception ^ e)
    {
    ...
    pDirtyToken->asyncOperation->Completed = ref new AsyncOperationCompletedHandler<GetTokenAndSignatureResult ^>(
        ...
        if (status == Windows::Foundation::AsyncStatus::Completed)
        {
            ...
            if (pDirtyToken->pToken != NULL)
            {
                ...
                memset(pDirtyToken->pToken, 0, pDirtyToken->iTokenBufSize);
