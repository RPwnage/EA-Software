...
static NetConnIEAUserEventT * _NetConnCommonGetFreeIEAUserEvent(NetConnCommonRefT *pCommonRef)
{
    ...
    if (pCommonRef->pIEAUserFreeEventList == NULL)
    {
        ...
        for (iLoop = 0; iLoop < 4; iLoop++)
        {
            ...
            if (pUserEventEntry)
            {
                memset(pUserEventEntry, 0, sizeof(*pUserEventEntry));
...
#endif
{
    ...
    memset(pCommonRef, 0, iNetConnRefSize);
...
#ifdef DIRTYSDK_IEAUSER_ENABLED
    NetPrintf(("netconncommon: automatic internal detection of local users is disabled  - netconn relies on NetConnAddLocalUser()/NetConnRemoveLocalUser() being called\n"));
#else
    NetPrintf(("netconncommon: automatic internal detection of local users is enabled - NetConnAddLocalUser()/NetConnRemoveLocalUser() not exposed in public api\n"));
#endif
