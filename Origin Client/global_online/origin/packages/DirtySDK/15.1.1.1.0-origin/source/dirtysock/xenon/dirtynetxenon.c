/*H*************************************************************************************/
/*!
    \File dirtynetxenon.c

    \Description
        Provide a wrapper that translates the Winsock network interface into DirtySock
        calls, as well as handling Xenon-specific functionality. In the case of Winsock,
        little translation is needed since it is based off BSD sockets (as is DirtySock).

    \Copyright
        Copyright (c) Electronic Arts 2004-2013.

    \Version 1.0 01/02/2004 (jbrookes) First Version
*/
/*************************************************************************************H*/
...
static const char *_DisplayAddrMapEntry(SocketAddrMapT *pAddrMap)
{
    ...
    ds_snzprintf(strOutput, sizeof(strOutput), "[%a/%a:%d] -> [%a:%d]", pAddrMap->uAddr,
...
static const char *_DisplayNameMapEntry(SocketNameMapT *pNameMap)
{
    ...
    {
        ds_strnzcpy(strDstAddr, SocketInAddrGetText(pNameMap->uRemap, strAddrBuf, sizeof(strAddrBuf)), sizeof(strDstAddr));
    }
    if (pNameMap->strRemap[0] != '\0')
    {
        if (strDstAddr[0] != '\0')
        {
            ds_strnzcat(strDstAddr, "/", sizeof(strDstAddr));
        }
        ds_strnzcat(strDstAddr, pNameMap->strRemap, sizeof(strDstAddr));
    }
    if (strDstAddr[0] == '\0')
    {
        // special-case for a blank entry
    ...
    // format output and return to caller
    ds_snzprintf(strOutput, sizeof(strOutput), "[%s:%d] -> [%s:%d]", pNameMap->strDnsName,
...
static const SocketNameMapT *_MapName(SocketStateT *pState, const char *pName)
{
    ...
    for (iMap = 0, pMap = NULL; iMap < sizeof(pState->aSockNameMap)/sizeof(pState->aSockNameMap[0]); iMap += 1)
    {
        if (_MapNameMatch(pName, pState->aSockNameMap[iMap].strDnsName))
        {
            ...
            NetPrintf(("dirtynetxenon: name remap %s to %s\n", pName,
...
static const struct sockaddr *_MapAddress(SocketT *pSocket, struct sockaddr *pTemp, const struct sockaddr *pAddr)
{
    ...
    {
        NetPrintf(("dirtynetxenon: skipping address remap as socket is marked for insecure use\n"));
        return(pAddr);
    }
    ...
    if ((pNameMap = _MapName(pState, strAddrText)) != NULL)
    {
        if (pNameMap->uRemap != 0)
        {
            ...
            ds_memcpy_s(pTemp, sizeof(*pTemp), pAddr, sizeof(*pAddr));
            SockaddrInSetAddr(pTemp, uAddr);
            pAddr = pTemp;
        }
        else
        {
            NetPrintf(("dirtynetxenon: warning -- could not map address %s using name map, as remap address is not yet specified\n", strAddrText));
        }
    }
    ...
    for (iMap = 0; iMap < sizeof(pState->aSockAddrMap)/sizeof(pState->aSockAddrMap[0]); iMap += 1)
    {
        ...
        NetPrintf(("dirtynetxenon: addr remap %a:%d to %a:%d\n", uAddr, uSrcPort, pMap->uRemap, uDstPort));
...
static void _RemoveLSPMapping(SocketStateT *pState, uint32_t uAddress)
{
    ...
    for (iAddrMap = 0; iAddrMap < sizeof(pState->aSockAddrMap)/sizeof(pState->aSockAddrMap[0]); iAddrMap += 1)
    {
        ...
        {
            // check for name map(s) to this addr map
            for (iNameMap = 0; iNameMap < sizeof(pState->aSockNameMap)/sizeof(pState->aSockNameMap[0]); iNameMap += 1)
            {
                ...
                if (pNameMap->uRemap == pAddrMap->uAddr)
                {
                    // scrub entry
                    NetPrintf(("dirtynetxenon: scrubbing name map entry %s\n", _DisplayNameMapEntry(pNameMap)));
                    pNameMap->uRemap = 0;
                }
            }

            // scrub entry
            NetPrintf(("dirtynetxenon: scrubbing addr map entry %s\n", _DisplayAddrMapEntry(pAddrMap)));
...
static SocketT *_SocketOpen(SOCKET s, int32_t af, int32_t type, int32_t proto)
{
    ...
    if (s == INVALID_SOCKET)
    {
        ...
        {
            NetPrintf(("dirtynetxenon: error %d creating socket\n", WSAGetLastError()));
            return(NULL);
        }
    }
...
static int32_t _SocketClose(SocketT *pSocket, uint32_t bShutdown)
{
    ...
    {
        NetPrintf(("dirtynetxenon: warning, trying to close socket 0x%08x that is not in the socket list\n", (uintptr_t)pSocket));
        return(-1);
    }
    ...
    // mark as closed
    if ((bShutdown == TRUE) && (pSocket->socket != INVALID_SOCKET))
    {
        // close winsock socket
        shutdown(pSocket->socket, 2);
...
static int32_t _SocketRead(SocketT *pSocket)
{
    ...
    if (iResult != 0)
    {
        ...
        {
            NetPrintf(("dirtynetxenon: error %d trying to receive from socket %d\n", pSocket->recverr, pSocket->socket));
        }
    }
...
static void _SocketRecvThread(uint32_t uUnused)
{
    ...
    // show we are alive
    NetPrintf(("dirtynetxenon: receive thread started (thid=%d)\n", GetCurrentThreadId()));
                ...
                if (pSocket->recvstat <= 0)
                {
                    ...
                    if (pSocket->recvinp == 1)
                    {
                        ...
                        else if ((iResult = WSAGetLastError()) != WSA_IO_INCOMPLETE)
                        {
                            ...
                            {
                                NetPrintf(("dirtynetxenon: WSAGetOverlappedResult error %d\n", iResult));
                            }
                            #endif
    ...
    NetPrintf(("dirtynetxenon: receive thread exit\n"));
    pState->iRecvLife = 0;
}
...
int32_t SocketCreate(int32_t iThreadPrio, int32_t iThreadStackSize, int32_t iThreadCpuAffinity)
{
    ...
    if (iResult != 0)
    {
        NetPrintf(("dirtynetxenon: error %d loading winsock library\n", iResult));
    ...
    {
        NetPrintf(("dirtynetxenon: error %d creating state event\n", GetLastError()));
        SocketDestroy(0);
        return(-4);
    }
    ...
    if ((pState->hRecvThread = CreateThread(NULL, iThreadStackSize, (LPTHREAD_START_ROUTINE)&_SocketRecvThread, NULL, 0, (LPDWORD)&pid)) != NULL)
    {
        ...
        {
            NetPrintf(("dirtynetxenon: error %d adjusting thread priority\n", GetLastError()));
        }
    ...
    else
    {
        ...
        NetPrintf(("dirtynetxenon: error %d creating socket receive thread\n", GetLastError()));
        SocketDestroy(0);
        return(-5);
    }
    #endif
...
int32_t SocketDestroy(uint32_t uShutdownFlags)
{
    ...
    if (pState == NULL)
    {
        NetPrintf(("dirtynetxenon: SocketDestroy() called while module is not active\n"));
        return(-1);
    }

    NetPrintf(("dirtynetxenon: shutting down\n"));
...
int32_t SocketBind(SocketT *pSocket, const struct sockaddr *name, int32_t namelen)
{
    ...
    if (pSocket->type == SOCK_DGRAM)
    {
        ...
        if ((uPort = SockaddrInGetPort(name)) != 0)
        {
            ...
            {
                // close winsock socket
                NetPrintf(("dirtynetxenon: [%p] making socket bound to port %d virtual\n", pSocket, uPort));
                if (pSocket->socket != INVALID_SOCKET)
                {
    ...
    // execute the bind
    iResult = _XlatError(bind(pSocket->socket, name, namelen));
...
int32_t SocketConnect(SocketT *pSocket, struct sockaddr *name, int32_t namelen)
{
    ...
    if (((iResult = SocketBind(pSocket, &temp, sizeof(temp))) < 0) && (iResult != SOCKERR_INVALID))
    {
        pSocket->iLastError = iResult;
        return(pSocket->iLastError);
    }
    ...
    // map the address
    pRemap = _MapAddress(pSocket, &temp, name);

    // execute the connect
    iResult = _XlatError(connect(pSocket->socket, pRemap, namelen));

    // save connect address
    ds_memcpy_s(&pSocket->remote, sizeof(pSocket->remote), pRemap, sizeof(*pRemap));

    // update if lsp connection
    DirtyLSPConnectionUpdate(SockaddrInGetAddr(&pSocket->remote));
...
SocketT *SocketAccept(SocketT *pSocket, struct sockaddr *addr, int32_t *addrlen)
{
    ...
    {
        incoming = accept(pSocket->socket, addr, addrlen);
        if (incoming != INVALID_SOCKET)
        {
            open = _SocketOpen(incoming, pSocket->family, pSocket->type, pSocket->proto);
        ...
        else
        {
            ...
            {
                NetPrintf(("dirtynetxenon: accept() failed err=%d\n", WSAGetLastError()));
            }
        }
    }
...
int32_t SocketSendto(SocketT *pSocket, const char *buf, int32_t len, int32_t flags, const struct sockaddr *to, int32_t tolen)
{
    ...
    if (pSocket->bSendCbs)
    {
        ...
        if ((iResult = SocketSendCallbackInvoke(&pState->aSendCbEntries[0], pSocket, pSocket->type, buf, len, to)) > 0)
        {
            return(iResult);
        }
    }
    ...
    if (pSocket->socket == INVALID_SOCKET)
    {
        ...
        if (to)
        {
            uAddr = SockaddrInGetAddr(to);
            uPort = SockaddrInGetPort(to);
        }
        NetPrintf(("dirtynetxenon: attempting to send to %a:%d on invalid socket\n", uAddr, uPort));
        #endif
        pSocket->iLastError = SOCKERR_INVALID;
        return(pSocket->iLastError);
    }
    ...
    if ((len = SocketRateThrottle(&pSocket->SendRate, pSocket->type, len, "send")) == 0)
    {
        return(0);
    }
...
int32_t SocketRecvfrom(SocketT *pSock, char *pBuf, int32_t iLen, int32_t iFlags, struct sockaddr *pFrom, int32_t *pFromLen)
{
    ...
    if ((iLen = SocketRateThrottle(&pSock->RecvRate, pSock->type, iLen, "recv")) == 0)
    {
        return(0);
    }
    ...
    if (pSock->bAsyncRecv)
    {
        ...
        if (((iRecv = pSock->recvstat) != 0) && (iLen > 0))
        {
            ...
            if (iRecv > 0)
            {
                ...
                ds_memcpy(pBuf, pSock->recvdata, iRecv);
                // got data, no error
    ...
    if (pSock->recvstat != 0)
    {
        ...
        ds_memcpy(pBuf, pSock->recvdata, iLen);
        pSock->recvstat = 0;
        return(iLen);
    }
    #endif
    {
        ...
        // update if xlsp connection and we received data
        if (iRecv > 0)
        {
...
int32_t SocketInfo(SocketT *pSocket, int32_t iInfo, int32_t iData, void *pBuf, int32_t iLen)
{
    ...
    if (pSocket == NULL)
    {
        ...
        if (iInfo == 'sdcf')
        {
            ...
            NetPrintf(("dirtynetxenon: 'sdcf' selector used with invalid paramaters\n"));
            return(-1);
        }
        ...
        if (iInfo == 'sdcu')
        {
            ...
            NetPrintf(("dirtynetxenon: 'sdcu' selector used with invalid paramaters\n"));
            return(-1);
        }
        ...
        if ((iInfo == 'xmap') && (pBuf != NULL) && (iLen > 0))
        {
            ...
            ds_memcpy(pBuf, pState->aSockAddrMap, iLen);
            return(0);
        }
        ...
        if ((iInfo == 'xnam') && (pBuf != NULL) && (iLen > 0))
        {
            ...
            ds_memcpy(pBuf, pState->aSockNameMap, iLen);
            return(0);
        }
        ...
        {
            int32_t iMapEntry;
            NetPrintf(("dirtynetxenon: name mapping table\n"));
        ...
        NetPrintf(("dirtynetxenon: unhandled global SocketInfo() selector '%C'\n", iInfo));
    ...
    if (iInfo == 'stat')
    {
        ...
        if (pSocket->opened == 0)
        {
            ...
            if (select(1, NULL, &fdwrite, &fdexcept, &tv) > 0)
            {
                ...
                {
                    NetPrintf(("dirtynetxenon: connection failed\n"));
                    pSocket->opened = -1;
                }
                // if socket is writable, that means connect succeeded
                else if (fdwrite.fd_count > 0)
                {
                    NetPrintf(("dirtynetxenon: connection open\n"));
                    pSocket->opened = 1;
                }
            }
        }
        ...
        if (pSocket->opened > 0)
        {
            ...
            if (select(1, &fdread, NULL, &fdexcept, &tv) > 0)
            {
                ...
                if (fdexcept.fd_count > 0)
                {
                    NetPrintf(("dirtynetxenon: connection failure\n"));
                    pSocket->opened = -1;
                }
                else if (fdread.fd_count > 0)
                {
                    ...
                    if ((ioctlsocket(pSocket->socket, FIONREAD, &uAvailBytes) != 0) || (uAvailBytes == 0))
                    {
                        ...
                        NetPrintf(("dirtynetxenon: connection closed (wsaerr=%d)\n", (uAvailBytes==0) ? WSAECONNRESET : WSAGetLastError()));
                        pSocket->opened = -1;
                    }
                }
            }
...
int32_t SocketCallback(SocketT *pSocket, int32_t mask, int32_t idle, void *ref, int32_t (*proc)(SocketT *sock, int32_t flags, void *ref))
{
    pSocket->callidle = idle;
    pSocket->callmask = mask;
    pSocket->callref = ref;
    pSocket->callback = proc;
    return(0);
}
#pragma optimize("", on)
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'madr' - perform address remap (pData2=source sockaddr, pData3=remapped sockaddr)
            ...
            'xtim' - set xlsp cache timeout in milliseconds
...
int32_t SocketControl(SocketT *pSocket, int32_t iOption, int32_t iData1, void *pData2, void *pData3)
{
    ...
    if (iOption == 'conn')
    {
        ...
        {
            NetPrintf(("dirtynetxenon: could not create DirtyLSP\n"));
        }
        DirtyMemGroupLeave();
    ...
    if (iOption == 'madr')
    {
        ...
        ds_memcpy(pData3, pAddr, sizeof(*pAddr));
        return(0);
    }
    // set max udp packet size
    if (iOption == 'maxp')
    {
        NetPrintf(("dirtynetxenon: setting max udp packet size to %d\n", iData1));
    ...
    if (iOption == 'maxr')
    {
        NetPrintf(("dirtynetxenon: setting max recv rate to %d bytes/sec\n", iData1));
    ...
    if (iOption == 'maxs')
    {
        NetPrintf(("dirtynetxenon: setting max send rate to %d bytes/sec\n", iData1));
    ...
    if (iOption == 'push')
    {
        if (pSocket != NULL)
        {
            ...
            {
                NetPrintf(("dirtynetxenon: request to push %d bytes of data discarded (max=%d)\n", iData1, sizeof(pSocket->recvdata)));
                #if SOCKET_ASYNCRECVTHREAD
                NetCritLeave(&pSocket->recvcrit);
                #endif
                return(-1);
            ...
            if (pSocket->recvstat > 0)
            {
                NetPrintf(("dirtynetxenon: warning - overwriting packet data with SocketControl('push')\n"));
            }

            // save the size and copy the data
            pSocket->recvstat = iData1;
            ds_memcpy_s(pSocket->recvdata, sizeof(pSocket->recvdata), pData2, pSocket->recvstat);
        ...
        {
            NetPrintf(("dirtynetxenon: warning - call to SocketControl('push') ignored because pSocket is NULL\n"));
            return(-1);
        }
        return(0);
    }
    ...
    if ((iOption == 'rbuf') || (iOption == 'sbuf'))
    {
        ...
        NetPrintf(("dirtynetxenon: setsockopt(%s) changed buffer size from %d to %d\n", (iOption == 'rbuf') ? "SO_RCVBUF" : "SO_SNDBUF",
            iOldSize, iNewSize));
    ...
    if (iOption == 'scbk')
    {
        ...
        {
            NetPrintf(("dirtynetxenon: send callbacks usage changed from %s to %s for socket ref %p\n", (pSocket->bSendCbs?"ON":"OFF"), (iData1?"ON":"OFF"), pSocket));
    ...
    if (iOption == 'xins')
    {
        NetPrintf(("dirtynetxenon: secure bypass %s\n", iData1 ? "enabled" : "disabled"));
    ...
    if (iOption == 'xmap')
    {
        ...
        {
            NetPrintf(("dirtynetxenon: clamping addr map to %d entries\n", sizeof(pState->aSockAddrMap)/sizeof(pState->aSockAddrMap[0])));
            iData1 = sizeof(pState->aSockAddrMap);
        }
        ds_memcpy(pState->aSockAddrMap, pData2, iData1);
        return(0);
    }
    ...
    if (iOption == 'xnam')
    {
        ...
        {
            NetPrintf(("dirtynetxenon: clamping name map to %d entries\n", sizeof(pState->aSockNameMap)/sizeof(pState->aSockNameMap[0])));
            iData1 = sizeof(pState->aSockNameMap);
        }
        ds_memcpy(pState->aSockNameMap, pData2, iData1);
        return(0);
    }
    ...
    if (iOption == '+xam')
    {
        for (iMapEntry = 0; iMapEntry < sizeof(pState->aSockAddrMap)/sizeof(pState->aSockAddrMap[0]); iMapEntry++)
        {
            ...
            {
                ds_memcpy(&pState->aSockAddrMap[iMapEntry], pData2, sizeof(pState->aSockAddrMap[0]));
                NetPrintf(("dirtynetxenon: add addr map entry [%d] %s\n", iMapEntry, _DisplayAddrMapEntry(&pState->aSockAddrMap[iMapEntry])));
                return(iMapEntry);
            }
        }
        NetPrintf(("dirtynetxenon: unable to add addr map entry %s; table full\n", _DisplayAddrMapEntry((SocketAddrMapT *)pData2)));
        return(-1);
    }
    ...
    if (iOption == '+xnm')
    {
        for (iMapEntry = 0; iMapEntry < sizeof(pState->aSockNameMap)/sizeof(pState->aSockNameMap[0]); iMapEntry++)
        {
            ...
            {
                ds_memcpy(&pState->aSockNameMap[iMapEntry], pData2, sizeof(pState->aSockNameMap[0]));
                NetPrintf(("dirtynetxenon: add name map entry [%d] %s\n", iMapEntry, _DisplayNameMapEntry(&pState->aSockNameMap[iMapEntry])));
                return(iMapEntry);
            }
        }
        NetPrintf(("dirtynetxenon: unable to add name map entry %s; table full\n", _DisplayNameMapEntry((SocketNameMapT *)pData2)));
        return(-1);
    }
    ...
    if (iOption == '-xam')
    {
        ...
        if (pData2 != NULL)
        {
            ...
            {
                NetPrintf(("dirtynetxenon: could not find addr map entry %s to delete\n", _DisplayAddrMapEntry((SocketAddrMapT *)pData2)));
            }
        }
        ...
        {
            NetPrintf(("dirtynetxenon: could not delete addr map entry with index %d; invalid range\n", iData1));
        }
        // remove entry?
        if (iDelEntry != -1)
        {
            NetPrintf(("dirtynetxenon: del addr map entry [%d] %s\n", iDelEntry, _DisplayAddrMapEntry(&pState->aSockAddrMap[iDelEntry])));
    ...
    if (iOption == '-xnm')
    {
        ...
        if (pData2 != NULL)
        {
            ...
            if (iMapEntry == sizeof(pState->aSockAddrMap)/sizeof(pState->aSockAddrMap[0]))
            {
                NetPrintf(("dirtynetxenon: could not find name map entry %s to delete\n", _DisplayNameMapEntry((SocketNameMapT *)pData2)));
            }
        }
        ...
        {
            NetPrintf(("dirtynetxenon: could not delete name map entry with index %d; invalid range\n", iData1));
        }
        // remove entry?
        if (iDelEntry != -1)
        {
            NetPrintf(("dirtynetxenon: del name map entry [%d] %s\n", iDelEntry, _DisplayNameMapEntry(&pState->aSockNameMap[iDelEntry])));
    ...
    if (WaitForSingleObject((HANDLE)host->thread, 0) == WAIT_OBJECT_0)
    {
        if (pXndns->iStatus == 0)
        {
            ...
            if (pXndns->cina > 0)
            {
                ...
                NetPrintf(("dirtnetwin: _SocketLookupDone() success, addr=%a\n", host->addr));
            }
        }
        else if (pXndns->iStatus != WSAEINPROGRESS)
        {
            NetPrintf(("dirtnetwin: _SocketLookupDone() failure, err=%d\n", pXndns->iStatus));
            host->done = -1;
        }
    }
    ...
    if (pHost->done == 0)
    {
        ...
        if ((pHost->done = DirtyLSPQueryDone(pState->pDirtyLSP, pQueryRec)) > 0)
        {
            ...
            for (iAddrMap = 0; iAddrMap < sizeof(pState->aSockAddrMap)/sizeof(pState->aSockAddrMap[0]); iAddrMap += 1)
            {
                ...
                if (pAddrMap->uAddr == 0)
                {
                    // ref initial name map entry
                    SocketNameMapT *pNameMap = (SocketNameMapT *)pHost->pData;
            ...
            {
                NetPrintf(("dirtynetxenon: warning -- could not add lsp query result to address map\n"));
            }
        }
    }
    return(pHost->done);
...
HostentT *SocketLookup2(const char *text, int32_t timeout, uint32_t flags)
{
    ...
    NetPrintf(("dirtynetxenon: looking up address for '%s'\n", text));
    ...
    if ((pState->iDnsRemap != 0) && ((flags & SOCKLOOK_FLAGS_ALLOWXDNS) == 0))
    {
        ...
        if ((pSockNameMap = _MapName(pState, text)) != NULL)
        {
            ...
            if (pSockNameMap->uRemap == 0)
            {
                ...
                // start LSP query
                ds_strnzcpy(pHost->name, pSockNameMap->strRemap, sizeof(pHost->name));
                if ((pState->pDirtyLSP == NULL) || ((pQueryRec = DirtyLSPQueryStart(pState->pDirtyLSP, pHost->name)) == NULL))
                {
                    NetPrintf(("dirtynetxenon: error initiating service query\n"));
                    pHost->addr = 0;
                    pHost->done = -1;
                    return(pHost);
                }
                ...
                // save name remap entry
                pHost->pData = (void *)pSockNameMap;
            }
        ...
        {
            NetPrintf(("dirtynetxenon: dns remap %s to %a\n", text, pState->iDnsRemap));
    ...
    else // not in secure mode, so use real DNS
    {
        ...
        {
            NetPrintf(("dirtynetxenon: error %d initiating DNS lookup\n", iErr));
...
int32_t SocketHost(struct sockaddr *host, int32_t hostlen, const struct sockaddr *dest, int32_t destlen)
{
    ...
    if (hostlen != destlen)
    {
        NetPrintf(("dirtynetxenon: SocketHost() len mismatch\n"));
        return(-1);
    }
