...
struct DirtySessionManagerRefT
{
    ...
    #if DIRTYCODE_LOGGING
    uint8_t bPresMutexUnavailWarned;            //!< whether we have warned about the presence mutex being already taken
    #else
    uint8_t _pad[1];
    #endif
...
static int32_t _DirtySessionManagerWriteStats(DirtySessionManagerRefT *pRef, DWORD dwNumViews, XSESSION_VIEW_PROPERTIES *pViews)
{
    ...
    {
        ds_memcpy(&pRef->pViews[uView], &pViews[uView], sizeof(XSESSION_VIEW_PROPERTIES));
    }
...
//    }
    ...
    if (pRef->iUserIndex != -1)
    {
        ...
        // create the session
        NetPrintf(("dirtysessionmanagerxenon: [0x%08x] creating session with flags 0x%08x\n", pRef, iFlagsUsed));
        ...
        for (iUserIndex = 0; iUserIndex < NETCONN_MAXLOCALUSERS; iUserIndex++)
        {
            ...
            NetPrintf(("dirtysessionmanagerxenon: [0x%08x] creating session with controller=%d, xuid=%d\n", pRef, iUserIndex, Op.PeerXuid[iUserIndex]));
        }
...
void DirtySessionManagerUpdate(DirtySessionManagerRefT *pRef)
{
    ...
    if (!pRef->bOpInProgress && (pRef->iNumQueuedOperations > 0))
    {
        ...
        case ST_SESSMGR_CREA:
            ...
            if (pOp->iFlags[0] & XSESSION_CREATE_USES_PRESENCE)
            {
                ...
                {
                    #if DIRTYCODE_LOGGING
                    if (pRef->bPresMutexUnavailWarned == FALSE)
                    {
                        NetPrintf(("dirtysessionmanagerxenon: [0x%08x] Presence mutex already taken - starting to wait for it ... \n", pRef));
                        pRef->bPresMutexUnavailWarned = TRUE;
                    }
                    #endif
                    return;
                }
                ...
                #if DIRTYCODE_LOGGING
                pRef->bPresMutexUnavailWarned = FALSE;
                #endif
            ...
            //do we only create the session for one LOCAL user (yes), xsessionjoinlocal for other users
        ...
        default:
            NetPrintf(("dirtysessionmanagerxenon: [0x%08x] Terminating, unknown operation %d\n", pRef, pOp->eOp));
            _DirtySessionManagerTerminate(pRef);
    ...
    if ((pRef->bOpInProgress && (pRef->iNumQueuedOperations > 0)) || bOperationCompletedImmediately)
    {
        ...
        //
        //join local users, should be able to get local user info out of the Op
...
/*F*************************************************************************************************/
/*!
    \Function    DirtySessionManagerGetSecureIp
...
*/
/*************************************************************************************************F*/
int32_t DirtySessionManagerGetSecureIp(DirtySessionManagerRefT *pRef, int32_t *pAddr, const char* pMachineAddr)
{
    XNADDR XnAddr;
    IN_ADDR InAddr;
    HRESULT hResult;
    ...
    {
        NetPrintf(("dirtysessionmanagerxenon: [0x%08x] DirtySessionManagerGetSecureIp() used on a session not flagged with XSESSION_CREATE_USES_PEER_NETWORK\n", pRef));
        return(-1);
    }
    ...
    NetPrintf(("dirtysessionmanagerxenon: [0x%08x] XnAddr.inaOnline=%a\n", pRef, XnAddr.inaOnline.s_addr));
    NetPrintf(("dirtysessionmanagerxenon: [0x%08x] XnAddr.wPortOnline=%d\n", pRef, XnAddr.wPortOnline));

    // convert xnaddr to secure address
    if ((hResult = XNetXnAddrToInAddr(&XnAddr, &pRef->SessionInfo.sessionID, &InAddr)) != S_OK)
    {
        NetPrintf(("dirtysessionmanagerxenon: [0x%08x] XNetXnAddrToInAddr() = 0x%08x\n", pRef, hResult));
        return(-1);
    }
    NetPrintf(("dirtysessionmanagerxenon: [0x%08x] secure addr=%a\n", pRef, InAddr.s_addr));
