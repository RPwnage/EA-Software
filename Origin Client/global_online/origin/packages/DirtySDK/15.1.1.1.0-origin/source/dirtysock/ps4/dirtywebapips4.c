...
static int32_t _DirtyWebApiInitWebApi(DirtyWebApiRefT *pRef, const DirtyWebApiCreateParamsT* pCreateParams)
{
    ...
    if ((pRef->iNetPoolCtxId = sceNetPoolCreate(DIRTY_WEBAPI_NET_HEAP_NAME, pCreateParams->netHeapSize, 0)) < 0)
    {
        NetPrintf(("dirtywebapips4: [%p] sceNetPoolCreate() failed, (err=%s)\n", pRef, DirtyErrGetName(pRef->iNetPoolCtxId)));
        return(pRef->iNetPoolCtxId);
    }

    if ((pRef->iSslCtxId = sceSslInit(pCreateParams->sslHeapSize)) < 0)
    {
        NetPrintf(("dirtywebapips4: [%p] sceSslInit() failed, (err=%s)\n", pRef, DirtyErrGetName(pRef->iSslCtxId)));
        return(pRef->iSslCtxId);
    }

    if ((pRef->iHttpCtxId = sceHttpInit(pRef->iNetPoolCtxId, pRef->iSslCtxId, pCreateParams->httpHeapSize)) < 0)
    {
        NetPrintf(("dirtywebapips4: [%p] sceHttpInit() failed, (err=%s)\n", pRef, DirtyErrGetName(pRef->iHttpCtxId)));
...
static void _DirtyWebApiShutdownWebApi(DirtyWebApiRefT *pRef)
{
    ...
    {
        if ((iRet = sceHttpTerm(pRef->iHttpCtxId)) != 0)
        {
            NetPrintf(("dirtywebapips4: [%p] sceHttpTerm() failed, (err=%s)\n", pRef, DirtyErrGetName(iRet)));
        }
    }

    if (pRef->iSslCtxId >= 0)
    {
        if ((iRet = sceSslTerm(pRef->iSslCtxId)) != 0)
        {
            NetPrintf(("dirtywebapips4: [%p] sceSslTerm() failed, (err=%s)\n", pRef, DirtyErrGetName(iRet)));
        }
    }

    if (pRef->iNetPoolCtxId >= 0)
    {
        if ((iRet = sceNetPoolDestroy(pRef->iNetPoolCtxId)) != 0)
        {
            NetPrintf(("dirtywebapips4: [%p] sceNetPoolDestroy() failed, (err=%s)\n", pRef, DirtyErrGetName(iRet)));
        }
    }
}
...
static void *_DirtyWebApiThread(void *pArg)
{
    ...
    NetPrintf(("dirtywebapips4: [%p] Thread exit\n", pRef));
    return(NULL);
}
