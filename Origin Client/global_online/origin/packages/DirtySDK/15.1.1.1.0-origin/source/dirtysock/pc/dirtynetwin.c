...
struct SocketT
{
    ...
    SOCKET socket;              //!< winsock socket ref
...
static SocketT *_SocketOpen(SOCKET s, int32_t af, int32_t type, int32_t proto)
{
    ...
    if ((s == INVALID_SOCKET) && ((s = socket(af, type, proto)) == INVALID_SOCKET))
    {
    ...
    pSocket->type = type;
    pSocket->proto = proto;
    pSocket->bAsyncRecv = ((type == SOCK_DGRAM) || (type == SOCK_RAW)) ? TRUE : FALSE;
...
static int32_t _SocketClose(SocketT *sock, uint32_t bShutdown)
{
    ...
    if (sock->pRecvQueue != NULL)
    {
        SocketPacketQueueDestroy(sock->pRecvQueue);
    }
...
static int32_t _SocketRead(SocketT *pSocket)
{
    ...
    if (iResult != 0)
    {
        ...
        {
            NetPrintf(("dirtynetwin: [%p] error %d trying to receive from socket\n", pSocket->socket, pSocket->recverr));
        }
    }
...
int32_t SocketBind(SocketT *sock, const struct sockaddr *name, int32_t namelen)
{
    ...
    {
        ds_memcpy_s(&BindAddr, sizeof(BindAddr), name, sizeof(*name));
    ...
    if (sock->type == SOCK_DGRAM)
    {
        ...
        if ((uPort = SockaddrInGetPort(name)) != 0)
        {
            ...
            if (iPort < SOCKET_MAXVIRTUALPORTS)
            {
                ...
                sock->pRecvQueue = SocketPacketQueueResize(sock->pRecvQueue, 4, pState->iMemGroup, pState->pMemGroupUserData);
                // mark socket as virtual
                sock->virtualport = uPort;
                sock->bVirtual = TRUE;
                // release socket critical section
                #if SOCKET_ASYNCRECVTHREAD
                NetCritLeave(&sock->recvcrit);
                #endif
                return(0);
            }
        }
...
int32_t SocketInfo(SocketT *pSocket, int32_t iInfo, int32_t iData, void *pBuf, int32_t iLen)
{
    ...
    if (iInfo == 'sock')
    {
        if (pBuf != NULL)
        {
            if (iLen == (int32_t)sizeof(pSocket->socket))
            {
                memcpy(pBuf, &pSocket->socket, sizeof(pSocket->socket));
            }
        }
        return((int32_t)pSocket->socket);
    }
...
int32_t SocketControl(SocketT *pSocket, int32_t iOption, int32_t iData1, void *pData2, void *pData3)
{
    ...
    if (iOption == 'keep')
    {
        if (pSocket->type == SOCK_STREAM)
        {
            if (pSocket->socket != INVALID_SOCKET)
            {
                ...
                {
                    pSocket->iLastError = SOCKERR_NONE;
                    NetPrintf(("dirtynetwin: successfully %s the TCP keep-alive for low-level socket [%p] [timeout=%dms|interval=%dms]\n",
                        iData1?"enabled":"disabled", pSocket->socket, tcpKeepAlive.keepalivetime, tcpKeepAlive.keepaliveinterval));
                }
                else
                {
                    pSocket->iLastError = _XlatError(iResult);
                    NetPrintf(("dirtynetwin: failed to %s TCP keep-alive for low-level socket [%p] (err = %d)\n",
    ...
    if (iOption == 'push')
    {
        ...
        {
            NetPrintf(("dirtynetwin: call to SocketControl('push') ignored (pSocket=%p)\n", pSocket));
            return(-1);
        }
        return(0);
    }
...
/*!
    ...
    \Input iTimeout - number of milliseconds to wait for completion (ignored in this implementation)
...
HostentT *SocketLookup(const char *pText, int32_t iTimeout)
{
    ...
    if ((hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)&_SocketLookupThread, (LPVOID)pHost, 0, &pid)) != NULL)
    {
        CloseHandle(hThread);
    }
    else
        ...
        if (sock != INVALID_SOCKET)
        {
            ...
            if (pState->iVersion >= 0x200)
            {
                ...
                else
                {
                    ...
                    ds_memcpy(host->sa_data+2, addr.sa_data+2, 4);
                }

                // convert loopback to requested address
                if (SockaddrInGetAddr(host) == INADDR_LOOPBACK)
                {
                    ds_memcpy(host->sa_data+2, dest->sa_data+2, 4);
                }
            }
            ...
            if (SockaddrInGetAddr(host) == 0)
            {
                ...
                // lookup ip info
                if((iResult = getaddrinfo(hostname, NULL, &Hints, &pList)) == 0)
                {
                    ds_memcpy(host->sa_data+2, pList->ai_addr->sa_data+2, 4);
                }
                else
                {
                    NetPrintf(("dirtynetwin: getaddrinfo() failed err=%d\n", iResult));
