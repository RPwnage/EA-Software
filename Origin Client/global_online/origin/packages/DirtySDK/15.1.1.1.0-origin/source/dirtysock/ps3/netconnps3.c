...
#define NETCONN_NPTICKET_MAJOR_VERSION (3)
#define NETCONN_NPTICKET_MINOR_VERSION (0)
...
typedef struct NetConnRefT
{
    ...
    NetConnNpMgrCbRegT *pNpMgrCbReg;    //!< NP Mgr registration override
    NetConnNpMgrCbUnregT *pNpMgrCbUnreg;//!< NP Mgr unregistration override
     ...
     */
    struct CellNetCtlNetStartDialogResult NetStartResult;//!<result struct for cellNetCtlNetStartDialogUnloadAsync
    ...
    int8_t          iTSafeEndsDlgStatus;//!< end dialog async status (-1=idle, 0=pending, 1=completed)
    ...
    int32_t         iNpTicketStatus;    //!< ticket status (<0=failed, 0=pending, 1=available)
...
static void _NetConnUpdateConnStatus(NetConnRefT *pRef, uint32_t uNewConnStatus)
{
    ...
    NetPrintf(("netconnps3: netconn status changed to %s\n", strConnStatus));
}

/*F********************************************************************************/
/*!
    \Function _NetConnPsnUserMask
    ...
    \Output
        uint32_t    - mask of users bits 0-3

    \Version 12/09/2013 (cvienneau)
*/
/********************************************************************************F*/
static uint32_t _NetConnPsnUserMask(NetConnRefT *pRef)
{
    uint32_t iIndex;
    ...
    if ((iResult = sceNpManagerGetStatus(&iStateId)) == 0)
    {
        ...
        {
            //we know one user is signed in, since we're online with NP
            uUserMask = 1;
            //add the sub users, we can skip slot 0 since its not used
            for (iIndex = 1; iIndex < NETCONN_MAXLOCALUSERS; ++iIndex)
            {
    ...
    {
        NetPrintf(("netconnps3: sceNpManagerGetStatus() failed in _NetConnPsnUserMask(): err=%s\n", DirtyErrGetName(iResult)));
    }
    return(uUserMask);
}
...
static void _NetConnParseParams(NetConnRefT *pRef, const char *pParams)
{
    ...
    NetPrintf(("netconnps3: setting friend=%s\n", strTemp));
...
static void _NetConnSysUtilCallback(uint64_t uStatus, uint64_t uParam, void * pUserData)
{
    ...
    // iTSafeStrtDlgStatus and iTSafeEndsDlgStatus can only be accessed in the context
    ...
    switch (uStatus)
    {
        ...
        {
            NetPrintf(("netconnps3: delaying processing of new sysutil status CELL_SYSUTIL_NET_CTL_NETSTART_UNLOADED\n"));

            pRef->iTSafeEndsDlgStatus = 1; // unload dialog completed
            break;
        }
        default:
            break;
...
static void _NetConnSysutilDlgFinishDelayedEvent(NetConnRefT *pRef)
{
    ...
    memset(&pRef->NetStartResult, 0, sizeof(pRef->NetStartResult));
    pRef->NetStartResult.size = sizeof(pRef->NetStartResult);

    NetPrintf(("netconnps3: processing of delayed event CELL_SYSUTIL_NET_CTL_NETSTART_FINISHED\n"));

    if ((iResult = cellNetCtlNetStartDialogUnloadAsync(&pRef->NetStartResult)) != 0)
    {
        NetPrintf(("netconnps3: cellNetCtlNetStartDialogUnloadAsync() failed: err=%s\n", DirtyErrGetName(iResult)));
    }
    else
    {
        pRef->iTSafeEndsDlgStatus = 0; // mark the unload in progress
    }
}

/*F********************************************************************************/
/*!
    \Function _NetConnSysutilDlgFinishDelayedUnloadedEvent

    \Input *pRef    - module state

    \Description
        Process delayed sysutil CELL_SYSUTIL_NET_CTL_NETSTART_UNLOADED event.
        Not thread-safe - execute only during idle loop.

    \Version 02/20/2015 (amakoukji)
*/
/********************************************************************************F*/
static void _NetConnSysutilDlgFinishDelayedUnloadedEvent(NetConnRefT *pRef)
{
    if (pRef->NetStartResult.result == 0)
    {
    ...
    else if ((uint32_t)pRef->NetStartResult.result == CELL_NET_CTL_ERROR_NP_NO_ACCOUNT)
    {
        _NetConnUpdateConnStatus(pRef, '-act'); //the network won't be able to work on NP
        pRef->eState = ST_IDLE;
    }
    else
    {
        _NetConnUpdateConnStatus(pRef, '-err'); //the network won't be able to work on NP
        pRef->eState = ST_IDLE;
    }
...
static int32_t _NetConnNpInit(NetConnRefT *pRef)
{
    ...
    if (pRef->pNpMgrCbReg == NULL)
    {
        iResult = sceNpManagerRegisterCallback(_NetConnNpManagerCallback, _NetConn_pRef);
    }
    else
    {
        iResult = pRef->pNpMgrCbReg(_NetConnNpManagerCallback, _NetConn_pRef);
    }
    if (iResult != 0)
    {
        NetPrintf(("netconnps3: registration of callback with NP manager failed: err=%s\n", DirtyErrGetName(iResult)));
...
static void _NetConnNpTerm(NetConnRefT *pRef)
{
    ...
    if (pRef->pNpMgrCbUnreg == NULL)
    {
        iResult = sceNpManagerUnregisterCallback();
    }
    else
    {
        iResult = pRef->pNpMgrCbUnreg();
    }
    if (iResult != 0)
    {
        NetPrintf(("netconnps3: unregistration of callback with NP manager failed: err=%s\n", DirtyErrGetName(iResult)));
    }
...
/*!
    ...
    \Input *pData   - module state pointer, passed as void ptr userdata
...
static void _NetConnSubSigninCallback(int32_t iResult, SceNpId *pNpId, void *pData)
{
    ...
    if (iResult == SCE_NP_MANAGER_SUB_SIGNIN_RESULT_OK)
    {
        ...
        ds_memcpy_s(&pRef->aSubUsers[pRef->uSubUserSigninSlot], sizeof(SceNpId), pNpId, sizeof(SceNpId));
        ...
        if ((uRet = _NetConnIsTicketValid(pRef, &bValid, uValiditySlot)) == 0)
        {
            if (!bValid)
            {
                NetConnRequestTicket(NETCONN_NPTICKET_MAJOR_VERSION, NETCONN_NPTICKET_MINOR_VERSION, NULL, NULL, 0, NULL, 0, _NetConnTicketCallback, pRef);
            }
            else
            {
                pRef->uSubUserSigninSlot = 0;
    ...
    if ((iResult = sceNpManagerGetStatus(&iStateId)) == 0)
    {
        if (iStateId != SCE_NP_MANAGER_STATUS_ONLINE)
        {
            if ((iResult = cellSysutilRegisterCallback(pRef->uSysutilCbSlot, _NetConnSysUtilCallback, NULL)) == CELL_OK)
            {
                ...
                if (pRef->iTSafeStrtDlgStatus == -1)
                {
                    ...
                    if ((iResult = cellNetCtlNetStartDialogLoadAsync(&NetStartParam)) != 0)
                    {
                        ...
                        _NetConnUpdateConnStatus(pRef, '-dlg');
                        //there was an error, the callback wont be used.
                        cellSysutilUnregisterCallback(pRef->uSysutilCbSlot);
                    }
                    else
                    {
            ...
            else
            {
                ...
                _NetConnUpdateConnStatus(pRef, '-reg');
            }
        }
    }
    else
...
static void _NetConnProcessDelayedSysutilEvents(NetConnRefT *pRef)
{
    ...
    if (pRef->iTSafeEndsDlgStatus == 1)
    {
        _NetConnSysutilDlgFinishDelayedUnloadedEvent(pRef);
        pRef->iTSafeEndsDlgStatus = -1;   // back to idle
    }
...
static void _NetConnUpdate(void *pData, uint32_t uData)
{
    ...
    {
        _NetConnUpdateConnStatus(pRef, SocketInfo(NULL, 'conn', 0, NULL, 0));
    ...
    if (pRef->eState == ST_NPCN)
    {
        ...
        if (pRef->iNpStatus == SCE_NP_MANAGER_STATUS_OFFLINE)
        {
            if (pRef->iLastNpStatus != SCE_NP_MANAGER_STATUS_OFFLINE)
            {
                // signal that we want from not offline to offline
                _NetConnUpdateConnStatus(pRef, '-off');
            }
            else
            {
                // while offline, check whether we were already notified by the system about an NP signin error
                // this code path can be exercised when PSN is down
                if (((int32_t)pRef->uNpErrResult) < 0)
                {
                    // we had already received and processed the SCE_NP_MANAGER_STATUS_OFFLINE event in a previous eState
                    ...
                    {
                        _NetConnUpdateConnStatus(pRef, '-dsc');
                    }
                }
            }
        }
        ...
        if (pRef->iNpStatus == SCE_NP_MANAGER_STATUS_ONLINE)
        {
            ...
            {
                _NetConnUpdateConnStatus(pRef, '+npo');
    ...
    if (pRef->eState == ST_TICK)
    {
        ...
        if (!pRef->bNpTicketRequested && !pRef->bNoTicket)
        {
            ...
            if ((iResult = _NetConnIsTicketValid(pRef, &bValid, 0)) == 0)
            {
                ...
                else
                {
                    ...
                    if ((iResult = NetConnRequestTicket(NETCONN_NPTICKET_MAJOR_VERSION, NETCONN_NPTICKET_MINOR_VERSION, NULL, NULL, 0, NULL, 0, _NetConnTicketCallback, pRef)) == 1)
                    {
                        pRef->bNpTicketRequested = TRUE;
                    }
                    else if (iResult < 0)
                    {
                        _NetConnUpdateConnStatus(pRef, '-tkt');
                        pRef->iNpTicketStatus = iResult;
                    }
                    else
                    {
        ...
        else if (pRef->iNpTicketStatus == 0)
        {
            ...
            if ((uSocketStatus >> 24) == '-')
            {
                if (pRef->uConnStatus != uSocketStatus)
                {
...
/*!
    ...
    \Input *pOption - asciiz list of config parameters
                      "silent=<true/false>" to prevent/enable sysutils from displaying.
                      "peerport=<port>" to specify peer port to be opened by upnp.
                      "noticket" to disable ticketing support
...
int32_t NetConnConnect(const NetConfigRecT *pConfig, const char *pOption, int32_t iData)
{
    ...
    if ((pRef->eState == ST_INIT) || (pRef->eState == ST_EXT_CLEANUP))
    {
        ...
        NetPrintf(("netconnps3: upnp peerport=%d %s\n",
...
int32_t NetConnDisconnect(void)
{
    ...
    // stop the friend api to unregister callbacks, we need new callbacks if/when we reconnect
    if (pRef->pFriendApiRef)
    {
        FriendApiControl(pRef->pFriendApiRef, 'stop', 0, 0, NULL);
    }
...
int32_t NetConnControl(int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue, void *pValue2)
{
    ...
    if (iControl == 'tick')
    {
        ...
        return(NetConnRequestTicket(NETCONN_NPTICKET_MAJOR_VERSION, NETCONN_NPTICKET_MINOR_VERSION, NULL, NULL, 0, NULL, 0, _NetConnTicketCallback, pRef));
    }
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            nper: return NP error result from last NP event
...
int32_t NetConnStatus(int32_t iKind, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    if (iKind == 'acct')
    {
        ...
        ds_memcpy(pBuf, &pRef->aNpAccountId[iData], sizeof(pRef->aNpAccountId[iData]));
        return(0);
    }
    ...
    if (iKind == 'mask')
    {
        return(_NetConnPsnUserMask(pRef));
    }
    // np commid
    if ((iKind == 'npcm') && (pBuf != NULL) && (iBufSize == sizeof(SceNpCommunicationId)))
    {
        memset(pBuf, 0, sizeof(SceNpCommunicationId));
        ds_memcpy(pBuf, pRef->strCommId, 9);
        return(0);
    }
    // np error result from last NP event
    if (iKind == 'nper')
    {
        return((int32_t)pRef->uNpErrResult);
    }
    // np titleid
    if ((iKind == 'npti') && (pBuf != NULL) && (iBufSize == sizeof(SceNpTitleId)))
    {
        ...
        ds_memcpy_s(pNpTitleId->data, sizeof(pNpTitleId->data), pRef->strTitleId, sizeof(pRef->strTitleId));
        return(0);
    }
    // np id
    if (iKind == 'npid')
    {
        ...
        {
            ds_memcpy(pBuf, &pRef->NpId, sizeof(pRef->NpId));
            return(0);
        }
        return(-1);
    }
    ...
    {
        ds_memcpy(pBuf, pRef->strSpid, 16);
        return(0);
    }
    if ((iKind == 'subu') && (iBufSize >= (int32_t)sizeof(SceNpId)) && (iData < NETCONN_MAX_SUBUSER))
    {
        ds_memcpy(pBuf, &pRef->aSubUsers[iData], sizeof(SceNpId));
        return(0);
    }
    // sce service id
    if ((iKind == 'svid') && (pBuf != NULL) && (iBufSize >= 32))
    {
        ds_memcpy(pBuf, pRef->strServiceId, 32);
        return(0);
    }
    ...
    if (iKind == 'tick')
    {
        ...
        ds_memcpy(pBuf, pRef->pNpTicket[iData], pRef->iNpTicketSize[iData]);
        return(pRef->iNpTicketSize[iData]);
    }
...
int32_t NetConnShutdown(uint32_t uShutdownFlags)
{
    ...
    //there is a pending sysutil dialog, and we wont be around for its result, so cleanup now
    NetCritEnter(&_NetConn_pRef->callbackCrit);
    if (_NetConn_pRef->iTSafeStrtDlgStatus != -1)
    {
        ...
        NetPrintf(("netconnps3: NetConnShutdown, shutting down active sysutil.\n"));
        if ((iResult = cellSysutilUnregisterCallback(_NetConn_pRef->uSysutilCbSlot)) != 0)
        {
            NetPrintf(("netconnps3: cellSysutilUnregisterCallback() in NetConnShutdown failed: err=%s\n", DirtyErrGetName(iResult)));
        }

        if ((iResult = cellNetCtlNetStartDialogAbortAsync()) != 0)
        {
            NetPrintf(("netconnps3: cellNetCtlNetStartDialogAbortAsync() in NetConnShutdown failed: err=%s\n", DirtyErrGetName(iResult)));
        }

        if ((iResult = cellNetCtlNetStartDialogUnloadAsync(&NetStartResult)) != 0)
        {
            NetPrintf(("netconnps3: cellNetCtlNetStartDialogUnloadAsync() in NetConnShutdown failed: err=%s\n", DirtyErrGetName(iResult)));
        }
        _NetConn_pRef->iTSafeStrtDlgStatus = -1;
        _NetConn_pRef->iTSafeEndsDlgStatus = -1;
    }
    NetCritLeave(&_NetConn_pRef->callbackCrit);
...
/*F********************************************************************************/
/*!
    \Function NetConnNpMgrFuncSet

    \Description
        Tells netconn to use the supplied functions to register/unregister NP mgr event
        callback instead of the NP mgr api

    \Input *pNpMgrCbReg     - registration function to use in place of sceNpManagerRegisterCallback()
    \Input *pNpMgrCbUnreg   - registration function to use in place of sceNpManagerUnregisterCallback()

    \Version 04/1/2014 (mclouatre)
*/
/********************************************************************************F*/
void NetConnNpMgrFuncSet(NetConnNpMgrCbRegT *pNpMgrCbReg, NetConnNpMgrCbUnregT *pNpMgrCbUnreg)
{
    NetConnRefT *pRef = _NetConn_pRef;

    NetPrintf(("netconnps3: NP Mgr registration/unregistration overriden\n"));

    pRef->pNpMgrCbReg = pNpMgrCbReg;
    pRef->pNpMgrCbUnreg = pNpMgrCbUnreg;
}
