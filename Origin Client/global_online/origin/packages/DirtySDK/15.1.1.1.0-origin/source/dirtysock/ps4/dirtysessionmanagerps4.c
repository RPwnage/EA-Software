...
#define DIRTY_SESSION_WEBAPI_GROUP_SESSION              "sessionInvitation"
...
#define DIRTY_SESSION_WEBAPI_PATH_SESSION_GET_INVITE    DIRTY_SESSION_API_VERSION"/users/%s/invitations/%s?fields=@default"     // {0}=onlineid {1}=invitationId
#define DIRTY_SESSION_WEBAPI_PATH_SESSION_USED_INVITE   DIRTY_SESSION_API_VERSION"/users/%s/invitations/%s"                     // {0}=onlineid {1}=invitationId
#define DIRTY_SESSION_WEBAPI_PATH_SESSION_INVITATIONS   DIRTY_SESSION_API_VERSION"/users/%s/invitations?fields=@default,sessionId,expired,fromUser"  // {0}=onlineid
#define DIRTY_SESSION_WEBAPI_PATH_SESSION_MEMBER_OF     DIRTY_SESSION_API_VERSION"/users/%s/sessions"             // {0}=onlineid
...
#define DIRTY_SESSION_WEBAPI_PATH_SESSION_REMOVE_MEMBER DIRTY_SESSION_API_VERSION"/sessions/%s/members/%s"        // {0}=sessionid {1}=onlineid
...
#define DIRTY_SESSION_MAX_INVITATION_MESSAGE_LENGTH (512)                      //max lenght of invitation message from user
...
typedef struct DirtySessionRecievedInvitationT
{
    ...
    char strFromUser[DIRTY_SESSION_PLAYER_NAME_LENGTH];
    //extended meta data
...
{
    char strMessage[DIRTY_SESSION_MAX_INVITATION_MESSAGE_LENGTH];
    SceNpOnlineId aNpOnlineIds[DIRTY_SESSION_MAX_INVITATION_MEMBER_LIST_LENGTH];
    int32_t iMaxUsers;
    bool bEditList;
...
struct DirtySessionManagerRefT
{
    ...
    SceNpOnlineId           lastAcceptOnlineId;                     //!< Last user that accepted the invite
...
static void _DirtySessionManagerOnSystemEvent(void *pUserData, const SceSystemServiceEvent *pSystemServiceEvent)
{
    ...
    {
        if ((pSystemServiceEvent->eventType == SCE_SYSTEM_SERVICE_EVENT_SESSION_INVITATION) && ((pSystemServiceEvent->data.param) != NULL))
        {
            ...
            pRef->lastAcceptOnlineId = pSessionParam->onlineId;
...
*/
/*************************************************************************************F*/
static void _DirtySessionManagerOnWebApiSessionEvent(int32_t userCtxId, int32_t callbackId, const char *pNpServiceName, SceNpServiceLabel npServiceLabel, const SceNpPeerAddress *pTo, const SceNpPeerAddress *pFrom, const SceNpWebApiPushEventDataType *pDataType, const char *pData, size_t dataLen, void *pUserArg)
{
    if (pUserArg != NULL)
    {
        DirtySessionManagerRefT *pRef = (DirtySessionManagerRefT*)pUserArg;
        NetPrintf(("dirtysessionmanagerps4: [%p] detected invitation event.\n", pRef));
        ...
        for (int32_t iUserIndex = 0; iUserIndex < NETCONN_MAXLOCALUSERS; iUserIndex++)
        {
            ...
            //compare the onlineids to match
            if ((iRet = NetConnStatus('soid', iUserIndex, &onlineId, sizeof(onlineId))) >= 0) 
            {
                ...
                if((ds_strnicmp(onlineId.data, pTo->onlineId.data, SCE_NP_ONLINEID_MAX_LENGTH + 1) == 0))
                {
                    NetPrintf(("dirtysessionmanagerps4: [%p] Invite list for player %d:%s, has changed.\n", pRef, iUserIndex, onlineId.data));
...
{
    SceInvitationDialogParam        dialogParam;
    ...
    if (pRef->eSessionStatus == DIRTY_SESSION_ACTIVE)
    {
        if ((iRet =_DirtySessionManagerCanShowDialog(pRef)) >= 0)
        {
            ...
            memset(&dataParam, 0, sizeof(SceInvitationDialogDataParam));
            ...
            if (pRef->InvitationInfo.bEditList)
            {
                ...
                dataParam.SendInfo.addressParam.addressInfo.UserSelectEnableAddress.onlineIdsMaxCount = pRef->InvitationInfo.iMaxUsers;
            }
            else
            {
                //count the number of npids that have been set
                int32_t iNpIdCount = 0;
                for (int32_t i = 0; i < DIRTY_SESSION_MAX_INVITATION_MEMBER_LIST_LENGTH; i++)
                {
                    //special handling since SceNpOnlineId's .data field is not always terminated within its char array, but instead in the next .term character field
                    if ((ds_strnicmp(pRef->InvitationInfo.aNpOnlineIds[i].data, "", SCE_NP_ONLINEID_MAX_LENGTH + 1) == 0))
                    {
                        break;
                    }   
                    iNpIdCount++;
                ...
                dataParam.SendInfo.addressParam.addressInfo.UserSelectDisableAddress.onlineIds = pRef->InvitationInfo.aNpOnlineIds;
                dataParam.SendInfo.addressParam.addressInfo.UserSelectDisableAddress.onlineIdsCount = iNpIdCount;
            }
    

            // open dialog to send invite
            sceInvitationDialogParamInit( &dialogParam );
            ...
            if ((iRet = sceInvitationDialogOpen( &dialogParam )) == SCE_OK)
            {
...
static void _DirtySessionManagerCleanupSession(DirtySessionManagerRefT *pRef)
{
    ...
    memset(&pRef->ActiveSession, 0, sizeof(pRef->ActiveSession));
...
{
    NetPrintf(("dirtysessionmanagerps4: [%p] _DirtySessionManagerUserInvalidated a user that was expected to be part of the session is no longer online %s.\n", pRef, pRef->ActiveSession.strSessionID));
    pRef->aSessionUsers[iUserIndex].bSessionMember = FALSE;
    if (_DirtySessionManagerCountSessionMembers(pRef) == 0)
    {
        NetPrintf(("dirtysessionmanagerps4: [%p] _DirtySessionManagerUserInvalidated, the last user has left session %s, cleaning up.\n", pRef, pRef->ActiveSession.strSessionID));
...
static int32_t _DirtySessionManagerSetupCreateSession(DirtySessionManagerRefT *pRef, int32_t iUserIndex, char *pApiGroup, char *pApiPath, int32_t iStrSize, SceNpWebApiContentParameter *pContentParameter, SceNpWebApiHttpMethod *pHttpMethod, char *pSendBuff, int32_t iBuffSize)
{
    ...
    memset(pSendBuff, 0, iBuffSize);
    memset(aTempJson, 0, iTempBuffSize);
...
static int32_t _DirtySessionManagerPostInvitation(DirtySessionManagerRefT *pRef, int32_t iUserIndex, char *pApiGroup, char *pApiPath, int32_t iStrSize, SceNpWebApiContentParameter *pContentParameter, SceNpWebApiHttpMethod *pHttpMethod, char *pSendBuff, int32_t iBuffSize)
{
    ...
    memset(aTempToList, 0, iSendListSize);
    memset(aTempJson, 0, iTempBuffSize);
    ...
    *pHttpMethod = SCE_NP_WEBAPI_HTTP_METHOD_POST;
    ...
    {
        if ((ds_strnicmp(pRef->InvitationInfo.aNpOnlineIds[index].data, "", SCE_NP_ONLINEID_MAX_LENGTH + 1) == 0))
        {
            break;
        }   

        bIsNotListEmpty = 1;
        char *strOnlineId = (char *)&(pRef->InvitationInfo.aNpOnlineIds[index]);
        ds_strnzcat(aTempToList, "\"", iSendListSize);
        ds_strnzcat(aTempToList, strOnlineId, iSendListSize);
        uWritten = ds_strnzcat(aTempToList, "\",\r\n    ", iSendListSize);
    }
...
static int32_t _DirtySessionManagerSetupGetInvites(DirtySessionManagerRefT *pRef, int32_t iUserIndex, char *pApiGroup, char *pApiPath, int32_t iStrSize, SceNpWebApiContentParameter *pContentParameter, SceNpWebApiHttpMethod *pHttpMethod, char *pSendBuff, int32_t iBuffSize)
{
    ...
    if ((iRet = NetConnStatus('soid', iUserIndex, &onlineId, sizeof(onlineId))) >= 0) 
    {
        ds_snzprintf(pApiPath, iStrSize, DIRTY_SESSION_WEBAPI_PATH_SESSION_INVITATIONS, onlineId.data);
        ds_snzprintf(pApiGroup, iStrSize, DIRTY_SESSION_WEBAPI_GROUP_SESSION);
        *pHttpMethod = SCE_NP_WEBAPI_HTTP_METHOD_GET;
    }
    else
    {
        NetPrintf(("dirtysessionmanagerps4: [%p] _DirtySessionManagerSetupGetInvites() failed, couldn't get online id.\n", pRef));
...
static int32_t _DirtySessionManagerSetupGetInviteMetadata(DirtySessionManagerRefT *pRef, int32_t iUserIndex, int32_t iInviteIndex, void ** pInviteId, char *pApiGroup, char *pApiPath, int32_t iStrSize, SceNpWebApiContentParameter *pContentParameter, SceNpWebApiHttpMethod *pHttpMethod, char *pSendBuff, int32_t iBuffSize)
{
    ...
    if ((iInviteIndex < pRef->aSessionUsers[iUserIndex].RecievedInvitations.iNumInvitations) && (iInviteIndex >= 0))
    {
        ...
        if ((iRet = NetConnStatus('soid', iUserIndex, &onlineId, sizeof(onlineId))) >= 0) 
        {
            ds_snzprintf(pApiPath, iStrSize, DIRTY_SESSION_WEBAPI_PATH_SESSION_GET_INVITE, onlineId.data, pRef->aSessionUsers[iUserIndex].RecievedInvitations.aInvitations[iInviteIndex].strInvitationID);
            ds_snzprintf(pApiGroup, iStrSize, DIRTY_SESSION_WEBAPI_GROUP_SESSION);
            *pHttpMethod = SCE_NP_WEBAPI_HTTP_METHOD_GET;
        ...
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] _DirtySessionManagerSetupGetInviteMetadata() failed, couldn't get online id.\n", pRef));
            _DirtySessionManagerUserInvalidated(pRef, iUserIndex);
        }
    }
    else
...
static int32_t _DirtySessionManagerSetupGetMemberOfSessions(DirtySessionManagerRefT *pRef, int32_t iUserIndex, char *pApiGroup, char *pApiPath, int32_t iStrSize, SceNpWebApiContentParameter *pContentParameter, SceNpWebApiHttpMethod *pHttpMethod, char *pSendBuff, int32_t iBuffSize)
{
    ...
    if ((iRet = NetConnStatus('soid', iUserIndex, &onlineId, sizeof(onlineId))) >= 0) 
    {
        ds_snzprintf(pApiPath, iStrSize, DIRTY_SESSION_WEBAPI_PATH_SESSION_MEMBER_OF, onlineId.data);
        ds_snzprintf(pApiGroup, iStrSize, DIRTY_SESSION_WEBAPI_GROUP_SESSION);
        *pHttpMethod = SCE_NP_WEBAPI_HTTP_METHOD_GET;
    }
    else
    {
        NetPrintf(("dirtysessionmanagerps4: [%p] _DirtySessionManagerSetupGetMemberOfSessions() failed, couldn't get online id.\n", pRef));
        _DirtySessionManagerUserInvalidated(pRef, iUserIndex);
    }
    return(iRet);
}
...
static int32_t _DirtySessionManagerSetupDeleteSession(DirtySessionManagerRefT *pRef, int32_t iUserIndex, char *pApiGroup, char *pApiPath, int32_t iStrSize, SceNpWebApiContentParameter *pContentParameter, SceNpWebApiHttpMethod *pHttpMethod, char *pSendBuff, int32_t iBuffSize)
{
    ...
    if ((iRet = NetConnStatus('soid', iUserIndex, &onlineId, sizeof(onlineId))) >= 0) 
    {
        ds_snzprintf(pApiPath, iStrSize, DIRTY_SESSION_WEBAPI_PATH_SESSION_REMOVE_MEMBER, pRef->ActiveSession.strSessionID, onlineId.data);
        ds_snzprintf(pApiGroup, iStrSize, DIRTY_SESSION_WEBAPI_GROUP_SESSION);
        *pHttpMethod = SCE_NP_WEBAPI_HTTP_METHOD_DELETE;
        ...
        memset(&(pRef->lastAcceptOnlineId), 0, sizeof(SceNpOnlineId));
    }
    else
    {
        NetPrintf(("dirtysessionmanagerps4: [%p] _DirtySessionManagerSetupDeleteSession() failed, couldn't get online id.\n", pRef));
        _DirtySessionManagerUserInvalidated(pRef, iUserIndex);
    }
    return(iRet);
}
...
static int32_t _DirtySessionManagerSetupUpdateInvite(DirtySessionManagerRefT *pRef, int32_t iUserIndex, int32_t iInviteIndex, void ** pInviteId, char *pApiGroup, char *pApiPath, int32_t iStrSize, SceNpWebApiContentParameter *pContentParameter, SceNpWebApiHttpMethod *pHttpMethod, char *pSendBuff, int32_t iBuffSize)
{
    ...
    if ((iInviteIndex < pRef->aSessionUsers[iUserIndex].RecievedInvitations.iNumInvitations) && (iInviteIndex >= 0))
    {
        ...
        {
            memset(pSendBuff, 0, iBuffSize);
            ds_snzprintf(pApiPath, iStrSize, DIRTY_SESSION_WEBAPI_PATH_SESSION_USED_INVITE, onlineId.data, pRef->aSessionUsers[iUserIndex].RecievedInvitations.aInvitations[iInviteIndex].strInvitationID);
            ds_snzprintf(pApiGroup, iStrSize, DIRTY_SESSION_WEBAPI_GROUP_SESSION);
            *pHttpMethod = SCE_NP_WEBAPI_HTTP_METHOD_PUT;
            pContentParameter->pContentType = "application/json; charset=utf-8";
        ...
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] _DirtySessionManagerSetupUpdateInvite() failed, couldn't get online id.\n", pRef));
            _DirtySessionManagerUserInvalidated(pRef, iUserIndex);
        }
    }
    else
...
static int32_t _DirtySessionManagerGetInvitesResult(DirtySessionManagerRefT *pRef, int32_t iUserIndex, int32_t iStatus, const char *pReadBuff, int32_t iBuffSize)
{
    ...
    if (iStatus == 200)
    {
        ...
        char strSessionId[DIRTY_SESSION_ID_LENGTH];
        char strFrom[DIRTY_SESSION_PLAYER_NAME_LENGTH];
        ...
        memset(&pRef->aSessionUsers[iUserIndex].RecievedInvitations, 0, sizeof(pRef->aSessionUsers[iUserIndex].RecievedInvitations));
        ...
        do
        {
            ...
            pCurrent = JsonFind2(aJsonParse, pInviteList, ".fromUser.onlineId", iIncommingListIndex);
            if (pCurrent != NULL)
            {
            ...
            if((bUsedFlag == FALSE) && (bExpired == FALSE))
            {
                ...
                ds_strnzcpy(pRef->aSessionUsers[iUserIndex].RecievedInvitations.aInvitations[pRef->aSessionUsers[iUserIndex].RecievedInvitations.iNumInvitations].strFromUser, strFrom, sizeof(pRef->aSessionUsers[iUserIndex].RecievedInvitations.aInvitations[pRef->aSessionUsers[iUserIndex].RecievedInvitations.iNumInvitations].strFromUser));
...
static int32_t _DirtySessionManagerDeleteSessionResult(DirtySessionManagerRefT *pRef, int32_t iUserIndex, int32_t iStatus, const char *pReadBuff, int32_t iBuffSize)
{
    ...
    if (_DirtySessionManagerCountSessionMembers(pRef) == 0)
    {
        NetPrintf(("dirtysessionmanagerps4: [%p] _DirtySessionManagerDeleteSessionResult, the last user has left session %s, cleaning up.\n", pRef, pRef->ActiveSession.strSessionID));
        _DirtySessionManagerCleanupSession(pRef);
    }
...
/*!
    ...
    \Description
        Encode the pending sessions binary header and data and write it to pBuff if there is sufficiet space
...
void DirtySessionManagerUpdate(DirtySessionManagerRefT *pRef)
{
    ...
    if(pRef->ResponseData.iControlCode != 0)
    {
        ...
        memset(&pRef->ResponseData, 0, sizeof(pRef->ResponseData));
    }
    NetCritLeave(&pRef->crit);
    ...
    if (pRef->bInvDialog)
    {
        ...
        if (status == SCE_COMMON_DIALOG_STATUS_FINISHED)
        {
            ...
            SceInvitationDialogOnlineIdList sentOnlineIdList;
            memset(&sentOnlineIdList, 0, sizeof(SceInvitationDialogOnlineIdList));
            memset(&dialogResult, 0, sizeof(SceInvitationDialogResult) );
            dialogResult.sentOnlineIds = &sentOnlineIdList;

            if ((iRet = sceInvitationDialogGetResult(&dialogResult)) == SCE_OK)
            {    
                NetPrintf(("dirtysessionmanagerps4: [%p] sceInvitationDialogGetResult result = %s, errorCode = %d, sendOnlineIdCount = %d\n", pRef, DirtyErrGetName(dialogResult.result), dialogResult.errorCode, dialogResult.sentOnlineIds->count));
            
                if (dialogResult.sentOnlineIds->count > 0)
                {
                    for (uint32_t i = 0; i < dialogResult.sentOnlineIds->count; i++)
                    {
                        NetPrintf(("dirtysessionmanagerps4: [%p] Invite OnlineId = [%s]\n", pRef, dialogResult.sentOnlineIds->onlineId[i].data));
                    }
                }                
            }
            else
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'ianp' - add a user name to list of npids to invite (only usable when not editable); via pValue
...
int32_t DirtySessionManagerControl(DirtySessionManagerRefT *pRef, int32_t iControl, int32_t iValue, int32_t iValue2, const void *pValue)
{
    ...
    {
        int32_t i;
        for (i = 0; i < DIRTY_SESSION_MAX_INVITATION_MEMBER_LIST_LENGTH; i++)
        {
            //if we found the name the user is attempting to add, then ignore the request
            //special handling since SceNpOnlineId's .data field is not always terminated within its char array, but instead in the next .term character field
            if ((ds_strnicmp(pRef->InvitationInfo.aNpOnlineIds[i].data, (const char *)pValue, SCE_NP_ONLINEID_MAX_LENGTH + 1) == 0))
            {
                iRet = 0;
                break;
            }
            ...
            if ((ds_strnicmp(pRef->InvitationInfo.aNpOnlineIds[i].data, "", SCE_NP_ONLINEID_MAX_LENGTH + 1) == 0))
            {
                iRet = 1;
                //special handling since SceNpOnlineId's .data field is not always terminated within its char array, but instead in the next .term character field
                memset(&pRef->InvitationInfo.aNpOnlineIds[i], 0, sizeof(SceNpOnlineId));
                ds_strnzcpy(pRef->InvitationInfo.aNpOnlineIds[i].data, (const char *)pValue, SCE_NP_ONLINEID_MAX_LENGTH + 1);
                break;
            }
        }
        if (i == DIRTY_SESSION_MAX_INVITATION_MEMBER_LIST_LENGTH)
        {
    ...
    {    
        memset(pRef->InvitationInfo.aNpOnlineIds, 0, sizeof(pRef->InvitationInfo.aNpOnlineIds));
        iRet = 1;
    }
    ...
    //'imsg' - set the sessin invitation message
    ...
    else if (iControl == 'lock')
    {
        pRef->PendingSession.bSessionLock = iValue ? TRUE : FALSE;
        iRet =_DirtySessionManagerEnqueueControlCode(pRef, iControl, 0, 0, NULL); 
    }
    //'osid' - open session invitation dialog (using data from 'imus', 'imsg', 'iued')    
    else if (iControl == 'osid')
    {
        iRet = _DirtySessionManagerShowInviteDialog(pRef, iValue);
    }
    //'osrd' - open session receive dialog
    else if (iControl == 'osrd')
    {
        iRet = _DirtySessionManagerShowReceiveDialog(pRef, iValue);
    }
    ...
    else if (iControl == 'sdel')
    {
        ...
        for (int32_t iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; iIndex++)
        {
            if (pRef->aSessionUsers[iIndex].bSessionMember)
            {
                ...
                {
                    NetPrintf(("dirtysessionmanagerps4: [%p] 'sdel' Warning failed to remove user at index %i", iIndex));
                }
            }
        }
        iRet = 1;
    ...
    {
        iRet = _DirtySessionManagerEnqueueControlCode(pRef, 'sind', 0, 0, NULL);
    }
    //'sidt' - call sceInvitationDialogTerminate, and relinquish responsibility for calling 'sidt'
    ...
    else if (iControl == 'sjoi')
    {
        ...
        if (iRet != -1)
        {
            ...
            {
                NetPrintf(("dirtysessionmanagerps4: [%p] Invalid User Index. Skipping Join", pRef));
                iRet = -2;
            }
        }
    }
    ...
    {
        iRet = _DirtySessionManagerSetSessionString(pRef, (const char *)pValue, iValue2, TRUE); 
    }
    //'spri' - set the pending session's privacy setting; iValue2 == 1 private (default public) 
    ...
    {
        iRet = _DirtySessionManagerSetSessionString(pRef, (const char *)pValue, iValue2, FALSE); 
    }
    ...
    else if (iControl == 'surm')
    {
        ...
        if ((iValue >= 0) && (iValue < NETCONN_MAXLOCALUSERS))
        {
            ...
            {
                NetPrintf(("dirtysessionmanagerps4: [%p] Skip removing user from session because %d is not a session member.\n", pRef, iValue));
            }
        }
        else
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] Skip removing user from session due to invalid user index.\n", pRef));
            iRet = -1;
        }
    }
    //'uimg' - queue updating the active image data from the pending session
...
int32_t DirtySessionManagerStatus2(DirtySessionManagerRefT *pRef, int32_t iSelect, int32_t iValue, int32_t iValue2, int32_t iValue3, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'giex')
    {
        ...
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] 'giex' invalid user index. \n", pRef));
        }
        return (iRet);
    }
    if (iSelect == 'ginv')
    {
        ...
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] 'ginv' invalid user index. \n", pRef));
        }
        return (iRet);
    }
    if (iSelect == 'gimd')
    {
        ...
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] 'gimd' invalid user index. \n", pRef));
        }
        return (iRet);
    }
    if (iSelect == 'gims')
    {
        ...
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] 'gims' invalid user index. \n", pRef));
        }
        return (iRet);
    }
    if (iSelect == 'giii')
    {
        ...
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] 'giii' invalid user index. \n", pRef));
        }
        return (iRet);
    }
    if (iSelect == 'gisi')
    {
        ...
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] 'gisi' invalid user index. \n", pRef));
        }
        return (iRet);
    }
    if (iSelect == 'giud')
    {
        ...
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] 'giud' invalid user index. \n", pRef));
        }
        return (iRet);
    }
    if (iSelect == 'giun')
    {
        if ((iValue >= 0) && (iValue < NETCONN_MAXLOCALUSERS))
        {
            ...
            else
            {
                ...
                ds_strnzcpy(npid.handle.data, pRef->aSessionUsers[iValue].RecievedInvitations.aInvitations[iValue2].strFromUser, SCE_NP_ONLINEID_MAX_LENGTH + 1);
                DirtyUserFromNativeUser(&dirtyUser, &npid);
        ...
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] 'giun' invalid user index. \n", pRef));
        }
        return (iRet);
    }
    if (iSelect == 'giuf')
    {
        ...
        {
            NetPrintf(("dirtysessionmanagerps4: [%p] 'giuf' invalid user index. \n", pRef));
        }
        return (iRet);
    }
...
int32_t DirtySessionManagerStatus(DirtySessionManagerRefT *pRef, int32_t iSelect, void *pBuf, int32_t iBufSize)
{
    ...
    //'gpda' - get the pending session's binary data size 
    ...
    //'gdat' - get the active session's binary data size 
    //returns -1 error, 0 in progress, > 0 size; pValue if large enough will have the data written to it
    ...
    else if (iSelect == 'laid')
    {
        ...
        {
            SceNpId npId;
            npId.handle = pRef->lastAcceptOnlineId;
            DirtyUserFromNativeUser((DirtyUserT *)pBuf, &npId);
        }
        else
        {
            iRet = -1;
