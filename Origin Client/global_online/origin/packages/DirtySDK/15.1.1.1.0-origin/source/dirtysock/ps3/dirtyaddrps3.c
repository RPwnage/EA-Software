...
/*!
    ...
    \Version 09/01/2006 (jbrookes) First Version
*/
/********************************************************************************H*/
...
/*!
    ...
    \Description
        A PSN DirtyAddrT contains the internal and external addresses, as well as
        the NpID (optional). This function can be used to extract the external
        IP address from the DirtyAddr
...
/*!
    ...
    \Notes
        A PSN DirtyAddrT contains the internal and external addresses, as well as
        the NpID (optional). This function uses a binary-encoded host-order
        address to fill a DirtyAddr struct with:
            * NP ID - empty
            * internal address - 0
            * external address - input value
...
{
    uint32_t uExtAddr;
    ds_memcpy(&uExtAddr, pInput, sizeof(uExtAddr));
...
/*!
    ...
    \Notes
        A PSN DirtyAddrT contains the internal and external addresses, as well as
        the NpID (optional). This function fills the DirtyAddr struct with:
            * NP ID - empty
            * internal address - local ip address obtained from NetConn
            * external address - 0
...
uint32_t DirtyAddrGetLocalAddr(DirtyAddrT *pAddr)
{
    ...
    DirtyAddrSetInfoPS3(pAddr, NULL, uLocalAddr, 0);
    return(TRUE);
}
...
/*!
    ...
    \Input *pId         - NpId to encode (can be NULL)
...
void DirtyAddrSetInfoPS3(DirtyAddrT *pDirtyAddr, const SceNpId *pId, uint32_t uIntAddr, uint32_t uExtAddr)
{
    ...
    // set NpOnlineId
    ds_strnzcpy(pAddr, pId->handle.data, sizeof(*pDirtyAddr));
    iIndex = strlen(pAddr);
    // set local address
