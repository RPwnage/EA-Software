...
{
    static uint64_t _NetLib_uTimeBaseFrequency = 0;
    uint64_t uCurTime;

    if (_NetLib_uTimeBaseFrequency == 0)
    {
        _NetLib_uTimeBaseFrequency = sys_time_get_timebase_frequency();
    }

    SYS_TIMEBASE_GET(uCurTime);
    return((uCurTime*uScale)/_NetLib_uTimeBaseFrequency);
}
...
void NetLibCreate(int32_t iThreadPrio, int32_t iThreadStackSize, int32_t iThreadCpuAffinity)
{
    ...
    if (sys_ppu_thread_create(&g_idlethread, _NetLibThread, 0, iThreadPrio, (iThreadStackSize?iThreadStackSize:(16*1024)), 0, "NetLib Idle") != CELL_OK)
    {
...
int32_t NetPrintfCode(const char *pFormat, ...)
{
    ...
    iTimeStampLen = NetTimeStamp(strText, sizeof(strText));
...
int32_t NetCritTry(NetCritT *pCrit)
{
    ...
    // try and gain access
    if ((iResult = sys_mutex_trylock(pPriv->uMutexId)) != CELL_OK)
    {
        if (iResult != EBUSY)
        {
...
void NetCritEnter(NetCritT *pCrit)
{
    ...
    if (!NetCritTry(pCrit))
    {
        ...
        if ((iResult = sys_mutex_lock(pPriv->uMutexId, 5*1000*1000)) != CELL_OK)
        {
            if (iResult == ETIMEDOUT)
            {
                ...
                if ((iResult = sys_mutex_lock(pPriv->uMutexId, 0)) != CELL_OK)
                {
...
void NetCritLeave(NetCritT *pCrit)
{
    ...
    if ((iResult = sys_mutex_unlock(pPriv->uMutexId)) != CELL_OK)
    {
