...
static LARGE_INTEGER _NetLib_lFreqUsec;
...
void NetLibCreate(int32_t iThreadPrio, int32_t iThreadStackSize, int32_t iThreadCpuAffinity)
{
   ...
   g_workItem = ThreadPool::RunAsync(refWorkItemHdlr, workItemPriority, WorkItemOptions::TimeSliced);
    ...
    {
        memcpy(&_NetLib_lFreqUsec, &_NetLib_lFreq, sizeof(_NetLib_lFreqUsec));
...
int32_t NetPrintfCode(const char *pFormat, ...)
{
    ...
    {
        ds_strnzcat(strText + iTimeStampLen, va_arg(pFmtArgs, const char *), (signed)sizeof(strText) - iTimeStampLen);
    }
    else
    {
        ds_vsnprintf(strText + iTimeStampLen, (signed)sizeof(strText) - iTimeStampLen, pFormat, pFmtArgs);
    }
    va_end(pFmtArgs);
...
uint64_t NetTickUsec(void)
{
    LARGE_INTEGER lCount;
    QueryPerformanceCounter(&lCount);
    return((uint64_t)((lCount.QuadPart*1000000)/_NetLib_lFreqUsec.QuadPart));
}
