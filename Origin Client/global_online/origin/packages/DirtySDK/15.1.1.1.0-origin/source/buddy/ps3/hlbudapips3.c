...
/*!
    ...
    \Description
        ...
        to Friends for PS3. This is LARGELY stubbed out, as PS3 does 
...
#define HLB_INVITE_BODY "This is the PS3 HLB Game Invite Message"
...
//! presence data for PS3 EA Titles
...
static void _HLBPrintfCode(HLBApiRefT *pHLBApi, const char *pFormat, ...)
{
    ...
    ds_strnzcpy(strText, "hlbudapips3: ", sizeof(strText));
    ...
    // add a line feed at the end of the debug line
    ds_strnzcat(strText, "\n", sizeof(strText));
...
/*F**************************************************************************************/
/*!
    \Function _HLBPlayerHistoryThread

    \Description
        Thread to add player(s) to player history list

    \Input pArg     - pointer to input data

    \Version 09/01/2006 (jbrookes)
*/
/**************************************************************************************F*/
static void _HLBPlayerHistoryThread(uint64_t pArg)
{
    HLBudAddPlyrHistDataT *pData = (HLBudAddPlyrHistDataT *)(uintptr_t)pArg;
    int32_t iId, iResult;
    
    NetPrintf(("hlbudapips3: addplayerhistory thread running (%d names)\n", pData->iNumIds));

    // add NP id to player history
    for (iId = 0; iId < pData->iNumIds; iId++)
    {
        #if DIRTYCODE_LOGGING
        uint32_t uStart = NetTick();
        #endif
        if ((iResult = sceNpBasicAddPlayersHistory(&pData->NpIds[iId], NULL)) >= 0)
        {
            NetPrintf(("hlbudapips3: added player %s to player history (%dms)\n", pData->NpIds[iId].handle.data, NetTick() - uStart));
        }
        else
        {
            NetPrintf(("hlbudapips3: sceNpBasicAddPlayersHistory() failed (err=%s)\n", DirtyErrGetName(iResult)));
        }
    }

    // release allocated id
    pData->pRef->pBuddyFree(pData, HLBUDAPI_MEMID, pData->pRef->iMemGroup, pData->pRef->pMemGroupUserData);

    // terminate ourselves
    NetPrintf(("hlbudapips3: player history thread exiting\n"));
...
static int32_t _HLBApiInit(HLBApiRefT *pHLBApi)
{
    ...
    DirtyMemGroupEnter(pHLBApi->iMemGroup, pHLBApi->pMemGroupUserData);
...
/*!
    ...
    \Description
        Sends presence data to the PS3 libraries.
...
static HLBBudT *_HLBGetBuddyRec(HLBApiRefT *pHLBApi, const SceNpId *pNpId, int32_t bAdd)
{
    ...
    if (pRec == NULL)
    {
        ...
        if (bAdd)
        {
            ...
            ds_memcpy_s(&pBuddy->UserId, sizeof(pBuddy->UserId), pNpId, sizeof(SceNpId));
...
static HLBBudT *_HLBUpdateBuddy(HLBApiRefT *pHLBApi, const SceNpId *pBuddyData, int32_t bOnline, int32_t bBlocked, int8_t *pPresenceData)
{
    ...
    if (pBuddy != NULL)
    {
        ...
        else if (bOnline)
        {
            ...
            ds_memcpy(&pBuddy->Presence, pPresenceData, sizeof(pBuddy->Presence));
        }
        else
        {
...
/*!
    ...
    \Description
        Removes events from the PS3 event queue.
...
*/
/********************************************************************************F*/
static void _HLBReceiveMessage(HLBApiRefT *pHLBApi, SceNpUserInfo *pBuddyData, int8_t *pMessageBuf, int32_t iDataSize)
{
    HLBBudT *pBuddy = NULL;

    // extract name and look for matching record
    pBuddy = _HLBGetBuddyRec(pHLBApi, &pBuddyData->userId, TRUE);
    ...
    if (pBuddy != NULL)
    {
        ...
        pNewMessage->BuddyMsg.pData = (char *)pNewMessage->pData;
        ds_memcpy(pNewMessage->pData, pMessageBuf, iDataSize);
...
*/
/********************************************************************************F*/
static void _HLBSignalMsgAttachmentReceived(HLBApiRefT *pHLBApi, SceNpUserInfo *pBuddyData)
{
    HLBBudT *pBuddy = NULL;
...
*/
/********************************************************************************F*/
static int32_t _HLBRecvMsgAttachmentResult(HLBApiRefT *pHLBApi, SceNpUserInfo *pBuddyData, SceNpBasicAttachmentData *ad)
{      
    int32_t iNpStatus = 0;
    ...
    if (pBuddy != NULL)
    {
        ...
        pData = (char *) DirtyMemAlloc(iDataSize, HLBUDAPI_MEMID, pHLBApi->iMemGroup, pHLBApi->pMemGroupUserData);
...
static void _FriendApiNotificationCb(FriendApiRefT *pFriendApiRef, FriendEventDataT *pEventData, void *pNotifyCbUserData)
{
    ...
    SceNpUserInfo *pUserInfo = NULL;

    switch(pEventData->eventType)
    {
        case FRIENDAPI_EVENT_UNKNOWN:
            if (*((int *)pEventData->pRawData1) == SCE_NP_BASIC_EVENT_INCOMING_ATTACHMENT)
            {
                pUserInfo = (SceNpUserInfo *)(pEventData->eventTypeData.Unknown.pFriendData->pRawData1);

                _HLBSignalMsgAttachmentReceived(pHLBApi, pUserInfo);
            }
            else if (*((int *)pEventData->pRawData1) == SCE_NP_BASIC_EVENT_RECV_ATTACHMENT_RESULT)
            {
                pUserInfo = (SceNpUserInfo *)(pEventData->eventTypeData.Unknown.pFriendData->pRawData1);

                _HLBRecvMsgAttachmentResult(pHLBApi, pUserInfo, (SceNpBasicAttachmentData *)pEventData->eventTypeData.Unknown.pRawData2);
            }
            else if (pHLBApi->pSceNpBasicEventCallback)
            {
                pHLBApi->pSceNpBasicEventCallback(
                    *(int32_t *)pEventData->pRawData1,              // event type
                    *(int32_t *)pEventData->pRawData2,              // error code
                    0,                                              // request id (unused)
                    pHLBApi->pSceNpBasicEventCallbackContext);      // user data
            }
            break;

        case FRIENDAPI_EVENT_FRIEND_UPDATED:
            pUserInfo = (SceNpUserInfo *)(pEventData->eventTypeData.FriendUpdated.pFriendData->pRawData1);

            if (pEventData->eventTypeData.FriendUpdated.bPresence == TRUE)
            {
                _HLBUpdateBuddy(pHLBApi, &pUserInfo->userId, TRUE, FALSE, pEventData->eventTypeData.FriendUpdated.pRawData2);
            }
            else
            {
                _HLBUpdateBuddy(pHLBApi, &pUserInfo->userId, FALSE, FALSE, NULL);
            }
            break;

        case FRIENDAPI_EVENT_FRIEND_REMOVED:
            pUserInfo = (SceNpUserInfo *)(pEventData->eventTypeData.FriendRemoved.pFriendData->pRawData1);

            _HLBRemoveBuddy(pHLBApi, &pUserInfo->userId);
            break;

        case FRIENDAPI_EVENT_BLOCKLIST_UPDATED:
            NetPrintf(("hlbudapips3: it is time to update the blocked list\n"));
            pHLBApi->bGetBlockedList = TRUE;
            break;

        case FRIENDAPI_EVENT_MSG_RCVED:
            pUserInfo = (SceNpUserInfo *)(pEventData->eventTypeData.MsgRcved.pFriendData->pRawData1);

            _HLBReceiveMessage(pHLBApi, pUserInfo, pEventData->eventTypeData.MsgRcved.pRawData2, *(int *)(pEventData->eventTypeData.MsgRcved.pRawData1));
            break;

        default:
            NetPrintf(("hlbudapips3: invalid friendapi event type\n"));
    }
}
...
void HLBApiSetDebugFunction(HLBApiRefT *pHLBApi, void *pDebugData, void (*pDebugProc)(void *pData, const char *pText))
{
    ...
    pDebugProc(NULL, "hlbudapips3: testing debug proc\n");
...
/*!
    ...
    \Description
        Configure and initialize any data structures in the API. No-OP on PS3.
...
/*!
    ...
    \Description
        Disconnect from the PS3 friends library.
...
{      
    ...
    if ((iNpStatus = sceNpBasicRecvMessageAttachment(SYS_MEMORY_CONTAINER_ID_INVALID)) < 0)
    {
        iStatus = HLB_ERROR_UNKNOWN;
        HLBPrintf((pHLBApi, "sceNpBasicRecvMessageAttachment failed: %s", DirtyErrGetName(iNpStatus)));
    }
...
/*!
    ...
    \Notes
        ...
        protocol on top of the PS3 messages.
...
/*!
    ...
    \Description
        Register callback for receiving cross-game invites. Not implemented on PS3.
...
/*!
    ...
    \Description
        Return XUID of buddy - not supported on PS3.
...
{
    return((uint64_t)HLB_ERROR_UNSUPPORTED);
}
...
/*!
    ...
    \Description
        Returns SceNpId of buddy –PS3 only! 
...
{
    ds_memcpy_s(pSceNpId, sizeof(*pSceNpId), &pBuddy->UserId, sizeof(pBuddy->UserId));
}

/*F*************************************************************************************************/
/*!
    \Function HLBBudSetSceNpBasicEventCallback

    \Description
        PS3 ONLY -- Set the callback that HlBudApi will forward sceNpBasicRegisterHandler() events to.

    \Input *pHLBApi                     - pointer to HLBApi module reference
    \Input *pSceNpBasicEventCallback    - event callback function pointer
    \Input *pCallbackContext            - pointer to user data

    \Notes
         Allows customers to implement additional PlayStation Network features.

    \Version 05/04/2010 (alanstone)
*/
/*************************************************************************************************F*/
void HLBBudSetSceNpBasicEventCallback(HLBApiRefT *pHLBApi, HLBSceNpBasicEventCallbackT *pSceNpBasicEventCallback, void *pCallbackContext)
{
    ...
    pHLBApi->pSceNpBasicEventCallback = pSceNpBasicEventCallback;
    pHLBApi->pSceNpBasicEventCallbackContext = pCallbackContext;
}
...
/*!
    ...
    \Description
        Determine if buddy is temporary. Not supported on PS3.
...
/*!
    ...
    \Description
        Has this buddy invited me to be his buddy? Not supported on PS3.
    ...
    \Notes
        TODO: can we implement this on PS3? I think it has to be handled in the OSD.
...
/*!
    ...
    \Description
        Have I invited this use to be my buddy? Not supported on PS3.
...
/*!
    ...
    \Description
        Is this buddy a temporary buddy? Not supported on PS3.
    ...
    \Notes
        TODO: we could implement something like this on PS3 using the presence data.
...
/*!
    ...
    \Description
        ...
        or added temporarily. PS3 has no temp buddies so everyone on the list that isn't
...
/*!
    ...
    \Output
        int32_t     - see HLBStatE enum; only HLB_STAT_DISC and HLB_STAT_CHAT are implemented for PS3
...
/*!
    ...
    \Description
        Indicate if buddy is online and logged into same product. Not implemented on PS3.
...
/*!
    ...
    \Description
        Indicates if buddy can be talked to. Not implemented on PS3
...
/*!
    ...
    \Description
        Join buddy in another game; not implmeneted on PS3.
...
/*!
    ...
    \Description
        Get message text from message. Not implemented on PS3.
...
{    
    ...
    {
        ds_memcpy(strText, pMsg->pData, iLen);
    }
}
...
int32_t HLBListGetBuddyCount (HLBApiRefT *pHLBApi)
{
    ...
    return(_HLBRosterCount(pHLBApi, (uint32_t)-1));
}
...
/*!
    ...
    \Description
        Get count of buddies that match. Not supported on PS3.
...
/*!
    ...
    \Description
        Add a buddy to the list of ignored buddies. Not implemented on PS3.
...
/*!
    ...
    \Description
        Remove a buddy from the list of ignored buddies. Not implemented on PS3.
...
/*!
    ...
    \Description
        Ensure user is no longer a buddy of mine. Not implemented on PS3.
...
/*!
    ...
    \Description
        Ask this user to be my buddy. Not implemented on PS3.
...
/*!
    ...
    \Description
        Respond to a buddy invitation. Not implemented on PS3.
...
/*!
    ...
    \Description
        Cancel any game invites I sent. Not implemented on PS3.
...
int32_t HLBListGameInviteBuddy(HLBApiRefT *pHLBApi, const char *strName,
                               ...
                               void *pContext)
{
    char *pSubject = HLB_GAME_INVITE;
    char *pBody = HLB_INVITE_BODY;
    uint8_t *pBuffer = (uint8_t *)HLB_INVITE_BODY;
    size_t iBufferSize = sizeof(HLB_INVITE_BODY); //Size of send buffer
    HLBBudT *pBuddy;
    int32_t iStatus = HLB_ERROR_NONE;
    int32_t iNpStatus = 0;

    pBuddy = HLBListGetBuddyByName(pHLBApi, strName);
    if (pBuddy == NULL)
    {
        HLBPrintf((pHLBApi, "Could not find buddy record for '%s'", strName));
        return(HLB_ERROR_BADPARM);
    }

    if ((iNpStatus = sceNpBasicSendMessageAttachment(&pBuddy->UserId, pSubject, pBody, pBuffer, iBufferSize, SYS_MEMORY_CONTAINER_ID_INVALID)) < 0)
    {
        iStatus = HLB_ERROR_UNKNOWN;
        HLBPrintf((pHLBApi, "sceNpBasicSendMessageAttachment failed: %s", DirtyErrGetName(iNpStatus)));
    }

    if (cb != NULL)
    {
        cb(pHLBApi, HLB_OP_SEND_MSG, iStatus, NULL);
    }
...
/*!
    ...
    \Description
        Temporarily flag a buddy as a temp or tourney buddy for convenience. Not implemented on PS3.
...
/*!
    ...
    \Description
        Remove a temporary buddy type flag from a buddy if set. Not implemented on PS3.
...
/*!
    ...
    \Description
        Delete a temporary buddy. Not implemented on PS3.
...
/*!
    ...
    \Description
        Do we have messages from any buddies? Not implemented on PS3.
...
/*!
    ...
    \Description
        Clear temp buddies without messages. Not implemented on PS3.
...
/*!
    ...
    \Description
        Add a buddy to the group to which I will send a common message. Not implemented on PS3.
...
/*!
    ...
    \Description
        Remove a buddy from the group to which I will send a common message. Not implemented on PS3.
...
/*!
    ...
    \Description
        Remove all users from the group. Not implemented on PS3.
...
/*!
    ...
    \Description
        Send a message to the group of buddies. Not implemented on PS3.
...
/*!
    ...
    \Description
        Set presence for other products, in appropriate language. Not implemented on PS3.
...
void HLBApiPresenceSame(HLBApiRefT *pHLBApi, int32_t iPresState, const char *strPresence)
{
    ...
    ds_memcpy(pHLBApi->BuddyData.Presence.strTitleData, strPresence, sizeof(pHLBApi->BuddyData.Presence.strTitleData));
    return;
}
...
/*!
    ...
    \Description
        Set extra presence data. No implemented on PS3.
...
/*!
    ...
    \Description
        Set presence to offline. Not implemented on PS3.
                           ...
                           const char *strTitleData, int32_t iWaitMsecs) 
{
    ...
    ds_memcpy(pHLBApi->BuddyData.Presence.strTitleData, strTitleData, sizeof(pHLBApi->BuddyData.Presence.strTitleData));
...
/*!
    ...
    \Description
        Update voice state and send presence data. Not implemented on PS3.
...
/*!
    ...
    \Description
        Send a chat message to a buddy in the list. Not implemented on PS3.
                           ...
                           HLBOpCallbackT *cb , void *pContext)
{
    ...
    HLBBudT *pBuddy = HLBListGetBuddyByName(pHLBApi, strName);
    ...
    if ((iNpStatus = sceNpBasicSendMessage(&pBuddy->UserId, strMessage, strlen(strMessage))) < 0)
    {
        iStatus = HLB_ERROR_UNKNOWN;
        HLBPrintf((pHLBApi, "sceNpBasicSendMessage failed: %s", DirtyErrGetName(iNpStatus)));
    }

    if (cb != NULL)
    {
        cb(pHLBApi, HLB_OP_SEND_MSG, iStatus, pContext);
    }
...
/*!
    ...
    \Description
        Turn email forwarding on or off. Not implemented on PS3.
...
/*!
    ...
    \Description
        Get email forwarding state. Not implemented on PS3.
...
/*!
    ...
    \Description
        Get state of cross game invites. Not implemented on PS3.
...
/*!
    ...
    \Description
        Set the enumerator index for XFriendsCreateEnumerator. Not implemented on PS3.
...
/*!
    ...
    \Description
        Get the enumerator index for XFriendsCreateEnumerator. Not implemented on PS3.
...
/*!
    ...
    \Description
        Convert a title id into a title string. Not implemented on PS3.
...
/*!
    ...
    \Description
        Get the title string for the currently running title. Not implemented on PS3.
...
/*!
    ...
    \Description
        Set the session id that is passed to the game invitation. Not implemented on PS3.
...
/*!
    ...
    \Description
        Get the SessionID for a particular user for game invites. Not implemented on PS3.
...
void HLBAddPlayersToHistory(HLBApiRefT *pHLBApi, DirtyAddrT *pPlyrAddrs[], int32_t iNumAddrs)
{
    ...
    int32_t iAddr, iDataSize;

    // calculate size of data
    iDataSize = sizeof(*pData) + (sizeof(pData->NpIds) * (iNumAddrs-1));

    // allocate and init NpId
    pData = pHLBApi->pBuddyAlloc(iDataSize, HLBUDAPI_MEMID, pHLBApi->iMemGroup, pHLBApi->pMemGroupUserData);
    memset(pData, 0, iDataSize);
    pData->pRef = pHLBApi;
    pData->iNumIds = iNumAddrs;

    // extract NpIds from dirtyaddrs
    for (iAddr = 0; iAddr < iNumAddrs; iAddr++)
    {
        DirtyAddrGetInfoPS3(pPlyrAddrs[iAddr], &pData->NpIds[iAddr], NULL, NULL);
    }

    // create thread to do the add
    sys_ppu_thread_create(&iThreadId, _HLBPlayerHistoryThread, (uintptr_t)pData, 10, 0x4000, 0, "HLBApiPlayerHistory");
    if (iThreadId <= 0)
    {
        NetPrintf(("hlbapips3: unable to create addplayerhistory thread (err=%s)\n", DirtyErrGetName((uint32_t)iThreadId)));
        pHLBApi->pBuddyFree(pData, HLBUDAPI_MEMID, pHLBApi->iMemGroup, pHLBApi->pMemGroupUserData);
    }
}
