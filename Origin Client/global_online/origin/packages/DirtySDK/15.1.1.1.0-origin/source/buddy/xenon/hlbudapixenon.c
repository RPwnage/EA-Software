...
struct HLBApiRefT
{
    ...
    int32_t iUserIndex;             //index of current user
...
/*F*************************************************************************************************/
/*!
    \Function   _HLBFriendsGetFriendByName

    \Description
        Search for friend with certain name, and return it if found

    \Input *iUserIndex      - user which is going to have his friends searched
    \Input *pName           - name of searched friend

    \Output XONLINE_FRIEND* - pointer to found friend, or NULL if not found
*/
/*************************************************************************************************F*/
static XONLINE_FRIEND *_HLBFriendsGetFriendByName(const int32_t iUserIndex, const char *pName)
{
    FriendApiRefT *pFriendApiRef;
    int32_t iFriendsCount;
    XONLINE_FRIEND aOnlineFriends[FRIENDAPI_MAX_FRIENDS];
    static XONLINE_FRIEND onlineFriend;
    XONLINE_FRIEND *pOnlineFriend = NULL;

    if (NetConnStatus('fref', 0, &pFriendApiRef, sizeof(pFriendApiRef)) == 0)
    {
        if (!FriendApiGetFriendsList(pFriendApiRef, iUserIndex, NULL, aOnlineFriends, &iFriendsCount))
        {
            int32_t iFriend;

            for (iFriend = 0; iFriend < iFriendsCount; iFriend++)
            {
                if (ds_stricmp(aOnlineFriends[iFriend].szGamertag, pName) == 0)
                {
                    ds_memcpy_s(&onlineFriend, sizeof(XONLINE_FRIEND), &aOnlineFriends[iFriend], sizeof(XONLINE_FRIEND));
                    pOnlineFriend = &onlineFriend;
                    break;
                }
            }
        }
    ...
    {
        NetPrintf(("hlbudapixenon: _HLBFriendsGetFriendByName() failed to get reference to friend module\n"));
    }

    return(pOnlineFriend);
}

/*F*************************************************************************************************/
/*!
    \Function   _HLBFriendsGetFriendByIndex

    \Description
        Return friend with a certain index if it exists

    \Input *iUserIndex      - user which is going to have his friends searched
    \Input *iFriendIndex    - friend index
    \Input *aOnlineFriends  - pointer to a container to store the friends list

    \Output XONLINE_FRIEND* - pointer to friend, or NULL if index is not valid
*/
/*************************************************************************************************F*/
static XONLINE_FRIEND * _HLBFriendsGetFriendByIndex(const int32_t iUserIndex, const int32_t iFriendIndex, XONLINE_FRIEND *aOnlineFriends)
{
    FriendApiRefT *pFriendApiRef;
    int32_t iFriendsCount;
    XONLINE_FRIEND *pOnlineFriend = 0;

    if (NetConnStatus('fref', 0, &pFriendApiRef, sizeof(pFriendApiRef)) == 0)
    {
        if (!FriendApiGetFriendsList(pFriendApiRef, iUserIndex, NULL, aOnlineFriends, &iFriendsCount))
        {
            if ((iFriendIndex >= 0) && (iFriendIndex < iFriendsCount))
            {
                pOnlineFriend = &aOnlineFriends[iFriendIndex];
            }
        }
    }
    else
    {
        NetPrintf(("hlbudapixenon: _HLBFriendsGetFriendByIndex() failed to get reference to friend module\n"));
    }

    return (pOnlineFriend);
}

static void _HLBFriendsNotifyCallback(FriendApiRefT *pFriendApiRef, FriendEventDataT *pEventData, void *pNotifyCbUserData)
{
    HLBApiRefT *pApiRef;

    if (pNotifyCbUserData == NULL)
    {
        return;
    }

    pApiRef = (HLBApiRefT*) pNotifyCbUserData;

    _reloadFriends (pApiRef);

    if (pEventData->eventType == FRIENDAPI_EVENT_FRIEND_UPDATED)
    {
        XONLINE_FRIEND *pXonlineFriend = (XONLINE_FRIEND *)pEventData->eventTypeData.FriendUpdated.pFriendData->pRawData1;

        HLBStatE eOldState;
        HLBStatE eNewState;
        HLBBudT *pBuddy = HLBListGetBuddyByName(pApiRef, pXonlineFriend->szGamertag);
        ...
        pBuddy = _updateFriendInRoster(pApiRef, pXonlineFriend);
        eNewState = HLBBudGetState(pBuddy);
...
HLBApiRefT *_HLBApiInit(HLBApiRefT *pApiRef)
{
    ...
    if (NetConnStatus('fref', 0, &pFriendApiRef, sizeof(pFriendApiRef)) < 0)
    {
        NetPrintf(("hlbudapixenon: _HLBApiInit() failed to get reference to friend module\n"));
        return (NULL);
    }

    if (pFriendApiRef == NULL)
    {
...
/*!
    ...
    \Output
        *HLBApiRefT                 - API reference pointer, to be used on subsequent calls.
*/
/*************************************************************************************************F*/
...
void HLBApiOverrideConstants(HLBApiRefT *pApiRef, int32_t bNoTextSupport, int32_t connect_timeout, int32_t op_timeout, int32_t max_perm_buddies, 
                                int32_t max_temp_buddies, int32_t max_msgs_allowed, int32_t bTalk_to_xbox, int32_t max_msg_len, int32_t bTrackSendMsgInOpState, int32_t iGameInviteAccept_timeout)
...
int32_t HLBApiConnect(HLBApiRefT  *pApiRef, const char * pServer, int32_t port, const  char *key, const char *pRsrc)
{
    ...
    if (NetConnStatus('fref', 0, &pFriendApiRef, sizeof(pFriendApiRef)) < 0)
    {
        NetPrintf(("hlbudapixenon: HLBApiConnect() failed to get reference to friend module\n"));
        return(HLB_ERROR_UNKNOWN);
    }

    if(FriendApiAddCallback(pFriendApiRef, _HLBFriendsNotifyCallback, pApiRef) < 0)
    {
        NetPrintf(("hlbudapixenon: failed to add callback\n"));
        return(HLB_ERROR_UNKNOWN);
    }
...
/*!
    ...
    \Output
        int32_t         - see HLBConnectState_Enum.
...
void  HLBApiDisconnect(HLBApiRefT *pApiRef)
{
    ...
    if (NetConnStatus('fref', 0, &pFriendApiRef, sizeof(pFriendApiRef)) < 0)
    {
        NetPrintf(("hlbudapixenon: HLBApiDisconnect() failed to get reference to friend module\n"));
        return;
    }

    if(FriendApiRemoveCallback(pFriendApiRef, _HLBFriendsNotifyCallback, pApiRef) < 0)
    {
        NetPrintf(("hlbudapixenon: failed to remove callback\n"));
        return;
    }
...
int32_t HLBApiSuspend(HLBApiRefT *pApiRef)
{
    ...
    if (NetConnStatus('fref', 0, &pFriendApiRef, sizeof(pFriendApiRef)) < 0)
    {
        NetPrintf(("hlbudapixenon: HLBApiSuspend() failed to get reference to friend module\n"));
        return HLB_ERROR_UNKNOWN;
    }
...
void * HLBApiResume(HLBApiRefT * pApiRef)
{
    ...
    if (NetConnStatus('fref', 0, &pFriendApiRef, sizeof(pFriendApiRef)) < 0)
    {
        NetPrintf(("hlbudapixenon: HLBApiResume() failed to get reference to friend module\n"));
        return(NULL);
    }
...
/*!
    ...
    \Output
        int32_t     - 1 if buddy is temporary and hence not persistent.
*/
/*************************************************************************************************F*/
...
/*!
    ...
    \Output
        int32_t         - if buddy is  ignored or blocked.
*/
/*************************************************************************************************F*/
...
/*!
    ...
    \Output
        int32_t         - 1 if buddy is a real buddy of mine.
*/
/*************************************************************************************************F*/
...
/*!
    ...
    \Output
        int32_t     - 1 if buddy is in group.
*/
/*************************************************************************************************F*/
...
/*!
    ...
    \Output
        int32_t     - 1 if buddy is in passive state, i.e. playing a game.
*/
/*************************************************************************************************F*/
...
/*!
    ...
    \Output
        int32_t     - 1 if buddy is online and available for chat.
*/
/*************************************************************************************************F*/
...
/*!
    ...
    \Output
        int32_t     - 1 if buddy is online and logged into same product.
*/
/*********************************************************************************************F*/
...
/*!
    ...
    \Output
        int32_t     -1 if buddy wont accept messages or invites.
*/
/*********************************************************************************************F*/
int32_t HLBBudIsNoReplyBud(HLBBudT *pBuddy)
{
...
/*!
    ...
    \Output
        int32_t     -1- if plugged in and can be talked to.   
*/
/*********************************************************************************************F*/
...
/*!
    ...
    \Input read         - if true, message will be marked as read.
...
/*!
    ...
    \Input *pBuddy       - pointer to the buddy.
    \Input bZapReadOnly  - if 1, will delete Read messages.
*/
/*********************************************************************************************F*/
...
HLBBudT* HLBListGetBuddyByIndex(HLBApiRefT *pApiRef, int32_t index)
{
    ...
    pFriend = _HLBFriendsGetFriendByIndex(pApiRef->iUserIndex, index, aOnlineFriends);
...
/*!
    ...
    \Output
        int32_t         - index to buddy; negative value ( HLB_ERROR_MISSING ) if not there.
*/
/*********************************************************************************************F*/
int32_t _HLBListGetIndexByName(HLBApiRefT *pApiRef, const char *name)
{
...
int32_t HLBListAnswerInvite(HLBApiRefT *pApiRef, const char * name , /*HLBInviteAction_Enum */ int32_t eInviteAction, HLBOpCallbackT *cb, void *pContext)
{
    return(HLB_ERROR_UNSUPPORTED);
}

//! Respond to an invite   NOT FOR XENON
int32_t HLBListAnswerGameInvite(HLBApiRefT *pApiRef, const char * name , /*HLBInviteAnswer_Enum */ int32_t eInviteAction, HLBOpCallbackT *cb, void * pContext)
{
    return(HLB_ERROR_UNSUPPORTED);
}
...
int32_t HLBListGameInviteBuddy( HLBApiRefT  *pApiRef, const char * name, const char * pGameInviteState, HLBOpCallbackT *cb, void * pContext)
{
    return(HLB_ERROR_UNSUPPORTED);
}
...
int32_t HLBListSendMsgToGroup(HLBApiRefT *pApiRef, /*HLBMsgType_Enum*/ int32_t msgType, const char * msg, int32_t bClearGroup, HLBOpCallbackT *cb, void *pContext)
{
    return(HLB_ERROR_UNSUPPORTED);
}
...
int32_t HLBListSendChatMsg(  HLBApiRefT *pApiRef, const char * name, const char * msg, HLBOpCallbackT *cb, void * pContext)
{
    return(HLB_ERROR_UNSUPPORTED);
}
...
HLBBudT *_updateFriendInRoster( HLBApiRefT  * pApiRef, PXONLINE_FRIEND pFriend)
{
    ...
    ds_strnzcpy( strTemp, pFriend->szGamertag, sizeof(strTemp) );
    // find user record
    ...
    if (pRec == NULL)
    {
        ...
        ds_memcpy_s(pBuddy,sizeof(*pBuddy),pFriend,sizeof(*pFriend));
...
void _reloadFriends (HLBApiRefT *pApiRef)
{
    ...
    int32_t totalBud = 0;
    XONLINE_FRIEND aOnlineFriends[FRIENDAPI_MAX_FRIENDS ];
    PXONLINE_FRIEND pFriend = _HLBFriendsGetFriendByIndex(pApiRef->iUserIndex, index, aOnlineFriends);
    ...
    for (index=totalBud; index >= 0; index--)
    {
        ...
        if ((pBud) && (_HLBFriendsGetFriendByName(pApiRef->iUserIndex, pBud->szGamertag) == NULL))
        {
...
{
    if (pApiRef->iUserIndex != iIndex)
    {
    pApiRef->iUserIndex = iIndex;
        _reloadFriends(pApiRef);
    }
    return (HLB_ERROR_NONE);
}
...
{
    int32_t retVal = HLB_ERROR_MISSING;

    XONLINE_FRIEND *pOnlineFriend = _HLBFriendsGetFriendByName(pApiRef->iUserIndex, name);

    if(pOnlineFriend)
    {
        ds_strnzcpy(sSessionID, (char *)pOnlineFriend->sessionID.ab, iBufSize);

        retVal = HLB_ERROR_NONE;
    }
