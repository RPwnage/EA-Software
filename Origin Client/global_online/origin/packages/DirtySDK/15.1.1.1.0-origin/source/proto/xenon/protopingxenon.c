...
static ProtoPingRequestT *_ProtoPingCreateRequest(ProtoPingRefT *pRef, int32_t eRequestType, const unsigned char *pData, int32_t iDataLen)
{
    ...
    if (iDataLen != 0)
    {
        ds_memcpy_s(pRequest->aData, sizeof(pRequest->aData), pData, iDataLen);
    }
    ...
    if (pRequest->eRequestState == ST_SESS)
    {
        ...
        if (dwResult == ERROR_SUCCESS)
        {
            ...
            if (pRequest->pSessionLookupResults->dwSearchResults == 1)
            {
                ...
                // store the results
                ds_memcpy_s(&pRequest->SessionInfo, 
                            sizeof(pRequest->SessionInfo), 
                            &pRequest->pSessionLookupResults->pResults[0].info, 
                            sizeof(pRequest->pSessionLookupResults->pResults[0].info));
...
int32_t ProtoPingControl(ProtoPingRefT *pProtoPing, int32_t iControl, int32_t iValue, void *pValue)
{
    ...
    if (iControl == 'sess')
    {
        ...
        if (pValue != NULL)
        {
            ...
            ds_memcpy_s(&pProtoPing->ListenSession, sizeof(pProtoPing->ListenSession), sessionId, sizeof(*sessionId));
...
int32_t ProtoPingRequest(ProtoPingRefT *pRef, DirtyAddrT *pAddr, const unsigned char *pData, int32_t iDataLen, int32_t iTimeout, int32_t iTtl)
{
    ...
    // set target address
    ds_memcpy_s(&pRequest->SessionInfo.sessionID, sizeof(pRequest->SessionInfo.sessionID), pAddr->strMachineAddr, sizeof(pAddr->strMachineAddr));
...
int32_t ProtoPingResponse(ProtoPingRefT *pRef, unsigned char *pBuffer, int32_t *pBufLen, ProtoPingResponseT *pResponse)
{
    ...
    if (pResponse != NULL)
    {
        ...
        ds_memcpy_s(&pResponse->Addr.strMachineAddr, sizeof(pResponse->Addr.strMachineAddr), &pRequest->SessionInfo.sessionID, sizeof(pRequest->SessionInfo.sessionID));
    }
    ...
    if (pBufLen != NULL)
    {
        ...
        if (*pBufLen != 0)
        {
            ds_memcpy(pBuffer, pRequest->aData, *pBufLen);
        }
    }
