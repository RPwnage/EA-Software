...
//! ideal internal buf size used when xhttp is enabled , experimentation showed that smaller size impacts reception throughput
#define PROTOHTTP_RECVBUFSIZE   (16 * 1024)

//! ideal xhttp read buffer size, experimentation showed that smaller size impacts reception throughput
#define PROTOHTTP_XHTTPREADBUFSIZE (16 * 1024)
...
typedef struct ProtoHttpRefT
{
    ...
    int32_t iRecvBufSize;   //!< receive buffer size to use with XHttpSetOption(XHTTP_OPTION_READ_BUFFER_SIZE)
    ...
    uint8_t bGetAuthToken;  //!< if TRUE get auth token before connecting
} ProtoHttpRefT;
...
static void _ProtoHttpXHttpStatusCallback(HINTERNET hInternet, DWORD_PTR dwContext, DWORD dwInternetStatus, void *pvStatusInformation, DWORD dwStatusInformationLength)
{
    ...
    if (dwInternetStatus == XHTTP_CALLBACK_STATUS_REQUEST_ERROR)
    {
        ...
        NetPrintf(("protohttpxenon: [0x%08x] dwResult=%s, dwError=%s\n", (uintptr_t)pState, DirtyErrGetNameList(pResult->dwResult, _XHttpCallbackResultList),
            DirtyErrGetName(pResult->dwError)));

        pState->eState = ST_FAIL;
        if (pResult->dwError == ERROR_XHTTP_TIMEOUT)
        {
...
/*F********************************************************************************/
/*!
    \Function _ProtoHttpCustomHeaderCallback

    \Description
        Custom header callback handler.  We need this callback handler, when using
        'normal' ProtoHttp, to translate the ProtoHttp ref into the ProtoHttpXenon
        ref that owns it, before passing it on to the user-specified callback
        handler.

    \Input *_pProtoHttp - 'normal' protohttp ref (unused)
    ...
    \Input *pUserRef    - ProtoHttpXenon ref

    \Output
        int32_t         - callback result

    \Version 04/08/2013 (jbrookes)
*/
/********************************************************************************F*/
static int32_t _ProtoHttpCustomHeaderCallback(__ProtoHttpRefT *_pProtoHttp, char *pHeader, uint32_t uHeaderSize, const char *pData, int64_t iDataLen, void *pUserRef)
{
    ProtoHttpRefT *pProtoHttp = (ProtoHttpRefT *)pUserRef;
    int32_t iResult = 0;
    if (pProtoHttp->pCustomHeaderCb != NULL)
    {
        iResult = pProtoHttp->pCustomHeaderCb(pProtoHttp, pHeader, uHeaderSize, pData, iDataLen, pProtoHttp->pCallbackRef);
    }
    return(iResult);
}
...
/*F********************************************************************************/
/*!
    \Function _ProtoHttpRecvHeaderCallback

    \Description
        Custom receive callback handler.  We need this callback handler, when using
        'normal' ProtoHttp, to translate the ProtoHttp ref into the ProtoHttpXenon
        ref that owns it, before passing it on to the user-specified callback
        handler.
...
*/
/********************************************************************************F*/
static int32_t _ProtoHttpRecvHeaderCallback(__ProtoHttpRefT *_pProtoHttp, const char *pHeader, uint32_t uHeaderSize, void *pUserRef)
{
    ProtoHttpRefT *pProtoHttp = (ProtoHttpRefT *)pUserRef;
    int32_t iResult = 0;
    if (pProtoHttp->pReceiveHeaderCb != NULL)
    {
        iResult = pProtoHttp->pReceiveHeaderCb(pProtoHttp, pHeader, uHeaderSize, pProtoHttp->pCallbackRef);
    }
    return(iResult);
}
...
static int32_t _ProtoHttpRequest(ProtoHttpRefT *pState)
{
    ...
    // set xhttp receive buffer size if needed
    if (pState->iRecvBufSize)
    {
        BOOL bResult;
        NetPrintf(("protohttpxenon: xhttp read buf size (tcp window size) successfully changed to %d\n", pState->iRecvBufSize));
        if ((bResult = XHttpSetOption(pState->hHttpConnect, XHTTP_OPTION_READ_BUFFER_SIZE, &pState->iRecvBufSize, sizeof(pState->iRecvBufSize))) == FALSE)
        {
            NetPrintf(("protohttpxenon: XHttpSetOption(XHTTP_OPTION_READ_BUFFER_SIZE, %d) failed (err=%s)\n",
                pState->iRecvBufSize, DirtyErrGetName(GetLastError())));
        }
    }
    ...
    // set timeout
    if (!XHttpSetOption(pState->hHttpRequest, XHTTP_OPTION_RECEIVE_TIMEOUT, &pState->uTimeout, sizeof(pState->uTimeout)))
    {
        NetPrintf(("protohttpxenon: error %s trying to set timeout to %dms\n", DirtyErrGetName(GetLastError()), pState->uTimeout));
    }
...
static int32_t _ProtoHttpResizeBuffer(ProtoHttpRefT *pState, int32_t iBufMax)
{
    ...
    {
        NetPrintf(("protohttpxenon: [0x%08x] warning; resize of input buffer is losing %d bytes of data\n", iCopySize - iBufMax));
        iCopySize = iBufMax;
    }
...
/*F********************************************************************************/
/*!
    \Function _ProtoHttpSwitchOperatingMode

    \Description
        Switch from generic protohttp to xhttp-based protohttp, and vice versa.

    \Input *pState      - reference pointer
    \Input bXhttp       - TRUE: use xttp; FALSE use generic mode
...
*/
/********************************************************************************F*/
static int32_t _ProtoHttpSwitchOperatingMode(ProtoHttpRefT *pState, int32_t bXhttp)
{
    if (bXhttp && (pState->pProtoHttp != NULL))
    {
        // protohttp mode switch:   Generic ProtoHttp ---> XHTTP-based
        __ProtoHttpDestroy(pState->pProtoHttp);
        pState->pProtoHttp = NULL;

        if (pState->iInpMax < PROTOHTTP_RECVBUFSIZE)
        {
            NetPrintf(("protohttpxenon: [0x%08x] inbound throughput optimization: resizing recv buffer\n", (uintptr_t)pState));
            if (_ProtoHttpResizeBuffer(pState, PROTOHTTP_RECVBUFSIZE) != 0)
            {
                return(-1);
            }
        }

        // allocate xhttp read buffer
        if ((pState->pXhttpReadBuf = DirtyMemAlloc(PROTOHTTP_XHTTPREADBUFSIZE, PROTOHTTP_MEMID, pState->iMemGroup, pState->pMemGroupUserData)) == NULL)
        {
            NetPrintf(("protohttpxenon: [0x%08x] unable to allocate xhttp read buffer\n", (uintptr_t)pState));
            return(-2);
        }

        pState->iXhttpReadBufSize = PROTOHTTP_XHTTPREADBUFSIZE;
    }
    else if (!bXhttp && (pState->pProtoHttp == NULL))
    {
        // protohttp mode switch:   XHTTP-based ---> Generic ProtoHttp
        pState->pProtoHttp = __ProtoHttpCreate(pState->iInpMax);

        if (pState->pXhttpReadBuf)
        {
            DirtyMemFree(pState->pXhttpReadBuf, PROTOHTTP_MEMID, pState->iMemGroup, pState->pMemGroupUserData);
            pState->pXhttpReadBuf = NULL;
            pState->iXhttpReadBufSize = 0;
        }
    }
...
static int32_t _ProtoHttpParseHeader(ProtoHttpRefT *pState, char *pStrHdr, int32_t iHdrLen)
{
    ...
    if ((ProtoHttpGetHeaderValue(NULL, pStrHdr, "content-length", strTemp, sizeof(strTemp), NULL)) != -1)
    {
    ...
    if ((ProtoHttpGetHeaderValue(NULL, pStrHdr, "last-modified", strTemp, sizeof(strTemp), NULL)) != -1)
    {
        pState->iHdrDate = ds_strtotime(strTemp);
    }
    else
    {
        pState->iHdrDate = 0;
    ...
    {
        ProtoHttpGetHeaderValue(NULL, pStrHdr, "connection", strTemp, sizeof(strTemp), NULL);
...
static int32_t _ProtoHttpGetHeader(ProtoHttpRefT *pState)
{
    DWORD dwHeaderLen = pState->iInpMax;
    BOOL bResult;
    if ((bResult = XHttpQueryHeaders(pState->hHttpRequest, XHTTP_QUERY_RAW_HEADERS_CRLF, XHTTP_HEADER_NAME_BY_INDEX, pState->pInpBuf, &dwHeaderLen, NULL)) == TRUE)
    {
        // parse the header
        _ProtoHttpParseHeader(pState, pState->pInpBuf, dwHeaderLen);
        // save to header cache
...
static int32_t _ProtoHttpRecv(ProtoHttpRefT *pState, char *pStrBuf, int32_t iSize)
{
    ...
    for (iReadSize = 0; (pState->bRecvInProgress == FALSE) && (pState->bRecvComplete == FALSE); )
    {
        ...
        if (pState->iRecvRslt > 0)
        {
            ...
            ds_memcpy(pStrBuf, pState->pXhttpReadBuf, pState->iRecvRslt);
        ...
        if (iSize > 0)
        {
            ...
            iRequestSize = (iSize > pState->iXhttpReadBufSize) ? pState->iXhttpReadBufSize : iSize;
            pState->bRecvInProgress = TRUE;
            if (!XHttpReadData(pState->hHttpRequest, pState->pXhttpReadBuf, iRequestSize, NULL))
            {
...
static int32_t _ProtoHttpSend(ProtoHttpRefT *pState, const char *pStrBuf, int32_t iSize)
{
    ...
    if ((iInpAvail = pState->iInpMax - pState->iInpLen) > 0)
    {
        ...
        ds_memcpy(pState->pInpBuf+pState->iInpLen, pStrBuf, iSize);
...
ProtoHttpRefT *ProtoHttpCreate(int32_t iBufSize)
{
    ...
    ds_snzprintf(pState->strUserAgent, sizeof(pState->strUserAgent), "ProtoHttp %d.%d/DS %d.%d.%d.%d.%d (" DIRTYCODE_PLATNAME ")",
...
void ProtoHttpCallback(ProtoHttpRefT *pState, ProtoHttpCustomHeaderCbT *pCustomHeaderCb, ProtoHttpReceiveHeaderCbT *pReceiveHeaderCb, void *pCallbackRef)
{
    ...
    {
        // set up protohttp ref to call our custom callback handlers, with the protohttpxenon ref state
        __ProtoHttpCallback(pState->pProtoHttp, _ProtoHttpCustomHeaderCallback, _ProtoHttpRecvHeaderCallback, pState);
    }
    else if (pCustomHeaderCb != NULL)
    {
        NetPrintf(("protohttpxenon: [0x%08x] custom header callbacks are not supported by XHttp; specification of callback %p will be ignored when XHTTP is enabled\n", pCustomHeaderCb));
    }
...
/*!
    ...
    \Notes
        Write callbacks are not supported when using XHTTP.
...
int32_t ProtoHttpRequestCb(ProtoHttpRefT *pState, const char *pUrl, const char *pData, int64_t iDataSize, ProtoHttpRequestTypeE eRequestType, ProtoHttpWriteCbT *pWriteCb, void *pWriteCbUserData)
{
    ...
    ds_strnzcpy(pState->pInpBuf, pUrl, pState->iInpMax);
    ...
    if (pState->iSecure && pState->bGetAuthToken)
    {
        // make token request
        if ((iResult = DirtyAuthGetToken(0, pState->strHost, FALSE)) == DIRTYAUTH_PENDING)
        {
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'rbuf'      Set recv buffer size to be used with XHTTP_OPTION_READ_BUFFER_SIZE
            ...
            'xtok'      Sets auth token acquisition enabled/disabled (default=disabled)
        \endverbatim
...
int32_t ProtoHttpControl(ProtoHttpRefT *pState, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iSelect == 'xhtp')
    {
        return(_ProtoHttpSwitchOperatingMode(pState, ((iValue == 0) ? FALSE : TRUE)));
    }
    ...
    if (iSelect == 'rbuf')
    {
        NetPrintf(("protohttpxenon: [0x%08x] xhttp receive buffer size set to %d\n", (uintptr_t)pState, iValue));
        pState->iRecvBufSize = (unsigned)iValue;
        return(0);
    }
    if (iSelect == 'rmax')
    {
    ...
    if (iSelect == 'xtok')
    {
        ...
        NetPrintf(("protohttpxenon: auth token acquisition %s\n", (pState->bGetAuthToken == TRUE) ? "enabled" : "disabled"));
        return(0);
    }
    // unhandled
...
int32_t ProtoHttpStatus(ProtoHttpRefT *pState, int32_t iSelect, void *pBuffer, int32_t iBufSize)
{
    ...
    if (iSelect == 'body')
    {
        int64_t iBodySize = pState->iBodySize;
        if ((pBuffer != NULL) && (iBufSize == sizeof(iBodySize)))
        {
            ds_memcpy(pBuffer, &iBodySize, iBufSize);
        }
...
void ProtoHttpUpdate(ProtoHttpRefT *pState)
{
    ...
    {
        int32_t iResult;
        if ((iResult = DirtyAuthCheckToken(pState->strHost, NULL, NULL)) == DIRTYAUTH_SUCCESS)
        {
            pState->eState = ST_CONN;
        }
    ...
    if ((pState->eState == ST_HEAD) && (pState->bHeadAvail == TRUE))
    {
        ...
        else if ((pState->bGetAuthToken) && (pState->iHdrCode == PROTOHTTP_RESPONSE_UNAUTHORIZED))
        {
            // xhttp: if we got an auth token and response is 401/UNAUTHORIZED, this indicates the token has expired
            ...
            // clear state that has been set so far
            if ((iResult = DirtyAuthGetToken(0, pState->strHost, TRUE)) == DIRTYAUTH_PENDING)
            {
