...
static const CipherSuiteT _SSL3_CipherSuite[] =
{
    ...
    { { 0, 0x35 }, SSL3_TLS1_0, SSL3_KEY_RSA, SSL3_KEYLEN_256, SSL3_ENC_AES, SSL3_MAC_SHA,    CRYPTHASH_SHA1,   16, SSL3_PRF_SHA256, PROTOSSL_CIPHER_RSA_WITH_AES_256_CBC_SHA,    "TLS_RSA_WITH_AES_256_CBC_SHA" },     // suite 53: rsa+aes+sha
    { { 0, 0x2f }, SSL3_TLS1_0, SSL3_KEY_RSA, SSL3_KEYLEN_128, SSL3_ENC_AES, SSL3_MAC_SHA,    CRYPTHASH_SHA1,   16, SSL3_PRF_SHA256, PROTOSSL_CIPHER_RSA_WITH_AES_128_CBC_SHA,    "TLS_RSA_WITH_AES_128_CBC_SHA" },     // suite 47: rsa+aes+sha
    { { 0, 0x05 }, SSL3_TLS1_0, SSL3_KEY_RSA, SSL3_KEYLEN_128, SSL3_ENC_RC4, SSL3_MAC_SHA,    CRYPTHASH_SHA1,    0, SSL3_PRF_SHA256, PROTOSSL_CIPHER_RSA_WITH_RC4_128_SHA,        "TLS_RSA_WITH_RC4_128_SHA" },         // suite  5: rsa+rc4+sha
    { { 0, 0x04 }, SSL3_TLS1_0, SSL3_KEY_RSA, SSL3_KEYLEN_128, SSL3_ENC_RC4, SSL3_MAC_MD5,    CRYPTHASH_MD5,     0, SSL3_PRF_SHA256, PROTOSSL_CIPHER_RSA_WITH_RC4_128_MD5,        "TLS_RSA_WITH_RC4_128_MD5" },         // suite  4: rsa+rc4+md5
};
...
// The 1024-bit public key modulus for (old) GOS CA Cert signed with md5 and an exponent of 3
#ifndef DIRTYCODE_XBOXONE
static const uint8_t _ProtoSSL_GOSServerModulus1024[] =
{
...
static const uint8_t _ProtoSSL_GOS2011ServerModulus2048[] =
{
...
static ProtoSSLCACertT _ProtoSSL_CACerts[] =
{
    ...
    { { "US", "California", "Redwood City", "Electronic Arts, Inc.",
      ...
      256, _ProtoSSL_GOS2011ServerModulus2048,
      1, { 0x11 },
    ...
    { { "US", "", "", "GeoTrust, Inc.", "", "GeoTrust SSL CA" },
      ...
      3, { 0x01, 0x00, 0x01 },
#ifndef DIRTYCODE_XBOXONE
      0, NULL, NULL, &_ProtoSSL_CACerts[7] },
    // OTG ca (pre-GOS, obsolete)
    { { "US", "California", "Redwood City", "Electronic Arts, Inc.",
...
static int32_t _ResetSecureState(ProtoSSLRefT *pState, int32_t iSecure)
{
    ...
    {
        // clear secure state
        memset(pSecure, 0, sizeof(*pSecure));
...
static int32_t _ParsePrivateKey(const char *pPrivateKeyData, int32_t iPrivateKeyLen, X509PrivateKeyT *pPrivateKey)
{
    ...
    memset(pPrivateKey, 0, sizeof(*pPrivateKey));
...
static int32_t _VerifySignature(ProtoSSLRefT *pState, X509CertificateT *pCert, const uint8_t *pKeyModData, int32_t iKeyModSize, const uint8_t *pKeyExpData, int32_t iKeyExpSize)
{
    ...
    return(memcmp(pCert->HashData, RSAContext.EncryptBlock+(pCert->iSigSize&65534)-pCert->iHashSize, pCert->iHashSize));
}

/*F***************************************************************************/
/*!
    \Function _VerifyPkcs1

    \Description
        Check PKCS#1 padding for validity
...
*/
/***************************************************************************F*/
static int32_t _VerifyPkcs1(const uint8_t *pData, int32_t iKeySize, int32_t iSecretSize)
{
    int32_t iIndex;
    uint8_t bValid = TRUE;
    ...
    // validate padding; must be non-zero
...
static int32_t _VerifyCertificate(ProtoSSLRefT *pState, X509CertificateT *pCert, uint8_t bCertIsCA)
{
    ...
    else
    {
        ...
        for (pCACert = _ProtoSSL_CACerts; pCACert != NULL; pCACert = pCACert->pNext)
        {
            ...
            NetPrintfVerbose((DEBUG_VAL_CERT, 0, "protossl: verifying succeded\n"));
...
static void _SessionHistoryInvalidate(const uint8_t *pSessionId)
{
    ...
    if ((pSessInfo = _SessionHistoryGet(NULL, pSessionId)) != NULL)
    {
        ...
        memset(pSessInfo, 0, sizeof(*pSessInfo));
    }
...
static CertValidHistoryT *_CertValidHistoryGet(const uint8_t *pFingerprintId, uint32_t uCertSize)
{
    ...
    for (iCert = 0, pCertInfo = NULL; iCert < SSL_CERTVALID_MAX; iCert += 1)
    {
        ...
        if (NetTickDiff(uCurTick, pCertHist->uCertValidTick) > SSL_CERTVALIDTIME_MAX)
        {
            ...
            memset(pCertHist, 0, sizeof(*pCertHist));
            continue;
        }
        // check for match
...
static int32_t _ProtoSSLAddHelloExtnServerName(ProtoSSLRefT *pState, uint8_t *pBuffer, int32_t iBufLen)
{
    ...
    if ((uHostLen+9) < (unsigned)iBufLen)
    {
        ...
        *pBuffer++ = (uint8_t)(uHostLen);
        ...
        // hostname
        memcpy(pBuffer, pState->strHost, uHostLen);
...
{
    const int32_t _iListLength = 2*3; // signature hash algorithms list length (three algorithms)
    ...
    if (_iExtnLength < iBufLen)
    {
        ...
        *pBuffer++ = SSL3_HASHID_SHA256;
        *pBuffer++ = SSL3_KEY_RSA;
        *pBuffer++ = SSL3_HASHID_SHA1;
        *pBuffer++ = SSL3_KEY_RSA;
        *pBuffer++ = SSL3_HASHID_MD5;
        *pBuffer++ = SSL3_KEY_RSA;
    }
...
*/
/********************************************************************************F*/
static uint8_t *_ProtoSSLAddHelloExtensions(ProtoSSLRefT *pState, uint8_t *pBuffer, int32_t iBufLen)
{
    uint32_t uExtnLen;
    int32_t iBufOff;
    ...
    if (pState->uHelloExtn & PROTOSSL_HELLOEXTN_SERVERNAME)
    {
        iBufOff += _ProtoSSLAddHelloExtnServerName(pState, pBuffer+iBufOff, iBufLen-iBufOff);
    }
    if (pState->uHelloExtn & PROTOSSL_HELLOEXTN_SIGALGS)
    {
...
static int32_t _ProtoSSLUpdateSendClientHello(ProtoSSLRefT *pState)
{
    ...
    *pData++ = (uint8_t)(pSecure->uSslClientVersion>>0);
    ...
    if (_SessionHistoryGetInfo(&SessHist, &pState->PeerAddr, NULL) != NULL)
    {
    ...
    *pData++ = 0;
    ...
    {
        pData = _ProtoSSLAddHelloExtensions(pState, pData, (uint32_t)sizeof(strBody)-(uint32_t)(pData-strBody));
    }
...
{
    SecureStateT *pSecure = pState->pSecure;
    int32_t iIndex, iCipher, iNumCiphers;
    SessionHistoryT SessHist;
    ...
    if ((*pData == sizeof(pSecure->SessionId)) && (_SessionHistoryGetInfo(&SessHist, NULL, pData+1) != NULL))
    {
    ...
    for (iCipher = 0, iIndex = 0, pSecure->pCipher = NULL; (iCipher < iNumCiphers) && (pSecure->pCipher == NULL); iCipher += 1, pData += 2)
    {
...
static int32_t _ProtoSSLUpdateRecvServerHello(ProtoSSLRefT *pState, const uint8_t *pData)
{
    ...
    {
        memset(pSecure->MasterKey, 0, sizeof(pSecure->MasterKey));
    }
...
static int32_t _ProtoSSLUpdateRecvCertificate(ProtoSSLRefT *pState, const uint8_t *pData)
{
    ...
    uint8_t aFingerprintId[SSL_FINGERPRINT_SIZE], bCertValid = FALSE;
    uint64_t uTime;
    #if DIRTYCODE_LOGGING
    char strIdentSubject[512], strIdentIssuer[512];
    #endif
    ...
    while (iSize1 > 3)
    {
        ...
        if (pState->bAllowAnyCert || bCertValid)
        {
            NetPrintfVerbose((pState->iVerbose, 0, "protossl: bypassing certificate validation (bypass=%d, cached=%d)\n", pState->bAllowAnyCert, bCertValid));
            return(ST3_RECV_HELLO);
        }
        ...
        if ((iParse == 0) || (iSize1 == (iSize2 + 3)))
        {
            ...
            else
            {
                ...
                iResult = ST3_RECV_HELLO;
                // add cert fingerprint to cert validation history
...
/*!
    ...
    \Description
        Send ClientKeyExchange handshake packet to the server; uses public key to encrypt
...
static int32_t _ProtoSSLUpdateRecvClientKeyExchange(ProtoSSLRefT *pState, const uint8_t *pData)
{
    ...
    if ((uSslVersion != pSecure->uSslClientVersion) || (_VerifyPkcs1(pSecure->RSAContext.EncryptBlock, iCryptKeySize, sizeof(pSecure->PreMasterKey)) == FALSE))
    {
...
static int32_t _ProtoSSLUpdateSend(ProtoSSLRefT *pState, SecureStateT *pSecure)
{
    ...
    if (pSecure->iSendProg == pSecure->iSendSize)
    {
        ...
        {
            pState->iState = _ProtoSSLUpdateSendClientKeyExchange(pState);
        }
    ...
    if (pSecure->iRecvProg < pSecure->iRecvSize)
    {
        ...
        if (iResult > 0)
        {
            ...
            if (pSecure->iRecvProg == pSecure->iRecvSize)
            {
                ...
                memset(pSecure->RecvData+pSecure->iRecvSize, 0xcc, SSL_BUFFER_DBGFENCE);
                #endif
