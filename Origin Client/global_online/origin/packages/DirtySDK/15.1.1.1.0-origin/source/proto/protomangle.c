...
#ifdef DIRTYCODE_PS4   //todo use the 6000 port range on all platforms, do post 13.3.x
...
static void _ProtoMangleHttpRequest(ProtoMangleRefT *pRef)
{
    ...
    // setup the url
    ds_snzprintf(strUrl, sizeof(strUrl), "http://%s:%d/getPeerAddress?myIP=%s&myPort=%d&version=1.0",
...
int32_t ProtoMangleReport(ProtoMangleRefT *pRef, ProtoMangleStatusE eStatus, int32_t iLatency)
{
    ...
    ds_snzprintf(strData, sizeof(strData), "myIP=%s&myPort=%d&version=1.0&status=%s&gameFeatureID=%s\n",
