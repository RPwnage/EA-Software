...
#define VOIP_RELIABLE_DEBUG     (DIRTYCODE_DEBUG && TRUE)
...
static LinkedReliableDataT *_VoipGetReliableDataBufferFromFreePool(VoipConnectionlistT *pConnectionlist, uint8_t bAllocIfEmpty)
{
    ...
    {
        if ((pReliableDataBuffer = (LinkedReliableDataT *)DirtyMemAlloc(sizeof(LinkedReliableDataT), VOIP_MEMID, iMemGroup, pMemGroupUserData)) != NULL)
        {
            memset(&pReliableDataBuffer, sizeof(*pReliableDataBuffer), 0);
...
static void _VoipConnectionReliableDataEnqueue(VoipConnectionlistT *pConnectionlist, int32_t iConnID, ReliableDataT *pReliableData)
{
    ...
    uint32_t bSeqNbIncremented = FALSE;    // used to increment sequence number only once
    ...
    while (iConnectionIndex < pConnectionlist->iMaxConnections)
    {
        ...
        if (pConnection->eState == ST_ACTV || pConnection->eState == ST_CONN)
        {
            ...
            if (pNewEntry)
            {
                ...
                pNewEntry->data.info.uSeq = pConnectionlist->uOutSeqNb;
                if (!bSeqNbIncremented)
                {
                    pConnectionlist->uOutSeqNb = _VoipConnectionIncrementReliableSeqNb(pConnectionlist->uOutSeqNb);
                    bSeqNbIncremented = TRUE;
                }

                // remember if this is the first outbound reliable data entry on this connection
                if (pConnection->uFirstReliableSeq == 0)
                {
                    pConnection->uFirstReliableSeq =  pNewEntry->data.info.uSeq;
                    #if VOIP_RELIABLE_DEBUG
                    NetPrintf(("voipconnection: [%d] first reliable outbound seq number is %d\n", iConnectionIndex, pConnection->uFirstReliableSeq));
                    #endif
                }
...
static void _VoipConnectionReliableDataDequeue(VoipConnectionlistT *pConnectionlist, int32_t iConnID)
{
    ...
    if (pRemovedEntry->data.info.uSeq == pConnectionlist->pConnections[iConnID].uFirstReliableSeq)
    {
        pConnectionlist->pConnections[iConnID].bFirstReliableDone = TRUE;

        #if VOIP_RELIABLE_DEBUG
        NetPrintf(("voipconnection: [%d] first reliable data sequence number successfully announced\n"));
        #endif
    }
...
*/
/********************************************************************************F*/
static uint8_t * _VoipReliableDataPackACKandACKCNF(VoipConnectionlistT *pConnectionlist, int32_t iConnID, uint8_t *pFlags, uint32_t bPing, uint8_t *pWrite)
{
    ...
    uint32_t uSpaceLeftInPkt = VOIP_MAXRELIABLEDATA;    // amount of space still available in packet for reliable data
    int32_t iConnectionIndex;
    ...
    while (iConnectionIndex < pConnectionlist->iMaxConnections)
    {
        ...
        if (sizeof(ReliableAckT) < uSpaceLeftInPkt)
        {
            ...
            if (pConnection->bPeerNeedsAck || bAckOrAckCnfNeeded)
            {
                ...
                // set remote peer id
                _VoipEncodeU32(&ackEntry.aRemoteClientId[0], pConnection->uClientId);
                ...
                ds_memcpy_s(pWrite, VOIP_MAXRELIABLEDATA, &ackEntry, sizeof(ackEntry));
                ...
                *pFlags |=  VOIP_PACKET_RELIABLE_FLAG_ACK_ACKCNF;
                ...
                if (pConnection->bPeerNeedsAck)
                {
                     NetPrintf(("voipconnection: [%d] reliable ACK+ACKCNF (acked seq# %03d, %s) added to outbound %s packet for clientId=0x%08x\n", iConnectionIndex,
                        (ackEntry.uAckedSeq & 0x7F), ((pConnection->pOutboundReliableDataQueue != NULL)?"SendAck":"HaltAck"), (bPing?"PNG":"MIC"), pConnection->uClientId));
                }
                else
                {
                     NetPrintf(("voipconnection: [%d] reliable ACK+ACKCNF (acked seq# INV, %s) added to outbound %s packet for clientId=0x%08x\n", iConnectionIndex,
                        ((pConnection->pOutboundReliableDataQueue != NULL) ?"SendAck":"HaltAck"), (bPing?"PNG":"MIC"), pConnection->uClientId));
                }
                #endif
            }
        }
...
*/
/********************************************************************************F*/
static uint8_t * _VoipReliableDataPackDATA(VoipConnectionlistT *pConnectionlist, int32_t iConnID, uint8_t *pFlags, uint32_t bPing, uint8_t *pWrite)
{
    uint8_t uEntryCount = 0;
    uint8_t *pEntryCount = pWrite;                      // remember where the entry count needs to be written if any entry added to packet buffer
    uint32_t uSpaceLeftInPkt = VOIP_MAXRELIABLEDATA;    // amount of space still available in packet for reliable data
    ...
    while (pCurrent != NULL)
    {
        ...
        if (uSize < uSpaceLeftInPkt)
        {
            ...
            *pFlags |= VOIP_PACKET_RELIABLE_FLAG_DATA;
            ...
            // if appropriate, flag that packet contains first outbound reliable data entry on the connection
            if ((pConnection->bFirstReliableDone == FALSE) && (pCurrent->data.info.uSeq == pConnection->uFirstReliableSeq))
            {
                *pFlags |= VOIP_PACKET_RELIABLE_FLAG_FIRST;
            }

            #if VOIP_RELIABLE_DEBUG
            NetPrintf(("voipconnection: [%d] reliable DATA (seq# %03d, %s, %s, %d bytes) added to outbound %s packet\n", iConnID, pCurrent->data.info.uSeq,
                _strReliableType[pCurrent->data.info.uType], ((*pFlags & VOIP_PACKET_RELIABLE_FLAG_FIRST)?"FIRST":"!FIRST"), uSize, (bPing?"PNG":"MIC")));
            #endif
...
*/
/********************************************************************************F*/
static uint8_t * _VoipReliableDataOutProcess(VoipConnectionlistT *pConnectionlist, int32_t iConnID, VoipPacketHeadT *pPacketHeader, uint8_t *pPacketPayload)
{
    uint32_t bPing = ((pPacketHeader->aType[0] == 'P' && pPacketHeader->aType[1] == 'N' && pPacketHeader->aType[2] == 'G') ? TRUE : FALSE);
    uint8_t *pWrite = _VoipReliableDataPackACKandACKCNF(pConnectionlist, iConnID, &pPacketHeader->uFlags, bPing, pPacketPayload);
    ...
    pWrite = _VoipReliableDataPackDATA(pConnectionlist, iConnID, &pPacketHeader->uFlags, bPing, pWrite);
...
static void _VoipReliableDataInboundUserAddProcess(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, ReliableDataT *pReliableData)
{
    ...
    if (VOIP_NullUser((VoipUserT *)&pConnection->RemoteUsers[uUserIndex]))
    {
        ...
        // register the user
        pConnectionlist->pRegUserCb((VoipUserT *)&pConnection->RemoteUsers[uUserIndex], TRUE, pConnectionlist->pCbUserData);
...
static void _VoipReliableDataInboundUserRemProcess(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, ReliableDataT *pReliableData)
{
    ...
    if (!VOIP_NullUser((VoipUserT *)&pConnection->RemoteUsers[uUserIndex]))
    {
        ...
        {
            // unregister the user
            pConnectionlist->pRegUserCb((VoipUserT *)&pConnection->RemoteUsers[uUserIndex], FALSE, pConnectionlist->pCbUserData);
...
/*!
    ...
    \Description
        Inbound processing of reliablity data of type VOIP_RELIABLE_TYPE_USERS.
...
static uint8_t * _VoipReliableDataUnpackACKandACKCNF(VoipConnectionlistT *pConnectionlist, uint32_t uSrcClientId, uint8_t uFlags, uint32_t bPing, uint8_t *pRead)
{
    ...
    for (iConnectionIndex = 0; iConnectionIndex < pConnectionlist->iMaxConnections; iConnectionIndex++)
    {
        if (pConnectionlist->pConnections[iConnectionIndex].uClientId == uSrcClientId)
        {
    ...
    if (uFlags &  VOIP_PACKET_RELIABLE_FLAG_ACK_ACKCNF)
    {
        ...
        for (iReliableAckEntryIndex = 0; iReliableAckEntryIndex < iReliableAckEntriesCount; iReliableAckEntryIndex++)
        {
            ...
            if (pConnection != NULL)
            {
                ...
                if (uClientId == pConnectionlist->uClientId)
                {
                    uint8_t uAckedSeq;
...
static uint8_t * _VoipReliableDataUnpackDATA(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, uint8_t uFlags, uint32_t bPing, uint8_t *pRead)
{
    ...
    if (uFlags & VOIP_PACKET_RELIABLE_FLAG_DATA)
    {
        ...
        for (iReliableDataEntryIndex = 0; iReliableDataEntryIndex < iReliableDataEntriesCount; iReliableDataEntryIndex++)
        {
            ...
            uint32_t bSkipEntry = FALSE;

            // move read pointer ahead
            pRead += sizeof(pReliableData->info) + uSize;

            // check for expected sequence number
            if (pConnection->uInSeqNb == 0)
            {
                if (uFlags & VOIP_PACKET_RELIABLE_FLAG_FIRST)
                {
                    pConnection->uInSeqNb = pReliableData->info.uSeq;

                    #if VOIP_RELIABLE_DEBUG
                    NetPrintf(("voipconnection: [%d] first inbound reliable data sequence number is %d\n", (pConnection - &pConnectionlist->pConnections[0]), pConnection->uInSeqNb));
                    #endif
                }
                else
                {
                    #if VOIP_RELIABLE_DEBUG
                    NetPrintf(("voipconnection: [%d] dropped inbound reliable data embedded in %s packet - first seq nb announcement not yet received (got: %d)\n",
                        (pConnection - &pConnectionlist->pConnections[0]), (bPing?"PNG":"MIC"), pReliableData->info.uSeq));
                    #endif
                    bSkipEntry = TRUE;
                }
            }
            ...
            // does this reliable data entry have the next expected sequence number
            if (bSkipEntry == FALSE)
            {
                ...
                #if VOIP_RELIABLE_DEBUG
                NetPrintf(("voipconnection: [%d] reliable DATA (seq# %03d, %s, %s, %d bytes) found in inbound %s packet\n", (pConnection - &pConnectionlist->pConnections[0]),
                    pReliableData->info.uSeq, _strReliableType[pReliableData->info.uType], ((uFlags & VOIP_PACKET_RELIABLE_FLAG_FIRST)?"FIRST":"!FIRST"), uSize, (bPing?"PNG":"MIC")));
                #endif
...
static void _VoipConnectionConn(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, uint32_t uTick)
{
    ...
    memset(&ConnPacket, 0, sizeof(VoipConnPacketT));
...
static void _VoipConnectionPing(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, uint32_t uTick)
{
    ...
    pWrite = _VoipReliableDataOutProcess(pConnectionlist, (pConnection - &pConnectionlist->pConnections[0]), &PingPacket.Head, &PingPacket.aData[0]);

    // send a 'ping' packet
    pConnection->iSendResult = _VoipSocketSendto(pConnectionlist->pSocket, pConnectionlist->uClientId, &PingPacket.Head, (pWrite - (uint8_t *)&PingPacket), 0, (struct sockaddr *)&pConnection->SendAddr, sizeof(pConnection->SendAddr));
...
static uint32_t _VoipConnectionIsSendPktReady(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, int32_t iUserIndex, uint32_t uTick)
{
    ...
    if (pVoipPkt->MicrInfo.uNumSubPackets == VOIP_MAXSUBPKTS_PER_PKT)
    {
        return(TRUE);
    }
...
static void _VoipConnectionSendSingle(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, int32_t iUserIndex, uint32_t uTick, uint32_t bFlush)
{
    ...
    if (bSendPacket == TRUE)
    {
        ...
        pConnection->iSendResult = _VoipSocketSendto(pConnectionlist->pSocket, pConnectionlist->uClientId, &pPacket->Head,
        ...
        memset(&pPacket->MicrInfo, 0, sizeof(pPacket->MicrInfo));       // clear micr packet info
...
static void _VoipConnectionRecvSingle(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, struct sockaddr_in *pSendAddr, VoipPacketBufferT *pPacket, int32_t iSize)
{
    ...
    switch (uPktType)
    {
        ...
        case 'P':
        {
            ...
            memset(strGlobalVoipChannelConfig, 0, sizeof(strGlobalVoipChannelConfig));
            memset(strUserVoipChannelConfig, 0, sizeof(strUserVoipChannelConfig));
        ...
        case 'M':
        {
            ...
            if (pConnection->bConnPktRecv)
            {
                ...
                if (pConnectionlist->uRecvMask & (1 << iConnID))
                {
                    ...
                    pConnectionlist->pVoiceCb((VoipUserT *)&pConnection->RemoteUsers[0], &pMicrPacket->MicrInfo, pRead, pConnectionlist->pCbUserData);
                }
...
static void _VoipConnectionStop(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, int32_t iConnID, int32_t bSendDiscMsg)
{
    ...
    memset(&pVoipCommon->uRemoteChannelSelection[iConnID], 0, sizeof(pVoipCommon->uRemoteChannelSelection[iConnID]));
...
int32_t VoipConnectionStartup(VoipConnectionlistT *pConnectionlist, int32_t iMaxPeers)
{
    ...
    // initialize outbound reliable data sequence number
    pConnectionlist->uOutSeqNb = 1;
...
int32_t VoipConnectionStart(VoipConnectionlistT *pConnectionlist, int32_t iConnID, uint32_t uAddr, uint32_t uConnPort, uint32_t uBindPort, uint32_t uClientId, uint32_t uSessionId)
{
    ...
    */
    ...
    NetPrintf(("voipconnection: [%d] connecting to %a:%d clientId=0x%08x\n", iConnID,
        SocketNtohl(pConnection->SendAddr.sin_addr.s_addr), SocketNtohs(pConnection->SendAddr.sin_port),
...
void VoipConnectionSend(VoipConnectionlistT *pConnectionlist, const uint8_t *pVoiceData, int32_t iDataSize, const uint8_t *pMetaData, int32_t iMetaDataSize, uint32_t uUserIndex, uint8_t uSendSeqn)
{
    ...
    for (iConnID = 0; iConnID < pConnectionlist->iMaxConnections; iConnID++)
    {
        ...
        if ((pConnection->eState == ST_ACTV) && (pConnectionlist->uSendMask & (1 << iConnID)))
        {
            ...
            VoipMicrPacketT *pMicrPacket = &pConnection->VoipMicrPacket[uUserIndex]; // ref packet
            ...
            if (pMicrPacket->MicrInfo.uNumSubPackets == 0)
            {
                ...
                if (pConnection->iVoipServerConnId != VOIP_CONNID_NONE)
                {
                    ...
                    pWrite = _VoipReliableDataOutProcess(pConnectionlist, VOIP_CONNID_ALL, &pMicrPacket->Head, &pMicrPacket->aData[0]);
                }
                else
                {
                    // p2p packet
                    pWrite = _VoipReliableDataOutProcess(pConnectionlist, iConnID, &pMicrPacket->Head, &pMicrPacket->aData[0]);
                }
            ...
            {
                NetPrintf(("voipconnection: [%d] critical error - no more space for a sub-packet in the voip packet (sub-packet count = %d)\n", iConnID, pMicrPacket->MicrInfo.uNumSubPackets));
            }
...
void VoipConnectionRegisterRemoteTalkers(VoipConnectionlistT *pConnectionlist, int32_t iConnID, uint32_t bRegister)
{
    ...
    for (iUser = 0; iUser < VOIP_MAXLOCALUSERS_EXTENDED; iUser++)
    {
        ...
        // register the user
        pConnectionlist->pRegUserCb((VoipUserT *)&pConnection->RemoteUsers[iUser], bRegister, pConnectionlist->pCbUserData);
