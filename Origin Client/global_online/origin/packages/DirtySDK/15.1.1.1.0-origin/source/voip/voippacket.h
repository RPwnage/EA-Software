...
//! maximum voice sub-packets in a single voip packet
...
//! maximum size of a single reliable data entry (in bytes)
#define VOIP_MAXRELIABLEDATA        (512)
...
typedef struct ReliableDataInfoT
{
    ...
    uint8_t uType;          //!< type (VOIP_RELIABLE_TYPE_USERS or VOIP_RELIABLE_TYPE_OPAQUE)
...
typedef struct ReliableAckT
{
    ...
    uint8_t     uAckedSeq;          //!< bit 7 = ACKCNF flag; bit 0 to 6 = [0,127] seq number
} ReliableAckT;
