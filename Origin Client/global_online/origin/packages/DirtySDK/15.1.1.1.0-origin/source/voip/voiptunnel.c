...
typedef struct VoipPacketHeadT
{
    ...
    uint8_t aFlags;             //!< field to store system state ie. headset status.
    ...
    for(iIndex = 0; iIndex < VOIPTUNNEL_MAXGROUPSIZE; iIndex++)
    {
        ...
        if ((pClient->uSendMask) & (1 << iIndex))
        {
            ...
            if (pTargetClient) 
            {
                ...
                {
                    pTargetClient->uFlags &= ~VOIPTUNNEL_CLIENTFLAG_MAX_VOICES_REACHED;
                    _VoipTunnelEventCallback(pVoipTunnel, VOIPTUNNEL_EVENT_AVLBVOICE, pTargetClient, pClient->iGameIdx);
...
void VoipTunnelClientListDel(VoipTunnelRefT *pVoipTunnel, VoipTunnelClientT *pClient, int32_t iClient)
{
    ...
    if (pClient != NULL)
    {
        ...
        iClient = (int32_t)(pClient - pVoipTunnel->ClientList);
    }
