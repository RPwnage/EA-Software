...
struct VoipHeadsetRefT
{
    ...
    uint32_t uHeadsetStatus[VOIP_MAXLOCALUSERS];
...
static void _VoipHeadsetPollForVoiceData(VoipHeadsetRefT *pHeadset)
{
    ...
    while ((iPort = _VoipHeadsetGetLocalChatData(pHeadset, aBuffer, sizeof(aBuffer))) >= 0)
    {
        ...
        pHeadset->pMicDataCb(aBuffer, sizeof(aBuffer),  NULL, 0, iPort, pHeadset->uSendSeq++, pHeadset->pCbUserData);
    }
}
...
static void _VoipHeadsetUpdateStatus(VoipHeadsetRefT *pHeadset)
{
    ...
    for (iPort = 0; iPort < VOIP_MAXLOCALUSERS; iPort++)
    {
        ...
        #if DIRTYCODE_LOGGING
        if (pHeadset->uHeadsetStatus[iPort] ^ bHeadsetPresent)
        {
            NetPrintf(("voipheadsetxenon: headset in port %d %s\n", iPort, bHeadsetPresent ? "inserted" : "removed"));
        }
        #endif
        
        // trigger callback
        pHeadset->pStatusCb(iPort, bHeadsetPresent, VOIP_HEADSET_STATUS_INOUT, pHeadset->pCbUserData);

        // save updated status
        pHeadset->uHeadsetStatus[iPort] = bHeadsetPresent;
    }
}
...
*/
/********************************************************************************F*/
static void _VoipHeadsetRegisterRemoteTalker(VoipHeadsetRefT *pHeadset, VoipUserT *pRemoteUser, uint32_t bRegister)
{
    ...
    if (bRegister)
    {
        ...
        for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex++)
        {
            NetPrintf(("voipheasetxenon: enabling playback of voice from remote user '%s' for local user %d because that remote user has just been registered\n",
                pRemoteUser->strUserId, iLocalUserIndex));
            pXHVEngine->SetPlaybackPriority(Xuid, iLocalUserIndex, XHV_PLAYBACK_PRIORITY_MAX);
        }
    }
    else
    {
        for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex++)
        {
            NetPrintf(("voipheasetxenon: disabling playback of voice from remote user '%s' for local user %d because that remote user is about to be unregistered\n",
                pRemoteUser->strUserId, iLocalUserIndex));
            pXHVEngine->SetPlaybackPriority(Xuid, iLocalUserIndex, XHV_PLAYBACK_PRIORITY_NEVER);
        }
        NetPrintf(("voipheadsetxenon: unregistering remote talker %s\n", pRemoteUser->strUserId));
...
static int32_t _VoipHeadsetRegisterLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iPortID, uint32_t bRegister, VoipUserT *pVoipUser)
{
    ...
    if (bRegister)
    {
        ...
        {
            // enable voice chat
            pHeadset->pXHVEngine->StartLocalProcessingModes(iPortID, &XHV_VOICECHAT_MODE, 1);   //i think we skip this if we don't want to start voip, but do want the headset status
    ...
    else
    {
        ...
        pHeadset->pXHVEngine->StopLocalProcessingModes(iPortID, &XHV_VOICECHAT_MODE, 1);    //this too
...
void VoipHeadsetReceiveVoiceDataCb(VoipUserT *pRemoteUsers, VoipMicrInfoT *pMicrInfo, uint8_t *pPacketData, void *pUserData)
{
    ...
    for (uMicrPkt = 0; uMicrPkt < pMicrInfo->uNumSubPackets; uMicrPkt++)
    {
        ...
        _VoipHeadsetSubmitVoiceData(pHeadset, &pRemoteUsers[pMicrInfo->uUserIndex], 1, pSubPacket);
    }
}
...
/*!
    ...
    \Description
        Connectionlist callback to register/unregister a new user with the XHV Engine.
...
{
    VoipHeadsetRefT *pHeadset = (VoipHeadsetRefT *)pUserData;
    _VoipHeadsetRegisterRemoteTalker(pHeadset, pRemoteUser, bRegister);
}
...
/*!
    ...
    \Notes
        ...
        \verbatim
            'ruid' - always return TRUE because MLU is not supported on Xenon
        \endverbatim
