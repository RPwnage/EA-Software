...
// default params
#define VOIP_HEADSET_DEFAULTSAMPLERATE          (16000)      //!< default sample rate in hz
...
int32_t _VoipHeadsetPS3_iDefaultCodecIdent = 'celp'; 
int32_t _VoipHeadsetPS3_iDefaultSampleRate = VOIP_HEADSET_DEFAULTSAMPLERATE ;
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetUnloadCelpModules

    \Description
        Unload the CELP encoder and decoder modules

    \Output
        int32_t         - negative=error, zero=success

    \Version 08/02/2012 (akirchner)
*/
/********************************************************************************F*/

static int32_t _VoipHeadsetUnloadCelpModules(VoipHeadsetRefT *pHeadset)
{
    if(NetConnControl('uprx', CELL_SYSMODULE_CELPENC, 0, NULL, NULL) < 0)
    {
        NetPrintf(("voipheadsetps3: Failed to unload celp encoder module\n"));
        return(-1);
    }
    else
    {
        NetPrintf(("voipheadsetps3: Unloaded celp encoder module\n"));
    }

    if(NetConnControl('uprx', CELL_SYSMODULE_ADEC_CELP, 0, NULL, NULL) < 0)
    {
        NetPrintf(("voipheadsetps3: Failed to unload celp decoder module\n"));
        return(-1);
    }
    else
    {
        NetPrintf(("voipheadsetps3: Unloaded celp decoder module\n"));
    }
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetUnloadCel8Modules

    \Description
        Unload the CEL8 encoder and decoder modules
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetUnloadCel8Modules(VoipHeadsetRefT *pHeadset)
{
    if(NetConnControl('uprx', CELL_SYSMODULE_CELP8ENC, 0, NULL, NULL) < 0)
    {
        NetPrintf(("voipheadsetps3: Failed to unload celp8 encoder module\n"));
        return(-1);
    }
    else
    {
        NetPrintf(("voipheadsetps3: Unloaded celp8 encoder module\n"));
    }

    if(NetConnControl('uprx', CELL_SYSMODULE_ADEC_CELP8, 0, NULL, NULL) < 0)
    {
        NetPrintf(("voipheadsetps3: Failed to unload celp8 decoder module\n"));
        return(-1);
    }
    else
    {
        NetPrintf(("voipheadsetps3: Unloaded celp8 decoder module\n"));
    }
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetLoadCelpModules

    \Description
        Load the CELP encoder and decoder modules
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetLoadCelpModules(VoipHeadsetRefT *pHeadset)
{
    if(NetConnControl('lprx', CELL_SYSMODULE_CELPENC, 0, NULL, NULL) < 0)
    {
        NetPrintf(("voipheadsetps3: Failed to load celp encoder module\n"));
        return(-1);
    }
    else
    {
        NetPrintf(("voipheadsetps3: Loaded celp encoder module\n"));
    }

    if(NetConnControl('lprx', CELL_SYSMODULE_ADEC_CELP, 0, NULL, NULL) < 0)
    {
        NetPrintf(("voipheadsetps3: Failed to load celp decoder module\n"));
        return(-1);
    }
    else
    {
        NetPrintf(("voipheadsetps3: Loaded celp decoder module\n"));
    }
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetLoadCel8Modules

    \Description
        Load the CEL8 encoder and decoder modules
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetLoadCel8Modules(VoipHeadsetRefT *pHeadset)
{
    if(NetConnControl('lprx', CELL_SYSMODULE_CELP8ENC, 0, NULL, NULL) < 0)
    {
        NetPrintf(("voipheadsetps3: Failed to load celp8 encoder module\n"));
        return(-1);
    }
    else
    {
        NetPrintf(("voipheadsetps3: Loaded celp8 encoder module\n"));
    }

    if(NetConnControl('lprx', CELL_SYSMODULE_ADEC_CELP8, 0, NULL, NULL) < 0)
    {
        NetPrintf(("voipheadsetps3: Failed to load celp8 decoder module\n"));
        return(-1);
    }
    else
    {
        NetPrintf(("voipheadsetps3: Loaded celp8 decoder module\n"));
    }
...
static void *_VoipHeadsetAudioWrite(VoipHeadsetRefT *pHeadset, void *pSourceData)
{
    ...
    ds_memcpy((void*)pHeadset->pNextWriteLoc, pSourceData, VOIP_HEADSET_BLOCK_SIZE);
...
static void _VoipHeadsetMicOpen(VoipHeadsetRefT *pHeadset)
{
    ...
    pHeadset->pStatusCb(0, TRUE, VOIP_HEADSET_STATUS_INOUT, pHeadset->pCbUserData);
...
static float _VoipHeadsetAudioUpsample8To48(float fPrevSample, float fCurrSample, float *pOutSamples)
{
    ...
    pOutSamples[6]  = fPrevSample * f3Sixths + fScaledCurr * f3Sixths;
...
static int32_t _VoipHeadsetSetCodec(VoipHeadsetRefT *pHeadset, int32_t iCodecIdent)
{
    ...
    {
        case 'celp':
            if((_VoipHeadsetUnloadCel8Modules(pHeadset) < 0) || (_VoipHeadsetLoadCelpModules(pHeadset) < 0))
            {
                NetPrintf(("voipheadsetps3: Failed to load to CELP codec\n"));
                return(-1);
            }
        ...
        case 'cel8':
            if((_VoipHeadsetUnloadCelpModules(pHeadset) < 0) || (_VoipHeadsetLoadCel8Modules(pHeadset) < 0))
            {
                NetPrintf(("voipheadsetps3: Failed to load to CEL8 codec\n"));
                return(-1);
            }
        ...
        case 'dvid':
        case 'lpcm':
            if((_VoipHeadsetUnloadCelpModules(pHeadset) < 0) || (_VoipHeadsetUnloadCel8Modules(pHeadset) < 0))
            {
                NetPrintf(("voipheadsetps3: Failed to unload to CELP or CEL8 codec\n"));
                return(-1);
            }
        ...
        default:
            NetPrintf(("voipheadsetps3: Unknown codec type\n"));
...
static void _VoipHeadsetMicProcess(VoipHeadsetRefT *pHeadset)
{
    ...
    if (pHeadset->bRecording)
    {
        ...
        if (!cellMicIsAttached(0))
        {
            ...
            pHeadset->pStatusCb(0, FALSE, VOIP_HEADSET_STATUS_INOUT, pHeadset->pCbUserData);
        }
        ...
        for (iNumSamples = _VoipHeadsetMicRead(pHeadset); iNumSamples >= pHeadset->Param.iFrameSamples; iNumSamples -= pHeadset->Param.iFrameSamples)
        {
            ...
            if (pHeadset->bLoopback == FALSE)
            {
                ...
                pHeadset->pMicDataCb(&pMicrPacket->aData, iCompBytes,  NULL, 0, 0, pHeadset->uSendSeq++, pHeadset->pCbUserData);
            ...
            if (!bExit)
            {
                ...
                if (iMixedBytes == (pHeadset->Param.iFrameSamples * 2))
                {
                    ...
                    if (pHeadset->Param.iUpsampleRate == 6)
                    {
                        ...
                        {
                            fPrevSample = _VoipHeadsetAudioUpsample16To48(fPrevSample, pHeadset->aFrameData[iIndex], &fTranslatedBuffer[iIndex*6]);
                        }
                    }
                    else if (pHeadset->Param.iUpsampleRate == 12)
                    {
                        for (iIndex = 0; iIndex < iNumSamples; iIndex += 1)
                        {
                            fPrevSample = _VoipHeadsetAudioUpsample8To48(fPrevSample, pHeadset->aFrameData[iIndex], &fTranslatedBuffer[iIndex*12]);
                        }
                    }
...
/*!
    ...
    \Input iMaxLocal    - max number of local users (ignored because only primary user can do voip on PS3)
...
*/
/********************************************************************************F*/
VoipHeadsetRefT *VoipHeadsetCreate(int32_t iMaxConduits, int32_t iMaxLocal, VoipHeadsetMicDataCbT *pMicDataCb, VoipHeadsetStatusCbT *pStatusCb, void *pCbUserData, int32_t iData)
{
    ...
    // set up to use celp codec with 16khz audio by default (note this also will allocate the mixer)
    pHeadset->Param.iSampleRate = _VoipHeadsetPS3_iDefaultSampleRate;
    _VoipHeadsetSetCodec(pHeadset, _VoipHeadsetPS3_iDefaultCodecIdent);
...
void VoipHeadsetDestroy(VoipHeadsetRefT *pHeadset)
{
    ...
    // unload codec modules
    if(_VoipHeadsetUnloadCel8Modules(pHeadset) < 0)
    {
        NetPrintf(("voipheadsetps3: Failed to unload CEL8 modules\n"));
    }

    if(_VoipHeadsetUnloadCelpModules(pHeadset) < 0)
    {
        NetPrintf(("voipheadsetps3: Failed to unload CELP modules\n"));
    }
}
...
void VoipHeadsetReceiveVoiceDataCb(VoipUserT *pRemoteUsers, VoipMicrInfoT *pMicrInfo, uint8_t *pPacketData, void *pUserData)
{
    ...
    if (pMicrInfo->uSubPacketSize != pHeadset->Param.iCmpFrameSize)
    {
        NetPrintf(("voipheadsetps3: discarding voice packet with %d voice bundles and mismatched sub-packet size %d (expecting %d)\n",
            pMicrInfo->uNumSubPackets, pMicrInfo->uSubPacketSize, pHeadset->Param.iCmpFrameSize));
        return;
    }
...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    if (iControl == 'aloc')
    {
        ...
        NetPrintf(("voipheadsetps3: %s participating state\n", pHeadset->bParticipating ? "entering" : "exiting"));
    ...
    if (iControl == 'dcde')
    {
        NetPrintf(("voipheadsetps3: modified default codec from %C to %C\n", _VoipHeadsetPS3_iDefaultCodecIdent, iValue));
        _VoipHeadsetPS3_iDefaultCodecIdent = iValue;
        return(0);
    }
    if (iControl == 'drat')
    {
        NetPrintf(("voipheadsetps3: modified default sample rate from %d to %d\n", _VoipHeadsetPS3_iDefaultSampleRate, iValue));
        _VoipHeadsetPS3_iDefaultSampleRate = iValue;
        return(0);
    }
    if (iControl == 'loop')
...
/*!
    ...
    \Notes
        ...
        \verbatim
            'ruid' - always return TRUE because MLU is not supported on PS3
        \endverbatim
