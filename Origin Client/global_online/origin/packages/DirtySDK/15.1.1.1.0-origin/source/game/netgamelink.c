...
/*!
    ...
    \Input uRPackSent       - count of packets sent by remote peer (cumulative psnt values of all received sync packets)
    ...
    \Notes
        \verbatim
        This function returns the number of packets lost at this end of the connection since the beginning. It is obtained by
        subtracting uLPackRcvd (the number of packets sent by the remote end - cumulative psnt values of all received sync packets)
        from uLPackSent (the number of packets received locally).

        The following two scenarios indicate that a sync packet itself suffered packet loss, and it relied on the packet resending
        mechanisms of commudp to finally make it to us:
            1 - uLPackRcvd is larger than uLPackSent (subtracting them would return a negative value)
            2 - uLPackRcvd is smaller than uLPackSent, but the last saved count of packets lost is larger than the new one.

        For both scenarios, we skip updating the packets lost count to avoid a possibly negative or under-valued packet loss calculation,
        and we deal with that iteration as if no packet loss was detected.

        The assumption here is that the next sync packet that makes it to us without being "resent" will end up
        updating the packet lost counters with a "coherent" value as its "psnt" field will include all packets that were used for packet
        retransmission. At the end, this is guaranteeing the coherency of the packets lost count (non negative value and only increasing
        over time) by delaying its update.
        \endverbatim
...
static void _NetGameLinkPrintStats(NetGameLinkRefT *pRef)
{
    ...
    NetPrintf(("netgamelink: QoS Results ------------------------------------------------------------------------------\n"));
    NetPrintf(("netgamelink: latency over QoS period                                                          %d\n", pRef->iQosAvgLate));
    NetPrintf(("netgamelink: latency over last sampling period                                                %d (sampl. prd = %d ms)\n", stats.late, RTT_SAMPLE_PERIOD));
    NetPrintf(("netgamelink: when connection established                                                      %d\n", stats.conn));
    NetPrintf(("netgamelink: when data most recently sent                                                     %d\n", stats.sentlast));
    NetPrintf(("netgamelink: when data most recently received                                                 %d\n", stats.rcvdlast));
    NetPrintf(("netgamelink: ping deviation                                                                   %d\n", stats.pingdev));
    NetPrintf(("netgamelink: ping average                                                                     %d\n", stats.pingavg));
    NetPrintf(("netgamelink: number of bytes sent                                                             %d\n", stats.sent));
    NetPrintf(("netgamelink: number of bytes received                                                         %d\n", stats.rcvd));
    NetPrintf(("netgamelink: number of packets sent to peer since start (at time = last inbound sync pkt)     %d\n", stats.lpacksent));
    NetPrintf(("netgamelink: number of packets sent to peer since start (at time = now)                       %d\n", stats.lpacksent_now));
    NetPrintf(("netgamelink: number of packets received from peer since start                                 %d\n", stats.lpackrcvd));
    NetPrintf(("netgamelink: number of packets sent by peer (to us) since start                               %d\n", stats.rpacksent));
    NetPrintf(("netgamelink: number of packets received by peer (from us) since start                         %d\n", stats.rpackrcvd));
    NetPrintf(("netgamelink: local->remote packets lost: number of packets (from us) lost by peer since start %d\n", stats.rpacklost));
    NetPrintf(("netgamelink: packets saved: number of packets recovered by via redundancy mechanisms          %d\n", stats.lpacksaved));
    NetPrintf(("netgamelink: number of NAKs sent by peer (to us) since start                                  %d\n", stats.rnaksent));
    NetPrintf(("netgamelink: packets per sec sent (user packets)                                              %d\n", stats.outpps));
    NetPrintf(("netgamelink: raw packets per sec sent (packets sent to network)                               %d\n", stats.outrpps));
    NetPrintf(("netgamelink: bytes per sec sent (user data)                                                   %d\n", stats.outbps));
    NetPrintf(("netgamelink: raw bytes per sec sent (data sent to network)                                    %d\n", stats.outrps));
    NetPrintf(("netgamelink: network bytes per sec sent (inrps + estimated UDP/Eth frame overhead)            %d\n", stats.outnps));
    NetPrintf(("netgamelink: packets per sec received (user packets)                                          %d\n", stats.inpps));
    NetPrintf(("netgamelink: raw packets per sec received (packets sent to network)                           %d\n", stats.inrpps));
    NetPrintf(("netgamelink: bytes per sec received (user data)                                               %d\n", stats.inbps));
    NetPrintf(("netgamelink: raw bytes per sec received (data sent to network)                                %d\n", stats.inrps));
    NetPrintf(("netgamelink: network bytes per sec received (inrps + estimated UDP/Eth frame overhead)        %d\n", stats.innps));
    NetPrintf(("netgamelink: ------------------------------------------------------------------------------------------\n"));
}
...
static void _NetGameLinkUpdateQos(NetGameLinkRefT *pRef)
{
    ...
    if ((pRef->uQosPacketsToSend > 0) && (NetTickDiff(now, pRef->uQosLastSendTime) > (signed)pRef->uQosSendInterval))
    {
        ...
        NetPrintf(("netgamelink: [0x%08x] sent QoS packet, size(%d), remaining packets to send (%d)\n", pRef, QosPacket.head.len, pRef->uQosPacketsToSend));
    }
    ...
    while ((pRef->uQosPacketsToRecv > 0) && NetGameLinkRecv(pRef, (NetGamePacketT*)&QosPacket, 1, FALSE))
    {
        ...
        NetPrintf(("netgamelink: [0x%08x] received QoS packet, size(%d), remaining packets to recv (%d)\n", pRef, QosPacket.head.len, pRef->uQosPacketsToRecv));
    }
...
int32_t NetGameLinkPeek2(NetGameLinkRefT *ref, NetGamePacketT **pkt, uint32_t mask)
{
    ...
    if (iBufLen == 0)
    {
        _NetGameLinkNotify(ref->m_port, 0);
    }
    #endif
