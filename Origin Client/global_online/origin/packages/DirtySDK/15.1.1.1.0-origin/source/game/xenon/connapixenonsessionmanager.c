...
/*F*************************************************************************************************/
/*!
    \Function    ConnApiXenonSessionManagerGetSecureIp

    \Description
        Get secure address for specified host
...
*/
/*************************************************************************************************F*/
int32_t ConnApiXenonSessionManagerGetSecureIp(ConnApiXenonSessionManagerRefT *pRef, int32_t *pAddr, const char* pMachineAddress)
{
    int32_t iResult = -1; // default to failure
    ...
    if (DirtySessionManagerGetSecureIp(pRef->pPrimarySessionManager, &iAddrPrimary, pMachineAddress) > 0)
    {
        *pAddr = iAddrPrimary;
        iResult = 1;
    }
...
int32_t ConnApiXenonSessionManagerCreateSess(ConnApiXenonSessionManagerRefT *pRef, uint32_t bRanked, uint32_t *uUserFlags, const char *pSession, DirtyAddrT *pLocalAddrs)
{
    ...
    ds_memcpy(pRef->uSlotType, uUserFlags, sizeof(pRef->uSlotType));
    ds_memcpy(pRef->aLocalAddrs, pLocalAddrs, sizeof(pRef->aLocalAddrs));
    ...
    {
        ds_memcpy(aBuffer1, pSession + 2, iLength1);
        ds_memcpy(aBuffer2, pSession + 2 + iLength1, iLength2);
...
int32_t ConnApiXenonSessionManagerStatus(ConnApiXenonSessionManagerRefT *pRef, int32_t iSelect, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'sess')
    {
        ...
        ds_memcpy(pOutput + 2, aBuffer1, iLength1);
        ds_memcpy(pOutput + 2 + iLength1, aBuffer2, iLength2);
