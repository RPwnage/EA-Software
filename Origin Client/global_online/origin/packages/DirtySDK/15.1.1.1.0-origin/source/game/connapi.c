...
#define CONNAPI_DEMANGLER_TIMEOUT   (20*1000)       // experimentation showed that creation of security association can be significantly long
#else
...
static void _ConnApiInitClientConnectionState(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex, uint32_t uConnMode)
{
    ...
    */
    ...
    {
        // if we're a xboxone client in phcc mode and we are currently dealing with the dedicated server client entry,
        // then make sure we do not attempt to demangle that entry with protomanglexboxone
        if ((pConnApi->bPcHostXboxOneClientMode) && (iClientIndex == pConnApi->iHostIndex || iClientIndex == pConnApi->iSelf))
        {
    ...
    {
        // if voip is routed through a dedicated server (dirtycast or not), then make sure we do not attempt to demangle
        // voip connections with protomanglexboxone
        if ((((pConnApi->uGameServConnMode & CONNAPI_CONNFLAG_VOIPCONN) || pConnApi->bVoipRedirectViaHost)) ||
...
static int32_t _ConnApiTunnelAlloc(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex, uint32_t uRemoteAddr, uint8_t bLocalAddr)
{
    ...
    NetPrintf(("connapi: [0x%08x] setting client %d clientId=%d TunnelInfo.uRemotePort %d TunnelInfo.uTunnelMode %d\n", pConnApi, iClientIndex, TunnelInfo.uRemoteClientId, TunnelInfo.uRemotePort, TunnelInfo.uTunnelMode));
...
static void _ConnApiVoipGroupConnSharingCallback(VoipGroupRefT *pVoipGroup, ConnSharingCbTypeE eCbType, int32_t iConnId, void *pUserData)
{
    ...
    else if (eCbType == VOIPGROUP_CBTYPE_CONNRESUME)
    {
        ...
        VoipGroupResume(pVoipGroup, iConnId, iConnectAddr, pClient->VoipInfo.uMnglPort, pClient->VoipInfo.uLocalPort, pClient->ClientInfo.uId, pConnApi->iSessId);
    }
    else
    {
...
static void _ConnApiInitClientList(ConnApiRefT *pConnApi, ConnApiClientInfoT *pClientList, int32_t iNumClients)
{
    ConnApiClientT *pClient;
    int32_t iClient;
    DirtyAddrT emptyDirtyAddr;

    memset(&emptyDirtyAddr, 0, sizeof(emptyDirtyAddr));
    ...
    for (iClient = 0, pConnApi->ClientList.iNumClients = 0; iClient < iNumClients; iClient++)
    {
        ...
        if (!DirtyAddrCompare(&emptyDirtyAddr, &pClientList[iClient].DirtyAddr))
        {
    ...
    if (pConnApi->iSelf >= 0)
    {
        ...
        if (pConnApi->GameServer.ClientInfo.uAddr == 0)
        {
            ...
            #if defined(DIRTYCODE_XBOXONE) || defined(DIRTYCODE_XBOXONEADK)
...
// on xboxone, we never want to go back to MGNL state from CONN state
#if defined(DIRTYCODE_XBOXONE)
...
static int32_t _ConnApiUpdateVoipClient(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex)
{
    ...
    if (pClient->VoipInfo.eStatus == CONNAPI_STATUS_INIT)
    {
        ...
        iVoipConnId = VoipGroupConnect(pConnApi->pVoipGroupRef, iClientIndex, iConnectAddr, pClient->VoipInfo.uMnglPort, pClient->VoipInfo.uLocalPort, pClient->ClientInfo.uId, pConnApi->iSessId);
        if (iVoipConnId >= 0)
        {
...
#if defined(DIRTYCODE_XBOXONE)
    if (pConnApi->uGameServFallback != 0)
    {
        // because we can failover connectivity to dirtycast upon failure of security association creation,
        // we make the wait period for creation of security associations signicantly shorter
        ConnApiControl(pConnApi, 'dtim', CONNAPI_CONNTIMEOUT_DEFAULT, 0, NULL);
    }
    else
    {
...
int32_t ConnApiControl(ConnApiRefT *pConnApi, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'dtim')
    {
        ...
        {
#if defined(DIRTYCODE_XBOXONE)
            int32_t iTimeout = pConnApi->iMnglTimeout;
#else
