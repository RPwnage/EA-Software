...
#include "DirtySDK/comm/pc/commser.h"
...
static int32_t _CommSerRead(CommSerRef *ref, RawSerPacketT *packet)
{
    ...
    for (need = 0;;) {
        ...
        ds_memcpy(&packet->body, buf+4, pktlen);
        ...
        if (ref->rawlen != 0)
            ds_memcpy(ref->rawbuf, ptr+4, ref->rawlen);
        return(1);
    }
}
...
static int32_t _CommSerWrite(CommSerRef *ref, RawSerPacketT *packet)
{
    ...
    if (packet != NULL) {
        ...
        // copy data
        ds_memcpy(frame+4, &packet->body, len);
...
int32_t CommSerRecover(CommSerRef *ref, const char *addr)
{
    ...
    ds_strnzcpy(device, addr, sizeof(device));
    if (strchr(device, ':'))
    {
        (strchr(device, ':'))[0] = 0;
        addr = strchr(addr, ':')+1;
...
int32_t CommSerPeek(CommSerRef *ref, void *target, int32_t length, uint32_t *when)
{
    ...
    ds_memcpy(target, packet->body.data, (packet->head.len < length ? packet->head.len : length));
    // get the timestamp
    if (when != NULL)
    {
