...
typedef struct
{
    ...
    struct {
        ...
        uint32_t ack;           //!< acknowledgement of last packet
        uint32_t cid;           //!< client id (only included in INIT and POKE packets)   // $todo - considering deprecating this field as it seems unused - metadata mode is conveying that info now (when enabled)
...
static int32_t _CommUDPThreadData(uint32_t currtick)
{
    ...
    for (ref = g_link; ref != NULL; ref = ref->link)
    {
        ...
        if ((ref->snderr == SOCKERR_UNREACH) && ((ref->state == OPEN) || (ref->state == CONN) || (ref->state == LIST))) {
            NetPrintf(("commudp: [0x%08x] closing connection as host is unreachable\n", (uintptr_t)ref));
            NetPrintf(("commudp: [0x%08x] tick=%d, rtick=%d, stick=%d\n", (uintptr_t)ref, currtick, ref->recvtick, ref->sendtick));
            _CommUDPClose(ref, currtick);
            ref->snderr = 0;
        }
