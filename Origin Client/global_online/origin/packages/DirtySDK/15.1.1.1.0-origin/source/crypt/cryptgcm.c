...
static int32_t _CryptGcmStart(CryptGcmT *pGcm, int32_t iMode, const uint8_t *pInitVec, int32_t iIvLen, const uint8_t *pAddData, int32_t iAddLen)
{
    ...
    memset(pGcm->aY, 0, sizeof(pGcm->aY));
    memset(pGcm->aBuf, 0, sizeof(pGcm->aBuf));
    ...
    {
        // generate y0
        memcpy(pGcm->aY, pInitVec, iIvLen);
        pGcm->aY[15] = 1;
    }
    else
    {
...
static int32_t _CryptGcmFinish(CryptGcmT *pGcm, uint8_t *pTag, int32_t iTagLen)
{
    ...
    if (iTagLen > 0)
    {
        memcpy(pTag, pGcm->aBaseEctr, iTagLen);
    }

    if (uLen || uAddLen)
    {
        memset(aTagBuf, 0, sizeof(aTagBuf));
