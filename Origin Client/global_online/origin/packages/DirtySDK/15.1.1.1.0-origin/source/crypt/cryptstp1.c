...
{
    ds_memcpy(pMasterSecret, pShared->strSecret, sizeof(pShared->strSecret));
    ds_memcpy(pMasterSecret + sizeof(pShared->strSecret), pTicket->Plain.IV, sizeof(pTicket->Plain.IV));
    ds_memcpy(pMasterSecret + sizeof(pShared->strSecret) + sizeof(pTicket->Plain.IV), pShared->strSecret, sizeof(pShared->strSecret));
...
void CryptStp1SetShared(CryptStp1SharedT *pShared, const void *pData, uint32_t uDataSize)
{
    ...
    {
        ds_memcpy(pShared->strSecret, pData, sizeof(pShared->strSecret));
    }
    else
    {
...
void CryptStp1MakeWallet(CryptStp1WalletT *pWallet, CryptStp1SharedT *pShared, uint32_t uTime, const char *pRandom)
{
    ...
    ds_memcpy_s(&pWallet->Ticket.Encrypted.Secret, sizeof(pWallet->Ticket.Encrypted.Secret), &pWallet->Secret, sizeof(pWallet->Secret));
...
int32_t CryptStp1OpenWallet(CryptStp1WalletT *pWallet, CryptStp1SecretT *pSecret, CryptStp1TicketT *pTicket)
{
    ...
    if (pSecret != NULL)
    {
        ds_memcpy_s(pSecret, sizeof(*pSecret), &pWallet->Secret, sizeof(pWallet->Secret));
    }
    if (pTicket != NULL)
    {
        ds_memcpy_s(pTicket, sizeof(*pTicket), &pWallet->Ticket, sizeof(pWallet->Ticket));
    }
    return(1);
}
...
int32_t CryptStp1UseTicket(CryptStp1StreamT *pStream, CryptStp1TicketT *pTicket, CryptStp1SharedT *pShared, uint32_t uTime)
{
    ...
    // decrypt the ticket
    ds_memcpy_s(&Ticket, sizeof(Ticket), pTicket, sizeof(*pTicket));
