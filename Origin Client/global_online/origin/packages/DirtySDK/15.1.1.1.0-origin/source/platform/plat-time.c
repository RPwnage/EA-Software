...
#include <xtl.h>
#elif defined(DIRTYCODE_PC) || defined(DIRTYCODE_XBOXONE) || defined(DIRTYCODE_WINRT)
#include <windows.h>
#include <time.h>
#include <sys/types.h>
#include <sys/timeb.h>
#elif defined (DIRTYCODE_LINUX) || defined(DIRTYCODE_ANDROID) || defined(DIRTYCODE_APPLEIOS)
#include <sys/time.h>
#endif
...
/*!
    ...
    \Input iDigits      - max number of digits to convert (0=no limit)
...
*/
/********************************************************************************F*/
static const char *_ds_strtoint(const char *pStr, int32_t *pValue, int32_t iDigits)
{
    int32_t iNum, iNumDigits = (iDigits > 0) ? iDigits : 0x7fffffff;
    for (iNum = 0, iNumDigits = 0; ((*pStr >= '0') && (*pStr <= '9') && (iNumDigits < iDigits)); pStr += 1, iNumDigits += 1)
    {
...
*/
/********************************************************************************F*/
struct tm *ds_localtime(struct tm *tm, uint64_t elap)
{
    ...
    #elif defined(DIRTYCODE_PS4)
    localtime_s(&uTimeT, tm);
    return(tm);
    #elif defined(DIRTYCODE_PC) || defined(DIRTYCODE_XENON) || defined(DIRTYCODE_XBOXONE)
...
*/
/********************************************************************************F*/
struct tm *ds_secstotime(struct tm *tm, uint64_t elap)
{
    ...
    // calculate weekday
    year -= tm->tm_mon < 3;
    tm->tm_wday = (year + year/4 - year/100 + year/400 + wday[tm->tm_mon] + tm->tm_mday) % 7;
    // clear dst
    tm->tm_isdst = 0;
...
*/
/********************************************************************************F*/
uint64_t ds_timetosecs(const struct tm *tm)
{
...
/*!
    ...
    \Notes
        ...
        Data type to be used with plattime:
            On PS3:      CellRtcTick
            On Xenon:    FILETIME
            On PS4:      SceKernelTimeval
            On PC:       __timeb64
            On Xbox One: SYSTEMTIME
            On WinRT:    SYSTEMTIME
            On Linux:    timespec
        \endverbatim
...
struct tm *ds_plattimetotime(struct tm *tm, void *pPlatTime)
{
    ...
    localtime_s(&kernelTime.tv_sec, tm);
    ...
    tm->tm_sec = resultTm.tm_sec;
    tm->tm_min = resultTm.tm_min;
    tm->tm_hour = resultTm.tm_hour;
    tm->tm_mday = resultTm.tm_mday;
    tm->tm_mon = resultTm.tm_mon;
    tm->tm_year = resultTm.tm_year;
    tm->tm_wday = resultTm.tm_wday;
    tm->tm_yday = resultTm.tm_yday;
    tm->tm_isdst =resultTm.tm_isdst;

#elif defined(DIRTYCODE_XBOXONE) || defined(DIRTYCODE_WINRT)
...
/*!
    ...
    \Input *tm          - generic time data structure to be filled by the function
    \Input *imsec       - output param for milisecond to be filled by the function (optional can be NULL)
...
*/
/********************************************************************************F*/
struct tm *ds_plattimetotimems(struct tm *tm , int32_t *imsec)
{
    void *pPlatTime;
    int32_t iMsec;
    ...
    return(ds_plattimetotime(tm, pPlatTime));
}
...
uint64_t ds_strtotime2(const char *pStr, TimeToStringConversionTypeE eConvType)
{
    ...
    if (eConvType == TIMETOSTRING_CONVERSION_ISO_8601)
    {
        ...
        pStr = _ds_strtoint(pStr, &tm.tm_sec, 2) + 1;      // get the second, skip ':'
    ...
    else if ((eConvType == TIMETOSTRING_CONVERSION_ASN1_UTCTIME) || (eConvType == TIMETOSTRING_CONVERSION_ASN1_GENTIME))
    {
        ...
        pStr = _ds_strtoint(pStr, &tm.tm_sec, 2);
        // adjust year and month
        tm.tm_year -= 1900;
        tm.tm_mon -= 1;
        // convert from struct tm to uint32_t and return to caller
