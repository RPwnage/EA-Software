...
#undef DIRTYSDK_SECURE_VALIDATE
...
static char *_ds_sockaddrtostr(char *pBuffer, int32_t iBufSize, struct sockaddr *pSockAddr)
{
    ...
    {
        ds_snzprintf(pBuffer, iBufSize, "[%a]:%d", SockaddrInGetAddr(pSockAddr), SockaddrInGetPort(pSockAddr));
    }
    if (pSockAddr->sa_family == AF_INET6)
    {
        const uint16_t aIpv4Prefix[] = { 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0xffff };
        struct sockaddr_in6 *pSockAddr6 = (struct sockaddr_in6 *)pSockAddr;
        if (!memcmp(aIpv4Prefix, pSockAddr6->sin6_addr.s6_addr, sizeof(aIpv4Prefix)))
        {
            ds_snzprintf(pBuffer, iBufSize, "[::ffff:%a]:%d", SockaddrIn6GetAddr4(pSockAddr6), SocketHtons(pSockAddr6->sin6_port));
        }
        else
        {
            uint16_t *pWords = (uint16_t *)pSockAddr6->sin6_addr.s6_addr;
            ds_snzprintf(pBuffer, iBufSize, "[%x:%x:%x:%x:%x:%x:%x:%x]:%d",
                SocketHtons(pWords[0]), SocketHtons(pWords[1]), SocketHtons(pWords[2]), SocketHtons(pWords[3]),
                SocketHtons(pWords[4]), SocketHtons(pWords[5]), SocketHtons(pWords[6]), SocketHtons(pWords[7]),
                SocketHtons(pSockAddr6->sin6_port));
        }
    }
