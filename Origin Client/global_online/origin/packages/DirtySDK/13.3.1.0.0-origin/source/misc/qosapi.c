...
typedef struct QosApiServiceReqT
{
    ...
    char aRecvBuf[4096];                                    //!< buffer to store the http response data in
...
static QosApiRequestT *_QosApiCreateRequest(QosApiRefT *pQosApi, uint32_t uRequestType)
{
    ...
    if (uRequestType == TP_SERVICE)
    {
        ...
        pQosApiRequest->pService->pProtoHttp = ProtoHttpCreate(4096);
        DirtyMemGroupLeave();
        
        if (pQosApiRequest->pService->pProtoHttp == NULL)
        {
...
static void _QosApiFormatServiceUrl(QosApiRefT *pQosApi, QosApiRequestT *pRequest, char *pBuffer, int32_t iBufSize, int32_t iType)
{
    ...
    {
        ds_snzprintf(pBuffer, iBufSize, "http://%s:%u/qos/firewall?vers=%d", pRequest->pService->strServiceUrl, pQosApi->uServicePort, QOSAPI_HTTP_VERSION);
    ...
    else if (iType == QOSAPI_BLAZE_FIRETYPE)
    {
        ...
        ds_snzprintf(pBuffer, iBufSize, "http://%s:%u/qos/firetype?vers=%d", pRequest->pService->strServiceUrl, pQosApi->uServicePort, QOSAPI_HTTP_VERSION);
    ...
    {
        ds_snzprintf(pBuffer, iBufSize, "http://%s:%u/qos/qos?vers=%d", pRequest->pService->strServiceUrl, pQosApi->uServicePort, QOSAPI_HTTP_VERSION);
...
static int32_t _QosApiParseHttpResponse(QosApiRefT *pQosApi, QosApiRequestT *pRequest)
{
    ...
    if ((pXml = XmlFind(pRequest->pService->aRecvBuf, "firewall")) != NULL)
    {
        uint32_t uInterface;
        const char *pXml3;
    ...
    else if ((pXml = XmlFind(pRequest->pService->aRecvBuf, "firetype")) != NULL)
    {
    ...
    else if ((pXml = XmlFind(pRequest->pService->aRecvBuf, "qos")) != NULL)
    {
...
static void _QosApiSendLatencyProbe(QosApiRefT *pQosApi, QosApiRequestT *pRequest)
{
    ...
    // check for completion, we use >= to handle a rare case where timed out probes arrive 
    // when we had all ready reissued probes to replace them; on completion iProbesSent can jump past uNumProbes
...
static void _QosApiSendFirewallProbeSet(QosApiRefT *pQosApi, QosApiRequestT *pRequest)
{
    ...
    for (uInterface = 0; uInterface < pRequest->pService->uNumInterfaces; uInterface++)
    {
     ...
     // check for completion, we use >= to match how it is done _QosApiSendLatencyProbe, incase similar timing issue exists
...
static void _QosApiRecvResponse(QosApiRefT *pQosApi, char *pPacketData, struct sockaddr *pRecvAddr, uint32_t uRequestId)
{
    ...
    if (uServiceReqId < 2)
    {
        ...
        if (pRequest->iValidRTTs >= QOSAPI_MAXPROBES)
        { 
            NetPrintf(("qosapi: ignoring probe, received QOSAPI_MAXPROBES already.\n"));
            return;
        }
    ...
    else if ((pRequest->pService->uServiceReqId == uServiceReqId) && (pRequest->pService->iServiceReqSecret == iServiceReqSecret))
    {
        ...
        // TODO this isn't good if we lose the last probe we dont do transition of FW
        // No use checking if >= for BW probes because we don't reissue them, lost probes reduce BW
                ...
                else
                {
                    if (pRequest->pService->eServiceState == SS_PROTONAME)
                    {
                        ...
                        else if (iAddress != 0)
                        {
                            ...
                            ds_strnzcpy(pRequest->pService->strServiceUrl, SocketInAddrGetText(pRequest->uTargetAddr, strAddrText, sizeof(strAddrText)), sizeof(pRequest->pService->strServiceUrl));
...
uint32_t QosApiServiceRequest(QosApiRefT *pQosApi, char *pServerAddress, uint16_t uProbePort, uint32_t uServiceId, uint32_t uNumProbes, uint32_t uBitsPerSec, uint32_t uFlags)
{
    ...
    if (pQosApi->pSocket == NULL)
    {
        ...
        SocketControl(pQosApi->pSocket, 'rbuf', 16*1024, NULL, NULL);
