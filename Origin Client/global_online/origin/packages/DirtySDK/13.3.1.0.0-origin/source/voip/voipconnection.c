...
#define VOIP_PACKET_VERSION     ('f')           //!< current packet version
...
static void _VoipConnectionConn(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, uint32_t uTick)
{
    ...
    NetPrintf(("voipconnection: [%d] (sess id 0x%08x) sending connect packet to user '%s' (%a:%d)\n",
...
static void _VoipConnectionInit(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, int32_t iConnID)
{
    ...
    // now that we know who the remote talkers are for this connection (obtained in the CONN packet)
...
static uint32_t _VoipConnectionIsSendPktReady(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, int32_t iUserIndex, uint32_t uTick)
{
    ...
    if (((pVoipPkt->MicrInfo.uNumSubPackets + 1) * pVoipPkt->MicrInfo.uSubPacketSize) > (signed)sizeof(pVoipPkt->aData))
    {
        return(TRUE);
    }
...
static void _VoipConnectionSendSingle(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, int32_t iUserIndex, uint32_t uTick, uint32_t bFlush)
{
    ...
    if (bSendPacket == TRUE)
    {
        ...
        {
            NetPrintf(("voipconnection: [%d] (sess id 0x%08x) local usr %d sending pkt (seq#:%d) with %d sub-pkt(s) of size %d (bytes) to %a:%d | voip channels:0x%08x\n",
                ...
                pPacket->MicrInfo.uSubPacketSize, SocketNtohl(pConnection->SendAddr.sin_addr.s_addr), SocketNtohs(pConnection->SendAddr.sin_port),
                pConnectionlist->aChannels[iUserIndex]));
        }
        #endif

        // send voice packet
        iPacketSize = sizeof(pConnection->VoipMicrPacket[iUserIndex]) - sizeof(pPacket->aData) + (pPacket->MicrInfo.uNumSubPackets * pPacket->MicrInfo.uSubPacketSize);
        ...
        memset(&pPacket->MicrInfo, 0, sizeof(VoipMicrInfoT));
    }
}
...
static void _VoipConnectionRecvSingle(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, struct sockaddr_in *pSendAddr, VoipPacketBufferT *pPacket, int32_t iSize)
{
    ...
    switch (uPktType)
    {
        ...
        case 'C':
        {
            ...
            NetPrintf(("voipconnection: [%d] (sess id 0x%08x) received conn packet bConnected=%d eState=%d uClientId=0x%08x, user=%s\n",
                iConnID, uSessionId, pConnPacket->bConnected, pConnection->eState, uClientId, pConnPacket->LocalUsers[pConnPacket->iPrimaryLocalUser].strUserId));
        ...
        case 'P':
        {
            ...
            if (pConnection->bConnPktRecv)
            {
                ...
                // update channel config for each remote user
                for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS; iUserIndex++)
                {
        ...
        case 'M':
        {
            ...
            {
                NetPrintf(("voipconnection: [%d] (sess id 0x%08x) received pkt (seq#:%d) with %d sub-pkt(s) of size %d (bytes) from remote usr idx %d | voip channels:0x%08x\n",
                    iConnID, uSessionId, pMicrPacket->MicrInfo.uSeqn, pMicrPacket->MicrInfo.uNumSubPackets, pMicrPacket->MicrInfo.uSubPacketSize, pMicrPacket->MicrInfo.uUserIndex,
                    _VoipDecodeU32(pMicrPacket->MicrInfo.channels.aChannelId)));
...
int32_t VoipConnectionStart(VoipConnectionlistT *pConnectionlist, int32_t iConnID, uint32_t uAddr, uint32_t uConnPort, uint32_t uBindPort, uint32_t uClientId, uint32_t uSessionId)
{
    ...
    for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS; iUserIndex++)
    {
        memcpy(&pConnection->VoipMicrPacket[iUserIndex].Head, &_Voip_MicrPacket, sizeof(VoipPacketHeadT));
    }
...
*/
/********************************************************************************F*/
void VoipConnectionSend(VoipConnectionlistT *pConnectionlist, const uint8_t *pVoiceData, int32_t iDataSize, uint32_t iUserIndex, uint8_t uSendSeqn)
{
    ...
    for (iConnID = 0; iConnID < pConnectionlist->iMaxConnections; iConnID++)
    {
        ...
        {
            // ref packet
            VoipMicrPacketT *pMicrPacket = &pConnection->VoipMicrPacket[iUserIndex];

            // if no voice packets are queued and the voice timer has elapsed, reset it
            if ((pMicrPacket->MicrInfo.uNumSubPackets == 0) && (NetTickDiff(uCurTick, pConnection->aVoiceSendTimer[iUserIndex]) >= 0))
            {
                pConnection->aVoiceSendTimer[iUserIndex] = uCurTick + VOIP_MSPERPACKET;
            }
            ...
            if (pMicrPacket->MicrInfo.uNumSubPackets < VOIP_MAXSUBPKTS_PER_PKT)
            {
                ...
                pMicrPacket->MicrInfo.uUserIndex = iUserIndex;
            }
            else
            {
            ...
            // see if we need to send any buffered voice data
            _VoipConnectionSendSingle(pConnectionlist, pConnection, iUserIndex, uCurTick, FALSE);
