...
//! Xbox/Xbox 360 VoIP data packet header
typedef struct VoipPacketHeadT
{
    uint8_t aType[3];           //!< type/version info
    uint8_t uSeqn;              //!< sequence number
    uint8_t aClientId[4];       //!< extra state bytes (used for session ID)
...
struct VoipTunnelRefT
{
    ...
    uint8_t bDirtyCastHack;         //!< temp hack to enable alternate DirtyCast functionality without impacting other uses
...
/*F********************************************************************************/
/*!
    \Function VoipTunnelClientListSend
...
static void _VoipTunnelClientListSend(VoipTunnelRefT *pVoipTunnel, VoipTunnelClientT *pSrcClient, uint32_t uDstClientId, const char *pPacketData, int32_t iPacketSize, struct sockaddr *pAddr, uint32_t uSendFlag)
{
    ...
    if (!(uSendFlag & VOIPTUNNEL_GAMEFLAG_SEND_SINGLE))
    {
        ...
        for (iClient = 0; iClient < VOIPTUNNEL_MAXGROUPSIZE; iClient++)
        {
            ...
            SocketSendto(pVoipTunnel->pVoipSocket, pPacketData, iPacketSize, 0, pAddr, sizeof(*pAddr));
...
/*!
    ...
    \Output
        const VoipPacketHeadT *pPacketHead  - pointer to packet head

    \Version 08/24/2006 (jbrookes)
*/
/********************************************************************************F*/
static VoipPacketHeadT *_VoipTunnelValidatePacket(VoipTunnelRefT *pVoipTunnel, char *pPacketData, int32_t iPacketSize, uint32_t *pVdpHeader)
{
    ...
    // validate size
    if (iPacketSize < (signed)sizeof(VoipPacketHeadT))
    {
        NetPrintf(("voiptunnel: voip packet size is too small; discarding\n"));
        return(NULL);
    }
    
    return((VoipPacketHeadT *)pPacketData);
}
...
/*!
    ...
    \Input *pPacketHead - pointer to packet head
    \Input iPacketSize  - size of received data
...
*/
/********************************************************************************F*/
static uint32_t _VoipTunnelGetClientId(const VoipPacketHeadT *pPacketHead, int32_t iPacketSize)
{
    uint32_t uClientId;
    
    // extract the client identifier
    uClientId = pPacketHead->aClientId[0] << 24;
    uClientId |= pPacketHead->aClientId[1] << 16;
    uClientId |= pPacketHead->aClientId[2] << 8;
    uClientId |= pPacketHead->aClientId[3];
...
/*!
    ...
    \Description
        LobbyMSort callback used to sort fast lookup array.
...
*/
/********************************************************************************F*/
static int32_t _VoipTunnelLookupSort(void *pRef, const void *_pElem0, const void *_pElem1)
{
    VoipTunnelLookupElemT *pElem0 = (VoipTunnelLookupElemT *)_pElem0;
    VoipTunnelLookupElemT *pElem1 = (VoipTunnelLookupElemT *)_pElem1;
...
/*!
    ...
    \Input uClientId        - client identifer
...
static void _VoipTunnelLookupBuild(VoipTunnelRefT *pVoipTunnel)
{
    ...
    // sort it by clientId
    LobbyMSort(pVoipTunnel, pVoipTunnel->pLookupTable, iClient, sizeof(pVoipTunnel->pLookupTable[0]), _VoipTunnelLookupSort);
}
...
static void _VoipTunnelLookupTest(VoipTunnelRefT *pVoipTunnel)
{
    ...
    memset(pVoipTunnel->ClientList, 0, sizeof(pVoipTunnel->ClientList));
}
#endif
...
/*!
    ...
    \Output int32_t         - acquired sendmask for the required client.
...
static uint32_t _VoipTunnelAcquireVoipBroadcastingSlot(VoipTunnelRefT *pVoipTunnel, VoipTunnelClientT *pClient, uint32_t uTargetSendMask)
{
    ...
    // if there's no target we have not yet acquired, all is good
    if (((~uAcquiredSendMask) & uTargetSendMask) == 0)
    {
    ...
    for(iIndex = 0; iIndex < VOIPTUNNEL_MAXGROUPSIZE; iIndex++)
    {
        ...
        if (((~uAcquiredSendMask) & uTargetSendMask) & (1 << iIndex))
        {
            ...
            if (pTargetClient) 
            {
                ...
                else
                {
                    ...
                    if (!(pTargetClient->uFlags & VOIPTUNNEL_CLIENTFLAG_MAX_VOICES_REACHED))
                    {
                        ...
                        _VoipTunnelEventCallback(pVoipTunnel, VOIPTUNNEL_EVENT_MAXDVOICE, pClient, pClient->iGameIdx);

                        NetPrintfVerbose((pVoipTunnel->uDebugLevel, 1, "voiptunnel: 0x%08x is not taking new talkers\n", pTargetClient->uClientId));
                    }
...
/*F********************************************************************************/
/*!
    \Function _VoipTunnelReleaseVoipBroadcastingSlot

    \Description
        Releases a broadcasting slot if the client has possession of one.
...
*/
/********************************************************************************F*/
static void _VoipTunnelReleaseVoipBroadcastingSlot(VoipTunnelRefT *pVoipTunnel, VoipTunnelClientT *pClient)
{
    int32_t iIndex;
    ...
    for(iIndex = 0; iIndex < VOIPTUNNEL_MAXGROUPSIZE; iIndex++)
    {
        ...
        if ((pClient->uSendMask) & (1 << iIndex))
        {
            ...
            {
                pTargetClient->iNumTalker--;
                pClient->uSendMask &= ~(1 << iIndex);
                NetPrintfVerbose((pVoipTunnel->uDebugLevel, 1, "voiptunnel: 0x%08x is not talking to 0x%08x anymore.\n", pClient->uClientId, pTargetClient->uClientId));
                
                if (pTargetClient->uFlags & VOIPTUNNEL_CLIENTFLAG_MAX_VOICES_REACHED)
                {
                    pTargetClient->uFlags &= ~VOIPTUNNEL_CLIENTFLAG_MAX_VOICES_REACHED;
                    _VoipTunnelEventCallback(pVoipTunnel, VOIPTUNNEL_EVENT_AVLBVOICE, pClient, pClient->iGameIdx);
...
*/
/********************************************************************************F*/
static void _VoipTunnelRouteVoipPacket(VoipTunnelRefT *pVoipTunnel, VoipTunnelClientT *pClient, const VoipPacketHeadT *pPacketHead, int32_t iPacketSize, struct sockaddr *pRecvAddr, uint32_t uCurTick, uint32_t bVdpHeader)
{
    uint32_t uSendFlag = (bVdpHeader) ? VOIPTUNNEL_GAMEFLAG_VDP : 0;

    // see if we have a packet type that is unicast
    if (!memcmp(pPacketHead->aType, "CO", 2) || !memcmp(pPacketHead->aType, "PNG", 3) || !memcmp(pPacketHead->aType, "DSC", 3))
    {
        uint32_t uRemoteClientId;

        // extract voip packet version from connection packet if we haven't done so already
        if ((pPacketHead->aType[0] == 'C') && (pClient->uPacketVersion == 0))
        {
            pClient->uPacketVersion = pPacketHead->aType[2];
        ...
        uRemoteClientId = pPacketHead->aRemoteClientId[0] << 24;
        uRemoteClientId |= pPacketHead->aRemoteClientId[1] << 16;
        uRemoteClientId |= pPacketHead->aRemoteClientId[2] << 8;
        uRemoteClientId |= pPacketHead->aRemoteClientId[3];

        NetPrintfVerbose((pVoipTunnel->uDebugLevel, pPacketHead->aType[0] == 'P' ? 2 : 1, "voiptunnel: forwarding %d byte unicast packet of type %c%c%c from clientId=0x%08x to clientId=0x%08x\n",
            iPacketSize, pPacketHead->aType[0], pPacketHead->aType[1], pPacketHead->aType[2],
            pClient->uClientId, uRemoteClientId));
        
        // forward to other clients in our group
        _VoipTunnelClientListSend(pVoipTunnel, pClient, uRemoteClientId, (const char *)pPacketHead, iPacketSize, pRecvAddr, uSendFlag|VOIPTUNNEL_GAMEFLAG_SEND_SINGLE);
    }
    else if (!memcmp(pPacketHead->aType, "MIC", 3))
    {
        ...
        {
            uSendMask = pPacketHead->aRemoteClientId[0] << 24;
            uSendMask |= pPacketHead->aRemoteClientId[1] << 16;
            uSendMask |= pPacketHead->aRemoteClientId[2] << 8;
            uSendMask |= pPacketHead->aRemoteClientId[3];
        }
        ...
        {
            NetPrintfVerbose((pVoipTunnel->uDebugLevel, 1, "voiptunnel: broadcasting voip packet %d from clientId=0x%08x\n", pPacketHead->aRemoteClientId[0], pClient->uClientId));

            // broadcast to all other clients in our group
            _VoipTunnelClientListSend(pVoipTunnel, pClient, 0, (const char *)pPacketHead, iPacketSize, pRecvAddr, uSendFlag|VOIPTUNNEL_GAMEFLAG_BROADCAST_ALLOTHERS); 
        }
        else
        {
...
static int32_t _VoipTunnelVoipRecvCallback(SocketT *pSocket, int32_t iFlags, void *_pRef)
{
    ...
    char aPacketData[VOIPTUNNEL_MAXPACKET];
    const VoipPacketHeadT *pPacketHead;
    VoipTunnelClientT *pClient;
    ...
    // validate the packet
    if ((pPacketHead = _VoipTunnelValidatePacket(pVoipTunnel, aPacketData, iRecvLen, &bVdpHeader)) == NULL)
    {
        return(0);
    }

    // extract client identifier from voip packet header
    uClientId = _VoipTunnelGetClientId(pPacketHead, iRecvLen);
    ...
    if ((pClient = VoipTunnelClientListMatchId(pVoipTunnel, uClientId)) == NULL)
    {
        NetPrintf(("voiptunnel: ignoring '%c%c%c' voip packet from %a:%d with unregistered id=0x%08x\n",
            pPacketHead->aType[0], pPacketHead->aType[1],  pPacketHead->aType[2],
    ...
    _VoipTunnelRouteVoipPacket(pVoipTunnel, pClient, pPacketHead, iRecvLen, &RecvAddr, uCurTick, bVdpHeader); 
...
/*!
    ...
    \Input *pVoipTunnel     - voiptunnel ref
    \Input *pClient         - client to remove, or null
...
*/
/********************************************************************************F*/
static int32_t _VoipTunnelClientGameDel(VoipTunnelRefT *pVoipTunnel, VoipTunnelClientT *pClient)
{
    ...
    for (iGameClient = 0; iGameClient < VOIPTUNNEL_MAXGROUPSIZE; iGameClient++)
    {
        ...
        if (pGame->aClientList[iGameClient] == pClient->uClientId)
        {
            ...
            _VoipTunnelReleaseVoipBroadcastingSlot(pVoipTunnel, pClient);

            if (!pVoipTunnel->bDirtyCastHack)
            {
                // remove entry from game list
                if (iGameClient != (pGame->iNumClients - 1))
                {
                    int32_t iNumMoveClients = (pGame->iNumClients - 1) - iGameClient;

                    // move the clients to remove the gap
                    memmove(&pGame->aClientList[iGameClient], &pGame->aClientList[iGameClient+1], iNumMoveClients * sizeof(pGame->aClientList[0]));
                }
                // clear last entry from game list
                pGame->aClientList[pGame->iNumClients - 1] = 0;
            }
            else
            {
...
/*F********************************************************************************/
/*!
    \Function _VoipTunnelClientListDel

    \Description
        Remove client from the client list

    \Input *pVoipTunnel     - voiptunnel ref
    \Input iClient          - index of client to remove, if pClient is null
...
*/
/********************************************************************************F*/
static void _VoipTunnelClientListDel(VoipTunnelRefT *pVoipTunnel, int32_t iClient)
{
    VoipTunnelClientT *pClient = &pVoipTunnel->ClientList[iClient];

    // notify application of upcoming deletion
    _VoipTunnelEventCallback(pVoipTunnel, VOIPTUNNEL_EVENT_DELCLIENT, pClient, 0);

    // remove entry from game
    if (_VoipTunnelClientGameDel(pVoipTunnel, pClient) < 0)
    {
...
VoipTunnelRefT *VoipTunnelCreate(uint32_t uVoipPort, int32_t iMaxClients, int32_t iMaxGames)
{
    ...
    memset(pVoipTunnel->pGameList, -1, iGameListSize);
...
int32_t VoipTunnelClientListAdd2(VoipTunnelRefT *pVoipTunnel, const VoipTunnelClientT *pClientInfo, VoipTunnelClientT **ppNewClient, int32_t iStartIndex)
{
    ...
    memcpy(pNewClient, pClientInfo, sizeof(*pNewClient));
    ...
    // increment game and client counts
    pGame->iNumClients += 1;
    pVoipTunnel->iNumClients += 1;
...
{
    // if using client pointer, get the client id
    if (pClient != NULL)
    {
        // get client index from client pointer
        iClient = pClient - pVoipTunnel->ClientList;
    }
    ...
    _VoipTunnelClientListDel(pVoipTunnel, iClient);
}
...
int32_t VoipTunnelGameListAdd(VoipTunnelRefT *pVoipTunnel, int32_t iGameIdx)
{
    ...
    memset(&pVoipTunnel->pGameList[iGameIdx], 0, sizeof(pVoipTunnel->pGameList[0]));
    return(0);
}
...
int32_t VoipTunnelGameListDel(VoipTunnelRefT *pVoipTunnel, int32_t iGameIdx)
{
    ...
    if( iGameIdx < pVoipTunnel->iMaxGames)
    {
        ...
        for (iClient = 0, iClientsDeleted = 0; iClient < pVoipTunnel->iNumClients; )
        {
            ...
            if (pVoipTunnel->ClientList[iClient].iGameIdx != iGameIdx)
            {
                iClient++;
                continue;
            }
...
int32_t VoipTunnelStatus(VoipTunnelRefT *pVoipTunnel, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    ...
    {
        memcpy(pBuf, &pVoipTunnel->pGameList[iValue], iBufSize);
        return(0);
    }
    if (iSelect == 'nusr')
    {
        int32_t iClient, iNumClients;
        if (iValue == -1)
        {
        ...
        {
            for (iClient = 0, iNumClients = 0; iClient < pVoipTunnel->iNumClients; iClient++)
            {
                if (pVoipTunnel->ClientList[iClient].iGameIdx == iValue)
                {
                    iNumClients += 1;
                }
            }
    ...
    {
        memcpy(pBuf, &pVoipTunnel->pVoipSocket, sizeof(pVoipTunnel->pVoipSocket));
        return(0);
    }
    return(-1);
}
...
int32_t VoipTunnelControl(VoipTunnelRefT *pVoipTunnel, int32_t iControl, int32_t iValue, int32_t iValue2, const void *pValue)
{
    ...
    if (iControl == 'dcst')
    {
        NetPrintf(("voiptunnel: setting DirtyCast hack=%d\n", iValue));
        pVoipTunnel->bDirtyCastHack = iValue;
        return(0);
    }
    if (iControl == 'rtim')
    {
...
void VoipTunnelUpdate(VoipTunnelRefT *pVoipTunnel)
{
    ...
    // poll voip socket if no async recv thread
    _VoipTunnelVoipRecvCallback(pVoipTunnel->pVoipSocket, 0, pVoipTunnel);
    #endif
