...
static void _VoipHeadsetCloseDevice(VoipHeadsetRefT *pHeadset, VoipHeadsetDeviceInfoT *pDeviceInfo)
{
    ...
    pHeadset->pStatusCb(iDevice, pDeviceInfo->eDevType == VOIP_HEADSET_INPDEVICE ? VOIP_PC_INPUT_DEVICE_INACTIVE : VOIP_PC_OUTPUT_DEVICE_ACTIVE, pHeadset->pCbUserData);
}
    ...
    for (uPacket = 0; uPacket < uPackets; uPacket++)
    {
        ...
        if ((iCompBytes = VoipCodecEncode(voipMicrPacketT.aData, &pHeadset->pPlayerBuffer[VOIP_HEADSET_FRAMESAMPLES * pHeadset->uPlayerBufferFrameCurrent], VOIP_HEADSET_FRAMESAMPLES)) > 0)
        {
            if (pHeadset->bLoopback == FALSE)
            {
                ...
                pHeadset->pMicDataCb(voipMicrPacketT.aData, iCompBytes, 0, pHeadset->uSendSeq++, pHeadset->pCbUserData);
            }
            else
            {
...
static void _VoipHeadsetProcessRecord(VoipHeadsetRefT *pHeadset)
{
    ...
    if (pMicrInfo->bActive == TRUE)
    {
        ...
        for ( ; pMicrInfo->WaveData[pMicrInfo->iCurWaveBuffer].WaveHdr.dwFlags & WHDR_DONE; )
        {
            ...
            memcpy(FrameData, &pMicrInfo->WaveData[pMicrInfo->iCurWaveBuffer].FrameData, VOIP_HEADSET_FRAMESIZE);
            ...
            if ((iCompBytes = VoipCodecEncode(pData, (int16_t *)FrameData, VOIP_HEADSET_FRAMESAMPLES)) > 0)
            {
                ...
                {
                    // send to mic data callback
                    pHeadset->pMicDataCb(&pPacket->aData, iCompBytes, 0, pHeadset->uSendSeq++, pHeadset->pCbUserData);
                }
                else
                {
        ...
        for ( ; pSpkrInfo->WaveData[pSpkrInfo->iCurWaveBuffer].WaveHdr.dwFlags & pSpkrInfo->dwCheckFlag[pSpkrInfo->iCurWaveBuffer]; )
        {
            ...
            if ((iSampBytes = VoipMixerProcess(pHeadset->pMixerRef, FrameData)) == VOIP_HEADSET_FRAMESIZE)
            {
                ...
                if (iNumBufAvail == VOIP_HEADSET_NUMWAVEBUFFERS)
                {
                    ...
                    memset(FrameSilenceData, 0, sizeof(FrameSilenceData));
                    ...
                    {
                        memcpy(pSpkrInfo->WaveData[pSpkrInfo->iCurWaveBuffer].FrameData, FrameSilenceData, sizeof(FrameSilenceData));
                ...
                memcpy(pSpkrInfo->WaveData[pSpkrInfo->iCurWaveBuffer].FrameData, FrameData, VOIP_HEADSET_FRAMESIZE);
...
static void _VoipHeadsetProcessDeviceChange(VoipHeadsetRefT *pHeadset, VoipHeadsetDeviceInfoT *pDeviceInfo)
{
    ...
    {
        int32_t iMessage = (pDeviceInfo->eDevType == VOIP_HEADSET_INPDEVICE) ? VOIP_PC_INPUT_DEVICE_ACTIVE : VOIP_PC_OUTPUT_DEVICE_ACTIVE;
        if (_VoipHeadsetOpenDevice(pDeviceInfo, pDeviceInfo->iDeviceToOpen, VOIP_HEADSET_NUMWAVEBUFFERS) == 0)
        {
            pHeadset->pStatusCb(pDeviceInfo->iActiveDevice, iMessage, pHeadset->pCbUserData);
        }
...
/*!
    ...
    \Input *pMicrPacket  - voice packet we're receiving
...
void VoipHeadsetReceiveVoiceDataCb(VoipUserT *pRemoteUser, VoipMicrPacketT *pMicrPacket, void *pUserData)
{
    ...
    if (pMicrPacket->MicrInfo.uSubPacketSize != pHeadset->iCmpFrameSize)
    {
        NetPrintf(("voipheadsetps3: discarding voice packet with %d voice bundles and mismatched sub-packet size %d (expecting %d)\n",
            pMicrPacket->MicrInfo.uNumSubPackets, pMicrPacket->MicrInfo.uSubPacketSize, pHeadset->iCmpFrameSize));
        return;
    }
...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if ((iSelect == 'cide') || (iSelect == 'code'))
    {
        VoipHeadsetDeviceInfoT *pDeviceInfo = (iSelect == 'cide') ? &pHeadset->MicrInfo : &pHeadset->SpkrInfo;
        NetCritEnter(&pHeadset->DevChangeCrit);
    ...
    if (iSelect == 'spam')
    {
        ...
        return(VoipCodecControl(VOIP_CODEC_ACTIVE, iSelect, iValue, 0, NULL));
    }
    #endif
    if (iSelect == 'play')
    {
...
int32_t VoipHeadsetStatus(VoipHeadsetRefT *pHeadset, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    ...
    if ((iSelect == 'idev') || (iSelect == 'odev'))
    {
        ...
        {
            memcpy(pBuf, pDeviceInfo->WaveDeviceCaps[iValue].szPname, sizeof(pDeviceInfo->WaveDeviceCaps[iValue].szPname));
            return(iValue);
        }
    }
    if (iSelect == 'idft')
