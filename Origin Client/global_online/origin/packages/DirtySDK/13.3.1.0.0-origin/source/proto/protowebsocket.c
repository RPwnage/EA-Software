...
static char *_ProtoWebSocket_strOpcodeNames[16] =
{
    "cont",
    "text",
    "bina",
...
static char *_ProtoWebSocket_strCloseReasons[PROTOWEBSOCKET_CLOSEREASON_COUNT] =
{
    "normal",
    "going away",
    "protocol error",
...
static int32_t _ProtoWebSocketSetAppendHeader(ProtoWebSocketRefT *pWebSocket, const char *pAppendHdr)
{
    ...
    {
        NetPrintfVerbose((pWebSocket->iVerbose, 0, "protowebsocket: ignoring set of append header '%s' that is already set\n", pAppendHdr));
        return(0);
    }
    ...
    if (iAppendBufLen > pWebSocket->iAppendLen)
    {
        ...
        {
            NetPrintf(("protowebsocket: could not allocate %d byte buffer for append header\n", iAppendBufLen));
            pWebSocket->iAppendLen = 0;
            return(-1);
        }
    }
    ...
    if (pWebSocket->pAppendHdr[iAppendStrLen-2] != '\r' || pWebSocket->pAppendHdr[iAppendStrLen-1] != '\n')
    {
...
static void _ProtoWebSocketGenerateKey(ProtoWebSocketRefT *pWebSocket)
{
    ...
    Base64Encode(sizeof(aRandom), (const char *)aRandom, pWebSocket->strRawKey);
    ...
    Base64Encode(CRYPTSHA1_HASHSIZE, (const char *)aSha1Data, pWebSocket->strEncKey);
...
static int32_t _ProtoWebSocketProcessHeader(ProtoWebSocketRefT *pWebSocket, const char *pBuffer)
{
    ...
    //$$TODO - process HTTP error codes (e.g. 400 Bad Request)
    //$$TODO - support 30x redirection?
    //$$TODO - support 401 authentication?
...
static int32_t _ProtoWebSocketSend(ProtoWebSocketRefT *pWebSocket, const char *pStrBuf, int32_t iSize)
{
    ...
    {
        NetPrintf(("protowebsocket: [%s] error %d sending %d bytes\n", pWebSocket, iResult, iSize));
...
static int32_t _ProtoWebSocketSendData(ProtoWebSocketRefT *pWebSocket, const char *pBuffer, int32_t iLength, uint8_t uOpcode)
{
    ...
    // set packet opcode
    *pPacket++ = uOpcode|PROTOWEBSOCKET_OPCODE_FIN; // assume not fragmented
    // set packet length
    pPacket = _ProtoWebSocketSetLength(pWebSocket, pPacket, 1, iLength);
    // set packet mask
    pPacket = _ProtoWebSocketSetMask(pWebSocket, pPacket, &pMask);

    // mask the data (TODO - could this be done with 32bit words?)
...
static int32_t _ProtoWebSocketRecv(ProtoWebSocketRefT *pWebSocket, char *pStrBuf, int32_t iSize)
{
    ...
    if ((pWebSocket->iRecvRslt = ProtoSSLRecv(pWebSocket->pProtoSSL, pStrBuf, iSize)) > 0)
    {
...
static int32_t _ProtoWebSocketRecvHeader(ProtoWebSocketRefT *pWebSocket)
{
    ...
    if (iResult > 0)
    {
        NetPrintfVerbose((pWebSocket->iVerbose, 1, "protowebsocket: [%p] received frame fin=%d typ=%s len=%d\n", pWebSocket, ((uint8_t *)pWebSocket->strFrameHdr)[0]>>7,
...
static int32_t _ProtoWebSocketRecvCtrl(ProtoWebSocketRefT *pWebSocket)
{
    ...
    if (pWebSocket->iCtrlDataOff == pWebSocket->iCtrlDataLen)
    {
...
/*!
    ...
    \Description
        Send data to a server over a WebSocket connection.  Stream-type send, TODO
...
int32_t ProtoWebSocketSend(ProtoWebSocketRefT *pWebSocket, const char *pBuffer, int32_t iLength)
{
    ...
    // connected, do the send
    return(_ProtoWebSocketSendData(pWebSocket, pBuffer, iLength, PROTOWEBSOCKET_OPCODE_BINA));
}
...
void ProtoWebSocketUpdate(ProtoWebSocketRefT *pWebSocket)
{
    ...
    /* receive server handshake response: note that due to serialized nature of handshaking,
       we can reuse the output buffer to receive the server response */
    if (pWebSocket->eState == ST_RECV)
    {
        if ((iResult = _ProtoWebSocketRecv(pWebSocket, pWebSocket->pOutBuf+pWebSocket->iOutLen, pWebSocket->iOutMax)) > 0)
        {
            pWebSocket->iOutLen += iResult;
            // see if we have the complete header
            if ((pWebSocket->iOutLen > 4) && (!strcmp(pWebSocket->pOutBuf+pWebSocket->iOutLen-4, "\r\n\r\n")))
            {
                // process the header
                if ((iResult = _ProtoWebSocketProcessHeader(pWebSocket, pWebSocket->pOutBuf)) >= 0)
                {
