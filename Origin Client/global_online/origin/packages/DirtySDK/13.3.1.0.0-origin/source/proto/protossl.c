...
/*!
    ...
    \Version 4.0 10/11/2012 jbrookes    Updated to support TLS1.0
*/
/***************************************************************************H*/
...
#define SSL3_VERSION        0x0302  // TLS1.1 (highest version we support)
...
typedef struct ProtoSSLCACertT
{
    ...
    uint32_t uFlags;                //!< PROTOSSL_CACERTFLAG_*
...
typedef struct SecureStateT
{
    ...
    uint8_t bSessionValid;          //!< session id from server is valid
...
#if DEBUG_ALL_OBJS
const static char *_SSL3_strAsnTypes[] =
{
    "ASN_00",
...
const static char *_SSL3_strAsnObjs[] =
{
    "ASN_OBJ_NONE",
    "ASN_OBJ_COUNTRY",
    "ASN_OBJ_STATE",
...
static ProtoSSLAlertDescT _ProtoSSL_AlertList[] =
{
    ...
    { SSL3_ALERT_DESC_DECRYPTION_FAILED,          "decryption failed"         },    // 21 - TLS only; reserved
    ...
    { SSL3_ALERT_DESC_NO_CERTIFICATE,             "no certificate"            },    // 41 - SSL3 only; reserved
    ...
    { SSL3_ALERT_DESC_CERTIFICATE_UNKNOWN,        "unknown certificate"       },    // 46
...
// The 2048-bit modulus for the embedded VeriSign 2006 CA Cert
const static uint8_t _ProtoSSL_VeriSign2006ServerModulus[] =
{
...
{
    int32_t iErr;
    for (iErr = 0; _ProtoSSL_AlertList[iErr].iAlertType != -1; iErr++)
    {
        if (_ProtoSSL_AlertList[iErr].iAlertType == (int32_t)uAlertType)
        {
            NetPrintf(("protossl: ALERT: level=%d, type=%d, name=%s\n", uAlertLevel, uAlertType, _ProtoSSL_AlertList[iErr].pAlertDesc));
            return;
        }
    }

    NetPrintf(("protossl: ALERT: level=%d, type=%d, name=unknown\n", uAlertLevel, uAlertType));
}
#else
...
static int32_t _SubjectAlternativeMatch(const char *pStrHost, const uint8_t *pSubject, int32_t iSubjectAltLen)
{
    ...
    for (pSubjectEnd = pSubject + iSubjectAltLen; pSubject < pSubjectEnd; pSubject += iSize)
    {
        // parse the object
        pSubject = _ParseHeader(pSubject, pSubjectEnd, &iType, &iSize);
...
*/
/**************************************************************************F*/
static void _ProtoSSLDoMac(uint8_t *pBuffer, int32_t iBufLen, const uint8_t *pMessage, int32_t iMessageLen, const uint8_t *pMessage2, int32_t iMessageLen2, const uint8_t *pKey, int32_t iKeyLen, int32_t iHashSize)
{
    uint8_t MacTemp[20];

    // handle MD5 MAC
    if (iHashSize == SSL3_MAC_MD5)
    {
        CryptMD5T MD5Context;
        CryptMD5Init(&MD5Context);
        CryptMD5Update(&MD5Context, pKey, iKeyLen);
        CryptMD5Update(&MD5Context, _SSL3_Pad1, 48);
        CryptMD5Update(&MD5Context, pMessage, iMessageLen);
        if (pMessage2 != NULL)
        {
            CryptMD5Update(&MD5Context, pMessage2, iMessageLen2);
        }
        ...
        CryptMD5Final(&MD5Context, pBuffer, iBufLen);
    }

    // handle SHA MAC
    if (iHashSize == SSL3_MAC_SHA)
    {
        CryptSha1T SHAContext;
        CryptSha1Init(&SHAContext);
        CryptSha1Update(&SHAContext, pKey, iKeyLen);
        CryptSha1Update(&SHAContext, _SSL3_Pad1, 40);
        CryptSha1Update(&SHAContext, pMessage, iMessageLen);
        if (pMessage2 != NULL)
        {
            CryptSha1Update(&SHAContext, pMessage2, iMessageLen2);
        }
        ...
        CryptSha1Final(&SHAContext, pBuffer, iBufLen);
    }
}
...
*/
/**************************************************************************F*/
static void _ProtoSSLDoHmac(uint8_t *pBuffer, int32_t iBufLen, const uint8_t *pMessage, int32_t iMessageLen, const uint8_t *pMessage2, int32_t iMessageLen2, const uint8_t *pKey, int32_t iKeyLen, int32_t iHashSize)
{
    if (pMessage2 != NULL)
    {
        ...
        CryptHmacCalcMulti(pBuffer, iBufLen, Message, 2, pKey, iKeyLen, iHashSize);
    }
    else
    {
        CryptHmacCalc(pBuffer, iBufLen, pMessage, iMessageLen, pKey, iKeyLen, iHashSize);
    }
}
...
*/
/**************************************************************************F*/
static void _ProtoSSLDoPHash(uint8_t *pBuffer, int32_t iOutLen, const uint8_t *pSecret, int32_t iSecretLen, const uint8_t *pSeed, int32_t iSeedLen, int32_t iHashSize)
{
    uint8_t aWork[128];

    // A(1)
    _ProtoSSLDoHmac(aWork, sizeof(aWork), pSeed, iSeedLen, NULL, 0, pSecret, iSecretLen, iHashSize);
    memcpy(aWork+iHashSize, pSeed, iSeedLen);
    _ProtoSSLDoHmac(pBuffer, iOutLen, aWork, iSeedLen+iHashSize, NULL, 0, pSecret, iSecretLen, iHashSize);
    ...
    while (iOutLen > iHashSize)
    {
        ...
        _ProtoSSLDoHmac(aWork2, sizeof(aWork2), aWork, iHashSize, NULL, 0, pSecret, iSecretLen, iHashSize);
        memcpy(aWork, aWork2, iHashSize);
        _ProtoSSLDoHmac(pBuffer, iOutLen, aWork, iSeedLen+iHashSize, NULL, 0, pSecret, iSecretLen, iHashSize);
    }
}
...
static void _ProtoSSLDoPRF(uint8_t *pBuffer, int32_t iOutLen, const uint8_t *pSecret, int32_t iSecretLen, const uint8_t *pSeed, int32_t iSeedLen)
{
    ...
    // execute md5 p_hash
    _ProtoSSLDoPHash(aMD5Buf, iOutLen, pSecret, iLS, pSeed, iSeedLen, SSL3_MAC_MD5);

    // execute sha1 p_hash
    _ProtoSSLDoPHash(aSHABuf, iOutLen, pSecret+iLS, iLS, pSeed, iSeedLen, SSL3_MAC_SHA);
...
/*!
    ...
    \Input iRandomLen   - length of random data
    \Input *pLabel      - label for TLS1 PRF or NULL for SSLv3

    \Version 10/11/2012 (jbrookes)
*/
/**************************************************************************F*/
static void _ProtoSSLBuildKey(uint8_t *pBuffer, int32_t iBufSize, uint8_t *pSource, int32_t iSourceLen, uint8_t *pRandomA, uint8_t *pRandomB, int32_t iRandomLen, const char *pLabel)
{
    if (pLabel == NULL)
    {
        CryptMD5T MD5Context;
    ...
    else
    {
        ...
        memcpy(aWork+13, pRandomA, iRandomLen);
        memcpy(aWork+45, pRandomB, iRandomLen);
...
static int32_t _ProtoSSLDoFinishHash(uint8_t *pBuffer, SecureStateT *pSecure, const char *pLabelSSL, const char *pLabelTLS)
{
    ...
    else // TLS1
    {
        ...
        _ProtoSSLDoPRF(pBuffer, 12, pSecure->MasterKey, sizeof(pSecure->MasterKey), aMacTemp, 51);
        iHashSize = 12;
    }
    return(iHashSize);
}
...
static int32_t _SendPacket(ProtoSSLRefT *pState, uint8_t uType, const void *pHeadPtr, int32_t iHeadLen, const void *pBodyPtr, int32_t iBodyLen)
{
    ...
    if ((pState->iState >= ST3_SEND_FINISH) && (pState->iState <= ST3_SECURE) && (pSecure->uSslVersion >= 0x0302) &&
        ((pSecure->pCipher != NULL) && (pSecure->pCipher->uEnc == SSL3_ENC_AES)))
    {
        pSend += 16;
    }
    ...
    {
        #if DEBUG_MSG_LIST
        NetPrintf(("protossl: _SendPacket handshake update (size=%d)\n", iSize));
        #endif
    ...
    if ((pState->iState >= ST3_SEND_FINISH) && (pState->iState <= ST3_SECURE) && (pSecure->pCipher != NULL))
    {
        ...
        if (pSecure->uSslVersion == 0x0300)
        {
            _ProtoSSLDoMac(pSend+iSize, pSecure->pCipher->uMac, MacTemp, pMacTemp-MacTemp, pSend, iSize,
                pSecure->pClientMAC, pSecure->pCipher->uMac, pSecure->pCipher->uMac);
        }
        else
        {
            _ProtoSSLDoHmac(pSend+iSize, pSecure->pCipher->uMac, MacTemp, pMacTemp-MacTemp, pSend, iSize,
                pSecure->pClientMAC, pSecure->pCipher->uMac, pSecure->pCipher->uMac);
        }
        ...
        if (pSecure->pCipher->uEnc == SSL3_ENC_AES)
        {
            ...
            if (pSecure->uSslVersion >= 0x0302)
            {
                pSend -= 16;
                _GenerateRandom(pSend, 16, &pSecure->ReadArc4);
                #if DEBUG_RAW_DATA
...
static int32_t _RecvPacket(ProtoSSLRefT *pState)
{
    ...
    /* check the record type - $$TODO - as per http://tools.ietf.org/html/rfc4346#section-6
       we SHOULD simply ignore an unknown record type GREATER than SSL3_REC_APPLICATON, rather
       than erroring */
    ...
    if ((pState->iState >= ST3_RECV_FINISH) && (pState->iState <= ST3_SECURE) && (pSecure->pCipher != NULL))
    {
        ...
        if (pSecure->pCipher->uEnc == SSL3_ENC_AES)
        {
            ...
            // if TLS1.1 or greater, remove explicit IV
            if (pSecure->uSslVersion >= 0x0302)
            {
            ...
            {
                /* $$todo - need to validate padding; all padding bytes must be equal to the padding length
                   and padding errors should be responded to with a bad_record_mac alert as per TLS1.1 section 5.
                   the mac should still be computed to eliminate possible timing attacks */
                iSize -= iPadBytes;
            }
        }
        ...
        if (pSecure->uSslVersion == 0x0300)
        {
            _ProtoSSLDoMac(MacTemp, pSecure->pCipher->uMac, MacTemp, pMacTemp-MacTemp,
                pSecure->RecvData+pSecure->iRecvBase, pSecure->iRecvSize-pSecure->iRecvBase,
                pSecure->pServerMAC, pSecure->pCipher->uMac, pSecure->pCipher->uMac);
        }
        else // TLS1
        {
            _ProtoSSLDoHmac(MacTemp, pSecure->pCipher->uMac, MacTemp, pMacTemp-MacTemp,
                pSecure->RecvData+pSecure->iRecvBase, pSecure->iRecvSize-pSecure->iRecvBase,
                pSecure->pServerMAC, pSecure->pCipher->uMac, pSecure->pCipher->uMac);
        }

        // validate MAC
        if (memcmp(MacTemp, pSecure->RecvData+pSecure->iRecvSize, pSecure->pCipher->uMac) != 0)
        {
...
static void _RecvHandshakeFinish(ProtoSSLRefT *pState)
{
    ...
    if ((pSecure->RecvData[0] == SSL3_REC_HANDSHAKE) && (pSecure->iRecvBase == 5))
    {
        #if DEBUG_MSG_LIST
        NetPrintf(("protossl: _RecvPacket handshake update (size=%d)\n", pSecure->iRecvSize-pSecure->iRecvBase));
        #endif
...
*/
/***************************************************************************F*/
static const uint8_t *_ParseDate(const uint8_t *pData, int32_t iSize, char *pBuffer, int32_t iBufSize)
{
    int32_t iType;
...
/*!
    ...
    \Input *pBuffer - [out] output for binary object
    \Input iBufSize - size of output buffer
...
static int32_t _ParseOptional(const uint8_t *pData, int32_t iSize, X509CertificateT *pCert)
{
    ...
    {
        #if DEBUG_VAL_CERT
        NetPrintf(("protossl: parsed SAN; length=%d\n", pCert->iSubjectAltLen));
        #endif
    }
    return(0);
}
...
static int32_t _ParseCertificate(X509CertificateT *pCert, const uint8_t *pData, int32_t iSize)
{
    ...
    // get validity dates
    if ((pData = _ParseDate(pData, iSize, pCert->strGoodFrom, sizeof(pCert->strGoodFrom))) == NULL)
    {
        NetPrintf(("protossl: _ParseCertificate: could not get from date\n"));
        return(-10);
    }
    if ((pData = _ParseDate(pData, iSize, pCert->strGoodTill, sizeof(pCert->strGoodTill))) == NULL)
    {
...
static int32_t _VerifySignature(ProtoSSLRefT *pState, X509CertificateT *pCert, const uint8_t *pKeyModData, int32_t iKeyModSize, const uint8_t *pKeyExpData, int32_t iKeyExpSize)
{
    ...
    #if DEBUG_VAL_CERT
    NetPrintf(("protossl: verifying signature\n"));
    #endif
...
static int32_t _VerifyCertificate(ProtoSSLRefT *pState, X509CertificateT *pCert, uint8_t bCertIsCA)
{
    ...
    else
    {
        ...
        for (pCACert = _ProtoSSL_CACerts; pCACert != NULL; pCACert = pCACert->pNext)
        {
            ...
            #if DEBUG_VAL_CERT
            NetPrintf(("protossl: verifying succeded\n"));
            #endif

            // if we are validating against a GOS CA, make sure the domain of the certificate is an EA domain
...
static int32_t _SetCACert(const uint8_t *pCertData, int32_t iCertSize, uint8_t bVerify)
{
    ...
    if ((iResult = Base64Decode(pCertEnd-pCertBeg, (const char *)pCertBeg, NULL)) > 0)
    {
        ...
        // decode the cert
        Base64Decode(pCertEnd-pCertBeg, (const char *)pCertBeg, (char *)pCertBuffer);
    ...
    while ((iResult == 0) && (iCertSize > 0) && (_FindPEMCertificateData(pCertData, iCertSize, &pCertBeg, &pCertEnd) != 0))
    {
        ...
        if (((iResult = Base64Decode(pCertEnd-pCertBeg, (const char *)pCertBeg, NULL)) <= 0) || (iResult > _iMaxCertSize))
        {
            break;
        }
        Base64Decode(pCertEnd-pCertBeg, (const char *)pCertBeg, (char *)pCertBuffer);
...
static void _SessionHistorySet(SessionHistoryT *pSessHist, struct sockaddr *pPeerAddr, const uint8_t *pSessionId, const uint8_t *pMasterSecret, uint32_t uCurTick)
{
    ...
    {
        #if DEBUG_RES_SESS
        NetPrintf(("protossl: session history peer addr changed; updating\n"));
        #endif
        memcpy(&pSessHist->SessionAddr, pPeerAddr, sizeof(pSessHist->SessionAddr));
    }
    // update sessionid if it has changed
    if (memcmp(pSessHist->SessionId, pSessionId, sizeof(pSessHist->SessionId)))
    {
        #if DEBUG_RES_SESS
        NetPrintf(("protossl: session history session id changed; updating\n"));
        #endif
        memcpy(pSessHist->SessionId, pSessionId, sizeof(pSessHist->SessionId));
    }
    ...
    {
        #if DEBUG_RES_SESS
        NetPrintf(("protossl: session history master secret changed; updating\n"));
        #endif
        memcpy(pSessHist->MasterSecret, pMasterSecret, sizeof(pSessHist->MasterSecret));
    }
    // touch access timestamp
    pSessHist->uSessionTick = uCurTick;
}

// check to see if we can find a session for the specified address or session id in our history buffer
static SessionHistoryT *_SessionHistoryGet(SessionHistoryT *pSessBuff, struct sockaddr *pPeerAddr, const uint8_t *pSessionId)
{
    ...
    NetCritEnter(&pState->SessionHistoryCrit);
    ...
    if (pSessInfo != NULL)
    {
        ...
        // copy to caller
        memcpy(pSessBuff, pSessInfo, sizeof(*pSessBuff));
        pSessInfo = pSessBuff;
    }

    // release critical section, return to caller
    NetCritLeave(&pState->SessionHistoryCrit);
    return(pSessInfo);
}

// save a new session in the history buffer
static void _SessionHistoryAdd(struct sockaddr *pPeerAddr, const uint8_t *pSessionId, uint32_t iSessionLen, const uint8_t *pMasterSecret)
{
    ...
    for (iSess = 0; iSess < SSL_SESSHIST_MAX; iSess += 1)
    {
        ...
        {
            #if DEBUG_RES_SESS
            NetPrintf(("protossl: adding session history entry %d (addr=%a:%d)\n", iSess, SockaddrInGetAddr(pPeerAddr), SockaddrInGetPort(pPeerAddr)));
            #endif
            _SessionHistorySet(pSessHist, pPeerAddr, pSessionId, pMasterSecret, uCurTick);
            NetCritLeave(&pState->SessionHistoryCrit);
            return;
...
int32_t ProtoSSLStartup(void)
{
    ...
    NetCritInit(&pState->SessionHistoryCrit, "protossl session");
...
void ProtoSSLShutdown(void)
{
    ...
    NetCritKill(&pState->SessionHistoryCrit);
...
static int32_t _ProtoSSLUpdateSendClientHello(ProtoSSLRefT *pState)
{
    ...
    // set to the *lowest* version we support, for compatibility with SSLv3-only servers
    pSecure->uSslVersion = 0x0300;

    // set requested protocol version (we set the version we want here, which is the *highest* version we support)
    ...
    *pData++ = (uint8_t)(SSL3_VERSION>>0);
    ...
    if (_SessionHistoryGet(&SessHist, &pState->PeerAddr, NULL) != NULL)
    {
        #if DEBUG_RES_SESS
        NetPrintf(("protossl: setting session id for resume\n"));
        #if DEBUG_RAW_DATA
        ...
        *pData++ = (uint8_t)sizeof(SessHist.SessionId);
        memcpy(pData, SessHist.SessionId, sizeof(SessHist.SessionId));
...
{
    int32_t iIndex, iState = ST3_RECV_HELLO;
    SecureStateT *pSecure = pState->pSecure;
    ...
    // check the version the server wants to give us
    if ((pData[0] != (SSL3_VERSION>>8)) || (pData[1] != (SSL3_VERSION&255)))
    {
        if ((pData[0] == (SSL3_VERSION>>8)) && (pData[1] < (SSL3_VERSION&255)))
        {
            NetPrintfVerbose((pState->iVerbose, 0, "protossl: downgrading to SSL version %d.%d\n", pData[0], pData[1]));
        }
        else
        {
            NetPrintf(("protossl: _ServerHello: SSL version %d.%d unsupported\n", pData[0], pData[1]));
            return(ST_FAIL_SETUP);
        }
    }
    // set version of the protocol we will be using
    ...
    if (*pData == sizeof(pSecure->SessionId))
    {
        ...
        memcpy(pSecure->SessionId, pData+1, sizeof(pSecure->SessionId));
        pSecure->bSessionValid = TRUE;
        #if DEBUG_RAW_DATA
        NetPrintMem(pSecure->SessionId, sizeof(pSecure->SessionId), "ServerHello session id");
        #endif

        // check for ClientHello/ServerHello session match; a match indicates we are resuming the session and bypassing key exchange
        if (_SessionHistoryGet(&SessHist, NULL, pSecure->SessionId) != NULL)
        {
            NetPrintfVerbose((pState->iVerbose, 0, "protossl: resuming previous session\n"));
            // copy session id and master secret
            memcpy(pSecure->MasterKey, SessHist.MasterSecret, sizeof(pSecure->MasterKey));
            memcpy(pSecure->SessionId, SessHist.SessionId, sizeof(pSecure->SessionId));
            // update state
    ...
    else
    {
        ...
        memset(pSecure->SessionId, 0, sizeof(pSecure->SessionId));
        pSecure->bSessionValid = FALSE;
    }
    pData += *pData+1;
...
{
    if (!pState->bCARequested && pState->bCertInfoSet)
    {
...
static int32_t _ProtoSSLUpdateRecvServerCert(ProtoSSLRefT *pState, const uint8_t *pData)
{
    ...
    while (iSize1 > 3)
    {
        ...
        {
            NetPrintfVerbose((pState->iVerbose, 0, "protossl: bypassing certificate validation\n"));
            return(ST3_RECV_HELLO);
        }
        ...
        if (iCertHeight == 0)
        {
            ...
            if (_WildcardMatchSubdomainNoCase(pState->strHost, pSecure->Cert.Subject.strCommon) != 0)
            {
        ...
        if ((iParse == 0) || (iSize1 == (iSize2 + 3)))
        {
            ...
            if ((iParse != 0) && (iParse != SSL_ERR_GOSCA_INVALIDUSE) && (_ProtoSSLUpdateRequestCA(pState) == 0))
            {
                iResult = ST_WAIT_CA;
            }
            else
            {
                ...
                iResult = (iParse == 0) ? ST3_RECV_HELLO : ST_FAIL_CERT_NOTRUST;
            }
...
// build and distribute key material from master secret, server random, and client random
static void _ProtoSSLBuildKeyMaterial(ProtoSSLRefT *pState)
{
    ...
    /// build key material
    _ProtoSSLBuildKey(pSecure->KeyBlock, sizeof(pSecure->KeyBlock), pSecure->MasterKey, sizeof(pSecure->MasterKey),
        pSecure->ServerRandom, pSecure->ClientRandom, sizeof(pSecure->ServerRandom),
        pSecure->uSslVersion > 0x0300 ? "key expansion" : NULL);
...
static int32_t _ProtoSSLUpdateSendClientKey(ProtoSSLRefT *pState)
{
    ...
    uint8_t strHead[6];
    uint32_t uTick = NetTick(), uHeadSize;
    ...
    /* From TLS1.1 RFC: client_version The latest (newest) version supported by the
       client.  This is used to detect version roll-back attacks. Upon receiving the premaster
       secret, the server SHOULD check that this value matches the value transmitted by the
       client in the client hello message. */
    ...
    // init rsa state
    if (CryptRSAInit(&RSAContext, pSecure->Cert.KeyModData, pSecure->Cert.iKeyModSize, pSecure->Cert.KeyExpData, pSecure->Cert.iKeyExpSize))
    {
    ...
    // build master secret
    _ProtoSSLBuildKey(pSecure->MasterKey, sizeof(pSecure->MasterKey), pSecure->PreMasterKey, sizeof(pSecure->PreMasterKey),
        pSecure->ClientRandom, pSecure->ServerRandom, sizeof(pSecure->ClientRandom),
        pSecure->uSslVersion > 0x0300 ? "master secret" : NULL);
    ...
    // setup packet for send
    _SendPacket(pState, SSL3_REC_HANDSHAKE, strHead, uHeadSize, RSAContext.EncryptBlock, pSecure->Cert.iKeyModSize);
...
static int32_t _ProtoSSLUpdateSendClientFinish(ProtoSSLRefT *pState)
{
    ...
    // do finish hash
    strHead[3] = (uint8_t)_ProtoSSLDoFinishHash(strBody, pSecure, "CLNT", "client finished");
    ...
    // if resuming, switch to secure mode as the handshaking is complete; else receive cipher change
    return(pSecure->bSessionResume ? ST3_SECURE : ST3_RECV_CHANGE);
}
...
static int32_t _ProtoSSLUpdateRecvChange(ProtoSSLRefT *pState, const uint8_t *pData)
{
    ...
    /* if this is a new connection we have already built the key material immediately
       after creating the master secret; in a resume scenario we are reusing the previous
       master secret and build the key material here */
...
static int32_t _ProtoSSLUpdateRecvServerFinish(ProtoSSLRefT *pState, const uint8_t *pData)
{
    ...
    #if DEBUG_MSG_LIST
    NetPrintf(("protossl: SSL Msg: Process Server Finish\n"));
    #endif

    // calculate the finish verification hash
    iMacSize = _ProtoSSLDoFinishHash(MacFinal, pSecure, "SRVR", "server finished");

    // verify the hash
    if (memcmp(MacFinal, pData, iMacSize))
    {
        NetPrintf(("protossl: failed setup\n"));
        NetPrintMem(MacFinal, iMacSize, "Server Final MAC Calc");
        NetPrintMem(pData, iMacSize, "Server Final MAC Recv");
        return(ST_FAIL_SETUP);
    }
    ...
    // if resuming send cipher change; else switch to secure mode as the handshaking is complete
    return(pSecure->bSessionResume ? ST3_SEND_CHANGE : ST3_SECURE);
}
...
static void _ProtoSSLUpdateRecvHandshake(ProtoSSLRefT *pState, SecureStateT *pSecure)
{
    ...
    {
        if ((pSecure->RecvData[5] == SSL3_ALERT_LEVEL_WARNING) && (pSecure->RecvData[6] == SSL3_ALERT_DESC_CLOSE_NOTIFY))
        {
    ...
    else if (pState->iState == ST3_RECV_HELLO)
    {
        ...
        {
            pState->iState = _ProtoSSLUpdateSendAlert(pState, SSL3_ALERT_LEVEL_WARNING, SSL3_ALERT_DESC_NO_CERTIFICATE);
        }
...
static int32_t _ProtoSSLUpdateRecv(ProtoSSLRefT *pState, SecureStateT *pSecure)
{
    ...
    if (pSecure->iRecvSize == SSL_MIN_PACKET)
    {
        // validate the version -- make sure it is 3.x, where we support minor version x
        if ((pSecure->RecvData[1] != (SSL3_VERSION>>8)) || (pSecure->RecvData[2] > (SSL3_VERSION&255)))
        {
            // $$TODO proper solution is to turn packet into alert to force shutdown; temp hack is to kill length bytes to avoid processing
            NetPrintf(("protossl: unsupported ssl version %d.%d\n", pSecure->RecvData[1], pSecure->RecvData[2]));
    ...
    if ((pSecure->iRecvProg == pSecure->iRecvSize) && (pSecure->RecvData[0] != SSL3_REC_APPLICATION))
    {
...
int32_t ProtoSSLControl(ProtoSSLRefT *pState, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iSelect == 'xdns')
    {
        NetPrintfVerbose((pState->iVerbose, 0, "protossl: %s secure mode DNS lookups\n", iValue ? "enabling" : "disabling"));
