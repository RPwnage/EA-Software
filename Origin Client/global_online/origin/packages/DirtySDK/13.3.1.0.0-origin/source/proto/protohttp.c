...
static void _ProtoHttpClose(ProtoHttpRefT *pState, const char *pReason)
{
    ...
    NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] closing connection: %s\n", (uintptr_t)pState, pReason));
...
*/
/********************************************************************************F*/
static int32_t _ProtoHttpFormatRequestHeader(ProtoHttpRefT *pState, const char *pUrl, const char *pHost, int32_t iPort, const char *pRequest, const char *pData, int64_t iDataLen)
{
    ...
    iOffset += ds_snzprintf(pInpBuf+iOffset, iInpMax-iOffset, "%s %s HTTP/1.1\r\n", pRequest, pUrl);
...
static int32_t _ProtoHttpFormatRequest(ProtoHttpRefT *pState, const char *pUrl, const char *pData, int64_t iDataLen, ProtoHttpRequestTypeE eRequestType)
{
    ...
    if ((pData != NULL) && (iDataLen > 0))
    {
        ...
        memcpy(pState->pInpBuf + pState->iInpLen, pData, (int32_t)iDataLen);
...
int32_t ProtoHttpStatus(ProtoHttpRefT *pState, int32_t iSelect, void *pBuffer, int32_t iBufSize)
{
    ...
    if (iSelect == 'body')
    {
        ...
        {
            memcpy(pBuffer, &pState->iBodySize, iBufSize);
        }
