...
{
    int32_t iAddr, iLen, iResult, iStatus, iTimeout, iCount;
    ProtoHttpRefT *pHttp;
    char strBuffer[1024];
    ...
    {
        NetPrintf(("ticker: cellSysmoduleLoadModule(CELL_SYSMODULE_IO) failed (err=%s)\n", DirtyErrGetName(iResult)));
    }
    if ((iResult = cellSysmoduleLoadModule(CELL_SYSMODULE_NET)) < 0)
    {
        NetPrintf(("ticker: cellSysmoduleLoadModule(CELL_SYSMODULE_NET) failed (err=%s)\n", DirtyErrGetName(iResult)));
    }
    if ((iResult = cellSysmoduleLoadModule(CELL_SYSMODULE_SYSUTIL_NP)) < 0)
    {
        NetPrintf(("ticker: cellSysmoduleLoadModule(CELL_SYSMODULE_SYSUTIL_NP) failed (err=%s)\n", DirtyErrGetName(iResult)));
    }
    if ((iResult = cellSysmoduleLoadModule(CELL_SYSMODULE_RTC)) < 0)
    {
        NetPrintf(("ticker: cellSysmoduleLoadModule(CELL_SYSMODULE_RTC) failed (err=%s)\n", DirtyErrGetName(iResult)));
    }

    // register the Sony sysutil callback to avoid annoying debug spam
    if ((iResult = cellSysutilRegisterCallback(3, _TickerSysUtilCb, NULL)) < 0)
    {
        NetPrintf(("ticker: cellSysutilRegisterCallback() failed err=%s\n", DirtyErrGetName(iResult))   );
    }
