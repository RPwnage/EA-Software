...
static int32_t _HttpCustomHeaderCallback(ProtoHttpRefT *pProtoHttp, char *pHeader, uint32_t uHeaderSize, const char *pData, int64_t iDataLen, void *pUserRef)
{
    ...
    // find append point
    if ((pAppend = ds_stristr(pHeader, "Accept: */*\r\n")) != NULL)
    {
        int32_t uBufSize = uHeaderSize - (pAppend - pHeader);
        ...
        {
            ds_strnzcpy(pAppend, HTTP_APNDHDR, uBufSize);
        }
    ...
    {
        ZPrintf("httpmgr: could not find append point for custom header\n");
        uHeaderSize = 0;
    }
...
static void _HttpMgrMget(HttpMgrRefT *pRef)
{
    ...
    if (pRef->pMgetOffset != NULL)
    {
        ...
        for (pHref = strstr(pRef->pMgetOffset, "href=\""); pHref != NULL; pHref = strstr(pHref2, "href=\""))
        {
            ...
            // copy the url
            ds_strsubzcpy(strUrl, sizeof(strUrl), pHref2, pEndQuote-pHref2);
...
static int32_t _CmdHttpMgrIdleCB(ZContext *argz, int32_t argc, char *argv[])
{
    ...
    for (iState = 0, iNumActive = 0; iState < HTTP_MAXCMDS; iState += 1)
    {
    ...
    // keep on idling
    return((iNumActive > 0) ? ZCallback(&_CmdHttpMgrIdleCB, HTTP_RATE) : 0);
}
