...
static TesterHostCoreT *g_pHostCore;   //!< global host core pointer
...
/*!
    ...
    \Version 09/21/2012 (akirchner)
*/
/********************************************************************************F*/
static int32_t _T2TesterHostCoreCmd(ZContext *argz, int32_t argc, char **argv)
{
    extern TesterHostCoreT *g_pHostCore;
    ...
    if ((argc == 2) && (ds_stricmp(argv[1], "stop") == 0))
    {
        retVal = TesterHostCoreStopSavingLog(g_pHostCore);
    }
    else if ((argc == 3) && (ds_stricmp(argv[1], "start") == 0))
    {
        retVal = TesterHostCoreStartSavingLog(g_pHostCore, argv[2]);
    }
    else
    {
        ZPrintf("\tusage: logfile [start logfilename|stop]\n");
    }
...
static void _T2HostRegisterModules(void)
{
    ...
    TesterHostCoreRegister(g_pHostCore, "logfile", &_T2TesterHostCoreCmd);
}
...
HRESULT CT2HostSample::Initialize()
{
    ...
    TagFieldSetNumber(strParams, sizeof(strParams), "PLATFORM", TESTER_PLATFORM_XBDM);

    char * strToken = strtok(GetCommandLine(), " ");

    while(strToken)
    {
        if(strstr(strToken, "-logfile="))
        {
            TagFieldSetString(strParams, sizeof(strParams), "LOGFILE", strToken + strlen("-logfile="));
        }
