...
void DirtyErrName(char *pBuffer, int32_t iBufSize, uint32_t uError)
{
    ...
    for (iErr = 0; iErr < DIRTYERR_NUMERRORS; iErr++)
    {
        if ((_DirtyErr_List[iErr].uError & ~0x80000000) == (uError & ~0x80000000))
        {
    ...
    for (iErr = 0; iErr < DIRTYERR_NUMERRORS; iErr++)
    {
        if ((_DirtyErr_List[iErr].uError & ~0xffff0000) == (uError & ~0xffff0000))
        {
            snzprintf(pBuffer, iBufSize, "%s/0x%08x", _DirtyErr_List[iErr].pErrorName, uError);
            return;
        }
