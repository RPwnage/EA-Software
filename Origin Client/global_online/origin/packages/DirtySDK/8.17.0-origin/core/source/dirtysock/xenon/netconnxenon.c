...
static uint16_t _NetConn_XnLanguageMap[12][2] =
{
...
static uint16_t _NetConn_XnCountryMap[110][2] =
{
...
static void _NetConnExtractSitename(NetConnRefT *pRef, const char *pParms)
{
    ...
    {
        strcpy(pRef->strXlspSitename, "gos-core-sdev");
    }
...
static void _NetConnGetAccountInfoStart(NetConnRefT *pRef, NetConnUserT *pUser, int32_t iUser)
{
    ...
    if (pUser->pUserInfo == NULL)
    {
        ...
        if (pUser->pUserInfo == NULL)
        {
            ...
            pUser->eInfoState = ST_USERINFO_ERROR;
            return;
        }
    }
    ...
    {
        pUser->eInfoState = ST_USERINFO_PROCESS;
...
static int32_t _NetConnUpdateAccountInfo(NetConnRefT *pRef)
{
    ...
    for (iUser = 0; iUser < NETCONN_MAXLOCALUSERS; iUser++)
    {
        ...
        if ((pUser->eInfoState == ST_USERINFO_START) && (pRef->bUserInfoQuery == FALSE))
        {
            _NetConnGetAccountInfoStart(pRef, pUser, iUser);
            break;
        }
        else if (pUser->eInfoState == ST_USERINFO_PROCESS)
        {
...
static void _NetConnGetUserInfo(NetConnUserT *pUser, int32_t iUser)
{
    ...
    // mark as needing to get account info
    pUser->eInfoState = ST_USERINFO_START;
...
*/
/********************************************************************************F*/
static uint32_t _NetConnXblUserMask()
{
    uint32_t iIndex;
...
*/
/********************************************************************************F*/
static uint32_t _NetConnNumXblUsers()
{
    uint32_t iIndex;
...
*/
/************************************************************************************F*/
static void _NetConnUpdateXLSPResolution(NetConnRefT *pRef)
{
    int32_t iRet;
...
static void _NetConnUpdate(void *pData, uint32_t uCurTick)
{
    ...
    if (pRef->pDirtyLSP && (pRef->uXlspAddress != 0))
    {
        if (NetTickDiff(uCurTick, pRef->uLastUpdateTime) >= NETCONN_XLSP_UPDATE_TIME)
        {
    ...
    switch(pRef->eState)
    {
        ...
        case ST_INFO:
        {
            if ((iResult = _NetConnUpdateAccountInfo(pRef)) != FALSE)
            {
                // move to online state
                pRef->uConnStatus = '+onl';
            }

            // if we're done, transition to idle state
            if (iResult != 0)
            {
                pRef->eState = ST_IDLE;
            }
        ...
        case ST_XLSP_GATE:
        {
            _NetConnUpdateXLSPResolution(pRef);
            break;
        }
        default:
            break;
...
int32_t NetConnStartup(const char *pParams)
{
    ...
    strcpy(pRef->strXlspSitename, "gos-core");
    ...
    pRef->pDirtyLSP = DirtyLSPCreate(100);
...
int32_t NetConnShutdown(uint32_t uShutdownFlags)
{
    ...
    if (_NetConn_pRef->friendEnumerator)
    {
        if (XHasOverlappedIoCompleted(&_NetConn_pRef->XFriendOverlapped))
        {
            // overlapped operation completed, close enumeration handle
            CloseHandle(_NetConn_pRef->friendEnumerator);
            _NetConn_pRef->friendEnumerator = 0;
        }
        else
        {
            // signal "try again"
    ...
    if (_NetConn_pRef->uLookUpInProgress)
    {
        if (!XHasOverlappedIoCompleted(&_NetConn_pRef->XLookUpOverlapped))
        {
            return(NETCONN_ERROR_ISACTIVE);
        }
        _NetConn_pRef->uLookUpInProgress = FALSE;
    }

    // dispose of DirtyLSP
    if (_NetConn_pRef->pDirtyLSP != NULL)
    {
        DirtyLSPDestroy(_NetConn_pRef->pDirtyLSP);
        _NetConn_pRef->pDirtyLSP = NULL;
    }

    // destroy listener for live events
    if (_NetConn_pRef->hListener != NULL)
    {
        CloseHandle(_NetConn_pRef->hListener);
        _NetConn_pRef->hListener = NULL;
    }
    ...
    if ((uShutdownFlags & NETCONN_SHUTDOWN_NETACTIVE) == 0)
    {
        ...
        if (_NetConn_pRef->bSyslink == FALSE)
        {
            XOnlineCleanup();
        }
    ...
    if (_NetConn_pRef->pFindUserResponse)
    {
        DirtyMemFree(_NetConn_pRef->pFindUserResponse, NETCONN_MEMID, _NetConn_pRef->iMemGroup, _NetConn_pRef->pMemGroupUserData);
        _NetConn_pRef->pFindUserResponse = NULL;
    }
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'locl' - returns locality for logged-in user
...
int32_t NetConnStatus(int32_t iKind, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    if ((iKind == 'locl') || (iKind == 'locn'))
    {
        ...
        if (pUser == NULL)
        {
            NetPrintf(("netconnxenon: Warning -- locality (%c%c%c%c) unavailable (uRequestedUserMask=%u).\n", LOBBYAPI_LocalizerTokenPrintCharArray(iKind), pRef->uRequestedUserMask));
            return(LOBBYAPI_LOCALITY_UNKNOWN);
        }

        // get the language code
        for (iMap = 0, uLanguage = LOBBYAPI_LANGUAGE_UNKNOWN; iMap < (sizeof(_NetConn_XnLanguageMap)/sizeof(_NetConn_XnLanguageMap[0])); iMap++)
        {
            if (_NetConn_XnLanguageMap[iMap][0] == pUser->pUserInfo->wLanguageId)
            {
                uLanguage = _NetConn_XnLanguageMap[iMap][1];
                break;
            }
        }

        // get the country code
        for (iMap = 0, uCountry = LOBBYAPI_COUNTRY_UNKNOWN; iMap < (sizeof(_NetConn_XnCountryMap)/sizeof(_NetConn_XnCountryMap[0])); iMap++)
        {
            if (_NetConn_XnCountryMap[iMap][0] == pUser->pUserInfo->bCountryId)
            {
                uCountry = _NetConn_XnCountryMap[iMap][1];
                break;
            }
        }
    ...
    if (iKind == 'xnfo')
    {
        ...
        if ((pBuf != NULL) && (pUser->pUserInfo != NULL) && (pUser->eInfoState == ST_USERINFO_DONE) && (iBufSize == sizeof(pRef->aUsers[iData].pUserInfo)))
        {
