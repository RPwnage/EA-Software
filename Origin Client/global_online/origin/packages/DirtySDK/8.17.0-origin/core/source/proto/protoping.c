    ...
    while ((iLen = SocketRecvfrom(pProtoPing->pSocket, strResponse, sizeof(strResponse), 0, &SockAddr, (iLen=sizeof(SockAddr), &iLen))) > 0)
    {
        ...
        for (iQueue = 0; iQueue < pProtoPing->iQueueTail; iQueue++)
        {
            ...
            if (uSeqnMatch != 0)
            {
                ...
                #if DIRTYCODE_PLATFORM != DIRTYCODE_REVOLUTION
                pRequest->uPing = Response.uTick - pRequest->uTick;
                #else
                pRequest->uPing = Response.uTick;
                #endif
                if (pRequest->uPing == 0)
                {
                    pRequest->uPing = 1;
                ...
                if ((pRequest->uSize = Response.uSize) != 0)
                {
                    ...
                    {
                        memcpy(pRequest->pData, pEchoResponse->body.data.userdata, pRequest->uSize);
                    }
                    else
                    {
...
static void _ProtoPingQosCallback(QosApiRefT *pQosApi, QosApiCBInfoT *pCBInfo, QosApiCBTypeE eCBType, void *pUserData)
{
    ...
    for (iQueue = 0; iQueue < pProtoPing->iQueueTail; iQueue++)
    {
        ...
        if (pRequest->uSeqn == pCBInfo->pQosInfo->uRequestId)
        {
            ...
            if ((pRequest->uSize = pCBInfo->pQosInfo->iDataLen) != 0)
            {
                if ((pRequest->pData = (char *)DirtyMemAlloc(pRequest->uSize, PROTOPING_MEMID, pProtoPing->iMemGroup, pProtoPing->pMemGroupUserData)) != NULL)
                {
                    memcpy(pRequest->pData, pCBInfo->pQosInfo->aData, pRequest->uSize);
                }
                else
                {
                    NetPrintf(("protoping: unable to allocate memory for ping response data\n"));
...
static int32_t _ProtoPingRequest(ProtoPingRefT *pProtoPing, uint32_t uAddress, const unsigned char *pData, int32_t iDataLen, int32_t iTimeout, int32_t iTtl, uint32_t bServer)
{
    ...
    if (pProtoPing->eType == PROTOPING_TYPE_ICMP)
    {
        ...
        if (iDataLen < 0)
        {
            iDataLen = (int32_t)strlen((char *)pData)+1;
        }
        ...
        // copy over the data
        memcpy(EchoRequest.data.userdata, pData, iDataLen);
        // align the size
        ...
        if (pProtoPing->uProtocol == IPPROTO_UDP)
        {
            ...
            // udp header
            memcpy(&aHeader[12], &EchoRequest.header.proto.udp, sizeof(EchoRequest.header.proto.udp));
            // udp payload
    ...
    else
    {
        ...
        {
            char strAddrText[20];
            pProtoPing->iSeqn = QosApiServiceRequest(pProtoPing->pQosApi, SocketInAddrGetText(uAddress, strAddrText, sizeof(strAddrText)), 0, 0, 1, 0, QOSAPI_REQUESTFL_LATENCY);
        }
        else
        {
            DirtyAddrT DirtyAddr;
            DirtyAddrFromHostAddr(&DirtyAddr, &uAddress);

            pProtoPing->iSeqn = QosApiRequest(pProtoPing->pQosApi, &DirtyAddr, 1, 0, QOSAPI_REQUESTFL_LATENCY);
        }
...
int32_t ProtoPingControl(ProtoPingRefT *pProtoPing, int32_t iControl, int32_t iValue, void *pValue)
{
    ...
    if (iControl == 'list')
    {
        ...
        {
            return(QosApiListen(pProtoPing->pQosApi, NULL, 0, 0, iValue));
        }
        NetPrintf(("protoping: warning, list is only available when type is set to qos\n"));
        return(-1);
    }
    if (iControl == 'port')
    {
...
int32_t ProtoPingResponse(ProtoPingRefT *pProtoPing, unsigned char *pBuffer, int32_t *pBufLen, ProtoPingResponseT *pResponse)
{
    ...
    if (pProtoPing->eType == PROTOPING_TYPE_ICMP)
    {
        _ProtoPingIcmpCallback(pProtoPing->pSocket, 0, pProtoPing);
    }
    #endif
    ...
    for (iPing=0, iQueue=0, uCurTime=NetTick(); iQueue < pProtoPing->iQueueTail; iQueue++)
    {
        ...
        #else
        /* don't timeout on Wii; ping requests block and a ping timeout
           would cause subsequent ping requests to timeout incorrectly */
        bTimeout = (pRequest->uFlag & PROTOPING_FLAG_RECEIVED) && (pRequest->uPing == 0xffff);
        #endif
        ...
        if ((pRequest->uFlag & PROTOPING_FLAG_RECEIVED) || (bTimeout == TRUE))
        {
            ...
            if (pRequest->pData != NULL)
            {
                if (pBufLen != NULL)
                {
                    ...
                    if (pBuffer != NULL)
                    {
                        memcpy(pBuffer, pRequest->pData, *pBufLen);
                    }
                }
