...
/*!
    ...
    \Description
        This file provides some upper layer protocol abstractions such as streams
        (a virtual connection over which packets can be sent) and controller packet
        buffering and exchange logic. In the case of PS2, this module runs on the EE
        to allow direct access by the application.
...
#define GMDIST_META_ARRAY_SIZE          (16)    // how many past versions of sparse multipacket to keep
...
static char _strNetGameDistDataTypes[5][32] =
    {"INVALID",
     "GMDIST_DATA_NONE",
     "GMDIST_DATA_INPUT",
...
static int32_t _SetDataPtrTry(NetGameDistRefT *pRef, GameBufferDataT *pBuffer, uint16_t uLength)
{
    ...
    if (posB >= posA)
    {
        ...
        {
            snzprintf(pRef->m_errortext, sizeof(pRef->m_errortext), "overflow in _SetDataPtrTry. indexA was %d, indexB was %d, posA was %d, posB was %d, ", indexA, indexB, pBuffer->m_lkup[indexA].pos + pBuffer->m_lkup[indexA].len, posB);
    ...
    snzprintf(pRef->m_errortext, sizeof(pRef->m_errortext), "_SetDataPtrTry reached unreachable code.");
...
/*F*************************************************************************************************/
/*!
    \Function    _NetGameDistSyncStream

    \Description
        Extract and distribute stream sync packets

    \Input *pRef    - reference pointer

    \Output
        int32_t     - processed packet count

    \Version    1.0        12/20/00 (GWS) First Version
*/
/*************************************************************************************************F*/
static int32_t _NetGameDistSyncStream(NetGameDistRefT *pRef)
{
    int32_t iCount=0, iSize;
    GameStreamB *pBlock;
    NetGameDistStreamT *pStream;
    // check for completed sync packets
    for (pStream = pRef->m_stream; pStream != NULL; pStream = pStream->pNext)
    {
        // see if any sync data
        while ((pStream->iSynSize > 0) && (((GameStreamB *)pStream->pSynData)->sync >= pRef->m_globalseq))
        {
            // point to sync data
            pBlock = (GameStreamB *)pStream->pSynData;
            iSize = sizeof(*pBlock)+pBlock->size;
            // deliver packet
            if (pStream->Recv != NULL)
            {
                (pStream->Recv)(pStream, pStream->iSubchan, pBlock->kind, (pBlock+1), pBlock->size);
            }
            // remove from buffer
            if (iSize >= pStream->iSynSize)
            {
                pStream->iSynSize = 0;
            }
            else
            {
                memcpy(pStream->pSynData, pStream->pSynData+iSize, pStream->iSynSize-iSize);
                pStream->iSynSize -= iSize;
            }
            // count the packet
            ++iCount;
        }
    ...
    // return processed packet count
    return(iCount);
}

/*F*************************************************************************************************/
/*!
    \Function    _NetGameDistRecvStream
...
*/
/*************************************************************************************************F*/
static int32_t _NetGameDistRecvStream(NetGameDistRefT *pRef, NetGamePacketT *pPacket)
{
    int32_t iRes = -1, iSize;
    NetGameDistStreamT *pStream;
    NetGameDistStreamInpT *pInp;
    int32_t iSubChannel;
    #if DIRTYCODE_LOGGING
    char strIdent[5];
    ...
    for (pStream = pRef->m_stream; pStream != NULL; pStream = pStream->pNext)
    {
        ...
        if (pStream->iIdent == pPacket->body.strm.ident)
        {
            ...
            {
                NetPrintf(("netgamedist: [0x%08x] warning, received packet on stream '%s' with invalid packet subchannel %d\n", pRef, strIdent, iSubChannel));
                return(-1);
            }
            // ref channel
            ...
            if (iSize <= pStream->iInpMaxm)
            {
                ...
                {
                    NetPrintf(("netgamedist: [0x%08x] clamped stream packet size from %d to %d on stream '%s'\n", pRef, iSize, pInp->iInpSize-pInp->iInpProg, strIdent));
                    iSize = pInp->iInpSize-pInp->iInpProg;
                }
                // append to existing packet
                memcpy(pInp->pInpData+pInp->iInpProg, pPacket->body.strm.data, iSize);
                pInp->iInpProg += iSize;
                ...
                if (pInp->iInpProg == pInp->iInpSize)
                {
                    ...
                    {
                        NetPrintf(("netgamedist: [0x%08x] no registered stream recv handler on stream '%s'\n", pRef, strIdent));
                    }
                    // clear from buffer
                    pInp->iInpProg = 0;
                }
            ...
            {
                NetPrintf(("netgamedist: [0x%08x] invalid stream packet size (%d >= %d) on stream '%s'\n",
    ...
    NetPrintf(("netgamedist: [0x%08x] could not find stream for stream packet with iIdent '%s'\n", pRef, strIdent));
...
/*F*************************************************************************************************/
/*!
    \Function    _NetGameDistPollStream
...
*/
/*************************************************************************************************F*/
static int32_t _NetGameDistPollStream(NetGameDistRefT *pRef)
{
    ...
    for (pStream = pRef->m_stream; pStream != NULL; pStream = pStream->pNext)
    {
        ...
        while (pStream->iOutProg < pStream->iOutSize)
        {
            ...
            // get data block
            memcpy(&Block, pStream->pOutData+pStream->iOutProg, sizeof(Block));
            // setup data packet
            Packet.head.kind = GAME_PACKET_STRM;
            ...
            memcpy(Packet.body.strm.data, pStream->pOutData+pStream->iOutProg+sizeof(Block)+Block.sent, iSize);
            Packet.head.len = (uint16_t)(sizeof(Packet.body.strm)-sizeof(Packet.body.strm.data)+iSize);
            // try and queue/send the packet
            result = pRef->sendproc(pRef->linkref, (NetGamePacketT *)&Packet, 1);
            if (result < 0)
            {
                NetPrintf(("netgamedist: [0x%08x] stream send failed !\n", pRef));
                break;
            }
            else if (result == 0)
            {
            ...
            if (Block.sent != Block.size)
            {
                ...
                memcpy(pStream->pOutData+pStream->iOutProg, &Block, sizeof(Block));
            }
            else
            {
...
/*F*************************************************************************************************/
/*!
    \Function    _NetGameDistSendStream
...
*/
/*************************************************************************************************F*/
static int32_t _NetGameDistSendStream(NetGameDistStreamT *pStream, int32_t iSubChan, int32_t iKind, void *pBuffer, int32_t iLength)
{
    GameStreamB Block;
    ...
    if (iKind > '~   ')
    {
        ...
        Block.sync = pStream->pClient->m_localseq;
        // copy into sync byffer
        memcpy(pStream->pSynData+pStream->iSynSize, &Block, sizeof(Block));
        memcpy(pStream->pSynData+pStream->iSynSize+sizeof(Block), pBuffer, iLength);
    ...
    memcpy(pStream->pOutData+pStream->iOutSize, &Block, sizeof(Block));
    ...
    _NetGameDistPollStream(pStream->pClient);
...
static int32_t _NetGameDistSendInput(NetGameDistRefT *pRef, void *pBuffer, int32_t iLength, int32_t iLengthSize)
{
    ...
    if (pBuffer != NULL)
    {
        ...
        if (iLength < 0)
        {
            snzprintf(pRef->m_errortext, sizeof(pRef->m_errortext), "netgamedist: _NetGameDistSendInput with iLength %d.", iLength);
        ...
        {
            snzprintf(pRef->m_errortext, sizeof(pRef->m_errortext), "netgamedist: _NetGameDistSendInput with iNext %d and iBeg %d.", iNext, iBeg);
            return(GMDIST_OVERFLOW);
        }

        if (iNext == pRef->m_out.m_beg)
        {
            snzprintf(pRef->m_errortext, sizeof(pRef->m_errortext), "netgamedist: _NetGameDistSendInput with iNext %d and m_out.m_beg %d.", iNext, pRef->m_out.m_beg);
            return(GMDIST_OVERFLOW);
        }
    ...
    while ((pRef->m_out.m_beg != pRef->m_out.m_end) && (pRef->m_updatemetainfobeg == pRef->m_updatemetainfoend))
    {
        ...
        memcpy(pRef->m_maxpkt.body.data, pData, pRef->m_maxpkt.head.len);
        ...
        {
            snzprintf(pRef->m_errortext, sizeof(pRef->m_errortext), "netgamedist: GMDIST_SENDPROC_FAILED result is %d.", result);
...
NetGameDistRefT *NetGameDistCreate(void *pLinkRef, NetGameDistStatProc *pStatProc, NetGameDistSendProc *pSendProc, NetGameDistRecvProc *pRecvProc, uint32_t uInBufferSize, uint32_t uOutBufferSize )
{
    ...
    // set default stream packet size    
    pRef->iStrmMaxPktSize = NETGAME_STRMPKT_DEFSIZE;
...
int32_t NetGameDistStatus(NetGameDistRefT *pRef, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    if (iSelect == 'mult')
    {
        ...
        {
            memcpy(pBuf, pRef->m_recvstats, sizeof(NetGameDistStatT) * pRef->m_totplrs);
    ...
    if (iSelect == 'stat')
    {
        ...
        {
            memcpy(pBuf, &pRef->m_stats, sizeof(NetGameLinkStatT));
            return(0);
        }
        else
        {
...
/*F*************************************************************************************************/
/*!
    \Function    NetGameDistCreateStream
    ...
    \Output
        NetGameDistStreamT * - new stream, or NULL if the iIdent was not unique

    \Version    1.0        12/20/00 (GWS) First Version
*/
/*************************************************************************************************F*/
NetGameDistStreamT *NetGameDistCreateStream(NetGameDistRefT *pRef, int32_t iSubChan, int32_t iIdent, int32_t iInpLen, int32_t iOutLen, int32_t iSynLen)
{
    NetGameDistStreamT *pStream;
    int32_t iInpSize;
    char *pInpData;
    ...
    for (pStream = pRef->m_stream; pStream != NULL; pStream = pStream->pNext)
    {
        ...
        {
            NetPrintf(("netgamedist: [0x%08x] error -- attempting to create duplicate stream '%c%c%c%c'\n",
    ...
    if ((pStream = (NetGameDistStreamT *) DirtyMemAlloc(sizeof(*pStream), NETGAMEDIST_MEMID, pRef->m_memgroup, pRef->m_memgrpusrdata)) == NULL)
    {
        NetPrintf(("netgamedist: [0x%08x] unable to allocate stream\n", pRef));
        return(NULL);
    }
    ...
    pStream->Send = _NetGameDistSendStream;
    pStream->Recv = NULL;
    ...
    pStream->pInp = DirtyMemAlloc(iInpSize + (iInpLen * (pStream->iSubchan + 1)), NETGAMEDIST_MEMID, pRef->m_memgroup, pRef->m_memgrpusrdata);
    ...
    pStream->pOutData = DirtyMemAlloc(iOutLen, NETGAMEDIST_MEMID, pRef->m_memgroup, pRef->m_memgrpusrdata);
    
    // allocate sync buffer
    pStream->iSynMaxm = iSynLen;
    if (iSynLen > 0)
    {
        pStream->pSynData = DirtyMemAlloc(iSynLen, NETGAMEDIST_MEMID, pRef->m_memgroup, pRef->m_memgrpusrdata);
    }
...
*/
/*************************************************************************************************F*/
void NetGameDistDestroyStream(NetGameDistRefT *pRef, NetGameDistStreamT *pStream)
{
    NetGameDistStreamT **link;
    ...
    if (pStream != NULL)
    {
        ...
        {
            DirtyMemFree(pStream->pSynData, NETGAMEDIST_MEMID, pRef->m_memgroup, pRef->m_memgrpusrdata);
        }
        DirtyMemFree(pStream->pInp, NETGAMEDIST_MEMID, pRef->m_memgroup, pRef->m_memgrpusrdata);
        DirtyMemFree(pStream->pOutData, NETGAMEDIST_MEMID, pRef->m_memgroup, pRef->m_memgrpusrdata);
        DirtyMemFree(pStream, NETGAMEDIST_MEMID, pRef->m_memgroup, pRef->m_memgrpusrdata);
    }
}
...
int32_t NetGameDistInputPeek(NetGameDistRefT *pRef, uint8_t *pType, void *pPeer, int32_t *pPlen)
{
    ...
    if (pPeer != NULL)
    {
        ...
        if (length > *pPlen)
        {
            snzprintf(pRef->m_errortext, sizeof(pRef->m_errortext), "NetGameDistInputPeek error. length is %d, *pLen is %d.", length, *pPlen);
        ...
        memcpy(pPeer, pPsrc + offsetBuffer, length);
    }
...
int32_t NetGameDistInputQueryMulti(NetGameDistRefT *pRef, uint8_t *pDataTypes, void **pInputs, int32_t *pLen)
{
    ...
    if (pInputs)
    {
        ...
        for (i = 0; i<pRef->m_totplrs; i++)
        {
            ...
            if (i != pRef->m_ourindex)
            {
                if (!pRef->m_gotMetaInfo || (pRef->m_metaInfo[uVer].uMask & (1 << i)))
                {
                    ...
                    {
                        memcpy(pInputs[i], recvBuff + pos, pLen[i]);
                        pos += pLen[i];
                    }
                    else
                    {
        ...
        if (delta)
        {
            ...
            memcpy(pInputs[pRef->m_ourindex], pOsrc, pLen[pRef->m_ourindex]);
    ...
    if (pInputs)
    {
        for (i = 0; i<pRef->m_totplrs; i++)
        {
            ...
            NetPrintfVerbose((pRef->iVerbose, NETGAMEDIST_VERBOSITY, "    == Player %d --> len=%d type=%s\n", i, pLen[i], 
                _strNetGameDistDataTypes[((dataType >= GMDIST_DATA_NONE && dataType <= GMDIST_DATA_DISCONNECT) ? dataType : 0)]));
        }
    }
...
int32_t NetGameDistInputLocal(NetGameDistRefT *pRef, void *pBuffer, int32_t iLength)
{
    ...
    memcpy(pRef->m_multibuf + 1, pBuffer, iLength);
...
int32_t NetGameDistInputLocalMulti(NetGameDistRefT *pRef, uint8_t* uTypes, void **pBuffer, int32_t* iLengths, int32_t iDelta)
{
    ...
    if (pRef->m_sendmulti)
    {
        ...
        {
            snzprintf(pRef->m_errortext, sizeof(pRef->m_errortext), "Multipacket bigger than NETGAME_DATAPKT_MAXSIZE (%d). Discarding and reporting overflow.\n", NETGAME_DATAPKT_MAXSIZE);
        ...
        else
        {
            ...
            {
                snzprintf(pRef->m_errortext, sizeof(pRef->m_errortext), "GMDIST_OVERFLOW_WINDOW (pRef->m_packetid[pRef->m_inp.m_beg]-1) is %d, pRef->iLastSentDelta is %d.\n", (pRef->m_packetid[pRef->m_inp.m_beg]-1), pRef->iLastSentDelta);
    ...
    for (index = 0; index < uLastPlayer; index++)
    {
        ...
        {
            //put the data in
            memcpy(pRef->m_multibuf + pos, pBuffer[index], iLengths[index]);
            pos += iLengths[index];
    ...
    {        
        ...
        NetPrintfVerbose((pRef->iVerbose, NETGAMEDIST_VERBOSITY, "netgamedist: [0x%08x] NetGameDistInputLocalMulti() called with tick=%d len=%d type=%s\n",
            pRef, NetTick(), iLengths[0], _strNetGameDistDataTypes[((dataType >= GMDIST_DATA_NONE && dataType <= GMDIST_DATA_DISCONNECT) ? dataType : 0)]));
...
/*!
    ...
    \Notes
        ...
        \n
        \n --- advanced setup (sending multipackets)
        \n    'meta' - enable or disable meta-information sending (disabling after enabling is currently not-supported on the client)
        \n    'metm' - set the mask for the meta-information. To be done at the same time as 'meta'
        \n    'metv' - set the version number for the meta-information. To be done at the same time as 'meta'
        \n
        \n ---- provide information ----
        \n    'hwsn' - highwater stream needed
        \n    'hwsu' - highwater stream used
...
int32_t NetGameDistControl(NetGameDistRefT *pRef, int32_t iSelect, int32_t iValue, void *pValue)
{
    ...
    if (iSelect == 'meta')
    {
        pRef->m_uSparse = iValue;
        return(0);
    }
    if (iSelect == 'metm')
    {
        pRef->m_metaInfoToSend[pRef->m_updatemetainfoend].uMask = iValue;
        pRef->m_metaInfoToSend[pRef->m_updatemetainfoend].uPlayerCount = _NetGameDistCountBits(pRef, pRef->m_metaInfoToSend[pRef->m_updatemetainfoend].uMask);

        return(0);
    }
    if (iSelect == 'metv')
    {
// TODO: deprecate in V9. Replace with a new function call doing meta, metv and metm at once
        pRef->m_metaInfoToSend[pRef->m_updatemetainfoend].uVer = _Modulo(iValue, GMDIST_META_ARRAY_SIZE);

        pRef->m_updatemetainfoend = _Modulo(pRef->m_updatemetainfoend + 1, GMDIST_META_ARRAY_SIZE);
        return(0);
    }
    if ((iSelect == 'hwsu') || (iSelect == 'hwsn'))
    {
        int32_t highWaterStreamUsed = 0;
        int32_t highWaterStreamNeeded = 0;

        pStream = pRef->m_stream;
        while(pStream != NULL)
        {
            if (pStream->iHighWaterUsed > highWaterStreamUsed)
            {
                highWaterStreamUsed = pStream->iHighWaterUsed;
            }
            if (pStream->iHighWaterNeeded > highWaterStreamNeeded)
            {
                highWaterStreamNeeded = pStream->iHighWaterNeeded;
            }
            pStream = pStream->pNext;
        }
        return((iSelect == 'hwsu') ? highWaterStreamUsed : highWaterStreamNeeded);
    }
    if (iSelect == 'late')
    {
    ...
    if (iSelect == '?cws')
    {
        ...
        {
            snzprintf(pRef->m_errortext, sizeof(pRef->m_errortext), "'?cws' failure.");
            return(FALSE);
        }
...
uint32_t NetGameDistUpdate(NetGameDistRefT *pRef)
{
    ...
    while (( _Modulo(pRef->m_inp.m_end + 1, PACKET_WINDOW) != pRef->m_inp.m_beg ) && (NetGameDistInputPeekRecv(pRef, pPacket, 1) > 0))
    {
            ...
            (pPacket->head.kind == GAME_PACKET_INPUT_MULTI_FAT))
        {
            ...
            if (next == pRef->m_inp.m_beg)
            {
                ...
                snzprintf(pRef->m_errortext, sizeof(pRef->m_errortext), "NetGameDistUpdate, buffer overflow (queue full).");
            ...
            else
            {
                ...
                if (_SetDataPtr(pRef, &pRef->m_inp, pPacket->head.len + 1))
                {
                    ...
                    // copy into buffer
                    memcpy(pData, pPacket->body.data, pPacket->head.len);
        ...
        else if (pPacket->head.kind == GAME_PACKET_STRM)
        {
            // convert stream header from network to host order
            pPacket->body.strm.ident = SocketNtohl(pPacket->body.strm.ident);
            pPacket->body.strm.kind = SocketNtohl(pPacket->body.strm.kind);
            pPacket->body.strm.size = SocketNtohl(pPacket->body.strm.size);

            // process the packet        
            _NetGameDistRecvStream(pRef, pPacket);
        }
        else if (pPacket->head.kind == GAME_PACKET_STATS)
        {
            ...
            for (index = 0; index < pRef->m_totplrs; index++)
            {
                memcpy(&pRef->m_recvstats[index], pPacket->body.data + pos, sizeof(*pRef->m_recvstats) );
    ...
    // distribute sync packets
    _NetGameDistSyncStream(pRef);
...
void NetGameDistSetProc(NetGameDistRefT *pRef, int32_t kind, void* proc)
{
    ...
    case 'link':
        ...
        // set max stream packet size
        if (pRef->linkctrlproc)
        {
            pRef->iStrmMaxPktSize = pRef->linkctrlproc(pRef->linkref, 'mwid', 0) 
                - (sizeof(((NetGamePacketT *)NULL)->body.strm) - NETGAME_STRMPKT_MAXSIZE);
        }
        else
        {
            pRef->iStrmMaxPktSize = NETGAME_STRMPKT_DEFSIZE;
        }
        NetPrintf(("netgamedist: [0x%08x] using stream max packet size %d\n", pRef, pRef->iStrmMaxPktSize));
...
void NetGameDistSendStats(NetGameDistRefT *pRef, NetGameDistStatT *pStats)
{
    ...
    for (index = 0; index<pRef->m_totplrs; index++)
    {
        memcpy(pRef->m_maxpkt.body.data + pos, &pStats[index], sizeof(NetGameDistStatT));
