...
typedef struct VoipGroupManagerT
{
    ...
    uint32_t        uUserRecvMask;  //<! the recv mask assocaited with the user mute requests
    uint32_t        uUserSendMask;  //<! the send mask assocaited with the user mute requests
    uint32_t        uChanRecvMask;  //<! the recv mask assocaited with the channel selection
    uint32_t        uChanSendMask;  //<! the send mask assocaited with the channel selection
...
/*!
    ...
    \Input iChannel     - The connection ID
...
int32_t VoipGroupSelectChannel(VoipGroupRefT *pVoipGroup, int32_t iChannel, VoipGroupChanModeE eMode)
{
    ...
    for(iIndex = 0; iIndex < MAX_CONCURRENT_CHANNEL; iIndex++)
    {
        ...
        if ((pManager->iLocalChannels[iIndex] == iChannel) || ((pManager->uLocalModes[iIndex] == VOIPGROUP_CHANNONE) && (iSlot == MAX_CONCURRENT_CHANNEL)))
        {
            iSlot = iIndex;
        }
    ...
    NetPrintf(("voipgroup: [channel] set local channel iLocalChannels[%d] = %d, uLocalModes[%d] = %d", iSlot, iSlot, pManager->iLocalChannels[iSlot], pManager->uLocalModes[iSlot]));
    pManager->uLocalChannelSelection = _VoipGroupManagerEncode(pManager->iLocalChannels, pManager->uLocalModes);
    NetPrintf(("voipgroup: [channel] set local channels 0x%08x", pManager->uLocalChannelSelection));
