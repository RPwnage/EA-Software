...
*/
/********************************************************************************F*/
static uint32_t _ProtoHttpApplyBaseUrl(ProtoHttpRefT *pState, const char *pKind, char *pHost, int32_t iHostSize, int32_t *pPort, int32_t *pSecure)
{
...
{
    // get append header length (include null)
    int32_t iAppendLen = (int32_t)strlen(pAppendHdr) + 1;
    
    // see if we need to allocate a new buffer
    if (iAppendLen > pState->iAppendLen)
    {
        if (pState->pAppendHdr != NULL)
        {
            DirtyMemFree(pState->pAppendHdr, PROTOHTTP_MEMID, pState->iMemGroup, pState->pMemGroupUserData);
        }
        if ((pState->pAppendHdr = DirtyMemAlloc(iAppendLen, PROTOHTTP_MEMID, pState->iMemGroup, pState->pMemGroupUserData)) != NULL)
        {
            pState->iAppendLen = iAppendLen;
        }
        else
        {
    ...
    // check to see if we fit in the current append header buffer
    if (iAppendLen <= pState->iAppendLen)
    {
        strnzcpy(pState->pAppendHdr, pAppendHdr, pState->iAppendLen);
        return(0);
    }
    else
    {
...
static int32_t _ProtoHttpFormatRequest(ProtoHttpRefT *pState, const char *pUrl, const char *pData, int32_t iDataLen, ProtoHttpRequestTypeE eRequestType)
{
    ...
    int32_t iPort = 0, iResult, iSecure;
    ...
    if (_ProtoHttpApplyBaseUrl(pState, strKind, strHost, sizeof(strHost), &iPort, &iSecure) != 0)
    {
...
static int32_t _ProtoHttpGetLocationHeader(ProtoHttpRefT *pState, const char *pInpBuf, char *pBuffer, int32_t iBufSize, const char **pHdrEnd)
{
    ...
    /* according to RFC location headers should be absolute, but some webservers respond with relative
       ...
       request */
    if (strnicmp(pLocHdr, "http://", 7) && strnicmp(pLocHdr, "https://", 8))
    {
...
void ProtoHttpSetBaseUrl(ProtoHttpRefT *pState, const char *pUrl)
{
    ...
    // parse the url for kind, host, and port
    _ProtoHttpParseUrl(pUrl, strKind, sizeof(strKind), strHost, sizeof(strHost), &iPort, &iSecure);
...
int32_t ProtoHttpCheckKeepAlive(ProtoHttpRefT *pState, const char *pUrl)
{
    ...
    if (pState->bConnOpen && (pState->iKeepAlive > 0) && (pState->iPort == iPort) && (pState->iSecure == iSecure) && !stricmp(pState->strHost, strHost))
    {
        return(1);
    }
    else
...
const char *ProtoHttpUrlParse(const char *pUrl, char *pKind, int32_t iKindSize, char *pHost, int32_t iHostSize, int32_t *pPort, int32_t *pSecure)
{
    ...
    return(_ProtoHttpParseUrl(pUrl, pKind, iKindSize, pHost, iHostSize, pPort, pSecure));
}
