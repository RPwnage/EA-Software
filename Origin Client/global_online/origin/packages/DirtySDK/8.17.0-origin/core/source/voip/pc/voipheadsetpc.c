...
static void _VoipHeadsetProcessDeviceChange(VoipHeadsetRefT *pHeadset, VoipHeadsetDeviceInfoT *pDeviceInfo)
{
    ...
    {
        int32_t iMessage = (pDeviceInfo->eDevType == VOIP_HEADSET_INPDEVICE) ? VOIP_PC_INPUT_DEVICE_ACTIVE : VOIP_PC_INPUT_DEVICE_INACTIVE;
        if (_VoipHeadsetOpenDevice(pDeviceInfo, pDeviceInfo->iDeviceToOpen, VOIP_HEADSET_NUMWAVEBUFFERS) == S_OK)
        {
...
VoipHeadsetRefT *VoipHeadsetCreate(int32_t iMaxConduits, int32_t iMaxLocal, VoipHeadsetMicDataCbT *pMicDataCb, VoipHeadsetStatusCbT *pStatusCb, void *pCbUserData)
{
    ...
    // enable microphone $$todo -- remove this and make it always on?
