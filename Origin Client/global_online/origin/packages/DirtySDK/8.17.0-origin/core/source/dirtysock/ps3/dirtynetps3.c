...
int32_t SocketInfo(SocketT *pSocket, int32_t iInfo, int32_t iData, void *pBuf, int32_t iLen)
{
    ...
    if (pSocket == NULL)
    {
        ...
        if (iInfo == 'addr')
        {
            if (pState->uLocalAddr == 0)
            {
                ...
                {
                    pState->uLocalAddr = SocketInTextGetAddr(Info.ip_address);
                }
                else
                {
    ...
    if (iInfo == 'stat')
    {
        ...
        if (pSocket->opened == 0)
        {
            ...
            if (socketselect(1, NULL, &fdwrite, &fdexcept, &tv) != 0)
            {
                NetPrintf(("dirtynetps3: socketselect() != 0\n"));
                // if we got an exception, that means connect failed
                if (FD_ISSET(pSocket->socket, &fdexcept))
                {
                    NetPrintf(("dirtynetwin: connection failed\n"));
                    pSocket->opened = -1;
                }
                // if socket is writable, that means connect succeeded
                if (FD_ISSET(pSocket->socket, &fdwrite))
                {
