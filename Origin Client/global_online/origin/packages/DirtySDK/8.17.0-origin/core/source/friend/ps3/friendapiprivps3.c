...
static int _NpBasicHandler(int iEvent, int iRetCode, uint32_t uRequestId, void *pArg)
{
    ...
    if (!bDelayedNotifyEvent)
    {
        ...
        pFriendApiRef->eventData.pRawData1 = &iEvent;

        _FriendApiNotifyEvent(pFriendApiRef, FRIENDAPI_EVENT_UNKNOWN, &pFriendApiRef->eventData, NULL);
    }
...
{
    if (pFriendApiRef->iEventsInQueue > 0)
    {
        --pFriendApiRef->iEventsInQueue;
    ...
    if (pFriendApiRef->bGetBlockList)
    {
        int32_t iResult;
...
int32_t FriendApiPrivStatus(FriendApiRefT *pFriendApiRef, int32_t iSelect, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'eoip')
    {
        return(pFriendApiRef->bEndOfInitialPresence);
    }
