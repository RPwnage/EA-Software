...
void CryptStp1MakeWallet(CryptStp1WalletT *pWallet, CryptStp1SharedT *pShared, uint32_t uTime, const char *pRandom)
{
    ...
    // build cipher IV
    snzprintf(strCrypt, sizeof(strCrypt), "iv-%s-iv", pRandom);
    CryptMD5Init(&MD5);
    CryptMD5Update(&MD5, strCrypt, -1);
