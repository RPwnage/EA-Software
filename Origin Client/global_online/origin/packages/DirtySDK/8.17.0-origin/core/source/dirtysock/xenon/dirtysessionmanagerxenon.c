...
struct DirtySessionManagerRefT
{
    ...
    int32_t iSessionAppliedFlags;               //!< flags applied so far

    XUID    LocalXuid;                          //!< XUIDs of local player, from netconn
...
static void _DirtySessionManagerQueueOpAtFront(DirtySessionManagerRefT *pRef, DirtySessionManagerOperation *pOp)
{
    ...
    {
        NetPrintf(("dirtysessionmanagerxenon: [0x%08x] Queueing %s at front\n", pRef, _DsmStateNames[pOp->eOp]));
...
static void _DirtySessionManagerQueueOp(DirtySessionManagerRefT *pRef, DirtySessionManagerOperation *pOp)
{
    if (pRef->iNumQueuedOperations < DIRTYSESSIONMANAGER_MAX_OPERATIONS)
    {
        NetPrintf(("dirtysessionmanagerxenon: [0x%08x] Queueing %s\n", pRef, _DsmStateNames[pOp->eOp]));
...
static int32_t _DirtySessionManagerMigrateHost(DirtySessionManagerRefT *pRef, int32_t bLocalUserIsHost, const char *pSession)
{
    ...
    {
        strnzcpy(pRef->strSession, pSession, sizeof(pRef->strSession));
...
/*!
    ...
    \Input *pRef        - pointer to module state
    \Input *pDirtyAddr  - pointer to XUID to remove
...
*/
/*************************************************************************************************F*/
static int32_t _DirtySessionManagerRemove(DirtySessionManagerRefT *pRef, const DirtyAddrT *pDirtyAddr)
{
    DirtySessionManagerOperation Op;

    DirtyAddrToHostAddr(&Op.PeerXuid[0], sizeof(Op.PeerXuid[0]), pDirtyAddr);
...
//    }
    ...
    if ((iUserIndex = NetConnQuery(NULL, (NetConfigRecT *)&pRef->LocalXuid, 0)) >= 0)
    {
        DirtySessionManagerOperation Op;
        ...
        // save user index used to create the session
        ...
        if (pLocalAddrs[0].strMachineAddr[0])
        {
            DirtyAddrToHostAddr(&Op.PeerXuid[0], sizeof(Op.PeerXuid[0]), &pLocalAddrs[0]);
            NetPrintf(("dirtysessionmanagerxenon: added user %d, %s\n", iUserIndex, pLocalAddrs[0].strMachineAddr));
        }
        else
        {
            Op.PeerXuid[0] = 0;
        ...
        Op.dwIndex[0] = iUserIndex;
        Op.iMaxPublicSlots = pRef->iMaxPublicSlots;
        Op.iMaxPrivateSlots = pRef->iMaxPublicSlots;
    ...
    {
        NetPrintf(("dirtysessionmanagerxenon: [0x%08x] unable to find a valid user index for session creation\n", pRef));
...
void DirtySessionManagerUpdate(DirtySessionManagerRefT *pRef)
{
    ...
    if (!pRef->bOpInProgress && (pRef->iNumQueuedOperations > 0))
    {
        ...
        case ST_SESSMGR_CREA:
            ...
            {
                // if somehow else has presence and we're not terminating, let's wait
            ...
            uUserMask = NetConnStatus('mask', 0, NULL, 0);
            for(iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; iIndex++)
            {
                if ((1 << iIndex) & uUserMask)
                {
                    NetPrintf(("dirtysessionmanagerxenon: setting gametype=%d and gamemode=%d\n", pOp->uGameType, pOp->uGameMode));
        ...
        case ST_SESSMGR_ARBI:
            if (NetTickDiff(NetTick(), pOp->iTimer) > 0)
            {
                NetPrintf(("dirtysessionmanagerxenon: [0x%08x] starting operation %s\n", pRef, _DsmStateNames[pOp->eOp]));
                ...
                else if ((Op.eOp == ST_SESSMGR_ARBI) && (!pRef->bTerminating))
                {
                    ...
                    if (pRef->bHosting)
                    {
                        ...
                        if (( pRef->pRegistrationResults->wNumRegistrants < (unsigned)Op.iNumArbitrationClients) && (Op.iNumArbitrationAttempts > 0))
                        {
                            ...
                            OpNext.iTimer = NetTick() + DIRTYSESSIONMANAGER_ARBITRATIONTIMER;
...
int32_t DirtySessionManagerControl(DirtySessionManagerRefT *pRef, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'remv')
    {
        return(_DirtySessionManagerRemove(pRef, (const DirtyAddrT *)pValue));
    }

    // set local xuid
    if (iControl == 'self')
    {
        DirtyAddrToHostAddr(&pRef->LocalXuid, sizeof(pRef->LocalXuid), (DirtyAddrT *)pValue);
        return(0);
    }
...
#if DIRTYCODE_LOGGING
        snzprintf(strTemp, sizeof(strTemp), "%I64d", pRef->StatXuid);
...
int32_t DirtySessionManagerStatus(DirtySessionManagerRefT *pRef, int32_t iSelect, void *pBuf, int32_t iBufSize)
{
    ...
    if ((iSelect == 'sess') || (iSelect == 'nonc'))
    {
        ...
        else if (pRef->strSession[0] == '$')
        {
            if (iSelect == 'sess')
            {
                strnzcpy((char *)pBuf, pRef->strSession, iBufSize);
            }
            else
            {
                NetPrintMem(&pRef->qwSessionNonce, sizeof(pRef->qwSessionNonce), "sessionNonce");
    ...
    if (iSelect == 'sflg')
    {
        return(pRef->iSessionCreationFlags);
    }
...
int32_t DirtySessionManagerComplete(DirtySessionManagerRefT *pRef, int32_t *pAddr, const char* pMachineAddr)
{
    ...
    {
        NetPrintf(("dirtysessionmanagerxenon: Warning! XNetXnAddrToInAddr() returned error code 0x%08x\n", hResult));
        NetPrintf(("dirtysessionmanagerxenon: %s\n", DirtyErrGetName(hResult)));
...
/*!
    ...
    \Input *pLocalAddr              - local dirtyaddr
...
DirtySessionManagerRefT *DirtySessionManagerCreate(DirtyAddrT *pHostAddr, DirtyAddrT *pLocalAddr)
{
    ...
    // save a copy of our local XUID
    if (pLocalAddr != NULL)
    {
        DirtyAddrToHostAddr(&pRef->LocalXuid, sizeof(pRef->LocalXuid), pLocalAddr);
    }
