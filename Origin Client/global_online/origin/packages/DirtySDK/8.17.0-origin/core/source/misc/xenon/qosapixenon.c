...
struct QosApiRefT
{
    ...
    DirtyAddrT SelfAddr;                                    //!< local address
...
QosApiRefT *QosApiCreate(QosApiCallbackT *pCallback, void *pUserData, int32_t iServicePort)
{
    ...
    // set maddr
    DirtyAddrSetInfoXenon(&pQosApi->SelfAddr, &Xuid, &XnAddr);

    // create the session ref - local is the host
    if ((pQosApi->pSessionManager = DirtySessionManagerCreate(&pQosApi->SelfAddr, &pQosApi->SelfAddr)) == NULL)
    {
    ...
    // create the session
    if (DirtySessionManagerCreateSess(pQosApi->pSessionManager, FALSE, 0, NULL, &pQosApi->SelfAddr) != 0)
    {
