...
static void _StreamOpen(StreamCmdRefT *pCmdRef, StreamStationT *pStation, int32_t iRestartFreq)
{
    ...
    if (!strchr(pUrl+7, '/'))
    {
        strnzcpy(strTempUrl, pUrl, sizeof(strTempUrl));
        strnzcat(strTempUrl, "/", sizeof(strTempUrl));
        pUrl = strTempUrl;
    }
    ...
    mp3decodereset(pCmdRef->pDecoder);
...
*/
/********************************************************************************F*/
static int32_t _StreamXmlGetString(const unsigned char *pXml, const char *pName, char *pBuffer, int32_t iBufSize)
{
    memset(pBuffer, 0, iBufSize);
    if ((pXml = XmlFind(pXml, (const unsigned char *)pName)) != NULL)
    {
        return(XmlContentGetString(pXml, (unsigned char *)pBuffer, iBufSize, (unsigned char *)""));
    }
    return(-1);
}
...
{
    const unsigned char *pPlayFile, *pXml, *pXml2;
    int32_t iFileSize, iNumStations;

    // open the playlist
    ZPrintf("stream: opening playlist '%s'\n", pName);
    if ((pPlayFile = (const unsigned char *)ZFileLoad(pName, &iFileSize, FALSE)) == NULL)
    {
    ...
    if ((pXml = XmlFind(pPlayFile, (unsigned char *)"playlist.station")) == NULL)
    {
    ...
    for (pXml2 = pXml, iNumStations = 1; ; )
    {
        ...
        if (!strncmp((char *)pXml2+1, "station", 7))
        {
            iNumStations += 1;
        }
    }
    ...
    if (iNumStations != 0)
    {
        ...
        memset(pCmdRef->pPlaylist, 0, iPlaylistSize);
...
static void _StreamParseHeader(StreamCmdRefT *pCmdRef, const char *pHeader)
{
    ...
    if ((pHeaderInfo = stristr(pHeader, "icy-metaint")) != NULL)
    {
        // parse meta interval
    ...
    if ((pHeaderInfo = stristr(pHeader, "icy-name:")) != NULL)
    {
    ...
    if ((pHeaderInfo = stristr(pHeader, "icy-genre:")) != NULL)
    {
    ...
    if ((pHeaderInfo = stristr(pHeader, "icy-br:")) != NULL)
    {
...
static int32_t _StreamProcessMetaData(StreamCmdRefT *pCmdRef, const uint8_t *pData, int32_t iSize)
{
    ...
    for ( iConsumed = 0; iSize > 0; )
    {
        ...
        if (pCmdRef->iMetaSize > 0)
        {
            ...
            strsubzcat(pCmdRef->strMetaBuffer, sizeof(pCmdRef->strMetaBuffer), (char *)pData, iConsume);
...
static int32_t _ProtoStreamCallback(ProtoStreamRefT *pProtoStream, ProtoStreamStatusE eStatus, const uint8_t *pBuffer, int32_t iBufSize, void *pUserData)
{
    ...
    if ((eStatus == PROTOSTREAM_STATUS_BEGIN) || (eStatus == PROTOSTREAM_STATUS_DATA))
    {
        ...
        for ( iResult = 1; iResult > 0; )
        {
            ...
            if (iBufSize > 0)
            {
                iResult = mp3decodeprocess(pCmdRef->pDecoder, pBuffer, iBufSize);
                if (iResult > 0)
                {
                    pBuffer += iResult;
                    iBufSize -= iResult;
...
static int32_t _StreamUpdate(StreamCmdRefT *pCmdRef)
{
    ...
    if ((iResult = ProtoStreamRead(pCmdRef->pProtoStream, pBuffer, sizeof(aBuffer), sizeof(aBuffer))) > 0)
    {
...
static void _SubcmdStreamCreate(void *_pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    // create the decoder
    pCmdRef->pDecoder = mp3decodecreate();
...
static void _SubcmdStreamDestroy(void *_pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    // destroy decoder
    mp3decodedestroy(pCmdRef->pDecoder);
...
static void _SubcmdStreamOpen(void *_pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    else if (!strnicmp(argv[2], "http", 4)) // built-in url?
    {
        memset(&Station, 0, sizeof(Station));
        strnzcpy(Station.strUrl[0], argv[2], sizeof(Station.strUrl[0]));
        pCmdRef->iCurUrl = 0;
        pStation = &Station;
    }
