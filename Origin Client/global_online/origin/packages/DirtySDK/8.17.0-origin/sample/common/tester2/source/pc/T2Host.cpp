...
static LRESULT CALLBACK _T2HostCommandEditProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (msg == WM_KEYDOWN)
    {
        ...
        if (wParam == VK_UP)
        {
            TesterHostCoreGetHistory(g_pHostCore, -1, strCommandLine, sizeof(strCommandLine));
            iRedraw = 1;
        }
...
static LRESULT CALLBACK _T2HostDialogProc(HWND win, UINT msg, WPARAM wparm, LPARAM lparm)
{
    ...
    if (msg == WM_INITDIALOG) 
    {
        ...
        g_hEditProc = reinterpret_cast<WNDPROC>(SetWindowLong(GetDlgItem(win, IDC_INPUT), GWL_WNDPROC, (LPARAM)_T2HostCommandEditProc));
...
int32_t APIENTRY WinMain(HINSTANCE inst, HINSTANCE prev, char *cmdline, int32_t show)
{
    ...
    {
        pBase += strlen("-path=");
        strnzcpy(strBase, pBase, sizeof(strBase));
    ...
    if ((pBase = strstr(cmdline, "-connect=")) != NULL)
    {
        ...
        strnzcpy(strHostName, pBase, sizeof(strHostName));
    ...
    TagFieldSetNumber(strParams, sizeof(strParams), "PLATFORM", TESTER_PLATFORM_PC);
