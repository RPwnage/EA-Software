...
static void _MbTestKBrd(void *pCmdRef, int32_t argc, char **argv, unsigned char bHelp)
{
	...
	strnzcpy((char*)strMessageBuffer, "This is the message buffer", sizeof(strMessageBuffer));
    if (argc > 2)
    {
        int32_t iWord;

        strnzcpy((char*)strMessageBuffer, argv[2], sizeof(strMessageBuffer));
        for (iWord = 3; iWord < argc; iWord++)
        {
            strnzcat((char*)strMessageBuffer, argv[iWord], sizeof(strMessageBuffer));
        }
    }

	strnzcpy((char*)strInitTextBuffer, "This is the initial text", sizeof(strInitTextBuffer));
