...
static void _TesterProfileGenerateHistoryFilename(TesterProfileEntryT *pEntry)
{
    ...
    iChecksum=0;
    ZMemSet(strChecksum, 0, sizeof(strChecksum));
    ZMemSet(pEntry->strHistoryFile, 0, sizeof(pEntry->strHistoryFile));
    for(iNumChars = 0; 
...
static void _TesterProfileKillAllEntries(TesterProfileT *pState)
{
    ...
    ZMemSet(pState->Profiles, 0, sizeof(pState->Profiles));
}
...
static void _TesterProfileParseLine(char *pData, TesterProfileEntryT *pEntry)
{
    ...
    ZMemSet(pEntry, 0, sizeof(*pEntry));

    // populate with data
    TagFieldGetString(TagFieldFind(pData, TESTERPROFILE_PROFILENAME_TAGVALUE),  pEntry->strProfileName,      sizeof(pEntry->strProfileName),      TESTERPROFILE_PROFILENAME_VALUEDEFAULT);
    TagFieldGetString(TagFieldFind(pData, TESTERPROFILE_CONTROLDIR_TAGVALUE),   pEntry->strControlDirectory, sizeof(pEntry->strControlDirectory), TESTERPROFILE_CONTROLDIR_VALUEDEFAULT);
    TagFieldGetString(TagFieldFind(pData, TESTERPROFILE_COMMANDLINE_TAGVALUE),  pEntry->strCommandLine,      sizeof(pEntry->strCommandLine),      TESTERPROFILE_COMMANDLINE_VALUEDEFAULT);
    TagFieldGetString(TagFieldFind(pData, TESTERPROFILE_LOGDIRECTORY_TAGVALUE), pEntry->strLogDirectory,     sizeof(pEntry->strLogDirectory),     TESTERPROFILE_LOGDIRECTORY_VALUEDEFAULT);
    TagFieldGetString(TagFieldFind(pData, TESTERPROFILE_PLATFORM_TAGVALUE),     pEntry->strPlatform,         sizeof(pEntry->strPlatform),         TESTERPROFILE_PLATFORM_VALUEDEFAULT);
    TagFieldGetString(TagFieldFind(pData, TESTERPROFILE_HOSTNAME_TAGVALUE),     pEntry->strHostname,         sizeof(pEntry->strHostname),         TESTERPROFILE_HOSTNAME_VALUEDEFAULT);
    TagFieldGetString(TagFieldFind(pData, TESTERPROFILE_HISTORYFILE_TAGVALUE),  pEntry->strHistoryFile,      sizeof(pEntry->strHistoryFile),      TESTERPROFILE_HISTORYFILE_VALUEDEFAULT);
    pEntry->uLogEnable =      (unsigned char)TagFieldGetNumber(TagFieldFind(pData, TESTERPROFILE_LOGENABLE_TAGVALUE),      TESTERPROFILE_LOGENABLE_VALUEDEFAULT);
    pEntry->uNetworkStartup = (unsigned char)TagFieldGetNumber(TagFieldFind(pData, TESTERPROFILE_NETWORKSTART_TAGVALUE),   TESTERPROFILE_NETWORKSTART_VALUEDEFAULT);
    pEntry->uDefault =        (unsigned char)TagFieldGetNumber(TagFieldFind(pData, TESTERPROFILE_DEFAULTPROFILE_TAGVALUE), 0);
...
TesterProfileT *TesterProfileCreate(void)
{
    ...
    ZMemSet(pState, 0, sizeof(TesterProfileT));
...
int32_t TesterProfileConnect(TesterProfileT *pState, const char *pFilename)
{
    ...
    strnzcpy(pState->strFilename, pFilename, sizeof(pState->strFilename));
...
int32_t TesterProfileAdd(TesterProfileT *pState, TesterProfileEntryT *pEntry) 
{
    ...
    if(pTarget != NULL)
    {
        ZMemCpy(pTarget, pEntry, sizeof(TesterProfileEntryT));
    }
...
int32_t TesterProfileDelete(TesterProfileT *pState, const char *pName)
{
    ...
    for(uLoop = 0; uLoop < pState->uProfileTail; uLoop++)
    {
          ...
          (strcmp(pStoredProfileName, pName) == 0))
        {
            ...
            ZMemSet(&pState->Profiles[uLoop],0,sizeof(TesterProfileEntryT));
        }
    }
...
int32_t TesterProfileSave(TesterProfileT *pState)
{
    ...
    for (iEntryNum = 0; iEntryNum < TESTERPROFILE_NUMPROFILES_DEFAULT; iEntryNum++)
    {
        ...
        if (TesterProfileGetTagField(pState, iEntryNum, strProfile, sizeof(strProfile)) >= 0)
        {
...
int32_t TesterProfileGet(TesterProfileT *pState, int32_t iIndex, TesterProfileEntryT *pDest)
{
    ...
    // copy the data in
    ZMemCpy(pDest, pEntry, sizeof(TesterProfileEntryT));
...
/*F********************************************************************************/
/*!
    \Function TesterProfileGetTagField
...
*/
/********************************************************************************F*/
int32_t TesterProfileGetTagField(TesterProfileT *pState, int32_t iIndex, char *pDest, int32_t iSize)
{
    TesterProfileEntryT Entry;
    int32_t iNumChars, iResult;
    char cDivider;
    ...
    // otherwise dump the tagfield into the buffer
    TagFieldClear(pDest, iSize);
    cDivider = TagFieldDivider('\t');
    iNumChars = TagFieldPrintf(pDest, (iSize - 2), 
        "%s=%s %s=%s %s=%s %s=%s %s=%s %s=%s %s=%s %s=%d %s=%d %s=%d\n",
        TESTERPROFILE_PROFILENAME_TAGVALUE,     Entry.strProfileName,  
        TESTERPROFILE_PLATFORM_TAGVALUE,        Entry.strPlatform, 
        TESTERPROFILE_HOSTNAME_TAGVALUE,        Entry.strHostname,
        TESTERPROFILE_CONTROLDIR_TAGVALUE,      Entry.strControlDirectory, 
        TESTERPROFILE_COMMANDLINE_TAGVALUE,     Entry.strCommandLine,
        TESTERPROFILE_LOGDIRECTORY_TAGVALUE,    Entry.strLogDirectory, 
        TESTERPROFILE_HISTORYFILE_TAGVALUE,     Entry.strHistoryFile,
        TESTERPROFILE_LOGENABLE_TAGVALUE,       Entry.uLogEnable,  
        TESTERPROFILE_NETWORKSTART_TAGVALUE,    Entry.uNetworkStartup,     
        TESTERPROFILE_DEFAULTPROFILE_TAGVALUE,  Entry.uDefault);
...
int32_t TesterProfileDisconnect(TesterProfileT *pState)
{
    ...
    ZMemSet(pState->strFilename, 0, sizeof(pState->strFilename));
