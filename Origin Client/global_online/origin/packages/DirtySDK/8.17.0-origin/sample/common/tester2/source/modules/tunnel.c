...
static void _TunnelCreate(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    {
        ZPrintf("   usage: %s create <clientId>\n", argv[0]);
        return;
    }

    // get the port
    iTunnelPort = (argc == 4) ? strtol(argv[3], NULL, 10) : 9600;

    // create the tunnel
    pApp->pProtoTunnel = ProtoTunnelCreate(4, iTunnelPort);

    // set client id
    uClientId = strtol(argv[2], NULL, 16);
...
static void _TunnelAlloc(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    {
        ZPrintf("   usage: %s alloc [clientId] [addr] <key>\n", argv[0]);
        return;
    }

    // ref key
    pKey = (argc == 5) ? argv[4] : strKey;
    
    memset(&Info, 0, sizeof(Info));
    Info.uRemoteClientId = strtol(argv[2], 0, 16);
...
static void _TunnelFree(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    // free a mapping
    uTunnelId = strtol(argv[2], NULL, 16);
...
static void _TunnelControl(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    iValue = (argc > 3) ? strtol(argv[3], NULL, 10) : 0;
    iValue2 = (argc > 4) ? strtol(argv[4], NULL, 10) : 0;
