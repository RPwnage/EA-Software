...
*/
/********************************************************************************F*/
static int32_t _NetPrintOverflow(int32_t iVersion, int32_t iBufferLimit, const char *pPreGen, int32_t iExpectedResult, const char *pFmt, ...)
{
    char strTest[128];
    ...
    // format the output
    va_start(Args, pFmt);
    iResult = (iVersion == 0) ? ds_vsnzprintf(strTest, iBufferLimit, pFmt, Args) : ds_vsnzprintf2(strTest, iBufferLimit, pFmt, Args);
    va_end(Args);
...
#endif
#if DIRTYCODE_PLATFORM == DIRTYCODE_LINUX || DIRTYCODE_PLATFORM == DIRTYCODE_MAC
...
static int32_t _NetPrintTestMisc(void)
{
    ...
    strnzcat(strTest, strTest2, sizeof(strTest));
...
static int32_t _NetPrintTestCust(void)
{
    ...
    iResult += _NetPrintPregen("addr=192.168.0.1", "addr=%a", 0xC0A80001);
...
static int32_t _NetPrintTestOver(void)
{
    ...
    ZPrintf("netprint: Overflow tests (old)\n");
    // test some overflow scenarios with snzprintf 
    iResult0 += _NetPrintOverflow(0,  0, "",                      0,    "%s", strOver);
    iResult0 += _NetPrintOverflow(0,  1, "",                      0,    "%s", strOver);
    iResult0 += _NetPrintOverflow(0, 12, "",                      0,    "%s", strOver);
    iResult0 += _NetPrintOverflow(0, 14, "",                      0,    "%s", strOver); 
    iResult0 += _NetPrintOverflow(0, 15, "<<<overflow>>>",       14,    "%s", strOver);
    iResult0 += _NetPrintOverflow(0, 20, "",                      0,  "%20s", strOver);
    iResult0 += _NetPrintOverflow(0, 20, "",                      0, "%-20s", strOver);
    iResult0 += _NetPrintOverflow(0, 21, "      <<<overflow>>>", 20,  "%20s", strOver);
    iResult0 += _NetPrintOverflow(0, 16, "",                      0,  "-%s-", strOver);
    iResult0 += _NetPrintOverflow(0, 17, "-<<<overflow>>>-",     16,  "-%s-", strOver);
    ZPrintf("netprint: %d overflow test discrepencies\n", iResult0);
    ZPrintf("netprint: ------------------------------------\n");

    // test some overflow scenarios with snzprintf2
    ZPrintf("netprint: Overflow tests (C99)\n");
    iResult1 += _NetPrintOverflow(1,  0, "",                     14,    "%s", strOver);
    iResult1 += _NetPrintOverflow(1,  1, "",                     14,    "%s", strOver);
    iResult1 += _NetPrintOverflow(1, 12, "<<<overflow",          14,    "%s", strOver);
    iResult1 += _NetPrintOverflow(1, 14, "<<<overflow>>",        14,    "%s", strOver); 
    iResult1 += _NetPrintOverflow(1, 15, "<<<overflow>>>",       14,    "%s", strOver);
    iResult1 += _NetPrintOverflow(1, 20, "      <<<overflow>>",  20,  "%20s", strOver);
    iResult1 += _NetPrintOverflow(1, 20, "<<<overflow>>>     ",  20, "%-20s", strOver);
    iResult1 += _NetPrintOverflow(1, 21, "      <<<overflow>>>", 20,  "%20s", strOver);
    iResult1 += _NetPrintOverflow(1, 16, "-<<<overflow>>>",      16,  "-%s-", strOver);
    iResult1 += _NetPrintOverflow(1, 17, "-<<<overflow>>>-",     16,  "-%s-", strOver);
