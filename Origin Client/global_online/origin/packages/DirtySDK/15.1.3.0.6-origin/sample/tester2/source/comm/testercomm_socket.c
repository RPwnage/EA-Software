...
static int32_t _TesterCommCheckInput(TesterCommT *pState)
{
    ...
    uint16_t aJson[sizeof(pSocketState->strInputData)];
    ...
    JsonParse(aJson, sizeof(aJson)/sizeof(*aJson), pSocketState->strInputData, -1);

    // push each line onto the list
    iLinesRecv = 0;
    while ((pInputLoop = JsonFind2(aJson, NULL, "msg[", iLinesRecv)) != NULL)
    {
        char strBuffer[sizeof(pState->LineData.strBuffer)];
        int32_t iBufLen;

        ds_memclr(&pState->LineData, sizeof(pState->LineData));
        pState->LineData.iType = JsonGetInteger(JsonFind2(aJson, pInputLoop, ".TYPE", iLinesRecv), 0);
        iBufLen = JsonGetString(JsonFind2(aJson, pInputLoop, ".TEXT", iLinesRecv), strBuffer, sizeof(strBuffer), "");
...
static int32_t _TesterCommCheckOutput(TesterCommT *pState)
{
    ...
    for (iLinesSent = 0; (pPeekResult != NULL); iLinesSent += 1)
    {
        char strBuffer[sizeof(pState->LineData.strBuffer)];
