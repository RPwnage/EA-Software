...
static void _TesterProfileParseLine(char *pData, TesterProfileEntryT *pEntry)
{
    ...
    JsonGetString(JsonFind(aJson, TESTERPROFILE_PLATFORM_TAGVALUE), pEntry->strPlatform, sizeof(pEntry->strPlatform), TESTERPROFILE_PLATFORM_VALUEDEFAULT);
...
int32_t TesterProfileGetJson(TesterProfileT *pState, int32_t iIndex, char *pDest, int32_t iSize)
{
    ...
    JsonAddStr(pDest, TESTERPROFILE_PROFILENAME_TAGVALUE, Entry.strPlatform);
