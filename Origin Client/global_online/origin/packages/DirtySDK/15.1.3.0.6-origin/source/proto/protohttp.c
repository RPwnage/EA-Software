...
static void _ProtoHttpClose(ProtoHttpRefT *pState, const char *pReason)
{
    ...
    NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] closing connection: %s\n", (uintptr_t)pState, pReason));
...
static int32_t _ProtoHttpFormatRequest(ProtoHttpRefT *pState, const char *pUrl, const char *pData, int64_t iDataLen, ProtoHttpRequestTypeE eRequestType)
{
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] %s %s://%s:%d%s\n", (uintptr_t)pState, _ProtoHttp_strRequestNames[eRequestType],
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] requesting new connection -- url change to %s\n", (uintptr_t)pState, strHost));
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] requesting new connection -- previous connection was closed\n", (uintptr_t)pState));
        pState->bNewConnection = TRUE;
    }
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] requesting new connection -- current state of %d does not allow connection reuse\n", (uintptr_t)pState, eState));
        pState->bNewConnection = TRUE;
    }
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] requesting new connection -- reuse on put/post disabled\n", (uintptr_t)pState));
        pState->bNewConnection = TRUE;
    }
...
static void _ProtoHttpSendRequest(ProtoHttpRefT *pState)
{
    ...
    if (pState->bNewConnection == FALSE)
    {
        ...
        {
            NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] requesting new connection -- receive on previous connection returned data (0x%02x)\n", (uintptr_t)pState, (uint8_t)cTest));
            pState->bNewConnection = TRUE;
        }
        else if (iResult < 0)
        {
            NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] requesting new connection -- received %d error response from previous connection\n", (uintptr_t)pState, iResult));
            pState->bNewConnection = TRUE;
        }
    }
    ...
    if (pState->bNewConnection == TRUE)
    {
        ...
        // start connect
        NetPrintfVerbose((pState->iVerbose, 2, "protohttp: [0x%08x] connect start (tick=%u)\n", (uintptr_t)pState, NetTick()));
    ...
    {
        // advance state
        NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] reusing previous connection (keep-alive)\n", (uintptr_t)pState));
...
static uint32_t _ProtoHttpRetrySendRequest(ProtoHttpRefT *pState)
{
    ...
    NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] request failure on keep-alive connection; retrying\n", (uintptr_t)pState));
...
static int32_t _ProtoHttpResizeBuffer(ProtoHttpRefT *pState, int32_t iBufMax)
{
    ...
    NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] resizing input buffer from %d to %d bytes\n", (uintptr_t)pState, pState->iInpMax, iBufMax));
...
static int32_t _ProtoHttpParseHeader(ProtoHttpRefT *pState)
{
    ...
    NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] received %d response (%d bytes)\n", (uintptr_t)pState, pState->iHdrCode, pState->iHeadSize));
    ...
    if (pState->bCloseHdr == FALSE)
    {
        ...
        {
            NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] lost %d piped requests due to server connection-close request\n", (uintptr_t)pState, pState->iPipedRequests));
        ...
        for (iRequestType = 0; iRequestType < PROTOHTTP_NUMREQUESTTYPES; iRequestType += 1)
        {
            if (!strncmp(pState->pInpBuf, _ProtoHttp_strRequestNames[iRequestType], strlen(_ProtoHttp_strRequestNames[iRequestType])))
            {
                ...
                *pHdrEnd = '\0';
                ...
                NetPrintfVerbose((pState->iVerbose, 2, "protohttp: [0x%08x] tick=%u\n", (uintptr_t)pState, NetTick()));
                NetPrintWrap(pState->pInpBuf, 80);
                *pHdrEnd = cHdrChr;
                break;
            }
...
{
    // ignore the response
    NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] ignoring %d response from server\n", (uintptr_t)pState, pState->iHdrCode));
...
static void _ProtoHttpProcessRedirect(ProtoHttpRefT *pState)
{
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] skipping auto-redirection of non-http protocol '%s'\n", pState, strKind));
        return;
    }
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] auto-redirection disabled\n", (uintptr_t)pState));
        return;
    }
...
static int32_t _ProtoHttpChunkProcess(ProtoHttpRefT *pState, int32_t iBufMax)
{
    ...
    if (pState->iChkLen == 0)
    {
        ...
        if ((pState->iChkLen = (int32_t)strtol(s, NULL, 16)) == 0)
        {
            ...
            NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] parsed end chunk\n", (uintptr_t)pState));
        ...
        {
            NetPrintfVerbose((pState->iVerbose, 2, "protohttp: [0x%08x] parsed chunk size=%d\n", (uintptr_t)pState, pState->iChkLen));
        }
    ...
    if (iLen >= pState->iChkLen)
    {
        /* got chunk and trailer, so consume it.  we use the internal data size rather than the data size
           ...
           should therefore must not be limited by a requirement to read it.  not doing so results in the
           user not being able to consume the whole chunk if their buffer does not have room for the chunk
           trailer */
        ...
        else
        {
            ...
            {
                NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] waiting for chunk trailer\n", (uintptr_t)pState));
                iChkSize = 0;
            }
            // tell ProtoHttpRecv() to compact recv buffer next time around
            pState->bCompactRecv = TRUE;
...
static int32_t _ProtoHttpSendBuff(ProtoHttpRefT *pState)
{
    ...
    {
        NetPrintf(("protohttp: [0x%08x] failed to send request data (err=%d)\n", (uintptr_t)pState, iResult));
        pState->iInpLen = 0;
        pState->eState = ST_FAIL;
        iSentSize = -1;
    }
...
static int32_t _ProtoHttpSendChunk(ProtoHttpRefT *pState, const char *pStrBuf, int32_t iSize)
{
    ...
    if ((iBuffSize = _ProtoHttpFormatChunk(pState, pStrBuf, iSize)) < 0)
    {
        ...
        if ((iCompSize = _ProtoHttpCompactBuffer(pState)) > 0)
        {
            ...
            NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] compacted send buffer (%d bytes)\n", (uintptr_t)pState, iCompSize));
...
static void _ProtoHttpHeaderProcess(ProtoHttpRefT *pState)
{
    ...
    if (_ProtoHttpParseHeader(pState) < 0)
    {
        ...
        if (pState->iInpLen == pState->iInpMax)
        {
            if (pState->iInpOvr == 0)
            {
                NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] input buffer too small for response header\n", (uintptr_t)pState));
            }
...
static void _ProtoHttpWriteCbProcess(ProtoHttpRefT *pState)
{
    ...
    if ((pState->eState == ST_BODY) || ((pState->eState == ST_DONE) && (pState->iBodyRcvd < pState->iBodySize)))
    {
...
void ProtoHttpSetBaseUrl(ProtoHttpRefT *pState, const char *pUrl)
{
    ...
    // set base info
    NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] setting base url to %s://%s:%d\n", (uintptr_t)pState, iSecure ? "https" : "http", strHost, iPort));
...
int32_t ProtoHttpControl(ProtoHttpRefT *pState, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iSelect == 'hver')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] header type verification %s\n", (uintptr_t)pState, iValue ? "enabled" : "disabled"));
    ...
    if (iSelect == 'keep')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] setting keep-alive to %d\n", (uintptr_t)pState, iValue));
        pState->iKeepAlive = pState->iKeepAliveDflt = iValue;
        return(0);
    }
    if (iSelect == 'pipe')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] pipelining %s\n", (uintptr_t)pState, iValue ? "enabled" : "disabled"));
        pState->bPipelining = iValue ? TRUE : FALSE;
        return(0);
    }
    if (iSelect == 'pnxt')
    {
        NetPrintfVerbose((pState->iVerbose, 2, "protohttp: [0x%08x] proceeding to next pipeline result\n", (uintptr_t)pState));
        pState->bPipeGetNext = TRUE;
        return(0);
    }
    if (iSelect == 'prxy')
    {
        NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] setting %s as proxy server\n", (uintptr_t)pState, pValue));
    ...
    if (iSelect == 'rput')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] connection reuse on put/post %s\n", (uintptr_t)pState, iValue ? "enabled" : "disabled"));
    ...
    if (iSelect == 'time')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] setting timeout to %d ms\n", (uintptr_t)pState, iValue));
...
int32_t ProtoHttpStatus(ProtoHttpRefT *pState, int32_t iSelect, void *pBuffer, int32_t iBufSize)
{
    ...
    if ((iSelect == 'alrt') && (pState->pSsl != NULL))
    {
        return(ProtoSSLStat(pState->pSsl, iSelect, pBuffer, iBufSize));
    }
    ...
    if ((iSelect == 'hres') && (pState->pSsl != NULL))
    {
    ...
    if ((iSelect == 'serr') && (pState->pSsl != NULL))
    {
...
int32_t ProtoHttpCheckKeepAlive(ProtoHttpRefT *pState, const char *pUrl)
{
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] check for keep-alive detected connection close\n", (uintptr_t)pState));
        pState->bConnOpen = FALSE;
    }
...
void ProtoHttpUpdate(ProtoHttpRefT *pState)
{
    ...
    if (pState->eState == ST_DONE)
    {
        if (pState->bPipelining && (pState->iPipedRequests > 0))
        {
            ...
            {
                NetPrintfVerbose((pState->iVerbose, 1, "protohttp: [0x%08x] handling piped request\n", (uintptr_t)pState));
                _ProtoHttpCompactBuffer(pState);
                pState->eState = ST_HEAD;
