...
#define PROTPHTTP_KEEPALIVE_MAX_IDLE_DEFAULT (120*1000)  // 120 seconds
...
static uint8_t _ProtoHttpIsPEMFormat(const uint8_t *pCACert, int32_t iCertSize);
...
void _ProtoHttpCreateContexts(ProtoHttpRefT *pState)
{
    ...
    NetPrintf(("protohttpps4: creating http contexts\n"));
...
{
    NetPrintf(("protohttpps4: freeing http contexts\n"));
...
void _ProtoHttpApplyCAs(ProtoHttpRefT *pState)
{
    ...
    if (pState->uCaListVersionLoaded != uCaListVersion || pState->bCAsApplied == FALSE)
    {
        // assume iHttpContextId is set at this point
        if ((iResult = sceHttpsUnloadCert(pState->iHttpContextId)) < 0)
        {
            NetPrintf(("protohttpps4: [0x%08x] sceHttpsUnloadCert failed with error %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
        }
        ...
        for (iCert = 0; iCert < PROTOHTTP_GOSCA_COUNT; ++iCert)
        {
            ...
            if (iResult < 0)
            {
                NetPrintf(("protohttpps4: [0x%08x] sceHttpsLoadCert failed to load a GOS CA with error %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            }
        }
        ...
        while (pCert != NULL)
        {
            ...
            if (iResult < 0)
            {
                NetPrintf(("protohttpps4: [0x%08x] sceHttpsLoadCert failed to load a CA with error %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            }
            pCert = pCert->pNext;
        }
    ...
    if (pState->bUsePrivateKey)
    {
        ...
        {
            NetPrintfVerbose((pState->iVerbose, 0, "protohttpps4: [0x%08x] sceHttpsLoadCert failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
        }
    }
...
void _ProtoHttpCreateConnectionTemplate(ProtoHttpRefT *pState)
{
    ...
    if (pState->iSceTemplateId < 0)
    {
        NetPrintf(("protohttpps4: [0x%08x] sceHttpCreateTemplate failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(pState->iSceConnId), pState->iSceConnId));
    ...
    else
    {
        ...
        if (pState->bSetSslVers)
        {
            ...
            {
                NetPrintfVerbose((pState->iVerbose, 0, "protohttpps4: [0x%08x] sceHttpsSetSslVersion failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
        ...
        if (pState->bSetHttpsFlags)
        {
            if (pState->iSceTemplateId > 0)
            {
                ...
                {
                    NetPrintfVerbose((pState->iVerbose, 0, "protohttpps4: [0x%08x] sceHttpsDisableOption failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
                ...
                {
                    NetPrintfVerbose((pState->iVerbose, 0, "protohttpps4: [0x%08x] sceHttpsEnableOption failed with code %s (0x%08x), flags 0x%08x\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult, pState->uSceHttpFlags));
                    pState->bSetHttpsFlags = FALSE;
                    pState->eState = ST_FAIL;
                    pState->bClosed = TRUE;
                    sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps4: [0x%08x] sceHttpSetRedirectCallback failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDeleteTemplate(pState->iSceTemplateId);
            pState->iSceTemplateId = 0;
        ...
        {
            NetPrintf(("protohttpps4: [0x%08x] sceHttpSetNonblock failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDeleteTemplate(pState->iSceTemplateId);
            pState->iSceTemplateId = 0;
        ...
        if (pState->eState != ST_FAIL)
        {
            ...
            {
                NetPrintf(("protohttpps4: [0x%08x] sceHttpCreateEpoll failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
                pState->eState = ST_FAIL;
                pState->bClosed = TRUE;
                pState->bEpollInitialized = FALSE;
                sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        // set the Sony connection timeouts to a value 5 seconds higher than our timeout so they do not complete
        if (pState->eState != ST_FAIL && (iResult = sceHttpSetConnectTimeOut(pState->iSceTemplateId, (pState->uTimeout * 1000) + 5000000)) < 0)
        {
            NetPrintf(("protohttpps4: [0x%08x] sceHttpSetConnectTimeOut failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps4: [0x%08x] sceHttpSetRecvTimeOut failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps4: [0x%08x] sceHttpSetResolveTimeOut failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps4: [0x%08x] sceHttpSetSendTimeOut failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps4: [0x%08x] sceHttpSetResolveRetry failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps4: [0x%08x] sceHttpSetInflateGZIPEnabled failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
...
static void _ProtoHttpClose(ProtoHttpRefT *pState, const char *pReason)
{
    ...
    if (pState->iSceRequestId > 0)
    {
        ...
        if (iResult != SCE_OK)
        {
            NetPrintf(("protohttpps4: [0x%08x] unable to delete request %d, err=%s/0x%08x\n", (uintptr_t)pState, pState->iSceRequestId, DirtyErrGetName(iResult), iResult));
        }
...
static int32_t _ProtoHttpFormatRequestHeader(ProtoHttpRefT *pState, const char *pUrl, const char *pHost, int32_t iPort, int32_t iSecure, const char *pRequest)
{
    ...
    {
        if ((iOffset = pState->pCustomHeaderCb(pState, pInpBuf, iInpMax, pState->pData, pState->iPostSize, pState->pCallbackRef)) < 0)
        {
    ...
    if (iOffset > 0)
    {
        char strHeaderName[1024];
        char strValuerName[1024];
        const char *pCurrentHdr = pState->strRequestHdr;

        NetPrintfVerbose((pState->iVerbose, 3, "protohttpps4: [0x%08x] Adding headers \n", (uintptr_t)pState));
        ...
        {
            NetPrintfVerbose((pState->iVerbose, 3, "%s%s%s\n", strHeaderName, 
                                  ds_strnlen(strValuerName, 1) > 0 ? ":" : " ", strValuerName));
            ...
            {
                if ((iResult = sceHttpAddRequestHeader(pState->iSceRequestId, strHeaderName, strValuerName, SCE_HTTP_HEADER_OVERWRITE)) < 0)
                {
                    NetPrintf(("protohttpps4: unable to add header %s, error %s (0x%08x)\n", strHeaderName, DirtyErrGetName(iResult), iResult));
                }
            }
        }
    }
...
static void _ProtoHttpSendRequest(ProtoHttpRefT *pState)
{
    ...
    if (pState->bNewConnection == TRUE)
    {
        ...
        if (pState->iSceTemplateId > 0)
        {
            ...
            {
                NetPrintf(("protohttpps4: [0x%08x] sceHttpCreateConnectionWithURL failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(pState->iSceConnId), pState->iSceConnId));
                pState->eState = ST_FAIL;
                pState->bClosed = TRUE;
                pState->iSceConnId = 0;
            }
    ...
    if (pState->iSceConnId > 0)
    {
        ...
        if (pState->iSceRequestId < 0)
        {
            NetPrintf(("protohttpps4: [0x%08x] sceHttpCreateRequestWithURL failed with type %s and error code %s (0x%08x)\n",
                (uintptr_t)pState, _ProtoHttp_strRequestNames[pState->eRequestType], DirtyErrGetName(pState->iSceRequestId), pState->iSceRequestId));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            pState->iSceRequestId = 0;
            sceHttpDeleteConnection(pState->iSceConnId);
    ...
    if (pState->iSceRequestId > 0)
    {
        ...
        {
            NetPrintf(("protohttpps4: [0x%08x] failed to setup request headers %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(pState->iSceConnId), pState->iSceConnId));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            iResult = sceHttpDeleteRequest(pState->iSceRequestId);
            if (iResult != SCE_OK)
...
{
    char *s = pState->strSceRespHeader;
    char *t = pState->strSceRespHeader+pState->iSceRespHeaderLength-3;
    char strTemp[128];
    ...
    // save the head size
    pState->iHeadSize = (int32_t)(s+4-pState->strSceRespHeader);
    ...
    {
        if (strncmp(pState->strSceRespHeader, "HTTP", 4))
        {
            // header is invalid
    ...
    pState->bHttp1_0 = !strncmp(pState->strSceRespHeader, "HTTP/1.0", 8);

    // parse header code
    pState->iHdrCode = ProtoHttpParseHeaderCode(pState->strSceRespHeader);
    
    // copy header to header cache
    ds_strnzcpy(pState->strHdr, pState->strSceRespHeader, sizeof(pState->strHdr));
    ...
    {
        NetPrintWrap(pState->strHdr, 80);
    }
    #endif

    // parse content-length field
    if (ProtoHttpGetHeaderValue(pState, pState->strSceRespHeader, "content-length", strTemp, sizeof(strTemp), NULL) != -1)
    {
    ...
    if (ProtoHttpGetHeaderValue(pState, pState->strSceRespHeader, "last-modified", strTemp, sizeof(strTemp), NULL) != -1)
    {
    ...
    if (ProtoHttpGetHeaderValue(pState, pState->strSceRespHeader, "transfer-encoding", strTemp, sizeof(strTemp), NULL) != -1)
    {
    ...
    {
        ProtoHttpGetHeaderValue(pState, pState->strSceRespHeader, "connection", strTemp, sizeof(strTemp), NULL);
...
{
    int32_t iResult = 0;
    char *pSceRespHeader = NULL;
    if ((iResult = sceHttpGetAllResponseHeaders(pState->iSceRequestId, &pSceRespHeader, &pState->iSceRespHeaderLength)) >= 0)
    {
        ds_strnzcpy(pState->strSceRespHeader, pSceRespHeader, DS_MIN(pState->iSceRespHeaderLength+1, (int32_t)sizeof(pState->strSceRespHeader)));
    }
    else
    {
        ...
        NetPrintf(("protohttpps4: [0x%08x] sceHttpGetAllResponseHeaders failed with code %s (0x%08x) for request id %d\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult, pState->iSceRequestId));
    }
...
static int32_t _ProtoHttpRedirectCallback(int32_t iRequest, int32_t iStatusCode, int32_t *pMethod, const char *pLocation, void *pUserArg)
{
    ...
    int32_t iResult = 0;
    uint8_t bFail = FALSE; 
    NetPrintfVerbose((pState->iVerbose, 2, "protohttpps4: [0x%08x] processing redirect for request 0x%08x code %d\n", (uintptr_t)pState, iRequest, iStatusCode));
    
    if ((iResult = _ProtoHttpGetHeader(pState)) >= 0)
    {
        // trigger recv header user callback, if specified
        pState->iHdrCode = ProtoHttpParseHeaderCode(pState->strSceRespHeader);
        ds_strnzcpy(strHeader, pState->strSceRespHeader, (int32_t)sizeof(strHeader));
        if (pState->pReceiveHeaderCb != NULL)
        {
            pState->pReceiveHeaderCb(pState, strHeader, (uint32_t)ds_strnlen(strHeader, (int32_t)sizeof(strHeader)) , pState->pCallbackRef);
        }
    }
    ...
    // set up the next http query
    sceHttpAbortRequest(pState->iSceRequestId);
    ...
    if (ProtoHttpGetHeaderValue(pState, pState->strSceRespHeader, "location", strLocation, sizeof(strLocation), NULL) != -1)
    {
    ...
    // process the redirect next frame
...
static int32_t _ProtoHttpCookieRecvCallback(int request, const char *url, const char *cookieHeader, size_t headerLen, void *userArg)
{
    ...
    NetPrintfVerbose((pState->iVerbose, 2, "protohttpps4: [0x%08x] received cookie header, %d bytes\n", (uintptr_t)pState, headerLen));
    NetPrintfVerbose((pState->iVerbose, 3, "protohttpps4: [0x%08x] cookie: %s\n", (uintptr_t)pState, cookieHeader));
#endif
    return(-1);
}
...
static int32_t _ProtoHttpSend(ProtoHttpRefT *pState, const char *pStrBuf, int32_t iSize)
{
    ...
    else if (iResult == 0)
    {
        ...
        {
            NetPrintMem(pStrBuf, iSize, "http-send");
        }
        #endif

        // send data, so update timeout
        pState->uTimer = NetTick() + pState->uTimeout;
    ...
    {
        NetPrintf(("protohttpps4: [0x%08x] error %s (0x%08x) sending %d bytes\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult, iSize));
        pState->eState = ST_FAIL;
        pState->iSslFail = 0;
        pState->uSslFailDet = 0;
        pState->iHresult = 0;
...
static int32_t _ProtoHttpSendBuff(ProtoHttpRefT *pState)
{
    ...
    {
        NetPrintf(("protohttpps4: [0x%08x] failed to send request data (err=%s/0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
        pState->iInpLen = 0;
        pState->eState = ST_FAIL;
        iSentSize = -1;
    }
...
static int32_t _ProtoHttpRecvBody(ProtoHttpRefT *pState)
{
    ...
    {
        NetPrintf(("protohttpps4: [0x%08x] ST_FAIL (err=%s/0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
        pState->eState = ST_FAIL;
        pState->iSslFail = 0;
        pState->uSslFailDet = 0;
        pState->iHresult = 0;
...
static void _ProtoHttpWriteCbProcess(ProtoHttpRefT *pState)
{
    ...
    else if (pState->eState > ST_BODY)
    {
        ...
        if (pState->eState == ST_FAIL)
        {
            pState->pWriteCb(pState, &WriteCbInfo, "", pState->bTimeout ? PROTOHTTP_TIMEOUT :PROTOHTTP_RECVFAIL, pState->pWriteCbUserData);
        }
...
ProtoHttpRefT *ProtoHttpCreate(int32_t iBufSize)
{
    ...
    pState->uIdleTimeStart = 0;
    pState->iIdleTimeMax = PROTPHTTP_KEEPALIVE_MAX_IDLE_DEFAULT;
...
int32_t ProtoHttpControl(ProtoHttpRefT *pState, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iSelect == 'time')
    {
        ...
        if (pState->iSceTemplateId > 0)
        {
            ...
            {
                NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] sceHttpSetConnectTimeOut failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            }
            if ((iResult = sceHttpSetRecvTimeOut(pState->iSceTemplateId, (pState->uTimeout * 1000) + 5000000)) < 0)
            {
                NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] sceHttpSetRecvTimeOut failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            }
            if ((iResult = sceHttpSetResolveTimeOut(pState->iSceTemplateId, (pState->uTimeout * 1000) + 5000000)) < 0)
            {
                NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] sceHttpSetResolveTimeOut failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            }
            if ((iResult = sceHttpSetSendTimeOut(pState->iSceTemplateId, (pState->uTimeout * 1000) + 5000000)) < 0)
            {
                NetPrintfVerbose((pState->iVerbose, 0, "protohttp: [0x%08x] sceHttpSetSendTimeOut failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            }
        }
        return(0);
    }
    if (iSelect == 'resr')
    {
        ...
        if (pState->iSceTemplateId > 0)
        {
            ...
            {
                NetPrintfVerbose((pState->iVerbose, 0, "protohttpps4: [0x%08x] sceHttpSetSendTimeOut failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            }
        }
        return(0);
    }
...
void ProtoHttpUpdate(ProtoHttpRefT *pState)
{
    ...
    if (pState->eState != ST_IDLE && pState->eState != ST_DONE && pState->eState != ST_FAIL)
    {
        if (pState->iSceRequestId > 0 && pState->bEpollInitialized)
        {
            ...
            if ( iResult > 0 && nbEvent.id == pState->iSceRequestId ) 
            {
                ...
                {
                    NetPrintf(("protohttpps4: [0x%08x] sceHttpWaitRequest failed (err=0x%08x)\n", (uintptr_t)pState, iResult));
                    pState->eState = ST_FAIL;
                    pState->iSslFail = 0;
                    pState->uSslFailDet = 0;
                    pState->iHresult = 0;
            ...
            {
                NetPrintf(("protohttpps4: [0x%08x] sceHttpWaitRequest failed with code %s (0x%08x)\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult));
            }
        }
    }
    ...
    if (pState->eState == ST_RESP)
    {
        ...
        else if (iResult < 0)
        {
            ...
            sceHttpsGetSslError(pState->iSceRequestId, &pState->iSslFail, &pState->uSslFailDet);
            sceHttpGetLastErrno(pState->iSceRequestId, &pState->iHresult);
            NetPrintf(("protohttpps4: [0x%08x] sceHttpGetStatusCode failed with code %s (0x%08x) for request id %d\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult, pState->iSceRequestId));
        }
        // regardless of whether the header code is a success or failure
        else
        {
            NetPrintf(("protohttpps4: [0x%08x] sceHttpGetStatusCode returned http code %d for request id %d, code %s (0x%08x)\n", (uintptr_t)pState, iStatusCode, pState->iSceRequestId, DirtyErrGetName(iResult), iResult));
            pState->eState = ST_HEAD;
        }
    }
    ...
    if (pState->eState == ST_HEAD)
    {
        ...
        if ((iResult = sceHttpGetResponseContentLength(pState->iSceRequestId, &contentLengthType, (uint64_t*)(&(pState->iBodySize)))) < 0)
        {
            ...
            sceHttpsGetSslError(pState->iSceRequestId, &pState->iSslFail, &pState->uSslFailDet);
            sceHttpGetLastErrno(pState->iSceRequestId, &pState->iHresult);
            NetPrintf(("protohttpps4: [0x%08x] sceHttpGetResponseContentLength failed with code %s (0x%08x) for request id %d\n", (uintptr_t)pState, DirtyErrGetName(iResult), iResult, pState->iSceRequestId));
        }
