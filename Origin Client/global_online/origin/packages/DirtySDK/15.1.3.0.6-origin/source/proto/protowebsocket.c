...
#define PROTOWEBSOCKET_INPUTBUFSIZE (256)
...
struct ProtoWebSocketRefT
{
    ...
    char strInpBuf[PROTOWEBSOCKET_INPUTBUFSIZE];    //!< temporary input buffer used for connection establishment
...
static int32_t _ProtoWebSocketSetAppendHeader(ProtoWebSocketRefT *pWebSocket, const char *pAppendHdr)
{
    ...
    {
        NetPrintfVerbose((pWebSocket->iVerbose, 0, "protowebsocket: [%p] ignoring set of append header '%s' that is already set\n", pWebSocket, pAppendHdr));
        return(0);
    }
...
static int32_t _ProtoWebSocketRecv(ProtoWebSocketRefT *pWebSocket, char *pStrBuf, int32_t iSize)
{
    ...
    else
    {
        ...
        ds_memcpy(pStrBuf, pWebSocket->strInpBuf+pWebSocket->iInpOff, iSize);
...
void ProtoWebSocketUpdate(ProtoWebSocketRefT *pWebSocket)
{
    ...
    {
        if ((iResult = _ProtoWebSocketRecv(pWebSocket, pWebSocket->strInpBuf+pWebSocket->iInpLen, sizeof(pWebSocket->strInpBuf)-pWebSocket->iInpLen)) > 0)
        {
            pWebSocket->iInpLen += iResult;
            // see if we have the complete header
            pStart = pWebSocket->strInpBuf;
            pEnd = pWebSocket->strInpBuf+pWebSocket->iInpLen-3;
            ...
            {
                // process the header
                if ((iResult = _ProtoWebSocketProcessHeader(pWebSocket, pWebSocket->strInpBuf)) >= 0)
                {
                ...
                pWebSocket->iInpOff = (int32_t)(pStart+4-pWebSocket->strInpBuf);
            }
        }
        else if (iResult < 0)
        {
