...
bool _ProtoWebSocketInit(ProtoWebSocketRefT *pWebSocket)
{
    ...
    pWebSocket->pWebSocket->Closed += ref new TypedEventHandler<IWebSocket^, WebSocketClosedEventArgs^> ([pWebSocket] (IWebSocket^ sender, WebSocketClosedEventArgs^ args)
    {
        ...
        ds_memclr(pWebSocket->strReason, PROTO_WEBSOCKET_XBOXONE_MAX_CLOSE_REASON_SIZE);
...
{
    NetPrintf(("protowebsocketxboxone: [%p] ProtoWebSocketRecvMessage is not implemented.\n", pWebSocket));
    return(-1);
}
...
{
    NetPrintf(("protowebsocketxboxone: [%p] ProtoWebSocketSendMessage is not implemented.\n", pWebSocket));
    return(-1);
}
...
{
    NetPrintf(("protowebsocketxboxone: [%p] ProtoWebSocketSendText is not implemented.\n", pWebSocket));
    return(-1);
}
...
{
    NetPrintf(("protowebsocketxboxone: [%p] ProtoWebSocketSendMessageText is not implemented.\n", pWebSocket));
    return(-1);
}
