...
typedef struct
{
    ...
    struct {
        ...
        uint32_t ack;           //!< acknowledgement of last packet
        uint32_t cid;           //!< client id (v1.0: in INIT/POKE packets; v1.1+: in INIT/POKE/CONN packets)
        uint8_t  vers[2];       //!< protocol version (v1.1+ only)
        uint8_t  data[62];      //!< space for possible metadata included in control packets
    } body;
} RawUDPPacketHeadT;
...
/*F*************************************************************************************************/
/*!
    \Function    _CommUDPEncodeSubpacketSize
    ...
    \Input *buf     - buffer holding encoded subpacket size
    \Input  val     - subpacket size
...
*/
/*************************************************************************************************F*/
static uint32_t _CommUDPEncodeSubpacketSize(uint8_t *buf, uint32_t val)
{
...
/*F*************************************************************************************************/
/*!
    \Function    _CommUDPDecodeSubpacketSize

    \Description
        Decodes a subpacket size field (see _CommUDPEncodeSubpacketSize for notes)

    \Input *buf     - buffer holding encoded subpacket size
    \Input *val     - [out] storage for decoded subpacket size
...
*/
/*************************************************************************************************F*/
static uint32_t _CommUDPDecodeSubpacketSize(const uint8_t *buf, int32_t *val)
{
    uint32_t offset, temp;
    if (buf[0] > 250)
    {
...
static void _CommUDPSetAddrInfo(CommUDPRef *ref, struct sockaddr *sin)
{
    ...
    NetPrintf(("commudp: [0x%08x] peer=%a:%d\n", (uintptr_t)ref, ref->common.peerip, ref->common.peerport));
}
...
static void _CommUDPSetSocket(CommUDPRef *pRef, SocketT *pSocket)
{
    ...
    if (pSocket != NULL)
    {
        ...
        NetPrintf(("commudp: [0x%08x] host=%a:%d\n", (uintptr_t)pRef, pRef->common.hostip, pRef->common.hostport));
    }
    else
    {
...
static void _CommUDPResetTransfer(CommUDPRef *ref)
{
    ...
    ref->sendtick = ref->uSendReliableTick = NetTick()-5000;
...
static int32_t _CommUDPWrite(CommUDPRef *ref, RawUDPPacketT *packet, struct sockaddr *peeraddr, uint32_t currtick)
{
    ...
    #if COMM_PRINT > 1
    NetPrintf(("commudp: [0x%08x] seq:0x%08x ack:0x%08x  send %d bytes to %a:%d\n", (uintptr_t)ref, packet->body.seq, packet->body.ack, len, SockaddrInGetAddr(peeraddr), SockaddrInGetPort(peeraddr)));
    #endif
    #if COMM_PRINT > 2
    ...
    {
        NetPrintf(("commudp: [0x%08x] dropping packet to simulate packet loss (seq=0x%08x)\n", (uintptr_t)ref, SocketNtohl(packet->body.seq)));
        err = len;
    }
    else
        // send the packet
    ...
    if (err == len)
    {
        ...
        if ((packet->body.seq & SEQ_MASK) >= RAW_PACKET_DATA)
        {
            ...
            ref->uSendReliableTick = currtick;
        }
    }
    else
    {
        NetPrintf(("commudp: [0x%08x] SocketSendto() returned %d\n", (uintptr_t)ref, err));
        ref->snderr = err;
        err = -1;
    }
...
static int32_t _CommUDPClose(CommUDPRef *ref, uint32_t currtick)
{
    ...
    if (ref->state == OPEN)
    {
        ...
        {
            NetPrintf(("commudp: [0x%08x] unsent data pending\n", (uintptr_t)ref));
        }
        else if (ref->sndout != ref->sndinp)
        {
            NetPrintf(("commudp: [0x%08x] unacked data pending\n", (uintptr_t)ref));
        }
    ...
    NetPrintf(("commudp: [0x%08x] closed connection\n", (uintptr_t)ref));
    ref->connident = 0;
    ref->state = CLOSE;
    return(0);
}
...
/*!
    ...
    \Input *ref     - reference pointer
    \Input *packet  - [out] buffer to format handshake packet into
    \Input kind     - packet kind (RAW_PACKET_INIT, RAW_PACKET_POKE, RAW_PACKET_CONN)

    \Version 10/29/2015 (jbrookes)
*/
/*************************************************************************************************F*/
static void _CommUDPFormatHandshake(CommUDPRef *ref, RawUDPPacketHeadT *packet, uint32_t kind)
{
    packet->body.seq = kind;
    packet->body.ack = ref->connident;
    #if COMMUDP_VERSION > COMMUDP_VERSION_1_0
    packet->body.cid = SocketHtonl(ref->clientident);
    packet->body.vers[0] = COMMUDP_VERSION>>8;
    packet->body.vers[1] = COMMUDP_VERSION&0x0f;
    packet->head.len = 6;
    #else
    if ((kind == RAW_PACKET_INIT) || (kind == RAW_PACKET_POKE))
    {
        packet->body.cid = SocketHtonl(ref->clientident);
        packet->head.len = 4;
    }
...
static void _CommUDPProcessSetup(CommUDPRef *ref, const RawUDPPacketT *packet, struct sockaddr *sin, uint32_t currtick)
{
    ...
    {
        NetPrintf(("commudp: [0x%08x] warning - connident mismatch (expected=0x%08x got=0x%08x)\n", (uintptr_t)ref, ref->connident, packet->body.ack));
    ...
    #if COMMUDP_VERSION > COMMUDP_VERSION_1_0
    if ((ref->vers == 0) && ((packet->body.seq == RAW_PACKET_INIT) || (packet->body.seq == RAW_PACKET_CONN) || (packet->body.seq == RAW_PACKET_POKE)))
    {
        RawUDPPacketHeadT *hshk = (RawUDPPacketHeadT *)packet;
        ref->vers = (packet->head.len > 4) ? (hshk->body.vers[0] << 8) | hshk->body.vers[1] : COMMUDP_VERSION_1_0;
        NetPrintf(("commudp: [0x%08x] vers=%d.%d\n", (uintptr_t)ref, ref->vers >> 8, ref->vers & 0xff));
    }
    #else
    ref->vers = COMMUDP_VERSION_1_0;
    #endif
    ...
    if ((packet->body.seq == RAW_PACKET_INIT) || (packet->body.seq == RAW_PACKET_POKE))
    {
        ...
        NetPrintf(("commudp: [0x%08x] sending CONN packet to %a:%d connident=0x%08x\n", (uintptr_t)ref, SockaddrInGetAddr(&ref->peeraddr),
            SockaddrInGetPort(&ref->peeraddr), ref->connident));
        _CommUDPFormatHandshake(ref, &connpacket, RAW_PACKET_CONN);
    ...
    if (packet->body.seq == RAW_PACKET_CONN)
    {
        ...
        if (ref->state == CONN)
        {
            // set host/peer addr/port info
            _CommUDPSetAddrInfo(ref, sin);
            NetPrintf(("commudp: [0x%08x] transitioning to OPEN state due to received RAW_PACKET_CONN\n", (uintptr_t)ref));
            ref->state = OPEN;
        }
        return;
    }
    ...
    if (packet->body.seq == RAW_PACKET_DISC)
    {
        ...
        NetPrintf(("commudp: [0x%08x] received DISC packet\n", (uintptr_t)ref));
    }
...
static void _CommUDPProcessInit(CommUDPRef *ref, uint32_t currtick)
{
    ...
    // send INIT to peer
    NetPrintf(("commudp: [0x%08x] sending INIT packet to %a:%d connident=0x%08x clientident=0x%08x\n", (uintptr_t)ref, SockaddrInGetAddr(&ref->peeraddr),
        SockaddrInGetPort(&ref->peeraddr), ref->connident, ref->clientident));
    _CommUDPFormatHandshake(ref, &packet, RAW_PACKET_INIT);
    _CommUDPWrite(ref, (RawUDPPacketT *)&packet, &ref->peeraddr, currtick);
}
...
static void _CommUDPProcessOutput(CommUDPRef *ref, uint32_t currtick)
{
    ...
    int32_t subpacketlimit = (ref->vers > COMMUDP_VERSION_1_0) ? 1530 : 250;
    ...
    while (tlimit > 0) {
        ...
        for (count = 0; (count < 8) && (plimit > 0) && (ref->sndnxt != ref->sndinp); ++count) {
            ...
            if (buffer->head.len > subpacketlimit) {
                ++count;
                break;
            }
        }
        ...
        ds_memcpy(&multi, buffer, sizeof(buffer->head)+8+buffer->head.len);
        count -= 1;
        ...
        for (; count > 0; --count) {
            ...
            ds_memcpy(multi.body.data+multi.head.len, buffer->body.data, buffer->head.len);
            multi.head.len += buffer->head.len;
            multi.head.len += _CommUDPEncodeSubpacketSize(multi.body.data+multi.head.len, buffer->head.len);
        }
...
static void _CommUDPProcessFlow(CommUDPRef *ref, RawUDPPacketHeadT *packet, uint32_t currtick)
{
    ...
    {
        #if COMM_PRINT
        NetPrintf(("commudp: [0x%08x] got NAK for packet %d\n", (uintptr_t)ref, ack+1));
        #endif
        // reset send point
...
static int32_t _CommUDPProcessInput(CommUDPRef *ref, RawUDPPacketHeadT *packet, uint8_t *data, uint32_t currtick)
{
    ...
    if ((ref->rcvinp+ref->rcvwid)%ref->rcvlen == ref->rcvout)
    {
        ...
        NetPrintf(("commudp: [0x%08x] input buffer overflow\n", (uintptr_t)ref));
        return(2);
    }
    ...
    else
    {
        ...
        if (_CommUDPSeqnDiff(packet->body.seq, ref->rcvseq) > 0)
        {
            ...
            // send a nak packet
            #if COMM_PRINT
            NetPrintf(("commudp: [0x%08x] sending a NAK of packet %d (tick=%u)\n", (uintptr_t)ref, ref->rcvseq, NetTick()));
            #endif
    ...
    // copy the packet
    ds_memcpy_s(buffer, sizeof(*buffer), packet, sizeof(*packet));
    ds_memcpy(buffer->body.data, data, packet->head.len);
...
static void _CommUDPProcessPoke(CommUDPRef *ref, uint32_t currtick)
{
    ...
    // send POKE to peer
    NetPrintf(("commudp: [0x%08x] sending POKE packet to %a:%d connident=0x%08x clientident=0x%08x\n", (uintptr_t)ref, SockaddrInGetAddr(&ref->peeraddr),
        SockaddrInGetPort(&ref->peeraddr), ref->connident, ref->clientident));
    _CommUDPFormatHandshake(ref, &packet, RAW_PACKET_POKE);
    _CommUDPWrite(ref, (RawUDPPacketT *)&packet, &ref->peeraddr, currtick);
}
...
static void _CommUDPProcessAlive(CommUDPRef *ref, uint32_t currtick)
{
    ...
    // send it
    #if COMM_PRINT
    NetPrintf(("commudp: [0x%08x] sending keep-alive\n", (uintptr_t)ref));
    #endif
    _CommUDPWrite(ref, (RawUDPPacketT *)&packet, &ref->peeraddr, currtick);
}
...
static uint32_t _CommUDPFlush(CommUDPRef *ref, uint32_t limit, uint32_t currtick)
{
    ...
    NetPrintf(("commudp: [0x%08x] flushing %d packets in send queue\n", (uintptr_t)ref, iNumPackets));
...
static int32_t _CommUDPThreadData(uint32_t currtick)
{
    ...
    for (ref = g_link; ref != NULL; ref = ref->link) {
        ...
        if ((ref->socket != NULL) && (ref->socket != pSocket)) {
            ...
            if (len > 0) {
                ...
                ref->common.rcvoverhead += sizeof(packet.body)-sizeof(packet.body.data) + _CommUDPOverhead(ref, len);
                
                if (ref->metatype == 1)
                {
                    ref->common.rcvoverhead += RAW_METATYPE1_SIZE;
                }
                ...
                #if COMM_PRINT > 1
                NetPrintf(("commudp: [0x%08x] seq:0x%08x ack:0x%08x recv %d bytes from %a:%d (head.len=%d, meta=%d) at tick=%d\n",
                    (uintptr_t)ref, packet.body.seq, packet.body.ack, len, SockaddrInGetAddr(&sin), SockaddrInGetPort(&sin), packet.head.len, packet.head.meta, packet.head.when));
                #endif
                #if COMM_PRINT > 2
                ...
                if (packet.body.seq == RAW_PACKET_CONN || (packet.body.seq == RAW_PACKET_INIT) || (packet.body.seq == RAW_PACKET_POKE))
                    NetPrintf(("commudp: [0x%08x] got %s connident=0x%08x len=%d\n", (uintptr_t)ref, g_strConnNames[packet.body.seq], packet.body.ack, packet.head.len));
                // count the packet
                ++count;
    ...
    if ((packet.head.len >= RAW_METATYPE1_SIZE) && (packet.head.meta == 1))
    {
        ...
        #if COMM_PRINT > 2
        NetPrintf(("commudp: [0x%08x] processed metadata chunk\n", (uintptr_t)ref));
        #endif
    }
    ...
    for (ref = g_link; ref != NULL; ref = ref->link)
    {
            ...
            (SockaddrCompare(&ref->peeraddr, &sin) == 0) && (connidentmatch))
        {
            ...
            {
                NetPrintf(("commudp: [0x%08x] transitioning to OPEN state due to received data from peer\n", (uintptr_t)ref));
                ref->state = OPEN;
            }
            ...
            if ((packet.body.seq == RAW_PACKET_INIT) ||
                ...
                for (; count2 >= 0; --count2) {
                    if (count2 > 0) {
                        packet.head.len -= _CommUDPDecodeSubpacketSize(packet.body.data+packet.head.len-1, &multi.head.len);
                    } else {
                    ...
                    {
                        ref->common.packsaved += 1;
                        #if COMM_PRINT
                        NetPrintf(("commudp: [0x%08x] redundant packet %d prevented loss of packet %d\n", (uintptr_t)ref, packet.body.seq & SEQ_MASK, oldseq));
                        #endif
                        oldseq = ref->rcvseq;
                    }
        ...
        if ((ref->state == OPEN) && (NetTickDiff(currtick, ref->recvtick) > 120*1000) && (NetTickDiff(currtick, ref->sendtick) < 2000)) {
            NetPrintf(("commudp: [0x%08x] closing connection due to timeout\n", (uintptr_t)ref));
            NetPrintf(("commudp: [0x%08x] tick=%d, rtick=%d, stick=%d\n", (uintptr_t)ref, currtick, ref->recvtick, ref->sendtick));
        ...
        if ((ref->callback == 0) && (ref->gotevent != 0)) {
            ...
            {
                #if COMM_PRINT
                NetPrintf(("commudp: [0x%08x] no upper layer callback\n", (uintptr_t)ref));
                #endif
            }
            // limit callback access
            ref->callback -= 1;
        ...
        if ((ref->state == OPEN) && (ref->sndnxt == ref->sndinp)) {
            ...
            int32_t reliabletimeout = NetTickDiff(currtick, ref->uSendReliableTick);
            if (((reliabletimeout > BUSY_KEEPALIVE) && (ref->rcvack != ref->rcvseq)) ||
                ((reliabletimeout > BUSY_KEEPALIVE) && (ref->sndinp != ref->sndout)) ||
         ...
         (packet.body.seq == RAW_PACKET_CONN)))
    {
        ...
        NetPrintf(("commudp: [0x%08x] received %s packet from %a:%d\n", (uintptr_t)ref, g_strConnNames[packet.body.seq], SockaddrInGetAddr(&sin), SockaddrInGetPort(&sin)));
            ...
            if (((ref->state == CONN) || (ref->state == LIST)) && (ref->peeraddr.sa_family == AF_INET))
            {
                if (ref->connident == packet.body.ack)
                {
                    ...
                    if ((pSocket == ref->socket) && (SockaddrInGetAddr(&ref->peeraddr) == SockaddrInGetAddr(&sin)) && (SockaddrInGetPort(&ref->peeraddr) != SockaddrInGetPort(&sin)))
                    {
                        ...
                        NetPrintf(("commudp: [0x%08x] changing peer to %a:%d (was expecting %a:%d)\n", (uintptr_t)ref, SockaddrInGetAddr(&sin), SockaddrInGetPort(&sin),
                            SockaddrInGetAddr(&ref->peeraddr), SockaddrInGetPort(&ref->peeraddr)));
                        ds_memcpy_s(&ref->peeraddr, sizeof(ref->peeraddr), &sin, sizeof(sin));
                    }
                    break;
                }
        ...
        {
            NetPrintf(("commudp: [0x%08x] ignoring %s packet with connident 0x%08x\n", (uintptr_t)ref, g_strConnNames[packet.body.seq], packet.body.ack));
        }
    }
    ...
    if ((pListen != NULL) && (packet.head.len >= 0) && ((packet.body.seq == RAW_PACKET_INIT) || (packet.body.seq == RAW_PACKET_CONN)))
    {
        ...
        {
            NetPrintf(("commudp: [0x%08x] transitioning to CONN state after receiving %s packet\n", (uintptr_t)ref, g_strConnNames[packet.body.seq]));
...
CommUDPRef *CommUDPConstruct(int32_t maxwid, int32_t maxinp, int32_t maxout)
{
    ...
    // reset peer address
    ds_memclr(&ref->peeraddr, sizeof(ref->peeraddr));

    // reset the state
    ref->state = IDLE;
    ref->connident = 0;
    ref->unacklimit = UNACK_LIMIT;
    ref->redundantlimit = REDUNDANT_LIMIT_DEFAULT;
...
{
    NetPrintf(("commudp: [0x%08x] resolve functionality not supported\n", (uintptr_t)ref));
    return(-1);
}
...
static int32_t _CommUDPListen0(CommUDPRef *ref, SocketT *sock, struct sockaddr *bindaddr)
{
    ...
    */
    ...
    if (find == NULL)
    {
        ...
        if (err < 0)
        {
            NetPrintf(("commudp: [0x%08x] bind to %d failed with %d\n", (uintptr_t)ref, SockaddrInGetPort(bindaddr), err));
            SockaddrInSetPort(bindaddr, 0);
            if ((err = SocketBind(sock, bindaddr, sizeof(*bindaddr))) < 0)
            {
                NetPrintf(("commudp: [0x%08x] bind to 0 failed with result %d\n", (uintptr_t)ref, err));
                ref->state = DEAD;
                SocketClose(sock);
...
static int32_t _CommUDPConnect0(CommUDPRef *ref, SocketT *sock, struct sockaddr *peeraddr)
{
    ...
    // setup target info
    ds_memcpy_s(&ref->peeraddr, sizeof(ref->peeraddr), peeraddr, sizeof(*peeraddr));
...
int32_t CommUDPControl(CommUDPRef *pRef, int32_t iControl, int32_t iValue, void *pValue)
{
    ...
    if (iControl == 'meta')
    {
        NetPrintf(("commudp: [0x%08x] setting metatype=%d\n", (uintptr_t)pRef, iValue));
    ...
    if (iControl == 'rlmt')
    {
        ...
        NetPrintf(("commudp: [0x%08x] redundant limit changed from %d bytes to %d bytes\n", (uintptr_t)pRef, pRef->redundantlimit, iValue));
    ...
    if (iControl == 'ulmt')
    {
        ...
        NetPrintf(("commudp: [0x%08x] setting ulimit to %d bytes\n", (uintptr_t)pRef, pRef->unacklimit));
        return(0);
    }
    // unhandled; pass through to socket module if we have a socket
    if (pRef->socket != NULL)
    {
        return(SocketControl(pRef->socket, iControl, iValue, pValue, NULL));
    }
    return(-1);
}
...
{
    RawUDPPacketT packetbuffer, *packet;
    uint32_t currtick = NetTick();
    int32_t pos, metalen, result=1; // result=1 is default (min queue) return value
    ...
    {
        NetPrintf(("commudp: [0x%08x] oversized packet send (%d bytes)\n", (uintptr_t)ref, length));
    ...
    else
    {
        ...
        {
            NetPrintfVerbose((COMM_PRINT, 0, "commudp: [0x%08x] send overflow (connident=0x%08x)\n", (uintptr_t)ref, ref->connident));
            return(0);
        }
    ...
    if (flags & COMM_FLAGS_UNRELIABLE)
    {
        ...
        {
            struct sockaddr peeraddr;
            ds_memcpy_s(&peeraddr, sizeof(peeraddr), &ref->peeraddr, sizeof(ref->peeraddr));
        ...
        if (err < 0)
        {
            result = (ref->snderr == SOCKERR_NONE) ? 0 : err;
        }
    }
    else
    {
        ...
        if (pos < (ref->common.maxout/2))
        {
            ...
            if (g_missed != 0)
            {
                ...
                while (_CommUDPThreadData(currtick) > 0)
                    ;
                NetPrintfVerbose((COMM_PRINT, 0, "commudp: [0x%08x] processing %d after send\n", (uintptr_t)ref, g_missed));
            }
            NetCritLeave(&g_crit);
            // return buffer depth
            if (pos > 0)
...
int32_t CommUDPPeek(CommUDPRef *ref, void *target, int32_t length, uint32_t *when)
{
    ...
    if (length > 0)
    {
        ...
        ds_memcpy(target, packet->body.data, packet->head.len);
    }
...
int32_t CommUDPListen(CommUDPRef *ref, const char *addr)
{
    ...
    NetPrintf(("commudp: [0x%08x] listen err=%d, bind=%d, connident=0x%08x\n", (uintptr_t)ref, err, iListenPort, ref->connident));
    ...
    if ((err == 0) && (poke != 0))
    {
        ...
        NetPrintf(("commudp: [0x%08x] poke address=%08x:%d\n", (uintptr_t)ref, poke, iConnPort));
...
int32_t CommUDPConnect(CommUDPRef *ref, const char *addr)
{
    ...
    // set connection identifier
    _CommUDPSetConnID(ref, addr);
    NetPrintf(("commudp: [0x%08x] connect addr=%08x, bind=%d, peer=%d connident=0x%08x\n",
        (uintptr_t)ref, uAddr, iListenPort, iConnPort, ref->connident));
    ...
    if (sock == NULL)
    {
        ...
        if ((err = SocketBind(sock, &bindaddr, sizeof(bindaddr))) < 0)
        {
            ...
            else
            {
                ...
                NetPrintf(("commudp: [0x%08x] bound socket to port %d\n", (uintptr_t)ref, SockaddrInGetPort(&bindaddr)));
            }
        }
    }
