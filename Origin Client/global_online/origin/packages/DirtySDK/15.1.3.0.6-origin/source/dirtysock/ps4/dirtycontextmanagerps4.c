...
int32_t _DirtyContextManagerShareNetPool(int32_t iNetPoolSize)
{
    int32_t iSlot = 0;
    int32_t iLowerRefCountSlot = -1;
    int32_t iLowerRefCount = 0x7FFFFF;
    for (iSlot = 0; iSlot < DIRTYCM_MAX_NET_CONTEXTS; ++iSlot)
    {
        ...
        if (_DirtyContextManagerRef->aNetContexts[iSlot].iRefCount < iLowerRefCount)
        {
