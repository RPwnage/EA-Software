...
static int32_t _DirtyWebApiAddPushEvent(DirtyWebApiRefT *pRef, const char * eventType, const char * pNpServiceName, SceNpServiceLabel npServiceLabel, DirtyWebApiPushEventCallbackT *callback, void *pUserData)
{
    ...
    if (pRef->PushEvents.iEventCount < DIRTY_WEBAPI_MAX_PUSH_EVENTS)
    {
        ...
        ds_snzprintf(filterParam.dataType.val, SCE_NP_WEBAPI_PUSH_EVENT_DATA_TYPE_LEN_MAX, eventType);

        if ((iFilterId = sceNpWebApiCreateExtdPushEventFilter(pRef->iWebApiCtxId, pRef->PushEvents.iHandleId, pNpServiceName, npServiceLabel, &filterParam, 1)) < 0)
        {
...
static int32_t _DirtyWebApiInitWebApi(DirtyWebApiRefT *pRef, const DirtyWebApiCreateParamsT* pCreateParams)
{
    ...
    iRet = scePthreadMutexInit(&pRef->dirtyWebApiThreadMutex, NULL, "WakeupWebApiThreadMutex");
    if (iRet == SCE_KERNEL_ERROR_EAGAIN)
    {
        iRet = scePthreadMutexInit(&pRef->dirtyWebApiThreadMutex, NULL, NULL);
    }
    if (iRet < 0)
    {
