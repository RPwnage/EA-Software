...
int32_t NetConnStartup(const char *pParams)
{
    ...
    {
        ++(pRef->iRefCount);
        NetPrintf(("netconnwin: NetConnStartup() called with params='%s' while the module is already started, refcounting to %d\n", pParams, pRef->iRefCount));
        return(0);
    }
...
int32_t NetConnShutdown(uint32_t bStopEE)
{
    ...
    {
        NetPrintf(("netconnwin: NetConnShutdown() called, new refcount is %d\n", _NetConn_pRef->iRefCount));
