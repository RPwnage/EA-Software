...
static uint32_t _NetConnGetInterfaceType(NetConnRefT *pRef)
{
    ...
    uIfType = NetConnStatusIos('type', 0, NULL, 0);
...
int32_t NetConnStartup(const char *pParams)
{
    ...
    {
        ++(pRef->iRefCount);
        NetPrintf(("netconnunix: NetConnStartup() called with params='%s' while the module is already started, refcounting to %d\n", pParams, pRef->iRefCount));
        return(0);
    }
...
int32_t NetConnStatus(int32_t iKind, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    return(NetConnStatusIos(iKind, iData, pBuf, iBufSize));
...
int32_t NetConnShutdown(uint32_t uShutdownFlags)
{
    ...
    // check the refcount
    if (--(_NetConn_pRef->iRefCount) > 0)
    {
        NetPrintf(("netconnunix: NetConnShutdown() called, new refcount is %d\n", _NetConn_pRef->iRefCount));
