...
static void _SocketRecvToPacketQueue(SocketStateT *pState, SocketT *pSocket)
{
    ...
    else
    {
        ...
        {
            NetPrintf(("dirtynetps4: _SocketRecvfrom() to packet queue returned %d (err=%s)\n", iRecvResult, DirtyErrGetName(sce_net_errno)));
        }
...
int32_t SocketCreate(int32_t iThreadPrio, int32_t iThreadStackSize, int32_t iThreadCpuAffinity)
{
    ...
    if ((iResult = scePthreadAttrInit(&attr)) == SCE_OK)
    {
        ...
        {
            NetPrintf(("dirtynetps4: scePthreadAttrSetschedpolicy SCHED_RR failed (err=%s)\n", DirtyErrGetName(iResult)));
        }
        ...
        {
            NetPrintf(("dirtynetps4: scePthreadAttrSetschedparam SCE_KERNEL_PRIO_FIFO_HIGHEST failed (err=%s)\n", DirtyErrGetName(iResult)));
        }
        ...
        {
            NetPrintf(("dirtynetps4: scePthreadAttrSetinheritsched SCE_PTHREAD_EXPLICIT_SCHED failed (err=%s)\n", DirtyErrGetName(iResult)));
        }
