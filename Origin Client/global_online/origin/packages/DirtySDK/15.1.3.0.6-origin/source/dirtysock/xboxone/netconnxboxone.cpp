...
int32_t NetConnStartup(const char *pParams)
{
    ...
    {
        ++(pRef->iRefCount);
        NetPrintf(("netconnxboxone: NetConnStartup() called with params='%s' while the module is already started, refcounting to %d\n", pParams, pRef->iRefCount));
        return(0);
    }
...
/*!
    ...
    \Input *pOption - asciiz list of config parameters (tbd)
                      ...
                      "-nologin=<true/false>" to specifiy whether to skip login to MS.
...
int32_t NetConnConnect(const NetConfigRecT *pConfig, const char *pOption, int32_t iData)
{
    ...
    if ((pRef->eNetConnState == NetConnStateE::ST_IDLE) || (pRef->eNetConnState == NetConnStateE::ST_CLEANUP))
    {
        ...
        if (pOption != NULL)
        {
            ...
            // nologin
            if (strstr(pOption, "-nologin"))
            {
                if (strstr(pOption, "-nologin=false"))
                {
                    NetPrintf(("netconnxboxone: -nologin set to FALSE\n"));
                    if (pRef->bNoLogin == TRUE)
                    {
                        NetPrintf(("netconnxboxone: -nologin setting from NetConnStartup (TRUE) overwritten in NetConnConnect (FALSE)\n"));
                    }
                    pRef->bNoLogin = FALSE;
                }
                else
                {
                    NetPrintf(("netconnxboxone: -nologin set to TRUE\n"));
                    pRef->bNoLogin = TRUE;
                }
            }
        }
...
int32_t NetConnShutdown(uint32_t bStopEE)
{
    ...
    if (--(pRef->iRefCount) > 0)
    {
        NetPrintf(("netconnxboxone: NetConnShutdown() called, new refcount is %d\n", pRef->iRefCount));
