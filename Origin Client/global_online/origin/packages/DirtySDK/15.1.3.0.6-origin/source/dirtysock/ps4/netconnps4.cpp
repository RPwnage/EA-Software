...
{
    SceNpId                         aNpid[NETCONN_MAXLOCALUSERS];           //!< associated NP id
    SceNpState                      aNpState[NETCONN_MAXLOCALUSERS];        //!< tracks the login status of players to the Playstation Network
    SceUserServiceUserId            aUserServiceUserId[NETCONN_MAXLOCALUSERS]; //!< associated sceUserService id
    NetConnUserAuthCodeT            aAuthCode[NETCONN_MAXLOCALUSERS];       //!< tracks the progress of getting an auth code
    NetConnParentalControlsT        aParentalCtrls[NETCONN_MAXLOCALUSERS];  //!< tracks the progress of getting the parental controls
...
static void _NetConnInvalidateSceNpOnlineIdCache(NetConnRefT *pRef)
{
    ...
    {
        pRef->aSceNpOnlineIdCache[i].uInvalid = TRUE;
    }
...
static int32_t _NetConnAddToExternalCleanupList(NetConnRefT *pRef, NetConnExternalCleanupCallbackT pCleanupCb, void *pCleanupData)
{
    ...
    if (pRef->iExternalCleanupListCnt == pRef->iExternalCleanupListMax)
    {
        ...
        ds_memclr(pNewList, 2 * pRef->iExternalCleanupListMax * sizeof(NetConnExternalCleanupEntryT));
...
{
    SceUserServiceUserId userId = NetConnStatus('suid', iUserIndex,  NULL, 0);
    int32_t iRet = 0; 
    ...
    if (userId >= 0)
    {
        if (pRef->NpUsers.aAuthCode[iUserIndex].eAuthCodeState != NETCONN_NP_REQUEST_STATE_INPROG)
        {
            ...
            if (iRet > 0)
            {
                ...
                ds_memclr(&pRef->NpUsers.aAuthCode[iUserIndex].NpAuthCode, sizeof(pRef->NpUsers.aAuthCode[iUserIndex].NpAuthCode));
                ...
                iRet = _NetConnSaveErr(pRef, sceNpAuthGetAuthorizationCodeA(pRef->NpUsers.aAuthCode[iUserIndex].iAuthCodeRequestId, &authParam, &pRef->NpUsers.aAuthCode[iUserIndex].NpAuthCode, &pRef->NpUsers.aAuthCode[iUserIndex].iIssuerId ));
                if (iRet == 0)
                {
    ...
    if (iUserIndex >= 0 && iUserIndex < NETCONN_MAXLOCALUSERS)
    {
        if (pRef->NpUsers.aAuthCode[iUserIndex].eAuthCodeState == NETCONN_NP_REQUEST_STATE_VALID)
        {
            ...
            else if ((pBuf != NULL) && (iBufSize >= (int32_t)sizeof(SceNpAuthorizationCode)))
            {
                ...
                ds_memcpy(pBuf, &pRef->NpUsers.aAuthCode[iUserIndex].NpAuthCode, sizeof(SceNpAuthorizationCode));
                iRet = sizeof(SceNpAuthorizationCode);
            }
            else
            {
...
static int32_t _NetConnNpAsyncRequest(NetConnRefT *pRef, int32_t iUserIndex, NetConnAsyncRequestDataT *pRequestData, NetConnAsyncRequestT pRequestCallback)
{
    ...
    if (userId >= 0)
    {
        if (pRequestData->eRequestState != NETCONN_NP_REQUEST_STATE_INPROG)
        {
            ...
            if (iRet > 0)
            {
                iRet = _NetConnSaveErr(pRef, pRequestCallback(pRef, iUserIndex, userId, pRequestData));
                if (iRet == 0)
                {
...
*/
/********************************************************************************F*/
static int32_t _NetConnParentalControlsRequest(NetConnRefT *pRef, int32_t iUserIndex, SceUserServiceUserId userId, NetConnAsyncRequestDataT *pRequestData)
{
    return sceNpGetParentalControlInfoA(pRequestData->iRequestId, userId, &pRef->NpUsers.aParentalCtrls[iUserIndex].iAge, &pRef->NpUsers.aParentalCtrls[iUserIndex].NpParentalCtrls);
}
...
static int32_t _NetConnParentalControlsResponse(NetConnRefT *pRef, int32_t iUserIndex, void *pBuf, int32_t iBufSize)
{
    ...
    {
        ds_memclr(pBuf, iBufSize);
        ds_memcpy(pBuf, &pRef->NpUsers.aParentalCtrls[iUserIndex].NpParentalCtrls, sizeof(SceNpParentalControlInfo));
...
*/
/********************************************************************************F*/
static int32_t _NetConnPsPlusRequest(NetConnRefT *pRef, int32_t iUserIndex, SceUserServiceUserId userId, NetConnAsyncRequestDataT *pRequestData)
{
    return sceNpCheckPlus(pRequestData->iRequestId, &pRef->NpUsers.aPsPlus[iUserIndex].NpCheckPlusParam, &pRef->NpUsers.aPsPlus[iUserIndex].NpCheckPlusResult);
}
...
static int32_t _NetConnPsPlusResponse(NetConnRefT *pRef, int32_t iUserIndex, void *pBuf, int32_t iBufSize)
{
    ...
    {
        ds_memclr(pBuf, iBufSize);
        ds_memcpy(pBuf, &pRef->NpUsers.aPsPlus[iUserIndex].NpCheckPlusResult, sizeof(SceNpCheckPlusResult));
...
static void _NetConnUpdateLocalUserNpSate(NetConnRefT *pRef, int32_t iLocalUserIndex)
{
    ...
    if (pRef->bNpInit)
    {
        SceNpState sceOldNpState = pRef->NpUsers.aNpState[iLocalUserIndex];
        if ((iResult = _NetConnSaveErr(pRef, sceNpGetState(pRef->NpUsers.aUserServiceUserId[iLocalUserIndex], &pRef->NpUsers.aNpState[iLocalUserIndex]))) == SCE_OK)
        {
            if (sceOldNpState != pRef->NpUsers.aNpState[iLocalUserIndex])
            {
                NetPrintf(("netconnps4: NP state of local user 0x%08x at index %d changed from %s to %s\n",
                    pRef->NpUsers.aUserServiceUserId[iLocalUserIndex], iLocalUserIndex, _NpState[sceOldNpState], _NpState[pRef->NpUsers.aNpState[iLocalUserIndex]]));
            }

            if (pRef->NpUsers.aNpState[iLocalUserIndex] == SCE_NP_STATE_SIGNED_IN)
            {
                if ((iResult = _NetConnSaveErr(pRef, sceNpGetNpId(pRef->NpUsers.aUserServiceUserId[iLocalUserIndex], &pRef->NpUsers.aNpid[iLocalUserIndex]))) != SCE_OK)
                {
                    NetPrintf(("netconnps4: sceNpGetNpId(0x%08x) failed (idx=%d); err=%s\n", pRef->NpUsers.aUserServiceUserId[iLocalUserIndex], iLocalUserIndex, DirtyErrGetName(iResult)));
                }

                // refresh the parental control settings
                if (pRef->NpUsers.aParentalCtrls[iLocalUserIndex].ReqData.eRequestState == NETCONN_NP_REQUEST_STATE_INVALID)
                {
                    if ((iResult = _NetConnNpAsyncRequest(pRef, iLocalUserIndex, (NetConnAsyncRequestDataT*)&pRef->NpUsers.aParentalCtrls[iLocalUserIndex], _NetConnParentalControlsRequest)) < 0)
                    {
...
static void _NetConnAddLocalUser(NetConnCommonRefT *pCommonRef, int32_t iLocalUserIndex, const EA::User::IEAUser *pIEAUser)
{
    ...
    if (pRef->aIEAUsers[iLocalUserIndex] == NULL)
    {
        ...
        if (pRef->NpUsers.aUserServiceUserId[iLocalUserIndex] == SCE_USER_SERVICE_USER_ID_INVALID)
        {
            // obtain the local user identifier from the IEAUser
            pRef->NpUsers.aUserServiceUserId[iLocalUserIndex] = (SceUserServiceUserId)pIEAUser->GetUserID();

            NetPrintf(("netconnps4: netconn user added at local user index %d --> local user id: 0x%08x\n",
                iLocalUserIndex, pRef->NpUsers.aUserServiceUserId[iLocalUserIndex]));
        ...
        {
            NetPrintf(("netconnps4: failed to add IEAUser at index %d with local user id 0x%08x because aUserServiceUserId is out of sync.\n",
    ...
    if (pRef->aIEAUsers[iLocalUserIndex] != NULL)
    {
        if (pIEAUser->GetUserID() == pRef->aIEAUsers[iLocalUserIndex]->GetUserID())
        {
            if (pRef->NpUsers.aUserServiceUserId[iLocalUserIndex] != SCE_USER_SERVICE_USER_ID_INVALID)
            {
                ...
                pRef->NpUsers.aUserServiceUserId[iLocalUserIndex] = SCE_USER_SERVICE_USER_ID_INVALID;

                ds_memclr(&pRef->NpUsers.aNpid[iLocalUserIndex], sizeof(pRef->NpUsers.aNpid[iLocalUserIndex]));
                pRef->NpUsers.aNpState[iLocalUserIndex] = SCE_NP_STATE_UNKNOWN;
                ...
                _NetConnCleanupNpRequest(pRef, iLocalUserIndex, (NetConnAsyncRequestDataT*)&pRef->NpUsers.aParentalCtrls[iLocalUserIndex]);
                pRef->NpUsers.aParentalCtrls[iLocalUserIndex].ReqData.eRequestState = NETCONN_NP_REQUEST_STATE_INVALID;

                _NetConnCleanupNpRequest(pRef, iLocalUserIndex, (NetConnAsyncRequestDataT*)&pRef->NpUsers.aPsPlus[iLocalUserIndex]);
                pRef->NpUsers.aPsPlus[iLocalUserIndex].ReqData.eRequestState = NETCONN_NP_REQUEST_STATE_INVALID;

                pRef->aSceNpOnlineIdCache[iLocalUserIndex].uInvalid = TRUE;
...
static void _NetConnNpCheckNpAvailabilityError(NetConnRefT *pRef, uint8_t bErrorAtStart)
{
    ...
    for (int32_t i = 0; i < NETCONN_MAXLOCALUSERS; ++i)
    {
        if (pRef->NpUsers.aNpAvailability[i].iResult < 0)
        {
            iFirstErrorResult = pRef->NpUsers.aNpAvailability[i].iResult;
            if (pRef->NpUsers.aNpAvailability[i].iResult != SCE_NP_ERROR_SIGNED_OUT)
            {
                break;
            }
        }
...
static int32_t _NetConnStartQueryNpUser(NetConnRefT *pRef)
{
    ...
    for (int32_t iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; ++iIndex)
    {
        ...
        if (iRequest > 0)
        {
            ...
            if (!((userId = NetConnStatus('suid', iIndex, NULL, 0)) < 0))
            {
                if ((iResult = sceNpCheckNpAvailabilityA(iRequest, userId)) == SCE_OK)
                {
            ...
            else
            {
                ...
                pRef->NpUsers.aNpAvailability[iIndex].bDone = TRUE;
                sceNpDeleteRequest(iRequest);
                NetPrintf(("netconnps4: user at index %d failed to retrieve user id\n", iIndex));
            }
...
static int32_t _NetConnQueryTrial(void)
{
    ...
    if (sceSysmoduleIsLoaded(SCE_SYSMODULE_APP_CONTENT) == SCE_SYSMODULE_LOADED)
    {
        ...
        {
            int32_t iValue = 0;
            if ((iReturn = sceAppContentAppParamGetInt(SCE_APP_CONTENT_APPPARAM_ID_SKU_FLAG, &iValue)) == SCE_OK)
            {
...
static uint32_t _NetConnInitParentalSettings(NetConnRefT *pRef)
{
    ...
    _NetConnUpdateNpRequests(pRef, &pRef->NpUsers.aParentalCtrls, sizeof(pRef->NpUsers.aParentalCtrls[0]));
    ...
    for (iUserIndex = 0; iUserIndex < NETCONN_MAXLOCALUSERS; iUserIndex++)
    {
        ...
        if (pRef->NpUsers.aNpState[iUserIndex] == SCE_NP_STATE_SIGNED_IN)
        {
...
static void _NetConnPrintNpState(NetConnRefT *pRef)
{
    ...
    for(iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; iIndex++)
    {
        if (pRef->NpUsers.aNpState[iIndex] == SCE_NP_STATE_SIGNED_IN)
        {
            NetPrintf(("netconnps4: user #%d, %s\n", iIndex, pRef->NpUsers.aNpid[iIndex].handle.data));
        }
        else
        {
            NetPrintf(("netconnps4: user #%d, not signed in (%s)\n", iIndex, _NpState[pRef->NpUsers.aNpState[iIndex]]));
        }
    }
}
#endif
...
static void _NetConnSceNpStateCallback(SceUserServiceUserId sceUserServiceUserId, SceNpState state, void *pUserData)
{
    ...
    if (pRef)
    {
        ...
        {
            if (pRef->NpUsers.aUserServiceUserId[iIndex] == sceUserServiceUserId)
            {
                break;
            }
        }
...
{
    int32_t iResult, iIndex, iCallbackId;

    ds_memclr(pRef->NpUsers.aNpid, sizeof(pRef->NpUsers.aNpid));
    ds_memclr(pRef->NpUsers.aNpState, sizeof(pRef->NpUsers.aNpState));
    ...
    for(iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; iIndex++)
    {
        if (pRef->NpUsers.aUserServiceUserId[iIndex] != SCE_USER_SERVICE_USER_ID_INVALID)
        {
            // update NP state of this user
            _NetConnUpdateLocalUserNpSate(pRef, iIndex);
        }
...
int32_t NetConnStartup(const char *pParams)
{
    ...
    {
        ++(pRef->iRefCount);
        NetPrintf(("netconnps4: NetConnStartup() called with params='%s' while the module is already started, refcounting to %d\n", pParams, pRef->iRefCount));
        return(0);
    }
    ...
    bCheckTick = strstr(pParams, "-checktick") != NULL;
    if (DirtyEventManagerInit(bCheckTick) < 0)
    {
    ...
    if (DirtyContextManagerInit() < 0)
    {
        ...
        NetPrintf(("netconnps4: could not initialize DirtyHttpContextManager\n"));
        return(NETCONN_ERROR_PLATFORM_SPECIFIC);
    }
    ...
    ds_memclr(pRef->pExternalCleanupList, pRef->iExternalCleanupListMax * sizeof(NetConnExternalCleanupEntryT));

    // create network instance
    if (SocketCreate(10, 0, pRef->iThreadCpuAffinity) != 0)
    {
        NetConnShutdown(0);
    ...
    for (iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; ++iIndex)
    {
        pRef->NpUsers.aUserServiceUserId[iIndex] = SCE_USER_SERVICE_USER_ID_INVALID;
    }
    ...
    if (pGamertag != NULL)
    {
        ...
        for (int32_t iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; ++iIndex)
        {
            if (pRef->NpUsers.aNpState[iIndex] == SCE_NP_STATE_SIGNED_IN)
            {
                ...
                if (ds_strnicmp(pRef->NpUsers.aNpid[iIndex].handle.data, pGamertag, SCE_NP_ONLINEID_MAX_LENGTH + 1) == 0)
                {
...
int32_t NetConnDisconnect(void)
{
    ...
    {
        // psplus
        if (pRef->NpUsers.aPsPlus[iIndex].ReqData.iRequestId != 0)
        {
            sceNpDeleteRequest(pRef->NpUsers.aPsPlus[iIndex].ReqData.iRequestId);
        }
        pRef->NpUsers.aPsPlus[iIndex].ReqData.eRequestState = NETCONN_NP_REQUEST_STATE_INVALID;
        pRef->NpUsers.aPsPlus[iIndex].ReqData.iRequestId = 0;

        // parental controls
        if (pRef->NpUsers.aParentalCtrls[iIndex].ReqData.iRequestId != 0)
        {
            sceNpDeleteRequest(pRef->NpUsers.aParentalCtrls[iIndex].ReqData.iRequestId);
        }
        pRef->NpUsers.aParentalCtrls[iIndex].ReqData.eRequestState = NETCONN_NP_REQUEST_STATE_INVALID;
        pRef->NpUsers.aParentalCtrls[iIndex].ReqData.iRequestId = 0;
...
int32_t NetConnControl(int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue, void *pValue2)
{
    ...
    if (iControl == 'stat')
    {
        if (pValue != NULL && iValue == sizeof(NetConnNpUserStatusT))
        {
            if (pRef->bNpToolKit)
            {
                ...
                _NetConnSceNpStateCallback(userStatus->iUserId, userStatus->state, pRef);
                return(0);
            }
            else
            {
    ...
    if (iControl == 'pctl')
    {
        return(_NetConnNpAsyncRequest(pRef, iValue, (NetConnAsyncRequestDataT*)&pRef->NpUsers.aParentalCtrls[iValue], _NetConnParentalControlsRequest));
    }
    
    // request refresh of PS+ membership
    if (iControl == 'pspl')
    {
        pRef->NpUsers.aPsPlus[iValue].NpCheckPlusParam.size = sizeof(pRef->NpUsers.aPsPlus[iValue].NpCheckPlusParam);
        pRef->NpUsers.aPsPlus[iValue].NpCheckPlusParam.userId = pRef->NpUsers.aUserServiceUserId[iValue];
        pRef->NpUsers.aPsPlus[iValue].NpCheckPlusParam.features = iValue2;
        return(_NetConnNpAsyncRequest(pRef, iValue, (NetConnAsyncRequestDataT*)&pRef->NpUsers.aPsPlus[iValue], _NetConnPsPlusRequest));
    }
    if (iControl == 'scid')
    {
        // iValue is supposed to contain the size of the buffer to which pValue points
...
int32_t NetConnStatus(int32_t iKind, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    if (iKind == 'acct')
    {
        ...
        userId = NetConnStatus('suid', iData, NULL, 0);
        if (userId < 0)
        {
            NetPrintf(("netconnps4: NetConnStatus('acct') Get user id for user index: %i failed.\n", iData));
            return(-2);
        }

        iRet = sceNpGetAccountIdA(userId, (SceNpAccountId *)pBuf);
        if (iRet < 0)
        {
            NetPrintf(("netconnps4: NetConnStatus('acct') Get AccountId for SceUserServiceUserId: %i at user index: %i failed.\n", userId, iData));
            return(-3);
        }
    ...
    if (iKind == 'eusr')
    {
        if ((iData >= 0) && (iData < NETCONN_MAXLOCALUSERS))
        {
           if ((pBuf != NULL) && (iBufSize >= (int32_t)sizeof(void *)))
           {
               ...
               *pIEAUser = (void *)pRef->aIEAUsers[iData];
               return(0);
           }
        }
        return(-1);
    ...
    if ((iKind == 'npid') && (iBufSize >= (int32_t)sizeof(SceNpId)) && (iData < NETCONN_MAXLOCALUSERS))
    {
        if (iData >= 0)
        {
            ds_memcpy(pBuf, &pRef->NpUsers.aNpid[iData], sizeof(SceNpId));
            return(0);
        }
    ...
    if ((iKind == 'npst') && (iData < NETCONN_MAXLOCALUSERS))
    {
        if (iData >= 0)
        {
            return(pRef->NpUsers.aNpState[iData]);
        }
    ...
    if (iKind == 'soid')
    {
        ...
        {
            SceUserServiceUserId sceUserServiceUserId = pRef->NpUsers.aUserServiceUserId[iData];

            // ignore if user is not SIGNEDIN with PSN
            if (pRef->NpUsers.aNpState[iData] == SCE_NP_STATE_SIGNED_IN)
            {
                // check if the cache is dirty
                NetCritEnter(&pRef->Common.crit);
                if( pRef->aSceNpOnlineIdCache[iData].uInvalid == TRUE )
                {
                    //get a fresh value for onlineid
                    pRef->aSceNpOnlineIdCache[iData].iResult = _NetConnSaveErr(pRef, sceNpGetOnlineId(sceUserServiceUserId, &pRef->aSceNpOnlineIdCache[iData].NpOnlineId));
                    pRef->aSceNpOnlineIdCache[iData].uInvalid = FALSE;
                }

                if(pRef->aSceNpOnlineIdCache[iData].iResult == SCE_OK)
                {
                    ds_memcpy(pBuf, &pRef->aSceNpOnlineIdCache[iData].NpOnlineId, iBufSize);
                }
                else
                {
                    NetPrintf(("netconnps4: sceNpGetOnlineId(0x%08x) (idx=%d) failed with %s\n", sceUserServiceUserId, iData, DirtyErrGetName(pRef->aSceNpOnlineIdCache[iData].iResult)));
                }
                NetCritLeave(&pRef->Common.crit);

                return(pRef->aSceNpOnlineIdCache[iData].iResult);
            }
            NetPrintf(("netconnps4: NetConnStatus('soid') sceUserServiceUserId(0x%08x) (idx=%d) no op, user status(%d) not signed in\n", sceUserServiceUserId, iData, pRef->NpUsers.aNpState[iData]));
        }
        else
        {
    ...
    if (iKind == 'suid')
    {
        if (iData >= 0 && iData < NETCONN_MAXLOCALUSERS)
        {
            return(pRef->NpUsers.aUserServiceUserId[iData]);
        }
        else
        {
            return(-1);
    ...
    if (iKind == 'pctl')
    {
        ...
        {
            return(_NetConnReadAsyncResponse(pRef, iData, pBuf, iBufSize, (NetConnAsyncRequestDataT*)&pRef->NpUsers.aParentalCtrls[iData], _NetConnParentalControlsRequest, _NetConnParentalControlsResponse));
        }
    }

    if (iKind == 'pspl')
    {
        if (iData < 0 || iData >= NETCONN_MAXLOCALUSERS || pBuf == NULL || iBufSize != sizeof(pRef->NpUsers.aPsPlus[iData].NpCheckPlusResult.authorized))
        {
        ...
        else
        {
            ...
            int32_t iRet = _NetConnReadAsyncResponse(pRef, iData, &plusResult, sizeof(plusResult), (NetConnAsyncRequestDataT*)&pRef->NpUsers.aPsPlus[iData], NULL, _NetConnPsPlusResponse);
            if (iRet > 0)
            {
                NetPrintf(("netconnps4: NetConnStatus('pspl') PS+ membership request for user (%d) returned (%s).\n", iData, pRef->NpUsers.aPsPlus[iData].NpCheckPlusResult.authorized ? "true" : "false"));
                ds_memcpy(pBuf, &pRef->NpUsers.aPsPlus[iData].NpCheckPlusResult.authorized, sizeof(pRef->NpUsers.aPsPlus[iData].NpCheckPlusResult.authorized));
            }
            else if (iRet == -2)
            {
    ...
    if (iKind == 'iind')
    {
        ...
        */
        ...
        if (sceUserServiceGetInitialUser(&userId) == SCE_OK)
        {
            ...
            for (iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; ++iIndex)
            {
                if (pRef->NpUsers.aUserServiceUserId[iIndex] == userId)
                {
                    return(iIndex);
                }
            }
    ...
    if (iKind == 'indx')
    {
        ...
        for (iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; ++iIndex)
        {
            if (pRef->NpUsers.aUserServiceUserId[iIndex] == iData)
            {
                return(iIndex);
            }
        }
    ...
    if (iKind == 'mask')
    {
        ...
        for (iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; ++iIndex)
        {
            if (iIndex >= 0 && pRef->NpUsers.aNpState[iIndex] == SCE_NP_STATE_SIGNED_IN)
            {
                iUserMask |= (1 << iIndex);
            }
        }
    ...
    if (iKind == 'gtag')
    {
        ...
        if (pRef->NpUsers.aUserServiceUserId[iData] == SCE_USER_SERVICE_USER_ID_INVALID)
        {
        ...
        if (pRef->NpUsers.aNpState[iData] == SCE_NP_STATE_SIGNED_IN)
        {
            ds_strnzcpy((char*)pBuf, pRef->NpUsers.aNpid[iData].handle.data, iBufSize);
            return(0);
        }
    ...
    if (iKind == 'chav')
    {
        ...
        {
            if (pRef->NpUsers.aNpAvailability[iData].bDone == TRUE)
            {
                return(pRef->NpUsers.aNpAvailability[iData].iResult);
            }
            else
            {
...
int32_t NetConnShutdown(uint32_t uShutdownFlags)
{
    ...
    if (--(pRef->iRefCount) > 0)
    {
        NetPrintf(("netconnps4: NetConnShutdown() called, new refcount is %d\n", pRef->iRefCount));
    ...
    for (iUserIndex = 0; iUserIndex < NETCONN_MAXLOCALUSERS; iUserIndex++)
    {
        ...
        _NetConnCleanupNpRequest(pRef, iUserIndex, (NetConnAsyncRequestDataT*)&pRef->NpUsers.aParentalCtrls[iUserIndex]);
    ...
    for (iUserIndex = 0; iUserIndex < NETCONN_MAXLOCALUSERS; iUserIndex++)
    {
        // release IEAUser if we still have a reference on it.
        if (pRef->aIEAUsers[iUserIndex])
        {
            pRef->aIEAUsers[iUserIndex]->Release();
            pRef->aIEAUsers[iUserIndex] = NULL;
        }
    }
