...
int32_t UserApiPlatRequestProfile(UserApiRefT *pRef, uint32_t uUserIndex, DirtyUserT *pLookupUsers, int32_t iLookupUsersLength)
{
    ...
    // format request body
    JsonInit(strJson, sizeof(strJson), FALSE);

    JsonArrayStart(strJson, "settings");
    JsonAddStr(strJson, NULL, "GameDisplayName");
    JsonAddStr(strJson, NULL, "GameDisplayPicRaw");
    JsonAddStr(strJson, NULL, "Gamertag");
    JsonAddStr(strJson, NULL, "SpeechAccessibility");
    JsonArrayEnd(strJson);

    JsonArrayStart(strJson, "userIds");
    for (iIndex = 0; iIndex < iLookupUsersLength; iIndex += 1)
    {
        ...
        JsonAddStr(strJson, NULL, strXuid);
    }
    JsonArrayEnd(strJson);

    if ((iResult = ProtoHttpRequest(pUserContext->pHttp, "https://profile.xboxlive.com/users/batch/profile/settings", JsonFinish(strJson), -1, PROTOHTTP_REQUESTTYPE_POST)) > 0)
    {
...
int32_t UserApiPlatUpdate(UserApiRefT *pRef)
{
    ...
    for (uUserIndex = 0; uUserIndex < NETCONN_MAXLOCALUSERS; ++uUserIndex)
    {
        ...
        if (pUserContext->pHttp != NULL)
        {
            ...
            if (pUserContext->bInProgress)
            {
                if ((iResult = ProtoHttpRecvAll(pUserContext->pHttp, pUserContext->pProfileResponse, pUserContext->iProfileResponseSize)) > 0)
                {
                    NetPrintf(("userapixboxone: The profile request succeeded\n"));
                    pUserContext->bInProgress = FALSE;
                    pRef->bAvailableDataIndex[uUserIndex] = TRUE;
                }
                else if (iResult == PROTOHTTP_RECVFAIL)
                {
                ...
                else if (iResult == PROTOHTTP_RECVBUFF)
                {
                    ...
                    pUserContext->iProfileResponseSize = (pUserContext->iProfileResponseSize == 0) ? USERAPI_PROFILE_DEFAULT_SIZE : (pUserContext->iProfileResponseSize * 2);
                    // allocate new memory
                    if ((pNewProfileResponse = (char *)DirtyMemAlloc(pUserContext->iProfileResponseSize, USERAPI_MEMID, pRef->iMemGroup, pRef->pMemGroupUserData)) != NULL)
                    {
                        ...
                        {
                            ds_memclr(pNewProfileResponse, pUserContext->iProfileResponseSize);
                        }
