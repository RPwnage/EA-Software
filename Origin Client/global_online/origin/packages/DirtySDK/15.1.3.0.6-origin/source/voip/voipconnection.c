...
#define VOIP_PACKET_VERSION     ('g')           //!< current packet version
...
/*F********************************************************************************/
/*!
    \Function   _VoipDecodeWStringWithoutCompresssion

    \Description
        Decode a wchar string previously encoded with _VoipEncodeWSTring(compression disabled)

    \Input *pBuf            - destination buffer
    \Input iBufSize         - size of destination buffer (in bytes)
    \Input *pEncodedText    - encoded text string
    \Input iEncodedTextSize - size of encoded text string (in bytes)

    \Output
        int32_t             - amount of bytes written in destination buffer

    \Notes
        This function does not assume the encoded output is NULL terminated.

    \Version 03/03/2017 (mclouatre)
*/
/********************************************************************************F*/
static int32_t _VoipDecodeWStringWithoutCompresssion(uint8_t *pBuf, int32_t iBufSize, const uint8_t *pEncodedText, int32_t iEncodedTextSize)
{
    wchar_t *pWrite = (wchar_t *)pBuf;
    wchar_t *pEnd = (wchar_t *)(pBuf + iBufSize);
    const uint8_t *pRead = pEncodedText;
    int32_t iByteIndex;

    if ((sizeof(wchar_t) != 2) && (sizeof(wchar_t) != 4))
    {
        NetPrintf(("voipconnection: critical error - unsupported wchar length (%d) in _VoipDecodeWString()\n", sizeof(wchar_t)));
        return(0);
    }

    if (iBufSize < (signed)sizeof(wchar_t))
    {
        NetPrintf(("voipconnection: _VoipDecodeWString() cannot even fit a single widechar in the destination buffer (size is %d).\n", iBufSize));
        return(0);
    }
    ...
    for (iByteIndex = 0; (iByteIndex < iEncodedTextSize) && ((pEnd - pWrite) > (signed)sizeof(wchar_t)); iByteIndex += sizeof(wchar_t))
    {
        switch (sizeof(wchar_t))
        {
            case 2:
                *pWrite = (wchar_t)_VoipDecodeU16(pRead);
                break;
            case 4:
                *pWrite = (wchar_t)_VoipDecodeU32(pRead);
                break;
            default:
                break;
        }
    ...
    *pWrite = L'\0';
    ...
    #if DIRTYCODE_LOGGING
    if (iByteIndex == (iBufSize - (signed)sizeof(wchar_t)))
    {
        NetPrintf(("voipconnection: _VoipDecodeWStringWithoutCompresssion() only partially decoded a string that was too long (%d vs %d)\n", iByteIndex, (iBufSize - (signed)sizeof(wchar_t))));
    }
    #endif

    return((uint8_t *)pWrite - pBuf);
}

/*F********************************************************************************/
/*!
    \Function   _VoipDecodeWStringWithCompresssion

    \Description
        Decode a wchar string previously encoded with _VoipEncodeWSTring(compression enabled)
    ...
    \Notes
        This function assumes the encoded output is NULL terminated.

    \Version 03/03/2017 (mclouatre)
*/
/********************************************************************************F*/
static int32_t _VoipDecodeWStringWithCompresssion(uint8_t *pBuf, int32_t iBufSize, const uint8_t *pEncodedText, int32_t iEncodedTextSize)
{
#if defined(DIRTYCODE_XBOXONE)
    int32_t iSizeNeeded = MultiByteToWideChar(CP_UTF8, 0, (char *)pEncodedText, -1, NULL, 0) * sizeof(wchar_t);
    int32_t iBytesWritten = 0;

    if (iSizeNeeded < iBufSize)
    {
        MultiByteToWideChar(CP_UTF8, 0, (char *)pEncodedText, -1, (wchar_t *)pBuf, (iBufSize / sizeof(wchar_t)));
        iBytesWritten = iSizeNeeded;
    }
    else
    {
        NetPrintf(("voipconnection: _VoipDecodeWStringWithCompresssion() could not decode string because destination buffer is too small (%d vs %d)\n", iBufSize, iSizeNeeded));
    }

    return(iBytesWritten);
#else
    NetPrintf(("voipconnection: _VoipDecodeWStringWithCompresssion() not yet implemented\n"));
    return(0);
#endif
}
...
/*F********************************************************************************/
/*!
    \Function   _VoipDecodeWString

    \Description
        Decode a wchar string previously encoded with _VoipEncodeWSTring() 

    \Input *pBuf            - destination buffer
    \Input iBufSize         - size of destination buffer (in bytes)
    \Input *pEncodedText    - encoded text string
    \Input iEncodedTextSize - size of encoded text string (in bytes)
    \Input bUseCompression  - TRUE if text compression is requested, FALSE otherwise
...
*/
/********************************************************************************F*/
static int32_t _VoipDecodeWString(uint8_t *pBuf, int32_t iBufSize, const uint8_t *pEncodedText, int32_t iEncodedTextSize, int32_t bUseCompression)
{
    if (bUseCompression)
    {
        return(_VoipDecodeWStringWithCompresssion(pBuf, iBufSize, pEncodedText, iEncodedTextSize));
    }
    else
    {
        return(_VoipDecodeWStringWithoutCompresssion(pBuf, iBufSize, pEncodedText, iEncodedTextSize));
    }
}
...
/*F********************************************************************************/
/*!
    \Function   _VoipEncodeWStringWithoutCompression

    \Description
        Transform specified widechars string into an uncompressed encoded format ready
        for transmission over the network.

    \Input *pBuf    - destination buffer
    \Input iBufSize - size of destination buffer (in bytes)
    \Input *pText   - text string to be packed

    \Output
        int32_t     - amount of bytes written in destination buffer

    \Notes
        This function does not add the NULL termination character to the encoded output.

    \Version 03/03/2017 (mclouatre)
*/
/********************************************************************************F*/
static int32_t _VoipEncodeWStringWithoutCompression(uint8_t *pBuf, int32_t iBufSize, const wchar_t *pText)
{
    uint8_t *pWrite = pBuf;
    uint8_t *pEnd = pBuf + iBufSize;
    const wchar_t *pRead = pText;
    int32_t iStringLength = (signed)wcslen(pText);
    int32_t iCharIndex;

    if ((sizeof(wchar_t) != 2) && (sizeof(wchar_t) != 4))
    {
        NetPrintf(("voipconnection: critical error - unsupported wchar length (%d) in _VoipEncodeWString()\n", sizeof(wchar_t)));
        return(0);
    }

    for (iCharIndex = 0; (iCharIndex < iStringLength) && ((pEnd - pWrite) > (signed)sizeof(wchar_t)); iCharIndex++)
    {
        switch (sizeof(wchar_t))
        {
            case 2:
                _VoipEncodeU16(pWrite, *pRead);
                break;
            case 4:
                _VoipEncodeU32(pWrite, *pRead);
                break;
            default:
                break;
        }

        pWrite += sizeof(wchar_t);
        pRead++;
    }

    #if DIRTYCODE_LOGGING
    if (iCharIndex != iStringLength)
    {
        NetPrintf(("voipconnection: _VoipEncodeWStringWithoutCompression() only partially encoded a string that was too long (%d vs %d)\n", iCharIndex, iStringLength));
    }
    #endif
...
/*F********************************************************************************/
/*!
    \Function   _VoipEncodeWStringWithCompression

    \Description
        Transform specified widechars string into a compressed encoded format
    ...
    \Notes
        This function adds the NULL termination character to the encoded output.

    \Version 03/03/2017 (mclouatre)
*/
/********************************************************************************F*/
static int32_t _VoipEncodeWStringWithCompression(uint8_t *pBuf, int32_t iBufSize, const wchar_t *pText)
{
#if defined(DIRTYCODE_XBOXONE)
    int32_t iSizeNeeded = WideCharToMultiByte(CP_UTF8, 0, pText, -1, NULL, 0, NULL, 0);
    int32_t iBytesWritten = 0;

    if (iSizeNeeded < iBufSize)
    {
        iBytesWritten = WideCharToMultiByte(CP_UTF8, 0, pText, -1, (char *)pBuf, iBufSize, NULL, 0);
    }
    else
    {
        int32_t iPartialWCharCount = iBufSize / sizeof(wchar_t);

        // translate and leave space for null termination
        iBytesWritten = WideCharToMultiByte(CP_UTF8, 0, pText, (iPartialWCharCount - 1), (char *)pBuf, iBufSize, NULL, 0);

        // add null termination
        pBuf[iBytesWritten] = '\0';
        iBytesWritten++;

        NetPrintf(("voipconnection: _VoipEncodeWStringWithCompression() only partially encoded a string that was too long (%d vs %d)\n", iPartialWCharCount, (signed)wcslen(pText)));
    }

    return(iBytesWritten);
#else
    NetPrintf(("voipconnection: _VoipEncodeWStringWithCompression() not yet implemented\n"));
    return(0);
#endif
}
...
/*F********************************************************************************/
/*!
    \Function   _VoipEncodeWString

    \Description
        Transform specified widechars string into an encoded format ready for transmission
        over the network.
...
*/
/********************************************************************************F*/
static int32_t _VoipEncodeWString(uint8_t *pBuf, int32_t iBufSize, const wchar_t *pText, int32_t bUseCompression)
{
    if (bUseCompression)
    {
        return(_VoipEncodeWStringWithCompression(pBuf, iBufSize, pText));
    }
    else
    {
        return(_VoipEncodeWStringWithoutCompression(pBuf, iBufSize, pText));
    }
}
...
/*F********************************************************************************/
/*!
    \Function   _VoipReliableDatatEnqueue
...
static void _VoipConnectionPrintRemoteUsers(VoipConnectionT *pConnection, int32_t iConnID)
{
    ...
    for (iIndex = 0; iIndex < VOIP_MAXLOCALUSERS; iIndex++)
    {
        ...
        {
            NetPrintf(("voipconnection: [%d]     remote user #%d --> %s\n", iConnID, iIndex, ((VoipUserT *)&pConnection->RemoteUsers[iIndex])->strUserId));
        }
    }
}
...
static void _VoipReliableDataInboundUserAddProcess(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, ReliableDataT *pReliableData)
{
    ...
    #if defined(DIRTYCODE_XBOXONE)
    VoipUserExT *pRemoteUser = (VoipUserExT *)(&pReliableData->aData[0] + 1);
    #else
    ...
    if (VOIP_NullUser((VoipUserT *)&pConnection->RemoteUsers[uUserIndex]))
    {
        ...
        // register the user
        pConnectionlist->pRegUserCb((VoipUserT *)&pConnection->RemoteUsers[uUserIndex], TRUE, pConnectionlist->pRegUserUserData);
    ...
    {
        NetPrintf(("voipconnection: [%d] join-in-progress for remote user %s failed because index %d already used by %s\n", (pConnection - &pConnectionlist->pConnections[0]),
            ((VoipUserT *)pRemoteUser)->strUserId, uUserIndex, ((VoipUserT *)&pConnection->RemoteUsers[uUserIndex])->strUserId));
    }
}
...
static void _VoipReliableDataInboundUserRemProcess(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, ReliableDataT *pReliableData)
{
    ...
    if (!VOIP_NullUser((VoipUserT *)&pConnection->RemoteUsers[uUserIndex]))
    {
        ...
        {
            // unregister the user
            pConnectionlist->pRegUserCb((VoipUserT *)&pConnection->RemoteUsers[uUserIndex], FALSE, pConnectionlist->pRegUserUserData);
        ...
        {
            NetPrintf(("voipconnection: [%d] leave-in-progress for remote user %s failed because remote user at index %d rather is %s\n", (pConnection - &pConnectionlist->pConnections[0]),
                ((VoipUserT *)pRemoteUser)->strUserId, uUserIndex, ((VoipUserT *)&pConnection->RemoteUsers[uUserIndex])->strUserId));
        }
    }
    else
    {
        NetPrintf(("voipconnection: [%d] leave-in-progress for remote user %s failed because no remote user at index %d\n", (pConnection - &pConnectionlist->pConnections[0]),
            ((VoipUserT *)pRemoteUser)->strUserId, uUserIndex));
    }
}
...
{
    NetPrintf(("voipconnection: [%d] warning - processing of inbound reliable data of type OPAQUE not yet implemented\n", (pConnection - &pConnectionlist->pConnections[0])));
}
...
static void _VoipReliableDataInboundTextProcess(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, ReliableDataT *pReliableData)
{
    ...
    uint8_t DecodeBuffer[VOIP_MAXRELIABLEDATA * sizeof(wchar_t)];
    int32_t iDecodedBytes;

    iDecodedBytes = _VoipDecodeWString(DecodeBuffer, sizeof(DecodeBuffer), (uint8_t *)&pReliableData->aData[1], uTextSize, TRUE);
    ...
    if (iDecodedBytes > 0)
    {
        if (_VoipConnectionIsTranscribedTextRequested(pConnectionlist))
        {
            ...
            {
                pConnectionlist->pTextCb(iConnID, uUserIndex, (wchar_t *)&DecodeBuffer[0], (void *)pConnectionlist->pTextUserData);
            }
            else
            {
    ...
    if (uFlags &  VOIP_PACKET_RELIABLE_FLAG_ACK_ACKCNF)
    {
        ...
        for (iReliableAckEntryIndex = 0; iReliableAckEntryIndex < iReliableAckEntriesCount; iReliableAckEntryIndex++)
        {
            ...
            if (pConnection != NULL)
            {
                ...
                uint32_t uLocalClientId = pConnection->uLocalClientId == 0 ?  pConnectionlist->uClientId : pConnection->uLocalClientId;
...
static uint8_t * _VoipReliableDataUnpackDATA(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, uint8_t uFlags, uint32_t bPing, uint8_t *pRead)
{
    ...
    if (uFlags & VOIP_PACKET_RELIABLE_FLAG_DATA)
    {
        ...
        for (iReliableDataEntryIndex = 0; iReliableDataEntryIndex < iReliableDataEntriesCount; iReliableDataEntryIndex++)
        {
            ...
            if (uClientId == pConnection->uLocalClientId)
            {
            ...
            else
            {
                ...
                NetPrintf(("voipconnection: [%d] dropped inbound reliable data embedded in %s packet - client id mismatch (target: 0x%08x - local: 0x%08x)\n",
                    (pConnection - &pConnectionlist->pConnections[0]), (bPing ? "PNG" : "MIC"), uClientId, pConnection->uLocalClientId));
                #endif
            }
        }
    }
...
static void _VoipConnectionConn(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, uint32_t uTick)
{
    ...
    for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS_EXTENDED; iUserIndex++)
    {
        if (pConnectionlist->aIsParticipating[iUserIndex] != FALSE)
        {
            ConnPacket.LocalUsers[iUserIndex] = pConnectionlist->LocalUsers[iUserIndex];
        }
    }
    ...
    pConnection->iSendResult = _VoipSocketSendto(pConnectionlist->pSocket, uLocalClientId, &ConnPacket.Head, sizeof(ConnPacket), 0, (struct sockaddr *)&pConnection->SendAddr, sizeof(pConnection->SendAddr));
...
static void _VoipConnectionPing(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, uint32_t uTick)
{
    ...
    for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS_EXTENDED; iUserIndex++)
    {
        _VoipEncodeU32(PingPacket.aChannels[iUserIndex].aChannelId, pConnectionlist->aChannels[iUserIndex]);
    }

    // if needed, add reliable protocol components to this ping packet
    pWrite = _VoipReliableDataOutProcess(pConnectionlist, (pConnection - &pConnectionlist->pConnections[0]), &PingPacket.Head, &PingPacket.aData[0], sizeof(PingPacket.aData));
...
static void _VoipConnectionRecvSingle(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, struct sockaddr_in *pSendAddr, VoipPacketBufferT *pPacket, int32_t iSize)
{
    ...
    switch (uPktType)
    {
        ...
        case 'C':
        {
            ...
            ds_memcpy_s(pConnection->RemoteUsers, sizeof(pConnection->RemoteUsers), pConnPacket->LocalUsers, sizeof(pConnPacket->LocalUsers));
        ...
        case 'P':
        {
            ...
            if (pConnection->bConnPktRecv)
            {
                ...
                for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS; iUserIndex++)
                {
                    pConnection->uRemoteUserStatus[iUserIndex] &= ~VOIP_REMOTE_USER_RECVVOICE;
                }
                ...
                for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS_EXTENDED; iUserIndex++)
                {
                    pConnection->aRecvChannels[iUserIndex] = _VoipDecodeU32(pPingPacket->aChannels[iUserIndex].aChannelId);
                    #if VOIP_CONNECTION_DEBUG
                ...
                _VoipReliableDataInProcess(pConnectionlist, pConnection, &pPingPacket->Head, &pPingPacket->aData[0]);
        ...
        case 'M':
        {
            ...
            if (pConnection->bConnPktRecv)
            {
                ...
                // if the voice packet is from a remote shared user update the last receive for all the remote user
                if (pMicrPacket->MicrInfo.uUserIndex == VOIP_SHARED_USER_INDEX)
                {
                    for (iRemoteUserIndex = 0; iRemoteUserIndex < VOIP_MAXLOCALUSERS; ++iRemoteUserIndex)
                    {
                        if (!VOIP_NullUser((VoipUserT *)&pConnection->RemoteUsers[iRemoteUserIndex]))
                        {
                ...
                if (pConnectionlist->uRecvMask & (1 << iConnID))
                {
                    ...
                    {
                        pConnectionlist->pVoiceCb((VoipUserT *)&pConnection->RemoteUsers[0], &pMicrPacket->MicrInfo, pRead, pConnectionlist->pVoiceUserData);
                    }
                }
...
*/
/********************************************************************************F*/
void VoipConnectionSetCallbacks(VoipConnectionlistT *pConnectionlist, VoipConnectRecvVoiceCbT *pVoiceCb, void *pVoiceUserData, VoipConnectRecvTextCbT *pTextCb, void *pTextUserData, VoipConnectRegUserCbT *pRegUserCb, void *pRegUserUserData)
{
...
int32_t VoipConnectionStart(VoipConnectionlistT *pConnectionlist, int32_t iConnID, uint32_t uAddr, uint32_t uConnPort, uint32_t uBindPort, uint32_t uClientId, uint32_t uSessionId)
{
    ...
    */
    ...
    pConnection->uInSeqNb = 127;  // use 127 as the last received inbound sequence number (such that 1 is the next expected inbound sequence number)
...
void VoipConnectionRemove(VoipConnectionlistT *pConnectionlist, int32_t iConnID)
{
    if ((iConnID >= 0) && (iConnID < pConnectionlist->iMaxConnections))
    {
        ...
        // unregister all the remote users from this connection
        VoipConnectionRegisterRemoteTalkers(pConnectionlist, iConnID, FALSE);
...
*/
/********************************************************************************F*/
void VoipConnectionSend(VoipConnectionlistT *pConnectionlist, const uint8_t *pVoiceData, int32_t iDataSize, const uint8_t *pMetaData, int32_t iMetaDataSize, uint32_t uUserIndex, uint8_t uSendSeqn)
{
    ...
    for (iConnID = 0; iConnID < pConnectionlist->iMaxConnections; iConnID++)
    {
        ...
        if ((pConnection->eState == ST_ACTV) && (pConnectionlist->uSendMask & (1 << iConnID)))
        {
            ...
            {
                pMicrPacket = &pConnection->VoipMicrPacket[uUserIndex]; // ref packet
            }
            else
            {
                // to avoid a coverity warning about a potential out of bound access if uUserIndex is VOIP_INVALID_SHARED_USER_INDEX
                // which will never happen in practice because VoipConnectionSend() is always called with uUserIndex 0 on PC
                pMicrPacket = &pConnection->VoipMicrPacket[0];
            }
...
int32_t VoipConnectionFlush(VoipConnectionlistT *pConnectionlist, int32_t iConnID)
{
    ...
    for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS_EXTENDED; iUserIndex++)
    {
        ...
        // is the connection active, and are we sending to it?
        if ((pConnection->eState == ST_ACTV) && (pConnectionlist->uSendMask & (1 << iConnID)) && (iNumSubPackets > 0))
        {
            // see if we need to send any buffered voice data
...
void VoipConnectionRegisterRemoteTalkers(VoipConnectionlistT *pConnectionlist, int32_t iConnID, uint32_t bRegister)
{
    ...
    for (iUser = 0; iUser < VOIP_MAXLOCALUSERS_EXTENDED; iUser++)
    {
        // if no user, don't register
        if (VOIP_NullUser((VoipUserT *)(&pConnection->RemoteUsers[iUser])))
        {
            continue;
        }

        // register the user
        pConnectionlist->pRegUserCb((VoipUserT *)&pConnection->RemoteUsers[iUser], bRegister, pConnectionlist->pRegUserUserData);
    }
...
/*!
    ...
    \Input *pConnectionlist - connectionlist
    \Input uLocalUserIndex  - local user index
    \Input *pText           - pointer to text message to be sent

    \Version 09/18/2014 (mclouatre)
*/
/********************************************************************************F*/
void VoipConnectionReliableTranscribedTextMessage(VoipConnectionlistT *pConnectionlist, uint8_t uLocalUserIndex, const wchar_t *pText)
{
    int32_t iConnectionIndex;
    ReliableDataT ReliableData;
    uint8_t *pWrite = &ReliableData.aData[0];
    ...
    *pWrite++ = uLocalUserIndex;
    ...
    // then append text
    pWrite += _VoipEncodeWString(pWrite, sizeof(ReliableData.aData) - 1, pText, TRUE);
    ...
    for (iConnectionIndex = 0; iConnectionIndex < pConnectionlist->iMaxConnections; iConnectionIndex++)
    {
        ...
        if (pConnection->eState == ST_ACTV && pConnection->bTranscribedTextRequested)
        {
...
/*F********************************************************************************/
/*!
    \Function VoipConnectionReliableBroadcastOpaque

    \Description
        Use this function to broadcast opaque data that needs to be sent reliably
        on all active connections (or connections in ST_CONN state).
