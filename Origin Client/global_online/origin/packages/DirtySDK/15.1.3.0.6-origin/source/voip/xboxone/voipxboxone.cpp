...
/*F********************************************************************************/
/*!
    \Function   _VoipApplyBadRepAutoMute

    \Description
        applies the bad rep auto muting on a per connection basis

    \Input *pVoip  - VoipRefT

    \Version 10/28/2015 (tcho)
*/
/********************************************************************************F*/
static void _VoipApplyBadRepAutoMute(VoipRefT *pVoip)
{
    ...
    {
       uint32_t uNewSpkMask = VoipCommonStatus(pVoipCommon, 'uspk', 0, NULL, 0);
       uint32_t uNewMicMask = VoipCommonStatus(pVoipCommon, 'umic', 0, NULL, 0);
       uint32_t uOldSpkMask = uNewSpkMask;
       uint32_t uOldMicMask = uNewMicMask;

        // only apply the bad rep auto mute when the bad rep check is enabled and the bad rep check is completed and if there is a change in the connection list
        if (VoipCommonStatus(pVoipCommon, 'amcc', 0, NULL, 0) && VoipCommonStatus(pVoipCommon, 'ambr', 0, NULL, 0) && pVoipCommon->Connectionlist.bUpdateMute)
        {
            for (iConnID = 0; iConnID < pVoipCommon->Connectionlist.iMaxConnections; ++iConnID)
            {
                uint8_t bShouldMute = FALSE;
                int32_t iRemoteUserIndex;
                VoipConnectionT pConnection = pVoipCommon->Connectionlist.pConnections[iConnID];

                // only apply auto muting if we are connected on this connection and auto muting has not been done on this connection
                if ((pConnection.uRemoteConnStatus & VOIP_CONN_CONNECTED) && (pConnection.bAutoMuted == FALSE))
                {
                    for (iRemoteUserIndex = 0; iRemoteUserIndex < VOIP_MAXLOCALUSERS; ++iRemoteUserIndex)
                    {
                        VoipUserT *pRemoteUser = (VoipUserT *)(&pConnection.RemoteUsers[iRemoteUserIndex]);
                        bShouldMute |= VoipCommonStatus(pVoipCommon, 'amuc', 0, pRemoteUser, sizeof(VoipUserT));
                    }

                    if (bShouldMute == TRUE)
                    {
                        uNewSpkMask &= ~(1 << iConnID);
                        uNewMicMask &= ~(1 << iConnID);
                        pConnection.bAutoMuted = TRUE;
                    }
                }
            }

            if (uNewSpkMask != uOldSpkMask)
            {
                NetPrintf(("voipxboxone: applying bad rep inbound auto muting (old value: 0x%08x new value: 0x%08x\n)", uOldSpkMask, uNewSpkMask));
                VoipCommonControl(pVoipCommon, 'uspk', uNewSpkMask, NULL);
            }

            if (pVoip->bOnewayBadRepMute != TRUE)
            {
                if (uNewMicMask != uOldMicMask)
                {
                    NetPrintf(("voipxboxone: applying bad rep outbound auto muting (old value: 0x%08x new value: 0x%08x)\n", uOldMicMask, uNewMicMask));
                    VoipCommonControl(pVoipCommon, 'umic', uNewMicMask, NULL);
                }
            }

            // update voipgroup send/recv flags
            if (pVoipCommon->pCallback != NULL)
            {
                pVoipCommon->pCallback(pVoip, VOIP_CBTYPE_AMBREVENT, 0, pVoipCommon->pUserData);
            }

            pVoipCommon->Connectionlist.bUpdateMute = FALSE;
        }
    }
}
...
static DWORD _VoipThread(void *pParam)
{
    ...
    for (pVoip->iThreadActive = 1; pVoip->iThreadActive != 0 || (VoipHeadsetStatus(pVoip->Common.pHeadset, 'supd', 0, NULL, 0) && !NetConnStatus('susp', 0, NULL, 0)); )
    {
        ...
        */
        ...
        _VoipApplyBadRepAutoMute(pVoip);
...
static int32_t _VoipShutdownInternal(void *pDestroyData)
{
    ...
    // terminate voip serialization module
    VoipSerializeTerm();
...
VoipRefT *VoipStartup(int32_t iMaxPeers, int32_t iData)
{
    ...
    // init voip serialization module
    VoipSerializeInit();
    ...
    SetThreadPriority(pVoip->hThread, THREAD_PRIORITY_TIME_CRITICAL);

    // set the thread's cpu affinity based on netconn's settings
    if ((iThreadCpuAffinity = NetConnStatus('affn', 0, NULL, 0)) > 0)
    {
        if (SetThreadAffinityMask(pVoip->hThread, iThreadCpuAffinity) == 0)
        {
            NetPrintf(("voipxboxone: failed to set voip thread's cpu affinity (err=%d)\n", GetLastError()));
        }
    }
...
void VoipActivateLocalUser(VoipRefT *pVoip, int32_t iLocalUserIndex, uint8_t bActivate)
{
    ...
    if ((pVoip != NULL) && (iLocalUserIndex >= 0) && (iLocalUserIndex < NETCONN_MAXLOCALUSERS))
    {
        ...
        if (!VOIP_NullUser((VoipUserT *)(&pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex])))
        {
            ...
            {
                if ((iResult = VoipSerializeUser(refLocalUser, &pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex])) > sizeof(pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex].aSerializedUserBlob))
                {
                    NetPrintf(("voipxboxone: critical error - voip user serialization is overflowing the destination buffer\n"));
                }
