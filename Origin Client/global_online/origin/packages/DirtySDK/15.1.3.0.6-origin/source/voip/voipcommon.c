...
static void _VoipCommonComputeDefaultSharedChannelConfig(VoipCommonRefT *pVoipCommon)
{
    ...
    ds_memclr(&pVoipCommon->iDefaultSharedChannels, sizeof(pVoipCommon->iDefaultSharedChannels));
    ds_memclr(&pVoipCommon->uDefaultSharedModes, sizeof(pVoipCommon->uDefaultSharedModes));
...
/*F********************************************************************************/
/*!
    \Function   _VoipCommonPrepareChannelConfigApplication

    \Description
        Prepare for notifying the voip thread that it can now re-apply the most
        recent channel config... and then notify the voip thread by setting
        bApplyChannelConfig to TRUE.

    \Input *pVoipCommon - voip common state

    \Version 10/25/2016 (mclouatre)
*/
/********************************************************************************F*/
static void _VoipCommonPrepareChannelConfigApplication(VoipCommonRefT *pVoipCommon)
{
    // calculate the shared user config
...
/*!
    ...
    \Description
       ...
       \Input *pText            - pointer to beginning of text
       \Input *pUserData        - user data

    \Version 05/02/2017 (mclouatre)
*/
/********************************************************************************F*/
static void _VoipCommonReceiveTextDataCb(int32_t iConnId, int32_t iRemoteUserIndex, const wchar_t *pText, void *pUserData)
{
    VoipCommonRefT *pVoipCommon = (VoipCommonRefT *)pUserData;

    if (pVoipCommon->pTranscribedTextRecvCb != NULL)
    { 
        pVoipCommon->pTranscribedTextRecvCb(iConnId, iRemoteUserIndex, pText, pVoipCommon->pTranscribedTextUserData);
    }
    else
    {
        VoipUserT VoipUser;
        ...
        if (VoipStatus(VoipGetRef(), 'ruid', (int32_t)uConnUserPair, &VoipUser, sizeof(VoipUser)) == 0)
        {
            VoipHeadsetDisplayTranscribedText(pVoipCommon->pHeadset, &VoipUser, (void *)pText);
        }
        else
        {
            NetPrintf(("voipcommon: inbound transcribed text dropped because voipheaset can't find remote user index %d on connection %d\n", iRemoteUserIndex, iConnId));
        }
    }
}
...
/*!
    ...
    \Input *pTextDataCb - text data ready callback
...
*/
/********************************************************************************F*/
VoipRefT *VoipCommonStartup(int32_t iMaxPeers, int32_t iVoipRefSize, VoipHeadsetMicDataCbT *pMicDataCb, VoipHeadsetTextDataCbT *pTextDataCb, VoipHeadsetStatusCbT *pStatusCb, int32_t iData)
{
    int32_t iMemGroup;
    ...
    pVoipCommon->pHeadset = VoipHeadsetCreate(iMaxPeers, pMicDataCb, pTextDataCb, pStatusCb, pVoipCommon, iData);
    ...
    VoipConnectionSetCallbacks(&pVoipCommon->Connectionlist, &VoipHeadsetReceiveVoiceDataCb, (void *)pVoipCommon->pHeadset, &_VoipCommonReceiveTextDataCb, (void *)pVoipCommon, &VoipHeadsetRegisterUserCb, (void *)pVoipCommon->pHeadset);
...
void VoipCommonUpdateRemoteStatus(VoipCommonRefT *pVoipCommon)
{
    ...
    for (iConnection = 0; iConnection < pVoipCommon->Connectionlist.iMaxConnections; iConnection++)
    {
        ...
        if (pConnection->bTranscribedTextRequested)
        {
    ...
    if (bTextTranscriptionEnabled != pVoipCommon->bTextTranscriptionEnabled)
    {
        ...
        VoipHeadsetControl(pVoipCommon->pHeadset, 'stot', bTextTranscriptionEnabled, 0, NULL);
    }
}
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'stot' - turn on/off STT for local user specified with pValue
...
int32_t VoipCommonControl(VoipCommonRefT *pVoipCommon, int32_t iControl, int32_t iValue, void *pValue)
{
    ...
    if (iControl == 'vcid')
    {
        NetPrintf(("voipcommon: assigning local client id %08d to conn id %d\n", iValue, *(int32_t*)pValue));
        pVoipCommon->Connectionlist.pConnections[iValue].uLocalClientId = *(int32_t*)pValue;
        return(0);
    }
    if (iControl == 'conm')
    {
    ...
    if (iControl == 'shch')
    {
        ...
        {
            NetPrintf(("voipcommon: VoipCommonControl('shch') shared channel config is not supported on this platform\n"));
        }
        else
        {
            pVoipCommon->bUseDefaultSharedChannelConfig = iValue;
            _VoipCommonPrepareChannelConfigApplication(pVoipCommon);
    ...
    if (iControl == 'stot')
    {
        ...
        if (iUserIndex >= 0 && iUserIndex < VOIP_MAXLOCALUSERS)
        {
            NetPrintf(("voipcommon: %s STT for local user index %d\n", (iValue?"enabling":"disabling"), iUserIndex));
            pVoipCommon->Connectionlist.bTranscribedTextRequested[iUserIndex] = iValue;
            return(0);
        }
        else
        {
...
int32_t VoipCommonSelectChannel(VoipCommonRefT *pVoipCommon, int32_t iUserIndex, int32_t iChannel, VoipChanModeE eMode)
{
    ...
    if (iUserIndex != VOIP_SHARED_USER_INDEX)
    {
        ...
        else if (iUserIndex >=  VOIP_MAXLOCALUSERS)
        {
