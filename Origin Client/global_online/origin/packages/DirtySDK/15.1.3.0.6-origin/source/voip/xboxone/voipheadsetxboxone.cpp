...
#include <ppl.h>        // required for Concurrency namespace
#include <ppltasks.h>
#include <xaudio2.h>
...
#define VOIPHEADSET_USERLISTAPI_RESPONSE_SIZE           (1000)                                      // response size of the userlistapi
#define VOIPHEADSET_STATISTIC_SERVICE_SCID              "7492baca-c1b4-440d-a391-b7ef364a8d40"      // SCID for the statistic service
#define VOIPHEADSET_BADREP_CHECK_TIMEOUT                (5000)                                      // bad rep check timeout value
...
typedef struct LocalVoipUserT
{
    ...
    */
    ...
    Windows::Media::SpeechSynthesis::VoiceInformation ^refVoice;            //! voice type used when synthesizing text from speech impaired person
} LocalVoipUserT;
...
struct VoipHeadsetRefT
{
    ...
    Windows::Foundation::EventRegistrationToken ChatParticipantTranscribedTextReadyToken;
    ...
    UserListApiRefT *pUserListApiRef;
    void *pBadRepUserData;
    int32_t iPendingBadRepCheck;
    int32_t iPendingFrdCheck;
    int32_t iCompletedFrdCheck;
    int32_t iCompletedBadRepCheck;

    HMODULE hModule;                //!< handle to the module loaded with LoadLibraryEx()

    uint8_t bAutoMuteCheckCompleted;
    uint8_t bAutoMuteBadRep;
    uint8_t bDestroy;
    uint8_t uSendSeq;
    ...
    uint8_t bMicFocus;
    uint8_t bHeadsetFound;
    uint8_t bTextTranscriptionEnabled;
    uint8_t _pad[1];
};
...
void VoipHeadsetIsFriendCallback(UserListApiRefT *pRef, UserListApiIfATypeE eResponseType, UserListApiIsADataT *pUserApiEventData, void *pUserData);
...
/*F********************************************************************************/
/*!
    \Function   _VoipHeadsetParticipantTranscribedTextReadyHandler

    \Description
        Event handler (delegate) registered for transcribed text ready events.

    \Input *pHeadset        - module reference
    \Input ^refParticipant  - local participant for which the speech was transcribed to text
    \Input ^refText         - transcribed text

    \Version 05/31/2013 (mclouatre)
*/
/********************************************************************************F*/
static void _VoipHeadsetParticipantTranscribedTextReadyHandler(VoipHeadsetRefT *pHeadset, IChatParticipant ^refParticipant, Platform::String ^refText)
{
    int32_t iLocalUserIndex;

    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex++)
    {
        if (pHeadset->aLocalUsers[iLocalUserIndex].refChatParticipant != nullptr && pHeadset->aLocalUsers[iLocalUserIndex].refChatParticipant->User->Id == refParticipant->User->Id)
        {
            break;
        }
    }

    if (iLocalUserIndex != VOIP_MAXLOCALUSERS)
    {
        NetPrintfVerbose((pHeadset->iDebugLevel, 0, "voipheadsetxboxone: transcribed text ready event (%d wchars) for local user index %d --> %S\n", wcslen(refText->Data()), iLocalUserIndex, refText->Data()));

        pHeadset->pTextDataCb(refText->Data(), iLocalUserIndex, pHeadset->pCbUserData);
    }
    else
    {
        NetPrintf(("voipheadsetxboxone: failed to find the local user for transcribed text ready event --> %S\n", refText->Data()));
    }
}
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetRegisterParticipantTranscribedTextReadyHandler

    \Description
        Register the transcribed text ready handler.
    ...
    \Notes
        \verbatim
            Reference: https://forums.xboxlive.com/questions/63670/chat-ttsstt-what-is-the-maximum-length-wchar-count.html

            From MS, about the size of the text string provided with the TranscribedTextReadyHandler:
            "There is no limit on this size. It depends on how much text gets transcribed in a particular phrase. 
            The service generally doesn’t allow a single phrase to exceed 50 seconds of audio. GameChat.dll (our consumer
            of this event) enforces an arbitrary limit of 1024 characters."
        \endverbatim

    \Version 05/02/2017 (mclouatre)
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetRegisterParticipantTranscribedTextReadyHandler(VoipHeadsetRefT *pHeadset)
{
    int32_t iRetCode = 0; // default to success

    try
    {
        pHeadset->ChatParticipantTranscribedTextReadyToken = pHeadset->refChatSession->TranscribedTextReadyEvent += ref new ChatParticipantTranscribedTextReadyHandler(
            [ pHeadset ](IChatParticipant^ refParticipant, Platform::String^ refText)
        {
            _VoipHeadsetParticipantTranscribedTextReadyHandler(pHeadset, refParticipant, refText);
        });

        NetPrintf(("voipheadsetxboxone: successfully registered the transcribed text ready handler\n"));
    }
    catch (Exception ^ e)
    {
        NetPrintf(("voipheadsetxboxone: exception caught while trying to register for transcribed text ready events (err=%s: %S)\n", DirtyErrGetName(e->HResult), e->Message->Data()));
        iRetCode = -1;
    }
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetUnregisterParticipantTranscribedTextReadyHandler

    \Description
        Unregister the transcribed text ready handler.
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetUnregisterParticipantTranscribedTextReadyHandler(VoipHeadsetRefT *pHeadset)
{
    int32_t iRetCode = 0; // default to success

    try
    {
        pHeadset->refChatSession->TranscribedTextReadyEvent -= pHeadset->ChatParticipantTranscribedTextReadyToken;
        NetPrintf(("voipheadsetxboxone: successfully unregistered the transcribed text ready handler\n"));
    }
    catch (Exception ^ e)
    {
        NetPrintf(("voipheadsetxboxone: exception caught while trying to unregister for transcribed text ready events (err=%s: %S)\n", DirtyErrGetName(e->HResult), e->Message->Data()));
        iRetCode = -1;
    }
...
static int32_t _VoipHeadsetCreateChatSession(VoipHeadsetRefT *pHeadset)
{
    ...
    try
    {
        ...
        */
        ...
        pHeadset->refChatSession = ref new ChatSession(timeSpan, XAUDIO2_DEFAULT_PROCESSOR, Windows::Xbox::Chat::ChatFeatures::Default);
    }
    catch (Exception ^ e)
    {
    ...
    if (pHeadset->bTextTranscriptionEnabled)
    {
        pHeadset->refChatSession->TranscriptionEnabled = true;
    }
    else
    {
        pHeadset->refChatSession->TranscriptionEnabled = false;
    }
...
static int32_t _VoipHeadsetCreateChatEncoder(VoipHeadsetRefT *pHeadset, String ^refChatCaptureSourceId, IFormat ^refFormat)
{
    ...
    ChatParticipant ^refChatParticipant = nullptr;

    // need to find a valid local user to create a chat encoder
    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; ++iLocalUserIndex)
    {
        if (!VOIP_NullUser(&pHeadset->aLocalUsers[iLocalUserIndex].user) && pHeadset->aLocalUsers[iLocalUserIndex].refChatParticipant != nullptr)
        {
            // user found
            refChatParticipant = pHeadset->aLocalUsers[iLocalUserIndex].refChatParticipant;
            break;
        }
    }

    if (refChatParticipant != nullptr)
    {
        try
        {
            ChatEncoder ^refChatEncoder = pHeadset->refChatSession->CreateTranscribingEncoder(refChatParticipant, refFormat, pHeadset->EncodingQual);
    ...
    {
        NetPrintf(("voipheadsetxboxone: no valid chat participant exists, cannot create chat encoder.\n"));
        iRetCode = -1;
    }
...
static int32_t _VoipHeadsetDeleteLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, VoipUserT *pLocalUser)
{
    ...
    if (pHeadset->aLocalUsers[iLocalUserIndex].refChatParticipant == nullptr)
    {
        ...
        try
        {
            pHeadset->aLocalUsers[iLocalUserIndex].refAudioDevsVector->Clear();
            pHeadset->aLocalUsers[iLocalUserIndex].refAudioDevsVector = nullptr;
            pHeadset->aLocalUsers[iLocalUserIndex].refVoice = nullptr;
        }
        catch (Exception ^ e)
        {
...
static int32_t _VoipHeadsetAddLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, VoipUserT *pLocalUser)
{
    ...
    pHeadset->aLocalUsers[iLocalUserIndex].refVoice = Windows::Media::SpeechSynthesis::SpeechSynthesizer::DefaultVoice;
...
static int32_t _VoipHeadsetSynthesizeSpeech(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, const wchar_t *pText)
{
    ...
    if (NetConnStatus('xusr', iLocalUserIndex, &refUser, sizeof(refUser)) < 0)
    {
        NetPrintf(("voipheadsetxboxone: can't synthesize text because no local user at index %d\n", iLocalUserIndex));
        return(-1);
    }

    if (pHeadset->aLocalUsers[iLocalUserIndex].refChatParticipant == nullptr)
    {
        NetPrintf(("voipheadsetxboxone: cannot synthesize voip from local user %s at local user index %d because no a participating user\n",
            pHeadset->aLocalUsers[iLocalUserIndex].user.strUserId, iLocalUserIndex));
        return(-2);
    }
    ...
    try
    {
        Platform::String ^refMessage = ref new Platform::String(pText);

        // Spin the synthesis off into it's own task, because it makes cross-VM calls.
        Windows::Xbox::Chat::IChatParticipant2^ refChatParticipant2 = safe_cast<Windows::Xbox::Chat::IChatParticipant2^>(pHeadset->aLocalUsers[iLocalUserIndex].refChatParticipant);
        Windows::Media::SpeechSynthesis::VoiceInformation^ refVoice = pHeadset->aLocalUsers[iLocalUserIndex].refVoice;

        Concurrency::create_async([refChatParticipant2, refVoice, refMessage]()
        {
            refChatParticipant2->SynthesizeTextMessageAsAudioBuffers(refMessage, refVoice);
        });

    }
    catch (Exception ^ e)
    {
        NetPrintf(("voipheadsetxboxone: exception caught while trying to synthesize text from user %s at local user index %d (err=%s: %S)\n",
            pHeadset->aLocalUsers[iLocalUserIndex].user.strUserId, iLocalUserIndex, DirtyErrGetName(e->HResult), e->Message->Data()));
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetDisplayTranscribedText

    \Description
        Text-to-speech for the specified user.

    \Input *pHeadset        - headset module
    \Input *pOriginatorName - originator's name
    \Input *pText           - text to by displayed in system ui
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetDisplayTranscribedText(VoipHeadsetRefT *pHeadset, const wchar_t *pOriginatorName, const wchar_t *pText)
{
    try
    {
        Platform::String ^refOriginatorName = ref new Platform::String(pOriginatorName);
        Platform::String ^refMessage = ref new Platform::String(pText);
        Windows::Xbox::UI::Accessibility::SendSpeechToTextString(refOriginatorName, refMessage, Windows::Xbox::UI::SpeechToTextType::Voice);
    }
    catch (Exception ^ e)
    {
        NetPrintf(("voipheadsetxboxone: exception caught while trying to send transcribed text from user %S (err=%s: %S)\n",
            pOriginatorName, DirtyErrGetName(e->HResult), e->Message->Data()));
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetConfigureSynthesizedSpeech

    \Description
        Configure voice to use when synthesizing speech.

    \Input *pHeadset        - headset module
    \Input iLocalUserIndex  - local user index (text originator)
    \Input *pConfig         - config for text synthesis

    \Version 05/25/2017 (mclouatre)
*/
/********************************************************************************F*/
static void _VoipHeadsetConfigureSynthesizedSpeech(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, const XoneSynthesizedSpeechCfgT *pConfig)
{
    pHeadset->aLocalUsers[iLocalUserIndex].refVoice = Windows::Media::SpeechSynthesis::SpeechSynthesizer::DefaultVoice;
    ...
    if (pConfig != NULL)
    {
        if (pHeadset->hModule)
        {
            /* we cast the result of GetProcAddress to void * which removes the FARPROC. the FARPROC isn't actually being returned
               from this fuction we are getting the address of the function that matches this signature.
               this is cleaner then having to go through the hassle of disabling the warning */
            HRESULT(*GetMatchingVoiceProfile)(PCWSTR, PCWSTR, Windows::Media::SpeechSynthesis::VoiceGender, PCWSTR, HSTRING*) =
                (HRESULT(__cdecl *)(PCWSTR, PCWSTR, Windows::Media::SpeechSynthesis::VoiceGender, PCWSTR, HSTRING*)) (void *)GetProcAddress(pHeadset->hModule, "XnuGetMatchingVoiceProfile");

            if (GetMatchingVoiceProfile != nullptr)
            {
                HRESULT hResult;
                Microsoft::WRL::Wrappers::HString hMatchingId;
                // GetMatchingVoiceProfile finds the ID of the closest matching installed voice, with this order of precedence (high to low)
                // Id, Display Name, Language, Gender.
                hResult = GetMatchingVoiceProfile(pConfig->wstrPersonaId,
                                                  pConfig->wstrPersonaDisplayName,
                                                  static_cast<Windows::Media::SpeechSynthesis::VoiceGender>(pConfig->iPersonaGender),
                                                  pConfig->wstrPersonaLang, hMatchingId.GetAddressOf());
                if (SUCCEEDED(hResult))
                {
                    // Find the object associated with the matchingId
                    Windows::Foundation::Collections::IVectorView<Windows::Media::SpeechSynthesis::VoiceInformation^> ^refInstalledVoices = Windows::Media::SpeechSynthesis::SpeechSynthesizer::AllVoices;
                    for (uint32 uVoiceIndex = 0; uVoiceIndex < refInstalledVoices->Size; uVoiceIndex++)
                    {
                        Windows::Media::SpeechSynthesis::VoiceInformation ^refVoice = refInstalledVoices->GetAt(uVoiceIndex);
                        if (0 == _wcsicmp(refVoice->Id->Data(), hMatchingId.GetRawBuffer(nullptr)))
                        {
                            pHeadset->aLocalUsers[iLocalUserIndex].refVoice = refVoice;
                            break;
                        }
                    }
                }
                ...
                {
                    NetPrintf(("voipheadsetxboxone: failed to get matching voice profile\n"));
                }
            }
            else
            {
                NetPrintf(("voipheadsetxboxone: failed to load XnuGetMatchingVoiceProfile()\n"));
            }
        }
    }

    NetPrintf(("voipheadsetxboxone: local user at index %d is now using %s voice (%S) for synthesized speech\n", iLocalUserIndex,
        ((pConfig == NULL) ? "default" : "personalized"),
        pHeadset->aLocalUsers[iLocalUserIndex].refVoice->Description->Data()));
}
...
static void _VoipHeadsetPollForVoiceData(VoipHeadsetRefT *pHeadset)
{
    ...
    for (uSourceIndex = 0; uSourceIndex < pHeadset->refCurrentChatSessionState->CaptureSources->Size; uSourceIndex++)
    {
        ...
        IChatCaptureSource2^ refChatCaptureSource2 = safe_cast<Windows::Xbox::Chat::IChatCaptureSource2^>(refChatCaptureSource);
        IAudioDeviceInfo ^refAudioDeviceInfo = nullptr;
        ...
        *pWrite++  = (uint8_t)uConsoleId;
        ...
        Windows::Xbox::Chat::ChatBufferSource refBufferSource = Windows::Xbox::Chat::ChatBufferSource::Microphone;
        eCaptureBufferStatus = refChatCaptureSource2->GetNextBufferWithType(&refBuffer, &refBufferSource);
        ...
        if (refEncodedBuffer != nullptr && refEncodedBuffer->Length > 0)
        {
...
static void _VoipHeadsetProcessInboundQueues(VoipHeadsetRefT *pHeadset)
{
            ...
            else
            {
                ...
                if ((bPreBuffering && (refInboundQueue->Size > VOIPHEADSET_INBOUND_QUEUE_LOW_WATERMARK)) || !bPreBuffering)
                {
                    ...
                    for (uRenderTargetIndex = 0; uRenderTargetIndex < pHeadset->refCurrentChatSessionState->RenderTargets->Size; uRenderTargetIndex++)
                    {
                        ...
                        if (pHeadset->bMicFocus == TRUE)
                        {
                            // only submit buffer from inbound queue for mixing if voice playback is enabled for our capture source 
    ...
    #if DIRTYCODE_LOGGING
    uint64_t uCurrentTick;
    int64_t iTickDiff;
    uCurrentTick = NetTickUsec();
    iTickDiff = NetTickDiff(uCurrentTick, pHeadset->uLastTick);
    if ((iTickDiff > 20500) && (pHeadset->uLastTick != 0))
    {
        NetPrintfVerbose((pHeadset->iDebugLevel, 1, "voipheadsetxboxone: voice submission is not fast enough (%d us since last attempt)\n", iTickDiff));
    }
...
static int32_t _VoipHeadsetSetRemoteUserVoicePlayback(VoipHeadsetRefT *pHeadset, VoipUserT *pRemoteUser, int32_t iLocalUserIndex, uint8_t bEnablePlayback)
{
    ...
    for (uParticipantIndex = 0; uParticipantIndex < refParticipantImmutableVector->Size; ++uParticipantIndex)
    {
        ...
        if (VOIP_SameUser(&voipUser, pRemoteUser))
        {
            ...
            {
                refRemoteUser->EnablePlaybackFlag &= ~(1 << iLocalUserIndex);
                NetPrintf(("voipheadsetxboxone: voice playback disabled for remote user %s at local index %d, uEnablePlaybackFlag bit field 0x%08X\n", voipUser.strUserId, iLocalUserIndex, refRemoteUser->EnablePlaybackFlag));                           
            }
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetFriendQuery

    \Description
        Helper function to check if the remote user is a friend of local users

    \Input *pHeadset            - pointer to headset state
    \Input *refRemoteUser       - remote user ref

    \Version 10/26/2015 (tcho)
*/
/********************************************************************************F*/
static void _VoipHeadsetFriendQuery(VoipHeadsetRefT *pHeadset, RemoteUser^ refRemoteUser)
{
    int32_t iLocalUserIndex;
    uint64_t uXuid =  _wcstoui64(refRemoteUser->XboxUserId->Data(), NULL, 10);

    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; ++iLocalUserIndex)
    {
        if (!VOIP_NullUser(&pHeadset->aLocalUsers[iLocalUserIndex].user))
        {
            DirtyUserT remoteUser;            
            DirtyUserFromNativeUser(&remoteUser, &uXuid);
            pHeadset->iPendingFrdCheck++;
            
            if(UserListApiIsFriendAsync(pHeadset->pUserListApiRef, iLocalUserIndex, &remoteUser, &VoipHeadsetIsFriendCallback, (void *)pHeadset) < 0)
            {
                // friend query has failed
                pHeadset->iPendingFrdCheck--;
            }
        }
    }
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetReputationQuery

    \Description
        Helper function to query for the OverallRepIsBad statistic
...
*/
/********************************************************************************F*/
static void _VoipHeadsetReputationQuery(VoipHeadsetRefT *pHeadset, RemoteUser^ refRemoteUser)
{
    int32_t iLocalUserIndex;
    
    // flag bad rep check as not completed
    pHeadset->bAutoMuteCheckCompleted = FALSE;

    // need to find a valid local user to create the xbox live context
    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; ++iLocalUserIndex)
    {
        if (!VOIP_NullUser(&pHeadset->aLocalUsers[iLocalUserIndex].user) && pHeadset->aLocalUsers[iLocalUserIndex].refChatParticipant != nullptr)
        {
    ...
    // if we can't create the context assume good rep
    if (iLocalUserIndex != VOIP_MAXLOCALUSERS)
    {
        try
        {
            // create the context
            Microsoft::Xbox::Services::XboxLiveContext^ refXboxLiveContext = ref new Microsoft::Xbox::Services::XboxLiveContext(static_cast<Windows::Xbox::System::User^>(pHeadset->aLocalUsers[iLocalUserIndex].refChatParticipant->User));

#if DIRTYCODE_LOGGING
            refXboxLiveContext->Settings->DiagnosticsTraceLevel = Microsoft::Xbox::Services::XboxServicesDiagnosticsTraceLevel::Verbose;
            refXboxLiveContext->Settings->EnableServiceCallRoutedEvents = true;
            refXboxLiveContext->Settings->ServiceCallRouted += ref new EventHandler<Microsoft::Xbox::Services::XboxServiceCallRoutedEventArgs^>(
                [=](Platform::Object^, Microsoft::Xbox::Services::XboxServiceCallRoutedEventArgs^ args)
            {
                NetPrintf(("voipheadsetxboxone: [XboxLiveServices] %S %S\n", args->HttpMethod->Data(), args->Url->AbsoluteUri->Data()));
                NetPrintf(("voipheadsetxboxone: [XboxLiveServices] %S\n", args->RequestHeaders->Data()));
                NetPrintf(("voipheadsetxboxone: [XboxLiveServices] %S\n", args->RequestBody->RequestMessageString->Data())); 
                NetPrintf(("voipheadsetxboxone: [XboxLiveServices] Response Status: %S\n", args->HttpStatus.ToString()->Data()));
                NetPrintf(("voipheadsetxboxone: [XboxLiveServices] %S\n", args->ResponseHeaders->Data()));
                NetPrintf(("voipheadsetxboxone: [XboxLiveServices] %S\n", args->ResponseBody->Data()));
            });
#endif
            refRemoteUser->BadReqOp = refXboxLiveContext->UserStatisticsService->GetSingleUserStatisticsAsync (refRemoteUser->XboxUserId, VOIPHEADSET_STATISTIC_SERVICE_SCID, "OverallReputationIsBad");
            refRemoteUser->BadRepTimer = NetTick();
            pHeadset->iPendingBadRepCheck++;
        }
        catch (Exception ^e)
        {
            NetPrintf(("voipheadset: (Error) exception thrown when performing bad rep check GetSingleUserStatisticsAsync() (%S/0x%08x)\n", e->Message->Data(), e->HResult));
            refRemoteUser->IsBadRepMute = FALSE;
        }
        
        refRemoteUser->BadReqOp->Completed = ref new AsyncOperationCompletedHandler<UserStatistics::UserStatisticsResult^>
            ([pHeadset,refRemoteUser](IAsyncOperation<UserStatistics::UserStatisticsResult^>^ refAsyncOp, Windows::Foundation::AsyncStatus status)
        {
            UserStatistics::UserStatisticsResult^ refUserStatResult = nullptr;

            if (pHeadset->bDestroy != TRUE)
            {
                try
                {
                    refUserStatResult = refAsyncOp->GetResults();
                    IVectorView<UserStatistics::ServiceConfigurationStatistic^>^ refServiceConfigStats =  refUserStatResult->ServiceConfigurationStatistics;
                
                    // loop through the returned stats for OverallReputationIsBad
                    for each (Microsoft::Xbox::Services::UserStatistics::ServiceConfigurationStatistic^ refServiceConfigStat in refServiceConfigStats)
                    {
                        IVectorView<Microsoft::Xbox::Services::UserStatistics::Statistic^ >^ refStats = refServiceConfigStat->Statistics;
                        for each (Microsoft::Xbox::Services::UserStatistics::Statistic^ refStat in refStats)
                        {
                            if ((wcscmp(refStat->StatisticName->Data(), L"OverallReputationIsBad") == 0) && (wcscmp(refStat->Value->Data(), L"1") == 0))
                            {
                                // we have a bed rep user mute by default
                                refRemoteUser->IsBadRepMute = TRUE;
                            }
                            else
                            {
                                // we don't have a bed rep user don't mute
                                refRemoteUser->IsBadRepMute = FALSE;
                            }
                        }
                    }
                ...
                {
                    NetPrintf(("voipheadset: (Error) exception thrown when performing bad rep check (%S/0x%08x)\n", e->Message->Data(), e->HResult));
                    refRemoteUser->IsBadRepMute = FALSE;
                }

                // if this user have bad rep than check if he is a friend
                if (refRemoteUser->IsBadRepMute)
                {
                    _VoipHeadsetFriendQuery(pHeadset, refRemoteUser);
                }
            }

            pHeadset->iCompletedBadRepCheck++;
            refRemoteUser->BadReqOp = nullptr;
            refRemoteUser->BadRepTimer = 0;
        });
    }
}
/*F*************************************************************************************************/
/*!
    \Function _VoipHeadsetAutoMuteCompleteCheck

    \Description
        Check to see if the Auto Mute check has been completed

    \Input *pHeadset            - pointer to headset state

    \Version  04/18/2013 (tcho)
*/
/*************************************************************************************************F*/
static void _VoipHeadsetAutoMuteCompleteCheck(VoipHeadsetRefT *pHeadset)
{
    // check if auto mute checks has completed or not
    if ((pHeadset->bAutoMuteCheckCompleted != TRUE) && (pHeadset->bAutoMuteBadRep == TRUE))
    {
        // if we have BadRep op in progress check for the timeout
        if ((pHeadset->iPendingBadRepCheck != 0) && (pHeadset->iCompletedBadRepCheck < pHeadset->iPendingBadRepCheck))
        {
            ...
            for (uParticipantIndex = 0; uParticipantIndex < refParticipantImmutableVector->Size; ++uParticipantIndex)
            {
                ...
                // if we have a remote user and the bad rep op is not done check for timeout
                if ((refRemoteUser != nullptr) && (refRemoteUser->BadReqOp != nullptr))
                {
                    if (NetTickDiff(uCurrentTime, refRemoteUser->BadRepTimer) > VOIPHEADSET_BADREP_CHECK_TIMEOUT)
                    {
                        refRemoteUser->BadReqOp = nullptr;
                        refRemoteUser->BadRepTimer = 0;
                        pHeadset->iPendingBadRepCheck --;
        ...
        // if we completed the bad rep check now check to see if we have completed the frd check
        if ((pHeadset->iPendingBadRepCheck != 0) && (pHeadset->iCompletedBadRepCheck == pHeadset->iPendingBadRepCheck))
        {
            if (pHeadset->iCompletedFrdCheck == pHeadset->iPendingFrdCheck)
            {
                pHeadset->iPendingBadRepCheck = 0;
                pHeadset->iCompletedBadRepCheck = 0;
                pHeadset->iPendingFrdCheck = 0;
                pHeadset->iCompletedFrdCheck = 0;
            }
        }

        // if we don't have any pending bad rep or frd checks in progress we are consider bad rep auto mute done
        if ((pHeadset->iPendingBadRepCheck == 0) && (pHeadset->iCompletedFrdCheck == 0))
        {
            pHeadset->bAutoMuteCheckCompleted = TRUE;
        }
    }
}
...
/*F*************************************************************************************************/
/*!
    \Function _VoipHeadsetFreeCallback
    ...
    \Input *pMem     - pointer to the VoipHeadset memory buffer
...
*/
/*************************************************************************************************F*/
static int32_t _VoipHeadsetFreeCallback(void *pMem)
{
    int32_t iMemGroup;
    void *pMemGroupUserData;
    VoipHeadsetRefT *pHeadset = (VoipHeadsetRefT *)pMem;

    // do we have pending bad rep check
    if (pHeadset->iPendingBadRepCheck != 0)
    {
        return(-1);
    }

    // cancel friend query
    if (pHeadset->iPendingFrdCheck != 0)
    {
       UserListApiCancelAll(pHeadset->pUserListApiRef);
    }

    if (pHeadset->hModule)
    {
        FreeLibrary(pHeadset->hModule);
        pHeadset->hModule = 0;
    }

    _VoipHeadsetUnregisterKinectAvailabilityChangedHandler(pHeadset);

    _VoipHeadsetUnregisterParticipantTranscribedTextReadyHandler(pHeadset);
    ...
    if (pHeadset->pUserListApiRef)
    {
        UserListApiDestroy(pHeadset->pUserListApiRef);
    }
...
VoipHeadsetRefT *VoipHeadsetCreate(int32_t iMaxChannels, VoipHeadsetMicDataCbT *pMicDataCb, VoipHeadsetTextDataCbT *pTextDataCb, VoipHeadsetStatusCbT *pStatusCb, void *pCbUserData, int32_t iData)
{
    ...
    pHeadset->bAutoMuteBadRep = TRUE;

    if ((pHeadset->pUserListApiRef = UserListApiCreate(VOIPHEADSET_USERLISTAPI_RESPONSE_SIZE)) == NULL)
    {
        NetPrintf(("voipheadsetxboxone: failed to create userlistapi\n"));
    ...
    if ((iResult = _VoipHeadsetRegisterParticipantTranscribedTextReadyHandler(pHeadset)) != 0)
    {
        NetPrintf(("voipheadsetxboxone: failed to register participant transcribed text ready handler\n"));
        VoipHeadsetDestroy(pHeadset);
        return(NULL);
    }
    ...
    pHeadset->hModule = LoadLibraryEx(L"C:\\windows\\system32\\Chat.dll", NULL, 0);
...
{
    pHeadset->bDestroy = TRUE;

    if (_VoipHeadsetFreeCallback((void *)pHeadset) < 0)
    {
        NetPrintf(("voipheadsetxboxone: [%p] destroy deferred to netconn due to pending async operation.\n", pHeadset));
        NetConnControl('recu', 0, 0, (void *)_VoipHeadsetFreeCallback, pHeadset);
    }
}
...
void VoipHeadsetRegisterUserCb(VoipUserT *pRemoteUser, uint32_t bRegister, void *pUserData)
{
    ...
    if (ds_stristr(pRemoteUser->strUserId, "shared") != NULL)
    {
        ...
        for (iRemoteSharedUserIndex = 0; iRemoteSharedUserIndex < VOIP_MAX_LOW_LEVEL_CONNS; ++iRemoteSharedUserIndex)
        {
            ...
            else
            {
                ...
                {
                    ds_memclr(&pHeadset->aRemoteSharedUsers[iRemoteSharedUserIndex], sizeof(pHeadset->aRemoteSharedUsers[iRemoteSharedUserIndex]));
    ...
    else
    {
        ...
        if (bRegister)
        {
            ...
            if (pHeadset->bAutoMuteBadRep)
            {
                // begin bad rep auto mute check
                _VoipHeadsetReputationQuery(pHeadset, refRemoteUser);
            }
        }
        else
        {
...
void VoipHeadsetProcess(VoipHeadsetRefT *pHeadset, uint32_t uFrameCount)
{
    ...
    // check for Auto Mute completion
    _VoipHeadsetAutoMuteCompleteCheck(pHeadset);

    // give life to UserListApi module
    UserListApiUpdate(pHeadset->pUserListApiRef);
}
...
/*!
    ...
    \Notes
        ...
        \verbatim
            'ambr' - enable or disable auto muting for bad rep users (default is enabled)
            ...
            'stot' - speech-to-text: enable local generation of transcribed text to be sent reliably to all remote peers that requested it.
            ...
            'voic' - platform-specific params used for identifying "voice" used when creating synthesized speech
        \endverbatim
...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    if (iControl == 'ambr')
    {
        pHeadset->bAutoMuteBadRep =  (uint8_t)(iValue);
        pHeadset->bAutoMuteCheckCompleted = FALSE;
    }
    ...
    if (iControl == 'stot')
    {
        if (iValue != pHeadset->bTextTranscriptionEnabled)
        {
            pHeadset->bTextTranscriptionEnabled = iValue;
            NetPrintf(("voipheadsetxboxone: %s text transcription locally\n", pHeadset->bTextTranscriptionEnabled ? "enabling" : "disabling"));

            if (pHeadset->refChatSession != nullptr)
            {
                if (pHeadset->bTextTranscriptionEnabled)
                {
                    pHeadset->refChatSession->TranscriptionEnabled = true;
    ...
    if (iControl == 'ttos')
    {
        return(_VoipHeadsetSynthesizeSpeech(pHeadset, iValue, (const wchar_t *)pValue));
    }
    if (iControl == 'voic')
    {
        const XoneSynthesizedSpeechCfgT *pXoneSynthesizedSpeechCfg = (const XoneSynthesizedSpeechCfgT *)pValue;
        _VoipHeadsetConfigureSynthesizedSpeech(pHeadset, iValue, pXoneSynthesizedSpeechCfg);
        return(0);
    }
    return(-1);
}
...
/*!
    ...
    \Notes
        ...
        \verbatim
            'ambr' - TRUE if auto mute bad reputation check is enabled
            'amcc' - TRUE if auto mute bad reputation check is completed
            'amuc' - TRUE if the given user should be muted because of bad reputation
            ...
            'sact' - TRUE if the shared device (kinect) is active
...
int32_t VoipHeadsetStatus(VoipHeadsetRefT *pHeadset, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    if (iSelect == 'ambr')
    {
        return(pHeadset->bAutoMuteBadRep);
    }
    if (iSelect == 'amcc')
    {
        return(pHeadset->bAutoMuteCheckCompleted);
    }
    if (iSelect == 'amuc')
    {
        uint32_t uParticipantIndex = 0;
        ...
        for (uParticipantIndex = 0; uParticipantIndex < refParticipantImmutableVector->Size; ++uParticipantIndex)
        {
            ...
            if (refRemoteUser != nullptr)
            {
               ...
               {
                   // we only want to mute if this remote user is not a friend of any local users
                   return ((!refRemoteUser->IsFriend) && (refRemoteUser->IsBadRepMute));
               }
            }
        }
    ...
    if (iSelect == 'sact')
    {
        return(pHeadset->bIsKinectAvailable && !pHeadset->bHeadsetFound);
    }
    if (iSelect == 'supd')
    {
...
/*F********************************************************************************/
/*!
    \Function VoipHeadsetIsFriendCallback

    \Description
        Set speaker output callback.

    \Input *pRef                - user list ref
    \Input eResponseType        - user list response type
    \Input *pUserApiEventData   - user list event data
    \Input *pUserData           - customer user data (VoipHeadsetRef)
...
{
    VoipHeadsetRefT *pHeadset = (VoipHeadsetRefT *)pUserData;
    RemoteUser ^refRemoteUser = nullptr;
    uint64_t uRemoteXuid;

    DirtyUserToNativeUser(&uRemoteXuid, sizeof(uRemoteXuid), &pUserApiEventData->DirtyUser);
    
    if (eResponseType == USERLISTAPI_IS_FRIENDS)
    {  
        uint32_t uParticipantIndex = 0;

        // search for the remote user
        IVectorView<IChatChannel ^> ^refChannelImmutableVector;
        refChannelImmutableVector = pHeadset->refChatSession->Channels->GetView();
        ...
        {
            IChatParticipant ^refParticipant = refParticipantImmutableVector->GetAt(uParticipantIndex); 
            uint64_t uXuid = _wcstoui64(refParticipant->User->XboxUserId->Data(), NULL, 10);
            refRemoteUser = dynamic_cast<RemoteUser ^>(refParticipant->User); 

            if (refRemoteUser != nullptr)
            {
                // we have found the remote user if the xuid match
                if (uRemoteXuid == uXuid)
                {
                    if ((pUserApiEventData->bIsFavorite) || (pUserApiEventData->bIsMutualFriend))
                    {
                        refRemoteUser->IsFriend |= TRUE;
                    }
                    else
                    {
                        refRemoteUser->IsFriend |= FALSE;
                    }
                    
                    pHeadset->iCompletedFrdCheck++;
                    break;
                }
            }
        }
...
/*!
    ...
    \Description
        Display transcribed text in XboxOne UI overlay.
    ...
    \Notes
        $$todo The originator received as an input here matches the "chat participant" that the
        system passed to the TranscribedTextReady handler on the originating console. It is not 
        clear right now (needs to be discussed with MS) if there is any value in finding out
        whether that originator was using a shared mic (kinect) to flag the text as "shared
        transcribed text" when locally displaying it (because the transcribed  text
        could be from another user on the originating console).
...
int32_t VoipHeadsetDisplayTranscribedText(VoipHeadsetRefT *pHeadset, VoipUserT *pOriginator, const wchar_t *pText)
{
    ...
    for (uParticipantIndex = 0; uParticipantIndex < refParticipantImmutableVector->Size; ++uParticipantIndex)
    {
        ...
        if (refRemoteUser != nullptr)
        {
            // check to see if the xuids match
            uXuid = _wcstoui64(refParticipant->User->XboxUserId->Data(), NULL, 10);
            ds_memclr(&voipUser, sizeof(voipUser));
            ds_snzprintf(voipUser.strUserId, (int32_t)sizeof(voipUser.strUserId), "%llX", uXuid);

            if (VOIP_SameUser(&voipUser, pOriginator))
            {
                // June 2017 version of XR-046 states: "Game titles that use Xbox Live must use either the GameDisplayName or Gamertag function to display the user’s name."
                // We used gamertag because it has a known 15 char + null termination size limit.  (fits in in serialized user object)

                wcscpy_s(wstrOriginatorName, sizeof(wstrOriginatorName) / sizeof(wchar_t), refRemoteUser->Gamertag->Data());
                bFound = TRUE;
            }
        }
    }
    ...
    if (bFound)
    {
        iResult = _VoipHeadsetDisplayTranscribedText(pHeadset, wstrOriginatorName, (const wchar_t *)pText);
    }
