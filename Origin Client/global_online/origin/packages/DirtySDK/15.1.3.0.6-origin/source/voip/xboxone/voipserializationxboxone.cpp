...
/*!
    \relates     RemoteUser
    \Function    Gamertag::get

    \Description
        Getter for Gamertag property

    \Output
        String ^    - Gamertag

    \Version 16/06/2017 (mclouatre)
*/
/********************************************************************************F*/
String ^RemoteUser::Gamertag::get()
{
    return(m_Gamertag);
}
...
/*!
    \relates     RemoteUser
    \Function    Gamertag::set

    \Description
        Setter for Gamertag property

    \Input ^refGamertag   - Gamertag

    \Version 16/06/2017 (mclouatre)
*/
/********************************************************************************F*/
void RemoteUser::Gamertag::set(String ^refGamertag)
{
    m_Gamertag = refGamertag;
}
...
{
    ds_memclr(pSerializedDevice, sizeof(VoipSerializedDeviceT));
...
/*!
    ...
    \Input *pDeviceId               - [out] to be filled with numerical device id associated with device
...
int32_t VoipSerializeUser(IUser ^refLocalUser, VoipUserExT *pRemoteUser)
{
    ...
    *pWrite++ = (uint8_t)refLocalUser->IsGuest;
    ...
    // serialize the XboxUserId property
    uXuid = _wcstoui64(refLocalUser->XboxUserId->Data(), NULL, 10);
    *pWrite++ = (uint8_t)(uXuid >> 56);
    *pWrite++ = (uint8_t)(uXuid >> 48);
    *pWrite++ = (uint8_t)(uXuid >> 40);
    *pWrite++ = (uint8_t)(uXuid >> 32);
    *pWrite++ = (uint8_t)(uXuid >> 24);
    *pWrite++ = (uint8_t)(uXuid >> 16);
    *pWrite++ = (uint8_t)(uXuid >> 8);
    *pWrite++ = (uint8_t)uXuid;

    // serialize the gamertag (known to be max 15 ascii chars + null termination)
    iCharCount = (int32_t)wcstombs((char *)pWrite, refLocalUser->DisplayInfo->Gamertag->Data(), (sizeof(pRemoteUser->aSerializedUserBlob) - (pWrite - &pRemoteUser->aSerializedUserBlob[0])));
    if (iCharCount > 15)
    {
        NetPrintf(("voipserializationxboxone: warning! gamertag serialization is larger than expected (%d vs %d) - clamping\n", iCharCount, 15));
    }
    pWrite += iCharCount;
    ...
    *pCaptureSourceCount = 0;
    ...
    for (uIndex = 0; uIndex < refLocalUser->AudioDevices->Size; uIndex++)
    {
        ...
        {
            NetPrintf(("voipserializationxboxone: not enough space to serialize device #%d\n", uIndex));
            continue;
        }
...
*/
/********************************************************************************F*/
void VoipDeserializeUser(uint8_t *pRemoteUser, RemoteUser ^refRemoteUser)
{
    int32_t iIndex, iCount;
    uint32_t uConsoleId;
    uint64_t uXuid;
    wchar_t wstrString[64];
    ...
    uXuid = (uint64_t)(*pRead) << 56;
    pRead++;
    uXuid |= (uint64_t)(*pRead) << 48;
    pRead++;
    uXuid |= (uint64_t)(*pRead) << 40;
    pRead++;
    uXuid |= (uint64_t)(*pRead) << 32;
    pRead++;
    uXuid |= (uint64_t)(*pRead) << 24;
    pRead++;
    uXuid |= (uint64_t)(*pRead) << 16;
    pRead++;
    uXuid |= (uint64_t)(*pRead) << 8;
    pRead++;
    uXuid |= *pRead;
    pRead++;
    swprintf_s(wstrString, L"%llu", uXuid);
    refRemoteUser->XboxUserId = ref new String(wstrString);

    // de-serialize gamertag property
    mbstowcs(wstrString, (char *)pRead, sizeof(wstrString) - 1);
    refRemoteUser->Gamertag = ref new String(wstrString);
...
*/
/********************************************************************************F*/
void VoipDumpUser(RemoteUser ^refUser)
{
    uint32_t uIndex;
    ...
    if (refUser->Gamertag != nullptr)
    {
        NetPrintf(("  Gamertag = %S\n", refUser->Gamertag->Data()));
    }
    else
    {
        NetPrintf(("  Gamertag = nullptr\n"));
    }
