...
#define VOIP_MAXPINGPKTSIZE         (SOCKET_MAXUDPRECV - sizeof(VoipPacketHeadT) - 4 /* aRemoteClientId */ - (VOIP_MAXLOCALUSERS * sizeof(VoipPacketChanT)) /* aChannels */)
...
#define VOIP_RELIABLE_TYPE_TEXT             (3)     //!< reliable data contains transcribed text (Speech-to-Text) that needs to be sent relibably
...
typedef struct VoipConnPacketT
{
    ...
    VoipUserExT     LocalUsers[VOIP_MAXLOCALUSERS_EXTENDED];
#else
...
typedef struct VoipPingPacketT
{
    VoipPacketHeadT Head;
    uint8_t         aRemoteClientId[4];
    VoipPacketChanT aChannels[VOIP_MAXLOCALUSERS_EXTENDED];
    uint8_t         aData[VOIP_MAXPINGPKTSIZE];   //!< ping packet data (this is where piggy-backed reliable data is added when needed)
} VoipPingPacketT;
...
{
    uint8_t aRemoteClientId[4]; //!< remote peer to which this dat entry is for (required for 1x send to voipserver)
...
//! maximum size of a single reliable data entry (in bytes) is what can fit in a PING packet (taking into account that thery can be up to 32 ReliableAckT entries before it)
