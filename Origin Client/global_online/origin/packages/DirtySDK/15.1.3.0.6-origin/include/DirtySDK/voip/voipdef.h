...
{
    VOIP_CBTYPE_AMBREVENT,          //<! a bad rep auto mute event (only used on xbox one)
...
//! config data to be used with VoipControl('voic')
typedef struct XoneSynthesizedSpeechCfgT
{
    int32_t iPersonaGender;
    wchar_t wstrPersonaId[VOIPXBOXONE_ID_MAXLEN + 1];
    wchar_t wstrPersonaDisplayName[VOIPXBOXONE_PERSONA_DISPLAYNAME_MAXLEN + 1];
    wchar_t wstrPersonaLang[VOIPXBOXONE_PERSONA_LANG_MAXLEN + 1];
} XoneSynthesizedSpeechCfgT;
#endif
...
//! inbound transcribed text event callback function prototype
typedef void (VoipTranscribedTextReceivedCallbackT)(int32_t iConnId, int32_t iRemoteUserIndex, const wchar_t *pText, void *pUserData);
