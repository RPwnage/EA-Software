...
//! QOS services can provide the following tests
typedef enum QosApiRequestTypeE
{
    QOSAPI_REQUEST_LATENCY = 1,                       //!< ping for latency to server
    QOSAPI_REQUEST_BANDWIDTH = 2,                     //!< measure throughput to server
    QOSAPI_REQUEST_FIREWALL = 3                       //!< calculate firewall/NAT type with server
} QosApiRequestTypeE;   
...
DIRTYCODE_API QosApiRefT *QosApiCreate(QosApiCallbackT *pCallback, void *pUserData, uint16_t uListenPort);

// lookup QoS information for the QoS service provider
DIRTYCODE_API uint32_t QosApiServiceRequest(QosApiRefT *pQosApi, const char *pServerAddress, uint16_t uServerPort, QosApiRequestTypeE eRequestType);
