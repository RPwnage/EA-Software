...
// construct the class
DIRTYCODE_API CommUDPRef *CommUDPConstruct(int32_t maxwid, int32_t maxinp, int32_t maxout);

// destruct the class
DIRTYCODE_API void CommUDPDestroy(CommUDPRef *what);

// resolve an address
DIRTYCODE_API int32_t CommUDPResolve(CommUDPRef *what, const char *addr, char *buf, int32_t len, char iDiv);

// resolve an address
DIRTYCODE_API void CommUDPUnresolve(CommUDPRef *what);

// listen for a connection
DIRTYCODE_API int32_t CommUDPListen(CommUDPRef *what, const char *addr);

// stop listening
DIRTYCODE_API int32_t CommUDPUnlisten(CommUDPRef *what);

// initiate a connection to a peer
DIRTYCODE_API int32_t CommUDPConnect(CommUDPRef *what, const char *addr);

// terminate a connection
DIRTYCODE_API int32_t CommUDPUnconnect(CommUDPRef *what);

// set event callback hook
DIRTYCODE_API void CommUDPCallback(CommUDPRef *what, void (*callback)(void *ref, int32_t event));

// return current stream status
DIRTYCODE_API int32_t CommUDPStatus(CommUDPRef *what);
...
DIRTYCODE_API uint32_t CommUDPTick(CommUDPRef *what);

// send a packet
DIRTYCODE_API int32_t CommUDPSend(CommUDPRef *what, const void *buffer, int32_t length, uint32_t flags);

// peek at waiting packet
DIRTYCODE_API int32_t CommUDPPeek(CommUDPRef *what, void *target, int32_t length, uint32_t *when);

// receive a packet from the buffer
DIRTYCODE_API int32_t CommUDPRecv(CommUDPRef *what, void *target, int32_t length, uint32_t *when);
