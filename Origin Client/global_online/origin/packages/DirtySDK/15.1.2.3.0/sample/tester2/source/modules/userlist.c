...
static void _CmdUlaCb(UserListApiRefT *pRef, UserListApiReturnTypeE eResponseType, UserListApiEventDataT *pUserApiEventData, void *pUserData)
{
    ...
    if (eResponseType == TYPE_USER_DATA)
    {
        if (pUserApiEventData->UserData.ExtendedUserData.uUserDataMask != 0)
        {
            ...
            DirtyUserToNativeUser(&sceNpId, sizeof(SceNpId), &pUserApiEventData->UserData.DirtyUser);
            ds_strnzcpy(strTargetUserId, sceNpId.handle.data, SCE_NP_ONLINEID_MAX_LENGTH + 1);
            ZPrintf("Friend: online id: %s, gamertag %s\n", strTargetUserId, pUserApiEventData->UserData.ExtendedUserData.Profile.strGamertag);
#elif defined (DIRTYCODE_XBOXONE)
            DirtyUserToNativeUser(&uXuid, sizeof(uint64_t), &pUserApiEventData->UserData.DirtyUser);
            ZPrintf("Friend: xuid: %llu, gamertag %s\n", uXuid, pUserApiEventData->UserData.ExtendedUserData.Profile.strGamertag);
#endif
        }
        else
        {
            ZPrintf("Friend: ERROR - Failed to fetch friend profile\n");
        }
    }
...
static void _UlaFriends(void *_pApp, int32_t argc, char **argv, unsigned char bHelp)
{
    ...
    iResult = UserListApiGetListAsync(pApp->pUla, uUserIndex, USERLISTAPI_TYPE_FRIENDS, 1000, 0, NULL, &_CmdUlaCb, USERLISTAPI_MASK_PROFILE, NULL);
...
static void _UlaIsFriend(void *_pApp, int32_t argc, char **argv, unsigned char bHelp)
{
    ...
    {
#if defined(DIRTYCODE_PS4)
        ZPrintf("   usage: %s isfriend [user index] [onlineId]\n", argv[0]);
#elif defined (DIRTYCODE_XBOXONE)
        ZPrintf("   usage: %s isfriend [user index] [XUID]\n", argv[0]);
#endif
        return;
    }
    ...
    uUserIndex = (uint32_t)strtol(argv[2], NULL, 10);
        ...
        DirtyUserFromNativeUser(&User, &sceNpId);
#elif defined (DIRTYCODE_XBOXONE)
        uXuid = _strtoui64(argv[3], NULL, 10);
        DirtyUserFromNativeUser(&User, &uXuid);
#endif
...
static void _UlaIsBlocked(void *_pApp, int32_t argc, char **argv, unsigned char bHelp)
{
    ...
    {
#if defined(DIRTYCODE_PS4)
        ZPrintf("   usage: %s isblocked [user index] [onlineId]\n", argv[0]);
#elif defined (DIRTYCODE_XBOXONE)
        ZPrintf("   usage: %s isblocked [user index] [XUID]\n", argv[0]);
#endif
        return;
    }
