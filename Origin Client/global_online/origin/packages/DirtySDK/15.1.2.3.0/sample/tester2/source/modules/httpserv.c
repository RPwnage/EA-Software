...
static int32_t _HttpServReceiveCb(ProtoHttpServRequestT *pRequest, const char *pBuffer, int32_t iBufSize, void *pUserData)
{
    ...
    if (pRequest->eRequestType != PROTOHTTP_REQUESTTYPE_POST && pRequest->eRequestType != PROTOHTTP_REQUESTTYPE_PUT)
    {
        return(iBufSize);
    }
...
static void _CmdHttpServUsage(int argc, char *argv[])
{
    if (argc <= 2)
    {
        ...
        ZPrintf("httpserv:    usage: %s [chunked|filedir|listen|setcert|setkey|stop|ciph|vers|vmin]", argv[0]);
    }
    else if (argc == 3)
    {
