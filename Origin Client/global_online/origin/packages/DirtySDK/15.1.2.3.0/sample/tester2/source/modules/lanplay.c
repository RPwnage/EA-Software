    ...
    if ((iTick - pRef->iStatTick) > 1000)
    {
        ...
        if (LobbyLanStatus(pRef->pLobbyLanRef, 'stat', uPlyr, &NetStats, sizeof(NetStats)) == 0)
        {
            ...
            ZPrintf("lanplay: %2.2f k/s (%4dk) pps/rpps=%d/%d bps/rps/nps %d/%d/%d over=%d/%d pct=%d%%/%d%%\n",
                ...
                NetStats.outpps, NetStats.outrpps,
                NetStats.outbps, NetStats.outrps, NetStats.outnps,
                NetStats.outrps - NetStats.outbps, NetStats.outnps - NetStats.outbps,
                NetStats.outbps ? ((NetStats.outrps - NetStats.outbps) * 100) / NetStats.outbps : 0,
                NetStats.outbps ? ((NetStats.outnps - NetStats.outbps) * 100) / NetStats.outbps : 0);
...
static void _LanPlayRecv(LanPlayRefT *pRef, uint32_t uPlyr)
{
    ...
    // check for receive
    while (LobbyLanRecvfrom(pRef->pLobbyLanRef, (NetGamePacketT *)&Packet, uPlyr) > 0)
    {
...
static uint32_t _LanPlaySend(LanPlayRefT *pRef, uint32_t uPlyr)
{
    ...
    // send packet
    if (LobbyLanSendto(pRef->pLobbyLanRef, (NetGamePacketT *)&Packet, uPlyr) > 0)
    {
...
static void _LanPlayCreate(void *_pRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    for (iArg = 2; iArg < argc; iArg++)
    {
        ...
        {
            pRef->iMaxPacketSize = (int32_t)(atol(argv[iArg]+sizeof("maxp=")-1));
...
static void _LanPlayHost(void *_pRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    for (iArg = 2; iArg < argc; iArg++)
    {
        ...
        {
            iMaxPlayers = (int32_t)(strtol(argv[iArg] + sizeof("max=")-1, NULL, 10));
...
static void _LanPlayControl(void *_pRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    {
        ZPrintf("   usage: %s ctrl [selector] [value] [pvalue]\n", argv[0]);
        return;
    }
    ...
    LobbyLanControl(pRef->pLobbyLanRef, iCtrl, (int32_t)strtol(argv[3], NULL, 10), argv[4]);
}
