...
/*F********************************************************************************/
/*!
    \Function _PrivilegeApi_sceNpCommerceDialogParamInitialize

    \Description
        Code extracted from the SCE header file because we get a linking error in a debug build.

    \Input  *param          - the dialog params
    
    \Version 09/02/2013 (mcorcoran)
*/
/********************************************************************************F*/
static void _PrivilegeApi_sceNpCommerceDialogParamInitialize( SceNpCommerceDialogParam *param )
{
...
/*!
    ...
    \Output int32_t    - 0=no processing was done, 1=the given state was handled
...
static int32_t _PrivilegeApiUpdateParentalProgress(PrivilegeApiRefT *pRef, PrivilegeApiSceStateE *pState, int32_t iMsgDialogType, int32_t iUserIndex)
{
    ...
    if (*pState == PRIVILEGEAPI_SCE_STATE_DIALOG_PENDING)
    {
        ...
        if (status != SCE_COMMON_DIALOG_STATUS_RUNNING)
        {
            ...
            memset(&systemMsgParam, 0, sizeof(systemMsgParam));
...
static int32_t _PrivilegeApiUpdatePlusProgress(PrivilegeApiRefT *pRef, int32_t iUserIndex)
{
    ...
    if (pRef->aUserContext[iUserIndex].ePlus == PRIVILEGEAPI_SCE_STATE_DIALOG_PENDING)
    {
        ...
        if (status != SCE_COMMON_DIALOG_STATUS_RUNNING)
        {
            ...
            _PrivilegeApi_sceNpCommerceDialogParamInitialize(&param);
...
*/
/********************************************************************************F*/
int32_t PrivilegeApiCheckPrivilegesAsync(PrivilegeApiRefT *pRef, int32_t iUserIndex, const PrivilegeApiPrivT *pPrivileges, int32_t iPrivilegeCount, int32_t *pHint)
{
...
*/
/********************************************************************************F*/
int32_t PrivilegeApiCheckPrivilegesAsyncWithUi(PrivilegeApiRefT *pRef, int32_t iUserIndex, const PrivilegeApiPrivT *pPrivileges, int32_t iPrivilegeCount, int32_t *pHint, const char *pUiMessage)
{
    ...
    iRet = NetConnStatus('soid', iUserIndex, &onlineId, sizeof(onlineId));
    if (iRet != SCE_OK)
    {
        NetPrintf(("privilegeapips4: Get OnlineId for User Index: %i failed.\n", iUserIndex));
        return(-2);
    }
...
int32_t PrivilegeApiCheckResult(PrivilegeApiRefT *pRef, int32_t iRequestId)
{
    ...
    {
        bool bAborted = false;
        uint8_t bParentalControlErrorOccurred = FALSE;
            ...
            (pRef->aUserContext[iUserIndex].eContent == PRIVILEGEAPI_SCE_STATE_IN_PROGRESS))
        {
            ...
            {
                bParentalControlErrorOccurred = TRUE;
            }
        }
        ...
        iRet = _PrivilegeApiUpdateParentalProgress(pRef, &pRef->aUserContext[iUserIndex].eChat, SCE_MSG_DIALOG_SYSMSG_TYPE_TRC_PSN_CHAT_RESTRICTION, iUserIndex);

        if (iRet == 0)
        {
            iRet = _PrivilegeApiUpdateParentalProgress(pRef, &pRef->aUserContext[iUserIndex].eUgc, SCE_MSG_DIALOG_SYSMSG_TYPE_TRC_PSN_UGC_RESTRICTION, iUserIndex);
            if (iRet == 0)
            {
                iRet = _PrivilegeApiUpdatePlusProgress(pRef, iUserIndex);
                if (iRet == 2)
                {
                    bAborted = true;
                }
        ...
        if(bPlusPassErrorOccurred || bParentalControlErrorOccurred)
        {
...
int32_t PrivilegeApiReleaseRequest(PrivilegeApiRefT *pRef, int32_t iRequestId)
{
    ...
    memset(&pRef->aUserContext[iUserIndex], 0, sizeof(PrivilegeApiUserContextT));
