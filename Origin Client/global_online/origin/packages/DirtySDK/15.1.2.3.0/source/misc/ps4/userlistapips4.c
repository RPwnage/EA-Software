...
#define USERLISTAPI_PATH_BLOCKED    "blockList"
...
static int32_t _UserListApiIsQuery(UserListApiRefT *pRef, uint32_t uUserIndex, UserListApiRequestTypeE eType, UserListApiIsAQueryCallbackT *pNotifyCb, char *strTargetUserId, void *pUserData);
static int32_t _UserListApiQueuePeopleQuery(UserListApiRefT *pRef, uint32_t uUserIndex, UserListApiTypeE eType, int32_t iLimit,  int32_t iOffset, UserListApiFiltersT *filter, UserListApiCallbackT *pNotifyCb, uint32_t uTypeMask, SceNpOnlineId *onlineId, void *pUserData);
...
void _DirtyWebApiCallback(int32_t iSceError, int32_t iUserIndex, int32_t iStatusCode, const char *pRespBody, int32_t iRespBodyLength, void *pUserData)
{
    ...
    if (iSceError == SCE_OK)
    {
        ...
        {
            char *pTmp;
            NetPrintf(("userlistapips4: [0x%08x] resizing pReadBuffer from %d to %d bytes\n", (uintptr_t)pRef, pRef->iReadBufferSize, iRespBodyLength + 1));
...
int32_t _UserListApiParseResponse(UserListApiRefT *pRef, UserListRequestT *pRequest)
{
    ...
    for (int64_t iElement = 0; iElement < iSize; ++iElement)
    {
        ...
        // Extract onlineId
        pFriendList = JsonFind2(pJsonParse, NULL, (pRequest->eRequestType == (USERLISTAPI_TYPE_FRIENDS) || pRequest->eRequestType == (USERLISTAPI_TYPE_FRIENDS_OF_FRIEND)) ? "friendList[" : "blockList[", iElement);
        if (pFriendList == NULL)
        {
            memset(&data, 0, sizeof(data));
            switch (pRequest->eRequestType)
        ...
        pCurrent    = JsonFind2(pJsonParse, pFriendList, ".npId", iElement);
        JsonGetString(pCurrent, strNativeId, sizeof(strNativeId), "");

        iRes = sceNpWebApiUtilityParseNpId(strNativeId, &npId);

        if (iRes < 0)
        {
            NetPrintf(("userlistapips4: cannot parse npId.\n" ));
            memset(&data, 0, sizeof(data));
            switch (pRequest->eRequestType)
            {
                case USERLISTAPI_REQUEST_TYPE_FRIENDS:
        ...
        DirtyUserFromNativeUser(&data.UserData.DirtyUser, &npId);
        ...
        if (pRequest->uTypeMask & USERLISTAPI_MASK_PROFILE)
        {
            ...
            // Copy the gamertag from the npid
            ds_strnzcpy(pUserData->Profile.strGamertag, npId.handle.data, SCE_NP_ONLINEID_MAX_LENGTH+1);
...
int32_t _UserListApiParseIsResponse(UserListApiRefT *pRef, UserListRequestT *pRequest)
{
    ...
    // prepare the data
    memset(&data, 0, sizeof(data));
    memset(&sceNpId, 0, sizeof(SceNpId));
    ds_strnzcpy(sceNpId.handle.data, pRequest->strTargetUserId, SCE_NP_ONLINEID_MAX_LENGTH + 1);
    DirtyUserFromNativeUser(&data.DirtyUser, &sceNpId);
    ...
    if (pCurrent != NULL)
    {
        ...
        if (iCode == USERLISTAPI_USER_NOT_FOUND_ERROR_CODE)
        {
            ...
            NetPrintf(("userlistapips4: UserListApiParseIsFriendResponse() User: %s not found.\n", pRequest->strTargetUserId));
        }
        else
        {
    ...
    else
    {
        ...
        if (pRef->ActiveRequest->eRequestType == USERLISTAPI_REQUEST_TYPE_ISFRIENDS)
        {
            if (strcmp(strRelation, "friend") == 0)
            {
                ...
                NetPrintf(("userlistapips4: UserListApiParseIsFriendResponse() User: %s is a friend.\n", pRequest->strTargetUserId));
            }
            else if (strcmp(strRelation, "me") == 0)
            {
                //Target user is myself
                data.eIsaType = USERLISTAPI_IS_MYSELF;
                NetPrintf(("userlistapips4: UserListApiParseIsFriendResponse() User: %s is myself.\n", pRequest->strTargetUserId));
            }
            else
            {
                //Other relationships are not supported and is treated as not a friend
                data.eIsaType = USERLISTAPI_IS_NOT_OF_TYPE;
                NetPrintf(("userlistapips4: UserListApiParseIsFriendResponse() User: %s is not a friend.\n", pRequest->strTargetUserId));
            }
        }
        else if (pRef->ActiveRequest->eRequestType == USERLISTAPI_REQUEST_TYPE_ISBLOCKED)
        {
            if (strcmp(strRelation, "blocked") == 0)
            {
                //Target user is a friend
                data.eIsaType = USERLISTAPI_IS_OF_TYPE;
                NetPrintf(("userlistapips4: UserListApiParseIsFriendResponse() User: %s is blocked.\n", pRequest->strTargetUserId));
            }
            else if (strcmp(strRelation, "me") == 0)
            {
                //Target user is myself
            ...
            else
            {
                //Other relationships are not supported and is treated as not a friend
                data.eIsaType = USERLISTAPI_IS_NOT_OF_TYPE;
                NetPrintf(("userlistapips4: UserListApiParseIsFriendResponse() User: %s is not blocked.\n", pRequest->strTargetUserId));
            }
        }
    }
...
int32_t _UserListApiAddOrderToQuery(UserListApiRefT *pRef, UserListApiFiltersT *filter, char *pBuffer, uint32_t uBufferSize)
{
    ...
    {
        iReturn = ds_strnzcat(pBuffer, "&orderby=", uBufferSize);
        if (iReturn < 0)
        {
             return(iReturn);
        }
...
int32_t _UserListApiAddFieldsToQuery(UserListApiRefT *pRef, UserListApiFiltersT *filter, char *pBuffer, uint32_t uBufferSize, uint32_t uTypeMask, UserListApiRequestTypeE eRequestType)
{
    ...
    uint8_t bNoFields = !(filter->Fields.bOnlineId || filter->Fields.bRegion || filter->Fields.bNpId || filter->Fields.bAvatarUrl || filter->Fields.bPersonalDetail);
...
/*!
    ...
    \Input  uTypeMask   - mask
    \Input *onlineId    - pointer to user's online Id
...
*/
/*************************************************************************************************F*/
int32_t _UserListApiQueuePeopleQuery(UserListApiRefT *pRef, uint32_t uUserIndex, UserListApiTypeE eType, int32_t iLimit,  int32_t iOffset, UserListApiFiltersT *filter, UserListApiCallbackT *pNotifyCb, uint32_t uTypeMask, SceNpOnlineId *onlineId, void *pUserData)
{
    NetCritEnter(&pRef->crit);
    ...
    memset(request, 0, sizeof(UserListRequestT));
    ...
    ds_strnzcpy(request->strTargetUserId, onlineId->data, SCE_NP_ONLINEID_MAX_LENGTH+1);
    if(filter)
    {
...
/*!
    ...
    \Input  eType               - type of query
    \Input *pNotifyCb           - user callback
    \Input *strTargetUserId     - pointer to target user id (online id)
    \Input *pUserData           - pointer to user data for callback
...
*/
/*************************************************************************************************F*/
int32_t _UserListApiIsQuery(UserListApiRefT *pRef, uint32_t uUserIndex, UserListApiRequestTypeE eType, UserListApiIsAQueryCallbackT *pNotifyCb, char *strTargetUserId, void *pUserData)
{
    NetCritEnter(&pRef->crit);
    ...
    request->pUserData    = pUserData;
    request->uUserIndex   = uUserIndex;
    ds_strnzcpy(request->strTargetUserId, strTargetUserId, SCE_NP_ONLINEID_MAX_LENGTH+1);
...
int32_t _UserListApiSendPeopleQuery(UserListApiRefT *pRef)
{
    ...
    iReturn = NetConnStatus('soid', (signed)pCurrent->uUserIndex, &onlineId, sizeof(onlineId));
    if (iReturn < 0)
    {
    ...
    switch(pCurrent->eRequestType)
    {
        case USERLISTAPI_REQUEST_TYPE_BLOCKED:
        {
            iReturn = ds_snzprintf(requestPath, sizeof(requestPath), "%s/%s/%s?limit=%d&offset=%d", USERLISTAPI_PATH_BASE, onlineId.data, USERLISTAPI_PATH_BLOCKED, pCurrent->iLimit, pCurrent->iOffset);
            if (iReturn < 0)
            {
                return(iReturn);
            }
            ...
            {
                if (!(pCurrent->Filters.Fields.bNpId == FALSE && pCurrent->Filters.Fields.bAvatarUrl == FALSE && pCurrent->Filters.Fields.bOnlineId == FALSE && pCurrent->Filters.Fields.bRegion == FALSE && pCurrent->Filters.Fields.bPersonalDetail == FALSE))
                {
        ...
        case USERLISTAPI_REQUEST_TYPE_ISBLOCKED:
        {
            iReturn = ds_snzprintf(requestPath, sizeof(requestPath), "%s/%s/%s", USERLISTAPI_PATH_BASE, pCurrent->strTargetUserId, "profile?fields=@default,relation");
        ...
        default:
        {
            ...
            if (pCurrent->eRequestType == USERLISTAPI_REQUEST_TYPE_FRIENDS_OF_FRIEND)
            {
                ...
                {
                    NetPrintf(("userlistapips4: friends-of-friend query requires a specified friend name\n"));
                    return(USERLISTAPI_ERROR_BAD_PARAM);
                }

                DirtyUserToNativeUser(&FriendNpid, sizeof(SceNpId), pCurrent->Filters.pFriend);
            }

            iReturn = ds_snzprintf(requestPath, sizeof(requestPath), "%s/%s/%s?friendStatus=%s&limit=%d&offset=%d", USERLISTAPI_PATH_BASE, (pCurrent->eRequestType == USERLISTAPI_REQUEST_TYPE_FRIENDS_OF_FRIEND ? FriendNpid.handle.data : onlineId.data), USERLISTAPI_PATH_FRIENDS, USERLISTAPI_FRIEND_TYPE_FRIEND , pCurrent->iLimit, pCurrent->iOffset);
            if (iReturn < 0)
            {
                return(iReturn);
            }
...
*/
/*************************************************************************************************F*/
static void _UserListOnWebApiUpdate(int32_t userCtxId, int32_t callbackId, const char *pNpServiceName, SceNpServiceLabel npServiceLabel, const SceNpPeerAddress *pTo, const SceNpPeerAddress *pFrom, const SceNpWebApiPushEventDataType *pDataType, const char *pData, size_t dataLen, void *pUserArg)
{
    ...
    if (pUserArg != NULL)
    {
        ...
        memset(data, 0, sizeof(UserListApiNotifyDataT));

        // find the correct user whom this is for
        for (int32_t iIndex = 0; iIndex < NETCONN_MAXLOCALUSERS; ++iIndex)
        {
            SceNpOnlineId OnlineId;
            int32_t iReturn = NetConnStatus('soid', iIndex, &OnlineId, sizeof(OnlineId));

            if (iReturn >= 0 && ds_stricmp(OnlineId.data, pTo->onlineId.data) == 0)
            {
        ...
        ds_strnzcpy(npid.handle.data, pTo->onlineId.data, SCE_NP_ONLINEID_MAX_LENGTH + 1);
...
int32_t UserListApiIsFriendAsync(UserListApiRefT *pRef, uint32_t uUserIndex, DirtyUserT *pUser, UserListApiIsAQueryCallbackT *pNotifyCb, void *pUserData)
{
    ...
    memset(&sceNpId, 0, sizeof(SceNpId));
    memset(&strTargetUserId, 0, sizeof(strTargetUserId));

    if (DirtyUserToNativeUser(&sceNpId, sizeof(SceNpId), pUser))
    {
        ds_strnzcpy(strTargetUserId, sceNpId.handle.data, SCE_NP_ONLINEID_MAX_LENGTH + 1);
    }
    else
    {
        return(USERLISTAPI_ERROR_BAD_PARAM);
    ...
    iReturn = _UserListApiIsQuery(pRef, uUserIndex, USERLISTAPI_REQUEST_TYPE_ISFRIENDS , pNotifyCb, strTargetUserId, pUserData);
    return(iReturn);
}
...
int32_t UserListApiIsBlockedAsync(UserListApiRefT *pRef, uint32_t uUserIndex, DirtyUserT *pUser, UserListApiIsAQueryCallbackT *pNotifyCb, void *pUserData)
{
    ...
    iReturn = _UserListApiIsQuery(pRef, uUserIndex, USERLISTAPI_REQUEST_TYPE_ISBLOCKED , pNotifyCb, strTargetUserId, pUserData);
    return(iReturn);
}
    ...
    if (pRef->bDataReady && pRef->ActiveRequest)
    {
        if (pRef->iReadError != 0)
        {
            ...
            else if (pRef->ActiveRequest->pIsAQueryNotifyCb)
            {
                ...
                data.eIsaType = USERLISTAPI_IS_TYPE_UNKNOWN;
                ds_strnzcpy(scenpId.handle.data, pRef->ActiveRequest->strTargetUserId, SCE_NP_ONLINEID_MAX_LENGTH+1);
                DirtyUserFromNativeUser(&data.DirtyUser, &scenpId);
    ...
    if (pRef != NULL)
    {
        ...
        else 
        {
            switch (eType)
            {
                ...
                case USERLISTAPI_TYPE_BLOCKED:
                    {
                        iReturn = _UserListApiQueuePeopleQuery(pRef, uUserIndex, eType, iLimit,  iOffset, filter, pNotifyCb, uTypeMask, &onlineId, pUserData);
                    }
                    break;
