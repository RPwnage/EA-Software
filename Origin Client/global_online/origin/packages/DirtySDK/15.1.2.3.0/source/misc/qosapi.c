...
struct QosApiRefT
{
    ...
    uint32_t uPacketQueueLengthOverride;            //!< passed to SocketControl 'pque' if not set to 0, otherwise uNumProbes is used
...
static void _QosApiFirewallRequestSendProbes(QosApiRefT *pQosApi, QosApiRequestT *pRequest)
{
    ...
    for (uInterface = 0; uInterface < pRequest->data.Firewall.uNumInterfaces; uInterface++)
    {
        ...
        NetPrintfVerbose((pQosApi->iSpamValue, 2, "qosapi: request[%02d] sent fire wall probe %d to %A (result=%d)\n", pRequest->uRequestId, pRequest->data.Firewall.uProbesSent, &SendAddr, iResult));
    ...
    if (iResult == sizeof(aPacketData))
    {
        NetPrintfVerbose((pQosApi->iSpamValue, 0, "qosapi: request[%02d] sent %d firewall probes to address %A\n", pRequest->uRequestId, uInterface, &SendAddr));
        _QosApiStateTransition(pQosApi, pRequest, REQUEST_STATE_UPDATE, QOSAPI_STATFL_NONE, DIRTYAPI_QOS, QOSAPI_STATUS_PENDING, 0);
        pRequest->uTimeIssued = NetTick();
    }
    else if (iResult == 0)  // need to try again
...
static QosApiRequestT *_QosApiRequestCommon(QosApiRefT *pQosApi, QosApiRequestTypeE eType)
{
    ...
    /* By default non-virtual sockets have a 1-deep packet queue. We want that queue to be
       at least as big as the number of probes we send to avoid the socket receive thread
       not extracting packets from the socket receive buffer when _QosApiRecvCB() fails
       to enter pQosApi->ThreadCrit. If extracting packets from the socket receive buffer
       is delayed, then rtt time calculated from those probe replies ends up being
       incorrectly higher than it should be. */
    if (pQosApi->uPacketQueueLengthOverride == 0)
    {
        // todo 2 concerns with this logic 
        // 1) uNumProbes is only used for the latency test, the others are configured via the QOS server.  
        //  If BW or FW tests were to have more probes than latency, this fix wouldn't be sufficient.
        //  As of today Blaze is often configured to use 10 probes for Latency, 10 for BW, 
        //  and 2 for FW; so this concern typically wouldn't manifest itself.
        // 2) We are typically communicating with multiple ping sites, each using uNumProbes.  
        //  perhaps everything might arrive at the same time, meaning we would need 
        //  (PingSiteCount * uNumProbes) to effectively cover this.

        if (SocketInfo(pQosApi->pSocket, 'size', 0, NULL, 0) < (signed)uNumProbes)
        {
            SocketControl(pQosApi->pSocket, 'pque', uNumProbes, NULL, NULL);
        }
    }
    else
    {
        SocketControl(pQosApi->pSocket, 'pque', pQosApi->uPacketQueueLengthOverride, NULL, NULL);
    }
...
int32_t QosApiControl(QosApiRefT *pQosApi, int32_t iControl, int32_t iValue, void *pValue)
{
    ...
    {
        pQosApi->uPacketQueueLengthOverride = iValue;
    }  
...
int32_t QosApiStatus(QosApiRefT *pQosApi, int32_t iSelect, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    {
        iRet = pQosApi->uPacketQueueLengthOverride;
    }  
