...
struct VoipHeadsetRefT
{
    ...
    //! variables used for threaded operations (calls to sceVoiceStart(), sceVoicePortCreate())
    volatile ScePthread iThreadId;
    int32_t iThreadUserIndex;
    uint32_t uThreadCreatedPort;
...
/*F********************************************************************************/
/*!
    \Function   _VoipHeadsetVoiceLibStartThread

    \Description
        Ephemeral high priority thread used to perform sceVoiceStart(). 
    ...
    \Notes
    Experimentation showed that omitting to call sceVoiceStart() from a high priority
    thread resulted in crackling in sampled voice.

    From Sony Doc  (voice lib overview pdf)
    "The Voice Library is a user land library that runs on game CPUs. When it starts,
    an internal voice update thread is created automatically. In addition, a separate
    audio in device thread is created each time a device in port is created. [...] 
    only one audio out device thread is created no matter how many device out ports 
    are created. [...] All voice threads inherit the same CPU affinity as that of the
    calling thread. Good coordination of the CPU usage in a game assures that the
    Voice Library has enough CPU time to run, which will avoid any voice crackling."

    \Version 08/14/2015 (mclouatre)
*/
/********************************************************************************F*/
static void *_VoipHeadsetVoiceLibStartThread(void *pArg)
{
    VoipHeadsetRefT * pHeadset = (VoipHeadsetRefT *)pArg;

    _VoipHeadsetVoiceLibStart(pHeadset);

    // notify main thread we're done
    pHeadset->iThreadId = 0;
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetThreadedVoiceLibStart

    \Description
        Blocking operation that starts the ephemeral high priority thread used to
        perform sceVoiceStart() and then waits for its completion.
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetThreadedVoiceLibStart(VoipHeadsetRefT *pHeadset)
{
    int32_t iResult;
    ScePthreadAttr attr;

    // create worker thread required for calling voice lib functions that needs to be called from a high priority thread
    if ((iResult = scePthreadAttrInit(&attr)) == SCE_OK)
    {
        ...
        {
            NetPrintf(("voipheadsetps4: scePthreadAttrSetdetachstate failed (err=%s)\n", DirtyErrGetName(iResult)));
        }

        if ((iResult = scePthreadAttrSetaffinity(&attr, pHeadset->cpuAffinity)) != SCE_OK)
        {
            NetPrintf(("voipheadsetps4: scePthreadAttrSetaffinity failed (err=%s)\n", DirtyErrGetName(iResult)));
        }
        ...
        {
            NetPrintf(("voipheadsetps4: scePthreadAttrSetschedpolicy SCHED_RR failed (err=%s)\n", DirtyErrGetName(iResult)));
        }

        // set thread priority 
        schedParam.sched_priority = pHeadset->iSchedPriority;
        if ((iResult = scePthreadAttrSetschedparam(&attr, &schedParam)) != SCE_OK)
        {
            NetPrintf(("voipheadsetps4: scePthreadAttrSetschedparam failed (err=%s)\n", DirtyErrGetName(iResult)));
        }
        ...
        {
            NetPrintf(("voipheadsetps4: scePthreadAttrSetinheritsched SCE_PTHREAD_EXPLICIT_SCHED failed (err=%s)\n", DirtyErrGetName(iResult)));
        }

        if ((iResult = scePthreadCreate((ScePthread *)&pHeadset->iThreadId, &attr, _VoipHeadsetVoiceLibStartThread, pHeadset, "VoipHeadsetVoiceLibStartThread")) != SCE_OK)
        {
            NetPrintf(("voipheadsetps4: scePthreadCreate failed (err=%s)\n", DirtyErrGetName(iResult)));
            return(-1);
        }

        if ((iResult = scePthreadAttrDestroy(&attr)) != SCE_OK)
        {
            NetPrintf(("voipheadsetps4: scePthreadAttrDestroy failed (err=%s)\n", DirtyErrGetName(iResult)));
        }
    }
    else
    {
        NetPrintf(("voipheadsetps4: scePthreadAttrInit failed (err=%s)\n", DirtyErrGetName(iResult)));
        return(-2);
    }

    // wait for threaded operation to complete
    while (pHeadset->iThreadId != 0)
    {
        NetConnSleep(1);
    }
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetThreadedPortCreate

    \Description
        Blocking operation that starts the ephemeral high priority thread used to
        perform sceVoiceCreatePort() and then waits for its completion.

    \Input *pHeadset    - module reference
    \Input threadFunc   - thread function
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetThreadedPortCreate(VoipHeadsetRefT *pHeadset, void *threadFunc(void *pArg), int32_t iUserIndex, uint32_t *pPortId)
{
    int32_t iResult;
    ScePthreadAttr attr;

    pHeadset->iThreadUserIndex = iUserIndex;
    ...
    if ((iResult = scePthreadAttrInit(&attr)) == SCE_OK)
    {
        ...
        if ((iResult = scePthreadCreate((ScePthread *)&pHeadset->iThreadId, &attr, threadFunc, pHeadset, "VoipHeadsetPortCreateThread")) != SCE_OK)
        {
            NetPrintf(("voipheadsetps4: scePthreadCreate failed (err=%s)\n", DirtyErrGetName(iResult)));
            return(-1);
        }
    ...
    *pPortId = pHeadset->uThreadCreatedPort;
...
/*F********************************************************************************/
/*!
    \Function   _VoipHeadsetMicPortCreateThread

    \Description
        Ephemeral high priority thread used to perform sceVoiceCreatePort() for
        a IN_DEVICE port.
    ...
    \Notes
    Experimentation showed that omitting to call sceVoiceCreatePort() from a
    high priority thread resulted in crackling in sampled voice.
...
*/
/********************************************************************************F*/
static void *_VoipHeadsetMicPortCreateThread(void *pArg)
{
    VoipHeadsetRefT * pHeadset = (VoipHeadsetRefT *)pArg;

    _VoipHeadsetMicPortCreate(pHeadset->iThreadUserIndex, &pHeadset->uThreadCreatedPort);
...
/*F********************************************************************************/
/*!
    \Function   _VoipHeadsetSpkrPortCreateThread
...
*/
/********************************************************************************F*/
static void *_VoipHeadsetSpkrPortCreateThread(void *pArg)
{
    VoipHeadsetRefT * pHeadset = (VoipHeadsetRefT *)pArg;

    _VoipHeadsetSpkrPortCreate(pHeadset->iThreadUserIndex, &pHeadset->uThreadCreatedPort);
...
static int32_t _VoipHeadsetAddLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, VoipUserT *pLocalUser)
{
    ...
    uStartTick = NetTick();
    #endif
    if (_VoipHeadsetThreadedPortCreate(pHeadset, _VoipHeadsetMicPortCreateThread, iLocalUserIndex, &pHeadset->aLocalUsers[iLocalUserIndex].uMicPortId) == 0)
    {
        iRetCode = 0; // signal success
    }
    #if DIRTYCODE_LOGGING
    NetPrintfVerbose((pHeadset->iDebugLevel, 1, "voipheadsetps4: creation of mic port with id %d blocked for %d ms\n", pHeadset->aLocalUsers[iLocalUserIndex].uMicPortId, NetTickDiff(NetTick(), uStartTick)));
    #endif
...
static int32_t _VoipHeadsetActivateLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, VoipUserT *pLocalUser)
{
    ...
    {
        #if DIRTYCODE_LOGGING
        uint32_t uStartTick = NetTick();
        #endif
        iResult = _VoipHeadsetThreadedPortCreate(pHeadset, _VoipHeadsetSpkrPortCreateThread, iLocalUserIndex, &pHeadset->aLocalUsers[iLocalUserIndex].uSpkrPortId);
        #if DIRTYCODE_LOGGING
        NetPrintfVerbose((pHeadset->iDebugLevel, 1, "voipheadsetps4: creation of speaker port with id %d blocked for %d ms\n", pHeadset->aLocalUsers[iLocalUserIndex].uSpkrPortId, NetTickDiff(NetTick(), uStartTick)));
        #endif
        ...
        if (uMask & (1 << iLocalUserIndex))
        {
            ...
            if (!VOIP_NullUser(&pHeadset->aLocalUsers[iLocalUserIndex].user))
            {
                /* The shared privilege is set to be the most restrictive one from the set of "participating" users.
                   Reference:  see R4061G from PS4-TestCase_for_TRC1.5_e.pdf */
...
/*!
    ...
    \Notes
        \verbatim
            ...
            For example iAttr = (SCE_KERNEL_CPUMASK_USER_ALL << 16) | SCE_KERNEL_PRIO_FIFO_HIGHEST.
        \endverbatim

    \Version 16/01/2013 (mclouatre)
*/
/********************************************************************************F*/
VoipHeadsetRefT *VoipHeadsetCreate(int32_t iMaxChannels, VoipHeadsetMicDataCbT *pMicDataCb, VoipHeadsetStatusCbT *pStatusCb, void *pCbUserData, int32_t iAttr)
{
    VoipHeadsetRefT *pHeadset;
    int32_t iMemGroup;
    void *pMemGroupUserData;
    int32_t iSize;
    int32_t iThreadCpuAffinity;
    #if DIRTYCODE_LOGGING
    uint32_t uStartTick;
    #endif
    ...
    {
        pHeadset->cpuAffinity = iThreadCpuAffinity;
    }
    else
    {
        pHeadset->cpuAffinity = SCE_KERNEL_CPUMASK_USER_ALL;
    }
    pHeadset->iSchedPriority = SCE_KERNEL_PRIO_FIFO_HIGHEST;

    // applying custom voice lib thread attributes if specified
    if (iAttr != 0)
    {
        pHeadset->iSchedPriority = iAttr & 0xFFFF;
        pHeadset->cpuAffinity = (SceKernelCpumask)(iAttr >> 16);
    }
    ...
    _VoipHeadsetThreadedVoiceLibStart(pHeadset);
    #if DIRTYCODE_LOGGING
    NetPrintfVerbose((pHeadset->iDebugLevel, 1, "voipheadsetps4: starting the voice lib blocked for %d ms\n", NetTickDiff(NetTick(), uStartTick)));
    #endif
...
void VoipHeadsetProcess(VoipHeadsetRefT *pHeadset, uint32_t uFrameCount)
{
    ...
    VoipHeadsetStatus(pHeadset, 'hpao', 0, NULL, 0);
