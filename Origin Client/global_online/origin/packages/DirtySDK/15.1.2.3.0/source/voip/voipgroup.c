...
struct VoipGroupRefT
{
    ...
    uint32_t        uLowLevelConnectivityId[MAX_CLIENTS_PER_GROUP]; //<! low level  local connectivity id
...
static int32_t _VoipGroupMatch(VoipGroupRefT *pVoipGroup, int32_t iConnId)
{
    ...
    uClientId = pVoipGroup->connections[iConnId].uClientId;
    if (uClientId == 0)
    {
        return(VOIP_CONNID_NONE);
    }
    ...
    if (pVoipGroup->connections[iConnId].eConnState == VOIPGROUP_CONNSTATE_SUSPENDED)
    {
        for(iIndex = 0; iIndex < MAX_GROUPS; iIndex++)
        {
            ...
            {
                if ( (pManager->groups[iIndex].connections[iConn].uClientId == uClientId) &&
                     (pManager->groups[iIndex].connections[iConn].eConnState == VOIPGROUP_CONNSTATE_ACTIVE) )
                {
                    iRetVal = pManager->groups[iIndex].connections[iConn].iMappedLowLevelConnId;
                    break;
                }
            }
        }
    ...
    {
        iRetVal = pVoipGroup->connections[iConnId].iMappedLowLevelConnId;
    }
...
int32_t VoipGroupConnect2(VoipGroupRefT *pVoipGroup, int32_t iConnId, uint32_t uAddress, uint32_t uManglePort, uint32_t uGamePort, uint32_t uClientId, uint32_t uSessionId, uint8_t bIsConnectivityHosted, uint32_t uLowLevelConnectivityId)
{
    ...
    if ( (pCollidingVoipGroup = _VoipGroupFindCollision(pVoipGroup, uClientId, VOIPGROUP_CONNSTATE_ACTIVE, &iCollidingConnId)) != NULL )
    {
        ...
        if ((pCollidingVoipGroup->iCcMode == VOIPGROUP_CCMODE_HOSTEDFALLBACK) && pCollidingVoipGroup->connections[iCollidingConnId].bIsConnectivityHosted && !bIsConnectivityHosted)
        {
        ...
        if ( (pCollidingVoipGroup->bServer != pVoipGroup->bServer) ||
             ...
             pCollidingVoipGroup->connections[iCollidingConnId].bIsConnectivityHosted)
        {
            /*
            Note:
            ...
            */
            ...
            if (pCollidingVoipGroup->bServer)
            {
                ...
                pVoipGroup->connections[iConnId].uLowLevelConnectivityId = uLowLevelConnectivityId;
                pVoipGroup->connections[iConnId].uSessionId = uSessionId;
                pVoipGroup->connections[iConnId].bIsConnectivityHosted = bIsConnectivityHosted;
    ...
    VoipControl(VoipGetRef(), 'vcid', iMappedLowLevelConnId, &pVoipGroup->uLowLevelConnectivityId[iConnId]);
...
void VoipGroupResume2(VoipGroupRefT *pVoipGroup, int32_t iConnId, uint32_t uAddress, uint32_t uManglePort, uint32_t uGamePort, uint32_t uClientId, uint32_t uSessionId, uint8_t bIsConnectivityHosted)
{
    ...
    {
        // conn id is available, use it!
        iMappedLowLevelConnId = VoipConnect(VoipGetRef(), iConnId, uAddress, uManglePort, uGamePort, pVoipGroup->connections[iConnId].uLowLevelConnectivityId, uSessionId);
    }
    else
    {
        // else, let Voip allocate one.
        iMappedLowLevelConnId = VoipConnect(VoipGetRef(), VOIP_CONNID_NONE, uAddress, uManglePort, uGamePort, pVoipGroup->connections[iConnId].uLowLevelConnectivityId, uSessionId);
    }
...
void VoipGroupDisconnect(VoipGroupRefT *pVoipGroup, int32_t iConnId)
{
    ...
    pVoipGroup->connections[iConnId].uLowLevelConnectivityId = 0;
    pVoipGroup->connections[iConnId].uSessionId = 0;
    pVoipGroup->connections[iConnId].bIsConnectivityHosted = FALSE;
...
int32_t VoipGroupControl(VoipGroupRefT *pVoipGroup, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'vcid')
    {
        ...
        pVoipGroup->uLowLevelConnectivityId[iValue] = *(uint32_t*)pValue;
        return(0);
    }
    if (VoipGetRef() != NULL)
    {
