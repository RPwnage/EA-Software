    ...
    if (!pState->bServer)
    {
        ...
        for (iProtocol = 0; iProtocol < pState->uNumAlpnProtocols; iProtocol += 1)
        {
            ...
            *pBuffer++ = pProtocol->uLength;
            memcpy(pBuffer, pProtocol->strName, pProtocol->uLength);
            pBuffer += pProtocol->uLength;
        }
    }
    else
...
static uint8_t *_ProtoSSLAddHelloExtensions(ProtoSSLRefT *pState, uint8_t *pBuffer, int32_t iBufLen)
{
    ...
    if (pState->uHelloExtn & PROTOSSL_HELLOEXTN_ALPN)
    {
...
int32_t ProtoSSLControl(ProtoSSLRefT *pState, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iSelect == 'alpn')
    {
        ...
        memset(pState->aAlpnProtocols, 0, sizeof(pState->aAlpnProtocols));
