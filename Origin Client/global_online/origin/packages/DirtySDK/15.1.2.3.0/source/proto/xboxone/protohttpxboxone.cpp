...
struct ProtoHttpXmlHttpT
{
    ...
    int32_t iAbortStartTime;                                //!< when the abortion was initiated
...
static int32_t _ProtoHttpAbort(ProtoHttpRefT *pState)
{
    ...
    // remember when the abortion is initiated
    pState->critical.pXmlHttp->iAbortStartTime = NetTick();
...
static int32_t _ProtoHttpFormatRequestHeader(ProtoHttpRefT *pState, char *pStrTempBuf, int32_t iBufLen)
{
    ...
    {
        int32_t iTokenLen;
        if ((iResult = DirtyAuthCheckToken(pState->iAuthTokenIdx, pState->strTokenUrl, &iTokenLen, pStrTempBuf)) == DIRTYAUTH_SUCCESS)
        {
...
static int32_t _ProtoHttpXmlHttpDestroyCallback(void *pRef)
{
    ...
    {
        // if operation was aborted more than 5 sec ago, stop waiting
        if (pXmlHttp->bAborted && (NetTickDiff(NetTick(), pXmlHttp->iAbortStartTime) > 5 * 1000))
        {
            NetPrintf(("protohttpxboxone: [%p] operation abortion completion for xmlhttp object [%p] has not come in 5 sec... we stop waiting and fake it!\n", pXmlHttp->pState, pXmlHttp));
            pXmlHttp->cpXmlHttpCallback->OnError(NULL, E_ABORT);
        }
        else
        {
...
{
    /* 
    acquire the critical section before destroying the pXmlHttp. By acquiring this critical section, we make sure
    that we are not in a callback. If a request is active, _ProtoHttpXmlHttpDestroy call Abort() on IXMLHTTPRequest2. After that,
...
int32_t ProtoHttpStatus(ProtoHttpRefT *pState, int32_t iSelect, void *pBuffer, int32_t iBufSize)
{
    ...
    if (iSelect == 'data')
    {
        ...
        {
            iResult = pState->responseHeader.iHeadSize + pState->responseHeader.iBodySize;
            return(iResult);
        }
        return(0);
    }
...
void ProtoHttpUpdate(ProtoHttpRefT *pState)
{
    ...
    {
        if ((iResult = DirtyAuthCheckToken(pState->iAuthTokenIdx, pState->strTokenUrl, NULL, NULL)) == DIRTYAUTH_SUCCESS)
        {
