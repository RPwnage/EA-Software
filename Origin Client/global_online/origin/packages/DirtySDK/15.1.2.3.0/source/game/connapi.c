...
#define CONNAPI_DEMANGLER_WITH_FAILOVER_TIMEOUT (CONNAPI_CONNTIMEOUT_DEFAULT)    // however for scenarios involving DirtyCast failover or CC assistance, we don't wait that long
#else
...
struct ConnApiRefT
{
    ...
    //! demangler timeouts
    int32_t                 iConfigMnglTimeout;         // configured timeout for cases where dirtycast failover or CC assistance are not applicable
    int32_t                 iConfigMnglTimeoutFailover; // configured timeout for cases where dirtycast failover or CC assistance are applicable
...
/*!
    ...
    \Notes
        Really needed for Xbox One only. Ends up being a no-op on other platform.
...
static int32_t _ConnApiGetConnectAddr(ConnApiRefT *pConnApi, ConnApiClientT *pClient, uint8_t *pLocalAddr, const uint32_t uConnMode, ConnApiClientT **pClientUsedRet)
{
    ...
    {
        NetPrintf(("connapi: [0x%08x] [%s] using game server address%sto connect to (or disconnect from) client 0x%08x\n", pConnApi, pConn,
...
static void _ConnApiVoipGroupConnSharingCallback(VoipGroupRefT *pVoipGroup, ConnSharingCbTypeE eCbType, int32_t iConnId, void *pUserData, uint8_t bSending, uint8_t bReceiving)
{
    ...
    if (eCbType == VOIPGROUP_CBTYPE_CONNSUSPEND)
    {
        ...
        // de-virtualize voip port as connection was suspended
        if (pConnApi->bTunnelEnabled == TRUE)
        {
            NetConnControl('vdel', pClient->VoipInfo.uLocalPort, 0, NULL, NULL);
        }
    }
    else if (eCbType == VOIPGROUP_CBTYPE_CONNRESUME)
    {
        ...
        // virtualize voip port as connection is resuming
        if (pConnApi->bTunnelEnabled == TRUE)
        {
            NetConnControl('vadd', pClient->VoipInfo.uLocalPort, 0, NULL, NULL);
        }
...
static void _ConnApiDestroyGameConnection(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex, const char *pReason)
{
    ...
    // de-virtualize game port
    if (pConnApi->bTunnelEnabled == TRUE)
    {
        NetConnControl('vdel', pClient->GameInfo.uLocalPort, 0, NULL, NULL);
    }
...
static void _ConnApiInitClientList(ConnApiRefT *pConnApi, ConnApiClientInfoT *pClientList, int32_t iNumClients)
{
    ...
    if (pConnApi->iSelf >= 0)
    {
        ...
        if (pConnApi->GameServer.ClientInfo.uAddr == 0)
        {
            ...
            // set up local (bind) port info
            pConnApi->GameServer.GameInfo.uLocalPort = (pClient->ClientInfo.uLocalGamePort == 0) ? pConnApi->uGamePort : pClient->ClientInfo.uLocalGamePort;
            pConnApi->GameServer.VoipInfo.uLocalPort = (pClient->ClientInfo.uLocalVoipPort == 0) ? pConnApi->uVoipPort : pClient->ClientInfo.uLocalVoipPort;
            ...
            if (pConnApi->bTunnelEnabled == FALSE && pConnApi->uFallbackGamePort)
            {
...
static ConnApiConnStatusE _ConnApiCheckGameServerFallback(ConnApiRefT *pConnApi, ConnApiClientT *pClient, uint32_t uGameServConnMode, int32_t iClientIndex)
{
    ...
    {
        NetPrintf(("connapi: [%s] status=init, connection failed, using gameserver fallback for connection to client 0x%08x\n", pConn, pClient->ClientInfo.uId));
        pInfo->uPeerConnFailed |= uGameServConnMode;
        eStatus  = CONNAPI_STATUS_INIT;

        if (pConnApi->bTunnelEnabled == FALSE)
        {
        ...
        NetPrintf(("connapi: freeing previous tunnel for client 0x%08x\n", pClient->ClientInfo.uId));
        _ConnApiTunnelFree(pConnApi, pClient);
    }
    else
    {
        NetPrintf(("connapi: [0x%08x] [%s] status=disc, connection failed%sfor connection to client 0x%08x\n", pConnApi, pConn,
...
static int32_t _ConnApiUpdateGameClient(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex)
{
    ...
    if (pClient->GameInfo.eStatus == CONNAPI_STATUS_INIT)
    {
        ...
        if (pClient->pGameUtilRef != NULL)
        {
            ...
            // virtualize game port
            if (pConnApi->bTunnelEnabled == TRUE)
            {
                NetConnControl('vadd', pClient->GameInfo.uLocalPort, 0, NULL, NULL);
            }
            ...
            NetGameUtilControl(pClient->pGameUtilRef, 'meta', (pConnApi->bCommUdpMetadata || ((pConnApi->uGameServAddr != 0) && (pConnApi->eGameServMode == CONNAPI_GAMESERV_BROADCAST)) || (pClient->ClientInfo.bIsConnectivityHosted)) ? 1 : 0);
        }
        else
        {
    ...
    if (pClient->GameInfo.eStatus == CONNAPI_STATUS_CONN)
    {
        ...
        if ((pClient->GameInfo.eStatus == CONNAPI_STATUS_CONN) && (pClient->pGameLinkRef == NULL) && (NetTickDiff(NetTick(), pClient->iConnStart) > iConnTimeout))
        {
...
#if defined(DIRTYCODE_XBOXONE)
        {
            char aSecureDeviceAddressBlob[256];
            int32_t iBlobSize;
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'dtif' - set demangler timeout for scenarios involving dirtycast failover or CC assistance - iValue=timeout in milliseconds
            ...
            'fbkp' - set fallback ports  iValue=fallback game port, iValue2=fallback remote tunnel port
...
int32_t ConnApiControl(ConnApiRefT *pConnApi, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if ((iControl == 'dtim') || (iControl == 'dtif'))
    {
        ...
        // special case to allow using a different timeout value for connapi involving DirtyCast failover or CC assistance
        if (iControl == 'dtif')
        {
        ...
        // check if the special timeout value for failover scenarios is applicable
        if (pConnApi->uGameServFallback != 0 || (pConnApi->iCcMode != CONNAPI_CCMODE_PEERONLY))
        {
            #if DIRTYCODE_LOGGING
        ...
        {
            NetPrintf(("connapi: [0x%08x] applying new demangler timeout (%d ms --> %d ms) for a demangling scenario %s possibility of failover\n",
