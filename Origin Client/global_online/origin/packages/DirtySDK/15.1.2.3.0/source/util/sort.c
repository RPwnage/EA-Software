        ...
        int32_t (*compare)(void *refptr, const void *, const void *))
{
    ...
    if (nmemb < (signed)(sizeof(rtmp)/sizeof(rtmp[0]))) {
        ...
        rbuf = (int32_t *) DirtyMemAlloc(nmemb*sizeof(int32_t), SORT_MEMID, iMemGroup, pMemGroupUserData);
        mbuf[0] = (char **) DirtyMemAlloc(nmemb*sizeof(mbuf[0]), SORT_MEMID, iMemGroup, pMemGroupUserData);
        mbuf[1] = (char **) DirtyMemAlloc(nmemb*sizeof(mbuf[1]), SORT_MEMID, iMemGroup, pMemGroupUserData);
    }
    if (size <= (signed)sizeof(xtmp)) {
        ...
        xbuf = (char *) DirtyMemAlloc(size, SORT_MEMID, iMemGroup, pMemGroupUserData);
    }
    ...
    if (pass != 0) {
        ...
        msrc = mbuf[pass & 1];
        for (i = 0; i < nmemb; ++i) {
            ds_memcpy(tmp+i*size, msrc[i], size);
        }
        ds_memcpy(base, base, nmemb*size);
        ...
        for (i = 0; i < nmemb; ++i) {
            ...
            ds_memcpy(xbuf, msrc[i], size);
            // shift all other elements into place
            for (j = i;;) {
                k = (int32_t)((msrc[j]-(char *)base)/size);
                if (k == i)
                    break;
                ds_memcpy(msrc[j], msrc[k], size);
                msrc[j] = NULL;
                j = k;
            }
            ds_memcpy(msrc[j], xbuf, size);
            msrc[j] = NULL;
        }
#endif
    }
    ...
    if (rbuf != rtmp) {
        DirtyMemFree(rbuf, SORT_MEMID, iMemGroup, pMemGroupUserData);
        DirtyMemFree(mbuf[0], SORT_MEMID, iMemGroup, pMemGroupUserData);
        DirtyMemFree(mbuf[1], SORT_MEMID, iMemGroup, pMemGroupUserData);
    }
    if (xbuf != xtmp)
        DirtyMemFree(xbuf, SORT_MEMID, iMemGroup, pMemGroupUserData);
