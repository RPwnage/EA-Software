...
struct DirtySessionManagerRefT
{
    ...
    SceNpOnlineId launchSoid;
    SceNpOnlineId aPTInviteList[SCE_NP_PLAY_TOGETHER_MAX_ONLINE_ID_LIST_NUM];
};
...
static void _DirtySessionManagerOnSystemEvent(void *pUserData, const SceSystemServiceEvent *pSystemServiceEvent)
{
    if (pSystemServiceEvent && pUserData)
    {
        ...
        else if (pSystemServiceEvent->eventType == SCE_SYSTEM_SERVICE_EVENT_PLAY_TOGETHER_HOST)
        {
            SceNpPlayTogetherHostEventParam *pPlayTogetherHostParam = (SceNpPlayTogetherHostEventParam *)(pSystemServiceEvent->data.param);
            int iIndex;
            int iUserIndex;
            ...
            NetPrintf(("dirtysessionmanagerps4: [%p] detected play together event, user would like to invite %i players to the game.\n", pRef, pPlayTogetherHostParam->onlineIDListLen));

            pRef->iPTInviteListLen = pPlayTogetherHostParam->onlineIDListLen;
            NetConnStatus('soid', iUserIndex, &pRef->launchSoid, sizeof(SceNpOnlineId));

            for (iIndex = 0; iIndex < SCE_NP_PLAY_TOGETHER_MAX_ONLINE_ID_LIST_NUM; ++iIndex)
            {
                if ( iIndex < pRef->iPTInviteListLen)
                {
                    ds_memcpy_s(&pRef->aPTInviteList[iIndex], sizeof(SceNpOnlineId), &pPlayTogetherHostParam->onlineIDList[iIndex],sizeof(SceNpOnlineId));
                }
                else
                {
                    memset(&pRef->aPTInviteList[iIndex], 0, sizeof(SceNpOnlineId));
                }
            }
...
int32_t DirtySessionManagerControl(DirtySessionManagerRefT *pRef, int32_t iControl, int32_t iValue, int32_t iValue2, const void *pValue)
{
    ...
    switch (iControl)
    {
        ...
        default:
            if (pRef->bServerDrivenModeEnabled)
            {
                NetPrintf(("dirtysessionmanagerps4: [%p] DirtySessionManagerControl('%C') ignored because server-driven operational mode enabled\n", pRef, iControl));
                return(iRet);
            }
            break;
    }
    ...
    {
        NetPrintf(("dirtysessionmanagerps4: [%p] server-driven mode enabled\n", pRef));
    ...
    \Notes
        ...
        \verbatim
            ...
            'pthe' - check to see if the client has received a Play Togther Host Event (it will be reset after this call) return true or false.
                     If true the Online ID of the user that launched the game will be return in pBuf.
...
int32_t DirtySessionManagerStatus(DirtySessionManagerRefT *pRef, int32_t iSelect, void *pBuf, int32_t iBufSize)
{
    ...
    else if (iSelect == 'pthe')
    {
        ...
        {
            if ((pBuf != NULL) && (iBufSize >= (int32_t)sizeof(SceNpOnlineId)))
            {
                ds_memcpy_s(pBuf, iBufSize, &pRef->launchSoid, sizeof(pRef->launchSoid));
            }
    ...
    else if (iSelect == 'ptil')
    {
        ...
        if ((pBuf != NULL) && (iBufSize >= (int32_t)(pRef->iPTInviteListLen * sizeof(SceNpOnlineId))))
        {
            int32_t iIndex;
            SceNpOnlineId *pInviteList = (SceNpOnlineId *)pBuf;

            for (iIndex = 0; iIndex < pRef->iPTInviteListLen; ++iIndex)
            {
                ds_memcpy_s(&pInviteList[iIndex], sizeof(SceNpOnlineId), &pRef->aPTInviteList[iIndex], sizeof(SceNpOnlineId));
            }
        }
