...
static void *_SocketLookupThread(void *_pRef)
{
    ...
    if ((iResult = getaddrinfo(pHost->name, NULL, &Hints, &pList)) == 0)
    {
        ...
        // first loop we look for IPv6 addresses (prefer IPv6 to IPv4)
        for (pAddrInfo = pList; pAddrInfo != NULL; pAddrInfo = pAddrInfo->ai_next)
        {
            ...
            if ((pHost->addr == 0) && (pAddrInfo->ai_family == AF_INET6))
            {
...
static void _SocketRecvToPacketQueue(SocketStateT *pState, SocketT *pSocket)
{
    ...
    if (SocketPacketQueueStatus(pSocket->pRecvQueue, 'full') || (pSocket->bVirtual == TRUE))
    {
        return;
    }
    ...
    // if the read completed successfully, save the originator address and forward data to socket callback if needed
    if (iRecvResult > 0)
    {
    ...
    while(pState->iRecvLife == 1)
    {
        ...
        for (pSocket = pState->pSockList; (pSocket != NULL) && (pollList.iCount < SOCKET_MAXPOLL); pSocket = pSocket->pNext)
        {
            ...
            if ((pSocket->bVirtual == FALSE) && (pSocket->uSocket != INVALID_SOCKET) && (pSocket->bAsyncRecv == TRUE))
            {
                ...
                // if the socket is not virtual and if there is room in the recv queue, then add this socket
                // to the poll list to be used by the next poll() call
                if (!SocketPacketQueueStatus(pSocket->pRecvQueue, 'full') && (pSocket->uSocket != INVALID_SOCKET) && (pSocket->uBrokenFlag <= 1))
                {
...
int32_t SocketCreate(int32_t iThreadPrio, int32_t iThreadStackSize, int32_t iThreadCpuAffinity)
{
    ...
    if (!pState->bSingleThreaded)
    {
        ...
        if (iThreadCpuAffinity != 0)
        {
            ...
            {
                NetPrintf(("dirtynetunix: failed to set socket receive thread's cpu affinity (err=%d)\n", iResult));
            }
        }
        #endif
...
int32_t SocketConnect(SocketT *pSocket, struct sockaddr *pName, int32_t iNameLen)
{
    ...
    NetPrintfVerbose((pSocket->iVerbose, 0, "dirtynetunix: connecting to %a:%d\n", SockaddrInGetAddr(pName), SockaddrInGetPort(pName)));
...
int32_t SocketControl(SocketT *pSocket, int32_t iOption, int32_t iData1, void *pData2, void *pData3)
{
    ...
    if (iOption == 'keep')
    {
        ...
        if (pSocket->uSocket == INVALID_SOCKET)
        {
            NetPrintf(("dirtynetunix: [%p] 'keep' control used with an invalid socket\n", pSocket));
            return(-1);
        }
...
HostentT *SocketLookup(const char *pText, int32_t iTimeout)
{
    ...
    int32_t iAddr, iResult, iThreadCpuAffinity;
    ...
    if ((iThreadCpuAffinity = NetConnStatus('affn', 0, NULL, 0)) > 0)
    {
        ...
        if ((iResult = pthread_attr_setaffinity_np(&attr, sizeof(CpuSetMask), &CpuSetMask)) != 0)
        {
            NetPrintf(("dirtynetunix: failed to set socket lookup thread's cpu affinity (err=%d)\n", iResult));
        }
    }
    #else
        (void)iThreadCpuAffinity;
    #endif
