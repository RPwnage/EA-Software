...
#define DIRTYNET_STREAMSOCK_DEFAULT_RECVBUFSIZE (16 * 1024)
...
typedef struct SocketLookupPrivT
{
    ...
    IAsyncOperation<IVectorView<EndpointPair ^> ^>^ refAsyncOp;
} SocketLookupPrivT;
...
struct SocketT
{
    ...
    DataWriterStoreOperation ^refDataWriterStoreOperation;
...
static void _SocketSendData(SocketT *pSocket, const char *pBuf, int32_t iLen, DataWriter ^refDataWriter)
{
    ...
    if (pSocket->iLastError > 0)
    {
        ...
        pSocket->refDataWriterStoreOperation = refDataWriter->StoreAsync();
        pSocket->refDataWriterStoreOperation->Completed = ref new AsyncOperationCompletedHandler<unsigned int>(
            [pSocket, iLen] (IAsyncOperation<unsigned int> ^refAsyncOp, Windows::Foundation::AsyncStatus asyncStatus)
        {
            switch (asyncStatus)
            {
                case AsyncStatus::Completed:
                    ...
                    try
                    {
                        int32_t iBytesStored;

                        // try getting an exception.
                        iBytesStored = refAsyncOp->GetResults();

                        if (iBytesStored != iLen)
                        {
                            NetPrintf(("dirtynetwinrt: [%p] DataWriter::StoreAsync() failed to send all bytes: %d vs %d\n", pSocket, iBytesStored, iLen));
                            pSocket->iOpened = -1;      // mark the socket in failure
                        }
                    }
                    catch (Exception ^exception)
                    {
                ...
                case AsyncStatus::Error:
                    NetPrintf(("dirtynetwinrt: [%p] DataWriter::StoreAsync() failed ErrorCode=%S/0x%08x\n",
                        pSocket, pSocket->refDataWriterStoreOperation->ErrorCode.ToString()->Data(), pSocket->refDataWriterStoreOperation->ErrorCode.Value));
                    pSocket->iOpened = -1;      // mark the socket in failure
                    break;

                case AsyncStatus::Canceled:
                    NetPrintf(("dirtynetwinrt: [%p] DataWriter::StoreAsync() canceled\n", pSocket));
                    pSocket->iOpened = -1;      // mark the socket in failure
                    break;

                default:
                    NetPrintf(("dirtynetwinrt: [%p] DataWriter::StoreAsync() - unknown asyncStatus code (%d)\n",
                        pSocket, asyncStatus));
                    pSocket->iOpened = -1;      // mark the socket in failure
                    break;
            }
...
void DatagramSocketRecvContext::OnMessage(DatagramSocket^ refDgramSock, DatagramSocketMessageReceivedEventArgs^ refEventArgs)
{
    ...
    // mclouatre - june 11 2014
    // hack for event handler firing after socket destroy
    // ignore event if socket is no longer in socket list
    ...
    if (ppSocket == NULL)
    {
        NetPrintf(("dirtynetwinrt: [%p] DatagramSocketRecvContext::OnMessage() inbound dgram dropped because socket ref is no longer valid\n", mSocket));
        NetCritLeave(NULL);
        return;
    }
...
static SocketT *_SocketOpen(Object^ refSock, int32_t iFamily, int32_t iType, int32_t iProto, int32_t iOpened)
{
    ...
    else if (iType == SOCK_STREAM)
    {
        ...
        pSocket->un.streamSpace.iRecvBufferSize = DIRTYNET_STREAMSOCK_DEFAULT_RECVBUFSIZE;
    ...
    pSocket->iOpened = (int8_t)iOpened;
...
#if 0
/*
The following code used to unregister event handler does not work.
The MS doc states: To unregister teh MessageReceived event, the socket object must be closed. For apps
written in C++, the Close method will be called when using the delete keyword on the object.
*/

    if (pSocket->iType == SOCK_DGRAM)
    {
        // unregister inbound datagram event handler
        if (pSocket->refSock != nullptr)
        {
            dynamic_cast<DatagramSocket^>(pSocket->refSock)->MessageReceived -= pSocket->MessageReceivedEventToken;
        }
    }

    if (pSocket->iType == SOCK_STREAM && pSocket->un.streamSpace.bSocketListenCalled != FALSE)
    {
        // unregister connection event handler
        dynamic_cast<StreamSocketListener^>(pSocket->refSock)->ConnectionReceived -= pSocket->ConnectionReceivedEventToken;
    }
#endif
    ...
    /*
    The socket needs to be closed BEFORE the socket event context to implicitly unregister the event handler (see above comment)
    before destroying the event context on which it relies. When this order is not enforced, we experimented the Platform::DisconnectedException
    being thrown because of OnMessage being invoked while refSockEventCtx was no longer valid.
    */
    if (pSocket->refSock != nullptr)
    {
...
static void _StreamSocketAsyncRecv(SocketT *pSocket)
{
    ...
    if ((pSocket->iOpened > 0) && (pSocket->un.streamSpace.bLoadAsyncInProgress == FALSE))
    {
        ...
        task<unsigned int>(pSocket->refDataReader->LoadAsync(pSocket->un.streamSpace.iRecvBufferSize)).then([pSocket] (unsigned int iBytesLoaded)
        {
            if (iBytesLoaded == 0)
            {
...
static void _SocketKill(SocketT *pSocket)
{
    ...
    if (pSocket->refAsyncAction)
    {
        delete pSocket->refAsyncAction;
        pSocket->refAsyncAction = nullptr;
    }

    if (pSocket->refDataWriterStoreOperation)
    {
        delete pSocket->refDataWriterStoreOperation;
        pSocket->refDataWriterStoreOperation = nullptr;
    }
...
int32_t SocketConnect(SocketT *pSocket, struct sockaddr *pName, int32_t iNameLen)
{
    ...
    if (pSocket->iType == SOCK_DGRAM)
    {
        ...
        */
        ...
        pSocket->refAsyncAction = static_cast<DatagramSocket^>(pSocket->refSock)->ConnectAsync(refRemoteHostName, refRemoteServNameStr);
        pSocket->refAsyncAction->Completed = ref new AsyncActionCompletedHandler(
            [pSocket] (IAsyncAction ^refAsyncAction, Windows::Foundation::AsyncStatus asyncStatus)
        {
            switch(asyncStatus)
            {
                case AsyncStatus::Completed:
                    ...
                    {
                        // try getting an exception.
                        refAsyncAction->GetResults();

                        pSocket->refDataWriter = ref new DataWriter(dynamic_cast<DatagramSocket^>(pSocket->refSock)->OutputStream);

                        pSocket->iOpened = 1;       // mark the socket as opened
                    }
                    catch (Exception^ exception)
                    {
                        NetPrintf(("dirtynetwinrt: [%p] (datagram socket) DatagramSocket::ConnectAsync(HostName) threw an exception: %S\n",
                            pSocket, exception->ToString()->Data()));
                ...
                case AsyncStatus::Error:
                    NetPrintf(("dirtynetwinrt: [%p] (datagram socket) DatagramSocket::ConnectAsync(HostName) failed ErrorCode=%S/0x%08x\n",
                        pSocket, refAsyncAction->ErrorCode.ToString()->Data(), refAsyncAction->ErrorCode.Value));
                    pSocket->iOpened = -1;      // mark the socket in failure
                    break;

                case AsyncStatus::Canceled:
                    NetPrintf(("dirtynetwinrt: [%p] (datagram socket) DatagramSocket::ConnectAsync(HostName) canceled\n", pSocket));
                    pSocket->iOpened = -1;      // mark the socket in failure
                    break;

                default:
                    NetPrintf(("dirtynetwinrt: [%p] (datagram socket) DatagramSocket::ConnectAsync(HostName) - unknown asyncStatus code (%d)\n",
                        pSocket, asyncStatus));
                    pSocket->iOpened = -1;      // mark the socket in failure
                    break;
            }
    ...
    else if (pSocket->iType == SOCK_STREAM)
    {
        ...
        {
            /*
            connect using ConnectAsync(Endpoint) passing in the bindAddr prepared in SocketBind()
            */
            ...
            pSocket->bConnectAsyncInProgress = TRUE;
            pSocket->refAsyncAction = static_cast<StreamSocket^>(pSocket->refSock)->ConnectAsync(refEndPointPair, SocketProtectionLevel::PlainSocket);
            pSocket->refAsyncAction->Completed = ref new AsyncActionCompletedHandler(
                [pSocket] (IAsyncAction ^refAsyncAction, Windows::Foundation::AsyncStatus asyncStatus)
            {
                switch(asyncStatus)
                {
                    case AsyncStatus::Completed:
                        ...
                        {
                            NetPrintf(("dirtynetwinrt: [%p] (stream socket) StreamSocket::ConnectAsync(EndPointPair) threw an exception: %S\n",
                                pSocket, exception->ToString()->Data()));
                    ...
                    case AsyncStatus::Error:
                        NetPrintf(("dirtynetwinrt: [%p] (stream socket) StreamSocket::ConnectAsync(EndPointPair) failed ErrorCode=%S/0x%08x\n",
                            pSocket, refAsyncAction->ErrorCode.ToString()->Data(), refAsyncAction->ErrorCode.Value));
                        pSocket->iOpened = -1;      // mark the socket in failure
                        break;

                    case AsyncStatus::Canceled:
                        NetPrintf(("dirtynetwinrt: [%p] (stream socket) StreamSocket::ConnectAsync(EndPointPair) canceled\n", pSocket));
                        pSocket->iOpened = -1;      // mark the socket in failure
                        break;

                    default:
                        NetPrintf(("dirtynetwinrt: [%p] (stream socket) StreamSocket::ConnectAsync(EndPointPair) - unknown asyncStatus code (%d)\n",
                            pSocket, asyncStatus));
                        pSocket->iOpened = -1;      // mark the socket in failure
                        break;
                }
        ...
        else
        {
            ...
            */
            ...
            pSocket->bConnectAsyncInProgress = TRUE;
            pSocket->refAsyncAction = static_cast<StreamSocket^>(pSocket->refSock)->ConnectAsync(refRemoteHostName, refRemoteServNameStr, SocketProtectionLevel::PlainSocket);
            pSocket->refAsyncAction->Completed = ref new AsyncActionCompletedHandler(
                [pSocket] (IAsyncAction ^refAsyncAction, Windows::Foundation::AsyncStatus asyncStatus)
            {
                switch(asyncStatus)
                {
                    case AsyncStatus::Completed:
                        ...
                        {
                            NetPrintf(("dirtynetwinrt: [%p] (stream socket) StreamSocket::ConnectAsync(HostName) threw an exception: %S\n",
                                pSocket, exception->ToString()->Data()));
                    ...
                    case AsyncStatus::Error:
                        NetPrintf(("dirtynetwinrt: [%p] (stream socket) StreamSocket::ConnectAsync(HostName) failed ErrorCode=%S/0x%08x\n",
                            pSocket, refAsyncAction->ErrorCode.ToString()->Data(), refAsyncAction->ErrorCode.Value));
                        pSocket->iOpened = -1;      // mark the socket in failure
                        break;

                    case AsyncStatus::Canceled:
                        NetPrintf(("dirtynetwinrt: [%p] (stream socket) StreamSocket::ConnectAsync(HostName) canceled\n", pSocket));
                        pSocket->iOpened = -1;      // mark the socket in failure
                        break;

                    default:
                        NetPrintf(("dirtynetwinrt: [%p] (stream socket) StreamSocket::ConnectAsync(HostName) - unknown asyncStatus code (%d)\n",
                            pSocket, asyncStatus));
                        pSocket->iOpened = -1;      // mark the socket in failure
                        break;
                }
...
int32_t SocketListen(SocketT *pSocket, int32_t iBacklog)
{
    ...
    if ((pSocket->iType == SOCK_STREAM) && (pSocket->un.streamSpace.bSocketListenCalled == FALSE))
    {
        ...
        pSocket->un.streamSpace.iMaxPendingConnections = (int8_t)iBacklog;
...
SocketT *SocketAccept(SocketT *pSocket, struct sockaddr *pAddr, int32_t *pAddrLen)
{
        ...
        (*pAddrLen >= sizeof(struct sockaddr)))
    {
        ...
        if (uVectorSize != 0)
        {
            ...
            pAcceptedSocket = _SocketOpen(pSocket->refStreamConnectVect->GetAt(0), pSocket->iFamily, pSocket->iType, pSocket->iProto, 0);
...
int32_t SocketSendto(SocketT *pSocket, const char *pBuf, int32_t iLen, int32_t iFlags, const struct sockaddr *pTo, int32_t iToLen)
{
    ...
    if (pSocket->refSock == nullptr)
    {
        ...
        NetPrintf(("dirtynetwinrt: [%p] attempting to send to %a:%d on invalid socket\n", pSocket, uAddr, uPort));
        #endif
        pSocket->iLastError = SOCKERR_INVALID;
        return(pSocket->iLastError);
    }
...
int32_t SocketInfo(SocketT *pSocket, int32_t iInfo, int32_t iData, void *pBuf, int32_t iLen)
{
    ...
    if (pSocket == NULL)
    {
        ...
        if (iInfo == 'sdcf')
        {
            ...
            {
                memcpy(pBuf, &pState->aSendCbEntries[iData].pSendCallback, sizeof(pState->aSendCbEntries[iData].pSendCallback));
                return(0);
            }

            NetPrintf(("dirtynetwinrt: 'sdcf' selector used with invalid paramaters\n"));
            return(-1);
        }
        ...
        if (iInfo == 'sdcu')
        {
            ...
            {
                memcpy(pBuf, &pState->aSendCbEntries[iData].pSendCallref, sizeof(pState->aSendCbEntries[iData].pSendCallref));
                return(0);
            }

            NetPrintf(("dirtynetwinrt: 'sdcu' selector used with invalid paramaters\n"));
            return(-1);
        }

        NetPrintf(("dirtynetwinrt: unhandled global SocketInfo() selector '%C'\n", iInfo));
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'sdcb' - set/unset send callback (iData1=TRUE for set - FALSE for unset, pData2=callback, pData3=callref), pData3=callref)
...
int32_t SocketControl(SocketT *pSocket, int32_t iOption, int32_t iData1, void *pData2, void *pData3)
{
    ...
    if (iOption == 'maxr')
    {
        NetPrintf(("dirtynetwinrt: setting max recv rate to %d bytes/sec\n", iData1));
    ...
    if (iOption == 'maxs')
    {
        NetPrintf(("dirtynetwinrt: setting max send rate to %d bytes/sec\n", iData1));
    ...
    if (iOption == 'rbuf')
    {
        if ((pSocket != NULL) && (pSocket->iType == SOCK_STREAM))
        {
            NetPrintf(("dirtynetwinrt: [%p] stream socket recv buf size changed from %d to %d\n",
                pSocket, pSocket->un.streamSpace.iRecvBufferSize, iData1));
            pSocket->un.streamSpace.iRecvBufferSize = iData1;
        }
        else
        {
            NetPrintf(("dirtynetwinrt: [%p] warning - call to SocketControl('rbuf') ignored because pSocket is NULL or socket type is not SOCK_STREAM\n", pSocket));
            return(-1);
        }
        return(0);
    }
    ...
    if (iOption == 'scbk')
    {
        ...
        {
            NetPrintf(("dirtynetwinrt: send callbacks usage changed from %s to %s for socket ref %p\n", (pSocket->bSendCbs?"ON":"OFF"), (iData1?"ON":"OFF"), pSocket));
...
/*!
    ...
    \Notes
        Our WINRT implementation of SocketLookup() has to leverage the DatagramSocket::GetEndpointPairAsync()
        functionality to resolve an hostname into an ipv4 address. This method gets a list of EndpointPair
        objects (based on a remote hostname and remote service name (a port)) than can be used to send
        datagrams to a remote network destination. When the function returns, we inspect the collection
        of EndpointPairs that it returns and we assume that the first entry with an ipv4 remote host name
        can be used as the “resolved” ip. We don't really care about the service itself. All we want
        is an ip address to reach the specified host name. So, we fill the second parameter of
        DatagramSocket::GetEndpointPairAsync() with 9, which is the discard protocol
        (http://en.wikipedia.org/wiki/Discard_Protocol)
...
HostentT *SocketLookup(const char *pText, int32_t iTimeout)
{
    ...
    pPriv->refAsyncOp = pPriv->refDgramSock->GetEndpointPairsAsync(refRemoteHostName, L"9");
    pPriv->refAsyncOp->Completed = ref new Windows::Foundation::AsyncOperationCompletedHandler<IVectorView<EndpointPair^>^>(
        [pPriv, pState] (IAsyncOperation<IVectorView<EndpointPair ^> ^>^ refAsyncOp, Windows::Foundation::AsyncStatus asyncStatus) {
        ...
        switch (asyncStatus)
        {
            case AsyncStatus::Completed:
                try
                {
                    ...
                    auto it = refAsyncOp->GetResults()->First();
                    while (it->HasCurrent)
                    {
                    ...
                    if (pPriv->host.done == 1)
                    {
                        ...
                        SocketHostnameCacheAdd(pState->pHostnameCache, pPriv->host.name, pPriv->host.addr, 1 /*pState->iVerbose*/);
                    }
                    else
                    {
                ...
                {
                    pPriv->host.done = -1;
                    NetPrintf(("dirtynetwinrt: DatagramSocket::GetEndpointPairAsync(%s) failed: %S\n", pPriv->host.name, e->ToString()->Data()));
                }
                break;
            case AsyncStatus::Error:
                NetPrintf(("dirtynetwinrt: DatagramSocket::GetEndpointPairAsync(%s) ErrorCode=%S (0x%08x)\n", pPriv->host.name, refAsyncOp->ErrorCode.ToString()->Data(), refAsyncOp->ErrorCode.Value));
                break;
            case AsyncStatus::Canceled:
                NetPrintf(("dirtynetwinrt: DatagramSocket::GetEndpointPairAsync(%s) canceled\n", pPriv->host.name));
                break;
            default:
                NetPrintf(( "dirtynetwinrt: unknown AsyncStatus code (%d) with DatagramSocket::GetEndpointPairAsync(%s)\n", pPriv->host.name, asyncStatus));
                break;
        }
        ...
        delete pPriv->refAsyncOp;
        pPriv->refAsyncOp = nullptr;
