...
static SocketT *_SocketOpen(SOCKET uSocket, int32_t iFamily, int32_t iType, int32_t iProto)
{
    ...
    if ((uSocket == INVALID_SOCKET) && ((uSocket = WSASocketW(iFamily = AF_INET6, iType, iProto, NULL, 0, WSA_FLAG_OVERLAPPED)) == INVALID_SOCKET))
    {
...
static void _SocketRecvfromAsyncComplete(SocketT *pSocket, int32_t iBytesReceived)
{
    ...
    // complete setup of packet queue entry (already has data)
...
static void _SocketRecvfromAsync(SocketT *pSocket)
{
    ...
    {
        NetPrintf(("dirtynetwin: [%p] error %d when trying to initiate async receive on socket\n", pSocket->uSocket, iRecvErr));
    }
    else
    {
        ...
        for (pSocket = pState->pSockList; (pSocket != NULL) && (iNumEvents < SOCKET_MAXEVENTS); pSocket = pSocket->pNext)
        {
            ...
            if ((!pSocket->bVirtual) && (pSocket->uSocket != INVALID_SOCKET) && pSocket->bAsyncRecv)
            {
                ...
                if (pSocket->uSocket != INVALID_SOCKET)
                {
                    ...
                    if (!pSocket->bRecvInp && !SocketPacketQueueStatus(pSocket->pRecvQueue, 'full'))
                    {
...
int32_t SocketCreate(int32_t iThreadPrio, int32_t iThreadStackSize, int32_t iThreadCpuAffinity)
{
    ...
    if ((pState->hRecvThread = CreateThread(NULL, iThreadStackSize, (LPTHREAD_START_ROUTINE)&_SocketRecvThread, NULL, 0, (LPDWORD)&iPid)) != NULL)
    {
        ...
        if ((iThreadCpuAffinity != 0) && (SetThreadAffinityMask(pState->hRecvThread, iThreadCpuAffinity) == 0))
        {
            NetPrintf(("dirtynetwin: failed to set socket receive thread's cpu affinity (err=%d)\n", GetLastError()));
        }
...
{
    return(_SocketOpen(INVALID_SOCKET, iFamily, iType, iProto));
}
...
int32_t SocketConnect(SocketT *pSocket, struct sockaddr *pName, int32_t iNameLen)
{
    ...
    if (((iResult = SocketBind(pSocket, (const struct sockaddr *)&SockAddr6, sizeof(SockAddr6))) < 0) && (iResult != SOCKERR_INVALID))
    {
        pSocket->iLastError = iResult;
        return(pSocket->iLastError);
    }
...
int32_t SocketSendto(SocketT *pSocket, const char *pBuf, int32_t iLen, int32_t iFlags, const struct sockaddr *pTo, int32_t iToLen)
{
    ...
    else
    {
        ...
        iResult = sendto(pSocket->uSocket, pBuf, iLen, 0, (struct sockaddr *)&SockAddr6, iToLen);
    }
...
int32_t SocketRecvfrom(SocketT *pSocket, char *pBuf, int32_t iLen, int32_t iFlags, struct sockaddr *pFrom, int32_t *pFromLen)
{
    ...
    if (pSocket->bAsyncRecv)
    {
        ...
        bSocketPacketQueueFull = SocketPacketQueueStatus(pSocket->pRecvQueue, 'full');

        iRecv = _SocketRecvfromPacketQueue(pSocket, pBuf, iLen, pFrom, pFromLen);

        // when data is obtained from the packet queue, we lose visibility on system socket errors
        pSocket->iLastError = SOCKERR_NONE;

        /* If the socket is not virtual and if no packet extracted from the packet queue,
           then consider polling the physical socket directly.

           Continuous polling of SocketRecvfrom by the application layer can result in
           a complete drain of the socket packet queue. At that point, it is very possible
           that there is inbound traffic sitting in the system socket receive buffer
           because the socket recv thread is not adding the socket to the poll list when
           its packet queue is full. To optimize receive behavior, instead of waiting for
           the socket recv thread to wakeup and finally re-add this socket to the poll list,
           we immediately perform a direct recv call on the socket in case there is something
           waiting to be processed there. */
        if (!pSocket->bVirtual && (iRecv == 0))
        {
            // avoid performing direct recv call while the socket recv thread already has an async recv initiated on that system socket
            if (!pSocket->bRecvInp)
            {
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'size' - return socket packet queue max size
...
int32_t SocketInfo(SocketT *pSocket, int32_t iInfo, int32_t iData, void *pBuf, int32_t iLen)
{
    ...
    if ((iInfo == 'pdrp') || (iInfo == 'pmax') || (iInfo == 'size'))
    {
        int32_t iResult;
        // acquire socket receive critical section
        NetCritEnter(&pSocket->RecvCrit);
...
int32_t SocketControl(SocketT *pSocket, int32_t iOption, int32_t iData1, void *pData2, void *pData3)
{
    ...
    if (iOption == 'keep')
    {
        ...
        memset(&TcpKeepAlive, 0, sizeof(TcpKeepAlive));
    ...
    if (iOption == 'poll')
    {
        ...
        return(select(iMaxSocket + 1, &FdRead, NULL, &FdExcept, &TimeVal));
    }
...
static int32_t _SocketLookupThread(HostentT *pHost)
{
    ...
    {
        // unsuccessful
        NetPrintf(("dirtynetwin: getaddrinfo() failed err=%d\n", iResult));
        pHost->done = -1;
    }
...
HostentT *SocketLookup(const char *pText, int32_t iTimeout)
{
    ...
    if ((hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)&_SocketLookupThread, (LPVOID)pHost, 0, &dwPid)) != NULL)
    {
        ...
        // set thread's cpu affinity based on netconn's settings
        if ((iThreadCpuAffinity = NetConnStatus('affn', 0, NULL, 0)) > 0)
        {
            if (SetThreadAffinityMask(hThread, iThreadCpuAffinity) == 0)
            {
                NetPrintf(("dirtynetwin: failed to set socket lookup thread's cpu affinity (err=%d)\n", GetLastError()));
            }
        }
