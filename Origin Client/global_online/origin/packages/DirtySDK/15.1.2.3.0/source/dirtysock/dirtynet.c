...
/*!
    ...
    \Input *pSockAddr           - address to identify
...
*/
/********************************************************************************F*/
static uint8_t _SockaddrIn6Identify(const struct sockaddr *pSockAddr, const uint8_t *pCheckVal, int32_t iValLen)
{
    struct sockaddr_in6 *pSockAddr6 = (struct sockaddr_in6 *)pSockAddr;
    uint8_t bResult = FALSE;
    if ((pSockAddr->sa_family == AF_INET6) && (!memcmp(pCheckVal, pSockAddr6->sin6_addr.s6_addr, iValLen)))
    {
        bResult = TRUE;
    }
    return(bResult);
...
/*F********************************************************************************/
/*!
    \Function SockaddrIn6IsIPv4

    \Description
        Identify if specified sockaddr is an IPv4-mapped IPv6 address
...
*/
/********************************************************************************F*/
uint8_t SockaddrIn6IsIPv4(const struct sockaddr *pSockAddr)
{
    const uint8_t aIpv4Prefix[]  = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff };
    return(_SockaddrIn6Identify(pSockAddr, aIpv4Prefix, sizeof(aIpv4Prefix)));
}

/*F********************************************************************************/
/*!
    \Function SockaddrIn6IsNAT64

    \Description
        Identify if specified sockaddr is a NAT64 IPv6 address
...
*/
/********************************************************************************F*/
uint8_t SockaddrIn6IsNAT64(const struct sockaddr *pSockAddr)
{
    const uint8_t aNat64Prefix[] = { 0x00, 0x64, 0xff, 0x9b };
    return(_SockaddrIn6Identify(pSockAddr, aNat64Prefix, sizeof(aNat64Prefix)));
}

/*F********************************************************************************/
/*!
    \Function SockaddrIn6IsZero

    \Description
        Identify if specified sockaddr is zero (unspecified)
...
*/
/********************************************************************************F*/
uint8_t SockaddrIn6IsZero(const struct sockaddr *pSockAddr)
{
    const uint8_t aIpv6Zero[]  = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    return(_SockaddrIn6Identify(pSockAddr, aIpv6Zero, sizeof(aIpv6Zero)));
}
...
*/
/************************************************************************************F*/
static SocketAddrMapEntryT *_SocketAddrMapGet(SocketAddrMapT *pAddrMap, const struct sockaddr *pAddr, int32_t iAddrSize)
{
...
*/
/************************************************************************************F*/
static void _SocketAddrMapRemap(SocketAddrMapT *pAddrMap, SocketAddrMapEntryT *pMapEntry, const struct sockaddr_in6 *pAddr6, int32_t iAddrSize)
{
...
void SocketAddrMapInit(SocketAddrMapT *pAddrMap, int32_t iMemGroup, void *pMemGroupUserData)
{
    ...
    pAddrMap->iMemGroup = iMemGroup;
    pAddrMap->pMemGroupUserData = pMemGroupUserData;
    // init ipv6 map virtual address ($$TODO: maybe should just init lower 16 bits?
...
int32_t SocketAddrMapAddress(SocketAddrMapT *pAddrMap, const struct sockaddr *pAddr, int32_t iAddrSize)
{
    ...
    if (SockaddrIn6IsIPv4(pAddr) || SockaddrIn6IsZero(pAddr))
    {
        const struct sockaddr_in6 *pAddr6 = (const struct sockaddr_in6 *)pAddr;
    ...
    // add it to the map
    return(_SocketAddrMapAdd(pAddrMap, (const struct sockaddr_in6 *)pAddr, iAddrSize));
}
...
/*!
    ...
    \Output
        int32_t         - negative=error, else virtual IPv4 address for remapped IPv6 address
...
*/
/************************************************************************************F*/
struct sockaddr *SocketAddrMapTranslate(SocketAddrMapT *pAddrMap, struct sockaddr *pResult, const struct sockaddr *pSource, int32_t *pNameLen)
{
    SocketAddrMapEntryT *pMapEntry;
    ...
    else if ((pSource->sa_family == AF_INET6) && (pResult->sa_family == AF_INET))
    {
        ...
        else if (SockaddrIn6IsIPv4(pSource))
        {
            struct sockaddr *pResult4 = (struct sockaddr *)pResult;
    ...
    else
    {
        ...
        pReturn = pResult;
#else
        NetPrintf(("dirtynet: unsupported address translation type %d->%d\n", pSource->sa_family, pResult->sa_family));
#endif
    }
    if (pReturn != NULL)
    {
        NetPrintfVerbose((SocketInfo(NULL, 'spam', 0, NULL, 0), 1, "dirtynet: map translate %A->%A\n", pSource, pReturn));
    }
    else
    {
