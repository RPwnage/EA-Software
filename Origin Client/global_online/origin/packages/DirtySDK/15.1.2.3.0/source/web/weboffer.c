...
*/
/*******************************************************************H*/
// todo: document this, sprintfs are left in place even though unsafe, fixing them would require an api change
/*** Include files ***************************************************/
...
static void _WebOfferGetColor(WebOfferT *pOffer, WebOfferColorT *pColor, const char *pTag)
{
    ...
    {
        uColor = (uint32_t)TagFieldGetNumber64(TagFieldFind(pOffer->strSetup, "COLOR"), 0xffff0000);
    }
...
static void _WebOfferGetButton(WebOfferT *pOffer, const char *pTagName, WebOfferButtonT *pButton, int32_t iItem)
{
    ...
    // get the button
    ds_snzprintf(strFind, sizeof(strFind), "%s%d", pTagName, iItem);
...
static char *_WebOfferFind(WebOfferT *pOffer, char *pMatch, const char *pFind)
{
    ...
    while (pMatch != NULL)
    {
        ...
        if (iNext && (ds_stricmp(pFind, "$next") == 0))
        {
            pResult = pMatch;
            break;
        }
        ...
        if ((ds_stricmp(pFind, "$incl") == 0) && (pMatch[2] == '*'))
        {
            pResult = pMatch;
            break;
        }
        ...
        if ((ds_stristr(pMatch, pFind) == pMatch+2) && ((pMatch+2)[strlen(pFind)] <= ' '))
        {
            pResult = pMatch;
            break;
        }
        ...
        if (pMatch[1] == '$')
        {
            ...
            iSkip = (int32_t)strtol(pMatch+2, &pBogus, 16);
            iSkip += 12;
...
static void _WebOfferActionGoto(WebOfferT *pOffer, const char *pTarget)
{
    ...
    if (ds_stricmp(pTarget, "$http") == 0)
    {
        pOffer->pExec = pOffer->pHttpData;
    }
    // handle http-clear link special
    else if (ds_stricmp(pTarget, "$home") == 0)
    {
        if (pOffer->pHttpData != NULL)
        {
            pOffer->iHttpSize = (int32_t)(_WebOfferFind(pOffer, pOffer->pHttpData, "$last")-pOffer->pHttpData);
    ...
    // handle quit special
    else if (ds_stricmp(pTarget, "$quit") == 0)
    {
        pOffer->pExec = NULL;
    }
    ...
    else if (ds_stricmp(pTarget, "$next") == 0)
    {
    }
    ...
    for (pNext = pAction; *pNext != 0; ++pNext)
    {
        if (*pNext == ';')
        {
            ...
            *pParm = 0;
            ...
            // dispatch to command
            if (ds_stricmp(pAction, "goto") == 0)
            {
                _WebOfferActionGoto(pOffer, pParm);
            }
            else if (ds_stricmp(pAction, "clear") == 0)
            {
                _WebOfferActionClear(pOffer, pParm);
            }
            else if (ds_stricmp(pAction, "exit") == 0)
            {
...
static void _WebOfferUrl(char *pUrl, int32_t iUrl, const char *pVars)
{
    ...
    while ((pHead = strchr(pUrl, '$')) != NULL)
    {
        ...
        iSize = (int32_t)(pTail-(pHead+1));
        ds_strsubzcpy(strFind, sizeof(strFind), pHead+1, iSize);
...
void WebOfferExecute(WebOfferT *pOffer, const char *pScript)
{
    ...
    else if (ds_stristr(pScript, "%{*") != NULL)
    {
...
WebOfferCommandT *WebOfferCommand(WebOfferT *pOffer)
{
    ...
    for (iLimit = 0; iLimit < WEBOFFER_GOTO_LIMIT; ++iLimit)
    {
        ...
        iSize = (int32_t)(pOffer->pCmdTail - pOffer->pCmdHead);
        ...
        ds_memcpy_s(pCmd->strParms, sizeof(pCmd->strParms), pOffer->pCmdHead, iSize);
...
void WebOfferAction(WebOfferT *pOffer, int32_t iAction)
{
    ...
    iOldSize = (int32_t)(pOffer->pCmdTail - pOffer->pCmdHead);
    iNewSize = (int32_t)strlen(pOffer->Command.strParms);

    // see if this will fit in script
    iSize = (int32_t)(_WebOfferFind(pOffer, pOffer->pScript, "$last")-pOffer->pScript+1);
    ...
    ds_memcpy(pOffer->pCmdHead, pOffer->Command.strParms, iNewSize);
...
void WebOfferHttp(WebOfferT *pOffer)
{
    ...
    pOffer->iHttpSize = (int32_t)((pOffer->pScript+pOffer->iScript)-pOffer->pHttpData-1);
...
void WebOfferUpdate(WebOfferT *pOffer)
{
    ...
    if (pOffer->eState == ST_DONE)
    {
        ...
        if ((pHead != NULL) && (pTail != NULL) && (pTail-pHead < pOffer->iHttpSize) && (ds_stristr(pHead, "URL=") != NULL))
        {
            ...
            TagFieldRename(pHead, (int32_t)(pTail-pHead), "URL", "UUU");
            // default to failure 
            pOffer->eState = ST_FAIL;
...
int32_t WebOfferResource(WebOfferT *pOffer, WebOfferResourceT *pRsrc, const char *pName)
{
    ...
    if ((pName != NULL) && (*pName != 0) && (*pName != '$'))
    {
        ...
        if ((pFind != NULL) && (pData != NULL))
        {
            ...
            pRsrc->pParm = pFind;
            pRsrc->iSize = (int32_t)strtol(pData+2, &pBogus, 16);
...
/*!
    ...
    \Notes
        $$todo v9:
            * deprecate WebOfferGetBusy
            * rename WebOfferGetBusy2 --> WebOfferGetBusy
...
{
    WebOfferGetBusy2(pOffer, pInfo, NULL);
}

/*F*******************************************************************/
/*!
    \Function WebOfferGetBusy2
    ...
    \Input pOffer               - module state from WebOfferCreate
    \Input pInfo                - return structure for busy screen data
    \Input strDefaultMessage    - The default message to use

    \Version 02/07/2012 jchen
*/
/*******************************************************************F*/
void WebOfferGetBusy2(WebOfferT *pOffer, WebOfferBusyT *pInfo, const char *strDefaultMessage)
{
    ...
    TagFieldGetString(TagFieldFind(pBuf, "MESG"), pInfo->strMessage, sizeof(pInfo->strMessage), strDefaultMessage != NULL ? strDefaultMessage : "PLEASE WAIT");
...
void WebOfferSetForm(WebOfferT *pOffer, WebOfferFormT *pInfo)
{
    ...
    {
        char strTag[128];
        ds_snzprintf(strTag, sizeof(strTag), "FLD-USERVALUE%d", iLoop + 1);
