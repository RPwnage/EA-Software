...
static void _T2ClientProfileLoadPlatforms(HWND hDlg)
{
    ...
    SendMessage( GetDlgItem(hDlg, IDC_COMBO_PLATFORMTYPE), (UINT)CB_INSERTSTRING, TESTER_PLATFORM_PC,    (LPARAM)(LPCTSTR)"PC\0");  
    if (TesterCommCanAttachXenon())
    {
        SendMessage( GetDlgItem(hDlg, IDC_COMBO_PLATFORMTYPE), (UINT)CB_INSERTSTRING, TESTER_PLATFORM_XENON, (LPARAM)(LPCTSTR)"Xenon\0");  
    }
    else
    {
        SendMessage( GetDlgItem(hDlg, IDC_COMBO_PLATFORMTYPE), (UINT)CB_INSERTSTRING, TESTER_PLATFORM_XENON, (LPARAM)(LPCTSTR)"Disabled - Xenon\0");
    }
    SendMessage( GetDlgItem(hDlg, IDC_COMBO_PLATFORMTYPE), (UINT)CB_INSERTSTRING, TESTER_PLATFORM_PS3,   (LPARAM)(LPCTSTR)"PS3\0");  
}
...
static void _T2ClientSetActiveProfile(HWND hDlg, int32_t iItemNum)
{
    ...
    if (strcmp(Entry.strPlatform,"PC") == 0)
        iPlatform = TESTER_PLATFORM_PC;
    else if (strcmp(Entry.strPlatform, "Xenon") == 0)
        iPlatform = TESTER_PLATFORM_XENON;
    else if (strcmp(Entry.strPlatform, "PS3") == 0)
        iPlatform = TESTER_PLATFORM_PS3;
...
static LRESULT CALLBACK _T2ClientProfileSelect(HWND hDlg, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        {
            int32_t iPlatform = TESTER_PLATFORM_PC;
            char buffer[64];
            ...
            if(strcmp(Entry.strPlatform, "PC") == 0)    iPlatform = TESTER_PLATFORM_PC;
            if(strcmp(Entry.strPlatform, "PS3") == 0)   iPlatform = TESTER_PLATFORM_PS3;
            if(strcmp(Entry.strPlatform, "Xenon") == 0)     iPlatform = TESTER_PLATFORM_XENON;
                        ...
                        LPTSTR    lpCmdLine, int32_t       iCmdShow)
{
    ...
    else
    {
        ...
        if(strcmp(strPlatform, "PC") == 0)    iPlatform = TESTER_PLATFORM_PC;
        if(strcmp(strPlatform, "PS3") == 0)   iPlatform = TESTER_PLATFORM_PS3;
        if(strcmp(strPlatform, "Xenon") == 0)     iPlatform = TESTER_PLATFORM_XENON;
