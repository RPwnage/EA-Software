...
#if (DIRTYCODE_PLATFORM == DIRTYCODE_LINUX) || (DIRTYCODE_PLATFORM == DIRTYCODE_APPLEIOS)
#include <stdio.h>
#endif
...
{
    int32_t iSize;                          //!< file size in bytes
...
// seek in a file
int32_t ZFileSeek(ZFileT iFileId, int32_t iOffset, uint32_t uFlags);
...
// get file size
int32_t ZFileSize(ZFileT iFileId);

// open a file and load it into memory and null-terminate (in case it is a text file)
char *ZFileLoad(const char *pFileName, int32_t *pFileSize, uint32_t bBinary);

// save (overwrite) null-terminated data to a file
int32_t ZFileSave(const char *pFileName, const char *pData, int32_t iSize, uint32_t uFlags);
