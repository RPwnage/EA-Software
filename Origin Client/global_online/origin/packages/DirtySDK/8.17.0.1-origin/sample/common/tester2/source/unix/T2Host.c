...
int main(int32_t argc, char *argv[])
{
    ...
    TagFieldSetNumber(strParams, sizeof(strParams), "PLATFORM", TESTER_PLATFORM_LINUX);
    ...
    for (iArg = 1; iArg < argc; iArg += 1)
    {
        ...
        strnzcat(strCommandLine, argv[iArg], sizeof(strCommandLine));
        strnzcat(strCommandLine, " ", sizeof(strCommandLine));
    }
    NetPrintf(("> %s\n", strCommandLine));
