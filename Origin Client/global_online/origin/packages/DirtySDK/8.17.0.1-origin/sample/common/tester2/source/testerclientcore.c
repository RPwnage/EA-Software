...
TesterClientCoreT *TesterClientCoreCreate(char *strCmdLine)
{
    ...
    ZMemSet((void *)pState, 0, sizeof(TesterClientCoreT));
...
int32_t TesterClientCoreConnect(TesterClientCoreT *pState, const char *pParams)
{
    ...
    int32_t iPlatform;
    char strHostname[64], strProfilename[64], strMessage[128];
    ...
    switch(iPlatform)
    {
        case TESTER_PLATFORM_PC:
        case TESTER_PLATFORM_PS3:
        {
    ...
    TagFieldGetString(TagFieldFind(pParams, "HOSTNAME"), strHostname, sizeof(strHostname), "");
    TagFieldGetString(TagFieldFind(pParams, "PROFILENAME"), strProfilename, sizeof(strProfilename), "");
    snzprintf(strMessage, sizeof(strMessage), "t2client: client '%s' connected to host %s\n", strProfilename, strHostname);
    TesterCommMessage(pState->pComm, TESTER_MSGTYPE_CONSOLE, strMessage);

    // load historical commands
    TesterProfileGet(pState->pProfile, TagFieldGetNumber(TagFieldFind(pParams, "PROFILENUM"), -2), &CurProfile);
...
/*F********************************************************************************/
/*!
    \Function TesterClientCoreSendConsole

    \Description
        Send some console output for the host to display.
...
*/
/********************************************************************************F*/
int32_t TesterClientCoreSendConsole(TesterClientCoreT *pState, const char *pData)
{
    return(TesterCommMessage(pState->pComm, TESTER_MSGTYPE_CONSOLE, pData));
}
...
*/
/********************************************************************************F*/
void TesterClientCoreGetHistoricalCommand(TesterClientCoreT *pState,
                                          int32_t *pOffsetFromCurrent,
                                          char *pBuf, int32_t iSize)
{
    ...
    *pOffsetFromCurrent = iOffsetFromCurrent;
    ...
    {
        // set a blank entry
        ZMemSet(pBuf, 0, iSize);
    }
    else
    {
        TesterHistoryGet(pState->pHistory, (iTailCount + iOffsetFromCurrent), pBuf, iSize);
    }
}
