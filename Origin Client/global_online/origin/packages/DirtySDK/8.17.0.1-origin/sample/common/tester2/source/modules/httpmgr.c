...
int32_t CmdHttpMgr(ZContext *argz, int32_t argc, char *argv[])
{
    ...
    if (pRef->pHttpManager == NULL)
    {
        ...
        pRef->pHttpManager = HttpManagerCreate(iBufSize, 4);
        if (pRef->pHttpManager != NULL)
        {
    ...
    if (!stricmp(argv[1], "put") || !stricmp(argv[1], "post") || !stricmp(argv[1], "puts"))
    {
        ...
        if ((pFileData = ZFileLoad(pFileName, &iFileSize, ZFILE_OPENFLAG_RDONLY|ZFILE_OPENFLAG_BINARY)) != NULL)
        {
            if (stricmp(argv[1], "puts"))
            {
                ...
                if ((pState->iSentSize = HttpManagerPost(pRef->pHttpManager, pState->iHandle, pUrl, pFileData, iFileSize, !stricmp(argv[1], "put") ? PROTOHTTP_PUT : PROTOHTTP_POST)) < 0)
                {
    ...
    else if (!stricmp(argv[1], "head") || !stricmp(argv[1], "get"))
    {
        if (pFileName != NULL)
        {
            ...
            pState->iOutFile = ZFileOpen(pFileName, ZFILE_OPENFLAG_WRONLY|ZFILE_OPENFLAG_BINARY);
        }
        else
        {
        ...
        if ((iResult = HttpManagerGet(pRef->pHttpManager, pState->iHandle, pUrl, !stricmp(argv[1], "head") ? PROTOHTTP_HEADONLY : PROTOHTTP_HEADBODY)) == 0)
        {
            pState->state = DNLOAD;
        }
    }
    else if (!stricmp(argv[1], "mget") && (pRef->pMgetBuffer == NULL))
    {
