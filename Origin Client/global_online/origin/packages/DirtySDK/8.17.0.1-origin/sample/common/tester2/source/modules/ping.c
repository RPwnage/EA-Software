...
static int32_t _CmdPingGetArgs(PingCmdRefT *pCmdRef, int32_t argc, char *argv[])
{
    ...
    for (iArg = 1; iArg < argc; iArg++)
    {
        if (argv[iArg][0] == '-')
        {
            ...
            {
                pCmdRef->iCt = strtol(pOpt + 1,NULL,10);
            }
            if ((pOpt = strchr(argv[iArg], 'q')) != NULL)
            {
                pCmdRef->uPort = strtol(pOpt+1, NULL, 10);
                pCmdRef->bQos = TRUE;
            }
            ...
            if ((pOpt = strchr(argv[iArg], 's')) != NULL)
            {
                ...
                pCmdRef->uServiceId = strtol(pOpt+1, NULL, 16);
            }
            if ((pOpt = strchr(argv[iArg], 'w')) != NULL)
            {
                pCmdRef->iTimeout = strtol(pOpt + 1, NULL, 10);
            }
        }
        else
        {
...
int32_t CmdPing(ZContext *argz, int32_t argc, char *argv[])
{
    ...
    // copy hostname
    strnzcpy(pCmdRef->strHostname, argv[iName], sizeof(pCmdRef->strHostname));
