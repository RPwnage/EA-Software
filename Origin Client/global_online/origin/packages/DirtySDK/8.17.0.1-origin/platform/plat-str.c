...
#undef DS_PLATFORM      // make sure we are using ds_* names
#include "platform.h"
...
*/
/********************************************************************************F*/
static int32_t _ds_printstr(char *pBuffer, int32_t iLength, const char *pString, int32_t iOutput, int32_t iWidth, uint8_t bRightJust, char cPad, char cLead)
{
    int32_t iIndex, iStrLen;
...
/*!
    ...
    \Input bReturnOverflowSize - if TRUE, follow C99 overflow semantics
    
    \Output
        int32_t     - number of characters written to output buffer, not including
                      the null character; zero on error or overflow (see Notes)
    ...
    \Notes
        ...
        the C99 standard if bReturnOverflowSize is TRUE.
...
*/
/********************************************************************************F*/
static int32_t _ds_vsnprintf(char *pBuffer, int32_t iLength, const char *pFormat, va_list Args, uint8_t bReturnOverflowSize)
{
    ...
    for (iInput = 0, iOutput = 0; ; )
    {
        ...
        if (cInput == '%')
        {
            ...
            uint8_t bRightJust = TRUE, bPrintPlus = FALSE;
            char cPad = ' ', cLead = 0;
            ...
            switch(cInput)
            {
                case 'a':
                {
                    ...
                    iOutput = _ds_printstr(pBuffer, iLength, pString, iOutput, iWidth, bRightJust, cPad, 0);
                }
                break;
                ...
                case 'c':
                {
                    ...
                    strTempBuf[1] = '\0';
                    iOutput = _ds_printstr(pBuffer, iLength, strTempBuf, iOutput, iWidth, bRightJust, cPad, 0);
                }
                break;
                ...
                case 'i':
                {
                    ...
                    iOutput = _ds_printstr(pBuffer, iLength, pString, iOutput, iWidth, bRightJust, cPad, cLead);
                }
                break;
                ...
                case 'p':
                {
                    ...
                    iOutput = _ds_printstr(pBuffer, iLength, pString, iOutput, iWidth, bRightJust, '0', cLead); // always print leading zeros
                }
                break;
...
{
    return(_ds_vsnprintf(pBuffer, iLength, pFormat, Args, FALSE));
}
...
/*!
    ...
    \Description
        ...
        the string.  The overflow semantics are also slightly different.

        [DEPRECATE]
        This function will be deprecated in V9 in favor of ds_vsnzprintf2
...
/*F********************************************************************************/
/*!
    \Function ds_vsnzprintf2

    \Description
        Replacement for vsnprintf that always includes a terminator at the end of
        the string.  The overflow semantics of this version meet C99 specifications.
...
{
    return(_ds_vsnprintf(pBuffer, iLength, pFormat, Args, TRUE));
}
...
/*!
    ...
    \Description
        ...
        [DEPRECATE]
        This function will be deprecated in V9 in favor of ds_snzprintf2
...
int32_t ds_snzprintf(char *pBuffer, int32_t iLength, const char *pFormat, ...)
{
    ...
    iResult = _ds_vsnprintf(pBuffer, iLength, pFormat, args, FALSE);
    va_end(args);
...
/*F********************************************************************************/
/*!
    \Function ds_snzprintf2

    \Description
        Replacement for snprintf that always includes a terminator at the end of
        the string.  The overflow semantics of this version follow C99.
...
int32_t ds_snzprintf2(char *pBuffer, int32_t iLength, const char *pFormat, ...)
{
    ...
    // format the text
    va_start(args, pFormat);
    iResult = _ds_vsnprintf(pBuffer, iLength, pFormat, args, TRUE);
    va_end(args);
