...
#if BUDDYAPI_XBFRIENDS
    uint32_t    uTitleId;           // only really valid for Xbox.
    XbFriendsRefT   *pXbf;
    int32_t             bTalkToXbox; //!should we send to Xbox Xdk?
#else
...
char *_BuddyApi_Status[] = { "DISC", "CHAT", "AWAY", "XA", "DND", "PASS", "GAME" };
...
#if BUDDYAPI_XBFRIENDS
    /*
    BUDDY_INVITE_RESPONSE_ACCEPT ,
    BUDDY_INVITE_RESPONSE_REJECT ,
    BUDDY_INVITE_RESPONSE_REVOKE ,
    BUDDY_INVITE_RESPONSE_BLOCK  ,
    BUDDY_INVITE_RESPONSE_REMOVE ,
    */
//convert buddy responses to "actions" for XbFriend to act on..
// see BuddyInviteResponseTypeE
    int32_t aConvertAction[] ={ACTION_ACCEPTED, ACTION_REJECTED, ACTION_REVOKED, ACTION_BLOCKED, ACTION_REMOVED,
           ACTION_INVITED, ACTION_MUTE, ACTION_JOIN  };

#define BUDDY_MASK_GAME_INVITE_XBOX ( BUDDY_FLAG_GAME_INVITE_SENT \
                | BUDDY_FLAG_GAME_INVITE_RECEIVED \
                | BUDDY_FLAG_GAME_INVITE_ACCEPTED \
                | BUDDY_FLAG_GAME_INVITE_REJECTED)
#define XBFRIENDS_MASK_GAME_INVITE_XBOX ( XBFRIENDS_FLAG_GAME_INVITE_SENT \
                | XBFRIENDS_FLAG_GAME_INVITE_RECEIVED \
                | XBFRIENDS_FLAG_GAME_INVITE_ACCEPTED \
                | XBFRIENDS_FLAG_GAME_INVITE_REJECTED)
...
static uint16_t _BuddyApiExtractRandomSeed (BuddyApiRefT *pState)
{
    ...
    {
        snzprintf (tempStr, sizeof(tempStr), "$%c%c%c%c",   pState->pLKey[12], pState->pLKey[13], 
...
static void _BuddyApiParseResource (char *pResource, int32_t domSize, char * pDomain, int32_t prodSize, char * pProduct )
{
        char strTemp[255];
        char *pDiv;
        strnzcpy(strTemp, pResource, sizeof(strTemp));
        // locate the divider
        if ( ( pDiv = strchr(strTemp, '/'))  == NULL)
        {  //only domain, no product.
            strnzcpy (pDomain, strTemp, domSize );
            *pProduct =0;
        }
        else
        {  // turn into two strings
            *pDiv =0;
            strnzcpy (pDomain, strTemp, domSize );
            strnzcpy ( pProduct, pDiv, prodSize);
        }
}
...
static int32_t _BuddyApiPresRecv(BuddyApiRefT *pState, BuddyApiMsgT *pMsg, BuddyRosterT *pBuddy)
{
    ...
    if ( strlen(pBuddy->strTitle) == 0)
    {  //xbox only --if user was "appear offline", try to get it from XDK.   
        uint32_t uTitleID = XbFriendsGetUserTitleID(pState->pXbf, pBuddy->strUser);
        BuddyApiGetTitleName( pState, uTitleID, sizeof(pBuddy->strTitle), pBuddy->strTitle);
    }
#endif
    ...
    {
        iProd = TagFieldGetToken(TagFieldFind(strStatTag, "P"), 0);
...
static void _BuddyApiPresSend(BuddyApiRefT *pState)
{
    ...
    {
        strnzcpy(strPresSame, pState->strPresSame, sizeof (strPresSame) );
    }
...
#if BUDDYAPI_XBFRIENDS
    {  //Xbox needs the Session Id, and if this bud is Joinable..
        XbFriendsXNKID  xnkid;
        int32_t rc=0;
        rc = NetConnStatus('xkid', 0, &xnkid, sizeof (xnkid) );
        TagFieldSetBinary(strPacket, sizeof(strPacket), "SESS", &xnkid, sizeof (xnkid));
    }
#endif
...
#if BUDDYAPI_XBFRIENDS
//!CB when XDK finds out Friend is back online, after a Cross Game Challenge. 
static void _BuddyApiGameInviteReadyCB ( XbFriendsRefT *pFRef, char * strGamertag, int32_t action, int32_t dwState, uint32_t dwTitleId, void *pUserData)
{
    BuddyApiRefT *pState = (BuddyApiRefT*) pUserData;
    BuddyRosterT *pBuddy;
    BuddyApiPrintf( ( pState, "*********\nBuddy: %s  Action%d, state %x, Title %x \n", strGamertag, action, dwState, dwTitleId)); 

    if (action ==0)
    { //was a test to see if online/offline state changed.
    }
    else if ( ( pBuddy = BuddyApiFind ( pState, strGamertag)) != NULL )
    {   
        //lower level checks title, so dont here, as Presence may not be in from server.
        //Dont set flag here, cause may not be online yet, 
        //Save user and wait for presence change to signal to caller.
        BuddyApiPrintf( ( pState, "********\nBuddy: READY READY %s Title %x \n", strGamertag, dwTitleId)); 
        strcpy (pState->pBuddyChangeUser , strGamertag); //save and wait.
    }
}
...
static void _BuddyApiStatus(BuddyApiRefT *pState, int32_t iStat)
{
    ...
    #if DIRTYCODE_LOGGING
    char * strOld = _BuddyApi_Xlate_Stat[  pState->iStat];//.pState;
    char * strNew = _BuddyApi_Xlate_Stat[  iStat]; //.pState;
    #endif
...
static void _BuddyApiHandlePGETMsg ( BuddyApiRefT *pState, BuddyApiMsgT msg)
{
    ...
    if (pRec != NULL)
    {
        ...
        // make a copy
        memcpy(&PrevBuddy,pBuddy,sizeof(PrevBuddy));
...
int32_t BuddyApiDisconnect(BuddyApiRefT *pState )
{
    ...
    if (pState->pXbf )
    {
        XbFriendsDisconnect ( pState->pXbf );
    }
#endif
...
static BuddyRosterT *_BuddyApiAddOrSetFlags (BuddyApiRefT *pState, BuddyRosterT *pBuddy)
{
    ...
    if (pState->pDisp != NULL)
    {
        ...
        if (pRec == NULL)
        {
            ...
            memcpy(pOwned, pBuddy, sizeof(*pOwned));
...
static int32_t _BuddyApiHandleGNOTMsg ( BuddyApiRefT *pState, BuddyApiMsgT msg)
{
    ...
    */
...
#if BUDDYAPI_XBFRIENDS
    return(0); // SHOULD never get this for xbox, as we go direct to XDK.
#endif
    ...
    if (cAction =='I') //invite
    {
        ...
        strnzcpy(pBuddy->strSEID,strSEID,sizeof(pBuddy->strSEID));
    }
        ...
        { //change case
            ...
            if ( (pOwned = BuddyApiFind ( pState, buddy.strUser )) != NULL )
            {
                ...
                { //now a true buddy..
                    ...
                    //02/June//05 -Since server is dumb,  have to deal with a race condition..
                    //I can have sent(8000),  and received(0400), and now have "bud, with received  flag" 
                    //So, just leave the RECEIVED flag on, as other side shows SENT.
                    //--Issue seen only in xbox so be safe and do conditional fix for xbox only..
#if BUDDYAPI_XBFRIENDS
                    if ( (buddy.uFlags & BUDDY_FLAG_BUD_REQ_RECEIVED) >0 )
                        buddy.uFlags  &= ~BUDDY_FLAG_BUDDY; //not buddy, just a dumb race..
#endif
...
//! Based on flags reported from XDK, determine the "action" taken and set new flags in buddy.
static unsigned _setGameInviteFlags (BuddyApiRefT *pState, BuddyRosterT *pBuddy, unsigned uNewFlags, unsigned uChangeFlags)
{
    //return(0); //YAMS -- if talking to server, dont set flags here.
    unsigned eAction =0;
    if (pBuddy )
    {
        pBuddy->uFlags &= ~BUDDY_MASK_GAME_INVITE_XBOX; //clear xbox controlled&set whats needed
        if (uNewFlags &  XBFRIENDS_FLAG_GAME_INVITE_RECEIVED )
        {
            pBuddy->uFlags |= BUDDY_FLAG_GAME_INVITE_RECEIVED;
            if (uChangeFlags & XBFRIENDS_FLAG_GAME_INVITE_RECEIVED )
                eAction = BUDDY_FLAG_GAME_INVITE_RECEIVED;
        }

        if (uNewFlags &  XBFRIENDS_FLAG_GAME_INVITE_SENT  )
            pBuddy->uFlags |= BUDDY_FLAG_GAME_INVITE_SENT;

        if (uNewFlags &  XBFRIENDS_FLAG_GAME_INVITE_REJECTED  )
        {
            pBuddy->uFlags |= BUDDY_FLAG_GAME_INVITE_REJECTED;
            //if got a reject, really, having sent flag on is bogus. 
            pBuddy->uFlags &= ~BUDDY_FLAG_GAME_INVITE_SENT; 
            if (uChangeFlags & XBFRIENDS_FLAG_GAME_INVITE_REJECTED )
                eAction = BUDDY_FLAG_GAME_INVITE_REJECTED;

        }
        
        if (uNewFlags &  XBFRIENDS_FLAG_GAME_INVITE_ACCEPTED  )
        {
            pBuddy->uFlags |= BUDDY_FLAG_GAME_INVITE_ACCEPTED;
            if (uChangeFlags & XBFRIENDS_FLAG_GAME_INVITE_ACCEPTED )
                eAction = BUDDY_FLAG_GAME_INVITE_ACCEPTED;
        }

        //If all  game related flags are off, and, it did change, then a Revoke 
        if (((uNewFlags & XBFRIENDS_MASK_GAME_INVITE_XBOX )== 0 ) && 
            ((uChangeFlags & XBFRIENDS_MASK_GAME_INVITE_XBOX) >0 ))
        { //it went away -- a revoke..
            eAction = BUDDY_FLAG_GAME_INVITE_REVOKE;
        } 
    }
    return(eAction);
...
//! When Xbox determines state of friends changes, we need to send msg to server.
static void _BuddyApiFriendsChangeCB(XbFriendsRefT * pXbfRef, int32_t count,int32_t bFirstEnumDone, XbFriendsChangeT *aChange, void * pData)
{
    int32_t i=0, rc=0;
    #define BUFF_SIZE  2000
    char strPacket [ BUFF_SIZE+100] ;
    char strBuff  [ BUFF_SIZE];
    char strField  [100];
    int32_t iPtr =0;
    uint32_t eAction=0;
    BuddyRosterT *pBuddy = NULL;
    BuddyApiRefT *pRef = ( BuddyApiRefT *)(pData);
    TagFieldClear(strPacket, sizeof(strPacket));
    strBuff[0] =0;
    ...
    for (i=0; i < count;i++)
    {   
        if ((count > XBFRIENDS_MAX_CHANGED_FRIENDS )  || (iPtr > (BUFF_SIZE - 100 )))
        { //too big -- send empty users, sever will  refresh all..
            strBuff[0] =0;
        }
        else
        {
            snzprintf ( strField, sizeof(strField), "%s|%c|%x|%x,", aChange[i].strGamertag, aChange[i].eAction,
                        aChange[i].uState, aChange[i].uChangeFlags );
            strcpy ( (strBuff+ iPtr), strField);
            iPtr += strlen(strField);
        }

        // Workaround Xbox server limitation -- It does not know buddy invites I sent out,
        // so we get from XDK and send a "roster msg" to ourselves.
        if ((aChange[i].eAction == ACTION_ADD ) && (aChange[i].uState & XBFRIENDS_FLAG_BUD_INVITE_SENT) )
        { //have invited someone to be my bud -- add to our list.
            BuddyApiMsgT msg;
            char strPacket [100]; 
            BuddyApiPrintf((pRef, "\n changeCb: INVITED Buddy: %s \n", aChange[i].strGamertag));
            TagFieldClear(strPacket, sizeof(strPacket));
            //use id that tell this is buddy list, not ignor/blocked list.
            TagFieldSetNumber(strPacket, sizeof(strPacket), "ID", ROSTER_BUDDY);
            TagFieldSetString(strPacket, sizeof(strPacket), "USER", aChange[i].strGamertag);
            TagFieldSetString(strPacket, sizeof(strPacket), "ATTR", RNOT_CHAR_SENT);
            msg.iType = 'ROST'; //??
            msg.iKind = 'ROST';  
            msg.iCode = 0;
            msg.pData = strPacket;
            _BuddyApiHandleROSTMsg(pRef, msg);
        }
        //--check what changed and called gameinvite callback..
        
        pBuddy = BuddyApiFind (pRef, aChange[i].strGamertag);
        // see if we had any pending game invites so we don't clobber them
        if ((pBuddy == NULL) && ((aChange[i].uState & XBFRIENDS_MASK_GAME_INVITE_XBOX) !=0))
        { //Find Game Invite flag set, before got the Roster from server -- so add it to list.
            BuddyRosterT newBuddy;
            memset(&newBuddy,0, sizeof(newBuddy));
            strnzcpy(newBuddy.strUser, aChange[i].strGamertag, sizeof(newBuddy.strUser));
            _BuddyApiAddOrSetFlags(pRef, &newBuddy);
            pBuddy = BuddyApiFind(pRef, aChange[i].strGamertag);
        }
        eAction = _setGameInviteFlags ( pRef, pBuddy,  aChange[i].uState, aChange[i].uChangeFlags);
        if ((eAction !=0 ) && (pRef->pBuddyChangeProc != NULL)  && (pBuddy)  )
        { //game invite changed state, treat like a GNOT..
            // tell caller of game notification state change.
            //in case dolt went off line and invited me, lets get title..
            if ( strlen(pBuddy->strTitle) == 0)
            {  //xbox only --if user was "appear offline", try to get it from XDK.   
                    uint32_t uTitleID = XbFriendsGetUserTitleID(pRef->pXbf, pBuddy->strUser);
                    BuddyApiGetTitleName( pRef, uTitleID,  sizeof(pBuddy->strTitle), pBuddy->strTitle);
            }
            (pRef->pBuddyChangeProc)(pRef, pBuddy, BUDDY_MSG_GAME_INVITE, eAction, pRef->pBuddyChangeData);
        }

        if ((aChange[i].eAction == ACTION_CHANGE ) && (aChange[i].uState & XBFRIENDS_FLAG_GAME_INVITE_SENT) )
        {//have invited bud to game -- add to list
          //Never get these, xbox cancels game invite if ya logout...
            BuddyApiPrintf((pRef, "\n FriendsChangeCB: GAME_INVITE_SENT was set Buddy: %s, state=%d \n", aChange[i].strGamertag, aChange[i].uState));
        }
    }

    //If first time, dont wear out server by sending TCKL, will just have lots of "ADDs".  
    if ( bFirstEnumDone != 0 )
    {
        TagFieldSetString(strPacket, sizeof(strPacket), "RSRC", pRef->strDomain);
        TagFieldSetString(strPacket, sizeof(strPacket), "USER", strBuff);
        _BuddyApiTransact(pRef, 'TCKL', strPacket);
    }
}

//! When Xbox determines state of friends changes, we need to send msg to server.
static void _BuddyApiFriendsMuteListCB(XbFriendsRefT * pXbfRef,  char *strMuteListCSV, void * pData)
{
    int32_t i=0, rc=0;

    char strPacket [ XBFRIENDS_MUTE_LIST_CSV_SIZE+30] ;
    if (strlen (strMuteListCSV) >0)
    {
        BuddyApiRefT *pRef = ( BuddyApiRefT *)(pData);
        TagFieldClear(strPacket, sizeof(strPacket));
        TagFieldSetString(strPacket, sizeof(strPacket), "RSRC", pRef->strDomain);
        TagFieldSetString(strPacket, sizeof(strPacket), "USER", strMuteListCSV);
        _BuddyApiTransact(pRef, 'MLST', strPacket);
    }
}
#endif
...
static int32_t  _BuddyApiHandleSearchedUserMsg(BuddyApiRefT *pState, BuddyApiMsgT msg)
{
    ...
    if ( TagFieldGetNumber(TagFieldFind(msg.pData, "ID"), 0)  == USCH_FIXED_ID)
    {
        ...
        {
            strnzcpy (pState->foundUsers[i].name, strTemp, sizeof  (pState->foundUsers[i].name )  );
...
#if BUDDYAPI_XBFRIENDS
    pState->pXbf = XbFriendsCreate ( pState->pBuddyAlloc, pState->pBuddyFree);

#if DIRTYCODE_DEBUG
    if (pState->pXbf)
    {
        XbFriendsSetDebugFunction(pState->pXbf, pState ,_BuddyApiPrintfCode );
    }
#endif
#endif
...
int32_t BuddyApiConnect(BuddyApiRefT *pState, const char *pServer, int32_t iPort, int32_t iTimeout, const char *pLKey, const char *pRsrc, BuddyApiCallbackT *pCallback, void *pCalldata)
{
    ...
    strnzcpy (pState->pServer , pServer, sizeof (pState->pServer ));
    strnzcpy (pState->pLKey, pLKey, sizeof (pState->pLKey ));
    strnzcpy (pState->pRsrc, pRsrc, sizeof (pState->pRsrc ));
    ...
    else
    {
        ...
        strnzcpy(strTemp, pLKey, sizeof(strTemp));
...
int32_t BuddyApiAdd(BuddyApiRefT *pState, BuddyRosterT *pBuddy, BuddyApiCallbackT *pCallback, void *pCalldata, int32_t iTimeout)
{
    ...
    if (pBuddy->uFlags & BUDDY_FLAG_IGNORE ) //check this first ---could mute a bud.
        rc = _BuddyApiDoXboxAction ( pState, pBuddy,0, BUDDY_INVITE_RESPONSE_MUTE);
    else if (pBuddy->uFlags & BUDDY_FLAG_BUDDY)
        rc = _BuddyApiDoXboxAction ( pState, pBuddy,0, BUDDY_INVITE_RESPONSE_INVITE ) ;
...
{
#if BUDDYAPI_XBFRIENDS
    int32_t rc =_BuddyApiDoXboxAction ( pState, pBuddy, 0, BUDDY_INVITE_RESPONSE_REMOVE +1 ) ;
#endif
...
#if BUDDYAPI_XBFRIENDS
    if ((pGameInviteState == NULL) || (strlen ( pGameInviteState ) == 0 ) )
        XbFriendsSetGameInviteStateString ( pState->pXbf, " "); //XDK does NOT like empty string..
    else
        XbFriendsSetGameInviteStateString ( pState->pXbf, pGameInviteState);    
    rc = _BuddyApiDoXboxAction( pState, pBuddy, 1,BUDDY_INVITE_RESPONSE_INVITE ) ;
#else
...
{
    int32_t rc= BUDDY_ERROR_UNSUPPORTED; //XBox only -- not for PS2
#if BUDDYAPI_XBFRIENDS
    rc = _BuddyApiDoXboxAction( pState, pBuddy, 1,BUDDY_INVITE_RESPONSE_JOIN ) ;
#endif
    return(rc);
}
...
#if BUDDYAPI_XBFRIENDS
                _BuddyApiDoXboxAction(pState, pBuddy,0, BUDDY_INVITE_RESPONSE_REMOVE);
#endif
                             ...
                             BuddyApiCallbackT *pCallback, void *pCalldata, int32_t iTimeout)
{
    ...
    if (strEmail != NULL )
    {
        strnzcpy ( pState->strEmail, strEmail, sizeof (pState->strEmail) );
    }
...
{
    if (strEmail != NULL )
        strnzcpy (strEmail, pState->strEmail, sizeof (pState->strEmail) );
...
{
    //ensure xbox calllbacks  dont fire, or else will try to alloc  buddy and mem etc. and blow up.
#if BUDDYAPI_XBFRIENDS
    XbFriendsSuspend (pState->pXbf );
#endif
}
...
{
   //ensure xbox callbacks fire,  
#if BUDDYAPI_XBFRIENDS
    XbFriendsResume (pState->pXbf );
#endif
}
/*F*************************************************************************************************/
/*!
...
#if BUDDYAPI_XBFRIENDS
        if ((  (pState->iShow == BUDDY_STAT_PASS) || (pState->iShow ==BUDDY_STAT_GAME) )
            && (iShow == BUDDY_STAT_CHAT ))
        {  //back, after a game..
            XbFriendsResume (pState->pXbf );
        }
        if ((iShow == BUDDY_STAT_PASS) || (iShow ==BUDDY_STAT_GAME))
        { //into game..
            XbFriendsSuspend (pState->pXbf );
        }
#endif
...
#if BUDDYAPI_XBFRIENDS
    //let xbox friends do its friend enumeration thing --
    // callback will be called if it changes
    if (pState->iStat == BUDDY_SERV_ONLN)      
    { //online, .. pump friends.
        XbFriendsUpdate (pState->pXbf );
    }
#endif
    ...
    { //pending packets
        ...
        {   
            ...
            { //server has connected,
                    //now do xbox stuff..
#if BUDDYAPI_XBFRIENDS
                pState->uTitleId = TagFieldGetNumber(TagFieldFind(msg.pData, "TIID"), -1);   
                XbFriendsSetMuteListCallback( pState->pXbf, _BuddyApiFriendsMuteListCB, pState);
                XbFriendsConnect ( pState->pXbf,0,  _BuddyApiFriendsChangeCB , pState );
                
                //initiate getting title name, as it takes a few pumps.
                //! Pull the language (int16_t) from a localizer token (int32_t)
                XbFriendsGetTitleName ( pState->pXbf, pState->uTitleId, 
                    LOBBYAPI_LocalizerTokenGetLanguage(pState->uLocale), 
                    sizeof(pState->strTitleName), pState->strTitleName );              
#else
        ...
        {  //get user settings/forwarding
            ...
            if (TagFieldGetNumber(TagFieldFind(msg.pData, "ID"), 0) == USERFWD_FIXED_ID)
            {
                if ( msg.iCode == 0)
                {
                    // no longer unkown, so clear the bit..
                    pState->uForwardFlags &= ~BUDDY_FORWARD_FLAG_UNKNOWN; 
...
#if BUDDYAPI_XBFRIENDS
    _BuddyApiDoXboxAction ( pState, pBuddy, 0, eResponse ) ;
#endif
                    ...
                    BuddyApiCallbackT *pCallback, void *pCalldata, int32_t iTimeout, int32_t iSessionId)
{
...
#if BUDDYAPI_XBFRIENDS
    {
        if (pBuddy)
        {
             rc = _BuddyApiDoXboxAction ( pState, pBuddy, 1, eResponse );
        }
    }
#else
...
{
#if BUDDYAPI_XBFRIENDS
    int32_t rc = 0, iPump = 0;
    char strTmp[100];
    memset(strTmp,0,sizeof(strTmp));
    while ((rc ==0)&& (uTitleId >0) &&(strlen(strTmp)== 0)&& iPump< 30)
    { //not there -- pump a few and try again.      
        XbFriendsUpdate( pState->pXbf);
        rc = XbFriendsGetTitleName ( pState->pXbf, uTitleId, 
            LOBBYAPI_LocalizerTokenGetLanguage(pState->uLocale), sizeof(strTmp), strTmp);
        iPump++;
    }
    strnzcpy(pTitleName, strTmp, iBufSize);
    return(rc);
#else
...
int32_t BuddyApiGetMyTitleName ( BuddyApiRefT  * pState,   const char * pLang, const int32_t iBufSize, char * pTitleName)
{
    int32_t rc=0; 
#if BUDDYAPI_XBFRIENDS
    if ( (strlen(pState->strTitleName)== 0 ) && (pState->uTitleId >0) )
    { //dont have, but have title id, so, go for it..
        rc = BuddyApiGetTitleName(pState, pState->uTitleId, sizeof(pState->strTitleName), pState->strTitleName);
    }
#endif
    strnzcpy ( pTitleName, pState->strTitleName, iBufSize);
...
#if BUDDYAPI_XBFRIENDS
// See if buddy is playing same title as mine--Xbox Only--works even when user is "appear offline".
int32_t BuddyApiXboxBuddyIsSameTitle(BuddyApiRefT *pState, BuddyRosterT *pBuddy)  
{ // check with XDK -it should know even if user is "appear offline" state. 
    uint32_t uTitleID;
    if (!pBuddy)
        return(0);

    uTitleID = XbFriendsGetUserTitleID( pState->pXbf, pBuddy->strUser);
    return(XbFriendsIsSameTitleID(pState->pXbf, uTitleID));
}
#endif
...
{
#if BUDDYAPI_XBFRIENDS
    XbFriendsSetGameInviteSessionID(pState->pXbf, sSessionID);
#else
    //PS2 just save it for when send the invite etc.
    strnzcpy(pState->strSEID, sSessionID, sizeof(pState->strSEID));
#endif
     return(BUDDY_ERROR_NONE);
}
...
{
#if BUDDYAPI_XBFRIENDS
    return(XbFriendsGetUserSessionID(pState->pXbf, strGamertag, sSessionID, iBufSize));
#else
    ...
    {
        strnzcpy(sSessionID, pBuddy->strSEID, iBufSize); 
        return(BUDDY_ERROR_NONE);
    }
#endif
}
...
#if BUDDYAPI_XBFRIENDS
//secret debug for getting lower level friends app pointer.
XbFriendsRefT * _BuddyApiGetFriendsRef ( BuddyApiRefT * pBudRef)
{
    return((pBudRef)? pBudRef->pXbf: 0);
}
#endif
...
{  //only for xbox, cannot do much for other platforms..
#if BUDDYAPI_XBFRIENDS
    uint32_t uTitleID =0;
    if ((!pBuddy) || (!pBudRef))
    {
    ...
    //if ( strlen(pBuddy->strTitle) == 0)
     //xbox only --if user was "appear offline", try to get it from XDK.
    uTitleID = XbFriendsGetUserTitleID(pBudRef->pXbf, pBuddy->strUser);
    if ((uTitleID == 0) )//&& (pBudRef->uTitleId != 0))
        return;
//        uTitleID = pBudRef->uTitleId;
    BuddyApiGetTitleName( pBudRef, uTitleID, sizeof(pBuddy->strTitle), pBuddy->strTitle);
#endif
}
