...
static int32_t _ConnApiUpdateGameClient(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex)
{
    ...
    if (pClient->GameInfo.eStatus == CONNAPI_STATUS_CONN)
    {
        ...
        if (NetTickDiff(NetTick(), pClient->iConnStart) > CONNAPI_CONNTIMEOUT)
        {
...
/*!
    ...
    \Description
        Switch to a presence enabled(or disabled) session.
...
{
    uint32_t uFlags, uFlags0;

    uFlags = ConnApiStatus(pConnApi, 'sflg', NULL, 0);
    uFlags0 = uFlags;

    if (uPresenceEnabled)
    {
        uFlags |= XSESSION_CREATE_USES_PRESENCE;
    }
    else
    {
        uFlags &= ~(XSESSION_CREATE_USES_PRESENCE);
    ...
    if (uFlags != uFlags0)
    {
        ConnApiControl(pConnApi, 'sflg', uFlags, 0, NULL);
