...
static void _HLBPrintfCode(HLBApiRefT *pHLBApi, const char *pFormat, ...)
{
    ...
    strnzcpy(strText, "hlbudapips3: ", sizeof(strText));
    iPrefixLen = strlen(strText);

    // parse the data
    va_start(args, pFormat);
    vsnzprintf(strText+iPrefixLen, sizeof(strText)-iPrefixLen, pFormat, args);
    va_end(args);
...
static int32_t _HLBRosterSort(void *pSortref, int32_t iSortcon, void *pRecptr1, void *pRecptr2)
{
    ...
    { 
        ...
        iResult = stricmp(pBuddy1->UserId.handle.data, pBuddy2->UserId.handle.data);
    }
    else
    { 
...
*/
/********************************************************************************F*/
static void _HLBSignalMsgAttachmentReceived(HLBApiRefT *pHLBApi, SceNpUserInfo *pBuddyData) //, HLBMessageT *pMessage)
{
    HLBBudT *pBuddy = NULL;
...
/*!
    ...
    \Input pFriendData          - poitner to friend data
...
static void _FriendApiNotificationCb(FriendApiRefT *pFriendApiRef, FriendApiEventsE eventType, FriendEventDataT *pEventData, FriendDataT *pFriendData, void *pNotifyCbUserData)
{
    ...
    if (pFriendData)
    {
        pUserInfo = (SceNpUserInfo *)(pFriendData->pRawData1);
    }
    ...
    switch(eventType)
    {
        ...
        case FRIENDAPI_EVENT_FRIEND_UPDATED:
            if (pEventData->bPresence == TRUE)
            {
                _HLBUpdateBuddy(pHLBApi, &pUserInfo->userId, TRUE, FALSE, pEventData->strMsgBuf);
            }
            else
            {
        ...
        case FRIENDAPI_EVENT_MSG_RCVED:
            _HLBReceiveMessage(pHLBApi, pUserInfo, pEventData->strMsgBuf, pEventData->iMsgSize);
            break;
        ...
        case FRIENDAPI_EVENT_MSG_ATTACH_RESULT:
            _HLBRecvMsgAttachmentResult(pHLBApi, pUserInfo, (SceNpBasicAttachmentData *)pEventData->strMsgBuf);
            break;
        default:
...
void HLBBudSetSceNpBasicEventCallback(HLBApiRefT *pHLBApi, HLBSceNpBasicEventCallbackT *pSceNpBasicEventCallback, void *pCallbackContext)
{
    ...
    // get the FriendApi ref from NetConn
    NetConnStatus('fref', 0, &pFriendApiRef, sizeof(pFriendApiRef));

    // register NP Basic events callback with friend api module
    FriendApiControl(pFriendApiRef, 'cbfp', 0, 0, (void *) pSceNpBasicEventCallback);
    FriendApiControl(pFriendApiRef, 'cbup', 0, 0, pCallbackContext);
}
...
void HLBBudGetPresence(HLBBudT *pBuddy , char *strPresence)
{
    if (pBuddy)
    {
        strnzcpy(strPresence, pBuddy->Presence.strTitleData, sizeof(pBuddy->Presence.strTitleData));
    }
}
