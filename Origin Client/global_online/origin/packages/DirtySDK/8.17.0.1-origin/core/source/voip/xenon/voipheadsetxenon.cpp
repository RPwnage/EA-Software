...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetXHVCreate(VoipHeadsetRefT *pHeadset, int32_t iMaxPeers, int32_t iMaxLocal)
{
    ...
    HRESULT error = XAudio2Create(&pXAudio2, 0, XAUDIO2_DEFAULT_PROCESSOR);
...
/*!
    ...
    \Output
        HRESULT - return code from GetLocalChatData/GetHighQualityLocalChatData
...
*/
/********************************************************************************F*/
VoipHeadsetRefT *VoipHeadsetCreate(int32_t iMaxChannels, int32_t iMaxLocal, VoipHeadsetMicDataCbT *pMicDataCb, VoipHeadsetStatusCbT *pStatusCb, void *pCbUserData)
{
    ...
    // start up XHV
    if (_VoipHeadsetXHVCreate(pHeadset, iMaxChannels, iMaxLocal) < 0)
    {
...
void VoipHeadsetReceiveVoiceDataCb(VoipUserT *pRemoteUser, VoipMicrPacketT *pMicrPacket, void *pUserData)
{
    ...
    for (uMicrPkt = 0; uMicrPkt < pMicrPacket->MicrInfo.uNumSubPackets; uMicrPkt++)
    {
        ...
        _VoipHeadsetSubmitVoiceData(pHeadset, &pRemoteUser[uRemoteIndex], 1, pDataPacket);
    }
}
