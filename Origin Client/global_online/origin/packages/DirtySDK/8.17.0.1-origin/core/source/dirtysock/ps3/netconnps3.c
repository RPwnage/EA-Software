...
typedef struct NetConnRefT
{
    ...
    //! critical section used to ensure thread safety in handlers registered with sony apis
    NetCritT        callbackCrit;
    ...
    uint32_t        uNumSubUser;        //!< number of sub-users signed-in
    SceNpId         *aSubUsers[4];      //NP Ids of singed-in sub-users
...
static uint64_t _NetConnGetAccountId(NetConnRefT *pRef)
{
    ...
    {
        // copy account id in local variable
        memcpy(&uNpAccountId, &TicketParam.u64, sizeof(uNpAccountId));
    }
    else
    {
...
static void _NetConnSubSigninCallback(int32_t iResult, SceNpId *pNpId, void *pData)
{
    ...
    {
        pRef->aSubUsers[pRef->uNumSubUser] = pNpId;
        pRef->uNumSubUser++;
        NetPrintf(("netconnps3: SubSignin success\n"));
    }
    ...
    size_t iCookieSize, const char *pEntitlementId, uint32_t uConsumedCount, NetConnTicketCallbackT *pCallback, void *pUserData)
{
    ...
    // reset ticket status to pending state
    ...
    // format serviceId if not specified
    if (pServiceId == NULL)
    {
        strnzcpy(pRef->strServiceId, pRef->strSpid, sizeof(pRef->strServiceId));
        strnzcat(pRef->strServiceId, "-", sizeof(pRef->strServiceId));
        strnzcat(pRef->strServiceId, pRef->strTitleId, sizeof(pRef->strServiceId));
    }
    else
    {
        strnzcpy(pRef->strServiceId, pServiceId, 32);
    }
    ...
    if (pRef->iTSafeTktReqStatus == -1)
    {
        ...
        NetPrintf(("netconnps3: requesting ticket with serviceId=%s, cookie=%s/%d, entitlement=%s, consumedcount=%d\n",
            pRef->strServiceId, pCookie, iCookieSize, pEntitlementId, uConsumedCount));

        iResult = sceNpManagerRequestTicket2(
            &pRef->NpId,            // const SceNpId *npId,
            &TicketVersion,         // const SceNpTicketVersion *version,
            pRef->strServiceId,     // const char *serviceId,
        ...
        {
            pRef->iTSafeTktReqStatus = 0;         // mark ticket req as pending
            iRetVal = 1;
        }
    }
    else
...
*/
/********************************************************************************F*/
static uint8_t *_NetConnTicketGet(NetConnRefT *pRef, int32_t *pTicketSize)
{
    int32_t iResult;
    size_t iTicketSize;
    uint8_t *pTicketBuf;
...
static void _NetConnTicketCallback(int32_t iResult, void *pUserData)
{
    ...
    if (iResult > 0)
    {
        ...
        pRef->uNpAccountId = _NetConnGetAccountId(pRef);
        ...
        // get the ticket
        pRef->pNpTicket = _NetConnTicketGet(pRef, &pRef->iNpTicketSize);
    }
    else if (iResult < 0)
    {
        ...
        if (!pRef->bNpTicketRequested && !pRef->bNoTicket)
        {
            ...
            if ((iResult = _NetConnIsTicketValid(pRef, &bValid)) == 0)
            {
                ...
                else
                {
                    ...
                    if ((iResult = _NetConnTicketRequest(pRef, 3, 0, NULL, NULL, 0, NULL, 0, _NetConnTicketCallback, pRef)) == 1)
                    {
...
int32_t NetConnStartup(const char *pParams)
{
    ...
    NetCritInit(&pRef->callbackCrit, "PS3 callback crit");
...
int32_t NetConnConnectSub(uint32_t uUserId)
{
    ...
    NetPrintf(("[jeff] sceNpManagerSubSignin returned 0x%08x\n", iRet));
...
int32_t NetConnDisconnectSub(void *pNpId)
{
    ...
    for(iIndex = 0; iIndex < (int)pRef->uNumSubUser; iIndex++)
    {
        if (pRef->aSubUsers[iIndex] == pNpId)
        {
            //replace with last;
            pRef->aSubUsers[iIndex] = pRef->aSubUsers[pRef->uNumSubUser - 1];
            break;
        }
    }

    pRef->uNumSubUser--;
    pRef->aSubUsers[pRef->uNumSubUser] = 0;
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            subn: Return the number of sub-users signed-in by DirtySDK
...
int32_t NetConnStatus(int32_t iKind, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    if (iKind == 'subn')
    {
        return(pRef->uNumSubUser);
    }
    if ((iKind == 'subu') && (iBufSize >= (int32_t)sizeof(SceNpId *)) && (iData < (int32_t)pRef->uNumSubUser))
    {
