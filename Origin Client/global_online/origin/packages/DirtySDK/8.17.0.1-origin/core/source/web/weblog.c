...
static void _WebLogFlush(WebLogRefT *pWebLog)
{
    ...
    if ((pWebLog->bLogging == TRUE) && (pWebLog->bPosting == FALSE))
    {
        char strUrl[256];
        // format url
        snzprintf(strUrl, sizeof(strUrl), "http://%s%s", pWebLog->strWebHost, pWebLog->strWebUrl);
...
WebLogRefT *WebLogCreate(int32_t iBufSize)
{
    ...
    strnzcpy(pWebLog->strWebHost, "easo.stest.ea.com:8080", sizeof(pWebLog->strWebHost));
    #if 0
    // stesteasoweb01 bypasses the VIP but is an internal address
    strnzcpy(pWebLog->strWebHost, "stesteasoweb01.pt.abn-iad.ea.com:8001", sizeof(pWebLog->strWebHost));
    // eggplant is the dev EAWS server
    strnzcpy(pWebLog->strWebHost, "eggplant.online.ea.com", sizeof(pWebLog->strWebHost));
    // eggplant:8001 bypasses Apache and goes directly to Tomcat
    strnzcpy(pWebLog->strWebHost, "eggplant.online.ea.com:8001", sizeof(pWebLog->strWebHost));
    #endif
    
    // set default URL
    strnzcpy(pWebLog->strWebUrl, "/tool/easo/logrequest.jsp", sizeof(pWebLog->strWebUrl));
...
void WebLogConfigure(WebLogRefT *pWebLog, const char *pServer, const char *pUrl)
{
    if (pServer != NULL)
    {
        strnzcpy(pWebLog->strWebHost, pServer, sizeof(pWebLog->strWebHost));
    }
    if (pUrl != NULL)
    {
        strnzcpy(pWebLog->strWebUrl, pServer, sizeof(pWebLog->strWebUrl));
    }
}
...
void WebLogPrintf(WebLogRefT *pWebLog, const char *pFormat, ...)
{
    ...
    iStrLen = vsnzprintf(strText, sizeof(strText), pFormat, pFmtArgs);
    va_end(pFmtArgs);
    
    // queue the text for sending if we are logging
    if (pWebLog->bLogging && !pWebLog->bUpdating)
    {
        strnzcat(pWebLog->strText, strText, pWebLog->iBufLen);
    }
