...
static void _HLBPrintfCode( HLBApiRefT *pApiRef, const char *pFormat, ...)
{
    ...
    va_end(args);
    snzprintf (finalText, sizeof(finalText), "HLB: %s \n", strText); //todo fixup -- we dont need
...
static BuddyApiMsgT *_cloneMsg(HLBApiRefT *pApiRef, const BuddyApiMsgT *pMsg)
{
    ...
    strnzcpy((char*)pClonedMsg->pData, pMsg->pData, iDataSize);
    return(pClonedMsg);
}
...
static void _preConnTempBudAdd (HLBApiRefT *pApiRef, const char * pName, int32_t eTempBudType)
{
    ...
    {         
        ...
        strnzcpy(pPCTBud->strName, pName, sizeof(pPCTBud->strName));
    }
...
//hack for offline testing: pMsg->iType= BUDDY_MSG_AUTHORIZE; pMsg->iCode = BUDDY_CODE_SUCCESS;
    ...
    default:
        {
            char message[128];
            snzprintf(message, sizeof(message), "Received unknown response '%c%c%c%c'",
                (char)pMsg->iType>>24,
                (char)pMsg->iType>>16,
                (char)pMsg->iType>>8,
                (char)pMsg->iType);
...
static int32_t _HLBListRemoveBuddy (   HLBApiRefT  *pApiRef, const char * name, uint32_t typeFlag, HLBOpCallbackT *cb , void * context)   
{
    ...
    strnzcpy(buddy.strUser, name, sizeof(buddy.strUser));
    ...
    { //zap it ... or change it, as case may be. API uses "del" to remove a flag, or really delete a buddy.
        ...
        strnzcpy (buddyName, pFoundHLB->pLLBud->strUser, sizeof (buddyName)); //save  name as LL bud can vanish here..
...
{   
    ...
    newLLBud = oldLLBud;
    strnzcpy (newLLBud.strUser , name, sizeof( newLLBud.strUser) );
...
int32_t HLBListCancelAllInvites ( HLBApiRefT  *pApiRef, const char *pName)
{
    ...
    {
        rc+=BuddyApiRespondGame(pApiRef->pLLRef, pBuddy->pLLBud, HLB_INVITEANSWER_REJECT, NULL, NULL, pApiRef->op_timeout, pApiRef->uGISessionId);
    }
    // check to see if I invited this buddy to a game
    if ((uGameFlags & HLB_FLAG_GAME_INVITE_SENT) > 0)
    {
        rc+=BuddyApiRespondGame(pApiRef->pLLRef, pBuddy->pLLBud, HLB_INVITEANSWER_REVOKE, NULL, NULL, pApiRef->op_timeout, pApiRef->uGISessionId);
    }
    if (  HLBBudIsIWannaBeHisBuddy (pBuddy))
    { //this may nuke the bud, so re-fetch
        rc +=BuddyApiRespondBuddy( pApiRef->pLLRef,   pBuddy->pLLBud,   HLB_INVITEANSWER_REVOKE, NULL , NULL, pApiRef->op_timeout);
        pBuddy = HLBListGetBuddyByName ( pApiRef,pName);
    }

    if ( (pBuddy) && (pBuddy->pLLBud) && HLBBudIsWannaBeMyBuddy (pBuddy))
    {
        rc+=BuddyApiRespondBuddy( pApiRef->pLLRef,   pBuddy->pLLBud,   HLB_INVITEANSWER_REJECT, NULL , NULL, pApiRef->op_timeout);
    }
    return(rc);
...
static int32_t _HLBRosterSort(void *pSortref, int32_t iSortcon, void *pRecptr1, void *pRecptr2)
{
    ...
    { // are same, so sort by name
        iResult = stricmp (pBuddy1->strUser, pBuddy2->strUser);
    }
    else
    ...
    { //game invite
        ...
        {  
            ...
            {  
                ...
                strnzcpy (pApiRef->strGIAcceptBuddy, HLBBudGetName (pBuddy), sizeof (pApiRef->strGIAcceptBuddy));
            }
        }
...
int32_t HLBApiConnect( HLBApiRefT  * pApiRef, const char * pServer, int32_t port, const  char *key, const char *pRsrc )
{
    ...
    {
        strRsrc[0] = '/';
        strnzcpy(&strRsrc[1], pRsrc, sizeof(strRsrc)-1);
        pRsrc = strRsrc;
    }
...
void HLBBudGetPresence( HLBBudT  *pBuddy , char * strPresence)
{
    ...
    {
        strnzcpy (strPresence,pBuddy->pLLBud->strPres, HLB_PRESENCE_LEN) ; 
    }
}
...
int32_t HLBListGetIndexByName(HLBApiRefT *pApiRef, const char * name)
{
    ...
    for ( index=0, max = HLBListGetBuddyCount ( pApiRef); index <max; index++ )
    {
        ...
        if ( ( pHLBBud ) && (stricmp(name, pHLBBud->pLLBud->strUser) == 0))
        {
            break;
        }
    }
...
int32_t HLBListInviteBuddy( HLBApiRefT  *pApiRef, const char * name , HLBOpCallbackT *cb , void * pContext )
{
    ...
    strnzcpy (llBud.strUser, name, sizeof (llBud.strUser));
                                        ...
                                        HLBOpCallbackT *cb , void * pContext)
{
    ...
    *buffPtr =0;
    ...
    strcpy(strResource, BuddyApiResource(pApiRef->pLLRef)); // add resource --check for / tho
                           ...
                           HLBOpCallbackT *cb, void * pContext)
{
    ...
    { 
        ...
        if (BuddyApiResource (pApiRef->pLLRef)[0]=='/' )
             strcpy( strTemp, BuddyApiResource (pApiRef->pLLRef)+1);
        else
            strcpy( strTemp, BuddyApiResource (pApiRef->pLLRef)); // add resource --check for / tho
        // locate the second divider  
        if ( ( pDiv = strchr(strTemp, '/'))  != NULL)
            *pDiv =0;

        snzprintf (strBody, sizeof(strBody), "%s/%s", name, strTemp);   
    ...
    strnzcpy (strBody, msg, pApiRef->max_msg_len );
