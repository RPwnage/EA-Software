...
//! format of a sync packet (piggybacks onto regular packets)
typedef struct NetGamePacketSyncT
{
    ...
    /*
      NOTE: If you change the size of this structure, you must also
      update NETGAME_DATAPKT_MAXTAIL in netgamepkt as well!  That
      define should be the size of the Sync structure plus one byte
      for the packet kind.
    */
} NetGamePacketSyncT;
...
/*!
    ...
    \Output
        NetGamePacketSyncT * - pointer to start of sync packet in buffer

    \Version 09/09/11 (jbrookes)
*/
/*************************************************************************************F*/
static NetGamePacketSyncT *_NetGameLinkGetSync(NetGamePacketT *pPacket, const uint8_t *pPacketData, NetGamePacketSyncT *pSync)
{
    // read sync size and locate sync packet
    uint32_t uSyncSize = (uint32_t)pPacketData[pPacket->head.len-1];
    pPacket->head.len -= uSyncSize;
    // clear sync packet before copy
    memset(pSync, 0, sizeof(*pSync));
    // size validation
    if (uSyncSize > sizeof(*pSync))
    {
        NetPrintf(("netgamelink: warning -- received a sync packet larger than expected (%d/%d)\n", uSyncSize, sizeof(*pSync)));
        uSyncSize = sizeof(*pSync);
    }
    if (uSyncSize < sizeof(*pSync))
    {
        NetPrintf(("netgamelink: warning -- received a sync packet smaller than expected (%d/%d)\n", uSyncSize, sizeof(*pSync)));
    }
    memcpy(pSync, pPacketData+pPacket->head.len, uSyncSize);
...
static int32_t _NetGameLinkSendPacket(NetGameLinkRefT *ref, NetGamePacketT *packet, uint32_t currtick)
{
    ...
    if (((packet->head.kind & GAME_PACKET_SYNC) || (currtick-ref->m_lastsync > KEEP_ALIVE_TIME/2)) && (ref->m_syncenabled == TRUE))
    {
        ...
        memcpy(packet->body.data+packet->head.len, &sync, sizeof(sync));
    ...
    if (((uPacketType < GAME_PACKET_STRM) || (uPacketType >= GAME_PACKET_ONE_PAST_LAST)) && (packet->head.kind != GAME_PACKET_SYNC))
    {
...
static int32_t _NetGameLinkRecvPacket0(NetGameLinkRefT *ref, uint32_t currtick)
{
    ...
    if (((kind < GAME_PACKET_STRM) || (kind >= GAME_PACKET_ONE_PAST_LAST)) && (packet->head.kind != GAME_PACKET_SYNC))
    {
    ...
    if (packet->head.kind & GAME_PACKET_SYNC)
    {
        ...
        NetGamePacketSyncT sync;
        _NetGameLinkGetSync(packet, packet->body.data, &sync);

        // remove sync bit
        packet->head.kind ^= GAME_PACKET_SYNC;

        // figure out elapsed time since last packet
        elapsed = when-ref->m_lastrecv;
        if (elapsed < 10)
            elapsed = 10;
        ...
        delta = (int16_t)(ref->m_lastrecv - SocketNtohl(sync.repl) + 1);
        ...
        {
            NetPrintf(("netgamelink: sync delta %d is out of range (m_lastrecv=%u,sync.repl=%u)\n", delta, ref->m_lastrecv, sync.repl));
        }

        // update remote peer's packets sent/recv stat tracker
        ref->m_stats.rpacksent += sync.psnt;
        // update packets received; this is done here to keep in sync with rpacksent
        ref->m_stats.lpackrcvd = ref->m_port->packrcvd;
    }

    // check for termination packets
    if (packet->head.kind == GAME_PACKET_DEAD)
    {
        // just disconnect
        ref->m_port->Unconnect(ref->m_port);
        return(1);
    }
...
static void _NetGameLinkProcess(NetGameLinkRefT *ref, uint32_t currtick)
{
    ...
    if (range >= 1000)
    {
        ...
        ref->m_stats.bps = (ref->m_inpbyte*1000)/range;
        ref->m_stats.rps = (ref->m_port->datasent-ref->m_inprawb)*1000;
        ref->m_stats.nps = ( (ref->m_port->datasent-ref->m_inprawb) + (ref->m_port->overhead-ref->m_overhead) )*1000;
        ref->m_stats.rps /= range;
        ref->m_stats.nps /= range;
        // calculate packets per second and raw packets per second
        ref->m_stats.pps = ((ref->m_inppack*1000)+500)/range;
        ref->m_stats.rpps = ((ref->m_port->packsent-ref->m_inprawp)*1000+500)/range;
...
static void _NetGameLinkSendCallback(CommRef *pComm, void *pPacket, int32_t iPacketLen, uint32_t uCurrTick)
{
    ...
    pSync = _NetGameLinkGetSync((NetGamePacketT *)&head, pPacketData, &sync);
...
/*!
    ...
    \Version    1.0        01/11/10 (jrainy) Pulled from netgamedist
*/
/*************************************************************************************************F*/
...
int32_t NetGameLinkSend(NetGameLinkRefT *ref, NetGamePacketT *pkt, int32_t len)
{
    ...
    while (len > 0)
    {
        // handle the different kinds
        if (pkt->head.kind == GAME_PACKET_DEAD)
        {
            ref->m_port->Unconnect(ref->m_port);
        }
        else if ((err = _NetGameLinkSendPacket(ref, pkt, currtick)) <= 0)
        {
        ...
        for (idx = 0;idx < ref->m_buflen;)
        {
            ...
            if (mask & (1 << iKind))
            {
                ...
                if ((pktsize <= len) || (len == 1))
                {
                    ...
                    memcpy(buf, ref->m_buffer + idx, pktsize);
                    len -= pktsize;
...
{
    uint32_t uDistMask =    (1 << GAME_PACKET_STRM) |
                            (1 << GAME_PACKET_INPUT) |
...
/*!
    ...
    \Version    1.0        07/07/03 (JLB) First Version
*/
/*************************************************************************************************F*/
int32_t NetGameLinkControl(NetGameLinkRefT *ref, int32_t iSelect, int32_t iValue, void *pValue)
{
     // $$ read mwid field -- this should be a NetGameLinkStatus() selector in V8
     if (iSelect == 'mwid')
     {
...
/*!
    ...
    \Version    1.0        01/11/10 (jrainy) First Version
*/
/*************************************************************************************************F*/
