...
static int32_t _ProtoAdvtSendPacket(ProtoAdvtRef *pRef, const ProtoAdvtPacketT *pPacket)
{
    ...
    return(SocketSendto(pRef->sock, (char *)pPacket, iPacketSize, 0, &pRef->addr, sizeof(pRef->addr)));
}
...
static int32_t _ProtoAdvtCallback(SocketT *sock, int32_t flags, void *_ref)
{
    ...
    while (ref->sock != NULL)
    {
        ...
        for (list = ref->snoopbuf; list != ref->snoopend; ++list)
        {
            ...
            if ((stricmp(packet.kind, list->packet.kind) == 0) &&
                (stricmp(packet.name, list->packet.name) == 0) &&
        ...
        if ((list == ref->snoopend) && (item != NULL))
        {
            ...
            memcpy(list->packet.seqn, packet.seqn, sizeof(packet.seqn));
            strnzcpy(list->packet.kind, packet.kind, sizeof(list->packet.kind));
            strnzcpy(list->packet.name, packet.name, sizeof(list->packet.name));
            strnzcpy(list->packet.note, packet.note, sizeof(list->packet.note));
            // indicate snoop buffer changed
            ref->snoop += 1;
            NetPrintf(("protoadvt: added new advert name=%s freq=%d addr=%a seqn=%02x%02x%02x%02x\n", list->packet.name, packet.freq, SockaddrInGetAddr(&from)));
        }

        // update the record (assuming we found/allocated)
        if (list != ref->snoopend)
        {
            strnzcpy(list->packet.addr, packet.addr, sizeof(list->packet.addr));
            strnzcpy(list->packet.note, packet.note, sizeof(list->packet.note));
                      ...
                      const char *note, const char *addr, int32_t freq)
{
    ...
    {
        // check for dupl
        if ((stricmp(kind, list->packet.kind) == 0) &&
            (stricmp(name, list->packet.name) == 0))
        {
            // update the addr field if necessary
            if (stricmp(addr, list->packet.addr) != 0)
            {
                // update address & force immediate broadcast of new info
                strnzcpy(list->packet.addr, addr, sizeof(list->packet.addr));
                list->timeout = NetTick()-1;
            }

            // update the note field if necessary
            if (stricmp(note, list->packet.note) != 0)
            {
                // update note & force immediate broadcast of new info
                strnzcpy(list->packet.note, note, sizeof(list->packet.note));
                list->timeout = NetTick()-1;
            }
            // all done
            return(0);
    ...
    strnzcpy(item->packet.kind, kind, sizeof(item->packet.kind));
    strnzcpy(item->packet.name, name, sizeof(item->packet.name));
    strnzcpy(item->packet.addr, addr, sizeof(item->packet.addr));
    strnzcpy(item->packet.note, note, sizeof(item->packet.note));
...
int32_t ProtoAdvtCancel(ProtoAdvtRef *ref, const char *kind, const char *name)
{
    ...
    for (list = ref->announce; list != NULL; list = list->next)
    {
        ...
        if (((kind == NULL) || (stricmp(kind, list->packet.kind) == 0)) &&
            ((name == NULL) || (stricmp(name, list->packet.name) == 0)))
        {
            // mark as deleted
            list->timeout = 0;
            break;
    ...
    for (list = ref->snoopbuf; list != ref->snoopend; ++list)
    {
        ...
        for (s = list->packet.addr; *s != 0;)
        {
            ...
            *t++ = 0;
            ...
            addr[3] = 0;
            if ((proto[0] != 0) && (strstr((char *)proto, addr) == NULL))
                continue;
            addr[3] = ':';
            // compile data into a record
            strnzcpy(record, list->packet.name, sizeof(record));
            for (t = record; *t != 0; ++t)
                ;
            *t++ = '\t';
                             ...
                             uint32_t *host, uint32_t defval)
{
    ...
    for (list = ref->snoopbuf; list != ref->snoopend; ++list)
    {
        ...
        if (list->local)
            continue;
        // see if the kind matches
        if ((kind != NULL) && (kind[0] != 0) && (stricmp(kind, list->packet.kind) != 0))
            continue;
        // make sure the name matches
        if ((name != NULL) && (name[0] != 0) && (stricmp(list->packet.name, name) != 0))
            continue;
...
ProtoAdvtRef *ProtoAdvtConstruct(int32_t limit)
{
    ...
    memset(ref->snoopbuf, 0, (char *)ref->snoopend-(char *)ref->snoopbuf);
    ...
    memcpy(&ref->addr, &peeraddr, sizeof(ref->addr));
