...
static unsigned char *_TagFieldSetupAppend(unsigned char *pRecord, int32_t iReclen, const char *pName, int32_t iMinData)
{
    ...
    for (; *pFind != 0; ++pFind)
    {
        ...
        if (*pFind == '=')
        {
            ...
            for (pNameCmp = (unsigned char *)pName; to_upper[*pFindCmp] == to_upper[*pNameCmp]; ++pFindCmp, ++pNameCmp)
                ;
            if (*pNameCmp == 0)
            {
                break;
...
int32_t TagFieldRename(char *_record, int32_t reclen, char *oldname, char *newname)
{
    ...
    diff = (int32_t)strlen(newname) - (data-head);
...
int32_t TagFieldMerge(char *_pRecord, int32_t iReclen, const char *_pMerge)
{
    ...
    const unsigned char *pMerge = (unsigned char *)_pMerge;
    unsigned char *pSrc;
    unsigned char *pDst;
    unsigned char *pName;
    ...
    for (pSrc = pDst = pName = pRecord; *pSrc != 0;)
    {
        ...
        *pSrc = 0;
        if (TagFieldFind((char *)pMerge, (char *)pName) == NULL)
        {
            *pDst++ = *pSrc++ = '=';
            continue;
        }
...
const char *TagFieldFind(const char *_pRecord, const char *_pName)
{
    ...
    const unsigned char *pName = (unsigned char *)_pName;
    const unsigned char *pRecord = (unsigned char *)_pRecord;
...
int32_t TagFieldSetRaw(char *_pRecord, int32_t iReclen, const char *pName, const char *_pData)
{
    ...
    unsigned char *pRecord = (unsigned char *)_pRecord;
    const unsigned char *pData = (unsigned char *)_pData;
...
int32_t TagFieldSetAddress(char *pRecord, int32_t iReclen, const char *pName, uint32_t uAddr)
{
    ...
    *pData = 0;
    ...
    // figure out length and setup for data append
    pAppend = _TagFieldSetupAppend((unsigned char *)pRecord, iReclen, pName, pData-strAddr);
    if (pAppend == NULL)
    {
        return(-1);
    }
...
int32_t TagFieldSetToken(char *pRecord, int32_t iReclen, const char *pName, int32_t iToken)
{
    ...
    *--pData = 0;
    ...
    pAppend = _TagFieldSetupAppend((unsigned char *)pRecord, iReclen, pName, strToken+sizeof(strToken)-pData-1);
    if (pAppend == NULL)
    {
        return(-1);
    }
...
int32_t TagFieldSetString(char *_pRecord, int32_t iReclen, const char *pName, const char *_pValue)
{
    ...
    unsigned char *pRecord = (unsigned char *)_pRecord;
    const unsigned char *pValue = (unsigned char *)_pValue;
...
int32_t TagFieldGetString(const char *data, char *buffer, int32_t buflen, const char *defval)
{
    ...
    {
        len = 0;
        for (ptr = (unsigned char *)data; (*ptr != term) && (*ptr >= ' '); ++ptr)
        {
...
int32_t TagFieldSetFloat(char *pRecord, int32_t iReclen, const char *pName, float fValue)
{
    ...
    *pData2++ = '.';
    ...
    // get the append point for max-size date format
    pAppend = _TagFieldSetupAppend((unsigned char *)pRecord, iReclen, pName, pData2-pData1);
    if (pAppend == NULL)
    {
        return(-1);
    }
...
int32_t TagFieldPrintf(char *_pRecord, int32_t iLength, const char *pFormat, ...)
{
    ...
    while (((uCh = *pFormat++) != 0) && (iLength > 1))
    {
        ...
        else if (uCh == '#')
        {
            ...
            iCount = snzprintf((char *)pRecord, iLength, "%d", iNum);
            pRecord += iCount;
            iLength -= iCount;
        }
...
int32_t TagFieldGetDelim(const char *data, char *buffer, int32_t buflen, const char *defval, int32_t index, int32_t delim)
{
    ...
    {
        len = 0;
        for (ptr = (unsigned char *)data; (*ptr != term) && (*ptr >= stop); ++ptr)
        {
            if ((ptr[0] == '%') && (ptr[1] >= ' ') && (ptr[2] >= ' '))
                ptr += 2;
            len += 1;
...
int32_t TagFieldFirst(const char *_record, char *nameptr, int32_t namelen)
{
    ...
    const unsigned char *record = (unsigned char *)_record;
...
{
    const unsigned char *pRec = (unsigned char *)pRecord;
