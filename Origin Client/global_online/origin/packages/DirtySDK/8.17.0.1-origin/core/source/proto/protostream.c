...
struct ProtoStreamRefT
{
    ...
    uint8_t bVerbose;                   //!< verbose mode (debug only)
...
/*!
    ...
    \Input *pProtoStream    - pointer to module state
    \Input iDataRead        - to be completed
...
static void _ProtoStreamUpdateStats(ProtoStreamRefT *pProtoStream, int32_t iDataRead)
{
    ...
    if ((iRecvElapsed >= 0) && (iRecvElapsed < PROTOSTREAM_SAMPLE_PERIOD))
    {
        ...
        if (pProtoStream->bVerbose)
        {
            NetPrintf(("protostream: dev=%5d avg=%5d len=%5d thr=%d\n",
...
static int32_t _ProtoStreamRecv(ProtoStreamRefT *pProtoStream)
{
    ...
    {
        // mark stream as complete
        NetPrintf(("protostream: stream finished\n"));
...
static int32_t _ProtoStreamDataCallback(ProtoStreamRefT *pProtoStream, ProtoStreamStatusE eStatus)
{
    ...
    // allow the user to grab some data
...
static void _ProtoStreamDataCallbackProcess(ProtoStreamRefT *pProtoStream)
{
    ...
    if (pProtoStream->uLastCallback != 0)
    {
        ...
        // only update if update rate exceeded
        if (NetTickDiff(uTick, pProtoStream->uLastCallback) < pProtoStream->iCallbackRate)
        {
            return;
        }
    }
...
*/
/********************************************************************************F*/
static int32_t _ProtoStreamOpen(ProtoStreamRefT *pProtoStream, const char *pUrl, int32_t iFreq)
{
    int32_t iResult;
    ...
    strnzcpy(pProtoStream->strUrl, pUrl, sizeof(pProtoStream->strUrl));

    // open stream    
    if ((iResult = ProtoHttpGet(pProtoStream->pProtoHttp, pUrl, PROTOHTTP_HEADBODY)) >= 0)
    {
...
ProtoStreamRefT *ProtoStreamCreate(int32_t iBufSize)
{
    ...
    memset(pProtoStream, 0, iRefSize);
    ...
    // for Xenon, set TCP receive buffer to 64k, as default 16k greatly constrains throughput
    #if DIRTYCODE_PLATFORM == DIRTYCODE_XENON
    ProtoHttpControl(pProtoStream->pProtoHttp, 'rbuf', 64*1024, 0, NULL);
    #endif
...
int32_t ProtoStreamOpen(ProtoStreamRefT *pProtoStream, const char *pUrl, int32_t iFreq)
{
    ...
    // open stream
    return(_ProtoStreamOpen(pProtoStream, pUrl, iFreq));
}
...
int32_t ProtoStreamRead(ProtoStreamRefT *pProtoStream, char *pBuffer, int32_t iBufLen, int32_t iMinLen)
{
    ...
    if (pProtoStream->iBufLen < iMinLen)
    {
        return(0);
    }
    ...
    if ((pProtoStream->iBufOut + iBufCopy) > pProtoStream->iBufSize)
    {
        ...
        memcpy(pBuffer, &pProtoStream->aBuffer[pProtoStream->iBufOut], iBufCopy2);
    ...
    // copy data to buffer
    memcpy(pBuffer, &pProtoStream->aBuffer[pProtoStream->iBufOut], iBufCopy);
    pBuffer += iBufCopy;
...
void ProtoStreamUpdate(ProtoStreamRefT *pProtoStream)
{
    ...
    if (pProtoStream->eState == ST_IDLE)
    {
        ...
        if ((pProtoStream->strUrl[0] != '\0') && (pProtoStream->iRestartTime != PROTOSTREAM_FREQ_ONCE))
        {
            // time to restart?
            if (NetTickDiff(NetTick(), pProtoStream->iRestartTime) > 0)
            {
                char strUrl[PROTOSTREAM_MAXURL];
                strnzcpy(strUrl, pProtoStream->strUrl, sizeof(strUrl));
                _ProtoStreamOpen(pProtoStream, strUrl, pProtoStream->iRestartFreq);
            }
        }
    }
