...
/*!
    ...
    \Notes
        ...
        References:
            [1] UPnP Device Architecture: http://www.upnp.org/download/UPnPDA10_20000613.htm
            [2] UPnP Resources: http://www.upnp.org/resources/default.asp
            [3] Intel Software for UPnP Technology: http://intel.com/technology/upnp/
            [4] Internet Gateway Device v1.0: http://www.upnp.org/standardizeddcps/igd.asp
            [5] SOAP 1.2: http://www.w3.org/TR/2003/REC-soap12-part0-20030624/
            [6] UPnP test program (Windows configuration): http://www.mgillespie.plus.com/
            [7] UPnP compatibility list: http://ccgi.mgillespie.plus.com/upnp/test.php
            [8] DLink UPnP supported routers: http://support.dlink.com/faq/view.asp?prod_id=879&question=General%20Routers
...
typedef struct ProtoUpnpDeviceT
{
    ...
    unsigned char       bDiscovered;        //!< TRUE if we've discovered a UPnP device
...
struct ProtoUpnpRefT
{
    ...
    ProtoUdpT       *pProtoUdp;             //!< udp module, used for discovery process
    ...
    uint8_t         bDiscSockBound;         //!< TRUE if discovery socket has been successfully bound, else FALSE
...
static const char *_ProtoUpnpIntToStr(int32_t iVal)
{
    ...
    snzprintf(_strVal, sizeof(_strVal), "%d", iVal);
    return(_strVal);
}
...
static const char *_ProtoUpnpFindUrl(const char *pUrl)
{
    ...
    if ((pUrl = stristr(pUrl, "http")) == NULL)
    {
        return(NULL);
    }

    // skip past double slash
    if ((pUrl = stristr(pUrl, "//")) == NULL)
    {
        return(NULL);
    }
    pUrl += 2;
...
static void _ProtoUpnpMakeFullUrl(ProtoUpnpDeviceT *pDevice, char *pBuffer, int iBufSize, const char *pUrl)
{
    ...
    if (strnicmp(pUrl, "http", 4))
    {
        strnzcpy(pBuffer, pDevice->strUrlBase, iBufSize);
        ...
        if (*pUrl != '/')
        {
            ...
            strnzcat(pBuffer, "/", iBufSize);
        }
    }
    strnzcat(pBuffer, pUrl, iBufSize);
}
...
static void _ProtoUpnpReset(ProtoUpnpRefT *pProtoUpnp)
{
    ...
    memset(&pProtoUpnp->Device, 0, sizeof(pProtoUpnp->Device));
...
static void _ProtoUpnpSendDiscoveryRequest(ProtoUpnpRefT *pProtoUpnp)
{
    ...
    // if we haven't tried binding discovery socket, do it now
    if (!pProtoUpnp->bDiscSockBound)
    {
        int32_t iResult;

        if ((iResult = ProtoUdpBind(pProtoUpnp->pProtoUdp, 0)) == SOCKERR_NONE)
        {
            #if DIRTYCODE_LOGGING
            struct sockaddr BindAddr;
            ProtoUdpGetLocalAddr(pProtoUpnp->pProtoUdp, (struct sockaddr_in *)&BindAddr);
            NetPrintf(("protoupnp: bound discovery socket to port %d\n", SockaddrInGetPort(&BindAddr)));
            #endif
            pProtoUpnp->bDiscSockBound = TRUE;
        }
        else
        {
    ...
    ProtoUdpSendTo(pProtoUpnp->pProtoUdp, _strDiscoveryPacket, sizeof(_strDiscoveryPacket),
        &pProtoUpnp->DiscoveryAddr);
}
...
static int32_t _ProtoUpnpParseDiscoveryResponse(ProtoUpnpRefT *pProtoUpnp, const char *pResponse)
{
    ...
    if (!stristr(pResponse, "urn:schemas-upnp-org:device:wanconnectiondevice"))
    {
    ...
    if ((pLocation = stristr(pResponse, strLocation)) == NULL)
    {
    ...
    {
        strsubzcpy(pProtoUpnp->Device.strUrlBase, sizeof(pProtoUpnp->Device.strUrlBase), pProtoUpnp->Device.strUrl, pUrl-pProtoUpnp->Device.strUrl);
...
static void _ProtoUpnpHttpReset(ProtoUpnpRefT *pProtoUpnp)
{
    ...
    memset(pProtoUpnp->strRecvBuf, 0, sizeof(pProtoUpnp->strRecvBuf));
...
static void _ProtoUpnpSoapFormatRequestHeader(ProtoUpnpRefT *pProtoUpnp, const char *pRequestName)
{
    ...
    snzprintf(strHdrExtra, sizeof(strHdrExtra),
...
static void _ProtoUpnpSoapRequestOpen(ProtoUpnpRefT *pProtoUpnp, const char *pRequestName)
{
    ...
    pProtoUpnp->iSendSize = snzprintf(pBuf, iBufSize,
    ...
    // save request name
    strnzcpy(pProtoUpnp->strRequestName, pRequestName, sizeof(pProtoUpnp->strRequestName));
}
...
static void _ProtoUpnpSoapRequestAdd(ProtoUpnpRefT *pProtoUpnp, const char *pName, const char *pType, const char *pData)
{
    ...
    if (pData[0] != '\0')
    {
        pProtoUpnp->iSendSize += snzprintf(pBuf, iBufSize, "   <%s>%s</%s>\r\n", pName, pData, pName);
    }
    else
    {
        pProtoUpnp->iSendSize += snzprintf(pBuf, iBufSize, "   <%s />\r\n", pName);
    }
    #endif
}
...
static void _ProtoUpnpSoapRequestPost(ProtoUpnpRefT *pProtoUpnp)
{
    ...
    ProtoHttpPost(pProtoUpnp->pProtoHttp,
...
static int32_t _ProtoUpnpSoapWaitResponse(ProtoUpnpRefT *pProtoUpnp)
{
    ...
    if (eResponse != PROTOHTTP_RESPONSE_SUCCESSFUL)
    {
        ...
        if (eResponse == PROTOHTTP_RESPONSE_SERVERERROR)
        {
            ...
            // find UPnPError
            if ((pXml = XmlFind((unsigned char *)pProtoUpnp->strRecvBuf, (unsigned char *)"%*:Envelope.%*:Body.%*:Fault.detail.UPnPError")) != NULL)
            {
                // get error code
                if ((pXml2 = XmlFind(pXml, (unsigned char *)".errorCode")) != NULL)
                {
                ...
                #if DIRTYCODE_LOGGING
                {
                    unsigned char strErrorText[96] = "unknown";
                    if ((pXml2 = XmlFind(pXml, (unsigned char *)".errorDescription")) != NULL)
                    {
                        XmlContentGetString(pXml2, strErrorText, sizeof(strErrorText), (unsigned char *)"");
                    }
...
*/
/********************************************************************************F*/
static int32_t _ProtoUpnpXmlGetAddress(const unsigned char *pXml, const char *pName, int32_t *pAddress)
{
    if ((pXml = XmlFind(pXml, (unsigned char *)pName)) != NULL)
    {
...
*/
/********************************************************************************F*/
static int32_t _ProtoUpnpXmlGetBoolean(const unsigned char *pXml, const char *pName, unsigned char *pBoolean)
{
    if ((pXml = XmlFind(pXml, (unsigned char *)pName)) != NULL)
    {
        *pBoolean = (unsigned char)XmlContentGetInteger(pXml, 0);
        return(0);
    }
    return(-1);
}
...
*/
/********************************************************************************F*/
static int32_t _ProtoUpnpXmlGetInteger(const unsigned char *pXml, const char *pName, int32_t *pInteger)
{
    if ((pXml = XmlFind(pXml, (unsigned char *)pName)) != NULL)
    {
...
*/
/********************************************************************************F*/
static int32_t _ProtoUpnpXmlGetString(const unsigned char *pXml, const char *pName, unsigned char *pBuffer, int32_t iBufSize)
{
    memset(pBuffer, 0, iBufSize);
    if ((pXml = XmlFind(pXml, (unsigned char *)pName)) != NULL)
    {
        return(XmlContentGetString(pXml, pBuffer, iBufSize, (unsigned char *)""));
    }
    return(-1);
}
...
{
    unsigned char strContentA[256], strContentB[256], strContentC[256];
    ProtoUpnpDeviceT *pDevice = &pProtoUpnp->Device;
    const unsigned char *pXml, *pXml2;

    // parse optional URLBase field if present
    if ((pXml = XmlFind((unsigned char *)pProtoUpnp->strRecvBuf, (unsigned char *)"root.URLBase")) != NULL)
    {
        // get urlbase string
        int32_t iUrlBaseLen = XmlContentGetString(pXml, (unsigned char *)pProtoUpnp->Device.strUrlBase, sizeof(pProtoUpnp->Device.strUrlBase), (unsigned char *)"");
        if (iUrlBaseLen > 0)
        {
    ...
    for (pXml = XmlFind((unsigned char *)pProtoUpnp->strRecvBuf, (unsigned char *)"root.device"); pXml != NULL; )
    {
        // get deviceType
        pXml2 = XmlFind(pXml, (unsigned char *)".deviceType");

        // get deviceType contents
        XmlContentGetString(pXml2, strContentA, sizeof(strContentA), (unsigned char *)"");
        NetPrintf(("protoupnp:    deviceType=%s\n", strContentA));

        // is this the deviceType we are looking for?
        if (stristr((char *)strContentA, "WANConnectionDevice"))
        {
            // get UDN
            _ProtoUpnpXmlGetString(pXml2, "UDN", (unsigned char *)pDevice->strUdn, sizeof(pDevice->strUdn));
            ...
            // format model name
            snzprintf(pProtoUpnp->Device.strModel, sizeof(pProtoUpnp->Device.strModel), "%s %s %s",
        ...
        if ((pXml2 = XmlFind(pXml, (unsigned char *)".deviceList.device")) == NULL)
        {
    ...
    for (pXml = XmlFind(pXml, (unsigned char *)".serviceList.service"); pXml != NULL; pXml = XmlNext(pXml))
    {
        ...
        if (_ProtoUpnpXmlGetString(pXml, ".serviceType", strContentB, sizeof(strContentB)) >= 0)
        {
            ...
            if (!stristr((char *)strContentB, "connection"))
            {
                continue;
            }
            ...
            if (_ProtoUpnpXmlGetString(pXml, ".controlURL", strContentA, sizeof(strContentA)) >= 0)
            {
                ...
                // create controlURL
                _ProtoUpnpMakeFullUrl(&pProtoUpnp->Device, pService->strControlUrl, sizeof(pService->strControlUrl), (char *)strContentA);
                NetPrintf(("protoupnp:       controlURL=%s\n", pService->strControlUrl));

                // create serviceURL
                _ProtoUpnpXmlGetString(pXml, ".SCPDURL", strContentA, sizeof(strContentA));
                _ProtoUpnpMakeFullUrl(&pProtoUpnp->Device, pService->strDescriptionUrl, sizeof(pService->strDescriptionUrl), (char *)strContentA);
                NetPrintf(("protoupnp:       SCPDURL=%s\n", pService->strDescriptionUrl));

                // save serviceType
                strnzcpy(pService->strServiceType, (char *)strContentB, sizeof(pService->strServiceType));
...
/*F********************************************************************************/
/*!
    \Function _ProtoUpnpParseXmlGetExtAddr
...
static int32_t _ProtoUpnpXmlParseGetExtAddr(ProtoUpnpRefT *pProtoUpnp)
{
    ...
    if ((pXml = XmlFind((unsigned char *)pProtoUpnp->strRecvBuf, (unsigned char *)"%*:Envelope.%*:Body.%*:GetExternalIPAddressResponse")) != NULL)
    {
...
/*F********************************************************************************/
/*!
    \Function _ProtoUpnpParseXmlGetPortMapping
...
static void _ProtoUpnpXmlParseGetPortMapping(ProtoUpnpRefT *pProtoUpnp)
{
    ...
    if ((pXml = XmlFind((unsigned char *)pProtoUpnp->strRecvBuf, (unsigned char *)"%*:Envelope.%*:Body.%*:GetSpecificPortMappingEntryResponse")) != NULL)
    {
        // parse addr
        ...
        // parse description
        _ProtoUpnpXmlGetString(pXml, ".NewPortMappingDescription", (unsigned char *)pProtoUpnp->Device.CurPortMap.strDesc,
...
ProtoUpnpRefT *ProtoUpnpCreate(void)
{
    ...
    memset(pProtoUpnp, 0, sizeof(*pProtoUpnp));
    pProtoUpnp->iMemGroup = iMemGroup;
    pProtoUpnp->pMemGroupUserData = pMemGroupUserData;

    // allocate ProtoUdp module
    if ((pProtoUpnp->pProtoUdp = ProtoUdpCreate(1024, 4)) == NULL)
    {
        NetPrintf(("protoupnp: unable to create protoudp module\n"));
    ...
    if ((pProtoUpnp->pProtoHttp = ProtoHttpCreate(1024)) == NULL)
    {
        ...
        ProtoUdpDestroy(pProtoUpnp->pProtoUdp);
        DirtyMemFree(pProtoUpnp, PROTOUPNP_MEMID, pProtoUpnp->iMemGroup, pProtoUpnp->pMemGroupUserData);
        return(NULL);
    }
...
int32_t ProtoUpnpStatus(ProtoUpnpRefT *pProtoUpnp, int32_t iSelect, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'body')
    {
        strnzcpy(pBuf, pProtoUpnp->strRecvBuf, iBufSize);
        return(0);
    }
    ...
    if (iSelect == 'dnam')
    {
        strnzcpy(pBuf, pProtoUpnp->Device.strModel, iBufSize);
        return(0);
    }
    if (iSelect == 'done')
    {
    ...
    if (iSelect == 'durn')
    {
        strnzcpy(pBuf, pProtoUpnp->Device.strUdn, iBufSize);
        return(0);
    }
    ...
    if (iSelect == 'macr')
    {
        ...
        if (pBuf != NULL)
        {
            strnzcpy(pBuf, pProtoUpnp->strRequestName, iBufSize);
        }
        return(iMacro);
    }
    ...
    if (iSelect == 'rbdy')
    {
        strnzcpy(pBuf, pProtoUpnp->strSendBuf, iBufSize);
        return(0);
    }
...
int32_t ProtoUpnpControl(ProtoUpnpRefT *pProtoUpnp, int32_t iControl, int32_t iValue, int32_t iValue2, const void *pValue)
{
    ...
    if (iControl == 'fake')
    {
        ...
        strnzcpy(pProtoUpnp->strRecvBuf, pValue, sizeof(pProtoUpnp->strRecvBuf));
    ...
    */
    ...
    if (iControl == 'desc')
    {
        ...
        strnzcpy(pProtoUpnp->strRequestName, "Description", sizeof(pProtoUpnp->strRequestName));
        _ProtoUpnpHttpReset(pProtoUpnp);
    ...
    if (iControl == 'sdsc')
    {
        ...
        strnzcpy(pProtoUpnp->strRequestName, "ServiceDesc", sizeof(pProtoUpnp->strRequestName));
        _ProtoUpnpHttpReset(pProtoUpnp);
...
void ProtoUpnpUpdate(ProtoUpnpRefT *pProtoUpnp)
{
    ...
    if (pProtoUpnp->eState == ST_DISCOVERY)
    {
        ...
        if ((iCurTick - pProtoUpnp->iDiscoveryTimer) >= PROTOUPNP_DISCOVERYINTERVAL)
        {
            // send discovery request
            _ProtoUpnpSendDiscoveryRequest(pProtoUpnp);

            // set timer for next broadcast
            pProtoUpnp->iDiscoveryTimer += PROTOUPNP_DISCOVERYINTERVAL;
        }

        // update udp module
        ProtoUdpUpdate(pProtoUpnp->pProtoUdp);

        // poll for discovery response
        if ((iResponseSize = ProtoUdpRecvFrom(pProtoUpnp->pProtoUdp, pProtoUpnp->strRecvBuf, 1024, &RemoteAddr)) > 0)
        {
