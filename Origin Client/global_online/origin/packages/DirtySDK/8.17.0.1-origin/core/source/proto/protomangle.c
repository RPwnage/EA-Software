...
#define PROTOMANGLE_RAND_PORTRANGE  (8000)
...
static void _ProtoMangleSockClose(ProtoMangleRefT *pRef, uint32_t bRelease)
{
    ...
    {
        if ((pRef->pProbeSock != pRef->pSharedProbeSock) && (pRef->pProbeSock != NULL))
        {
            NetPrintf(("protomangle: closing probe socket 0x%08x\n", (intptr_t) pRef->pProbeSock));
            SocketShutdown(pRef->pProbeSock, 2);
            SocketClose(pRef->pProbeSock);
            pRef->pProbeSock = NULL;
        }
        else if (bRelease)
        {
            NetPrintf(("protomangle: releasing shared probe socket 0x%08x\n", (intptr_t) pRef->pProbeSock));
...
static int32_t _ProtoMangleHttpPost(ProtoMangleRefT *pRef, const char *pUrl, const char *pCookie, const char *pData)
{
    ...
    snzprintf(strBuffer, sizeof(strBuffer),
        "Cookie: sessionID=%s\r\n"
    ...
    // setup the url
    snzprintf(strBuffer, sizeof(strBuffer), "http://%s:%d/%s", pRef->strServer, pRef->iServerPort, pUrl);
...
{
    char strAppend[256], strUrl[256], strAddrText[20];

    // setup the url
    snzprintf(strUrl, sizeof(strUrl), "http://%s:%d/getPeerAddress?myIP=%s&myPort=%d&version=1.0",
        pRef->strServer, pRef->iServerPort, SocketInAddrGetText(pRef->iHostAddr, strAddrText, sizeof(strAddrText)), pRef->iGamePort);

    // setup the extra header fields
    snzprintf(strAppend, sizeof(strAppend), "Cookie: sessionID=%s\r\n", pRef->strCookie);
...
static int32_t _ProtoMangleParseProbeRequest(ProtoMangleProbeT *pProbe, ProtoMangleRefT *pRef, const char *pSrc)
{
    ...
    {
        strnzcpy(pProbe->strTag, strParm, sizeof(pProbe->strTag));
        bLinesParsed++;
    }
...
static int32_t _ProtoMangleSetupProbeSocket(ProtoMangleRefT *pRef, ProtoMangleProbeT *pProbe)
{
    ...
    {
        NetPrintf(("protomangle: using shared socket ref 0x%08x to probe port %d\n", (intptr_t)pRef->pSharedProbeSock, pRef->iGamePort));
    ...
    else
    {
        ...
        {
            NetPrintf(("protomangle: error creating probe socket\n"));
            return(-1);
        }

        NetPrintf(("protomangle: created probe socket 0x%08x\n", (intptr_t) pRef->pProbeSock));
        ...
        // bind locally
        SocketBind(pRef->pProbeSock, &BindAddr, sizeof(BindAddr));
    }
...
static void _ProtoMangleSendProbe(ProtoMangleRefT *pRef, ProtoMangleProbeT *pProbe, int32_t iSrcPort, int32_t iProbeReq)
{
    ...
    snzprintf(strMesg, sizeof(strMesg), "sourceIP=%s\r\nsourcePort=%d\r\ntag=%s\r\nsendCount=%d\r\n",
    ...
    if (iErr == iMsgSize)
    {
        NetPrintf(("protomangle: success "));
    }
    else
    {
        NetPrintf(("protomangle: error %d ",iErr));
    }
...
static int32_t _ProtoMangleHandleProbeRequest(ProtoMangleRefT *pRef, const char *pSrc)
{
    ...
    for (iProbeReq = 1; _ProtoMangleParseProbeRequest(&Probe, pRef, pSrc) != FALSE; iProbeReq++)
    {
        ...
        {
            NetPrintf(("protomangle: warning, probe sequence mismatch\n"));
        }
...
ProtoMangleRefT *ProtoMangleCreate(const char *pServer, int32_t iPort, const char *pGameID, const char *pLKey)
{
    ...
    strnzcpy(pRef->strServer, pServer, sizeof(pRef->strServer));
    pRef->iServerPort = iPort;
    strnzcpy(pRef->strGameID, pGameID, sizeof(pRef->strGameID));
    strnzcpy(pRef->strLKey, pLKey, sizeof(pRef->strLKey));
...
{
    NetPrintf(("protomangle: shutting down\n"));
...
/*!
    ...
    \Version    1.0        04/07/03 (JLB) First Version
*/
/*************************************************************************************************F*/
void ProtoMangleConnect(ProtoMangleRefT *pRef, int32_t iMyPort, const char *pSessID)
{
    ...
    // save cookie
    strnzcpy(pRef->strCookie, pSessID, sizeof(pRef->strCookie));
...
void ProtoMangleConnectSocket(ProtoMangleRefT *pRef, intptr_t uSockRef, const char *pSessID)
{
    ...
    {
        NetPrintf(("protomangle: unable to import sockref 0x%08x\n", uSockRef));
        return;
    }
    ...
    NetPrintf(("protomangle: created shared socket using platform socket ref 0x%08x bound to port %d\n", uSockRef, pRef->iGamePort));
...
void ProtoMangleUpdate(ProtoMangleRefT *pRef)
{
    ...
    if (pRef->eMangleState == ST_MNGL_WAIT)
    {
        ...
        if ((iRecvLen = ProtoHttpRecvAll(pRef->pHttp, pRef->strBuf, sizeof(pRef->strBuf))) >= 0)
        {
            ...
            // get data
            NetPrintf(("protomangle: server response\n"));
...
{
    char strData[128], strAddrText[20];
    ...
    snzprintf(strData, sizeof(strData), "myIP=%s&myPort=%d&version=1.0&status=%s&gameFeatureID=%s\n",
    ...
    {
        char strData2[32];
        snzprintf(strData2, sizeof(strData2), "&latency=%d", iLatency);
        strnzcat(strData, strData2, sizeof(strData));
    }
