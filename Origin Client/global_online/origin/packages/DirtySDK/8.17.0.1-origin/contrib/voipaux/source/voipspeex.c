...
#ifdef _WIN64
#define VOIPSPEEX_ENABLED (FALSE)
#else
#define VOIPSPEEX_ENABLED (TRUE)
#endif
...
#ifndef VOIPSPEEX_DTX
#define VOIPSPEEX_DTX           (1)   // discontinuous transmission
#endif
...
//!  number of frames of silence that must be detected before we stop transmitting ($$TODO should be rate dependent)
#define VOIPSPEEX_VADFRAMES     (8)
...
static VoipCodecRefT *_VoipSpeexCreate(int32_t iDecodeChannels)
{
    ...
    memset(pState, 0, sizeof(VoipSpeexStateT));
    ...
    pState->fPowerThreshold = 0.00015f;
...
static int32_t _VoipSpeexEncodeBlock(VoipCodecRefT *pCodecState, unsigned char *pOut, const int16_t *pInp, int32_t iNumSamples)
{
    ...
    for (iFrame = 0; iFrame < iNumFrames; iFrame++)
    {
        ...
        for (iSample = 0; iSample < VOIPSPEEX_FRAME_SIZE; iSample++)
        {
            ...
            fAmp = (float)abs(iFrameBuffer[iSample]);
            fPowerSum += (fAmp * fAmp);
        }
...
#if VOIPSPEEX_DTX
    fPower = (fPowerSum / (32768.0f * 32768.0f * (float)VOIPSPEEX_FRAME_SIZE * (float)iNumFrames));

    if (fPower < pState->fPowerThreshold)
    {
        if (++pState->iDtxCount == VOIPSPEEX_VADFRAMES)
        {
            NetPrintfVerbose((pState->iDebugLevel, 1, "voipspeex: silence detected\n"));
        }
        if (pState->iDtxCount > VOIPSPEEX_VADFRAMES)
        {
            iNumBytesEncoded = 0;
    ...
    else if (pState->iDtxCount != 0)
    {
        if (pState->iDtxCount >= VOIPSPEEX_VADFRAMES)
        {
            NetPrintfVerbose((pState->iDebugLevel, 1, "voipspeex: voice detected\n"));
        }
        pState->iDtxCount = 0;
    }
#endif
...
static int32_t _VoipSpeexStatus(VoipCodecRefT *pCodecState, int32_t iSelect, int32_t iValue, void *pBuffer, int32_t iBufSize)
{
    ...
    NetPrintf(("voipspeex: unhandled status selector '%C'\n", iSelect));
    return(-1);
}
