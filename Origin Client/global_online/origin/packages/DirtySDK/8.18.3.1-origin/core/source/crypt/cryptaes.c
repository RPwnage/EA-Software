...
*/
/********************************************************************************F*/
static void _CryptAesInit(CryptAesT *pAes, const uint8_t *pKeyBuf, int32_t iKeyLen, const uint8_t *pInitVec)
{
    ...
    // save in state
    pAes->uNumRounds = uNumRounds;
    pAes->uKeyWords = uKeyWords;
    
    // copy key to key state
    for (uWord = 0, pState = pAes->aKeySchedule; uWord < uKeyWords; uWord += 1, pKeyBuf += 4)
    {
        pState[uWord] = ((uint32_t)pKeyBuf[0]<<24) | ((uint32_t)pKeyBuf[1]<<16) | ((uint32_t)pKeyBuf[2]<<8) | ((uint32_t)pKeyBuf[3]);
    }
    
    // key expansion
    for (uWord = uKeyWords, uCount = (pAes->uNumRounds+1) * 4; uWord < uCount; uWord += 1)
    {
    ...
    memcpy(pAes->aInitVec, pInitVec, sizeof(pAes->aInitVec));
}
...
*/
/********************************************************************************F*/
static void _CryptAesInvertKey(CryptAesT *pAes)
{
    uint32_t uCount, *pState;
    for (uCount = 4, pState = pAes->aKeySchedule; uCount < (unsigned)pAes->uNumRounds*4; uCount += 1)
    {
...
*/
/********************************************************************************F*/
static void _CryptAesEncrypt(const CryptAesT *pAes, uint32_t *pData)
{
    const uint32_t *pState = pAes->aKeySchedule;
    const uint8_t *pSbox = _CryptAes_Sbox;
    ...
    for (uRound = 0, uNumRounds = pAes->uNumRounds; uRound < uNumRounds; uRound += 1)
    {
        // ByteSub+ShiftRow
...
*/
/********************************************************************************F*/
static void _CryptAesDecrypt(const CryptAesT *pAes, uint32_t *pData)
{
    uint32_t uRound, uNumRounds = pAes->uNumRounds;
    const uint32_t *pState = pAes->aKeySchedule + ((uNumRounds + 1) * 4);
...
void CryptAesInit(CryptAesT *pAes, const uint8_t *pKeyBuf, int32_t iKeyLen, uint32_t uKeyType, const uint8_t *pInitVec)
{
    ...
    // init state
    _CryptAesInit(pAes, pKeyBuf, iKeyLen, pInitVec);
    // if decrypt, invert key schedule
    if (uKeyType == CRYPTAES_KEYTYPE_DECRYPT)
    {
        _CryptAesInvertKey(pAes);
    }
}
