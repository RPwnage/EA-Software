...
/*!
    ...
    \Input bClearStats      - TRUE to wipe stats after display, else FALSE
...
*/
/********************************************************************************F*/
static int32_t _HttpManagerDisplayStats(HttpManagerRefT *pHttpManager, uint8_t bClearStats)
{
    // display stats
...
static int32_t _HttpManagerReceiveHeaderCb(ProtoHttpRefT *pState, const char *pHeader, uint32_t uHeaderSize, void *pUserRef)
{
   ...
   if (PROTOHTTP_GetResponseClass(iResult) != PROTOHTTP_RESPONSE_SUCCESSFUL)
   {
    ...
    if (ProtoHttpStatus(pState, 'plst', NULL, 0))
    {
...
static int32_t _HttpManagerResizeInputBuffer(HttpManagerRefT *pHttpManager, HttpManagerHttpRefT *pHttpRef)
{
    ...
    iCurrSize = ProtoHttpStatus(pHttpRef->pProtoHttp, 'imax', NULL, 0);
...
static int32_t _HttpManagerRequestPipe(HttpManagerRefT *pHttpManager, HttpManagerHttpRefT *pHttpRef, HttpManagerHttpCmdT *pHttpCmd)
{
    ...
    // set timeout if requested (only for the first request)
...
static int32_t _HttpManagerCreateRef(HttpManagerRefT *pHttpManager, int32_t iHttpRef)
{
    ...
    // set up callbacks
    ProtoHttpCallback(pHttpRef->pProtoHttp, _HttpManagerCustomHeaderCb, _HttpManagerReceiveHeaderCb, pHttpRef);
    // init to idle state
    pHttpRef->uHttpState = HTTPMANAGER_REFSTATE_IDLE;
...
void HttpManagerDestroy(HttpManagerRefT *pHttpManager)
{
    ...
    // display stats
    #if DIRTYCODE_LOGGING || HTTPMANAGER_FINALDEBUG
    _HttpManagerDisplayStats(pHttpManager, FALSE);
    #endif
...
/*!
    ...
    \Description
        HttpManager control function.  Different selectors control different behaviors.
    ...
    \Notes
        If a handle is specified, HttpManagerControl() will call ProtoHttpControl() for
        the appropriate ref.  HttpManager control selectors are as follows:

        \verbatim
            SELECTOR    DESCRIPTION
            'auto'      enable/disable auto-update (polling of HttpManagerUpdate by NetConnIdle(); default enabled)
            'cbup'      sets callback user data pointer (pValue=callback)
            'copy'      sets whether urls are copied internally or not (default=true)
            ...
            'xhtp'      enable xhttp (xbox 360 only, global setting only) (default=disabled)
        \endverbatim
...
int32_t HttpManagerControl(HttpManagerRefT *pHttpManager, int32_t iHandle, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    // resolve http command from handle?
    if (iHandle > 0)
    {
        HttpManagerHttpCmdT *pHttpCmd;
        if ((pHttpCmd = _HttpManagerGetCmd(pHttpManager, iHandle)) == NULL)
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'stat'      (Debug only) Display httpmanager stats
            'stcl'      (Debug only) Display httpmanager stats and clear them
...
int32_t HttpManagerStatus(HttpManagerRefT *pHttpManager, int32_t iHandle, int32_t iSelect, void *pBuffer, int32_t iBufSize)
{
    ...
    else
    {
        ...
        #if DIRTYCODE_LOGGING || HTTPMANAGER_FINALDEBUG
        // display and clear stats
        if (iSelect == 'stcl')
        {
            return(_HttpManagerDisplayStats(pHttpManager, TRUE));
        }
        if (iSelect == 'stat')
        {
            return(_HttpManagerDisplayStats(pHttpManager, FALSE));
        }
        #endif
    }
    // unhandled
