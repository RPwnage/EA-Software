...
static void _SocketRead(SocketStateT *pState, SocketT *pSocket)
{
    ...
    {
        NetPrintf(("dirtynetpsp2: trying to read from unsupported socket type %d\n", pSocket->type));
        return;
    }
...
/*!
    ...
    \Input  iParam      - index of CPU to start socket thread on
...
int32_t SocketCreate(int32_t iParam)
{
    ...
    // print version info
    NetPrintf(("dirtynetpsp2: DirtySock SDK v%d.%d.%d.%d\n",
