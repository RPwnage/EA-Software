...
/*!
    ...
    \Description
        ...
        online. It is more important on the PS2 than the other platforms (though it
        works on all) because this module has an EE side interface while the lower
        layer "comm" modules it manages do not.
...
*/
/*************************************************************************************F*/
static ProtoAdvtRef *_NetGameUtilAdvtConstruct(NetGameUtilRefT *pRef, int32_t iSize)
{
    ProtoAdvtRef *pAdvt;
    DirtyMemGroupEnter(pRef->memgroup, pRef->memgrpusrdata);
    pAdvt = ProtoAdvtConstruct(iSize);
    DirtyMemGroupLeave();
    return(pAdvt);
}
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'mwid': set maximum packet width (must be <= NETGAME_MAXPACKETSIZE)*
                * must be set before NetGameLinkConnect() is called to be effective.
        \endverbatim

    \Version    1.0        11/12/03 (JLB) First Version
*/
/*************************************************************************************************F*/
...
/*!
    ...
    \Output
        int32_t         - zero=success, negative=failure (COMM_* error code)
...
int32_t NetGameUtilConnect(NetGameUtilRefT *ref, int32_t conn, const char *addr, CommAllConstructT *pConstruct)
{
    ...
    strnzcpy(ref->addr, addr, sizeof(ref->addr));
    // save host/join mode
    ...
    if ((conn & NETGAME_CONN_AUTO) == NETGAME_CONN_AUTO)
    {
        ...
        if (ref->find == NULL)
        {
            ref->find = _NetGameUtilAdvtConstruct(ref, 8);
        }
        ...
        if (ref->comm == NULL)
        {
            ...
            if (peer != 0)
            {
                ...
                if (peer > host)
                {
                    ref->hostip = peer;
                    ref->peerip = host;
                    snzprintf(text, sizeof(text), "%d.%d.%d.%d%s",
                ...
                else
                {
                    ref->hostip = host;
                    ref->peerip = peer;
                    strnzcpy(text, ref->addr, sizeof(text));
...
int32_t NetGameUtilStatus(NetGameUtilRefT *ref, int32_t iSelect, void *pBuf, int32_t iBufSize)
{
    ...
    {
        memcpy(pBuf, &ref->pSocket, sizeof(ref->pSocket));
...
void NetGameUtilAdvert(NetGameUtilRefT *ref, const char *kind, const char *name, const char *note)
{
    ...
    if (ref->advt == NULL)
    {
        ref->advt = _NetGameUtilAdvtConstruct(ref, 16);
    }

    // save the kind for future queries
    strnzcpy(ref->kind, kind, sizeof(ref->kind));
...
uint32_t NetGameUtilLocate(NetGameUtilRefT *ref, const char *kind, const char *name)
{
    ...
    return((ref->advt == NULL) ? 0 : ProtoAdvtLocate(ref->advt, kind, name, NULL, 0));
}
...
int32_t NetGameUtilQuery(NetGameUtilRefT *ref, const char *kind, char *buf, int32_t max)
{
    ...
    return((ref->advt == NULL) ? 0 : ProtoAdvtQuery(ref->advt, kind, "", buf, max, ref->uLocalAdvt));
}
