...
*/
/********************************************************************************F*/
static int32_t _NetConnFindMacAddress(uint8_t *pMacAddr)
{
    ...
    note:
        ...
        * it is not a good idea to allocate this buffer on the stack as it can be quite huge and could lead to a
          stack overflow.
    */
    if ((pAdaptersAddrBuf = DirtyMemAlloc(ulOutBufLen, NETCONN_MEMID, pRef->iMemGroup, pRef->pMemGroupUserData)) == NULL)
    {
    ...
    if (dwMsRetVal == NO_ERROR)
    {
        ...
        while (pCurrAddr)
        {
            ...
            if ((pCurrAddr->IfType == IF_TYPE_ETHERNET_CSMACD) || (pCurrAddr->IfType == IF_TYPE_IEEE80211))
            {
                ...
                // make sure physical address is 6-byte long; if not, it is not a MAC address
                if (pCurrAddr->PhysicalAddressLength == 6)
                {
                    NetPrintf(("netconnwin: valid MAC address found --> "));
                    for (i = 0; i < (int) pCurrAddr->PhysicalAddressLength; i++)
                    {
                        if (i == (pCurrAddr->PhysicalAddressLength - 1))
                        {
                            NetPrintf(("%.2X\n", (int) pCurrAddr->PhysicalAddress[i]));
                        }
                        else
                        {
                            NetPrintf(("%.2X-", (int) pCurrAddr->PhysicalAddress[i]));
                        }

                        pMacAddr[i] = pCurrAddr->PhysicalAddress[i];
                    }
    ...
    DirtyMemFree(pAdaptersAddrBuf, NETCONN_MEMID, pRef->iMemGroup, pRef->pMemGroupUserData);
...
int32_t NetConnStartup(const char *pParams)
{
    ...
    if (pRef != NULL)
    {
        NetPrintf(("netconnwin: NetConnStartup() called while the module is already active\n"));
        return(-1);
    }
    ...
    {
        NetPrintf(("netconnwin: unable to allocate module state\n"));
        return(-2);
    }
...
int32_t NetConnConnect(const NetConfigRecT *pConfig, const char *pOption, int32_t iData)
{
    ...
    if (pOption != NULL)
    {
        ...
        if ((pOpt = strstr(pOption, "peerport=")) != NULL)
        {
            ...
            NetPrintf(("netconnwin: peerport=%d\n", pRef->iPeerPort));
        }
        // check for CA preload request
        if ((pOpt = strstr(pOption, "capreload=")) != NULL)
        {
            ds_strnzcpy(strPreloadName, pOpt+10, sizeof(strPreloadName));
            NetPrintf(("netconnwin: capreload=%s\n", strPreloadName));
        }
    }
    ...
    if (pRef->eState == ST_INIT)
    {
        ...
        // kick off CA preload if requested
        if ((pRef->bPreloadIssued == FALSE) && (strPreloadName[0] != '\0'))
        {
            DirtyCertCAPreloadCerts(strPreloadName);
...
{
    if (_NetConn_pRef->eState == ST_CONN)
    {
...
int32_t NetConnStatus(int32_t iKind, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    if ((iKind == 'ethr') || (iKind == 'macx'))
    {
        ...
        return(_NetConnFindMacAddress(pBuf));
    }
