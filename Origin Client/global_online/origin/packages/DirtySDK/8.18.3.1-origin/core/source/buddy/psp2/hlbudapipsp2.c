...
static void _HLBPrintfCode(HLBApiRefT *pHLBApi, const char *pFormat, ...)
{
    ...
    strnzcpy(strText, "hlbudapipsp2: ", sizeof(strText));
...
static void _FriendApiNotificationCb(FriendApiRefT *pFriendApiRef, FriendApiEventsE eventType, FriendEventDataT *pEventData, FriendDataT *pFriendData, void *pNotifyCbUserData)
{
    ...
    if (pFriendData)
    {
        pUserId = (SceNpId *)(pFriendData->pRawData1);
    }
    ...
    switch(eventType)
    {
        ...
        case FRIENDAPI_EVENT_FRIEND_UPDATED:
            ...
            {
                _HLBUpdateBuddy(pHLBApi, pUserId, TRUE, FALSE, pEventData->strMsgBuf);
            }
            else
            {
        ...
        case FRIENDAPI_EVENT_MSG_RCVED:
            _HLBReceiveMessage(pHLBApi, pUserId, pEventData->strMsgBuf, pEventData->iMsgSize);
            break;
        case FRIENDAPI_EVENT_MSG_ATTACH_RCVED:
            _HLBSignalMsgAttachmentReceived(pHLBApi, pUserId); 
            break;
        #if 0
        case FRIENDAPI_EVENT_MSG_ATTACH_RESULT:
            _HLBRecvMsgAttachmentResult(pHLBApi, pUserId, (SceNpBasicAttachmentData *)pEventData->strMsgBuf);
            break;
        #endif
        default:
...
/*!
    ...
    \Description
        PSP2 ONLY -- Set the callback that HlBudApi will forward sceNpBasicRegisterHandler() events to.
