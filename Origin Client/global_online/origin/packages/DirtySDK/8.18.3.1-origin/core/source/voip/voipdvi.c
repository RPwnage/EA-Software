...
static void _VoipDVIDestroy(VoipCodecRefT *pState);
static int32_t _VoipDVIEncodeBlock3(VoipCodecRefT *pState, uint8_t *pOut, const int16_t *pInp, int32_t iNumSamples);
static int32_t _VoipDVIDecodeBlock3(VoipCodecRefT *pState, int32_t *pOut, const uint8_t *pInp, int32_t iInputBytes, int32_t iChannel);
static void _VoipDVIReset(VoipCodecRefT *pState);
static int32_t _VoipDVIControl(VoipCodecRefT *pState, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue);
...
/*!
    ...
    \Input *pState  - pointer to state to destroy
...
static int32_t _VoipDVIEncodeBlock3(VoipCodecRefT *pState, uint8_t *pOut, const int16_t *pInp, int32_t iNumSamples)
{
    ...
    DVICompStateT *pCompState = &((VoipDVIStateT *)pState)->EncodeState;

    // DVI encoder requires input samples to be a multiple of eight
    if ((iNumSamples % 8) != 0)
    {
        NetPrintf(("voipdvi: error - dvi encoder can only encode multiples of 8 samples (%d submitted).\n", iNumSamples));
        return(0);
    }

    // save initial state to output buffer
    memcpy(pOut, pCompState, sizeof(*pCompState));
...
*/
/********************************************************************************F*/
static int32_t _VoipDVIDecodeBlock3(VoipCodecRefT *pState, int32_t *pOut, const unsigned char *pInp, int32_t iInputBytes, int32_t iChannel)
{
    VoipDVIStateT *pDVIState = (VoipDVIStateT *)pState;
    ...
    memcpy(&DecodeState, pInp, sizeof(DecodeState));
    ...
    for (iCount = 0; iCount < iNumSamples; ++iCount)
    {
        ...
        *pOut++ += ((iEstimate * pDVIState->iOutputVolume) >> VOIP_CODEC_OUTPUT_FRACTIONAL) * DIV_RANGE;
    }
...
/*!
    ...
    \Input *pState  - pointer to DVI module
...
{
    VoipDVIStateT *pDVIState = (VoipDVIStateT *)pState;
    memset(&pDVIState->EncodeState, 0, sizeof(pDVIState->EncodeState));
}
...
static int32_t _VoipDVIStatus(VoipCodecRefT *pCodecState, int32_t iSelect, int32_t iValue, void *pBuffer, int32_t iBufSize)
{
    VoipDVIStateT *pModuleState = (VoipDVIStateT *)pCodecState;
    ...
    if (pModuleState != NULL)
    {
        if (iSelect == 'fsiz')
        {
            ...
            return(((iValue*3)/8) + sizeof(DVICompStateT));
        }
    }
    NetPrintf(("voipdvi: unhandled status selector '%C'\n", iSelect));
    return(-1);
}
