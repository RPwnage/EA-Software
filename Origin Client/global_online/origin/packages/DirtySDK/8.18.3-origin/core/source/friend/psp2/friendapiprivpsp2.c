...
/*!
    ...
    \Description
        Add a friend to the friendlist. Does not duplicate if friend is already in list.
...
*/
/********************************************************************************F*/
static int32_t _FriendApiAddFriend(FriendApiRefT *pFriendApiRef, const SceNpId *pFriendId)
{
    int32_t iFriendIndex;
    int32_t iResult;
...
static void _NpBasicFriendListHandler(SceNpBasicFriendListEventType eEventType, const SceNpId *pFriendId, void *pUserData)
{
    ...
    switch (eEventType)
    {
        ...
        case SCE_NP_BASIC_FRIEND_LIST_EVENT_TYPE_ADDED:
            ...
            _FriendApiAddFriend(pFriendApiRef, pFriendId);
            break;
...
static void _NpBasicFriendOnlineStatusHandler(const SceNpId *pFriendId, SceNpBasicOnlineStatus eOnlineStatus, void *pUserData)
{
    ...
    switch (eOnlineStatus)
    {
        ...
        case SCE_NP_BASIC_ONLINE_STATUS_ONLINE:
            pFriendApiRef->eventData.bPresence = TRUE;
            break;
...
static void _NpBasicBlockListHandler(SceNpBasicBlockListEventType eEventType, const SceNpId *pPlayerId, void *pUserData)
{
    ...
    switch (eEventType)
    {
        ...
        case SCE_NP_BASIC_BLOCK_LIST_EVENT_TYPE_ADDED:
            ...
            friendApiEvent = FRIENDAPI_EVENT_BLOCKEDLIST_UPDATED;
            break;
...
static void _NpBasicInGameDataMessageHandler(const SceNpId *pFrom, const SceNpBasicInGameDataMessage *pMessage, void *pUserData)
{
    ...
    strnzcpy(pFriendApiRef->eventData.strMsgBuf, (char *)pMessage->data, sizeof(pFriendApiRef->eventData.strMsgBuf));
    pFriendApiRef->eventData.iMsgSize = strlen(pFriendApiRef->eventData.strMsgBuf);
...
/*!
    ...
    \Notes
        ...
        \verbatim
            'strt' - notifies the Friend API that it is time to register the handler to get PSP2 friends events (if not done already)
            'stop' - notifies the Friend API that it is time to unregister the handler to get PSP2 friends events (if nobody else relies on it)
        \endverbatim
...
int32_t FriendApiPrivControl(FriendApiRefT *pFriendApiRef, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    if (iControl == 'strt')
    {
        ...
        {
            int32_t iResult;
            #if SCE_PSP2_SDK_VERSION < 0x00940050
            SceNpBasicEventHandlers _EventHandlers[] =
            {
                _NpBasicFriendListHandler,
                _NpBasicFriendOnlineStatusHandler,
            };

            // add np basic callback
            NetPrintf(("friendapiprivpsp2: registering NP basic event handler\n"));
            if ((iResult = sceNpBasicRegisterHandler(_EventHandlers, pFriendApiRef)) != 0)
            {
        ...
        NetPrintf(("friendapiprivpsp2: friend api started (ref count = %d)\n", pFriendApiRef->iNpBasicHandlerRefCount));
        return(0);
    }
    ...
    if (iControl == 'stop')
    {
        ...
        {
            pFriendApiRef->iNpBasicHandlerRefCount--;
            NetPrintf(("friendapiprivpsp2: friend api stopped (ref count = %d)\n", pFriendApiRef->iNpBasicHandlerRefCount));

            if (!pFriendApiRef->iNpBasicHandlerRefCount)
            {
                int32_t iResult;

                NetPrintf(("friendapiprivpsp2: unregistering NP basic event handler\n"));
                ...
                /*
                Note: XXX_SYNC_DONE events are only notified by the PSP2 system each time a new handler is being
                registered with sceNpBasicRegisterHandler().

                FriendApi is only interested in the very first of these events because its internally maintained friendslist
                remains valid even if the handler is unregistered with the PSP2 system. Consequently, here, booleans used to
                mark that XXX_SYNC_DONE events have been receive are intentionally not reset.
                */
            }
        }
    ...
    NetPrintf(("friendapiprivpsp2: unhandled control selector ('%c%c%c%c')\n",
