...
typedef struct NetConnRefT
{
    ...
    uint8_t         bPreloadIssued;     //!< true if we have issued a CA preload, else false
    ...
    char            strPreloadName[32]; //!< preload CA service name, if specified
...
static void _NetConnUpdate(void *pData, uint32_t uData)
{
    ...
    if (pRef->eState == ST_TICK)
    {
        ...
        {
            // if ca preload is requested, and we haven't done it already, do it now
            if ((pRef->bPreloadIssued == FALSE) && (pRef->strPreloadName[0] != '\0'))
            {
                DirtyCertCAPreloadCerts(pRef->strPreloadName);
                pRef->bPreloadIssued = TRUE;
            }
...
int32_t NetConnConnect(const NetConfigRecT *pConfig, const char *pOption, int32_t iData)
{
    ...
    if ((pRef->eState == ST_INIT) || (pRef->eState == ST_EXT_CLEANUP))
    {
        ...
        if (pOption != NULL)
        {
            ...
            // check for CA preload request
            if ((pOpt = strstr(pOption, "capreload=")) != NULL)
            {
                ds_strnzcpy(pRef->strPreloadName, pOpt+10, sizeof(pRef->strPreloadName));
                NetPrintf(("netconnps3: capreload=%s\n", pRef->strPreloadName));
            }
        }
