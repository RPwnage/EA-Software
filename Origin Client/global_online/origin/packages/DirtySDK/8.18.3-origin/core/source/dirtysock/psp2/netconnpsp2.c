    ...
    size_t iCookieSize, const char *pEntitlementId, uint32_t uConsumedCount, NetConnTicketCallbackT *pCallback, void *pUserData)
{
    ...
    TicketRequestParam.serviceId = pRef->strServiceId;
...
static void _NetConnUpdateTICK(NetConnRefT *pRef)
{
    ...
    // request ticket if not already requested
    if (!pRef->bNpTicketRequested)
    {
...
int32_t NetConnStartup(const char *pParams)
{
    ...
    SocketCreate(SCE_KERNEL_CPU_MASK_USER_0);
    ...
    if (pRef->eState == ST_INIT)
    {
        ...
        if (pOption != NULL)
        {
            ...
            if ((pOpt = strstr(pOption, "capreload=")) != NULL)
            {
                ...
                NetPrintf(("netconnpsp2: capreload=%s\n", pRef->strPreloadName));
            }
        }
