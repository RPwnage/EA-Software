...
//! callback registered by OSDK (via HLBUDAPI) to directly gets NP basic events
//! $todo this should be depecrated in v9. OSDK should rather use FriendApiNotifyCbT
typedef void (FriendApiSceNpBasicEventCallbackT)(int iEvent, int iRetCode, uint32_t uRequestId, void *pContext);
...
struct FriendApiRefT
{
    ...
    FriendEventDataT eventData;     //!< event message data buffer - to avoid stack allocation - cannot be used in _NpBasicHandler 
    FriendEventDataT eventData2;    //!< event message data buffer - to avoid stack allocation - can be used only in _NpBasicHandler.

    NetCritT callbackCrit;          //!< critical section used to ensure thread safety in handler registered with NP Basic utility

    FriendApiSceNpBasicEventCallbackT *pSceNpBasicEventCallback; //<! callback to forward sceNpBasic library events to.
    void *pSceNpBasicEventCallbackContext; //<! arbitrary caller-specified context to pass to the HLBSceNpBasicEventCallbackT.

    int32_t iNpBasicHandlerRefCount;//!< NpBasic handler ref count

    /*
    The following three variables are to be accessed safely within callbackCrit. They are
    all used in the friendapiprivps3 code and in the callback registered with the NP Basic utility.
...
/*F********************************************************************************/
/*!
    \Function _FriendApiNotifyEvent

    \Description
        Invokes the different callbacks registered by higher-level modules.

    \Input *pFriendApiRef   - module reference
    \Input eventType        - event type to be passed to the callbacks
    \Input pEventData       - pointer to event data to be passed to the callbacks
    \Input pFriendData      - pointer to friend data to be passed to the callbacks

    \Version 06/15/2010 (mclouatre)
*/
/********************************************************************************F*/
static void _FriendApiNotifyEvent(FriendApiRefT *pFriendApiRef, FriendApiEventsE eventType, FriendEventDataT *pEventData, FriendDataT *pFriendData)
{
    int32_t iIndex;

    pFriendApiRef->common.bInCallback = TRUE;
    for(iIndex = 0; iIndex < FRIENDAPI_MAX_CALLBACKS; iIndex++)
    {
        NetCritEnter(&pFriendApiRef->common.notifyCrit);
        if (pFriendApiRef->common.pNotifyCb[iIndex])
        {
            pFriendApiRef->common.pNotifyCb[iIndex](pFriendApiRef, eventType, pEventData, pFriendData, pFriendApiRef->common.pNotifyCbUserData[iIndex]);
        }
        NetCritLeave(&pFriendApiRef->common.notifyCrit);
    }
    pFriendApiRef->common.bInCallback = FALSE;
}
...
static int _NpBasicHandler(int iEvent, int iRetCode, uint32_t uRequestId, void *pArg)
{
    ...
    NetCritEnter(&pFriendApiRef->callbackCrit);

    // increment the number of events we need to pop off the queue. TODO: is this thread safe?
    // we are incrementing this even for events that don't require a call to sceNpBasicGetEvent(), just to be safe.
    // extra calls to sceNpBasicGetEvent() are harmless
    pFriendApiRef->iEventsInQueue++;

    if (iEvent == SCE_NP_BASIC_EVENT_END_OF_INITIAL_PRESENCE)
    {
        // flag initial presence as completed
        pFriendApiRef->bEndOfInitialPresence = TRUE;

        // tell the update function that it needs to get the list of blocked players
        pFriendApiRef->bGetBlockList = TRUE;
    ...
    {
        // Note: This event does not occur following a call to sceNpBasicAddBlockListEntry(). It only occurs when the user is
        //       added to the blocklist via the friends menu of the PS3 XMB.
    ...
    {
        // Note: This event occurs following a call to sceNpBasicAddBlockListEntry()

        // tell the update function that it needs to get the list of blocked players if add to blocklist succeeded
        if (iRetCode == 0)
        {
            pFriendApiRef->bGetBlockList = TRUE;
        }
    ...
    if (!bDelayedNotifyEvent)
    {
        memset(&pFriendApiRef->eventData2, 0, sizeof(pFriendApiRef->eventData2));
        pFriendApiRef->eventData2.iMsgSize = 0;
        pFriendApiRef->eventData2.bPresence = FALSE;

        // on ps3, the pRawData1 field of the eventData structure points to the PS3 event type value
        pFriendApiRef->eventData2.pRawData1 = &iEvent;

        _FriendApiNotifyEvent(pFriendApiRef, FRIENDAPI_EVENT_UNKNOWN, &pFriendApiRef->eventData2, NULL);
    }

    // notify the registered event callback.
    // ($todo this was specifically added for OSDK. should be depecrated in v9. OSDK should rather use FriendApiNotifyCbT)
    // current assumption: this feature is always used in a context where cellSysutilCheckCallback is invoked from same
    // thread as DirtySDK.
    if (pFriendApiRef->pSceNpBasicEventCallback)
    {
        pFriendApiRef->pSceNpBasicEventCallback(iEvent, iRetCode, uRequestId, pFriendApiRef->pSceNpBasicEventCallbackContext);
    }

    NetCritLeave(&pFriendApiRef->callbackCrit);
...
static void _FriendApiIncrementFriendslistVersion(FriendApiRefT *pFriendApiRef)
{
    ...
    NetPrintf(("friendapiprivps3: friendslist updated (new friend count = %d, new version = %d)\n",
        iFriendsCount, pFriendApiRef->common.uVersion));
#endif
}
...
static void _FriendApiIncrementBlocklistVersion(FriendApiRefT *pFriendApiRef)
{
    ...
    pFriendApiRef->blockList.uVersion++;
    ...
    NetPrintf(("friendapiprivps3: blocklist updated (new blocked user count = %d, new version = %d)\n",
        iBlockedUsersCount, pFriendApiRef->blockList.uVersion));
#endif
}
...
static int32_t _FriendApiIsFriendInList(FriendApiRefT *pFriendApiRef, SceNpId npId)
{
    ...
    for (iFriendIndex = 0; iFriendIndex < FRIENDAPI_MAX_FRIENDS; iFriendIndex++)
    {
        ...
        {
            if (strcmp(pFriendApiRef->friendsList.aFriends[iFriendIndex].userInfo.userId.handle.data, npId.handle.data) == 0)
            {
...
/*F********************************************************************************/
/*!
    \Function _FriendApiAddFriend

    \Description
        Add a friend to the friendlist. Doest not duplicate if friend is already in list.
...
*/
/********************************************************************************F*/
static int32_t _FriendApiAddFriend(FriendApiRefT *pFriendApiRef, SceNpUserInfo userInfo)
{
    int32_t iFriendIndex;
    int32_t iResult;
    ...
    for (iFriendIndex = 0; iFriendIndex < FRIENDAPI_MAX_FRIENDS; iFriendIndex++)
    {
        if (!pFriendApiRef->friendsList.aFriends[iFriendIndex].bValid)
        {
            ...
            memcpy(&pFriendApiRef->friendsList.aFriends[iFriendIndex].userInfo, &userInfo, sizeof(pFriendApiRef->friendsList.aFriends[iFriendIndex].userInfo));
...
static void _FriendApiProcessEvent(FriendApiRefT *pFriendApiRef)
{
    ...
    uint8_t bEventValid = TRUE;

    memset(&pFriendApiRef->eventData, 0, sizeof(pFriendApiRef->eventData));
    pFriendApiRef->eventData.iMsgSize = sizeof(pFriendApiRef->eventData.strMsgBuf);
    pFriendApiRef->eventData.bPresence = TRUE;   // online presence defaults to TRUE

    if ((iNpStatus = sceNpBasicGetEvent(&iNpEvent, &userInfo, &pFriendApiRef->eventData.strMsgBuf, (size_t *)&pFriendApiRef->eventData.iMsgSize)) == 0)
    {
        ...
        case SCE_NP_BASIC_EVENT_OFFLINE:
            ...
            pFriendApiRef->eventData.bPresence = FALSE;
            friendApiEvent = FRIENDAPI_EVENT_FRIEND_UPDATED;
            _FriendApiAddFriend(pFriendApiRef, userInfo);
            break;
        ...
        case SCE_NP_BASIC_EVENT_MESSAGE:
            ...
            friendApiEvent = FRIENDAPI_EVENT_MSG_RCVED;
            break;

        case SCE_NP_BASIC_EVENT_INCOMING_ATTACHMENT:
            NetPrintf(("friendapiprivps3: received a message attachment event from user %s\n", userInfo.userId.handle.data));
            friendApiEvent = FRIENDAPI_EVENT_MSG_ATTACH_RCVED;
            break;

        case SCE_NP_BASIC_EVENT_RECV_ATTACHMENT_RESULT:
            NetPrintf(("friendapiprivps3: received a message attachment result event from user %s\n", userInfo.userId.handle.data));
            friendApiEvent = FRIENDAPI_EVENT_MSG_ATTACH_RESULT;
            break;
        ...
        default:
            NetPrintf(("friendapiprivps3: received a unhandled event (%d) from user %s. event data is: %s\n",
                iNpEvent, userInfo.userId.handle.data, pFriendApiRef->eventData.strMsgBuf));
            break;
        }
        ...
        if (bEventValid)
        {
            ...
            // on ps3, the pRawData1 field of the eventData structure points to the PS3 event type value
            pFriendApiRef->eventData.pRawData1 = &iNpEvent;

            _FriendApiNotifyEvent(pFriendApiRef, friendApiEvent, &pFriendApiRef->eventData, &friendData);
        }
    }
    else
    {
...
FriendApiRefT *FriendApiPrivCreate()
{
    ...
    NetCritInit(&pFriendApiRef->callbackCrit, "friendapiprivps3 callback crit");
...
void FriendApiPrivUpdate(FriendApiRefT *pFriendApiRef)
{
    ...
    // iEventsInQueue can only be accessed in the context of the callbackCrit
    NetCritEnter(&pFriendApiRef->callbackCrit);
    iEventsInQueue = pFriendApiRef->iEventsInQueue;
    if (iEventsInQueue > 0)
    {
        --pFriendApiRef->iEventsInQueue;
    }
    ...
    // bGetBlockList can only be accessed in the context of the callbackCrit
    NetCritEnter(&pFriendApiRef->callbackCrit);
    bGetBlockList = pFriendApiRef->bGetBlockList;
    pFriendApiRef->bGetBlockList = FALSE;
    NetCritLeave(&pFriendApiRef->callbackCrit);
    ...
    if (bGetBlockList)
    {
        ...
        memset(&pFriendApiRef->eventData, 0, sizeof(pFriendApiRef->eventData));
        pFriendApiRef->eventData.iMsgSize = 0;
        pFriendApiRef->eventData.bPresence = FALSE;

        // on ps3, the pRawData1 field of the eventData structure points to the PS3 event type value
        pFriendApiRef->eventData.pRawData1 = &iNpEvent;

        _FriendApiNotifyEvent(pFriendApiRef, FRIENDAPI_EVENT_BLOCKEDLIST_UPDATED, &pFriendApiRef->eventData, NULL);
    }
}
        ...
        if (pFriendApiRef->friendsList.aFriends[iFriendIndex].bValid)
        {
            if (pFriendsList)
            {
                ...
                if (pRawDataList1)
                {
                    ...
                    memcpy(&pNpUserInfoList[iFriendsCount], &pFriendApiRef->friendsList.aFriends[iFriendIndex].userInfo, sizeof(SceNpUserInfo));
...
int32_t FriendApiPrivStatus(FriendApiRefT *pFriendApiRef, int32_t iSelect, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'eoip')
    {
        ...
        // bEndOfInitialPresence can only be accessed in the context of the callbackCrit
        NetCritEnter(&pFriendApiRef->callbackCrit);
        bEndOfInitialPresence = pFriendApiRef->bEndOfInitialPresence;
        NetCritLeave(&pFriendApiRef->callbackCrit);
...
/*!
    ...
    \Notes
        ...
        \verbatim
            'cbfp' - sets NP Basic events callback ($todo this was specifically added for OSDK. should be depecrated in v9. OSDK should rather use FriendApiNotifyCbT)
            'cbup' - sets NP Basic event user ref  ($todo this was specifically added for OSDK. should be depecrated in v9. OSDK should rather use FriendApiNotifyCbT)
            'strt' - notifies the Friend API that it is time to register the handler to get PS3 friends events (if not done already)
...
int32_t FriendApiPrivControl(FriendApiRefT *pFriendApiRef, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    if (iControl == 'cbfp')
    {
        pFriendApiRef->pSceNpBasicEventCallback = (FriendApiSceNpBasicEventCallbackT *) pValue;
        return(0);
    }

    if (iControl == 'cbup')
    {
        pFriendApiRef->pSceNpBasicEventCallbackContext = pValue;
        return(0);
    }

    if (iControl == 'strt')
    {
        if (!pFriendApiRef->iNpBasicHandlerRefCount)
        {
            int32_t iResult;
            ...
            NetPrintf(("friendapiprivps3: registering NP basic context sensitive event handler\n"));
        ...
        pFriendApiRef->iNpBasicHandlerRefCount++;
        NetPrintf(("friendapiprivps3: friend api started (ref count = %d)\n", pFriendApiRef->iNpBasicHandlerRefCount));
        return(0);
    }
    ...
    if (iControl == 'stop')
    {
        ...
        {
            pFriendApiRef->iNpBasicHandlerRefCount--;
            NetPrintf(("friendapiprivps3: friend api stopped (ref count = %d)\n", pFriendApiRef->iNpBasicHandlerRefCount));

            if (!pFriendApiRef->iNpBasicHandlerRefCount)
            {
                int32_t iResult;

                NetPrintf(("friendapiprivps3: unregistering NP basic event handler\n"));
                ...
                /*
                Note: Experimenation showed that the event SCE_NP_BASIC_EVENT_END_OF_INITIAL_PRESENCE is notified by the PS3
                system each time a new handler is being registered with sceNpBasicRegisterHandler().

                FriendApi is only interested in the very first of these events because its internal maintained friendslist
                remains valid even if the handler is unregistered with the PS3 system. Consequently, here, bEndOfInitialPresence
...
void FriendApiPrivDestroy(FriendApiRefT *pFriendApiRef)
{
    ...
    if (pFriendApiRef->iNpBasicHandlerRefCount)
    {
        ...
        pFriendApiRef->iNpBasicHandlerRefCount = 0;
    }
    ...
    // destroy critical section used to ensure thread safety in handler registered with NP basic utility
    NetCritKill(&pFriendApiRef->callbackCrit);
...
{
    return(pFriendApiRef->blockList.uVersion);
}
