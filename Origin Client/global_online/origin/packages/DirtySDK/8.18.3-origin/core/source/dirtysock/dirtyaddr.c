...
uint32_t DirtyAddrToHostAddr(void *pOutput, int32_t iBufLen, const DirtyAddrT *pAddr)
{
    ...
    uAddress = strtoul(&pAddr->strMachineAddr[1], NULL, 16);
...
uint32_t DirtyAddrFromHostAddr(DirtyAddrT *pAddr, const void *pInput)
{
    ...
    uAddress = SocketHtonl(*(uint32_t *)pInput);
    snzprintf(pAddr->strMachineAddr, sizeof(pAddr->strMachineAddr), "$%08x", uAddress);
    return(TRUE);
}
