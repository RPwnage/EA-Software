...
#ifndef _lobbytagfield_h
#define _lobbytagfield_h
...
int32_t TagFieldUnitTest(int32_t iVerbose);
...
void TagFieldFormat(char *record, unsigned char marker);

// search the buffer and locate the named field
const char *TagFieldFind(const char *record, const char *name);

// search the buffer and locate the two-part named field (the names are catenated together)
const char *TagFieldFind2(const char *record, const char *name1, const char *name2);
...
int32_t TagFieldDelete(char *record, const char *name);

// rename a field within the record
int32_t TagFieldRename(char *record, int32_t reclen, char *oldname, char *newname);

// duplicate an entire record
int32_t TagFieldDupl(char *record, int32_t reclen, const char *data);

// extract the first field name in record
int32_t TagFieldFirst(const char *record, char *nameptr, int32_t namelen);

// set a numeric field
int32_t TagFieldSetNumber(char *record, int32_t reclen, const char *name, int32_t value);

// set a 64-bit numeric field
int32_t TagFieldSetNumber64(char *record, int32_t reclen, const char *name, int64_t value);

// set a flags field
int32_t TagFieldSetFlags(char *record, int32_t reclen, const char *name, int32_t value);

// set an ip-address field
int32_t TagFieldSetAddress(char *record, int32_t reclen, const char *name, uint32_t addr);

// set a token field
int32_t TagFieldSetToken(char *record, int32_t reclen, const char *name, int32_t token);

// set a string field
int32_t TagFieldSetString(char *record, int32_t reclen, const char *name, const char *value);

// set a binary field
int32_t TagFieldSetBinary(char *record, int32_t reclen, const char *name, const void *value, int32_t vallen);

// set a binary field using more efficient encoding
int32_t TagFieldSetBinary7(char *record, int32_t reclen, const char *name, const void *value, int32_t vallen);

// save an entire structure as a single field
int32_t TagFieldSetStructure(char *record, int32_t reclen, const char *name, const void *_bin, int32_t len, const char *pat);

// set a date from epoch time using std C time.h lib
int32_t TagFieldSetEpoch(char *record, int32_t reclen, const char *name, uint32_t epoch);

// set a date field from discrete date components
int32_t TagFieldSetDate(char *record, int32_t reclen, const char *name,
...
int32_t TagFieldSetRaw(char *pRecord, int32_t iReclen, const char *pName, const char *pData_);

// merge two records together (does not merge bulk data)
int32_t TagFieldMerge(char *record, int32_t reclen, const char *merge);

// create a tagfield record using printf style descriptor
int32_t TagFieldPrintf(char *pRecord, int32_t iLength, const char *pFormat, ...);

// get a numeric field
int32_t TagFieldGetNumber(const char *data, int32_t defval);

// get a 64-bit numeric field
int64_t TagFieldGetNumber64(const char *data, int64_t defval);

// get a flags field
int32_t TagFieldGetFlags(const char *data, int32_t defval);

// get an ip address field
uint32_t TagFieldGetAddress(const char *data, uint32_t defval);

// get a token
int32_t TagFieldGetToken(const char *data, int32_t deftok);
...
int32_t TagFieldGetBinary(const char *data, void *buffer, int32_t buflen);

// get packed structure field contents
int32_t TagFieldGetStructure(const char *data, void *_buf, int32_t len, const char *pat);

// get date in epoch time using std C time.h lib
uint32_t TagFieldGetEpoch(const char *data, uint32_t defval);
