...
/*!
    ...
    \Version    0.5        02/23/99 (GWS) First Version
    \Version    1.0        02/25/99 (GWS) Alpha release
    \Version    1.1        11/20/02 (JLB) Added Send() flags parameter, protocol definitions.
...
/*!
    ...
    \Description
        The Comm module provides packet-level communications protocols.

        <b>Overview</b>

        The Comm module provides a unified calling structure as well as unified
        status and error values for a collection of transport protocols.  Currently
        supported are TCP and a reliable UDP implementation, however, the TCP
        implementation exists for demonstration purposes only and should not be used
        otherwise.

        Comm module members are not directly accessible from the PlayStation2's EE processor.
        Instead, the NetGame module must be used to utilize Comm functionality.

        <b>Module Dependency Graph</b>

            <img alt="" src="comm.png">
...
CommRef *CommTCPConstruct(int32_t maxwid, int32_t maxinp, int32_t maxout);
...
CommRef *CommTAPIConstruct(int32_t maxwid, int32_t maxinp, int32_t maxout);

//typedef struct CommSerRef CommSerRef;
CommRef *CommSerConstruct(int32_t maxwid, int32_t maxinp, int32_t maxout);
#endif
