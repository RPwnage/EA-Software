...
static const char *_TesterHistoryGetFullPath(const char *pFilename)
{
    ...
    {
        strnzcat(strHistoryName, "\\", sizeof(strHistoryName));
        strnzcat(strHistoryName, pFilename, sizeof(strHistoryName));
    }
    else
    {
        strnzcpy(strHistoryName, ".\\", sizeof(strHistoryName));
    }
...
int32_t TesterHistoryGet(TesterHistoryT *pState, int32_t iNum, char *pBuf, int32_t iSize)
{
    ...
    // copy in the data
    strnzcpy(pBuf, pEntry->strText, iSize);
...
int32_t TesterHistoryAdd(TesterHistoryT *pState, const char *pBuf)
{
    ...
    strnzcpy(pEntry->strText, pBuf, sizeof(pEntry->strText));
