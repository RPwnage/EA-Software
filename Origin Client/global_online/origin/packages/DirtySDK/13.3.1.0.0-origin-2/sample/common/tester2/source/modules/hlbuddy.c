...
static void _FriendApiNotificationCallback(FriendApiRefT *pFriendApiRef, FriendEventDataT *pEventData, void *pNotifyCbUserData)
{
    ...
    if (pEventData->pRawData1)
    {
        ...
        switch(pEventData->eventType)
        {
            case FRIENDAPI_EVENT_FRIEND_UPDATED:
                ZPrintf("    message: %s\n", pEventData->eventTypeData.FriendUpdated.pRawData2);
                ZPrintf("    friend's NP Online ID: %s\n", ((SceNpUserInfo *)pEventData->eventTypeData.FriendUpdated.pFriendData->pRawData1)->userId.handle.data);
                break;

            case FRIENDAPI_EVENT_FRIEND_REMOVED:
                ZPrintf("    friend's NP Online ID: %s\n", ((SceNpUserInfo *)pEventData->eventTypeData.FriendRemoved.pFriendData->pRawData1)->userId.handle.data);
                break;

            case FRIENDAPI_EVENT_MSG_RCVED:
                ZPrintf("    message: %s\n", pEventData->eventTypeData.MsgRcved.pRawData2);
                ZPrintf("    friend's NP Online ID: %s\n", ((SceNpUserInfo *)pEventData->eventTypeData.MsgRcved.pFriendData->pRawData1)->userId.handle.data);
                break;
