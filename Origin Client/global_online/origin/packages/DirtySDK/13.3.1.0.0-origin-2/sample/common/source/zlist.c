...
int32_t ZListPushBack(ZListT *pList, void *pEntry)
{
    ...
    if ((pList->uTail - pList->uHead) >= pList->iNumEntries)
    {
    ...
    ZMemCpy(pDest, pEntry, pList->iEntrySize);
...
int32_t ZListPopFront(ZListT *pList, void *pEntry)
{
    ...
    else
    {
        ...
        if (pEntry)
        {
            ...
            ZMemCpy(pEntry, (void *)pSrc, pList->iEntrySize);
        }
        pList->uHead++;
...
void ZListClear(ZListT *pList)
{
    if(pList)
    {
        ZMemSet(pList->pData, 0, pList->iNumEntries * pList->iEntrySize);
        pList->uHead = 0;
        pList->uTail = 0;
    }
}
