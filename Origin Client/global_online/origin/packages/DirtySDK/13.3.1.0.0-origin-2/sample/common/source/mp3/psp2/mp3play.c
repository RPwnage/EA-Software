...
MP3PlayStateT *mp3playcreate(int32_t iNumChannels, int32_t iSampleRate, int32_t iFrameSize, int32_t iNumFrames)
{
    ...
    if (iNumChannels == 1)
    {
        if ((pState->iSpeakerPort = sceAudioOutOpenPort(SCE_AUDIO_OUT_PORT_TYPE_VOICE, iFrameSize, iSampleRate, SCE_AUDIO_OUT_PARAM_FORMAT_S16_MONO)) < 0)
        {
    ...
    {
        if ((pState->iSpeakerPort = sceAudioOutOpenPort(SCE_AUDIO_OUT_PORT_TYPE_VOICE, iFrameSize, iSampleRate, SCE_AUDIO_OUT_PARAM_FORMAT_S16_STEREO)) < 0)
        {
            ZPrintf("mp3play: unable to init output device\n");
            ZMemFree(pState);
            return(NULL);
...
void mp3playdestroy(MP3PlayStateT *pState)
{
    ...
    sceAudioOutReleasePort(pState->iSpeakerPort);
