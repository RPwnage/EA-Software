...
/*!
    ...
    \Notes
        Native implementations used on: Apple OSX, Apple iOS, Linux, PS3, Windows

        References: http://tools.ietf.org/html/rfc4086
                    http://randomnumber.org/links.htm

    \Todo
        - Native implementations for: Android, Playbook, PSP2, WinRT, WinPRT
...
#elif defined(DIRTYCODE_APPLEIOS) || defined(DIRTYCODE_PS3)
...
{
    #if defined(DIRTYCODE_APPLEIOS) || defined(DIRTYCODE_PS3)
...
void CryptRandGet(uint8_t *pBuffer, int32_t iBufSize)
{
    ...
    #elif defined(DIRTYCODE_PS3)
    int32_t iResult;
    if ((iResult = sys_get_random_number(pBuffer, iBufSize)) == CELL_OK)
    {
        return;
    }
    NetPrintf(("cryptrand: sys_get_random_number() failed (err=%s); falling through to general-purpose code\n", DirtyErrGetName(iResult)));
    #endif
