...
int32_t NetConnStartup(const char *pParams)
{
    ...
    if (pRef != NULL)
    {
        NetPrintf(("netconnwinrt: NetConnStartup() called while the module is already active\n"));
        return(-1);
    }
...
/*!
    ...
    \Input *pOption - asciiz list of config parameters
                      "peepport=<port>" to specify peer port to be opened by upnp.
...
int32_t NetConnConnect(const NetConfigRecT *pConfig, const char *pOption, int32_t iData)
{
    ...
    if (pOption != NULL)
    {
        ...
        if ((pOpt = strstr(pOption, "peerport=")) != NULL)
        {
            ...
            NetPrintf(("netconnwinrt: peerport=%d\n", pRef->iPeerPort));
        }
    }
...
int32_t NetConnStatus(int32_t iKind, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    // return status of plug (PC is assumed to be an "always-connected" platform)
    if (iKind == 'plug')
    {
        return(TRUE);
    }
