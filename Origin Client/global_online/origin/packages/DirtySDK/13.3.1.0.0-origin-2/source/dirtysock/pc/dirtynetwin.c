...
static int32_t _SocketLookupThread(HostentT *host)
{
    ...
    if ((iResult = getaddrinfo(host->name, NULL, &Hints, &pList)) == 0)
    {
        // extract ip address
        host->addr = SocketNtohl(((struct sockaddr_in *)pList->ai_addr)->sin_addr.s_addr);
    ...
    if (dest->sa_family == AF_INET)
    {
        ...
        if (sock != INVALID_SOCKET)
        {
            ...
            if (SockaddrInGetAddr(host) == 0)
            {
                ...
                {
                    memcpy(host->sa_data + 2, pList->ai_addr->sa_data + 2, 4);
                }
                else
                {
                    NetPrintf(("dirtynetwin: getaddrinfo() failed err=%d\n", iResult));
