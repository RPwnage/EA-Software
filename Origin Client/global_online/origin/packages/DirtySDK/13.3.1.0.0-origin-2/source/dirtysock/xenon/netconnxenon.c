...
typedef struct NetConnRefT
{
    ...
    uint32_t                uMachineId;         //!< Id of the local machine (cached)
...
static void _NetConnExtractSitename(NetConnRefT *pRef, const char *pParams)
{
    ...
    if (strstr(pParams, "dev"))
    {
        ds_strnzcpy(pRef->strXlspSitename, "gos-core-sdev", sizeof(pRef->strXlspSitename));
    }
...
uint32_t NetConnMachineId(void)
{
    NetConnRefT *pRef = _NetConn_pRef;
    char strBuffer[64];
    int iMacAddressSize = 6;

    // return cached value
    if (pRef->uMachineId != 0)
    {
        return(pRef->uMachineId);
    }

    NetConnStatus('xadd', 0, strBuffer, sizeof(XNADDR));
    NetConnStatus('macx', 0, strBuffer + sizeof(XNADDR), iMacAddressSize);

    pRef->uMachineId = NetHashBin(strBuffer, sizeof(XNADDR) + iMacAddressSize);
