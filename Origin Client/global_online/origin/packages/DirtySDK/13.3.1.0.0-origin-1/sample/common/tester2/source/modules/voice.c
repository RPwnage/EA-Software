...
static void _VoiceCreate(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    {
        pApp->pVoip = VoipStartup(iMaxPeers, 1, 0);
    }
    ...
    ds_snzprintf(strUniqueId, sizeof(strUniqueId), "$%08x", uLocalAddr);
...
static void _VoiceConnect(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    if (argc == 3)
    {
        ...
        ZPrintf("%s: connecting to %a (clientId=0x%08x)\n", argv[0], uRemoteAddr, uRemoteAddr);

        // start connect to remote user
        pApp->iConnID = VoipConnect(pApp->pVoip, VOIP_CONNID_NONE, uRemoteAddr, VOIP_PORT, VOIP_PORT, uRemoteAddr, 0);
    }
    else
    {
        // set loopback
