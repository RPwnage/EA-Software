...
static void _NetStatus(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    if (strcmp("tick", argv[2]) == 0)
    {
        ...
        // for now this is the only URN we can use for XSTS token
        // acquisition to work
        NetConnControl('aurn', 0, 0, "10.88.48.71", NULL);

        uResult = NetConnStatus(uToken, 0, (void *)strToken, sizeof(strToken));
    }
    else if (strcmp("gtag", argv[2]) == 0)
    {
        char strGamerTag[16];
    ...
    {
        ZPrintf("%s: status of ('%C') is {'%C')\n", argv[0], uToken, uResult);
    }
    else // display result as decimal and hex
    {
        ZPrintf("%s: status of ('%C') is {%d/0x%08X)\n", argv[0], uToken, uResult, uResult);
    }
}
