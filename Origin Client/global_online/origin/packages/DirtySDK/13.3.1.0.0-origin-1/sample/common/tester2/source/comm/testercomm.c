...
TesterCommT *TesterCommCreate(void)
{
    ...
    ZMemSet((void *)pState, 0, sizeof(TesterCommT));
    pState->pInterface = (TesterCommInterfaceT *)ZMemAlloc(sizeof(TesterCommInterfaceT));
    ZMemSet((void *)pState->pInterface, 0, sizeof(TesterCommInterfaceT));

    // create the lists
    pState->pInputData  = ZListCreate(TESTERCOMM_NUMCOMMANDS_DEFAULT, sizeof(TesterCommDataT));
    pState->pOutputData = ZListCreate(TESTERCOMM_NUMCOMMANDS_DEFAULT, sizeof(TesterCommDataT));
...
int32_t TesterCommMessage(TesterCommT *pState, int32_t iMsgType, const char *pMsgText)
{
    ...
    ds_strnzcpy(Message.strBuffer, pMsgText, sizeof(Message.strBuffer));
    Message.iType = iMsgType;
    return(ZListPushBack(pState->pOutputData, &Message));
}
