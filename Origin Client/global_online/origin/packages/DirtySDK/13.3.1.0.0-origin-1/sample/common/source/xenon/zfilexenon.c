...
/*!
    ...
    \Notes
        ...
        GAME:\, with GAME:\ being the executable directory.
...
int32_t ZFileOpen(const char *pFileName, uint32_t uFlags)
{
    ...
    for (iFile=0; iFile<ZFILE_XENONNUMFILES_DEFAULT; iFile++)
    {
        if (INVALID_HANDLE_VALUE == _FileHandles[iFile])
        {
            ...
            if (_FileHandles[iFile] == INVALID_HANDLE_VALUE)
            {
                // CreateFile failed.
                ZPrintfDbg(("zfilexenon: could not create file '%s' err=%s\n", pFileName, DirtyErrGetName(GetLastError())));
                return(-1);
            }
...
int32_t ZFileWrite(int32_t iFileId, void *pData, int32_t iSize)
{
    ...
    {
        ZPrintfDbg(("zfilexenon: WriteFile() failed err=0x%08x\n", GetLastError()));
        return(-1);
    }
...
*/
/********************************************************************************F*/
int64_t ZFileSeek(int32_t iFileId, int64_t iOffset, uint32_t uFlags)
{
    int64_t iRet = INVALID_SET_FILE_POINTER;
...
int32_t ZFileMkdir(const char *pPathName)
{
    ...
    for (pPath = strPath; ; pPath += 1)
    {
        ...
        {
            int32_t iResult;
            if ((iResult = CreateDirectory(strPath, NULL)) != 0)
            {
                if ((iResult = GetLastError()) == ERROR_ALREADY_EXISTS)
                {
                    ZPrintfDbg(("zfilepc: directory %s already exists\n", strPath));
                }
                else
                {
                    ZPrintfDbg(("zfilepc: could not create directory '%s' err=%s\n", strPath, DirtyErrGetName(iResult)));
                    return(-1);
                }
            }
        }
