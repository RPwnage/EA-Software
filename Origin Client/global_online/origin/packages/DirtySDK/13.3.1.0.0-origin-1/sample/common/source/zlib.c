...
#if !defined(_WIN32) || defined(DIRTYCODE_WINRT) || defined(DIRTYCODE_WINPRT)
...
struct ZEnviron
{
    ...
    char strArgb[1024];         //!< buffer to hold entry parms
};
...
*/
/********************************************************************************F*/
ZEnviron *ZCreate(ZConsole *console, char *cmdline)
{
    char term;
    char *s;
    ZEnviron *env;
...
void ZPrintf(const char *fmt, ...)
{
    ...
    if (iOutput != 0)
    {
        #if defined(DIRTYCODE_PC) || defined(DIRTYCODE_WINRT) || defined(DIRTYCODE_WINPRT)
