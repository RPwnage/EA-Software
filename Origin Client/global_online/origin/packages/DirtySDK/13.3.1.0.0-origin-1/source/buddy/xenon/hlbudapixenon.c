    ...
    if (NetConnStatus('fref', 0, &pFriendApiRef, sizeof(pFriendApiRef)) == 0)
    {
        if (!FriendApiGetFriendsList(pFriendApiRef, iUserIndex, NULL, aOnlineFriends, &iFriendsCount))
        {
            ...
            for (iFriend = 0; iFriend < iFriendsCount; iFriend++)
            {
                ...
                {
                    pOnlineFriend = & aOnlineFriends[iFriend];
                    break;
                }
            }
        }
...
*/
/*************************************************************************************************F*/
static XONLINE_FRIEND * _HLBFriendsGetFriendByIndex(const int32_t iUserIndex, const int32_t iFriendIndex)
{
    FriendApiRefT *pFriendApiRef;
    int32_t iFriendsCount;
    XONLINE_FRIEND *pOnlineFriend = 0;
...
{ 
    ...
    pFriend = _HLBFriendsGetFriendByIndex(pApiRef->iUserIndex, index);
...
void _reloadFriends (HLBApiRefT *pApiRef)
{
    int32_t index = 0;
    int32_t totalBud = 0;
    PXONLINE_FRIEND pFriend = _HLBFriendsGetFriendByIndex(pApiRef->iUserIndex, index);
