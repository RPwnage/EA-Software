...
/*!
    ...
    \Copyright
        Copyright (c) Tiburon Entertainment / Electronic Arts 2001-2004.    ALL RIGHTS RESERVED.
...
#define NETCONN_MAXIDLEHANDLERS     (32)
...
/*!
    ...
    \Description
        ...
        This function needs to be called periodically for the network code to function
        properly.  Internally it calls RpcDone(), followed by any idle functions that have been
        installed.  It also limits RpcDone() and idle functions from executing more than once
        every 5ms.
...
/*!
    ...
    \Description
        Generates a hopefully unique id for this machine by hashing together a bunch
        of local values.

    \Output
        uint32_t        - machine id of this machine.

    \Version 02/14/2011 (jrainy)
*/
/********************************************************************************F*/
uint32_t NetConnMachineId(void)
{
    static uint32_t _MachineId = 0;
    char strMAC[6];
    ...
    _MachineId = NetConnStatus('addr', 0, NULL, 0);
    NetConnStatus('macx', 0, strMAC, sizeof(strMAC));

    _MachineId ^= NetHashBin(strMAC, sizeof(strMAC));
