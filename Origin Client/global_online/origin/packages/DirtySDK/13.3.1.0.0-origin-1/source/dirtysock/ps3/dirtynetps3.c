...
static void _SocketLookupThread(uint64_t _pRef)
{
    ...
    /*
    This function explicitly frees thread information created implicitly within libnet.
    We call it here to avoid these resources remaining allocated while we sleep.
    This proved to help avoiding gethostbyname() returning NETDB_INTERNAL while several
    name resolutions were in progress concurrently.
    */
