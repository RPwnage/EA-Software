...
static void _HttpManagerRequestCheckCompletion(HttpManagerRefT *pHttpManager, HttpManagerHttpCmdT *pHttpCmd)
{
    ...
    int32_t iHeadSize, iStatus, iRsltCode = 0;
...
*/
/********************************************************************************F*/
static int32_t _HttpManagerRequestCb(HttpManagerRefT *pHttpManager, int32_t iHandle, const char *pUrl, const char *pData, int64_t iDataSize, ProtoHttpRequestTypeE eRequestType, ProtoHttpWriteCbT *pWriteCb, void *pWriteCbUserData)
{
    HttpManagerHttpCmdT *pHttpCmd;
...
static void _HttpManagerDestroyRef(HttpManagerRefT *pHttpManager, int32_t iHttpRef)
{
    ...
    for (iTransaction = 0; iTransaction < pHttpRef->iTransactions; iTransaction += 1)
    {
        ...
        {
            NetPrintfVerbose((pHttpManager->iVerbose, 0, "protohttpmanager: destroying active ref %2d; handle %d is being terminated\n", iHttpRef, pHttpCmd->iHttpHandle));
    ...
    // destroy ref
    NetPrintfVerbose((pHttpManager->iVerbose, 0, "protohttpmanager: destroying http ref %d\n", iHttpRef));
...
int32_t HttpManagerControl(HttpManagerRefT *pHttpManager, int32_t iHandle, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    else // global (not command-specific) selectors
    {
        ...
        if (iSelect == 'auto')
        {
            NetPrintfVerbose((pHttpManager->iVerbose, 0, "protohttpmanager: auto-update %s\n", iValue ? "enabled" : "disabled"));
        ...
        if (iSelect == 'copy')
        {
            NetPrintfVerbose((pHttpManager->iVerbose, 0, "protohttpmanager: urlcopy %s\n", iValue ? "enabled" : "disabled"));
        ...
        if (iSelect == 'maxp')
        {
            NetPrintfVerbose((pHttpManager->iVerbose, 0, "protohttpmanager: setting max pipeline depth to %d\n", iValue));
        ...
        if (iSelect == 'pipe')
        {
            NetPrintfVerbose((pHttpManager->iVerbose, 0, "protohttpmanager: pipelining %s\n", iValue ? "enabled" : "disabled"));
        ...
        if (iSelect == 'pwka')
        {
            NetPrintfVerbose((pHttpManager->iVerbose, 0, "protohttpmanager: pipelining without keep-alive %s\n", iValue ? "enabled" : "disabled"));
        ...
        if (iSelect == 'xhtp')
        {
            NetPrintfVerbose((pHttpManager->iVerbose, 0, "protohttpmanager: enabling xhttp\n"));
    ...
    if (iHandle > 0)
    {
        ...
        if (iSelect == 'href')
        {
            if ((pHttpCmd->pHttpRef != NULL) && (pHttpCmd->pHttpRef->pProtoHttp != NULL))
            {
                ...
                {
                    memcpy(pBuffer, &pHttpCmd->pHttpRef->pProtoHttp, sizeof(void *));
                }
                return(0);
            }
            return(-1);
    ...
    else
    {
        ...
        if (iSelect == 'hndl')
        {
            if ((pBuffer != NULL) && (iBufSize == sizeof(void *)))
            {
                ...
                memcpy(&pProtoHttpFind, pBuffer, sizeof(void *));
