...
/* Grafted from DirtySDK version 15.1.1.1.0 - per jbrookes (07/16/2015) to get IcmpSendEcho() version that doesn't require the user to be an admin (fixes ORPLAT-988) */
...
typedef struct ProtoPingIcmpResponseT
{
    ...
    EchoDataT data;
    char icmpError[8];
    char buffer[32];  // extra buffer needed for XP (for some reason)
