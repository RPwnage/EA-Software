...
/*!
    ...
    \verbatim
        ...
        Packet format
            ...
            0x0000...0x07ff (Xbox 360)
            0x0800 = Other  (no encryption / Durango)
            0x0801...0x7fff (reserved, future use)
            0x8000 = Other  (with encryption)
               &0x7fff = encrypted stream offset
...
#if !defined(DIRTYCODE_XENON)
 #define PROTOTUNNEL_GetHeaderFromPacket(__pPacketData) (((__pPacketData)[1] << 8) | (__pPacketData)[0])
#else
 #define PROTOTUNNEL_GetHeaderFromPacket(__pPacketData) (((__pPacketData)[0] << 8) | (__pPacketData)[1])
#endif
#define PROTOTUNNEL_GetOffsetFromPacket(__pPacketData) (PROTOTUNNEL_GetHeaderFromPacket(__pPacketData) & 0x7fff)
...
static void _ProtoTunnelBufferSend(ProtoTunnelRefT *pProtoTunnel, ProtoTunnelT *pTunnel, uint32_t uCurTick)
{
    ...
    if (pTunnel->Info.uTunnelMode == PROTOTUNNEL_MODE_GENERIC)
    {
        ...
        aPacketData[0] = (uint8_t)(uSendHeader >> 0);
        aPacketData[1] = (uint8_t)(uSendHeader >> 8);
    ...
    else if (pTunnel->Info.uTunnelMode == PROTOTUNNEL_MODE_XENON)
    {
        ...
        #else
        aPacketData[0] = (uint8_t)(iCryptSize >> 0);
        aPacketData[1] = (uint8_t)(iCryptSize >> 8);
        #endif
    }
    else // PROTOTUNNEL_MODE_NOCRYPT)
    {
        aPacketData[0] = (uint8_t)(PROTOTUNNEL_HEADER_NOCRYPT >> 0);
        aPacketData[1] = (uint8_t)(PROTOTUNNEL_HEADER_NOCRYPT >> 8);
    }

    // accumulate stats
    pTunnel->SendStat.uLastUpdate = uCurTick;
...
static int32_t _ProtoTunnelBufferData(ProtoTunnelRefT *pProtoTunnel, ProtoTunnelT *pTunnel, const uint8_t *pData, uint32_t uSize, uint32_t uPortIdx, uint8_t bForceCrypt)
{
    ...
    {
        NetPrintf(("prototunnel: packet of size %d will not be tunneled (max size = %d)\n", uSize, PROTOTUNNEL_MAXPACKET));
...
*/
/********************************************************************************F*/
static ProtoTunnelModeE _ProtoTunnelGetModeFromPacket(const uint8_t *pPacketData, int32_t iRecvLen)
{
    uint16_t uHeader = PROTOTUNNEL_GetHeaderFromPacket(pPacketData);
    ProtoTunnelModeE eMode = PROTOTUNNEL_MODE_GENERIC;
    if (uHeader < 0x800)
    {
        eMode = PROTOTUNNEL_MODE_XENON;
    }
    else if (uHeader == 0x800)
    {
...
*/
/********************************************************************************F*/
static int32_t _ProtoTunnelDecryptAndValidatePacket(ProtoTunnelRefT *pProtoTunnel, ProtoTunnelT *pTunnel, uint8_t *pPacketData, int32_t iRecvLen)
{
    ...
    uRecvOffset = PROTOTUNNEL_GetOffsetFromPacket(pPacketData);
...
{
    uint32_t uRecvAddr, uRecvPort, uRecvOffset;
    int32_t iNumPackets, iTunnel;
    ProtoTunnelT *pTunnel;
    uint32_t uCurTick = NetTick();
    ProtoTunnelModeE ePacketMode = _ProtoTunnelGetModeFromPacket(pPacketData, iRecvLen);
    ...
    if (iTunnel == pProtoTunnel->iMaxTunnels)
    {
        ...
        if (((uRecvOffset = PROTOTUNNEL_GetOffsetFromPacket(pPacketData)) < (8192/PROTOTUNNEL_CRYPTALGN)) || (ePacketMode != PROTOTUNNEL_MODE_GENERIC))
        {
    ...
    if ((iTunnel == pProtoTunnel->iMaxTunnels) && (ePacketMode == PROTOTUNNEL_MODE_GENERIC))
    {
        for (iTunnel = 0, uRecvAddr = SockaddrInGetAddr(pRecvAddr), uRecvPort = SockaddrInGetPort(pRecvAddr); iTunnel < pProtoTunnel->iMaxTunnels; iTunnel++)
        {
            ...
            /* exclude tunnels we have received data on recently (within the last minute).  a port change
               ...
               instances */
            if (NetTickDiff(uCurTick, pTunnel->RecvStat.uLastUpdate) < 60*1000)
            {
                continue;
            }
            ...
            if ((pTunnel->Info.uRemoteAddr == uRecvAddr) && (pTunnel->uActive == 1))
            {
                ...
                if (_ProtoTunnelMatchTunnel(pProtoTunnel, pTunnel, pPacketData, iRecvLen) != 0)
                {
                    ...
                    NetPrintf(("prototunnel: detected port change; updating remote port from %d to %d\n", pTunnel->Info.uRemotePort, uRecvPort));
                    pTunnel->Info.uRemotePort = uRecvPort;
                    break;
                }
            }
    ...
    if (iTunnel < pProtoTunnel->iMaxTunnels)
    {
        ...
        if (pTunnel->Info.uTunnelMode == PROTOTUNNEL_MODE_GENERIC)
        {
            iNumPackets = _ProtoTunnelDecryptAndValidatePacket(pProtoTunnel, pTunnel, pPacketData, iRecvLen);
        }
        else
        {
        ...
        if (iNumPackets > 0)
        {
            ...
            pTunnel->RecvStat.uLastUpdate = uCurTick;
...
int32_t ProtoTunnelValidatePacket(ProtoTunnelRefT *pProtoTunnel, ProtoTunnelT *pTunnel, uint8_t *pOutputData, const uint8_t *pPacketData, int32_t iPacketSize, const char *pKey)
{
    ...
    // get stream offset from packet header - this tells us where we need to be to be able to decrypt the packet
    uRecvOffset = PROTOTUNNEL_GetOffsetFromPacket(_pPacketData);
    ...
    {
        // calc how many data units we've missed
        int32_t iRecvDiff = (signed)uRecvOffset << PROTOTUNNEL_CRYPTBITS;

        // advance the cipher to resync
        NetPrintf(("prototunnel: stream skip %d bytes\n", iRecvDiff));
        CryptArc4Advance(&CryptRecvState, iRecvDiff);
    }
