...
static void _ProtoStreamUpdateStats(ProtoStreamRefT *pProtoStream, int32_t iDataRead)
{
    ...
    if ((iRecvElapsed >= 0) && (iRecvElapsed < PROTOSTREAM_SAMPLE_PERIOD))
    {
        ...
        NetPrintfVerbose((pProtoStream->bVerbose, 0, "protostream: dev=%5d avg=%5d len=%5d thr=%d\n",
...
static int32_t _ProtoStreamDataCallback(ProtoStreamRefT *pProtoStream, ProtoStreamStatusE eStatus)
{
    ...
    if (iRead > 0)
    {
        NetPrintfVerbose((pProtoStream->bVerbose, 0, "protostream: read [0x%04x,0x%04x]\n", pProtoStream->iBufOut, pProtoStream->iBufOut+iRead));
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'xhtp' - Select XHTTP (TRUE) or ProtoHttp (FALSE) (default=FALSE) (xenon only)
        \endverbatim
...
int32_t ProtoStreamControl(ProtoStreamRefT *pProtoStream, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'xhtp')
    {
        // enable/disable xhttp
        ProtoHttpControl(pProtoStream->pProtoHttp, 'xhtp', iValue, 0, NULL);

        if (iValue != FALSE)
        {
            // resize input buffer to provide better throughput; testing shows our XHTTP implementation needs this
            ProtoHttpControl(pProtoStream->pProtoHttp, 'ires', (64 * 1024), 0, NULL);
        }
