...
typedef struct VoipConnectionT
{
    ...
    VoipMicrPacketT VoipMicrPacket[VOIP_MAXLOCALUSERS]; //!< buffered voip packet (for sending), one per local user

    uint32_t        aVoiceSendTimer[VOIP_MAXLOCALUSERS]; //!< timer used to rate limit voice packets to ten per second
    uint32_t        aRecvChannels[VOIP_MAXLOCALUSERS]; //!< id of the channels to which remote users on this connection are subscribed to
    uint32_t        aLastRecvChannels[VOIP_MAXLOCALUSERS]; //!< last value of aRecvChannels
...
typedef struct VoipConnectionlistT
{
    ...
    uint32_t        aChannels[VOIP_MAXLOCALUSERS]; //!< ids of the voip channels that local users are subscribed to
