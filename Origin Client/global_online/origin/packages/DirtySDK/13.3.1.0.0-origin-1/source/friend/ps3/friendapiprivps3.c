...
static int32_t _FriendApiUpdateFriend(FriendApiRefT *pFriendApiRef, SceNpUserInfo userInfo, uint8_t bPresent, int32_t iBufferSize, int8_t *strBuffer)
{
    ...
    if (iResult >= 0)
    {
        ...
        if(strBuffer)
        {
            memcpy(pFriendApiRef->friendsList.aFriends[iResult].strPresenceData, strBuffer, iBufferSize);
        }
    }
    ...
    if ((iNpStatus = sceNpBasicGetEvent(&iNpEvent, &userInfo, &aDataBuf, (size_t *)&iDataBufSize)) == 0)
    {
        ...
        else
        {
            ...
            default:
                ...
                eventData.eventTypeData.Unkown.pRawData1 = & iDataBufSize;
                eventData.eventTypeData.Unkown.pRawData2 = aDataBuf;
                eventData.eventTypeData.Unkown.pFriendData = & friendData;
