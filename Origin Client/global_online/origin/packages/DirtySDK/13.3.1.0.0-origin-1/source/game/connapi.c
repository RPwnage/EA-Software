...
struct ConnApiRefT
{
    ...
    /* Multiple virtual machines is a debugging feature, and should not be used in production.
       This feature is PC only, and disables MLU allowing multiple clients running on the same
       machine */
    uint8_t                 bMultipleVirtualMachines;
...
static void _ConnApiInitClient(ConnApiRefT *pConnApi, ConnApiClientT *pClient, ConnApiClientInfoT *pClientInfo, int32_t iClientIndex)
{
    ...
    if (pClient->ClientInfo.strUniqueId[0] == '\0')
    {
    ...
    if (pClient->ClientInfo.uTunnelMode != PROTOTUNNEL_MODE_GENERIC)
    {
...
static int32_t _ConnApiFindClientByName(ConnApiClientListT *pClientList, const char *pClientName)
{
    ...
    for (iClientIndex = 0; iClientIndex < pClientList->iMaxClients; iClientIndex++)
    {
        ...
        {
            if (!DirtyUsernameCompare(pClientList->Clients[iClientIndex].ClientInfo.UserInfo[iUserIndex].strName, pClientName))
        {
            return(iClientIndex);
        }
    }
...
static int32_t _ConnApiTunnelAlloc(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex, uint32_t uRemoteAddr, uint8_t bLocalAddr)
{
    ...
    if (!pConnApi->bMultipleVirtualMachines)
    {
        TunnelInfo.uRemoteClientId =  pClient->ClientInfo.uMachineId;
    }
    else
    {
        TunnelInfo.uRemoteClientId = NetHash(pClient->ClientInfo.UserInfo[0].strName);
    }
    ...
    TunnelInfo.uTunnelMode = pClient->ClientInfo.uTunnelMode;
    NetPrintf(("connapi: [0x%08x] setting client %d TunnelInfo.uRemotePort %d TunnelInfo.uTunnelMode %d\n", pConnApi, iClientIndex, TunnelInfo.uRemotePort, TunnelInfo.uTunnelMode));
...
static int32_t _ConnApiGetConnectAddr(ConnApiRefT *pConnApi, ConnApiClientT *pClient, uint8_t *pLocalAddr, const uint32_t uConnMode, ConnApiClientT **pClientUsedRet)
{
    ...
    if (!_ConnApiGameServerEnabled(pConnApi, pClient, uConnMode))
    {
        ...
        if ((uConnMode == CONNAPI_CONNFLAG_VOIPCONN) && pConnApi->bVoipRedirectViaHost)
        {
            ...
            NetPrintf(("connapi: [%s] using host address to connect to (or disconnect from) user '%s'\n", pConn, _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
        }
        else
        {
            ...
            {
                NetPrintf(("connapi: [%s] using local address to connect to (or disconnect from) user '%s'\n", pConn, _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
            ...
            {
                NetPrintf(("connapi: [%s] using peer address to connect to (or disconnect from) user '%s'\n", pConn, _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
...
static void _ConnApiVoipGroupConnSharingCallback(VoipGroupRefT *pVoipGroup, ConnSharingCbTypeE eCbType, int32_t iConnId, void *pUserData)
{
    ...
    switch (eCbType)
    {
        case (VOIPGROUP_CBTYPE_CONNSUSPEND):
            NetPrintf(("connapi: suspending voip connection to user %s:%a at %d\n", _ConnApiUserNames(pConnApi, &pClient->ClientInfo), iConnectAddr, NetTick()));
            VoipGroupSuspend(pVoipGroup, iConnId);
            break;

        case (VOIPGROUP_CBTYPE_CONNRESUME):
            NetPrintf(("connapi: resuming voip connection to user %s:%a at %d\n", _ConnApiUserNames(pConnApi, &pClient->ClientInfo), iConnectAddr, NetTick()));
            VoipGroupResume(pVoipGroup, iConnId, iConnectAddr, pClient->VoipInfo.uMnglPort, pClient->VoipInfo.uLocalPort, pClient->ClientInfo.uMachineId, pConnApi->iSessId);
            break;
...
static void _ConnApiDestroyGameConnection(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex, const char *pReason)
{
    ...
    // destroy the refs
    NetPrintf(("connapi: destroying game connection to user %s: %s at %d\n", _ConnApiUserNames(pConnApi, &pClient->ClientInfo), pReason, NetTick()));
...
static void _ConnApiDestroyVoipConnection(ConnApiRefT *pConnApi, ConnApiClientT *pClient, const char *pReason)
{
    ...
    {
        NetPrintf(("connapi: destroying voip connection to user %s: %s at %d\n", _ConnApiUserNames(pConnApi, &pClient->ClientInfo), pReason, NetTick()));
...
static void _ConnApiInitClientList(ConnApiRefT *pConnApi, ConnApiClientInfoT *pClientList, int32_t iNumClients)
{
    ...
    for (iClient = 0, pConnApi->ClientList.iNumClients = 0; iClient < iNumClients; iClient++)
    {
        ...
        if (iUser != NETCONN_MAXLOCALUSERS)
        {
            ...
            if (((!pConnApi->bMultipleVirtualMachines) && (pClient->ClientInfo.uMachineId == NetConnMachineId()))
             || (( pConnApi->bMultipleVirtualMachines) && (!ds_stricmp(pConnApi->strSelfName, pClientList[iClient].UserInfo[0].strName))))
            {
    ...
    if (pConnApi->pVoipRef != NULL)
    {
        ...
        {
            VoipGroupControl(pConnApi->pVoipGroupRef, 'lusr', TRUE, pClient->ClientInfo.strUniqueId);
        }
    ...
    if (pConnApi->pProtoTunnel != NULL)
    {
        ...
        {
            ProtoTunnelControl(pConnApi->pProtoTunnel, 'clid', NetHash(pClient->ClientInfo.UserInfo[0].strName), 0, NULL);
        }
    }
    ...
    if (pConnApi->iSelf == -1)
    {
        NetPrintf(("connapi: local machine %08x not found in client list\n", NetConnMachineId()));
    }
    // display client list
    ...
    for (pAdvt = strAdvtList, iAdvt = 0; iAdvt < iNumAdvt; iAdvt++)
    {
        ...
        *pTemp = '\0';
        ...
        for (iClient = 0; iClient < pConnApi->ClientList.iMaxClients; iClient++)
        {
            ...
            if ((uAdvtId == pClient->ClientInfo.uMachineId) && (pClient->ClientInfo.uLocalAddr != uLocalAddr))
            {
...
static void _ConnApiRemoveClient(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex)
{
    ...
    {
        NetPrintf(("connapi: aborting demangle of user %s being removed from client list\n", _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
...
static ConnApiConnStatusE _ConnApiCheckGameServerFallback(ConnApiRefT *pConnApi, ConnApiClientT *pClient, uint32_t uGameServConnMode, int32_t iClientIndex)
{
    ...
    {
        NetPrintf(("connapi: [%s] status=init, connection failed, using gameserver fallback for connection to user '%s'\n", pConn, _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
        ...
        NetPrintf(("connapi: freeing previous tunnel for user '%s'\n", _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
        _ConnApiTunnelFree(pConnApi, pClient);
    }
    else
    {
...
static int32_t _ConnApiUpdateGameClient(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex)
{
    ...
    if (pClient->GameInfo.eStatus == CONNAPI_STATUS_INIT)
    {
        ...
        {
            NetPrintf(("connapi: unable to allocate util ref for connection %d to user %s\n", iClientIndex, _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
            pClient->GameInfo.eStatus = CONNAPI_STATUS_DISC;
        }
    }
    ...
    if (pClient->GameInfo.eStatus == CONNAPI_STATUS_CONN)
    {
        ...
        if ((pCommRef = NetGameUtilComplete(pClient->pGameUtilRef)) != NULL)
        {
            ...
            if (pClient->pGameLinkRef != NULL)
            {
                NetPrintf(("connapi: game connection %d to user %s established at %d\n", iClientIndex, _ConnApiUserNames(pConnApi, &pClient->ClientInfo), NetTick()));
            ...
            {
                NetPrintf(("connapi: unable to allocate link ref for connection %d to user %s\n", iClientIndex, _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
                pClient->GameInfo.eStatus = CONNAPI_STATUS_DISC;
            }
        }
        
        // check for connection timeout
        if ((pClient->GameInfo.eStatus == CONNAPI_STATUS_CONN) && (NetTickDiff(NetTick(), pClient->iConnStart) > pConnApi->iConnTimeout))
        {
            ...
            if (pClient->GameInfo.bDemangling == FALSE)
            {
                ...
                {
                    NetPrintf(("connapi: game status=mngl for connection %d to user %s\n", iClientIndex, _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
            ...
            {
                NetPrintf(("connapi: game connection to user %s after demangle failed\n", _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
...
static int32_t _ConnApiUpdateVoipClient(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex)
{
    ...
    if (pClient->VoipInfo.eStatus == CONNAPI_STATUS_INIT)
    {
        ...
        iVoipConnId = VoipGroupConnect(pConnApi->pVoipGroupRef, iClientIndex, iConnectAddr, pClient->VoipInfo.uMnglPort, pClient->VoipInfo.uLocalPort, pClient->ClientInfo.uMachineId, pConnApi->iSessId);
        if (iVoipConnId >= 0)
        {
        ...
        {
            NetPrintf(("connapi: unable to init voip for connection %d to user %s\n", iClientIndex, _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
            pClient->VoipInfo.eStatus = CONNAPI_STATUS_DISC;
        }
    }
    ...
    if (pClient->VoipInfo.eStatus == CONNAPI_STATUS_CONN)
    {
        ...
        {
            NetPrintf(("connapi: voip connection %d to user %s established at %d\n", iClientIndex, _ConnApiUserNames(pConnApi, &pClient->ClientInfo), NetTick()));
        ...
        {
            NetPrintf(("connapi: voip connection attempt to user %s failed at %d\n", _ConnApiUserNames(pConnApi, &pClient->ClientInfo), NetTick()));
        
            if (pClient->VoipInfo.bDemangling == TRUE)
            {
                NetPrintf(("connapi: voip connection attempt to user %s after demangle failed\n", _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
            ...
            else
            {
                if (pConnApi->bDemanglerEnabled)
                {
                    NetPrintf(("connapi: voip status=mngl for connection %d to user %s\n", iClientIndex, _ConnApiUserNames(pConnApi, &pClient->ClientInfo)));
    ...
    if (pClient->VoipInfo.eStatus == CONNAPI_STATUS_ACTV)
    {
        ...
        {
            NetPrintf(("connapi: voip connection to user %s terminated at %d\n", _ConnApiUserNames(pConnApi, &pClient->ClientInfo), NetTick()));
            pClient->VoipInfo.eStatus = CONNAPI_STATUS_DISC;
        }
    }        
...
static void _ConnApiUpdateDemangle(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex, ConnApiConnInfoT *pConnInfo, int32_t iType)
{
    ...
    if (ProtoMangleStatus(pConnApi->pProtoMangle, 'idle', NULL, 0))
    {
        ...
        if (uSockRef == 0)
        {
            NetPrintf(("connapi: initiating %s:%d demangle of user %s at %d\n", _Types[iType], uDemanglePort, _ConnApiUserNames(pConnApi, &pClient->ClientInfo), NetTick()));
            ProtoMangleConnect(pConnApi->pProtoMangle, uDemanglePort, strSess);
        }
        else
        {        
            NetPrintf(("connapi: initiating %s demangle of user %s using sockref 0x%08x at %d\n", _Types[iType], _ConnApiUserNames(pConnApi, &pClient->ClientInfo), uSockRef, NetTick()));
    ...
    else
    {
        ...
        if ((iResult = ProtoMangleComplete(pConnApi->pProtoMangle, &iAddr, &iPort)) != 0)
        {
            ...
            if (eStatus != CONNAPI_STATUS_ACTV)
            {
                if (iResult > 0)
                {
                    NetPrintf(("connapi: %s demangle of user %s successful port=%d at %d\n", _Types[iType], _ConnApiUserNames(pConnApi, &pClient->ClientInfo), iPort, NetTick()));
                ...
                {
                    NetPrintf(("connapi: %s demangle of user %s failed (timeout=%s)\n", _Types[iType], _ConnApiUserNames(pConnApi, &pClient->ClientInfo),
            ...
            else
            {
                NetPrintf(("connapi: [0x%08x] %s demangle of user %s finished with %d but ignored because connection aleady active (timeout=%s)\n", pConnApi,
                    _Types[iType], _ConnApiUserNames(pConnApi, &pClient->ClientInfo), iResult, ProtoMangleStatus(pConnApi->pProtoMangle, 'time', NULL, 0) ? "true" : "false"));
            }
...
void ConnApiOnline(ConnApiRefT *pConnApi, const char *pGameName, const char *pSelfName, DirtyAddrT *pSelfAddr)
{
    ...
    ds_strnzcpy(pConnApi->strSelfName, pSelfName, sizeof(pConnApi->strSelfName));
    ...
    ds_snzprintf(strAdvt, sizeof(strAdvt), "%u", NetConnMachineId());
...
*/
/********************************************************************************F*/
int32_t ConnApiAddUser(ConnApiRefT *pConnApi, int32_t iClientIdx, ConnApiUserInfoT *pUserInfo)
{
    return(0);
}
...
/*!
    ...
    \Input *pConnApi    - pointer to module state
    \Input iClientIdx   - index of client to add user to
    \Input iUserIdx     - index of the user to remove

    \Version 04/07/2011 (jrainy)
*/
/********************************************************************************F*/
void ConnApiRemoveUser(ConnApiRefT *pConnApi, int32_t iClientIdx, int32_t iUserIdx)
{
...
void ConnApiDisconnect(ConnApiRefT *pConnApi)
{
    ...
    {
        pClient = &pConnApi->ClientList.Clients[pConnApi->iSelf];
        VoipGroupControl(pConnApi->pVoipGroupRef, 'lusr', FALSE, pClient->ClientInfo.strUniqueId);
    }
...
int32_t ConnApiStatus2(ConnApiRefT *pConnApi, int32_t iSelect, void *pData, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'mvtm')
    {
        return(pConnApi->bMultipleVirtualMachines);
    }
    if (iSelect == 'mwid')
    {
...
int32_t ConnApiControl(ConnApiRefT *pConnApi, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'mvtm')
    {
        // set machine id generation mode
        pConnApi->bMultipleVirtualMachines = iValue;

        if (pConnApi->bMultipleVirtualMachines)
        {
            NetPrintf(("connapi: [0x%08x] multiple virtual machines mode enabled\n", pConnApi));
        }
