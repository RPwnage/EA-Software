...
typedef struct DirtyGifDecoderT
{
    ...
    uint8_t *pByteStream;                       //!< pointer to byte stream
    ...
    uint8_t *pCodeData;                         //!< current image data pointer
    uint8_t *pEndCodeData;                      //!< end of image data
...
*/
/********************************************************************************F*/
static uint8_t *_DirtyGifParseColorTable(DirtyGifHdrT *pGifHdr, uint8_t *pGifData, uint8_t *pGifEnd, uint32_t uBits)
{
    if (uBits & 0x80)
    {
...
*/
/********************************************************************************F*/
static int32_t _DirtyGifParseHeader(DirtyGifHdrT *pGifHdr, uint8_t *pGifData, uint8_t *pGifEnd)
{
    // iClearCode header
    memset(pGifHdr, 0, sizeof(*pGifHdr));
...
{
    return(_DirtyGifParseHeader(pGifHdr, (uint8_t *)pGifData, (uint8_t *)pGifEnd));
}
...
{
    uint8_t *pColorTable, *pPalStart = pPalette;
...
int32_t DirtyGifDecodeImage(DirtyGifHdrT *pGifHdr, uint8_t *pImageData, int32_t iBufWidth, int32_t iBufHeight, uint32_t bVflip)
{
    ...
    return(pDecoder->pCodeData - pGifHdr->pImageData);
}
