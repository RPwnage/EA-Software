...
namespace EA.Eaconfig
{
    ...
    public class LoadPlatformConfigTask : Task
    {
        ...
        protected override void ExecuteTask()
        {
            ...
            bool hasPlatformConfigPackage = PlatformConfigPackageVersion != null && (false == PlatformConfigPackageVersion.Equals("ignore", StringComparison.OrdinalIgnoreCase));

            if (!hasPlatformConfigPackage)
            {
                // Load traditional config file.
                
                ConfigFile = Path.Combine(PackageMap.Instance.ConfigDir, Config + ".xml");
            ...
            else // Try to load config package
            {
                ...
                Project.Properties["nant.platform-config-package-name"] = PlatformConfigPackage;
            ...
            if (hasPlatformConfigPackage)
            {
                ...
                foreach (string line in StringUtil.ToArray(Include.Value, "\n\r"))
                {
                    ...
                    {
                        string fullpath = Path.Combine(Project.BaseDirectory, StringUtil.TrimQuotes(StringUtil.Trim(Project.ExpandProperties(line))));
                        string file = PathString.MakeNormalized(fullpath, PathString.PathParam.NormalizeOnly).Path;
                        if (File.Exists(file))
                        {
                            IncludeFile(file);
                        }
                        ...
                        {
                            Project.Log.Warning.WriteLine("PlatformLoader: file '{0}' does not exist.", file);
                        }
                    }
                }
            }
        ...
        {
            Log.Info.WriteLine(LogPrefix + "confifuration file: '{0}'", file);
