	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		public int FindEntry(string name, bool ignoreCase)
		{
			...
			for (int i = 0; i < entries_.Length; i++) {
                if (String.Equals(name, entries_[i].Name, StringComparison.OrdinalIgnoreCase))
                {
					return i;
				}
			}
...
/*
			...
			for (int index = 0; index < updates_.Count; ++index)
			{
				...
				if ( (zu.Entry.ZipFileIndex == entry.ZipFileIndex) &&
					(string.Compare(convertedName, zu.Entry.Name, true, CultureInfo.Ordinal) == 0) ) {
					result = index;
					break;
				}
			}
