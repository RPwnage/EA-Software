...
namespace NAnt.DotNetTasks {
    ...
    public abstract class CompilerBase : ExternalProgramBase {
        ...
        protected override void ExecuteTask() {
            ...
            using (PackageAutoBuilCleanMap.PackageAutoBuilCleanState buildState = PackageAutoBuilCleanMap.AssemblyAutoBuilCleanMap.StartBuild(output, "dotnet"))
            {
                if (buildState.IsDone())
                {
                    Log.Debug.WriteLine("{0} already built.", output);
                ...
                else
                {
                    ...
                    if (NeedsCompiling())
                    {
                        ...
                        try
                        {
                            ...
                            foreach (var file in NantBackCompatibility.GetCompatibleReferences(References))
                            {
                                WriteOption(writer, "reference", file);
                            }
