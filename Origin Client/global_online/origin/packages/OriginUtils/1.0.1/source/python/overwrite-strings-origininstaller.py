...
PATH_PATTERN_LIST_OF_LISTS = [
	#["../activation/assets/lang", "*AccessStrings*"]
]

#portions of paths in p4 to be opened
P4_DEST_PATHS = [
	"/resources/Translations.nsh"
]

INSTALLER_WINDOWS_DEST_PATH = "installer/resources"

#source paths for language strings
#XML_FILE_SOURCE_PATH = 'target\XML\\'
NSH_FILE_SOURCE_PATH = 'temp/lang/Translations.nsh'
...
def main():
    ...
    changelist["Description"] = "[00000] Hudson Language Strings update: see CL for affected files"
...
def overwriteOrAddLanguageFiles():
    ...
    installerFileP4Path = "/".join([basepath, "/".join(INSTALLER_WINDOWS_DEST_PATH.split("/")[1:]), os.path.basename(NSH_FILE_SOURCE_PATH)])
    
    openedFiles = [installerFileP4Path]
...
#    print "Parent project path = " + destParentProject
    for file in files:
        ...
        p4.run_add(relativeWindowsPath)
