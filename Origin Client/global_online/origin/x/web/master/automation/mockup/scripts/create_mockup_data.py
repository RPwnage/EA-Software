...
ENDPOINTS = { 'userPID' : 'https://gateway.int.ea.com/proxy/identity/pids/me',
			  ...
			  'contentUpdates':'https://qa.ecommerce2.dm.origin.com/ecommerce2/contentupdates/{0}?lastModifiedDate={1}&machine_hash=1',
			  ...
			  'friendsForUserData' : 'https://qa.atom.dm.origin.com/atom/users/{0}/other/{1}/friends',
			  'gamesOwnedForUserData' : 'https://qa.atom.dm.origin.com/atom/users/{0}/other/{1}/games',
			  'friendRecommendations' : 'https://integration.friends.dm.origin.com/friends/user/{0}/recommendations',
			  'searchPeopleData' : 'https://qa-main.web.dm.origin.com/web/searchInXml?searchkeyword={0}&authToken={1}&currentPageNo={2}',
			  'searchStoreData' : 'https://dev-proxy.x.origin.com:1405/storesearch/search?locale=en_US&storeId=ww&env=author&q={0}'
			  }
...
def write_content_updates(access_token, userid, lmd, base_dir, logger):
    logger.info('retrieving content updates for user: ' + userid)

    headers = { 'AuthToken':access_token,
                'Accept':'application/vnd.origin.v3+json',
                'User-Agent': 'Mozilla/5.0 EA Download Manager Origin/9.9.1.62656'}

    resp = requests.get(ENDPOINTS['contentUpdates'].format(userid, lmd), headers=headers, verify=False)
    resp.encoding = UTF_8
    debug_resp(resp, logger)

    if 200 == resp.status_code:
        try:
            contentUpdatesDir = os.path.join(base_dir, 'contentUpdates')
            if not os.path.exists(contentUpdatesDir):
                os.mkdir(contentUpdatesDir)

            file = os.path.join(contentUpdatesDir, urllib.quote(userid, '') + '.json')
            write_pretty_json(file, resp.json())
        except Exception as e:
            logger.exception(str(e))
...
def write_atom_friends_for_user(access_token, pid_id, other_id, base_dir, logger):
	...
	resp = requests.get(ENDPOINTS['friendsForUserData'].format(pid_id, other_id), headers=headers, verify=False)
	resp.encoding = UTF_8
	debug_resp(resp, logger)
...
def write_atom_games_owned_by_user(access_token, pid_id, other_id, base_dir, logger):
	...
	resp = requests.get(ENDPOINTS['gamesOwnedForUserData'].format(pid_id, other_id), headers=headers, verify=False)
	resp.encoding = UTF_8
	debug_resp(resp, logger)
...
def get_write_feed_story_data(feed_type, locale, index, base_dir, logger):
    ...
    resp = requests.get(ENDPOINTS['feedStories'].format(feed_type, locale, index, 4), headers=headers, verify=False)
    resp.encoding = UTF_8
    debug_resp(resp, logger)
...
def get_friend_recommendations(accessToken, pid_id, baseDir, logger):
	...
	if 200 == resp.status_code:
		try:
			atomDir = os.path.join(baseDir, 'friends')
			if not os.path.exists(atomDir):
				os.mkdir(atomDir)

			file = os.path.join(atomDir, 'friendRecommendations.xml')
			write_utf8(file, resp.content)
		except Exception as e:
			logger.exception(str(e))
	else:
...
def write_search_people(access_token, search_string, page_num, base_dir, logger):
	logger.info('Searching string '+search_string)
	
	header = { 'Accept': 'application/json' }
	

	resp = requests.get(ENDPOINTS['searchPeopleData'].format(search_string, access_token, page_num), headers=header, verify=False)
	resp.encoding = UTF_8
	debug_resp(resp, logger)

	if 200 == resp.status_code:
		try:
			atomDir = os.path.join(base_dir, 'search')
			if not os.path.exists(atomDir):
				os.mkdir(atomDir)

			file = os.path.join(atomDir, 'searchPeople_'+search_string+'_'+str(page_num)+'.xml')
			write_utf8(file, resp.content)
		except Exception as e:
			logger.exception(str(e))
			
def write_search_store(access_token, search_string, options, base_dir, logger):
	logger.info('Searching string '+search_string)
	
	header = { 'Accept': 'application/json' }
	
	url = ENDPOINTS['searchStoreData'].format(search_string)
	...
	if 200 == resp.status_code:
		try:
			...
			file = os.path.join(atomDir, 'searchStore_'+search_string+'.xml')
			write_utf8(file, resp.content)
		except Exception as e:
			logger.exception(str(e))
...
def main(argv):
	...
	for locale in locales:
		for feedType in feedTypes:
			if feedType == 'mostpopulargame':
				get_write_feed_story_data(feedType, locale, '0', baseDir, logger)
			else:
				get_write_feed_story_data(feedType, locale, '0', baseDir, logger)
				get_write_feed_story_data(feedType, locale, '1', baseDir, logger)
			
	#get friend recommendation
    get_friend_recommendations(accessToken, pidId, baseDir, logger)
	
	#search data
    for store in storeSearch:
		write_search_store(accessToken, store, {}, baseDir, logger)
	
    for friend in friends:
		write_search_people(accessToken, friend, '1', baseDir, logger)
