...
(function() {
    ...
    function FeedFactory($timeout, GamesDataFactory, RosterDataFactory, ComponentsLogFactory, ArrayHelperFactory, ObjectHelperFactory, AuthFactory, SubscriptionFactory) {
        ...
        function feedRecentlyReleased(config) {
            ...
            return baseGameCatalogArray.filter(function(baseGameCatalogItem) {
        ...
        function feedNotRecentlyPlayed(config) {
            var minTime = Number(config.notplayedlow),
                maxTime = Number(config.notplayedhigh);

            return lastPlayedDateArray.filter(function(lastPlayedDateItem) {
                var diff = dayDiffFromCurrent(getLastPlayedDate(lastPlayedDateItem.offerId));
        ...
        function feedNeverPlayed(config) {
            ...
            neverPlayedArray = entitlementsArray.filter(function(entitlementsItem) {
                var offerId = entitlementsItem.offerId,
                    diff = dayDiffFromCurrent(getGrantDate(offerId));
        ...
        function feedRecentlyPlayedNoFriends(config) {
            ...
            return lastPlayedDateArray.filter(function(lastPlayedDateItem) {
                var diff = dayDiffFromCurrent(getLastPlayedDate(lastPlayedDateItem.offerId));
                return ((diff < daysSincePlayed) && findArrayElementByOfferId(lastPlayedDateItem.offerId));
            });
        }
        ...
        function feedJustAcquired() {
            var offerToReturn = null,
                isGrantDateLaterThanLastPlayed = entitlementsArray.length && lastPlayedDateArray.length && (entitlementsArray[0].grantDate > lastPlayedDateArray[0].lastPlayedDate),
            ...
            if (Origin.client.isEmbeddedBrowser() && (isGrantDateLaterThanLastPlayed || isNeverPlayedGameButOwnsEntitlement)) {
        ...
        function feedNewestReleased() {
            ...
            if (baseGameCatalogArray.length) {
                diff = dayDiffFromCurrent(getReleaseDate(baseGameCatalogArray[0]));
        ...
        function feedNewestPreload() {
            ...
            for (var i = 0, length = baseGameCatalogArray.length; i < length; i++) {
                if ((getReleaseDate(baseGameCatalogArray[i]) > currentEpoch) && baseGameCatalogArray[i].downloadable) {
        ...
        function parseOutMutualFriendEntries(item) {
            ...
            for (var i = 0; i < item.rs.length; i++) {
                //go through each of the reasons and see if one of them is the friend of friends
                if (item.rs[i].r === Origin.friends.filter.FRIEND_OF_FRIEND) {
                    var objectTemp = item.rs[i];
                    //clear out the array and reassign the friend of friendo object because we only care about that entry
                    item.rs = [];
                    item.rs[0] = objectTemp;
        ...
        function generateFriendsInCommmonArray(data) {
            var filteredResponse = data.rfs.filter(parseOutMutualFriendEntries);

            //convert the filtered response to a format that we can use in the discovery story
            for (var i = 0; i < filteredResponse.length; i++) {
                friendsInCommonArray.push({
                    userid: filteredResponse[i].id,
                    friendsInCommonStr: filteredResponse[i].rs[0].rp
                });
            }

            return friendsInCommonArray;
        }

        function handleRecommendedFriendsError() {
            return [];
        }
        ...
        function feedMostRecentSubsEntitlementNotInstalled () {
            ...
            if (SubscriptionFactory.userHasSubscription() && SubscriptionFactory.isActive() && feedGamesInstalled().length === 0) {
         ...
         */
        function retrieveFriendsInCommon() {
            return Origin.friends.friendRecommendations()
                .catch(handleRecommendedFriendsError);
        }
        /**
        ...
        function storeUnownedExtraContent(response) {
            for (var p in response) {
                if (response.hasOwnProperty(p)) {
                    ...
                    if ((unownedDLCOfferIds.indexOf(p) < 0) && Origin.utils.getProperty(response[p], ['platforms', Origin.utils.os()])) {
                        unownedDLCOfferIds.push(p);
        ...
        function processFeedInfo(response) {
            ...
            generateFriendsInCommmonArray(response[2]);
        ...
        function onAuthChanged(loginObj) {
            ...
            myEvents.fire('authStateChanged');
        }
        ...
        function handleAuthReadyError(error) {
            ComponentsLogFactory.error('FEEDFACTORY - waitForAuthReady error:', error.message);
        }
    ...
    function FeedFactorySingleton($timeout, GamesDataFactory, RosterDataFactory, ComponentsLogFactory, ArrayHelperFactory, ObjectHelperFactory, AuthFactory, SubscriptionFactory, SingletonRegistryFactory) {
