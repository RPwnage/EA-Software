    ...
    function GamesContextFactory(GamesDataFactory, GamesContextActionsFactory, ComponentsLogFactory) {
        ...
        function makeOperationVisible(clientActions, operationLabel, nowAction) {
            ...
            if (nowAction && indexInQueue > 0) {
                makeVisible(moveToTopOfQueueAction, clientActions.queueSkippingEnabled);
            }
        ...
        function checkPlayableState(clientActions, currentCatalog) {
            ...
            } else if (GamesDataFactory.isBaseGame(currentCatalog) && clientActions.installed && !cloudSyncInProgress(clientActions)) {
        ...
        function checkShowGameDetailsState(currentCatalog, ownsGame) {
            ...
            //if (!nonOriginGame) {
            if (ownsGame && GamesDataFactory.isBaseGame(currentCatalog)) {
        ...
        function checkBaseGameState(offerId, clientActions, ownsGame) {
            ...
            if (GamesDataFactory.isFavorite(offerId)) {
                makeVisible('removeasfavoritecta', Origin.client.onlineStatus.isOnline());
            } else {
                makeVisible('addasfavoritecta', Origin.client.onlineStatus.isOnline());
            }
            if (GamesDataFactory.isNonOriginGame(offerId) || ownsGame) {
                makeVisible('customizeboxartcta', Origin.client.isEmbeddedBrowser());
            }

            if (!clientActions.playing && GamesDataFactory.isNonOriginGame(offerId)) {
        ...
        function updateForGame(gameInfo) {
            ...
            if (clientActions) {
                if (currentCatalog.trial) {
                    makeVisible('viewspecialoffercta', Origin.client.onlineStatus.isOnline());

                } else if (!ownsGame && isPurchasable) {
                    makeVisible('viewinstorecta', Origin.client.onlineStatus.isOnline());
                }
            ...
            if (hasAchievements(gameInfo.offerId)) {
                makeVisible('showachievementscta', Origin.client.onlineStatus.isOnline());
            }

            if (GamesDataFactory.isBaseGame(currentCatalog)) {
        ...
        function playVerb(currentCatalog) {
            ...
            if (currentCatalog.limitedTrial) {
                if (!terminationDateValid) {
                    return ('starttrialcta');
                } else if (currentTime > ent.terminationDate) {
                    return ('primarybuynowcta');
                }
            }
        ...
        function getPrimaryContext(isMoreDetailsDisabled) {
            return function(gameInfo) {
                var contextObj = {},
                        ...
                        availableOnCurrentPlatform = Origin.utils.getProperty(currentCatalog, ['platforms', Origin.utils.os()]);
                ...
                if (clientActions) {
                    ...
                    // Owned, compatible platform, but client not available.
                    else if (ownsGame && availableOnCurrentPlatform && !Origin.client.isEmbeddedBrowser()) {
