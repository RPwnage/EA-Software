...
(function() {
    ...
    function OriginDialogCtrl($scope, $compile, AppCommFactory, UtilFactory) {
        ...
        this.show = function(conf) {
            ...
            //create a new scope and save it off so we can destroy it manually and not leak memory
            newScope = $scope.$new();
            $scope.modalContent = $compile(createTag(conf))(newScope);
            $scope.showContent();
            if(!$scope.$$phase) {
                $scope.$digest();
            }
    ...
    function originDialog(ComponentsConfigFactory, UtilFactory, AppCommFactory, ComponentsLogFactory, $document, $window) {
