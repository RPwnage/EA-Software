...
(function () {
    ...
    * The controller
    */
    function OriginSocialHubRosterSectionCtrl($scope, $filter, $timeout, RosterDataFactory, UtilFactory, SocialHubFactory) {
        $scope.roster = {};
        $scope.rosterArray = [];
        ...
        function handleUpdateFilter(filter) {
            ...
            if ($scope.filterText.length) {
                ...
                $scope.dataIsFiltered = false;
                if (localStorage.getItem('origin-social-hub-section-FRIENDSLIST-'+$scope.filter+'-closed-'+Origin.user.originId())) {
                    self.closeSection();
                }
            }
...
//            $scope.topPadding = ($scope.rosterLength > 0) ? ($scope.start * panelHeight) + ($scope.start ? $scope.headerHeight : 0) : 0;
        ...
        this.toggleSectionVisible = function() {
            if ($scope.$viewportElement.is(':hidden')) {                
                self.openSection();
                localStorage.removeItem('origin-social-hub-section-FRIENDSLIST-'+$scope.filter+'-closed-'+Origin.user.originId());                
            } else {
                self.closeSection();                    
                localStorage.setItem('origin-social-hub-section-FRIENDSLIST-'+$scope.filter+'-closed-'+Origin.user.originId(), 'true');                
            }
            $scope.$digest();
        };
        ...
        this.getOnlineCount = function() {            
            ...
            function handleClearRosterCount() {
                $scope.onlineCount = 0;
            }
            ...
            RosterDataFactory.events.on('socialRosterClear' + 'ONLINE', handleClearRosterCount);
            ...
            $scope.$on('$destroy', function() {
                ...
                RosterDataFactory.events.off('socialRosterClear' + 'ONLINE', handleClearRosterCount);
            });
