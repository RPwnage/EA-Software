...
(function() {
    ...
    function initCQTargeting(CQTargetingFactory, GamesDataFactory, SubscriptionFactory) {
        //we have to init the dependecies this way to avoid circular dependency
        //CQTargeting requires entitlement information but ocd requires CQTargeting
        //but both OCD and Entitlement info come through GamesDataFactory
        //
        //This should be safe as this is the only place init is called and GamesDataFactory and SubscriptionFactory
        //are only called from functions which aren't triggered until after init is called.
        CQTargetingFactory.init(GamesDataFactory, SubscriptionFactory);
    }
    ...
    function initSocialFactories(ConversationFactory, RosterDataFactory, SocialDataFactory, VoiceFactory, AuthFactory, ComponentsLogFactory) {
        ...
        function authReadyError(error) {
            ComponentsLogFactory.error('INITSOCIALFACTORIES: authReady error', error.message);
        }
    ...
    function initEndpoints(ComponentsUrlsFactory) {
        ComponentsUrlsFactory.init();
    }
     ...
     *
     */
    angular.module('origin-components', ['origin-i18n', 'angular-toArrayFilter', 'origin-components-config', 'ui.router'])
        .run(bubbleSetup)
        .run(initEndpoints)
