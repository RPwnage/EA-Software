(function() {
    ...
    function ConversationFactory(UserDataFactory, SocialDataFactory, RosterDataFactory, AppCommFactory, ChatWindowFactory, SocialHubFactory, AuthFactory, VoiceFactory, EventsHelperFactory) {
        ...
        function createConversation(id, state, title, participants) {
            return {
                ...
                pendingText : '',
                isVoiceSupportedRemotely: false
            };
        }
        ...
        function updateVoiceSupportedRemotely(conversationId) {
            ...
            for (var p = 0; p < conversations[conversationId].participants.length; ++p) {
                promiseArray.push(Origin.voice.isSupportedBy(conversations[conversationId].participants[p]));
            }
            ...
            Promise.all(promiseArray).then(function(results) {
                ...
                conversations[conversationId].isVoiceSupportedRemotely = doesAnyParticipantSupportVoice;
            });            
        }
        ...
        function closeConversationById(id) {
            ...
            if (Origin.voice.supported()) {
                if (id === VoiceFactory.getActiveVoiceConversationId()) {
                    Origin.voice.leaveVoice();
                }
            }
        ...
        function handleVoiceCallEvent(voiceCallEventObj) {
            ...
            switch (event) {
                ...
                case 'STARTED':
                    ...
                    VoiceFactory.setActiveVoiceConversationId(nucleusId);
                    mostRecentChannelId = Origin.voice.channelId();
                    break;
        ...
        function addDeviceChangedEvent(conversationId, deviceName) {
            ...
            conversations[conversationId].formattedEvents.push(formattedVoiceMessage);
     ...
     */
    function ConversationFactorySingleton(UserDataFactory, SocialDataFactory, RosterDataFactory, AppCommFactory, ChatWindowFactory, SocialHubFactory, AuthFactory, VoiceFactory, EventsHelperFactory, SingletonRegistryFactory) {
