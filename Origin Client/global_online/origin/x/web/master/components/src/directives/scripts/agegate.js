...
(function() {
    ...
    function OriginAgegateCtrl($scope, UtilFactory, ComponentsLogFactory, AuthFactory, AppCommFactory, $window) {
        ...
        function refreshAgeData(loginObj) {
            ...
            if($scope.underage === 'no') {
                AppCommFactory.events.fire(PASSED_EVENT, {});
            } else {
                AppCommFactory.events.fire(FAILED_EVENT, {});
            }
        ...
        function authReadyError(error) {
            ComponentsLogFactory.error('AGEGATE - authReadyError:', error.message);
        }
