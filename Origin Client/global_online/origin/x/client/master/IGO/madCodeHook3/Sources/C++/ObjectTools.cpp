// ***************************************************************
//  ObjectTools.cpp           version: 1.0.3  ·  date: 2012-09-03
//  -------------------------------------------------------------
...
// browse through the process handles to find a specific memory mapped file
{
  ...
  if (pfnNtQueryObject)
  {
    ...
    {
      if ( (SUCCEEDED(pfnNtQueryObject((HANDLE) (i1 * 4), 2, buf, 2048, NULL))) &&
           (!_wcsicmp((LPWSTR) buf[1], L"Section")) &&
           (SUCCEEDED(pfnNtQueryObject((HANDLE) (i1 * 4), 1, buf, 2048, NULL))) )
      {
