// ***************************************************************
//  mchInjDrv.c               version: 2.0.1  ·  date: 2010-06-17
//  -------------------------------------------------------------
...
// the entry point of our driver
{
  ...
  if (PsSetCreateProcessNotifyRoutine(ProcessCallback, FALSE))
  {
    ...
    if (IsVistaOrNewer())
    {
      // this is only needed for the new wow64 injection method
      // which works only in Vista x64 and newer OSs
      GlobalDriverObject = DriverObject;
      InitWow64ProcessList();
