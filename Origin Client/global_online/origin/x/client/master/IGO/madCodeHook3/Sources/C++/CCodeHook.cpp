// ***************************************************************
//  CCodeHook.cpp             version: 1.0.2  ·  date: 2012-05-21
//  -------------------------------------------------------------
...
#pragma warning(disable:4509) // warning C4509: nonstandard extension used: 'CCodeHook::~CCodeHook' uses SEH and 'threads' has destructor
              ...
              __try
              {
                ...
                *mpNextHook = mpHookStub;
                ...
                if ( (mNewHook) || (GetMadCHookOption(RENEW_OVERWRITTEN_HOOKS)) ||
                     ( (mPatchAddr != NULL /*!IsBadReadPtr(mPatchAddr, sizeof(ABSOLUTE_JUMP))*/) &&
      ...
      if (buf)
      {
        ...
        if ((nh2) && (address < GetSizeOfImage(nh2)))
        {
          __try
          {
            ...
            if (result)
            {
              CHAR arr[COLLECT_RELOCATION_CACHE_RECORD_BUFFER_SIZE];
...
SYSTEMS_API BOOL WINAPI WasTargetChanged(HMODULE hModule, LPVOID pCode, DWORD opcodeSize)
{
    ...
    if (hModule != NULL)
    {
        // Limit opcodeSize to fit into the relocation buffer (removing DWORD prefix/suffix used for non-aligned code)
        if (opcodeSize > COLLECT_RELOCATION_CACHE_RECORD_BUFFER_SIZE - 8)
            opcodeSize = COLLECT_RELOCATION_CACHE_RECORD_BUFFER_SIZE - 8;

        IMAGE_NT_HEADERS *nh1 = GetImageNtHeaders(hModule);
        if ((nh1) && (pCode > hModule) && ((ULONG_PTR)pCode < ((ULONG_PTR)hModule + GetSizeOfImage(nh1))))
        {
            LPVOID buf = CCollectCache2::CacheModuleFileMap(hModule);
            if (buf)
            {
                DWORD address = VirtualToRaw(nh1, (DWORD)((ULONG_PTR)pCode - (ULONG_PTR)hModule));
                PIMAGE_NT_HEADERS nh2 = GetImageNtHeaders((HMODULE)buf);
                if ((nh2) && (address < GetSizeOfImage(nh2)))
                {
                    __try
                    {
                        ...
                        if (result)
                        {
                            if (!opcodeSize)
                            {
                                result = ((*((DWORD *)((ULONG_PTR)pCode + 0)) != *((DWORD *)((ULONG_PTR)buf + address + 0))) ||
                                            (*((WORD  *)((ULONG_PTR)pCode + 4)) != *((WORD  *)((ULONG_PTR)buf + address + 4))));
                            }

                            else
                            {
                                PBYTE src = (PBYTE)pCode;
                                PBYTE dest = (PBYTE)((ULONG_PTR)buf + address);
                                result = FALSE;
                                for (DWORD idx = 0; idx < opcodeSize; ++idx)
                                    result = result || (src[idx] != dest[idx]);
                            }
                        }

                        if (result)
                        {
                            CHAR *arrPtr = CCollectCache2::CacheRelocationBuffer(pCode);
                            if (arrPtr)
                            {
                                if (!opcodeSize)
                                {
                                    result = (*((DWORD *)((ULONG_PTR)&(*(arrPtr + 4)) + 0)) != *((DWORD *)((ULONG_PTR)pCode + 0))) ||
                                        (*((WORD  *)((ULONG_PTR)&(*(arrPtr + 4)) + 4)) != *((WORD  *)((ULONG_PTR)pCode + 4)));
                                }

                                else
                                {
                                    PBYTE src = (PBYTE)pCode;
                                    PBYTE dest = (PBYTE)&arrPtr[4];
                                    result = FALSE;
                                    for (DWORD idx = 0; idx < opcodeSize; ++idx)
                                        result = result || (src[idx] != dest[idx]);
                                }
                            ...
                            else
                            {
                                ...
                                CCollectCache2::AddCacheRelocationBuffer(pCode, arr);   // cache the relocation buffer
        ...
        for (int i = 1; i <= pHookQueueHeader->ItemCount - 2; i++)
        {
               ...
               (pHookRecords[i].ppNextHook == &mpHookStub->AbsoluteAddress) )
          {
            ...
            *pHookRecords[i - 1].ppNextHook = pHookRecords[i].pHookProc;
            ...
            {
              if ((pHookQueueHeader->pPatchAddress != NULL /*!IsBadReadPtr(pHookQueueHeader->pPatchAddress, sizeof(ABSOLUTE_JUMP))*/) &&
