// ***************************************************************
//  IPC.cpp                   version: 1.0.3  ·  date: 2012-09-03
//  -------------------------------------------------------------
  ...
  if ((pGetSecurityInfo != NULL) && (pSetSecurityInfo != NULL) && (pSetEntriesInACL != NULL))
  {
    ...
    if (type != SE_UNKNOWN_OBJECT_TYPE)
    {
      if (AllocateAndInitializeSid((PSID_IDENTIFIER_AUTHORITY) &CEveryoneSia, 1, 0, 0, 0, 0, 0, 0, 0, 0, &pSid1))
      {
        ...
        ea[count].grfAccessPermissions = access;
        ea[count].grfAccessMode = GRANT_ACCESS;
        ea[count].Trustee.TrusteeForm = TRUSTEE_IS_SID;
        ea[count].Trustee.TrusteeType = TRUSTEE_IS_USER;
        ea[count].Trustee.ptstrName = (LPWSTR) pSid1;
		count++;
        if ((IsMetro()) && (AllocateAndInitializeSid((PSID_IDENTIFIER_AUTHORITY) &CSecurityAppPackageAuthority, 2, 2, 1, 0, 0, 0, 0, 0, 0, &pSid2)))
        {
          ...
          ea[count].grfAccessMode = GRANT_ACCESS;
          ea[count].Trustee.TrusteeForm = TRUSTEE_IS_SID;
          ea[count].Trustee.TrusteeType = TRUSTEE_IS_GROUP;
          ea[count].Trustee.ptstrName = (LPWSTR) pSid2;
          count++;
        }
...
static bool DestroyLpcQueue(LPCSTR ipcName)
{
  ...
  if (LpcReady)
  {
    ...
    if (pLpcQueue != NULL)
    {
      ...
      {
        for (int i = 0; i < pLpcQueue->WorkerThreads->GetCount(); i++)
        {
...
static bool InitIpcAnswer(bool create, LPCSTR name, DWORD counter, DWORD processId, IPC_ANSWER *answer, DWORD session)
{
  ...
  if (answer->Length > 0)
  {
    ...
    if (processId != 0)
      sprintf_s(nameBuf, 350, "%s%s%lu$%lx", name, answerBufName, counter, processId);
    else
      sprintf_s(nameBuf, 350, "%s%s%lu", name, answerBufName, counter);
    ...
    else
    {
      ...
      sprintf_s(sessionPath, MAX_PATH, "%s%lu\\", sessionName, session);
      DecryptStr(CMap, &nameBuf[nameBufLen], 8);
      ...
      if (answer->Map == NULL)
      {
        ...
        if (processId != 0)
          sprintf_s(nameBuf, 350, "%s%s%s%lu$%lx", sessionPath, name, answerBufName, counter, processId);
        else
          sprintf_s(nameBuf, 350, "%s%s%s%lu", sessionPath, name, answerBufName, counter);
        ...
        if ((answer->Map == NULL) && (IsMetro()))
        {
          ...
          {
            sprintf_s(sessionPath, MAX_PATH, "%s%lu\\%s\\", sessionName, session, metroPath);
            sprintf_s(nameBuf, 350, "%s%s%s%lu$%lx", sessionPath, name, answerBufName, counter, processId);
            answer->Map = OpenGlobalFileMapping(nameBuf, true);
          }
        }
