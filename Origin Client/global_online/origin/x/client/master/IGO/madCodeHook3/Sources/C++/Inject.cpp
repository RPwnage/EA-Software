// ***************************************************************
//  Inject.cpp                version: 1.0.4  ·  date: 2012-06-12
//  -------------------------------------------------------------
...
// so in these OSs we skip uninjection when the dll is not injected
{
    ...
    if (is64bit)
	{
        WCHAR arrCh[MAX_PATH + 1] = { 0 };
        return (GetRemoteModuleHandle64(hProcess, libraryName, arrCh) == NULL);
	}
    else
	{
        WCHAR arrCh[MAX_PATH + 1] = { 0 };
...
SYSTEMS_API BOOL WINAPI InjectLibraryA(LPCSTR pLibFileName, HANDLE hProcessHandle, DWORD dwTimeOut)
{
  ...
  return InternalInjectLibrary(GetCallingModule(_ReturnAddress()), wideLib.GetBuffer(), false, hProcessHandle, NULL, 0, true, NULL, NULL, NULL, dwTimeOut, true, true, false);
}

SYSTEMS_API BOOL WINAPI InjectLibraryW(LPCWSTR pLibFileName, HANDLE hProcessHandle, DWORD dwTimeOut)
{
  return InternalInjectLibrary(GetCallingModule(_ReturnAddress()), pLibFileName, false, hProcessHandle, NULL, 0, true, NULL, NULL, NULL, dwTimeOut, true, true, false);
}
...
SYSTEMS_API BOOL WINAPI InjectLibrarySystemWideA(LPCSTR pDriverName, LPCSTR pLibFileName, DWORD dwSession, BOOL bSystemProcesses, LPCSTR pIncludeMask, LPCSTR pExcludeMask, PULONG pExcludePIDs, DWORD dwTimeOut)
{
  ...
  return InternalInjectLibrary(GetCallingModule(_ReturnAddress()), wideLib.GetBuffer(), true, 0, wideDrv.GetBuffer(), dwSession, bSystemProcesses, wideInc.GetBuffer(), wideExc.GetBuffer(), pExcludePIDs, dwTimeOut, true, true, false);
}
...
{
  return InternalInjectLibrary(GetCallingModule(_ReturnAddress()), pLibFileName, true, 0, pDriverName, dwSession, bSystemProcesses, pIncludeMask, pExcludeMask, pExcludePIDs, dwTimeOut, true, true, false);
}
                                         ...
                                         LPCWSTR loadLibrary)
{
  ...
  {
    if (InternalInjectLibrary(GetCallingModule(_ReturnAddress()), loadLibrary, false, processInfo->hProcess, NULL, 0, true, NULL, NULL, NULL, INFINITE, true, false, true))
    {
                                         ...
                                         LPCSTR loadLibrary)
{
                     ...
                     environment, currentDirectory, startupInfo, processInfo))
  {
    ...
    if (InternalInjectLibrary(GetCallingModule(_ReturnAddress()), wideLib.GetBuffer(), false, processInfo->hProcess, NULL, 0, true, NULL, NULL, NULL, INFINITE, true, false, true))
    {
      if ((creationFlags & CREATE_SUSPENDED) == 0)
        ResumeThread(processInfo->hThread);
      return true;
...
static HANDLE StartInjectLibraryX(LPCWSTR libraryName, HANDLE hProcess, DWORD processId, bool inject)
{
  ...
  const UCHAR CInjectThread32[552] =
    {,
...
static LPVOID InstallInjectThreadProc(HANDLE hProcess, DWORD processId)
{
  ...
  if (processId != 0)
  {
    ...
    #endif
    sprintf_s(mapName, MAX_PATH, "%s$%lx", s1, processId);
...
// this structure has the same size in both the 32bit and 64bit driver
{
  ...
  //   p1 := buf.pOldApi;
  //   c1 := 5;
  //   buf.npvm(INVALID_HANDLE_VALUE, p1, c1, PAGE_EXECUTE_READWRITE, c2);
  //   buf.pOldApi^ := buf.oldApi;
  //   c1 := 5;
  //   buf.npvm(INVALID_HANDLE_VALUE, p1, c1, c2, c2);
  //   buf.lld(0, nil, @buf.dll, c1);
  // end;
...
// inject a dll into a newly started 64bit process
{
  ...
  if ((Nta64 != NULL) && (Npvm64 != NULL) && (Lld64 != NULL))
  {
    ...
    #ifdef _WIN64
      PInjectLib64 buf = (PInjectLib64) AllocMemEx(sizeof(InjectLib64) + sizeof(CInjectLibFunc64), ProcessHandle, Nta64);
    #else
...
static LPWSTR PathCombine(LPWSTR dest, LPCWSTR dir, LPCWSTR file)
{
  ...
  ASSERT(hr >= 0); hr; // remove warning unused var
  return dest;
}
