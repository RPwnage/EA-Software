...
SYSTEMS_API PVOID WINAPI NeedModuleFileMap(HMODULE hModule)
{
  ...
  PVOID result = NULL;
  // OIG: sorry - dynamic allocations scare me
  //wchar_t *moduleName = (wchar_t*) LocalAlloc(LPTR, 32 * 1024 * 2);
  wchar_t moduleName[2048];
  GetModuleFileNameW(hModule, moduleName, _countof(moduleName));
...
SYSTEMS_API BOOL WINAPI FindModule(LPVOID pCode, HMODULE *phModule, LPSTR moduleName, DWORD moduleNameSize)
{
  ...
  __try
  {
      ...
      MEMORY_BASIC_INFORMATION *mbiPtr = CCollectCache2::CacheMemoryQueryBuffer(pCode);

      if (!mbiPtr || mbiPtr->BaseAddress == NULL)
      {
          MEMORY_BASIC_INFORMATION mbi;
          if (VirtualQuery(pCode, &mbi, sizeof(MEMORY_BASIC_INFORMATION)) == sizeof(MEMORY_BASIC_INFORMATION))
          {
              if (mbi.BaseAddress)
              {
                  CCollectCache2::AddCacheMemoryQueryBuffer(pCode, &mbi);   // caching is safe, becasue we only need the AllocationBase & the State

                  mbiPtr = CCollectCache2::CacheMemoryQueryBuffer(pCode);
              }
          }
      }

    result = mbiPtr != NULL && (mbiPtr->State == MEM_COMMIT) && (mbiPtr->AllocationBase != NULL);
    if (result)
    {
      result = (GetModuleFileNameA((HMODULE) mbiPtr->AllocationBase, moduleName, moduleNameSize) != 0);
      if (result)
      {
        *phModule = (HMODULE) mbiPtr->AllocationBase;
      }
      else
...
SYSTEMS_API IMAGE_NT_HEADERS* WINAPI GetImageNtHeaders(HMODULE hModule)
{
  ...
  __try
  {
    if (((CONST VOID *) hModule) != NULL /*!IsBadReadPtr((CONST VOID *) hModule, 2)*/)
    {
...
SYSTEMS_API BOOL WINAPI GetExportDirectory(LPVOID pCode, HMODULE *phModule, IMAGE_EXPORT_DIRECTORY **pImageExportDirectory)
{
  ...
  if (mbiPtr)
  {
    if ((mbiPtr->State == MEM_COMMIT) && (mbiPtr->AllocationBase != NULL))
    {
      wchar_t moduleFileName[8];
      if (GetModuleFileName((HMODULE) mbiPtr->AllocationBase, moduleFileName, 2) != 0)
      {
        *phModule = (HMODULE) mbiPtr->AllocationBase;
