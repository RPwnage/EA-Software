// ***************************************************************
//  ToolFuncs.c               version: 1.0.3  ·  date: 2012-07-05
//  -------------------------------------------------------------
...
// 64bit: so we have to link to these APIs by dynamically parsing ntdll.dll
  ...
  NTSTATUS NTAPI ZwReadVirtualMemory(HANDLE ProcessHandle, PVOID BaseAddress, PVOID Buffer, ULONG NumberOfBytesToRead, PULONG NumberOfBytesRead)
  {
  ...
  NTSTATUS NTAPI ZwWriteVirtualMemory(HANDLE ProcessHandle, PVOID BaseAddress, PVOID Buffer, ULONG NumberOfBytesToWrite, PULONG NumberOfBytesWritten)
  {
  ...
  NTSTATUS NTAPI ZwQueryVirtualMemory(HANDLE ProcessHandle, PVOID BaseAddress, ULONG MemoryInformationClass, PVOID MemoryInformation, ULONG MemoryInformationLength, PULONG ReturnLength)
  {
...
{
  PVOID ntdllApi = GetProcAddress(ntdllModule, ServiceName, virt);
  *ServiceNo = NtdllApiToServiceNo(ntdllApi);
  ...
  // this function tries to locate the internal "Zw" API in the NtOsKrnl.exe
  {
    if ( ServiceNo != -1 )
    {
      ...
      PVOID ZwAllocateNtdllApi = GetProcAddress(ntdllModule, "ZwAllocateVirtualMemory", virt);
      ...
      if ( ZwAllocateServiceNo != -1 )
      {
        ...
        UCHAR *buf = (PVOID) ZwAllocateVirtualMemory;
        int i1;
...
PVOID GetProcAddress(HANDLE ModuleHandle, PCSTR ProcName, BOOLEAN Virtual)
      ...
      for (i1 = 0; i1 < exports->NumberOfNames; i1++)
      {
        ...
        if ((functions[ordinals[i1]] < addr) || (functions[ordinals[i1]] >= addr + size))
        {
          ...
          if (!strcmp(ProcName, (PSTR) ((ULONG_PTR) ModuleHandle + names[i1] + nameDif)))
          {
            ...
            return (PVOID) ((ULONG_PTR) ModuleHandle + ((Virtual) ? (functions[ordinals[i1]]) : VirtualToRaw(nh, functions[ordinals[i1]])));
          }
        }
      }
    }
...
// is the specified process a system process?
{
  ...
  if (NT_SUCCESS(ZwOpenProcessTokenEx(ProcessHandle, 8 /*TOKEN_QUERY*/, OBJ_KERNEL_HANDLE, &token)))
  {
    ...
    c1 = c1 * 2;
    PSID_AND_ATTRIBUTES saa = ExAllocatePool(PagedPool, c1);
    if (saa)
    {
...
BOOLEAN ReadProcessMemory(HANDLE ProcessHandle, PVOID BaseAddress, PVOID Buffer, ULONG Size)
{
  ULONG bytesRead;
  return (NT_SUCCESS(ZwReadVirtualMemory(ProcessHandle, BaseAddress, Buffer, Size, &bytesRead)) && (bytesRead == Size));
}

BOOLEAN WriteProcessMemory(HANDLE ProcessHandle, PVOID BaseAddress, PVOID Buffer, ULONG Size)
{
  ULONG bytesWritten;
...
// allocate memory in the specified process at 0x71b00000 (or slightly below that)
{
  ...
  else
  {
    ...
    #ifdef _WIN64
      if (Is64bitProcess(ProcessHandle))
        // allocate <= 0x7ff00000000 to avoid fragmentation
        p1 = (PVOID) 0x7ff00000000;
    #endif
  ...
  if (NT_SUCCESS(ZwAllocateVirtualMemory(ProcessHandle, &result, 0, &Size, MEM_COMMIT, PAGE_EXECUTE_READWRITE)))
    return result;
