...
typedef void (*MadCHookLogger)(const char* file, int line, const wchar_t* msg);
struct MadCHookLoggers
{
    MadCHookLogger trace;
    MadCHookLogger debugTrace;
    MadCHookLogger verboseTrace;
};

SYSTEMS_API2 BOOL WINAPI MADCHOOK_DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved);
SYSTEMS_API2 VOID WINAPI StaticLibHelper_Init (MadCHookLoggers* loggers, PVOID dllMain);
SYSTEMS_API2 VOID WINAPI StaticLibHelper_Final(PVOID dllMain);
SYSTEMS_API BOOL WINAPI VersionSupported(void);
SYSTEMS_API2 void WINAPI EnableAllPrivileges(void);
...
SYSTEMS_API2 HANDLE WINAPI CreateLocalFileMapping(LPCSTR name, DWORD size);
SYSTEMS_API2 HANDLE WINAPI OpenLocalFileMapping(LPCSTR name, BOOL write);
...
SYSTEMS_API2 BOOL WINAPI GetImageProcAddressesRaw(LPCWSTR ModuleFileName, PVOID ImageBaseAddress, LPCSTR *ApiNames, PVOID *ProcAddresses, int ApiCount);
SYSTEMS_API2 HMODULE WINAPI GetRemoteModuleHandle64(HANDLE ProcessHandle, LPCWSTR DllName, LPWSTR DllFullPath);
SYSTEMS_API2 HMODULE WINAPI GetRemoteModuleHandle32(HANDLE ProcessHandle, LPCWSTR DllName, LPWSTR DllFullPath);
SYSTEMS_API2 BOOL    WINAPI Is64bitModule(LPCWSTR FileName);
SYSTEMS_API2 LPVOID  WINAPI GetImageProcAddress(HMODULE hModule, LPCSTR name, BOOL doubleCheck = TRUE);
SYSTEMS_API2 BOOL    WINAPI GetImageProcAddressesRaw(LPCWSTR ModuleFileName, PVOID ImageBaseAddress, LPCSTR *ApiNames, PVOID *ProcAddresses, int ApiCount);
SYSTEMS_API LPVOID  WINAPI GetImageProcAddress(HMODULE hModule, int index);
SYSTEMS_API2 BOOL    WINAPI GetImageProcName(HMODULE hModule, LPVOID proc, LPSTR apiName, DWORD procNameSize);
SYSTEMS_API2 HMODULE WINAPI GetCallingModule(PVOID pReturnAddress = NULL);
SYSTEMS_API2 BOOL    WINAPI FindModule(LPVOID pCode, HMODULE *hModule, LPSTR moduleName, DWORD moduleNameSize);
...
SYSTEMS_API BOOL    WINAPI WasTargetChanged(HMODULE hModule, LPVOID pCode, DWORD opcodeSize);
...
SYSTEMS_API2 DWORD  WINAPI GetProcessSessionId(DWORD processId);
...
SYSTEMS_API2 LPVOID WINAPI FindRealCode(LPVOID pCode, bool loadCheckOnly = false);
