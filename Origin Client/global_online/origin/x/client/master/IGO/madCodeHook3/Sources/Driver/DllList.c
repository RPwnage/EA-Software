// ***************************************************************
//  DllList.c                 version: 1.0.1  ·  date: 2010-03-25
//  -------------------------------------------------------------
...
// remove the specified dll injection request from our list, if it's in there
{
  ...
  for (i1 = 0; i1 < (int) DllCount; i1++)
  {
         ...
         (!memcmp(DllList[i1]->Data, Dll->Data, Dll->IncludeLen * 2 + 2 + Dll->ExcludeLen * 2 + 2)) )
    {
      ...
      if (Dll->IncPathBuf) ExFreePool(DllList[i1]->IncPathBuf);
      if (Dll->IncNameBuf) ExFreePool(DllList[i1]->IncNameBuf);
      if (Dll->IncPathLen) ExFreePool(DllList[i1]->IncPathLen);
      if (Dll->IncNameLen) ExFreePool(DllList[i1]->IncNameLen);
      if (Dll->ExcPathBuf) ExFreePool(DllList[i1]->ExcPathBuf);
      if (Dll->ExcNameBuf) ExFreePool(DllList[i1]->ExcNameBuf);
      if (Dll->ExcPathLen) ExFreePool(DllList[i1]->ExcPathLen);
      if (Dll->ExcNameLen) ExFreePool(DllList[i1]->ExcNameLen);
