...
static MadCHookLogger traceLogger = NULL;
static MadCHookLogger debugTraceLogger = NULL;
static MadCHookLogger verboseTraceLogger = NULL;
...
void TraceImpl(const char* file, int line, const wchar_t* fmt, ...)
{
    if (traceLogger)
    {
        ...
        size_t count = _countof(msg) - 1;
        _vsnwprintf(msg, count, fmt, argp);
        traceLogger(file, line, msg);
...
void TraceDebugImpl(const char* file, int line, const wchar_t* fmt, ...)
{
    ...
    if (debugTraceLogger)
    {
        ...
        size_t count = _countof(msg) - 1;
        _vsnwprintf(msg, count, fmt, argp);
        debugTraceLogger(file, line, msg);
...
void TraceVerboseImpl(const char* file, int line, const wchar_t* fmt, ...)
{
    if (verboseTraceLogger)
    {
        ...
        size_t count = _countof(msg) - 1;
        _vsnwprintf(msg, count, fmt, argp);
        verboseTraceLogger(file, line, msg);

        va_end(argp);
    }
}

SYSTEMS_API VOID WINAPI StaticLibHelper_Init(MadCHookLoggers* loggers, PVOID)
{
  traceLogger = loggers->trace;
  debugTraceLogger = loggers->debugTrace;
  verboseTraceLogger = loggers->verboseTrace;
