...
madCHookApi(BOOL) UninjectAllLibrariesA(LPCSTR  pDriverName, PULONG pExcludePIDs = NULL, DWORD dwTimeOutPerUninject = 7000);
madCHookApi(BOOL) UninjectAllLibrariesW(LPCWSTR pDriverName, PULONG pExcludePIDs = NULL, DWORD dwTimeOutPerUninject = 7000);
...
madCHookApi(DWORD) GetProcessSessionId(DWORD dwProcessId);
...
madCHookApi(HANDLE) CreateLocalFileMapping(
  LPCSTR  pName,
  DWORD   dwSize
);
madCHookApi(HANDLE) OpenLocalFileMapping(
  LPCSTR  pName,
  BOOL    bWrite
);
...
#define InitializeMadCHook(loggers) StaticLibHelper_Init(loggers, (PVOID) &MADCHOOK_DllMain)
...
madCHookApi(VOID) StaticLibHelper_Init (MadCHookLoggers* loggers, PVOID dllMain);
...
// ****************************************************************
// internal stuff we want access to for OIG
  extern IMAGE_NT_HEADERS* WINAPI GetImageNtHeaders(HMODULE hModule);
  extern ULONG WINAPI GetSizeOfImage(PIMAGE_NT_HEADERS pNtHeaders);
