// ***************************************************************
//  ProcessTools.cpp          version: 1.0.3  ·  date: 2012-07-18
//  -------------------------------------------------------------
...
bool CheckDllName(LPCWSTR subStr, LPCWSTR str)
{
  ...
  static PFN_WOW64_GET_THREAD_SELECTOR_ENTRY Wow64GetThreadSelectorEntryFunction = NULL;
  static PFN_WOW64_GET_THREAD_CONTEXT Wow64GetThreadContextFunction = NULL;
  ...
  ULONG_PTR GetWow64Teb(HANDLE threadHandle)
  {
    ...
    if (!Wow64GetThreadContextFunction)
      Wow64GetThreadContextFunction = (PFN_WOW64_GET_THREAD_CONTEXT) KernelProc(CWow64GetThreadContext);
    if (!Wow64GetThreadSelectorEntryFunction)
      Wow64GetThreadSelectorEntryFunction = (PFN_WOW64_GET_THREAD_SELECTOR_ENTRY) KernelProc(CWow64GetThreadSelectorEntry);
    LDT_ENTRY ldt;
    if ( (Wow64GetThreadContextFunction) && (Wow64GetThreadSelectorEntryFunction) && 
         (Wow64GetThreadContextFunction(threadHandle, &context)) &&
         (Wow64GetThreadSelectorEntryFunction(threadHandle, context.SegFs, &ldt)) )
    {
  ...
  SYSTEMS_API  PVOID32 WINAPI GetPeb32(HANDLE processHandle)
  ...
  SYSTEMS_API HMODULE WINAPI GetRemoteModuleHandle32(HANDLE ProcessHandle, LPCWSTR DllName, LPWSTR DllFullPath)
  ...
  PVOID LargeAddressAwareApiTo2GB(HANDLE hProcess, PVOID largeApi)
  {
         ...
         (GetModuleFileNameW((HMODULE) mbi.AllocationBase, arrCh, MAX_PATH)) )
    {
      ...
      remoteModule = GetRemoteModuleHandle64(hProcess, dllName, notUsed);
      if (remoteModule)
...
// retrieve the module handle and full file path of a 32bit module in a remote 32bit process
{
    ...
    {
        // first let's get the 32bit PEB (Process Environment Block)
        DWORD peb = (DWORD)GetPeb(ProcessHandle);
        if (peb)
        {
            // we got it, now let's get the loader data
            ULONG ldr;
            ...
            ULONG mi[9];
            SIZE_T c1;
            if ((ReadProcessMemory(ProcessHandle, (PVOID)(peb + 0x0c), &ldr, 4, &c1)) && (c1 == 4) &&
                (ReadProcessMemory(ProcessHandle, (PVOID)(ldr + 0x14), &loopEnd, 4, &c1)) && (c1 == 4) &&
                (ReadProcessMemory(ProcessHandle, (PVOID)loopEnd, &mi, 36, &c1)) && (c1 == 36))
            {
                while ((mi[0] != loopEnd) &&
                    (ReadProcessMemory(ProcessHandle, (PVOID)mi[0], &mi, 36, &c1)) && (c1 == 36))
                {
                    // mi[0] = pointer to next dll
                    // mi[4] = dll handle
                    // mi[8] = full dll file name
                    if ((mi[8] != NULL) &&
                        (ReadProcessMemory(ProcessHandle, (PVOID)mi[8], (PVOID)DllFullPath, MAX_PATH * 2, &c1)) && (c1 == MAX_PATH * 2) &&
                        CheckDllName(DllName, DllFullPath))
                    {
                        result = (HMODULE)mi[4];
                        break;
...
SYSTEMS_API bool WINAPI IsProcessDotNet64(HANDLE hProcess)
...
SYSTEMS_API bool WINAPI IsProcessDotNet32(HANDLE hProcess)
      ...
      __try
      {
        ...
        if ((size == 0) || WriteProcessMemory(hProcess, pMem, pParameters, size, &bytesWritten))
        {
          ...
          if (hThread != NULL)
          {
            ...
            {
              if (pParameters != NULL/*!IsBadWritePtr(pParameters, size)*/)
              {
