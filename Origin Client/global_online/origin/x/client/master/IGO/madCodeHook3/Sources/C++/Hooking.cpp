// ***************************************************************
//  Hooking.cpp               version: 1.0.3  ·  date: 2012-07-03
//  -------------------------------------------------------------
//  hooking groundwork
//  -------------------------------------------------------------
      ...
      __try
      {
        for (int i = 0; i < ach.GetCount(); i++)
        {
          if ( (strcmp(ach[i].ModuleName, "")) &&
               ( (!ach[i].pCodeHook) ||
                 (ach[i].pCodeHook->mPatchAddr == NULL) ||
      ...
      if (map)
      {
        ...
        if (buf1)
        {
          ...
          if (newMap || (buf1->name != *nameAsInt64))
          {
              ...
              if (fi.mFarCalls[i1].Target == p1)
              {
                if ((fi.mFarCalls[i1].pTarget) || (fi.mFarCalls[i1].ppTarget))
                {
                  ...
                  else
                  {
                    ...
                    {
                      BOOL b1 = IsMemoryProtected(fi.mFarCalls[i1].pTarget);
                      if ((!b1) || UnprotectMemory(fi.mFarCalls[i1].pTarget, sizeof(PVOID)))
                      {
                        if (!AtomicMove(&buf2, fi.mFarCalls[i1].pTarget, sizeof(PVOID)))
                          *(PVOID*) fi.mFarCalls[i1].pTarget = buf2;
                        if (b1)
                          ProtectMemory(fi.mFarCalls[i1].pTarget, sizeof(PVOID));
                        LoadLibraryCallsNtDll = (PVOID) buf2;
                        map = 0;
                      }
                    }
                    ...
                    {
                      BOOL b1 = IsMemoryProtected(fi.mFarCalls[i1].ppTarget);
                      if ((!b1) || UnprotectMemory(fi.mFarCalls[i1].ppTarget, sizeof(DWORD)))
                      {
                        #ifdef _WIN64
                          // RIP-relative addressing
                        ...
                        if (b1)
                          ProtectMemory(fi.mFarCalls[i1].ppTarget, sizeof(DWORD));
                        LoadLibraryCallsNtDll = (PVOID) buf2;
                        map = 0;
                      }
                    }
            ...
            if (orgCode)
            {
              BOOL b1 = IsMemoryProtected(api);
              if ((!b1) || UnprotectMemory(api, 8))
              {
