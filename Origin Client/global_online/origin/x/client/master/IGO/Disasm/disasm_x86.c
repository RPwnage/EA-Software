...
#ifdef DEBUG_DISASM
#define DISASM_OUTPUT(x) DisasmLogInfo x
#else
...
#ifdef ORIGIN_MAC
#define PRINTF_ERRORS DisasmLogError(__FILE__, __LINE__,
#define X64_PRINTF_PREFIX "ll"
#define DWORD_PRINTF_PREFIX "l"
#else
#define PRINTF_ERRORS DisasmLogError(__FILE__, __LINE__,
#define X64_PRINTF_PREFIX "I64"
#define DWORD_PRINTF_PREFIX
#endif
...
{ \
	...
	{ \
		if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Unexpected operand size prefix", VIRTUAL_ADDRESS); \
...
{ \
	...
	{ \
		if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Unexpected address size prefix", VIRTUAL_ADDRESS); \
		Instruction->AnomalyOccurred = TRUE; \
	} \
...
#define SANITY_CHECK_SEGMENT_OVERRIDE() \
	...
	{ \
		if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Unexpected segment override", VIRTUAL_ADDRESS); \
		Instruction->AnomalyOccurred = TRUE; \
	}
...
{ \
	...
	{ \
		...
		{ \
			if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: unexpected segment 0x%02" DWORD_PRINTF_PREFIX "X", VIRTUAL_ADDRESS, X86Instruction->Selector); \
			Instruction->AnomalyOccurred = TRUE; \
		} \
	} \
	else \
...
{ \
	...
	{ \
		...
		case 4: addr = (U64)((U32)((addr + Instruction->VirtualAddressDelta)&0xffffffff)); break; \
		case 2: addr = (U64)((U8)((addr + Instruction->VirtualAddressDelta)&0xff)); break; \
...
{ \
	...
	{ \
		case 8: \
			APPEND(OPCSTR, SIZE_LEFT, "0x%02" X64_PRINTF_PREFIX "X=", Operand->Value_U64); \
			if (Operand->Value_S64 >= 0 || !(Operand->Flags & OP_SIGNED)) APPEND(OPCSTR, SIZE_LEFT, "%" X64_PRINTF_PREFIX "u", Operand->Value_U64); \
			/*else APPEND(OPCSTR, SIZE_LEFT, "-0x%02I64X=%I64d", -Operand->Value_S64, Operand->Value_S64);*/ \
			else APPEND(OPCSTR, SIZE_LEFT, "%" X64_PRINTF_PREFIX "d", Operand->Value_S64); \
			break; \
		case 4: \
			APPEND(OPCSTR, SIZE_LEFT, "0x%02X=", (U32)Operand->Value_U64); \
			if (Operand->Value_S64 >= 0 || !(Operand->Flags & OP_SIGNED)) APPEND(OPCSTR, SIZE_LEFT, "%u", (U32)(Operand->Value_U64&0xffffffff)); \
		...
		case 2: \
			...
			if (Operand->Value_S64 >= 0 || !(Operand->Flags & OP_SIGNED)) APPEND(OPCSTR, SIZE_LEFT, "%u", (U16)(Operand->Value_U64&0x10000)); \
		...
		case 1: \
			...
			if (Operand->Value_S64 >= 0 || !(Operand->Flags & OP_SIGNED)) APPEND(OPCSTR, SIZE_LEFT, "%u", (U8)(Operand->Value_U64&0xff)); \
...
{  \
	...
	{ \
		case 8: \
			APPEND(OPCSTR, SIZE_LEFT, "0x%04" X64_PRINTF_PREFIX "X", X86Instruction->Displacement); \
			break; \
		case 4: \
			APPEND(OPCSTR, SIZE_LEFT, "0x%04X", (U32)(X86Instruction->Displacement&0xffffffff)); \
			break; \
		case 2: \
			APPEND(OPCSTR, SIZE_LEFT, "0x%04X", (U16)(X86Instruction->Displacement&0x10000)); \
			break; \
		default: assert(0); break; \
	} \
}
...
#define X86_WRITE_RELATIVE_DISPLACEMENT64() \
	if (X86Instruction->Displacement >= 0) APPEND(OPCSTR, SIZE_LEFT, "+0x%02" X64_PRINTF_PREFIX "X", X86Instruction->Displacement); \
	else APPEND(OPCSTR, SIZE_LEFT, "-0x%02" X64_PRINTF_PREFIX "X", -X86Instruction->Displacement);

#define X86_WRITE_RELATIVE_DISPLACEMENT32() \
	if (X86Instruction->Displacement >= 0) APPEND(OPCSTR, SIZE_LEFT, "+0x%02X", (U32)(X86Instruction->Displacement&0xffffffff)); \
	else APPEND(OPCSTR, SIZE_LEFT, "-0x%02X", (U32)-X86Instruction->Displacement);

#define X86_WRITE_RELATIVE_DISPLACEMENT16() \
	if (X86Instruction->Displacement >= 0) APPEND(OPCSTR, SIZE_LEFT, "+0x%02X", (U16)(X86Instruction->Displacement&0x10000)); \
...
{ \
	...
	{ \
		case 8: \
			...
			X86_WRITE_RELATIVE_DISPLACEMENT64() \
			APPEND(OPCSTR, SIZE_LEFT, "]=0x%04" X64_PRINTF_PREFIX "X", (op)->TargetAddress+Instruction->VirtualAddressDelta); \
			break; \
		case 4: \
			APPENDS("[eip+ilen"); \
			assert((op)->TargetAddress); \
			X86_WRITE_RELATIVE_DISPLACEMENT32() \
			APPEND(OPCSTR, SIZE_LEFT, "]=0x%04X", (U32)(((op)->TargetAddress+Instruction->VirtualAddressDelta)&0xffffffff)); \
			break; \
		case 2: \
			APPENDS("[ip+ilen"); \
			X86_WRITE_RELATIVE_DISPLACEMENT16() \
			APPEND(OPCSTR, SIZE_LEFT, "]=0x%04X", (U16)(((op)->TargetAddress+Instruction->VirtualAddressDelta)&0x10000)); \
			break; \
		default: assert(0); break; \
	} \
}
...
{ \
	...
	{ \
		...
		APPEND(OPCSTR, SIZE_LEFT, "%s 0x%02" DWORD_PRINTF_PREFIX "X:[", DataSizes[((op)->Length >> 1)], X86Instruction->Selector); \
	} \
	else \
	{ \
		assert(!((op)->Flags & OP_FAR)); \
...
void OutputAddress(INSTRUCTION *Instruction, INSTRUCTION_OPERAND *Operand, U32 OperandIndex)
{
	...
	if (X86Instruction->HasBaseRegister)
	{
		...
		if (X86Instruction->Relative)
		{
			...
			APPEND(OPCSTR, SIZE_LEFT, "=[0x%04" X64_PRINTF_PREFIX "X]", Address);
		}
	}
...
BOOL X86_GetInstruction(INSTRUCTION *Instruction, U8 *Address, U32 Flags)
{
	...
	while (TRUE)
	{
		...
		if (IS_AMD64() && (Opcode >= REX_PREFIX_START && Opcode <= REX_PREFIX_END) && X86_PREFIX((&X86_Opcodes_1[*Address])))
		{
			if (!Instruction->AnomalyOccurred)
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: REX prefix before legacy prefix 0x%02X", VIRTUAL_ADDRESS, Opcode);
				Instruction->AnomalyOccurred = TRUE;
			}
			continue;
		}
		...
		if (X86_PREFIX(X86Opcode))
		{
			if (!Instruction->AnomalyOccurred)
			{
				for (i = 0; i < Instruction->PrefixCount; i++)
				{
					...
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Duplicate prefix 0x%02X", VIRTUAL_ADDRESS, Opcode);
						Instruction->AnomalyOccurred = TRUE;
						break;
					}
				}
			...
			switch (Opcode)
			{
				case PREFIX_REPNE: // may be three byte opcode
					...
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Conflicting prefix", VIRTUAL_ADDRESS);
						Instruction->AnomalyOccurred = TRUE;
					}
				...
				case PREFIX_SEGMENT_OVERRIDE_ES:
					...
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Meaningless segment override", VIRTUAL_ADDRESS);
						Instruction->AnomalyOccurred = TRUE;
					}
					break;
			...
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Reached maximum prefix count %d", VIRTUAL_ADDRESS, X86_MAX_PREFIX_LENGTH);
				goto abort;
			}
			else if (Instruction->PrefixCount == X86_MAX_PREFIX_LENGTH)
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Reached maximum prefix count %d", VIRTUAL_ADDRESS, X86_MAX_PREFIX_LENGTH);
				Instruction->AnomalyOccurred = TRUE;
			}
			...
			DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] Prefix 0x%02X (prefix count %d)", VIRTUAL_ADDRESS, Opcode, Instruction->PrefixCount));
		}
		else
		{
			break;
	...
	if (IS_AMD64() && (Opcode >= REX_PREFIX_START && Opcode <= REX_PREFIX_END))
	{
        ...
        DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] REX prefix 0x%02X (prefix count %d, w=%d, r=%d, x=%d, b=%d)", VIRTUAL_ADDRESS, Opcode, Instruction->PrefixCount, X86Instruction->rex.w, X86Instruction->rex.r, X86Instruction->rex.x, X86Instruction->rex.b));
		...
		else if (X86Instruction->rex_b == REX_PREFIX_START)
		{
			if (!Instruction->AnomalyOccurred)
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: meaningless REX prefix used", VIRTUAL_ADDRESS);
				Instruction->AnomalyOccurred = TRUE;
			}
    ...
    DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] OperandSize = %d, AddressSize = %d", VIRTUAL_ADDRESS, X86Instruction->OperandSize, X86Instruction->AddressSize));
	Instruction->LastOpcode = Opcode;
	Instruction->OpcodeAddress = Address-1;

	if (X86_INVALID(X86Opcode))
	{
		if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Invalid opcode 0x%02X", VIRTUAL_ADDRESS, Opcode);
		goto abort;
	}
	...
	if (Opcode == X86_TWO_BYTE_OPCODE)
	{
		...
		if (X86_INVALID(X86Opcode))
		{
			if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Invalid two byte opcode 0x%02X 0x%02X", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode);
			goto abort;
		}
		...
		if (X86Instruction->AddressSize == 8)
		{
			...
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Opcode 0x%02X 0x%02X (\"%s\") illegal in 64-bit mode", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode, X86Opcode->Mnemonic);
				goto abort;
			}
#if 0
			...
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Illegal REX prefix 0x%02X for opcode 0x%02X 0x%02X", VIRTUAL_ADDRESS, X86Instruction->rex_b, X86_TWO_BYTE_OPCODE, Opcode);
				assert(0);
				goto abort;
			}
#endif
		...
		{
			if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Opcode 0x%02X 0x%02X (\"%s\") illegal with 16-bit operand size", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode, X86Opcode->Mnemonic);
			goto abort;
		}
		...
		{
            DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] Special opcode extension 0x%02X 0x%02X", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode));
		...
		else if (SSE_Prefix && !X86_INVALID(&X86_SSE[Opcode])) // SSEx instruction
		{
			...
			if (IS_X86_16())
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: SSE invalid in 16-bit mode", VIRTUAL_ADDRESS);
				goto abort;
			}
			...
			if (X86_INVALID(X86Opcode))
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Illegal SSE instruction opcode 0x%02X 0x%02X + prefix 0x%02X", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], Instruction->OpcodeBytes[2]);
				goto abort;
			}
			else if (X86_EXTENDED_OPCODE(X86Opcode))
			{
				...
				if (X86_INVALID(X86Opcode))
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Illegal SSE instruction opcode 0x%02X 0x%02X + prefix 0x%02X + extension %d", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], Instruction->OpcodeBytes[2], OpcodeExtension);
					goto abort;
				}
			}
		...
		else if (X86_EXTENDED_OPCODE(X86Opcode)) // 2 byte group
		{
			...
			{
				Instruction->Length++;
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Invalid group opcode 0x%02X 0x%02X extension 0x%02X", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode, OpcodeExtension);
				goto abort;
			}
            ...
            DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] Group %d (bytes 0x%02X 0x%02X) extension 0x%02X (\"%s\")", VIRTUAL_ADDRESS, Group, X86_TWO_BYTE_OPCODE, Opcode, OpcodeExtension, X86Opcode->Mnemonic));
		}
		else
		{
			assert(X86Opcode->Mnemonic);
            DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] Two byte opcode 0x%02X 0x%02X (\"%s\")", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode, X86Opcode->Mnemonic));
			X86Instruction->HasModRM = X86_ModRM_2[Opcode];
			if (X86Instruction->HasModRM) X86Instruction->modrm_b = *Address;
		}
	}
	else // 1-byte opcode
	{
		if (X86Instruction->AddressSize == 8)
		{
			...
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Opcode 0x%02X (\"%s\") illegal in 64-bit mode", VIRTUAL_ADDRESS, Opcode, X86Opcode->Mnemonic);
				goto abort;
			}
			...
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Illegal REX prefix 0x%02X for opcode 0x%02X", VIRTUAL_ADDRESS, X86Instruction->rex_b, Opcode);
				assert(0);
				goto abort;
			}
#endif
		...
		{
			if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Opcode 0x%02X (\"%s\") illegal with 16-bit operand size", VIRTUAL_ADDRESS, Opcode, X86Opcode->Mnemonic);
			goto abort;
		}
		...
		if (X86_EXTENDED_OPCODE(X86Opcode)) // a group
		{
			...
			{
				Instruction->Length++;
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Invalid group opcode 0x%02X extension 0x%02X", VIRTUAL_ADDRESS, Opcode, OpcodeExtension);
				goto abort;
			}

			Group = X86_Groups_1[Opcode];
			X86Instruction->Group = (U8)Group;
            DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] Group %d (byte 0x%02X) extension 0x%02X (\"%s\")", VIRTUAL_ADDRESS, Group, Opcode, OpcodeExtension, X86Opcode->Mnemonic));
		...
		else
		{
			if (X86_SPECIAL_EXTENSION(X86Opcode))
			{
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] Special opcode extension 0x%02X", VIRTUAL_ADDRESS, Opcode));
				SpecialExtension = TRUE;
				goto HasSpecialExtension;
			}

            DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] One byte opcode 0x%02X (\"%s\")", VIRTUAL_ADDRESS, Opcode, X86Opcode->Mnemonic));
		}
	}
...
HasSpecialExtension:
	if (SpecialExtension)
	{
		if (X86Opcode->MnemonicFlags & ITYPE_EXT_MODRM)
		{
			...
			if (X86_INVALID(X86Opcode))
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Illegal opcode 0x%02X 0x%02X + modrm 0x%02X", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], *Address);
				goto abort;
			}
			else if (X86_EXTENDED_OPCODE(X86Opcode))
			{
		...
		else if (X86Opcode->MnemonicFlags & ITYPE_EXT_FPU)
		{
			...
			if (X86_INVALID(X86Opcode))
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Invalid FPU opcode 0x%02X + modrm extension 0x%02X (index 0x%02X)", VIRTUAL_ADDRESS, Opcode, X86Instruction->modrm_b, 0x08 + OpcodeExtension);
				goto abort;
			}

            DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] FPU instruction is (\"%s\"): 0x%02X + modrm 0x%02X (index 0x%02X)", VIRTUAL_ADDRESS, X86Opcode->Mnemonic, Opcode, X86Instruction->modrm_b, 0x08 + OpcodeExtension));
		...
		else if (X86Opcode->MnemonicFlags & ITYPE_EXT_SUFFIX)
		{
			if (X86Instruction->HasOperandSizePrefix)
			{
				...
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: operand size prefix used with 3DNOW instruction", VIRTUAL_ADDRESS);
					Instruction->AnomalyOccurred = TRUE;
				}
				X86Instruction->HasOperandSizePrefix = FALSE;
				X86Instruction->OperandSize = 4;
			...
			if (!SetOperands(Instruction, Address, Flags)) goto abort;
			...
			if (X86_INVALID(X86Opcode))
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Illegal opcode 0x%02X 0x%02X + suffix 0x%02X", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], Suffix);
				goto abort;
			}
	...
	{
		if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Instruction \"%s\" (opcode 0x%02X) can't be used in 16-bit X86", VIRTUAL_ADDRESS, X86Opcode->Mnemonic, Instruction->LastOpcode);
		goto abort;
	}
	if (!IS_AMD64() && X86Opcode->CPU >= CPU_AMD64)
	{
		if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Instruction \"%s\" (opcode 0x%02X) can only be used in X86-64", VIRTUAL_ADDRESS, X86Opcode->Mnemonic, Instruction->LastOpcode);
		goto abort;
	}
		...
		for (i = 0; i < Instruction->PrefixCount; i++)
		{
			switch (Instruction->Prefixes[i])
			{
				case PREFIX_OPERAND_SIZE:
					...
					if (!Instruction->AnomalyOccurred)
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: operand size prefix used with FPU/MMX/SSEx", VIRTUAL_ADDRESS);
						goto abort;
					}
					X86Instruction->HasOperandSizePrefix = FALSE;
				...
				case PREFIX_REP:
					...
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: rep/repne used with MMX/SSEx", VIRTUAL_ADDRESS);
					goto abort;
	...
	if (IS_AMD64())
	{
		...
		if (X86Instruction->rex.w)
		{
			...
			for (i = 0; i < Instruction->PrefixCount; i++)
			{
				if (Instruction->Prefixes[i] == PREFIX_OPERAND_SIZE)
				{
					...
					if (!Instruction->AnomalyOccurred)
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: use of operand size prefix meaningless when REX.w=1", VIRTUAL_ADDRESS);
						Instruction->AnomalyOccurred = TRUE;
					}				
				}
			}
		...
		if (X86Instruction->HasDefault64Operand)
		{
			if (X86Instruction->rex.w)
			{
				if (!Instruction->AnomalyOccurred)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: use of REX.w is meaningless (default operand size is 64)", VIRTUAL_ADDRESS);
					Instruction->AnomalyOccurred = TRUE;
				}
		...
		switch (Instruction->Type)
		{
			...
			case ITYPE_STRLOAD:
				if (X86Instruction->HasRepeatWhileNotEqualPrefix)
				{
					if (!Instruction->AnomalyOccurred)
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: REPNE should only be used with cmps/scas", VIRTUAL_ADDRESS);
						Instruction->AnomalyOccurred = TRUE;
					}
			...
			default:
				if (!Instruction->AnomalyOccurred)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Repeat prefix used with non-string instruction", VIRTUAL_ADDRESS);
					Instruction->AnomalyOccurred = TRUE;
				}
				Instruction->Repeat = FALSE;
				X86Instruction->HasRepeatWhileEqualPrefix = FALSE;
	...
	if (IS_X86_32())
	{
		...
		{
			PRINTF_ERRORS "[0x%08" X64_PRINTF_FORMAT "X] WARNING: instruction lengths differ (%d vs %d)", VIRTUAL_ADDRESS, Instruction->Length, InstructionLength);
	...
	if (!X86Instruction->HasDstAddressing && !X86Instruction->HasSrcAddressing)
	{
		if (X86Instruction->HasAddressSizePrefix)
		{
			if (!Instruction->AnomalyOccurred)
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: address size prefix used with no addressing", VIRTUAL_ADDRESS);
				Instruction->AnomalyOccurred = TRUE;
			}
			X86Instruction->HasAddressSizePrefix = FALSE;
		}
		...
		if (X86Instruction->HasSegmentOverridePrefix)
		{
			if (!Instruction->AnomalyOccurred)
			{
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: segment override used with no addressing", VIRTUAL_ADDRESS);
				Instruction->AnomalyOccurred = TRUE;
			}
			X86Instruction->HasSegmentOverridePrefix = FALSE;
		}	
	...
	if (!Instruction->AnomalyOccurred && !IS_X86_16())
	{
		switch (X86Instruction->Segment)
		{
			...
			case SEG_ES:
				switch (Instruction->Type)
				{
 					case ITYPE_IN: case ITYPE_STRMOV: case ITYPE_STRCMP: case ITYPE_STRSTOR:
						break;
					default:
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: use of unexpected segment ES", VIRTUAL_ADDRESS);
						Instruction->AnomalyOccurred = TRUE;
						break;
				}
				break;
			case SEG_FS:
				...
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: use of unexpected segment FS", VIRTUAL_ADDRESS);
				Instruction->AnomalyOccurred = TRUE;
				break;
			case SEG_GS:
				if (IS_AMD64() && !(Instruction->Groups & ITYPE_EXEC)) break;
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: use of unexpected segment GS", VIRTUAL_ADDRESS);
				Instruction->AnomalyOccurred = TRUE;
				break;
			default:
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: unexpected segment 0x%02" DWORD_PRINTF_PREFIX "X", VIRTUAL_ADDRESS, X86Instruction->Selector);
		...
		for (i = 0; i < Instruction->PrefixCount; i++)
		{
			switch (Instruction->Prefixes[i])
			{
				case PREFIX_BRANCH_NOT_TAKEN:
					...
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Segment override used with conditional branch", VIRTUAL_ADDRESS);
						Instruction->AnomalyOccurred = TRUE;
					}
					X86Instruction->HasSegmentOverridePrefix = FALSE;
					X86Instruction->Segment = SEG_CS;
	...
	{
		if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Illegal use of lock prefix for instruction \"%s\"", VIRTUAL_ADDRESS, X86Opcode->Mnemonic);
		goto abort;
	}
	...
	{
		if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: maximum instruction length reached (\"%s\")", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic);
		goto abort;
	}
	...
	if (Instruction->Groups & ITYPE_EXEC)
	{
		...
		{
			if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: branch into the middle of an instruction", VIRTUAL_ADDRESS);
			Instruction->AnomalyOccurred = TRUE;
		}
			...
			case ITYPE_CALL:
				...
				// to even guess
					...
					((X86Instruction->HasBaseRegister && !X86Instruction->HasIndexRegister) ||
					 (!X86Instruction->HasBaseRegister && X86Instruction->HasIndexRegister)))
				{
                    DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] Scale %d, displacement 0x%08" X64_PRINTF_PREFIX "x", VIRTUAL_ADDRESS, X86Instruction->Scale, X86Instruction->Displacement));
					if (!X86Instruction->Scale)
					{
						assert(Operand1->Length <= 0xFF);
						if (Operand1->Length) X86Instruction->Scale = (U8)Operand1->Length;
		...
		for (i = 0, Operand = Instruction->Operands; i < Instruction->OperandCount; i++, Operand++)
		{
			if (Operand->TargetAddress)
			{
				if (Operand->Flags & OP_DST)
				{
                    ...
                    DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] Write of size %d to 0x%04" X64_PRINTF_PREFIX "X", VIRTUAL_ADDRESS, Operand->Length, Operand->TargetAddress));
				}
				if (Operand->Flags & OP_SRC)
				{
                    ...
                    DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] Read of size %d to 0x%04" X64_PRINTF_PREFIX "X", VIRTUAL_ADDRESS, Operand->Length, Operand->TargetAddress));
				}
			}
			...
			// to even guess
				...
				((X86Instruction->HasBaseRegister && !X86Instruction->HasIndexRegister) ||
				 (!X86Instruction->HasBaseRegister && X86Instruction->HasIndexRegister)))
			{
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] Data reference (scale %d, size %d, displacement 0x%08" X64_PRINTF_PREFIX "x)", VIRTUAL_ADDRESS, X86Instruction->Scale, Operand->Length, X86Instruction->Displacement));
				if (!X86Instruction->Scale)
				{
	...
	if (Instruction->Groups & ITYPE_STACK)
	{
		switch (Instruction->Type)
		{
			...
			case ITYPE_ENTER:
				if (!Instruction->AnomalyOccurred)
				{
					...
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: ENTER has invalid operand 2", VIRTUAL_ADDRESS);
						Instruction->AnomalyOccurred = TRUE;
					}
					if (Instruction->Operands[2].Value_U64 & ~0x1F)
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: ENTER has invalid operand 3", VIRTUAL_ADDRESS);
						Instruction->AnomalyOccurred = TRUE;
					}
				}
				SANITY_CHECK_ADDRESS_SIZE();
			...
			case ITYPE_RET:
				...
				switch (Opcode)
				{
					...
					case 0xC2: // ret with 1 arg
						...
						{
							if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: ret has invalid operand 1", VIRTUAL_ADDRESS);
							Instruction->AnomalyOccurred = TRUE;
						}
					...
					case 0xCA: // far ret with 1 arg
						if (!Instruction->AnomalyOccurred && (Operand1->Value_U64 & 3))
						{
							if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: retf has invalid operand 1", VIRTUAL_ADDRESS);
							Instruction->AnomalyOccurred = TRUE;
						}
						Instruction->StackChange *= 2; // account for segment
						Instruction->StackChange += (LONG)Operand1->Value_U64;
			...
			default:
				if (!Instruction->AnomalyOccurred)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Instruction \"%s\" is modifying the stack", VIRTUAL_ADDRESS, X86Opcode->Mnemonic);
					Instruction->AnomalyOccurred = TRUE;
				}
				break;
		}
		...
		{
			if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: \"%s\" has invalid stack change 0x%02" DWORD_PRINTF_PREFIX "X", VIRTUAL_ADDRESS, X86Opcode->Mnemonic, Instruction->StackChange);
			Instruction->AnomalyOccurred = TRUE;
		}
	}
...
abort:
	...
	{
#ifdef TEST_DISASM
		PRINTF_ERRORS "Dump of 0x%04" X64_PRINTF_PREFIX "X:", VIRTUAL_ADDRESS);
...
INTERNAL U8 *SetOperands(INSTRUCTION *Instruction, U8 *Address, U32 Flags)
{
	...
	if (X86Instruction->HasModRM)
	{
        ...
        DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] ModRM = 0x%02X (mod=%d, reg=%d, rm=%d)", VIRTUAL_ADDRESS, X86Instruction->modrm_b, modrm.mod, rex_modrm.reg, rex_modrm.rm));
		INSTR_INC(1); // increment Instruction->Length and address
	}
	else
	{
	...
	for (OperandIndex = 0; OperandIndex < Instruction->OperandCount; OperandIndex++)
	{
		...
		switch (OperandType)
		{
			...
			case OPTYPE_0:
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] const 0"));
				if (Disassemble)
				{
					APPENDS("<0>");
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_1:
				...
				Operand->Value_U64 = 1;
				Operand->Type = OPTYPE_IMM;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] const 1"));
				if (Disassemble)
				{
					APPENDS("<1>");
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_FF:
				...
				Operand->Type = OPTYPE_IMM;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] const 0xff"));
				if (Disassemble)
				{
					APPENDS("<0xFF>");
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_TSC:
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] TSC"));
				if (Disassemble)
				{
					APPENDS("<TSC_MSR>");
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_CS_MSR:
				...
				Operand->Length = 8;
				Operand->Type = OPTYPE_SPECIAL;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] CS MSR"));
				if (Disassemble)
				{
					APPENDS("<CS_MSR>");
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_EIP_MSR:
				...
				Operand->Length = 8;
				Operand->Type = OPTYPE_SPECIAL;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] EIP MSR"));
				if (Disassemble)
				{
					APPENDS("<EIP_MSR>");
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_ESP_MSR:
				...
				Operand->Length = 8;
				Operand->Type = OPTYPE_SPECIAL;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] ESP MSR"));
				if (Disassemble)
				{
					APPENDS("<ESP_MSR>");
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_KERNELBASE_MSR:
				...
				Operand->Length = 8;
				Operand->Type = OPTYPE_SPECIAL;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] KernelBase MSR"));
				if (Disassemble)
				{
			...
			case OPTYPE_CSTAR_MSR:
				...
				Operand->Length = 8;
				Operand->Type = OPTYPE_SPECIAL;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] CSTAR MSR"));
				if (Disassemble)
				{
					APPENDS("<CSTAR_MSR>");
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_LSTAR_MSR:
				...
				Operand->Length = 8;
				Operand->Type = OPTYPE_SPECIAL;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] LSTAR MSR"));
				if (Disassemble)
				{
					APPENDS("<LSTAR_MSR>");
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_FMASK_MSR:
				...
				Operand->Length = 8;
				Operand->Type = OPTYPE_SPECIAL;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] FMASK MSR"));
				if (Disassemble)
				{
					APPENDS("<FMASK_MSR>");
					X86_WRITE_OPFLAGS();
			...
			case OP_REG:
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OP_REG %s", X86_Registers[Operand->Register]));
				if (Disassemble)
				{
			...
			case OPTYPE_REG_AL:
				...
				X86_SET_REG(0);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] reg AL"));
				if (Disassemble)
				{
			...
			case OPTYPE_REG_CL:
				...
				X86_SET_REG(0);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] reg CL"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_REG_AH:
				...
				X86_SET_REG(0);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] reg AH"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_REG_AX:
				...
				X86_SET_REG(0);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] reg AX"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_REG_DX:
				...
				X86_SET_REG(0);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] reg DX"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_REG_ECX:
				...
				X86_SET_REG(0);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] reg ECX"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_REG_xBP:
				...
				X86_SET_REG(0);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] xAX_BIG (size = %d)", Operand->Length));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_REG_xAX_SMALL:
				...
				X86_SET_REG(0);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] xAX_SMALL (size = %d)", Operand->Length));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_xCX_HI_xBX_LO:
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] xCX_BIG:xBX_BIG (size = %d)", Operand->Length));
				continue;
			case OPTYPE_xDX_HI_xAX_LO:
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] xDX_BIG:xAX_BIG (size = %d)", Operand->Length));
				continue;

			case OPTYPE_EDX_HI_EAX_LO:
				if (!Decode) continue;
				Operand->Length = 8;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] EDX:EAX"));
				if (Disassemble)
				{
					APPENDS("<EDX:EAX>");
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_EDX_ECX_EBX_EAX:
				Operand->Length = 32;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] EDX:ECX:EBX:EAX"));
				if (Disassemble)
				{
			...
			case OPTYPE_FLAGS:
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] reg FLAGS"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_xFLAGS:
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] reg xFLAGS (size = %d)", Operand->Length));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_CS:
				...
				Operand->Flags |= OP_REG;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] seg CS"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_DS:
				...
				Operand->Flags |= OP_REG;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] seg DS"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_ES:
				...
				Operand->Flags |= OP_REG;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] seg ES"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_FS:
				...
				Operand->Flags |= OP_REG;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] seg FS"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_GS:
				...
				Operand->Flags |= OP_REG;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] seg GS"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_SS:
				...
				Operand->Flags |= OP_REG;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] seg SS"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_CR0:
				...
				Operand->Flags |= OP_REG;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] reg CR0"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_STx:
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_STx: reg st(%d)", Register));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "<%s>", X86_Registers[Operand->Register]);
					X86_WRITE_OPFLAGS();
			...
			case OPTYPE_b: // byte regardless of operand size
				Operand->Length = 1;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_b (size 1, signed %d)", ((OperandFlags & OP_SIGNED) != 0)));
				break;

			case OPTYPE_w: // word regardless of operand size
				Operand->Length = 2;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_w (size 2)"));
				break;

			case OPTYPE_d: // dword regardless of operand size
				Operand->Length = 4;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_d (size 4)"));
				break;

			case OPTYPE_q: // qword regardless of operand size
				Operand->Length = 8;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_q (size 8)"));
				break;

			case OPTYPE_o: // oword regardless of operand size
				Operand->Length = 16;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_o (size 16)"));
				break;

			case OPTYPE_dt: // 6-byte or 10-byte pseudo descriptor (sgdt, lgdt, sidt, lidt)
				if (IS_AMD64()) Operand->Length = 10;
				else Operand->Length = 6;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_dt (%d bytes)", Operand->Length));
				break;

			case OPTYPE_cpu:
				if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Undocumented loadall instruction?", VIRTUAL_ADDRESS);
				Instruction->AnomalyOccurred = TRUE;
				Operand->Length = 204;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_cpu (size 204)"));
				break;
			...
			case OPTYPE_z: // word if operand size is 16 bits and dword otherwise
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_z (length %d)", Operand->Length));
				break;

			case OPTYPE_v: // word, dword, or qword
				Operand->Length = X86Instruction->OperandSize;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_v (size %d, signed = %d)", Operand->Length, ((OperandFlags & OP_SIGNED) != 0)));
				break;
			...
			case OPTYPE_a: // two word or dword operands in memory (used only by bound)
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_a (size %d)", Operand->Length));
				break;

			case OPTYPE_p: // 32-bit or 48-bit pointer depending on operand size
				if (!Instruction->AnomalyOccurred && X86Instruction->HasSegmentOverridePrefix)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Segment override used when segment is explicit", VIRTUAL_ADDRESS);
					Instruction->AnomalyOccurred = TRUE;
				}
				switch (X86Instruction->OperandSize)
				{
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_p (length %d)", Operand->Length));
				break;

			case OPTYPE_dq: // dword or qword
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_dq (size 4 or 8)"));
				switch (X86Instruction->OperandSize)
				{
			...
			case OPTYPE_mw: // a word if the destination operand is memory
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_mw (size 0)"));
				assert(X86Instruction->HasModRM);		
			...
			case OPTYPE_lea:
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_lea (size 0)"));
			...
			case OPTYPE_ps: // packed single real
				Operand->Length = 4;
				Operand->Type = OPTYPE_FLOAT;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_ps (packed single real)"));
				break;
			case OPTYPE_pd: // packed double real
				Operand->Length = 8;
				Operand->Type = OPTYPE_FLOAT;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_pd (packed double real)"));
				break;
			case OPTYPE_pb: // packed BCD
				Operand->Length = 10;
				Operand->Type = OPTYPE_BCD;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_pb (packed BCD)"));
				break;
			case OPTYPE_ss: // scalar single real
				Operand->Length = 4;
				Operand->Type = OPTYPE_FLOAT;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_ss (single real)"));
				break;
			case OPTYPE_sd: // scalar double real
				Operand->Length = 8;
				Operand->Type = OPTYPE_FLOAT;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_sd (double real)"));
				break;
			case OPTYPE_se: // extended real
				Operand->Length = 10;
				Operand->Type = OPTYPE_FLOAT;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_se (extended real)"));
				break;
			...
			case OPTYPE_fev: // FPU environment (28 bytes in 32-bit modes, 14 bytes in 16-bit real mode)
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_fev (FPU environment, length %d)", Operand->Length));
				break;
			...
			case OPTYPE_fst1: // FPU state (108 bytes in 32-bit modes, 94 bytes in 16-bit real mode)
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_fst1 (FPU state, length %d)", Operand->Length));
				break;

			case OPTYPE_fst2: // 512 bytes for FPU state (FPU + MMX + XXM + MXCSR)
				Operand->Length = 512;
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_fst2 (FPU + MMX + XXM + MXCSR state, length 512)"));
				break;
			...
			case OPTYPE_sso:
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_sso (single real or oword)"));
				break;
			...
			case OPTYPE_sdo:
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] OPTYPE_sso (double real or oword)"));
				break;
		...
		switch (AddressMode)
		{
			...
			case AMODE_xlat: // DS:[EBX+AL]
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_xlat (DS:[EBX+AL])"));
				if (Disassemble)
				{
			...
			case AMODE_I: // immediate value
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_I (immediate data)"));
				if (Disassemble)
				{
					X86_WRITE_IMMEDIATE();
					X86_WRITE_OPFLAGS();
			...
			case AMODE_J: // IP-relative jump offset
				...
				if (Decode)
				{
					...
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: Both conditions of branch go to same address", VIRTUAL_ADDRESS);
						Instruction->AnomalyOccurred = TRUE;
					}
				}
				...
				X86Instruction->HasBaseRegister = TRUE;
				assert(Instruction->OperandCount == 1);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_J (branch with relative offset)"));
				if (Disassemble)
				{
			...
			case AMODE_O: // word/dword offset
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_O (offset)"));
				if (Disassemble)
				{
			...
			case AMODE_A: // absolute address
				...
				X86_SET_ADDR();
				X86_SET_TARGET();
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_A (absolute address)"));
				if (Disassemble)
				{
					X86_WRITE_OFFSET(Operand);
					X86_WRITE_OPFLAGS();
			...
			case AMODE_X: // DS:[ESI]
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_X (addressing via DS:[ESI])"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "%s:[%s]", 
						Segments[X86Instruction->Segment], X86_Registers[Operand->Register]);
			...
			case AMODE_Y: // ES:[EDI]
				...
				if (X86Instruction->HasSegmentOverridePrefix)
				{
					if (!Instruction->AnomalyOccurred)
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ANOMALY: segment override used with AMODE_Y", VIRTUAL_ADDRESS);
						Instruction->AnomalyOccurred = TRUE;
					}
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_Y (addressing via ES:[EDI])"));
				if (Disassemble)
				{
					APPEND(OPCSTR, SIZE_LEFT, "%s:[%s]", 
			...
			case AMODE_PR: // modrm.rm = mmx register and modrm.mod = 11
				...
				if (modrm.mod != 3)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: mod != 3 for AMODE_PR (\"%s\")", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}
				else if (rex_modrm.rm > 7)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: invalid mmx register %d for AMODE_PR (\"%s\")", VIRTUAL_ADDRESS, rex_modrm.rm, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}
				else if (X86Instruction->OperandSize == 2)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: AMODE_PR illegal in 16-bit mode (\"%s\")", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}
				if (!Decode) continue;
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_PR (MMX register)"));
				continue;
			...
			case AMODE_VR: // modrm.rm = xmm register and modrm.mod = 11
				...
				if (modrm.mod != 3)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: mod != 3 for AMODE_VR (\"%s\")", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}
				else if (X86Instruction->OperandSize == 2)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: AMODE_VR illegal in 16-bit mode (\"%s\")", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}
				if (!Decode) continue;
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_VR (XMM register)"));
				continue;
			...
			case AMODE_P: // modrm.reg = mmx register
				...
				if (rex_modrm.reg > 7)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: invalid mmx register %d for AMODE_P (\"%s\")", VIRTUAL_ADDRESS, rex_modrm.reg, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}
				else if (X86Instruction->OperandSize == 2)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: AMODE_P illegal in 16-bit mode (\"%s\")", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}
				if (!Decode) continue;
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_P (MMX register)"));
				continue;
			...
			case AMODE_V: // modrm.reg = xmm register
				...
				//DISASM_OUTPUT((__FILE__, __LINE__, "[SetOperand] AMODE_V (XMM register)"));
				continue;
				*/
			...
			case AMODE_R: // modrm.rm is general register and modrm.mod = 11
				...
				if (modrm.mod != 3)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: mod != 3 for AMODE_R (\"%s\")", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}
				if (!Decode) continue;
				Operand->Flags |= OP_REG;
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_R (general register)"));
				continue;
			...
			case AMODE_G: // modrm.reg = general register
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_G (general register)"));
				continue;
			...
			case AMODE_S: // modrm.reg = segment register
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_S (segment register)"));
				continue;
			...
			case AMODE_T: // modrm.reg = test register
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_T (test register)"));
				continue;
			...
			case AMODE_C: // modrm.reg = control register
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_C (control register)"));
				continue;
			...
			case AMODE_D: // modrm.reg = debug register
                ...
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_D (debug register)"));
				continue;
			...
			case AMODE_M: // memory only
				...
				if (modrm.mod == 3)
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: mod = 3 for AMODE_M (\"%s\")", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}
				assert(X86Instruction->Segment == SEG_DS || X86Instruction->HasSegmentOverridePrefix);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_M (memory only)"));
			...
			case AMODE_E: // general register or memory
				...
				{
					if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: mod = 3 for AMODE_E with OPTYPE_p (\"%s\")", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic);
					goto abort;
				}

                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_E (general register or memory)"));
				Address = SetModRM32(Instruction, Address, Operand, OperandIndex, SuppressErrors);
				if (!Address) return NULL;
			...
			case AMODE_Q: // mmx register or memory address
				assert(X86Instruction->HasModRM);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_Q (MMX register or memory address)"));
				if (modrm.mod == 3) // it is a register
				{
					if (rex_modrm.rm > 7)
					{
						if (!SuppressErrors) PRINTF_ERRORS "[0x%08" X64_PRINTF_PREFIX "X] ERROR: invalid mmx register %d for AMODE_P (\"%s\")", VIRTUAL_ADDRESS, rex_modrm.rm, X86Instruction->Opcode.Mnemonic);
						goto abort;
					}
					Operand->Register = X86_MMX_OFFSET + rex_modrm.rm;
					Operand->Flags |= OP_REG;
			...
			case AMODE_W: // xmm register or memory address
				assert(X86Instruction->HasModRM);
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetOperand] AMODE_W (XMM register or memory address)"));
				if (modrm.mod == 3) // it is a register
				{
					Operand->Register = X86_XMM_OFFSET + rex_modrm.rm;
					Operand->Flags |= OP_REG;
...
INTERNAL U8 *SetModRM16(INSTRUCTION *Instruction, U8 *Address, INSTRUCTION_OPERAND *Operand, U32 OperandIndex, BOOL SuppressErrors)
{
    ...
    DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Current instruction length = %d", Instruction->Length));
	modrm = X86Instruction->modrm;
	...
	if (modrm.mod == 3)
	{
		//DISASM_OUTPUT((__FILE__, __LINE__, "[SetModRM16] Both regs (rm_reg %d)", modrm.rm));
		switch (Operand->Length)
		{
	...
	{
        DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Absolute addressing (displacement 0x%04X)", *(S16 *)Address));
	...
	else
	{
		switch (modrm.mod)
		{
			case 0: // no displacement
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Indirect addressing (no displacement)"));
				break; 
			case 1: // 8-bit signed displacement
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Indirect addressing (displacement = 0x%02X, reg_rm = %d)", *(S8 *)Address, modrm.rm));
				X86Instruction->Displacement = (S64)(*((S8 *)Address));
				INSTR_INC(1); // increment Instruction->Length and address
				break;
			case 2: // 16-bit displacement
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Indirect addressing (displacement = 0x%04X, reg_rm = %d)", *(S16 *)Address, modrm.rm));
				X86Instruction->Displacement = (S64)(*((S16 *)Address));
				if (IS_VALID_ADDRESS(X86Instruction->Displacement))
				{
					Operand->Flags |= OP_GLOBAL;
		...
		{
			case 0:
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Addressing mode [BX+SI]"));
				X86Instruction->BaseRegister = X86_REG_BX;
				X86Instruction->IndexRegister = X86_REG_SI;
				X86Instruction->HasIndexRegister = TRUE;
				break;
			case 1:
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Addressing mode [BX+DI]"));
				X86Instruction->BaseRegister = X86_REG_BX;
				X86Instruction->IndexRegister = X86_REG_DI;
				X86Instruction->HasIndexRegister = TRUE;
				break;
			case 2:
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Addressing mode [BP+SI]"));
				...
				break;
			case 3:
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Addressing mode [BP+DI]"));
				X86Instruction->BaseRegister = X86_REG_BP;
				X86Instruction->IndexRegister = X86_REG_DI;
				X86Instruction->HasIndexRegister = TRUE;
				X86_SET_SEG(REG_BP);
				break;
			case 4:
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Addressing mode [SI]"));
				X86Instruction->BaseRegister = X86_REG_SI;
				break;
			case 5:
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Addressing mode [DI]"));
				X86Instruction->BaseRegister = X86_REG_DI;
				break;
			case 6:
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Addressing mode [BP]"));
				X86Instruction->BaseRegister = X86_REG_BP;
				break;
			case 7:
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM16] Addressing mode [BX]"));
				X86Instruction->BaseRegister = X86_REG_BX;
				break;
		}
...
INTERNAL U8 *SetModRM32(INSTRUCTION *Instruction, U8 *Address, INSTRUCTION_OPERAND *Operand, U32 OperandIndex, BOOL SuppressErrors)
{
    ...
    DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM32] Length %d, modrm = 0x%02X", Instruction->Length, X86Instruction->modrm_b));
	modrm = X86Instruction->modrm;
	rex_modrm = X86Instruction->rex_modrm;
	...
	{
        DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM32] Absolute addressing (displacement 0x%08lX)", *(S32 *)Address));
		Operand->Flags |= OP_ADDRESS;
		...
		if (X86Instruction->sib.base != 5) // if base == 5, the displacement is handled in SetSIB
		{
			switch (modrm.mod)
			{
				case 1: // 8-bit displacement
                    DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM32] After SIB: displacement 0x%02X", *((S8 *)Address)));
					X86Instruction->Displacement = (S64)(*((S8 *)Address));
					INSTR_INC(1); // increment Instruction->Length and address
					break;
				case 2: // 32-bit displacement
                    DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM32] After SIB: displacement 0x%08lX", *((S32 *)Address)));
					X86Instruction->Displacement = (S64)*((S32 *)Address);
					if (IS_VALID_ADDRESS(X86Instruction->Displacement))
					{
						Operand->Flags |= OP_GLOBAL;
	...
	else
	{
		...
		{
			case 0: // no displacement
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM32] Indirect addressing (no displacement, reg_rm = %d)", rex_modrm.rm));
				break; 
			case 1: // 8-bit signed displacement
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM32] Indirect addressing (displacement = 0x%02X, reg_rm = %d)", *(S8 *)Address, rex_modrm.rm));
				X86Instruction->Displacement = (S64)(*((S8 *)Address));
				INSTR_INC(1); // increment Instruction->Length and address
				break;
			case 2: // 32-bit displacement
                DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetModRM32] Indirect addressing (displacement = 0x%08lX, reg_rm = %d)", *(S32 *)Address, rex_modrm.rm));
				X86Instruction->Displacement = (S64)*((S32 *)Address);
				if (IS_VALID_ADDRESS(X86Instruction->Displacement))
				{
					Operand->Flags |= OP_GLOBAL;
...
INTERNAL U8 *SetSIB(INSTRUCTION *Instruction, U8 *Address, INSTRUCTION_OPERAND *Operand, U32 OperandIndex, BOOL SuppressErrors)
{
    ...
    if (!X86Instruction->rex_b) DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] SIB = 0x%02X (scale=%d, index=%d, base=%d)", VIRTUAL_ADDRESS, *Address, sib.scale, sib.index, sib.base)); \
    else DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] SIB = 0x%02X (scale=%d, index=%d, base=%d)", VIRTUAL_ADDRESS, *Address, sib.scale, rex_sib.index, rex_sib.base)); \
    DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetSIB] Current instruction length = %d", Instruction->Length));
	...
	if (rex_sib.index != 4)
	{
		...
		X86Instruction->HasIndexRegister = TRUE;
        DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetSIB] Index register = %s", X86_Registers[X86_32BIT_OFFSET + rex_sib.index]));
        ...
        DISASM_OUTPUT((__FILE__, __LINE__, 0, "[SetSIB] Scale = %d", X86Instruction->Scale));
	}
...
INTERNAL U64 ApplyDisplacement(U64 Address, INSTRUCTION *Instruction)
{
	...
	switch (X86Instruction->OperandSize)
	{
		...
		case 4:
		{
			...
			// due to wraparound
			U32 PreAddr = (U32)(VirtualAddress&0xffffffff);
		...
		case 2:
		{
			...
			// due to wraparound
			U16 PreAddr = (U16)(VirtualAddress&0x10000);
...
INTERNAL BOOL IsValidLockPrefix(X86_INSTRUCTION *X86Instruction, U8 Opcode, U32 OpcodeLength, U8 Group, U8 OpcodeExtension)
{
	...
	if (!X86Instruction->HasModRM || X86Instruction->modrm.mod == 3 || !X86Instruction->HasDstAddressing)
	{
        ...
        DISASM_OUTPUT((__FILE__, __LINE__, 0, "[0x%08" X64_PRINTF_PREFIX "X] ERROR: Instruction \"%s\" with LOCK prefix has invalid ModRM addressing", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic, X86Instruction->Instruction->Address));
		return FALSE;
	}
