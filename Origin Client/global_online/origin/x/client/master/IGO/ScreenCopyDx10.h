...
#ifndef __SCREEN_COPY_DX10_H__
#define __SCREEN_COPY_DX10_H__
...
namespace OriginIGO {
    ...
    class ScreenCopyDx10 : public IScreenCopy
    {
    public:
    ...
    private:
        ...
        bool CreateSurfaces(CComPtr<ID3D10Device> &pDevice10, const D3D10_TEXTURE2D_DESC &desc);
...
#pragma endregion
    ...
    private:
        ...
        ID3D10Texture2D *mBackBufferOffscreenCPU[NUM_OFFSCREEN_BUFFERS];
        ID3D10Texture2D *mBackBuffer;
        ID3D10Texture2D *mBackBufferCopy;
        ID3D10ShaderResourceView *mShaderResourceViewScaled;
        ID3D10ShaderResourceView *mRenderTargetBBResource;
        ID3D10RenderTargetView *mRenderTargetViewScaled;
        ID3D10Texture2D *mRenderTargetTextureScaled;

        ID3D10Effect*           mEffect;
        ID3D10EffectTechnique*  mTechnique;

        ID3D10InputLayout      *mScreenQuadVBLayout;
        ID3D10Buffer           *mScreenQuadVB;

        ID3D10EffectShaderResourceVariable *mTextureVariable;

        D3D10_VIEWPORT mVideoStreamerVP[1];
        D3D10_VIEWPORT mBackupVP[1];
