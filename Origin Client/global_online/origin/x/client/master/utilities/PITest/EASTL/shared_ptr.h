...
namespace eastl
{
    ...
    struct ref_count_sp
    {
        ...
        unsigned mnSize          : 15; /// Size of the ref_count_sp object, so we know how much to pass to the memory free function.
        unsigned mbCustomDispose :  1; /// True if we are using ref_count_sp_t instead of ref_count_sp. We are taking strong measures to attempt to avoid virtual function dispose calls.
    ...
    public:
        ...
        {
            Deleter del;
            del(static_cast<typename Deleter::value_type*>(mValue));
            mValue = NULL;
        }
    };
    ...
    public:
        ...
        {
            // Allocate memory for the reference count.
            // We explicitly use smart_ptr_deleter<U>. You can use the other version of this constructor to provide a custom version.
        ...
        explicit shared_ptr(U* pValue, const UDeleter&)
            : mpValue(pValue),
              mpRefCount(NULL),
              mAllocator(EASTL_SHARED_PTR_DEFAULT_ALLOCATOR)
        {
            typedef ref_count_sp_t<U*, UDeleter> ref_count_type;
              ...
              mAllocator(weakPtr.mAllocator)
        {
            ...
            {
                typedef ref_count_sp_t<T*, deleter_type> ref_count_type;
        ...
        template <typename U, typename A, typename UDeleter>
        explicit shared_ptr(const weak_ptr<U, A>& weakPtr, const UDeleter&)
            : mpValue(weakPtr.mpValue),
              mpRefCount(weakPtr.mpRefCount),
              mAllocator(weakPtr.mAllocator)
        {
            ...
            {
                typedef ref_count_sp_t<T*, UDeleter> ref_count_type;
              ...
              mAllocator(sharedPtr.mAllocator)
        {
                  ...
                  mAllocator(sharedPtr.mAllocator)
            {
                ...
                else // Else we empty-construct.
                {
                    // Should we use the mAllocator value we got from sharedPtr.mAllocator, 
                    // or should re-initialize mAllocator to the default?
                    // mAllocator = EASTL_SHARED_PTR_DEFAULT_ALLOCATOR; <- re-initialization to default.
                    //
                    // Also, should we create a custom ref_count_type here or use ref_count_sp like in 
                    // our default constructor?
                    typedef ref_count_sp_t<T*, deleter_type> ref_count_type;

                    void* const pMemory = EASTLAlloc(mAllocator, sizeof(ref_count_type));
                    if(pMemory)
                        mpRefCount = ::new(pMemory) ref_count_type(NULL);
                }
            }
        ...
        void swap(this_type& sharedPtr)
        {
                ...
                allocator_type a     = sharedPtr.mAllocator;
                sharedPtr.mAllocator = mAllocator;
                mAllocator           = a;
            #endif
