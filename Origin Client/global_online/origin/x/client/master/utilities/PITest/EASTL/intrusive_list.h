...
namespace eastl
{
    ...
    struct intrusive_list_node
    {
        ...
        #endif
    } EASTL_MAY_ALIAS;  // It's not clear if this really should be needed. The SN PS3 compiler is generating some crashing optimized code when strict aliasing is enabled, but analysis of it seems to blame the compiler. However, this topic can be tricky.
    ...
    public:
       ...
       ~intrusive_list_base();
        ...
        bool            empty() const;
        eastl_size_t    size() const;              ///< Returns the number of elements in the list; O(n).
        ...
        EASTL_API void  reverse();                 ///< Reverses a list so that front and back are swapped; O(n).
    ...
    public:
        ...
        reverse_iterator erase(reverse_iterator pos, reverse_iterator last);
    ...
    intrusive_list<T>::erase(iterator pos)
    {
        ...
        #if EASTL_VALIDATE_INTRUSIVE_LIST
            pos.mpNode->mpPrev = pos.mpNode->mpNext = NULL;
        #endif
    ...
    intrusive_list<T>::erase(reverse_iterator position)
    {
    ...
    template <typename T>
    inline typename intrusive_list<T>::reverse_iterator
    intrusive_list<T>::erase(reverse_iterator first, reverse_iterator last)
    {
    ...
    template <typename T>
    void intrusive_list<T>::splice(iterator pos, T& value)
    {
        ...
        if(pos.mpNode != &value)
        {
            ...
            intrusive_list_node& newNext = *pos;
    ...
    template <typename T>
    void intrusive_list<T>::splice(iterator pos, intrusive_list& x)
    {
    ...
    template <typename T>
    void intrusive_list<T>::splice(iterator pos, intrusive_list& /*x*/, iterator i)
    {
        // Note: &x == this is prohibited, so self-insertion is not a problem.
        ...
        {
            // Unlink item from old list.
            intrusive_list_node& oldNext = *i.mpNode->mpNext;
            intrusive_list_node& oldPrev = *i.mpNode->mpPrev;
            oldNext.mpPrev = &oldPrev;
            oldPrev.mpNext = &oldNext;
            ...
            newPrev.mpNext = i.mpNode;
            newNext.mpPrev = i.mpNode;
            i.mpNode->mpPrev = &newPrev;
            i.mpNode->mpNext = &newNext;
        }
    }


    template <typename T>
    void intrusive_list<T>::splice(iterator pos, intrusive_list& /*x*/, iterator first, iterator last)
    {
        // Note: &x == this is prohibited, so self-insertion is not a problem.
        if(first != last)
        {
    ...
    template <typename T>
    inline void intrusive_list<T>::remove(T& value)
    {
