 ...
 * eabase.h
 *
 * Copyright (c) 2002 - 2011 Electronic Arts Inc. All rights reserved.
...
///////////////////////////////////////////////////////////////////////////////
    ...
    #define EABASE_VERSION   "2.00.40"
    #define EABASE_VERSION_N  20040
#endif
...
#if defined(EA_COMPILER_GNUC) || defined(EA_COMPILER_MSVC) || defined(EA_WCHAR_T_NON_NATIVE) || defined(EA_COMPILER_SN)
...
// would have put this into the compilertraits header.
    ...
    #if defined(__S3E__) // S3E refers to the Airplay platform, which is a mobile middleware platform that uses a crippled version of GCC.
        typedef float float_t;
        typedef double double_t;
    ...
    // width used for floating point _t types.
        ...
        // Airplay defines UINT32_MAX incorrectly to unsigned long so we are override it here.
        #undef UINT32_MAX
       ...
       typedef unsigned __int64    uint64_t;
        ...
        // Airplay/Marmalade defines int64_t itself though not by the standardized methods of other C99 compilers.
    ...
    // e.g. uint64_t x = UINT64_C(0x1111111122222222);
        ...
        // VC++ 7.0 and earlier don't handle the LL suffix.
            ...
            #endif
        #elif defined(EA_PLATFORM_PLAYSTATION2) // On the PS2, all compilers treat long as 64 bits and long long as 128 bits.
    ...
    // %l refers to 32 bit data while %ll refers to 64 bit data. 
       ...
       #define __STDC_FORMAT_MACROS
        ...
        #define EA_PRI_64_LENGTH_SPECIFIER "l"
        #define EA_SCN_64_LENGTH_SPECIFIER "l"
    #else
        #define EA_PRI_64_LENGTH_SPECIFIER "ll"
        #define EA_SCN_64_LENGTH_SPECIFIER "ll"
...
//
#if defined(EA_PLATFORM_CTR) // must check for CTR first since the compiler defines __GNUC__ (in gcc compatibility mode)
    typedef int ssize_t;
#elif !defined(__GNUC__) || defined(__ghs)
...
// takes a unicode string.
    ...
    // To do: Change this to be based on EA_COMPILER_NO_NEW_CHARACTER_TYPES.
        ...
        #endif
    #elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 404) && defined(EA_COMPILER_CPP11_ENABLED)// EDG 4.4+.
        #define EA_CHAR16_NATIVE 1
...
//
    ...
    // it can tell the difference between arrays and pointers.
    template<typename T, size_t N> 
    constexpr size_t EAArrayCount(T (&)[N]) 
    {
        return N;
    }
#endif
