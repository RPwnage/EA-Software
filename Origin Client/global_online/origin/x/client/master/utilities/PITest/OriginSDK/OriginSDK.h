...
/// \return Returns \ref ORIGIN_SUCCESS if the OriginSDK update occurred without errors. 
...
OriginErrorT ORIGIN_SDK_API OriginRequestTicketSync(OriginUserT user, OriginHandleT *ticket, OriginSizeT *length);
...
/// \note For Blaze integrators: DIGITAL goods project ID should be used when setting up titledefines.cfg. Do NOT use the packaged goods project Id.
/// https://developer.origin.com/documentation/display/blaze/Login+And+Account+Creation+Flows#LoginAndAccountCreationFlows-ConfiguringPROJECTIDandManagingONLINEACCESSEntitlements
OriginErrorT ORIGIN_SDK_API OriginRequestAuthCodeSync(OriginUserT user, const OriginCharT *clientId, OriginHandleT *authcode, OriginSizeT *length, const OriginCharT *scope = NULL);
...
/// \note For Blaze integrators: DIGITAL goods project ID should be used when setting up titledefines.cfg. Do NOT use the packaged goods project Id.
/// https://developer.origin.com/documentation/display/blaze/Login+And+Account+Creation+Flows#LoginAndAccountCreationFlows-ConfiguringPROJECTIDandManagingONLINEACCESSEntitlements
OriginErrorT ORIGIN_SDK_API OriginRequestAuthCode(OriginUserT user, const OriginCharT *clientId, OriginAuthCodeCallback callback, void * pcontext, uint32_t timeout, const OriginCharT *scope = NULL);

/// \ingroup sdk
/// \brief Get the client settings
///
...
OriginErrorT ORIGIN_SDK_API OriginGetSetting(enumSettings settingId, OriginCharT *pSetting, OriginSizeT *length);

/// \ingroup sdk
/// \brief Get the game info
///
...
/// \param[in, out] length the length of the string. (Make the string at least 16 characters long.)
/// \return Returns \ref ORIGIN_SUCCESS if the request succeeded. Returns \ref ORIGIN_ERROR_BUFFER_TOO_SMALL if there is not enough space in the provided buffer.
OriginErrorT ORIGIN_SDK_API OriginGetGameInfo(enumGameInfo gameInfoId, OriginCharT *pInfo, OriginSizeT *length);
