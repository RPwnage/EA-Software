...
namespace eastl
{
    ...
    #if (!defined(__GNUC__) || (__GNUC__ >= 3)) && !defined(__MWERKS__)
        template <typename U, typename A> friend class basic_string;
    #else
        public: // GCC 2.x and Metrowerks/Codewarrior are broken.
    #endif
    ...
    abstract_string<T>::rbegin() const
    {
    ...
    template <typename T>
    inline bool abstract_string<T>::empty() const
    {
    ...
    template <typename T>
    inline typename abstract_string<T>::size_type
    abstract_string<T>::length() const
    {
        return (size_type)(mpEnd - mpBegin);
    }
    ...
    template <typename T>
    inline typename abstract_string<T>::size_type
    abstract_string<T>::max_size() const
    {
        return kMaxSize;
    }
    ...
    template <typename T>
    inline typename abstract_string<T>::size_type
    abstract_string<T>::capacity() const
    {
    ...
    abstract_string<T>::find(const value_type* p, size_type position) const
    {
        ...
        // It turns out that the 360 compiler generates better 
        // code when we use a memcmp/memchr-based search.
