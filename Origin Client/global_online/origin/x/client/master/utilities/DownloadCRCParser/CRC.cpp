...
namespace CRCMap
{
quint32 gs_crc32TableReverse[256] =
{
...
quint32 CalculateCRC(quint32 previous, const void* data, unsigned int dataLen)
{
    ...
    quint32 nInitialValue = previous;

    const quint8* pData8 = (const quint8*)data;

    while (nLength >= 8)
    {
        nInitialValue = gs_crc32TableReverse[(nInitialValue ^ *pData8++) & 0xff] ^ (nInitialValue >> 8);
        nInitialValue = gs_crc32TableReverse[(nInitialValue ^ *pData8++) & 0xff] ^ (nInitialValue >> 8);
        nInitialValue = gs_crc32TableReverse[(nInitialValue ^ *pData8++) & 0xff] ^ (nInitialValue >> 8);
        nInitialValue = gs_crc32TableReverse[(nInitialValue ^ *pData8++) & 0xff] ^ (nInitialValue >> 8);
        nInitialValue = gs_crc32TableReverse[(nInitialValue ^ *pData8++) & 0xff] ^ (nInitialValue >> 8);
...
{
    return CalculateFinalCRC(CalculateCRC(GetCRCInitialValue(), data, dataLen));
}
...
quint32 CalculateCRCForFile(const QString& file, qint64 maxReadBytes, bool finalize)
{
    ...
    quint8* buffer = new quint8[maxBytesPerRead];
