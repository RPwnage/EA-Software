...
QString g_logFolder = "/Origin/Logs/OriginWebHelperService_Log";
QString g_logFolderTool = "/Origin/Logs/OriginWebHelperService_InstallTool_Log"; // Used on Windows only for when running in 'install tool' mode
...
int InstallToolMain(ArgsParser& args, bool installed)
{
    if (args.toolMode() == ArgsParser::Install || args.toolMode() == ArgsParser::SelfInstall)
    {
        ...
        if (installed)
        {
            ORIGIN_LOG_EVENT << "Origin Web Helper Service already installed, checking config...";
        }

        if (RegisterService(QS16(ORIGIN_BEACON_SERVICE_NAME), args.binPath(), false, true))
        {
    ...
    else if (args.toolMode() == ArgsParser::Uninstall)
    {
        ...
        if (installed)
        {
            if (UnRegisterService(QS16(ORIGIN_BEACON_SERVICE_NAME)))
            {
                ORIGIN_LOG_EVENT << "Operation Successful.";
                return 1;
            }
        ...
        {
            ORIGIN_LOG_EVENT << "Origin Web Helper Service not installed.";
            return 0;
        }
    }
    else if (args.toolMode() == ArgsParser::ReadConfig)
    {
        ...
        if (QueryOriginServiceConfig(QS16(ORIGIN_BEACON_SERVICE_NAME)))
        {
            ORIGIN_LOG_EVENT << "Operation Successful.";
            return 1;
        }
