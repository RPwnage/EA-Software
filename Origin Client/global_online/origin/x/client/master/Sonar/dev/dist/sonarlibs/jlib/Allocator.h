#pragma once
#include <assert.h>
#include <jlib/types.h>

namespace jlib
{

#define ALLOCATOR_GUARD 0xaaaaaaaa 

  template <typename _T, size_t _PREALLOC, bool _DETECTLEAKS>
  class Allocator
  {
  public:
    Allocator (void)
    {
      m_pFreeList = (ITEM *) 0;
      m_cItems = 0;

      for (size_t index = 0; index < _PREALLOC; index ++)
      {
        (m_preAlloc + index)->pNext = m_pFreeList;
#ifdef _DEBUG
        (m_preAlloc + index)->guard = ALLOCATOR_GUARD;
#endif
        m_pFreeList = m_preAlloc + index;
        m_cItems ++;
      }
    }

    ~Allocator (void)
    {
      ITEM *ptrStart = m_preAlloc + 0;
      ITEM *ptrEnd = m_preAlloc + _PREALLOC;

      size_t cFree = 0;

      while (m_pFreeList)
      {
#ifdef _DEBUG
        assert (m_pFreeList->guard == ALLOCATOR_GUARD);
#endif
        if (m_pFreeList < ptrStart && m_pFreeList > ptrEnd)
        {
          ITEM *pItem = m_pFreeList;
          m_pFreeList = m_pFreeList->pNext;
          delete pItem;
        }
        else
        {
          m_pFreeList = m_pFreeList->pNext;
        }

        cFree ++;
      }

      if (_DETECTLEAKS)
      {	
        assert (cFree == m_cItems);
      }
    }

    _T *alloc (void)
    {
      ITEM *pItem;

      if (!m_pFreeList)
      {
        m_pFreeList = new ITEM;
        m_cItems ++;
        m_pFreeList->pNext = (ITEM *) 0;
#ifdef _DEBUG
        m_pFreeList->guard = ALLOCATOR_GUARD;
#endif
      }

      pItem = m_pFreeList;
      m_pFreeList = m_pFreeList->pNext;

#ifdef _DEBUG
      assert (pItem->guard == ALLOCATOR_GUARD);
#endif
      return (_T *) &pItem->data;
    }

    void free (const _T *_pItem)
    {
      ITEM *pPtr = (ITEM *) _pItem;

#ifdef _DEBUG
      assert (pPtr->guard == ALLOCATOR_GUARD);
#endif

      ITEM *pItem = m_pFreeList;
      m_pFreeList = (ITEM *) pPtr;
      m_pFreeList->pNext = pItem;
    }

  private:

    struct ITEM
    {
      jlib::UINT8 data[sizeof(_T)];
#ifdef _DEBUG
      unsigned int guard;
#endif
      ITEM *pNext;
    };

    ITEM *m_pFreeList;
    ITEM m_preAlloc[_PREALLOC];

    size_t m_cItems;

  };

}