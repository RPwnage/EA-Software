...
@implementation NSObject (OJRSwizzle)
...
+ (BOOL)ojr_swizzleMethod:(SEL)origSel_ withMethod:(SEL)altSel_ error:(NSError**)error_ {
	...
	if (!directOriginalMethod || !directAlternateMethod) {
		...
		struct objc_method_list *hoisted_method_list = (struct objc_method_list *)malloc(sizeof(struct objc_method_list) + (sizeof(struct objc_method)*(hoisted_method_count-1)));
...
+ (BOOL)ojr_swizzleClassMethod:(SEL)origSel_ withClassMethod:(SEL)altSel_ error:(NSError**)error_ {
	return [GetClass((id)self) ojr_swizzleMethod:origSel_ withMethod:altSel_ error:error_];
}

@end

// Simple wrapper for services/log/LogServices to help with Objective-C files
