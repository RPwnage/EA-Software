...
namespace Downloader
{
	...
	const qint32 nValueStartLength = VALUE_START.size();
	const qint32 nValueEndLength = VALUE_END.size();
	...
	{
		ORIGIN_ASSERT(ValidateKey(sKey));
	...
	bool Parameterizer::GetString(QString sKey, QString& sValue, bool bIgnoreCase) const
	{
		...
		{	
			...
			sValue = QUrl::fromPercentEncoding((*it).second.toUtf8());
			return true;
		}
	...
	bool Parameterizer::GetULong(const QString& sKey, unsigned long& nValue) const
	{
		...
		bool ok;
		nValue = sNumberString.toULong(&ok);
	...
	bool Parameterizer::Get64BitInt(const QString& sKey, qint64& nValue) const
	{
		QString sNumberString;
		if (!GetString(sKey, sNumberString))
			return false;
	...
	qint64 Parameterizer::Get64BitInt(const QString& sKey) const
	{
		qint64 result = 0;
	...
	bool Parameterizer::Set64BitInt(const QString& sKey, qint64 nValue)
	{
		ORIGIN_ASSERT(ValidateKey(sKey));
	...
	{
		return SetString(sKey, QString::number((qint64)pVoid));
	}
