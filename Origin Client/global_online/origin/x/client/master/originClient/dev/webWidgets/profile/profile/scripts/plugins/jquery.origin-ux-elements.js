...
// Test for the "on" function in EA.originUxElements.$ (1.7+)
...
(function($, document){
    ...
    // due to possible conflicts in jQuery’s namespace with lots of namespace-polluting methods, we use this method-delegation pattern (well known from jquery-UI):
    //
    ...
    $.fn.scrollbar.defaults = {
        ...
        handleMinHeight     : 16,     // minimum height of handle [Number in px]. This property will only be used if handleHeight-option is set to 'auto'.
        scrollTimeout       : 150,     // timeout of handle speed while mousedown on arrows [Number in milli sec].
        ...
        scrollStepArrows    : 1       // increment of handle position between two mousedowns in the handle container [px].
    };
    ...
    $.fn.scrollbar.Scrollbar.prototype = {
        ...
        setHandle: function(){
            ...
            this.props.contentHeight =  $.fn.scrollbar.contentHeight( this.pane );
        ...
        appendEvents: function(){
            ...
            this.handle.on('mousedown.handle', $.proxy(this, 'startOfHandleMove'));

            // append mousedown event on handle-container
            this.handleContainer.on('mousedown.handle', $.proxy(this, 'onHandleContainerMousedown'));

            // append hover event on handle-container
            this.handleContainer.on('mouseenter.container mouseleave.container', $.proxy(this, 'onHandleContainerHover'));

            // append click event on scrollbar-up- and scrollbar-down-handles
            this.handleArrows.on('mousedown.arrows', $.proxy(this, 'onArrowsMousedown'));

            // append mousewheel event on content container
            this.container.on('mousewheel.container', $.proxy(this, 'onMouseWheel'));
            //this.container.on('scroll.container', $.proxy(this, 'onScroll'));

            // append hover event on content container
            this.container.on('mouseenter.container mouseleave.container', $.proxy(this, 'onContentHover'));
        ...
        startOfHandleMove: function(ev){
            ...
            $(document).on('mousemove.handle', $.proxy(this, 'onHandleMove')).on('mouseup.handle', $.proxy(this, 'endOfHandleMove'));
        ...
        onHandleContainerMousedown: function(ev){
            ...
            var that = this;
            $(document).on('mouseup.handlecontainer', function(){
                clearInterval(timer);
    ...
    $.fn.scrollbar.contentHeight = function(container){
        ...
        if (wrapper.height() > 0) {
            ...
            var copied_elem = wrapper.clone()
                .attr("id", false)
                .css({visibility:"hidden", display:"block", position:"absolute", top:0, left:0});
            $("body").append(copied_elem);
            //console.log("height: " + copied_elem.height() + "body: " + $("body").is(":visible") );
            height = copied_elem.height();
            copied_elem.remove();
        }

        //if (isBodyHidden){ $("body").hide(); }
    ...
    $.fn.scrollbar.mouseWheelHandler = function(event) {
        ...
        //alert("mouseWheelHandler " + event.wheelDelta );
...
})(EA.originUxElements.$, document);  // inject global jQuery object
/************************ end: scroll.js ************************/

(function( $, undefined ) {
    $.fn.originUxElements = function( options ) {
        ...
        var methods = {
            ...
            init : function() {
                ...
                $(".origin-ux-checkbox-control input[checked=checked]").parents(".origin-ux-checkbox-control").addClass("checked");

                $(".origin-ux-radio-button-control span input[checked=checked]").parents(".origin-ux-radio-button-control").addClass("checked");


                /* build custom drop down */

                $(".origin-ux-drop-down-control").each(function(){

                    $("select", this).wrap( "<div class='origin-ux-drop-down-selection' />"  );
                    $(".origin-ux-drop-down-selection", this).prepend( "<span value='" + $("select option:selected", this).val()  +  "'>" + $("select option:selected", this).text() + "</span>" );


                    var dropDownOptions = $("<div class='drop-down-options-container'><div class='drop-down-options'><div class='scrollbar'/></div></div>");

                    $("select option", this).each(function(){

                        $(".scrollbar", dropDownOptions).append( "<div class='drop-down-item'><a href='#' value='" + $(this).val() + "'><span>" + $(this).text() + "</span></a></div>");
                    });

                    if ( $(".scrollbar .drop-down-item", dropDownOptions ).length <= 1 ) { $(".scrollbar", dropDownOptions).addClass("items-one"); }
                    if ( $(".scrollbar .drop-down-item", dropDownOptions ).length == 2 ) { $(".scrollbar", dropDownOptions).addClass("items-two"); }
                    if ( $(".scrollbar .drop-down-item", dropDownOptions ).length == 3 ) { $(".scrollbar", dropDownOptions).addClass("items-three"); }
                    if ( $(".scrollbar .drop-down-item", dropDownOptions ).length == 4 ) { $(".scrollbar", dropDownOptions).addClass("items-four"); }
                    if ( $(".scrollbar .drop-down-item", dropDownOptions ).length == 5 ) { $(".scrollbar", dropDownOptions).addClass("items-five"); }
                ...
                $(".origin-ux-button").on( "mousedown.originUxElements", methods.events.uxButtonMouseDown );
                $(".origin-ux-button").on( "mouseup.originUxElements", methods.events.uxButtonMouseUp );
                $(".origin-ux-button").on( "click.originUxElements", methods.events.uxButtonClick );
                $(".origin-ux-button").on( "focus.originUxElements", methods.events.uxButtonFocus );
                $(".origin-ux-button").on( "blur.originUxElements", methods.events.uxButtonBlur );
                $(".origin-ux-button").on( "mouseout.originUxElements", methods.events.uxButtonMouseOut );


                //origin-ux-textbox

                $(".origin-ux-textbox-control input").on( "focus.originUxElements", methods.events.uxTextboxFocus );
                $(".origin-ux-textbox-control input").on( "blur.originUxElements", methods.events.uxTextboxBlur );

                $(".origin-ux-textbox-control input[type=password]").on( "keypress.originUxElements", methods.events.uxTextboxCapLock )

                //origin-ux-checkbox

                $(".origin-ux-checkbox-control input").on("focus.originUxElements", methods.events.uxCheckboxFocus );
                $(".origin-ux-checkbox-control span").on("click.originUxElements", methods.events.uxCheckboxClick );
                $(".origin-ux-checkbox-control label").on("click.originUxElements", methods.events.uxCheckboxLabelClick );
                $(".origin-ux-checkbox-control input").on("blur.originUxElements", methods.events.uxCheckboxBlur );


                //origin-ux-radio-button

                $(".origin-ux-radio-button-control input").on("focus.originUxElements", methods.events.uxRadioButtonFocus );
                $(".origin-ux-radio-button-control input").on("blur.originUxElements", methods.events.uxRadioButtonBlur );
                $(".origin-ux-radio-button-control").on("click.originUxElements", methods.events.uxRadioButtonClick );

                //origin-ux-drop-down

                $(".origin-ux-drop-down-control select").on("focus.originUxElements", methods.events.uxDropDownFocus );
                $(".origin-ux-drop-down-control select").on("blur.originUxElements", methods.events.uxDropDownBlur );
                $(".origin-ux-drop-down-control select").on("keyup.originUxElements", methods.events.uxDropDownKeyup );
                $(".origin-ux-drop-down-control select").change( methods.events.uxDropDownKeyup );
                $(".origin-ux-drop-down-control .origin-ux-drop-down-selection").on("click.originUxElements", methods.events.uxDropDownShow  );
                $(".origin-ux-drop-down-control .drop-down-options-container a").on("click.originUxElements", methods.events.uxDropDownSelect);
                methods.helpers.determineDropDownSize();
                
                //origin-ux-tooltip

                $(".origin-ux-tooltip a").on("click.originUxElements", methods.events.uxTooltipClick );


                // document and control level handling
                $("html").on("click.originUxElements", methods.events.uxHtmlClick );
                $("html").on("keyup.originUxElements", methods.events.uxHtmlKeyup );
                $(".origin-ux-control").on("click.originUxElements", methods.events.uxControlClick )
            ...
            events : {
                ...
                uxControlClick : function ( event ){
                    //
                },


                uxHtmlClick : function ( event ){
                    //event.preventDefault();

                    // hides drop downs
                    $(".drop-down-options-container").hide();
                    $(".origin-ux-drop-down-control").removeClass("selected");
                    $(".origin-ux-drop-down-control").removeClass("focus");

                    //$(".origin-ux-checkbox-control").removeClass("focus");
                    //$(".origin-ux-radio-button-control").removeClass("focus");


                },

                uxHtmlKeyup : function ( event ){

                    // this is the normalize checkbox checking by spacebar
                    if ( event.which == 32 && $(document.activeElement).is(':checkbox') && $(document.activeElement).parent().parent(".origin-ux-checkbox-control").length  ){
                        event.preventDefault();
                        methods.events.uxCheckboxClick( event, $(document.activeElement).parent() );
                    }
                ...
                uxButtonMouseDown : function ( event ){

                    if ( $(this).attr("propagate") != "true"){ event.preventDefault(); }

                    if ( !$(this).hasClass('disabled')) { $(this).addClass("active");}
                },

                uxButtonMouseUp : function ( event ){
                ...
                uxButtonBlur : function ( event ) {
                    if ($(this).hasClass('disabled')){ return false; }
                    $(this).removeClass('focus');

                },

                uxButtonMouseOut : function ( event ){
                ...
                uxTextboxFocus : function ( event ){
                    $(this).parents(".origin-ux-textbox").addClass("focus");
                },

                uxTextboxBlur : function ( event ){
                    $(this).parents(".origin-ux-textbox").removeClass("focus");
                },

                uxTextboxCapLock : function( event ) {
                    // browser detection because this is not a feature that can be detected
                    // Webkit and MSIE version 10 automatically provides caplock detection and notification
                    if ( $.browser.webkit || ($.browser.msie && $.browser.version > 9 )){ return true; }


                    var s = String.fromCharCode( event.which );
                    if ( s.toUpperCase() === s && s.toLowerCase() !== s && !event.shiftKey ) {
                        $(this).parents(".origin-ux-textbox").addClass("caplock");
                    } else {
                        $(this).parents(".origin-ux-textbox").removeClass("caplock");
                    }
                },
                ...
                uxCheckboxFocus : function ( event ){


                    if ( $(this).parents(".origin-ux-checkbox-control").hasClass("disabled") ){ return false; };
                    $(".origin-ux-checkbox-control").removeClass("focus");
                    $(this).parents(".origin-ux-checkbox-control").addClass("focus");

                },

                uxCheckboxBlur : function ( event ){

                    if ( $(this).parents(".origin-ux-checkbox-control").hasClass("disabled") ){ return false; };
                    $(this).parents(".origin-ux-checkbox-control").removeClass("focus");

                },


                uxCheckboxLabelClick : function ( event, control ){
                    $(this).siblings("span").click();
                },


                uxCheckboxClick : function ( event, control ){

                    event.preventDefault();

                    var checkboxControl = $(this);
                    if ( control ){  checkboxControl = control; }

                    if ( $(checkboxControl).parents(".origin-ux-checkbox-control").hasClass("disabled") ){ return false; };

                    $(checkboxControl).parents(".origin-ux-checkbox-control").removeClass("field-error");

                    if ( $("input", checkboxControl).prop("checked") ) {

                        $("input", checkboxControl).prop("checked", false);
                        $(checkboxControl).parents(".origin-ux-checkbox-control").removeClass("checked");
                    } else {

                        $("input", checkboxControl).prop("checked", true);
                        $(checkboxControl).parents(".origin-ux-checkbox-control").addClass("checked");
                    }

                    $("input", checkboxControl).trigger( "change" );
                    //$("input", checkboxControl).focus();
                ...
                uxRadioButtonFocus : function( event ){

                    if ( $(this).parents(".origin-ux-radio-button-control").hasClass("disabled") ){ return false; };
                    $(this).parents(".origin-ux-radio-button-control").addClass("focus");

                },

                uxRadioButtonBlur : function( event ){

                    $(this).parents(".origin-ux-radio-button-control").removeClass("focus");

                },

                uxRadioButtonClick : function( event ){
                    ...
                    $(this).parents(".origin-ux-radio-button-control").removeClass("field-error");

                    radioButtonSet = $(this).find("input").attr("name");

                    $("input[name='"+ radioButtonSet  +"']").parents(".origin-ux-radio-button-control").removeClass("checked");
                ...
                uxDropDownFocus : function ( event ){
                    // need to figure out the side effect of this. I have a vague memory that this might screw us up.
                    if ( $(this).attr("propagate") != "true"){ event.preventDefault(); }

                    if ( $(this).parents(".origin-ux-drop-down-control").hasClass("disabled") ){ return false; };
                    $(".origin-ux-drop-down-control").removeClass("focus");
                    $(this).parents(".origin-ux-drop-down-control").addClass("focus");
                },

                uxDropDownBlur : function ( event ){

                    if ( $(this).attr("propagate") != "true"){ event.preventDefault(); }

                    $(this).parents(".origin-ux-drop-down-control").removeClass("focus");
                    $(this).parents(".origin-ux-drop-down-control").removeClass("selected");
                    $(this).parents(".origin-ux-drop-down-control").find(".drop-down-options-container").hide();
                },

                uxDropDownKeyup : function ( event ){
                    $(this).parents(".origin-ux-drop-down-control").find(".origin-ux-drop-down-selection span").text( $( "option:selected", this).text() );
                },

                uxDropDownShow : function ( event ){
                
                    if ( $(this).parents(".origin-ux-drop-down-control").hasClass("disabled") ){ return false; };

                    //console.log('uxDropDownShow');

                    if ( $(this).attr("propagate") != "true"){ event.preventDefault(); event.stopPropagation(); }

                    $(".drop-down-options-container").hide();
                    $(".origin-ux-drop-down .origin-ux-drop-down-selection").removeClass("selected");
                    $(".origin-ux-drop-down-control").removeClass("focus");

                    if ( $(this).parents(".origin-ux-drop-down-control").hasClass("selected")  ){

                        $(this).parents(".origin-ux-drop-down-control").removeClass("selected");
                        $(this).parents(".origin-ux-drop-down-control").addClass("focus");

                        $(this).next(".drop-down-options-container").hide();

                    } else {

                        $(this).parents(".origin-ux-drop-down-control").addClass("selected");
                        $(this).parents(".origin-ux-drop-down-control").removeClass("focus");

                        $(this).next(".drop-down-options-container").show();
                    }

                    $("select", this).focus();

                    var selectedItem = $(this).parent().parent().find("div.drop-down-item > a.selected");
                    $(selectedItem).addClass('show-selected');
                },

                uxDropDownHover : function (event) {
                    var selectedItem = $(this).parent().parent().find("div.drop-down-item > a.selected");
                    $(selectedItem).removeClass('show-selected');
                },
                
                uxDropDownSelect : function ( event ){
                    if ( $(this).attr("propagate") != "true"){ event.preventDefault(); event.stopPropagation(); }

                    var isChange = false;

                    var currentText = $(this).parents(".origin-ux-drop-down-control").find(".origin-ux-drop-down-selection span").text();

                    $(this).parents(".drop-down-options-container").hide();
                    $(this).parents(".origin-ux-drop-down-control").find(".origin-ux-drop-down-selection span").text( $(this).text() );
                    $(this).parents(".origin-ux-drop-down-control").removeClass("selected");
                    $(this).parents(".origin-ux-drop-down-control").addClass("focus");
                    $(this).parents(".origin-ux-drop-down-control").find("select").val( $(this).attr("value") );

                    if ( currentText != $(this).text() ){
                        $(this).parents(".origin-ux-drop-down-control").find("select").change();
                    }

                    $(this).parents(".origin-ux-drop-down-control").find("select").focus();

                    var curSelectedItem = $(this).parent().parent().find("div.drop-down-item > a.selected");
                    $(curSelectedItem).removeClass('show-selected').removeClass('selected');
                    
                    $(this).addClass('selected').addClass('show-selected');
                ...
                uxDropDownRepaint : function (){
                    ...
                    $(".origin-ux-drop-down-control").each(function(){
                        ...
                        $(".drop-down-options-container", this).remove();
                        ...
                        if ( $(".scrollbar .drop-down-item", dropDownOptions ).length == 4 ) { $(".scrollbar", dropDownOptions).addClass("items-four"); }
                        if ( $(".scrollbar .drop-down-item", dropDownOptions ).length == 5 ) { $(".scrollbar", dropDownOptions).addClass("items-five"); }
                        if ( $(".scrollbar .drop-down-item", dropDownOptions ).length >= 6 ) { $(".scrollbar", dropDownOptions).addClass("items-six"); }

                        $("a", dropDownOptions).on("click.originUxElements", methods.events.uxDropDownSelect);

                        $(this).append( dropDownOptions );

                        $(".drop-down-item").mouseenter(methods.events.uxDropDownHover);
                    });
            ...
            helpers : {
                determineDropDownSize : function () {
                    $(".origin-ux-drop-down-control").each(function(){
                        var origWidth = $(this).width();
                        var maxWidth = 0;
                        var listOfElements = $(this).find(".scrollbar .drop-down-item");
                        $(listOfElements).each(function() {     
                            var textVal = $(this).text();       
                            var newDivText = "<div class='textWidthDiv' style='position:absolute;visibility:hidden; display:block;'>";
                            newDivText += textVal;
                            newDivText += "</div>";
                            var retVal = $('body').append(newDivText);
                            $(".textWidthDiv").css("font-family", $(this).css("font-family"));
                            $(".textWidthDiv").css("font-size", $(this).css("font-size"));
                            $(".textWidthDiv").css("font-weight", $(this).css("font-weight"));
                            $(".textWidthDiv").css("font-style", $(this).css("font-style"));
                            var thisWidth = $(".textWidthDiv").width() + 20;
                            $(".textWidthDiv").css("This width: " + thisWidth);                         
        ...
        var publicMethods = [ "init", "dropdown-repaint" ];
        ...
        if ( typeof options === 'object' || ! options ) {
            ...
            // call specific methods with arguements

            switch(options)
            {
                case "dropdown-repaint":
                    methods.events.uxDropDownRepaint();
                    break;
            }
        };


    };  // END: $.fn.originUxElements = function() {


    /*
     $.fn.originUxElementsPostRender = function( ) {
     ...
     $(".origin-ux-button").each( function(index, element){
     if ( $(this).width() % 2 == 0 ){
     var rightPadding = $(this).find("span span").css("padding-right").replace("px", "");
     $(this).find("span span").css("padding-right", ( parseInt( rightPadding ) + 1) + "px" );
     }
     });
     ...
     }; // END; $.fn.originUxElementsPostRender = function() {
     */
})( EA.originUxElements.$ ); // END: (function( $ ){
