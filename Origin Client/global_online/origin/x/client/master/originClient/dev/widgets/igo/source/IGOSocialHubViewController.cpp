//
//  IGOSocialHubViewController.cpp
//  originClient
//
...
IGOSocialHubViewController::IGOSocialHubViewController()
{
    ...
    OriginWebController(mWindow->webview()->webview(), NULL, false);
...
// IGO specific
bool IGOSocialHubViewController::ivcPreSetup(Origin::Engine::IIGOWindowManager::WindowProperties* properties, Origin::Engine::IIGOWindowManager::WindowBehavior* behavior)
{
    QSize screen = Origin::Engine::IGOController::instance()->igowm()->screenSize();
...
void IGOSocialHubViewController::ivcPostSetup()
{
    // Time to request the view from the ASP
    QUrlQuery query = Origin::Engine::IGOController::instance()->createJSRequestParams(this);

    ClientFlow::instance()->view()->openJSWindow("socialhub", query.toString(QUrl::FullyEncoded));
}

QWebPage* IGOSocialHubViewController::createJSWindow(const QUrl& url)
{
    QUrlQuery query(url);
...
void IGOSocialHubViewController::onSizeChanged(uint32_t width, uint32_t height)
{
    Origin::Engine::IGOController::instance()->igowm()->setWindowPosition(mWindow, defaultPosition(width, height));
}

QPoint IGOSocialHubViewController::defaultPosition(uint32_t width, uint32_t height)
{
