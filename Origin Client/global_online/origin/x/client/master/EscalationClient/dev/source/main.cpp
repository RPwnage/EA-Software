...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        sharedMemory.setKey(Origin::Escalation::ESCALATION_SERVICE_NAME);
    ...
    {
        ORIGIN_LOG_EVENT << "Origin Client Service: Running as normal executable.";
    }
    else
    {
        ORIGIN_LOG_EVENT << "Origin Client Service: Running as service.";
    ...
    {
        bool running = false;
        bool installed = QueryOriginServiceStatus(QS16(ORIGIN_ESCALATION_SERVICE_NAME), running);

        if (args.toolMode() != ArgsParser::Normal)
        {
            ORIGIN_LOG_EVENT << "Origin Client Service Installed: " << installed << " Running: " << running;
...
// to remove the Escalation Service (if they know how) and it will not be automatically re-installed until Origin updates again.
        ...
        if (osvi.dwMajorVersion >= 6)
        {
            if (!installed)
            {
                ORIGIN_LOG_EVENT << "Origin Client Service not installed, self-installing...";
    ...
    if (runningAsWindowsService)
    {
        ORIGIN_LOG_EVENT << "Origin Client Service starting up...";
        ...
        {
            {ORIGIN_ESCALATION_SERVICE_NAME, (LPSERVICE_MAIN_FUNCTION) ServiceMain},
            {NULL, NULL}
        };
    ...
    else
    {
        ...
        Origin::Escalation::EscalationServiceWin service(false, QStringList(), 0);
