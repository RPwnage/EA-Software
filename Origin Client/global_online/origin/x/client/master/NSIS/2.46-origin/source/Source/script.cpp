...
int CEXEBuild::doCommand(int which_token, LineParser &line)
{
  ...
  switch (which_token)
  {
    ...
    case TOK_LICENSELANGSTRING:
    {
      ...
      memset(data, 0, datalen+2*sizeof(TCHAR));
      if (!data)
      {
    ...
    case TOK_P_DEFINE:
    {
      ...
      TCHAR *value;
      GrowBuf file_buf;
      std::vector<TCHAR> datebuf(256);
      std::vector<TCHAR> mathbuf(256);
      ...
      if (!_tcsicmp(define,_T("/date")) || !_tcsicmp(define,_T("/utcdate"))) {
        ...
        datebuf[0]=0;
        size_t s=_tcsftime(&datebuf[0],datebuf.size(),value,localtime(&rawtime));

        if (s == 0)
          datebuf[0]=0;
        else
          datebuf[max(s,datebuf.size()-1)]=0;
        ...
        if (fp)  {
          ...
          for (;;) {
            ...
            *++p=0;
            ...
            file_buf.add(&str[0],_tcsclen(&str[0]));
          }
          fclose(fp);
        }
        file_buf.add(_T("\0"),1);
