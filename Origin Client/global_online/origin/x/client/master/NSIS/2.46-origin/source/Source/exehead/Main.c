...
NSIS_STRING state_temp_dir_backup = {0};
...
//      if (*(LPDWORD)cmdline == CHAR4_TO_DWORD('N','C','R','C') && END_OF_ARG(cmdline[4]))
  ...
  // add an Origin sub folder to the global NSIS TEMP folder
  if (ValidateTempDir())
  	mystrcat(state_temp_dir, _T("\\Origin"));
  	
  if (!ValidateTempDir())
  {
    GetWindowsDirectory(state_temp_dir, NSIS_MAX_STRLEN - (5+7)); // leave space for \Temp & \Origin
  ...
  DeleteFile(state_language);
	...
	// save our temp folder for later usage -> deletion
  mystrcpy(state_temp_dir_backup, state_temp_dir);
...
void NSISCALL CleanUp()
{
  ...
  if (state_temp_dir_backup[0] != 0)
  	myDelete(state_temp_dir_backup, DEL_DIR | DEL_RECURSE | DEL_REBOOT);
