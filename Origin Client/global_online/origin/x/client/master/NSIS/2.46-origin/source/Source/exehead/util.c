...
int NSISCALL my_MessageBox(const TCHAR *text, UINT type) {
  ...
  GetEnvironmentVariableW( _T("ORIGIN_INSTALLER_LANGUAGE"), languageId, sizeof(languageId)/sizeof(languageId[0]) );
  langId = myatoi(languageId);

  return MessageBoxEx(mbp.hwndOwner, mbp.lpszText, mbp.lpszCaption, mbp.dwStyle, langId);
}
    ...
    if (NS_IS_CODE(nVarIdx) && nVarIdx > NS_CODES_START)
    {
      ...
      else if (nVarIdx == NS_VAR_CODE)
      {
        ...
        if ((unsigned int)(nData - 21) < 7) {
          ...
          if( (nData - 21) != 0 /*$INSTDIR*/)	// remove filtering for $INSTDIR - do this in the NSI script!!!
			validate_filename(out);
        }
      } // == VAR_CODES_START
