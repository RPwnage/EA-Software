  ...
  if (uMsg == WM_INITDIALOG || uMsg == WM_NOTIFY_OUTER_NEXT)
  {
    ...
    if (uMsg == WM_INITDIALOG)
    {
      ...
      g_quit_flag = ExecuteCallbackFunction(CB_ONGUIINIT);
    	...
    	ShowWindow(GetDlgItem(g_hwnd,IDC_LICENSEPRINT), SW_HIDE); 
...
static BOOL CALLBACK LicenseProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  EDITSTREAM es;
  static TCHAR g_LicenseFileName[1024] = {0};
  es.dwError = 0;
  ...
  if (uMsg == WM_INITDIALOG)
  {
    ...
    ShowWindow(GetDlgItem(g_hwnd,IDC_LICENSEPRINT), SW_SHOW); 

    bCookieNuked = FALSE;
    // SF_UNICODE is never set for ANSI NSIS
    
    lstrcpy(g_LicenseFileName, state_temp_dir);

    if (lt & SF_RTF)
    	lstrcat(g_LicenseFileName, _T("\\EULA.rtf"));
    else
    	lstrcat(g_LicenseFileName, _T("\\EULA.txt"));
    	
    if (lt & SF_UNICODE)
    {
      es.dwCookie = (DWORD_PTR)(++l);
      es.pfnCallback = StreamLicenseW;
			hf = CreateFile(g_LicenseFileName, GENERIC_WRITE, 0, 0, OPEN_ALWAYS, 0, 0);
			if (hf != INVALID_HANDLE_VALUE)
			{
				DWORD wr=0;
				WriteFile(hf, (LPCVOID)es.dwCookie, (lstrlenW((wchar_t*)es.dwCookie)+1)*2, &wr, NULL);
				CloseHandle(hf);
   		}
    }
    else
    {
...
#ifdef _UNICODE
      {
         ...
         es.dwCookie = (DWORD_PTR) tmp;
					...
					{
						DWORD wr=0;
						WriteFile(hf, (LPCVOID)es.dwCookie, lstrlenA((char*)es.dwCookie)+1, &wr, NULL);
						CloseHandle(hf);
		   		}        
      }
#else
...
#undef lbg
  ...
  if (uMsg == WM_COMMAND)
  {
    ...
    if (id == IDC_LICENSEPRINT)
    {
      ShellExecuteW(hwndDlg,_T("open"),g_LicenseFileName,NULL,NULL,SW_SHOWNORMAL);   	
    }
  }
...
static BOOL CALLBACK DirProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if (uMsg == WM_NOTIFY_INIGO_MONTOYA)
  {
	...
	//validate_filename(dir);	// remove filtering for $INSTDIR - do this in the NSI script!!!
#ifdef NSIS_CONFIG_LOG
#if !defined(NSIS_CONFIG_LOG_ODS) && !defined(NSIS_CONFIG_LOG_STDOUT)
    build_g_logfile();
#endif
