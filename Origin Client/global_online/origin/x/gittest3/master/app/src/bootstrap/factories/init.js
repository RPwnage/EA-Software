...
(function() {
    ...
    function BootstrapInitFactory($cookies, HttpUtilFactory, AppUrlsFactory, PreloadFactory, OverridesFactory, LogFactory, LOCDICTIONARY) {
        var locale,
            promiseMap = {
        ...
        function setOfflineCapable(offlineGoodToGo) {
            if ((typeof OriginOnlineStatus !== 'undefined') && OriginOnlineStatus.onlineState) {
                ...
                if (typeof OriginClientSettings !== 'undefined') {
                    OriginClientSettings.writeSetting('SPAPreloadSuccess', offlineGoodToGo);
                    LogFactory.log('BootstrapInitFactory:OriginClientSettings defined, setting SPAPreloadSuccess=', offlineGoodToGo);
                }
            }
        }
        ...
        function initAppUrlsAndOfflineCache() {
            return AppUrlsFactory.retrieveUrls(locale.replace('_', '-'))
                .then(PreloadFactory.preload)
        ...
        function getAsynchronousDependencies() {
            ...
            asyncDependencyArray[promiseMap.initJSSDKPromise] = Origin.init(OverridesFactory.getJSSDKOverrides());
