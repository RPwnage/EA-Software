        ...
        wss.on('connection', function(ws) {
            ...
            ws.on('message', function(message) {
                ...
                if (message.indexOf('<stream:stream ') === 0) {
                            ...
                            try {
                                ...
                                http.get('http://127.0.0.1:1400/data/' + username + '/persona?', function(personaRes) {
                                    ...
                                    personaRes.on("data", function(personaChunk) {
                                        ...
                                        var user = new xmppObjects.Person(persona['pidId'] + '@' + localAddress + '/mockup', persona['name'], persona['personaId'], persona['displayName']);
                        ...
                        if (session) {
                            if (stub) {
                                if (session.id) {
                                    ...
                                    if (friend) {
                                        stub.respondFriendPresence(friend);
                                    }
                    ...
                    } else if (undefined === presence.attrs.type) {
                        // Origin.xmpp.updatePresence

                        if (stub) {
                            var jids = friends.getFriendJIDs();
                            for (var jid in jids) {
                                // don't know what server should response when Origin.xmpp.updatePresence() is called
                                //stub.sendPresence(friends.getFriendByJID(jids[jid]));
                            }
                        }
