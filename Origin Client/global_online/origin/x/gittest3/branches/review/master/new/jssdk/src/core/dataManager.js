...
define([
    ...
    function objKeyMatches(obj1, obj2, key, blockStr) {
        ...
        //logger.log ('objKeyMatches - ', blockStr, ': ', key,' failed:', obj1[key], ',', obj2[key]);
        return false;
    }
            ...
            for (i = 0; i < queue.length; i++) {
                if (queue[i].baseUrl === endpoint) { //check endpoint first
                    ...
                    if (configmatch &&
                        ...
                        logger.log('enQueue: found one in queue already', endpoint);
            ...
            if (triggerRelogin) {
                return new Promise(function(resolve, reject) {
                    ...
                    loginSucceeded = function() {
                        events.off(events.AUTH_FAILEDCREDENTIAL, loginFailed);
                        //AUTH_SUCCESSRETRY.off already because of events.once
                    ...
                    loginFailed = function() {
                        events.off(events.AUTH_SUCCESSRETRY, loginSucceeded);
                        //AUTH_FAILEDCREDENTIAL.off already because of events.once
                    ...
                    events.once(events.AUTH_SUCCESSRETRY, loginSucceeded);
                    events.once(events.AUTH_FAILEDCREDENTIAL, loginFailed);
                    events.fire(events.priv.AUTH_TRIGGERLOGIN);
                });
            } else {
                return Promise.reject(error);
            }
