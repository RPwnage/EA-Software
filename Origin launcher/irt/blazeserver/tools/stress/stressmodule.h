...
public:
    ...
    virtual bool initialize(const ConfigMap& config, const ConfigBootUtil* bootUtil = nullptr);
    ...
    int32_t getCrossplayOptInChance() const { return mCrossplayOptInChance; }
    ...
    void addTransactionTime(const TimeValue& ms);
    ...
    int64_t getTotalTestTimeInMillis() const {
        return TimeValue::getTimeOfDay().getMillis() - mTestStartTime/1000;
    }
    double getTotalTestTime() const {
        return (TimeValue::getTimeOfDay().getMicroSeconds() - mTestStartTime) * 1E-6;
    }
...
protected:
    ...
    int32_t mDisconnectChance; // percentage chance for instance to disconnect after login (may or may not have logged out first)
    int32_t mCrossplayOptInChance; // percentage chance for instance to opt in for crossplay
    ...
    uint32_t mInactivityTimeout_ms; // If the stress client has not received anything from the server for this length of time,
                                   // the stress client will disconnect.
