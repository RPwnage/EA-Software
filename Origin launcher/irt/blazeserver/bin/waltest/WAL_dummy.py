...
comp = unitTest.ComponentTest("arson",hostname,port,redirect,0,verbose);

## TEST 1 - plain int
cmd = unitTest.CommandTest("PlainInt","GET","/test/int/34");
cmd.addCondition("int",34);
comp.testCommand(cmd);
    
## TEST 2 - plain string
cmd = unitTest.CommandTest("PlainStr","GET","/test/str/hello");
cmd.addCondition("string","hello");
comp.testCommand(cmd);
    
## TEST 3 - now both together!
cmd = unitTest.CommandTest("IntStr","GET","/test/int/-50/str/hello");
cmd.addCondition("int",-50);
cmd.addCondition("string","hello");
comp.testCommand(cmd);
    
## TEST 4 - bunch of ints
cmd = unitTest.CommandTest("FourInts","GET","/test/int4/1/2/3/4");
...
## TEST 5 - list of ints
cmd = unitTest.CommandTest("IntList","GET","/test?f[0]=3&f[1]=6&f[2]=9&f[3]=12");
...
cmd = unitTest.CommandTest("StringList","GET","/test?h[0]=milk&h[1]=bread&h[2]=eggs&h[3]=coke");
...
## TEST 7 - int,int map
cmd = unitTest.CommandTest("IntIntMap","GET","/test?n(0)=12&n(6)=-56&n(-8)=100");
...
cmd = unitTest.CommandTest("StrStrMap","GET","/test?p(foo)=bar&p(grapes)=fruit&p(tomato)=vegetable");
...
## TEST 9 - struct
cmd = unitTest.CommandTest("Structs","GET","/test?c.fa=923");
cmd.addCondition("fooint",923);
comp.testCommand(cmd);

## TEST 10 - nested struct
cmd = unitTest.CommandTest("NestedStruct","GET","/test?b.ba=17&b.bb.fa=88");
cmd.addCondition("fooint",88);
cmd.addCondition("barint",17);
comp.testCommand(cmd);

## TEST 11 - list of structs
cmd = unitTest.CommandTest("ListOfStructs","GET","/test?d[0].ba=-34&d[0].bb.fa=25&d[1].ba=777&d[1].bb.fa=-4");
...
cmd = unitTest.CommandTest("IntStructMap","GET","/test?l(12).ba=99&l(12).bb.fa=-88&l(-2).ba=100&l(-2).bb.fa=535");
...
cmd = unitTest.CommandTest("StringStructMap","GET","/test?j(ketchup).ba=99&j(ketchup).bb.fa=-88&j(mustard).ba=100&j(mustard).bb.fa=535");
...
## TEST 14 - unions
cmd = unitTest.CommandTest("Unions","GET","/test?r.mInt=45&t.mString=hello&v.mFoo.fa=-378&x.mBar.ba=56&x.mBar.bb.fa=1002");
comp.testCommand(cmd);
