/*H********************************************************************************/
/*!
    \File dirtyauthxbox.cpp

    \Description
        Xbox Auth wrapper module
...
static int32_t _DirtyAuthCheckTokenGDK(DirtyAuthRefT *pDirtyAuth, DirtyAuthTokenT *pDirtyToken, int32_t *pTokenLen, char *pToken)
{
    ...
    {
        NetPrintf(("dirtyauthxbox: error %s getting token result\n", DirtyErrGetName(hResult)));
    ...
    {
        NetPrintf(("dirtyauthxbox: error %s getting token result size\n", DirtyErrGetName(hResult)));
        return(DIRTYAUTH_ERROR);
    }
    ...
    {
        NetPrintf(("dirtyauthxbox: unable to allocate memory for token result buffer\n"));
        return(DIRTYAUTH_ERROR);
    }
    ...
    {
        NetPrintf(("dirtyauthxbox: unable to allocate memory for token\n"));
        DirtyMemFree(pResultBuffer, DIRTYAUTH_MEMID, pDirtyAuth->iMemGroup, pDirtyAuth->pMemGroupUserData);
        return(DIRTYAUTH_ERROR);
    }
...
static int32_t _DirtyAuthCheckToken(DirtyAuthTokenT *pDirtyToken, int32_t *pTokenLen, char *pToken)
{
    ...
    else
    {
        ...
        {
            NetPrintf(("dirtyauthxbox: failed to acquire token for host '%s'\n", pDirtyToken->strHost));
...
static int32_t _DirtyAuthGetToken(DirtyAuthRefT *pDirtyAuth, const char *pHost, int32_t iUserIndex, DirtyAuthTokenT *pDirtyToken)
{
    ...
    {
        NetPrintf(("dirtyauthxbox: At least 1 user must be signed in to fetch XSTS token.\n"));
        return(DIRTYAUTH_ERROR);
    }
    ...
    {
        NetPrintf(("dirtyauthxbox: No user at index %d.\n", iUserIndex));
        return(DIRTYAUTH_ERROR);
    }

    NetPrintf(("dirtyauthxbox: getting token for host '%s' (user index = %d)\n", pHost, iUserIndex));
    try
    {
    ...
    {
        NetPrintf(("dirtyauthxbox: exception caught while trying to get token (%S/0x%08x)\n",
    ...
    pDirtyToken->asyncOperation->Completed = ref new AsyncOperationCompletedHandler<GetTokenAndSignatureResult ^>(
        ...
        if (status == Windows::Foundation::AsyncStatus::Completed)
        {
            ...
            {    
                ...
                pDirtyToken->bFailed = true;
                NetPrintf(("dirtyauthxbox: token acquisition for user %d, host %s failed - memory error\n", iUserIndex, pDirtyToken->strHost));
                return;
            }
        }
        else
        {
            pDirtyToken->bValid = false;
            pDirtyToken->bFailed = true;
            NetPrintf(("dirtyauthxbox: token acquisition for user %d, host %s failed - auth error: %S/0x%x\n", iUserIndex, pDirtyToken->strHost,
...
static int32_t _DirtyAuthGetTokenGDK(DirtyAuthRefT *pDirtyAuth, const char *pHost, int32_t iUserIndex, DirtyAuthTokenT *pDirtyToken)
{
    ...
    {
        NetPrintf(("dirtyauthxbox: error %s requesting token\n", DirtyErrGetName(hResult)));
        return(DIRTYAUTH_ERROR);
    }
...
DirtyAuthRefT *DirtyAuthCreate(uint8_t bSecureBypass)
{
    ...
    {
        NetPrintf(("dirtyauthxbox: dirtyauth already created\n"));
        return(NULL);
    }
    ...
    {
        NetPrintf(("dirtyauthxbox: could not allocate module state\n"));
        return(NULL);
    }
...
void DirtyAuthDestroy(DirtyAuthRefT *pDirtyAuth)
{
    ...
    if (_DirtyAuth_pRef == NULL)
    {
        NetPrintf(("dirtyauthxbox: dirtyauth not created and cannot be destroyed\n"));
        return;
    }
    ...
    for (pDirtyToken = pDirtyAuth->pTokenList; pDirtyToken != NULL; )
    {
        ...
        if (_DirtyAuthCheckToken(pTokenToFree, NULL, NULL) == DIRTYAUTH_PENDING)
        {
            ...
            NetPrintf(("dirtyauthxbox: [%08x] token free on module shutdown deferred to netconn due to pending token acquisition\n", (uintptr_t)pDirtyToken));
...
int32_t DirtyAuthGetToken2(int32_t iUserIndex, const char *pHost, uint8_t bForceNew, uint32_t uTimeout)
{
    ...
    if (pDirtyAuth == NULL)
    {
        NetPrintf(("dirtyauthxbox: attempt to use dirtyauth when the module is not started\n"));
    ...
    if ((pDirtyToken = _DirtyAuthFindToken(pDirtyAuth, pHost, iUserIndex)) != NULL)
    {
        ...
        if (((iResult == DIRTYAUTH_SUCCESS) && (bForceNew == FALSE)) || (iResult == DIRTYAUTH_PENDING))
        {
            ...
            NetPrintf(("dirtyauthxbox: reusing token for host '%s' (user index = %d, result = %d)\n", pHost, iUserIndex, iResult));
            return(iResult);
        }

        NetPrintf(("dirtyauthxbox: forcing refresh of token for host '%s' (user index = %d, result = %d)\n", pHost, iUserIndex));
        // remove token from token list
