...
{
    uint8_t bTranscriptionEnabled = (pHeadset->bVoiceTranscriptionEnabled && !pHeadset->bTextChatAccessibility && !pHeadset->bTranslated);
    int32_t iCompBytes;
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'txta' - enable/disable text chat accessibility
...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'txta')
    {
        pHeadset->bTextChatAccessibility = iValue ? TRUE : FALSE;
    ...
    if (iControl == 'ttos')
    {
        ...
        if (pHeadset->pNarrateRef != NULL)
        {
            if (!pHeadset->bChatRestricted || pHeadset->bTextChatAccessibility)
            {
                ...
                iResult = VoipNarrateInput(pHeadset->pNarrateRef, iValue, eGender, pHeadset->bTextChatAccessibility ? VOIPNARRATE_CBFLAG_LOCAL : VOIPNARRATE_CBFLAG_NONE, (const char *)pValue);
            }
        }
        return(iResult);
    }
