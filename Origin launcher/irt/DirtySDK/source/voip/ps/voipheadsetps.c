...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'txta')
    {
        ...
        NetPrintf(("voipheadsetps: text chat accessibility mode %s\n", (iValue ? "enabled" : "disabled")));
        return(0);
    }
    if (iControl == 'tran')
    {
    ...
    if ((iControl == 'ttos') && (iValue < VOIP_MAXLOCALUSERS))
    {
        ...
        if (pLocalUser->pNarrateRef != NULL)
        {
            return(VoipNarrateInput(pLocalUser->pNarrateRef, iValue, pLocalUser->uGender, VOIPNARRATE_CBFLAG_NONE, (const char *)pValue));
        }
        else
        {
