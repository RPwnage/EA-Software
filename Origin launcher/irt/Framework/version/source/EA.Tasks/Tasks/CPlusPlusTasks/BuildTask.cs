...
namespace EA.CPlusPlusTasks
{
	...
	public class BuildTask : Task 
	{
		...
		// special build types to maintain backwards compatibility with Framework version < 0.7.0
		...
		public FileSet ForceUsingAssemblies
		{
			get { return _complierTask.ForceUsingAssemblies; }
		}
		...
		public FileSet Dependencies { get; } = new FileSet();

		/// <summary>primary output extension</summary>
		[TaskAttribute("outputextension", Required = false)]
		public string PrimaryOutputExtension { get; set; } = String.Empty;

		/// <summary>primary output prefix</summary>
		[TaskAttribute("outputprefix", Required = false)]
		public string PrimaryOutputPrefix { get; set; } = String.Empty;
		...
		protected override void ExecuteTask() {
			...
			_complierTask.CollectCompilationTime = CollectCompilationTime;
			_assemblerTask.CollectCompilationTime = CollectCompilationTime;

			_complierTask.GenerateDependencyOnly = GenerateDependencyOnly;
			_assemblerTask.GenerateDependencyOnly = GenerateDependencyOnly;
		...
		void ExecuteLinkTask(OptionSet optionSet) {
			...
			BuildExecuted = _linkTask.BuildExecuted;
		}
		...
		void ExecuteLibTask(OptionSet optionSet) {
			...
			_libTask.LibraryPrefix = PrimaryOutputPrefix;
			...
			BuildExecuted = _libTask.BuildExecuted;
		}
		...
		void ExecuteCustomTask(string taskName, OptionSet optionSet) 
		{
			...
			{
				string msg = String.Format("Custom build tasks must inherit from: {0}", typeof(IExternalBuildTask).ToString());
				throw new BuildException(msg, Location);
			}
		}
	}
