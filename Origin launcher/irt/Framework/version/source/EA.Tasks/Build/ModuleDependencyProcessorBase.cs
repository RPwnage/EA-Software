			...
			foreach (KeyValuePair<string, uint> propertySuffixToType in Mappings.DependencyPropertyMapping)
			{
				foreach (string dependentPackage in GetModulePropertyValue(propertySuffixToType.Key).ToArray())
				{
					...
					if (Project.UseNugetResolution && PackageMap.Instance.MasterConfigHasNugetPackage(dependentPackage))
					{
						...
						FileSet additionalNugetReferences = module.PropGroupFileSet("additional-nuget-references") ?? new FileSet();
						additionalNugetReferences.Include(dependentPackage, failonmissing: false, asIs: true);
						module.Package.Project.NamedFileSets[module.PropGroupName("additional-nuget-references")] = additionalNugetReferences;
						continue;
					}
