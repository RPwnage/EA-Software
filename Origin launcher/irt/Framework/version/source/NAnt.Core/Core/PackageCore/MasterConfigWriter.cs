	...
	public class MasterConfigWriter
	{
		...
		private void WriteConfig()
		{
			...
			m_Writer.WriteStartElement("config");
			{
				...
				if (m_Masterconfig.Config != MasterConfig.ConfigInfo.DefaultConfigInfo)
				{
					if (!String.IsNullOrEmpty(m_Masterconfig.Config.Default))
					{
						m_Writer.WriteAttributeString("default", m_Masterconfig.Config.Default);
					}
					if (!m_Masterconfig.Config.Includes.IsNullOrEmpty())
					{
						m_Writer.WriteAttributeString("includes", m_Masterconfig.Config.Includes.ToString(" "));
					}
					if (!m_Masterconfig.Config.Excludes.IsNullOrEmpty())
					{
						m_Writer.WriteAttributeString("excludes", m_Masterconfig.Config.Excludes.ToString(" "));
					}
