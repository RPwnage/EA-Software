...
namespace NAnt.Core.Tasks
{
	...
	/// </remarks>
	/// <include file='Examples/Warn/Simple.example' path='example'/>
	[TaskName("warn", NestedElementsCheck = true)]
	public class WarnTask : Task
	{
		...
		/// </summary>
		[TaskAttribute("group")]
		public Log.WarningId Group { get; set; } = Log.WarningId.NoId;

		protected override void ExecuteTask()
		{
			string message = Message ?? "No message.";
			if (Group == Log.WarningId.NoId)
			{
				Log.Warning.WriteLine("{0} {1}", Location.ToString(), message);
			}
			else
			{
				Log.ThrowWarning(Group, Log.WarnLevel.Normal, "{0} {1}", Location.ToString(), message);
			}
		}
	}
}
