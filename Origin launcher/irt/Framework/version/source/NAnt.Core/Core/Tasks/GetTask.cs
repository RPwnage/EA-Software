...
namespace NAnt.Core.Tasks
{
	...
	/// </remarks>
	/// <include file='Examples/Get/Get.example' path='example'/>
	[TaskName("get")]
	public class GetTask : Task {
		...
		[FileSet("fileset")]
		public FileSet FileSet { get; } = new FileSet();
		...
		protected override void InitializeTask(System.Xml.XmlNode taskNode) {
			...
			Destination = Project.GetFullPath(Destination);
		}
		...
		protected override void ExecuteTask() {
			try {
				...
				if (useTimeStamp && File.Exists(Destination)) {
					...
					Log.Debug.WriteLine(LogPrefix + "Local file time stamp: " + fileTimeStamp.ToString());
				}
				...
				if (useTimeStamp && webRequest is HttpWebRequest) {
					HttpWebResponse httpResponse = (HttpWebResponse) webResponse;
					if (httpResponse.LastModified <= fileTimeStamp) {
						return;
					}
				}
				...
				for (int i = 0; i < 3; i++) {
					try {
						...
						Log.Status.WriteLine(LogPrefix + "Error opening connection: " + ex.Message);
					}
				}

				if (responseStream == null) {
					Log.Status.WriteLine(LogPrefix + "Cannot get " + Source + " to " + Destination);
					if (ignoreErrors) {
						return;
					}
				...
				Log.Status.WriteLine(LogPrefix + Source);
				BinaryWriter destWriter = new BinaryWriter(new FileStream(Destination, FileMode.Create));

				// Read in stream from URL and write data in chunks to the destination file.
				...
				do {
					...
					if ( totalReadCount != 0 ) { // zero means EOF
						...
						// display progress
						if (totalBytesReadSinceLastDot > bufferSize) {
							if (totalBytesReadSinceLastDot == totalBytesReadFromStream) {
								Log.Info.Write(LogPrefix);
							}
							Log.Info.Write( ".");
				...
				Log.Info.WriteLine(LogPrefix + "Number of bytes read: " + totalBytesReadFromStream.ToString());
				...
				if (useTimeStamp)  {
					...
					if (webRequest is HttpWebRequest) {
						...
						Log.Info.WriteLine(LogPrefix + Destination + " last modified on " + remoteTimestamp.ToString());
				...
				if (webException.Status == WebExceptionStatus.ProtocolError) {
					...
					if (httpResponse.StatusCode == HttpStatusCode.NotModified) {
						...
						Log.Info.WriteLine(LogPrefix + Destination + " not downloaded.  Not modified since " + httpResponse.LastModified.ToString());
						return;
					} else {
						Log.Status.WriteLine(LogPrefix + (int)httpResponse.StatusCode + ": " + httpResponse.StatusDescription);
						return;
					}
				} else {
					Log.Status.WriteLine(LogPrefix + webException.Status + ": " + webException.Message);
					return;
				}
			} catch (IOException e) {
				string msg = "Error getting " + Source + " to " + Destination;
				Log.Error.WriteLine(LogPrefix + msg);
		...
		private WebRequest GetWebRequest(string url, DateTime fileLastModified) {
			...
			if (uri.Scheme == Uri.UriSchemeHttp || uri.Scheme == Uri.UriSchemeHttps) {
				...
				// modify the headers
				// NOTE: things like user authentication could go in here too
				if (!fileLastModified.Equals(new DateTime())) {
					...
					TimeSpan timeSpan = TimeZoneInfo.Local.GetUtcOffset(DateTime.Now);
					DateTime gmtTime = fileLastModified.AddSeconds(1).Subtract(timeSpan);
					httpRequest.IfModifiedSince = gmtTime;

					// NOTE: at this point even non HTTP connections may support the if-modified-since
					// behaviour -we just check the date of the content and skip the write if it is not
					// newer. Some protocols (FTP) don't include dates, of course.
		...
		protected void TouchFile(string fileName, DateTime touchDateTime) {
			...
			try {
				if (File.Exists(fileName)) {
					Log.Info.WriteLine(LogPrefix + "Touching file {0} with {1}", fileName, touchDateTime.ToString());
				...
				Log.Info.WriteLine(LogPrefix + "Error: {0}", e.Message);
			}
		}
	}
}
