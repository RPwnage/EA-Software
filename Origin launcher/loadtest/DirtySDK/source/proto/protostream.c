...
struct ProtoStreamRefT
{
    ...
    uint8_t bKeepAlive;                 //!< keep the connection alive between requests
...
static int32_t _ProtoStreamOpen(ProtoStreamRefT *pProtoStream, const char *pUrl, const char *pReq, int32_t iFreq)
{
    ...
    // if keep-alive and we're using a post request, set ProtoHttp flag to allow reuse on put/post
    if (pProtoStream->bKeepAlive && (pReq != NULL))
    {
        ProtoHttpControl(pProtoStream->pProtoHttp, 'rput', 1, 0, NULL);
    }
...
/*!
    ...
    \Notes
        ...
        \verbatim
            'keep' - set keepalive for the http connection
...
int32_t ProtoStreamControl(ProtoStreamRefT *pProtoStream, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'keep')
    {
        pProtoStream->bKeepAlive = iValue ? TRUE : FALSE;
        // let it fall through to protohttp
    }
