...
static int32_t _SocketCreateSocket(int32_t iAddrFamily, int32_t iType, int32_t iProto)
{
    ...
    if ((iSocket = socket(iAddrFamily, iType, iProto)) >= 0)
    {
        ...
        if (iType == SOCK_DGRAM)
        {
            ...
            // set socket option for early timestamping of inbound udp traffic by kernel
            if (setsockopt(iSocket, SOL_SOCKET, SO_TIMESTAMP, (char*)&uTrue, sizeof(uTrue)) < 0)
            {
                NetPrintf(("dirtynetunix: error trying to enable SO_TIMESTAMP on DGRAM socket (err=%d)\n", DirtyErrGetName(errno)));
            }
        }
...
static void _SocketLookupThread(void *_pRef)
{
    ...
    if ((iResult = getaddrinfo(pHost->strName, NULL, &Hints, &pList)) == 0)
    {
    ...
    {
        // unsuccessful
        NetPrintf(("dirtynetunix: getaddrinfo('%s', ...) failed err=%s\n", pHost->strName, DirtyErrGetNameList(iResult, _GAI_ErrList)));
        pHost->done = -1;
    }
...
/*!
    ...
    \Version 09/14/2020 (jbrookes)
*/
/********************************************************************************F*/
static int32_t _SocketRecvfrom(SocketT* pSocket, char* pBuf, int32_t iLen, struct sockaddr* pFrom, int32_t* pFromLen)
{
    int32_t iResult;
    struct msghdr RecvMsgParams;        // input structure to feed recvmsg() with
    struct iovec IoVec;                 // scatter-gather buffers to save inbound packets in  (currently 1-packet long)
    uint8_t uAncillaryData[512];        // buffer used by the kernel to store ancillary data associated with inbound traffic
    struct sockaddr_in6 SockAddr6;      // buffer to be filled in with originator's IP address and UDP port 
    ...
    // setup input params for recvmsg()
    ds_memclr(&RecvMsgParams, sizeof(RecvMsgParams));
    IoVec.iov_base = pBuf;
    IoVec.iov_len = iLen;
    RecvMsgParams.msg_iov = &IoVec;
    RecvMsgParams.msg_iovlen = 1;
    SockaddrInit6(&SockAddr6, AF_INET6);
    RecvMsgParams.msg_name = (caddr_t)&SockAddr6;
    RecvMsgParams.msg_namelen = sizeof(SockAddr6);
    RecvMsgParams.msg_control = &uAncillaryData;
    RecvMsgParams.msg_controllen = sizeof(uAncillaryData);

    if ((iResult = (int32_t)recvmsg(pSocket->uSocket, &RecvMsgParams, MSG_DONTWAIT)) > 0)
    {
        if (pFrom != NULL)
        {
            struct cmsghdr *pControlMsgHdr;
            uint32_t bKernelTimestampFound = FALSE;
            uint32_t uKernelRecvTick;

            // fill up caller's output vars
            *pFromLen = sizeof(SockAddr6);
            SocketAddrMapTranslate(&_Socket_pState->AddrMap, pFrom, (struct sockaddr*)&SockAddr6, pFromLen);

            // inspect ancillary data for kernel-applied receive timestamp
            for (pControlMsgHdr = CMSG_FIRSTHDR(&RecvMsgParams); pControlMsgHdr != NULL; pControlMsgHdr = CMSG_NXTHDR(&RecvMsgParams, pControlMsgHdr))
            {
                if ((pControlMsgHdr->cmsg_level == SOL_SOCKET) && (pControlMsgHdr->cmsg_type == SO_TIMESTAMP))
                {

                    struct timeval* pTimestamp = (struct timeval*)CMSG_DATA(pControlMsgHdr);
                    uKernelRecvTick = (uint32_t)(((uint64_t)pTimestamp->tv_sec * 1000) + (uint64_t)pTimestamp->tv_usec / 1000);
                    bKernelTimestampFound = TRUE;
                    break;
                }
            }

            // save reception timestamp into originator's sockaddr (needs to be done after SocketAddrMapTranslate())
            if (bKernelTimestampFound == TRUE)
            {
                // append kernel-applied timestamp to originator's sockaddr - tick at which kernel first saw the packet
                SockaddrInSetMisc(pFrom, uKernelRecvTick);
            }
            else
            {
                // append current timestamp to originator's sockaddr - tick at which dirtynet first saw the packet
                SockaddrInSetMisc(pFrom, NetTick());

                NetPrintf(("dirtynetunix: warning - failed to retrieve kernel-applied timestamp on inbound packet\n"));
            }

            #if DIRTYCODE_LOGGING
            int32_t iTickDiff = NetTickDiff(NetTick(), SockaddrInGetMisc(pFrom));
            if (iTickDiff > 5)
            {
                NetPrintfVerbose((pSocket->iVerbose, 1, "dirtynetunix: warning - kernel is inducing non negligible latency (> 5 ms) in reception path (%d ms)\n", iTickDiff));
            }
            #endif
        }
    }
