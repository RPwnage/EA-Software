...
bool GameManagerUtil::initialize(const ConfigMap& config, const ConfigBootUtil* bootUtil)
{
    ...
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmMaxGames = " << mMaxGames);

    mMaxVirtualCreators = config.getUInt32("gmMaxVirtualCreators", (uint32_t)(0));
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmMaxVirtualCreators = " << mMaxVirtualCreators);

    mGameLifespan = config.getUInt32("gmGameLifespan", (uint32_t)(-1));
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmGameLifespan = " << mGameLifespan);

    mGameLifespanDeviation = config.getUInt32("gmGameLifespanDeviation", 0);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmGameLifespanDeviation = " << mGameLifespanDeviation);

    mGameStartTimer = config.getUInt32("gmGameStartTimer", 0);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmGameStartTimer = " << mGameStartTimer);

    mGamePlayerLifespan = config.getUInt32("gmGamePlayerLifespan", (uint32_t)(-1));
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmGamePlayerLifespan = " << mGamePlayerLifespan);

    mGamePlayerLifespanDeviation = config.getUInt32("gmGamePlayerLifespanDeviation", 0);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmGamePlayerLifespanDeviation = " << mGamePlayerLifespanDeviation);

    mGamePlayerLowerLimit = config.getUInt16("gmGamePlayerLowerLimit", 2);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmGamePlayerLowerLimit = " << mGamePlayerLowerLimit);

    mGamePlayerSeed = config.getUInt16("gmGamePlayerSeed", 8);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmGamePlayerSeed = " << mGamePlayerSeed);

    mGamePlayerMinRequired = config.getUInt16("gmGamePlayerMin", mGamePlayerLowerLimit);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmGamePlayerMin = " << mGamePlayerMinRequired);

    bool netParse = ParseGameNetworkTopology(config.getString("gmNetworkTopology", "PEER_TO_PEER_FULL_MESH"), mGameTopology);
    if (netParse == false)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : Unrecognizable game network topology " 
                       << config.getString("gmNetworkTopology", "PEER_TO_PEER_FULL_MESH"));
    }
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmNetworkTopology =" << GameNetworkTopologyToString(mGameTopology));

    // 0 is off
    mGameSettingsRandomUpdates = (float)config.getDouble("gmGameSettingsRandomUpdates", 0.0);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmGameSettingsRandomUpdates = " << mGameSettingsRandomUpdates);
    // 0 is off
    mGameNameRandomUpdates = (float)config.getDouble("gmGameNameRandomUpdates", 0.0);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmGameNameRandomUpdates = " << mGameNameRandomUpdates);
    // 0 is off
    mGameExternalStatusRandomUpdates = (float)config.getDouble("gmGameExternalStatusRandomUpdates", 0.0);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmGameExternalStatusRandomUpdates = " << mGameExternalStatusRandomUpdates);

    bool resetPlayerAdmin = config.getBool("gmResetPlayerAdmin", false);
    if (resetPlayerAdmin)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmResetPlayerAdmin FEATURE NOT IMPLEMENTED!");
    }

    mJoinInProgress = config.getBool("gmJoinInProgress", false);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmJoinInProgress = " << (mJoinInProgress ? "true" : "false"));

    mShooterIntelligentDedicatedServer = config.getBool("gmShooterIntelligentDedicatedServer", false);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmShooterIntelligentDedicatedServer = " << (mShooterIntelligentDedicatedServer ? "true" : "false"));

    mCensusSubscribe = config.getBool("gmCensusSubscribe", true);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : gmCensusSubscribe = " << (mCensusSubscribe ? "true" : "false"));

    mUseArson = config.getBool("rspUseArson", true);
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : rspUseArson = " << (mUseArson ? "true" : "false"));
    ...
    if (bootUtil != nullptr)
    {
        ...
        if (itr != bootUtil->getConfigFile()->getDefineMap().end())
        {
            ...
            BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : mockExternalSessionPlatform defined " << mMockExternalSessions.c_str());
        }
    }
...
void GameManagerUtil::dumpStats(StressModule *module)
{
    ...
    BLAZE_INFO_LOG(Log::CONTROLLER, "GMUtil: " << statbuffer << " @ " << metricOutput);
...
void GameManagerUtil::setInstanceCreateGame(StressInstance *inst, CreateGameRequest& req)
{
    ...
    BLAZE_SPAM_LOG(BlazeRpcLog::gamemanager, "[GameManagerUtil] setting up CreateGameRequest GameName: " << req.getGameCreationData().getGameName());
...
GameData* GameManagerUtil::registerGame(ReplicatedGameData &replGameData, ReplicatedGamePlayerList &replPlayerData)
{
    ...
    else
    {
        ...
        {
            BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil: updating game report id (" << gameData->getGameReportingId() << " to " << replGameData.getGameReportingId() << ") for game (" << gameData->getGameId() << ")");
...
void GameManagerUtil::advanceGameState(BlazeRpcError *err, GameManagerSlave *proxy, GameId gameId, GameState newState)
{
    ...
    *err = proxy->advanceGameState(req);
    if (*err != ERR_OK)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : advance game state for game(" << gameId << ") failed. Error(" 
...
void GameManagerUtil::setGameSettings(BlazeRpcError *err, GameManagerSlave *proxy, GameId gameId, GameSettings *newSettings)
{
    ...
    *err = proxy->setGameSettings(req);
    if (*err != ERR_OK)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : set game settings for game(" << gameId << ") failed. Error(" 
                       << (ErrorHelp::getErrorName(*err)) << ")");
    }
}
#ifdef TARGET_arson
void GameManagerUtil::setGameSettingsArson(BlazeRpcError *err, Arson::ArsonSlave *proxy, GameId gameId, GameSettings *newSettings)
{
    ...
    *err = proxy->setGameSettings(req);
    if (*err != ERR_OK)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : set game settings via Arson for game(" << gameId << ") failed. Error(" 
            << (ErrorHelp::getErrorName(*err)) << ")");
    }
}
#endif
void GameManagerUtil::udpateGameName(BlazeRpcError *err, GameManagerSlave *proxy, GameId gameId, const char8_t *newName)
{
    ...
    *err = proxy->updateGameName(req);
    if (*err != ERR_OK)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : set game name(" << newName << ") for game(" << gameId << ") failed. Error(" 
            << (ErrorHelp::getErrorName(*err)) << ")");
    }
}
...
GameId GameManagerUtil::joinGame(BlazeRpcError *err, GameManagerSlave *proxy, JoinGameResponse *resp, StressInstance *instance, bool joinByUser, const JoinGameRequest *reqOverride)
{
    ...
    else if (mResetableServers > 0)
    {
        ...
        *err = proxy->resetDedicatedServer(req, *resp);
        ...
        {
            BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : failed to reset game (error = " << *err << ")");
        }
    }
...
GameId GameManagerUtil::joinGame(BlazeRpcError *err, GameManagerSlave *proxy, GameId gameId, JoinGameResponse *resp, bool joinByUser, const JoinGameRequest *reqOverride)
{
    ...
    *err = proxy->joinGame(req, *resp, entrycrit);
    ...
    if (*err == GAMEMANAGER_ERR_INVALID_GAME_ID)
    {
        ...
        {
            BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : tried to join invalid game ID " << gameId << " - local store of games may be out of sync.");
        ...
        {
            BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : invalid game ID " << gameId << " to join - game was likely killed during the join send.");
        }
    }
    else if (*err == ERR_OK)
    {
...
void GameManagerUtil::removePlayer(BlazeRpcError *err, GameManagerSlave *proxy, GameId gameId, PlayerId playerId, PlayerRemovedReason reason)
{
    ...
    *err = proxy->removePlayer(req);
    ...
    if (*err != ERR_OK)
    {
        BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : removing player(" << playerId << ") from game(" << gameId 
...
void GameManagerUtil::startMatchmakingScenario(BlazeRpcError *err, GameManagerSlave *proxy, StartMatchmakingScenarioRequest *request, StartMatchmakingScenarioResponse *response, MatchmakingCriteriaError *error)
{
    *err = proxy->startMatchmakingScenario(*request, *response, *error);
    ...
    if (*err != ERR_OK)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : start matchmaking failed. Error(" << (ErrorHelp::getErrorName(*err)) 
                       << ") " << error->getErrMessage());
    }
}
...
void GameManagerUtil::cancelMatchmakingScenario(BlazeRpcError *err, GameManagerSlave *proxy, MatchmakingScenarioId scenarioId)
{
    ...
    *err = proxy->cancelMatchmakingScenario(req);
    ...
    if (*err != ERR_OK)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : cancel matchmaking failed. Error(" << (ErrorHelp::getErrorName(*err)) << ")");
    }
}
        ...
        */
            ...
            gamedata->isPlatformHost(getStressInstance()->getLogin()->getUserLoginInfo()->getBlazeId())) )
        {
            if (mSetGameSettingsRequest != nullptr)
            {
...
#else
                    BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: setGameSettingsArson cannot be used when arson is not included.");
#endif
                }
                else
                {
                ...
                if (err != ERR_OK)
                {
                    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: setGameSettings failed.");
                    return err;
                }
            }
    ...
    case GameManagerUtil::ROLE_HOST:
        {
            if (mGameId == 0)
            {
                ...
                if (mUtil->queryCreateNewGame())
                {
                    ...
                    if (customReq == nullptr)
                    {
                        ...
                        {
                            BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() 
                                << "]: created game of " << req->getGameCreationData().getMaxPlayerCapacity() 
                    ...
                    if (error != ERR_OK)
                    {
                        BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: createGame failed.");
            ...
            else
            {
                ...
                {
                    BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Game host is waiting for game info.");
                }
                else
                {
                    ...
                    else if (mCyclesLeft > 0)  
                    {
                        ...
                        if (game->getGameState() == MIGRATING)
                        {
                            ...
                            BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Game host is migrating.");
                        }
                            ...
                            else if ((game->getGameState() == POST_GAME && mPostGameCountdown == 0) || game->getGameState() != POST_GAME)
                            {
                                if (game->getTopology() == CLIENT_SERVER_DEDICATED)
                                {
                                    ...
                                    if (game->getGameSettings()->getVirtualized())
                                    {
                                        ...
                                        if (error != ERR_OK)
                                        {
                                            BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: ejectHost failed for id=" << mGameId << ".");
                                            mGameId = 0;
                                            return error;
                                        }
                                        else 
                                ...
                                else
                                {
                                    ...
                                    if (error != ERR_OK)
                                    {
                                        BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: destroyGame failed for id=" << mGameId << ".");
                                        mGameId = 0;
                                        return error;
                                    }
                                    else 
    ...
    case GameManagerUtil::ROLE_JOINER:
        {
            if(mGameId == 0)
            {
                ...
                {                    
                    ...
                    {
                        BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: join game failed (id=" << gameId << ")");
                        return err;
                    }
                    mGameId = resp.getGameId();
            ...
            else if (mCyclesLeft <= 0)
            {
                ...
                if (mGameId != 0)
                {
                    ...
                    if (err != ERR_OK)
                    {
                        BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: removePlayer failed (gameid=" << myGameId << ")");
                        return err;
                    }
                }
                else
                {                    
                    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: game id cleared during removePlayer");
                }
            }
        }
        break;
...
#else
            BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "] virtualized games are not supported without Arson");
#endif
           }
        break;
...
void GameManagerUtilInstance::setNotificationCb(GameManagerSlave::NotificationType type, const AsyncNotifyCb& cb)
{
    ...
    {
        BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "] : unable to set notification callback on uninitialized instance.");
        return;
    }
...
void GameManagerUtilInstance::onAsync(uint16_t component, uint16_t iType, RawBuffer* payload)
{
    ...
    case GameManagerSlave::NOTIFY_NOTIFYJOININGPLAYERINITIATECONNECTIONS:
        {
            ...
            if (decoder.decode(*payload, notify) != ERR_OK)
            {
                BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Failed to decode notification(" << type << ")");
            }
            else
            {
                ...
                {
                    BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: receiving notification(" << type << ") for game (" 
                ...
                if (roster.size() > 0)
                {
                    ...
                    {
                        UpdateMeshConnectionRequest meshRequest;
                        for (ReplicatedGamePlayerList::const_iterator cit = roster.begin(), citEnd = roster.end(); cit != citEnd; ++cit)
                        {
    ...
    case GameManagerSlave::NOTIFY_NOTIFYPLAYERCLAIMINGRESERVATION:
        {
            ...
            else 
            {
                ...
                if (data != nullptr)
                {
                    ...
                    else
                    {
                        if (!data->isHost(getBlazeId()))
                        {
                            ...
                            BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Instance user " << getBlazeId() << " is neither a player nor the expected topology host (" << data->getHostId() << ") in game " << 
                ...
                {
                    BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring PlayerJoining notification for unregistered GameId " << notify.getGameId());
                }
    ...
    case GameManagerSlave::NOTIFY_NOTIFYPLAYERJOINCOMPLETED:
        {
            ...
            else
            {
                ...
                if (data != nullptr)
                {
                    ...
                    if (player == nullptr)
                    {
                        BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Player (" << notify.getPlayerId() 
                ...
                {
                    BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring PlayerJoinCompleted notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    case GameManagerSlave::NOTIFY_NOTIFYPLAYERREMOVED:
        {
            ...
            else 
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring PlayerRemoved notification for unregistered GameId " << notify.getGameId());
                }
    ...
    case GameManagerSlave::NOTIFY_NOTIFYGAMESTATECHANGE:
        {
            ...
            else 
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring GameStateChange notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    case GameManagerSlave::NOTIFY_NOTIFYGAMESETTINGSCHANGE:
        {
            ...
            else 
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring GameSettingsChange notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    ...
    case GameManagerSlave::NOTIFY_NOTIFYHOSTMIGRATIONSTART:
        {
            ...
            else
            {
                    ...
                    if (notify.getNewHostId() == getBlazeId())
                    {
                        ...
                        if (notify.getMigrationType() == TOPOLOGY_HOST_MIGRATION || notify.getMigrationType() == TOPOLOGY_PLATFORM_HOST_MIGRATION)
                        {
                            ...
                            if (newHostPlayerData != nullptr)
                            {
                                ...
                                BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Failed to attempt connection post migration for game(" << notify.getGameId() << ") for new host (" << notify.getNewHostId() << ")");
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring HostMigrationStart notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    case GameManagerSlave::NOTIFY_NOTIFYHOSTMIGRATIONFINISHED:
        {
            ...
            else
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring HostMigrationFinished notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    ...
    case GameManagerSlave::NOTIFY_NOTIFYGAMEREMOVED:
        {
            ...
            else
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring GameRemoved notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    case GameManagerSlave::NOTIFY_NOTIFYPLATFORMHOSTINITIALIZED:
        {
            ...
            else
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring PlatformHostInitialized notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    case GameManagerSlave::NOTIFY_NOTIFYGAMEATTRIBCHANGE:
        {
            ...
            else
            {
                ...
                if (data != nullptr) // && data->getHostId() == getBlazeId())
                {
                    ...
                    {
                        Collections::AttributeMap::iterator gameAttrIt = data->getAttributeMap()->find(cit->first);
                        if (gameAttrIt != data->getAttributeMap()->end())
                        {
                            //  modify this value
                        ...
                        {
                            //  add this value.
                            eastl::pair<Collections::AttributeMap::iterator, bool> inserter = data->getAttributeMap()->insert(cit->first);
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring GameAttribChange notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
            ...
            else
            {
                ...
                if (data != nullptr) // && data->getHostId() == getBlazeId())
                {
                    ...
                    if (player != nullptr)
                    {
                        ...
                        {
                            Collections::AttributeMap::iterator playerAttrIt = player->getAttributeMap()->find(cit->first);
                            if (playerAttrIt != player->getAttributeMap()->end())
                            {
                                //  modify this value
                            ...
                            {
                                //  add this value.
                                eastl::pair<Collections::AttributeMap::iterator, bool> inserter = player->getAttributeMap()->insert(cit->first);
                                inserter.first->second = cit->second;
                            }
                        }
                    }
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring PlayerAttribChange notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    case GameManagerSlave::NOTIFY_NOTIFYPLAYERCUSTOMDATACHANGE:
        {
            ...
            else
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring PlayerCustomDataChange notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    case GameManagerSlave::NOTIFY_NOTIFYGAMECAPACITYCHANGE:
        {
            ...
            else
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring GameCapacityChange notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    case GameManagerSlave::NOTIFY_NOTIFYGAMEREPORTINGIDCHANGE:
        {
            ...
            else
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring GameReportingIdChange notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    case GameManagerSlave::NOTIFY_NOTIFYGAMESESSIONUPDATED:
        {
            ...
            else
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring GameSessionUpdated notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    case GameManagerSlave::NOTIFY_NOTIFYGAMEPLAYERSTATECHANGE:
        {
            ...
            else
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring GamePlayerStateChange notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    case GameManagerSlave::NOTIFY_NOTIFYADMINLISTCHANGE:
        {
            ...
            else
            {
                ...
                {
                     BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Ignoring AdminListChange notification for unregistered GameId " << notify.getGameId());
                }
            }
        }
        break;
    ...
    default:
        BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Unsupported notification(" << type << ") - addition to GameManagerUtilInstance required.");
        break;
    }
}
...
void GameManagerUtilInstance::advanceGameState(GameId gameId, GameState state)
{
    if (!isWaitingForGameStateNotification())
    {
        ...
        if(err != ERR_OK)
        {
            ...
            BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: error " << (ErrorHelp::getErrorName(err)) << "(" << err 
...
void GameManagerUtilInstance::updateGameSession(GameId gameId, bool migration, HostMigrationType migrationType)
{
    ...
    if(err != ERR_OK)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, 
            "GameManagerUtilInstance[" << getIdent() << "]: error " << err << " while updating game session game id " << gameId << ".");
    }
    else if (migration)
    {
        ...
        {
            BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, 
                "GameManagerUtilInstance[" << getIdent() << "]: error " << err << " while updating host migration status for game id " << gameId << ".");
        }
    }
...
void GameManagerUtilInstance::updateAllPlayerConnsToHost(GameId gameId, ConnectionGroupId hostConnId, BlazeId hostId)
{
    ...
    if (data == nullptr)
    {
        BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Game Data null. Failed to attempt connection post migration for game(" << gameId << ") for host(" << hostId << ") connid(" << hostConnId << ")");
        return;
    }
    PlayerData* hostPlayerData = data->getPlayerData(hostId);
    if (hostPlayerData == nullptr) 
    {
        BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Host Player Data null. Failed to attempt connection post migration for game(" << gameId << ") for host(" << hostId << ") connid(" << hostConnId << ")");
        return;
    }
...
void GameManagerUtilInstance::updatePlayerConns(GameId gameId, ConnectionGroupId sourceConnId, ConnectionGroupId targetConnId)
{
    ...
    if (err != ERR_OK)
    {
        BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: Failed to attempt connection post migration for game(" << gameId << ") from source player connid(" << sourceConnId << ") to target connid(" << targetConnId << ")");
    }
}
...
void GameManagerUtilInstance::updatePlayerConns(GameId gameId, UpdateMeshConnectionRequest req)
{
    ...
    {
        /*BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, 
            "GameManagerUtilInstance[" << getIdent() << "]: error " << err << " updating player connection to player " << connStatusList[i]->getTargetPlayer() 
             << " for game id " << mGameId << ".");*/   
    }
}
...
void GameManagerUtilInstance::finalizeGameCreation(GameId gameId, bool platformHost, bool topologyHost)
{
    ...
    if (err != ERR_OK)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: error " << err << " calling finalizeGameCreation (platformHost=" 
...
void GameManagerUtilInstance::updateHostConnection(GameId gameId, ConnectionGroupId sourceGroupId, ConnectionGroupId topologyGroupId, PlayerNetConnectionStatus connStatus)
{
    ...
    if (err != ERR_OK)
    {
        BLAZE_ERR_LOG(BlazeRpcLog::gamemanager,  "GameManagerUtilInstance[" << getIdent() << "]: error " << err << " calling updateMeshConnection for game id " << gameId);
    }
}
...
void GameManagerUtilInstance::censusDataSubscribe()
{
    ...
    if (err != ERR_OK)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::censusdata, "GameManagerUtilInstance[" << getIdent() << "]: error " << err << " subscribing to census data.");
        mIsSubscribedCensusData = false;
    }
#endif
}
...
void GameManagerUtilInstance::censusDataUnsubscribe()
{
    ...
    if (err != ERR_OK)
    {
        BLAZE_WARN_LOG(BlazeRpcLog::censusdata, "GameManagerUtilInstance[" << getIdent() << "]: error " << err << " unsubscribing to census data.");
    }
#endif
}
...
void GameManagerUtilInstance::newGameState()
{
    ...
    BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: " 
    ...
    if (mGameReportingUtilInstance != nullptr)
    {
        ...
        case POST_GAME:
        {
            ...
            {
                BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: " 
...
void GameManagerUtilInstance::updateGamePresenceForLocalUser(const GameId changedGameId, UpdateExternalSessionPresenceForUserReason change, PlayerRemovedReason removeReason /*= SYS_PLAYER_REMOVE_REASON_INVALID*/)
{
    ...
    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerInstance[" << getIdent() << "]: Determining which of " << (gsData != nullptr? 1 : 0) <<
    ...
    if (rpcResult != ERR_OK)
    {
        BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerInstance[" << getIdent() << "]: update of primary game for user(" << getBlazeId() << ") result: " << ErrorHelp::getErrorName(rpcResult) << ".\n");
...
Blaze::BlazeRpcError GameManagerUtilInstance::updateGameName()
{
    ...
    if (roll < (mUtil->getGameNameRandomUpdates() * 100))
    {
        ...
        BLAZE_SPAM_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: updating game(" << gamedata->getGameId() << ") name: '" << getGameData()->getGameName() << "' ==to==> '" << mUpdateGameNameRequest << "'");
        ...
        if (err != ERR_OK)
        {
            BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: update of game(" << gamedata->getGameId() << ") name failed. Error(" << ErrorHelp::getErrorName(err) << ").");
        }
    }
    return err;
}
...
Blaze::BlazeRpcError GameManagerUtilInstance::updateGameExternalStatus()
{
    ...
    if (roll < (mUtil->getGameExternalStatusRandomUpdates() * 100))
    {
        ...
        BLAZE_SPAM_LOG(BlazeRpcLog::gamemanager, "GameManagerUtilInstance[" << getIdent() << "]: updating game(" << gamedata->getGameId() << ") external session status: ==to==> '" << mUpdateGameExternalStatusRequest.c_str() << "'");
        ...
        if (err != ERR_OK)
        {
            BLAZE_WARN_LOG(BlazeRpcLog::gamemanager, "GameManagerUtil : set game external session status(" << mUpdateGameExternalStatusRequest.c_str() << ") for game(" << mGameId << ") failed. Error(" 
                << (ErrorHelp::getErrorName(err)) << ")");
        }
    }
    return err;
...
bool GameData::addPlayer(const ReplicatedGamePlayer &player)
{
    ...
    playerData.mJoinCompleted = player.getPlayerState() == ACTIVE_CONNECTED;
    ...
    eastl::pair<PlayerMap::iterator, bool> inserter = mPlayerMap.insert(PlayerMap::value_type(playerData.mPlayerId, playerData));
    if (inserter.second)
    {       
        inserter.first->second = playerData;
        inserter.first->second.getAttributeMap()->insert(player.getPlayerAttribs().begin(), player.getPlayerAttribs().end());
    ...
    else
    {
        ...
        inserter.first->second.mJoinCompleted = playerData.mJoinCompleted;
    }
