...
{
    mGameTypeCollection = NULL;
    mComponentConfig = NULL;
...
bool GameReportingUtil::initialize(const ConfigMap& config, const ConfigBootUtil* bootUtil)
{
    ...
    if (onlineGameTypeSequence == NULL)
    {
    ...
    else
    {
        while (onlineGameTypeSequence->hasNext())
        {
            ...
            if (gameTypeMap != NULL)
            {
                const char8_t* gameReportNameStr = gameTypeMap->getString("typeName", NULL);
                uint32_t weight = gameTypeMap->getUInt32("weight", 0);

                if (gameReportNameStr != NULL)
                {
    ...
    if (offlineGameTypeSequence == NULL)
    {
...
void GameReportingUtil::updateReport(GameManager::GameReportingId gameReportingId, const Collections::AttributeMap* gameAttrs, const PlayerAttributeMap* players)
{
    ...
    if (it == mReportingSessions.end() || it->second == NULL)
        return;
...
BlazeRpcError GameReportingUtil::submitOfflineReport(BlazeId blazeId, GameReporting::GameReportingSlave* proxy)
{
    ...
    if (reporter != NULL)
    {
        ...
        player[blazeId] = NULL;         // no attribute map, but used by the reporter if the submitter ID is needed.
        reporter->update(NULL, &player);
        SubmitGameReportRequest req;
        gameReport->copyInto(req.getGameReport());
...
void GameReportingUtilInstance::setNotificationCb(const AsyncNotifyCb& cb)
{
    if (mStressInst == NULL)
    {
        BLAZE_ERR_LOG(BlazeRpcLog::gamereporting, "GameReportingUtilInstance[" << getIdent() << "] : unable to set notification callback on uninitialized instance.");
        return;
    }
    mResultNotifyCb = cb;
}
    ...
    case GameReportingSlave::NOTIFY_RESULTNOTIFICATION:
        {
            ...
            {
                BLAZE_ERR_LOG(BlazeRpcLog::gamereporting, "GameReportingUtilInstance[" << getIdent() << "]: Failed to decode notification(" << type << ")");
            }
            else
            {
