...
namespace GameManager
{
    ...
    ***************************************************************************************************/
    bool GameBrowserList::sendListUpdateNotifications(uint32_t maxGameUpdatesPerNotification, uint32_t maxListUpdateNotifications, GameBrowser::NotifyGameListReason notifyReason, uint32_t msgNum)
    {
        ...
        UserSessionMasterPtr ownerSession = gUserSessionMaster->getLocalSession(mOwnerSessionId);
        if (ownerSession == nullptr)
        {
            TRACE_LOG("[GameBrowserList].sendListUpdateNotifications: owner(" << mOwnerSessionId << ") of this GameBrowserList(" 
                      << getListId() << ") has logged out");
            return false;
        }
        ...
        do
        {
            ...
            mGameBrowser.sendNotifyGameListUpdateToUserSessionById(ownerSession->getUserSessionId(), &notifyGameListUpdate, false, msgNum);
