...
public:
    ...
    BlazeRpcError processToken(const char8_t* accessToken, eastl::string& tokenScopes, NucleusIdentity::IpGeoLocation& ipGeoLocation);
...
BlazeRpcError TrustedLoginCommand::processToken(const char8_t* accessToken, eastl::string& tokenScopes, NucleusIdentity::IpGeoLocation& ipGeoLocation)
{
    Blaze::OAuth::CachedTrustedToken trustedToken;
    if (mComponent->oAuthSlaveImpl()->getCachedTrustedToken(accessToken, trustedToken))
    {
    ...
    BlazeRpcError err = AuthenticationUtil::authFromOAuthError(mComponent->oAuthSlaveImpl()->getTokenInfo(tokenInfoRequest, tokenInfoResponse));
    if (err == ERR_OK)
    {
        ...
        mComponent->oAuthSlaveImpl()->cacheTrustedToken(accessToken, tokenInfoResponse);
    }
    ...
    {
        WARN_LOG("[TrustedLoginCommand].validateAccessToken: token is most likely expired or not valid for some other reason. Client need to retry after getting a new one from Nucleus.");
...
BlazeRpcError TrustedLoginCommand::doLogin(ClientPlatformType clientPlatform, ClientType clientType, const char8_t* productName)
{
    ...
    BlazeRpcError err = processToken(token, tokenScopes, ipGeoLocation);
...
TrustedLoginCommandStub::Errors TrustedLoginCommand::execute()
{
    ...
    if (err != Blaze::ERR_OK)
    {
        ...
        gUserSessionMaster->generateLoginEvent(err, getUserSessionId(), USER_SESSION_TRUSTED, platformInfo, "", DEVICELOCALITY_UNKNOWN,
            ...
            addrStr, serviceName, geoIpData, mComponent->getProjectId(productName), LOCALE_BLANK, 0, clientVersion, sendPINErrorEvent);
    }
    else
    {
