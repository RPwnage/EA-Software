...
void AssociationListMember::handleExistenceChanged()
{
    if (mOwnerList.getOwnerPrimarySession() != nullptr)
    {
        ...
        UserSessionId listMemberPrimaryUserSessionId = gUserSessionManager->getPrimarySessionId(getBlazeId());
        bool listMemberIsOnline = UserSession::isValidUserSessionId(listMemberPrimaryUserSessionId);

        UserStatus userStatus;
        userStatus.setBlazeId(getBlazeId());
        ...
        if (listMemberIsOnline)
        {
            ...
            if ((gUserSessionManager->getSession(ownerPrimaryUserSessionId) == nullptr) || (mOwnerList.getOwnerPrimarySession() == nullptr))
            {
                ERR_LOG("[AssociationListMember].handleExistenceChanged: AssociationList for owner=" << ownerBlazeId
                    << " has had its primary user session =" << ownerPrimaryUserSessionId << " erased before updatingNotifiers for list member=" << getBlazeId() << ".");
            }
        ...
        else if (mSubscribed && mOwnerList.getInfo().getStatusFlags().getOfflineUED())
        {
            ...
            ued.setBlazeId(getBlazeId());

            // Diagnostic logging: did the state of the user session change since the initial checks?
            if ((gUserSessionManager->getSession(ownerPrimaryUserSessionId) == nullptr) || (mOwnerList.getOwnerPrimarySession() == nullptr))
            {
                ERR_LOG("[AssociationListMember].handleExistenceChanged: AssociationList for owner=" << ownerBlazeId
                    << " has had its primary user session =" << ownerPrimaryUserSessionId << " erased before updating the UED update for list member=" << getBlazeId() << ".");
            }

            BlazeRpcError err = gUserSessionManager->requestUserExtendedData(*mUser, ued.getExtendedData());
            if (err == ERR_OK)
            {
                // Diagnostic logging: did the state of the user session change since the initial checks?
                if ((gUserSessionManager->getSession(ownerPrimaryUserSessionId) == nullptr) || (mOwnerList.getOwnerPrimarySession() == nullptr))
                {
                    ERR_LOG("[AssociationListMember].handleExistenceChanged: AssociationList for owner=" << ownerBlazeId
                        << " has had its primary user session =" << ownerPrimaryUserSessionId << " erased after updating UED for list member=" << getBlazeId() << ".");
                }
