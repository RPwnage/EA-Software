...
BlazeRpcError ClubsDatabase::getClubMembers(ClubId clubId, uint32_t maxResultCount, uint32_t offset, ClubMemberList &clubMemberList, MemberOrder orderType, OrderMode orderMode, MemberTypeFilter memberType, const char8_t* personNamePattern, uint32_t *totalCount)
{
    ...
    if (mDbConn != nullptr)
    {
        ...
        if (query != nullptr)
        {
            ...
            {
                eastl::string memShipStr = "";
                eastl::string orderStr = "";

                if (memberType != ALL_MEMBERS)
                {
                    memShipStr.sprintf("%s %u", (memberType != GM_MEMBERS ? " AND `cm`.`membershipStatus` < " : " AND `cm`.`membershipStatus` >= "), CLUBS_GM);
                }

                if (orderType != MEMBER_NO_ORDER)
                {
                    orderStr.sprintf(" ORDER BY `cm`.`memberSince` %s ", (orderMode == DESC_ORDER) ? CLUBSDB_SORT_DESC : CLUBSDB_SORT_ASC);
                }

                query->append(CLUBSDB_SELECT_CLUB_MEMBERS, isCalcFoundRows ? CLUBSDB_SQL_CALC_FOUND_ROWS : "", clubId);
                query->append(memShipStr.c_str());
                query->append(orderStr.c_str());
            ...
            else
            {
                ...
                if (userInfoConn != nullptr)
                {
                    ...
                    if (orderType != MEMBER_NO_ORDER)
                    {
                        orderStr.sprintf(" ORDER BY `userinfo`.`persona` %s ", (orderMode == DESC_ORDER) ? CLUBSDB_SORT_DESC : CLUBSDB_SORT_ASC);
                    }
                    userInfoQuery->append(orderStr.c_str());
