...
void GrpcEndpoint::shutdown()
{
    ...
    {
        bool readyToShutdown = entry.second->setupGracefulShutdown();
        if (readyToShutdown)
    ...
    else
    {
        ...
        {
            BLAZE_INFO_LOG(Log::GRPC, "[GrpcEndpoint].shutdown: Wait for the force shutdown to finish...");
            Fiber::getAndWait(mAllRequestHandlersDoneEvent, "GrpcEndpoint::shutdown wait for all request handlers to be finished after force shutdown.");
            BLAZE_INFO_LOG(Log::GRPC, "[GrpcEndpoint].shutdown: Forced shutdown of request handlers finished. Proceeed with shutdown. ");
        }
    }
...
bool GrpcEndpoint::initialize(const EndpointConfig::GrpcEndpointConfig& config, const RateConfig& rateConfig, const EndpointConfig::CommandListByNameMap& rpcControlLists, const EndpointConfig::PlatformSpecificControlListsMap& platformSpecificControlLists)
{
    ...
    mTrustedNetworks.reset(BLAZE_NEW Blaze::InetFilter());
    if (!mTrustedNetworks->initialize(true, mConfig.getTrust()))
    {
        BLAZE_ERR_LOG(Log::GRPC, "[GrpcEndpoint].initialize: invalid trust configuration for endpoint '" << getName() << "'");
        return false;
    }

    mInternalNicAccessNetworks.reset(BLAZE_NEW Blaze::InetFilter());
    if (!mInternalNicAccessNetworks->initialize(true, mConfig.getHasInternalNicAccess()))
    {
...
void GrpcEndpoint::expireRequestHandlers()
{
    ...
    for (auto& entry : mSessionKeyToRequestHandlerMap) 
    {
        ...
        if (userSession == nullptr && entry.second->isAttachedToUserSession())
        {
            ...
            BLAZE_TRACE_LOG(Log::GRPC, "[GrpcEndpoint].expireRequestHandlers: Queueing up destruction of request handler associated with session key (" << entry.first.c_str() << ") as session no longer present");
            destroyRequestHandler(entry.second.get());
        }
    }
...
void GrpcEndpoint::reconfigure(const EndpointConfig::GrpcEndpointConfig& newConfig, const RateConfig& newRateConfig, const EndpointConfig::CommandListByNameMap& rpcControlLists, const EndpointConfig::PlatformSpecificControlListsMap& platformSpecificControlLists)
{
    ...
    if (!newFilter->initialize(true, newConfig.getTrust()))
    {
        BLAZE_ERR_LOG(Log::GRPC, "[GrpcEndpoint].reconfigure: invalid trust configuration for endpoint '" << getName() << "'");
        delete newFilter;
    }
    else
    {
        mTrustedNetworks.reset(newFilter); // destroy old one and take ownership of new one
    }

    newFilter = BLAZE_NEW Blaze::InetFilter();
    if (!newFilter->initialize(true, newConfig.getHasInternalNicAccess()))
    {
...
InboundGrpcRequestHandler* GrpcEndpoint::getRequestHandler(const char8_t* sessionKey)
{
    ...
    {
        BLAZE_WARN_LOG(Log::GRPC, "No request handler found for sessionKey - " << sessionKey);
    }

    return nullptr;
}

void GrpcEndpoint::changeRequestHandlerSessionKey(const char8_t* newSessionKey, const char8_t* tempSessionKey)
{
    ...
    {
        BLAZE_WARN_LOG(Log::GRPC, "Request Handler does not have a valid temp session key.");
    }
}
...
{
    if (getEnvoyAccessEnabled())
        return false;

    return (getBindType() == BIND_INTERNAL);
}

bool GrpcEndpoint::isTrustedInternalSecureEnvoyEndpoint() const
{
    // Check if Envoy is enabled for this endpoint.
    if (getEnvoyAccessEnabled() == true)
    {
        // Check if is an internal endpoint.
        if (getBindType() == BIND_INTERNAL)
        {
            // Check if this is a secure endpoint.
            // The ingress Envoy node terminates mTLS between itself and the downstream client.
            // However, the Envoy node originates a new TLS handshake between itself and the upstream server (Blaze).
            if (blaze_stricmp(getChannelType(), "ssl") == 0)
            {
                // Check if endpoint access is limited to trusted peers (like the ingress Envoy node).
                if (mTrustedNetworks->getCount() != 0)
                {
