    ...
    RawBuffer* responseRaw /*= nullptr*/)
{
    ...
    if (BLAZE_IS_LOGGING_ENABLED(Log::REST, Logging::TRACE)) // Optimization to prevent building the request log if the requisite logging isn't enabled
    {
        StringBuilder httpLog;
        HeaderVector censoredHeaders;
        HttpParamVector censoredHttpParams;
        if (requestTdf != nullptr)
        {
            buildCustomHeaderVector(*restInfo, *requestTdf, censoredHeaders, true);
            buildCustomParamVector(*restInfo, *requestTdf, censoredHttpParams, true);
        }

        printHttpRequest(uri, *restInfo, censoredHeaders, censoredHttpParams, requestTdf, nullptr, httpLog);
        BLAZE_TRACE_LOG(Log::REST, "[RestProtocolUtil].sendHttpRequest: Send request to " << connMgr->getAddress().getHostname() << " " << httpLog);

        for (auto& param : censoredHttpParams)
        {
            // clean up memory allocated via str_dup
            BLAZE_FREE(param.value);
        }
    ...
    if (logErr || BLAZE_IS_LOGGING_ENABLED(Log::REST, Logging::TRACE))
    {
        ...
        if (!logErr)
        {
            BLAZE_TRACE_LOG(Log::REST, "[RestProtocolUtil].sendHttpRequest: Sent request for call to " << requestLog);
            BLAZE_TRACE_LOG(Log::REST, "[RestProtocolUtil].sendHttpRequest: Received response " << ErrorHelp::getErrorName(requestError) << ", HttpStatus:" << httpResult.getStatusCode() << " for call to " << responseLog);
        }
        else
        {
            BLAZE_ERR_LOG(Log::REST, "[RestProtocolUtil].sendHttpRequest: Sent request for call to " << requestLog);
            BLAZE_ERR_LOG(Log::REST, "[RestProtocolUtil].sendHttpRequest: Received response " << ErrorHelp::getErrorName(requestError) << ", HttpStatus:" << httpResult.getStatusCode() << " for call to " << responseLog);
        }
    }
