...
PLATFORM:
    ALTER TABLE `userinfo`
      ADD COLUMN `externalstring` varchar(190) DEFAULT NULL AFTER `externalblob`,
      ADD UNIQUE KEY `externalstring_idx` (`externalstring`) USING BTREE;
    ...
    BEGIN
      ...
      DECLARE currentExternalString varchar(190);
      ...
      BEGIN
        ...
        SELECT `accountlocale`,`accountcountry`,`externalid`,`externalblob`,`externalstring`
          INTO currentAccountLocale, currentAccountCountry, currentExternalId, currentExternalBlob, currentExternalString
          FROM `userinfo`
        ...
        SET initExternalData =
          ...
          ((currentExternalBlob IS NULL OR (LENGTH(currentExternalBlob) = 0)) AND (_externalblob IS NOT NULL AND (LENGTH(_externalblob) != 0))) OR
        ...
        IF userNotFound THEN
          INSERT INTO `userinfo`
            (`blazeid`, `accountid`, `externalid`, `externalblob`, `externalstring`, `persona`, `canonicalpersona`, `personanamespace`, `accountlocale`, `accountcountry`, `status`,
              `firstlogindatetime`,`lastlogindatetime`, `lastautherror`, `lastlocaleused`, `lastcountryused`, `childaccount`, `originpersonaid`, `pidid`)
            VALUES (_blazeid, _accountid, _externalid, _externalblob, _externalstring, _persona, _canonicalpersona, _personanamespace, _accountlocale, _accountcountry, _status,
