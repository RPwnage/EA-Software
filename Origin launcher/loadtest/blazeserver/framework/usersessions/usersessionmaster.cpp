...
BlazeRpcError UserSessionMaster::addUser(BlazeId blazeId, bool sendImmediately)
{
    ...
    if (insertRet.second)
    {
        ...
        if (err == ERR_OK)
        {
            ...
            {
                BlazeRpcError extDataErr = ERR_OK;
                if (getSliverId() == GetSliverIdFromSliverKey(primarySessionId))
                {
                    UserSessionMasterPtr primaryUserSession = gUserSessionMaster->getLocalSession(primarySessionId);
                    if (primaryUserSession != nullptr)
                        primaryUserSession->getExtendedData().copyInto(notifyUserAdded.getExtendedData());
                    else
                        extDataErr = USER_ERR_SESSION_NOT_FOUND;
                }
                else
                    extDataErr = gUserSessionManager->getRemoteUserExtendedData(primarySessionId, notifyUserAdded.getExtendedData());
            ...
            notifyUserAdded.getUserInfo().setBlazeId(blazeId);
             ...
             if ((blazeId != getBlazeId()) && !isAuthorized(Blaze::Authorization::PERMISSION_OTHER_NETWORK_ADDRESS, true))
             {
                 ...
                 unsetNetworkAddress.copyInto(notifyUserAdded.getExtendedData().getAddress());
             }
...
BlazeRpcError UserSessionMaster::addUsers(const BlazeIdList& blazeIds, bool sendImmediately)
{
    BlazeRpcError err = ERR_OK;
    for (BlazeIdList::const_iterator it = blazeIds.begin(), end = blazeIds.end(); it != end; ++it)
    {
        err = addUser(*it, sendImmediately);
    }
    return err;
}

void UserSessionMaster::addUsersAsync(const BlazeIdList& blazeIds)
{
    if (EA_UNLIKELY(mNotificationJobQueue == nullptr))
        return;

    EA::TDF::tdf_ptr<BlazeIdList> _blazeIds = BLAZE_NEW BlazeIdList(blazeIds);
    mNotificationJobQueue->queueFiberJob<UserSessionId, EA::TDF::tdf_ptr<BlazeIdList> >(&UserSessionMaster::finish_addUsersAsync, getUserSessionId(), _blazeIds, "UserSessionMaster::finish_addUsersAsync");
}

void UserSessionMaster::finish_addUsersAsync(UserSessionId userSessionId, EA::TDF::tdf_ptr<BlazeIdList> _blazeIds)
{
    UserSessionMasterPtr userSession = gUserSessionMaster->getLocalSession(userSessionId);
    if (userSession == nullptr)
        return;

    userSession->addUsers(*_blazeIds, true);
}

BlazeRpcError UserSessionMaster::removeUser(BlazeId blazeId, bool sendImmediately)
{
    BlazeRpcError err = ERR_OK;
...
BlazeRpcError UserSessionMaster::removeUsers(const BlazeIdList& blazeIds, bool sendImmediately)
{
    BlazeRpcError err = ERR_OK;
    for (BlazeIdList::const_iterator it = blazeIds.begin(), end = blazeIds.end(); it != end; ++it)
    {
        err = removeUser(*it, sendImmediately);
    }
    return err;
}

void UserSessionMaster::removeUsersAsync(const BlazeIdList& blazeIds)
{
    if (EA_UNLIKELY(mNotificationJobQueue == nullptr))
        return;

    EA::TDF::tdf_ptr<BlazeIdList> _blazeIds = BLAZE_NEW BlazeIdList(blazeIds);
    mNotificationJobQueue->queueFiberJob<UserSessionId, EA::TDF::tdf_ptr<BlazeIdList> >(&UserSessionMaster::finish_removeUsersAsync, getUserSessionId(), _blazeIds, "UserSessionMaster::finish_removeUsersAsync");
}

void UserSessionMaster::finish_removeUsersAsync(UserSessionId userSessionId, EA::TDF::tdf_ptr<BlazeIdList> _blazeIds)
{
    UserSessionMasterPtr userSession = gUserSessionMaster->getLocalSession(userSessionId);
    if (userSession == nullptr)
        return;

    userSession->removeUsers(*_blazeIds, true);
}
