...
private:
    ...
    ValidateAccessTokenFormatCommandStub::Errors execute() override
    {
        ...
        if (gCurrentUserSession != nullptr)
        {
            ...
            {
                ERR_LOG("[ValidateAccessTokenFormatCommand].execute: server token (" << serverTokenUtil.getAccessToken() << ") is jwt but should never be JWT");
                return ERR_SYSTEM;
            }
            ...
            switch (getRequest()->getTokenFormat())
            {
            ...
            default:
            {
                ERR_LOG("[ValidateAccessTokenFormatCommand].execute: unknown token format (" << TokenFormatToString(getRequest()->getTokenFormat()) << ").");
                return ERR_SYSTEM;
            }
            }
