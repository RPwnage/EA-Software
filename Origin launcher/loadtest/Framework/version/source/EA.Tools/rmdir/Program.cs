	...
	class Program
	{
		...
		static int Main(string[] args)
		{
			try
			{
				...
				foreach (var dir in Directories)
				{
					try
					{
						NAnt.Core.Util.PathUtil.DeleteDirectory(dir, verify: true, failOnError: true);
					}
					catch (Exception er)
					{
						SetError(er);
		...
		private static void SetOption(char o)
		{
			switch (o)
			{
				...
				default:
					if (!Silent)
					{
						Console.WriteLine("rmdir: unknown option '-{0}'", o);
					}
					break;
			}
		}
		...
		private static void SetError(Exception e)
		{
			if (!SupressError)
			{
				if (!Silent)
				{
					Console.WriteLine("rmdir error: '{0}'", e.Message);
				}
				ret = 1;
			}
		}
		...
		{
			Console.WriteLine("Usage: rmdir [-esvh] dir1 [dir2, dir3 ...] [@responsefile]");
			Console.WriteLine();
