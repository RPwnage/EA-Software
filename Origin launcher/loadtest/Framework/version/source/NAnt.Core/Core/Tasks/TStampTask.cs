...
namespace NAnt.Core.Tasks
{
	...
	/// </remarks>
	/// <include file='Examples/TStamp/TStamp1Property.example' path='example'/>
	/// <include file='Examples/TStamp/TStampMultipleProperties.example' path='example'/>
	[TaskName("tstamp", NestedElementsCheck = true)]
	public class TStampTask : Task {
		...
		[TaskAttribute("printtime", Required = false)]
		...
		protected override void ExecuteTask() {
			...
			if(PrintTime)
				Log.Status.WriteLine(LogPrefix + now.ToString(Pattern));
			...
			// set custom property
			if (Property != null) {
				Properties[Property] = now.ToString(Pattern);
				Log.Info.WriteLine(LogPrefix + Property + " = " + Properties[Property].ToString());
			}
			...
			foreach (FormatterElement f in _formatterElements) {
				...
				Log.Info.WriteLine(LogPrefix + f.Property + " = " + Properties[f.Property].ToString());
			}
