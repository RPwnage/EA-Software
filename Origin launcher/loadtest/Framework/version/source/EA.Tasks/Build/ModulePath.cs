...
namespace EA.Eaconfig.Build
{
	...
	public class ModulePath
	{
		...
		private static string GetBinarySuffix(Project project, BuildType buildType)
		{
			...
			if (buildType.IsCSharp || buildType.IsFSharp)
			{
				OptionSet buildOptionSet = project.NamedOptionSets[buildType.Name];
				DotNetCompiler.Targets target = ConvertUtil.ToEnum<DotNetCompiler.Targets>(buildOptionSet.Options[buildType.IsCSharp ? "csc.target" : "fsc.target"]);				
