...
{
	internal sealed class VS15Solution : VSSolutionBase
	{
		internal VS15Solution(Log log, string name, PathString outputdir, string vsVersion, string vsToolSetVersion, bool enableSndbs)
		...
		protected override ModuleGenerator CreateModuleGenerator(IEnumerable<IModule> modules)
		{
			switch (GetVSProjectType(modules))
			{
				case VSProjectTypes.CSharp:
					return new VS15CSProject(this, modules);
				case VSProjectTypes.FSharp:
					return new VS15FSProject(this, modules);
				...
				default:
					return new VS15Project(this, modules);
			}
		}
		...
		protected override void WriteHeader(IMakeWriter writer)
		{
			...
			writer.WriteLine("# Visual Studio " + VSVersion.Substring(0, 2));
		}
