...
namespace EA.Eaconfig.Core
{
	public abstract class ProcessableModule : Module, IProcessableModule
	{
		...
		/// ensures that dependencies are processed before their parents e.g. <see cref="ForEachModule"/>
		/// </summary>
		public int GraphOrder { get; internal set; } = -1;
		...
		public void SetModuleBuildProperties(Project buildproject = null)
		{
			var project = buildproject ?? Package.Project;
			if (project != null)
			{
				project.Properties["groupname"] = GroupName;
				project.Properties["build.module"] = Name;
				project.Properties["groupsourcedir"] = GroupSourceDir;
				// For FW3 to work with build graph from targets
				project.Properties["build.module.key"] = Key;

				project.Properties["build.buildtype"] = BuildType.Name;
				project.Properties["build.buildtype.base"] = BuildType.BaseName;
			}
		}
	}
}
