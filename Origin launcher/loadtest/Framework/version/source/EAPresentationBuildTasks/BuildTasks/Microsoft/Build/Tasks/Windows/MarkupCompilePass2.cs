...
#pragma warning restore 6500
            ...
            else
            {
				...
				IEnumerable<string> duplicateBamlPaths = GeneratedBaml.Select(item => item.ItemSpec).GroupBy(path => path).Where(grp => grp.Count() > 1).Select(grp => grp.First());
				if (duplicateBamlPaths.Any())
				{
					Log.LogError("MarkupCompilePass2 task generated duplicate output BAML paths:\n\t{0}\nThis will cause incorrect runtime or build behaviour.", String.Join("\n\t", duplicateBamlPaths));
					return false;
				}
        ...
        private void GenerateOutputItems( )
        {
            ...
            if (_localApplicationFile != null)
            {
				...
				relativeFile = String.IsNullOrWhiteSpace(_localApplicationFile.LinkAlias) ? GetResolvedFilePath(_localApplicationFile.FilePath, ref newSourceDir) : _localApplicationFile.LinkAlias;
            ...
            if (_localMarkupPages != null && _localMarkupPages.Length > 0)
            {
                for (int i = 0; i < _localMarkupPages.Length; i++)
                {
					...
					relativeFile = String.IsNullOrWhiteSpace(localRefFile.LinkAlias) ? GetResolvedFilePath(localRefFile.FilePath, ref newSourceDir) : localRefFile.LinkAlias;
