...
static int32_t _ConciergeMetricsCcsStatus(VoipConciergeRefT* pVoipConcierge, const char* pMetricName, ServerMetricsFormatE eFormat, char* pBuffer, int32_t iBufSize, const char* pCommonDimensions);
...
static ConciergeMetricHandlerEntryT _ConciergeMetrics_MetricHandlers[] =
{
    ...
    { "voipmetrics.ccs.status",                 _ConciergeMetricsCcsStatus, 0, 0 },
...
/*F********************************************************************************/
/*!
    \Function _ConciergeMetricsCcsStatus

    \Description
        Adds the "voipmetrics.ccs.status" metric to the specified buffer.
        Metric description: 1 if online (known to CCS), 0 if offline (unknown to CCS)
    ...
    \Version 01/26/2021 (mclouatre)
*/
/********************************************************************************F*/
static int32_t _ConciergeMetricsCcsStatus(VoipConciergeRefT *pVoipConcierge, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    uint32_t bOffline = VoipConciergeStatus(pVoipConcierge, 'offl', 0, NULL, 0);  // query if we are known to the CCS
    return(ServerMetricsFormatFloat(pBuffer, iBufSize, pMetricName, eFormat, (bOffline ? 0 : 1), SERVERMETRICS_TYPE_GAUGE, 100, pCommonDimensions));
}
