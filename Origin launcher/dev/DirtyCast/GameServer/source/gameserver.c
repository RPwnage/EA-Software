...
#define GAMESERVER_TCPMETRICS_RATE  (1000)
...
struct GameServerRefT
{
    ...
    //! netlink socket fd (for querying buffer stats of the gs-to-vs tcp socket)
...
static void _GameServerVoipServerDisconnect(GameServerRefT *pGameServer, int32_t iError, const char *pReason)
{
    ...
    if (pGameServer->iTcpMetricsNetlinkSocket >= 0)
    {
        TcpMetricsSocketClose(pGameServer->iTcpMetricsNetlinkSocket);
        pGameServer->iTcpMetricsNetlinkSocket = -1;
    }
...
/*F********************************************************************************/
/*!
    \Function _GameServerTcpMetricsCb
...
*/
/********************************************************************************F*/
static void _GameServerTcpMetricsCb(const TcpMetricsT *pTcpMetrics, void *pUserData)
{
    GameServerRefT *pGameServer = (GameServerRefT *)pUserData;

    if (pGameServer->pVoipServerSocket != NULL)
    {
        uint16_t uSrcPort = (uint16_t)BufferedSocketInfo(pGameServer->pVoipServerSocket, 'srcp', 0, NULL, 0);
        uint16_t uDstPort = (uint16_t)BufferedSocketInfo(pGameServer->pVoipServerSocket, 'dstp', 0, NULL, 0);

        if ((uSrcPort == pTcpMetrics->uSrcPort) && (uDstPort == pTcpMetrics->uDstPort))
        {
            // pass new samples to BufferedSocket
            BufferedSocketControl(pGameServer->pVoipServerSocket, 'sndq', pTcpMetrics->uSendQueueLen, NULL, 0);
            BufferedSocketControl(pGameServer->pVoipServerSocket, 'rcvq', pTcpMetrics->uSendQueueLen, NULL, 0);
        }
        else
        {
            DirtyCastLogPrintf("gameserver: got a netlink msg that does not match our buffered socket (srcPort: %d != %d | destport; %d != %d)\n",
                uSrcPort, pTcpMetrics->uSrcPort, uDstPort, pTcpMetrics->uDstPort);
        }
    }
}
...
*/
/********************************************************************************F*/
static void _GameServerUpdateVoipServer(GameServerRefT *pGameServer)
{
    GameServerPacketT GamePacket;
    ...
    if (pGameServer->eVoipServerState == GAMESERVER_VOIPSERVER_INIT)
    {
        ...
        // create netlink socket eventually used to query buffer sizes of AF_INET socket used to interact with voipserver
        if ((pGameServer->iTcpMetricsNetlinkSocket = TcpMetricsSocketOpen(_GameServerTcpMetricsCb, pGameServer)) < 0)
        {
            DirtyCastLogPrintf("gameserver: unable to create netlink socket with TcpMetricsSocketOpen()\n");
        }
        pGameServer->uLastTcpMetricsUpdate = NetTick();
    ...
    if ((pGameServer->eVoipServerState == GAMESERVER_VOIPSERVER_CONNECTED) || (pGameServer->eVoipServerState == GAMESERVER_VOIPSERVER_ONLINE))
    {
        ...
        // check for timeout
        if ((signed)(NetTickDiff(uCurTick, pGameServer->uLastVoipServerRecv)) > (signed)pGameServer->Config.uVoipServTime)
        {
        ...
        {
            if ((uint32_t)NetTickDiff(uCurTick, pGameServer->uLastTokenRetrieve) > GAMESERVER_TOKENTIME)
            {
                ...
                pGameServer->uLastTokenRetrieve = uCurTick;
            }
        }

        // pump tcpmetrics module
        if ((pGameServer->iTcpMetricsNetlinkSocket >= 0) && (NetTickDiff(uCurTick, pGameServer->uLastTcpMetricsUpdate) > GAMESERVER_TCPMETRICS_RATE))
        {
            // update our metrics update tick
            pGameServer->uLastTcpMetricsUpdate = uCurTick;

            // recv data from the last query and kick off new request
            TcpMetricsQueryRecv(pGameServer->iTcpMetricsNetlinkSocket);
            TcpMetricsQuerySend(pGameServer->iTcpMetricsNetlinkSocket, BufferedSocketInfo(pGameServer->pVoipServerSocket, 'srcp', 0, NULL, 0));
        }
