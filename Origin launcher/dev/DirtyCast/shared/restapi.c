...
struct RestApiRefT
{
    ...
    //! complete callback
    RestApiCompleteCbT *pCompleteCb;
    void *pUserData;
...
#if DIRTYVERS >= DIRTYSDK_VERSION_MAKE(15, 1, 7, 0, 2)
/*F********************************************************************************/
/*!
    \Function _RestApiCompleteCb

    \Description
        Complete function exercised from ProtoHttpServ

    \Input *pRequest    - data about the request (can be NULL)
    \Input *pResponse   - data on how we want to respond (can be NULL)
    \Input *pUserData   - user data

    \Version 03/04/2021 (mclouatre)
*/
/********************************************************************************F*/
static void _RestApiCompleteCb(ProtoHttpServRequestT *pRequest, ProtoHttpServResponseT *pResponse, void *pUserData)
{
    RestApiRefT *pApi = (RestApiRefT *)pUserData;

    pApi->pCompleteCb(pRequest, pResponse, pApi->pUserData);

    if ((pRequest != NULL) && (pRequest->pData != NULL))
    {
        DirtyMemFree(pRequest->pData, RESTAPI_MEMID, pApi->iMemGroup, pApi->pMemGroupUserdata);
        pRequest->pData = NULL;
    }

    if ((pResponse != NULL) && (pResponse->pData != NULL))
    {
        DirtyMemFree(pResponse->pData, RESTAPI_MEMID, pApi->iMemGroup, pApi->pMemGroupUserdata);
        pResponse->pData = NULL;
    }
...
RestApiRefT *RestApiCreate(uint32_t uPort, const char *pName, int32_t iNumResources, int32_t iMaxBodySize, uint8_t bSecure, uint32_t uFlags)
{
    ...
    // configure callbacks
    #if DIRTYVERS >= DIRTYSDK_VERSION_MAKE(15, 1, 7, 0, 2)
    ProtoHttpServCallback2(pProtoHttpServ, _RestApiRequestCb, _RestApiReceiveCb, _RestApiSendCb, _RestApiHeaderCb, _RestApiCompleteCb, _RestApiLogCb, pApi);
    #else
...
/*!
    ...
    \Input *pApi            - instance of module
    \Input *pCompleteCb     - callback to be invoked when a request is complete
    \Input *pUserData       - user-specific data to pass to the callbacks
...
{
    pApi->pCompleteCb = pCompleteCb;
    pApi->pUserData = pUserData;
}
