...
public class MkverTask : Task
{
    ...
    {
        string changeList = "";
        _ExecCmd("p4", "changes -m1 " + filePath + "/...", ref changeList);
        if (changeList != "")
        {
    ...
    protected override void ExecuteTask()
    {
        ...
        string generatedData = string.Join(Environment.NewLine, new string[] {
            ...
            string.Format("const char _ServerBuildLocation[] = \"{0}@{1}:{2}\";", Environment.UserName, File.Exists("/.dockerenv") ? "docker" : Environment.MachineName, buildDir),
