...
private:
    ...
    XblDoReputationUpdateCommandStub::Errors execute() override
    {
        ...
        for (ExternalIdList::const_iterator iter = mRequest.getExternalIds().begin(); iter != mRequest.getExternalIds().end(); ++iter)
        {
            ...
            convertToPlatformInfo(newId.getPlatformInfo(), *iter, nullptr, INVALID_ACCOUNT_ID, xone);//ARSON_TODO: to also support 'xbsx' here, this should take input mRequest.getUserIdentificationList(), and do by iter->getPlatformInfo().getClientPlatform() rather than hardcoding to use xone here.
