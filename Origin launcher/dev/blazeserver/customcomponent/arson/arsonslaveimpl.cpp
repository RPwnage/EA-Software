...
bool ArsonSlaveImpl::onConfigure()
{
    ...
    if (rc != ERR_OK)
    {
        ERR_LOG(logPrefix() << ".onConfigure: Failure fetching config for gamemanager, unable to init ExternalSessionUtils, error=" << ErrorHelp::getErrorName(rc));
    }
    else
    {
        // Just for test debugging log availability of configs/platforms blazeserver is hosting. (a config is typically required for the createExternalSessionServices below per ext-sess-enabled platform)
        eastl::string logStr;
        INFO_LOG(logPrefix() << ".onConfigure: " << toCurrPlatformsHavingExtSessConfigLogStr(logStr, &gameManagerServerConfig));
...
Blaze::BlazeRpcError ArsonSlaveImpl::getXblExternalSessionParams(Blaze::ExternalUserAuthInfo& authInfo, Blaze::GameManager::GetExternalSessionInfoMasterResponse& externalSessionInfo,
    Blaze::XblContractVersion& contractVersion,
    Blaze::EntityId gameMmSessId,
    const Blaze::UserIdentification& ident,
    EA::TDF::ObjectType entityType)
{
    ...
    if ((gameMmSessId != Blaze::GameManager::INVALID_GAME_ID) && ((entityType == Blaze::GameManager::ENTITY_TYPE_GAME) || (entityType == Blaze::GameManager::ENTITY_TYPE_GAME_GROUP)))
    {
        ...
        eastl::string logBuf;
        TRACE_LOG("[ArsonSlaveImpl].getXblExternalSessionParams: (Arson) Retrieving 1P session data from game manager master. Caller(" << GameManager::externalUserIdentificationToString(ident, logBuf) << ").");

        Blaze::GameManager::GetExternalSessionInfoMasterRequest req;
        req.setGameId(gameMmSessId);
...
Blaze::BlazeRpcError ArsonSlaveImpl::retrieveExternalSessionXbox(
    ...
    const UserIdentification* onBehalfUser)
{
    // Side: the get request is patterned after externalsessionutilxboxone.cpp's calls.
    if (!onBehalfUser)
        TRACE_LOG("[ArsonSlaveImpl].retrieveExternalSessionXbox: retrieving external session. SessionTemplateName: " << sessionTemplateName << ", sessionName: " << sessionName << ".");
    Blaze::XBLServices::MultiplayerSessionResponse result;
    BlazeRpcError error = getExternalSessionXbox(contractVersion, sessionTemplateName, sessionName, result, onBehalfUser);
    if (error != ERR_OK)
    {
        TRACE_LOG("[ArsonSlaveImpl].retrieveExternalSession: Failed to retrieve external session. SessionTemplateName: " << sessionTemplateName << ", sessionName: " << sessionName << ", error " << ErrorHelp::getErrorName(error));
...
// Overload for Large MPSD sessions to populate all users, uses multiple onBehalfUsers headers in GET calls
Blaze::BlazeRpcError ArsonSlaveImpl::retrieveExternalSessionXbox(
    const Blaze::XblSessionTemplateName& sessionTemplateName, const Blaze::XblSessionName& sessionName,
    const Blaze::XblContractVersion& contractVersion,
    Arson::GetXblMultiplayerSessionResponse &arsonResponse,
    const UserIdentificationList& onBehalfUsers)
{
    // By MS spec each call would only return in members list one item, that's for the 1 user in the X-On-behalf-of-user header (By MS spec you can only use 1 with that header also for Large sessions, fails http 403 if more).
    // Make multiple calls, merge the members into one list in response
    TRACE_LOG(logPrefix() << ".retrieveExternalSession: retrieving MPS for (" << onBehalfUsers.size() << ") users(" << (onBehalfUsers.empty() ? "" : GameManager::toExtUserLogStr(*onBehalfUsers.front()).c_str()) << " ..)");
    BlazeRpcError result = ERR_OK;
    for (auto itUser : onBehalfUsers)
    {
        Blaze::Arson::GetXblMultiplayerSessionResponse nextRsp;
        auto retErr = retrieveExternalSessionXbox(sessionTemplateName, sessionName, contractVersion, nextRsp, &(*itUser));
        if (retErr != ERR_OK)
        {
            result = retErr;
            continue; //logged
        }
        if (arsonResponse.getMultiplayerSessionResponse().getMembers().empty()) //1st user
        {
            nextRsp.copyInto(arsonResponse); //fixup format of the 1st member for consistency, and readd it below in the consistent format (without the 'me', adding the xuid)
            arsonResponse.getMultiplayerSessionResponse().getMembers().clear();
        }
        // Don't wipe out other fields, just add the member info, switching the key from 'me' to be the xuid
        for (auto itMemb : nextRsp.getMultiplayerSessionResponse().getMembers())
        {
            eastl::string xuidStr(eastl::string::CtorSprintf(), "%" PRIu64 "", itMemb.second->getConstants().getSystem().getXuid());
            auto* newMemb = arsonResponse.getMultiplayerSessionResponse().getMembers().allocate_element();
            itMemb.second->copyInto(*newMemb);
            arsonResponse.getMultiplayerSessionResponse().getMembers()[xuidStr.c_str()] = newMemb;
        }
    }
    return result;
}
...
Blaze::BlazeRpcError ArsonSlaveImpl::getExternalSessionXbox(const Blaze::XblContractVersion& contractVersion,
    const char8_t* sessionTemplateName, const char8_t* sessionName, XBLServices::MultiplayerSessionResponse& result,
    const UserIdentification* onBehalfUser)
{
    Blaze::ClientPlatformType extSesUtilPlatform = (onBehalfUser ? onBehalfUser->getPlatformInfo().getClientPlatform() : xone);

    auto* extSessUtil = getExternalSessionServiceXbox(extSesUtilPlatform);
    if (extSessUtil == nullptr)
    {
        extSesUtilPlatform = (extSesUtilPlatform == xone ? xbsx : xone);//try the other
        extSessUtil = getExternalSessionServiceXbox(extSesUtilPlatform);
    }
    if (extSessUtil == nullptr)
    {
        eastl::string logBuf;
        ERR_LOG(logPrefix() << ".getExternalSessionXbox: test error, null ExternalSessionUtilXboxOne for platform(" << Blaze::ClientPlatformTypeToString(extSesUtilPlatform) << "), check test setup code. " << toCurrPlatformsHavingExtSessConfigLogStr(logBuf, nullptr));
        return ERR_SYSTEM;
    }
    return extSessUtil->getExternalSession(sessionTemplateName, sessionName, result, onBehalfUser);
}
...
Blaze::BlazeRpcError ArsonSlaveImpl::retrieveCurrentPrimarySessionXbox(const Blaze::UserIdentification& ident, XBLServices::HandlesGetActivityResult& currentPrimarySession) const
{
    ...
    auto* extSessUtil = getExternalSessionServiceXbox(ident.getPlatformInfo().getClientPlatform());
    if (extSessUtil == nullptr)
    {
        eastl::string logBuf;
        ERR_LOG(logPrefix() << ".retrieveCurrentPrimarySessionXbox: test error, null ExternalSessionUtilXboxOne for platform(" << Blaze::ClientPlatformTypeToString(platform) << "), check test setup code. " <<  toCurrPlatformsHavingExtSessConfigLogStr(logBuf, nullptr));
        return ERR_SYSTEM;
    }
    return extSessUtil->getPrimaryExternalSession(ident.getPlatformInfo().getExternalIds().getXblAccountId(), currentPrimarySession);
}
...
void ArsonSlaveImpl::blazeXblMultiplayerSessionResponseToArsonResponse(const Blaze::XBLServices::MultiplayerSessionResponse& restReponse,
    Arson::GetXblMultiplayerSessionResponse &arsonResponse)
{
    ...
    arsonResponse.getMultiplayerSessionResponse().getConstants().getSystem().getCapabilities().setCrossPlay(restReponse.getConstants().getSystem().getCapabilities().getCrossPlay());
    arsonResponse.getMultiplayerSessionResponse().getConstants().getSystem().getCapabilities().setLarge(restReponse.getConstants().getSystem().getCapabilities().getLarge());
    ...
    arsonResponse.getMultiplayerSessionResponse().getConstants().getSystem().setMaxMembersCount(restReponse.getConstants().getSystem().getMaxMembersCount());
    ...
    #if 0 //No longer used, old XBL Tournys:
    ...
    #endif
    // MPS members section
    auto itr = restReponse.getMembers().begin();
    auto end = restReponse.getMembers().end();
    for (; itr != end; ++itr)
    {
        ...
        for (auto itGrp : itr->second->getProperties().getSystem().getGroups())
        {
            nextMember->getProperties().getSystem().getGroups().push_back(itGrp.c_str());
        }
        #if 0 //No longer used, old XBL Tournys:
...
bool ArsonSlaveImpl::hasHigherRateLimitRemaining(const Blaze::RateLimitInfo& a, const Blaze::RateLimitInfo& b)
{
    ...
    if (!aHasInfo)
        return true;
    if (b.isSetRecursive() && !a.isSetRecursive())
        return true;
...
Blaze::GameManager::ExternalSessionUtil* ArsonSlaveImpl::getExternalSessionService(ClientPlatformType platform) const
{
    ...
    // Unlike w/GMslave's own ExternalSessionUtils copies which create w/a GMslave up front, ArsonSlave's onConfigure/create-time can occur before a GMslave was available:
    // Ensure the local GM slave is the owner of our ExternalSessionUtilPS4:
    ...
    // Ensure the local GM slave is the owner of our ExternalSessionUtilPS5:
...
BlazeRpcError ArsonSlaveImpl::leaveAllMpsesForUser(const UserIdentification& ident)
{
    ...
    {
        eastl::string logBuf;
        ERR_LOG(logPrefix() << ".leaveAllMpsesForUser: test error, null ExternalSessionUtilXboxOne for platform(" << Blaze::ClientPlatformTypeToString(ident.getPlatformInfo().getClientPlatform()) << "), check test setup code. " << toCurrPlatformsHavingExtSessConfigLogStr(logBuf, nullptr));
        return ARSON_ERR_EXTERNAL_SESSION_FAILED;//logged
    }
    ...
    const Blaze::UserIdentification& user) const
{
    ...
    {
        eastl::string logBuf;
        ERR_LOG(logPrefix() << ".getExternalSessionsForUser: test error, null ExternalSessionUtilXboxOne for platform(" << Blaze::ClientPlatformTypeToString(user.getPlatformInfo().getClientPlatform()) << "), check test setup code. " << toCurrPlatformsHavingExtSessConfigLogStr(logBuf, nullptr));
        return ARSON_ERR_EXTERNAL_SESSION_FAILED;//logged
    }
...
/*! \brief Just for test debugging logs availability of configs/platforms blazeserver is hosting. (a config is typically required for the createExternalSessionServices below per ext-sess-enabled platform) */
eastl::string& ArsonSlaveImpl::toCurrPlatformsHavingExtSessConfigLogStr(eastl::string& buf, const Blaze::GameManager::GameManagerServerConfig* gmServerConfig) const
{
    if (!gmServerConfig)
    {
        Blaze::GameManager::GameManagerServerConfig tmpConfig;
        BlazeRpcError rc = gController->getConfigTdf("gamemanager", false, tmpConfig);
        if (rc != ERR_OK)
        {
            return buf.sprintf("Failure fetching config for gamemanager, error=%s", ErrorHelp::getErrorName(rc));
        }
        gmServerConfig = &tmpConfig;
    }

    buf.sprintf("Current external sessions config sections available in your Blaze server's .cfg :  ");
    for (ClientPlatformType curPlatform : gController->getHostedPlatforms())
    {
        auto curConfig = gmServerConfig->getGameSession().getExternalSessions().find(curPlatform);
        buf.append_sprintf(" (%s:%s)", ClientPlatformTypeToString(curPlatform), ((curConfig != gmServerConfig->getGameSession().getExternalSessions().end()) ? "has_config" : "no_config"));
    }
    return buf;
}
