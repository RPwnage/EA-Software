...
namespace Metrics 
{
    ...
    TEST_F(MetricsTest, FiltersCheck)
    {
        ...
        list->pull_back()->setMatch("^uptime");
        list->pull_back()->setMatch("fibers.cpuUsageForProcessPercent");
        list->pull_back()->setMatch("^util.stt");

        list = &oiConfig.getExclude();
        list->pull_back()->setMatch("uptime.whatsthat");   // Exclude specific value
        list->pull_back()->setMatch("^fibers");             // Excludes all fibers values including cpuUsageForProcessPercent
        list->pull_back()->setMatch("^util.sttTimeToGo");  // Excludes all util values sttTimeToGo
