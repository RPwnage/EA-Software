...
private:
    ...
    GetAccountCommandStub::Errors execute() override
    {
        ...
        if (rc != ERR_OK)
        {
            ERR_LOG("[GetAccountCommand].execute: Failed to retrieve server access token with error(" << ErrorHelp::getErrorName(rc) << ")");
        ...
        if (rc != Blaze::ERR_OK)
        {
            ERR_LOG("[GetAccountCommand].execute : Failed to retrieve user access token with error(" << ErrorHelp::getErrorName(rc) << ") for user " << blazeId);
            return commandErrorFromBlazeError(rc);
        }
