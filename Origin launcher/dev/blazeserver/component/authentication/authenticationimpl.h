...
public:
    ...
    BlazeRpcError DEFINE_ASYNC_RET(commonEntitlementCheck(
        ...
        const char8_t* userAccessToken = nullptr,
    ...
    BlazeRpcError DEFINE_ASYNC_RET( tickAndCheckLoginRateLimit());
...
private:
    ...
    {
        uint32_t mLimitPerSecond;
        uint32_t mCurrentWindowCounter;
        int64_t mCurrentWindowStart; // The timestamp aligned to 1s boundary when the current Window began
        FiberMutex mQueueMutex;

        LoginRateLimiter()
            : mLimitPerSecond(0)
            , mCurrentWindowCounter(0)
    ...
    LoginRateLimiter mLoginRateLimiter;
