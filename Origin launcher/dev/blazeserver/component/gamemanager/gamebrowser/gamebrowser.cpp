...
namespace GameManager
{
    ...
    ***************************************************************************************************/
    void GameBrowser::onShutdown()
    {
        ...
        {
            deleteGameList(listIter->second);
        }
    ...
    *************************************************************************************************/
    bool GameBrowser::reconfigure(const GameManagerServerConfig& configTdf, bool evaluateGameProtocolVersionString)
    {
        ...
        while (!mAllLists.empty())
        {
            ...
            terminatedList->sendListUpdateNotifications(getMaxGamesPerListUpdateNotification(), getMaxListUpdateNotificationsPerIdle(), LIST_DESTROYED, 0); 
    ...
    ***************************************************************************************************/
        ...
        const GetUserSetGameListSubscriptionRequest &request, GetGameListResponse &response, MatchmakingCriteriaError &error, const Message *message)
    {
        ...
        if (mGameManagerSlave.getNextUniqueGameBrowserSessionId(listId) != ERR_OK)
        {
        ...
        GameBrowserListOwnerUserSession* listOwner = BLAZE_NEW_NAMED("GameBrowserListOwnerUserSession") GameBrowserListOwnerUserSession(listId, *this, UserSession::getCurrentUserSessionId(), message->getMsgNum());
        UserSetGameBrowserList* newList = BLAZE_NEW UserSetGameBrowserList(*this, listId, USERSET_GAME_LIST_CONFIG.c_str(), blazeObjectId, listConfig->getMaxUpdateInterval(), *listOwner);
        ...
        mAllListOwners.insert(eastl::make_pair(newList->getListId(), listOwner));

        Search::CreateUserSetGameListRequest req;
        req.setGameBrowserListId(newList->getListId());
        req.setUserSessionId(listOwner->getOwnerSessionId());
        ...
        mGameBrowserListUserSessionMap.insert(eastl::make_pair(listOwner->getOwnerSessionId(), newList->getListId()));
    ...
    *************************************************************************************************/
        ...
        GetGameListResponse &response, MatchmakingCriteriaError &error, uint32_t msgNum /*=0*/)
    {
        ...
        errCode = processCreateGameList(scenarioData.getGetGameListRequest(), listType, response, error, msgNum, nullptr);
    ...
    *************************************************************************************************/
    ...
    /*! \brief create a game list
        ...
        \param[out] error the matchmaking error object
        \param[in] msgNum allows using notification cache's fetch by seq no
        \param[in] listOwner allows an external owner for this list (vs a user session)
        \return the blaze error code
    ***************************************************************************************************/
        ...
        GetGameListResponse &response, MatchmakingCriteriaError &error, uint32_t msgNum, IGameBrowserListOwnerPtr listOwner)
    {
        ...
        BlazeRpcError errCode = createGameList(request, listType, newList, error, msgNum, listOwner, gamesToBeDownloaded, maxPossibleFitScore);
        if (errCode != ERR_OK)
        {
            return errCode;
        }
    ...
    void GameBrowser::sendNotifyGameListUpdateToUserSessionById(UserSessionId ownerSessionId, const NotifyGameListUpdate* notifyGameListUpdate, bool sendImmediately, uint32_t msgNum)
    {
        ...
        const char8_t* preferredPingSite, GameNetworkTopology netTopology, const UserSessionInfo* ownerUserInfo)
    {
        ...
        BlazeRpcError errCode = createGameList(request, GAME_BROWSER_LIST_SNAPSHOT, newList, error, msgNum, nullptr, gamesToBeDownloaded, maxPossibleFitScore, requiredPlayerCapacity, preferredPingSite, netTopology, ownerUserInfo);
        if (errCode != ERR_OK)
        {
            return errCode;
        }
    ...
    ***************************************************************************************************/
    BlazeRpcError GameBrowser::processDestroyGameList(const DestroyGameListRequest& request)
    {
        ...
        if (listMapIter->second->getOwnerType() == GAMEBROWSERLIST_OWNERTYPE_USERSESSION)
        {
            UserSessionId ownerSessionId = static_cast<GameBrowserListOwnerUserSession&>(listMapIter->second->getOwner()).getOwnerSessionId();
            if (EA_UNLIKELY(ownerSessionId != UserSession::getCurrentUserSessionId()))
            {
    ...
    void GameBrowser::deleteGameList(GameBrowserList* list)
    {
        if (list != nullptr)
        {
            list->getOwner().onDestroy(); // Let the owner know that the list is going away
            
            auto iter = mAllListOwners.find(list->getListId());
            if (iter != mAllListOwners.end())
            {
                mAllListOwners.erase(list->getListId()); // Erasing will cause the ref count on the list owner to decrement (and deletion if ref count drops to 0)
            }
    ...
    *************************************************************************************************/
    BlazeRpcError GameBrowser::destroyGameList(GameBrowserListId gameBrowserListId)
    {
        ...
        if (listIter != mAllLists.end())
        {
            ...
            BlazeRpcError err = terminateGameBrowserList(*listIter->second, true/*terminateRemoteList*/);
            if (err != Blaze::ERR_OK)
            {
    ...
    *************************************************************************************************/
    void GameBrowser::destroyGameList(const GameBrowserListMap::iterator& listIter)
    {
        ...
        if (gameBrowserList->getOwnerType() == GAMEBROWSERLIST_OWNERTYPE_USERSESSION)
        {
            removeGameListFromUserOwnership(static_cast<GameBrowserListOwnerUserSession&>(gameBrowserList->getOwner()).getOwnerSessionId(), gameBrowserList->getListId());
        ...
        deleteGameList(gameBrowserList);
    }
    ...
    ***************************************************************************************************/
    ...
    Blaze::BlazeRpcError GameBrowser::createGameList(GetGameListRequest& request, ListType listType, GameBrowserList*& newList,
        MatchmakingCriteriaError &error, uint32_t msgNum, IGameBrowserListOwnerPtr listOwner, uint32_t& numberOfGamesToBeDownloaded, FitScore& maxPossibleFitScore,
        uint16_t maxPlayerCapacity, const char8_t* preferredPingSite, GameNetworkTopology netTopology, const UserSessionInfo* ownerUserInfo)
    {
        ...
        if (listOwner != nullptr)
            gameBrowserListId = listOwner->getListId();
        else if (mGameManagerSlave.getNextUniqueGameBrowserSessionId(gameBrowserListId) != ERR_OK)
        {
        ...
        if (listOwner == nullptr)
        {
            sessionId = (ownerUserInfo == nullptr) ? UserSession::getCurrentUserSessionId() : ownerUserInfo->getSessionId();
        ...
        if (listOwner == nullptr)
            listOwner = BLAZE_NEW_NAMED("GameBrowserListOwnerUserSession") GameBrowserListOwnerUserSession(gameBrowserListId, *this, sessionId, msgNum);

        newList = BLAZE_NEW GameBrowserList(*this, gameBrowserListId, listType, request.getListConfigName(), request.getListCapacity(),
            listConfig->getMaxUpdateInterval(), *listOwner);
        ...
        mAllListOwners.insert(eastl::make_pair(gameBrowserListId, listOwner));
        ...
        //match users to lists to improve onUserSessionLogout efficiency
        if (listOwner->getOwnerType() == GAMEBROWSERLIST_OWNERTYPE_USERSESSION)
            mGameBrowserListUserSessionMap.insert(eastl::make_pair(static_cast<GameBrowserListOwnerUserSession*>(listOwner.get())->getOwnerSessionId(), gameBrowserListId));
    ...
    ***************************************************************************************************/
        ...
        uint16_t maxPlayerCapacity, const char8_t* preferredPingSite, GameNetworkTopology netTopology, const UserSessionInfo* ownerUserInfo)
    {
        ...
        UserSessionPtr ownerSession;
        if (list.getOwnerType() == GAMEBROWSERLIST_OWNERTYPE_USERSESSION)
        {
            EA_ASSERT(gUserSessionMaster != nullptr);
            UserSessionId ownerSessionId = static_cast<GameBrowserListOwnerUserSession&>(list.getOwner()).getOwnerSessionId();
            ownerSession = gUserSessionManager->getSession(ownerSessionId);
            if (ownerSession == nullptr && ownerUserInfo == nullptr)
            {
                criteriaError.setErrMessage((buf << "[GameBrowser].createGameBrowserList(): Session does not exist for session id " << ownerSessionId).get());
                ERR_LOG(buf.get());
                return Blaze::ERR_SYSTEM;
            }
        }
        ...
        request.setIsUserless(list.getOwnerType() == GAMEBROWSERLIST_OWNERTYPE_USERSESSION ? false : true); 
        ...
        if (list.getOwnerType() == GAMEBROWSERLIST_OWNERTYPE_USERSESSION)
        {
            ...
            {
                // Use the current session if an external owner does not exist.
