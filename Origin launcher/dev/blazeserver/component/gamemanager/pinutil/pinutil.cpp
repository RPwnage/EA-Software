...
namespace Blaze
{
    namespace GameManager
    {
        ...
        void PINEventHelper::buildMatchInfoForGameSession(const GameSessionMaster &gameSession, RiverPoster::MPMatchInfoEvent &matchInfoEvent, const char8_t* statusString)
        {
            ...
            if (gameSession.getGameDataMaster()->getTournamentIdentification().getTournamentId()[0] != '\0')
                matchInfoEvent.setTournamentId(gameSession.getGameDataMaster()->getTournamentIdentification().getTournamentId());
            if (gameSession.getGameDataMaster()->getTournamentIdentification().getTournamentOrganizer()[0] != '\0')
                matchInfoEvent.setTournamentOrganizer(gameSession.getGameDataMaster()->getTournamentIdentification().getTournamentOrganizer());
        ...
        void PINEventHelper::buildMatchJoinForFailedPackerScenario(const GameManager::PackerScenarioInternalRequest& internalRequest, const GameManager::UserJoinInfo& initiatorJoinInfo,
            const GameManager::UserJoinInfoList friendsList, const TimeValue& duration, GameManager::MatchmakingResult result, const GameManager::UserSessionInfo& info, PINSubmission& pinSubmission)
        {
            ...
            matchJoinEvent->getHeaderCore().setEventName(RiverPoster::MP_MATCH_JOIN_EVENT_NAME);
            matchJoinEvent->setBlazeGameType(getBlazeGameTypeAsString(internalRequest.getCommonGameData().getGameType()));
            ...
            if (internalRequest.getScenarioInfo().getOriginatingScenarioId() != INVALID_SCENARIO_ID)
            {
                matchJoinEvent->setScenarioName(internalRequest.getScenarioInfo().getScenarioName());
                matchJoinEvent->setScenarioSubsessionName(internalRequest.getScenarioInfo().getSubSessionName());
                sb.reset() << internalRequest.getScenarioInfo().getScenarioVersion();
                matchJoinEvent->setScenarioVersion(sb.get());
                matchJoinEvent->setScenarioVariant(internalRequest.getScenarioInfo().getScenarioVariant());
                // set tracking tag only if populated from reco service
                ...
                for (auto& scenAttr : internalRequest.getScenarioInfo().getScenarioAttributes())
                {
        ...
        void PINEventHelper::buildMatchJoinForGameSession(const GameSessionMaster &gameSession, RiverPoster::MPMatchJoinEvent &matchJoinEvent)
        {
            ...
            if (gameSession.getGameDataMaster()->getTournamentIdentification().getTournamentId()[0] != '\0')
                matchJoinEvent.setTournamentId(gameSession.getGameDataMaster()->getTournamentIdentification().getTournamentId());
            if (gameSession.getGameDataMaster()->getTournamentIdentification().getTournamentOrganizer()[0] != '\0')
                matchJoinEvent.setTournamentOrganizer(gameSession.getGameDataMaster()->getTournamentIdentification().getTournamentOrganizer());
        ...
        void PINEventHelper::gameEnd(PlayerId playerId, GameId gameId, const GameInfo& gameInfo, const EA::TDF::Tdf& report, PINSubmission& pinSubmission, bool includeCustom /*=true*/)
        {
            ...
            if (gameInfo.getTournamentIdentification().getTournamentId()[0] != '\0')
                gameEndEvent->getGameAttributes()["tournament_id"] = gameInfo.getTournamentIdentification().getTournamentId();
            if (gameInfo.getTournamentIdentification().getTournamentOrganizer()[0] != '\0')
                gameEndEvent->getGameAttributes()["tournament_organizer"] = gameInfo.getTournamentIdentification().getTournamentOrganizer();
