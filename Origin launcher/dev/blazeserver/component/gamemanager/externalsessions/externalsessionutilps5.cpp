    ...
    ***************************************************************************************************/
    void ExternalSessionUtilPs5::getNewWebApiConnMgr(HttpConnectionManagerPtr& connMgr, HttpServiceName httpServiceName)
    {
        ...
        if ((connMgr == nullptr) || isConfigChanged)
        {
            ...
            newConnMgr->initialize(InetAddress(hostname), InetAddress(proxyHostname), config->getNumPooledConnections(), secure, true, OutboundHttpConnectionManager::SSLVERSION_DEFAULT, config->getEncodingType());
            if (connMgr != nullptr)
            {
                connMgr.reset();
            }
