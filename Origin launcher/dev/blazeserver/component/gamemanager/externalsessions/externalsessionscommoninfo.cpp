...
namespace GameManager
{
    ...
    ***************************************************************************************************/
    ...
    void setExternalUserJoinInfoFromUserSessionInfo(ExternalUserJoinInfo& externalInfo, const UserSessionInfo& userInfo, bool isReserved, TeamIndex teamIndex)
    {
        ...
        externalInfo.setTeamIndex(teamIndex);
    ...
    void setExternalSessionUpdateInfoFromGame(ExternalSessionUpdateInfo& updateInfo, const GameSessionMaster& gameSession)
    {
        ...
        gameSession.getGameDataMaster()->getExternalSessionRecentPlayersSettings().copyInto(updateInfo.getRecentPlayersSettings());
        updateInfo.setXblLarge(gameSession.getGameDataMaster()->getXblLargeSession());
    }
    ...
    void updateExternalSessionJoinParams(JoinExternalSessionParameters& params, const UserSessionInfo& userToAdd, bool isReserved, TeamIndex teamIndex, const GameSessionMaster& gameSession)
    {
        ...
        setExternalUserJoinInfoFromUserSessionInfo(*params.getExternalUserJoinInfos().pull_back(), userToAdd, isReserved, teamIndex);
    ...
    void updateExternalSessionLeaveParams(LeaveGroupExternalSessionParameters& params, const UserSessionInfo& userToAdd, const GameSessionMaster& gameSession)
    {
        ...
        {
            gameSession.getExternalSessionIdentification().copyInto(params.getSessionIdentification());
            if (EA_LIKELY(gameSession.getGameDataMaster()))
                params.setXblLarge(gameSession.getGameDataMaster()->getXblLargeSession()); //MPSD optimization
        }
    ...
    void updateExternalSessionLeaveParams(LeaveGroupExternalSessionParameters& params, ExternalId externalId, const GameSessionMaster& gameSession, ClientPlatformType platform)
    {
        ...
        if (!isExtSessIdentSet(params.getSessionIdentification()))
        {
            gameSession.getExternalSessionIdentification().copyInto(params.getSessionIdentification());
            if (EA_LIKELY(gameSession.getGameDataMaster()))
                params.setXblLarge(gameSession.getGameDataMaster()->getXblLargeSession());
        }
    ...
    const Blaze::FixedString256 toLogStr(const ExternalSessionIdentification& sessionIdentification)
    {
        ...
        {
            sessionStr.append_sprintf("xone:%s/%s ", sessionIdentification.getXone().getTemplateName(), sessionIdentification.getXone().getSessionName());
        }
    ...
    {
        Blaze::FixedString256 sessionStr(FixedString256::CtorSprintf(), "gid:%" PRIu64 ", %s", gameInfo.getGameId(), toLogStr(gameInfo.getSessionIdentification()).c_str());
        return sessionStr;
    }
