        ...
        const ExternalServiceCallOptions &config)
    {
        ...
        while ((retryCountServiceUnavailable <= config.getServiceUnavailableRetryLimit()) && (retryCountTotal++ <= config.getTotalRetryLimit()))
        {
            ...
            if (isRetryError(err))
            {
                ...
                EA::TDF::TimeValue waitTime = ((err == PSNMATCHES_CONFLICTING_REQUEST) ? (config.getConflictBackoff() + (gRandom->getRandomNumber((int)config.getConflictBackoffRange().getMillis()) * 1000)) : config.getServiceUnavailableBackoff());
