...
GamePackerSlaveImpl::GamePackerSlaveImpl()
    ...
    , mPackerPlayersGauge(getMetricsCollection(), "players", Metrics::Tag::packer_game_template_name)
    , mPackerPartiesGauge(getMetricsCollection(), "parties", Metrics::Tag::packer_game_template_name)
    , mPackerViableGamesGauge(getMetricsCollection(), "viableGames", Metrics::Tag::packer_game_template_name)
    , mPackerGamesGauge(getMetricsCollection(), "games", Metrics::Tag::packer_game_template_name)
    , mPackerSilosGauge(getMetricsCollection(), "silos", Metrics::Tag::packer_game_template_name)
    , mPackerSilosCreatedCounter(getMetricsCollection(), "silosCreated", Metrics::Tag::packer_game_template_name)
    , mPackerSilosDestroyedCounter(getMetricsCollection(), "silosDestroyed", Metrics::Tag::packer_game_template_name)
    , mGamesCreatedCounter(getMetricsCollection(), "gamesCreated", Metrics::Tag::packer_game_template_name)
    , mGamesReapedCounter(getMetricsCollection(), "gamesReaped", Metrics::Tag::packer_game_template_name)
    , mViableGamesReapedCounter(getMetricsCollection(), "viableGamesReaped", Metrics::Tag::packer_game_template_name)
    , mFactorScoreFreq(getMetricsCollection(), "factorScoreFrequency", Metrics::Tag::packer_game_template_name, Metrics::Tag::factor_name, Metrics::Tag::score_percent)
...
void GamePackerSlaveImpl::migrateSession(PackerSessionId packerSessionId)
{
    ...
    if (isRootLayer())
    {
        INFO_LOG("[GamePackerSlaveImpl].migrateSession: session(" << packerSessionId << ") exceeded viability deadline time quantum(" << packerSession.getMigrateUnfulfilledTime().getMillis() << "ms) on layer(" << getAssignedLayerId() << "), inPackerSilos(" << packerSession.mMatchedPackerSiloMap.size() << "), newCandidatesSessionsSinceStart(" << newTemplateSessionsSinceStart << "); retain session"); // PACKER_TODO: change to trace
    }
    else
    {
        INFO_LOG("[GamePackerSlaveImpl].migrateSession: session(" << packerSessionId << ") exceeded viability deadline time quantum(" << packerSession.getMigrateUnfulfilledTime().getMillis() << "ms) on layer(" << getAssignedLayerId() << "), inPackerSilos(" << packerSession.mMatchedPackerSiloMap.size() << "), newCandidatesSessionsSinceStart(" << newTemplateSessionsSinceStart << "); relinquish session to owner"); // PACKER_TODO: change to trace
        ...
        request.setPackerScenarioId(packerSession.getPackerScenarioId());
...
void GamePackerSlaveImpl::abortSession(PackerSessionId packerSessionId)
{
    auto packerSessionItr = mPackerSessions.find(packerSessionId);
    if (packerSessionItr == mPackerSessions.end())
    {
        ERR_LOG("[GamePackerSlaveImpl].abortSession: Expected session(" << packerSessionId << ") not found, unable to abort.  (Use abortSessionInternal if needed when no session is currently mapped.)");
        return;
    }
    PackerSessionSlave* packerSession = packerSessionItr->second;

    abortSessionInternal(packerSessionId, packerSession->getPackerScenarioId(), packerSession->getScenarioName(), packerSession->getSubSessionName());
}
void GamePackerSlaveImpl::abortSessionInternal(PackerSessionId packerSessionId, PackerScenarioId packerScenarioId, const GameManager::ScenarioName& scenarioName, const GameManager::SubSessionName& subsessionName)
{
    RpcCallOptions opts;
    opts.ignoreReply = true;
    Blaze::GameManager::WorkerAbortPackerSessionRequest abortReq;
    abortReq.setPackerSessionId(packerSessionId);
    abortReq.setPackerScenarioId(packerScenarioId);
    BlazeError rc = getMaster()->workerAbortPackerSession(abortReq, opts);
    if (rc != ERR_OK)
    {
        WARN_LOG("[GamePackerSlaveImpl].abortSession: Failed to abort session(" << packerSessionId << "), scenario(" << packerScenarioId << "), error: " << rc);
    }

    mSessionsAborted.increment(1, scenarioName, subsessionName);
}
...
void GamePackerSlaveImpl::cleanupSession(PackerSessionId packerSessionId, GameManager::WorkerSessionCleanupReason removalReason, bool repack)
{
    ...
    TRACE_LOG("[GamePackerSlaveImpl].cleanupSession: Cleanup session(" << packerSessionId << ") in packerScenario(" << packerSession.getPackerScenarioId() << "), remove from packer silos count(" << packerSession.mMatchedPackerSiloMap.size() << ")");
    EA_ASSERT(packerSession.mState == PackerSessionSlave::PACKING || packerSession.mState == PackerSessionSlave::AWAITING_RELEASE);
    ...
    if (isPlaceholderSessionId(packerSessionId))
    {
        ...
        for (auto& sessionSiloInfoItr : packerSession.mMatchedPackerSiloMap)
        {
            auto packerSiloId = sessionSiloInfoItr.first;
    ...
    else
    {
        ...
        mSessionsActive.decrement(1, packerSession.getScenarioName(), packerSession.getSubSessionName());
        mSessionsRemoved.increment(1, packerSession.getScenarioName(), packerSession.getSubSessionName(), removalReasonStr);
        ...
        {
            WARN_LOG("[GamePackerSlaveImpl].cleanupSession: Cleanup session(" << packerSessionId << ") in packerScenario(" << packerSession.getPackerScenarioId()
...
void GamePackerSlaveImpl::sendSessionStatus(PackerSessionId packerSessionId)
{
    ...
    request.setPackerSessionId(packerSessionId);
    request.setPackerScenarioId(packerSession.getPackerScenarioId());
    packerSession.populatePinData(request.getPinData());
...
void GamePackerSlaveImpl::handlePackerImproveGame(const Packer::Game& game, const Packer::Party& incomingParty)
{
    ...
    auto packerSiloId = packer.getPackerSiloId();
    partyAddedToGame.setPackerSiloId(packerSiloId);
    partyAddedToGame.setPackerGameId(game.mGameId);
    partyAddedToGame.setJoinedPartyId(incomingParty.mPartyId);
    auto& gameFactorScores = partyAddedToGame.getGameFactorScores();
    gameFactorScores.reserve(game.mFactorScores.size());
    StringBuilder sb;
    for (auto score : game.mFactorScores)
    {
        if (IS_LOGGING_ENABLED(Logging::TRACE))
            sb << score << ",";
        gameFactorScores.push_back(score);
    }
    if (!sb.isEmpty())
    ...
    auto otherPlayersInGameCount = game.getPlayerCount(Packer::GameState::PRESENT) - incomingParty.getPlayerCount();
    GameManager::ScoringAndFactorDetailsPtr scoringAndFactorDetails; // store scoring snapshot
    ...
    for (int32_t partyIndex = 0; partyIndex < partyCount; ++partyIndex)
    {
        ...
        // track the number of players matched/cycled through a game the session is currently in, counted from the moment the session was assigned to the game
        auto matchedSiloMapItr = packerSession.mMatchedPackerSiloMap.find(packerSiloId);
        EA_ASSERT(matchedSiloMapItr != packerSession.mMatchedPackerSiloMap.end());
        auto& playersMatchedCount = matchedSiloMapItr->second.mPlayersMatched;
        if (incomingParty.mPartyId == partyId)
        {
            playersMatchedCount = otherPlayersInGameCount;
            if (packerSession.mMaxPlayersMatched < otherPlayersInGameCount)
                packerSession.mMaxPlayersMatched = otherPlayersInGameCount;
        }
        else
        {
            playersMatchedCount += incomingParty.getPlayerCount();
            if (packerSession.mMaxPlayersMatched < playersMatchedCount)
                packerSession.mMaxPlayersMatched = playersMatchedCount;
        }

        // Get the best game the session is part of; calculate scoring info and factor details from the game
        {
            const Packer::Game* bestGame = nullptr;

            // find the best game the session is currently part of
            FloatList factorScores;
            uint64_t maxSiloPlayerCount = 0;
            for (auto& sessionSiloInfoItr : packerSession.mMatchedPackerSiloMap)
            {
                auto siloId = sessionSiloInfoItr.first;
                auto* packerSilo = getPackerSilo(siloId);
                if (packerSilo == nullptr)
                {
                    WARN_LOG("[GamePackerSlaveImpl].handlePackerImproveGame: PackerSilo(" << siloId << ") does not exist.");
                    continue;
                }
                auto siloPlayerCount = packerSilo->getPlayers().size();
                if (siloPlayerCount > maxSiloPlayerCount)
                    maxSiloPlayerCount = siloPlayerCount;
                auto* party = packerSilo->getPackerParty(partyId);
                if (party == nullptr)
                {
                    WARN_LOG("[GamePackerSlaveImpl].handlePackerImproveGame: Session(" << partyId << ") has no party in PackerSilo(" << siloId << ").");
                    continue;
                }
                auto gameItr = packerSilo->getGames().find(party->mGameId);
                if (gameItr == packerSilo->getGames().end())
                {
                    WARN_LOG("[GamePackerSlaveImpl].handlePackerImproveGame: PackerSilo(" << siloId << ") doesn't have game(" <<  party->mGameId << ").");
                    continue;
                }
                auto& gameRef = gameItr->second;

                if (factorScores.size() != packerSilo->getFactors().size())
                    factorScores.resize(packerSilo->getFactors().size(), 0.0f);

                bool replace = false;
                for (auto& factor : packerSilo->getFactors())
                {
                    auto newScore = gameRef.mFactorScores[factor.mFactorIndex];
                    ...
                    if (replace || newScore > curScore)
                    {
                        if (!replace)
                        {
                            if (bestGame != &gameRef)
                            {
                                if (bestGame != nullptr)
                                {
                                    TRACE_LOG("[GamePackerSlaveImpl].handlePackerImproveGame: session(" << partyId << ") quality factor(" << factor.mFactorIndex << ":" << factor.mFactorName << ") new score(" << newScore << ") in game(" << gameRef.mGameId << ") supercedes previous best score(" << curScore << ") in game(" << bestGame->mGameId << ")");
                                }
                                bestGame = &gameRef;
                            }
                            replace = true;
            ...
            packerSession.mMaxSiloPlayerCount = maxSiloPlayerCount;

            if (bestGame == nullptr)
            {
                ERR_LOG("[GamePackerSlaveImpl].handlePackerImproveGame: PackerSession(" << partyId << ") is not part of any game!");
                return;
            }

            // if the improved game is the best game for this session, calculate and save overall scoring info and factor details of the game
            if (bestGame->mGameId == game.mGameId)
            {
                if (scoringAndFactorDetails == nullptr)
                {
                    scoringAndFactorDetails = getScoringAndFactorDetailsFromGame(game);
                }
                packerSession.mScoringAndFactorDetails = scoringAndFactorDetails;
            }
        }

        if (packerSession.mSendUpdateTimerId == INVALID_TIMER_ID)
        {
            packerSession.mSendUpdateTimerId = gSelector->scheduleTimerCall(now + 1000 * PackerSessionSlave::SEND_UPDATE_PERIOD_MS, this, &GamePackerSlaveImpl::sendSessionStatus, packerSession.getPackerSessionId(), "GamePackerSlaveImpl::sendSessionStatus");
        }
        ...
        if (packerSession.mMigrateSessionTimerId == INVALID_TIMER_ID)
        {
            if (!nowViable)
            {
                auto curSiloId = packerSiloId;
                bool otherViable = false;
                for (auto& sessionSiloInfoItr : packerSession.mMatchedPackerSiloMap)
                {
                    auto matchedSiloId = sessionSiloInfoItr.first;
                    if (curSiloId == matchedSiloId)
                        continue; // skip current silo
                    otherViable = isPackerPartyInViableGame(matchedSiloId, partyId);
                    if (otherViable)
                        break;
                }
                if (!otherViable)
                {
                    ...
                    packerSession.mMigrateSessionTimerId = gSelector->scheduleTimerCall(migrateAtTime, this, &GamePackerSlaveImpl::migrateSession, packerSession.getPackerSessionId(), "GamePackerSlaveImpl::migrateSession");
                }
            }
        }
        else if (nowViable)
...
{
    // Empty for now
    // PACKER_TODO - What is this supposed to do?
}
...
void GamePackerSlaveImpl::updatePackerReapGameMetrics(const Packer::Game& game, bool viable)
{
    ...
    {
        // We don't remove entries from the mPackerDetailedMetricsByGameTemplateMap (even for reconfigs that remove cg templates), so check if the SiloInfo for this PackerSilo is missing somehow.
        PackerSiloInfo* siloInfo = getPackerSiloInfo((GameManager::PackerSiloId)packer.getPackerSiloId());
        ERR_LOG("[GamePackerSlaveImpl].handlePackerReapGame: PackerSiloName(" << packerSiloName << ") did not have any detailed metrics mapped.  SiloInfo " << (siloInfo == nullptr ? "MISSING" : "FOUND") <<" for silo" << packer.getPackerSiloId() << "), game(" << game.mGameId << ")")
        return;
    }
    ...
    for (auto& factor : packer.getFactors())
    {
        ...
        if (viable)
        {
            ...
            mFactorScoreFreq.increment(1, packerSiloName.c_str(), factor.mFactorName, roundToNearest);
        }
    }
    ...
    if (viable)
    {
        ...
        mViableGamesReapedCounter.increment(1, packerSiloName.c_str()); // PACKER_TODO: make reaped counter use tags, viable, and total
    }

    mGamesReapedCounter.increment(1, packerSiloName.c_str());
}

void GamePackerSlaveImpl::handlePackerReapGame(const Packer::Game& game, bool viable)
{
    updatePackerReapGameMetrics(game, viable);
    ...
    eastl::vector<PackerSessionSlave*> pendingSessions;      // These are the Sessions that have Matched and are being Packed into the Game. 
    pendingSessions.reserve(partyCount);
    BlazeId hostUserId = INVALID_BLAZE_ID;
    PackerSessionSlave* ownerSession = nullptr;
    EA::TDF::tdf_ptr<GameManager::PackerCreateGameRequest> createGameRequest = BLAZE_NEW GameManager::PackerCreateGameRequest;
    ...
    for (auto partyIndex = 0; partyIndex < partyCount; ++partyIndex)
    {
        ...
        if (packerSessionItr == mPackerSessions.end())
        {
            ...
            // Design - Since this should never happen, just output an ERR_LOG and skip the missing party.  Packer should be informed that the Party is missing, and remove it. 
            continue;
        }
        auto& packerSession = *packerSessionItr->second;
        if (party->mImmutable)
        {
            ...
            TRACE_LOG("[GamePackerSlaveImpl].handlePackerReapGame: queue cleanup for placeholder session(" << partyId << ") in packerScenario(" << packerSession.getPackerScenarioId() << ") from game(" << game.mGameId << ") prior to finalization");
            packerSession.mState = PackerSessionSlave::AWAITING_RELEASE;
            packerSession.setCleanupReason(GameManager::WorkerSessionCleanupReasonToString(GameManager::WORKER_SESSION_COMPLETED));
        ...
        if (pendingSessions.empty())
        {
            ...
            if (templateItr != getConfig().getCreateGameTemplatesConfig().getCreateGameTemplates().end())
            {
                ...
                for (auto& factor : packer.getFactors())
                {
                    ...
                    if (blaze_strnicmp(inputPropertyName, GameManager::PROPERTY_NAME_PING_SITE_LATENCIES, strlen(GameManager::PROPERTY_NAME_PING_SITE_LATENCIES)) == 0)
                    {
                        ...
                        auto score = factor.computeSortedFieldScores(fieldScoreList, game);
                        EA_ASSERT(score != Packer::INVALID_SCORE);
            ...
            cgGameRequest.getPlayerJoinData().setGameEntryType(packerSession.getPlayerJoinData().getGameEntryType());
            ...
            cgMasterRequest.setFinalizingMatchmakingSessionId(partyId);
            {
                ...
                packerSession.getCommonGameData().copyInto(cgGameRequest.getCommonGameData());
                packerSession.getExternalSessionData().copyInto(cgMasterRequest.getExternalSessionData());
        ...
        for (auto& partyPlayer : party->mPartyPlayers)
        {
            ...
            auto& playerJoinData = packerSession.getPlayerJoinData();
            for (auto& playerDataFrom : playerJoinData.getPlayerDataList())
            {
                ...
                if (playerDataFrom->getUser().getBlazeId() == partyPlayer.mPlayerId)
                {
                    ...
                    for (auto& userInfoFrom : packerSession.getUsersInfo())
                    {
                        if (userInfoFrom->getUser().getUserInfo().getId() == partyPlayer.mPlayerId)
                        {
                            userInfoFrom->copyInto(*userInfoTo);
                            // Reconstruct, since we split subsession/rest:  (PACKER_TODO - This is duplicating ScenarioAttributes per-player.)
                            packerSession.mPackerScenarioRequest->getScenarioInfo().copyInto(userInfoTo->getScenarioInfoPerUser());  // Duplicate the ScenarioInfo per-player, from the single copy in the source request. 
                            userInfoTo->getScenarioInfoPerUser().setSubSessionName(packerSession.getSubSessionName());       // Since subsession names are held in subsession info, we have to get them here,
            ...
            if (!foundPlayerJoinData)
            {
                ...
                // Design - This indicates that the Party on Packer did not match the PlayerJoinData held in the source request.  We need to inform Packer, and find out where the value changed (so it can remove the Party at the correct place). 
                return;
            }
        }
    }
    ...
    if (pendingSessions.empty())
    {
        ...
        {
            ERR_LOG("Mutable parties exist, must have pending sessions!");  // Dupe of EA_FAIL above with mPackerSessions.find(partyId);  If we're just going to return, we should do it there and inform Packer of the missing Parties.
        }
        return;
    }
    EA_ASSERT_MSG(ownerSession != nullptr, "Finalization owner must exist!");

    TRACE_LOG("[GamePackerSlaveImpl].handlePackerReapGame: PackerSilo(" << packerSiloId << ") initiate finalize session(" << ownerSession->getPackerSessionId() << ")");
    ...
    for (auto& session : pendingSessions)
    {
        ...
        auto packerSessionId = packerSession.getPackerSessionId();
        sendSessionStatus(packerSessionId);
    }

    // calculate overall scoring info and factor details before finalization
    auto scoringAndFactorDetails = getScoringAndFactorDetailsFromGame(game);
    ...
    for (auto& session : pendingSessions)
    {
        ...
        packerSession.mState = PackerSessionSlave::AWAITING_JOB_START;
        packerSession.mFinalizationJobId = finalizationJobId; // link to job
        ++packerSession.mFinalizationAttempts;

        for (auto& sessionSiloInfoItr : packerSession.mMatchedPackerSiloMap)
        {
            auto otherPackerSiloId = sessionSiloInfoItr.first;
        ...
        auto scenarioId = packerSession.getPackerScenarioId();
    ...
    {
        // PACKER_TODO: This pack/schedulereap invocation is making the flow more complex than I would like because it is currently synchronously triggered from within handlePackerReapGame() which is itself a callback already. 
        // It is also less efficient than we would want. What we would ideally want to do is queue silo packing requests to be processed at the end of this phase to avoid spending CPU time immediately before kicking off the job below 
        // (which will block several times and thus should be less impacted by the CPU overhead from repacking other silos inline.
    ...
    submitJob.mInitiatorPackerSessionId = ownerSession->getPackerSessionId();
    submitJob.mInitiatorPackerScenarioId = ownerSession->getPackerScenarioId();
    ...
    const float wgeomean = ownerSession->mScoringAndFactorDetails->getOverallScoringInfo().getGameQualityScore();
...
void GamePackerSlaveImpl::updatePackerReapPartyMetrics(const Packer::Game& game, const Packer::Party& party, bool expired)
{
    auto& packer = game.mPacker;
    ...
    if (packerDetailedMetricsItr == mPackerDetailedMetricsByGameTemplateMap.end())
    {
        // We don't remove entries from the mPackerDetailedMetricsByGameTemplateMap (even for reconfigs that remove cg templates), so check if the SiloInfo for this PackerSilo is missing somehow.
        PackerSiloInfo* siloInfo = getPackerSiloInfo((GameManager::PackerSiloId)packer.getPackerSiloId());
        ERR_LOG("[GamePackerSlaveImpl].handlePackerReapGame: PackerSiloName(" << packer.getPackerName() << ") did not have any detailed metrics mapped.  SiloInfo " << (siloInfo == nullptr ? "MISSING" : "FOUND") << " for silo" << packer.getPackerSiloId() << "), game(" << game.mGameId << ")");
        return;
    }
    auto& detailedMetrics = packerDetailedMetricsItr->second;
...
{
    updatePackerReapPartyMetrics(game, party, expired);
    if (expired)
    {
        ...
        EA_ASSERT_MSG(packerSessionItr != mPackerSessions.end(), "PackerSessionSlave owns the party, must exist!");
        auto& packerSession = *packerSessionItr->second;
        ...
        if (packerSession.mDelayCleanupTimestamp == 0) // NOTE: Only add to delayed cleanup queue once (same partyId is used within all silos that need to clean up this party)
        {
            packerSession.mDelayCleanupTimestamp = TimeValue::getTimeOfDay();
            packerSession.mState = PackerSessionSlave::AWAITING_RELEASE;
            packerSession.setCleanupReason(GameManager::WorkerSessionCleanupReasonToString(GameManager::WORKER_SESSION_EXPIRED));
...
void GamePackerSlaveImpl::finalizePackedGame(PackerFinalizationJobId jobId)
{
    ...
    if (!isJobValid(job, PackerSessionSlave::AWAITING_JOB_START))
    {
    ...
    EA_ASSERT(gmMaster != nullptr);  // GM Master should always exist, or Packer can't work. 
    ...
    for (auto& packerSessByScenarioIdItr : job.mPackerSessionByScenarioIdMap)
    {
        ...
        auto packerSessionId = jobPackerSession.getPackerSessionId();
        ...
        req.setPackerScenarioId(jobPackerSession.getPackerScenarioId());

        jobPackerSession.mState = PackerSessionSlave::AWAITING_LOCK;
        ...
        if (overallResult == ERR_OK)
        {
            ...
            jobPackerSession.mState = PackerSessionSlave::LOCKED;
        }
        else
        {
            ...
            if (overallResult == GAMEPACKER_ERR_SCENARIO_FINALIZING)
            {
            ...
            {
                if (jobPackerSession.mOrphanStatus == PackerSessionSlave::MUST_TERMINATE)
                {
                ...
                {
                    ERR_LOG("[GamePackerSlaveImpl].finalizePackedGame: " LOG_JOB(job) " unable to lock session(" << packerSessionId << ") in scenario(" << scenarioId << ") while finalizing because owner did not find the session, remaining sessions(" << job.mPackerSessionByScenarioIdMap.size() - 1 << ") will be unlocked (if needed) and resubmitted to packer");
                }
        ...
        auto& callerPropertyMap = jobPackerSession.getPropertyMap();
    ...
    {
        ERR_LOG("[GamePackerSlaveImpl].finalizePackedGame: " LOG_JOB(job) " is missing HostJoinInfo.  This should not be possible (host info should be set via Scenarios when the Session starts, even in cases where no 'real' host exists.)")
    ...
    job.mGmMasterRequest->setTrackingTag(initiatorPackerSession.mPackerScenarioRequest->getTrackingTag()); // PACKER_TODO: this should be set already?
    static const auto MAX_GAME_QUALITY_SCORE = 100; // always 100%
    {
        ...
        setupReasonMap.reserve(initiatorPackerSession.getGameCreationData().getMaxPlayerCapacity());
        ...
        for (auto& packerSessByScenarioIdItr : job.mPackerSessionByScenarioIdMap)
        {
            ...
            auto& packerSessionMmRequest = *packerSession.mPackerScenarioRequest;
            auto gameEntryType = packerSession.getPlayerJoinData().getGameEntryType();
            auto groupId = packerSession.getPlayerJoinData().getGroupId();
    ...
    if (isProvisionalGame)
    {
        ...
        if (isDedicatedHostedTopology(networkTopology))
        {
            if (privilegedGameIdPreference != GameManager::INVALID_GAME_ID)
            {
                ...
                overallResult = gmMaster->packerCreateGameWithPrivilegedId(*job.mGmMasterRequest, job.mGmMasterResponse);
            ...
            else
            {
                ...
                overallResult = findDedicatedServerHelper(
                    ...
                    &initiatorPackerSession.mPackerSessionRequest->getFindDedicatedServerRulesMap(),
                    ...
                    if (!gamesToSend.empty())
                    {
                        ...
                        for (auto gameIdsMapItr = gamesToSend.rbegin(), gameIdsMapEnd = gamesToSend.rend(); gameIdsMapItr != gameIdsMapEnd; ++gameIdsMapItr)
                        {
                            ...
                            while (!gameIds->empty())
                            {
                                ...
                                before = TimeValue::getTimeOfDay();
                                BlazeError err = gmMaster->packerCreateGame(*job.mGmMasterRequest, job.mGmMasterResponse, opts);
                                job.mFinStageTimes[JOB_CREATE_GAME] += TimeValue::getTimeOfDay() - before;
                                if (err == ERR_OK)
                                {
        ...
        else
        {
            ...
            before = TimeValue::getTimeOfDay();
            overallResult = gmMaster->packerCreateGame(*job.mGmMasterRequest, job.mGmMasterResponse, opts);
            job.mFinStageTimes[JOB_CREATE_GAME] += TimeValue::getTimeOfDay() - before;
            if (overallResult == ERR_OK)
            {
    ...
    else  // Active Game Logic:
    {
        // game found, attempt to join it
        ...
        // We don't have to duplicate the ScenarioInfo here since the mGmMasterRequest already did the duplication in handlePackerReapGame
        ...
        initiatorPackerSession.getCommonGameData().copyInto(joinByGroupRequest.getJoinRequest().getCommonGameData());
        initiatorPackerSession.getExternalSessionData().copyInto(joinByGroupRequest.getExternalSessionData());
    ...
    TRACE_LOG("[GamePackerSlaveImpl].finalizePackedGame: " LOG_JOB(job) " successfully created new game(" << gameId << ")");
...
void GamePackerSlaveImpl::completeJob(BlazeError overallResult, PackerFinalizationJobId jobId, GameManager::MatchmakingResult result, GameManager::PlayerRemovedReason reason /*= GameManager::PLAYER_LEFT_CANCELLED_MATCHMAKING*/)
{
    ...
    PackerSessionSlave* initiatorPackerSession = nullptr;
    ...
    if (overallResult == ERR_OK)
    {
        ...
        if (initiatorPackerSessionItr != mPackerSessions.end())
        {
            initiatorPackerSession = initiatorPackerSessionItr->second;
    ...
    else
    {
        switch (overallResult)
        {
        case GAMEPACKER_ERR_SCENARIO_FINALIZING:
        ...
        if (gameId != GameManager::INVALID_GAME_ID)
        {
            ...
            else
            {
                ...
                // b) with explicit non-fatal GM master errors like TEAM_FULL, where we shouldn't need to issue an explicit removal call
                // c) with explicit fatal GM master errors like INVALID_ENTRY_CRITERIA, where we shouldn't need to issue an explicit removal call, but should probably still abort all the sessions and avoid repacking them...
    ...
    for (auto& packerSessByScenarioIdItr : job.mPackerSessionByScenarioIdMap)
    {
        ...
        switch (jobResult)
        {
            case JOB_SUCCESS:
            {
                ...
                request.setPackerSessionId(packerSessionId);
                request.setPackerScenarioId(jobPackerSession.getPackerScenarioId());
                ...
                for (auto userSessionId : jobPackerSession.mUserSessionIds)
                {
                    ...
                    {
                        GameManager::PINEventHelper::addScenarioAttributesToMatchJoinEvent(*(pinEventsMapItr->second), initiatorPackerSession->getAttributeMap());
                    }
                }

                request.setPinSubmission(pinSubmission);    // Note: This just sets the TdfPtr held internally.  pinSubmission must persist until request goes away. 
                jobPackerSession.populatePinData(request.getPinData(), job.mGamePackerSiloId);

                scoreBucket = getBucketFromScore(jobPackerSession.mScoringAndFactorDetails->getOverallScoringInfo().getGameQualityScore());
                ...
                jobPackerSession.mState = PackerSessionSlave::AWAITING_RELEASE;
                break;
            }
            case JOB_RESUMABLE_ERROR:
            {
                if (jobPackerSession.mOrphanStatus != PackerSessionSlave::NONE)
                {
                    ...
                    jobPackerSession.mState = PackerSessionSlave::AWAITING_RELEASE;
                    // this session was being allowed to finish up, but no error is resumable in this situation
                    if (jobPackerSession.mOrphanStatus == PackerSessionSlave::CAN_COMPLETE_FINALIZATION)
                    {
                        abortSessionInternal(packerSessionId, jobPackerSession.getPackerScenarioId(), jobPackerSession.getScenarioName(), jobPackerSession.getSubSessionName());
                    }
                }
                else if (jobPackerSession.mState == PackerSessionSlave::AWAITING_LOCK)
                {
                ...
                else if (jobPackerSession.mState == PackerSessionSlave::AWAITING_RELEASE)
                {
                    ...
                    EA_ASSERT(jobPackerSession.mState == PackerSessionSlave::AWAITING_RELEASE);
                }
                else
                {
                    if (jobPackerSession.mState == PackerSessionSlave::LOCKED)
                    {
                    ...
                    if (now < jobPackerSession.mSessionExpirationTime)
                    {
                        ...
                        jobPackerSession.mState = PackerSessionSlave::PACKING;

                        for (auto& sessionSiloInfoItr : jobPackerSession.mMatchedPackerSiloMap)
                        {
                            auto packerSiloId = sessionSiloInfoItr.first;
                            if (!addSessionToPackerSilo(jobPackerSession.getPackerSessionId(), packerSiloId))
                            {
                                ERR_LOG("[GamePackerSlaveImpl].completeJob: Failed to readd packer session " << jobPackerSession.getPackerSessionId() << " to Silo "<< packerSiloId <<" on job completion!  Check if session was properly removed the first time.");
                            }
                            else 
                            {
            ...
            case JOB_FAILURE:
            {
                if (jobPackerSession.mOrphanStatus == PackerSessionSlave::MUST_TERMINATE)
                {
                    // MUST_TERMINATE means the subsession was already cleaned up master-side
                    cleanupReason = GameManager::WORKER_SESSION_TERMINATED;
        ...
        if (jobPackerSession.mState == PackerSessionSlave::AWAITING_RELEASE)
        {    
    ...
    // Drain the server while not reaping or finalizing
    drainIfNeeded();
}
...
void GamePackerSlaveImpl::utilPackerUpdateMetric(Packer::PackerSilo& packer, Packer::PackerMetricType type, int64_t delta)
{
    ...
    case Packer::PackerMetricType::GAMES:
        if (delta > 0)
            mGamesCreatedCounter.increment(delta, packer.getPackerName().c_str());
        mPackerGamesGauge.increment(delta, packer.getPackerName().c_str());
        break;
    case Packer::PackerMetricType::VIABLE_GAMES:
        mPackerViableGamesGauge.increment(delta, packer.getPackerName().c_str());
        break;
    case Packer::PackerMetricType::PARTIES:
        mPackerPartiesGauge.increment(delta, packer.getPackerName().c_str());
        break;
    case Packer::PackerMetricType::PLAYERS:
        mPackerPlayersGauge.increment(delta, packer.getPackerName().c_str());
        break;
    default:
        ERR_LOG("[GamePackerSlaveImpl].utilPackerUpdateMetric: Unhandled type, no metrics added!");
    }
}
...
int32_t GamePackerSlaveImpl::utilPackerRand()
{
    return Random::getRandomNumber(Random::MAX);
}
...
bool GamePackerSlaveImpl::unmarkPackerSiloForRemoval(PackerSiloId packerSiloId)
{
    if (mPackersMarkedForRemoval.erase(packerSiloId) > 0)
    {
        ...
        TRACE_LOG("[GamePackerSlaveImpl].unmarkPackerSiloForRemoval: PackerSilo(" << packerSiloId << ") reinstated, remaining removal list size(" << mPackersMarkedForRemoval.size() << ")");
        return true;
    }
    return false;
}
...
{
    // PACKER_TODO: Inserting many unordered metrics into the TdfVectorMap can be *very* inefficient O(N^2); therefore, we should use a temporary map<> to sort all the metrics, then populate the TdfVectorMap
...
#define ADD_VALUE(name, value) { parentStatusMap[ (tempNameString << name).get() ]= (tempValueString << value).get(); tempNameString.reset(); tempValueString.reset(); }
    ADD_VALUE("AssignedLayerId", getAssignedLayerId());
    ADD_VALUE("AssignedSlotId", mAssignedWorkSlotId);
#undef ADD_VALUE
...
bool GamePackerSlaveImpl::initPackerSilo(Packer::PackerSilo& packer, PackerSessionSlave& packerSession, const Blaze::GameManager::CreateGameTemplate& cfg)
{
...
bool GamePackerSlaveImpl::addPackerSilo(PackerSessionSlave& packerSession, PackerSiloId packerSiloId, Blaze::GameManager::Rete::ProductionNode* node)
{
    ...
    if (initPackerSilo(packer, packerSession, *cfgItr))
    {
        ...
        // PACKER_TODO - String maps are inefficient, given that we have things like SiloIds that could be used for mapping.  (Note that this code wants >1 silos to map to a single set of detailed metrics.)
...
bool GamePackerSlaveImpl::isJobValid(GamePackerSlaveImpl::FinalizationJob& job, PackerSessionSlave::PackerSessionState state) const
{
    for (auto& packerSessByScenarioIdItr : job.mPackerSessionByScenarioIdMap)
    {
        auto& packerSession = *packerSessByScenarioIdItr.second;
        if ((packerSession.mOrphanStatus > PackerSessionSlave::CAN_COMPLETE_FINALIZATION) || packerSession.mState != state)
        {
            return false;
        }
    }
...
void GamePackerSlaveImpl::onNotifyWorkerPackerSessionAvailable(const Blaze::GameManager::NotifyWorkerPackerSessionAvailable& data, ::Blaze::UserSession *)
{
    ...
    {
        TRACE_LOG("[GamePackerSlaveImpl].onNotifyWorkerPackerSessionAvailable: Ignoring new work available on layer(" << getLayerIdFromSlotId(mAssignedWorkSlotId) << ").  We are shutting down/draining.");
        return;
    }
...
void GamePackerSlaveImpl::onNotifyWorkerPackerSessionTerminated(const Blaze::GameManager::NotifyWorkerPackerSessionTerminated& data, ::Blaze::UserSession *)
{
    ...
    if (packerSession.mFinalizationJobId != 0)
    {
        ...
        switch (reason)
        {
        case GameManager::PackerScenarioData::SCENARIO_SYSTEM_TERMINATED:
            packerSession.mOrphanStatus = PackerSessionSlave::CAN_COMPLETE_FINALIZATION;
            break;
        default:
            packerSession.mOrphanStatus = PackerSessionSlave::MUST_TERMINATE;
            break;
        }
        // PACKER_TODO: If the Finalization job fiber ends up taking an exception(which prevents cleaning it up) we should validate that the job still exists, and then perform a forced cleanup on it here instead of returning...
        //    Or better yet, let's remove all the exceptions and replace them with ERR logs so we don't have to worry about random partial execution of functions.
        return;
    }

    INFO_LOG("[GamePackerSlaveImpl].onNotifyWorkerPackerSessionTerminated: session(" << packerSessionId << "), packerScenario(" << packerSession.getPackerScenarioId() << ") terminated by owner, reason(" << GameManager::PackerScenarioData::TerminationReasonToString(reason) << "), cleanup session"); // PACKER_TODO: trace?
...
{
    // Once we tell the master that a drain is being done, we don't need to worry about the sessions anymore. 
    return mDrainMessageSent;  // mPackerSessions.empty();
}

void GamePackerSlaveImpl::onControllerDrain()
{
    // Tell all of the Masters that we are draining.   They will remove us, and move all our owned sessions to Unassigned. 

    // [Request for Drain RPC] --> [Controller marks isDraining() == true] --> [Worker stops Reaping games, Stops obtaining new Games]
    //                         --> [Worker WAITS for all current Reaping/Finalizing to complete (mFinalizationJobs), THEN]
    //                            --> [Worker informs Masters] --> [Master moves all sessions to Unassigned, removes Worker] 
    //                         --> [Worker removes all sessions, shuts down]
    // (Drain attempt done in 3 places - onControllerDrain, after reapGames completes, after finalizeJob completes.)
    if (mFinalizationJobs.empty() && !mIsReapingGames)
    {
        WARN_LOG("[GamePackerSlaveImpl].onControllerDrain: Drain delayed due to (" << mFinalizationJobs.empty() << ") active finalization jobs, or (" << mIsReapingGames << ") currently reapping games.");
    }

    drainIfNeeded();
}

void GamePackerSlaveImpl::drainIfNeeded()
{
    if (!gController->isDraining() || mDrainMessageSending)
        return;

    // We don't want to trigger the drain while we are currently finalizing or reaping Games. 
    if (mFinalizationJobs.empty() && !mIsReapingGames)
    {
        mDrainMessageSending = true;
        TRACE_LOG("[GamePackerSlaveImpl].drainIfNeeded:  Draining server while reaping is inactive and no finalization jobs exist.");

        Component::InstanceIdList gmInstances;
        getMaster()->getComponentInstances(gmInstances, true);
        Component::MultiRequestResponseHelper<void, void> responses(gmInstances);
        ...
        auto err = getMaster()->sendMultiRequest(Blaze::GamePacker::GamePackerMaster::CMD_INFO_WORKERDRAININSTANCE, nullptr, responses);
        if (err == ERR_OK)
        {
            for (auto& respItr : responses)
            {
                ...
                {
                    // This could lead to orphaned sessions, but should be handled when the Master notices that the worker is missing. 
                    WARN_LOG("[GamePackerSlaveImpl].claimWorkSlotIdAndNotifyMasters: Failed to send WorkerDrain to  master(" << respItr->instanceId << "), error(" << ErrorHelp::getErrorName(respItr->error) << ").");
                }
            }
        }
        else
        {
            ERR_LOG("[GamePackerSlaveImpl].claimWorkSlotIdAndNotifyMasters: Failed sending multi-request to drain worker, error(" << ErrorHelp::getErrorName(err) << ").");
        }

       mDrainMessageSent = true;
    }
}
...
bool GamePackerSlaveImpl::onValidateConfig(GamePackerConfig& config, const GamePackerConfig* referenceConfigPtr, ::Blaze::ConfigureValidationErrors& validationErrors) const
{
    ...
    GameManager::ExternalSessionUtilManager externalGameSessionServices;
    ...
    if (!GameManager::ExternalSessionUtil::createExternalSessionServices(externalGameSessionServices, config.getGameSession().getExternalSessions(), config.getGameSession().getPlayerReservationTimeout()))
    {
        return false;   // (err already logged)
    }

    if (!mReteNetwork->validateConfig(config.getRete(), &(referenceConfigPtr->getRete()), validationErrors))
    {
        ERR_LOG("[GamePackerSlaveImpl].onValidateConfig: Failed to validate rete configuration.");
        return false;
    }

    GameManager::Rete::StringTable stringTable;
    Blaze::Metrics::MetricsCollection metricsCollection;
    GameManager::Matchmaker::RuleDefinitionCollection collection(metricsCollection, stringTable, GamePackerSlave::LOGGING_CATEGORY, true);
    if (!collection.initFilterDefinitions(config.getFilters(), config.getGlobalFilters(), config.getProperties()))
    {
        ERR_LOG("[GamePackerSlaveImpl].onValidateConfig: Failed to initialize filter rule definition.");
        return false;
    }

    // validate external session config
    eastl::hash_set<eastl::string> templateNameSet;
    for (auto& templateItr : config.getCreateGameTemplatesConfig().getCreateGameTemplates())
    {
        ...
        for (auto& curPacker : packerConfig.getQualityFactors())
        {
            ...
            GameManager::AttributeToTypeDescMap attrToTypeDesc;
            GameManager::TdfIdHashSet acceptableTdfs = { Blaze::GameManager::DefaultShaper::TDF_ID };
            ...
            const GameManager::PropertyName& propertyName = curPacker->getGameProperty();
        ...
        auto ret = templateNameSet.insert(templateName.c_str());
        if (!ret.second)
        {

            StringBuilder sb;
            sb << "[GamePackerSlaveImpl].onValidateConfig: Duplicate template(" << templateName << ")";
            validationErrors.getErrorMessages().push_back(sb.get());
            return false;
        }
    }
...
bool GamePackerSlaveImpl::onConfigure()
{
    ...
    // Temp Sanity Check to ensure that the RequiredPackerGameProperties were set in the properties config. 
    if (mPropertyManager.getRequiredPackerGameProperties().empty())
    {
        ERR_LOG("[GamePackerSlaveImpl].onConfigure: RequiredGamePackerProperties were empty.");
        return false;
    }
    ...
    for (auto& templateItr : createTemplatesMap)
    {
        ...
        const auto& gameQualityFactors = templateItr.second->getPackerConfig().getQualityFactors();

        if (gameQualityFactors.empty())
            continue; // if there are no quality factors then we don't need to do filtering!
        
        for (const auto& gqf : gameQualityFactors)
        {
            eastl::string cgrAttribute = mPropertyManager.getMappedCgrAttribute(gqf->getGameProperty());
            if(!cgrAttribute.empty())
            {
                //check if the attribute is used in CreateGameRequst.GameCreationData
                bool found = false;
                for (const auto& attribute : templateItr.second->getAttributes())
                {
                    if (blaze_stricmp(attribute.first.c_str(), cgrAttribute.c_str()) == 0)
                    {
                        found = true;
                        break;
                    }
                ...
                if (!found)
                {
                    INFO_LOG("[GamePackerSlaveImpl].onConfigure: GameProperty (" << gqf->getGameProperty() <<
                        ") from GameQualityFactor is not used for GameCreationData in CreateGameTemplate: " << createGameTemplateName);
                }
            }
        }
...
bool GamePackerSlaveImpl::onReconfigure()
{
    ...
    // remove all the sessions that aren't finalizing
    auto packerSessionIter = mPackerSessions.begin();
    while (packerSessionIter != mPackerSessions.end())
    {
        auto& packerScenario = *packerSessionIter->second;
        // iterate here because we may subsequently invalidate it by terminating the session
        packerSessionIter++;
        if (packerScenario.mFinalizationJobId == 0)
        {
            // pass in false, as there's no reason to attempt a repack here. we're cleaning things up!
            cleanupSession(packerScenario.getPackerSessionId(), GameManager::WORKER_SESSION_TERMINATED, false);
        }
    }
    ...
    // add every silo to the sweep list
    for (auto& siloItr : mPackerSiloInfoMap)
    {
        mPackersMarkedForRemoval.insert(siloItr.first);
    }
    ...
    // destroy outstanding GB lists
    // this should be a no-op, as removing all the silos and WMEs should have already handled this, but be certain! 
    // search component doesn't *really* need the notification, as a search reconfigure tears these down on the other side.
    cleanupAllGameQueries();

    // clean up the property manager
    mPropertyManager.onReconfigure(config.getProperties());
    // clear out the rete network
    delete mReteNetwork;
    mReteNetwork = nullptr;
    ...
    if (!mReteNetwork->configure(config.getRete()))
    {
        ERR_LOG("[GamePackerSlaveImpl].onReconfigure: problem while parsing Rete Network config file.");
    ...
    if (!mRuleDefinitionCollection->initFilterDefinitions(config.getFilters(), config.getGlobalFilters(), config.getProperties()))
    {
        ERR_LOG("[GamePackerSlaveImpl].onReconfigure: Failed to initialize filter rule definition.");
        return true;
    }
    ...
    for (auto& templateItr : createTemplatesMap)
    {
        ...
        {
            ERR_LOG("[GamePackerSlaveImpl].onConfigure: Duplicate template(" << createGameTemplateName << ")");
            // keep going, we keep the inital one (onValidate will have caught this)
        }
        else // not a dupe
        {
            ret.first->second = BLAZE_NEW GameTemplate(createGameTemplateName);
...
void GamePackerSlaveImpl::onShutdown()
{
    ...
    // cancel all outstanding timers, stop reaping games

    //Fiber::join(mFiberGroupId); // wait for all scheduled fibers to exit

    // this will assert if any of the remaining sessions have outstanding jobs
...
void GamePackerSlaveImpl::cleanupAllGameQueries()
{
    while (!mActiveGameQueryBySiloIdMap.empty())
    {
        // removed last session from packer silo, silo needs to be marked for removal
        auto gameQueryItr = mActiveGameQueryBySiloIdMap.begin();
        PackerSiloId packerSiloId = gameQueryItr->first;
        ...
        if (gameQueryItr->second.mInitiatorSiloIdSet.erase(packerSiloId) > 0)
        {
            ...
            {
                if (EA_LIKELY(gameBrowserListId != 0 && !gameQueryItr->second.mResolvedSearchSlaveList.empty()))
                {
                    auto* searchSlave = getSearchSlave();
                    if (searchSlave != nullptr)  // SearchSlave should always exist, or Packer can't work.     
                    {
                        // Send the GameList Termination Request:
                        RpcCallOptions opts;
                        opts.ignoreReply = true;
                        ...
                        if (rc == ERR_OK)
                        {
                            TRACE_LOG("[GamePackerSlaveImpl].cleanupAllGameQueries: PackerSilo(" << packerSiloId << ") empty, mark for removal, terminate associated list(" << gameBrowserListId << ")");
                        }
                        else
                        {
                            ERR_LOG("[GamePackerSlaveImpl].cleanupAllGameQueries: PackerSilo(" << packerSiloId << ") empty, mark for removal, terminate associated list(" << gameBrowserListId << "), error(" << ErrorHelp::getErrorName(rc) << ")");
                        }
                    }
                    else
                    {
                        ERR_LOG("[GamePackerSlaveImpl].cleanupAllGameQueries: PackerSilo(" << packerSiloId << ") empty, mark for removal, terminate associated list(" << gameBrowserListId << "), getSearchSlave() returned nullptr.");
                    }
                }
                else
                {
                    ERR_LOG("[GamePackerSlaveImpl].cleanupAllGameQueries: PackerSilo(" << packerSiloId << ") empty, mark for removal, terminate associated list(" << gameBrowserListId 
                        << "), had (" << gameQueryItr->second.mResolvedSearchSlaveList.size() << ") resolved search instances.");
                }
                // erase either way
            ...
            {
                TRACE_LOG("[GamePackerSlaveImpl].cleanupAllGameQueries: PackerSilo(" << packerSiloId << ") empty, mark for removal, deref associated list(" << gameBrowserListId << ")");
            }
        }
        else
        {
            TRACE_LOG("[GamePackerSlaveImpl].cleanupAllGameQueries: PackerSilo(" << packerSiloId << ") empty, matches condition but not tracking active games, mark for removal");
        }

    }
}

void GamePackerSlaveImpl::convertGameListToPackerActiveGameData(const Blaze::Search::NotifyGameListUpdate& data, ActiveGameDataHashMap& gameDataIndex)
{
    ...
    else
    {
        ...
        if (gameDataItr != data.getGameDataMap().end())
        {
            ...
            {
                // Convert all Property data into the types known to Packer (TDF)
                // PACKER_TODO - (Discuss) An alternative would be to just send this data type back directly, rather than doing the Properties conversion. 
                EA::TDF::TdfPtr tempPtr;
                if (!mPropertyManager.convertPropertiesToPackerActiveGameData(tempPtr, gameData->getPropertyNameMap()))
                {
                    WARN_LOG("[GamePackerSlaveImpl].onNotifyGameListUpdate: Update to decode ActiveGameData!");
                    continue;
                }

                ActiveGameDataPair dataPair;
                dataPair.first =*(GameManager::PackerActiveGameDataPtr*)&tempPtr;
                dataPair.second = &gameData->getPropertyNameMap();

                if (dataPair.first->getGameId() == GameManager::INVALID_GAME_ID)
                {
                    WARN_LOG("[GamePackerSlaveImpl].onNotifyGameListUpdate: Missing GameId property!");
                    continue;
                }

                gameDataIndex.insert(eastl::make_pair(dataPair.first->getGameId(), dataPair));
            }
        }
        else
        {
...
uint32_t GamePackerSlaveImpl::unpackGameAndCleanupPlaceholderSessions(const Packer::Game* packerGame, GameManager::WorkerSessionCleanupReason reason)
{
    ...
    Packer::GamePartyList copiedGameParties(packerGame->mGameParties.begin(), packerGame->mGameParties.begin() + partyCount); // need to copy here because removeSessionFromPackerSilo destroys packerGame...
    ...
    for (auto& existingGameParty : copiedGameParties)
    {
        ...
        if (existingGameParty.mImmutable)
        {
            ...
            TRACE_LOG("[GamePackerSlaveImpl].unpackGameAndCleanupPlaceholderSessions: remove placeholder session(" << existingPackerSessionId << ") before " << reason << " active game(" << gameId << ")");
        ...
        else
        {
            ...
            TRACE_LOG("[GamePackerSlaveImpl].unpackGameAndCleanupPlaceholderSessions: requeue session(" << existingPackerSessionId << ") prior to repacking");
        }
    }
...
{
    // PACKER_TODO: annoyingly we don't have a way to quickly look up added/removed games from list here, therefore we need to first index all the games.
    // At the same time we convert the GameDataMap to the Packer data we use:
    ActiveGameDataHashMap gameDataIndex;
    convertGameListToPackerActiveGameData(data, gameDataIndex);

    // PACKER_TODO: need to handle a game list update that has isFinalUpdate set to true-
    // Search component may reconfigure without packer (or individual instances go down), and we need to have full coverage for accurate matchmaking
    // in case of final update, we should probably schedule a timer to cleanup the lists and recreate, which will be cancelled if the silo goes away
    for (auto& listUpdate : data.getGameListUpdate())
    {
        ...
        GameTemplatePtr gameTemplate = activeGameQueryItr->second->mGameTemplate;
        ...
        for (auto packerSiloId : activeGameQueryItr->second->mInitiatorSiloIdSet)
        {
            ...
            if (packerSilo == nullptr)
            {
                // Currently, silos are only destroyed by onTokenRemoved, which should clean up the ActiveGameQuery list.   If that changes, and more removal locations are added, consider updating the list in removePackerSilo(). 
                ERR_LOG("[GamePackerSlaveImpl].onNotifyGameListUpdate: GameTemplate(" << gameTemplateName << "), game query(" << gameListId << ") does not have a valid PackerSilo for id " << packerSiloId << "!");
                continue;
            }
            ...
            for (auto& matchedGameList : matchedGameLists)
            {
                ...
                for (auto matchedGameItr = matchedGameList->begin(), matchedGameEnd = matchedGameList->end(); matchedGameItr != matchedGameEnd;)
                {
                    ...
                    if (gameItr == gameDataIndex.end())
                    {
                        ERR_LOG("[GamePackerSlaveImpl].onNotifyGameListUpdate: PackerSilo(" << packerSiloId << ") game query(" << gameListId << ") matched(" << incomingListType << ") active game(" << incomingGameId << ") was not found in the ActiveGameDataHashMap gameDataIndex generated from the PropertyManager, skipping due to invalid data.");
                        continue;
                    }
                    ...
                    auto& activeGameData = *gameItr->second.first;
                    auto& propertyNameMap = *gameItr->second.second;


                    uint64_t gameRecordVersion = activeGameData.getGameVersion();
                    ...
                    if (packerGame == nullptr)
                    {
                        ...
                        GameManager::UserGroupIdList& userGroupIds = activeGameData.getUserGroupIds();
                        GameManager::TeamIndexList& teamIndexes = activeGameData.getTeamIndexes();
                        GameManager::PlayerIdList& playerIds = activeGameData.getParticipantIds();
                        UserSessionIdList& sessionIds = activeGameData.getUserSessionIds();
                        ...
                        eastl::vector_map<GameManager::UserGroupId, PackerSessionSlave*> immutableSessionByGroupMap;
                        ...
                        for (int32_t userIndex = 0; userIndex < userGroupIds.size(); ++userIndex)
                        {
                            ...
                            if (ret.second)
                            {
                                ...
                                // No ScenarioId to set, since this wasn't from a MM request.
                                PackerSessionSlave& newPackerSession = *(BLAZE_NEW PackerSessionSlave(newPackerSessionId, GameManager::INVALID_PACKER_SCENARIO_ID, gameTemplate));
                            ...
                            ret.first->second->mUserSessionIds.push_back(curSessionId); // PackerSessionSlave takes a permanent ref to UserSessionInfo object
                            ++numActivePlayers;
                        ...
                        for (auto& immutableSessionItr : immutableSessionByGroupMap)
                        {
                            ...
                            auto immutablePackerSessionId = immutableSession->getPackerSessionId();
                            if (!addSessionToPackerSilo(immutablePackerSessionId, packerSiloId))
                            {
                                ERR_LOG("[GamePackerSlaveImpl].onNotifyGameListUpdate: Failed to add immutable session ("<< immutablePackerSessionId << ") to packer silo ("<< packerSiloId <<")!");
                            }
...
void GamePackerSlaveImpl::claimWorkSlotIdAndNotifyMasters()
{
    ...
    if (cluster != nullptr)
    {
        ...
        if (rc == REDIS_ERR_OK)
        {
            ...
            // TODO: If newSlotCount changes enough that the number of layers changes as well, we'll need to update the PackerSessionSlave::mRelinquishTimerId because it was computed based on the number of worker layers.
    ...
    {
        ERR_LOG("[GamePackerSlaveImpl].claimWorkSlotIdAndNotifyMasters: Unable to talk to Redis Cluster for component " << getComponentInfo().baseInfo.name << "!");
    }
...
void GamePackerSlaveImpl::obtainSession(InstanceId instanceId)
{
    ...
    if (rc == ERR_OK)
    {
        auto packerScenarioId = response.getPackerScenarioId();
        auto& packerSessionRequest = response.getPackerSessionRequest();
        auto& packerScenarioRequest = response.getPackerScenarioRequest();

        auto packerSessionId = packerSessionRequest.getPackerSessionId();
        if (packerSessionId != GameManager::INVALID_PACKER_SESSION_ID)
        {
            EA_ASSERT(!isPlaceholderSessionId(packerSessionId));
            ...
            const auto& usersInfo = packerScenarioRequest.getUsersInfo();

            LogContextOverride logAudit(packerScenarioRequest.getOwnerUserSessionInfo().getSessionId());

            const GameManager::ScenarioName& scenarioName = packerScenarioRequest.getScenarioInfo().getScenarioName();
            const GameManager::SubSessionName& subSessionName = packerSessionRequest.getSubSessionName();
            mSessionsAcquired.increment(1, scenarioName, subSessionName);
            
            const auto* createGameTemplateName = packerSessionRequest.getCreateGameTemplateName();
            const auto gameTemplateItr = mGameTemplateMap.find(createGameTemplateName);
            if (gameTemplateItr != mGameTemplateMap.end())
            {
                GameTemplatePtr gameTemplate = gameTemplateItr->second;
                ...
                if (response.getExpiryDeadline() >= cutoff)
                {
                    ...
                    {
                        // PACKER_TODO = Maybe fix allowref to actually store data via a Ptr rather than a *   
                        auto& packerSession = *(BLAZE_NEW PackerSessionSlave(packerSessionId, packerScenarioId, gameTemplate, packerSessionRequest.clone(), packerScenarioRequest.clone()));
                        ...
                        packerSession.mUserSessionIds.reserve(usersInfo.size());
                        for (auto& joinInfoItr : usersInfo)
                        {
                        ...
                        {
                            mSessionsActive.increment(1, scenarioName, subSessionName);
                            
                            packerSession.mParentMetricsSnapshot.mTotalNewPartyCount = gameTemplate->mMetrics.mTotalNewPartyCount++;
                            TRACE_LOG("[GamePackerSlaveImpl].obtainSession: Started session(" << packerSessionId << "), packerScenario(" << packerScenarioId << "), on layer(" << getLayerIdFromSlotId(mAssignedWorkSlotId) << ")");
                        ...
                        {
                            ERR_LOG("[GamePackerSlaveImpl].obtainSession: Failed to start session(" << packerSessionId << "), packerScenario(" << packerScenarioId << ") on layer(" << getLayerIdFromSlotId(mAssignedWorkSlotId) << "), failed to initialize matchmaking criteria: " << criteriaError.getErrMessage());
                            cleanupSession(packerSessionId, GameManager::WORKER_SESSION_CREATE_FAILED);
                            abortSession(packerSessionId);
                        }
                    }
                    else
                    {
                        ERR_LOG("[GamePackerSlaveImpl].obtainSession: Failed obtain property rule definition for session(" << packerSessionId << "), packerScenario(" << packerScenarioId << ")");
                        abortSessionInternal(packerSessionId, packerScenarioId, scenarioName, subSessionName);
                        // PACKER_TODO: handle this error below by returning the session to the master.
                    }
                }
                else
                {
                    WARN_LOG("[GamePackerSlaveImpl].obtainSession: session(" << packerSessionId << ") ttl(" << (now - response.getExpiryDeadline()).getMillis() << "ms) below threshold(" << minTTL.getMicroSeconds() << "), packerScenario(" << packerScenarioId << "), on layer(" << getLayerIdFromSlotId(mAssignedWorkSlotId) << "), refusing session back to owner.");
                    abortSessionInternal(packerSessionId, packerScenarioId, scenarioName, subSessionName);
                }
            }
            else
...
bool GamePackerSlaveImpl::addSessionToPackerSilo(PackerSessionId packerSessionId, PackerSiloId packerSiloId)
{
    ...
    if (packerSessionItr == mPackerSessions.end())
    {
        ERR_LOG("[GamePackerSlaveImpl].addSessionToPackerSilo: PackerSilo(" << packerSiloId << ") already has entry for PackerSessionSlave (" << packerSessionId << ").  Check if the value was properly removed first, if it's being readded.");
        return false;
    }
    ...
    {
        // This indicates that the config changed, but we didn't remove the PackerSessions that we using the older template.  We should remove them if the config changes (or handle it).
        ERR_LOG("[GamePackerSlaveImpl].addSessionToPackerSilo: GamePacker sessions uses invalid CG template name(" << gameTemplateName << "), for PackerSilo(" << packerSiloId << ") session id(" << packerSessionId << "),");
        return false;
    }
    auto* packerSilo = getPackerSilo(packerSiloId);
    if (packerSilo == nullptr)
    {
        ERR_LOG("[GamePackerSlaveImpl].addSessionToPackerSilo: internal error: unexpected state, no packer silo for id "<< packerSiloId <<". Unable to submit to packer");
        return false;
    }
    ...
    {
        if (packerSession.mMatchedPackerSiloMap.find(packerSiloId) == packerSession.mMatchedPackerSiloMap.end())
        {
            ERR_LOG("[GamePackerSlaveImpl].addSessionToPackerSilo: PackerSilo(" << packerSiloId << ") already contains party(" << packerSessionId << "), BUT NO ENTRY EXISTS ON PACKER SESSION.  Check for code for paths where data could get out of sync.")
        }
    ...
    packerSession.mMatchedPackerSiloMap.insert(packerSiloId);
    ...
    for (const auto& qualityFactor : packer.getFactors())
    {
        ...
        else
        {
            ...
            const int32_t playerCount = party.getPlayerCount();
            scoringMapList.reserve(playerCount);        // Have to reserve before setting any values, since reserve() clears the list.
            ...
            // Special case for the 'actually a per-game/mm request scoring map, but we have to store the scoring map per-player' problem
            if (scoringMapList.size() == 1 && playerCount > 1)
            {
                auto& partyScoringMap = *scoringMapList.begin(); // one scoring map shared by all players
                // Start a 1 to skip the first entry which is already there.
                for (int32_t i = 1; i < playerCount; ++i)
                {
                    scoringMapList.push_back(partyScoringMap);  // Each element of the list points to the same ref-counted value. 
                }
            }
...
void GamePackerSlaveImpl::removeSessionFromPackerSilo(PackerSessionSlave& packerSession, Packer::PackerSilo& packerSilo, const char8_t* removalReason)
{
    ...
    auto partyId = packerSession.getPackerSessionId();
    auto matchedItr = packerSession.mMatchedPackerSiloMap.find(packerSilo.getPackerSiloId());
    EA_ASSERT(matchedItr != packerSession.mMatchedPackerSiloMap.end());
...
void GamePackerSlaveImpl::reapGames(PackerSiloId packerSiloId)
{
    if (gController->isShuttingDown() || gController->isDraining())
    {
        ERR_LOG("[GamePackerSlaveImpl].reapGames: Controller is shutting down/draining.  No new Games will be packed, since sessions are being unassigned from this worker.");
        return;
    }
    ...
    mIsReapingGames = true;
    packerSilo.reapGames(this); // PACKER_TODO: Note currently reap can actually result in packGames getting invoked! UUGH! This is due to the poor choice of using callbacks to trigger events from the packer. 
      // We need to fix all this with our shiny new design by buffering state instead of invoking callbacks, and using an explicit pumping loop and breaking all operations of significance into atomic units. IE: 
    ...
    while (!mDelayCleanupSessions.empty())
    {
        ...
        for (auto packerSessionId : delayCleanupSessions)
        {
            ...
            cleanupSession(packerSessionId, GameManager::WORKER_SESSION_COMPLETED); // PACKER_TODO: #SIMPLIFY This is necessary for now to avoid the complexity involved with trying to destroy PackerSessionSlave directly from within PackerSilo::reapGames() and cleaning up the associated Packer::Party,Packer::Game,Packer::Player objects in *all* the corresponding silos while correctly updating metrics... Solution: Do not use callbacks!!!! Change to multi-pass approach.
        }
    }

    // Drain the server while not reaping
    drainIfNeeded();
}
...
bool GamePackerSlaveImpl::onTokenAdded(const GameManager::Rete::ProductionToken& token, GameManager::Rete::ProductionInfo& info)
{
    ...
    // Lookpu the PackerSilo Id Based on the ProdNode 
    ...
    PackerSiloId packerSiloId = isNewProductionNode ? GameManager::INVALID_PACKER_SILO_ID : iter->second;

    {
        ...
        {
            TRACE_LOG("[GamePackerSlaveImpl].onTokenAdded: PackerSilo("<< packerSiloId <<") ProdNodeId(" << prodNodeId << ") referenced by PackerSessionSlave(" << packerSessionId << ") is not terminal, skip.");
            return true;
        }
        ...
        for (auto& orConditionList : conditionBlock)
        {
            ...
            {
                TRACE_LOG("[GamePackerSlaveImpl].onTokenAdded: PackerSilo(" << packerSiloId << ") ProdNodeId(" << prodNodeId << ") conditions don't meet PackerSessionSlave(" << packerSessionId << ") filter criteria, skip.");
                return true;
            }
        }

        TRACE_LOG("[GamePackerSlaveImpl].onTokenAdded: PackerSilo(" << packerSiloId << ") ProdNodeId(" << prodNodeId << ") referenced by PackerSessionSlave(" << packerSessionId << ") is a terminal node, continue.");
    }

    // Attempt to add the new PackerSilo (if one didn't already exist):
    if (isNewProductionNode)
    {
        ...
        if (errRet == ERR_OK)
        {
            ...
            {
                ERR_LOG("[GamePackerSlaveImpl].onTokenAdded: PackerSilo(" << packerSiloId << ") ProdNodeId(" << prodNodeId << ") referenced by PackerSessionSlave(" << packerSessionId << ") was unable to add a Packer Silo.");
                return true;
            }
        }
        else
    ...
    if (packerSilo == nullptr)
    {
        ERR_LOG("[GamePackerSlaveImpl].onTokenAdded: Getting here indicates that a PackerSilo was mapped to the ProdNodeId, but has been removed somehow. ");
        return true;
    }
    ...
    bool queryActiveGames = packerSession.isFindGame(); 
    if (queryActiveGames)
    {
        ...
        if (inserted)
        {
            ...
            if (error != ERR_OK)
            {
                ...
                abortSession(packerSessionId); // PACKER_TODO: add abort reason, in this case because we couldn't allocate an active query gb list id, in theory the CREATE flow should still proceed (we'll need to check if we have a CREATE option and avoid aborting in this case...)
                // getId already outputs an ERR log, so we don't need more here. 

                // PACKER_TODO - DANGER abortSession doesn't remove the packerSession itself, so that data is still sitting in the Map. 
                return true;
            }
            ...
            auto* searchSlave = getSearchSlave();
            EA_ASSERT(searchSlave != nullptr);  // We don't need to handle extreme cases like this. 
            ...
            {
                ERR_LOG("[GamePackerSlaveImpl].onTokenAdded: Error (" << rc << ") returned when attempting to create the GameList on the GameBrowser slaves for PackerSilo(" << packerSiloId << ").");
                // PACKER_TODO:  This is fatal, as it means that the Silo will not gather Active Games.  We need to remove the Silo before it gets used further.
            }
        }
        else
        {
    ...
    if (!result)
    {
        ERR_LOG("[GamePackerSlaveImpl].onTokenAdded: Failed to add session (" << packerSessionId << ") for PackerSilo(" << packerSiloId << ")");

        // PACKER_TODO: Failed to submit mm session to packer.  Add metrics for failure/inform other systems.
        return true;
    }
...
void GamePackerSlaveImpl::onTokenRemoved(const GameManager::Rete::ProductionToken& token, GameManager::Rete::ProductionInfo& info)
{
    ...
    {
        ERR_LOG("[GamePackerSlaveImpl].onTokenRemoved: ProdNodeId(" << productionNodeId << ") not mapped to any PackerSilo - found when removing PackerSessionSlave(" << packerSessionId << ")!");
        return;
    }
    ...
    if (packerSilo == nullptr)
    {
        ERR_LOG("[GamePackerSlaveImpl].onTokenRemoved: PackerSilo(" << packerSiloId << ") not found when removing PackerSessionSlave(" << packerSessionId << ")!");
        return;
    }
    ...
    bool isSessionInSilo = (packerSession.mMatchedPackerSiloMap.find(packerSiloId) != packerSession.mMatchedPackerSiloMap.end());
        ...
        if (gameQueryItr != mActiveGameQueryBySiloIdMap.end())
        {
            if (gameQueryItr->second.mInitiatorSiloIdSet.erase(packerSiloId) > 0)
            {
                ...
                if (gameQueryItr->second.mInitiatorSiloIdSet.empty())
                {
                    ...
                    auto* searchSlave = getSearchSlave();
                    EA_ASSERT(searchSlave != nullptr);  // SearchSlave should always exist, or Packer can't work. 
...
GamePackerSlaveImpl::PackerSessionSlave::PackerSessionSlave(PackerSessionId packerSessionId, PackerScenarioId packerScenarioId, GameTemplatePtr gameTemplate, GameManager::PackerSessionInternalRequestPtr sessionRequest, GameManager::PackerScenarioInternalRequestPtr scenarioRequest)
    : mGameTemplate(gameTemplate)
{
    mPackerScenarioId = packerScenarioId;
    mSessionStartTime = TimeValue::getTimeOfDay();
    if (sessionRequest == nullptr)
    {
        sessionRequest = BLAZE_NEW GameManager::PackerSessionInternalRequest;
        sessionRequest->setPackerSessionId(packerSessionId);
    }
    if (scenarioRequest == nullptr)
    {
        scenarioRequest = BLAZE_NEW GameManager::PackerScenarioInternalRequest;
    }

    mPackerSessionRequest = sessionRequest;
    mPackerScenarioRequest = scenarioRequest;
}

GamePackerSlaveImpl::PackerSessionSlave::~PackerSessionSlave()
{
...
void GamePackerSlaveImpl::PackerSessionSlave::addToReteIndex()
{
    if (mRegisteredWithRete)
    {
        ERR_LOG("[GamePackerSlaveImpl].addToReteIndex: Packer session (id: " << getPackerSessionId() << ") is attempting to add to RETE when already indexed.");
        return;
    }
    ...
    wmeManager.registerMatchAnyWME(getPackerSessionId());
    ...
    gPackerSlaveImpl->getPropertyRuleDefinition()->addPackerSessionWMEs(getPackerSessionId(), getFilterMap(), getMatchmakingPropertyList(), gPackerSlaveImpl->mReteNetwork->getWMEManager());      // Data used in the Filters WMEs should be in the config defined structure.
}

/** triggers removeFromPackerSilo() */
void GamePackerSlaveImpl::PackerSessionSlave::removeFromReteIndex()
{
    if (!mRegisteredWithRete)
        return;
    ...
    wmeManager.unregisterMatchAnyWME(getPackerSessionId());
    if (usingFilters)
    {
        // Since there's only one Rule (PropertyFilter) in use, we just remove all the WMEs here: 
        wmeManager.removeAllWorkingMemoryElements(getPackerSessionId());
    }
    wmeManager.unregisterWMEId(getPackerSessionId());

    gPackerSlaveImpl->mReteNetwork->idle();
}

void GamePackerSlaveImpl::PackerSessionSlave::cancelUpdateTimer()
{
...
void GamePackerSlaveImpl::PackerSessionSlave::cancelMigrateTimer()
{
...
void GamePackerSlaveImpl::PackerSessionSlave::setCleanupReason(const char8_t* reason)
{
...
void GamePackerSlaveImpl::PackerSessionSlave::setRemoveFromSiloReason(const char8_t* reason)
{
    EA_ASSERT(reason != nullptr);
...
Blaze::UserSessionId GamePackerSlaveImpl::PackerSessionSlave::getLogContextOverrideUserSessionId() const
{
    ...
    return mPackerScenarioRequest->getOwnerUserSessionInfo().getSessionId();
}

Blaze::GameManager::UserJoinInfo* GamePackerSlaveImpl::PackerSessionSlave::getHostJoinInfo() const
{
    for (auto& userJoinInfo : mPackerScenarioRequest->getUsersInfo())
    {
...
GameManager::MatchmakingScenarioId GamePackerSlaveImpl::PackerSessionSlave::getOriginatingScenarioId() const
{
    return mPackerScenarioRequest->getScenarioInfo().getOriginatingScenarioId();
}

EA::TDF::TimeValue GamePackerSlaveImpl::PackerSessionSlave::getMigrateUnfulfilledTime() const
{
#if 1
...
GameManager::ScoringAndFactorDetailsPtr GamePackerSlaveImpl::getScoringAndFactorDetailsFromGame(const Packer::Game& game)
{
    GameManager::ScoringAndFactorDetailsPtr scoringAndFactorDetails = BLAZE_NEW GameManager::ScoringAndFactorDetails();
    auto& scoringInfo = scoringAndFactorDetails->getOverallScoringInfo();
    auto& factorDetailsList = scoringAndFactorDetails->getFactorDetailsList();

    const auto& packerSilo = game.mPacker;
    scoringInfo.setBestGameId(game.mGameId);
    scoringInfo.setBestPackerSiloId(packerSilo.getPackerSiloId());

    // get factor scores from the game
    auto& factorScores = scoringInfo.getGameFactorScores();
    if (factorScores.size() != game.mFactorScores.size())
        factorScores.resize(game.mFactorScores.size(), 0.0f);
    int index = 0;
    for (auto score : game.mFactorScores)
    {
        factorScores[index] = score;
        index++;
    }

    // calculate viable factor scores
    auto& viableFactorScores = scoringInfo.getViableFactorScores();
    if (viableFactorScores.size() != factorScores.size())
        viableFactorScores.resize(factorScores.size(), 0.0f);
    for (auto& factor : packerSilo.getFactors())
    {
        auto viableScore = factor.computeViableScore();
        viableFactorScores[factor.mFactorIndex] = viableScore;
    }

    // calculate game quality score and viable quality score
    auto gameQualityScore = calculateWeightedGeometricMean((uint32_t)factorScores.size(), factorScores.begin(), nullptr);
    scoringInfo.setGameQualityScore(gameQualityScore);
    auto viableQualityScore = calculateWeightedGeometricMean((uint32_t)viableFactorScores.size(), viableFactorScores.begin(), nullptr);
    scoringInfo.setViableQualityScore(viableQualityScore);

    // record the highest game quality score achieved by this session over its life
    if (gameQualityScore > scoringInfo.getBestGameQualityScore())
        scoringInfo.setBestGameQualityScore(gameQualityScore);

    // populate factor details list
    factorDetailsList.clear();
    for (auto& factor : packerSilo.getFactors())
    {
        auto gqfInfo = factorDetailsList.pull_back();
        gqfInfo->setFactorName(factor.mFactorName.c_str());
        gqfInfo->setFactorTargetValue(factor.mBestValue);
        gqfInfo->setFactorViableValue(factor.mViableValue);

        Packer::FieldScoreList fieldScoreList;
        auto score = factor.computeSortedFieldScores(fieldScoreList, game);
        if (!fieldScoreList.empty())
        {
            auto& fieldScore = fieldScoreList.at(0);
            if (fieldScore.mFieldDescriptor != nullptr)
                gqfInfo->setFactorFieldValue(fieldScore.mFieldDescriptor->mFieldName.c_str());
            gqfInfo->setFactorEvaluatedValue(factor.getEvaluatedValueFromScore(score));
            gqfInfo->setFactorQualityScore(fieldScore.mScore);
            gqfInfo->setFactorParticipationRatio(fieldScore.mScoreParticipation);
        }
        else
        {
            gqfInfo->setFactorFieldValue("");
            gqfInfo->setFactorEvaluatedValue(factor.getEvaluatedValueFromScore(score));
            gqfInfo->setFactorQualityScore(score);
            gqfInfo->setFactorParticipationRatio(1);
        }
    }
    return scoringAndFactorDetails;
}

void GamePackerSlaveImpl::PackerSessionSlave::populatePinData(PackerPinData& outPinData, PackerSiloId packerSiloId) const
{
    outPinData.setSiloCount(mMatchedPackerSiloMap.size());
    outPinData.setMaxSiloPlayerCount(mMaxSiloPlayerCount);
    outPinData.setEvictionCount(mEvictedCount);
    mScoringAndFactorDetails->getOverallScoringInfo().copyInto(outPinData.getOverallScoringInfo());
    mScoringAndFactorDetails->getFactorDetailsList().copyInto(outPinData.getFactorDetailsList());
    if (packerSiloId == GameManager::INVALID_PACKER_SILO_ID)
    {
        outPinData.setMatchedPlayersInGameCount(mMaxPlayersMatched);
    }
    else
    {
        auto sessionSiloIter = mMatchedPackerSiloMap.find(packerSiloId);
        EA_ASSERT_MSG(sessionSiloIter != mMatchedPackerSiloMap.end(), "This session is expected to be part of the silo identified by 'packerSiloId' parameter!");
        outPinData.setMatchedPlayersInGameCount(sessionSiloIter->second.mPlayersMatched);
    }
}
...
void intrusive_ptr_add_ref(GamePackerSlaveImpl::GameTemplate* ptr) { ++ptr->mRefCount; }
