...
const uint32_t MAX_REDIRECTOR_RETRY_ATTEMPT = 20;
...
void StressInstance::setupForDoTask()
{
    ...
    while (!mConnection->connect())
    {
        ...
        sleep((uint32_t) (10000 * gRandom->getRandomFloat(0.5, 1.5)));
    }
...
bool StressModule::initialize(const ConfigMap& config, const ConfigBootUtil* bootUtil)
{
    ...
    mReconnect = config.getBool("reconnect", true);
