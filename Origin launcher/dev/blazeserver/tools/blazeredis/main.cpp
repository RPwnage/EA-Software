...
static eastl::hash_map<eastl::string, HANDLE> sServerProcessHandleMap;
...
bool start_process(char8_t* commandLine, const char8_t* instanceName)
{
    eastl::string msg;
        ...
        &processInfo))
    {
        ...
        {
            sServerProcessHandleMap[instanceName] = processInfo.hProcess;
        }
    }
    else
    {
...
bool start_cluster(const char8_t* clusterName, uint16_t totalNodes, uint16_t numReplicas)
{
    ...
    for (; basePort<=lastPort; ++basePort)
    {
        ...
        if (!start_process(commandLine, instanceName.c_str()))
            return false;
    }
    ...
    blaze_snzprintf(commandLine, sizeof(commandLine),
        ...
        allHosts.c_str(),
        numReplicas);
    return start_process(commandLine, nullptr);
}
...
int main(int argc, char **argv)
{
    ...
    // Every 5 seconds, verify that the redis-server.exe instances are still active
    while (WaitForMultipleObjects(copyOfHandles.size(), allHandles, TRUE, 5000) == WAIT_TIMEOUT)
    {
        // Don't bother checking if we have begun the shutdown process
        if (sKillCount == 0)
        {
            int inactiveCount = 0;
            for (auto serverPair : sServerProcessHandleMap)
            {
                DWORD exitCode = 0;
                GetExitCodeProcess(serverPair.second, &exitCode);
                if (exitCode != STILL_ACTIVE)
                {
                    msg.sprintf("[main] The %s redis-server.exe instance is down!!!\n", serverPair.first.c_str());
                    printMessage(msg.c_str());
                    ++inactiveCount;
                }
            }
            ...
            if (inactiveCount != 0)
            {
                msg.sprintf("[main] Check if there are conflicting redis-server.exe processes running on your host.\n");
                printMessage(msg.c_str());
            }
        }
    }
