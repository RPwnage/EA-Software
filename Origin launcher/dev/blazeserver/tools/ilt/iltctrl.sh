...
# Which built config do you want to work with.  Can be overridden in ilt.cfg.
# The options are:
...
# default sliver namespace settings. Can be overridden in ilt.cfg
...
genserverboot()
{
    ...
    echo ""
    if [[ "$iltVaultEnabled" != "" ]]; then
        if [ $iltVaultEnabled -ne 0 ];
        then
            echo "#define VAULT_ENABLED \"true\""
        else
            echo "#define VAULT_ENABLED \"false\""
        fi
        echo "#define ILT_VAULT_ROLE_ID \"$iltVaultRoleId\""
        echo "#define ILT_VAULT_ROOT_PREFIX \"$iltVaultRootPrefix\""
        echo ""
    fi
    echo "// templated settings..."
    while read line
...
genclientcfg()
{
    ...
    echo "buildConfigDir=$buildConfigDir"
    ...
    echo "loginByJwt=$loginByJwt"
    echo "cacheJwtToken=$cacheJwtToken"
...
makeserver()
{
    ...
    pushd $blazeDir
    rm -rf bin/archive
    echo "`date +%Y/%m/%d-%T` [INFO] Executing: sudo /opt/gs/blazepkg -c $pkgcfg pack --stage -b $buildPackage --rebuild-images -t $iltTag"
    sudo /opt/gs/blazepkg -c $pkgcfg pack --stage -b $buildPackage --rebuild-images -t $iltTag
    local ret=$?
    if [ $ret -ne 0 ];
    then
...
_stripstress()
{
    ...
    if [ -f build/blazeserver/$blazevers/$buildConfigDir/bin/stress ];
    then
        cp -f build/blazeserver/$blazevers/$buildConfigDir/bin/stress build/blazeserver/$blazevers/$buildConfigDir/bin/stress_stripped
        strip build/blazeserver/$blazevers/$buildConfigDir/bin/stress_stripped
    fi
    popd >/dev/null
}
...
geniltpkgcfg()
{
    ...
    echo ""
    echo "[build]"
    echo "build_target = $buildPackage"
    echo "debug = unix64-clang-debug"
    echo "release = unix64-clang-debug-opt"
    echo "final = unix64-clang-opt"
    echo ""
    echo "[pack ilt]"
    echo "includes ="
    echo "    build/blazeserver/${blazevers}/$buildConfigDir/bin/stress_stripped"
    echo "    build/blazeserver/${blazevers}/$buildConfigDir/container_lib/*"
...
checkrunning()
{
    ...
    do
        ...
        ssh $sshOpts $host "ps aux | egrep 'blaze|stress|redis'"
    done
...
deploymockservice()
{
    ...
    for i in "${!deployMockServiceHosts[@]}"; do
        ...
        cd $origDir
        cat ${deployMockService_tar} | ssh $sshOpts ${deployMockServiceHosts[$i]} "rm -rf ${deployMockServiceHostFolder}; mkdir --parents ${deployMockServiceHostFolder} && tar xvzf - -C ${deployMockServiceHostFolder} && cp -rf ${deployMockServiceHostFolder}/common ${deployMockServiceHostFolder}/.."
        if [ $? -ne 0 ]; then
...
case "$iltBuildConfig" in
debug)
    buildConfigDir="unix64-clang-debug"
    buildPackage="debug"
    ;;
release)
    buildConfigDir="unix64-clang-debug-opt"
    buildPackage="release"
    ;;
final)
    buildDirConfig="unix64-clang-opt"
    buildPackage="final"
    ;;
*)
    echo "`date +%Y/%m/%d-%T` [ERR] Unknown build config '$iltBuildConfig' specified for iltBuildConfig.  Please use debug, release, or final."
    exit 1
esac

echo "`date +%Y/%m/%d-%T` [INFO] Using $buildPackage build from $buildConfigDir"
