...
void OutboundHttpService::registerService(const char8_t* serviceName, const HttpServiceConnection& connConfig, const ComponentId owningComponentId)
{
    ...
    InetAddress addr(serviceHostnamePort);

    const char8_t* proxyUrl = nullptr;
    bool proxySecure = false;
    HttpProtocolUtil::getHostnameFromConfig(connConfig.getProxyUrl(), proxyUrl, proxySecure);

    if (proxySecure)
    {
        BLAZE_WARN_LOG(Log::HTTP, "[OutboundHttpService].registerService: Service " << serviceName
            << " is configured with secure https proxy, defaulting to http proxy as the only supported option.");
    }
    InetAddress proxyAddr(proxyUrl);
    ...
    if (connConfig.getHasProxyComponent())
    {
        ...
        Component* component = info->createRemoteComponent(*resolver, addr);
    ...
    connMgr->initialize(addr, proxyAddr, connConfig.getNumPooledConnections(), serviceSecure, secureVerifyPeer, 
...
bool OutboundHttpService::validateConfigEqualsExistingManager(const HttpServiceConnection& connConfig, HttpConnectionManagerPtr& connMgr) const
{
    ...
    bool proxySecure = false;
    HttpProtocolUtil::getHostnameFromConfig(connConfig.getProxyUrl(), proxyUrl, proxySecure);
    InetAddress newProxyAddress(proxyUrl);
    ...
    if ((connMgr->getAddress() == newAddress) &&
        (connMgr->getProxyAddress() == newProxyAddress) &&
