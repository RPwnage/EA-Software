...
namespace OAuth
{
    ...
    //decode jwt access token, verify signature, output payload info
    //return OAUTH error code
    BlazeRpcError JwtUtil::decodeJwtAccessToken(const char8_t* token, NucleusConnect::JwtPayloadInfo& payloadInfo)
    {
        ...
        BLAZE_TRACE_LOG(Log::OAUTH, "[JwtUtil].decodeJwtAccessToken: Decoding JWT access token: " << token);
    ...
    BlazeRpcError JwtUtil::getTokenInfo(const NucleusConnect::JwtPayloadInfo& payloadInfo, const ClientPlatformType platform, NucleusConnect::GetTokenInfoResponse& response)
    {
        ...
        response.setIsUnderage(payloadInfo.getNexusClaims().getIsUnderage());
        ...
        {
            //Special case for pc and steam to make the values of GetTokenInfoResponse consistent with directly calling Nucleus API.
            //Nucleus has some backwards compatibility logic on the API response but not on the jwt token payload. This causes difference between API response and jwt token.
            //Logic here is to make the caller function get the same values in response regardless the response is from calling Nucleus API or jwt payload.
            response.setPidId(payloadInfo.getNexusClaims().getUserId());
            response.setPersonaId(INVALID_BLAZE_ID);
        }
        else
        {
            response.setPidId(payloadInfo.getNexusClaims().getPidId());
            response.setPersonaId(payloadInfo.getNexusClaims().getPersonaId());
        }
        ...
        BLAZE_TRACE_LOG(Log::OAUTH, "[JwtUtil].getTokenInfo: JWT payload info -> " << response.getClassName() << response);
        return ERR_OK;
    }

    BlazeRpcError JwtUtil::getAccount(const NucleusConnect::JwtPayloadInfo& payloadInfo, const ClientPlatformType platform, NucleusIdentity::GetAccountResponse& response)
    {
        response.getPid().setUnderagePid(payloadInfo.getNexusClaims().getAccountInfo().getIsUnderage());
        response.getPid().setAnonymousPid(payloadInfo.getNexusClaims().getAccountInfo().getIsAnonymous());
        response.getPid().setCountry(payloadInfo.getNexusClaims().getAccountInfo().getCountry());
        response.getPid().setLanguage(payloadInfo.getNexusClaims().getAccountInfo().getLanguage());
        response.getPid().setStatus(payloadInfo.getNexusClaims().getAccountInfo().getStatus());

        eastl::string locale(eastl::string::CtorSprintf(), "%s_%s", payloadInfo.getNexusClaims().getAccountInfo().getLanguage(), payloadInfo.getNexusClaims().getAccountInfo().getCountry());
        response.getPid().setLocale(locale.c_str());

        if (platform == pc || platform == steam)
        {
            //Special case for pc and steam to make the values of GetAccountResponse consistent with directly calling Nucleus API.
            //Nucleus has some backwards compatibility logic on the API response but not on the jwt token payload. This causes difference between API response and jwt token.
            //Logic here is to make the caller function get the same values in response regardless the response is from calling Nucleus API or jwt payload.
            response.getPid().setExternalRefValue(payloadInfo.getNexusClaims().getUserId());
        }
        else
        {
            bool personaFound = false;
            PersonaId personaId = payloadInfo.getNexusClaims().getPersonaId();

            for (NucleusConnect::JwtPersonaInfoList::const_iterator it = payloadInfo.getNexusClaims().getPersonaInfoList().begin(); it != payloadInfo.getNexusClaims().getPersonaInfoList().end(); ++it)
            {
                NucleusConnect::JwtPersonaInfoPtr personaInfo = *it;
                if (personaInfo->getPersonaId() == personaId)
                {
                    response.getPid().setExternalRefValue(personaInfo->getExternalRefValue());
                    personaFound = true;
                    break;
                }
            }
            if (!personaFound)
            {
                BLAZE_ERR_LOG(Log::OAUTH, "[JwtUtil].getAccount: JWT payload info does not contain persona info for PersonaId(" << personaId << "), Platform(" << platform << "). Unable to get " << response.getClassName() << "." << payloadInfo);
                return ERR_SYSTEM;
            }
        }

        BLAZE_TRACE_LOG(Log::OAUTH, "[JwtUtil].getAccount: JWT payload info -> " << response.getClassName() << response);
        return ERR_OK;
    }

    BlazeRpcError JwtUtil::getPersonaList(const NucleusConnect::JwtPayloadInfo& payloadInfo, const char8_t* namespaceName, NucleusIdentity::GetPersonaListResponse& response)
    {
        bool personaFound = false;
        ...
        {
            NucleusConnect::JwtPersonaInfoPtr personaInfo = *it;
            if (blaze_strcmp(personaInfo->getNamespace(), namespaceName) == 0)
            {
                NucleusIdentity::PersonaInfo* info = response.getPersonas().getPersona().allocate_element();
                info->setPersonaId(personaInfo->getPersonaId());
                info->setNamespaceName(personaInfo->getNamespace());
                info->setDisplayName(personaInfo->getDisplayName());
                response.getPersonas().getPersona().push_back(info);
        ...
        if (!personaFound)
        {
            BLAZE_ERR_LOG(Log::OAUTH, "[JwtUtil].getPersonaList: JWT payload info does not contain persona info for Namespace(" << namespaceName << "). Unable to get " << response.getClassName() << "." << payloadInfo);
            return ERR_SYSTEM;
        }

        BLAZE_TRACE_LOG(Log::OAUTH, "[JwtUtil].getPersonaList: JWT payload info -> " << response.getClassName() << " for Namespace(" << namespaceName << ")" << response);
        return ERR_OK;
    }

    BlazeRpcError JwtUtil::getExternalRef(const NucleusConnect::JwtPayloadInfo& payloadInfo, const PersonaId personaId, NucleusIdentity::GetExternalRefResponse& response)
    {
        bool personaFound = false;
        ...
        for (NucleusConnect::JwtPersonaInfoList::const_iterator it = payloadInfo.getNexusClaims().getPersonaInfoList().begin(); it != payloadInfo.getNexusClaims().getPersonaInfoList().end(); ++it)
        {
            ...
            if (personaInfo->getPersonaId() == personaId)
            {
                NucleusIdentity::PersonaReference* reference = response.getPersonaReference().allocate_element();
                reference->setReferenceValue(personaInfo->getExternalRefValue());
                response.getPersonaReference().push_back(reference);
        ...
        if (!personaFound)
        {
            BLAZE_ERR_LOG(Log::OAUTH, "[JwtUtil].getExternalRef: JWT payload info does not contain persona info for PersonaId(" << personaId << "). Unable to get " << response.getClassName() << "." << payloadInfo);
            return ERR_SYSTEM;
        }

        BLAZE_TRACE_LOG(Log::OAUTH, "[JwtUtil].getExternalRef: JWT payload info -> " << response.getClassName() << " for PersonaId(" << personaId << ")" << response);
        return ERR_OK;
    }

    BlazeRpcError JwtUtil::getPersonaInfo(const NucleusConnect::JwtPayloadInfo& payloadInfo, const PersonaId personaId, NucleusIdentity::GetPersonaInfoResponse& response)
    {
        bool personaFound = false;
        ...
        for (NucleusConnect::JwtPersonaInfoList::const_iterator it = payloadInfo.getNexusClaims().getPersonaInfoList().begin(); it != payloadInfo.getNexusClaims().getPersonaInfoList().end(); ++it)
        {
            ...
            if (personaInfo->getPersonaId() == personaId)
            {
                response.getPersona().setPersonaId(personaInfo->getPersonaId());
                response.getPersona().setNamespaceName(personaInfo->getNamespace());
                response.getPersona().setDisplayName(personaInfo->getDisplayName());
        ...
        if (!personaFound)
        {
            BLAZE_ERR_LOG(Log::OAUTH, "[JwtUtil].getPersonaInfo: JWT payload info does not contain persona info for PersonaId(" << personaId << "). Unable to get " << response.getClassName() << "." << payloadInfo);
            return ERR_SYSTEM;
        }

        BLAZE_TRACE_LOG(Log::OAUTH, "[JwtUtil].getPersonaInfo: JWT payload info -> " << response.getClassName() << " for PersonaId(" << personaId << ")" << response);
        return ERR_OK;
    }

    //returns token expiresIn without verifying token authenticity
    int64_t JwtUtil::getExpiresIn(const char8_t* token)
    {
        JwtSegmentInfo segments;
        if (!getJwtSegmentInfo(token, segments))
        {
            BLAZE_WARN_LOG(Log::OAUTH, "[JwtUtil].getExpiresIn: Token is not JWT format. Returning 0 as expiresIn.");
            return 0;
        }

        NucleusConnect::JwtPayloadInfo payloadInfo;
        if (decodeJwtSegment(segments.payload, segments.payloadSize, payloadInfo) != ERR_OK)
        {
            BLAZE_WARN_LOG(Log::OAUTH, "[JwtUtil].getExpiresIn: Failed to read expiresIn from token. Returning 0 as expiresIn.");
            return 0;
        }

        int64_t expiresIn = payloadInfo.getExpirationTime() - TimeValue::getTimeOfDay().getSec();
        BLAZE_TRACE_LOG(Log::OAUTH, "[JwtUtil].getExpiresIn: Token expires in " << expiresIn << " seconds");
        return expiresIn;
    }
