    ...
    if (error == ERR_OK)
    {
        ...
        *request.mutable_profilename() = profileName;
        ...
        else
        {
            ...
            mLastRefreshProfileVersion = response->profileversion();
...
************************************************************************************/
bool QosConfig::validateAndCleanupPingSiteAliasMap(PingSiteLatencyByAliasMap &map, int32_t profileVersion) const
{
    ...
    for (Blaze::PingSiteLatencyByAliasMap::iterator itMap = map.begin(); itMap != map.end();)
    {
        ...
        {
            BLAZE_WARN_LOG(Log::USER, "QOSConfig.validateAndCleanupPingSiteAliasMap validation failed for ping site alias:" << itMap->first.c_str());
            allPingsitesWereKnown = false;

            if (!isAcceptablePingSiteAliasName(itMap->first.c_str()))
            {
                BLAZE_WARN_LOG(Log::USER, "[QosConfig].validateAndCleanupPingSiteAliasMap: Non alpha numeric character other than - _ : found in the pingsite name.  "
    ...
    if (mQosConfigInfo.getCheckQosProfileVersion())
    {
        if (profileVersion > getProfileVersion())
        {
            BLAZE_WARN_LOG(Log::USER, "[QosConfig].validateAndCleanupPingSiteAliasMap: Qos version changed, from " << getProfileVersion() << " to " << profileVersion << "as provided by the client.");
            return false;
        }
    }
    else
    {
        // Only do the 'different sized list from clients' check if qos versions check is not in use.
...
// Returns true if the versions are the same, otherwise returns false:
bool QosConfig::validatePingSiteAliasMap(const PingSiteInfoByAliasMap &map, int32_t profileVersion)
{
