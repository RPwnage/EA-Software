...
{
    ConfigFile* overrideFile = nullptr;
    if (!overrideFilename.empty())
    {
        overrideFile = createFromFile(overrideRootDir, overrideFilename, allowUnquotedStrings, predefines, errResult, stringEscapes);
        if (overrideFile == nullptr)
        {
            BLAZE_ERR_LOG(Log::CONFIG, "[ConfigFile].createFromFile: Failed to create override configuration file: " << overrideFilename << " for: " << filePath);
            return nullptr;
        }
    }
    
    // Merge of predefines and the override file itself.
    // For the boot config, predefines are external defines (built-in and passed from command lines) 
    // For every other config, the predefines come from the adjusted boot config operation. 
    PredefineMap mergedDefines; 
    for (PredefineMap::const_iterator pitr = predefines.begin(), pend = predefines.end(); pitr != pend; ++pitr)
        mergedDefines[pitr->first] = pitr->second;
    
    if (overrideFile != nullptr)
    {
        for (PredefineMap::const_iterator pitr = overrideFile->mDefineMap.begin(), pend = overrideFile->mDefineMap.end(); pitr != pend; ++pitr)
            mergedDefines[pitr->first] = pitr->second;
    }
    
    ConfigFile* result = createFromFile(rootDir, filePath, allowUnquotedStrings, mergedDefines, errResult, stringEscapes);

    if (result != nullptr)
    {
        if (overrideFile && !result->applyOverride(*overrideFile))
        {
...
{
    // At this point, all the pre-processors are already processed and part of the Config data strings so we don't need any special processing of 
    // override file before processing the base config.
