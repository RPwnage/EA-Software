...
void ConnApiVoipManager::refreshSendLists(const Blaze::Mesh *pMesh)
{
    ...
    for(iIndex = 0; iIndex < VOIPTUNNEL_MAXGROUPSIZE; iIndex++)
    {
        ...
        if (aClientIds[iIndex] != 0)
        {
            ...
            {
                /* The code in this file is only used with voip switching on a dedicated server.
                   In that mode, we can assume that a client is only involved in a single game at a time.
                   Therefore, with DirtySDK 15.1.7.0.1 and beyond, we assume only the first entry of the
                ...
                memcpy(&pClient->aGameMappings[0].aClientIds[0], aClientIds, sizeof(pClient->aGameMappings[0].aClientIds));
                VoipTunnelClientRefreshSendMask(mVoipTunnelRef, pClient, 0);
                #endif
            }
        }
    }
...
void ConnApiVoipManager::connectToEndpoint(const Blaze::MeshEndpoint *pEndpoint)
{
    ...
    if (mVoipTunnelRef != nullptr)
    {
        ...
        if (clientRef == nullptr)
        {
            ...
            // add client to the voip tunnel at its corresponding connection slot -1 to account for the server being part of the ConnApi client list
            // we offset so that we do not lose the voip slot, which supports a maximum of 32 clients 
            int32_t ret = VoipTunnelClientListAdd(mVoipTunnelRef, clientId, 0, NULL, pEndpoint->getConnectionSlotId() - 1, &clientRef);
            #endif
