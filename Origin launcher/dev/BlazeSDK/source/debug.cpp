...
void Debug::Logf(bool trace, const char *msg, ...)
{
    ...
    if (mEnableBlazeLogTimeStamp)
    {
        ...
        printtimeoffset = ds_snzprintf(strText, DEFAULT_LOGGER_ALLOC_SIZE, "%d/%02d/%02d-%02d:%02d:%02d.%03.3d%+03d ",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, iMsec, iGmtOffset);
    }
#endif

    va_start(args, msg);
    int textLogLen = vsnprintf(strText+printtimeoffset, sizeof(strText)-printtimeoffset-1, msg, args); 
    // The generated string has a length of at most sizeof(strText)-2, leaving space for the additional new line and terminating null character.
    va_end(args);

    if (textLogLen > (int) sizeof(strText) - printtimeoffset - 1)
        textLogLen = sizeof(strText) - printtimeoffset - 1;

    //make sure strText has \n\0 at the end if last character is not newline in the debug log message.
    if (textLogLen > 0 &&
        ((printtimeoffset + textLogLen + 1) < sizeof(strText)) &&
        (strText[printtimeoffset + textLogLen - 1] != '\n'))
    {
        strText[printtimeoffset + textLogLen] = '\n';
        strText[printtimeoffset + textLogLen + 1] = '\0';
    }
