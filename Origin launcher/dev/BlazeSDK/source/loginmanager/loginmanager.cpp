...
void LoginManagerImpl::startAccessTokenLoginProcess(const char8_t *accessToken, const char8_t* productName/* = ""*/, Authentication::CrossPlatformOptSetting crossPlatformOpt /* = DEFAULT*/) const
{
    mStateMachine->getState()->onStartAccessTokenLoginProcess(accessToken, productName, crossPlatformOpt);
}
...
void LoginManagerImpl::updateAccessToken(const char8_t* accessToken, Functor1<Blaze::BlazeError> cb) const
{
    mStateMachine->getState()->onUpdateAccessToken(accessToken, cb);
}
