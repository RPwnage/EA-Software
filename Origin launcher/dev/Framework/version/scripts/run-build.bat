...
%eapm_path% install DotNetSdk -masterconfigfile:%masterconfig_path%
REM Find where the package got installed:
for /f %%i in ('%eapm_path% where DotNetSdk -masterconfigfile:%masterconfig_path%') do set DOTNETSDK_PACKAGE=%%i
echo Extracted DotNetSdk package path: %DOTNETSDK_PACKAGE%

REM non-proxy package only has one SDK folder in it so use FOR DIR to get it and store it
FOR /F "delims=" %%i IN ('dir %DOTNETSDK_PACKAGE%\Installed\dotnet\sdk\ /b') DO SET DOTNETSDK_PACKAGE_VERSION=%%i
...
set DOTNET_MSBUILD_SDK_RESOLVER_SDKS_DIR=%DOTNETSDK_PACKAGE%\Installed\dotnet\sdk\%DOTNETSDK_PACKAGE_VERSION%\Sdks
