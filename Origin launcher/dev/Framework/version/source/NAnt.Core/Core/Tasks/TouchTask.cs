...
namespace NAnt.Core.Tasks {
	...
	[TaskName("touch", NestedElementsCheck = true)]
	public class TouchTask : Task {
		...
		public string Millis { get; set; } = null;
		...
		[FileSet("fileset")]
		public FileSet TouchFileSet { get; } = new FileSet();
		...
		protected override void ExecuteTask() {
			...
			{
				try {
					touchDateTime = GetDateTime(Convert.ToDouble(Millis));
				} catch(Exception e) {
					string msg = String.Format("Could not convert time '{0}'.", Millis);
