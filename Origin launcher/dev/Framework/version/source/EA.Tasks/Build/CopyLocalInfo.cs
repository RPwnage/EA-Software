...
namespace EA.Eaconfig.Build
{
	...
	public class CopyLocalInfo : CopyLocalInfoBase
	{
		...
		private static IEnumerable<CopyLocalFileSet> GetManagedCopyLocalOutputs(IModule module, IModule parent)
		{
			...
			(
				module, 
				includePrimaryOutputs: module.IsKindOf(Module.Managed | Module.DotNet), 
				additionalFileSets: additional
			);
		...
		private static List<CopyLocalFileSet> GetCopyLocalFileSets(IModule module, bool includePrimaryOutputs, List<FileSet> additionalFileSets)
		{
			...
			if (includePrimaryOutputs)
			{
				...
				if (module is Module_ExternalVisualStudio externalModule)
				{// external projects explicitly set their output paths
					primaryOutput = externalModule.VisualStudioProjectOutput.Path;
					primaryDebugSymbolsOutput = externalModule.VisualStudioProjectDebugSymbols.Path;
				}

				// add the primary output, if it wasn't already part of additional files
				FileItem primaryFileItem = additionalFileSets.SelectMany(fileset => fileset.FileItems).FirstOrDefault(item => String.Equals(item.Path.NormalizedPath, primaryOutput, PathUtil.PathStringComparison));
				if (primaryFileItem == null)
				{
					...
					primaryFileItem = primaryOuputFileSet.FileItems.FirstOrDefault();
					fileSets.Add(new CopyLocalFileSet(primaryOuputFileSet));
				}

				if (string.IsNullOrWhiteSpace(primaryDebugSymbolsOutput) == true && primaryOutput != null)
				{
					if (primaryOutput.ToLower().EndsWith(".dll") || primaryOutput.ToLower().EndsWith(".exe"))
					{
						primaryDebugSymbolsOutput = primaryOutput.Substring(0, primaryOutput.Length - ".dll".Length) + ".pdb";
					}
				}

				if (string.IsNullOrWhiteSpace(primaryDebugSymbolsOutput) == false && primaryDebugSymbolsOutput != primaryOutput) // extra safety, should be be null for platforms that don't have a separate symnols file but there's an outside chance it has been set to same as binary path
				{
					...
					debugSymbolsFileSet.Include(primaryDebugSymbolsOutput, primaryFileItem.BaseDirectory, optionSetName: primaryFileItem.OptionSetName); // copy primary optionset, as we want to use same copy options
