	...
	internal static class DotNetTargeting
	{
		internal static string GetNetVersion(Project project, DotNetFrameworkFamilies family)
		{
			...
			else
			{
				...
				if (PackageMap.Instance.TryGetMasterPackage(project, "DotNetSdk", out MasterConfig.IPackage dotNetMasterPackage))
				{
					TaskUtil.Dependent(project, "DotNetSdk", Project.TargetStyleType.Use);
					return project.Properties.GetPropertyOrDefault("package.DotNetSdk.defaulttargetframeworkversion" + propNameExt, dotNetMasterPackage.Version);
				}
				throw new BuildException("Cannot determine target .NET version - this is required by the build system even when not building .NET code. Please add DotNetSdk package version to your masterconfig."); // NUGET-TODO is this true? this really only applies .vcxproj trying to resolve .net framework
			}
		}
	}
}
