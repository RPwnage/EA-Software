...
#else
				appDomain = AppDomain.CreateDomain("markupCompilationAppDomain");
#endif

				// this hack create a new instance in the other app domain of a class that registers special assembly resolve event handler that allows the other app domain to find 
				// this dll (i.e curAsm)
				// we're executing inside of msbuild so while msbuild found this dll because we used an explict path in our <UsingTask> a separate app domain won't find this
				// dll in its default search paths
				// in *theory* we could just use CreateInstanceFromAndUnwrap which takes a file path rather than just an assembly name to create our CompilerWrapper but for reasons
				// unknown some of the referenced members of the compilerWrapper object created this way cannot be marshalled and throw an invalidargument exception
				appDomain.CreateInstanceFrom(typeof(EA.AssemblyResolver).Assembly.Location, typeof(EA.AssemblyResolver).FullName, false, BindingFlags.Default, null, new object[] { }, null, null);
...
#if NETFRAMEWORK
				compilerWrapper = appDomain.CreateInstanceAndUnwrap(curAsm.FullName, typeof(CompilerWrapper).FullName, false, bf, null, null, null, null, null) as CompilerWrapper;
#else																
				compilerWrapper = appDomain.CreateInstanceAndUnwrap(curAsm.FullName, typeof(CompilerWrapper).FullName, false, bf, null, null, null, null) as CompilerWrapper;
#endif
			}
			else
			{
