	...
	internal sealed class ConsoleRunner : IDisposable
	{
		...
		internal int Start(string[] args)
		{
			try
			{
				...
				// get build root now - it's used in masterconfig init (see InitMasterConfig for details)
				string buildRootOverride = ((string)Option.BuildRoot.Value).TrimWhiteSpace().TrimQuotes();
				...
				{
					masterConfig = InitMasterConfig(globalInitProject, buildRootOverride, out buildFilePath, safe: true, customFragments: masterConfigFragments);
				...
				if (masterConfig == null)
				{
					masterConfig = InitMasterConfig(globalInitProject, buildRootOverride, out buildFilePath, safe: false, customFragments: masterConfigFragments);
				}
		...
		private static MasterConfig InitMasterConfig(Project project, string buildRootOverride, out string buildFilePath, bool safe = false, List<string> customFragments = null)
		{
			MasterConfig masterConfig = null;
			...
			if (!safe || File.Exists(masterConfigFilePath) || LineInfoDocument.IsInCache(masterConfigFilePath)) // don't fail if this is missing when running "safe", we don't need it for '-help' for example
			{
				...
				(
					project, 
					masterConfig, 
					autobuildAll: (bool)Option.ForceAutobuild.Value, 
					autobuildPackages: (string)Option.ForceAutobuildPackage.Value, 
					autobuildGroups: (string)Option.ForceAutobuildGroup.Value, 
					buildRootOverride: buildRootOverride,
					token: (string)Option.PackageServerToken.Value, 
					useV1: (bool)Option.UsePackageServerV1.Value
				);
