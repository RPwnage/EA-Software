...
struct PackerSilo
{
    ...
    void setMaxPackerIterations(uint64_t maxRepeatPackings);
    ...
    const auto& getGameTemplate() const { return mGameTemplate; }
...
private:
    ...
    uint64_t mMaxPackerIterations = MAX_REPEAT_PACKING_ITERATIONS; // last line of defense to guard against infinite loops (due to internal bugs)
