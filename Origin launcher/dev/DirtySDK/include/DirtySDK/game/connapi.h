...
typedef struct ConnApiClientInfoT
{
    ...
    uint32_t            uHostingServerId;       //!< id of the hosting server (only valid if bIsConnectivityHosted is TRUE)
    uint32_t            uHostingServerConnSetId;//!< id of connection set to which the player belongs on the hosting server (only valid if bIsConnectivityHosted is TRUE)
    ...
    uint8_t             bIsConnectivityHosted;  //!< flag for whether the connectivity to this client is direct or server-h
    uint8_t             bEnableQos;             //!< enable QoS for this client, call ConnApiControl() with 'sqos' and 'lqos' to configure QoS settings
    uint8_t             bIsVoipContributer;     //!< allow this client to contribute to voip based on Player Settings Service
    uint8_t             _pad[1];
