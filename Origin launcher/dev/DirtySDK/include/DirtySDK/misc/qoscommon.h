...
#define QOS_COMMON_CLIENT_VERSION_MAJOR         (2)                             //!< identifies changes to qosclient->qoscoordinator api, 2 bytes, first byte indicates api breaking
#define QOS_COMMON_CLIENT_VERSION_MINOR         (1)                             //!< second byte compatible bug fixes.
#define QOS_COMMON_CLIENT_VERSION               ((QOS_COMMON_CLIENT_VERSION_MAJOR << 8) | QOS_COMMON_CLIENT_VERSION_MINOR) //!< the combined major, minor version bytes.
...
typedef struct QosCommonProcessedResultsT
{
    ...
    int32_t iProfileVersion;                                    //!< the version of the qos profile the coordinator provided for this test
...
typedef struct QosCommonClientToCoordinatorRequestT
{
    ...
    uint16_t uProbeVersion;                                 //!< uniquely identifies protocol of client->qosserver probe packets
    uint16_t uClientVersion;                                //!< uniquely identifies protocol of client->qoscoordinator communications
