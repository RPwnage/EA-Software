...
static int32_t _VoipOpusResample(VoipCodecRefT *pState, uint8_t *pOutBytes, uint32_t uOutBuffSize, const int16_t *pInput, int32_t iNumSamples);
...
static int32_t _VoipOpusConvertInt32ToFloat(VoipCodecRefT *pState, uint8_t *pInBytes, int32_t iNumSamples)
{
    ...
    if (pOpus->bInt32Input)
    {
        ...
        {
            pOutput[iBufferIndex] = (float)pInput[iBufferIndex] / (float)INT32_MAX;
        }
        return (iBufferIndex);
    }
    return(0);
...
/*!
    ...
    \Input *pState        - codec state
    \Input *pOutBytes     - [out] output samples 
    \Input uOutBuffSize   - size of pOutBytes in bytes
    \Input *pInput        - input samples
    \Input iNumSamples    - number of samples in pInput

    \Output
        int32_t         - number of samples written, negative for error

    \Version 02/17/2021 (tcho)
*/
/*************************************************************************************F*/
static int32_t _VoipOpusResample(VoipCodecRefT *pState, uint8_t *pOutBytes, uint32_t uOutBuffSize, const int16_t *pInput, int32_t iNumSamples)
{
    ...
    uint32_t uMaxResampleBufferSize = (pOpus->bFloatInput) ? VOIPOPUS_RESAMPLE_BUFFER_SIZE : VOIPOPUS_RESAMPLE_BUFFER_SIZE / 2;

    if (uOutBuffSize < uMaxResampleBufferSize)
    {
        NetPrintf(("voipopus: resampling output buffer size too small\n"));
        return(-1);
    }
    ...
    if (uSampleRateIn != VOIPOPUS_DEFAULT_SAMPLING_RATE)
    {
        ...
        if (pOpus->bFloatInput)
        {
            ...
            if ((iResult = speex_resampler_process_float(pOpus->pSpeexResampler, 0, (const float *)pInput, (uint32_t *)&iNumSamples, (float *)pOutBytes, (uint32_t *)&iOutputSamples)) != 0)
            {
        ...
        else
        {
            ...
            if ((iResult = speex_resampler_process_int(pOpus->pSpeexResampler, 0, (const int16_t *)pInput, (uint32_t *)&iNumSamples, (int16_t *)pOutBytes, (uint32_t *)&iOutputSamples)) != 0)
            {
...
static int32_t _VoipOpusEncodeBlock(VoipCodecRefT *pState, uint8_t *pOutput, const int16_t *pInput, int32_t iNumSamples)
{
    ...
    iResult = _VoipOpusResample(pState, aResampledData, sizeof(aResampledData),pInput, iNumSamples);
    if (iResult < 0)
    {
        return(iResult);
    }
