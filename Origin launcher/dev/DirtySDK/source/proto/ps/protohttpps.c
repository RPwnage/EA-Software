...
/*!
    ...
    \Description
        ...
        (get/put) with an HTTP server using the PS4/PS5 SDK.  
...
static int32_t _ProtoHttpSendRequestImpl(ProtoHttpRefT *pState, const char *pStrBuf, const char *pPostData, int32_t iSize, int32_t iSizeFinal)
{
    ...
    else if (iResult == 0)
    {
        ...
        {
            NetPrintf(("protohttpps: [%p] sent %d bytes\n", pState, iSizeFinal));
        }
    ...
    {
        NetPrintf(("protohttpps: [%p] error %s sending %d bytes\n", pState, DirtyErrGetName(iResult), iSize));
...
static void _ProtoHttpReleaseWorkerThread(ProtoHttpRefT *pState)
{
    ...
    {
        NetPrintf(("protohttpps: [%p] failed to signal worker thread.\n", pState));
    }
...
void _ProtoHttpCreateContexts(ProtoHttpRefT *pState)
{
    ...
    NetPrintf(("protohttpps: [%p] creating http contexts\n", pState));
    ...
    if (pState->iNetMemId == -1)
    {
        ...
        if (iNetMemId < 0)
        {
            NetPrintf(("protohttpps: unable to allocate sceNetPool, error 0x%08x\n", iNetMemId));
            pState->iNetMemId = -1;
    ...
    if (pState->iSslContextId == -1)
    {
        ...
        {
            NetPrintf(("protohttpps: unable to initialize sceSslContext, error 0x%08x\n", iSslContextId));
    ...
    if (pState->iHttpContextId == -1)
    {
        ...
        {
            NetPrintf(("protohttpps: unable to initialize sceHttpContext, error 0x%08x\n", iHttpContextId));
...
{
    NetPrintf(("protohttpps: [%p] freeing http contexts\n", pState));
...
static void _ProtoHttpApplyCAs(ProtoHttpRefT *pState)
{
    ...
    if (pState->uCaListVersionLoaded != _ProtoHttp_State.uCaListVersion || pState->bCAsApplied == FALSE)
    {
        ...
        {
            NetPrintf(("protohttpps: [%p] sceHttpsUnloadCert failed with error %s\n", pState, DirtyErrGetName(iResult)));
        }
        ...
        for (iCert = 0; iCert < (int32_t)(sizeof(_ProtoHttp_aGosCACerts)/sizeof(_ProtoHttp_aGosCACerts[0])); ++iCert)
        {
            ...
            if (iResult < 0)
            {
                NetPrintf(("protohttpps: [%p] sceHttpsLoadCert failed to load a GOS CA with error %s\n", pState, DirtyErrGetName(iResult)));
            }
        }
        ...
        while (pCert != NULL)
        {
            ...
            if (iResult < 0)
            {
                NetPrintf(("protohttpps: [%p] sceHttpsLoadCert failed to load a CA with error %s\n", pState, DirtyErrGetName(iResult)));
            }
            pCert = pCert->pNext;
        }
    ...
    if (pState->bUsePrivateKey)
    {
        ...
        {
            NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] sceHttpsLoadCert failed with code %s\n", pState, DirtyErrGetName(iResult)));
        }
    }
...
static void _ProtoHttpCreateConnectionTemplate(ProtoHttpRefT *pState)
{
    ...
    if (pState->iSceTemplateId < 0)
    {
        NetPrintf(("protohttpps: [%p] sceHttpCreateTemplate failed with code %s\n", pState, DirtyErrGetName(pState->iSceConnId)));
        pState->eState = ST_FAIL;
        pState->bClosed = TRUE;
        pState->iSceTemplateId = 0;
    }
    else
    {
        ...
        NetPrintfVerbose((pState->iVerbose, 3, "protohttpps: [%p] template created with id 0x%08x\n", pState, pState->iSceTemplateId));
        ...
        if (pState->bSetSslVers)
        {
            ...
            {
                NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] sceHttpsSetSslVersion failed with code %s\n", pState, DirtyErrGetName(iResult)));
        ...
        if (pState->bSetHttpsFlags)
        {
            if (pState->iSceTemplateId > 0)
            {
                ...
                {
                    NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] sceHttpsDisableOption failed with code %s\n", pState, DirtyErrGetName(iResult)));
                ...
                {
                    NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] sceHttpsEnableOption failed with code %s, flags 0x%08x\n", pState, DirtyErrGetName(iResult), pState->uSceHttpFlags));
                    pState->bSetHttpsFlags = FALSE;
                    pState->eState = ST_FAIL;
                    pState->bClosed = TRUE;
                    sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps: [%p] sceHttpSetRedirectCallback failed with code %s\n", pState, DirtyErrGetName(iResult)));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDeleteTemplate(pState->iSceTemplateId);
            pState->iSceTemplateId = 0;
        ...
        {
            NetPrintf(("protohttpps: [%p] sceHttpSetNonblock failed with code %s\n", pState, DirtyErrGetName(iResult)));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDeleteTemplate(pState->iSceTemplateId);
            pState->iSceTemplateId = 0;
        ...
        if (pState->eState != ST_FAIL)
        {
            ...
            {
                NetPrintf(("protohttpps: [%p] sceHttpCreateEpoll failed with code %s\n", pState, DirtyErrGetName(iResult)));
                pState->eState = ST_FAIL;
                pState->bClosed = TRUE;
                pState->bEpollInitialized = FALSE;
                sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps: [%p] sceHttpSetConnectTimeOut failed with code %s\n", pState, DirtyErrGetName(iResult)));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps: [%p] sceHttpSetRecvTimeOut failed with code %s\n", pState, DirtyErrGetName(iResult)));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps: [%p] sceHttpSetResolveTimeOut failed with code %s\n", pState, DirtyErrGetName(iResult)));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps: [%p] sceHttpSetSendTimeOut failed with code %s\n", pState, DirtyErrGetName(iResult)));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps: [%p] sceHttpSetResolveRetry failed with code %s\n", pState, DirtyErrGetName(iResult)));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
        ...
        {
            NetPrintf(("protohttpps: [%p] sceHttpSetInflateGZIPEnabled failed with code %s\n", pState, DirtyErrGetName(iResult)));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            sceHttpDestroyEpoll(pState->iHttpContextId, pState->EpollHandle);
            sceHttpDeleteTemplate(pState->iSceTemplateId);
...
static uint32_t _ProtoHttpApplyBaseUrl(ProtoHttpRefT *pState, const char *pKind, char *pHost, int32_t iHostSize, int32_t *pPort, int32_t *pSecure, uint8_t bPortSpecified)
{
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: host not present; setting to %s\n", pState->strBaseHost));
        ds_strnzcpy(pHost, pState->strBaseHost, iHostSize);
        bChanged = TRUE;
    }
    if ((bPortSpecified == FALSE) && (pState->iBasePort != 0))
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: port not present; setting to %d\n", pState->iBasePort));
        *pPort = pState->iBasePort;
        bChanged = TRUE;
    }
    if (*pKind == '\0')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: kind (protocol) not present; setting to %d\n", pState->iBaseSecure));
...
static void _ProtoHttpClose(ProtoHttpRefT *pState, const char *pReason)
{
    ...
    NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] closing connection: %s\n", pState, pReason));
    ...
    if (pState->iSceRequestId > 0)
    {
        ...
        if (iResult != SCE_OK)
        {
            NetPrintf(("protohttpps: [%p] unable to delete request %d, err=%s\n", pState, pState->iSceRequestId, DirtyErrGetName(iResult)));
        }
...
static int32_t _ProtoHttpSetAppendHeader(ProtoHttpRefT *pState, const char *pAppendHdr)
{
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: ignoring set of append header '%s' that is already set\n", pAppendHdr));
        return(0);
    }
    ...
    if (iAppendBufLen > pState->iAppendLen)
    {
        ...
        {
            NetPrintf(("protohttpps: could not allocate %d byte buffer for append header\n", iAppendBufLen));
            pState->iAppendLen = 0;
            return(-1);
        }
    }
    ...
    if (pState->iVerbose > 1)
    {
        ...
        for (iRequestType = 0; iRequestType < PROTOHTTP_NUMREQUESTTYPES; iRequestType += 1)
        {
            if (!strncmp(pState->pInpBuf, _ProtoHttp_strRequestNames[iRequestType], strlen(_ProtoHttp_strRequestNames[iRequestType])))
            {
                ...
                *pHdrEnd = '\0';
                NetPrintf(("protohttpps: [%p] sent request:\n", pState));
                NetPrintfVerbose((pState->iVerbose, 2, "protohttpps: [%p] tick=%u\n", pState, NetTick()));
...
static int32_t _ProtoHttpFormatRequestHeader(ProtoHttpRefT *pState, const char *pUrl, const char *pHost, int32_t iPort, int32_t iSecure, const char *pRequest)
{
    ...
    {
        if ((iOffset = pCustomHeaderCb(pState, pInpBuf, iInpMax, pUserData)) < 0)
        {
            NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: custom header callback error %d\n", iOffset));
            return(iOffset);
        }
    }
    ...
    if (iOffset > 0)
    {
        ...
        for ( ; ProtoHttpGetNextHeader(pState, pCurrentHdr, strName, sizeof(strName), strValue, sizeof(strValue), &pCurrentHdr) == 0; )
        {
            ...
            {
                NetPrintf(("protohttpps: unable to add header %s, error %s\n", strName, DirtyErrGetName(iResult)));
            }
        }
    }
...
/*!
    ...
    \Input *pState      - reference pointer
    \Input *pUrl        - [in/out] pointer to user-supplied url, path after parsing
...
static int32_t _ProtoHttpFormatRequest(ProtoHttpRefT *pState, const char **pUrl, const char *pData, int64_t iDataLen, ProtoHttpRequestTypeE eRequestType)
{
    ...
    NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] %s %s\n", pState, _ProtoHttp_strRequestNames[eRequestType], *pUrl));
    ...
    *pUrl = ProtoHttpUrlParse2(*pUrl, strKind, sizeof(strKind), strHost, sizeof(strHost), &iPort, &iSecure, &bPortSpecified);
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: using proxy server %s\n", pState->strProxy));
    ...
    */
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] %s %s://%s:%d%s\n", pState, _ProtoHttp_strRequestNames[eRequestType],
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] requesting new connection -- url change to %s\n", pState, strHost));
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] requesting new connection -- previous connection was closed\n", pState));
        pState->bNewConnection = TRUE;
    }
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] requesting new connection -- current state of %d does not allow connection reuse\n", pState, eState));
        pState->bNewConnection = TRUE;
    }
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] requesting new connection -- reuse on put/post disabled\n", pState));
        pState->bNewConnection = TRUE;
    }
...
*/
/********************************************************************************F*/
static int32_t _ProtoHttpSendRequest(ProtoHttpRefT *pState, const char *pUrl)
{
    ...
    if (pState->bNewConnection == TRUE)
    {
        ...
        // start connect
        NetPrintfVerbose((pState->iVerbose, 2, "protohttpps: [%p] connect start (tick=%u)\n", pState, NetTick()));
        ...
        if (pState->iSceTemplateId > 0)
        {
            if ((pState->iSceConnId = sceHttpCreateConnection(pState->iSceTemplateId, pState->strHost, pState->iSecure ? "https" : "http", pState->iPort, pState->iKeepAlive > 0)) > 0)
            {
                NetPrintfVerbose((pState->iVerbose, 3, "protohttpps: [%p] connection created with id 0x%08x\n", pState, pState->iSceConnId));
                pState->eState = ST_SEND;
                pState->bClosed = FALSE;
            }
            else
            {
                NetPrintf(("protohttpps: [%p] sceHttpCreateConnection failed with code %s\n", pState, DirtyErrGetName(pState->iSceConnId)));
                pState->eState = ST_FAIL;
                pState->bClosed = TRUE;
                pState->iSceConnId = 0;
            }
        ...
        else
        {
            ...
            NetPrintf(("protohttpps: [%p] unable to create a connection, template doesn't exist\n", pState));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            pState->iSceConnId = 0;
        }
    ...
    {
        // advance state
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] reusing previous connection (keep-alive)\n", pState));
        pState->eState = ST_SEND;
    }
    ...
    if (pState->iSceConnId > 0)
    {
        if ((pState->iSceRequestId = sceHttpCreateRequest2(pState->iSceConnId, _ProtoHttp_strRequestNames[pState->eRequestType], pUrl, (uint64_t)pState->iPostSize)) < 0)
        {
            NetPrintf(("protohttpps: [%p] sceHttpCreateRequest2 failed with type %s and error code %s\n",
        ...
        {
            NetPrintfVerbose((pState->iVerbose, 3, "protohttpps: [%p] request created with id 0x%08x\n", pState, pState->iSceRequestId));
        }
    }
    ...
    if (pState->iSceRequestId > 0)
    {
        ...
        if ((iResult = _ProtoHttpFormatRequestHeader(pState, pUrl, pState->strHost, pState->iPort, pState->iSecure, 
                                                     _ProtoHttp_strRequestNames[pState->eRequestType])) < 0)
        {
            NetPrintf(("protohttpps: [%p] failed to setup request headers %s\n", pState, DirtyErrGetName(pState->iSceConnId)));
            pState->eState = ST_FAIL;
            pState->bClosed = TRUE;
            iResult = sceHttpDeleteRequest(pState->iSceRequestId);
            if (iResult != SCE_OK)
        ...
        if (pState->iSceRequestId > 0)
        {
            ...
            if (pState->bEpollInitialized == TRUE)
            {
                ...
                {
                    NetPrintf(("protohttpps: [%p] sceHttpSetEpoll failed with code %s\n", pState, DirtyErrGetName(pState->iSceConnId)));
                    pState->eState = ST_FAIL;
                    pState->bClosed = TRUE;
                    sceHttpUnsetEpoll(pState->iSceRequestId);
                    iResult = sceHttpDeleteRequest(pState->iSceRequestId);
...
static int32_t _ProtoHttpResizeBuffer(ProtoHttpRefT *pState, int32_t iBufMax)
{
    ...
    NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] resizing input buffer from %d to %d bytes\n", pState, pState->iInpMax, iBufMax));
    if ((pInpBuf = DirtyMemAlloc(iBufMax, PROTOHTTP_MEMID, pState->iMemGroup, pState->pMemGroupUserData)) == NULL)
    {
        NetPrintf(("protohttpps: [%p] could not resize input buffer\n", pState));
        return(-1);
    }
    ...
    {
        NetPrintf(("protohttpps: [%p] warning; resize of input buffer is losing %d bytes of data\n", pState, iCopySize - iBufMax));
        iCopySize = iBufMax;
    }
...
static int32_t _ProtoHttpParseHeader(ProtoHttpRefT *pState)
{
    ...
    if (pState->bVerifyHdr)
    {
        ...
        {
            // header is invalid
            NetPrintf(("protohttpps: [%p] invalid result type\n", pState));
            pState->eState = ST_FAIL;
            return(-2);
        }
    }
    ...
    #if DIRTYCODE_LOGGING
    NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] received %d response (%d bytes)\n", pState, pState->iHdrCode, pState->iHeadSize));
    if (pState->iVerbose > 1)
    {
...
{
    // ignore the response
    NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] ignoring %d response from server\n", pState, pState->iHdrCode));
...
static int32_t _ProtoHttpGetHeader(ProtoHttpRefT *pState)
{
    ...
    if ((iResult = sceHttpGetAllResponseHeaders(pState->iSceRequestId, &pState->pSceRespHeader, &pState->iSceRespHeaderLength)) < 0)
    {
        sceHttpsGetSslError(pState->iSceRequestId, &pState->iSslFail, &pState->uSslFailDet);
        sceHttpGetLastErrno(pState->iSceRequestId, &pState->iHresult);
        NetPrintf(("protohttpps: [%p] sceHttpGetAllResponseHeaders failed with code %s for request id %d\n", pState, DirtyErrGetName(iResult), pState->iSceRequestId));
    }
...
static int32_t _ProtoHttpRedirectCallback(int32_t iRequest, int32_t iStatusCode, int32_t *pMethod, const char *pLocation, void *pUserArg)
{
    ...
    int32_t iResult = 0;
    NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] processing redirect for request 0x%08x code %d\n", pState, iRequest, iStatusCode));
    ...
    if ((iResult = _ProtoHttpGetHeader(pState)) >= 0)
    {
        ...
        {
            NetPrintf(("protohttpps: [%p] unable to allocate space to save off the redirect headers to be processed later\n", pState));
            pState->eState = ST_FAIL;
    ...
    if ((pState->iMaxRedirect == 0) || (++pState->iNumRedirect > pState->iMaxRedirect))
    {
        ...
        {
            NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] auto-redirection disabled\n", pState));
        }
        else
        {
            NetPrintf(("protohttpps: [%p] maximum number of redirections (%d) exceeded\n", pState, pState->iMaxRedirect));
        }
...
static int32_t _ProtoHttpCookieRecvCallback(int request, const char *url, const char *cookieHeader, size_t headerLen, void *userArg)
{
    ...
    NetPrintfVerbose((pState->iVerbose, 2, "protohttpps: [%p] received cookie header, %d bytes\n", pState, headerLen));
    NetPrintfVerbose((pState->iVerbose, 3, "protohttpps: [%p] cookie: %s\n", pState, cookieHeader));
#endif
    return(-1);
}
...
static int32_t _ProtoHttpSend(ProtoHttpRefT *pState, const char *pStrBuf, int32_t iSize)
{
    ...
    if (iSizeFinal != iSize)
    {
        NetPrintfVerbose((pState->iVerbose, 4, "protohttpps: [%p] data send throttled by max send rate from %d to %d bytes.\n", pState, iSize, iSizeFinal));
    }
    ...
    if ((iSizeFinal > 0) || (iSize == 0 && iSizeFinal == 0))
    {
        ...
        if ((bThreadAlive == TRUE) || (bCanStartThread == TRUE))
        {
            ...
            if ((bThreadAlive == FALSE) && (bCanStartThread == TRUE))
            {
                ...
                if ((iResult = DirtyThreadCreate(_ProtoHttpThread, pState, &ThreadConfig)) != 0)
                {
                    ...
                    NetCritLeave(NULL);
                    NetPrintf(("protohttpps: [%p] unable to create http worker thread (err=%d)\n", pState, iResult));
                    pState->iThreadLife = 0;
                }
            }
            else
        ...
        if (bDataHandled == FALSE)
        {
            if (bTooManyThreads)
            {
                ...
                NetPrintf(("protohttpps: [%p] unable to start worker thread.\n", pState));
...
static int32_t _ProtoHttpSendBuff(ProtoHttpRefT *pState)
{
    ...
    {
        NetPrintf(("protohttpps: [%p] failed to send request data (err=%s)\n", pState, DirtyErrGetName(iResult)));
        pState->iInpLen = 0;
        pState->eState = ST_FAIL;
        iSentSize = -1;
    }
...
static int32_t _ProtoHttpRecvData(ProtoHttpRefT *pState, char *pStrBuf, int32_t iSize)
{
    ...
    if ((pState->iRecvRslt = sceHttpReadData(pState->iSceRequestId, (void*)pStrBuf, (size_t)iSize)) > 0)
    {
        ...
        if (pState->iVerbose > 2)
        {
            NetPrintf(("protohttpps: [%p] recv %d bytes\n", pState, pState->iRecvRslt));
        }
        if (pState->iVerbose > 3)
        {
...
static void _ProtoHttpHeaderProcess(ProtoHttpRefT *pState)
{
    ...
    if (_ProtoHttpParseHeader(pState) < 0)
    {
        ...
        {
            NetPrintf(("protohttpps: [%p] ST_HEAD append got ST_FAIL (err=%d, len=%d)\n", pState, pState->iRecvRslt, pState->iInpLen));
            pState->eState = ST_FAIL;
        }
        ...
        if (pState->iInpLen == pState->iInpMax)
        {
            if (pState->iInpOvr == 0)
            {
                NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] input buffer too small for response header\n", pState));
            }
...
static int32_t _ProtoHttpRecvBody(ProtoHttpRefT *pState)
{
    ...
    {
        NetPrintfVerbose((pState->iVerbose, 4, "protohttpps: [%p] data recv throttled by max recv rate.\n", pState));
        return(0);
    }
    else
    {
    ...
    {
        NetPrintf(("protohttpps: [%p] ST_FAIL (err=%s)\n", pState, DirtyErrGetName(iResult)));
        pState->eState = ST_FAIL;
        pState->iSslFail = 0;
        pState->uSslFailDet = 0;
        pState->iHresult = 0;
...
static int32_t _ProtoHttpRecv(ProtoHttpRefT *pState, char *pBuffer, int32_t iBufMin, int32_t iBufMax)
{
    ...
    if ((iLen >= iBufMin) || (pState->iInpCnt == pState->iBodySize))
    {
        ...
        if (pBuffer != NULL)
        {
            ...
            if (pState->iVerbose > 3)
            {
                NetPrintf(("protohttpps: [%p] read %d bytes\n", pState, iLen));
            }
...
ProtoHttpRefT *ProtoHttpCreate(int32_t iBufSize)
{
    ...
    {
        NetPrintf(("protohttpps: [%p] unable to allocate module state\n", pState));
        return(NULL);
    }
    ...
    {
        NetPrintf(("protohttpps: [%p] unable to allocate protohttp buffer\n", pState));
    ...
    if ((pState->pPushDataThreadCond = DirtyConditionCreate("WakeupHttpWorkerThread")) == NULL)
    {
        ...
        NetPrintf(("protohttpps: [%p]  failed to create condition, worker thread will not be spawned\n", pState));
    }
...
int32_t ProtoHttpGet(ProtoHttpRefT *pState, const char *pUrl, uint32_t bHeadOnly)
{
    ...
    if (pUrl != NULL)
    {
        ...
        iResult = _ProtoHttpSendRequest(pState, pUrl);
...
int32_t ProtoHttpRecvAll(ProtoHttpRefT *pState, char *pBuffer, int32_t iBufSize)
{
    ...
    {
        // an error occurred
        NetPrintf(("protohttpps: [%p] error %d receiving response\n", pState, iRecvResult));
    }
...
int32_t ProtoHttpPost(ProtoHttpRefT *pState, const char *pUrl, const char *pData, int64_t iDataSize, uint32_t bDoPut)
{
    ...
    {
        // send the request
        iDataSent = _ProtoHttpSendRequest(pState, pUrl);
...
int32_t ProtoHttpRequestCb2(ProtoHttpRefT *pState, const char *pUrl, const char *pData, int64_t iDataSize, ProtoHttpRequestTypeE eRequestType, ProtoHttpWriteCbT *pWriteCb, ProtoHttpCustomHeaderCbT *pCustomHeaderCb, ProtoHttpReceiveHeaderCbT *pReceiveHeaderCb, void *pUserData)
{
    ...
    {
        NetPrintf(("protohttpps: [%p] unrecognized request type %d\n", pState, eRequestType));
        return(-1);
    }
}
...
void ProtoHttpSetBaseUrl(ProtoHttpRefT *pState, const char *pUrl)
{
    ...
    // set base info
    NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] setting base url to %s://%s:%d\n", pState, iSecure ? "https" : "http", strHost, iPort));
...
int32_t ProtoHttpGetLocationHeader(ProtoHttpRefT *pState, const char *pInpBuf, char *pBuffer, int32_t iBufSize, const char **pHdrEnd)
{
    ...
    {
        NetPrintf(("protohttpps: [%p] location header we not found in the header list\n", pState));
        return(-1);
    }
...
int32_t ProtoHttpControl(ProtoHttpRefT *pState, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iSelect == 'hver')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] header type verification %s\n", pState, iValue ? "enabled" : "disabled"));
    ...
    if (iSelect == 'keep')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] setting keep-alive to %d\n", pState, iValue));
    ...
    if (iSelect == 'ktim')
    {
        if (iValue > 0)
        {
            NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] setting keep-alive timeout to %d ms\n", pState, iValue));
    ...
    if (iSelect == 'maxr')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] setting max recv rate to %d bytes/sec\n", pState, iValue));
        pState->RecvRate.uMaxRate = iValue;
        return(0);
    }
    if (iSelect == 'maxs')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] setting max send rate to %d bytes/sec\n", pState, iValue));
    ...
    if (iSelect == 'mwrk')
    {
        NetPrintf(("protohttpps: [all] setting max worker threads to %d\n", iValue));
        NetCritEnter(NULL);
    ...
    if (iSelect == 'prxy')
    {
        NetPrintf(("protohttpps: [%p] 'prxy' control selector is not supported on PS4\n", pState));
        return(-1);
    }
    if (iSelect == 'rmax')
    {
    ...
    if (iSelect == 'rput')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] connection reuse on put/post %s\n", pState, iValue ? "enabled" : "disabled"));
    ...
    if (iSelect == 'time')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] setting connection timeout to %d ms\n", pState, iValue));
        
        // limit the timeout value, otherwise sceHttpSendRequest may fail with SCE_NET_ERROR_EINVAL
        if (iValue > 300000)
        {
            iValue = 300000;
            NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] capping connection timeout to %d ms\n", pState, iValue));
        }
    ...
    if (iSelect == 'resr')
    {
        NetPrintfVerbose((pState->iVerbose, 1, "protohttpps: [%p] dns resolve retry attempts to %d\n", pState, iValue));
        ...
        if (pState->iSceTemplateId > 0)
        {
            ...
            {
                NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] sceHttpSetSendTimeOut failed with code %s\n", pState, DirtyErrGetName(iResult)));
            }
        }
        return(0);
    }
    ...
    if (iSelect == 'scrt')
    {
        if (iValue > 0)
        {
            NetPrintf(("protohttpps: setting cert (%d bytes)\n", iValue));
        ...
        {
            NetPrintf(("protohttpps: removing cert\n"));
    ...
    if (iSelect == 'skey')
    {
        if (iValue > 0)
        {
            NetPrintf(("protohttpps: setting private key (%d bytes)\n", iValue));
        ...
        {
            NetPrintf(("protohttpps: removing private key\n", iValue));
    ...
    if (iSelect == 'vers')
    {
        NetPrintfVerbose((pState->iVerbose, 2, "protohttpps: [%p] using SSL version %s\n", pState, _HTTP_strVersionNames[iValue&0xff]));
...
int32_t ProtoHttpCheckKeepAlive(ProtoHttpRefT *pState, const char *pUrl)
{
    ...
    if (pState->bConnOpen)
    {
        NetPrintfVerbose((pState->iVerbose, 0, "protohttpps: [%p] check for keep-alive detected connection close\n", pState));
        pState->bConnOpen = FALSE;
    }
...
void ProtoHttpUpdate(ProtoHttpRefT *pState)
{
    ...
    if (pState->eState != ST_IDLE && pState->eState != ST_DONE && pState->eState != ST_FAIL)
    {
        ...
        {
            NetPrintf(("protohttpps: [%p] server timeout (state=%d)\n", pState, pState->eState));
            pState->eState = ST_FAIL;
    ...
    if (pState->eState != ST_IDLE && pState->eState != ST_DONE && pState->eState != ST_FAIL)
    {
        if (pState->iSceRequestId > 0 && pState->bEpollInitialized)
        {
            ...
            if ( iResult > 0 && nbEvent.id == pState->iSceRequestId ) 
            {
                ...
                {
                    NetPrintf(("protohttpps: [%p] sceHttpWaitRequest failed (err=0x%08x)\n", pState, iResult));
                    pState->eState = ST_FAIL;
                    pState->iSslFail = 0;
                    pState->uSslFailDet = 0;
                    pState->iHresult = 0;
                    sceHttpsGetSslError(pState->iSceRequestId, &pState->iSslFail, &pState->uSslFailDet);
                    sceHttpGetLastErrno(pState->iSceRequestId, &pState->iHresult);
                    NetPrintf(("protohttpps: [%p] sceHttpWaitRequest failed with code 0x%08x for request id %d\n", pState, nbEvent.events, pState->iSceRequestId));
                }
            }
            else if (iResult < 0)
            {
                NetPrintf(("protohttpps: [%p] sceHttpWaitRequest failed with code %s\n", pState, DirtyErrGetName(iResult)));
            }
        }
    }
    ...
    if (pState->eState == ST_REDIR)
    {
        ...
        const char *pLocation = strLocation;
        uint8_t bFail = FALSE;
        ...
        {
            if ((iResult = _ProtoHttpFormatRequest(pState, &pLocation, NULL, 0, pState->eRequestType)) < 0)
            {
                bFail = TRUE;
            }
        }
        ...
        {
            _ProtoHttpSendRequest(pState, pLocation);
        }
    ...
    if (pState->eState == ST_RESP)
    {
        ...
        else if (iResult < 0)
        {
            ...
            sceHttpsGetSslError(pState->iSceRequestId, &pState->iSslFail, &pState->uSslFailDet);
            sceHttpGetLastErrno(pState->iSceRequestId, &pState->iHresult);
            NetPrintf(("protohttpps: [%p] sceHttpGetStatusCode failed with code %s for request id %d\n", pState, DirtyErrGetName(iResult), pState->iSceRequestId));
        }
        // regardless of whether the header code is a success or failure
        else
        {
            NetPrintf(("protohttpps: [%p] sceHttpGetStatusCode returned http code %d for request id %d, code %s\n", pState, iStatusCode, pState->iSceRequestId, DirtyErrGetName(iResult)));
            pState->eState = ST_HEAD;
        }
    }
    ...
    if (pState->eState == ST_HEAD)
    {
        ...
        if ((iResult = sceHttpGetResponseContentLength(pState->iSceRequestId, &contentLengthType, (uint64_t*)(&(pState->iBodySize)))) < 0)
        {
            ...
            sceHttpsGetSslError(pState->iSceRequestId, &pState->iSslFail, &pState->uSslFailDet);
            sceHttpGetLastErrno(pState->iSceRequestId, &pState->iHresult);
            NetPrintf(("protohttpps: [%p] sceHttpGetResponseContentLength failed with code %s for request id %d\n", pState, DirtyErrGetName(iResult), pState->iSceRequestId));
        }
...
int32_t ProtoHttpSetCACert2(const uint8_t *pCACert, int32_t iCertSize)
{
    ...
    while (pCert != NULL)
    {
        if (pCert->iCertSize == _CertSize)
        {
            ...
            {
                NetPrintf(("protohttpps: ignoring redundant add of CA cert\n"));
                iReturn = 0;
                break;
            }
        }
    ...
    if (iReturn > 0)
    {
        ...
        if (pNewCert != NULL)
        {
            ...
            {
                NetPrintf(("protohttpps: failed to allocate memory for CA cert\n"));
