...
/*!
    ...
    \Notes
        ...
        Memory Allocation
            ...
            - MakeAndInitialize() used to instantiate the XmlHttpCallback object
        ...
        Multithreading
            ...
            - MS worker thread(s): system thread notifying events by invoking XmlHttpCallback(s) 
...
class XmlHttpCallback : public Microsoft::WRL::RuntimeClass<Microsoft::WRL::RuntimeClassFlags<Microsoft::WRL::ClassicCom>, IXMLHTTPRequest2Callback>
{
    public:
        XmlHttpCallback();
        virtual ~XmlHttpCallback();
...
typedef enum ProtoHttpXmlReqStateE
{
    ...
    REQ_ST_WAITING_RESPONSE,     //!< waiting for one of the XmlHttpCallback event to be fired by the system
...
struct ProtoHttpXmlHttpT
{
    ...
    Microsoft::WRL::ComPtr<XmlHttpCallback> cpXmlHttpCallback; //!< callback functor
...
/*
    XmlHttpCallback: implementation of IXMLHTTPRequest2Callback interface required by
    IXMLHTTPGetRequest2().
*/

/*F********************************************************************************/
/*!
    \relates  XmlHttpCallback
    \Function XmlHttpCallback

    \Description
        XmlHttpCallback constructor

    \Version 11/01/2012 (akirchner)
*/
/********************************************************************************F*/
XmlHttpCallback::XmlHttpCallback() : m_pXmlHttp(NULL)
{
}
...
/*!
    ...
    \Description
        XmlHttpCallback destructor

    \Version 11/01/2012 (akirchner)
*/
/********************************************************************************F*/
XmlHttpCallback::~XmlHttpCallback()
{
}
...
*/
/********************************************************************************F*/
IFACEMETHODIMP XmlHttpCallback::OnRedirect(IXMLHTTPRequest2* pXHR, const WCHAR *pRedirectUrl)
{
...
*/
/********************************************************************************F*/
IFACEMETHODIMP XmlHttpCallback::OnHeadersAvailable(IXMLHTTPRequest2 *pXHR, DWORD dwStatus, const WCHAR *pwszStatus)
{
    ProtoHttpXmlHttpT *pXmlHttp = GetXmlHttp();
    ProtoHttpRefT *pHttpRef = pXmlHttp->pHttpRef;
...
*/
/********************************************************************************F*/
IFACEMETHODIMP XmlHttpCallback::OnDataAvailable(IXMLHTTPRequest2 *pXHR, ISequentialStream *pResponseStream)
{
    ProtoHttpXmlHttpT *pXmlHttp = GetXmlHttp();
    ProtoHttpRefT *pHttpRef = pXmlHttp->pHttpRef;
...
*/
/********************************************************************************F*/
IFACEMETHODIMP XmlHttpCallback::OnResponseReceived(IXMLHTTPRequest2 *pXHR, ISequentialStream *pResponseStream)
{
    ProtoHttpXmlHttpT *pXmlHttp = GetXmlHttp();
    ProtoHttpRefT *pHttpRef = pXmlHttp->pHttpRef;
...
*/
/********************************************************************************F*/
IFACEMETHODIMP XmlHttpCallback::OnError(IXMLHTTPRequest2 *pXHR, HRESULT hError)
{
    ProtoHttpXmlHttpT *pXmlHttp = GetXmlHttp();
    ProtoHttpRefT *pHttpRef = pXmlHttp->pHttpRef;
...
*/
/********************************************************************************F*/
ProtoHttpXmlHttpT * XmlHttpCallback::GetXmlHttp()
{
    return(m_pXmlHttp);
}
...
*/
/********************************************************************************F*/
void XmlHttpCallback::SetXmlHttp(ProtoHttpXmlHttpT *pXmlHttp)
{
    m_pXmlHttp = pXmlHttp;
}
...
static int32_t _ProtoHttpFormatRequestHeader(ProtoHttpRefT *pHttpRef, char *pStrTempBuf, int32_t iBufLen)
{
    ...
    {
        if ((iInpLen = pCustomHeaderCb(pHttpRef, pInpBuf, iInpMax, pUserData)) < 0)
        {
...
static int32_t _ProtoHttpOffloadedIXMLHTTPRequest2Creation(ProtoHttpXmlHttpT *pXmlHttp)
{
    ...
    // instantiate XmlHttpCallback object and set state ref
    if ((hResult = Microsoft::WRL::Details::MakeAndInitialize<XmlHttpCallback>(&pXmlHttp->cpXmlHttpCallback)) != S_OK)
    {
        NetPrintf(("protohttpxboxone: [%p][%p] failed to instantiate XmlHttpCallback object (err=%s)\n", pXmlHttp->pHttpRef, pXmlHttp, DirtyErrGetName(hResult)));
        return(-2);
    }
...
static void _ProtoHttpUpdateActiveRequest_RESP(ProtoHttpRefT *pHttpRef)
{
    ...
    if (pXmlHttp->eState == ProtoHttpXmlReqStateE::REQ_ST_ACCUMULATING_DATA)
    {
        ...
        if (pHttpRef->critical.pXmlHttp->cpOutStream.Get()->GetDataSize() == pHttpRef->critical.iRequestSize)
        {
            ...
            // reset RequestBuffer now because it is re-used for response handling in XmlHttpCallbacks
