...
static int32_t _ProtoHttp2FormatRequestHeader(ProtoHttp2RefT *pState, StreamInfoT *pStreamInfo, const char *pUrl, int32_t iDataLen, char **pOutHdr)
{
    ...
    {
        char strUserAgent[48];
        iHeaderLen += ds_snzprintf(pHeader+iHeaderLen, iHeaderMax-iHeaderLen, "user-agent: %s\r\n", ProtoHttpFormatUserAgent(strUserAgent, sizeof(strUserAgent), PROTOHTTP2_VERSION));
    }
    ...
    {
        if ((iHeaderLen = pCustomHeaderCb(pState, pHeader, iHeaderMax, pUserData)) < 0)
        {
...
/*!
    ...
    \Notes
        Calling this function is a destructive action, it completely removes the
        stream from the array. The slot is previously occupied might now be occupied
        by a different stream. If you make this call with any of the callbacks
        registered in this module, you will get undefined results. When doing cleanup
        it is wise to defer the calls outside of our callbacks.
