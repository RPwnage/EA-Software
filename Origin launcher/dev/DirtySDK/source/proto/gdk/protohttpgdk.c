...
//! maximum handles that we can track
#define PROTOHTTP_MAX_HANDLES (256)
...
struct ProtoHttpRefT
{
    ...
    HINTERNET aHandles[PROTOHTTP_MAX_HANDLES];          //!< handles that we own, which need to be cleanup up before destruction
    uint32_t uHandleCount;                              //!< current number of handles we are tracking
    ...
    NetCritT RecvCrit;                                  //!< protects the recv operation between the update and the microsoft threads
};
...
static int32_t _ProtoHttpFormatRequestHeader(ProtoHttpRefT *pHttpRef, const char *pUrl, const char *pHost, int32_t iPort, int32_t bSecure, const char *pRequest, int64_t iDataLen, const char *pAuthToken)
{
    ...
    {
        if ((iOffset = pCustomHeaderCb(pHttpRef, pHttpRef->pInpBuf, pHttpRef->iInpMax, pUserData)) < 0)
        {
...
/*!
    ...
    \Input *pHttpHandle     - handle specific to the status event
...
*/
/********************************************************************************F*/
static void CALLBACK _ProtoHttpStatusCallback(HINTERNET pHttpHandle, DWORD_PTR pContext, DWORD uStatus, LPVOID pStatusInfo, DWORD uStatusInfoLen)
{
    ...
    if (pHttpRef != NULL)
    {
        NetPrintfVerbose((pHttpRef->iVerbose, 2, "protohttpgdk: [%p][%p] _ProtoHttpStatusCallback -> %s\n", pHttpRef, pHttpHandle, PROTOHTTP_GetCallstackStatus(uStatus)));
    }
    else
    {
    ...
    // check if the handle belongs to a request; based on the testing these events are only associated with the request
    if ((pHttpHandle == pHttpRef->pHttpConnection) || (pHttpHandle == pHttpRef->pHttpRequest))
    {
        ...
        else if (uStatus == WINHTTP_CALLBACK_STATUS_DATA_AVAILABLE)
        {
            ...
            /* attempt to immediately handle the data available event instead of waiting for the next update
               this has shown to improve the download speeds */
            NetCritEnter(&pHttpRef->RecvCrit);
            _ProtoHttpUpdateBody(pHttpRef);
            NetCritLeave(&pHttpRef->RecvCrit);
        }
        ...
        else if (uStatus == WINHTTP_CALLBACK_STATUS_READ_COMPLETE)
        {
            ...
            /* attempt to immediately handle the read complete event instead of waiting for the next update
               this has shown to improve download speeds */
            NetCritEnter(&pHttpRef->RecvCrit);
            _ProtoHttpUpdateBody(pHttpRef);
            NetCritLeave(&pHttpRef->RecvCrit);
        }
    ...
    // handle the session levels status, if the session is still active we expect to still receive events
    if (uStatus == WINHTTP_CALLBACK_STATUS_HANDLE_CREATED)
    {
        HINTERNET pHandle = *(HINTERNET *)pStatusInfo;

        if (pHttpRef->uHandleCount < PROTOHTTP_MAX_HANDLES)
        {
            pHttpRef->aHandles[pHttpRef->uHandleCount++] = pHandle;
            NetPrintfVerbose((pHttpRef->iVerbose, 1, "protohttpgdk: [%p] added handle %p to tracking\n", pHttpRef, pHandle));
        }
        else
        {
            NetPrintf(("protohttpgdk: [%p] could not add any more handles for tracking (full)\n", pHttpRef));
        }
    }
    else if (uStatus == WINHTTP_CALLBACK_STATUS_HANDLE_CLOSING)
    {
        uint32_t uHandleIndex;
        HINTERNET *pHandle = *(HINTERNET *)pStatusInfo;

        // find the handle being closed in the array and remove it
        for (uHandleIndex = 0; uHandleIndex < pHttpRef->uHandleCount; uHandleIndex += 1)
        {
            if (pHttpRef->aHandles[uHandleIndex] != pHandle)
            {
                continue;
            }

            NetPrintfVerbose((pHttpRef->iVerbose, 1, "protohttpgdk: [%p] removing handle %p from tracking\n", pHttpRef, pHandle));

            // if we are not the last entry in the array then swap
            if (uHandleIndex != (pHttpRef->uHandleCount - 1))
            {
                pHttpRef->aHandles[uHandleIndex] = pHttpRef->aHandles[pHttpRef->uHandleCount - 1];
            }
            // check if we were the last handle (session) to make sure we unregister the callback
            if (pHttpRef->uHandleCount == 1)
            {
                WinHttpSetStatusCallback(pHandle, NULL, WINHTTP_CALLBACK_FLAG_ALL_NOTIFICATIONS, 0);
            }
            pHttpRef->uHandleCount -= 1;
...
static int32_t _ProtoHttpSendRequest(ProtoHttpRefT *pHttpRef)
{
    ...
    if (pHttpRef->bNewConnection == TRUE)
    {
        ...
        // set the context
        if (!WinHttpSetOption(pHttpRef->pHttpConnection, WINHTTP_OPTION_CONTEXT_VALUE, &pHttpRef, sizeof(pHttpRef)))
        {
            NetPrintf(("protohttpgdk: [%p] WINHTTP_OPTION_SET_CONTEXT_VALUE failed with %s\n", pHttpRef, DirtyErrGetName(GetLastError())));
        }
    ...
    // set the context
    if (!WinHttpSetOption(pHttpRef->pHttpRequest, WINHTTP_OPTION_CONTEXT_VALUE, &pHttpRef, sizeof(pHttpRef)))
    {
        NetPrintf(("protohttpgdk: [%p] WINHTTP_OPTION_SET_CONTEXT_VALUE failed with %s\n", pHttpRef, DirtyErrGetName(GetLastError())));
    }
...
{
    // if we have an active request, close it which will ignore any further updates
...
static void _ProtoHttpUpdateInit(ProtoHttpRefT *pHttpRef)
{
    ...
    // set the context
    if (!WinHttpSetOption(pHttpSession, WINHTTP_OPTION_CONTEXT_VALUE, &pHttpRef, sizeof(pHttpRef)))
    {
        NetPrintf(("protohttpgdk: [%p] WINHTTP_OPTION_SET_CONTEXT_VALUE failed with %s\n", pHttpRef, DirtyErrGetName(GetLastError())));
        return;
    }

    /* we don't have to lock here as we add the session before the callback
       from my experimentation, I don't get the handle creation event for this handle because it gets created before the registration */
    pHttpRef->aHandles[pHttpRef->uHandleCount++] = pHttpSession;
    NetPrintfVerbose((pHttpRef->iVerbose, 1, "protohttpgdk: [%p] added handle %p to tracking\n", pHttpRef, pHttpSession));
...
static void _ProtoHttpUpdateResp(ProtoHttpRefT *pHttpRef)
{
    ...
    else if ((pHttpRef->iPostSize == 0) || (pHttpRef->iPostSize == pHttpRef->iInpCnt))
    {
        ...
        /* set the header state (for parsing the response header). do it before calling WinHttpReceiveResponse in
           case it completes before the end of that call (on a different thread) */
...
static int32_t _ProtoHttpProcessRedirect(ProtoHttpRefT *pHttpRef)
{
    ...
    else if ((iSecure != pHttpRef->bSecure) || (ds_stricmp(strHost, pHttpRef->strHost) != 0) || (iPort != pHttpRef->iPort))
    {
    ...
    // if new connection, perform the necessary changes
    if (pHttpRef->bNewConnection == TRUE)
    {
        // reset keep-alive
...
static void _ProtoHttpUpdateHead(ProtoHttpRefT *pHttpRef)
{
    ...
    if (iResult == 0)
    {
        // set the pending query and body state as it could trigger synchronously (the callback will fire within the same stack)
...
static void _ProtoHttpUpdateBody(ProtoHttpRefT *pHttpRef)
{
    ...
    if ((uBytesMax = DS_MIN(pHttpRef->uBytesAvailable, (unsigned)(pHttpRef->iInpMax - pHttpRef->iInpLen))) > 0)
    {
        ...
        /* kick off the read of data
           this MUST be the last function call in the function as if the callback fires within the same stack we need to ensure consistency */
...
ProtoHttpRefT *ProtoHttpCreate(int32_t iBufSize)
{
    ...
    NetCritInit(&pHttpRef->RecvCrit, "ProtoHttpGDK-Recv");
...
{
    // close all the handles and register the external cleanup while we wait for the asynchronous operations to complete
    if (pHttpRef->pHttpRequest != NULL)
    {
        WinHttpCloseHandle(pHttpRef->pHttpRequest);
        pHttpRef->pHttpRequest = NULL;
...
int32_t ProtoHttpSend(ProtoHttpRefT *pHttpRef, const char *pData, int32_t iDataSize)
{
    ...
    /* handle optional data rate throttling. if we are not ending the stream, early return if the rate limiting prevents
       us from sending any data to prevent ending the stream early. if we are ending the stream, we don't want to be subject
       to rate limiting; at most we will just be sending the end chunk. */
    if ((iDataSize > 0) && (iDataSize = SocketRateThrottle(&pHttpRef->SendRate, SOCK_STREAM, iDataSize, "send")) == 0)
    {
        return(0);
    }
...
void ProtoHttpUpdate(ProtoHttpRefT *pHttpRef)
{
    ...
    // check for errors on the status callback
    if (pHttpRef->eState != PROTOHTTP_ST_FAIL)
    {
        if (pHttpRef->uSecureError != 0)
        {
            _ProtoHttpOnError(pHttpRef, pHttpRef->uSecureError, "WINHTTP_CALLBACK_STATUS_SECURE_FAILURE");
        }
        else if (pHttpRef->uProtocolError != 0)
        {
            _ProtoHttpOnError(pHttpRef, pHttpRef->uProtocolError, "WINHTTP_CALLBACK_STATUS_REQUEST_ERROR");
        }
    }
    ...
    if ((pHttpRef->eState == PROTOHTTP_ST_BODY) && NetCritTry(&pHttpRef->RecvCrit))
    {
        _ProtoHttpUpdateBody(pHttpRef);
        NetCritLeave(&pHttpRef->RecvCrit);
    }
