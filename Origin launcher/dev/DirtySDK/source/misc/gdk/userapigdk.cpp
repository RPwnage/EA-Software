...
/*F********************************************************************************/
/*!
    \Function _UserApiDestroyInternal
...
*/
/********************************************************************************F*/
static int32_t _UserApiDestroyInternal(void *pDestroyData)
{
    UserApiRefT *pRef = (UserApiRefT *)pDestroyData;
    int32_t iUserIndex;

    // loop through all our requests, return try again if any are still pending
    for (iUserIndex = 0; iUserIndex < NETCONN_MAXLOCALUSERS; iUserIndex += 1)
    {
        ...
        else if (pRequest->pPostCallback != NULL)
        {
            ...
            if (XAsyncGetStatus(&pRequest->PresenceBlock, false) == E_PENDING)
            {
                return(-1);
            }
        }
...
int32_t UserApiDestroy(UserApiRefT *pRef)
{
    ...
    NetConnControl('recu', 0, 0, (void *)_UserApiDestroyInternal, pRef);
    return(0);
}
...
/*!
    ...
    \Notes
        Cancelled operations will still fire a callback as cancellations happen
        asynchronously
...
int32_t UserApiCancel(UserApiRefT *pRef, uint32_t uUserIndex)
{
    ...
    if (pRequest->pCallback != NULL)
    {
        ...
        if (((pRequest->uUserDataMask & USERAPI_MASK_PRESENCE) || (pRequest->uUserDataMask & USERAPI_MASK_RICH_PRESENCE)) && (XAsyncGetStatus(&pRequest->PresenceBlock, false) == E_PENDING))
        {
            XAsyncCancel(&pRequest->PresenceBlock);
            bCancelled = TRUE;
        }
    }
    else if ((pRequest->pPostCallback != NULL) && (XAsyncGetStatus(&pRequest->PresenceBlock, false) == E_PENDING))
    {
        XAsyncCancel(&pRequest->PresenceBlock);
        bCancelled = TRUE;
    }

    // if there were no pending operations left, we can clear the request data
    if (!bCancelled)
    {
        ds_memclr(pRequest, sizeof(*pRequest));
    }
