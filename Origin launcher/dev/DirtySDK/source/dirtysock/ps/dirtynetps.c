...
/*!
    ...
    \Description
        Provides a wrapper that translates the PlayStation network interface to the
...
struct SocketT
{
    ...
    uint8_t bInKillList;        //!< TRUE if socket is in queued kill list, FALSE otherwise
    uint8_t __pad[2];
...
static int32_t _AddrFamilyToSceAddrFamily(int32_t iAddrFamily)
{
    ...
    switch(iAddrFamily)
    {
        case AF_INET:
            ...
            break;
        default:
            NetPrintf(("dirtynetps: CRITICAL ERROR Unsupported address family %d, using SCE_NET_AF_INET\n", iAddrFamily));
            iRetVal = SCE_NET_AF_INET;
            break;
    }
...
static int32_t _SocketTypeToSceSocketType(int32_t iType)
{
    ...
    switch(iType)
    {
        ...
        case SOCK_RAW:
            ...
            break;
        default:
            NetPrintf(("dirtynetps: CRITICAL ERROR Unsupported socket type %d\n", iType));
            iRetVal = iType;
            break;
    }
...
static int32_t _ProtocolToSceProtocol(int32_t iProto)
{
    ...
    switch(iProto)
    {
        ...
        case IPPROTO_UDP:
            ...
            break;
        default:
            NetPrintf(("dirtynetps: CRITICAL ERROR Unsupported socket protocol %d\n", iProto));
            iRetVal = iProto;
            break;
    }
    return(iRetVal);
...
static void _SocketNetCtlUpdate(SocketStateT *pState)
{
    ...
    else
    {
        ...
        {
            NetPrintf(("dirtynetps: sceNetCtlGetState() failed (err=%s)\n", DirtyErrGetName(iResult)));
            return;
        }
    ...
    if ((pState->uNetCtlConnectionState != SCE_NET_CTL_STATE_IPOBTAINED) && (iState == SCE_NET_CTL_STATE_IPOBTAINED))
    {
        ...
        // mark us as online
        NetPrintf(("dirtynetps: addr=%a mac=%02x:%02x:%02x:%02x:%02x:%02x\n", pState->uLocalAddr,
    ...
    {
        NetPrintf(("dirtynetps: got disconnect event at %d\n", NetTick()));
...
static int32_t _SocketCreateSocket(int32_t iAddrFamily, int32_t iType, int32_t iProto)
{
    ...
    if ((iSocket = sceNetSocket("DS socket", _AddrFamilyToSceAddrFamily(iAddrFamily), _SocketTypeToSceSocketType(iType), _ProtocolToSceProtocol(iProto))) >= 0)
    {
        ...
        if (iType == SOCK_DGRAM)
        {
            ...
            {
                NetPrintf(("dirtynetps: sceNetSetsockopt(SCE_NET_SOL_SOCKET, SCE_NET_SO_BROADCAST) failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
            }
        }
        ...
        if (iType == SOCK_RAW)
        {
            ...
            {
                NetPrintf(("dirtynetps: sceNetSetsockopt(SCE_NET_IPPROTO_IP, SCE_NET_IP_HDRINCL) failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
            }
        }
        // set nonblocking operation
        if ((iResult = sceNetSetsockopt(iSocket, SCE_NET_SOL_SOCKET, SCE_NET_SO_NBIO, &uTrue, sizeof(uTrue))) != 0)
        {
            NetPrintf(("dirtynetps: sceNetSetsockopt(SCE_NET_SOL_SOCKET, SCE_NET_SO_NBIO) failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
        }
    }
    else
    {
        NetPrintf(("dirtynetps: sceNetSocket() failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
    }
    return(iSocket);
}
...
static SocketT *_SocketOpen(int32_t iSocket, int32_t iAddrFamily, int32_t iType, int32_t iProto, int32_t iOpened)
{
    ...
    {
        NetPrintf(("dirtynetps: unable to allocate memory for socket\n"));
        return(NULL);
    }
    ...
    {
        NetPrintf(("dirtynetps: error %s creating socket\n", DirtyErrGetName(iSocket)));
    ...
    {
        NetPrintf(("dirtynetps: failed to create socket queue for socket\n"));
...
static SocketT *_SocketReopen(SocketT *pSocket)
{
    ...
    if (!pSocket->bVirtual && ((pSocket->iType == SOCK_DGRAM) || (pSocket->iType == SOCK_RAW)))
    {
        ...
        NetPrintf(("dirtynetps: trying to recreate the socket handle for %x->%d\n", pSocket, pSocket->uSocket));
        ...
        if (pSocket->uSocket != INVALID_SOCKET)
        {
            ...
            {
                NetPrintf(("dirtynetps: sceNetSocketClose(%d) failed (err=%s)\n", pSocket->uSocket, DirtyErrGetName(sce_net_errno)));
            }
        ...
        if ((pSocket->uSocket = _SocketCreateSocket(pSocket->iFamily, pSocket->iType, pSocket->iProto)) >= 0)
        {
            ...
            if (SockaddrInGetPort(&pSocket->LocalAddr) != 0)
            {
                ...
                if ((iResult = sceNetBind(pSocket->uSocket, &sceSocketAddr, sizeof(pSocket->LocalAddr))) < 0)
                {
                    ...
                    NetPrintf(("dirtynetps: sceNetBind() to %a:%d failed (err=%s)\n",
            ...
            pResult = pSocket;
            NetPrintf(("dirtynetps: socket recreation (%x->%d) succeeded.\n", pSocket, pSocket->uSocket));
        }
        else
        {
            pSocket->iLastError = _SocketTranslateError(pSocket->uSocket);
            NetPrintf(("dirtynetps: socket recreation (%x) failed.\n", pSocket));
        }
...
static int32_t _SocketClose(SocketT *pSocket)
{
    ...
    {
        NetPrintf(("dirtynetps: warning, trying to close socket 0x%08x that is not in the socket list\n", (intptr_t)pSocket));
        return(-1);
    }
...
static int32_t _SocketLookupDone(HostentT *pHost)
{
    ...
    if (pHost->done == 0)
    {
        ...
        if (iPollResult > 0)
        {
            ...
            {
                NetPrintf(("dirtynetps: sceNetResolverGetError() failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
            }
            if (iErrorResult != 0)
            {
                NetPrintf(("dirtynetps: sceNetResolverStartNtoa() failed %s, (err=%s)\n", pHost->strName, DirtyErrGetName(iErrorResult)));
                pHost->done = -1;
            }
            ...
            if (pHost->addr != 0)
            {
                ...
                pHost->done = 1;
                NetPrintf(("dirtynetps: lookup success; %s=%a\n", pHost->strName, pHost->addr));
                // add hostname to cache
                SocketHostnameCacheAdd(pState->pHostnameCache, pHost->strName, pHost->addr, pState->iVerbose);
            }
        }
        else if (iPollResult < 0)
        {
            NetPrintf(("dirtynetps: sceNetEpollWait() dns failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
            pHost->done = -1;
        }
    }
...
static void _SocketLookupFree(HostentT *pHost)
{
    ...
    if (pPriv->iSceResolverId >= 0)
    {
        ...
        {
            NetPrintf(("dirtynetps: sceNetResolverAbort(%d) failed (err=%s)\n", pPriv->iSceResolverId, DirtyErrGetName(sce_net_errno)));
        }

        if ((iResult = sceNetResolverDestroy(pPriv->iSceResolverId)) < 0)
        {
            NetPrintf(("dirtynetps: sceNetResolverDestroy(%d) failed (err=%s)\n", pPriv->iSceResolverId, DirtyErrGetName(sce_net_errno)));
        }
    ...
    if (pPriv->iSceMemPoolId >= 0)
    {
        ...
        {
            NetPrintf(("dirtynetps: sceNetPoolDestroy(%d) failed (err=%s)\n", pPriv->iSceMemPoolId, DirtyErrGetName(sce_net_errno)));
        }
    ...
    if (pPriv->iSceEpollId >= 0)
    {
        ...
        {
            NetPrintf(("dirtynetps: sceNetEpollDestroy(%d) failed (err=%s)\n", pPriv->iSceEpollId, DirtyErrGetName(sce_net_errno)));
        }
        pPriv->iSceEpollId = -1;
    }
...
/*!
    ...
    \Description
        Receive data on specified network socket.
...
static void _SocketRecvToPacketQueue(SocketStateT *pState, SocketT *pSocket)
{
    ...
    else
    {
        if (sce_net_errno != SCE_NET_EAGAIN)
        {
            ...
            {
                NetPrintfVerbose((pSocket->iVerbose, 0, "dirtynetps: [%p] connection %s\n", pSocket, (iRecvResult == 0) ? "closed" : "failed"));
                pSocket->iOpened = -1;
            }
            else
            {
                NetPrintf(("dirtynetps: [%p] _SocketRecvfrom() to packet queue returned %d (err=%s)\n", pSocket, iRecvResult, DirtyErrGetName(sce_net_errno)));
            }
        }
...
static SceNetId _SocketCreateEpoll()
{
    ...
    if (newId < 0)
    {
        NetPrintf(("dirtynetps: error, failed sceNetEpollCreate DirtySDK_SocketRecvThread (err=%s)\n", DirtyErrGetName(sce_net_errno)));
    }
    return(newId);
}
...
static void _SocketDestroyEpoll(SceNetId sceEpollId)
{
    if (sceEpollId >= 0)
    {
        ...
        {
            NetPrintf(("dirtynetps: error, failed sceNetEpollDestroy (err=%s)\n", DirtyErrGetName(sce_net_errno)));
        }
    }
}
...
static void _SocketAddToEpoll(SceNetId sceEpollId, SocketT *pSocket)
{
    ...
    {
        NetPrintf(("dirtynetps: error, failed sceNetEpollControl SCE_NET_EPOLL_CTL_ADD (err=%s)\n", DirtyErrGetName(sce_net_errno)));
    }
}
...
static void _SocketAddDnsToEpoll(SocketLookupPrivT *pPriv)
{
    ...
    if ((pPriv->iSceEpollId = sceNetEpollCreate("DirtySDK_DNS", 0)) >= 0)
    {
        ...
        if ((pPriv->iSceMemPoolId = sceNetPoolCreate("SocketLookupMemPool", 4 * 1024, 0)) >= 0)
        {
            ...
            {
                //begin lookup
                if ((iResult = sceNetResolverStartNtoa(pPriv->iSceResolverId, pPriv->Host.strName, (SceNetInAddr *)(&pPriv->Host.addr), 0, 5, SCE_NET_RESOLVER_ASYNC)) >= 0)
                {
                    ...
                    {
                        NetPrintf(("dirtynetps: error, failed sceNetEpollControl dns, SCE_NET_EPOLL_CTL_ADD(err=%s)\n", DirtyErrGetName(sce_net_errno)));
                        pPriv->Host.done = -1;
                    }
                }
                else
                {
                    NetPrintf(("dirtynetps: sceNetResolverStartNtoA() failed to start (err=%s)\n", DirtyErrGetName(iResult)));
                    pPriv->Host.done = -1;
                }
            }
            else
            {
                NetPrintf(("dirtynetps: sceNetResolverCreate() failed (err=%s)\n", DirtyErrGetName(pPriv->iSceResolverId)));
                pPriv->Host.done = -1;
            }
        }
        else
        {
            NetPrintf(("dirtynetps: sceNetPoolCreate() failed (err=%s)\n", DirtyErrGetName(pPriv->iSceMemPoolId)));
            pPriv->Host.done = -1;
        }
    }
    else
    {
        NetPrintf(("dirtynetps: error, failed sceNetEpollCreate DirtySDK_DNS (err=%s)\n", DirtyErrGetName(sce_net_errno)));
        pPriv->Host.done = -1;
    }
}
...
static void _SocketRecvThread(void *pArg)
{
    ...
    // show we are alive
    NetPrintf(("dirtynetps: recv thread running (thid=%s)\n", strThreadId));
    pState->iRecvLife = 1;
    ...
    iPollCount = 0;
        ...
        if (iPollCount > 0)
        {
            ...
            if (iPollResult < 0)
            {
                NetPrintf(("dirtynetps: sceNetEpollWait() failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
    ...
    NetPrintf(("dirtynetps: receive thread exit\n"));
    pState->iRecvLife = 0;
}
...
static uint8_t _SocketGetMacAddress(SocketStateT *pState)
{
    ...
    {
        NetPrintf(("dirtynetps: could not acquire MAC address (err=%s)\n", DirtyErrGetName(sce_net_errno)));
        return(FALSE);
    }
...
static int32_t _SocketInfoGlobal(int32_t iInfo, int32_t iData, void *pBuf, int32_t iLen)
{
    ...
    if (iInfo == 'addr')
    {
        if (pState->uLocalAddr == 0)
        {
            ...
            {
                NetPrintf(("dirtynetps: sceNetCtrlGetInfo(IP_ADDRESS) failed (err=%s)\n", DirtyErrGetName(iResult)));
            }
        }
    ...
    if (iInfo == 'sdcf')
    {
        ...
        NetPrintf(("dirtynetps: 'sdcf' selector used with invalid paramaters\n"));
        return(-1);
    }
    ...
    if (iInfo == 'sdcu')
    {
        ...
        NetPrintf(("dirtynetps: 'sdcu' selector used with invalid paramaters\n"));
        return(-1);
    }
    ...
    NetPrintf(("dirtynetps: unhandled global SocketInfo() selector '%C'\n", iInfo));
    return(-1);
}
...
static int32_t _SocketControlGlobal(int32_t iOption, int32_t iData1, void *pData2, void *pData3)
{
    ...
    if (iOption == 'conn')
    {
        NetPrintf(("dirtynetps: setting network ~con state.\n"));
    ...
    if (iOption == 'disc')
    {
        NetPrintf(("dirtynetps: setting network -off state.\n"));
        pState->bNetEventOccured = TRUE;
    ...
    if (iOption == 'maxp')
    {
        NetPrintf(("dirtynetps: setting max udp packet size to %d\n", iData1));
    ...
    if (iOption == 'vadd')
    {
        ...
        {
            NetPrintfVerbose((pState->iVerbose, 1, "dirtynetps: added port %d to virtual port list\n", iData1));
    ...
    if (iOption == 'vdel')
    {
        ...
        if (iPort < SOCKET_MAXVIRTUALPORTS)
        {
            NetPrintfVerbose((pState->iVerbose, 1, "dirtynetps: removed port %d from virtual port list\n", iData1));
            pState->aVirtualPorts[iPort] = 0;
            return(0);
        }
    }
    // unhandled
    NetPrintf(("dirtynetps: unhandled global SocketControl() option '%C'\n", iOption));
    return(-1);
}
...
int32_t SocketCreate(int32_t iThreadPrio, int32_t iThreadStackSize, int32_t iThreadCpuAffinity)
{
    ...
    if ((iResult = sceNetInit()) < 0)
    {
        ...
        {
            NetPrintf(("dirtynetps: warning -- sceNet already initialized; sceNetTerm() will not be called on destroy\n"));
            pState->bNetAlreadyInitialized = TRUE;
        }
        else
        {
            NetPrintf(("dirtynetps: sceNetInit() failed err=%s\n", DirtyErrGetName(iResult)));
            return(-4);
        }
    }
    else
    ...
    {
        NetPrintf(("dirtynetps: unable to create hostname cache\n"));
    ...
    {
        NetPrintf(("dirtynetps: unable to create socket recv thread (err=%d)\n", iResult));
        pState->iRecvLife = 0;
        SocketDestroy(0);
        return(-6);
    }
...
int32_t SocketDestroy(uint32_t uShutdownFlags)
{
    ...
    if (pState == NULL)
    {
        NetPrintf(("dirtynetps: SocketDestroy() called while module is not active\n"));
        return(-1);
    }

    NetPrintf(("dirtynetps: shutting down\n"));
    ...
    {
        NetPrintf(("dirtynetps: sceNetTerm() failed (err=%s)\n", DirtyErrGetName(iResult)));
    }
    #endif
...
SocketT *SocketImport(intptr_t uSockRef)
{
    ...
    {
        NetPrintf(("dirtynetps: sceNetGetsockopt(SCE_NET_SO_TYPE) failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
    }
...
int32_t SocketShutdown(SocketT *pSocket, int32_t iHow)
{
    ...
    if (sceNetShutdown(pSocket->uSocket, iHow) < 0)
    {
        ...
        NetPrintf(("dirtynetps: sceNetShutdown(%d) failed (err=%s)\n", pSocket->uSocket, DirtyErrGetName(iResult)));
    }
...
int32_t SocketBind(SocketT *pSocket, const struct sockaddr *pName, int32_t iNameLen)
{
    ...
    {
        NetPrintf(("dirtynetps: attempt to bind invalid socket\n"));
        pSocket->iLastError = SOCKERR_INVALID;
        return(pSocket->iLastError);
    }
    ...
    if (pSocket->iType == SOCK_DGRAM)
    {
        ...
        if ((uPort = SockaddrInGetPort(pName)) != 0)
        {
            ...
            if (iPort < SOCKET_MAXVIRTUALPORTS)
            {
                ...
                {
                    NetPrintf(("dirtynetps: [%p] failed to bind socket to %u which was already bound to port %u virtual\n", pSocket, uPort, pSocket->uVirtualport));
                    NetCritLeave(&pSocket->RecvCrit);
                    return(pSocket->iLastError = SOCKERR_INVALID);
                }

                // close winsock socket
                NetPrintf(("dirtynetps: [%p] making socket bound to port %d virtual\n", pSocket, uPort));
                if (pSocket->uSocket != INVALID_SOCKET)
                {
    ...
    {
        NetPrintf(("dirtynetps: sceNetBind() to port %d failed (err=%s)\n", SockaddrInGetPort(pName), DirtyErrGetName(sce_net_errno)));
    }
    else if (SockaddrInGetPort(&pSocket->LocalAddr) == 0)
    {
        ...
        NetPrintf(("dirtynetps: sceNetBind(port=0) succeeded, local address=%a:%d.\n",
                   SockaddrInGetAddr(&pSocket->LocalAddr),
                   SockaddrInGetPort(&pSocket->LocalAddr)));
    }
...
int32_t SocketConnect(SocketT *pSocket, struct sockaddr *pName, int32_t iNameLen)
{
    ...
    NetPrintfVerbose((pSocket->iVerbose, 0, "dirtynetps: connecting to %a:%d\n", SockaddrInGetAddr(pName), SockaddrInGetPort(pName)));
    ...
    {
        NetPrintf(("dirtynetps: sceNetConnect() failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
    }
...
int32_t SocketListen(SocketT *pSocket, int32_t iBacklog)
{
    ...
    {
        NetPrintf(("dirtynetps: sceNetListen() failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
    }
...
SocketT *SocketAccept(SocketT *pSocket, struct sockaddr *pAddr, int32_t *pAddrLen)
{
    ...
    else
    {
        ...
        {
            NetPrintf(("dirtynetps: accept() failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
        }
    }
...
int32_t SocketSendto(SocketT *pSocket, const char *pBuf, int32_t iLen, int32_t iFlags, const struct sockaddr *pTo, int32_t iToLen)
{
    ...
    if (pSocket->uSocket < 0)
    {
        ...
        NetPrintf(("dirtynetps: attempting to send to %a:%d on invalid socket\n", uAddr, uPort));
        #endif
        pSocket->iLastError = SOCKERR_INVALID;
        return(pSocket->iLastError);
    }
    ...
    if (pTo == NULL)
    {
        ...
        {
            NetPrintf(("dirtynetps: sceNetSend() failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
        }
    }
    else
    {
...
int32_t SocketRecvfrom(SocketT *pSocket, char *pBuf, int32_t iLen, int32_t iFlags, struct sockaddr *pFrom, int32_t *pFromLen)
{
    ...
    else // non-async recvthread socket
    {
        ...
        {
            NetPrintf(("dirtynetps: _SocketRecvfrom() failed on a SOCK_STREAM socket (err=%s)\n", DirtyErrGetName(iErrno)));
        }
    }
...
int32_t SocketInfo(SocketT *pSocket, int32_t iInfo, int32_t iData, void *pBuf, int32_t iLen)
{
    ...
    */
    ...
    if (iInfo == 'stat')
    {
        ...
        {
            NetPrintf(("dirtynetps: sceNetEpollCreate failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
            pSocket->iOpened = -1;
        ...
        {
            NetPrintf(("dirtynetps: sceNetEpollControl failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
        ...
        if ((iResult = sceNetEpollWait(eId, &eventPollOut, 1, 0)) > 0)
        {
            if (pSocket->iOpened == 0)
            {
                if (eventPollOut.events & SCE_NET_EPOLLOUT)
                {
                    ...
                    {
                        NetPrintfVerbose((pSocket->iVerbose, 0, "dirtynetps: connection open\n"));
                        pSocket->iOpened = 1;
                    }
                    else if (iResult < 0)
                    {
                        NetPrintfVerbose((pSocket->iVerbose, 0, "dirtynetps: sceNetGetsockopt() failed (err=%s)\n", DirtyErrGetName(iResult)));
                        pSocket->iOpened = -1;
                    }
                    else
                    {
                        NetPrintfVerbose((pSocket->iVerbose, 0, "dirtynetps: connection failed (err=%s)\n", DirtyErrGetName(iError)));
                        pSocket->iOpened = -1;
                    }
                }
                else if (eventPollOut.events & (SCE_NET_EPOLLHUP|SCE_NET_EPOLLERR))
                {
                    NetPrintfVerbose((pSocket->iVerbose, 0, "dirtynetps: read exception on connect\n"));
                    pSocket->iOpened = -1;
                }
            }
            ...
            else if (!pSocket->bAsyncRecv && (pSocket->iOpened > 0))
            {
                if (eventPollOut.events & SCE_NET_EPOLLIN)
                {
                    ...
                    {
                        //sceNetRecv Error
                        NetPrintfVerbose((pSocket->iVerbose, 0, "dirtynetps: connection closed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
                ...
                {
                    NetPrintfVerbose((pSocket->iVerbose, 0, "dirtynetps: connection failure\n"));
                    pSocket->iOpened = -1;
                }
            }        
    ...
    // unhandled option?
    NetPrintf(("dirtynetps: unhandled SocketInfo() option '%C'\n", iInfo));
    return(-1);
}
...
int32_t SocketControl(SocketT *pSocket, int32_t iOption, int32_t iData1, void *pData2, void *pData3)
{
    ...
    if (iOption == 'maxr')
    {
        NetPrintf(("dirtynetps: setting max recv rate to %d bytes/sec\n", iData1));
    ...
    if (iOption == 'maxs')
    {
        NetPrintf(("dirtynetps: setting max send rate to %d bytes/sec\n", iData1));
    ...
    if (iOption == 'scbk')
    {
        ...
        {
            NetPrintf(("dirtynetps: send callbacks usage changed from %s to %s for socket ref %p\n", (pSocket->bSendCbs?"ON":"OFF"), (iData1?"ON":"OFF"), pSocket));
    ...
    */
    ...
    if ((iOption == 'ndly') && (pSocket->iType == SOCK_STREAM))
    {
        ...
        {
            NetPrintf(("dirtynetps: sceNetSetsockopt(SCE_NET_IPPROTO_TCP, SCE_NET_TCP_NODELAY) failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
        }
        pSocket->iLastError = _SocketTranslateError(iResult);
        return(pSocket->iLastError);
    }
    ...
    if (iOption == 'push')
    {
        ...
        {
            NetPrintf(("dirtynetps: request to push %d bytes of data discarded (max=%d)\n", iData1, SOCKET_MAXUDPRECV));
            NetCritLeave(&pSocket->RecvCrit);
            return(-1);
        }
    ...
    if (iOption == 'radr')
    {
        ...
        {
            NetPrintf(("dirtynetps: sceNetSetsockopt(SCE_NET_SOL_SOCKET, SCE_NET_SO_REUSEADDR) failed (err=%s)\n", DirtyErrGetName(sce_net_errno)));
        }
        pSocket->iLastError = _SocketTranslateError(iResult);
        return(pSocket->iLastError);
    }
    ...
    if ((iOption == 'rbuf') || (iOption == 'sbuf'))
    {
        ...
        NetPrintf(("dirtynetps: sceNetGetsockopt(%s) changed buffer size from %d to %d\n", (iOption == 'rbuf') ? "SCE_NET_SO_RCVBUF" : "SCE_NET_SO_SNDBUF",
            iOldSize, iNewSize));
    ...
    // unhandled
    NetPrintf(("dirtynetps: unhandled control option '%C'\n", iOption));
    return(-1);
}
...
HostentT *SocketLookup(const char *pText, int32_t iTimeout)
{
    ...
    NetPrintf(("dirtynetps: looking up address for host '%s'\n", pText));
    ...
    ds_strnzcpy(pPriv->Host.strName, pText, sizeof(pPriv->Host.strName));
    ...
    if ((pHostRef = (SocketLookupPrivT *)SocketHostnameAddRef(&pState->pHostList, &pPriv->Host, TRUE)) != NULL)
    {
        ...
        {
            NetPrintf(("dirtynetps: adding new lookup since current matching ref is being destroyed.\n"));
            // undo the refcount done by SocketHostnameAddRef(,,True)
            pHostRef->Host.iRefCount -= 1;
