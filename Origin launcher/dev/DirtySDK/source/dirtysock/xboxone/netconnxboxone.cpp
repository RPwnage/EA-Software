...
typedef struct NetConnRefT
{
    ...
    uint32_t                    uIfType;                     //!< interface adapter type
...
static void _NetworkStatusThread(void *pUserData)
{
    ...
    while (pRef->hNetworkStatusThread != 0)
    {
        ...
        if (WaitForSingleObject(pRef->hNetworkStatusEvent, INFINITE) == WAIT_OBJECT_0)
        {
            ...
            try
            {
                // Note this call takes ~3000 microseconds
                ...
                if (refConnectionProfile != nullptr)
                {
                   ...
                   // Experimentation on xboxone results in wired interface returning ethernetCsmacd(6) and wireless interface returning ieee80211(71)
                   pRef->uIfType = NetConnTranslateIANAIfType(refConnectionProfile->NetworkAdapter->IanaInterfaceType);
                }
                else
                {
                   ...
                   pRef->uIfType = NETCONN_IFTYPE_NONE;
                }
            }
            catch (Exception ^ e)
            {
...
static void _NetConnAddLocalUser(NetConnLocalUserRefT *pLocalUserRef, int32_t iLocalUserIndex, const EA::User::IEAUser *pIEAUser)
{
    ...
    if (pRef->aEAUsers[iLocalUserIndex] == NULL)
    {
        if (_NetConnInitUser(pRef, iLocalUserIndex, pCapilanoUser) == 0)
        {
            ...
            NetPrintf(("netconnxboxone: netconn user added at local user index %d - xuid=%llu, gtag=%s, localUserId=0x%08x\n",
...
static void _NetConnRemoveLocalUser(NetConnLocalUserRefT *pLocalUserRef, int32_t iLocalUserIndex, const EA::User::IEAUser *pIEAUser)
{
    ...
    if (pRef->aEAUsers[iLocalUserIndex] != NULL)
    {
        ...
        {
                NetPrintf(("netconnxboxone: netconn user removed at local user index %d - xuid=%llu, gtag=%s, localUserId=0x%08x\n",
...
int32_t NetConnStartup(const char *pParams)
{
    ...
    // default interface adapter type, will eventually be overwritten with value obtained from connection profile
    pRef->uIfType = NETCONN_IFTYPE_NONE;
