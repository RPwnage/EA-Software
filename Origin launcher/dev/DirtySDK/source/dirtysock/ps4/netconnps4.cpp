...
#if defined(DIRTYCODE_PS4_CROSSGEN)
#include <np_cg.h>
...
#if defined(DIRTYCODE_PS4_CROSSGEN)
//By Blaze/Identity spec this is intentionally the same/reused for both PS5 and cross-gen PS4:
...
static void _NetConnNpCheckNpAvailabilityError(NetConnRefT *pRef, uint8_t bErrorAtStart)
{
    ...
    switch (iFirstErrorResult)
    {
        ...
        case SCE_NP_ERROR_REQUEST_NOT_FOUND:
            NetPrintf(("netconnps4:%s() error: %s \n", bErrorAtStart == TRUE ? "_NetConnNpAvailabilityRequest" : "_NetConnQueryNpUserIdle", DirtyErrGetName(iFirstErrorResult)));
        ...
        default:
            NetPrintf(("netconnps4:%s() unknown error: %s\n", bErrorAtStart == TRUE ? "_NetConnStartQueryNpUser" : "_NetConnQueryNpUserIdle", DirtyErrGetName(iFirstErrorResult)));
            _NetConnUpdateConnStatus(pRef, '-err');
            break;
    }
}
...
static uint32_t _NetConnGetInterfaceType(NetConnRefT *pRef)
{
    ...
    if (uIfType != NETCONN_IFTYPE_NONE)
    {
        // check for pppoe
        if ((iResult = sceNetCtlGetInfo(SCE_NET_CTL_INFO_IP_CONFIG, &Info)) == 0)
        {
...
/*!
    ...
    \Notes
        ...
        \verbatim
            -notitleid                  - skip call to sceNpSetNpTitleId()
            -noupnp                     - disable upnp support
            -nptoolkit                  - enable nptoolkit-compliant mode, will override setting from NetConnStartup()
            -titleid=titleidstr         - specify title id (default "NPXX51027_00"), will override setting from NetConnStartup()
...
int32_t NetConnStartup(const char *pParams)
{
    ...
    // do not call sceNpSetNpTitleId() if "notitleid" is specified
    ...
    ds_memclr(&WebApiCreateParams, sizeof(WebApiCreateParams));
    if ((pRef->pWebApi = DirtyWebApiCreate(&WebApiCreateParams)) == NULL)
    {
        _NetConnShutdownInternal(pRef, 0);
        NetPrintf(("netconnps4: counld not initialize DirtyWebApi\n"));
        return(NETCONN_ERROR_PLATFORM_SPECIFIC);
    }
...
int32_t NetConnStatus(int32_t iKind, int32_t iData, void *pBuf, int32_t iBufSize)
{
    ...
    if (iKind == 'pscd')
    {
        ...
        {
            NetPrintf(("netconnps4: NetConnStatus('pscd') NetConnStatus('pscd')"));
            return(-1);
        }

        if (DirtyWebApiDeletePushContext(pRef->pWebApi, iData, (SceNpWebApi2PushEventPushContextId *)pBuf) < 0)
        {
            NetPrintf(("netconnps4: NetConnStatus('pscd') DirtyWebApiDeletePushContext failed for user index %d\n", iData));
            return(-2);
        }
    ...
    if (iKind == 'pscx')
    {
        if ((iData < 0) || (iData >= NETCONN_MAXLOCALUSERS) || (pBuf == NULL) || (iBufSize < (int32_t)sizeof(SceNpWebApi2PushEventPushContextId)))
        {
            NetPrintf(("netconnps4: NetConnStatus('pscx'), invalid arguments.\n"));
            return(-1);
        }

        if (pRef->pWebApi == NULL)
        {
            NetPrintf(("netconnps4: NetConnStatus('pscx') webapi ref is null!\n"));
            return(-2);
        }

        if (DirtyWebApiCreatePushContext(pRef->pWebApi, iData, (SceNpWebApi2PushEventPushContextId *)pBuf) < 0)
        {
            NetPrintf(("netconnps4: NetConnStatus('pscx') DirtyWebApiCreatePushContext failed for index %d\n", iData));
            return(-3);
        }
