...
//! ascii to hex translation table
static const uint8_t _ds_ascii2hex[128] =
{
...
/*!
    ...
    \Description
        Format a binary blob as hexadecimal text
...
/*F********************************************************************************/
/*!
    \Function ds_fmtoctstring2

    \Description
        Format a binary blob using hexidemical text

    \Input *pOutput - [out] out buffer for the blob
    \Input iOutLen  - size of output buffer
    \Input *pInput  - input text data
    \Input iInpLen  - size of input data
...
*/
/********************************************************************************F*/
uint8_t *ds_fmtoctstring2(uint8_t *pOutput, int32_t iOutLen, const char *pInput, int32_t iInpLen)
{
    int32_t iInput, iOutput;

    for (iInput = 0, iOutput = 0; (iInput < iInpLen) && (iOutput < iOutLen); iInput += 2, iOutput += 1)
    {
        pOutput[iOutput] = (_ds_ascii2hex[(int32_t)pInput[iInput+0]] << 4) | _ds_ascii2hex[(int32_t)pInput[iInput+1]];
    }
    return(pOutput);
}
