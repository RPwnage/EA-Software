...
#if !defined(DIRTYCODE_PS5) && !defined(DIRTYCODE_PS4_CROSSGEN)
...
#define VOIPHEADSET_BLOCKLIST_INTERVAL                  (60*1000)                                                   //!< TRCs don't specify how long we need to handle this in
...
typedef struct
{
    ...
    SceNpAccountId uSceAccountId;   //!< sce account identifier
    ...
    int8_t iOutboundNarrating;      //!< narration status; zero=inactive, one=narrating, negative=done
    ...
    int32_t iPushEventRequestId;    //!< request id associated with the block list updated event
    ...
    uint32_t uLocalNarratePortId;   //!< this is the port to write to when donig local narrate
} PS4LocalVoipUserT;
...
struct VoipHeadsetRefT
{
    ...
    #if defined(DIRTYCODE_PS5)
    uint8_t bChatEnabled;                   //!< has chat been enabled/disabled with the platform
    #else
    uint8_t _pad;
    #endif
    ...
    uint8_t aMemPool[SCE_VOICE_MEMORY_CONTAINER_SIZE_V3];
    ...
    DirtyWebApiRefT *pWebApi;               //!< webapi module to get blocked user change notifications
...
static int32_t _VoipHeadsetSilence(VoipHeadsetRefT *pHeadset, PS4LocalVoipUserT *pLocalUser, const int16_t *pSamples, int32_t iFrameBytes)
{
    ...
    if (iNumSamples > 0)
    {
        ...
        {
            NetPrintfVerbose(((pLocalUser->iVadSilenceSamples < pHeadset->iVadSilenceThreshold && (pLocalUser->iVadSilenceSamples+iNumSamples >= pHeadset->iVadSilenceThreshold)) ? pHeadset->iDebugLevel : 0, 1, "voipheadsetps: silence detected\n"));
            pLocalUser->iVadSilenceSamples += iNumSamples;
        }
        else
        {
            NetPrintfVerbose(((pLocalUser->iVadSilenceSamples >= pHeadset->iVadSilenceThreshold) ? pHeadset->iDebugLevel : 0, 1, "voipheadsetps: voice detected\n"));
...
static void _VoipAudioInputThread(void *pArg)
{
    ...
    for (iBuffer = 0; pLocalUser->uMicPortId != 0; )
    {
        ...
        {
            NetPrintf(("voipheadsetps: sceAudioInInput(%d) failed (err=%s)\n", pLocalUser->iLocalUserIndex, DirtyErrGetName(iResult)));
            break;
        }
        ...
        if (pLocalUser->iOutboundNarrating == 0)
        {
    ...
    // done
    NetPrintf(("voipheadsetps: audioin thread exiting\n"));
}
...
static int32_t _VoipHeadsetAudioThreadCreate(PS4LocalVoipUserT *pLocalUser)
{
    ...
    {
        NetPrintf(("voipheadsetps: audioin thread created (err=%d)\n", iResult));
    }
    else
    {
        NetPrintf(("voipheadsetps: audioin thread creation failed (err=%d)\n", iResult));
    }
...
static VoipTranscribeRefT *_VoipHeadsetTranscribeCheckCreate(VoipHeadsetRefT *pHeadset, PS4LocalVoipUserT *pLocalUser)
{
    ...
    if ((pLocalUser->pTranscribeRef == NULL) && ((pLocalUser->pTranscribeRef = VoipTranscribeCreate(0)) != NULL))
    {
        ...
        #endif
        // set language 
        VoipTranscribeControl(pLocalUser->pTranscribeRef, 'lang', 0, 0, pHeadset->strLang);
    }
...
static void _VoipHeadsetVoiceEventCb(SceVoiceEventData *pEventData)
{
    ...
    {
        NetPrintf(("voipheadsetps: SCE_VOICE_EVENT_AUDIOINPUT_CONTROL_STATUS event\n"));
    }
    else if (pEventData->eventType != SCE_VOICE_EVENT_DATA_READY)
    {
        NetPrintf(("voipheadsetps: unknown event\n"));
    }
}
...
static int32_t _VoipHeadsetVoiceLibInit(VoipHeadsetRefT *pHeadset)
{
    ...
    sceVoiceInitParams.appType = SCE_VOICE_APPTYPE_GAME_V3;
    ...
    {
        NetPrintf(("voipheadsetps: successfully initialized Sony Voice Lib\n", DirtyErrGetName(iResult)));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceInit() failed (err=%s)\n", DirtyErrGetName(iResult)));
    ...
    if ((iResult = sceVoiceGetResourceInfo(&sceVoiceResourceInfo)) == SCE_OK)
    {
        ...
        NetPrintf(("voipheadsetps: max IN_VOICE ports supported by the Voice Lib =        %d\n", sceVoiceResourceInfo.maxInVoicePort));
        NetPrintf(("voipheadsetps: max OUT_VOICE ports supported by the Voice Lib =       %d\n", sceVoiceResourceInfo.maxOutVoicePort));
        NetPrintf(("voipheadsetps: max IN_DEVICE ports supported by the Voice Lib =       %d\n", sceVoiceResourceInfo.maxInDevicePort));
        NetPrintf(("voipheadsetps: max OUT_DEVICE ports supported by the Voice Lib =      %d\n", sceVoiceResourceInfo.maxOutDevicePort));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceGetResourceInfo() failed (err=%s)\n", DirtyErrGetName(iResult)));
    }
...
static int32_t _VoipHeadsetVoiceLibShutdown(void)
{
    ...
    {
        NetPrintf(("voipheadsetps: successfully shut down Sony Voice Lib\n"));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceEnd() failed (err=%s)\n", DirtyErrGetName(iResult)));
        iRetCode = -1;  // signal failure
    }
...
static int32_t _VoipHeadsetVoiceSetThreadsParams(VoipHeadsetRefT *pHeadset, SceKernelCpumask cpuAffinity, int32_t iSchedPriority)
{
    ...
    {
        NetPrintf(("voipheadsetps: successfully modified attributes of Sony voip threads - affinity is 0x%02x and priority is %d\n", cpuAffinity, iSchedPriority));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceSetThreadsParams() failed (err=%s)\n", DirtyErrGetName(iResult)));
        iRetCode = -1;  // signal failure
    }
...
static int32_t _VoipHeadsetVoiceLibStart(VoipHeadsetRefT *pHeadset)
{
    ...
    {
        NetPrintf(("voipheadsetps: successfully started Sony Voice Lib\n"));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceStart() failed (err=%s)\n", DirtyErrGetName(iResult)));
        iRetCode = -1;  // signal failure
    }
...
static int32_t _VoipHeadsetVoiceLibStop(void)
{
    ...
    {
        NetPrintf(("voipheadsetps: successfully stopped Sony Voice Lib\n"));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceStop() failed (err=%s)\n", DirtyErrGetName(iResult)));
        iRetCode = -1;  // signal failure
    }
...
static int32_t _VoipHeadsetPortDelete(uint32_t uPortId)
{
    ...
    {
        NetPrintf(("voipheadsetps: sceVoiceDeletePort(%d) succeeded\n", uPortId));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceDeletePort(%d) failed (err=%s)\n", uPortId, DirtyErrGetName(iResult)));
        iRetCode = -1;  // signal failure
    }
...
static int32_t _VoipHeadsetSpkrPortCreate(int32_t iUserIndex, uint32_t *pPortId)
{
    ...
    {
        NetPrintf(("voipheadsetps: created OUT_DEVICE port %d for user index %d (user ID = %d/0x%08x)\n", *pPortId, iUserIndex, sceUserId, sceUserId));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceCreatePort(OUT_DEVICE, %d) failed (err=%s)\n", iUserIndex, DirtyErrGetName(iResult)));
        iRetCode = -1;  // signal failure
    }
...
static int32_t _VoipHeadsetEncoderPortCreate(int32_t iBitRate, uint32_t *pPortId)
{
    ...
    if ((iResult = sceVoiceCreatePort(pPortId, &sceVoicePortParam)) == SCE_OK)
    {
        NetPrintf(("voipheadsetps: created OUT_VOICE port %d\n", *pPortId));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceCreatePort(OUT_VOICE) failed (err=%s)\n", DirtyErrGetName(iResult)));
        iRetCode = -1;  // signal failure
    }
...
static int32_t _VoipHeadsetDecoderPortCreate(int32_t iBitRate, uint32_t *pPortId)
{
    ...
    if ((iResult = sceVoiceCreatePort(pPortId, &sceVoicePortParam)) == SCE_OK)
    {
        NetPrintf(("voipheadsetps: created IN_VOICE port %d\n", *pPortId));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceCreatePort(IN_VOICE) failed (err=%s)\n", DirtyErrGetName(iResult)));
        iRetCode = -1;  // signal failure
    }
...
static int32_t _VoipHeadsetConnectPorts(uint32_t uInPortId, uint32_t uOutPortId)
{
    ...
    {
        NetPrintf(("voipheadsetps: successfully connected IN port %d to OUT port %d\n", uInPortId, uOutPortId));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceCConnectIPortToOport(%d, %d) failed (err=%s)\n", uInPortId, uOutPortId, DirtyErrGetName(iResult)));
    }
    return(iResult);
}
...
static int32_t _VoipHeadsetDisconnectPorts(uint32_t uInPortId, uint32_t uOutPortId)
{
    ...
    {
        NetPrintf(("voipheadsetps: successfully disconnected IN port %d from OUT port %d\n", uInPortId, uOutPortId));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceDisconnectIPortFromOport(%d, %d) failed (err=%s)\n", uInPortId, uOutPortId, DirtyErrGetName(iResult)));
    }
    return(iResult);
}
...
static int32_t _VoipHeadsetWriteToPort(uint32_t uInPortId, const uint8_t *pFrame, uint32_t uFrameSize)
{
    ...
    {
        NetPrintf(("voipheadsetps: sceVoiceWriteToIPort(%d, %d) failed (err=%s)\n", uInPortId, uFrameSize, DirtyErrGetName(iResult)));
        iResult = -1;
    }
...
static int32_t _VoipHeadsetReadFromPort(uint32_t uOutPortId, const uint8_t *pFrame, uint32_t *pFrameSize)
{
    ...
    {
        NetPrintf(("voipheadsetps: sceVoiceReadFromOPort(%d) failed (err=%s)\n", uOutPortId,  DirtyErrGetName(iResult)));
    }
    return(iResult);
}
...
static int32_t _VoipHeadsetIsLocalVoiceFrameReady(uint32_t uOutPortId)
{
    ...
    if ((iResult = sceVoiceGetPortInfo(uOutPortId, &sceVoiceBasePortInfo)) == SCE_OK)
    {
        ...
        {
            NetPrintf(("voipheadsetps: sceVoiceGetPortInfo(%d) called on port of wrong type (%d)\n", uOutPortId, sceVoiceBasePortInfo.portType));
            return(-1);
        }
    ...
    {
        NetPrintf(("voipheadsetps: sceVoiceGetPortInfo(%d) failed (err=%s)\n", uOutPortId, DirtyErrGetName(iResult)));
    }
...
static int32_t _VoipHeadsetIsMicDetected(uint32_t uInPortId, uint8_t *pMicDetected)
{
    ...
    {
        NetPrintf(("voipheadsetps: sceAudioInGetSilentState(%d) failed (err=%s)\n", uInPortId, DirtyErrGetName(iResult)));
    }
...
static int32_t _VoipHeadsetDecoderDisconnect(VoipHeadsetRefT *pHeadset, PS4LocalVoipUserT *pLocalUser, PS4DecoderT *pDecoder)
{
    ...
    if ((iResult = _VoipHeadsetDisconnectPorts(*pDecoderPortId, pLocalUser->uSpkrPortId)) == SCE_OK)
    {
        ...
        if (pDecoder->uConnectedSpkrPort == 0)
        {
            ...
            {
                NetPrintf(("voipheadsetps: failed to reset decoder port %d (err=%s)\n", *pDecoderPortId, DirtyErrGetName(iResult)));
            }
        }
    }
    return(iResult);
...
static int32_t _VoipHeadsetCreateDecoder(VoipHeadsetRefT *pHeadset)
{
    ...
    if (iDecoderIndex < pHeadset->iDecoderPoolSize)
    {
        ...
        {
            NetPrintf(("voipheadsetps: added IN_VOICE port (%d) at index %d in pool of decoders\n", pDecoder->uDecoderPortId, iDecoderIndex));
        ...
        {
            NetPrintf(("voipheadsetps: failed to create a new decoder port)\n"));
            iRetCode = -1;  // signal failure
        }
        ...
        {
            NetPrintf(("voipheadsetps: failed to create pcm port for non-native codec\n"));
        }
    }
    else
    {
        NetPrintf(("voipheadsetps: IN_VOICE port creation ignored because pool of decoders is already full\n"));
...
static int32_t _VoipHeadsetReleaseDecoderOwnership(VoipHeadsetRefT *pHeadset, int32_t iDecoderIndex)
{
    ...
    if (pDecoder->bValid)
    {
        if (pDecoder->iOwnerIndex != VOIPHEADSET_INVALID_INDEX)
        {
            if (pDecoder->bOwnerRemote)
            {
                NetPrintfVerbose((pHeadset->iDebugLevel, 1, "voipheadsetps: remote user %d released ownership of decoder entry %d\n", pDecoder->iOwnerIndex, iDecoderIndex));
                pHeadset->aRemoteUsers[pDecoder->iOwnerIndex].iDecoderIndex = VOIPHEADSET_INVALID_INDEX;
            }
            else
            {
                NetPrintfVerbose((pHeadset->iDebugLevel, 1, "voipheadsetps: local user %d released ownership of decoder entry %d\n", pDecoder->iOwnerIndex, iDecoderIndex));
        ...
        {
            NetPrintf(("voipheadsetps: can't release ownership on a decoder entry with no owner (index = %d)\n", iDecoderIndex));
        }
    }
    else
    {
        NetPrintf(("voipheadsetps: can't release ownership on an invalid decoder entry (index = %d)\n", iDecoderIndex));
    }
...
static int32_t _VoipHeadsetTakeDecoderOwnership(VoipHeadsetRefT *pHeadset, int32_t iDecoderIndex, int32_t iUserIndex, uint8_t bRemote)
{
    ...
    if (pDecoder->bValid)
    {
        if (pDecoder->iOwnerIndex == VOIPHEADSET_INVALID_INDEX)
        {
            ...
            if (bRemote)
            {
                NetPrintfVerbose((pHeadset->iDebugLevel, 1, "voipheadsetps: remote user %d took ownership of decoder entry %d\n", iUserIndex, iDecoderIndex));
                pHeadset->aRemoteUsers[iUserIndex].iDecoderIndex = iDecoderIndex;
            }
            else
            {
                NetPrintfVerbose((pHeadset->iDebugLevel, 1, "voipheadsetps: local user %d took ownership of decoder entry %d\n", iUserIndex, iDecoderIndex));
        ...
        {
            NetPrintf(("voipheadsetps: can't take ownership on a decoder entry that already has an owner (index = %d)\n", iDecoderIndex));
        }
    }
    else
    {
        NetPrintf(("voipheadsetps: can't take ownership on an invalid decoder entry (index = %d)\n", iDecoderIndex));
    }
    ...
    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex++)
    {
        ...
        if (!VOIP_NullUser(&pLocalUser->user))
        {
            ...
            if ((iResult = _VoipHeadsetIsMicDetected(pLocalUser->uMicPortId, &bMicDetected)) == 0)
            {
                ...
                {
                    NetPrintf(("voipheadsetps: microphone %s for local user index %d\n", (bMicDetected ? "inserted" : "removed"), iLocalUserIndex));
...
/*!
    ...
    \Input uFlags       - VOIPNARRATE_FLAGS_*
...
static int32_t _VoipHeadsetNarrateCb(VoipNarrateRefT *pNarrateRef, int32_t iUserIndex, const int16_t *pSamples, int32_t iSize, uint32_t uFlags, void *pUserData)
{
    ...
    if (iSize > 0)
    {
        // write narrate output to local narrate port if tag as local
        // else write it into the same buffer as mic input
        if (bLocalNarrate)
        {
            iSize = _VoipHeadsetWriteToPort(pLocalUser->uLocalNarratePortId, (const uint8_t *)pSamples, iSize);
        }
        else
        {
    ...
    {
        // disable short audio discard during narration if transcription ref is available
       ...
       if (!bLocalNarrate)
        {
           pLocalUser->iOutboundNarrating = 1;
        }
    }
...
/*F*************************************************************************************************/
/*!
    \Function   _VoipHeadsetOnBlockListUpdate

    \Description
        Callback for when block list update happens
    ...
    \Input pNpServiceLabel  - more detail on the web service
    ...
    \Input uDataLen         - length of data with the event
    \Input pUserArg         - callback user data

    \Version 07/18/2020 (eesponda)
*/
/*************************************************************************************************F*/
#if !defined(DIRTYCODE_PS5) && !defined(DIRTYCODE_PS4_CROSSGEN)
static void _VoipHeadsetOnBlockListUpdate(int32_t iUserCtxId, int32_t iCallbackId, const char *pNpServiceName, SceNpServiceLabel pNpServiceLabel, const SceNpPeerAddressA *pTo, const SceNpPeerAddressA *pFrom, const SceNpWebApiPushEventDataType *pDataType, const char *pData, size_t uDataLen, void *pUserArg)
#else
static void _VoipHeadsetOnBlockListUpdate(int32_t iUserCtxId, int32_t iCallbackId, const char *pNpServiceName, SceNpServiceLabel pNpServiceLabel, const SceNpPeerAddressA *pTo, const SceNpPeerAddressA *pFrom, const SceNpWebApi2PushEventDataType *pDataType, const char *pData, size_t uDataLen, void *pUserArg)
#endif
{
    VoipHeadsetRefT *pHeadset = (VoipHeadsetRefT *)pUserArg;
    PS4LocalVoipUserT *pLocalUser = NULL;
    int32_t iLocalUserIndex;

    // find the correct user whom this is for
    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex += 1)
    {
        if (pHeadset->aLocalUsers[iLocalUserIndex].uSceAccountId == pTo->accountId)
        {
            pLocalUser = &pHeadset->aLocalUsers[iLocalUserIndex];
            break;
        }
    ...
    // if local user cannot be found then exit, the user is not participating in voip
    if (pLocalUser == NULL)
    {
        return;
    }

    // set the flag to update the block list
    pLocalUser->uNextBlockListCheck = NetTick();
    NetPrintfVerbose((pHeadset->iDebugLevel, 1, "voipheadsetps: received blocklist event for accountid=%llu userindex=%d\n", pTo->accountId, iLocalUserIndex));
}
...
static int32_t _VoipHeadsetDeleteLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, VoipUserT *pVoipUser)
{
    ...
    if (pVoipUser)
    {
        ...
        {
            NetPrintf(("voipheadsetps: deleting local talker %lld failed because it does not match user at index %d (%x)\n",
                pVoipUser->AccountInfo.iPersonaId, iLocalUserIndex, pLocalUser->user.AccountInfo.iPersonaId));
            return(-1);
        }
    }

    NetPrintf(("voipheadsetps: deleting local talker %lld at local user index %d\n", pLocalUser->user.AccountInfo.iPersonaId, iLocalUserIndex));
...
static int32_t _VoipHeadsetAddLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, VoipUserT *pVoipUser)
{
    ...
    {
        NetPrintf(("voipheadsetps: can't add NULL local voip user at local user index %d\n", iLocalUserIndex));
        return(-1);
    }
    NetPrintf(("voipheadsetps: adding voip local user %lld at local user index %d\n", pVoipUser->AccountInfo.iPersonaId, iLocalUserIndex));
    ...
    NetConnStatus('acct', iLocalUserIndex, &pLocalUser->uSceAccountId, sizeof(pLocalUser->uSceAccountId));
    ...
    {
        NetPrintf(("voipheadsetps: created IN_PCMAUDIO port %d for user index %d (user ID = %d/0x%08x)\n", pLocalUser->uPcmPortId, iLocalUserIndex, pLocalUser->sceUserId, pLocalUser->sceUserId));
    }
    else
    {
        NetPrintf(("voipheadsetps: sceVoiceCreatePort(IN_PCMAUDIO, %d) failed (err=%s)\n", iLocalUserIndex, DirtyErrGetName(iResult)));
    }
    ...
    // create both the speaker port and the local narrate port
    _VoipHeadsetSpkrPortCreate(iLocalUserIndex, &pLocalUser->uSpkrPortId);
    _VoipHeadsetPcmPortCreate(&pLocalUser->uLocalNarratePortId, 2 * 1024, 10);

    // connect the narrate port to speaker port
    if (_VoipHeadsetConnectPorts(pLocalUser->uLocalNarratePortId, pLocalUser->uSpkrPortId) != SCE_OK)
    {
        // clean up if we had a failure
        _VoipHeadsetDeleteLocalTalker(pHeadset, iLocalUserIndex, pVoipUser);
    }
...
static int32_t _VoipHeadsetDeactivateLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex)
{
    ...
    if (pLocalUser->bParticipating != FALSE)
    {
        ...
        // remove the listener
        DirtyWebApiRemovePushEventListener(pHeadset->pWebApi, pLocalUser->iPushEventRequestId, _VoipHeadsetOnBlockListUpdate);
        ...
        // disconnect the local narrate port with the speaker port
        if (_VoipHeadsetDisconnectPorts(pLocalUser->uSpkrPortId, pLocalUser->uLocalNarratePortId) < 0)
        {
            iRetCode = -1;  // signal failure
        }
        ...
        if (_VoipHeadsetPortDelete(pLocalUser->uLocalNarratePortId) < 0)
        {
            iRetCode = -3; // signal failure
        }

        #if DIRTYCODE_LOGGING
        if (iRetCode == 0)
        {
            NetPrintf(("voipheadsetps: local user %lld at local user index %d is no longer participating\n", pLocalUser->user.AccountInfo.iPersonaId, iLocalUserIndex));
        }
        #endif
    }
...
static int32_t _VoipHeadsetActivateLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, VoipUserT *pVoipUser)
{
    ...
    if (VOIP_NullUser(pVoipUser))
    {
        NetPrintf(("voipheadsetps: cannot promote a null user to a participating state (local user index %d)\n", iLocalUserIndex));
        return(iRetCode);
    }
    ...
    if (pLocalUser->bParticipating == FALSE)
    {
        ...
        pLocalUser->uNextBlockListCheck = NetTick();
        pLocalUser->iPushEventRequestId = DirtyWebApiAddPushEventListener(pHeadset->pWebApi, "np:service:blocklist", _VoipHeadsetOnBlockListUpdate, pHeadset);
        ...
        NetPrintf(("voipheadsetps: local user %lld at local user index %d is now participating\n", pLocalUser->user.AccountInfo.iPersonaId, iLocalUserIndex));
    }
    else
    {
        NetPrintf(("voipheadsetps: local user %lld at local user index %d is already participating\n", pLocalUser->user.AccountInfo.iPersonaId, iLocalUserIndex));
    }
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetEncodeRate

    \Description
        Rate-limit audio input to encoder to sample rate

    \Input *pLocalUser      - local user to encode voice for
    \Input iFrameSize       - size of data to encode

    \Output
        int32_t             - number of samples to encode

    \Notes
        At the start of rate limiting, we allow a small number of frames through
        unlimited to allow a bit of prebuffering to take place.

    \Version 01/08/2018 (jbrookes)
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetEncodeRate(PS4LocalVoipUserT *pLocalUser, int32_t iFrameSize)
{
    ...
    if ((pLocalUser->iEncAudioSamples > (4*VOIPHEADSET_FRAMESAMPLES)-1) && (iEncAudioRate > VOIPHEADSET_SAMPLERATE))
    {
        return(0);
    }
    // update sample tracker
    pLocalUser->iEncAudioSamples += iFrameSize/2;
    // return number of samples to encode
    return(iFrameSize/2);
}
...
static void _VoipHeadsetSendEncodedAudio(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, const uint8_t *pEncAudioData, int32_t iAudioSize, uint8_t bCameraMic)
{
    ...
    else
    {
        ...
        {
            NetPrintfVerbose((pHeadset->iDebugLevel, 1, "voipheadsetps: dropping chat data for local user index %d because no decoder available\n", iLocalUserIndex));
        }
    }
}
...
static int32_t _VoipHeadsetEncode(VoipHeadsetRefT *pHeadset, PS4LocalVoipUserT *pLocalUser, int32_t iLocalUserIndex, const uint8_t *pRawAudioBuffer, int32_t iFrameBytes, uint8_t bCameraMic)
{
    ...
    int32_t iFrameBytesEncoded=0, iFrameSamples, iEncAudioSize;
    uint8_t aEncAudioBuffer[VOIP_MAXMICRPKTSIZE]; //$$todo - this should be much smaller, but voipopus requires this size for now

    /* rate limit audio input; this is required for sources that can produce audio faster than our sample
       rate e.g. narration, but will not impact audio naturally produced at our sample rate (mic data) */
    if ((iFrameBytes < VOIPHEADSET_FRAMESIZE) || ((iFrameSamples = _VoipHeadsetEncodeRate(pLocalUser, VOIPHEADSET_FRAMESIZE)) <= 0))
    {
        return(0);
    }
    ...
    if (!pHeadset->bCrossPlay)
    {
        ...
        if (pTranscribeRef != NULL)
        {
            ...
            // handle native-path transcription (VoipCodecEncode performs transcription when cross-platform is enabled)
            if (bDoTranscription)
            {
    ...
    else if ((iEncAudioSize = VoipCodecEncode(aEncAudioBuffer, (const int16_t *)pRawAudioBuffer, iFrameSamples, pTranscribeRef)) > 0)
    {
...
static void _VoipHeadsetPollForVoiceData(VoipHeadsetRefT *pHeadset)
{
    ...
    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex += 1)
    {
        ...
        while ((iFrameBytes = iFrameBytesUsed = _VoipHeadsetReadAudioInput(pLocalUser, aRawAudioBuffer, VOIPHEADSET_FRAMESIZE)) > 0)
        {
            ...
            if ((pLocalUser->iOutboundNarrating == -1) && (pLocalUser->iAudioLen == 0))
            {
                pLocalUser->iAudioInp = pLocalUser->iAudioOut = 0;
                pLocalUser->iOutboundNarrating = 0;
            }
        ...
        if (!pHeadset->bCrossPlay)
        {
            ...
            while ((iFrameBytes = _VoipHeadsetIsLocalVoiceFrameReady(pLocalUser->uEncoderPortId)) > 0)
            {
                ...
                {
                    NetPrintf(("voipheadsetps: static buffer not big enough to read inbound voice frame on port %d for local user index %d\n",
...
static void _VoipHeadsetProcessVoiceData(VoipHeadsetRefT *pHeadset, VoipUserT *pPassInRemoteUser, VoipMicrInfoT *pMicrInfo, const uint8_t *pPacketData)
{
    ...
    for (uMicrPkt=0, pSubPacket=pPacketData; uMicrPkt < pMicrInfo->uNumSubPackets; uMicrPkt += 1, pSubPacket += uSubPacketSize)
    {
        ...
        NetPrintf(("voipheadsetps: submitting chat data for remote user %lld at index %d\n", pRemoteUsers[pMicrInfo->uUserIndex].iPersonaId, pMicrInfo->uUserIndex));
        #endif
        ...
        for (iRemoteUserSpaceIndex = 0; iRemoteUserSpaceIndex < pHeadset->iMaxRemoteUsers; iRemoteUserSpaceIndex++)
        {
            ...
            {
                NetPrintfVerbose((pHeadset->iDebugLevel, 0, "voipheadsetps: dropping chat data for remote user %lld at index %d because no decoder available\n",
            ...
            if (pDecoder->uConnectedSpkrPort == 0)
            {
                // it is normal not to have a connection if playback is disabled for every user
                if ((pRemoteUser->uEnablePlayback & (1 << pMicrInfo->uUserIndex)) != 0)
                {
                    NetPrintfVerbose((pHeadset->iDebugLevel, 0, "voipheadsetps: dropping chat data for remote user %lld at index %d because decoder has no connection\n",
                        pPassInRemoteUser->AccountInfo.iPersonaId, pMicrInfo->uUserIndex));
                }
                break;
            }
...
static void _VoipHeadsetUpdateChatPrivileges(VoipHeadsetRefT *pHeadset)
{
    ...
    {
        NetPrintf(("voipheadsetps: decoder-to-speaker connections update scheduled because of change in shared user's chat privilege (%s vs %s)\n",
...
static int32_t _VoipHeadsetChangeEncoderBps(VoipHeadsetRefT *pHeadset, int32_t iBitRate)
{
    ...
    {
        NetPrintf(("voipheadsetps: encoding configuration changed from %d bps to %d bps\n", pHeadset->iEncoderBps, iBitRate));
        pHeadset->iEncoderBps = iBitRate;
        ...
        for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS; iUserIndex++)
        {
            if (!VOIP_NullUser(&pHeadset->aLocalUsers[iUserIndex].user))
            {
                ...
                {
                    NetPrintf(("voipheadsetps: successfully changed bitrate for encoder port %d belonging to local user index %d\n",
                        pHeadset->aLocalUsers[iUserIndex].uEncoderPortId, iUserIndex));
                }
                else
                {
                    NetPrintf(("voipheadsetps: sceVoiceSetBitRate(%d) for local user index %d failed (err=%s)\n",
        ...
        for (iDecoderIndex = 0; iDecoderIndex < pHeadset->iDecoderPoolSize; iDecoderIndex++)
        {
            if (pHeadset->pDecoders[iDecoderIndex].bValid)
            {
                ...
                {
                    NetPrintf(("voipheadsetps: successfully changed bitrate for decoder port %d\n",
                        pHeadset->pDecoders[iDecoderIndex].uDecoderPortId));
                }
                else
                {
                    NetPrintf(("voipheadsetps: sceVoiceSetBitRate(%d) failed (err=%s)\n",
    ...
    {
        NetPrintf(("voipheadsetps: %d bps is not a supported encoding configuration\n", iBitRate));
        iRetCode = -1;
    }
...
static void _VoipHeadsetBlockListCallback(UserListApiRefT *pRef, UserListApiReturnTypeE eResponseType, UserListApiEventDataT *pEventData, void *pUserData)
{
    ...
    else if (eResponseType == TYPE_LIST_END)
    {
        ...
        // if we didn't get an error, commit and apply the changes
        if (pEventData->ListEndData.Error == 0)
        {
...
static void _VoipHeadsetProcessBlockList(VoipHeadsetRefT *pHeadset)
{
    ...
    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex += 1)
    {
        ...
        /* if the update was triggered from the webapi callback and we don't have a current request pending or have a pending mute list update to apply, kick off a new request.
           in the case we have a mute list to apply we need to make sure that it gets processed otherwise we run the risk of missing updates */
        if (!pLocalUser->bPendingBlockList && !pLocalUser->bMuteListUpdated && (pLocalUser->uNextBlockListCheck != 0) && (NetTickDiff(uNetTick, pLocalUser->uNextBlockListCheck) > 0))
        {
            if (UserListApiGetListAsync(pHeadset->pUserListApi, iLocalUserIndex, USERLISTAPI_TYPE_BLOCKED, VOIPHEADSET_BLOCKLIST_MAX, 0, NULL, _VoipHeadsetBlockListCallback, 0, pLocalUser) >= 0)
            {
                ...
                pLocalUser->uNextBlockListCheck = 0;
            }
            else
            {
    ...
    // update the webapi to fire any callbacks for the events we are listening for
    DirtyWebApiUpdate(pHeadset->pWebApi);
...
static void _VoipHeadsetProcessTranscription(VoipHeadsetRefT* pHeadset)
{
    ...
    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex += 1)
    {
        ...
        // skip if not enabled or user has no transcription ref
        if (!pHeadset->bVoiceTranscriptionEnabled || (pLocalUser->pTranscribeRef == NULL))
        {
            continue;
        }
        ...
        if (VoipTranscribeGet(pLocalUser->pTranscribeRef, strTranscribeBuf + 5, sizeof(strTranscribeBuf) - 5) > 0)
        {
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetNarrateInput
    ...
    \Input pHeadset     - headset ref
    \Input iUserIndex   - user index
    \Input uFlags       - narrate flags
    \Input *pText       - text to narrate

    \Output
        int32_t         - 0 for success; negative for failure

    \Version 01/18/2021 (tcho)
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetNarrateInput(VoipHeadsetRefT* pHeadset, int32_t iUserIndex, uint32_t uFlags, const char* pText)
{
    PS4LocalVoipUserT* pLocalUser = &pHeadset->aLocalUsers[iUserIndex];
    ...
    if (pLocalUser->pNarrateRef != NULL)
    {
        return(VoipNarrateInput(pLocalUser->pNarrateRef, iUserIndex, pLocalUser->uGender, uFlags, pText));
    }
    else
    {
        NetPrintf(("voipheadsetps: failed to start narration module\n"));
        return(-1);
    }
}
...
VoipHeadsetRefT *VoipHeadsetCreate(int32_t iMaxChannels, VoipHeadsetMicDataCbT *pMicDataCb, VoipHeadsetTextDataCbT *pTextDataCb, VoipHeadsetOpaqueDataCbT *pOpaqueDataCb, VoipHeadsetStatusCbT *pStatusCb, void *pCbUserData, int32_t iAttr)
{
    ...
    #else
    DirtyWebApi2CreateParamsT CreateParams;
    #endif
    ...
    {
        NetPrintf(("voipheadsetps: request for %d channels exceeds max\n", iMaxChannels));
        return(NULL);
    }
    ...
    {
        NetPrintf(("voipheadsetps: not enough memory to allocate module state\n"));
        return(NULL);
    }
    ...
    {
        NetPrintf(("voipheadsetps: unable to create the userlist module required for blocked user lookups\n"));
        VoipHeadsetDestroy(pHeadset);
        return(NULL);
    }

    // create the webapi module
    ds_memclr(&CreateParams, sizeof(CreateParams));
    if ((pHeadset->pWebApi = DirtyWebApiCreate(&CreateParams)) == NULL)
    {
        NetPrintf(("voipheadsetps: unable to create the webapi module required for blocked user functionality\n"));
        VoipHeadsetDestroy(pHeadset);
        return(NULL);
    }
    DirtyWebApiControl(pHeadset->pWebApi, 'qcal', TRUE, 0, NULL);
    DirtyWebApiControl(pHeadset->pWebApi, 'auto', FALSE, 0, NULL);

    // init sony voice lib (audio I/O systems)
    if (_VoipHeadsetVoiceLibInit(pHeadset) < 0)
    {
        NetPrintf(("voipheadsetps: failed to initialize voice lib\n"));
        VoipHeadsetDestroy(pHeadset);
        return(NULL);
    }
    ...
    {
        NetPrintf(("voipheadsetps: not enough memory to allocate pool of decoders\n"));
        VoipHeadsetDestroy(pHeadset);
        return(NULL);
    }
    ...
    {
        NetPrintf(("voipheadsetps: failed to modify attributes of internal voice trheads\n"));
        VoipHeadsetDestroy(pHeadset);
        return(NULL);
    }
    ...
    {
        NetPrintf(("voipheadsetps: failed to start voice lib\n"));
        VoipHeadsetDestroy(pHeadset);
        return(NULL);
    }
    ...
    #if defined(DIRTYCODE_PS5)
    // ensure that voice chat is disabled until we enter participating state
    sceVoiceDisableChat();
    #endif
...
int32_t VoipHeadsetDestroy(VoipHeadsetRefT *pHeadset)
{
    ...
    if (pHeadset->pWebApi != NULL)
    {
        DirtyWebApiDestroy(pHeadset->pWebApi);
        pHeadset->pWebApi = NULL;
    }
...
void VoipHeadsetReceiveVoiceDataCb(VoipUserT *pRemoteUsers, int32_t iRemoteUserSize, int32_t iConsoleId, VoipMicrInfoT *pMicrInfo, uint8_t *pPacketData, void *pUserData)
{
    ...
    if (pMicrInfo->uUserIndex == VOIP_SHARED_REMOTE_INDEX)
    {
        ...
        if (iIndex == iRemoteUserSize)
        {
            ...
            NetPrintf(("voipheadsetps: discarding voice packet from shared user because we cannot find a remote user to play it back as!\n"));
            return;
        }
    }
    else
...
void VoipHeadsetRegisterUserCb(VoipUserT *pRemoteUser, int32_t iConsoleId, uint32_t bRegister, void *pUserData)
{
    ...
    {
        NetPrintf(("voipheadsetps: can't %s NULL remote talker\n", (bRegister?"register":"unregister")));
        return;
    }
    ...
    if (bRegister)
    {
        ...
        for (iRemoteUserSpaceIndex = 0; iRemoteUserSpaceIndex < pHeadset->iMaxRemoteUsers; iRemoteUserSpaceIndex++)
        {
            if (VOIP_NullUser(&pHeadset->aRemoteUsers[iRemoteUserSpaceIndex].user))
            {
                ...
                {
                    NetPrintf(("voipheadsetps: cannot add remote talker if the first party id callback is not registered\n"));
                    return;
                }
                ...
                NetPrintf(("voipheadsetps: registered remote talker %lld at remote user index %d\n", pRemoteUser->AccountInfo.iPersonaId, iRemoteUserSpaceIndex));
                break;
            }
        }
        ...
        {
            NetPrintf(("voipheadsetps: failed to register remote talker %lld, max number of channels reached\n", pRemoteUser->AccountInfo.iPersonaId));
        }
    }
    else // unregister
    {
        ...
        for (iRemoteUserSpaceIndex = 0; iRemoteUserSpaceIndex < pHeadset->iMaxRemoteUsers; iRemoteUserSpaceIndex++)
        {
            if (VOIP_SameUser(&pHeadset->aRemoteUsers[iRemoteUserSpaceIndex].user, pRemoteUser))
            {
                ...
                NetPrintf(("voipheadsetps: unregistered remote talker %lld at remote user index %d\n", pRemoteUser->AccountInfo.iPersonaId, iRemoteUserSpaceIndex));
...
void VoipHeadsetProcess(VoipHeadsetRefT *pHeadset, uint32_t uFrameCount)
{
    ...
    int32_t iLocalUserIndex;
    uint8_t bParticipating;
    for (iLocalUserIndex = 0, bParticipating = FALSE; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex += 1)
    {
        PS4LocalVoipUserT *pLocalUser = &pHeadset->aLocalUsers[iLocalUserIndex];

        // update narrate for each user
        if (!VOIP_NullUser(&pLocalUser->user) && (pLocalUser->pNarrateRef != NULL))
        {
            VoipNarrateUpdate(pLocalUser->pNarrateRef);
        }

        #if defined(DIRTYCODE_PS5)
        // check the status of our participating users by looping and stopping on the user that is participating (as long as one user is participating chat should be enabled)
    ...
    #if defined(DIRTYCODE_PS5)
    // if any users are participating then enable voice chat
    if (bParticipating && !pHeadset->bChatEnabled && (sceVoiceEnableChat() == SCE_OK))
    {
        pHeadset->bChatEnabled = TRUE;
    }
    // else if no users left participating then disable voice chat
    else if (!bParticipating && pHeadset->bChatEnabled && (sceVoiceDisableChat() == SCE_OK))
    {
        pHeadset->bChatEnabled = FALSE;
    }
    #endif
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'txtn' - request local voice narration; iValue=local user index, pValue=utf8-encoded text
...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    {
        ds_strnzcpy(pHeadset->strLang, pValue, sizeof(pHeadset->strLang));
    ...
    if ((iControl == 'mute') && (iValue < VOIP_MAXLOCALUSERS))
    {
        ...
        {
            NetPrintfVerbose((pHeadset->iDebugLevel, 1, "voipheadsetps: mute %s\n", (pLocalUser->bMuted ? "enabled" : "disabled")));
    ...
    if ((iControl == '-pbk') || (iControl == '+pbk' ))
    {
        ...
        if ((pValue != NULL) && (iValue < VOIP_MAXLOCALUSERS_EXTENDED))
        {
            ...
            for (iIndex = 0; iIndex < pHeadset->iMaxRemoteUsers; ++iIndex)
            {
                ...
                {
                    uOldPlaybackFlags = pHeadset->aRemoteUsers[iIndex].uEnablePlayback;
                    ...
                    // note: on sony platforms the playback bitfield is store per remote player.The bit field represents which local player should playback the remote voice (each bit is a local player)
                    NetPrintf(("voipheadsetps: %s applied for ruser(%d):%llu & luser(%d):%llu | 0x%08x -> 0x%08x\n",
                        bVoiceEnable ? "+pbk" : "-pbk", iIndex, pHeadset->aRemoteUsers[iIndex].user.AccountInfo.iPersonaId, iValue, pHeadset->aLocalUsers[iValue].user.AccountInfo.iPersonaId,
                        uOldPlaybackFlags, pHeadset->aRemoteUsers[iIndex].uEnablePlayback));
            ...
            NetPrintf(("voipheadsetps: VoipHeadsetControl('%s') no remote user was found. User: %lld\n", bVoiceEnable? "+pbk":"-pbk", pRemoteUser->AccountInfo.iPersonaId));
            return(-2);
        }
        else
        {
            NetPrintf(("voipheadsetps: VoipHeadsetControl('%s') invalid arguments\n", bVoiceEnable? "+pbk":"-pbk"));
            return(-1);
        }
    }
    if (iControl == 'rloc')
    ...
    if (iControl == 'spam')
    {
        ...
        NetPrintf(("voipheadsetps: setting debuglevel=%d\n", iValue));
    ...
    if ((iControl == 'ttos') && (iValue < VOIP_MAXLOCALUSERS))
    {
        ...
        {
            return(_VoipHeadsetNarrateInput(pHeadset, iValue, VOIPNARRATE_CBFLAG_NONE, (const char*)pValue));
        }

        //user not participating
        return(-1);
    }
    if ((iControl == 'txtn') && (iValue < VOIP_MAXLOCALUSERS))
    {
        return(_VoipHeadsetNarrateInput(pHeadset, iValue, VOIPNARRATE_CBFLAG_LOCAL, (const char*)pValue));
    }
    if (iControl == 'voic')
    {
        if ((iValue < VOIP_MAXLOCALUSERS) && (pValue != NULL))
        {
            ...
            NetPrintf(("voipheadsetps: setting tts voice to %s\n", (pLocalUser->uGender == VOIPNARRATE_GENDER_MALE) ? "male" : "female"));
            return(0);
        }
        else
        {
            NetPrintf(("voipheadsetps: VoipHeadsetControl('voic') invalid parameters\n"));
            return(-1);
        }
    }
    if (iControl == 'vpri')
    ...
    if (iControl == 'xply')
    {
        ...
        if (pHeadset->bCrossPlay != bCrossPlay)
        {
            ...
            NetPrintf(("voipheadsetps: cross-play %s\n", pHeadset->bCrossPlay ? "enabled" : "disabled"));
...
int32_t VoipHeadsetStatus(VoipHeadsetRefT *pHeadset, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    ...
    {
        return(pHeadset->aLocalUsers[iValue].iOutboundNarrating != 0);
    }
    if (iSelect == 'ttsm')
    {
        VoipNarrateRefT *pNarrateRef = pHeadset->aLocalUsers[iValue].pNarrateRef;
...
/*!
    ...
    \Input *pHeadset    - headset module state
    \Input eProvider    - narrate provider
...
*/
/********************************************************************************F*/
void VoipHeadsetConfigNarration(VoipHeadsetRefT *pHeadset, VoipSpeechProviderE eProvider, const char *pUrl, const char *pKey)
{
    VoipNarrateConfig(eProvider, pUrl, pKey);
}
...
void VoipHeadsetTranscribedTextPermissionCheck(VoipHeadsetRefT *pHeadset, VoipUserT *pOriginator, const char *pStrUtf8)
{
    ...
    for (iRemoteUserSpaceIndex = 0; iRemoteUserSpaceIndex < pHeadset->iMaxRemoteUsers; iRemoteUserSpaceIndex++)
    {
        if (VOIP_SameUser(&pHeadset->aRemoteUsers[iRemoteUserSpaceIndex].user, pOriginator))
        {
            ...
            {
                NetPrintf(("voipheadsetps: transcribed text from %lld dropped because chat is restricted -- text was: %s\n",
