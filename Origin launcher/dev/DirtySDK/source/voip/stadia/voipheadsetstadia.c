...
#define VOIPHEADSET_BLOCKLIST_INTERVAL (60*1000)                                                //!< STR 137.2 states we need to react to blocking within 30s, use a safe value
...
typedef struct VoipHeadsetRingBufferT
{
    int16_t *pBuf;    //!< allocated narrate buffer based on the sample rate; may differ from system's sample rate
    int32_t iMax;     //!< maximum size (in number of samples) of the narrate buffer
    int32_t iLen;     //!< current length (in number of samples) of the narrate buffer
    int32_t iInp;     //!< write pointer; head of circular buffer
    int32_t iOut;     //!< read pointer; tail of circular buffer
} VoipHeadsetRingBufferT;
...
struct VoipHeadsetRefT
{
    ...
    VoipHeadsetRingBufferT OutboundNarrateBuffer;   //!< outbound narrate audio buffer
    VoipHeadsetRingBufferT LocalNarrateBuffer;      //!< local narrate audio buffer (used for local narrate)
    uint32_t uLastLocalNarrateMixTick;              //!< when was the last time we mixed a incoming voip packet with local narrate frame
    int32_t iNarrateFrameMixed;                     //!< number of local narrate frame mixed (reset per process call)
    int8_t iOutboundNarrating;                      //!< outbound narration status; zero=inactive, one=narrating, negative=done
    ...
    uint8_t bChatEndpointEnabled;           //!< is chat endpoint enabled
    ...
    GgpEventHandle uSocialGraphChanged;     //!< handle to the social list changed event (used for block lists)
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetWriteNarrateInput(VoipHeadsetRingBufferT* pRingBuffer, const int16_t* pSamples, int32_t iNumSamples)
{
    int32_t iWriteSize, iWriteSize2;

    // see how much room we have to write
    if ((iWriteSize = pRingBuffer->iMax - pRingBuffer->iLen) < iNumSamples)
    {
        iNumSamples = iWriteSize;
    }
    // see if we need to wrap the copy
    if ((pRingBuffer->iInp + iNumSamples) > pRingBuffer->iMax)
    {
        // write to end of buffer
        iWriteSize = pRingBuffer->iMax - pRingBuffer->iInp;
        ds_memcpy(pRingBuffer->pBuf + pRingBuffer->iInp, pSamples, sizeof(*pRingBuffer->pBuf) * iWriteSize);
        // wrap and write the rest
        iWriteSize2 = iNumSamples - iWriteSize;
        ds_memcpy(pRingBuffer->pBuf, pSamples + iWriteSize, sizeof(*pRingBuffer->pBuf) * iWriteSize2);
        pRingBuffer->iInp = iWriteSize2;
    }
    else
    {
        ds_memcpy(pRingBuffer->pBuf + pRingBuffer->iInp, pSamples, sizeof(*pRingBuffer->pBuf) * iNumSamples);
        pRingBuffer->iInp += iNumSamples;
    }
    // add to size
    pRingBuffer->iLen += iNumSamples;
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetReadNarrateInput(VoipHeadsetRingBufferT* pRingBuffer, int16_t* pSamples, int32_t iNumSamples)
{
    int32_t iReadSize;
    ...
    if ((iReadSize = pRingBuffer->iLen) == 0)
    {
    ...
    if ((pRingBuffer->iOut + iNumSamples) > pRingBuffer->iMax)
    {
        // get size to end of buffer
        int32_t iReadSize2 = pRingBuffer->iMax - pRingBuffer->iOut;
        // get size of the rest of the copy after wrapping
        iReadSize = iNumSamples - iReadSize2;
        // do the copy
        ds_memcpy(pSamples, pRingBuffer->pBuf + pRingBuffer->iOut, sizeof(*pSamples) * iReadSize2);
        ds_memcpy(pSamples + iReadSize2, pRingBuffer->pBuf, sizeof(*pSamples) * iReadSize);
        pRingBuffer->iOut = iReadSize;
        pRingBuffer->iLen -= iNumSamples;
    }
    // regular non-wrapping copy
    else
    {
        ds_memcpy(pSamples, pRingBuffer->pBuf + pRingBuffer->iOut, sizeof(*pSamples) * iNumSamples);
        pRingBuffer->iOut += iNumSamples;
        pRingBuffer->iLen -= iNumSamples;
    }
    return(iNumSamples);
}
...
static void _VoipHeadsetMicrophoneHotplug(const GgpMicrophoneHotplugEvent *pEvent, void *pUserData)
{
    ...
    pHeadset->MicrInfo.Device = pEvent->connected ? pEvent->microphone : kGgpInvalidReference;

    // update the microphone status for all registered users
    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex += 1)
    {
        ...
        {
            pHeadset->pStatusCb(iLocalUserIndex, pEvent->connected, VOIP_HEADSET_STATUS_INPUT, pHeadset->pCbUserData);
        }
    }
}
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetProcessLocalNarrate

    \Description
        Process local narrate audio

    \Input *pHeadset    - headset state

    \Version 02/16/2021 (tcho)
*/
/********************************************************************************F*/
static void _VoipHeadsetProcessLocalNarrate(VoipHeadsetRefT *pHeadset)
{
    int32_t iNumSamples, iNumResample;
    int16_t aFrameData[VOIPHEADSET_FRAMESAMPLES];
    int16_t aResampledData[VOIPHEADSET_FRAMESIZE];

    // reading samples from local narrate buffer
    if ((iNumSamples = _VoipHeadsetReadNarrateInput(&pHeadset->LocalNarrateBuffer, aFrameData, VOIPHEADSET_GetFrameSamples(VOIPNARRATE_SAMPLERATE))) == 0)
    {
        return;
    }

    // encoding the samples in order to resample it 
    if ((iNumResample = VoipCodecResample((uint8_t *)aResampledData, sizeof(aResampledData) * sizeof(aResampledData[0]), aFrameData, iNumSamples)) > 0)
    {
        VoipConduitReceiveVoiceData(pHeadset->pConduitRef, &pHeadset->aLocalUsers[pHeadset->iParticipatingUserIndex].User,
            (iNumResample == iNumSamples) ? (const uint8_t *)aFrameData : (const uint8_t *)aResampledData,
            (iNumResample == iNumSamples) ? sizeof(aFrameData[0])*iNumSamples : sizeof(aResampledData[0])*iNumResample, FALSE);
    }
}
...
static void _VoipHeadsetSendEncodedAudio(VoipHeadsetRefT *pHeadset, const uint8_t *pPacket, int32_t iCompBytes)
{
    ...
    else
    {
        ...
        VoipConduitReceiveVoiceData(pHeadset->pConduitRef, &NullUser, pPacket, iCompBytes, TRUE);
        _VoipHeadsetProcessLocalNarrate(pHeadset);
        pHeadset->uLastLocalNarrateMixTick = NetTick();
    }
}
...
static VoipTranscribeRefT *_VoipHeadsetTranscribeCreate(VoipHeadsetRefT *pHeadset)
{
    ...
    if ((pHeadset->pTranscribe == NULL) && ((pHeadset->pTranscribe = VoipTranscribeCreate(16 * 1024)) != NULL))
    {
        ...
        VoipTranscribeControl(pHeadset->pTranscribe, 'lang', 0, 0, pHeadset->strLang);
    }
...
static void _VoipHeadsetProcessTranscription(VoipHeadsetRefT *pHeadset)
{
    ...
    if (!pHeadset->bVoiceTranscriptionEnabled || (pHeadset->pTranscribe == NULL))
    {
        return;
    }
    ...
    // get transcription (leave room for language code)
    if (VoipTranscribeGet(pHeadset->pTranscribe, strTranscribeBuf+5, sizeof(strTranscribeBuf)-5) <= 0)
    {
        return;
    }
    ...
    if (pHeadset->iOutboundNarrating == 0)
    {
...
static void _VoipHeadsetProcessRecord(VoipHeadsetRefT *pHeadset)
{
    ...
    const uint8_t bVoiceTranscriptionEnabled = (pHeadset->iOutboundNarrating != 0) ? (pHeadset->bVoiceTranscriptionEnabled) : pHeadset->bVoiceTranscriptionEnabled;

    // nothing to do if we don't have a valid microphone
    if (pHeadset->MicrInfo.Device == kGgpInvalidReference)
    {
        return;
    }

    // if we have a valid user index and their mic is not yet listening, update the state to listening
    if ((pHeadset->iParticipatingUserIndex != VOIP_INVALID_LOCAL_USER_INDEX) && !GgpIsMicAudioListening(pHeadset->aLocalUsers[pHeadset->iParticipatingUserIndex].uPlayerId))
    {
        if (!GgpSetMicAudioListening(pHeadset->aLocalUsers[pHeadset->iParticipatingUserIndex].uPlayerId, TRUE, &Status))
        {
    ...
    // nothing to do if the microphone is not live
    if (!GgpMicrophoneIsLive(pHeadset->MicrInfo.Device))
    {
        return;
    }
    ...
    if ((pHeadset->iOutboundNarrating != 0) && (iNumSamples = _VoipHeadsetReadNarrateInput(&pHeadset->OutboundNarrateBuffer, aFrameData, VOIPHEADSET_GetFrameSamples(VOIPNARRATE_SAMPLERATE))) == 0)
    {
        return;
    }
    ...
    if ((pHeadset->iOutboundNarrating < 0) && (pHeadset->OutboundNarrateBuffer.iLen == 0))
    {
        pHeadset->iOutboundNarrating = 0;
    }
}
...
static int32_t _VoipHeadsetAddLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, const VoipUserT *pLocalUser)
{
    ...
    VoipConduitRegisterUser(pHeadset->pConduitRef, &pHeadset->aLocalUsers[iLocalUserIndex].User, TRUE);

    if (!pHeadset->bChatEndpointEnabled)
    {
        pHeadset->bChatEndpointEnabled = TRUE;
        GgpEnableVoiceChatEndpoint(TRUE);
    }
...
{
    uint8_t bDisableChatEndpoint = TRUE;
    ...
    VoipConduitRegisterUser(pHeadset->pConduitRef, &pHeadset->aLocalUsers[iLocalUserIndex].User, FALSE);

    VOIP_ClearUser(&pLocalUser->User);
    pLocalUser->uPlayerId = 0;
    pLocalUser->uPlaybackFlags = 0xffffffff;

    // check to see if we still have a registered local user
    // if not disable the chat endpoint
    for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS; iUserIndex += 1)
    {
        if (!VOIP_NullUser(&pHeadset->aLocalUsers[iLocalUserIndex].User))
        {
            bDisableChatEndpoint = FALSE;
            break;
        }
    }

    if (bDisableChatEndpoint)
    {
        GgpEnableVoiceChatEndpoint(FALSE);
    }
...
static uint8_t _VoipHeadsetConduitPlaybackCb(VoipMixerRefT *pMixer, VoipUserT *pRemoteUser, void *pUserData)
{
    ...
    {
        // the user can also be local for local narration
        for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex += 1)
        {
            if (VOIP_SameUser(&pHeadset->aLocalUsers[iLocalUserIndex].User, pRemoteUser))
            {
                return(TRUE);
            }
        }

        NetPrintf(("voipheadsetstadia : cannot find remote user %d\n", &pHeadset->aLocalUsers[iRemoteUserIndex].User.AccountInfo.iAccountId));
    }
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetNarrateCb(VoipNarrateRefT *pNarrate, int32_t iUserIndex, const int16_t *pSamples, int32_t iSize, uint32_t uFlags, void *pUserData)
{
    VoipHeadsetRefT *pHeadset = (VoipHeadsetRefT *)pUserData;
    int32_t iNumSamples;
    uint8_t bLocalNarration = (uFlags & VOIPNARRATE_CBFLAG_LOCAL) ? TRUE : FALSE;
    ...
    VoipHeadsetRingBufferT *pRingBuffer = (uFlags & VOIPNARRATE_CBFLAG_LOCAL) ? &pHeadset->LocalNarrateBuffer : &pHeadset->OutboundNarrateBuffer;
    return(_VoipHeadsetWriteNarrateInput(pRingBuffer, pSamples, iNumSamples) * sizeof(*pSamples));
}
...
static int32_t _VoipHeadsetCreateNarrate(VoipHeadsetRefT *pHeadset)
{
    ...
    // allocate the outbound narration buffer
    if ((pHeadset->OutboundNarrateBuffer.pBuf = (int16_t *)DirtyMemAlloc(sizeof(*pHeadset->OutboundNarrateBuffer.pBuf) * iNarrateMax, VOIP_MEMID, pHeadset->iMemGroup, pHeadset->pMemGroupUserData)) == NULL)
    {
        NetPrintf(("voipheadsetstadia: failed to allocate memory for the outbound narration buffer\n"));
        return(-1);
    }
    pHeadset->OutboundNarrateBuffer.iMax = iNarrateMax;

    // allocate the local narration buffer
    if ((pHeadset->LocalNarrateBuffer.pBuf = (int16_t *)DirtyMemAlloc(sizeof(*pHeadset->LocalNarrateBuffer.pBuf) * iNarrateMax, VOIP_MEMID, pHeadset->iMemGroup, pHeadset->pMemGroupUserData)) == NULL)
    {
        NetPrintf(("voipheadsetstadia: failed to allocate memory for the local narration buffer\n"));
        DirtyMemFree(pHeadset->OutboundNarrateBuffer.pBuf, VOIP_MEMID, pHeadset->iMemGroup, pHeadset->pMemGroupUserData);
        return(-1);
    }
    pHeadset->LocalNarrateBuffer.iMax = iNarrateMax;

    // create the narration module
    if ((pHeadset->pNarrate = VoipNarrateCreate(_VoipHeadsetNarrateCb, pHeadset)) == NULL)
    {
        DirtyMemFree(pHeadset->OutboundNarrateBuffer.pBuf, VOIP_MEMID, pHeadset->iMemGroup, pHeadset->pMemGroupUserData);
        DirtyMemFree(pHeadset->LocalNarrateBuffer.pBuf, VOIP_MEMID, pHeadset->iMemGroup, pHeadset->pMemGroupUserData);
        return(-1);
    }
    VoipNarrateControl(pHeadset->pNarrate, 'spam', pHeadset->iVerbose, 0, NULL);
    VoipNarrateControl(pHeadset->pNarrate, 'lang', 0, 0, pHeadset->strLang);
    return(0);
}
...
static void _VoipHeadsetProcessBlockList(VoipHeadsetRefT *pHeadset)
{
    ...
    {
        if ((pHeadset->iParticipatingUserIndex != VOIP_INVALID_LOCAL_USER_INDEX) && (pHeadset->uNextBlockListCheck != 0) && (NetTickDiff(uNetTick, pHeadset->uNextBlockListCheck) > 0))
        {
    ...
    pHeadset->uNextBlockListCheck = 0;
}

/*F********************************************************************************/
/*!
    \Function _VoipHeadsetSocialGraphChanged

    \Description
        Callback fired when the social graph changes

    \Input *pEvent      - the event data
    \Input *pUserData   - callback user data

    \Version 08/25/2020 (eesponda)
*/
/********************************************************************************F*/
static void _VoipHeadsetSocialGraphChanged(const GgpSocialGraphChangedEvent *pEvent, void *pUserData)
{
    #if DIRTYCODE_LOGGING
    static const char *_strSocialChangeTypes[] =
    {
        "kGgpSocialGraphChangeType_Invalid", "kGgpSocialGraphChangeType_Friends", "kGgpSocialGraphChangeType_Blocks"
    };
    #endif
    VoipHeadsetRefT *pHeadset = (VoipHeadsetRefT *)pUserData;
    int32_t iLocalUserIndex;

    NetPrintfVerbose((pHeadset->iVerbose, 1, "voipheadsetstadia: _VoipHeadsetSocialGraphChanged (player_id=%llu, type=%s\n",
        pEvent->player_id, _strSocialChangeTypes[pEvent->type]));

    // early return if the event type is not related to the block list
    if (pEvent->type != kGgpSocialGraphChangeType_Blocks)
    {
        return;
    }

    // check to see if the participating user index has its block list updated
    for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex += 1)
    {
        if ((pHeadset->aLocalUsers[iLocalUserIndex].uPlayerId != pEvent->player_id) || (pHeadset->iParticipatingUserIndex != iLocalUserIndex))
        {
            continue;
        }

        pHeadset->bPendingBlockList = TRUE;
        pHeadset->BlockListAsyncOp = GgpListBlockedPlayers(pEvent->player_id);
        break;
    }
}
...
VoipHeadsetRefT *VoipHeadsetCreate(int32_t iMaxConduits, VoipHeadsetMicDataCbT *pMicDataCb, VoipHeadsetTextDataCbT *pTextDataCb, VoipHeadsetOpaqueDataCbT *pOpaqueDataCb, VoipHeadsetStatusCbT *pStatusCb, void *pCbUserData, int32_t iData)
{
    ...
    // add the block list changed handler
    if ((pHeadset->uSocialGraphChanged = GgpAddSocialGraphChangedHandler(pHeadset->EventQueue, _VoipHeadsetSocialGraphChanged, pHeadset, NULL, NULL)) == 0)
    {
        NetPrintf(("voipheadsetstadia: failed to add the social graph changed handler\n"));
        VoipHeadsetDestroy(pHeadset);
        return(NULL);
    }
...
int32_t VoipHeadsetDestroy(VoipHeadsetRefT *pHeadset)
{
    ...
    if (pHeadset->uSocialGraphChanged != 0)
    {
        GgpRemoveSocialGraphChangedHandler(pHeadset->uSocialGraphChanged, NULL);
    }
...
{
    int32_t iNumFrameMixed = 0;
    int32_t iNumNarrateFrame = 0;
    uint32_t uMicrPkt;
    ...
    for (uMicrPkt = 0; uMicrPkt < pMicrInfo->uNumSubPackets; uMicrPkt += 1)
    {
        ...
        iNumFrameMixed++;
        VoipConduitReceiveVoiceData(pHeadset->pConduitRef, &pRemoteUsers[iRemoteUserIndex], pPacketData, uSubPacketSize, TRUE);

        // move to the next packet
        pPacketData += uSubPacketSize;
    }

    // mixed in the local narrate
    // assuming we only audio data once from a remote player per process
    if ((iNumNarrateFrame = iNumFrameMixed - pHeadset->iNarrateFrameMixed) > 0)
    {
        pHeadset->iNarrateFrameMixed += iNumNarrateFrame;
        while (iNumNarrateFrame > 0)
        {
            _VoipHeadsetProcessLocalNarrate(pHeadset);
            iNumNarrateFrame--;
        }
...
void VoipHeadsetProcess(VoipHeadsetRefT *pHeadset, uint32_t uFrameCount)
{
    ...
    // process local narrate
    pHeadset->iNarrateFrameMixed = 0;
    if ((uint32_t)NetTickDiff(NetTick(), pHeadset->uLastLocalNarrateMixTick) > 150)
    {
        _VoipHeadsetProcessLocalNarrate(pHeadset);
    }
...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    if ((iControl == '+pbk') || (iControl == '-pbk'))
    {
        ...
        uOldPlaybackFlag = pHeadset->aLocalUsers[iValue].uPlaybackFlags;
        ...
        NetPrintf(("voipheadsetstadia: %s applied for luser(%d):%llu & ruser(%d):%llu | 0x%08x -> 0x%08x\n",
            bPlaybackEnabled ? "+pbk" : "-pbk", iValue, pHeadset->aLocalUsers[iValue].User.AccountInfo.iPersonaId, iRemoteUserIndex, pRemoteUser->AccountInfo.iPersonaId,
            uOldPlaybackFlag, pHeadset->aLocalUsers[iValue].uPlaybackFlags));
    ...
    if (iControl == 'aloc')
    {
        ...
        /* we need to ensure that only one participating user index is set. stadia voip currently only works in an SLU
           context, if another user is activated it would break how the microphone listening state is handled. we want
           to ensure the same user that we set listening to TRUE gets set to FALSE. */
        if ((iValue2 != 0) && (pHeadset->iParticipatingUserIndex == VOIP_INVALID_LOCAL_USER_INDEX))
        {
            pHeadset->iParticipatingUserIndex = iValue;
            NetPrintf(("voipheadsetstadia: user %d, entering the participating state\n", iValue));
        }
        else if ((iValue == 0) && (pHeadset->iParticipatingUserIndex == iValue))
        {
            if (!GgpSetMicAudioListening(pHeadset->aLocalUsers[pHeadset->iParticipatingUserIndex].uPlayerId, FALSE, &Status))
            {
                GgpGetStatusMessage(Status.message_token, strError, sizeof(strError));
                NetPrintf(("voipheadsetstadia: failed to set the mic listening state (code=%d, msg=%s)\n", Status.status_code, strError));
                return(-1);
            ...
            pHeadset->iParticipatingUserIndex = VOIP_INVALID_LOCAL_USER_INDEX;
            NetPrintf(("voipheadsetstadia: user %d, exiting the participating state\n", iValue));
        }
        else
        {
            NetPrintf(("voipheadsetstadia: participating state change ignore for user %d as user %d is currently participating\n", iValue, pHeadset->iParticipatingUserIndex));
            return(-1);
        }
    ...
    if ((iControl == 'ttos') && (iValue < VOIP_MAXLOCALUSERS))
    {
        if (pHeadset->iParticipatingUserIndex != VOIP_INVALID_LOCAL_USER_INDEX)
        {
            return((_VoipHeadsetCreateNarrate(pHeadset) == 0) ? VoipNarrateInput(pHeadset->pNarrate, iValue, pHeadset->uNarrationGender, VOIPNARRATE_CBFLAG_NONE, (const char *)pValue) : -1);
        }
    ...
    {
        return((_VoipHeadsetCreateNarrate(pHeadset) == 0) ? VoipNarrateInput(pHeadset->pNarrate, iValue, pHeadset->uNarrationGender, VOIPNARRATE_CBFLAG_LOCAL, (const char *)pValue) : -1);
    }
...
int32_t VoipHeadsetStatus(VoipHeadsetRefT *pHeadset, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'ttos')
    {
        return(pHeadset->iOutboundNarrating != 0);
    }
    if (iSelect == 'ttsm')
    {
