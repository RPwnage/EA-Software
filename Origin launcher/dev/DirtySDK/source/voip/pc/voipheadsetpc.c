...
// config flags
#define VOIP_HEADSET_CONFIG_STT             (1)
#define VOIP_HEADSET_CONFIG_TTS             (2)
#define VOIP_HEADSET_CONFIG_XLA             (4)
...
struct VoipHeadsetRefT
{
    ...
    VoipHeadsetDeviceInfoT Spk2Info;      //!< second output for local narration
    ...
    VoipTranslateRefT *pTranslateRef;
    ...
    uint8_t bChatRestricted;            //!< TRUE if user is chat restricted(underage), else FALSE
    uint8_t _pad;
    ...
    VoipHeadsetTranscribedTextReceivedCbT *pTranscribedTextReceivedCb; //!< callback invoked when transcribed text from a remote orginator needs to be surfaced up to voipcommon to get displayed locally
    ...
    // source language for transcription/translation/narration
    char    strLang[6];  // e.g. "en-US"
    uint8_t bTranslated;

    // TTS
    uint8_t bNarrating;
    uint8_t bLocalNarration;
    uint8_t bTranslateNarration;

    uint8_t uConfigFlags;
    uint8_t _pad1;
    ...
    // tracking for gating of narrated audio (required when using cloud-based narration)
    int32_t iEncAudioSamples;
    uint32_t uEncAudioTick;
...
static void _VoipHeadsetStop(VoipHeadsetRefT *pHeadset)
{
    ...
    _VoipHeadsetCloseDevice(pHeadset, &pHeadset->Spk2Info);
}

/*F********************************************************************************/
/*!
    \Function _VoipHeadsetWriteAudio

    \Description
        Write audio to output device

    \Input *pHeadset        - module state
    \Input *pSpkrInfo       - output device to write to
    \Input *pFrameData      - audio to write
    \Input iFrameSamples    - number of samples of audio to write
    \Input bVoipAudio       - TRUE if voip audio, else FALSE

    \Version 09/17/2019 (jbrookes) Split from _VoipHeadsetProcessPlayback
*/
/********************************************************************************F*/
static void _VoipHeadsetWriteAudio(VoipHeadsetRefT *pHeadset, VoipHeadsetDeviceInfoT *pSpkrInfo, const uint8_t *pFrameData, int32_t iFrameSamples, uint8_t bVoipAudio)
{
    HRESULT hResult;
    int32_t iFrameSize = iFrameSamples*2;

    // copy data to prepared wave buffer
    ds_memcpy_s(pSpkrInfo->WaveData[pSpkrInfo->iCurWaveBuffer].FrameData, sizeof(pSpkrInfo->WaveData[pSpkrInfo->iCurWaveBuffer].FrameData), pFrameData, iFrameSize);

    // if we're playing voip audio while locally narrating, reduce voip volume to give the local narration priority
    if (bVoipAudio && pHeadset->bLocalNarration)
    {
        int32_t iSample;
        int16_t *pFrameData16 = (int16_t *)pSpkrInfo->WaveData[pSpkrInfo->iCurWaveBuffer].FrameData;
        for (iSample = 0; iSample < iFrameSamples; iSample += 1)
        {
            pFrameData16[iSample] >>= 2;
        }
    }
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetWriteAudioCheck

    \Description
        Check to see if we can write to the current wave buffer

    \Input *pHeadset        - module state
    \Input *pSpkrInfo       - output device to write to

    \Output
        uint32_t            - TRUE if we can write, else FALSE

    \Version 10/15/2019 (jbrookes)
*/
/********************************************************************************F*/
static uint32_t _VoipHeadsetWriteAudioCheck(VoipHeadsetRefT *pHeadset, VoipHeadsetDeviceInfoT *pSpkrInfo)
{
    return((pSpkrInfo->WaveData[pSpkrInfo->iCurWaveBuffer].WaveHdr.dwFlags & pSpkrInfo->dwCheckFlag[pSpkrInfo->iCurWaveBuffer]) == 0);
}
...
static void _VoipHeadsetEnumerate(VoipHeadsetRefT *pHeadset)
{
    ...
    // enumerate output devices
    _VoipHeadsetEnumerateDevices(pHeadset, &pHeadset->Spk2Info);
}
...
static void _VoipHeadsetSendEncodedAudio(VoipHeadsetRefT *pHeadset, VoipMicrPacketT *pPacket, int32_t iCompBytes)
{
    ...
    else
    {
        ...
        VoipConduitReceiveVoiceData(pHeadset->pConduitRef, &VoipUser, pPacket->aData, iCompBytes, TRUE);
    }
}

/*F********************************************************************************/
/*!
    \Function _VoipHeadsetNarrateGateAudio

    \Description
        Gates sending of narrated audio.  This is required for sources (e.g. cloud
        narration) that are not implicitly provided metered to the correct rate,
        like a mic is.

    \Input *pHeadset    - module state
    \Input iNumSamples  - number of samples to consider for playback

    \Version 09/13/2019 (jbrookes)
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetNarrateGateAudio(VoipHeadsetRefT *pHeadset, int32_t iNumSamples)
{
    int32_t iEncAudioTime, iEncAudioRate;
    // reset encoded audio timestamp if at start of phrase
    if ((pHeadset->iEncAudioSamples == 0) && (pHeadset->uEncAudioTick == 0))
    {
        pHeadset->uEncAudioTick = NetTick();
    }
    // calculate elapsed time for phrase and rate we've submitted audio so far
    iEncAudioTime = NetTickDiff(NetTick(), pHeadset->uEncAudioTick)+1;
    iEncAudioRate = ((pHeadset->iEncAudioSamples+iNumSamples)*1000)/iEncAudioTime;
    /* throttle rate to 16khz - we need to do this for audio that is not produced by a 16khz metered source (e.g. cloud narration);
       we skip this at the start of the stream to allow a little prebuffering to cover for any stalls in producing data */
    if ((iEncAudioRate > VOIP_HEADSET_SAMPLERATE) && (pHeadset->iEncAudioSamples > VOIP_HEADSET_FRAMESAMPLES*2))
    {
        return(1);
    }
    // update sample tracker and encode
    pHeadset->iEncAudioSamples += iNumSamples;
    return(0);
}

/*F********************************************************************************/
/*!
    \Function _VoipHeadsetNarrateWrite

    \Description
        Write narration audio - writes locally to speaker if narrating a translation
        of a received transcription, else writes encoded audio for sending over
        voip.

    \Input *pHeadset    - module state
    \Input bLocal       - TRUE for local playback, else FALSE

    \Version 09/13/2019 (jbrookes)
*/
/********************************************************************************F*/
static void _VoipHeadsetNarrateWrite(VoipHeadsetRefT *pHeadset, uint8_t bLocal)
{
    uint8_t bTranscriptionEnabled = (pHeadset->bVoiceTranscriptionEnabled && !pHeadset->bTranslated);
    int32_t iCompBytes;
    VoipMicrPacketT PacketData;
    pHeadset->iNarrateWritePos = 0;

    // if local, write directly to the output device
    if (bLocal)
    {
        _VoipHeadsetWriteAudio(pHeadset, &pHeadset->Spk2Info, (const uint8_t *)pHeadset->narrateBuffer, VOIP_HEADSET_FRAMESAMPLES, FALSE);
    }
    // compress the input data, and forward it to send as appropriate
    else if ((iCompBytes = VoipCodecEncode(PacketData.aData, (const int16_t*)pHeadset->narrateBuffer, VOIP_HEADSET_FRAMESAMPLES, bTranscriptionEnabled ? pHeadset->pTranscribeRef : NULL)) > 0)
    {
...
/*!
    ...
    \Output
        int32_t         - number of bytes of data consumed
...
static int32_t _VoipHeadsetNarrateCb(VoipNarrateRefT *pNarrateRef, int32_t iUserIndex, const int16_t *pSamples, int32_t iSize, uint32_t uFlags, void *pUserData)
{
    ...
    int32_t iResult = 0, iRemain, iCopySize;
    uint8_t bLocalNarration = uFlags & VOIPNARRATE_CBFLAG_LOCAL ? TRUE : FALSE, bGateAudio;

    // process audio data
    if (iSize > 0)
    {
        for (iRemain = iSize; iRemain > 0; iRemain -= iCopySize)
        {
            int8_t *pFrameBuffer = pHeadset->narrateBuffer + pHeadset->iNarrateWritePos;

            // calculate copy size
            iCopySize = (iRemain >= (VOIP_HEADSET_FRAMESIZE - pHeadset->iNarrateWritePos)) ? VOIP_HEADSET_FRAMESIZE - pHeadset->iNarrateWritePos : iRemain;

            // gate audio playback rate
            bGateAudio = bLocalNarration ? _VoipHeadsetWriteAudioCheck(pHeadset, &pHeadset->Spk2Info) : _VoipHeadsetNarrateGateAudio(pHeadset, iCopySize/2);
            if (bGateAudio)
            {
                break;
            }
            ...
            // submit audio if we have a full frame
            if ((pHeadset->iNarrateWritePos + iCopySize) == VOIP_HEADSET_FRAMESIZE)
            {
                _VoipHeadsetNarrateWrite(pHeadset, bLocalNarration);
            }
            else
            {
        ...
        // calculate amount of audio data consumed
        iResult = iSize - iRemain;
    }
    else if (iSize == VOIPNARRATE_STREAM_START)
    {
        pHeadset->bNarrating = TRUE;
        pHeadset->bLocalNarration = bLocalNarration;
        // reset audio rate gate variables at stream start
        pHeadset->iEncAudioSamples = pHeadset->uEncAudioTick = 0;
    }
    else if (iSize == VOIPNARRATE_STREAM_END)
    {
        pHeadset->bNarrating = FALSE;
        pHeadset->bLocalNarration = FALSE;
        // clear translated flag if narration from translation is complete
        pHeadset->bTranslated = FALSE;
    }
    return(iResult);
}
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetTranscribeCreate(VoipHeadsetRefT *pHeadset)
{
    // early out if we've already created the module
    ...
    // set debug level to current setting
    #if DIRTYCODE_LOGGING
    VoipTranscribeControl(pHeadset->pTranscribeRef, 'spam', pHeadset->iDebugLevel, 0, NULL);
    #endif
    // set language 
    VoipTranscribeControl(pHeadset->pTranscribeRef, 'lang', 0, 0, pHeadset->strLang);
    // return success
    return(0);
}
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetNarrateCreate

    \Description
        Create Narration module
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetNarrateCreate(VoipHeadsetRefT *pHeadset)
{
    // early out if we've already created the module
    if (pHeadset->pNarrateRef != NULL)
    {
        return(0);
    }
    ...
    {
        NetPrintf(("voipheadsetpc: failed to create narration module\n"));
        return(-1);
    }
    // set debug level to current setting
    #if DIRTYCODE_LOGGING
    VoipNarrateControl(pHeadset->pNarrateRef, 'spam', pHeadset->iDebugLevel, 0, NULL);
    #endif
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetTranslateCreate
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetTranslateCreate(VoipHeadsetRefT *pHeadset)
{
    // early out if we've already created the module
    if (pHeadset->pTranslateRef != NULL)
    {
        return(0);
    }
    // allocate translation module
    if ((pHeadset->pTranslateRef = VoipTranslateCreate()) == NULL)
    {
        return(-1);
    }
    // set debug level to current setting
    #if DIRTYCODE_LOGGING
    VoipTranslateControl(pHeadset->pTranslateRef, 'spam', pHeadset->iDebugLevel, 0, NULL);
    #endif
    // return success
    return(0);
}
...
static void _VoipHeadsetProcessPlay(VoipHeadsetRefT *pHeadset)
{
    ...
    for (uPacket = 0; uPacket < uPackets; uPacket++)
    {
        ...
        if ((!pHeadset->bChatRestricted) && ((iCompBytes = VoipCodecEncode(PacketData.aData, &pHeadset->pPlayerBuffer[VOIP_HEADSET_FRAMESAMPLES * pHeadset->uPlayerBufferFrameCurrent], VOIP_HEADSET_FRAMESAMPLES, pHeadset->pTranscribeRef)) > 0))
        {
            _VoipHeadsetSendEncodedAudio(pHeadset, &PacketData, iCompBytes);
        }
    }
...
static void _VoipHeadsetProcessRecord(VoipHeadsetRefT *pHeadset)
{
    ...
    if (pMicrInfo->bActive == TRUE)
    {
        ...
        for (; pMicrInfo->WaveData[pMicrInfo->iCurWaveBuffer].WaveHdr.dwFlags & WHDR_DONE; )
        {
            ...
            if ((!pHeadset->bNarrating || pHeadset->bLocalNarration) && (!pHeadset->bMuted || pHeadset->bLoopback))
            {
                if (!pHeadset->bChatRestricted && ((iCompBytes = VoipCodecEncode(PacketData.aData, (int16_t *)FrameData, VOIP_HEADSET_FRAMESAMPLES, pHeadset->bVoiceTranscriptionEnabled ? pHeadset->pTranscribeRef : NULL)) > 0))
                {
                    _VoipHeadsetSendEncodedAudio(pHeadset, &PacketData, iCompBytes);
                }
            }
...
static void _VoipHeadsetProcessPlayback(VoipHeadsetRefT *pHeadset)
{
    ...
    if (pSpkrInfo->bActive == TRUE)
    {
        ...
        for ( ; pSpkrInfo->WaveData[pSpkrInfo->iCurWaveBuffer].WaveHdr.dwFlags & pSpkrInfo->dwCheckFlag[pSpkrInfo->iCurWaveBuffer]; )
        {
            ...
            {
                // user is chat restricted so do not process the data
                if (pHeadset->bChatRestricted)
                {
                    continue;
                }
                ...
                _VoipHeadsetWriteAudio(pHeadset, pSpkrInfo, FrameData, VOIP_HEADSET_FRAMESAMPLES, TRUE);
            }
...
static void _VoipHeadsetProcessTranscription(VoipHeadsetRefT *pHeadset)
{
    ...
    // if config has changed, process it now
    if (pHeadset->uConfigFlags & VOIP_HEADSET_CONFIG_STT)
    {
        VoipTranscribeControl(pHeadset->pTranscribeRef, 'conf', 0, 0, NULL);
        pHeadset->uConfigFlags &= ~VOIP_HEADSET_CONFIG_STT;
    }
    ...
    if (VoipTranscribeGet(pHeadset->pTranscribeRef, strTranscribeBuf+5, sizeof(strTranscribeBuf)-5) > 0)
    {
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetProcessNarration
    ...
    \Version 10/15/2018 (jbrookes)
*/
/********************************************************************************F*/
static void _VoipHeadsetProcessNarration(VoipHeadsetRefT *pHeadset)
{
    // if config has changed, process it now
    if (pHeadset->uConfigFlags & VOIP_HEADSET_CONFIG_TTS)
    {
        VoipNarrateControl(pHeadset->pNarrateRef, 'conf', 0, 0, NULL);
        pHeadset->uConfigFlags &= ~VOIP_HEADSET_CONFIG_TTS;
    }
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetProcessTranslation
    ...
    \Version 01/30/2019 (jbrookes)
*/
/********************************************************************************F*/
static void _VoipHeadsetProcessTranslation(VoipHeadsetRefT *pHeadset)
{
    char strTranslateBuf[VOIPTRANSLATE_MAX];
    VoipUserT *pOriginator;

    // if config has changed, process it now
    if (pHeadset->uConfigFlags & VOIP_HEADSET_CONFIG_XLA)
    {
        VoipTranslateControl(pHeadset->pTranslateRef, 'conf', 0, 0, NULL);
        pHeadset->uConfigFlags &= ~VOIP_HEADSET_CONFIG_XLA;
    }

    // process translation
    VoipTranslateUpdate(pHeadset->pTranslateRef);

    // if a translation is available, get it
    if (VoipTranslateGet(pHeadset->pTranslateRef, strTranslateBuf, sizeof(strTranslateBuf), &pOriginator) > 0)
    {
        pHeadset->bTranslated = TRUE;
        // narrate, if narration of translation is enabled
        if ((pHeadset->pNarrateRef != NULL) && (pHeadset->bTranslateNarration))
        {
            VoipNarrateInput(pHeadset->pNarrateRef, 0, pHeadset->eDefaultGender, VOIPNARRATE_CBFLAG_LOCAL, strTranslateBuf);
        }
        // forward to transcription callack
        if (pHeadset->iParticipatingUserIndex != VOIP_INVALID_LOCAL_USER_INDEX)
        {
            if (!pHeadset->bLoopback)
            {
                pHeadset->pTranscribedTextReceivedCb(pOriginator, strTranslateBuf, pHeadset->pTranscribedTextReceivedCbUserData);
                // we made a copy of the originator - free it here
                DirtyMemFree(pOriginator, VOIP_MEMID, pHeadset->iMemGroup, pHeadset->pMemGroupUserData);
            }
            else
            {
                pHeadset->pTextDataCb(strTranslateBuf, pHeadset->iParticipatingUserIndex, pHeadset->pCbUserData);            
            }
        }
    }
}
...
static int32_t _VoipHeadsetSetRemoteUserVoicePlayback(VoipHeadsetRefT *pHeadset, VoipUserT *pRemoteUser, int32_t iLocalUserIndex, uint8_t bEnablePlayback)
{
    ...
    NetPrintf(("voipheadsetpc: %s applied for luser(%d) & ruser(%d):%llu | 0x%08x -> 0x%08x\n",
        bEnablePlayback ? "+pbk" : "-pbk", iLocalUserIndex, iRemoteUserIndex, pRemoteUser->AccountInfo.iPersonaId,
        uOldPlaybackFlags, pHeadset->aLocalUsers[iLocalUserIndex].uPlaybackFlags));
...
/*!
    ...
    \Description
        start narration on the given input text
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetNarrateInput(VoipHeadsetRefT* pHeadset, int32_t iUserIndex, VoipNarrateGenderE eGender, uint32_t uFlags, const char* pText)
{
    int32_t iResult = 0;

    // make sure narration ref is available
    iResult = _VoipHeadsetNarrateCreate(pHeadset);
    // do narration if we have a ref
    if (pHeadset->pNarrateRef != NULL)
    {
        if (!pHeadset->bChatRestricted)
        {
            iResult = VoipNarrateInput(pHeadset->pNarrateRef, iUserIndex, ((eGender > VOIPNARRATE_GENDER_NONE) && (eGender < VOIPNARRATE_NUMGENDERS)) ? (VoipNarrateGenderE)eGender : pHeadset->eDefaultGender, uFlags, (const char*)pText);
        }
    }
...
VoipHeadsetRefT *VoipHeadsetCreate(int32_t iMaxConduits, VoipHeadsetMicDataCbT *pMicDataCb, VoipHeadsetTextDataCbT *pTextDataCb, VoipHeadsetOpaqueDataCbT *pOpaqueDataCb, VoipHeadsetStatusCbT *pStatusCb, void *pCbUserData, int32_t iData)
{
    ...
    // calculate size of headset ref, including variable-length user list
    ...
    // set default language for transcription/translation/narration
    ...
    pHeadset->Spk2Info.eDevType = VOIP_HEADSET_OUTDEVICE;
    pHeadset->Spk2Info.iActiveDevice = -1;
    pHeadset->Spk2Info.iDeviceToOpen = WAVE_MAPPER;
    ...
    pHeadset->Spk2Info.iNewVolume = -1;
...
int32_t VoipHeadsetDestroy(VoipHeadsetRefT *pHeadset)
{
    ...
    // destroy translation ref
    if (pHeadset->pTranslateRef != NULL)
    {
        VoipTranslateDestroy(pHeadset->pTranslateRef);
    }
...
void VoipHeadsetReceiveVoiceDataCb(VoipUserT *pRemoteUsers, int32_t iRemoteUserSize, int32_t iConsoleId, VoipMicrInfoT *pMicrInfo, uint8_t *pPacketData, void *pUserData)
{
    ...
    for (uMicrPkt = 0; uMicrPkt < pMicrInfo->uNumSubPackets; uMicrPkt++)
    {
        ...
        VoipConduitReceiveVoiceData(pHeadset->pConduitRef, &pRemoteUsers[uRemoteUserIndex], pSubPacket, uSubPacketSize, TRUE);
...
{
    // process whether user is chat restricted
    pHeadset->bChatRestricted = NetConnStatus('chat', 0, NULL, 0);
    ...
    if (pHeadset->pTranslateRef != NULL)
    {
        _VoipHeadsetProcessTranslation(pHeadset);
    }
    // process narration
    if (pHeadset->pNarrateRef != NULL)
    {
        _VoipHeadsetProcessNarration(pHeadset);
    }
    ...
    _VoipHeadsetProcessDeviceChange(pHeadset, &pHeadset->Spk2Info);
}
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'txtn' - locally play back utf8 text in pValue as voice and initialize TTS voice
            ...
            'voic' - configure voipnarrate
            'xnar' - enable/disable local narration of translation result
            'xply' - enable/disable crossplay (no difference on PC)
            'xlat' - [DEBUG] translate text for test purposes
        \endverbatim
...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if ((iControl == 'cstm') && (pHeadset->pTranscribeRef != NULL))
    {
        return(VoipTranscribeControl(pHeadset->pTranscribeRef, iControl, iValue, 0, NULL));
    }
    if ((iControl == 'ctsm') && (pHeadset->pNarrateRef != NULL))
    {
    ...
    if ((iControl == 'idev') || (iControl == 'odev'))
    {
        ...
        if (pDeviceInfo->iActiveDevice != iValue)
        {
            ...
            if (iControl == 'odev')
            {
                pHeadset->Spk2Info.iDeviceToOpen = iValue;
                pHeadset->Spk2Info.bChangeDevice = TRUE;
            }
            NetCritLeave(&pHeadset->DevChangeCrit);
        }
        else
    ...
    if ((iControl == 'lang') && (pValue != NULL))
    {
        ...
        NetPrintf(("voipheadsetpc: setting language to %s\n", pHeadset->strLang));
        if (pHeadset->pNarrateRef != NULL)
        {
            VoipNarrateControl(pHeadset->pNarrateRef, 'lang', iValue, iValue2, pValue);
        }
        if (pHeadset->pTranscribeRef != NULL)
        {
            VoipTranscribeControl(pHeadset->pTranscribeRef, 'lang', iValue, iValue2, pValue);
        }
        return(0);
    }
    if (iControl == 'loop')
    ...
    if (iControl == 'tran')
    {
        if (pHeadset->bVoiceTranscriptionEnabled != iValue)
        {
            ...
            if (pHeadset->bVoiceTranscriptionEnabled && (pHeadset->pTranscribeRef == NULL))
            {
                return(_VoipHeadsetTranscribeCreate(pHeadset));
            }
        }
        return(0);
    }
    ...
    if ((iControl == 'voic') && (iValue < VOIP_MAXLOCALUSERS) && (pValue != NULL))
    {
        ...
        if ((pHeadset->pNarrateRef != NULL) && (pCfg->iLanguagePackCode != 0))
        {
            iResult = VoipNarrateControl(pHeadset->pNarrateRef, 'lang', pCfg->iLanguagePackCode, 0, NULL);
        }
        return(iResult);
    }
    if (iControl == 'ttos')
    {
        if (pHeadset->iParticipatingUserIndex != VOIP_INVALID_LOCAL_USER_INDEX)
        {
            return(_VoipHeadsetNarrateInput(pHeadset, iValue, iValue2, VOIPNARRATE_CBFLAG_NONE, (const char*)pValue));
        }
        else
        {
    ...
    if (iControl == 'txtn')
    {
        return(_VoipHeadsetNarrateInput(pHeadset, iValue, iValue2, VOIPNARRATE_CBFLAG_LOCAL, (const char*)pValue));
    }
    if ((iControl == 'uvoc') && (pHeadset->pNarrateRef != NULL))
    {
        VoipNarrateControl(pHeadset->pNarrateRef, 'uvoc', 0, 0, NULL);
        return(0);
    }
    if ((iControl == 'xlat') && (pHeadset->pTranslateRef != NULL) && (pValue != NULL))
    {
        char strDstLang[3];
        strDstLang[0] = (char)(iValue>>8);
        strDstLang[1] = (char)iValue;
        strDstLang[2] = '\0';
        return(VoipTranslateInput(pHeadset->pTranslateRef, iValue, pHeadset->strLang, strDstLang, (const char *)pValue, NULL));
    }
    if (iControl == 'xnar')
    {
        pHeadset->bTranslateNarration = iValue ? TRUE : FALSE;
        NetPrintf(("voipheadset: local narration of translation %s\n", pHeadset->bTranslateNarration ? "enabled" : "disabled"));
        return(0);
    }
    if (iControl == 'xply')
    {
        ...
        {
            NetPrintf(("voipheadsetpc: changing crossplay mode to: %s\n", bCrossplay ? "crossplay" : "native"));
...
int32_t VoipHeadsetStatus(VoipHeadsetRefT *pHeadset, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'ttsm')
    {
        if ((pHeadset->pNarrateRef != NULL) && (iValue == pHeadset->iParticipatingUserIndex))
        {
...
void VoipHeadsetConfigTranscription(VoipHeadsetRefT *pHeadset, uint32_t uProfile, const char *pUrl, const char *pKey)
{
    ...
    _VoipHeadsetTranscribeCreate(pHeadset);
    pHeadset->uConfigFlags |= VOIP_HEADSET_CONFIG_STT;
}
...
/*!
    ...
    \Input *pHeadset    - headset module state
    \Input eProvider    - narrate provider identifier
    \Input *pUrl        - narrate provider url
    \Input *pKey        - narrate key

    \Version 01/07/2019 (tcho)
*/
/********************************************************************************F*/
void VoipHeadsetConfigNarration(VoipHeadsetRefT *pHeadset, VoipSpeechProviderE eProvider, const char *pUrl, const char *pKey)
{
    ...
    _VoipHeadsetNarrateCreate(pHeadset);
    pHeadset->uConfigFlags |= VOIP_HEADSET_CONFIG_TTS;
}

/*F********************************************************************************/
/*!
    \Function VoipHeadsetConfigTranslation

    \Description
        Configure the translate module

    \Input *pHeadset    - headset module state
    \Input eProvider    - translate provider identifier
    \Input *pUrl        - translate provider url
    \Input *pKey        - translate key

    \Version 09/17/2019 (jbrookes)
*/
/********************************************************************************F*/
void VoipHeadsetConfigTranslation(VoipHeadsetRefT *pHeadset, VoipSpeechProviderE eProvider, const char *pUrl, const char *pKey)
{
    VoipTranslateConfig(eProvider, pUrl, pKey);
    _VoipHeadsetTranslateCreate(pHeadset);
    pHeadset->uConfigFlags |= VOIP_HEADSET_CONFIG_XLA;
}
...
/*!
    ...
    \Description
        Make sure inbound transcribed text can be displayed to local users; also
        starts translation of transcription if enabled.

    \Input *pHeadset    - headset module state
    \Input *pOriginator - originator of the message
    \Input *pStrUtf8    - message to be displayed

    \Version 09/04/2019 (migallant) copied from voipheadsetps4
*/
/********************************************************************************F*/
void VoipHeadsetTranscribedTextPermissionCheck(VoipHeadsetRefT *pHeadset, VoipUserT *pOriginator, const char *pStrUtf8)
{
    ...
    for (iRemoteUserSpaceIndex = 0; iRemoteUserSpaceIndex < pHeadset->iMaxRemoteUsers; iRemoteUserSpaceIndex++)
    {
        ...
        // extract source language prefix
        ds_strnzcpy(strSrcLang, pStrUtf8, sizeof(strSrcLang));
        pStrUtf8 += 5;

        // are there communication restrictions?
        if (!pHeadset->bChatRestricted)
        {
            // translate if a) enabled, b) a destination language is specified, and c) the source and dest languages don't already match
            if ((pHeadset->pTranslateRef != NULL) && (pHeadset->strLang[0] != '\0') && (strcmp(strSrcLang, pHeadset->strLang)))
            {
                // make a copy of originator for later reference when we forward the translated transcription to the callback
                VoipUserT *pOrigCopy = DirtyMemAlloc(sizeof(*pOrigCopy), VOIP_MEMID, pHeadset->iMemGroup, pHeadset->pMemGroupUserData);
                ds_memcpy_s(pOrigCopy, sizeof(*pOrigCopy), pOriginator, sizeof(*pOriginator));
                // start translation
                VoipTranslateInput(pHeadset->pTranslateRef, 0, strSrcLang, pHeadset->strLang, pStrUtf8, pOrigCopy);
            }
            else
            {
                // forward text to callback for display
                pHeadset->pTranscribedTextReceivedCb(pOriginator, pStrUtf8, pHeadset->pTranscribedTextReceivedCbUserData);
            }
        }
        else
        {
            NetPrintf(("voipheadsetpc: transcribed text from %lld dropped because chat is restricted -- text was: %s\n",
...
/*!
    ...
    \Description
        Set callback to be invoked by voipheadsetpc when inbound transcribed text
