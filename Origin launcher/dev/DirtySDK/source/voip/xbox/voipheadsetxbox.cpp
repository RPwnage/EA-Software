/*H********************************************************************************/
/*!
    \File voipheadsetxbox.cpp

    \Description
        VoIP headset manager. Wraps MS Game Chat 2 api.
...
typedef struct
{
    ...
    uint8_t                 bNeedDestroy;    //!< do we need to wait for the stream destroy notification
    uint8_t                 _pad[3];
...
typedef struct
{
    ...
    uint8_t bAddedToGC2;            //!< is the user added to gamechat 2
...
typedef struct
{
    ...
    uint8_t bAddedToGC2;    //!< is the user added to gamechat2
    uint8_t _pad[3];
} XONERemoteVoipUserT;
...
struct VoipHeadsetRefT
{
    ...
    uint8_t bGC2Initialized;           //!< is gamechat 2 initialzied
    uint8_t bGC2Cleanup;               //!< cleanup gc2
    uint8_t bCrossplayModeChange;      //!< are we performing a mode change
    uint8_t _pad1;

    char    strLang[6];                // e.g. "en-US"
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetFirstPartyChatIndicator

    \Description


    \Input *pHeadset    - headset ref
    \Input bLocal       - TRUE if local user, FALSE if remote user
    \Input iUserIndex   - index of user

    \Output
        int32_t         - chat_indicator value from gamechat2, negative if error

    \Version 3/25/2020 (mgallant)
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetFirstPartyChatIndicator(VoipHeadsetRefT *pHeadset, uint8_t bLocal, int32_t iUserIndex)
{
    chat_user *pChatUser;
    VoipUserT *pUser;

    // make sure indices are within bounds
    if ((iUserIndex < 0) || (iUserIndex >= (bLocal ? VOIP_MAXLOCALUSERS : pHeadset->iMaxRemoteUsers)))
    {
        NetPrintf(("voipheadsetxbox: _VoipHeadsetFirstPartyChatIndicator() indices out of bounds for %s user: %d",
            (bLocal ? "local" : "remote"),
            iUserIndex));
        return(-1);
    }
    ...
    if (bLocal)
    {
        pChatUser = pHeadset->aLocalUsers[iUserIndex].pChatUser;
        pUser = &pHeadset->aLocalUsers[iUserIndex].User;
    } 
    else
    {
        pChatUser = pHeadset->aRemoteUsers[iUserIndex].pChatUser;
        pUser = &pHeadset->aRemoteUsers[iUserIndex].User;
    }
    ...
    if ((pUser->ePlatform == VOIP_PLATFORM_XBOXONE) &&
        ...
        (pChatUser != NULL))
    {
        return((int32_t)pChatUser->chat_indicator());
    }
    else
    {
        NetPrintf(("voipheadsetxbox: _VoipHeadsetFirstPartyChatIndicator() %s user: %d was not a valid xbox user",
            (bLocal ? "local" : "remote"),
            iUserIndex));
        return(-2);
    }
...
static uint8_t _VoipHeadsetPlaybackCallback(VoipMixerRefT *pMixer, VoipUserT *pRemoteUser, void *pUserData)
{
    ...
    if (pSinkStream == NULL)
    {
        NetPrintf(("voipheadsetxbox: _VoipHeadsetPlaybackCallback() cannot find the sink stream for user iPersonaId: %lld", pRemoteUser->AccountInfo.iPersonaId));
        return(bPlayback);
    }
    ...
    {
        NetPrintf(("voipheadsetxbox: _VoipHeadsetPlaybackCallback() cannot find the remote user iPersonaId: %lld", pRemoteUser->AccountInfo.iPersonaId));
        return(bPlayback);
    }
...
static VoipTranscribeRefT *_VoipHeadsetTranscibeCreate(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex)
{
    ...
    if (iPreEncodeStreamIndex < VOIP_MAXLOCALUSERS)
    {
        if (pHeadset->aPreEncodedStreams[iPreEncodeStreamIndex].pTranscribeRef == NULL)
        {
            ...
            VoipTranscribeControl(pHeadset->aPreEncodedStreams[iPreEncodeStreamIndex].pTranscribeRef, 'lang', 0, 0, pHeadset->strLang);
        }
...
static void _VoipHeadsetDumpChatUsers(VoipHeadsetRefT *pHeadset)
{
    ...
    if (pHeadset->bGC2Initialized == FALSE)
    {
        return;
    }
    ...
    NetPrintf(("voipheadsetxbox: CHAT USERS DUMP  --  %d chat user(s) currently registered with the chat manager:\n", uChatUserCount));
    ...
    for (uChatUserIndex = 0; uChatUserIndex < uChatUserCount; uChatUserIndex += 1)
    {
        ...
        NetPrintf(("voipheadsetxbox: chat user #%d --> local:%s   xuid:%S   chat_indicator:%s\n",
...
/*!
    ...
    \Description
        Looks up the user and updates the remote user structure with the xuid

    \Input *pHeadset        - headset ref
    \Input *pUser           - [in/out] user for which the XBoxUserId is queried
     
    \Version 11/22/2018 (mclouatre)
*/
/********************************************************************************F*/
static void _VoipHeadsetGetXboxUserIdForVoipUser(VoipHeadsetRefT *pHeadset, XONERemoteVoipUserT *pUser)
{
    if ((pUser->User.ePlatform == VOIP_PLATFORM_XBOXONE) && (pHeadset->pFirstPartyIdCallback != NULL))
    {
        pUser->uXuid = pHeadset->pFirstPartyIdCallback(pUser->User.AccountInfo.iPersonaId, pHeadset->pFirstPartyIdCbUserData);
    }
}
...
static int32_t _VoipHeadsetAddLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, VoipUserT *pLocalUser)
{
    ...
    {
        NetPrintf(("voipheadsetxbox: can't add NULL local talker at local user index %d\n", iLocalUserIndex));
        return(-1);
    }
    ...
    if (pHeadset->aLocalUsers[iLocalUserIndex].pChatUser != NULL)
    {
        NetPrintf(("voipheadsetxbox: can't add multiple local talkers at the same index (%d)\n", iLocalUserIndex));
        return(-2);
    }
    ...
    {
        NetPrintf(("voipheadsetxbox: local user at index %d either doesn't exist or has invalid xuid\n", iLocalUserIndex));
        return(-4);
    }
    ...
    {
        NetPrintf(("voipheadsetxbox: could not retrieve gamertag for local user at index %d\n", iLocalUserIndex));
        return(-5);
    }
    ...
    if (_VoipHeadsetGetChatUserForXBoxUserId(wstrXuid.c_str()) == NULL)
    {
        ...
        pHeadset->aLocalUsers[iLocalUserIndex].bAddedToGC2 = FALSE;
        pHeadset->aLocalUsers[iLocalUserIndex].uPlaybackFlags = 0xffffffff;
        NetPrintf(("voipheadsetxbox: adding local talker %lld at index %d (gamertag: %s, xuid: %llu)\n",
            pLocalUser->AccountInfo.iPersonaId, iLocalUserIndex, strGamertag, uXuid));
    }
    else
    {
        NetPrintf(("voipheadsetxbox: local talker %lld at local user index %d is already known by the chat manager\n",
...
static int32_t _VoipHeadsetRemoveLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, VoipUserT *pLocalUser)
{
    ...
    if (pHeadset->aLocalUsers[iLocalUserIndex].bParticipating == FALSE)
    {
        if (pHeadset->aLocalUsers[iLocalUserIndex].pChatUser != NULL)
        {
            NetPrintf(("voipheadsetxbox: deleting local talker (xuid: %S) at index %d\n",
            ...
            pHeadset->aLocalUsers[iLocalUserIndex].bAddedToGC2 = FALSE;
            pHeadset->aLocalUsers[iLocalUserIndex].pChatUser = NULL;
        ...
        {
            NetPrintf(("voipheadsetxbox: local talker at local user index %d is already deleted\n", iLocalUserIndex));
            iRetCode = -1;
        }
    }
    else
    {
        NetPrintf(("voipheadsetxbox: cannot delete local talker (xuid: %S) at local user index %d because the user must first be deactivated\n",
            pHeadset->aLocalUsers[iLocalUserIndex].pChatUser->xbox_user_id(), iLocalUserIndex));
        iRetCode = -2;
    }
...
static int32_t _VoipHeadsetActivateLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, bool bParticipate)
{
    ...
    if (NetConnStatus('xuid', iLocalUserIndex, &uXuid, sizeof(uXuid)) < 0)
    {
        NetPrintf(("voipheadsetxbox: local user at index %d either doesn't exist or has an invalid xuid\n", iLocalUserIndex));
        return(-1);
    }

    if ((pHeadset->aLocalUsers[iLocalUserIndex].bParticipating == FALSE) && (bParticipate == FALSE))
    {
        NetPrintf(("voipheadsetxbox: cannot demote user at local index %d because not already a participating user. Ignoring.\n", iLocalUserIndex));
        return(-2);
    }

    pHeadset->aLocalUsers[iLocalUserIndex].bParticipating = bParticipate;
    NetPrintf(("voipheadsetxbox: local user (xuid: %llu) at index %d is %s participating\n",
        pHeadset->aLocalUsers[iLocalUserIndex].uXuid, iLocalUserIndex, (pHeadset->aLocalUsers[iLocalUserIndex].bParticipating ? "now" : "no longer")));
    return(0);
}
...
static int32_t _VoipHeadsetAddRemoteTalker(VoipHeadsetRefT *pHeadset, VoipUserT *pRemoteUser, uint64_t uConsoleId)
{
    ...
    if (pXoneRemoteUser == NULL)
    {
        ...
        if (pXoneRemoteUser != NULL)
        {
            ...
            {
                _VoipHeadsetGetXboxUserIdForVoipUser(pHeadset, pXoneRemoteUser);

                NetPrintf(("voipheadsetxbox: adding remote talker %lld (xuid: %llu console id: 0x%llx)\n",
                    pRemoteUser->AccountInfo.iPersonaId, pXoneRemoteUser->uXuid, uConsoleId));

                pXoneRemoteUser->bAddedToGC2 = FALSE;
            }
        ...
        {
            NetPrintf(("voipheadsetxbox: adding remote talker %lld failed because aRemoteUsers is full\n", pRemoteUser->AccountInfo.iPersonaId));
            iRetCode = -2;
        }
    }
    else
    {
        NetPrintf(("voipheadsetxbox: adding remote talker %lld failed because it already exists\n", pRemoteUser->AccountInfo.iPersonaId));
        iRetCode = -3;
    }
...
static int32_t _VoipHeadsetRemoveRemoteTalker(VoipHeadsetRefT *pHeadset, VoipUserT *pUser)
{
    ...
    if (pXoneRemoteUser != NULL)
    {
        if (pXoneRemoteUser->User.ePlatform == VOIP_PLATFORM_XBOXONE)
        {
            if (pChatUser != NULL)
            {
                NetPrintf(("voipheadsetxbox: deleting remote talker (xuid: %llu)\n", pXoneRemoteUser->uXuid));
                chat_manager::singleton_instance().remove_user(pChatUser);
                pXoneRemoteUser->pChatUser = NULL;
            }
            else
            {
                NetPrintf(("voipheadsetxbox: deleting remote talker %lld failed because it cannot not find the associated game chat user\n", pUser->AccountInfo.iPersonaId));
                iRetCode = -1;
            }
        }
    ...
    {
        NetPrintf(("voipheadsetxbox: deleting remote talker %lld failed because it does not exists\n", pUser->AccountInfo.iPersonaId));
        iRetCode = -2;
    }
...
static int32_t _VoipHeadsetSynthesizeSpeech(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, const char *pText)
{
    ...
    if ((pHeadset->bCrossplay == TRUE) && (pHeadset->aLocalUsers[iLocalUserIndex].bParticipating == TRUE))
    {
        ...
        {
            VoipNarrateInput(pHeadset->aLocalUsers[iLocalUserIndex].pNarrateRef, iLocalUserIndex, pHeadset->aLocalUsers[iLocalUserIndex].eGender, VOIPNARRATE_CBFLAG_NONE, pText);
        }
    }
    else if ((pHeadset->bCrossplay == FALSE) && (pHeadset->aLocalUsers[iLocalUserIndex].pChatUser != NULL))
    {
        ...
        if (pHeadset->aLocalUsers[iLocalUserIndex].pChatUser->local()->text_to_speech_conversion_preference_enabled() == true)
        {
            ...
            if (pHeadset->iTTSInputLen < iSizeNeeded)
            {
                ...
                {
                    NetPrintf(("voipheadsetxbox: cannot allocate ttos text!\n"));
                    iRetCode = -1;
                }
                else
                {
        ...
        {
            NetPrintf(("voipheadsetxbox: game chat 2 will not synthesize speech for local talker at local user index %d because it has TTS disabled\n", iLocalUserIndex));
        }
    }
    else
    {
        NetPrintf(("voipheadsetxbox: can't synthesize speech for local talker at local user index %d because no matching chat user found\n", iLocalUserIndex));
        iRetCode = -2;
    }
...
static void _VoipHeadsetHandlePostDecodeSinkStreamCreated(VoipHeadsetRefT *pHeadset, post_decode_audio_sink_stream *pSinkStream)
{
    ...
    {
        NetPrintf(("voipheadsetxbox: no space to add sink stream (ID:%s)\n", pSinkStream->get_device_id()));
        return;
    }

    if ((pCurMixerStream->pMixer = VoipMixerCreate(16, VOIPHEADSET_FRAMESAMPLES)) == NULL)
    {
        NetPrintf(("voipheadsetxbox: failed to create voip mixer for sink stream (ID:%s)\n", pSinkStream->get_device_id()));
        return;
    }
    ...
    pCurMixerStream->bNeedDestroy = TRUE;
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetHandlePostDecodeSinkStreamDestroy

    \Description
        Process stream notification  game_chat_stream_state_change_type::post_decode_audio_sink_stream_destroy

    \Input *pHeadset    - headset module
    \Input *pSinkStream - recently created pre encode audio stream

    \Version 08/13/2020 (cvienneau)
*/
/********************************************************************************F*/
static void _VoipHeadsetHandlePostDecodeSinkStreamDestroy(VoipHeadsetRefT *pHeadset, post_decode_audio_sink_stream *pSinkStream)
{
    int32_t iMixerStreamIndex;
    ...
    for (iMixerStreamIndex = 0; iMixerStreamIndex < VOIP_MAXLOCALUSERS; iMixerStreamIndex += 1)
    {
        if (pHeadset->aLocalMixerStreams[iMixerStreamIndex].pSinkStream == pSinkStream)
        {
            pHeadset->aLocalMixerStreams[iMixerStreamIndex].bNeedDestroy = FALSE;
...
static void _VoipHeadsetHandlePreEncodeAudioStreamCreated(VoipHeadsetRefT *pHeadset, pre_encode_audio_stream *pAudioStream)
{
    ...
    {
        NetPrintf(("voipheadsetxbox: no space to add pre-encoded audio streams\n"));
        return;
    }
    ...
    pCurrentStream->bNeedDestroy = TRUE;
}
...
static void _VoipHeadsetHandlePreEncodeAudioStreamClosed(VoipHeadsetRefT *pHeadset, pre_encode_audio_stream *pAudioStream)
{
    ...
    for (iPreEncodedStreamIndex = 0; iPreEncodedStreamIndex < VOIP_MAXLOCALUSERS_EXTENDED; iPreEncodedStreamIndex += 1)
    {
        if (pAudioStream == pHeadset->aPreEncodedStreams[iPreEncodedStreamIndex].pAudioStream)
        {
            ...
            if (pHeadset->aPreEncodedStreams[iPreEncodedStreamIndex].pCurrentAudioBuffer != NULL)
            {
                pAudioStream->return_buffer(pHeadset->aPreEncodedStreams[iPreEncodedStreamIndex].pCurrentAudioBuffer);
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetHandlePreEncodeAudioStreamDestroy

    \Description
        Process stream notification game_chat_stream_state_change_type::pre_encode_audio_stream_destroy
...
*/
/********************************************************************************F*/
static void _VoipHeadsetHandlePreEncodeAudioStreamDestroy(VoipHeadsetRefT *pHeadset, pre_encode_audio_stream *pAudioStream)
{
    int32_t iPreEncodedStreamIndex = 0;
    ...
    for (iPreEncodedStreamIndex = 0; iPreEncodedStreamIndex < VOIP_MAXLOCALUSERS_EXTENDED; iPreEncodedStreamIndex += 1)
    {
        if (pAudioStream == pHeadset->aPreEncodedStreams[iPreEncodedStreamIndex].pAudioStream)
        {
            pHeadset->aPreEncodedStreams[iPreEncodedStreamIndex].bNeedDestroy = FALSE;
...
static uint8_t _VoipHeadsetGetUserIndexofAudioStream(VoipHeadsetRefT *pHeadset, pre_encode_audio_stream *pAudioStream, int32_t *pLocalUserIndex)
{
    ...
    *pLocalUserIndex = -1;
    ...
    {
        NetPrintf(("voipheadsetxbox: stream user index not found.\n"));
        return(FALSE);
    }
...
static uint32_t _VoipHeadsetConfigureOpus(XONELocalPreEncodedStreamT *pStream)
{
    ...
    if (VoipCodecControl('opus', 'insr', pStream->pAudioStream->get_pre_processed_format().sample_rate, 0, NULL) == 0)
    {
        ...
        else if (pStream->pAudioStream->get_pre_processed_format().bits_per_sample == 32)
        {
            ...
            {
                NetPrintf(("voipheadsetxbox: unsupported sample_type from pre-encoded stream.\n"));
                iRet = -1;
            }
        }
        else
        {
            NetPrintf(("voipheadsetxbox: unsupported bits_per_sample(%d) from pre-encoded stream.\n", pStream->pAudioStream->get_pre_processed_format().bits_per_sample));
            iRet = -2;
        }
    }
    else
    {
        NetPrintf(("voipheadsetxbox: cannot send mic packet because 'insr' failed.\n"));
        iRet = -3;
    }
    return(iRet);
}
    ...
    for (uStateChangeIndex = 0; uStateChangeIndex < uStateChangeCount; uStateChangeIndex += 1)
    {
        switch (aGameChatStateChanges[uStateChangeIndex]->state_change_type)
        {
            ...
            {   
                ...
                #endif
                NetPrintf(("voipheadsetxbox: ignoring player-to-player text communication message from user with xuid %S\n", pStateChange->sender->xbox_user_id()));
                break;
            }
            ...
            case game_chat_state_change_type::transcribed_chat_received:
            {
                ...
                VoipUserT VoipUser;
                char strTranscribedTextUtf8[5 + (c_maxChatTextMessageLength) * sizeof(wchar_t) + 1];
                int32_t iMbStrLenNeeded;

                // for xbox gc2 native STT only support english, so we can hardcode the language tag
                strncpy(strTranscribedTextUtf8, "en-US", sizeof(strTranscribedTextUtf8));

                iMbStrLenNeeded = WideCharToMultiByte(CP_UTF8, 0, pStateChange->message, -1, NULL, 0, NULL, 0);
                if (iMbStrLenNeeded <= sizeof(strTranscribedTextUtf8) - 5)
                {
                    int32_t iUserIndex;
                    WideCharToMultiByte(CP_UTF8, 0, pStateChange->message, -1, strTranscribedTextUtf8 + 5, sizeof(strTranscribedTextUtf8) - 5, NULL, 0);
                    ...
                    for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS; iUserIndex++)
                    {
                        ...
                        if (NetConnStatus('peid', iUserIndex, &iLocalPersonaId, sizeof(iLocalPersonaId)) == 0)
                        {
                            if (iLocalPersonaId == VoipUser.AccountInfo.iPersonaId)
                            {
                                ...
                                pHeadset->aSTTMetrics[iUserIndex].uCharCountRecv += (uint32_t)strlen(strTranscribedTextUtf8) - 5;
                                break;
                            }
                        }
                    }
                ...
                {
                    NetPrintf(("voipheadsetxbox: transcribed text unexpectedly does not fit in statically allocated buffer during wcstring mbstring converstion\n"));
                }
            ...
            case game_chat_state_change_type::text_conversion_preference_changed:
            {
                ...
                #endif
                NetPrintf(("voipheadsetxbox: game chat 2 notifies a 'text conversion preference' change for user with xuid %S --> STT %s TTS %s\n",
            ...
            case game_chat_state_change_type::communication_relationship_adjuster_changed:
            {
                ...
                NetPrintf(("voipheadsetxbox: 'comm rel adjuster' event for localXUID %S target XUID %S --> RCV_AUD:%s, RECV_TXT:%s, SND_MIC_AUD:%s, SEND_TEXT:%s, SEND_TTS_AUD:%s | commRelAdj = %s\n",
...
static void _VoipHeadsetProcessStreamStateChanges(VoipHeadsetRefT *pHeadset)
{
    ...
    for (uStreamStateChangeIndex = 0; uStreamStateChangeIndex < uStreamStateChangeCount; uStreamStateChangeIndex += 1)
    {
        switch (aStreamStateChanges[uStreamStateChangeIndex]->state_change_type)
        {
            ...
            {
                post_decode_audio_sink_stream *pSinkStream = aStreamStateChanges[uStreamStateChangeIndex]->post_decode_audio_sink_stream;
                _VoipHeadsetHandlePostDecodeSinkStreamDestroy(pHeadset, pSinkStream);
                break;
            }
            ...
            case game_chat_stream_state_change_type::pre_encode_audio_stream_destroyed:
            {
                pre_encode_audio_stream *pAudioStream = aStreamStateChanges[uStreamStateChangeIndex]->pre_encode_audio_stream;
                _VoipHeadsetHandlePreEncodeAudioStreamDestroy(pHeadset, pAudioStream);
                break;
            }
...
static void _VoipHeadsetApplyEffectiveRelationship(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, chat_user *pRemoteChatUser)
{
    ...
    NetPrintf(("voipheadsetxbox: applying new comm rel for localXUID %S target XUID %S --> RCV_AUD:%s, RECV_TXT:%s, SND_MIC_AUD:%s, SEND_TEXT:%s, SEND_TTS_AUD:%s\n",
...
static int32_t _VoipHeadsetSetRemoteUserVoicePlayback(VoipHeadsetRefT *pHeadset, VoipUserT *pRemoteUser, int32_t iLocalUserIndex, uint8_t bEnablePlayback)
{
    ...
    if (pLocalChatUser != NULL)
    {
        ...
        if (iRemoteUserIndex < 0)
        {
            NetPrintf(("voipheadsetxbox: failed to change voice playback for remote user can't be found (pLocalChatUser = %p, pRemoteChatUser = %p)\n",
                pLocalChatUser, pRemoteChatUser));
            return(-1);
        }

        uOldPlaybackFlags = pHeadset->aLocalUsers[iLocalUserIndex].uPlaybackFlags;
        ...
        NetPrintf(("voipheadsetxbox: %s applied for luser(%d):%llu & ruser(%d):%llu | 0x%08x -> 0x%08x\n",
            bEnablePlayback ? "+pbk" : "-pbk", iLocalUserIndex, pHeadset->aLocalUsers[iLocalUserIndex].User.AccountInfo.iPersonaId, iRemoteUserIndex, pRemoteUser->AccountInfo.iPersonaId,
            uOldPlaybackFlags, pHeadset->aLocalUsers[iLocalUserIndex].uPlaybackFlags));
    }
    else
    {
        NetPrintf(("voipheadsetxbox: failed to change voice playback for local user index %d because chat users can't be found (pLocalChatUser = %p, pRemoteChatUser = %p)\n",
            pLocalChatUser, pRemoteChatUser));
        return(-2);
    }
...
static int32_t _VoipHeadsetSetRemoteUserOutboundMute(VoipHeadsetRefT *pHeadset, VoipUserT *pRemoteUser, int32_t iLocalUserIndex, uint8_t bSend)
{
    ...
    if ((pLocalChatUser != NULL) && (pRemoteChatUser != NULL))
    {
        ...
        {
            NetPrintf(("voipheadsetxbox: failed to change outbound mute for local user index %d because remote user can’t be found\n", iLocalUserIndex));
            return(-1);
        }
    }
    else
    {
        NetPrintf(("voipheadsetxbox: failed to change outbound mute for local user index %d because chat users can't be found (pLocalChatUser = %p, pRemoteChatUser = %p)\n",
...
static int32_t _VoipHeadsetSetRemoteUserInboundMute(VoipHeadsetRefT *pHeadset, VoipUserT *pRemoteUser, int32_t iLocalUserIndex, uint8_t bRecv)
{
    ...
    if ((pLocalChatUser != NULL) && (pRemoteChatUser != NULL))
    {
        ...
        {
            NetPrintf(("voipheadsetxbox: failed to change inbound mute for local user index %d because remote user can’t be found\n", iLocalUserIndex));
            return(-1);
        }
    }
    else
    {
        NetPrintf(("voipheadsetxbox: failed to change inbound mute for local user index %d because chat users can't be found (pLocalChatUser = %p, pRemoteChatUser = %p)\n",
            iLocalUserIndex, pLocalChatUser, pRemoteChatUser));
        return(-2);
    }
...
static void _VoipHeadsetProcessTranscribe(VoipHeadsetRefT *pHeadset)
{
    ...
    for (iPreEncodedStreamIndex = 0; iPreEncodedStreamIndex < VOIP_MAXLOCALUSERS; iPreEncodedStreamIndex += 1)
    {
        ...
        if (bPreEncodeParticipating && pHeadset->bVoiceTranscriptionEnabled && (pHeadset->aPreEncodedStreams[iPreEncodedStreamIndex].pTranscribeRef != NULL))
        {
            VoipTranscribeUpdate(pHeadset->aPreEncodedStreams[iPreEncodedStreamIndex].pTranscribeRef);

            // get transcription (leave room for language code); if a transcription is available, send it along
            if (VoipTranscribeGet(pHeadset->aPreEncodedStreams[iPreEncodedStreamIndex].pTranscribeRef, strTranscribeBuf+5, sizeof(strTranscribeBuf)-5) > 0)
            {
                // copy in language prefix
                memcpy(strTranscribeBuf, pHeadset->strLang, 5);
                // forward to transcription callack
...
static void * _VoipHeadsetAllocateMemory(size_t Size, uint32_t uMemoryTypeId)
{
    ...
    {
        NetPrintf(("voipheadsetxbox: failed to allocate a buffer of size %s with gamechat2 memory type %s\n", Size, uMemoryTypeId));
    }
...
static void _VoipHeadsetInitializeGameChat2(VoipHeadsetRefT *pHeadset, int8_t bCrossplay)
{
    ...
    pHeadset->bCrossplayModeChange = FALSE;
    pHeadset->bGC2Initialized = TRUE;
}
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetRemoveAllUsersFromGC2

    \Description
        Remove all users from gamechat 2

    \Input *pHeadset        - pointer to headset state

    \Version 17/08/2020 (tcho)
*/
/********************************************************************************F*/
static void _VoipHeadsetRemoveAllUsersFromGC2(VoipHeadsetRefT *pHeadset)
{
    ...
    for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS; iUserIndex += 1)
    {
        if (!VOIP_NullUser(&pHeadset->aLocalUsers[iUserIndex].User) && (pHeadset->aLocalUsers[iUserIndex].pChatUser != NULL))
        {
            chat_manager::singleton_instance().remove_user(pHeadset->aLocalUsers[iUserIndex].pChatUser);
            pHeadset->aLocalUsers[iUserIndex].pChatUser = NULL;
            pHeadset->aLocalUsers[iUserIndex].bAddedToGC2 = FALSE;
        }
    }
    ...
    for (iUserIndex = 0; iUserIndex < pHeadset->iMaxRemoteUsers; iUserIndex += 1)
    {
        ...
        if (!VOIP_NullUser(&pRemoteUser->User) && (pRemoteUser->User.ePlatform == VOIP_PLATFORM_XBOXONE) && (pRemoteUser->pChatUser != NULL))
        {
            chat_manager::singleton_instance().remove_user(pRemoteUser->pChatUser);
            pRemoteUser->pChatUser = NULL;
            pRemoteUser->bAddedToGC2 = FALSE;
        }
    }
}
...
*/
/********************************************************************************F*/
static void _VoipHeadsetToggleCrossplayMode(VoipHeadsetRefT *pHeadset, uint8_t bCrossplay)
{
    // cleanup game chat 2 prepare for mode change
    pHeadset->bGC2Cleanup = TRUE;
    pHeadset->bCrossplayModeChange = TRUE;
...
static int32_t _VoipHeadsetNarrateCrossplayCb(VoipHeadsetRefT *pHeadset, int32_t iUserIndex, const int16_t *pSamples, int32_t iSize)
{
    ...
    else
    {
        ...
        if (iSize == VOIPHEADSET_FRAMESIZE)
        {
            ...
            {
                NetPrintf(("voipheadsetxbox: cannot send narrate mic packet because 'insr' failed .\n"));
            }
...
/*!
    ...
    \Input uFlags       - VOIPNARRATE_CBFLAG_*
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetNarrateCb(VoipNarrateRefT *pNarrateRef, int32_t iUserIndex, const int16_t *pSamples, int32_t iSize, uint32_t uFlags, void *pUserData)
{
    VoipHeadsetRefT *pHeadset = (VoipHeadsetRefT *)pUserData;
    int32_t iRet = 0;
    uint8_t bLocalNarrate = (uFlags & VOIPNARRATE_CBFLAG_LOCAL) ? TRUE : FALSE;
...
static int32_t _VoipHeadsetCreateNarrate(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex)
{
    ...
    #endif
    {
        NetPrintf(("voipheadsetxbox: no valid user at index %d to create narration for\n", iLocalUserIndex));
        return(-1);
    }
    ...
    if ((pLocalUser->pNarrateRef = VoipNarrateCreate(_VoipHeadsetNarrateCb, pHeadset)) != NULL)
    {
        ...
        #endif
        VoipNarrateControl(pLocalUser->pNarrateRef, 'lang', 0, 0, pHeadset->strLang);
    }
    else
    {
        NetPrintf(("voipheadsetxbox: failed to start narration module\n"));
        return(-1);
    }
    ...
    #endif
    {
        NetPrintf(("voipheadsetxbox: failed to create xaudio2 (%s)", DirtyErrGetName(hResult)));
        return(-1);
    }
    ...
    {
        NetPrintf(("voipheadsetxbox: failed to get user's local id (%s)\n", DirtyErrGetName(hResult)));
        pLocalUser->pXaudio2 = nullptr;
        return(-1);
    }
    ...
    {
        NetPrintf(("voipheadsetxbox: failed to get default audio endpoint (%s)\n", DirtyErrGetName(hResult)));
        pLocalUser->pXaudio2 = nullptr;
        return(-1);
    }
    #endif
    ...
    {
        pLocalUser->pXaudio2 = nullptr;
        NetPrintf(("voipheadsetxbox: failed to create mastering voice (%s)\n", DirtyErrGetName(hResult)));
        return(-1);
    }
    ...
    if (FAILED(hResult = pLocalUser->pXaudio2->CreateSourceVoice(&pLocalUser->pSourceVoice, &WaveFormat, 0, XAUDIO2_DEFAULT_FREQ_RATIO)))
    {
        ...
        pLocalUser->pMasteringVoice = NULL;
        pLocalUser->pXaudio2 = nullptr;
        NetPrintf(("voipheadsetxbox: failed to create source voice (%s)\n", DirtyErrGetName(hResult)));
        return(-1);
    }
    return(0);
}
...
static int32_t _VoipHeadsetSynthesizeSpeechLoopback(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, const char *pText)
{
    ...
    if (_VoipHeadsetCreateNarrate(pHeadset, iLocalUserIndex) == 0)
    {
        return(VoipNarrateInput(pLocalUser->pNarrateRef, iLocalUserIndex, pLocalUser->eGender, VOIPNARRATE_CBFLAG_LOCAL, pText));
    }
    else
    {
        NetPrintf(("voipheadsetxbox: failed to start narration module\n"));
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetExecuteCrossplayModeChange

    \Description
        Execute crossplay mode change

    \Input *pHeadset        - headset module

    \Version 08/15/2020 (tcho)
*/
/********************************************************************************F*/
static void _VoipHeadsetExecuteCrossplayModeChange(VoipHeadsetRefT *pHeadset)
{
    int32_t  iUserIndex = 0;
    ...
    _VoipHeadsetInitializeGameChat2(pHeadset, pHeadset->bCrossplay);
    ...
    for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS; iUserIndex += 1)
    {
        if (!VOIP_NullUser(&aLocalVoipUsers[iUserIndex]))
        {
            ...
            NetPrintf(("voipheadsetxbox: re-adding local talker %lld\n", aLocalVoipUsers[iUserIndex].AccountInfo.iPersonaId));
        }
    }
...
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetAppStateChangeCb
    ...
    \Version 08/20/2020 (tcho)
*/
/********************************************************************************F*/
static void _VoipHeadsetAppStateChangeCb(BOOLEAN bQuiesced, void *pContext)
{
    VoipHeadsetRefT *pHeadset = (VoipHeadsetRefT *)pContext;

    if (bQuiesced == TRUE)
    {
        NetPrintf(("voipheadsetxbox: SuspendingEvent triggered\n"));
        
        pHeadset->bGC2Cleanup = TRUE;
        while (pHeadset->bGC2Initialized == TRUE)
        {
            NetConnSleep(20);
        }

        NetPrintf(("voipheadsetxbox: SuspendingEvent gc2 cleanup complete\n"));
    }
    else
    {
        NetPrintf(("voipheadsetxbox: ResumingEvent triggered\n"));
        _VoipHeadsetExecuteCrossplayModeChange(pHeadset);
    }
}
/*F********************************************************************************/
/*!
    \Function _VoipHeadsetSubscribeAppStateCallbacks

    \Description
        Subscribe to suspend and resume callbacks 

    \Input *pHeadset        - headset module
    \Input bSubscribe       - TRUE for subscribe, FALSE for unsubscribe
...
*/
/********************************************************************************F*/
static int32_t _VoipHeadsetSubscribeAppStateCallbacks(VoipHeadsetRefT *pHeadset, uint8_t bSubscribe)
{
    #if DIRTYCODE_GDK
    if (bSubscribe == TRUE)
    {
        ...
        if ((hResult = HRESULT_FROM_WIN32(RegisterAppStateChangeNotification(_VoipHeadsetAppStateChangeCb, pHeadset, &pHeadset->pAppStateReg))) != S_OK)
        {
            NetPrintf(("voipheadsetxbox: unable to install suspend/resume event handler (err=%s)\n", DirtyErrGetName(hResult)));
            return(-1);
        }
    }
    else
    {
        ...
        if (pHeadset->pAppStateReg != NULL)
        {
            UnregisterAppStateChangeNotification(pHeadset->pAppStateReg);
        }
    }
    #else
    if (bSubscribe == TRUE)
    {
        if (!pHeadset->SuspendingEventToken.Value)
        {
            NetPrintf(("voipheadsetxbox: adding suspending event handler \n"));
            pHeadset->SuspendingEventToken = (CoreApplication::Suspending += ref new EventHandler<Windows::ApplicationModel::SuspendingEventArgs ^>(
                [=](Platform::Object ^refObj, Platform::Object ^refObj2)
            {
                _VoipHeadsetAppStateChangeCb(TRUE, pHeadset);
            }));
        }

        // when you resume users are logged in fresh
        if (!pHeadset->ResumingEventToken.Value)
        {
            NetPrintf(("voipheadsetxbox: adding resuming event handler \n"));

            pHeadset->ResumingEventToken = (CoreApplication::Resuming += ref new EventHandler<Platform::Object^>(
                [=](Platform::Object ^refObj, Platform::Object ^refObj2)
            {
                _VoipHeadsetAppStateChangeCb(FALSE, pHeadset);
            }));
        }
    }
    else
    {
        if (pHeadset->SuspendingEventToken.Value)
        {
            CoreApplication::Suspending -= pHeadset->SuspendingEventToken;
            pHeadset->SuspendingEventToken.Value = 0;
        }

        if (pHeadset->ResumingEventToken.Value)
        {
            CoreApplication::Resuming -= pHeadset->ResumingEventToken;
            pHeadset->ResumingEventToken.Value = 0;
        }
    }
...
VoipHeadsetRefT *VoipHeadsetCreate(int32_t iMaxChannels, VoipHeadsetMicDataCbT *pMicDataCb, VoipHeadsetTextDataCbT *pTextDataCb, VoipHeadsetOpaqueDataCbT *pOpaqueDataCb, VoipHeadsetStatusCbT *pStatusCb, void *pCbUserData, int32_t iData)
{
    ...
    {
        NetPrintf(("voipheadsetxbox: request for %d channels exceeds max\n", iMaxChannels));
        return(NULL);
    }
    ...
    {
        NetPrintf(("voipheadsetxbox: not enough memory to allocate module state\n"));
        return(NULL);
    }
    ...
    // set default language for transcription/narration
    ds_strnzcpy(pHeadset->strLang, "en-US", sizeof(pHeadset->strLang));

    if (_VoipHeadsetSubscribeAppStateCallbacks(pHeadset, TRUE) < 0)
    {
        NetPrintf(("voipheadsetxbox: cannot subscribe to app state callback\n"));
    ...
    {
        NetPrintf(("voipheadsetxbox: cannot create voip conduit\n"));
        VoipHeadsetDestroy(pHeadset);
        return(NULL);
    }
...
*/
/********************************************************************************F*/
int32_t VoipHeadsetDestroy(VoipHeadsetRefT *pHeadset)
{  
    if (pHeadset->bGC2Initialized == TRUE)
    {
        pHeadset->bGC2Cleanup = TRUE;
        return(VOIPHEADSET_DESTROY_RETRY);
    }
    
    _VoipHeadsetSubscribeAppStateCallbacks(pHeadset, FALSE);
    ...
    NetPrintf(("voipheadsetxbox: [%p] destroy complete\n", pHeadset));
...
void VoipHeadsetReceiveVoiceDataCb(VoipUserT *pRemoteUsers, int32_t iRemoteUserSize, int32_t iConsoleId, VoipMicrInfoT *pMicrInfo, uint8_t *pPacketData, void *pUserData)
{
    ...
    for (uMicrPkt = 0; uMicrPkt < pMicrInfo->uNumSubPackets; uMicrPkt += 1)
    {
        ...
        {
            VoipConduitReceiveVoiceData(pHeadset->pConduitRef, &pRemoteUsers[uRemoteUserIndex], pRead, uSubPktSize, TRUE);        
            pRead += uSubPktSize;
        }
        // each sub-packet is an unreliabe gamechat2 data frame to be delivered to the local instance of gamechat2
        else
        {
            NetPrintfVerbose((pHeadset->iDebugLevel, 2, "voipheadsetxbox: chat_manager::process_incoming_data(%d) - unreliable data\n", uSubPktSize));
...
void VoipHeadsetReceiveOpaqueDataCb(int32_t iConsoleId, const uint8_t *pOpaqueData, int32_t iOpaqueDataSize, void *pUserData)
{
    ...
    NetPrintfVerbose((pHeadset->iDebugLevel, 2, "voipheadsetxbox: chat_manager::process_incoming_data(%d) - reliable data\n", iOpaqueDataSize));
    #endif
...
void VoipHeadsetProcess(VoipHeadsetRefT *pHeadset, uint32_t uFrameCount)
{
    ...
    // cleanup and re-initalize gamechat 2 if there is a mode change
    if (pHeadset->bGC2Cleanup == TRUE)
    {
        uint32_t uStreamIndex;

        // remove all user from game chat 2
        _VoipHeadsetRemoveAllUsersFromGC2(pHeadset);

        // check if we need to wait for streams to be destroyed
        for (uStreamIndex = 0; uStreamIndex < VOIP_MAXLOCALUSERS_EXTENDED; ++uStreamIndex)
        {
            if ((pHeadset->aPreEncodedStreams[uStreamIndex].bNeedDestroy == TRUE) || (pHeadset->aLocalMixerStreams[uStreamIndex].bNeedDestroy == TRUE))
            {
                // we can't perform the clean up
                break;
            }
        }

        if (uStreamIndex == VOIP_MAXLOCALUSERS_EXTENDED)
        {
            // can perform the cleanup
            chat_manager::singleton_instance().cleanup();
            pHeadset->bGC2Initialized = FALSE;
            pHeadset->bGC2Cleanup = FALSE;

            if (pHeadset->bCrossplayModeChange == TRUE)
            {
                _VoipHeadsetExecuteCrossplayModeChange(pHeadset);
            }
        }
    ...
    // if gamechat 2 is initialized traverse through the users and add them to game chat 2 if needed
    if (pHeadset->bGC2Initialized == TRUE)
    {
        if (pHeadset->bGC2Cleanup == FALSE)
        {
            int32_t iIndex;
            std::wstring wstrXuid;

            // loop through the local user array
            for (iIndex = 0; iIndex < VOIP_MAXLOCALUSERS; ++iIndex)
            {
                if (!VOIP_NullUser(&pHeadset->aLocalUsers[iIndex].User) && (pHeadset->aLocalUsers[iIndex].bAddedToGC2 == FALSE))
                {
                    wstrXuid = std::to_wstring(pHeadset->aLocalUsers[iIndex].uXuid);
                    pHeadset->aLocalUsers[iIndex].pChatUser = chat_manager::singleton_instance().add_local_user(wstrXuid.c_str());
                    pHeadset->aLocalUsers[iIndex].bAddedToGC2 = TRUE;

                    if (pHeadset->aLocalUsers[iIndex].pChatUser != NULL)
                    {
                        NetPrintf(("voipheadsetxbox: added a local user (xuid: %S) to gamechat 2\n", pHeadset->aLocalUsers[iIndex].pChatUser->xbox_user_id()));
                    }
                }
            }

            // loop through the remote user array
            for (iIndex = 0; iIndex < pHeadset->iMaxRemoteUsers; ++iIndex)
            {
                if (!VOIP_NullUser(&pHeadset->aRemoteUsers[iIndex].User) && (pHeadset->aRemoteUsers[iIndex].bAddedToGC2 == FALSE) && (pHeadset->aRemoteUsers[iIndex].User.ePlatform == VOIP_PLATFORM_XBOXONE))
                {
                    XONERemoteVoipUserT *pXoneRemoteUser = &pHeadset->aRemoteUsers[iIndex];

                    wstrXuid = std::to_wstring(pHeadset->aRemoteUsers[iIndex].uXuid);
                    pXoneRemoteUser->pChatUser = chat_manager::singleton_instance().add_remote_user(wstrXuid.c_str(), pHeadset->aRemoteUsers[iIndex].uConsoleId);
                    pXoneRemoteUser->bAddedToGC2 = TRUE;
                    ...
                    {
                        NetPrintf(("voipheadsetxbox: adding remote talker %lld failed because it chat_manager::add_remote_user() returned NULL\n", pXoneRemoteUser->User.AccountInfo.iPersonaId));
                        ds_memclr(pXoneRemoteUser, sizeof(XONERemoteVoipUserT));
                    }
                }
            }
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'lang' - set langauge for language services (narration/transcription/translation) - default en-US
            ...
            'txtn' - locally play back text in pValue 
...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (pHeadset->bGC2Initialized != TRUE)
    {
        NetPrintf(("voipheadsetxbox: gamechat 2 not initialized\n"));
        return(-1);
    }
    ...
    if ((iControl == '-pbk') || (iControl == '+pbk'))
    {
        ...
        {
            NetPrintf(("voipheadsetxbox: VoipHeadsetControl('%C', %d) invalid arguments\n", iControl, iValue));
            iRetCode = -2;
        }
        return(iRetCode);
    }
    ...
    if (iControl == 'loop')
    {
        ...
        */
        ...
        NetPrintf(("voipheadsetxboxone: loopback mode no longer supported on xboxone - can't do it with MS game chat 2\n"));
        return(-1);
    }
    if ((iControl == 'lang') && (pValue != NULL))
    {
        ds_strnzcpy(pHeadset->strLang, (const char *)pValue, sizeof(pHeadset->strLang));
        NetPrintf(("voipheadsetps: setting language to %s\n", pHeadset->strLang));
        return(0);
    }
    if (iControl == 'qual')
    {
    ...
    if (iControl == 'spam')
    {
        NetPrintf(("voipheadsetxbox: setting debuglevel=%d\n", iValue));
    ...
    {
        if (pHeadset->aLocalUsers[iValue].bParticipating)
        {
            return(_VoipHeadsetSynthesizeSpeech(pHeadset, iValue, (char *)pValue));
        }

        // not a praticipating user
        return(-1);
    }
    if (iControl == 'txtn')
    {
    ...
    if (iControl == 'xply')
    {
        ...
        {
            NetPrintf(("voipheadsetxbox: changing crossplay mode to: %s\n", bCrossplay ? "crossplay" : "native"));
            _VoipHeadsetToggleCrossplayMode(pHeadset, bCrossplay);
        }
...
/*!
    ...
    \Notes
        ...
        \verbatim
            'fpcl' - return the value of the first party chat indicator for given local user
            'fpcr' - return the value of the first party chat indicator for given remote user
...
int32_t VoipHeadsetStatus(VoipHeadsetRefT *pHeadset, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    ...
    if (iSelect == 'fpcl')
    {
        return(_VoipHeadsetFirstPartyChatIndicator(pHeadset, TRUE, iValue));
    }
    if (iSelect == 'fpcr')
    {
        return(_VoipHeadsetFirstPartyChatIndicator(pHeadset, FALSE, _VoipHeadsetGetRemoteUserIndex(pHeadset, (VoipUserT *)pBuf)));
    }
    if (iSelect == 'fpml')
    {
    ...
    if (iSelect == 'ttss')
    {
        if ((iValue < 0) || (iValue >= VOIP_MAXLOCALUSERS))
        {
            NetPrintf(("voipheadsetxbox: VoipHeadsetStatus('ttss') invalid agruments.\n"));
            return(FALSE);
        }
        ...
        NetPrintf(("voipheadsetxbox: VoipHeadsetStatus('ttss') did  not find a valid local user.\n"));
        return(FALSE);
    }
    if (iSelect == 'vlen')
    {
