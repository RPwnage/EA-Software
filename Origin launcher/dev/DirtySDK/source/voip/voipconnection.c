...
/*!
    ...
    \Input *pConnectionlist     - connection list
    \Input SendAddrCurrent      - address of the server we are currently sending to
    \Input uCurrentSessionId    - session id of the group we are sending to
...
*/
/********************************************************************************F*/
static uint32_t _VoipPrepareVoipServerSendMask(VoipConnectionlistT *pConnectionlist, struct sockaddr_in SendAddrCurrent, uint32_t uCurrentSessionId)
{
    int32_t iLocalConnId, iVoipServerConnId;
    uint32_t uLocalSendMask = ((VoipCommonRefT *)VoipGetRef())->uConnSendMask;
    uint32_t uVoipServerSendMask = 0;

    for (iLocalConnId = 0; iLocalConnId < pConnectionlist->iMaxConnections; iLocalConnId++)
    {
        iVoipServerConnId = pConnectionlist->pConnections[iLocalConnId].aSessionInfo[0].iVoipServerConnId;
        ...
        if (iVoipServerConnId != VOIP_CONNID_NONE)
        {
            ...
            {
                // check that the connection is on the server we are sending to and in correct game
                if ((memcmp(&pConnectionlist->pConnections[iLocalConnId].SendAddr, &SendAddrCurrent, sizeof(SendAddrCurrent)) == 0) 
                    && (pConnectionlist->pConnections[iLocalConnId].aSessionInfo[0].uSessionId == uCurrentSessionId))
                {
...
/*F********************************************************************************/
/*!
    \Function _VoipConnectionMatchSession

    \Description
        Check whether specified session ID matches the voip connection.
...
*/
/********************************************************************************F*/
static uint8_t _VoipConnectionMatchSession(VoipConnectionlistT *pConnectionlist, int32_t iConnId, uint32_t uSessionId)
{
    int32_t iSessionIndex;
    int32_t bMatch = FALSE;
    VoipConnectionT *pConnection = &pConnectionlist->pConnections[iConnId];

    for (iSessionIndex = 0; iSessionIndex < VOIP_MAXSESSIONINFOS; iSessionIndex++)
    {
        if (pConnection->aSessionInfo[iSessionIndex].uSessionId == uSessionId)
        {
            bMatch = TRUE;
        }
    }
...
/*F********************************************************************************/
/*!
    \Function   _VoipConnectionPrintAllSessionInfo

    \Description
        Prints all session info (IDs and iVoipServerConnIds) sharing this voip connection
...
*/
/********************************************************************************F*/
static void _VoipConnectionPrintAllSessionInfo(VoipConnectionlistT *pConnectionlist, int32_t iConnId)
{
    int32_t iSessionIndex;
    VoipConnectionT *pConnection = &pConnectionlist->pConnections[iConnId];
    ...
    if (pConnection->aSessionInfo[0].uSessionId != 0)
    {
        NetPrintf(("voipconnection: [%d] [session id : conn index : owner ref] of sessions sharing this voip connection are:\n", iConnId));
        for (iSessionIndex = 0; iSessionIndex < VOIP_MAXSESSIONINFOS; iSessionIndex++)
        {
            if (pConnection->aSessionInfo[iSessionIndex].uSessionId != 0)
            {
                NetPrintf(("[0x%08x : %d : %p]\n", 
                    pConnection->aSessionInfo[iSessionIndex].uSessionId, 
                    pConnection->aSessionInfo[iSessionIndex].iVoipServerConnId, 
                    pConnection->aSessionInfo[iSessionIndex].pOwnerRef));
            }
            else
            {
                break;
    ...
    {
        NetPrintf(("voipconnection: [%d] there is no session info associated with this connection\n", iConnId));
    }
    ...
    while (iConnectionIndex < pConnectionlist->iMaxConnections)
    {
        ...
        if (pConnection->eState == ST_ACTV || pConnection->eState == ST_CONN)
        {
            ...
            if (pNewEntry)
            {
                ...
                */
                if (pConnection->aSessionInfo[0].iVoipServerConnId != VOIP_CONNID_NONE)
                {
                    #if VOIP_RELIABLE_DEBUG
...
static void _VoipReliableDataInboundUserAddProcess(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, ReliableDataT *pReliableData)
{
    ...
    if (VOIP_NullUser((VoipUserT *)&pConnection->RemoteUsers[uUserIndex]))
    {
        ...
        ((VoipCommonRefT *)VoipGetRef())->bApplyMute = TRUE;
...
static void _VoipConnectionConn(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, uint32_t uTick)
{
    ...
    NetPrintf(("voipconnection: [%d] (sess id 0x%08x) sending connect packet to remote console (%a:%d)\n",
        VOIP_ConnID(pConnectionlist, pConnection), pConnection->aSessionInfo[0].uSessionId,
    ...
    _VoipEncodeU32(ConnPacket.Head.aSessionId, pConnection->aSessionInfo[0].uSessionId);
...
static void _VoipConnectionInit(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, int32_t iConnID)
{
    ...
    VoipCommonApplyMute((VoipCommonRefT *)VoipGetRef());
...
static void _VoipConnectionPing(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, uint32_t uTick)
{
    ...
    _VoipEncodeU32(PingPacket.Head.aSessionId, pConnection->aSessionInfo[0].uSessionId);
    if (_VoipConnectionIsTranscribedTextRequested(pConnectionlist))
    {
...
static void _VoipConnectionSendSingle(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, int32_t iUserIndex, uint32_t uTick, uint32_t bFlush)
{
    ...
    if (bSendPacket == TRUE)
    {
        ...
        // set the session id
        _VoipEncodeU32(pPacket->Head.aSessionId, pConnection->aSessionInfo[0].uSessionId);
        ...
        if (pConnection->aSessionInfo[0].iVoipServerConnId != VOIP_CONNID_NONE)
        {
            uVoipServerReadySendMask = _VoipPrepareVoipServerSendMask(pConnectionlist, pConnection->SendAddr, pConnection->aSessionInfo[0].uSessionId);
        }
        ...
        if ((pConnection->uSendSeqn % 30) == 0)
        {
            ...
            NetPrintf(("voipconnection: [%d] (sess id 0x%08x) local usr %d sending pkt (seq#:%d) with %d sub-pkt(s) of %s to %a:%d | voip channels:0x%08x | sendmask: 0x%08x\n",
                VOIP_ConnID(pConnectionlist, pConnection), pConnection->aSessionInfo[0].uSessionId, iUserIndex, pPacket->MicrInfo.uSeqn, pPacket->MicrInfo.uNumSubPackets,
                ...
                pConnectionlist->aChannels[iUserIndex], uVoipServerReadySendMask));
        }
        #endif
...
static void _VoipConnectionRecvSingle(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, struct sockaddr_in *pSendAddr, VoipPacketBufferT *pPacket, int32_t iSize)
{
    ...
    /* Ignore packet if session id does not match with any known connections

       This check is improving robustness when voipgroup connection sharing exercises either the supension of an existing
       connection in favor of a new server-hosted connection, or the resume of a 'suspended' connection after an existing
       connection is abandoned.

       In both cases, the exercised mechanism results in a low-level voip connection to remote player X being initiated 
       immediately after a previous low-level voip connection to the same remote player was abandoned. Experimentation has
       shown that the transition is happening fast enough to create a race condition where few inbound voip packets
       from the previous connection arrive while the local end has already initiated the handshaking for the new
       connection... thus resulting in that handshaking misbehaving because inbound packets associated with old connection
       are incorrectly assumed to be contributing to new connection.
    */
    if (_VoipConnectionMatchSession(pConnectionlist, iConnID, uSessionId) == FALSE)
    {
        #if DIRTYCODE_LOGGING
        ...
        _VoipConnectionPrintAllSessionInfo(pConnectionlist, iConnID);
    ...
    switch (uPktType)
    {
        ...
        case 'M':
        {
            ...
            if ((pConnection->iRecvPackets % 30) == 0)
            {
                ...
                NetPrintf(("voipconnection: [%d] (sess id 0x%08x) received pkt (seq#:%d) with %d sub-pkt(s) of %s from remote usr idx %d | voip channels:0x%08x | sendmask:0x%08x\n",
                    ...
                    pMicrPacket->MicrInfo.uUserIndex, _VoipDecodeU32(pMicrPacket->MicrInfo.channels.aChannelId), _VoipDecodeU32(pMicrPacket->MicrInfo.aSendMask)));
...
static uint32_t _VoipConnectionUpdateSingle(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, int32_t iConnId, uint32_t uTick)
{
    ...
    // fire connection event if connection flags changed
    if (pConnection->uLastRemoteConnStatus != pConnection->uRemoteConnStatus)
    {
        //$$todo - move to somewhere better?
        VoipCommonRefT *pVoipCommon = (VoipCommonRefT *)VoipGetRef();

        if ((pVoipCommon != NULL) && (pVoipCommon->pCallbackEx != NULL))
        {
            pVoipCommon->pCallbackEx((VoipRefT *)pVoipCommon, VOIP_CBTYPE_CONNEVENT, pConnection->uRemoteConnStatus, NULL, pVoipCommon->pUserData);
        }

        // update status
        pConnection->uLastRemoteConnStatus = pConnection->uRemoteConnStatus;
    }
    ...
    iTimeout = (pConnection->eState == ST_ACTV) ? pConnectionlist->iDataTimeout : pConnectionlist->iConnTimeout;
...
static int32_t _VoipConnectionRecv(VoipConnectionlistT *pConnectionlist)
{
    ...
    if ((iRecvSize = SocketRecvfrom(pConnectionlist->pSocket, (char *)&RecvPacket, sizeof(RecvPacket), 0, (struct sockaddr *)&RecvAddr, (iAddrLen=sizeof(RecvAddr),&iAddrLen))) > 0)
    {
        ...
        for (iConnID = 0, pConnection = NULL; iConnID < pConnectionlist->iMaxConnections; iConnID++)
        {
            ...
            /* if the clientID and IP address matches, this is the connection. Originally, it was enough to only check client ids here.
               However, when we later added support for different voip connections being hosted by different CC voipservers, it became also 
               necessary to compare source IPs. This was because for cc - hosted connections, the clientid is only guaranteed to be unique 
               to the voipserver. This means that it is possible for a player to have multiple connections with the same client id that are 
               with different players. In order to avoid choosing the wrong connection at this point and then later discarding the packet 
               for a source IP or session id mismatch, the source IP must be verified at this step. */
            if ((uClientId == pConnection->uRemoteClientId) && (SocketNtohl(pConnection->SendAddr.sin_addr.s_addr) == SocketNtohl(RecvAddr.sin_addr.s_addr)))
            {
                break;
            }
        }
        ...
        else
        {
            ...
            if ((pVoipCommon != NULL) && (pVoipCommon->pCallbackEx != NULL))
            {
                VoipConnEventInfoT ConnInfo;
                ConnInfo.uRecvAddr = SockaddrInGetAddr((struct sockaddr *)&RecvAddr);
                ConnInfo.uRecvPort = SockaddrInGetAddr((struct sockaddr *)&RecvAddr);
                ConnInfo.uClientId = uClientId;
                pVoipCommon->pCallbackEx((VoipRefT *)pVoipCommon, VOIP_CBTYPE_CONNEVENT, pConnection->uRemoteConnStatus, &ConnInfo, pVoipCommon->pUserData);
            }
        }
    }
...
/*F********************************************************************************/
/*!
    \Function VoipConnectionSocketOpen
    ...
    \Version 10/01/2019 (jbrookes) Split from VoipConnectionStart()
*/
/********************************************************************************F*/
static int32_t _VoipConnectionSocketOpen(VoipConnectionlistT *pConnectionlist, uint32_t uBindPort)
{
...
static void _VoipConnectionTrySocketClose(VoipConnectionlistT *pConnectionlist)
{
    ...
    // if no connections left, kill socket, unless we're listening
    if ((iNumConnections == 0) && (pConnectionlist->pSocket != NULL) && !pConnectionlist->bListening)
    {
...
static void _VoipConnectionStop(VoipConnectionlistT *pConnectionlist, VoipConnectionT *pConnection, int32_t iConnID, int32_t bSendDiscMsg)
{
    ...
    if (bSendDiscMsg)
    {
        ...
        uint32_t uLocalClientId = pConnection->bIsLocalClientIdValid ? pConnection->uLocalClientId : pConnectionlist->uClientId;
        _VoipConnectionDisc(iConnID, uLocalClientId, pConnection->uRemoteClientId, pConnection->aSessionInfo[0].uSessionId, &pConnection->SendAddr, pConnectionlist->pSocket);
    }
    else
    {
...
int32_t VoipConnectionStartup(VoipConnectionlistT *pConnectionlist, int32_t iMaxPeers)
{
    ...
    pConnectionlist->iConnTimeout = VOIP_TIMEOUT;
...
/*!
    ...
    \Input iVoipServerConnId    - voip server connection id, VOIP_CONNID_NONE if not hosted
...
*/
/********************************************************************************F*/
int32_t VoipConnectionStart(VoipConnectionlistT *pConnectionlist, int32_t iConnID, uint32_t uAddr, uint32_t uConnPort, uint32_t uBindPort, uint32_t uClientId, uint32_t uSessionId, int32_t iVoipServerConnId, void *pOwnerRef)
{
    VoipConnectionT *pConnection;
    ...
    if ((pConnectionlist->pSocket == NULL) && (iResult = _VoipConnectionSocketOpen(pConnectionlist, uBindPort)) < 0)
    {
        return(iResult);
    }
    ...
    */
    ...
    VoipConnectionAddSessionInfo(pConnectionlist, iConnID, uSessionId, iVoipServerConnId, pOwnerRef);

    pConnection->uRecvSeqn = 0xFFFFFFFF;   // intialize last receive sequence number with -1.
    pConnection->uRemoteConnStatus = VOIP_CONN_CONNECTING;
    for (iUserIndex = 0; iUserIndex < VOIP_MAXLOCALUSERS; ++iUserIndex)
    {
    ...
    NetPrintf(("voipconnection: [%d] connecting to %a:%d localId=<unknown> remoteId=0x%08x\n", iConnID,
        SocketNtohl(pConnection->SendAddr.sin_addr.s_addr), SocketNtohs(pConnection->SendAddr.sin_port), pConnection->uRemoteClientId));
...
*/
/********************************************************************************F*/
int32_t VoipConnectionListen(VoipConnectionlistT *pConnectionlist, uint32_t uListenPort)
{
    //$$todo - check for socket already existing on different port (pConnectionlist->pSocket == NULL)
    int32_t iResult = _VoipConnectionSocketOpen(pConnectionlist, uListenPort);
    if (iResult >= 0)
    {
        pConnectionlist->bListening = TRUE;
    }
    return(iResult);
}
...
void VoipConnectionSend(VoipConnectionlistT *pConnectionlist, uint32_t uSendMask, const uint8_t *pVoiceData, int32_t iDataSize, const uint8_t *pMetaData, int32_t iMetaDataSize, uint32_t uUserIndex, uint8_t uSendSeqn)
{
    ...
    int32_t iConnID;
    int32_t iParticipatingUserIndex;
    uint8_t aSentToServer[VOIP_MAXCONNECT];   // local variable used to remember which voipserver got the packet once and should then be ignored

    ds_memclr(&aSentToServer, sizeof(aSentToServer));
    ...
    for (iConnID = 0; iConnID < pConnectionlist->iMaxConnections; iConnID++)
    {
        ...
        // for connections via voipserver, do not send if packet was sent on a previous connection associated with same voipserver
        if (aSentToServer[iConnID] == TRUE)
        {
            continue;
        }

        /* is the connection active, and are we sending to it?
           xboxone: After rebasing VoipHeadsetXboxOne to MS game chat 2, per-connection muting for 
                    native VoIP specifically is handled by VoipHeadsetXboxOne (See usage of +snd, -snd, +rcv, -rcv).
                    The condition below is ignoring the send mask for native VoIP because we found that the 
                    integration with our per-connection muting here was not behaving properly during unmuting: resuming 
                    submission of data frames to MS game chat 2 resulted in a ~ 5-sec delay before speech would resume. */

        if ((pConnection->eState == ST_ACTV) && (
            #if defined(DIRTYCODE_XBOXONE) || defined(DIRTYCODE_GDK) 
                !VoipStatus(VoipGetRef(), 'xply', 0, NULL, 0) || 
            #endif
                (((VoipCommonRefT *)VoipGetRef())->uConnSendMask & (1 << iConnID)))
            )
        {
            ...
            // if connection is through voipserver, mark 'packet sending' as done for any other connections 
            // through same voipserver which belong to the same session
            if (pConnection->aSessionInfo[0].iVoipServerConnId != VOIP_CONNID_NONE)
            {
                for (iInnerConnID = iConnID; iInnerConnID < pConnectionlist->iMaxConnections; iInnerConnID++)
                {
                    VoipConnectionT *pNextConnection = &pConnectionlist->pConnections[iInnerConnID];
                    if (pNextConnection->aSessionInfo[0].iVoipServerConnId != VOIP_CONNID_NONE)
                    {
                        if ((memcmp(&pConnection->SendAddr, &pNextConnection->SendAddr, sizeof(pConnection->SendAddr)) == 0) 
                            && (pConnection->aSessionInfo[0].uSessionId == pNextConnection->aSessionInfo[0].uSessionId))
                        {
                            aSentToServer[iInnerConnID] = TRUE;
                        }
                    }
                }
            }
...
/*F********************************************************************************/
/*!
    \Function VoipConnectionAddSessionInfo
    ...
    \Input iVoipServerConnId    - high-level conn id associated with session ID to be added.
                                  VOIP_CONNID_NONE if the session is not server hosted
    \Input *pOwnerRef           - ref of owning higher level module
...
*/
/********************************************************************************F*/
int32_t VoipConnectionAddSessionInfo(VoipConnectionlistT *pConnectionlist, int32_t iConnId, uint32_t uSessionId, int32_t iVoipServerConnId, void *pOwnerRef)
{
    int32_t iSessionIndex;
    int32_t iRetCode = -1;
    VoipConnectionT *pConnection = &pConnectionlist->pConnections[iConnId];
    ...
    for (iSessionIndex = 0; iSessionIndex < VOIP_MAXSESSIONINFOS; iSessionIndex++)
    {
        if (pConnection->aSessionInfo[iSessionIndex].uSessionId == 0)
        {
            // free spot, insert session info here
            pConnection->aSessionInfo[iSessionIndex].uSessionId = uSessionId;
            pConnection->aSessionInfo[iSessionIndex].iVoipServerConnId = iVoipServerConnId;
            pConnection->aSessionInfo[iSessionIndex].pOwnerRef = pOwnerRef;

            NetPrintf(("voipconnection: [%d] added [session 0x%08x, voipserverid %d, owner %p] to set of session info sharing this voip connection\n",
                iConnId, uSessionId, iVoipServerConnId, pOwnerRef));
    ...
    if (iRetCode != 0)
    {
        NetPrintf(("voipconnection: [%d] warning - [session 0x%08x, voipserverid %d, owner %p] could not be added to set of sessions sharing this voip connection because the list is full.\n", 
            iConnId, uSessionId, iVoipServerConnId, pOwnerRef));
    }
...
/*F********************************************************************************/
/*!
    \Function VoipConnectionDeleteSessionInfo
...
*/
/********************************************************************************F*/
int32_t VoipConnectionDeleteSessionInfo(VoipConnectionlistT *pConnectionlist, int32_t iConnId, uint32_t uSessionId, void *pOwnerRef)
{
    int32_t iSessionIndex;
    int32_t iRetCode = -1;
    VoipConnectionT *pConnection = &pConnectionlist->pConnections[iConnId];
    ...
    for(iSessionIndex = 0; iSessionIndex < VOIP_MAXSESSIONINFOS; iSessionIndex++)
    {
            ...
            (pConnection->aSessionInfo[iSessionIndex].pOwnerRef == pOwnerRef))
        {
            ...
            for(iSessionIndex2 = iSessionIndex; iSessionIndex2 < VOIP_MAXSESSIONINFOS; iSessionIndex2++)
            {
                if (iSessionIndex2 == VOIP_MAXSESSIONINFOS-1)
                {
                    // last entry, reset id to 0 and clear conn id
                    pConnection->aSessionInfo[iSessionIndex2].uSessionId = 0;
                    pConnection->aSessionInfo[iSessionIndex2].iVoipServerConnId = VOIP_CONNID_NONE;
                    pConnection->aSessionInfo[iSessionIndex2].pOwnerRef = NULL;
                }
                else
                {
                    pConnection->aSessionInfo[iSessionIndex2].uSessionId = pConnection->aSessionInfo[iSessionIndex2+1].uSessionId;
                    pConnection->aSessionInfo[iSessionIndex2].iVoipServerConnId = pConnection->aSessionInfo[iSessionIndex2+1].iVoipServerConnId;
                    pConnection->aSessionInfo[iSessionIndex2].pOwnerRef = pConnection->aSessionInfo[iSessionIndex2+1].pOwnerRef;
                }
            }

            NetPrintf(("voipconnection: [%d] removed [session 0x%08x, voipserverid --, owner %p] from set of sessions sharing this voip connection\n", 
                iConnId, uSessionId, pOwnerRef));
    ...
    if (iRetCode != 0)
    {
        NetPrintf(("voipconnection: [%d] warning - [session 0x%08x, voipserverid --, owner %p]  not deleted because not found in set of sessions sharing this voip connection\n",
            iConnId, uSessionId, pOwnerRef));
    }
...
void VoipConnectionReliableTranscribedTextMessage(VoipConnectionlistT *pConnectionlist, uint8_t uLocalUserIndex, const char *pStrUtf8)
{
    ...
    *pWrite++ = uLocalUserIndex;
    ...
    for (iConnectionIndex = 0; iConnectionIndex < pConnectionlist->iMaxConnections; iConnectionIndex++)
    {
        ...
        if ((pConnection->eState == ST_ACTV) && pConnection->bTranscribedTextRequested && ((VoipCommonRefT *)VoipGetRef())->uConnSendMask & (1 << iConnectionIndex))
        {
