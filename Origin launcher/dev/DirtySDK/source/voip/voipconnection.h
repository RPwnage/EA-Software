...
/*** Defines **********************************************************************/
//! maximum number of session infos per VOIP connection
#define VOIP_MAXSESSIONINFOS  (8)
...
{
    uint32_t uSessionId;         //<! id for this session. blaze mesh id if non server-hosted, connection set id if cc-hosted
    ...
    void *   pOwnerRef;          //<! ref of higher level module owning this reference to the connection
} SessionInfoT;
...
typedef struct VoipConnectionT
{
    ...
    SessionInfoT    aSessionInfo[VOIP_MAXSESSIONINFOS]; //!< list of SessionInfo representing the set of sessions sharing this connection
    ...
    uint32_t        uLastRemoteConnStatus; //!< used by voipcommon to track connection status callback
    int32_t         iVoipServerConnId;  //!< conn id used by the voip server to identify this connection, contains VOIP_CONNID_NONE if unknown by voip server
...
typedef struct VoipConnectionlistT
{
    ...
    int32_t         iDataTimeout;       //!< data timeout in milliseconds
    int32_t         iConnTimeout;       //!< connection timeout in milliseconds
