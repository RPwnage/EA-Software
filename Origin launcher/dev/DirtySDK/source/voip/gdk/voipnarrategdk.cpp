...
static void _VoipNarrateProcessResult(VoipNarrateRefT *pVoipNarrate, VoipNarrateRequestT *pRequest)
{
    ...
    if ((iDataLen = DS_MIN(pVoipNarrate->iDataMax, (signed)(pRequest->uResultSize - pRequest->uResultPos))) > 0)
    {
        ...
        while (iDataOff < iDataLen)
        {
            ...
            if ((iRead = pVoipNarrate->pVoiceDataCb(pVoipNarrate, pRequest->iUserIndex, (const int16_t *)(pData + iDataOff), iLength, pRequest->uFlags, pVoipNarrate->pUserData)) > 0)
            {
                iDataOff += iRead;
            }
            else
...
int32_t VoipNarrateInput(VoipNarrateRefT *pVoipNarrate, int32_t iUserIndex, VoipNarrateGenderE eGender, uint32_t uFlags, const char *pText)
{
    ...
    NetPrintfVerbose((pVoipNarrate->iVerbose, 1, "voipnarrategdk: queueing request '%s'\n", pText));
