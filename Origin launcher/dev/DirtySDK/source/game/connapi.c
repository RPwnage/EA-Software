...
static void _ConnApiDisplayClientInfo(ConnApiClientT *pClient, int32_t iClient)
{
    if (pClient->bAllocated)
    {
        ...
        ds_snzprintf(strConnSetId, sizeof(strConnSetId), "%d", pClient->ClientInfo.uHostingServerConnSetId);
        NetPrintf(("connapi:   %d) id:0x%08x lid:0x%08x rid:0x%08x ip:%a hosted:%s connsetid:%s qos:%s dirtyaddr:%s\n",
            ...
            pClient->ClientInfo.bIsConnectivityHosted ? strConnSetId : "n/a",
...
static void _ConnApiUpdateClientFlags(ConnApiRefT *pConnApi)
{
    ...
    for (iClient = 0; iClient < pConnApi->ClientList.iMaxClients; iClient++)
    {
        ...
        /* if voip needs to be disabled while we do QoS measurements or due to the client's PSS value
            update the conn flags accordingly */
        if ((pClient->bEstablishVoip == FALSE) || (pClient->ClientInfo.bIsVoipContributer == FALSE))
        {
            pClient->uConnFlags &= ~CONNAPI_CONNFLAG_VOIPCONN;
        }
    }
...
static void _ConnApiVoipGroupConnSharingCallback(VoipGroupRefT *pVoipGroup, ConnSharingCbTypeE eCbType, int32_t iConnId, void *pUserData, uint8_t bSending, uint8_t bReceiving)
{
    ...
    else if (eCbType == VOIPGROUP_CBTYPE_CONNRESUME)
    {
        ...
        VoipGroupResume(pVoipGroup, iConnId, iConnectAddr, pClient->VoipInfo.uMnglPort, pClient->VoipInfo.uLocalPort, pClient->ClientInfo.uId, 
                        (pClient->ClientInfo.bIsConnectivityHosted ? pClient->ClientInfo.uHostingServerConnSetId : (unsigned)pConnApi->iSessId),
                        pClient->ClientInfo.bIsConnectivityHosted);
        #if DIRTYCODE_LOGGING
...
static int32_t _ConnApiUpdateGameClient(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex)
{
    ...
    if (pClient->GameInfo.eStatus == CONNAPI_STATUS_INIT)
    {
        ...
        if (pClient->pGameUtilRef != NULL)
        {
            ...
            _ConnApiSetGamelinkOpt(pClient->pGameUtilRef, 'time', pConnApi->iTimeout);
...
static int32_t _ConnApiUpdateVoipClient(ConnApiRefT *pConnApi, ConnApiClientT *pClient, int32_t iClientIndex)
{
    ...
    if (pClient->VoipInfo.eStatus == CONNAPI_STATUS_INIT)
    {
        ...
        iVoipConnId = VoipGroupConnect(pConnApi->pVoipGroupRef, iAdjustedVoipClientIndex, iConnectAddr, pClient->VoipInfo.uMnglPort, pClient->VoipInfo.uLocalPort,
                                       pClient->ClientInfo.uId, (pClient->ClientInfo.bIsConnectivityHosted ? pClient->ClientInfo.uHostingServerConnSetId : (unsigned)pConnApi->iSessId),
                                       pClient->ClientInfo.bIsConnectivityHosted, pClient->ClientInfo.uRemoteClientId, pClient->ClientInfo.uLocalClientId);
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'type' - set connection type - iValue = CONNAPI_CONNFLAG_*  $$todo: to be deprecated
            ...
            'vset' - set voip enable/disable (default=TRUE; call before ConnApiOnline())  $$todo: to be deprecated
...
int32_t ConnApiControl(ConnApiRefT *pConnApi, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    {
        /*
          $$todo - this selector is to be deprecated.
          Passing topologies is preferred so that connflags are only manipulated by connapi.
        */
