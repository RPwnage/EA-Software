...
namespace NAnt.Core.PackageCore
{
	...
	{
		public static IPackageServer CreateWebPackageServer(Project project)
		{
			IPackageServer server = LoadFromAssembly("WebServicePackageServer.dll");
			server.Init(project);
			return server;
		}

		public static IPackageServerV2 CreateWebPackageServer(Project project, bool useV1 = false, string token = "")
		{
			IPackageServerV2 server = (IPackageServerV2)LoadFromAssembly("WebServicePackageServer.dll");
			server.Init(project, useV1, token);
			return server;
		}

		public static IPackageServer CreateProtocolPackageServer(Project project)
		{
			IPackageServer server = LoadFromAssembly("ProtocolPackageServer.dll");
			server.Init(project);
			return server;
		}
		...
		private static IPackageServer LoadFromAssembly(string assemblyFile)
		{
			try
			{
				Assembly assembly = AssemblyLoader.Get(assemblyFile);
