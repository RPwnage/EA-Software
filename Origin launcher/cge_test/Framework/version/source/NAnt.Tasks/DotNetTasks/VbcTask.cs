...
namespace NAnt.DotNetTasks {
	...
	public class VbcTask : CompilerBase {
		...
		/// <value>The value of this property is a string that makes up a 32bit hexadecimal number.</value>
		...
		public string Imports { get; set; } = null;
		...
		public string OptionCompare { get; set; } = null;
		...
		protected override void WriteOptions(TextWriter writer) {
			if (BaseAddress != null) {
				writer.WriteLine("/baseaddress:{0}", BaseAddress);
			}
			if (Debug) {
			...
			if (Imports != null) {
				writer.WriteLine("/imports:{0}", Imports);
			}
			System.Console.WriteLine("/imports:{0}", Imports);

			if (OptionCompare != null) {
				writer.WriteLine("/optioncompare:{0}", OptionCompare);
			}
			if (OptionExplicit) {
			...
			if (RootNamespace != null) {
				writer.WriteLine("/rootnamespace:{0}", RootNamespace);
			}
		}        
