...
namespace EA.Eaconfig
{
	...
	sealed class CreatePrebuiltPackage : Task
	{
		...
		public class PrebuiltPublicData
		{
			...
			// Not sure we really need to do anything with this one.  For now, we're not overriding this info.
			public IEnumerable<PathString> InternalIncludeDirectories { get { return null; } }
		...
		private void CopyExplicitIncludeFiles()
		{
			...
			foreach (string file in mPackageData.mPrebuiltPackageIncludeFiles)
			{
				...
				if (!file.StartsWith(PackageDir) && !file.StartsWith(PackageBuildDir))
				{
					throw new BuildException("ERROR: The package.[package_name].PrebuiltIncludeSourceFiles fileset can only be used to specify files in the package tree or build artifacts, not outside the package.");
				}
				else
				{
