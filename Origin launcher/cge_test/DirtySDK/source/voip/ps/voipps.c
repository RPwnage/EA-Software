...
static void _VoipStatusCb(int32_t iLocalUserIndex, uint32_t bStatus, VoipHeadsetStatusUpdateE eUpdate, void *pUserData)
{
    ...
    {
        NetPrintf(("voipps: warning, unexpected headset event for this platform!\n"));
    }

    if (bStatus)
    {
        NetPrintf(("voipps: [%d] headset active (audio in: on, audio out: on)\n", iLocalUserIndex));
    ...
    {
        NetPrintf(("voipps: [%d] headset inactive (audio in: off, audio out: on)\n", iLocalUserIndex));
        pRef->Common.uPortHeadsetStatus &= ~(1 << iLocalUserIndex);
        pRef->Common.Connectionlist.uLocalUserStatus[iLocalUserIndex] &= ~(VOIP_LOCAL_USER_HEADSETOK|VOIP_LOCAL_USER_INPUTDEVICEOK);
    }
}
...
static void _VoipRegisterLocalSharedTalker(VoipRefT *pVoip, uint32_t bRegister)
{
    if (bRegister)
    {
        ...
        if (VOIP_NullUser((VoipUserT *)(&pVoip->Common.Connectionlist.LocalUsers[VOIP_SHARED_USER_INDEX])))
        {
            ...
            NetPrintf(("voipps: local shared user added at index %d\n", VOIP_SHARED_USER_INDEX));
        }
    }
    else
    {
        ...
        if (!VOIP_NullUser((VoipUserT *)(&pVoip->Common.Connectionlist.LocalUsers[VOIP_SHARED_USER_INDEX])))
        {
            ...
            NetPrintf(("voipps: shared user removed at index %d\n", VOIP_SHARED_USER_INDEX));
        }
...
static int32_t _VoipActivateLocalTalker(VoipCommonRefT *pVoipCommon, int32_t iLocalUserIndex, VoipUserT *pVoipUser, uint8_t bActivate)
{
    ...
    if (pVoipUser != NULL)
    {
        ...
        {
            NetPrintf(("voipps: cannot %s a null user %s participating state (local user index %d)\n",
...
static void _VoipThread(void *pArg)
{
    ...
    while(pVoip->iThreadActive == 1)
    {
        ...
        /* the connectionlist netcrit is entered before calling VoipHeadsetProcess() to protect access to the
           codec used between the socket recv thread (decode) and the voip thread (reset/encode) */
        NetCritEnter(&pVoip->Common.Connectionlist.NetCrit);
        ...
        {
            NetPrintf(("voipps: [WARNING] Overall voip thread update took %d ms\n", NetTickDiff(uTime1, uTime0)));
        }
    }
...
VoipRefT *VoipStartup(int32_t iMaxPeers, int32_t iData)
{
    ...
    {
        NetPrintf(("voipps: unable to create voip thread (err=%d)\n", iResult));
        pVoip->iThreadActive = 0;
...
void VoipShutdown(VoipRefT *pVoip, uint32_t uShutdownFlags)
{
    ...
    if (VoipGetRef() == NULL)
    {
        NetPrintf(("voipps: module shutdown called when not in a started state\n"));
        return;
    }
    ...
    if (pVoip->iThreadActive == 1)
    {
        ...
        {
            NetPrintf(("voipps: waiting for shutdown\n"));
            NetConnSleep(VOIP_THREAD_SLEEP_DURATION);
        }
    }
...
void VoipSetLocalUser(VoipRefT *pVoip, int32_t iLocalUserIndex, uint32_t bRegister)
{
    if ((iLocalUserIndex < VOIP_MAXLOCALUSERS) && (iLocalUserIndex >= 0))
    {
        ...
        if (bRegister)
        {
            if (VOIP_NullUser(&pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex]))
            {
                ...
                NetPrintf(("voipps: registering local user %lld at index %d\n", voipUser.AccountInfo.iPersonaId, iLocalUserIndex));
                _VoipRegisterLocalTalker(pVoip, iLocalUserIndex, TRUE, &voipUser);
            }
            else
            {
                NetPrintf(("voipps: warning -- ignoring attempt to register user index %d because user %lld is already registered at that index\n",
        ...
        else
        {
            if (!VOIP_NullUser(&pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex]))
            {
                ...
                NetPrintf(("voipps: unregistering local user %lld at index %d\n", pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex].AccountInfo.iPersonaId, iLocalUserIndex));
                _VoipRegisterLocalTalker(pVoip, iLocalUserIndex, FALSE, NULL);
            }
            else
            {
                NetPrintf(("voipps: warning -- ignoring attempt to unregister local user at index %d because it is no yet registered\n", iLocalUserIndex));
            }
        }
    ...
    {
        NetPrintf(("voipps: VoipSetLocalUser() invoked with an invalid user index %d\n", iLocalUserIndex));
    }
}
...
void VoipCommonActivateLocalUser(VoipCommonRefT *pVoipCommon, int32_t iLocalUserIndex, uint8_t bActivate)
{
    ...
    if ((iLocalUserIndex < NETCONN_MAXLOCALUSERS) && (iLocalUserIndex >= 0))
    {
        ...
        if (!VOIP_NullUser(&pConnectionlist->LocalUsers[iLocalUserIndex]))
        {
            ...
            {
                NetPrintf(("voipps: demoting local user %lld from participating state\n", &pConnectionlist->LocalUsers[iLocalUserIndex].AccountInfo.iPersonaId));
        ...
        {
            NetPrintf(("voipps: VoipCommonActivateLocalUser() failed because no local user registered at index %d\n", iLocalUserIndex));
        }
    ...
    {
        NetPrintf(("voipps: VoipCommonActivateLocalUser() invoked with an invalid user index %d\n", iLocalUserIndex));
    }
}
...
int32_t VoipControl(VoipRefT *pVoip, int32_t iControl, int32_t iValue, void *pValue)
{
    ...
    if (iControl == '+pri')
    {
        ...
        {
            NetPrintf(("voipps: local user at index %d is now an explicit contributor to most restrictive voip privilege determination\n", iValue));
            _VoipPs4_MostRestrictiveVoipContributors[iValue] = TRUE;
        }
        #if DIRTYCODE_LOGGING
        else
        {
            NetPrintf(("voipps: VoipControl('+pri', %d) ignored because user is already an explicit contributor to most restrictive voip privilege determination\n", iValue));
        }
        #endif
        return(0);
    }
    if (iControl == '-pri')
    {
        ...
        {
            NetPrintf(("voipps: local user at index %d is no longer an explicit contributor to most restrictive voip privilege determination\n", iValue));
            _VoipPs4_MostRestrictiveVoipContributors[iValue] = FALSE;
        }
        #if DIRTYCODE_LOGGING
        else
        {
            NetPrintf(("voipps: VoipControl('-pri', %d) ignored because user is already not explicitly contributing to most restrictive voip privilege determination\n", iValue));
        }
        #endif
        return(0);
    }
