...
/*!
    ...
    \Description
        Write a byte to output buffer safely

    \Input *pBuffer     - [out] buffer to write byte to
    \Input iBufSize     - size of output buffer
    \Input uValue       - source value to write

    \Output
        int32_t         - one

    \Version 11/11/2019 (jbrookes)
*/
/********************************************************************************F*/
static int32_t _AWSWrite8(uint8_t *pBuffer, int32_t iBufSize, const uint8_t uValue)
{
    if (iBufSize < (int32_t)sizeof(uValue))
    {
        return(0);
    }
    *pBuffer = uValue;
    return(sizeof(uValue));
}

/*F********************************************************************************/
/*!
    \Function _AWSWriteBytes

    \Description
        Write bytes to output buffer safely

    \Input *pBuffer     - [out] buffer to write bytes to
    \Input iBufSize     - size of output buffer
    \Input *pValue      - source data to write
    \Input iValueSize   - source data size
    \Output
        int32_t         - iValueSize
...
*/
/********************************************************************************F*/
static int32_t _AWSWriteBytes(uint8_t *pBuffer, int32_t iBufSize, const uint8_t *pValue, int32_t iValueSize)
{
    if (iBufSize < iValueSize)
    {
        return(0);
    }
    ds_memcpy_s(pBuffer, iBufSize, pValue, iValueSize);
    return(iValueSize);
}
...
/*!
    ...
    \Description
        Write binary event header to buffer; returns total size header, but will not
        exceed specified buffer length.
...
static int32_t _AWSWriteEventHeader(uint8_t *pBuffer, int32_t iBufSize, const char *pHeader, const uint8_t *pValue, int32_t iValueSize, AWSHeaderValueE eHeaderValue)
{
    ...
    // write header length
    iOffset += _AWSWrite8(pBuffer, iBufSize, (uint8_t)iStrLen);
    // write header name
    iOffset += _AWSWriteBytes(pBuffer+iOffset, iBufSize-iOffset, (const uint8_t *)pHeader, iStrLen);
    // copy header value type
    iOffset += _AWSWrite8(pBuffer+iOffset, iBufSize-iOffset, (uint8_t)eHeaderValue);
    // copy header value length
    if ((eHeaderValue == AWS_BYTE_ARRAY) || (eHeaderValue == AWS_STRING))
    {
        iOffset += _AWSWrite8(pBuffer+iOffset, iBufSize-iOffset, (uint8_t)(iValueSize>>8));
        iOffset += _AWSWrite8(pBuffer+iOffset, iBufSize-iOffset, (uint8_t)(iValueSize>>0));
    }
    // copy header value
    ...
    if ((eHeaderValue == AWS_SHORT) || (eHeaderValue == AWS_INTEGER) || (eHeaderValue == AWS_LONG) || (eHeaderValue == AWS_TIMESTAMP))
    {
        ...
        {
            iOffset += _AWSWrite8(pBuffer+iOffset, iBufSize-iOffset, pValue[iData--]);
        }
    }
    else
    #endif
    {
        iOffset += _AWSWriteBytes(pBuffer+iOffset, iBufSize-iOffset, pValue, iValueSize);
    }
...
/*F********************************************************************************/
/*!
    \Function _AWSReadEventPreludeAndValidate

    \Description
        Read event prelude, validate prelude and message crc32s

    \Input *pBuffer         - buffer pointing to event to read
    \Input iBufSize         - size of buffer
    \Input *pHdrLen         - [out] buffer to store header length
    \Input *pMsgLen         - [out] buffer to store message length

    \Output
        int32_t             - offset past header, or negative on error

    \Version 10/21/2019 (jbrookes)
*/
/********************************************************************************F*/
static int32_t _AWSReadEventPreludeAndValidate(const uint8_t *pBuffer, int32_t iBufSize, int32_t *pHdrLen, int32_t *pMsgLen)
{
    int32_t iOffset=0, iHdrLen, iMsgLen;
    uint32_t uCrc32;
    ...
    // calculate prelude crc32
    uCrc32 = NetCrc32(pBuffer, iBufSize, NULL);
    ...
    // sanity check sizes
    if ((iMsgLen > iBufSize) || (iHdrLen > iMsgLen))
    {
        NetPrintf(("aws: size validation failed on event\n"));
        return(-2);
    }

    // read prelude crc32
    uCrc32  = pBuffer[iOffset++] << 24;
    uCrc32 |= pBuffer[iOffset++] << 16;
    uCrc32 |= pBuffer[iOffset++] << 8;
    uCrc32 |= pBuffer[iOffset++] << 0;
    // validate  prelude crc32
    if (uCrc32 != (uint32_t)NetCrc32(pBuffer, 8, NULL))
    {
        NetPrintf(("aws: prelude crc32 validation failed on event\n"));
        return(-3);
    }

    // read message crc32
    uCrc32  = pBuffer[iMsgLen-4] << 24;
    uCrc32 |= pBuffer[iMsgLen-3] << 16;
    uCrc32 |= pBuffer[iMsgLen-2] << 8;
    uCrc32 |= pBuffer[iMsgLen-1] << 0;
    // validate message crc32
    if (uCrc32 != (uint32_t)NetCrc32(pBuffer, iMsgLen-4, NULL))
    {
        NetPrintf(("aws: message crc32 validation failed on event 0x%08x != 0x%08x\n", uCrc32, NetCrc32(pBuffer, iMsgLen-4, NULL)));
        return(-4);
    }
    ...
    *pMsgLen = iMsgLen-iHdrLen-AWS_EVENT_PRELUDE_SIZE-4;
    // return read offset to caller
    return(iOffset);
}
...
/*!
    ...
    \Description
        Write an AWS signed binary event; writes as much data as will fit in the
        buffer (not all data is guaranteed to be consumed).
    ...
    \Output
        int32_t             - size of event data written, zero if not enough room
...
{
    int32_t iOffset=0, iHdrOff, iHdrLen, iDatOff, iDatLen, iSigOff, uCrc32;
    ...
    // if we have data, add in as much as will fit
    if (pData != NULL)
    {
        ...
        iDatLen = iOffset-iDatOff-AWS_EVENT_PRELUDE_SIZE;

        // calculate how much data we can write, including the event chunk message_crc and outer chunk message_crc
        if ((*pDataSize = DS_MIN(*pDataSize, iBufSize-iOffset-4-4)) < 0)
        {
            NetPrintf(("aws: not enough room to write event with data\n"));
            return(0);
        }
        // copy event data
        ...
        _AWSWriteEventPrelude(pBuffer+iDatOff, iDatLen, iOffset+4-iDatOff);

        // calculate and write event chunk message_crc
        uCrc32 = NetCrc32(pBuffer+iDatOff, iOffset-iDatOff, NULL);
        ...
        // locate data to sign
        pSignData = pBuffer+iDatOff;
        iSignDataSize = iOffset-iDatOff;
    }
    // make sure we have room to fully write empty event
    else if (iBufSize < (iOffset+4))
    {
        NetPrintf(("aws: not enough room to write event\n"));
        return(0);
    }
...
int32_t AWSReadEvent(const uint8_t *pBuffer, int32_t iBufSize, char *pEventType, int32_t iEventSize, char *pMessage, int32_t *pMessageSize)
{
    ...
    int32_t iOffset, iHdrLen, iMsgLen, iValueLen;
    AWSHeaderValueE eValueType;

    // read prelude info and validate crcs
    if ((iOffset = _AWSReadEventPreludeAndValidate(pBuffer, iBufSize, &iHdrLen, &iMsgLen)) < 0)
    {
        return(iOffset);
    }
