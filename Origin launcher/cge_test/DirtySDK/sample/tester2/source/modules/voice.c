...
static int32_t _CmdVoiceCb(ZContext *argz, int32_t argc, char *argv[])
{
    ...
    if (pApp->pVoip != NULL)
    {
        ...
        {
            ZPrintf("voice: sttm change [EventCount: %3d DurationSent: %5dms CharsRecv: %4d EmptyRslt: %3d Delay: %5dms]\n",
                SttMetrics.uEventCount, SttMetrics.uDurationMsSent, SttMetrics.uCharCountRecv, SttMetrics.uErrorCount, SttMetrics.uDelay);
