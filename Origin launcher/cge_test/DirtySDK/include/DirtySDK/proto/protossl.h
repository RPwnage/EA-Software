...
//! cert info, returned by ProtoSSLStat() 'cert' and 'scrt' selector calls
typedef struct ProtoSSLCertInfoT
{
    ProtoSSLCertIdentT Ident;   //!< use issuer and subject field (deprecated)
    int32_t iKeyModSize;        //!< size of the key
    ProtoSSLCertIdentT Issuer;  //!< issuer of the certificate
    ProtoSSLCertIdentT Subject; //!< subject of the certificate
    uint64_t uGoodFrom;         //!< validity from date
    uint64_t uGoodTill;         //!< validity to date
} ProtoSSLCertInfoT;
