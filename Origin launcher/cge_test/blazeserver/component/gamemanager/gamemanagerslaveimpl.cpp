...
namespace GameManager
{
    ...
    ***************************************************************************************************/
    ...
    bool GameManagerSlaveImpl::onValidateConfig(GameManagerServerConfig& config, const GameManagerServerConfig* referenceConfigPtr, ConfigureValidationErrors& validationErrors) const
    {
        ...
        // PACKER_TODO - This code needs to be moved to the location where it matters, the Packer. 
        //  Either the Packer needs to know more about the Filters and CreateGameTemplates, or it should work regardless of them. 
        //  I think working regardless is best.  For example, Scenarios could create inconsistent CreateGameData values, but that creates obvious, trivially-fixed bugs. 

        bool isPackerScenario = false;
        auto& createGameTemplatesCfgMap = config.getCreateGameTemplatesConfig().getCreateGameTemplates();
        for (auto& scenarioItr : config.getScenariosConfig().getScenarios())
        {
            auto& scenarioName = scenarioItr.first;
            auto subsessionNum = 0;
            for (auto& subsessionItr : scenarioItr.second->getSubSessions())
            {
                ...
                auto itr = createGameTemplatesCfgMap.find(createGameTemplateName);
                ...
                if (itr != createGameTemplatesCfgMap.end())
                {
                    ...
                    auto& attributesMap = itr->second->getAttributes();
                    auto itrAttributes = attributesMap.find("CreateGameRequest.gameCreationData.networkTopology");
                    if (itrAttributes != attributesMap.end())
                    {
                        StringBuilder errMsg;
                        errMsg << "[GameManagerSlaveImpl].onValidateConfig: createGameTemplate(" << createGameTemplateName << ") attribute (" << itrAttributes->second->getAttrName() << ") overrides networkTopology, unsupported configuration.";
                        validationErrors.getErrorMessages().push_back(errMsg.get());
                    }
                }

                if (isPackerSubsession)
                {
                    bool hasParticipantCapacityFilter = false;

                    for (auto& filter : subsessionItr.second->getFiltersList())
                    {
                        auto itrFilter = config.getFilters().find(filter);
                        if (itrFilter == config.getFilters().end())
                        {
                            StringBuilder errMsg;
                            errMsg << "[GameManagerSlaveImpl].onValidateConfig: scenario(" << scenarioName << ") uses unknown filter(" << itrFilter->first.c_str() << ").";
                            validationErrors.getErrorMessages().push_back(errMsg.get());
                            continue;
                        }
                        
                        auto* gameProperty = itrFilter->second->getGameProperty();
                        if (GameManager::isGamePropertyName(gameProperty, GameManager::PROPERTY_NAME_PARTICIPANT_CAPACITY))
                        {
                            hasParticipantCapacityFilter = true;
                        }
                    }

                    if (!hasParticipantCapacityFilter)
                    {
                        StringBuilder errMsg;
                        errMsg << "[GameManagerSlaveImpl].onValidateConfig: scenario(" << scenarioName << ") filter required on property(" << GameManager::GAME_PROPERTY_PREFIX << GameManager::PROPERTY_NAME_PARTICIPANT_CAPACITY << ") - unsupported configuration with GamePacker.";
                        validationErrors.getErrorMessages().push_back(errMsg.get());
                    }
                }

                if (subsessionItr == *scenarioItr.second->getSubSessions().begin())
                {
                    // we're going to check the rest of this scenario's subsessions against its first subsession
                    isPackerScenario = isPackerSubsession;
                }
                else if (isPackerScenario != isPackerSubsession)
                {
                    StringBuilder errMsg;
                    errMsg << "[GameManagerSlaveImpl].onValidateConfig: session(" << subsessionItr.first.c_str() << ") #" << subsessionNum << "/" << scenarioItr.second->getSubSessions().size() << " using " << (isPackerSubsession ? "packerMM" : "legacyMM") << " in scenario(" << scenarioName << ") conflicts with other sessions(";
                    for (auto& subsessionUsesOtherPackerSettingItr : scenarioItr.second->getSubSessions())
                    {
                        if (subsessionUsesOtherPackerSettingItr != subsessionItr)
                        {
                            errMsg << subsessionUsesOtherPackerSettingItr.first.c_str() << ",";
                        }
                        else
                        {
                            errMsg.trimIfEndsWith(',');
                            break;
                        }

                    }
                    errMsg << ") using " << (isPackerScenario ? "packerMM" : "legacyMM") << ". Mixed packer/legacy MM usage in scenarios is not supported.";
                    validationErrors.getErrorMessages().push_back(errMsg.get());
                }
            }
        }
    ...
    ***************************************************************************************************/
    ...
    {
        // Copy the old DEPRECATED locations to the new locations:  (Remove this code when the deprecated values are killed.)
        if (request.getGameReportName()[0] != '\0')
        {
            request.getGameCreationData().setGameReportName(request.getGameReportName());
        }
    ...
    ***************************************************************************************************/
    ...
    Blaze::BlazeRpcError GameManagerSlaveImpl::getCensusDataInternal(GameManagerCensusData& gmCensusData, bool getAllData /*= true*/)
    {
        ...
        if (mmComponent != nullptr)
        {
            ...
            // Number of matchmaking sessions currently started on server from mm component
            uint32_t matchmakingSessionCount = 0;
            uint32_t matchmakingUserCount = 0;

            // build out initially based on scenario configs
            Blaze::Matchmaker::TimeToMatchEstimator timeToMatchEstimator;

            bool firstSlaveResponse = true;
            ...
            // Get the ttm estimates from the estimator:
            if (getConfig().getCensusData().getIncludeTimeToMatchEstimates() || getAllData)
            {
    ...
    ***************************************************************************************************/
    ...
    BlazeRpcError GameManagerSlaveImpl::validateMaxActiveMMSessions(UserSessionId userSessionId) const
    {
        if (getConfig().getMatchmakerSettings().getMaxActiveSessionsPerUserSession() > 0)
        {
            UserSessionMasterPtr userSession = gUserSessionMaster->getLocalSession(userSessionId);
            if (userSession != nullptr)
            {
                ...
                if (stateData != nullptr)
                {
                    size_t sessionCount = stateData->getMatchmakingSessionSlaveIdList().size();

                    UserSessionMMSessionReqCountMap::const_iterator it = mUserSessionMMSessionReqCountMap.find(userSessionId);
                    if (it != mUserSessionMMSessionReqCountMap.end())
                        sessionCount += it->second;

                    if (sessionCount >= getConfig().getMatchmakerSettings().getMaxActiveSessionsPerUserSession())
    ...
    ***************************************************************************************************/
    ...
    void GameManagerSlaveImpl::onRemoteInstanceChanged(const RemoteServerInstance& instance)
    {
        ...
        if (instance.hasComponent(Blaze::Search::SearchSlave::COMPONENT_ID) && !instance.isConnected())
        {
            TRACE_LOG("[GameManagerSlaveImpl].onRemoteInstanceChanged: cleanup GameBrowser data for seachSlave(" << instance.getInstanceId() << ").");
    ...
    ***************************************************************************************************/
    uint16_t GameManagerSlaveImpl::setUpDefaultTeams(TeamIdVector &teamIds, PlayerJoinData& playerJoinData, uint16_t participantCapacity)
    {
        if (teamIds.empty())
        {
            ...
            for (; curIter != endIter; ++curIter)
            {
                if ((*curIter)->getTeamId() != INVALID_TEAM_ID)
                {
                    perPlayerTeamIdSet = true;

                    bool teamFound = false;
                    TeamIndex teamIndex = 0;
                    TeamIdVector::iterator curTeam = teamIds.begin();
                    TeamIdVector::iterator endTeam = teamIds.end();
                    for (; curTeam != endTeam; ++curTeam)
                    {
                        if (*curTeam == (*curIter)->getTeamId())
                        {
                            teamFound = true;
                            break;
                        }
                    ...
                    if (teamFound == false)
                    {
                        teamIds.push_back((*curIter)->getTeamId());
                    }
                    (*curIter)->setTeamIndex(teamIndex);
                }
                else
                {
        ...
        return participantCapacity / teamIds.size();
    }
