...
#define AUTH_RATE_LIMIT_GRACE_PERIOD (1)
...
AuthenticationSlaveImpl::AuthenticationSlaveImpl()
  : mBypassNucleusEnabled(false), mGlobalLoginRateLimit(0.0f), mGlobalLoginRateLimitCounter(0),
    mCurrentLoginRateInterval(0), mCheckLegalDocId(INVALID_TIMER_ID),
...
BlazeRpcError AuthenticationSlaveImpl::commonEntitlementCheck(AccountId accountId, CheckEntitlementUtil& checkEntitlementUtil, ClientType clientType, const char8_t* productName,
                                                              const Command* callingCommand/* = nullptr*/, const char8_t* accessToken/* = nullptr*/,
                                                              const char8_t* clientId/* = nullptr*/, bool skipAutoGrantEntitlement /* = false*/)
{
    ...
    for (; type < INVALID_TYPE; type = Authentication::EntitlementCheckType(type + 1))
    {
        if (getNeedCheckEntitlement(clientType, type, productName, softCheck))
        {
            ...
            if (Blaze::ERR_OK == (entError = checkEntitlement(type, accountId, checkEntitlementUtil, entitlementStatus, productName, psuCutOff, isAutoGrantEnabled(type, productName),callingCommand, accessToken)))
            {
                break;
            }
            else if (entError == AUTH_ERR_NO_SUCH_ENTITLEMENT)
            {
                ...
                {
                    if (ERR_OK == (entError = postEntitlement(type, accountId, entitlementStatus, productName, psuCutOff, callingCommand, accessToken, clientId)))
                    {
                        break;
                    }
                }
...
BlazeRpcError AuthenticationSlaveImpl::tickAndCheckLoginRateLimit(bool doDelay)
{
    ...
    while (err == ERR_OK)
    {
        ...
        TimeValue intervalDelta = now - mCurrentLoginRateInterval;

        bool elapsed = (intervalDelta > AUTH_RATE_LIMIT_GRACE_PERIOD * 1000 * 1000);
        if (elapsed)
        {
            mGlobalLoginRateLimitCounter++;

            LoginIntervalInfo& info = mLoginIntervalInfoList.push_front();
            info.interval = intervalDelta;
            info.logins = mGlobalLoginRateLimitCounter;

            mCurrentLoginRateInterval = now;
            mGlobalLoginRateLimitCounter = 0;
            return ERR_OK;
        }

        // calculate login rate from most recent 60s
        TimeValue currentInterval = intervalDelta;
        uint32_t currentLogins = mGlobalLoginRateLimitCounter + 1;
        LoginIntervalInfoList::iterator itr = mLoginIntervalInfoList.begin();
        for (; itr != mLoginIntervalInfoList.end() && currentInterval.getSec() < 60; ++itr)
        {
            currentInterval += (*itr).interval;
            currentLogins += (*itr).logins;
        }
        // remove any info more than 60s old
        while (itr != mLoginIntervalInfoList.end())
        {
            itr = mLoginIntervalInfoList.erase(itr);
        }
        float_t currentLoginRate = ((float_t) currentLogins) / (((float_t) currentInterval.getMicroSeconds()) / 1000000.0f);

        if (mGlobalLoginRateLimit > currentLoginRate)
        {
            mGlobalLoginRateLimitCounter++;
            return ERR_OK;
        }
        ...
        if (doDelay)
        {
            err = gSelector->sleep((AUTH_RATE_LIMIT_GRACE_PERIOD * 1000 * 1000) - intervalDelta);
            ++sleepCount;
            if ((err == ERR_OK) && (sleepCount >= 3))   // This means that you've slept 3 times (3 seconds)
            {
                WARN_LOG("[AuthenticationSlaveImpl].tickAndCheckLoginRateLimit(): GlobalLoginRateLimit (" << mGlobalLoginRateLimit << " per second) has been exceeded for at least "
                         << 3 * AUTH_RATE_LIMIT_GRACE_PERIOD << " seconds.  Login attempts are being limited." );
                return ERR_TIMEOUT;
            }
        }
        else
    ...
    if (doDelay)
    {
        WARN_LOG("[AuthenticationSlaveImpl].tickAndCheckLoginRateLimit(): GlobalLoginRateLimit (" << mGlobalLoginRateLimit << " per second) has been exceeded and an error occurred with the sleep() command (" 
                 << ErrorHelp::getErrorName(err) << ").  Login attempts are being limited." );
    }
    else
    {
        WARN_LOG("[AuthenticationSlaveImpl].tickAndCheckLoginRateLimit(): GlobalLoginRateLimit (" << mGlobalLoginRateLimit << " per second) has been exceeded.  Login attempts are being limited." );
    }
    return err;
}
...
bool AuthenticationSlaveImpl::configure()
{
    ...
    mGlobalLoginRateLimit = ((getConfig().getTotalLoginsPerMinute() / 60.0f) * AUTH_RATE_LIMIT_GRACE_PERIOD);
        ...
        for (AccessList::const_iterator accessItr = accessListPtr->begin(), accessItrEnd = accessListPtr->end(); accessItr != accessItrEnd; ++accessItr)
        {
            ...
            if(accessToken == nullptr || accessToken[0] == '\0' || clientId == nullptr || clientId[0] == '\0')
            {
                ...
                if (Blaze::OAuth::shouldUseCurrentUserToken(INVALID_BLAZE_ID, accountId))
                    err = tokenUtil.retrieveCurrentUserAccessToken(OAuth::TOKEN_TYPE_BEARER);
                else
                {
...
void AuthenticationSlaveImpl::entitlementStatusCheck()
{
    ...
    while (!usersessionIds.empty())
    {
        ...
        const char8_t* accessToken = tokenUtil.getAccessToken();
        ...
        err = commonEntitlementCheck(accountId, checkEntUtil, clientType, productName, nullptr, accessToken, tokenUtil.getClientId(), true/* skipAutoGrantEntitlement */);
        if (err != ERR_OK)
        {
            // log an error
            TRACE_LOG("[AuthenticationSlaveImpl].entitlementStatusCheck: entitlement check returned false with error:" << ErrorHelp::getErrorName(err) << " for UserSessionId(" << userSessionId << ").");

            // we don't want to logout the user simply because Nucleus timed out
            if (err != ERR_TIMEOUT)
            {
