...
def getFIFA15GameReportAttributesModuleName():
    return __name__
...
class TitleSpecificParser:
    ...
    #Reference https://confluence.ea.com/display/FBA/FIFA+Sku+Ids
    ...
    def ParseReport(self, eventNode):
        ...
        if (prcoessStatusNode == None):
            prcoessStatusNode = eventNode.find('gamereportprocessfailed')
            if (prcoessStatusNode == None):
                print "Game Report Process unknown"
            else:
                reportNode = eventNode.find('gamereportprocessfailed/report')
        else:
            reportNode = eventNode.find('gamereportsucceeded/report')
        if(reportNode != None):
            eventTime = eventNode.get('date')
            reportString = TitleSpecificParser.ParseReportNode(self, reportNode, eventTime)    
        return reportString      
    
    
    def ParseReportNode(self, reportNode, eventTime):
        ...
        if skillGames == None:
            ...
            if playerReports != None:
              for entry in playerReports.findall('entry'):
                ...
                if string.atoi(personaId) != 0:                                
                    ...
                    attrib = attributeParser(eventTime, self.skuId, self.titleId, gameTime, gameReportId, gameType, personaId)
            ...
            attrib = attributeParser(eventTime, self.skuId, self.titleId)
...
class BaseMatchAttribute:
    ...
    def __init__(self, eventTime, skuId, titleId, gameTime, gameReportId, gameType, personaId):
...
class H2HReportAttribute(BaseMatchAttribute):
    ...
    def __init__(self, eventTime, skuId, titleId, gameTime, gameReportId, gameType, personaId):
        BaseMatchAttribute.__init__(self, eventTime, skuId, titleId, gameTime, gameReportId, gameType, personaId)
...
class SkillGameReportAttribute:
    ...
    def __init__(self, eventTime, skuId, titleId):
        self.eventTime = eventTime
        self.skuId = skuId
        self.titleId = titleId
        self.gameReportId = ""
