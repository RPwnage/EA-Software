...
Time PackerSilo::packGames(PackGameHandler* handler)
{
...
#if 1
    PartyTimeQueue evictedPartyQueue;
#else
    ...
    PartyTimeQueue& evictedPartyQueue = mPartyPriorityQueue;
#endif
    do
    {
        while (!mPartyPriorityQueue.empty())
        {
            auto partyItr = mPartyPriorityQueue.begin();
            auto& incomingParty = *partyItr->second;
            mPartyPriorityQueue.erase(partyItr);
            ...
            if (optimalGameId == INVALID_GAME_ID)
            {
                ...
                if ((mMaxWorkingSet > 0) && (mMaxWorkingSet < mGamesWorkingSet.size() + 1))
                {
                    ...
                    for (auto ritr = mGameByScoreSet.rbegin(), rend = mGameByScoreSet.rend(); ritr != rend; ++ritr)
                    {
                        ...
                        {
                            if (mUtilityMethods->utilPackerTraceFilterEnabled(TraceMode::GAMES, candidateGame->mGameId))
                            {
                                eastl::string buf;
                                buf += "\n TRIM working set ->";
                ...
                {
                    newGame.mEvalRandomSeed = getUtilityMethods()->utilRand();
                }
                ...
                for (auto& factor : mGameTemplate.mFactors)
                {
                    ...
                    if (score != INVALID_SCORE)
                    {
                        ...
                        if (nowViable)
                        {
                            ...
                            if (mUtilityMethods->utilPackerTraceEnabled(TraceMode::ALGO))
                            {
                ...
                if (mUtilityMethods->utilPackerTraceFilterEnabled(TraceMode::GAMES, incomingParty.mGameId))
                {
                    eastl::string buf;
                    buf.append("\nCREATE game -> ");
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (auto partyIndex = evictedPartyBits.find_last(), partySize = evictedPartyBits.size(); partyIndex != partySize; partyIndex = evictedPartyBits.find_prev(partyIndex))
                    {
                        ...
                        if (mUtilityMethods->utilPackerTraceFilterEnabled(TraceMode::GAMES, optimalGameId))
                        {
                        ...
                        evictedParty.mPartyPriorityQueueItr = evictedPartyQueue.insert({ evictedParty.mExpiryTime, &evictedParty });
                    }
                ...
                if (mUtilityMethods->utilPackerTraceFilterEnabled(TraceMode::GAMES, foundGame.mGameId))
                {
        ...
        evictedPartyQueue.swap(mPartyPriorityQueue);
    } while (!mPartyPriorityQueue.empty());
    ...
    GameIdByScoreMap gamesByScoreMap; // PACKER_TODO: Replace this structure with a heap of game pointers prioritized by score (and possibly gameid) This would eliminate a lot of allocations when games are pushed into a score based map while still allowing an order based visit.
    {
        ...
        for (GameId gameId : gameWorkingSet)
        {
            ...
            if (factorIndex == 0)
            {
                ...
                {
                    game.mEvalRandomSeed = getUtilityMethods()->utilRand();
                }
            ...
            traceLog(TraceMode::ALGO, "findOptimalGame: activeEval party(%" PRIi64 ") vs game(%" PRIi64 "), factor[%d](%s):  score poss(%.4f) vs old(%.4f)",
                incomingParty.mPartyId, gameId, factorIndex, factor.mFactorName.c_str(), newScore, (factorIndex >= 0 && game.mFactorScores.size() > (size_t)factorIndex ? game.mFactorScores[factorIndex] : 0));

            const char8_t* operation = nullptr;
            const char8_t* subOp = nullptr;
            uint64_t* metric = nullptr;
            auto curScore = game.mFactorScores[factorIndex];
            if (newScore >= MIN_SCORE)
            {
                ...
                */
                ...
                if (improvedAny || (tryAnyway && !isLastFactor))
                {
                    ...
                    if (becomesNonviable)
                    {
                        traceLog(TraceMode::ALGO, "findOptimalGame: on factor[%d](%s) game(%" PRIi64 ") would become no longer viable. Skip.", factor.mFactorIndex, factor.mFactorName.c_str(), game.mGameId);
                        operation = "nonviable";
                        subOp = "skip";
        ...
        else
        {
            ...
            for (auto gameId : gameIdList)
            {
                ...
                for (auto& improvedFactor : mGameTemplate.mFactors)
                {
                    ...
                    traceLog(TraceMode::ALGO, "findOptimalGame: passiveEval party(%" PRIi64 ") vs game(%" PRIi64 "), factor[%d](%s):  score poss(%.4f) vs old(%.4f)",
                        incomingParty.mPartyId, gameId, improvedFactor.mFactorIndex, improvedFactor.mFactorName.c_str(), futureScore, (improvedFactor.mFactorIndex >= 0 && game.mFactorScores.size() > (size_t)improvedFactor.mFactorIndex ? game.mFactorScores[improvedFactor.mFactorIndex] : 0));
...
void PackerSilo::assignPartyMembersToGame(const Party& incomingParty, Game& game)
{
    ...
    EA_ASSERT_MSG(assignedTeamIndex < MAX_TEAM_COUNT, "Internal error: Incoming party team index exceeds valid range");
    const auto gamePartyCount = game.getPartyCount(GameState::PRESENT);
    ...
    EA_ASSERT_MSG(gamePartyCount < MAX_PARTY_COUNT, "Internal error: Game player count exceeds max capacity");
    const PartyIndex newPartyIndex = gamePartyCount; // always last party
    ...
    EA_ASSERT_MSG(gamePlayerCount + incomingPlayerCount <= MAX_PLAYER_COUNT / MAX_TEAM_COUNT, "Assignment attempt exceeded team player capacity");
    ...
    EA_ASSERT_MSG(gamePlayerCount + incomingPlayerCount <= MAX_PLAYER_COUNT, "Assignment attempt exceeded game player capacity");
