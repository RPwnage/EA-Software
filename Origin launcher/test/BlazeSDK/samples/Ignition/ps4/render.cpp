...
static uint32_t s_barColor=0x80F50713; // bar color. in ARGB
...
static void *videoMalloc(int size)
{
    ...
    if (s_videoMemoryStackPointer>s_videoMemoryStackMax) return nullptr;
...
static int32_t initializeVideoOut(SceKernelEqueue *eqFlip)
{
    ...
    ret=sceVideoOutOpen(SCE_USER_SERVICE_USER_ID_SYSTEM, SCE_VIDEO_OUT_BUS_TYPE_MAIN, 0, nullptr);
    assert((ret>=0));
    ...
    ret=sceVideoOutAddFlipEvent(*eqFlip, videoOutHandle, nullptr);
    assert((ret>=0));
