...
{
    if (!(mAddress == address) || (mSecure != secure) || (secure && (mSslContext != gSslContextManager->get(sslContextName))))
    {
        char8_t buf[256];
        BLAZE_INFO_LOG(Log::HTTP, "[OutboundHttpConnectionManager:" << address.getHostname() <<"].initialize: " 
                       << address.toString(buf, sizeof(buf), InetAddress::HOST_PORT) <<", secure: " << (secure ? "yes" : "no"));
