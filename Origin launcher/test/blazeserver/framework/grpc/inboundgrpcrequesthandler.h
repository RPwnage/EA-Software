...
public:
    ...
    // setupGracefulShutdown is equivalent to Connection::gracefulClose. However, gracefulClose is a misnomer as that method does not really close the
    // connection. It sets the connection up for graceful shutdown if possible. We do the same thing here but name it correctly. 
    bool setupGracefulShutdown();
    void shutdown();
