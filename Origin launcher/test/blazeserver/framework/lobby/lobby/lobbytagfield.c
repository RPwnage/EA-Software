...
static unsigned char *_TagFieldSetupAppend(unsigned char *pRecord, int32_t iReclen, const char *pName, int32_t iMinData)
{
    ...
    for (; *pFind != 0; ++pFind)
    {
        ...
        if (*pFind == '=')
        {
            ...
            for (pNameCmp = pName; to_upper[*pFindCmp] == to_upper[*pNameCmp]; ++pFindCmp, ++pNameCmp)
                ;
            if (*pNameCmp == 0)
            {
                break;
...
static int32_t _TagFieldSetupCancel(unsigned char *pRecord, unsigned char *pAppend)
{
    ...
    logprintf(("lobbytagfield: tagfield buffer overflow appending record %s\n", pAppend));
...
/*!
    ...
    \Version 01/23/2000 (gschaefer)
    \Version 07/22/2004 (gschaefer) Fixed trailing tab problem
*/
/*************************************************************************************************F*/
int32_t TagFieldDelete(char *_record, const char *name)
{
    ...
    // find existing
    data = (unsigned char *) TagFieldFind(record, name);
    if (data == NULL)
        return(-1);
...
*/
/*************************************************************************************************F*/
int32_t TagFieldRename(char *_record, int32_t reclen, char *oldname, char *newname)
{
    int32_t diff;
    unsigned char *data;
    unsigned char *head;
    ...
    // find existing field
    data = (unsigned char *) TagFieldFind(record, oldname);
    if (data == NULL)
        return(-1);
...
/*!
    ...
    \Input *pRecord  - data record
    \Input  iReclen  - record length
    \Input *pMerge   - source record to merge from
...
*/
/*************************************************************************************************F*/
int32_t TagFieldMerge(char *pRecord_, int32_t iReclen, const char *pMerge_)
{
    unsigned char *pRecord = pRecord_;
    unsigned char *pLimit = pRecord+iReclen-1;
    const unsigned char *pMerge = pMerge_;
    unsigned char *pSrc;
    unsigned char *pDst;
    unsigned char *pName;
    ...
    for (pSrc = pDst = pName = pRecord; *pSrc != 0;)
    {
        ...
        *pSrc = 0;
        if (TagFieldFind(pMerge, pName) == NULL)
        {
            *pDst++ = *pSrc++ = '=';
            continue;
        }
    ...
    if (pDst >= pLimit-1)
    {
        ...
        *pDst = 0;
        ...
        {
            (void)_TagFieldSetupTerm(pRecord, pDst, "");
        }
        return(-1);
    }
...
/*!
    ...
    \Version 12/13/2004 (gschaefer) Rewrite of original
*/
/*************************************************************************************************F*/
const char *TagFieldFind(const char *pRecord_, const char *pName_)
{
    int32_t iNameLen;
    unsigned char uMatch = 0;
    const unsigned char *pCmp1;
    const unsigned char *pCmp2;
    const unsigned char *pName = pName_;
    const unsigned char *pRecord = pRecord_;
...
{
    char strName[256];
    const unsigned char *pFind = NULL;
    ...
    {
        strcpy(strName, pName1);
        strcat(strName, pName2);
...
const char *TagFieldFindIdx(const char *pRecord, const char *pName, int32_t iIdx)
{
    ...
    // build the tag
    snzprintf(strTag, sizeof(strTag), "%s%d", pName, iIdx);
    return (TagFieldFind(pRecord, strTag));
}

/*F*************************************************************************************************/
/*!
    \Function TagFieldFindBulk

    \Description
        Return pointer to bulk data area if present.

    \Input *pRecord - data record

    \Output const char * - Pointer to bulk data area or NULL

    \Version 01/23/2000 (gschaefer)
*/
/*************************************************************************************************F*/
const char *TagFieldFindBulk(const char *pRecord_)
{
    const unsigned char *pData;
    const unsigned char *pRecord = pRecord_;

    // search for the bulk data area
    for (pData = pRecord; *pData != 0; ++pData)
    {
        if ((*pData == '=') && ((pData == pRecord) || (pData[-1] <= ' ')))
        {
            // skip past leading white space
            for (++pData; (*pData != 0) && (*pData <= ' '); ++pData)
                ;
            return(pData);
        }
    }
...
/*!
    ...
    \Input iReclen  - record length
    \Input *pName   - field name
    \Input *pData   - source record to copy from
...
*/
/*************************************************************************************************F*/
int32_t TagFieldSetRaw(char *pRecord_, int32_t iReclen, const char *pName, const char *pData_)
{
    unsigned char uTerm;
    unsigned char *pAppend;
    unsigned char *pRecord = pRecord_;
    const unsigned char *pData = pData_;

    // make sure data is valid
    if (pData == NULL)
    {
        TagFieldDelete(pRecord, pName);
        return(0);
    }
    ...
    // terminate the field
    return(_TagFieldSetupTerm(pRecord, pAppend, pName));
}
...
int32_t TagFieldSetNumber(char *pRecord, int32_t iReclen, const char *pName, int32_t iValue)
{
    ...
    if (iValue < 10)
    {
        ...
        else
        {
            ...
            pAppend = _TagFieldSetupAppend(pRecord, iReclen, pName, 1);
            if (pAppend == NULL)
            {
                return(-1);
            }
    ...
    *--pData = 0;
    ...
    iLength = strData+sizeof(strData)-pData-1;
    pAppend = _TagFieldSetupAppend(pRecord, iReclen, pName, iLength);
    if (pAppend == NULL)
    {
        return(-1);
    }
...
int32_t TagFieldSetFlags(char *pRecord, int32_t iReclen, const char *pName, int32_t iValue)
{
    const char *pFlags;
    unsigned char *pAppend;
    unsigned char *pLimit = pRecord+iReclen-1-g_divafter;
...
int32_t TagFieldSetAddress(char *pRecord, int32_t iReclen, const char *pName, uint32_t uAddr)
{
    ...
    *pData = 0;
    ...
    // figure out length and setup for data append
    pAppend = _TagFieldSetupAppend(pRecord, iReclen, pName, pData-strAddr);
    if (pAppend == NULL)
    {
        return(-1);
    }
...
int32_t TagFieldSetToken(char *pRecord, int32_t iReclen, const char *pName, int32_t iToken)
{
    ...
    *--pData = 0;
    ...
    pAppend = _TagFieldSetupAppend(pRecord, iReclen, pName, strToken+sizeof(strToken)-pData-1);
    if (pAppend == NULL)
    {
        return(-1);
    }
...
*/
/*************************************************************************************************F*/
int32_t TagFieldGetToken(const char *pData_, int32_t iDefault)
{
    int32_t iIndex;
    unsigned char uByte;
    uint32_t uToken = 0;
...
*/
/*************************************************************************************************F*/
int32_t TagFieldSetString(char *pRecord_, int32_t iReclen, const char *pName, const char *pValue_)
{
    int32_t iRemain;
    unsigned char *pAppend;
    const unsigned char *pSpace;
    unsigned char *pRecord = pRecord_;
    const unsigned char *pValue = pValue_;
    ...
    iRemain = (pRecord+iReclen-1-pAppend);
...
/*!
    ...
    \Input  iReclen  - record length
    \Input *pName    - field name
    \Input *pValue_  - start of binary data
...
*/
/*************************************************************************************************F*/
int32_t TagFieldSetBinary(char *pRecord, int32_t iReclen, const char *pName, const void *pValue_, int32_t iVallen)
{
    unsigned char *pAppend;
    const unsigned char *pValue = pValue_;
    ...
    // get the append point
    pAppend = _TagFieldSetupAppend(pRecord, iReclen, pName, iVallen*2+1);
    if (pAppend == NULL)
    {
        return(-1);
    }
...
*/
/*************************************************************************************************F*/
int32_t TagFieldSetBinary7(char *pRecord, int32_t iReclen, const char *pName, const void *pValue_, int32_t iVallen)
{
    int32_t iBits;
    uint32_t uBuffer;
    unsigned char *pAppend;
    ...
    // get the append point
    pAppend = _TagFieldSetupAppend(pRecord, iReclen, pName, (iVallen*8+6)/7+1);
    if (pAppend == NULL)
    {
        return(-1);
    }
    ...
    *pAppend++ = '^';
    ...
    {
        // merge above bits already in buffer
        uBuffer |= (*pValue++ << iBits);
        iBits += 8;
        // may remove up to 14 bits (6 old + 8 new)
        for (; iBits >= 7; iBits -= 7)
        {
            // binary7 data is high-bit encoded
            *pAppend++ = 0x80 | (uBuffer & 0x7f);
            uBuffer >>= 7;
        }
    }

    // store remaining bits if present
    if (iBits > 0)
    {
        *pAppend++ = 0x80 | uBuffer;
    }
...
*/
/*************************************************************************************************F*/
int32_t TagFieldGetBinary(const char *pData_, void *pBuffer_, int32_t iBuflen)
{
    int32_t iBits;
    uint32_t uBin7;
    const unsigned char *pBinary;
    const unsigned char *pData = pData_;
    unsigned char *pBuffer = pBuffer_;
    ...
    {
        // copy over 7-bit encoded data
        uBin7 = 0;
        iBits = 0;
        for (pBinary = pData+1; (*pBinary >= 0x80) && (pBuffer < pBufend); ++pBinary)
        {
            // add new bits above old bits
            uBin7 |= (*pBinary & 0x7f) << iBits;
            iBits += 7;
            if (iBits >= 8)
            {
                // store off 8 bits and shift remaining
    ...
    return(iBuflen-(pBufend-pBuffer));
}
...
int32_t TagFieldSetStructure(char *pRecord, int32_t iReclen, const char *pName, const void *pStruct_, int32_t iLength, const char *pPattern)
{
    ...
    const unsigned char *pLimit = pRecord+iReclen-2;
...
/*!
    ...
    \Input *data    - data record
    \Input *_buf    - struct target
    \Input len      - length of target buffer
    \Input *pat     - structure pattern
...
int32_t TagFieldGetStructure(const char *_pData, void *_pBuf, int32_t iLen, const char *pPat)
{
    ...
    iDecodeLen = pBuf - (unsigned char *)_pBuf;
...
/*!
    ...
    \Input iReclen  - record length
    \Input *pName   - field name
    \Input uEpoch_  - date in psuedo epoch format
...
int32_t TagFieldSetEpoch(char *pRecord, int32_t iReclen, const char *pName, uint32_t uEpoch)
{
    ...
    iLength = snzprintf(strDate, sizeof(strDate), "%d.%d.%d-%d:%02d:%02d", tm->tm_year+1900, tm->tm_mon+1,
...
uint32_t TagFieldGetEpoch(const char *pData, uint32_t uDefval)
{
    ...
    else if ((pData[0] >= '0') && (pData[0] <= '9'))
    {
        ...
        tm.tm_isdst = -1;
        pData = _ParseNumber(pData, &tm.tm_year, 4);
        if (((*pData < '0') || (*pData > '9')) && (*pData != '\0'))
            ++pData;
        pData = _ParseNumber(pData, &tm.tm_mon, 2);
        if (((*pData < '0') || (*pData > '9')) && (*pData != '\0'))
            ++pData;
        pData = _ParseNumber(pData, &tm.tm_mday, 2);
        if (((*pData < '0') || (*pData > '9')) && (*pData != '\0'))
            ++pData;
        pData = _ParseNumber(pData, &tm.tm_hour, 2);
        if (((*pData < '0') || (*pData > '9')) && (*pData != '\0'))
            ++pData;
        pData = _ParseNumber(pData, &tm.tm_min, 2);
        if (((*pData < '0') || (*pData > '9')) && (*pData != '\0'))
            ++pData;
        pData = _ParseNumber(pData, &tm.tm_sec, 2);
    ...
    if (uEpoch == 0)
    {
        uEpoch = (uDefval == 0) ? (uint32_t)timeinsecs() : uDefval;
    }
...
int32_t TagFieldSetFloat(char *pRecord, int32_t iReclen, const char *pName, float fValue)
{
    ...
    *pData2++ = '.';
    ...
    // get the append point for max-size date format
    pAppend = _TagFieldSetupAppend(pRecord, iReclen, pName, pData2-pData1);
    if (pAppend == NULL)
    {
        return(-1);
    }
    ...
    memcpy(pAppend, pData1, pData2-pData1);
...
int32_t TagFieldPrintf(char *_pRecord, int32_t iLength, const char *pFormat, ...)
{
    ...
    while (((uCh = *pFormat++) != 0) && (iLength > 1))
    {
        ...
        else if (uCh == '#')
        {
            ...
            iCount = snzprintf(pRecord, iLength, "%d", iNum);
            pRecord += iCount;
            iLength -= iCount;
        }
        ...
        else if (*pFormat != 0)
        {
            ...
            *pRecord = 0;
            ...
            if (uCh == 's')
            {
                ...
                iCount = TagFieldSetString(pRecord, iLength, NULL, pStr);
            }
            // handle decimal
            else if (uCh == 'd')
            {
                int32_t iNum = va_arg(args, int32_t);
                iCount = TagFieldSetNumber(pRecord, iLength, NULL, iNum);
            }
            // handle token
            else if (uCh == 't')
            {
                uint32_t uToken = va_arg(args, uint32_t);
                iCount = TagFieldSetToken(pRecord, iLength, NULL, uToken);
            }
            // handle address
            else if (uCh == 'a')
            {
                uint32_t uAddr = va_arg(args, uint32_t);
                iCount = TagFieldSetAddress(pRecord, iLength, NULL, uAddr);
            }
            // handle flags
            else if (uCh == 'f')
            {
                int32_t flags = va_arg(args, int32_t);
                iCount = TagFieldSetFlags(pRecord, iLength, NULL, flags);
            }
            // handle epoch
            else if (uCh == 'e')
            {
                uint32_t epoch = va_arg(args, uint32_t);
                iCount = TagFieldSetEpoch(pRecord, iLength, NULL, epoch);
            }
            // set input source
            else if (uCh == 'i')
            {
            ...
            else if (uCh == 'x')
            {
                ...
                iCount = TagFieldSetRaw(pRecord, iLength, NULL, pFind);
            }
            // handle record copy
            else if (uCh == 'r')
            {
...
int32_t TagFieldGetDelim(const char *data, char *buffer, int32_t buflen, const char *defval, int32_t index, int32_t delim)
{
    ...
    {
        len = 0;
        for (ptr = data; (*ptr != term) && (*ptr >= stop); ++ptr)
        {
            if ((ptr[0] == '%') && (ptr[1] >= ' ') && (ptr[2] >= ' '))
                ptr += 2;
            len += 1;
...
int32_t TagFieldFirst(const char *_record, char *nameptr, int32_t namelen)
{
    ...
    const unsigned char *record = _record;
...
{
    const unsigned char *pRec = pRecord;
