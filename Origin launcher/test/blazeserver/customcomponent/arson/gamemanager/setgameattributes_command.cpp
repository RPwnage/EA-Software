/*************************************************************************************************/
/*!
    \file   setgameattributes_command.cpp
...
#include "arson/rpc/arsonslave/setgameattributes_stub.h"
...
namespace Arson
{
class SetGameAttributesCommand : public SetGameAttributesCommandStub
{
public:
    SetGameAttributesCommand(
        Message* message, Blaze::Arson::SetGameAttributesRequest* request, ArsonSlaveImpl* componentImpl)
        : SetGameAttributesCommandStub(message, request),
        mComponent(componentImpl)
    {
    }

    ~SetGameAttributesCommand() override { }

private:
    // Not owned memory
    ArsonSlaveImpl* mComponent;

    SetGameAttributesCommandStub::Errors execute() override
    {
        ...
        BlazeRpcError err = gameManagerComponent->setGameAttributes(mRequest.getSetGameAttributesRequest());
...
DEFINE_SETGAMEATTRIBUTES_CREATE()

SetGameAttributesCommandStub::Errors SetGameAttributesCommand::arsonErrorFromGameManagerError(BlazeRpcError error)
{
    ...
    switch (error)
    {
        ...
        default:
        {
            ...
            TRACE_LOG("[SetGameAttributesCommand].arsonErrorFromGameManagerError: unexpected error(" << SbFormats::HexLower(error) << "): return as ERR_SYSTEM");
            result = ERR_SYSTEM;
            break;
        }
    };
