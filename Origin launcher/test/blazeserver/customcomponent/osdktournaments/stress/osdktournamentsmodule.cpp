...
void OSDKTournamentsModule::addPlayerToTournamentPlayerIdMap(BlazeId playerId, TournamentId tourneyId)
{
...
void OSDKTournamentsModule::removePlayerFromTournamentPlayerIdMap(BlazeId playerId, TournamentId tourneyId)
{
	OSDKTournamentsModule::PlayerBlazeIdSet * playerBlazeIdSet = mBlazeIdsByTournamentIdMap[tourneyId];
	if (playerBlazeIdSet)
	{
...
bool OSDKTournamentsModule::reservePlayersForPlaymatch(TournamentId tourneyId, BlazeId selfBlazeId, BlazeId & oppBlazeId)
{
...
void OSDKTournamentsInstance::onLogin(Blaze::BlazeRpcError result)
{
	...
	mUserId = getLogin()->getSessionInfo()->getBlazeUserId();
    ...
    switch (action)
    {
        ...
        case OSDKTournamentsModule::ACTION_GET_MEMBER_COUNTS:
        {
            ...
            if (!mTournamentIds.empty())
            {
                ...
                TournamentId tournamentId = mTournamentIds.at(index);
...
Blaze::BlazeRpcError OSDKTournamentsInstance::getTrophies()
{
    ...
    request.setMemberId(getLogin()->getSessionInfo()->getBlazeUserId());
