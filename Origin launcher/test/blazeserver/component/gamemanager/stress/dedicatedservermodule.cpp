...
BlazeRpcError DedicatedServerInstance::execute()
{
    ...
    if (mRole == ROLE_DSCREATOR)
    {
        if (getGameId() == 0)
        {
            ...
            result = actionCreateGame_DSCreator(createRequest);
...
***************************************************************************************************/
BlazeRpcError DedicatedServerInstance::actionCreateGame_DSCreator(const Blaze::GameManager::CreateGameRequest &createGameRequest)
{
    Blaze::GameManager::CreateGameResponse createGameResponse;
    BlazeRpcError result = internalCreateGame(createGameRequest, createGameResponse);
    if(result == Blaze::ERR_OK)
    {
