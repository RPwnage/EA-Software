...
@members
{
    ...
    void fixupClassMembers(HashTree classNode)
    {
        ...
        Boolean classGenerateProto = (Boolean)classSymbol.get("generateProto");
        boolean generateProto = classGenerateProto != null && classGenerateProto.booleanValue();
        
        if (members != null)
        {
            Set<String> memberSet = new TreeSet<String>();
            for (HashToken memberToken : members)
            {
                if (!memberSet.add(memberToken.get("Name").toString()))
                {
                    throw new NullPointerException("Class " + classNode.get("Name") + " has multiple members named '" + memberToken.get("Name") + "'.");
                }
            }
            ...
            for (int i = 0; i < members.size(); i++)
            {
                ...
                { 
                    ...
                    else if (((Boolean)classNode.get("IsUnion")) == false || (!tag.equals("VALU")))     // We allow Unions to have matching tags named "VALU" for B/C with redirector.
                    {
                ...
                //ensure that all tdf members of a tdf that has change tracking enabled also have change tracking enabled  //PROTOPLUGIN_TODO: remember to add this same check in for the plugin.
                if (trackChanges)
                {
                ...
                //ensure that all tdf members of a tdf that has gRPC proto generation enabled also have gRPC proto generation enabled
                if (generateProto)
                {
                    TdfSymbol memberSymbol = ((TdfSymbolRef)members.get(i).get("TypeRef")).getSymbol();
                    if (memberSymbol.isActualCategoryClass() || memberSymbol.isActualCategoryUnion())
                    {
                        Boolean memberGenerateProto = (Boolean)memberSymbol.get("generateProto");
                        if ((memberGenerateProto != null) && (!memberGenerateProto.booleanValue()))
                        {
                            throw new NullPointerException("Class " + classNode.get("Name") + " has gRPC proto generation enabled, but class declaration of member m" 
                                + members.get(i).get("Name") + " does not." + " Set attribute \'generateProto = true\' to the definition of " + memberSymbol.get("Name"));
                        }
                        else
                        {
                            memberSymbol.put("generateProto", true);
                        }
                    }
                }
            }
    ...
    boolean validateTimeValueInterval(String timeValueStr)
    {
        // the validation logic here should match the parsing logic in TimeValue::parseTimeInterval (see cpp/source/time.cpp)
        if (timeValueStr.isEmpty())
        {
            return false;
        }

        String[] timeComponents = timeValueStr.split(":");
        for (int i = 0; i < timeComponents.length; ++i)
        {
            // parse each time component
            String[] subComponents = timeComponents[i].split("(?=[ydhms])", 2);
            if (subComponents.length != 2)
            {
                // not a "value" and "unit type"
                return false;
            }

            // is the first part an integer value?
            try
            {
                Integer intVal = Integer.parseInt(subComponents[0]);
                if (intVal < 0 && i > 0)
                {
                    // only the first component can be negative
                    return false;
                }
            }
            catch (Exception e)
            ...
            // is the second part an applicable time unit type?
            if (!Arrays.asList("y", "d", "h", "m", "s", "ms").contains(subComponents[1]))
            {
                return false;
            }
        }
    ...
    boolean isZeroTime(String timeValueStr)
    {
        try
        {
            String[] timeComponents = timeValueStr.split(":");
            String[] subComponents = timeComponents[0].split("(?=[ydhms])", 2);
    ...
    void validateMemberDefault(TdfSymbol member)
    {
        ...
        if (defaultVal != null)
        {
            member.put("zerodefault", false);
            member.put("booldefault", false);
            ...
            switch (memberSymbol.getActualCategory())
            {
                case INT_PRIMITIVE:
                {
                    if (!(defaultVal instanceof Boolean) && !(defaultVal instanceof Integer))
                    {
                        ...
                        if (defaultVal instanceof TdfSymbolRef)
                        {
                            ...
                            if (isValid && defaultSymbol.get("Value") != null)
                            {
                                if (defaultSymbol.get("Value").toString().equals("0"))
                                {
                                    member.put("zerodefault", true);
                                }
                            }
                        }
                        else
                    ...
                    {
                        member.put("booldefault", true);
                        boolean asBool = ((Boolean) defaultVal).booleanValue();
                        if (!asBool)
                        {
                            member.put("zerodefault", true);
                        }
                ...
                case FLOAT_PRIMITIVE:
                {
                    if (!(defaultVal instanceof Float))
                    {
                        ...
                        if (defaultVal instanceof TdfSymbolRef)
                        {
                            ...
                            {
                                if (defaultSymbol.get("Value").toString().equals("0")
                                || defaultSymbol.get("Value").toString().equals("0.0")
                                || defaultSymbol.get("Value").toString().equals("0.0f"))
                                {
                                    member.put("zerodefault", true);
                                }
                            }
                ...
                case ENUM:
                 {
                    ...
                    if (defaultVal instanceof TdfSymbolRef)
                    {
                        ...
                        if (isValid && defaultSymbol.get("MemberNumber") != null)
                        {
                            if (defaultSymbol.get("MemberNumber").toString().equals("0"))
                            {
                                member.put("zerodefault", true);
                            }
                        }
                ...
                case TIMEVALUE:
                {
                    ...
                    if (defaultVal instanceof String)
                    {
                        isValid = validateTimeValueInterval((String)defaultVal);
                        if (isValid && isZeroTime((String)defaultVal))
                        {
                            member.put("zerodefault", true);
                        }
                    }
    ...
    void checkUniqueIdsImpl(HashTree subNode, String collectionType)
    {
        ...
        if (nodesToCheck != null)
        {
            ...
            {
                Integer id = (Integer)nodesToCheck.get(i).get("id");
                ...
                {
                    throw new NullPointerException("Member " + nodesToCheck.get(i).get("Name") + " of " + collectionType + " in component " + subNode.get("Name") + " has duplicate id " + id);
                }                
    ...
    void fixupCommands(HashTree subNode)
    {
        ...
        if (commands != null)
        {
            ...
            {
                TdfSymbolRef request = (TdfSymbolRef)commands.get(i).get("RequestType");
                TdfSymbolRef response = (TdfSymbolRef)commands.get(i).get("ResponseType");
                TdfSymbolRef error = (TdfSymbolRef)commands.get(i).get("errortype");
        ...
        if (notifications != null)
        {
            ...
            {
                TdfSymbolRef notification = (TdfSymbolRef)notifications.get(i).get("NotificationType");                   
    ...
    {
        if (node != null && !allTypes.contains(node))
        {
            TdfSymbol symbolToAdd = node.getActualSymbol();
            if (symbolToAdd != null)
            {
                if (symbolToAdd.isActualCategoryClass() || symbolToAdd.isActualCategoryUnion())
                {
                    allTypes.add(node);

                    ArrayList<HashToken> children = (ArrayList<HashToken>)symbolToAdd.get("Members");
                    if (children != null)
                    {
                        for (int j = 0; j < children.size(); ++j)
                        {
                            TdfSymbolRef child = (TdfSymbolRef)((HashToken)children.get(j)).get("TypeRef");
                            if (child != null)
                            { 
                                ...
                                if (tempSymbol != null)
                                {
                                    ...
                                    {
                                        addSubTypes((TdfSymbolRef)tempSymbol.get("KeyTypeRef"), allTypes);
                                        addSubTypes((TdfSymbolRef)tempSymbol.get("ValueTypeRef"), allTypes);
                                    }
                ...
                else if (symbolToAdd.isActualCategoryMap())
                {
                    addSubTypes((TdfSymbolRef)symbolToAdd.get("KeyTypeRef"), allTypes);
                    addSubTypes((TdfSymbolRef)symbolToAdd.get("ValueTypeRef"), allTypes);
                }
                else if (symbolToAdd.isActualCategoryList())
                {
                    addSubTypes((TdfSymbolRef)symbolToAdd.get("ValueTypeRef"), allTypes);
                }
            }
    ...
    void calcComponentId(HashTree compNode)
    {
        ...
        // for proto generation, convert pair to string
        compNode.put("CompIdStr", idList.get(0) + "/" + idList.get(1));
    }
    ...
    {
        calcErrValue(errNode, 0);
    }
    void calcErrValue(HashTree errNode, Integer errFlag)
    {
        Integer compId = (Integer)getCurrentTokenOwner().get("CompId");
        Integer errVal = (Integer)errNode.get("Value");
                
        int fullVal = ((errVal.intValue() | errFlag)<< 16) + ((compId != null) ? compId.intValue() : 0);
    ...
    void fixupErrors(HashTree command)
    {
        ...
        if (errors != null)
        {
            ...
            ArrayList<HashToken> tokens = (ArrayList<HashToken>)getCurrentTokenOwner().get("Errors");
            ...
            if (tokens != null)
            {
                for (int i = 0; i < e.size(); ++i)
                {
                    ...
                    {
                        if (error.equals(tokens.get(j).get("Name")))
                        {
    ...
    void fixupHttp(HashTree command)
    {
        ...
        if (httpBlock != null)
        {
            ...
            {
                TdfSymbolRef responseType = (TdfSymbolRef)command.get("ResponseType");
                ...
                ArrayList<Long> responseTags = new ArrayList<Long>();
                ...
                if (responseTags.size() > 1)
                {
                    throw new NullPointerException("The responsePayloadMember '" + responsePayload + "' specified on command '" + commandName + "' does not support multiple tdf members");
                }
            ...
            {
                TdfSymbolRef requestType = (TdfSymbolRef)command.get("RequestType");
                ...
                ArrayList<Long> requestTags = new ArrayList<Long>();
                ...
                {
                    throw new NullPointerException("The requestPayloadMember '" + requestPayload + "' specified on command '" + commandName + "' does not support multiple tdf members");
                }
                ((TdfSymbol)command.getToken()).put("RequestPayloadMemberTags", requestTags);
            }

            String resource  = (String) httpBlock.get("resource");
            if (resource != null)
            {
                ...
                while (st.hasMoreElements())
                {
                    ...
                    if (currentSubstring.equals("{"))
                    {
                        if (st.hasMoreTokens())
                        {
                            ...
                            ArrayList<Long> tokenTags = new ArrayList<Long>();
            ...
            evaluateHttpTagFields(httpBlock, "custom_request_headers", "Custom request headers", commandName, (TdfSymbolRef) command.get("RequestType"), "request");
            evaluateHttpTagFields(httpBlock, "custom_response_headers", "Custom response headers", commandName, (TdfSymbolRef) command.get("ResponseType"), "response");
            evaluateHttpTagFields(httpBlock, "custom_error_headers", "Custom error headers", commandName, (TdfSymbolRef) command.get("errortype"), "error");
            evaluateHttpTagFields(httpBlock, "url_params", "Url params", commandName, (TdfSymbolRef) command.get("RequestType"), "request");

            ExtHashtable successCodes = (ExtHashtable)httpBlock.get("success_error_codes");
            if (successCodes != null)
            {
                ...
                {    
                    ArrayList<Long> codeTags = new ArrayList<Long>();
            ...
            ExtHashtable statusCodeErrorMap = (ExtHashtable) httpBlock.get("status_code_errors");
            if (statusCodeErrorMap != null)
            {
                ...
                {            
                    ...
                    for (int i = 0; i < s.size(); ++i)
                    {
                        ...
                        {
                            String error = (String) e.get(j).get("Name");
            ...
            httpBlock.putIfNotExists("compressPayload", false);
        }
    }
    ...
    {
        ExtHashtable headers = (ExtHashtable) httpBlock.get(attribName);
        if (headers != null)
        {
            if (baseTdf == null)
            {
            ...
            {    
                ArrayList<Long> headerTags = new ArrayList<Long>();
    ...
    boolean fetchPayloadTags(TdfSymbol rootSymbol, String payload, ArrayList<Long> responseTags)
    {
        String logString = "";
        ...
        ArrayList<Long> tempTags = new ArrayList<Long>();
            ...
            if ((members != null) && (members.size() > 0))
            {
                for (int j = 0; j < members.size(); ++j)
                {
                    ...
                    if (split[i].equalsIgnoreCase(memberName))
                    {
                        ...
                        tempTags.add((Long)currentMember.get("TagValue"));
                        if (i == (split.length - 1))
                        {
                            foundMatch = true;
...
/* ================================================================================
   ...
   ================================================================================*/
//TODO file params
file     [TdfSymbolTable symbolTable]
        @init { mSymbolTable = symbolTable; }
        :      file_def
        ;
...
const_block
        :       CONST
                {
                    addOwnedToken("Defs", $CONST);
                }
        |       STRING_CONST
                {
                    addSelfTyperef($STRING_CONST);
                    addOwnedToken("Defs", $STRING_CONST);
                }
        ;
...
root_component_members
    : ^(ERRORS (ERROR 
        { 
                ...
                calcErrValue($ERROR, 0x4000);       // These values come from rpcclienterrortypeheader.stg (BlazeSDK)
        } )*)
    |  ^(SDK_ERRORS (ERROR 
        {
                addOwnedToken("SdkErrors", $ERROR, 1);
                calcErrValue($ERROR, 0x8000); })*)  // These values come from rpcclienterrortypeheader.stg (BlazeSDK)
    ...
    | ^(ALLOC_GROUPS (ALLOC_GROUP 
        { 
                addOwnedToken("alloc_groups", $ALLOC_GROUP, 1); 
                calcErrValue($ALLOC_GROUP);
        } )*)
        ;
...
subcomponent_members
        : COMMAND
            {
                addOwnedToken("Commands", $COMMAND);
                fixupCommandDetails($COMMAND);
                fixupErrors($COMMAND);
                fixupHttp($COMMAND);
            }
        | NOTIFICATION
            { 
                addOwnedToken("Notifications", $NOTIFICATION);
                $NOTIFICATION.put("HandlerIndex", $NotificationIndexer::i++);
            }
        | EVENT
            {
                addOwnedToken("Events", $EVENT);
            }
        | DYNAMIC_MAP 
            {
...
alloc_groups_block
        : ^(ALLOC_GROUPS (ALLOC_GROUP 
        { 
                addOwnedToken("alloc_groups", $ALLOC_GROUP); 
                calcErrValue($ALLOC_GROUP);
        } )*)
        ;        
