...
bool TimeValue::parseDateTime(const char8_t* str, const char8_t* formats[], size_t numFormats, bool inGmt, bool prependsCurrentDateTime)
{
    ...
    for(size_t i = 0; i < numFormats; ++i)
    {
        ...
        if (s != nullptr)
        {
            ...
            // save the date time
            if (inGmt)
                mTime = mkgmTime(&timeInfo, true) * 1000 * 1000; // change to microseconds
            else
        ...
        uint32_t* second, uint32_t* millis)
{
    ...
    _localtime32_s(&time, &tval.tv_sec);
#elif EA_PLATFORM_LINUX
    localtime_r(&tval.tv_sec, &time);
#endif
    if(year)
        ...
        uint32_t* second, uint32_t* millis)
{
    ...
    struct tm time;
#ifdef EA_PLATFORM_WINDOWS
    _gmtime32_s(&time, &tval.tv_sec);
#elif EA_PLATFORM_LINUX
    gmtime_r(&tval.tv_sec, &time);
#endif
    if(year)
        *year = (uint32_t)(1900 + time.tm_year);
    if(month)
        ...
        uint32_t minute, uint32_t second)
{
    ...
    time_t result = mkgmTime(&time, true) * 1000 * 1000; // change to microseconds
...
*/
/*************************************************************************************************/
void TimeValue::gmTime(struct tm *tM, int64_t t)
{
#ifdef EA_PLATFORM_WINDOWS
    _gmtime64_s(tM, &t);
#elif EA_PLATFORM_LINUX
...
/*!
    ...
    \param[in]  - sourceIsGmt - if true, the source is gmt, and has no dst applied
    \return - 64 bit epoch time
*/
/*************************************************************************************************/
int64_t TimeValue::mkgmTime(struct tm *tM, bool sourceIsGmt)
{
    tM->tm_wday = 0;
    tM->tm_yday = 0;
    tM->tm_isdst = sourceIsGmt ? 0 : -1;
...
int TimeValue::getLocalTimeString(char8_t* buffer, size_t len)
{
    ...
    printtimeoffset = EA::StdC::Snprintf(buffer, len, "%d/%02d/%02d-%02d:%02d:%02d.%03.3d ", 
...
#elif EA_PLATFORM_KETTLE
    SceKernelTimeval now;
    sceKernelGettimeofday( &now );

    char tsbuf[25];
    strftime(tsbuf, sizeof(tsbuf), "%Y/%m/%d-%H:%M:%S", localtime(&now.tv_sec));
    printtimeoffset =  EA::StdC::Snprintf(buffer, len, "%s.%03ld ", tsbuf, (long int)(now.tv_usec/1000));
#elif EA_PLATFORM_WINDOWS
    //build timestamp for log
    struct __timeb64 timebuffer;
    _ftime64_s( &timebuffer );

    char tsbuf[25];

    strftime(tsbuf, sizeof(tsbuf), "%Y/%m/%d-%H:%M:%S", _localtime64( & ( timebuffer.time ) ) );
    // FIXME: the milltm only provide sub second info. but it is no where high resolution. 
    // need research on the high precision routine in windows. see: QueryPerformanceCounter mmsystem.h
    printtimeoffset = EA::StdC::Snprintf(buffer, len, "%s.%03.3d ", tsbuf, timebuffer.millitm);
#elif EA_PLATFORM_LINUX
    timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);

    tm timerec;
    localtime_r(&ts.tv_sec, &timerec);

    char tsbuf[25];
    strftime(tsbuf, sizeof(tsbuf), "%Y/%m/%d-%H:%M:%S", &timerec);

    printtimeoffset = EA::StdC::Snprintf(buffer, len, "%s.%03ld ", tsbuf, ts.tv_nsec/1000000);
#endif
