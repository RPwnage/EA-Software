...
//! is the metrics module enabled
#define BUFFEREDSOCKET_METRICS_ENABLED  (FALSE)

//! how often we query our metrics
#define BUFFEREDSOCKET_METRICS_RATE     (1000)
...
struct BufferedSocketT
{
    ...
    //! metrics socket fd
    int32_t iTcpMetricsSocket;

    //! last time we updated metrics
    uint32_t uLastMetricsUpdate;
...
BufferedSocketT *BufferedSocketOpen(int32_t iType, uint16_t uPort, int32_t iBufLen, SocketT *pSocket)
{
    ...
    // update the port based on what was provided or bound. the passed in port might have been zero to bind a random port
    ...
    #if BUFFEREDSOCKET_METRICS_ENABLED
    // open the metrics socket
    pBufferedSocket->iTcpMetricsSocket = TcpMetricsSocketOpen();
    #else
    // mark the metrics socket as closed
    pBufferedSocket->iTcpMetricsSocket = -1;
    #endif
...
{
    // close the metrics socket
    if (pBufferedSocket->iTcpMetricsSocket >= 0)
    {
        TcpMetricsSocketClose(pBufferedSocket->iTcpMetricsSocket);
        pBufferedSocket->iTcpMetricsSocket = -1;
    }
...
int32_t BufferedSocketFlush(BufferedSocketT *pBufferedSocket)
{
    ...
    if ((pBufferedSocket->iTcpMetricsSocket >= 0) && (NetTickDiff(uCurTick, pBufferedSocket->uLastMetricsUpdate) > BUFFEREDSOCKET_METRICS_RATE))
    {
        TcpMetricsT TcpMetrics;
        ds_memclr(&TcpMetrics, sizeof(TcpMetrics));

        // update our metrics update tick
        pBufferedSocket->uLastMetricsUpdate = uCurTick;

        // recv data from the last query and kick off new request
        TcpMetricsQueryRecv(pBufferedSocket->iTcpMetricsSocket, &TcpMetrics);
        TcpMetricsQuerySend(pBufferedSocket->iTcpMetricsSocket, pBufferedSocket->uPort);

        // save the values
        pBufferedSocket->BufferedSocketInfo.uRcvQueue = TcpMetrics.uRecvQueueLen;
        pBufferedSocket->BufferedSocketInfo.uSndQueue = TcpMetrics.uSendQueueLen;
