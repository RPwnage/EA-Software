...
static void _VoipServerAttemptFQDNLookup(VoipServerRefT *pVoipServer)
{
    ...
    else
    {
        ...
        if ((iResult = DirtyCastGetHostNameByAddr(&Addr, sizeof(Addr), pVoipServer->strHostname, sizeof(pVoipServer->strHostname), NULL, 0)) == 0)
        {
...
static void _VoipServerUpdateMetrics(VoipServerRefT *pVoipServer, uint32_t uCurTick)
{
    ...
    {
        UdpMetricsQueryRecv(pVoipServer->iUdpMetricsSocket, &pVoipServer->UdpMetrics);
        UdpMetricsQuerySend(pVoipServer->iUdpMetricsSocket, uPort);
    }
...
uint8_t VoipServerUpdate(VoipServerRefT *pVoipServer, uint32_t *pSignalFlags)
{
    ...
    {
        DirtyCastLogPrintfVerbose(pVoipServer->Config.uDebugLevel, 0, "voipserver: main loop took %dms\n", uLoopTick);
    }
