...
//! data structure used to save time info used to calculate xmit delay for a given sync packet
typedef struct {
    uint32_t uSendTime;  //!< remote timestamp applied by sender
    uint32_t uRecvTime;  //!< local timestamp applied in reception path
    uint32_t bValid;
} XmitDelaySampleT;
...
struct NetGameLinkRefT
{
    ...
    //! accumulator used to calculate jitter at end of each sampling interval
    struct XmitDelaySampleT {
        XmitDelaySampleT DelaySample1;
        XmitDelaySampleT DelaySample2;
    } JitterAccumulator;
...
/*F*************************************************************************************************/
/*!
    \Function    _NetGameLinkCalculateJitter

    \Description
        Calculate jitter. 

    \Input *pRef        - reference pointer
    \Input uSendTime    - timestamp applied by remote sender
    \Input uRecvTime    - timestamp applied by local reception path

    \Notes
        Algorithm used:
         * For each timestamped packet received during that interval, D(i): one-way delay is calculated and stored.
         * Every time D(i) is calculated, if that new D(i) is identified to be the new minimum (Dmin)
           or the new maximum (Dmax) for the interval, then the variables used to store the Dmin 
           and Dmax are updated accordingly.
         * At the end of the sampling interval, Jitter Summary Metric is calculated as Dmax - Dmin.
         * This approach uses a short interval and a small collection of samples to 
           derive a jitter metric that will model instantaneous variation of jitter known as 
           'transient jitter'. 
         * This approach is documented as Type-P-One-way-peak-to-peak-ipdv in Section 4.6 of RFC 3393. 
           It is known as a moving range statistics on an interval.

        Implementation:
            At the moment, only sync packets exchanged twice a second can contribute because
            they are the only packets timestamped with the timing info needed for delay variation calculation.

            2 packets per second when using a 1-sec interval (inspired from RTT_SAMPLE_PERIOD used
            to calculate other network throughput stats) implicitly means that we can simplify the
            implementation to calculate delay variation every 2 sync packets using timing info
            from those 2 last sync packets.

    \Version 06/26/20 (mclouatre)
*/
/*************************************************************************************************F*/
static void _NetGameLinkCalculateJitter(NetGameLinkRefT* pRef, uint32_t uSendTime, uint32_t uRecvTime)
{
    XmitDelaySampleT* pDelay1 = &pRef->JitterAccumulator.DelaySample1;
    XmitDelaySampleT* pDelay2 = &pRef->JitterAccumulator.DelaySample2;

    if (pDelay1->bValid == FALSE)
    {
        pDelay1->uSendTime = uSendTime;
        pDelay1->uRecvTime = uRecvTime;
        pDelay1->bValid = TRUE;
    }
    else if (pDelay2->bValid == FALSE)
    {
        pDelay2->uSendTime = uSendTime;
        pDelay2->uRecvTime = uRecvTime;
        pDelay2->bValid = TRUE;

        /* Calculate inter-sync-pkt delay variation
           Because jitter is a differential measurement, it is not very sensitive to sender's clock and receiver's clock being de-synchronized.
           'delay variation' can be calculated even if both ends of the connection don't have synchronized clocks.

           Delay variation associated with current sync packet =
              Delay(current) - Delay(previous)
              (RecvTimestamp_current - SendTimestamp_current) - (RecvTimestamp_previous - SendTimestamp_previous)
              RecvTimestamp_current - RecvTimestamp_previous - SendTimestamp_current + SendTimestamp_previous
              (RecvTimestamp_current - RecvTimestamp_previous) - (SendTimestamp_current - SendTimestamp_previous)  */
        pRef->NetGameLinkStats.jitter = abs(NetTickDiff(pDelay2->uRecvTime, pDelay1->uRecvTime) - NetTickDiff(pDelay2->uSendTime, pDelay1->uSendTime));

        // reset accumulator
        ds_memclr(&pRef->JitterAccumulator, sizeof(pRef->JitterAccumulator));
    }
}
...
static int32_t _NetGameLinkRecvPacket0(NetGameLinkRefT *pRef, uint32_t uCurrTick)
{
    ...
    if (pPacket->head.kind & GAME_PACKET_SYNC)
    {
        ...
        if ((Sync.flags & NETGAME_SYNCFLAG_REPLINVALID) == 0)
        {
            ...
            iDelta = (int16_t)(NetTickDiff(uWhen, SocketNtohl(Sync.repl)) + 1);
        }
        else
        {
        ...
        if ((iDelta >= 0) && (iDelta <= 2500))
        {
            ...
            int32_t iElapsed = NetTickDiff(uWhen, pRef->uLastAvg);
            if (iElapsed < 10)
            {
                iElapsed = 10;
            }
        ...
        _NetGameLinkCalculateJitter(pRef, SocketNtohl(Sync.echo), uWhen);
...
static void _NetGameLinkProcess(NetGameLinkRefT *pRef, uint32_t uCurrTick)
{
    ...
    pRef->NetGameLinkStats.tick = uCurrTick;
    pRef->NetGameLinkStats.tickseqn += 1;
    uRange = (unsigned)NetTickDiff(uCurrTick, pRef->NetGameLinkStats.stattick);
    if (uRange >= 1000)
    {
...
static void _NetGameLinkUpdateQos(NetGameLinkRefT *pRef)
{
    ...
    if ((pRef->uQosPacketsToSend > 0) && (pRef->uQosLastSendTime == 0))
    {
        ...
        pRef->uQosLastSendTime = (unsigned)NetTickDiff(uNow, (pRef->uQosSendInterval + 1));
    }
