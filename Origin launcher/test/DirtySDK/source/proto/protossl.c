...
#define SSL_SIGDATA_MAX     (512)                           //!< max signature message data
...
#define SSL_CERTDATA_MAX    (1024)                          //!< amount of data we can save for validating the certificate signature

// min/default/max protocol versions supported
#define SSL3_VERSION_MIN        (PROTOSSL_VERSION_TLS1_0)
#define SSL3_VERSION            (PROTOSSL_VERSION_TLS1_3)
...
// signature algorithm ids; ref: https://www.iana.org/assignments/tls-parameters/tls-parameters.xhtml#tls-parameters-16
#define SSL3_SIGALG_NONE        (0)
#define SSL3_SIGALG_RSA         (1)
#define SSL3_SIGALG_DSA         (2)
#define SSL3_SIGALG_ECDSA       (3)
#define SSL3_SIGALG_ED25519     (7)
...
// EdDSA algorithms
#define SSL3_SIGSCHEME_EDDSA_ED25519            (0x0807)
...
#define SSL_EXTN_EC_POINT_FORMATS       (0x000b)    //!< identifier for elliptic curves point formats extension
...
typedef struct X509CertificateT
{
    ...
    uint8_t HashData[SSL_CERTDATA_MAX]; //!< holds either the cert data hash (non-eddsa) or raw cert data (eddsa)
} X509CertificateT;
...
typedef struct SignatureVerifyT
{
    ...
    int32_t iSigSize;
    uint8_t aHashDigest[SSL_SIGDATA_MAX];   //!< will be the raw message data in the case of eddsa signatures
    int32_t iHashSize;
    CryptHashTypeE eHashType;
...
struct ProtoSSLRefT
{
    ...
    char strPrivateKeyPass[128];    //!< private key password (if encrypted)
...
static const SignatureSchemeT _SSL3_SignatureSchemes[] =
{
    ...
    { SSL3_SIGSCHEME_EDDSA_ED25519,       { SSL3_HASHID_NONE,   SSL3_SIGALG_ED25519 }, SSL3_SIGVERIFY_NONE,    ASN_OBJ_EDDSA_ED25519   },
...
static const CertificateSignatureT _SSL3_CertSignatures[] =
{
    ...
    { "-----BEGIN PRIVATE KEY-----",      "-----END PRIVATE KEY-----",      ASN_OBJ_NONE         },     // unknown type; see https://tools.ietf.org/html/rfc5958
...
/* ASN object identification table.
   for more info on the oid binary representation see: https://docs.microsoft.com/en-us/windows/win32/seccertenroll/about-object-identifier?redirectedfrom=MSDN */
static const struct ASNObjectT _SSL_ObjectList[] =
{
    ...
    // id-Ed25519 1.3.101.112; https://tools.ietf.org/html/rfc8410#section-3
    { ASN_OBJ_EDDSA_ED25519, 3, { 0x2b, 0x65, 0x70 } },
...
static const uint8_t *_AsnParseIdent(const uint8_t *pData, int32_t iSize, ProtoSSLCertIdentT *pIdent)
{
    ...
    for (iObjType = 0, pDataEnd = pData+iSize; (pData = _AsnParseHeader(pData, pDataEnd, &iType, &iSize)) != NULL; )
    {
        ...
        if ((iType == ASN_TYPE_PRINTSTR+ASN_PRIMITIVE) || (iType == ASN_TYPE_UTF8STR+ASN_PRIMITIVE) || (iType == ASN_TYPE_T61+ASN_PRIMITIVE) || (iType == ASN_TYPE_UNICODESTR+ASN_PRIMITIVE))
        {
            ...
            if ((iObjType == ASN_OBJ_UNIT) && ((iType != ASN_TYPE_UNICODESTR+ASN_PRIMITIVE)))
            {
...
static const uint8_t *_AsnParseSignatureAlgorithms(const uint8_t *pData, const uint8_t *pLast, X509CertificateT *pCert)
{
    ...
    else if (pCert->iSigType == ASN_OBJ_EDDSA_ED25519)
    {
        pCert->iSigHash = ASN_OBJ_NONE;
        return(pData);
    }
...
static int32_t _AsnParseCertificate(X509CertificateT *pCert, const uint8_t *pData, int32_t iSize)
{
    ...
    else if (pCert->iKeyType == ASN_OBJ_EDDSA_ED25519)
    {
        pCert->iCrvType = pCert->iKeyType;
    }
    ...
    if ((pCert->iSigType == ASN_OBJ_ECDSA_SHA256) || (pCert->iSigType == ASN_OBJ_ECDSA_SHA384) || (pCert->iSigType == ASN_OBJ_ECDSA_SHA512) || (pCert->iSigType == ASN_OBJ_EDDSA_ED25519))
    {
    ...
    else if ((pCert->iKeyType == ASN_OBJ_ECDSA_KEY) || (pCert->iKeyType == ASN_OBJ_EDDSA_ED25519))
    {
    ...
    {
        ds_memcpy_s(pCert->HashData, sizeof(pCert->HashData), pInfData, pCert->iHashSize = (int32_t)(pInfSkip - pInfData));
    }
    else
    {
...
/*F********************************************************************************/
/*!
    \Function _AsnDecryptPrivateKey

    \Description
        Decrypt a base64 encoded private key

    \Input iKeyLen          - size of the key (16, 32)
    \Input *pPass           - password used to decrypt
    \Input *pIv             - IV data from the private key DEK-Info
    \Input *pEncryptedDer   - [in/out] the encrypted DER encoded data that we decrypt
    \Input *pDerSize        - [in/out] size of the encrypted data

    \Output
        int32_t             - TRUE=success, FALSE=failure

    \Notes
        If the input parameters are invalid for decryption, we assume that the data
        is already unencrypted and return success.

        On successful decryption, the padding is removed from the clear text as it has
        already been validated within this fuction.

    \Version 10/16/2019 (eesponda)
*/
/********************************************************************************F*/
static int32_t _AsnDecryptPrivateKey(int32_t iKeyLen, const char *pPass, const uint8_t *pIv, uint8_t *pEncryptedDer, int32_t *pDerSize)
{
    CryptAesT Aes;
    CryptMD5T MD5;
    uint8_t aKey[32];
    int32_t iPadSize, iPad, iPadStart, iBytesWritten;

    // validate the inputs to make sure we want to try to decrypt, we return TRUE as it can be unencrypted
    if ((iKeyLen == 0) || (pPass == NULL) || (strlen(pPass) == 0))
    {
        return(TRUE);
    }
    ...
    CryptAesDecrypt(&Aes, pEncryptedDer, *pDerSize);

    // verify the padding, remove padding if successful
    iPadSize = pEncryptedDer[*pDerSize-1];
    for (iPad = 0, iPadStart = *pDerSize-iPadSize; iPad < iPadSize; iPad += 1)
    {
        if (pEncryptedDer[iPadStart+iPad] == iPadSize)
        {
            continue;
        }
        return(FALSE);
...
/*!
    ...
    \Input *pPass           - the private key password for decryption
    \Input *pIv             - the IV used for decryption
    \Input iKeyLen          - the AES keylen used for decryption
...
*/
/********************************************************************************F*/
static int32_t _AsnParsePrivateKeyRSA(const char *pPrivateKeyData, int32_t iPrivateKeyLen, const char *pPass, const uint8_t *pIv, int32_t iKeyLen, X509PrivateKeyT *pPrivateKey)
{
    ...
    if (!_AsnDecryptPrivateKey(iKeyLen, pPass, pIv, (uint8_t *)pPrivateKey->strPrivKeyData, &iPrivKeySize))
    {
        NetPrintf(("protossl: _AsnParsePrivateKey: decryption of the private key failed\n"));
        return(-3);
    }
    // parse the sequence
...
*/
/********************************************************************************F*/
static int32_t _AsnParsePrivateKeyEcdsa(const char *pPrivateKeyData, int32_t iPrivateKeyLen, const char *pPass, const uint8_t *pIv, int32_t iKeyLen, X509PrivateKeyT *pPrivateKey)
{
...
/*F********************************************************************************/
/*!
    \Function _AsnParsePrivateKeyEddsa

    \Description
        Parse the private key from eddsa private key certificate.
    ...
    \Notes
        ...
        ref: 
            ...
            https://tools.ietf.org/html/rfc8410#section-7
    ...
    \Version 06/17/2020 (eesponda)
*/
/********************************************************************************F*/
static int32_t _AsnParsePrivateKeyEddsa(const char* pPrivateKeyData, int32_t iPrivateKeyLen, const char* pPass, const uint8_t* pIv, int32_t iKeyLen, X509PrivateKeyT* pPrivateKey)
{
    const uint8_t* pData, * pData2;
    int32_t iSize, iSize2, iPrivKeySize, iTemp;
    ...
    // parse the key package data to get the type
    if ((pData2 = _AsnParseHeaderType(pData, pData + iSize, ASN_TYPE_SEQN + ASN_CONSTRUCT, &iSize2)) == NULL)
    {
        return(-6);
    }
    ...
    if (_AsnParseObject(pData, iSize2) != ASN_OBJ_EDDSA_ED25519)
    {
        return(-8);
    }
    pData += iSize2;
    ...
    // validate the uncompressed marker
    if (*pData != 0x04)
    {
        return(-10);
    }
    ...
    // read the size out
    pPrivateKey->Modulus.iObjSize = *pData;
    pData += 1;
    iSize -= 1;

    // validate the size is what we expect
    if ((pPrivateKey->Modulus.iObjSize != iSize) || (pPrivateKey->Modulus.iObjSize != 32))
    {
        return(-11);
    }

    // save the pointer to the data
    pPrivateKey->Modulus.pObjData = (uint8_t *)pData;
    return(iPrivKeySize);
}
...
static int32_t _CertificateFindData(const uint8_t *pCertData, int32_t iCertSize, const uint8_t **pCertBeg, const uint8_t **pCertEnd, uint32_t *pCertType)
{
    ...
    for (iCertSig = 0; iCertSig < (signed)(sizeof(_SSL3_CertSignatures)/sizeof(_SSL3_CertSignatures[0])); iCertSig += 1)
    {
        ...
        {
            // skip begin signature and newline
            *pCertBeg += strlen(pCertSig->pCertBeg) + 1;
            // write cert type
...
/*F********************************************************************************/
/*!
    \Function _CertificateParseDekInfo

    \Description
        Parsed the DEK-Info field that shows up in PEM encoded encrypted private key
        files

    \Input **pPrivateKeyData    - [in/out] private key certificate data in base64 form
    \Input iPrivateKeyLen       - length of private key certificate data
    \Input *pIv                 - [out] IV parsed from the DEK-Info
    \Input iIvLen               - length of IV buffer
    \Input *pKeySize            - the key size for AES decryption

    \Output
        int32_t                 - length of the data pointed to by pPrivateKeyData

    \Notes
        After parsing this data successfully all that is left to parse is the base64 encoded
        that we need to decrypt. If we don't successfully parse the DEK-Info the private key
        data will continue to point to the data that was initially passed into the function.

    \Version 10/16/2019 (eesponda)
*/
/********************************************************************************F*/
static int32_t _CertificateParseDekInfo(const char **pPrivateKeyData, int32_t iPrivateKeyLen, uint8_t *pIv, int32_t iIvLen, int32_t *pKeySize)
{
    static const char *_pDekInfo = "DEK-Info: ";
    const char *pStart = *pPrivateKeyData, *pEnd;
    char strCipher[64] = { 0 };
    int32_t iResult;

    // validate the iv is the correct size we expect
    if (iIvLen < 16)
    {
        NetPrintf(("protossl: invalid iv output passed in, must at least be 16 bytes long\n"));
        return(iPrivateKeyLen);
    }

    // make sure this is an encrypted key
    if ((pStart = strstr(pStart, _pDekInfo)) == NULL)
    {
        return(iPrivateKeyLen);
    }
    // skip over the dek-info label and find the end of the cipher
    pStart += strlen(_pDekInfo);
    if ((pEnd = strstr(pStart, ",")) == NULL)
    {
        return(iPrivateKeyLen);
    }

    // copy the cipher information
    ds_strsubzcpy(strCipher, sizeof(strCipher), pStart, (int32_t)(pEnd - pStart));

    // check the cipher and update the key size
    if (strcmp(strCipher, "AES-128-CBC") == 0)
    {
        *pKeySize = 16;
    }
    else if (strcmp(strCipher, "AES-256-CBC") == 0)
    {
        *pKeySize = 32;
    }
    else
    ...
    // update the positions to parse the IV
    pStart = pEnd + 1;
    ...
    // parse the octstring converting to the hexidecimal value
    ds_fmtoctstring2(pIv, iIvLen, pStart, (int32_t)(pEnd - pStart));

    // return the new location (skipping the 2 newlines)
    iResult = (int32_t)((pEnd + 2) - *pPrivateKeyData);
    *pPrivateKeyData = pEnd + 2;
    return(iPrivateKeyLen - iResult);
}

/*F********************************************************************************/
/*!
    \Function _CertificateDecodeKeyType

    \Description
        Figures out the private key algorithm using the OneAsymmetricKey sequence

    \Input *pCertBeg        - private key certificate data in base64 form
    \Input iCertSize        - length of private key certificate data
    \Input *pPrivateKeyPass - the private key password for decryption
    \Input *pIv             - the IV used for decryption
    \Input iKeySize         - the AES keylen used for decryption

    \Output
        int32_t             - ASN_OBJ_* or negative on error

    \Notes
    
        ref: https://tools.ietf.org/html/rfc5958
...
*/
/********************************************************************************F*/
static int32_t _CertificateDecodeKeyType(const char *pCertBeg, int32_t iCertSize, const char *pPrivateKeyPass, const uint8_t *pIv, int32_t iKeySize)
{
    ...
    // base64 decode into buffer
    if ((iPrivKeySize = Base64Decode3(pCertBeg, iCertSize, PrivateKey.strPrivKeyData, sizeof(PrivateKey.strPrivKeyData))) == 0)
    {
        NetPrintf(("protossl: _CertificateDecodeKeyType: could not decode private key\n"));
        return(-2);
    }
    // try to decrypt if encrypted
    if (!_AsnDecryptPrivateKey(iKeySize, pPrivateKeyPass, pIv, (uint8_t *)PrivateKey.strPrivKeyData, &iPrivKeySize))
    {
        NetPrintf(("protossl: _CertificateDecodeKeyType: decryption of the private key failed\n"));
        return(-3);
    }
    // parse the sequence
    if ((pData = _AsnParseHeaderType((uint8_t *)PrivateKey.strPrivKeyData, (uint8_t *)PrivateKey.strPrivKeyData+iPrivKeySize, ASN_TYPE_SEQN+ASN_CONSTRUCT, &iSize)) == NULL)
    {
        NetPrintf(("protossl: _CertificateDecodeKeyType: could not get private key sequence\n"));
        return(-4);
    }
    // skip the version
    if ((pData = _AsnParseBinary(pData, pData+iSize, ASN_TYPE_INTEGER+ASN_PRIMITIVE, NULL, 0, &iTemp, "version")) == NULL)
    ...
    return(_AsnParseObject(pData, iSize2));
}
...
/*!
    ...
    \Input *pPrivateKeyData - private key certificate data in base64 form
    \Input iPrivateKeyLen   - length of private key certificate data
    \Input *pPrivateKeyPass - the configured private key password we use to decrypt with
    \Input *pPrivateKey     - [out] parsed private key data
...
*/
/********************************************************************************F*/
static int32_t _CertificateDecodePrivate(const char *pPrivateKeyData, int32_t iPrivateKeyLen, const char *pPrivateKeyPass, X509PrivateKeyT *pPrivateKey)
{
    const uint8_t *pCertBeg, *pCertEnd;
    uint8_t aIv[16];
    int32_t iCertSize, iResult, iKeySize = 0;
    uint32_t uCertType;
    ...
    // attempt to parse the decryption information if it exists in the PEM
    iCertSize = _CertificateParseDekInfo((const char **)&pCertBeg, iCertSize, aIv, sizeof(aIv), &iKeySize);

    // if this is the new asymmetric key package, parse out the type
    if (uCertType == ASN_OBJ_NONE)
    {
        uCertType = _CertificateDecodeKeyType((const char *)pCertBeg, iCertSize, pPrivateKeyPass, aIv, iKeySize);
    }

    // parse ecdsa or rsa?
    if (uCertType == ASN_OBJ_ECDSA_KEY)
    {
        iResult = _AsnParsePrivateKeyEcdsa((const char *)pCertBeg, iCertSize, pPrivateKeyPass, aIv, iKeySize, pPrivateKey);
    }
    else if (uCertType == ASN_OBJ_EDDSA_ED25519)
    {
        iResult = _AsnParsePrivateKeyEddsa((const char *)pCertBeg, iCertSize, pPrivateKeyPass, aIv, iKeySize, pPrivateKey);
    }
    else
    {
        iResult = _AsnParsePrivateKeyRSA((const char *)pCertBeg, iCertSize, pPrivateKeyPass, aIv, iKeySize, pPrivateKey);
    }
    return(iResult);
}
...
/*F********************************************************************************/
/*!
    \Function _CertificateCopyInfo

    \Description
        Copy the certificate info from the X509CertificateT structure

    \Input *pCert       - source data for the certificate info
    \Input *pCertInfo   - destination where we store the public facing certificate info

    \Version 10/28/2020 (eesponda)
*/
/********************************************************************************F*/
static void _CertificateCopyInfo(const X509CertificateT *pCert, ProtoSSLCertInfoT *pCertInfo)
{
    ds_memcpy_s(&pCertInfo->Subject, sizeof(pCertInfo->Subject), &pCert->Subject, sizeof(pCert->Subject));
    ds_memcpy_s(&pCertInfo->Issuer, sizeof(pCertInfo->Issuer), &pCert->Issuer, sizeof(pCert->Issuer));
    pCertInfo->uGoodFrom = pCert->uGoodFrom;
    pCertInfo->uGoodTill = pCert->uGoodTill;
    pCertInfo->iKeyModSize = pCert->iSigSize; // CACert::KeyModSize == Cert::SigSize
}
...
static void _CertificateSetFailureInfo(ProtoSSLRefT *pState, X509CertificateT *pCert, uint8_t bIssuer)
{
    if ((pState != NULL) && (pCert != NULL) && (pState->bCertInfoSet == FALSE))
    {
        ...
        _CertificateCopyInfo(pCert, &pState->CertInfo);
...
static uint32_t _CertificateGetSigAlg(const CertificateDataT *pCertificate)
{
    ...
    {
        if (pCertificate->iKeyType == ASN_OBJ_ECDSA_KEY)
        {
            uCertSigAlg = SSL3_SIGALG_ECDSA;
        }
        else if (pCertificate->iKeyType == ASN_OBJ_EDDSA_ED25519)
        {
            uCertSigAlg = SSL3_SIGALG_ED25519;
        }
        else
        {
            uCertSigAlg = SSL3_SIGALG_RSA;
        }
    }
    return(uCertSigAlg);
}
...
static const CryptCurveDsaT *_ProtoSSLDsaInitContext(SignatureVerifyT *pSigVerify, int32_t iCrvType)
{
    ...
    else if (iCrvType == ASN_OBJ_EDDSA_ED25519)
    {
        iCurveIdent = CRYPTCURVE_X25519;
    }
...
/*F********************************************************************************/
/*!
    \Function _ProtoSSLGenerateSignatureEddsa
...
*/
/********************************************************************************F*/
static int32_t _ProtoSSLGenerateSignatureEddsa(SecureStateT* pSecure, const uint8_t* pSigData, int32_t iSigSize, const uint8_t* pPrivateKey, int32_t iKeySize, int32_t iCrvType)
{
    CryptEccPointT Signature;
    int32_t iResult;
    CryptBnT PrivKey;
    ...
    if ((iResult = pDsa->Sign(pSigVerify->DsaContext, &PrivKey, pSigData, iSigSize, &Signature, &uCryptUsecs)) != 1)
    {
        NetPrintfVerbose((DEBUG_ENC_PERF, 0, "protossl: SSL Perf (eddsa sig encrypt) %dms\n", uCryptUsecs / 1000));
        pSecure->uTimer += uCryptUsecs / 1000;
        CryptBnFinal(&Signature.X, pSigVerify->aSigData, 32);
        CryptBnFinal(&Signature.Y, pSigVerify->aSigData+32, 32);
        pSigVerify->iSigSize = 64;
        pSigVerify->bEccContextInitialized = FALSE;
    }
    return(iResult);
}
...
static int32_t _ProtoSSLGenerateSignatureAsync(ProtoSSLRefT *pState)
{
    ...
    else if ((pSigVerify->pSigScheme != NULL) && (pSigVerify->pSigScheme->SigAlg.uSigAlg == SSL3_SIGALG_ED25519))
    {
        iResult = _ProtoSSLGenerateSignatureEddsa(pSecure, pSigVerify->aSigData, pSigVerify->iSigSize, pSigVerify->aKeyData, pSigVerify->iKeySize, ASN_OBJ_EDDSA_ED25519);
    }
    else
    {
...
static int32_t _ProtoSSLGenerateSignature(ProtoSSLRefT *pState, SecureStateT *pSecure, const SignatureSchemeT *pSigScheme, const uint8_t *pSigData, int32_t iSigSize, X509PrivateKeyT *pPrivateKey, int32_t iNextState)
{
    ...
    pSigVerify->bEccContextInitialized = FALSE;
    if ((pSigScheme->SigAlg.uSigAlg == SSL3_SIGALG_ECDSA) || (pSigScheme->SigAlg.uSigAlg == SSL3_SIGALG_ED25519))
    {
...
/*F********************************************************************************/
/*!
    \Function _ProtoSSLVerifySignatureEddsa
    ...
    \Version 06/09/2020 (eesponda)
*/
/********************************************************************************F*/
static int32_t _ProtoSSLVerifySignatureEddsa(SecureStateT *pSecure, const uint8_t *pSigData, int32_t iSigSize, const uint8_t *pData, int32_t iDataSize, const uint8_t *pPubKey, int32_t iPubKeySize, int32_t iCrvType)
{
    CryptEccPointT PubKey, Signature;
    SignatureVerifyT* pSigVerify = &pSecure->SigVerify;
    const CryptCurveDsaT* pDsa;
    ...
    CryptBnInitFrom(&Signature.X, -1, pSigData, 32);
    // parse second component
    CryptBnInitFrom(&Signature.Y, -1, pSigData + 32, 32);

    // do the verify
    if ((iResult = pDsa->Verify(pSigVerify->DsaContext, &PubKey, pData, iDataSize, &Signature, &uCryptUsecs)) != 1)
    {
        NetPrintfVerbose((DEBUG_ENC_PERF, 0, "protossl: SSL Perf (eddsa sig verify) %dms\n", uCryptUsecs / 1000));
        pSecure->uTimer += uCryptUsecs / 1000;
        pSigVerify->bEccContextInitialized = FALSE;
    }
    return(iResult);
...
static int32_t _ProtoSSLVerifySignatureAsync(ProtoSSLRefT *pState)
{
    ...
    {
        iResult = _ProtoSSLVerifySignatureEddsa(pSecure, pSigVerify->aSigData, pSigVerify->iSigSize, pSigVerify->aHashDigest, pSigVerify->iHashSize, pSecure->Cert.KeyModData, pSecure->Cert.iKeyModSize, pSecure->Cert.iCrvType);
    }
    else
    {
    ...
    if (iResult <= 0)
    {
        if (iResult < 0)
        {
            NetPrintf(("protossl: %s signature verify failed\n", pSigVerify->pSigScheme->SigAlg.uSigAlg != SSL3_SIGALG_RSA ? "ecdsa" : "rsa"));
        }
        pSigVerify->pSigScheme = NULL;
    }
    return(iResult);
...
static int32_t _ProtoSSLVerifyCertificateSignature(SecureStateT *pSecure, X509CertificateT *pCert, const uint8_t *pKeyModData, int32_t iKeyModSize, const uint8_t *pKeyExpData, int32_t iKeyExpSize, int32_t iKeyType, int32_t iCrvType)
{
    ...
    else if (iKeyType == ASN_OBJ_EDDSA_ED25519)
    {
        iResult = _ProtoSSLVerifySignatureEddsa(pSecure, pCert->SigData, pCert->iSigSize, pCert->HashData, pCert->iHashSize, pKeyModData, iKeyModSize, iCrvType);
    }
    else
    {
    ...
    else
    {
        ...
        for (pCACert = _ProtoSSL_CACerts; pCACert != NULL; pCACert = pCACert->pNext)
        {
            ...
            if ((_CertificateCompareIdent(&pCACert->Subject, &pCert->Issuer, (bCertIsCA||pCert->iCertIsCA)) != 0) ||
                ((pCACert->iKeyModSize != pCert->iSigSize) && (pCert->iKeyType != ASN_OBJ_ECDSA_KEY) && (pCert->iKeyType != ASN_OBJ_EDDSA_ED25519)))
            {
                continue;
            }
...
static int32_t _ProtoSSLGenerateCertificateVerifyHash(uint8_t *pBuffer, CryptHashTypeE eHashType, const uint8_t *pHashData, int32_t iHashSize, uint8_t bServer, uint8_t bSending)
{
    ...
    *pTempBuf++ = '\0';
    ...
    // for null hash, just copy the contents to the buffer
    else
    {
        ds_memcpy(pBuffer, aTempBuf, pTempBuf - aTempBuf);
        iHashSize = pTempBuf - aTempBuf;
    }
...
static const SignatureSchemeT *_ProtoSSLChooseSignatureScheme(SecureStateT *pSecure, CertificateDataT *pCertificate, const uint8_t *pData, uint32_t uSigSchemeLen, const uint8_t *pDataEnd)
{
    ...
    for (iSigScheme = 0, iNumSigSchemes = (signed)(uSigSchemeLen/sizeof(SignatureAlgorithmT)); iSigScheme < iNumSigSchemes; iSigScheme += 1, pData += 2)
    {
        ...
        if (pSecure->uSslVersion >= PROTOSSL_VERSION_TLS1_3)
        {
            ...
            if (((uCertSigAlg == SSL3_SIGALG_ECDSA) || (uCertSigAlg == SSL3_SIGALG_ED25519)) && (pCertificate->iCrvType != pSigScheme->uOidType))
            {
                continue;
            }
        }
...
/*!
    ...
    \Description
        ...
        ref https://tools.ietf.org/html/rfc8422#section-5.1.1
...
/*F********************************************************************************/
/*!
    \Function _ProtoSSLHelloExtnWriteECPointFormats

    \Description
        Write the Elliptic Curves / Supported Point Formats extension to the ClientHello;
        ref https://tools.ietf.org/html/rfc8422#section-5.1.2
    ...
    \Notes
        As per https://tools.ietf.org/html/rfc8422#section-5.1:
        ...
        However, from https://tools.ietf.org/html/rfc8422#section-5.1.2:
        ...
        In practice, servers using golang tls https://golang.org/pkg/crypto/tls/
        require this extension to be present to negotiate an Elliptic Curve cipher,
        so it is included here for compatibility purposes with servers running
        golang tls.

    \Version 09/11/2020 (jbrookes)
*/
/********************************************************************************F*/
static int32_t _ProtoSSLHelloExtnWriteECPointFormats(ProtoSSLRefT *pState, uint8_t *pBuffer, int32_t iBufLen)
{
    uint8_t *pBufStart = pBuffer;
    ...
    if ((2+2+1+1) > iBufLen)
    {
        NetPrintf(("protossl: could not add extension ec_point_formats; insufficient buffer\n"));
        return(0);
    }
    ...
    // extension ident
    *pBuffer++ = (uint8_t)(SSL_EXTN_EC_POINT_FORMATS>>8);
    *pBuffer++ = (uint8_t)(SSL_EXTN_EC_POINT_FORMATS>>0);
    // extension length
    *pBuffer++ = 0;
    *pBuffer++ = 2;
    // point formats count
    ...
    *pBuffer++ = 0;
    ...
    // return extension length to caller
    return((int32_t)(pBuffer-pBufStart));
}
...
static uint8_t *_ProtoSSLAddHelloExtensions(ProtoSSLRefT *pState, uint8_t *pBuffer, int32_t iBufLen, const uint8_t *pHshkMsgBuf, uint32_t uHelloExtn)
{
    ...
    if (!pState->bServer)
    {
        ...
        if (uHelloExtn & PROTOSSL_HELLOEXTN_ELLIPTIC_CURVES)
        {
            ...
            iBufOff += _ProtoSSLHelloExtnWriteECPointFormats(pState, pBuffer+iBufOff, iBufLen-iBufOff);
        }
    }
...
static int32_t _ProtoSSLSendCertificate(ProtoSSLRefT *pState)
{
    ...
    if (iNumCerts && (pSecure->uSslVersion >= PROTOSSL_VERSION_TLS1_3))
    {
        strBody[iBodySize++] = 0;
        strBody[iBodySize++] = 0;
    }
    ...
    *pData++ = pSecure->uPubKeyLength;
    ...
    if (!pSecure->bSigGenerated)
    {
        ...
        if ((pHash = CryptHashGet(eHashType)) != NULL)
        {
            ...
            if ((iPrivateKeySize = _CertificateDecodePrivate(pState->pPrivateKey, pState->iPrivateKeyLen, pState->strPrivateKeyPass, &PrivateKey)) < 0)
            {
...
static int32_t _ProtoSSLRecvServerKeyExchange(ProtoSSLRefT *pState, const uint8_t *pData, int32_t iDataSize)
{
    ...
    // null hash is supported for eddsa, we just copy the contents for verifying the signature
    else if (pSigScheme->uIdent == SSL3_SIGSCHEME_EDDSA_ED25519)
    {
        uint8_t aMessageData[SSL_SIGDATA_MAX];
        int32_t iMessageSize = 0, iSigSize = _SafeRead16(pSigData, pDataEnd);
        pSigData += 2;

        // add the client random
        ds_memcpy(aMessageData+iMessageSize, pSecure->ClientRandom, sizeof(pSecure->ClientRandom));
        iMessageSize += sizeof(pSecure->ClientRandom);

        // add the server random
        ds_memcpy(aMessageData+iMessageSize, pSecure->ServerRandom, sizeof(pSecure->ServerRandom));
        iMessageSize += sizeof(pSecure->ServerRandom);

        // add the key exchange data to verify
        ds_memcpy(aMessageData+iMessageSize, pBegin, pData-pBegin);
        iMessageSize += (pData-pBegin);

        // verify the signature
        iNextState = _ProtoSSLVerifySignature(pState, pSecure, pSigScheme, pSigData, iSigSize, aMessageData, iMessageSize, CRYPTHASH_NULL, ST3_RECV_HELLO);
    }
    else
    {
...
static int32_t _ProtoSSLRecvClientKeyExchange(ProtoSSLRefT *pState, const uint8_t *pData, int32_t iDataSize)
{
    ...
    if (pSecure->pCipher->uKey == SSL3_KEY_RSA)
    {
        ...
        if ((iPrivKeySize = _CertificateDecodePrivate(pState->pPrivateKey, pState->iPrivateKeyLen, pState->strPrivateKeyPass, &PrivateKey)) < 0)
        {
            NetPrintf(("protossl: unable to decode private key\n"));
            _ProtoSSLSendAlert(pState, SSL3_ALERT_LEVEL_FATAL, SSL3_ALERT_DESC_INTERNAL_ERROR);
            return(ST_FAIL_SETUP);
...
static int32_t _ProtoSSLSendCertificateVerify(ProtoSSLRefT *pState)
{
    ...
    if (!pSecure->bSigGenerated)
    {
        ...
        {
            CryptHashTypeE eHashType = _SSL3_HashIdToCrypt[pSecure->pSigScheme->SigAlg.uHashId];
            uint8_t aMessageHash[SSL_SIGDATA_MAX], aTempHash[CRYPTHASH_MAXDIGEST];
...
static int32_t _ProtoSSLRecvCertificateVerify(ProtoSSLRefT *pState, const uint8_t *pData, int32_t iDataSize)
{
    ...
    const uint8_t *pDataEnd = pData+iDataSize;
    uint8_t aHandshakeHash[SSL_SIGDATA_MAX];
...
static int32_t _ProtoSSLUpdateSetAsyncState(ProtoSSLRefT *pState, AsyncOpT *pAsyncOp, int32_t iNextState, int32_t iFailState, int32_t iFailAlert)
{
    ...
    _ProtoSSLUpdateAsync(pState, pState->pSecure);
    return(pState->iState);
}
...
/*F********************************************************************************/
/*!
    \Function _ProtoSSLFreeSecureState
    ...
    \Version 11/23/2020 (jbrookes)
*/
/********************************************************************************F*/
static void _ProtoSSLFreeSecureState(SecureStateT *pSecure, int32_t iMemGroup, void *pMemGroupUserData)
{
    const CryptCurveDhT *pDh;
    const CryptCurveDsaT *pDsa;

    // if RSA context was left in an initalized state, clean it up
    if (pSecure->bRSAContextInitialized)
    {
        CryptRSACleanup(&pSecure->RSAContext);
    }
    // if ECC (key exchange) context was left in an initialized state, clean it up
    if (pSecure->bEccContextInitialized && ((pDh = _ProtoSSLEccInitContext(pSecure, pSecure->pEllipticCurve)) != NULL))
    {
        pDh->Cleanup(pSecure->EccContext);
    }
    // if ECC (DSA) context was left in an initialized state, clean it up
    if (pSecure->SigVerify.bEccContextInitialized && ((pDsa = _ProtoSSLDsaInitContext(&pSecure->SigVerify, pSecure->Cert.iCrvType)) != NULL))
    {
        pDsa->Cleanup(pSecure->SigVerify.DsaContext);
    }
    // free secure state
...
static int32_t _ProtoSSLResetSecureState(ProtoSSLRefT *pState, int32_t iSecure)
{
    ...
    // get rid of secure state, if allocated
    if (pState->pSecure != NULL)
    {
        _ProtoSSLFreeSecureState(pState->pSecure, pState->iMemGroup, pState->pMemGroupUserData);
        pState->pSecure = NULL;
    }
...
static int32_t _ProtoSSLSetCACert(const uint8_t *pCertData, int32_t iCertSize, uint8_t bVerify)
{
    ...
    if ((iResult = Base64Decode3((const char *)pCertBeg, (int32_t)(pCertEnd-pCertBeg), NULL, 0x7fffffff)) > 0)
    {
        ...
        // decode the cert
        Base64Decode3((const char *)pCertBeg, (int32_t)(pCertEnd-pCertBeg), (char *)pCertBuffer, _iMaxCertSize);
    ...
    while ((iResult == 0) && (iCertSize > 0) && (_CertificateFindData(pCertData, iCertSize, &pCertBeg, &pCertEnd, &uCertType) != 0))
    {
        ...
        if (((iResult = Base64Decode3((const char *)pCertBeg, (int32_t)(pCertEnd-pCertBeg), NULL, 0x7fffffff)) <= 0) || (iResult > _iMaxCertSize))
        {
            break;
        }
        Base64Decode3((const char *)pCertBeg, (int32_t)(pCertEnd-pCertBeg), (char *)pCertBuffer, _iMaxCertSize);
    ...
    _ProtoSSLFreeSecureState(pSecure, iMemGroup, pMemGroupUserData);
...
int32_t ProtoSSLStartup(void)
{
    ...
    pState->iDfltMinVers = SSL3_TLS1_2;
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'scrt'      Returns certificate information about the client cert used by the module
...
int32_t ProtoSSLStat(ProtoSSLRefT *pState, int32_t iSelect, void *pBuffer, int32_t iLength)
{
    ...
    // allow for querying the information of the loaded certificate
    if ((iSelect == 'scrt') && (pState->pCertificate != NULL) && (pBuffer != NULL) && (iLength == sizeof(ProtoSSLCertInfoT)))
    {
        ProtoSSLCertInfoT *pCertInfo = (ProtoSSLCertInfoT *)pBuffer;
        X509CertificateT Cert;

        // parse the certificate from the saved data
        _AsnParseCertificate(&Cert, pState->pCertificate->aCertData, pState->pCertificate->iCertSize);
        // copy the relevant data to the cert info
        ds_memclr(pCertInfo, sizeof(*pCertInfo));
        _CertificateCopyInfo(&Cert, pCertInfo);
        return(0);
    }
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'spas'      Set private key password (pValue=password)
...
int32_t ProtoSSLControl(ProtoSSLRefT *pState, int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iSelect == 'skey')
    {
        int32_t iKeyBufLen = iValue + 1;  // we need to be sure to save an extra byte for the null terminator
        if (pState->pPrivateKey != NULL)
        {
            DirtyMemFree(pState->pPrivateKey, PROTOSSL_MEMID, pState->iMemGroup, pState->pMemGroupUserData);
            pState->iPrivateKeyLen = 0;
        }
        if ((pState->pPrivateKey = DirtyMemAlloc(iKeyBufLen, PROTOSSL_MEMID, pState->iMemGroup, pState->pMemGroupUserData)) != NULL)
        {
            ds_strnzcpy(pState->pPrivateKey, (const char *)pValue, iKeyBufLen);
    ...
    if (iSelect == 'spas')
    {
        if (iValue <= (signed)sizeof(pState->strPrivateKeyPass))
        {
            ds_strnzcpy(pState->strPrivateKeyPass, (const char *)pValue, sizeof(pState->strPrivateKeyPass));
            return(0);
        }
        else
        {
            NetPrintf(("protossl: setting private key pass fail would have been truncated (source=%d, destination=%d)\n", iValue, sizeof(pState->strPrivateKeyPass)));
            return(-1);
        }
    }
    if (iSelect == 'vers')
