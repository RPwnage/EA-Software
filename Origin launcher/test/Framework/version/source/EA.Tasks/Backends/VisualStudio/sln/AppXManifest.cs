...
namespace EA.Eaconfig.Backends.VisualStudio
{
	internal class AppXManifest : AppManifestBase
	{
		internal static AppXManifest Generate(Log log, Module_Native module, PathString vsProjectDir, string vsProjectName, string vsProjectFileNameWithoutExtension)
		{
			if (module != null)
			{
				var manifest = new AppXManifest(log, module, vsProjectDir, vsProjectName, vsProjectFileNameWithoutExtension);

				var templateFile = manifest.GetTemplateFilePath("appxmanifest.template", "appxmanifest.template.file");
		...
		private AppXManifest(Log log, Module_Native module, PathString vsProjectDir, string vsProjectName, string vsProjectFileNameWithoutExtension)
			: base(log, module, module.GroupName + ".appxmanifestoptions", "config-options-appxmanifestoptions", vsProjectDir, vsProjectName, vsProjectFileNameWithoutExtension)
		{
		}
		...
		protected override PathString CreateImages(OptionSet manifestOptions, PathString appManifestFolder)
		{
			...
			{
				var image_directory = manifestOptions.Options["resource.image-directory"];
				appManifestFolder = new PathString(Path.Combine(appManifestFolder.Path, image_directory??String.Empty));

				AddImageAsResource(resourcefiles, ImageToolOptionSetName, manifestOptions.Options["properties.logo"], languages, appManifestFolder, 50, 50);

				AddImageAsResource(resourcefiles, ImageToolOptionSetName, manifestOptions.Options["visualelements.logo"], languages, appManifestFolder, 208, 208);
				AddImageAsResource(resourcefiles, ImageToolOptionSetName, manifestOptions.Options["visualelements.smalllogo"], languages, appManifestFolder, 100, 100);
				AddImageAsResource(resourcefiles, ImageToolOptionSetName, manifestOptions.Options["visualelements.splashscreen"], languages, appManifestFolder, 1920, 1080);

				if (manifestOptions.Options.Contains("visualelements.widelogo"))
				{
					AddImageAsResource(resourcefiles, ImageToolOptionSetName, manifestOptions.Options["visualelements.widelogo"], languages, appManifestFolder, 480, 480);
				}

				if (manifestOptions.Options.Contains("visualelements.lockscreen-badgelogo"))
				{
					AddImageAsResource(resourcefiles, ImageToolOptionSetName, manifestOptions.Options["visualelements.lockscreen-badgelogo"], languages, appManifestFolder, 24, 24);
				}
