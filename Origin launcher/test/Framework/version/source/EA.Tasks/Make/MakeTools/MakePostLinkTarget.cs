...
{
	class MakePostLinkTarget : MakeBuildStepBase<Tool>
	{
		public MakePostLinkTarget(MakeModuleBase makeModule, IModule module, Tool tool)
		...
		public void GenerateTarget()
		{
			...
			bool isPhony = false;
			string primaryTargetName = GetPrimaryTargetName(outputDependenciesCollection, "postlink", out isPhony);
			if (isPhony)
			{
			...
			MakeCommand(BuildStepTool); // modifies last added target
			primaryTarget.Prerequisites += inputDependenciesCollection.Select(path => MakeModule.ToMakeFilePath(path));
			
			// add secondary targets
			AddSerializationTargets(outputDependenciesCollection, inputDependenciesCollection);
