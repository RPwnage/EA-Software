...
namespace NAnt.Perforce
{
	namespace Internal
	{
		internal static class P4Caller
		{
				...
				if (args != null && args.Length > 0)
				{
					if (useResponseFile)
					{
						using (ResponseFile respFile = new ResponseFile(args))
						{
							...
							return Run(command, input, p4Args, context, args, useSyncOuput, timeoutMillisec, taskOutputMonitor);
						}
					}
					else
					{
						...
						return Run(command, input, p4Args, context, useSyncOuput: useSyncOuput, timeoutMillisec: timeoutMillisec, taskOutputMonitor: taskOutputMonitor);
					}
				}
				else
				{
			...
			private static P4Output Run(string command, string input, string p4Args, P4Context sanitizeContext = null, string[] responseFileArgs = null, bool useSyncOuput = false, int timeoutMillisec = P4GlobalOptions.DefaultLongTimeoutMs, ITaskSpecificOutputMonitor taskOutputMonitor = null)
			{
				...
				try
				{
					using (ChildProcess p4Proc = CreateP4Process(p4Args, p4output, tempWorkingDir, tempConfigFile, asyncOutput, taskOutputMonitor))
					{
			...
			private static ChildProcess CreateP4Process(string p4Args, P4Output p4output, string workingDir, string junkFile, bool asyncOutput, ITaskSpecificOutputMonitor taskOutputMonitor = null)
			{
