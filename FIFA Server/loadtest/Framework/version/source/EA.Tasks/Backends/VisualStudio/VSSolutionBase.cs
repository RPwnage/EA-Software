...
namespace EA.Eaconfig.Backends.VisualStudio
{
	...
	internal abstract class VSSolutionBase : Generator
	{
		...
		public override bool Initialize(Project project, IEnumerable<string> configurations, string startupconfigName, IEnumerable<IModule> topmodules, IEnumerable<IModule> excludedRootModules, bool generateSingleConfig = false, GeneratorTemplatesData generatorTemplates = null, int groupnumber = 0)
		{
			...
			foreach (var topmodule in topmodules.Where(m => Configurations.Contains(m.Configuration)))
			{
				...
				if (ModuleGenerators.TryGetValue(genkey, out topgenerator))
				{
					...
					if (
						vsproject != null && 
						!(vsproject is ExternalVSProject) && // external vs projects are ignored for the purposes of solution config names, as the override is specific to the modules, not the overall Framework configuration - we will map from the project config to the solution config in project section
		...
		internal void WriteToFile()
		{
			using (IMakeWriter writer = new MakeWriter(writeBOM: false))
			{
				...
				writer.WriteLine("Global");
				{
					...
					foreach (var project in ModuleGenerators.Values.OrderBy(g => (g as IVSProject).ProjectGuidString))
					{
						WriteProjectConfigMapping(writer, project as IVSProject);
					}
					foreach (var project in PreGeneratedModules.Values.OrderBy(g => (g as IVSProject).ProjectGuidString))
					{
						WriteProjectConfigMapping(writer, project as IVSProject);
					}
		...
		*/
		...
		protected virtual void WriteProjectConfigMapping(IMakeWriter writer, IVSProject project)
		{
		...
		protected virtual void WriteSolutionFoldersContent(IMakeWriter writer)
		{
			...
			if (SolutionFolders.Folders.Count > 0)
			{
				...
				foreach (var folder in SolutionFolders.Folders.Values.OrderBy(f => f.PathName.Path, StringComparer.OrdinalIgnoreCase))
				{
					...
					foreach (IVSProject gen in folder.FolderProjects.Values.OrderBy(g => g.Name, StringComparer.OrdinalIgnoreCase))
					{
