		...
		public void Process(Module_DotNet module)
		{
			...
			else if (module.Compiler is FscCompiler)
			{
				NAnt.DotNetTasks.FscTask fsc = new NAnt.DotNetTasks.FscTask
				{
					Project = Project,
					useDebugProperty = false,
					TargetFrameworkFamily = module.TargetFrameworkFamily.ToString()
				};
				compiler = fsc;
			...
			if (module.TargetFrameworkFamily == DotNetFrameworkFamilies.Core)
			{
				...
				{
					// run RunCreateAppHostMsbuildTask task from DotNetCoreSDK package
					...
					// run CreateBasicRuntimeConfigsJson task from DotNetCoreSDK package
					...
					{
						string runtimeTarget = module.Package.Project.GetPropertyOrFail("package.DotNetCoreSDK.runtimetarget.core");
