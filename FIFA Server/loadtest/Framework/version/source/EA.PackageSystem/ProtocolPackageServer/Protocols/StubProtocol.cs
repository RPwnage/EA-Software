...
namespace EA.PackageSystem
{
	...
	// It essentially just copies a package from one directory to another
	// The URI is just a path to a directory that contains the package
	public class StubProtocol : ProtocolBase
	{
		public const string LogPrefix = "Protocol package server (stub): ";
		...
		{
			string serverPath = Path.Combine(uri.LocalPath.TrimLeftSlash(), package.Name, package.Version);

			string releasePath = Path.Combine(rootDirectory, package.Name, package.Version);

			CopyDirectory(serverPath, releasePath);
