...
#pragma warning restore 6500
			...
			// additional output checking for known ea out-of-source build issues
			if (MainEmbeddedFiles != null)
			{
				IEnumerable<string> duplicateEmbeds = MainEmbeddedFiles.Select(ti => ti.ItemSpec).GroupBy(s => s).Where(grp => grp.Count() > 1).Select(grp => grp.First());
				if (duplicateEmbeds.Any())
				{
					Log.LogError("Duplicate main embed files as output: {0}!", String.Join(", ", duplicateEmbeds));
					return false;
				}
			}
        ...
        private bool VerifyTaskInputs()
        {
			...
			// additional checing for known ea-out-source build issues
			IEnumerable<string> duplicateSources = SourceFiles.Select(ti => ti.ItemSpec).GroupBy(s => s).Where(grp => grp.Count() > 1).Select(grp => grp.First());
			if (duplicateSources.Any())
			{
				Log.LogError("Duplicate sourcefiles as input: {0}!", String.Join(", ", duplicateSources));
				return false;
			}
