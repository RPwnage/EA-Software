...
namespace Microsoft.Build.Tasks.Windows
{
    ...
    public sealed class MarkupCompilePass1 : Task
    {
        ...
        public override bool Execute()
        {
			...
			// input arg checking for known EA out-of-source build issues
			foreach (ITaskItem inputSourceFile in SourceCodeFiles)
			{
				if (!File.Exists(inputSourceFile.ItemSpec))
				{
					Log.LogError("Missing source file: {0}!", inputSourceFile.ItemSpec);
					return false;
				}
			}
			foreach (ITaskItem inputReference in References)
			{
				if (!File.Exists(inputReference.ItemSpec))
				{
					Log.LogError("Missing reference assembly: {0}!", inputReference.ItemSpec);
					return false;
				}
			}
...
#pragma warning restore 6500
			...
			// output error checking for known EA out-of-source build problems
			IEnumerable<string> duplicateBamlPaths = GeneratedBamlFiles.Select(item => item.ItemSpec).GroupBy(path => path).Where(grp => grp.Count() > 1).Select(grp => grp.First());
			if (duplicateBamlPaths.Any())
			{
				Log.LogError("MarkupCompilePass1 task generated duplicate output BAML paths:\n\t{0}\nThis will cause incorrect runtime or build behaviour.", String.Join(", ", duplicateBamlPaths));
				bSuccess = false;
			}
			IEnumerable<string> duplicatedCodeFiles = GeneratedCodeFiles.Select(item => item.ItemSpec).GroupBy(path => path).Where(grp => grp.Count() > 1).Select(grp => grp.First());
			if (duplicatedCodeFiles.Any())
			{
				Log.LogError("MarkupCompilePass1 task generated duplicate output code paths:\n\t{0}\nThis will cause incorrect runtime or build behaviour.", String.Join(", ", duplicatedCodeFiles));
				bSuccess = false;
			}
			IEnumerable<string> duplicatedLocalFiles = GeneratedLocalizationFiles.Select(item => item.ItemSpec).GroupBy(path => path).Where(grp => grp.Count() > 1).Select(grp => grp.First());
			if (duplicatedLocalFiles.Any())
			{
				Log.LogError("MarkupCompilePass1 task generated duplicate output localization paths:\n\t{0}\nThis will cause incorrect runtime or build behaviour.", String.Join(", ", duplicatedLocalFiles));
				bSuccess = false;
			}
        ...
        private void CleanupGeneratedFiles( )
        {
            ...
            {
				string linkMetaData = CompilerAnalyzer.RecompileApplicationFile.LinkAlias;
				string xamlLogicalPath = String.IsNullOrWhiteSpace(linkMetaData) ? CompilerAnalyzer.RecompileApplicationFile.Path : linkMetaData;

				GetGeneratedFiles(xamlLogicalPath, out codeFile, out bamlFile);
            ...
            if (CompilerAnalyzer.RecompileMarkupPages != null)
            {
                for (int i = 0; i < CompilerAnalyzer.RecompileMarkupPages.Length; i++)
                {
					string linkMetaData = CompilerAnalyzer.RecompileMarkupPages[i].LinkAlias;
					string xamlLogicalPath = String.IsNullOrWhiteSpace(linkMetaData) ? CompilerAnalyzer.RecompileMarkupPages[i].Path : linkMetaData;
                                                             ...
                                                             ref ArrayList outputLocalRefXamlList)
        {
            ...
            for (int i = 0; i < inputXamlItemList.Length; i++)
            {
				...
				string linkMetaData = inputXamlItemList[i].GetMetadata("Link");
				string xamlLogicalPath = String.IsNullOrWhiteSpace(linkMetaData) ? inputXamlItemList[i].ItemSpec : linkMetaData;

                GetGeneratedFiles(xamlLogicalPath, out genLangFilePath, out bamlFile);
