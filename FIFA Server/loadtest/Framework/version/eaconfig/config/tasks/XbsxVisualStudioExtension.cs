...
namespace EA.XbsxConfig
{
	[TaskName("xbsx-vc-visualstudio-extension", XmlSchema = false)]
	public class XbsxVisualStudioExtension : IMCPPVisualStudioExtension
	{
		private bool DoNotInjectEnvironmentOverride
		{
			get
			{
				...
				return Project.Properties.GetBooleanPropertyOrDefault("package.GDK.DoNotInjectEnvironmentOverride", false);
			}
		}
		...
		public override void ProjectGlobals(IDictionary<string, string> projectGlobals)
		{
			...
			if (!projectGlobals.ContainsKey("DisableConfigurationRefresh") && Project.Properties.GetBooleanPropertyOrDefault("package.GDK.DisableConfigurationRefresh", false))
			{
				projectGlobals.Add("DisableConfigurationRefresh", "true");
			}

			// This is to allow users to add arbitrary elements to project globals
			if (Project.Properties["package.GDK.AdditionalProjectGlobals"] != null)
			{
				string[] elements = Project.Properties["package.GDK.AdditionalProjectGlobals"].Split();
				foreach (string element in elements)
				{
					string element_value = Project.Properties["package.GDK.AdditionalProjectGlobals." + element];
			...
			var osname = Project.Properties["package.GDK.osname"];
		...
		public override void WriteExtensionToolProperties(IXmlWriter writer)
		{
			...
			{
				var layoutDir = CapilanoFunctions.GetModuleLayoutDir(Module.Package.Project, "xbsx", Module.Name, Module.BuildGroup.ToString());
			...
			if (Project.Properties["package.GDK.use32bitBuildTools"] == "true")
			{
		...
		public override void WriteMsBuildOverrides(IXmlWriter writer)
		{
			...
			if (Project.Properties["package.GDK.use32bitBuildTools"] == "true")
			{
				Log.Warning.WriteLine("WARNING: Property 'package.GDK.use32bitBuildTools' was specified but 32-bit tools are not supported under Visual Studio 2015.  This property is being ignored!");
			}

			NAnt.Core.Tasks.DoOnce.Execute(Project, doOnceKey + "_GameCoreDeploy_Target", () =>
			{
				if (Module.Package.Project.Properties.GetBooleanPropertyOrDefault(Module.GroupName + ".GDK.skipDeployment", false)
				|| Module.Package.Project.Properties.GetBooleanPropertyOrDefault("package.GDK.skipDeployment", false)
				|| Module.IsKindOf(Model.Module.Utility))
				{
					writer.WriteStartElement("Target");
					writer.WriteAttributeString("Name", "_GameCoreDeploy");
			...
			{
				if (Module.Package.Project.Properties.GetBooleanPropertyOrDefault(Module.GroupName + ".GDK.skipLayout", false)
				|| Module.Package.Project.Properties.GetBooleanPropertyOrDefault("package.GDK.skipLayout", false)
