...
namespace Blaze {
	namespace Stress {
		...
		unsigned char *AES::EncryptCBC(unsigned char in[], unsigned int inLen, unsigned  char* key, unsigned char * iv, unsigned int &outLen)
		{
			...
			unsigned char *alignIn = Padding(in, inLen, outLen);
		...
		unsigned char * AES::Padding(unsigned char in[], unsigned int inLen, unsigned int alignLen)
		{
			unsigned char *alignIn = new unsigned char[alignLen];
			memcpy(alignIn, in, inLen);
			memset(alignIn + inLen, char(alignLen-inLen), alignLen - inLen);
			return alignIn;
		}
