//  *************************************************************************************************
//
//   File:    esltournamentsgamesession.cpp
//
//   Author:  systest
//
...
ESLTournamentsGameSession::ESLTournamentsGameSession(StressPlayerInfo* playerData, uint32_t GameSize, GameNetworkTopology Topology, GameProtocolVersionString GameProtocolVersionString)
    : GameSession(playerData, GameSize, Topology, GameProtocolVersionString)
{
	
}

ESLTournamentsGameSession::~ESLTournamentsGameSession()
{
    BLAZE_INFO(BlazeRpcLog::util, "[ESLTournamentsGameSession][Destructor][%" PRIu64 "]: destroy called", mPlayerData->getPlayerId());
}

void ESLTournamentsGameSession::asyncHandlerFunction(uint16_t type, EA::TDF::Tdf *notification)
{
...
void ESLTournamentsGameSession::onGameSessionDefaultAsyncHandler(uint16_t type, EA::TDF::Tdf* notification)
{
    //  This is abstract class and handle the async handlers in the derived class

    BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << "]:Message Type =" << type);
    switch (type)
    {
        case GameManagerSlave::NOTIFY_NOTIFYGAMESETUP:
            //  Async notification sent to the player who joined a game.  Triggered by JoinGame Request or a Matchmaking session finding a game.
            {
				...
				BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsSeasonsGameSession::asyncHandlerFunction]: Received  NOTIFY_NOTIFYGAMESETUP." << mPlayerData->getPlayerId());
				//if (notify->getGameSetupReason().getDatalessSetupContext()->getSetupContext() == INDIRECT_JOIN_GAME_FROM_RESERVATION_CONTEXT)
				//{
					joinGame(notify->getGameData().getGameId());
				//}
        ...
        case GameManagerSlave::NOTIFY_NOTIFYGAMESTATECHANGE:
            {
                ...
                BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << "]: Received  NOTIFY_NOTIFYGAMESTATECHANGE, state=" <<
                                notify->getNewGameState());
                if (myGameId == notify->getGameId())
                {
					if( getPlayerState() == IN_GAME && notify->getNewGameState() == POST_GAME )
					{
						BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::NOTIFY_NOTIFYGAMESTATECHANGE][" << mPlayerData->getPlayerId() << "]: submitGameReport");
						submitGameReport();	
						//if (isPlatformHost) {
						//	replayGame();
						//}
        ...
        case GameManagerSlave::NOTIFY_NOTIFYGAMERESET:
            {
                ...
                BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << "]: Received  NOTIFY_NOTIFYGAMERESET.gameId=" <<
        ...
        case GameManagerSlave::NOTIFY_NOTIFYPLAYERJOINING:
            {
                ...
                BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << "]: Received  NOTIFY_NOTIFYPLAYERJOINING.gameId=" << notify->getGameId());
				/*if (!isPlatformHost)
				{
				...
				{					
					...
					BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << " OpponentPlayerInfo PlayerId=" << getOpponentPlayerInfo().getPlayerId() << " ConnGroupId=" << getOpponentPlayerInfo().getConnGroupId().toString());
                ...
                //if ( myGameId == notify->getGameId() && ESLTournamentsGameDataRef(mPlayerData).isPlayerExistInGame(notify->getJoiningPlayer().getPlayerId(), myGameId) == false )
                //{
                //    ESLTournamentsGameDataRef(mPlayerData).addPlayerToGameInfo(notify);
                //    if (IsPlayerPresentInList((PlayerModule*)(mPlayerData->getOwner()), notify->getJoiningPlayer().getPlayerId(), FRIENDLIES) == true)
                //    {
                //        uint16_t count = ESLTournamentsGameDataRef(mPlayerData).getPlayerCount(myGameId);
                //        ESLTournamentsGameDataRef(mPlayerData).setPlayerCount(myGameId, ++count);
                //        BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameDataRef::asyncHandlerFunction::NOTIFYPLAYERJOINCOMPLETED][" << mPlayerData->getPlayerId() << "]:GameId=" << myGameId << " Player count = " <<
        ...
        case GameManagerSlave::NOTIFY_NOTIFYJOININGPLAYERINITIATECONNECTIONS:
            {
                BLAZE_INFO(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::handlerFunction][%" PRIu64 "]: Received  NOTIFY_NOTIFYJOININGPLAYERINITIATECONNECTIONS.", mPlayerData->getPlayerId());
                NotifyGameSetup* notify = (NotifyGameSetup*)notification;                           
				handleNotifyGameSetUp(notify);
            }
            break;
        case GameManagerSlave::NOTIFY_NOTIFYPLATFORMHOSTINITIALIZED:
            {
                ...
                BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << "]: Received  NOTIFY_NOTIFYPLATFORMHOSTINITIALIZED.gameId=" <<
                                notify->getGameId());
            }
            break;
        case GameManagerSlave::NOTIFY_NOTIFYGAMELISTUPDATE:
            {
                ...
                BLAZE_INFO(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][%" PRIu64 "]: Received  NOTIFY_NOTIFYGAMELISTUPDATE.", mPlayerData->getPlayerId());
        ...
        case GameManagerSlave::NOTIFY_NOTIFYPLAYERJOINCOMPLETED:
            {
                ...
                BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << "]: Received  NOTIFY_NOTIFYPLAYERJOINCOMPLETED.");
				...
				setConnectionState(mPlayerData);

                BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::PlayerJoinCompleted. Player][" << mPlayerData->getPlayerId()) ;
        ...
        case GameManagerSlave::NOTIFY_NOTIFYPLAYERREMOVED:
            {
                ...
                if (!ptrGameInfo)
                {
                    BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << "]::receiving notification(" << type << ") and gameId " << myGameId <<
                ...
                if (mPlayerData->getPlayerId() == notify->getPlayerId() && myGameId == notify->getGameId() )
                {
                    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << "]: Received  NOTIFY_NOTIFYPLAYERREMOVED for game [" << notify->getGameId() << "] reason: "<<notify->getPlayerRemovedReason());
					if (isPlatformHost)
					{
						BlazeObjectId targetGroupId = getOpponentPlayerInfo().getConnGroupId();
						BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << " meshEndpointsDisconnected for grouID" << targetGroupId.toString());
                ...
                if ( myGameId == notify->getGameId()  && FriendliesGameDataRef(mPlayerData).isPlayerExistInGame(notify->getPlayerId(), notify->getGameId()) == true )
                {
                    ...
                    if (IsPlayerPresentInList((PlayerModule*)(mPlayerData->getOwner()), notify->getPlayerId(), FRIENDLIES) == true)
                    {
                        ...
                        BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction]::NOTIFYPLAYERREMOVED[" << mPlayerData->getPlayerId() << "]:GameId=" << myGameId << " Player count = " << count
                ...
                {
                    BLAZE_INFO_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << "]: Received  NOTIFY_NOTIFYPLAYERREMOVED - Game Id = " <<
                                   notify->getGameId() << ".");
        ...
        case GameManagerSlave::NOTIFY_NOTIFYGAMEREMOVED:
            {
                ...
                BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << "]: Received  NOTIFY_NOTIFYGAMEREMOVED.");
            }
            break;
        case GameManagerSlave::NOTIFY_NOTIFYMATCHMAKINGFAILED:
            {
                ...
                switch (result)
                {
                    case GameManager::SESSION_TIMED_OUT:
                        {
                            BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::OnNotifyMatchmakingFailed][" << mPlayerData->getPlayerId() << "]: Matchmake SESSION_TIMED_OUT.");
                        }
                        break;
                    case GameManager::SESSION_CANCELED:
                        {
                            BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::OnNotifyMatchmakingFailed][" << mPlayerData->getPlayerId() << "]: Matchmake SESSION_CANCELED.");
                            //  Throw Execption;
                        }
						break;
                    default:
        ...
        case Blaze::Messaging::MessagingSlave::NOTIFY_NOTIFYMESSAGE:
            {
                BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::handlerFunction][" << mPlayerData->getPlayerId() << "]: Received  NOTIFY_NOTIFYMESSAGE.");
...
BlazeRpcError ESLTournamentsGameSession::submitGameReport()
{
    ...
    if (!ptrGameInfo)
    {
        BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::submitGameReport][" << mPlayerData->getPlayerId() << " gameId " << myGameId << " not present in ESLTournamentsGamesMap");
		return ERR_SYSTEM;
    }
	...
	gamereport.setGameReportName("gameType31");
    ...
    if (osdkReport != NULL) {
        ...
        osdkGamereport.setGameTime(5659);  // TODO - change to random number
		osdkGamereport.setGameType("gameType31");
		...
		commonGameReport.setAwayKitId(1361);	//	As per latest logs
		commonGameReport.setBallId(37);
		commonGameReport.setHomeKitId(1360);
		commonGameReport.setStadiumId(13);
		for (int i = 0; i<11; i++)
		{	
        ...
        h2hGameReport.setMatchInSeconds(5659);
	...
	if (isPlatformHost)
	{
		BLAZE_TRACE_RPC_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::submitgamereport] Platform Host --> " << mPlayerData->getPlayerId());
	}
    /*if (BLAZE_IS_TRACE_RPC_ENABLED(BlazeRpcLog::gamemanager)) {
        char8_t buf[20240];
        BLAZE_TRACE_RPC_LOG(BlazeRpcLog::gamemanager, "-> [ESLTournamentsGameSession][" << mPlayerData->getPlayerId() << "]::submitGameReport" << "\n" << request.print(buf, sizeof(buf)));
   }*/
    error = mPlayerData->getComponentProxy()->mGameReportingProxy->submitGameReport(request);
	if (error == ERR_OK) {
		BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "ESLTournamentsGameSession::submitgamereport success for " << mPlayerData->getPlayerId());
	}
	else
	{
		BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "ESLTournamentsGameSession::submitgamereport failed. Error(" << ErrorHelp::getErrorName(error) << ") for "<<mPlayerData->getPlayerId());
	}

    return error;
}

void ESLTournamentsGameSession::handleNotifyGameSetUp(NotifyGameSetup* notify, uint16_t type )
{
    ...
    {
        BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::asyncHandlerFunction][" << mPlayerData->getPlayerId() << "]::receiving notification(" << type << ") and gameId " <<
    ...
    if (!ptrGameInfo)
    {
        BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::handleNotifyGameSetUp][" << mPlayerData->getPlayerId() << "] gameId " << myGameId <<" not present in friendliesGamesMap");
		return ;
    }
	...
	for( ; start != end; ++start)
	{
		if(mPlayerData->getPlayerId() != start->first)
		{
			...
			BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::handleNotifyGameSetUp][" << mPlayerData->getPlayerId() << " OpponentPlayerInfo PlayerId=" << getOpponentPlayerInfo().getPlayerId() << " ConnGroupId=" << getOpponentPlayerInfo().getConnGroupId().toString());
		}
	}
	...
	attributes["RDY"] = 0;
	attributes["fifaCustomController"] = "0";
	attributes["fifaDrawMode"] = "3";
	attributes["fifaGameSpeed"] = "1";
	attributes["fifaHalfLength"] = "4";
	attributes["fifaRoster"] = "1";
	attributes["fifaTeamLevel"] = "0";
	attributes["fifaTourStadium"] = "-1";
	attributes["fifaTourToD"] = "-1";
	attributes["fifaTourWeather"] = "0";
	attributes["OSDK_rosterVersion"] = "1";
	attributes["tourAttributesMapped"] = "1";
	//setGameAttributes(myGameId,attributes);
	setPlayerAttributes("everJoined", "1", myGameId);
	if(getOpponentPlayerInfo().getPlayerId() != 0)
	{
		...
		if(StressConfigInst.isGeoLocation_MM_Enabled())
		{
			...
			//double distnace = haversine(mPlayerData->getLocation().latitude, mPlayerData->getLocation().longitude, opponentLocation.latitude, opponentLocation.longitude);
			//meshEndpointsConnected(targetGroupId, (uint32_t)calculateLatency(distnace));	
			BlazeObjectId ttargetGroupId = mPlayerData->getConnGroupId();
			//BLAZE_TRACE_RPC_LOG(BlazeRpcLog::gamemanager, "GameSession::meshEndpointsConnected RPC with target group id : \n" );
			meshEndpointsConnected(ttargetGroupId, 0, 0, notify->getGameData().getGameId());
		...
		{
			//meshEndpointsConnected(targetGroupId);	
			BlazeObjectId etargetGroupId = mPlayerData->getConnGroupId();
			//BLAZE_TRACE_RPC_LOG(BlazeRpcLog::gamemanager, "GameSession::meshEndpointsConnected RPC with target group id : \n" );
			meshEndpointsConnected(etargetGroupId, 0, 0, notify->getGameData().getGameId());
	...
	if (isPlatformHost)
    {
        BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::handleNotifyGameSetUp][" << mPlayerData->getPlayerId() << "calling finalizeGameCreation");
    ...
    if (IsPlayerPresentInList((PlayerModule*)(mPlayerData->getOwner()), mPlayerData->getPlayerId(), FRIENDLIES) == true)
    {
        ...
        BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::handleNotifyGameSetUp][" << mPlayerData->getPlayerId() << "]:GameId=" << myGameId << " Player count = " << count);
    }

}

BlazeRpcError ESLTournamentsGameSession::createGame()
{
	...
	{
		BLAZE_ERR_LOG(BlazeRpcLog::util, "[ESLTournaments::User Disconnected = " << mPlayerData->getPlayerId());
    ...
    gameAttribs["fifaMatchupHash"] = "4037339";  // 
	...
	/*if (BLAZE_IS_TRACE_RPC_ENABLED(BlazeRpcLog::gamemanager)) {
		char8_t buf[10240];
		BLAZE_TRACE_RPC_LOG(BlazeRpcLog::gamemanager, "CreateGame Request: \n"<< request.print(buf, sizeof(buf)));
	}*/
	err = mPlayerData->getComponentProxy()->mGameManagerProxy->createGame(request, response);

	if (err == ERR_OK) {
		BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "ESLTournaments::CreateGame success.");
	}
	else
	{
		BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "ESLTournaments::CreateGame failed. Error(" << ErrorHelp::getErrorName(err) << ")");
	}
	return err;
}

BlazeRpcError ESLTournamentsGameSession::joinGame(GameId gameId)
{
	BlazeRpcError err = ERR_OK;
	...
	request.getCommonGameData().setGameProtocolVersionString("qaonly");
	...
	joinerData->getUser().setAccountLocale(0);//1701729619);
	joinerData->getUser().setAccountCountry(0);//21843);
	...
	joinerData->setTeamId(ANY_TEAM_ID + 1);
	...
	request.getPlayerJoinData().setDefaultRole("");
	request.getPlayerJoinData().setGameEntryType(GAME_ENTRY_TYPE_CLAIM_RESERVATION);
	if (BLAZE_IS_TRACE_RPC_ENABLED(BlazeRpcLog::gamemanager)) {
		char8_t buf[10240];
	...
	if (err == ERR_OK) {
		BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "ESLTournaments::JoinGame success.");
	}
	else
	{
		BLAZE_ERR_LOG(BlazeRpcLog::gamemanager, "ESLTournaments::JoinGame failed. Error(" << ErrorHelp::getErrorName(err) << ")");
	}
	return err;
}

void ESLTournamentsGameSession::addFriendliesCreatedGame(GameId gameId)
{
...
GameId ESLTournamentsGameSession::getRandomCreatedGame()
{
		CreatedGamesList* gameListReference = ((Blaze::Stress::PlayerModule*)mPlayerData->getOwner())->getFriendliesGameList();
		if( gameListReference->size() == 0)
		{
			BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::getRandomCreatedGame] The gameId map is empty " << mPlayerData->getPlayerId() );
			return 0;
		}
		CreatedGamesList::iterator it = gameListReference->begin();
		...
		mMutex.Unlock();
		BLAZE_TRACE_LOG(BlazeRpcLog::gamemanager, "[ESLTournamentsGameSession::getRandomCreatedGame]  Randomly fetched GameId : "<< randomGameId);
		return randomGameId;
}
