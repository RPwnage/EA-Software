/*H********************************************************************************/
/*!
    \File dirtycontextmanagerps.c

   \Description
        dirtycontextManagerPs handles thread synchornization for creating and freeing
...
void DirtyContextManagerShutdown()
{
    ...
    for (i = 0; i < DIRTYCM_MAX_CONTEXTS; ++i)
    {
        ...
        {
            NetPrintf(("dirtycontextmanagerps: warning, http2 context %d was not freed\n", _DirtyContextManagerRef->aHttpContextId[i]));
    ...
    for (i = 0; i < DIRTYCM_MAX_CONTEXTS; ++i)
    {
        ...
        {
            NetPrintf(("dirtycontextmanagerps: warning, http context %d was not freed\n", _DirtyContextManagerRef->aHttpContextId[i]));
    ...
    for (i = 0; i < DIRTYCM_MAX_CONTEXTS; ++i)
    {
        ...
        {
            NetPrintf(("dirtycontextmanagerps: warning, ssl context %d was not freed\n", _DirtyContextManagerRef->aSslContextId[i]));
    ...
    for (i = 0; i < DIRTYCM_MAX_NET_CONTEXTS; ++i)
    {
        ...
        {
            NetPrintf(("dirtycontextmanagerps: warning, net pool %d was not freed\n", _DirtyContextManagerRef->aNetContexts[i]));
...
int32_t DirtyContextManagerCreateNetPoolContext(const char *strName, int32_t iNetPoolSize)
{
    ...
    if (i == DIRTYCM_MAX_NET_CONTEXTS)
    {
        ...
        NetPrintf(("dirtycontextmanagerps: finding a netPool to share.\n"));
    ...
    if (iReturn < 0)
    {
        ...
        NetPrintf(("dirtycontextmanagerps: failed to create netPool, error 0x%08x, finding a netPool to share.\n", iReturn));
        iReturn = _DirtyContextManagerShareNetPool(iNetPoolSize);
        NetCritLeave(&_DirtyContextManagerRef->crit);
        return((iReturn < 0) ? DIRTYCM_ERROR_MAX_CONTEXTS_REACHED : iReturn);
    }
...
int32_t DirtyContextManagerCreateSslContext(int32_t iSslBufferSize)
{
    ...
    if (iReturn < 0)
    {
        NetPrintf(("dirtycontextmanagerps: failed to create ssl context, error 0x%08x\n", iReturn));
    }
    else
    {
...
int32_t DirtyContextManagerCreateHttpContext(int32_t iSceNetContextId, int32_t iSslContextId, int32_t iHttpBufferSize)
{
    ...
    if ((iFinalSize % DIRTYCM_POOLSIZE_FACTOR) != 0)
    {
        ...
        NetPrintf(("dirtycontextmanagerps: the memory pool %d needs to be a multiple of 16k, using next largest multiple %d\n", iFinalSize, iNewBufSize));
        iFinalSize = iNewBufSize;
    }
    ...
    if (iReturn < 0)
    {
        NetPrintf(("dirtycontextmanagerps: failed to create http context, error 0x%08x\n", iReturn));
    }
    else
    {
...
int32_t DirtyContextManagerCreateHttp2Context(int32_t iSceNetContextId, int32_t iSslContextId, uint32_t uConcurrentRequest)
{
    ...
    {
        NetPrintf(("dirtycontextmanagerps: failed to create http2 context, error 0x%08x\n", iReturn));
    }
    else
    {
...
int32_t DirtyContextManagerControl(int32_t iSelect, int32_t iValue, int32_t iValue2, void *pValue)
{
    if (iSelect == 'mncs')
    {
        if (iValue > 0)
        {
            NetPrintf(("dirtycontextmanagerps: setting max net context shares to %d from %u\n", iValue, _DirtyContextManagerRef->uMaxNetContextShares));
            _DirtyContextManagerRef->uMaxNetContextShares = (uint32_t)iValue;
            return(0);
        }
        else
        {
            NetPrintf(("dirtycontextmanagerps: invalid value for max net context shares (%d)\n", iValue));
            return(-1);
        }
    }
    return(-1);
