...
/*!
    ...
    \Input iXboxUserIdLen   - size of caller's buffer (in count of wchar, not bytes)
...
static int32_t _VoipHeadsetActivateLocalTalker(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, bool bParticipate)
{
    ...
    NetPrintf(("voipheadsetxbox: local user (xuid: %llx) at index %d is %s participating\n",
    ...
    if (pXoneRemoteUser == NULL)
    {
        ...
        if (pXoneRemoteUser != NULL)
        {
            if (pXoneRemoteUser->User.ePlatform == VOIP_PLATFORM_XBOXONE)
            {
                ...
                _VoipHeadsetGetXboxUserIdForVoipUser(pHeadset, &pXoneRemoteUser->User, wstrXuid, VOIPHEADSET_XUID_SAFE_LEN);
                pXoneRemoteUser->uXuid = _wtoi64(wstrXuid);

                NetPrintf(("voipheadsetxbox: adding remote talker %lld (xuid: %S, console id: 0x%llx)\n",
...
static int32_t _VoipHeadsetSynthesizeSpeechLoopback(VoipHeadsetRefT *pHeadset, int32_t iLocalUserIndex, const char *pText)
{
    ...
    if (_VoipHeadsetCreateNarrate(pHeadset, iLocalUserIndex) == 0)
    {
        return(VoipNarrateInput(pLocalUser->pNarrateRef, iLocalUserIndex, pLocalUser->eGender, VOIPNARRATE_CBFLAG_NONE, pText));
    }
    else
    {
        NetPrintf(("voipheadsetxbox: failed to start narration module\n"));
...
int32_t VoipHeadsetControl(VoipHeadsetRefT *pHeadset, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'loop')
    {
        ...
        */
        ...
        NetPrintf(("voipheadsetxbox: loopback mode %s\n", (iValue ? "enabled" : "disabled")));
        return(0);
    }
    if (iControl == 'qual')
    {
