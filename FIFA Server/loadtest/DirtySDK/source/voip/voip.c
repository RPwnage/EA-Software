...
/*!
    ...
    \Version 03/26/2020 (mgallant)
*/
/********************************************************************************F*/
int32_t VoipLocalUserStatus2(VoipRefT *pVoip, int32_t iLocalUserIndex, int32_t *pFirstPartyChatIndicator)
{
    VoipCommonRefT *pVoipCommon = (VoipCommonRefT *)pVoip;
    uint32_t uLocalStatus = pVoipCommon->Connectionlist.uLocalUserStatus[iLocalUserIndex];
    ...
    {
        *pFirstPartyChatIndicator = VoipHeadsetStatus(pVoipCommon->pHeadset, 'fpcl', iLocalUserIndex, NULL, 0);
    }
...
void VoipConfigTranscription(VoipRefT *pVoip, uint32_t uProfile, const char *pUrl, const char *pKey)
{
    ...
    VoipCommonRefT *pVoipCommon = (VoipCommonRefT *)pVoip;
    VoipCommonSpeechCertSetup((VoipSpeechProviderE)VOIPTRANSCRIBE_PROFILE_PROVIDER(uProfile));
...
{
    #if defined(DIRTYCODE_PC) || defined(DIRTYCODE_PS4) || defined(DIRTYCODE_STADIA)
    VoipCommonRefT *pVoipCommon = (VoipCommonRefT *)pVoip;
    VoipCommonSpeechCertSetup((VoipSpeechProviderE)uProvider);
    VoipHeadsetConfigNarration(pVoipCommon->pHeadset, (VoipSpeechProviderE)uProvider, pUrl, pKey);
    #endif
}

/*F********************************************************************************/
/*!
    \Function   VoipConfigTranslation

    \Description
        Configure voice translation

    \Input *pVoip       - module ref
    \Input uProvider    - translate provider
    \Input *pUrl        - translate provider url
    \Input *pKey        - translate key

    \Version 09/24/2019 (jbrookes)
*/
/********************************************************************************F*/
void VoipConfigTranslation(VoipRefT *pVoip, uint32_t uProvider, const char *pUrl, const char *pKey)
{
    #if defined(DIRTYCODE_PC)
    VoipCommonRefT *pVoipCommon = (VoipCommonRefT *)pVoip;
    VoipCommonSpeechCertSetup((VoipSpeechProviderE)uProvider);
    VoipHeadsetConfigTranslation(pVoipCommon->pHeadset, (VoipSpeechProviderE)uProvider, pUrl, pKey);
    #endif
}
