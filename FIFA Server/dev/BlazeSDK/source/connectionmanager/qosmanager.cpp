...
#endif
{
    ...
    mNetworkInfo.getQosData().setQosProfileVersion(0);
...
void QosManager::qosClientCb(QosClientRefT *pQosClient, QosCommonProcessedResultsT *pCBInfo, void *pUserData)
{
    ...
    if (pCBInfo->uNumResults > 0)
    {
        ...
        //set the version of the qos profile, provided by the coordinator, that was used in generating these results
        qosManager->mNetworkInfo.getQosData().setQosProfileVersion(pCBInfo->iProfileVersion);
