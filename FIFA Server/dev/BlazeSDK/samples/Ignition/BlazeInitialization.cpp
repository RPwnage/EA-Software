...
#include "eadp/foundation/EnvironmentService.h"
#include "eadp/foundation/DirectorService.h"

namespace Ignition
{

extern void hexStrToBin(const char *pHexStr, uint8_t *pBinBuf, size_t binBufSize);
...
void BlazeInitialization::actionStartupBlaze(Pyro::UiNodeAction *action, Pyro::UiNodeParameterStructPtr parameters)
{
    ...
    if (blazeError == Blaze::ERR_OK)
    {
        ...
        //initialize eadp hub with hub config
        eadp::foundation::HubConfig hubConfig;
        hubConfig.allocator = EA::Allocator::ICoreAllocator::GetDefaultAllocator();
        hubConfig.clientId = IGNITION_CLIENT_ID;
#if defined(EA_PLATFORM_PS4) || defined(EA_PLATFORM_PS4_CROSSGEN) || defined(EA_PLATFORM_PS5)
        //title id and title secret are required for sony platforms only
        hubConfig.titleId = IGNITION_NP_TITLE_ID;
        uint8_t titleSecret[SCE_NP_TITLE_SECRET_SIZE];
        memset(titleSecret, 0, sizeof(titleSecret));
        hexStrToBin(IGNITION_NP_TITLE_SECRET_HEX, titleSecret, sizeof(titleSecret));
        hubConfig.titleSecret.append((char8_t*)titleSecret, sizeof(titleSecret));
#endif
        
        gEadpHub = eadp::foundation::Hub::create(hubConfig);
        gEadpHub->getLoggingService()->setThresholdLogLevel(eadp::foundation::ILoggingService::LogLevel::VERBOSE);
        gEadpHub->getEnvironmentService()->setEnvironmentIdentifier("INT");
        gEadpHub->getEnvironmentService()->setReleaseType("prod");
        gEadpHub->getDirectorService()->setConfigurationValue("eadp.identity", "https://accounts.int.ea.com");
