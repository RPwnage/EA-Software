...
struct GameQualityFactorSpec
{
    ...
    bool mRequiresPropertyFieldValues = false; // whether this factor handles unset property field values, or treats them as 0(default)
    bool mRequiresIncrementalTeamFill = false;
...
struct GameQualityFactor
{
    ...
    Score computeSortedFieldScores(FieldScoreList& fieldScoreList, const Game& game, GameState state = GameState::PRESENT) const;
    bool hasValidPropertyRequirements() const;
    PropertyDescriptor* getInputPropertyDescriptor() const { return mInputProperty; }
    float getEvaluatedValueFromScore(Score score) const;
