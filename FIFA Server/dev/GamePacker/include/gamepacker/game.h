...
struct GamePlayer
{
    PlayerId mPlayerId = 0; // PACKER_MAYBE: Point back to Player* instead of playerId. This way we don't need to do any lookups and we always have what we need, also makes debugging easier...
...
struct GameParty 
{
    PartyId mPartyId = 0; // PACKER_MAYBE: Point back to Party* instead of partyId. This way we don't need to do any lookups and we always have what we need, also makes debugging easier...
    uint16_t mPlayerCount = 0;
    uint16_t mPlayerOffset = 0;
    bool mImmutable = false; // whether the party can have its game or team assignment modified // IMPROVE: We can have a util function taht says whetehr a party is immutable (check id) stateless
};
...
struct GameInternalState
{
    ...
    uint16_t mPlayerCountByTeam[MAX_TEAM_COUNT];
    ...
    void unassignPartyToTeam(int32_t partyIndex, int32_t playerCount);
    uint32_t getPlayersAssignedToTeams(int32_t teamCount);
};
...
struct Game
{
    ...
    void markPartyFieldsForRemoval(PartyIndex partyIndex);
    ...
    const GameTemplate& getGameTemplate() const;
};
