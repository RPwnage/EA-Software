...
static CommandLineOption cmdOptTeamCount            = { "-teamcount","-tn" };
...
struct DefaultUtilityMethods : public PackerUtilityMethods {
    ...
    {
        EA_FAIL_MSG("Intentionally disabled!");
        return 0;
    }
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    int32_t teamCount = 2;
    if (cmdOptTeamCount.matchOption(*commandLine, matchResult))
    {
        const char8_t* arg = commandLine->Arg(matchResult + 1);
        int32_t num = EA::StdC::AtoI32(arg);
        if (num > 0)
            teamCount = (decltype(teamCount))num;
    }
    ...
    if (cmdOptQualityFactors.matchOption(*commandLine, matchResult))
    {
        ...
        while (token != nullptr)
        {
            ...
            bool exactTransformMatch = false;
            for (auto& spec : getAllFactorSpecs()) 
            {
                if (spec.mTransformName == transformAndShaper && propertyNameAndField.empty())
                {
                    exactTransformMatch = true;
                    break;
                }
            }

            eastl::string propertyName;
            if (!exactTransformMatch)
            {
                eastl::string fieldName;
            ...
            else
            {
                ...
                else if (config.mTransform == "size.count_underfilled")
                {
                    config.mBestValue = 0;
                    config.mViableValue = 1;
                    config.mWorstValue = (float)packer.getVar(GameTemplateVar::TEAM_COUNT);
                }
...
#define GAME_REPORT_COLUMN_PADDING " "
    ...
    for (auto& gameItr : packer.getGames())
    {
        ...
        eastl::vector<eastl::array<PlayerInfo, MAX_PLAYER_COUNT>> playerInfoByTeam;
        playerInfoByTeam.resize(teamCount);
        eastl::vector<int32_t> playerCountByTeam;
        playerCountByTeam.resize(teamCount);
        for (int32_t i = 0, size = game.getPartyCount(GameState::PRESENT); i < size; ++i)
        {
