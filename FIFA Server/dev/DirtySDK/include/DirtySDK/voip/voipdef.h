...
#define VOIP_CONN_CONNECTING      (16)      //!< in the process of connecting
...
typedef enum VoipCbTypeE
{
    ...
    VOIP_CBTYPE_CONNEVENT,          //!< unhandled connection event
} VoipCbTypeE;
...
typedef struct VoipConnEventInfoT
{
    uint32_t uRecvAddr;
    uint32_t uRecvPort;
    uint32_t uClientId;
...
//! possible speech service providers
typedef enum VoipSpeechProviderE
{
    VOIPSPEECH_PROVIDER_NONE,
    VOIPSPEECH_PROVIDER_IBMWATSON,          //!< IBM Watson speech services
    VOIPSPEECH_PROVIDER_MICROSOFT,          //!< Microsoft speech services
    VOIPSPEECH_PROVIDER_GOOGLE,             //!< Google speech services
    VOIPSPEECH_PROVIDER_AMAZON,             //!< Amazon speech services
    VOIPSPEECH_NUMPROVIDERS
} VoipSpeechProviderE;
...
//! extended event callback function prototype
typedef void (VoipCallbackExT)(VoipRefT *pVoip, VoipCbTypeE eCbType, int32_t iValue, const void *pCbData, void *pUserData);
