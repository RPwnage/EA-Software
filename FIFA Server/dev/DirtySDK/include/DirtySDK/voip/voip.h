...
// return information about local hardware state on a per-user basis
DIRTYCODE_API int32_t VoipLocalUserStatus2(VoipRefT *pVoip, int32_t iLocalUserIndex, int32_t *pFirstPartyChatIndicator);
...
// config text narration (for the default config set uProvider to -1)
DIRTYCODE_API void VoipConfigTranslation(VoipRefT *pVoip, uint32_t uProvider, const char *pUrl, const char *pKey);
