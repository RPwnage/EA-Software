...
//! defines for header parsing
#define HPACK_NAME_MAX                      (256)
#define HPACK_VALUE_MAX                     (8*1024)
...
{
    uint32_t aResult[HPACK_VALUE_MAX];
    uint32_t uCount;
...
static const HuffmanCodeT Hpack_aHuffmanCodes[HPACK_HUFFMAN_TABLE_SIZE] =
{
...
static uint8_t _HpackFindIndexedField(HpackRefT *pRef, const StaticTableEntryT *pEntry, uint32_t *pIndex)
{
    ...
    {
        if (strncmp(pEntry->pName, Hpack_aStaticTable[uIndex].pName, HPACK_NAME_MAX) == 0 &&
            strncmp(pEntry->pValue, Hpack_aStaticTable[uIndex].pValue, HPACK_VALUE_MAX) == 0)
        {
            *pIndex = uIndex+1;
            return(TRUE);
        }
    ...
    {
        if (strncmp(pEntry->pName, pFound->pName, HPACK_NAME_MAX) == 0 &&
            strncmp(pEntry->pValue, pFound->pValue, HPACK_VALUE_MAX) == 0)
        {
...
static uint8_t _HpackFindIndexedFieldName(HpackRefT *pRef, const char *pName, uint32_t *pIndex)
{
    ...
    for (uIndex = 0; uIndex < HPACK_STATIC_TABLE_SIZE; uIndex += 1)
    {
        if (strncmp(pName, Hpack_aStaticTable[uIndex].pName, HPACK_NAME_MAX) == 0)
        {
            *pIndex = uIndex+1;
            return(TRUE);
        }
    ...
    while ((pFound = _HpackDynamicTableGet(pRef, uIndex)) != NULL)
    {
        if (strncmp(pName, pFound->pName, HPACK_NAME_MAX) == 0)
        {
            *pIndex = uIndex+HPACK_STATIC_TABLE_SIZE+1;
            return(TRUE);
        }
...
{
    char strName[HPACK_NAME_MAX], strValue[HPACK_VALUE_MAX];
