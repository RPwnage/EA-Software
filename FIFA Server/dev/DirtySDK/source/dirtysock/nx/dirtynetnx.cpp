...
static void _SocketNIFMUpdate(SocketStateT *pState)
{
    ...
    if (pState->eSocketNIFMState == SocketNIFMStateE::SOCKET_NIFM_INITIALIZE_SOCKET)
    {
        if (_SocketInit() == 0)
        {
            #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
            nn::socket::InAddr netOutInAddr;
            #else
            ...
            NetPrintf(("dirtynetnx: setting network +onl state.\n"));
    ...
    {
        if (nn::nifm::IsNetworkAvailable() == false)
        {
            pState->uConnStatus = '-dsc';
            pState->eSocketNIFMState = SocketNIFMStateE::SOCKET_NIFM_UNINITIALIZED;
...
static int32_t _SocketCreateSocket(int32_t iAddrFamily, int32_t iType, int32_t iProto)
{
    ...
    // create socket
    #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
    if ((iSocket = nn::socket::Socket((nn::socket::Family)iAddrFamily, (nn::socket::Type)iType, (nn::socket::Protocol)iProto)) >= 0)
    {
        const uint32_t uTrue = 1;
        // if dgram, allow broadcast
        if (iType == SOCK_DGRAM)
        {
            nn::socket::SetSockOpt(iSocket, nn::socket::Level::Sol_Socket, nn::socket::Option::So_Broadcast, &uTrue, sizeof(uTrue));
        }
        // if a raw socket, set header include
        if (iType == SOCK_RAW)
        {
            nn::socket::SetSockOpt(iSocket, nn::socket::Level::Sol_Ip, nn::socket::Option::Ip_HdrIncl, &uTrue, sizeof(uTrue));
        }
        // set nonblocking operation
        if (nn::socket::Fcntl(iSocket, nn::socket::FcntlCommand::F_SetFl, O_NONBLOCK) < 0)
        {
    ...
    if (!pSocket->bVirtual && ((pSocket->iType == SOCK_DGRAM) || (pSocket->iType == SOCK_RAW)))
    {
        ...
        if ((pSocket->uSocket = _SocketCreateSocket(pSocket->iFamily, pSocket->iType, pSocket->iProto)) >= 0)
        {
            ...
            if (SockaddrInGetPort(&pSocket->LocalAddr) != 0)
            {
                ...
                #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
                if ((iResult = nn::socket::Bind(pSocket->uSocket, (nn::socket::SockAddr*)&pSocket->LocalAddr, sizeof(pSocket->LocalAddr))) < 0)
                #else
...
static int32_t _SocketSendFromPacketQueue(SocketT *pSocket)
{
    ...
    if ((iResult = SocketPacketQueueGetHead(pSocket->pSendQueue, &pPacketData, &iPacketSize, &pPacketAddr)) > 0)
    {
        ...
        {
            #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
            if ((iResult = (int32_t)nn::socket::Send(pSocket->uSocket, pPacketData, iPacketSize, nn::socket::MsgFlag::Msg_None)) < 0)
            #else
        ...
        else
        {
            ...
            #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
            if ((iResult = (int32_t)nn::socket::SendTo(pSocket->uSocket, pPacketData, iPacketSize, nn::socket::MsgFlag::Msg_None, (nn::socket::SockAddr*)pPacketAddr, sizeof(struct sockaddr))) < 0)
            #else
    ...
    {
        if (_SocketTranslateError(iResult) == SOCKERR_BADPIPE)
        {
...
{
    SocketStateT *pState = _Socket_pState;
    SocketLookupPrivT *pPriv = (SocketLookupPrivT *)_pRef;
    HostentT *pHost = &pPriv->Host;

    #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
    nn::socket::AiErrno iResult;
    nn::socket::AddrInfo Hints, *pList = NULL;
    #else
    int32_t iResult;
    ...
    Hints.ai_socktype = nn::socket::Type::Sock_Stream;     // set specific socktype to limit to one result per type
    Hints.ai_protocol = nn::socket::Protocol::IpProto_Tcp; // set specific proto to limit to one result per type
    #else
    ...
    // start lookup
    NetPrintf(("dirtynetnx: lookup thread start; name=%s (thid=%s)\n", pHost->strName, strThreadId));

    #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
    if ((iResult = nn::socket::GetAddrInfo(pHost->strName, NULL, &Hints, &pList)) == nn::socket::AiErrno::EAi_Success)
    #else
    if ((iResult = nn::socket::GetAddrInfo(pHost->strName, NULL, &Hints, &pList)) == 0)
    #endif
    {
        // extract ip address
        ...
        SocketHostnameCacheAdd(pState->pHostnameCache, pHost->strName, pHost->addr, 1 /*pState->iVerbose*/);
    ...
    {
        // unsuccessful
        NetPrintf(("dirtynetnx: getaddrinfo('%s', ...) failed err=%s\n", pHost->strName, DirtyErrGetNameList((uint32_t)iResult, _GAI_ErrList)));
        pHost->done = -1;
    }
...
static int32_t _SocketRecvfrom(SocketT *pSocket, char *pBuf, int32_t iLen, struct sockaddr *pFrom, int32_t *pFromLen)
{
    ...
    if (pFrom != NULL)
    {
        // do the receive
        #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
        if ((iResult = (int32_t)nn::socket::RecvFrom(pSocket->uSocket, pBuf, iLen, nn::socket::MsgFlag::Msg_None, (nn::socket::SockAddr*)pFrom, (socklen_t *)pFromLen)) > 0)
        #else
    ...
    {
        #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
        iResult = (int32_t)nn::socket::Recv(pSocket->uSocket, pBuf, iLen, nn::socket::MsgFlag::Msg_None);
        #else
...
static void _SocketRecvToPacketQueue(SocketStateT *pState, SocketT *pSocket)
{
    ...
    else
    {
        if (_SocketGetErrno() != (int32_t)nn::socket::Errno::EAgain)
        {
            ...
            /* on NX, Nintendo has implemented custom network stack behaviors related to the scenario where the network
               interface transitions to a disconnected state. Upon network interface disconnect all existing sockets are
               automatically shutdown by the system, causing current and future socket API calls made against them to
               immediately complete with error.
               on the recv side we get back an ENetDown error and on the send side we get an EBadPipe error.

               in this module, the broken UDP sockets will get re-created when send returns EBadPipe. until that time we do not
               want to add this socket back to the poll list. */
            else if ((pSocket->iType == SOCK_DGRAM) && (_SocketGetErrno() == (int32_t)nn::socket::Errno::ENetDown))
            {
                NetPrintf(("dirtynetnx: [%p] marking socket as broken as the network went down\n", pSocket));
                pSocket->uBrokenFlag += 1;
            }
            else
            {
...
static void _SocketRecvThread(void *pArg)
{
    ...
    while (pState->iRecvLife == 1)
    {
        ...
        if (pollList.iCount > 0)
        {
            ...
            #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
            iResult = nn::socket::Poll((nn::socket::PollFd*)pollList.aPollFds, pollList.iCount, 50);
            #else
...
int32_t SocketDestroy(uint32_t uShutdownFlags)
{
    ...
    while (pState->pHostList != NULL)
    {
        ...
        {
            iSocketLookups += (*(volatile SocketLookupPrivT **)(ppHost))->iThread ? 0 : 1;
        }
...
SocketT *SocketImport(intptr_t uSockRef)
{
    ...
    #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
    if (nn::socket::GetSockOpt((int32_t)uSockRef, nn::socket::Level::Sol_Socket, nn::socket::Option::So_Type, &iProto, &iProtoSize) == 0)
    #else
...
int32_t SocketShutdown(SocketT *pSocket, int32_t iHow)
{
    ...
    #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
    nn::socket::ShutdownMethod eMethod = nn::socket::ShutdownMethod::Shut_RdWr;

    // translate how
    if (iHow == SOCK_NOSEND)
    {
        eMethod = nn::socket::ShutdownMethod::Shut_Wr;
    }
    else if (iHow == SOCK_NORECV)
    {
        eMethod = nn::socket::ShutdownMethod::Shut_Rd;
    }
    else if (iHow == (SOCK_NOSEND | SOCK_NORECV))
    {
        eMethod = nn::socket::ShutdownMethod::Shut_RdWr;
    }

    // do the shutdown
    if (nn::socket::Shutdown(pSocket->uSocket, eMethod) < 0)
    #else
...
int32_t SocketBind(SocketT *pSocket, const struct sockaddr *pName, int32_t iNameLen)
{
    ...
    if (pSocket->iType == SOCK_DGRAM)
    {
        ...
        if ((uPort = SockaddrInGetPort(pName)) != 0)
        {
            ...
            if (iPort < SOCKET_MAXVIRTUALPORTS)
            {
                ...
                {
                    #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
                    nn::socket::Shutdown(pSocket->uSocket, nn::socket::ShutdownMethod::Shut_Wr);
                    #else
    ...
    // do the bind
    #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
    if ((iResult = nn::socket::Bind(pSocket->uSocket, (nn::socket::SockAddr*)pName, iNameLen)) < 0)
    #else
    ...
    else if (SockaddrInGetPort(&pSocket->LocalAddr) == 0)
    {
        ...
        #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
        iResult = nn::socket::GetSockName(pSocket->uSocket, (nn::socket::SockAddr*) &pSocket->LocalAddr, (socklen_t *)&iNameLen);
        #else
...
int32_t SocketConnect(SocketT *pSocket, struct sockaddr *pName, int32_t iNameLen)
{
    ...
    pSocket->iOpened = 0;
    #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
    if ((iResult = nn::socket::Connect(pSocket->uSocket, (nn::socket::SockAddr*)pName, iNameLen)) == 0)
    #else
...
SocketT *SocketAccept(SocketT *pSocket, struct sockaddr *pAddr, int32_t *pAddrLen)
{
    ...
    #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
    iResult = nn::socket::Accept(pSocket->uSocket, (nn::socket::SockAddr*)pAddr, (socklen_t *)pAddrLen);
    #else
...
int32_t SocketSendto(SocketT *pSocket, const char *pBuf, int32_t iLen, int32_t iFlags, const struct sockaddr *pTo, int32_t iToLen)
{
    ...
    if ((pSocket->bVirtual == TRUE) || (pSocket->uSocket == INVALID_SOCKET) || (pSocket->bAsyncSend == FALSE))
    {
        ...
        {
            #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
            if ((iResult = (int32_t)nn::socket::Send(pSocket->uSocket, pBuf, iLen, nn::socket::MsgFlag::Msg_None)) < 0)
            #else
        ...
        else
        {
            ...
            #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0) 
            if ((iResult = (int32_t)nn::socket::SendTo(pSocket->uSocket, pBuf, iLen, nn::socket::MsgFlag::Msg_None, (nn::socket::SockAddr*)pTo, iToLen)) < 0)
            #else
...
int32_t SocketInfo(SocketT *pSocket, int32_t iInfo, int32_t iData, void *pBuf, int32_t iLen)
{
    ...
    if (iInfo == 'bind')
    {
        if (iLen >= (signed)sizeof(pSocket->LocalAddr))
        {
            ...
            {
                #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0)
                nn::socket::GetSockName(pSocket->uSocket, (nn::socket::SockAddr*)pBuf, (socklen_t *)&iLen);
                #else
    ...
    */
    ...
    if ((iInfo == 'conn') || (iInfo == 'peer'))
    {
        ...
        {
            #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0)
            nn::socket::GetPeerName(pSocket->uSocket, (nn::socket::SockAddr*)pBuf, (socklen_t *)&iLen);
            #else
    ...
    if (iInfo == 'stat')
    {
        ...
        if (pSocket->iOpened == 0)
        {
            ...
            #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0)
            if (nn::socket::Poll((nn::socket::PollFd*)&PollFd, 1, 0) != 0)
            #else
        ...
        if (!pSocket->bAsyncRecv && (pSocket->iOpened > 0))
        {
            ...
            #endif
            {
                ...
                else if (PollFd.revents & POLLIN)
                {
                    ...
                    #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0)
                    if (nn::socket::Ioctl(pSocket->uSocket, nn::socket::IoctlCommand::FionRead, &iAvailBytes, sizeof(iAvailBytes)) != 0)
                    #else
...
int32_t SocketControl(SocketT *pSocket, int32_t iOption, int32_t iData1, void *pData2, void *pData3)
{
    ...
    */
    ...
    {
        #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0)
        int32_t iVal = nn::socket::Fcntl(pSocket->uSocket, nn::socket::FcntlCommand::F_GetFl, O_NONBLOCK);
        iVal = iData1 ? (iVal | O_NONBLOCK) : (iVal & ~O_NONBLOCK);
        iResult = nn::socket::Fcntl(pSocket->uSocket, nn::socket::FcntlCommand::F_SetFl, iVal);
        #else
    ...
    {
        #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0)
        iResult = nn::socket::SetSockOpt(pSocket->uSocket, nn::socket::Level::Sol_Tcp, nn::socket::Option::Tcp_NoDelay, &iData1, sizeof(iData1));
        #else
    ...
    {
        #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0)
        iResult = nn::socket::SetSockOpt(pSocket->uSocket, nn::socket::Level::Sol_Socket, nn::socket::Option::So_ReuseAddr, (const char *)&iData1, sizeof(iData1));
        #else
    ...
    if ((iOption == 'rbuf') || (iOption == 'sbuf'))
    {
        ...
        nn::socket::GetSockOpt(pSocket->uSocket, nn::socket::Level::Sol_Socket, (nn::socket::Option)iSockOpt, (char *)&iOldSize, &uOptLen);

        // set new size
        iResult = nn::socket::SetSockOpt(pSocket->uSocket, nn::socket::Level::Sol_Socket, (nn::socket::Option)iSockOpt, (const char *)&iData1, sizeof(iData1));
        #else
        // get current buffer size
        ...
        // get new size
        #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0)
        nn::socket::GetSockOpt(pSocket->uSocket, nn::socket::Level::Sol_Socket, (nn::socket::Option)iSockOpt, (char *)&iNewSize, &uOptLen);
        #else
    ...
    if (iOption == 'soli')
    {
        ...
        #if NN_SDK_CURRENT_VERSION_NUMBER >= NN_SDK_VERSION_NUMBER(11, 0, 0 ,0)
        iResult = nn::socket::SetSockOpt(pSocket->uSocket, nn::socket::Level::Sol_Socket, nn::socket::Option::So_Linger, &lingerOptions, sizeof(lingerOptions));
        #else
...
HostentT *SocketLookup(const char *pText, int32_t iTimeout)
{
    ...
    ds_strnzcpy(pHost->strName, pText, sizeof(pHost->strName));
