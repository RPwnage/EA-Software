...
struct QosClientRefT
{
    ...
    uint8_t aOverrideSiteKey[QOS_COMMON_SECURE_KEY_LENGTH];     //!< the secure key for the override qos site
...
static void _QosClientModuleStateTransition(QosClientRefT *pQosClient, QosClientModuleStateE eState, uint32_t uModule, int16_t iCode, int32_t iLogLevel)
{
    ...
    if ((eState == MODULE_STATE_FATAL) || (eState == MODULE_STATE_ERROR))
    {
        // emergency cleanup of connection, if we are going to be successful we may need to talk to a new coordinator instance
        if (pQosClient->iHttpStreamId != 0)
        {
            NetPrintf(("qosclient: aborting stream %d.\n", pQosClient->iHttpStreamId));
            ProtoHttp2Abort(pQosClient->pProtoHttp, pQosClient->iHttpStreamId);
            ProtoHttp2StreamFree(pQosClient->pProtoHttp, pQosClient->iHttpStreamId);
            pQosClient->iHttpStreamId = 0;
            ProtoHttp2Close(pQosClient->pProtoHttp);
        }
    }
...
/*F********************************************************************************/
/*!
    \Function _QosClientApplyTestOverrides

    \Description
        Hard code particular test settings, for debug purposes.

    \Input *pQosClient          - pointer to module state
    \Input *pTestCofigurations  - configurations to apply

    \Version 04/13/2020 (cvienneau)
*/
/********************************************************************************F*/
static void _QosClientApplyTestOverrides(QosClientRefT *pQosClient, QosCommonClientConfigT *pTestCofigurations)
{
    int32_t siteIdx;
    
    if (pQosClient->strOverrideSiteName[0] != '\0')
    {
        for (siteIdx = 0; siteIdx < pTestCofigurations->uNumSites; siteIdx++)
        {
            if (strcmp(pTestCofigurations->aSites[siteIdx].strSiteName, pQosClient->strOverrideSiteName) == 0)
            {
                NetPrintf(("qosclient: _QosClientApplyTestOverrides modifying site:%s.\n", pTestCofigurations->aSites[siteIdx].strSiteName));
                ds_memcpy(pTestCofigurations->aSites[siteIdx].aSecureKey, pQosClient->aOverrideSiteKey, sizeof(pTestCofigurations->aSites[siteIdx].aSecureKey));
                ds_strnzcpy(pTestCofigurations->aSites[siteIdx].strProbeAddr, pQosClient->strOverrideSiteAddr, sizeof(pTestCofigurations->aSites[siteIdx].strProbeAddr));
                pTestCofigurations->aSites[siteIdx].uProbePort = pQosClient->uOverrideSitePort;
                pTestCofigurations->aSites[siteIdx].uProbeVersion = pQosClient->uOverrideSiteVersion;
                break;
            }
        }
    }
...
static int32_t _QosClientProcessCoordinatorResponse(QosClientRefT *pQosClient)
{
    ...
    if (QosCommonCoordinatorToClientResponseDecode(&response, pQosClient->pHttpRecvBuff, pQosClient->uCurrentHttpRecvSize) == 0)
    {
        ...
        pQosClient->rawResultStore.uServiceRequestID = response.uServiceRequestID;  // used in all further communication with the coordinator 
        pQosClient->clientAddressFromCoordinator = response.configuration.clientAddressFromCoordinator; // becomes a part of the probe packets
        pQosClient->finalResults = response.results; // always capture some details that may be available even before test results have been processed, like iProfileVersion and uTimeTillRetry 
        pQosClient->uFinalResultsRecvTime = NetTick();  // used with uTimeTillRetry to kick off a delayed client side restart of the qos process (if the coordinator wants us to)
        ...
        {
            // we do have processed results to share with the client
        ...
        else if ((response.configuration.uNumQosTests > 0) && (response.configuration.uNumSites > 0))
        {
            ...
            // apply any debug overrides to the configuration
            _QosClientApplyTestOverrides(pQosClient, &response.configuration);
...
static void _QosClientReport(QosClientRefT *pQosClient)
{
    ...
    pQosClient->finalResults.hResult = pQosClient->rawResultStore.clientModifiers.hResult;
        ...
        NetPrintfVerbose((pQosClient->iSpamValue, 0, "qosclient: CLIENT(hResult:0x%08x, external addr:%s, fw:%s, profileVer:%d, time:%ums)\n",
            ...
            pQosClient->finalResults.iProfileVersion,
            uProcessTime));
...
/*!
    ...
    \Input *pStrQosProfile      - what profile should the QOS coordinator use for settings, likely a service name like "fifa-2017-pc"
...
void QosClientStart(QosClientRefT *pQosClient, const char *pStrCoordinatorAddr, uint16_t uPort, const char *pStrQosProfile)
{
    ...
    if (pQosClient->eModuleState == MODULE_STATE_IDLE)
    {
        ...
        pQosClient->rawResultStore.uClientVersion = QOS_COMMON_CLIENT_VERSION;
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'orta' - set the override site address for 'orts'
            'ortk' - set the override secure key for 'orts'
            'ortp' - set the override port for 'orts'
            'orts' - set which ping site name to apply overrides to
            'ortv' - set the override version for 'orts'
...
int32_t QosClientControl(QosClientRefT *pQosClient, int32_t iControl, int32_t iValue, void *pValue)
{
    ...
    else if (iControl == 'orta')
    {
        if (pValue != NULL)
        {
            ds_strnzcpy(pQosClient->strOverrideSiteAddr, pValue, sizeof(pQosClient->strOverrideSiteAddr));
        }
        else
        {
            iRet = -1;
    ...
    else if (iControl == 'ortk')
    {
        if (pValue != NULL)
        {
            ds_memcpy(pQosClient->aOverrideSiteKey, pValue, sizeof(pQosClient->aOverrideSiteKey));
        }
        else
        {
            iRet = -1;
    ...
    else if (iControl == 'ortp')
    {
        pQosClient->uOverrideSitePort = iValue;
    }
    else if (iControl == 'orts')
    {
        if (pValue != NULL)
        {
            ds_strnzcpy(pQosClient->strOverrideSiteName, pValue, sizeof(pQosClient->strOverrideSiteName));
        }
        else
        {
            iRet = -1;
    ...
    else if (iControl == 'ortv')
    {
        pQosClient->uOverrideSiteVersion = iValue;
    }
