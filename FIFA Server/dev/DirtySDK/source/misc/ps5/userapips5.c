...
#define USERAPI_LANG_HEADER                   "Accept-Language: %s"
...
#define USERAPI_ONE_ENTRY_BUFFER_SIZE         (4 * 1024)
#define USERAPI_LANGCODE_LEN_MAX              (8)
#define USERAPI_HEADER_LEN_MAX                (32)
...
typedef struct UserApiRefT
{
    ...
    char strLangCode[USERAPI_LANGCODE_LEN_MAX];
} UserApiRefT;
        ...
        if (iRespBodySize <= pRequest->iReadBufferSize)
        {
            if ((iStatusCode >= 200) && (iStatusCode < 300))
            {
                if (iRespBodyLength > 0)
                {
                    ...
                    ds_strnzcpy(pRequest->pReadBuffer, pRespBody, iRespBodyLength + 1);
...
static int32_t _UserApiGetList(UserApiRefT *pRef, uint32_t uUserIndex)
{
    ...
    char strHeader[USERAPI_HEADER_LEN_MAX];
    ...
    ds_snzprintf(strHeader, sizeof(strHeader), USERAPI_LANG_HEADER, pRef->strLangCode);

    //send request
    if ((iWebRequestId = DirtyWebApiRequestEx(pRef->pWebApi, uUserIndex, (pRequest->uUserDataMaskProcessed == USERAPI_MASK_RICH_PRESENCE) ? USERAPI_ACTIVITY_API_GROUP : USERAPI_API_GROUP, SCE_NP_WEBAPI2_HTTP_METHOD_GET, 
        strPath, NULL, 0, NULL, strHeader, &_UserApiDirtyWebApiCallback, pRef)) < 0)
    {
...
UserApiRefT *UserApiCreate(void)
{
    ...
    ds_snzprintf(pRef->strLangCode, sizeof(pRef->strLangCode), "en-US");
...
int32_t UserApiControl(UserApiRefT *pRef, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'lang')
    {
        if ((iValue2 > USERAPI_LANGCODE_LEN_MAX) || (pValue == NULL))
        {
