...
#define USERLISTAPI_ONE_ENTRY_BUFFER_SIZE       (4 * 1024)
...
#define USERLISTAPI_LANG_HEADER                 "Accept-Language: %s"
...
#define USERLISTAPI_LANGCODE_LEN_MAX            (8)
#define USERLISTAPI_HEADER_LEN_MAX              (32)
...
struct UserListApiRefT
{
    ...
    char strLangCode[USERLISTAPI_LANGCODE_LEN_MAX];        //!< the iso language pass into the header of the request
        ...
        if (iRespBodySize <= pRef->iReadBufferSize)
        {
            if ((iStatusCode >= 200) && (iStatusCode < 300))
            {
                if (iRespBodyLength > 0)
                {
                    ...
                    ds_strnzcpy(pRef->pReadBuffer, pRespBody, iRespBodyLength + 1);
...
static int32_t _UserListApiGetList(UserListApiRefT *pRef, UserListRequestT *pRequest)
{
    ...
    int64_t iWebRequestId;
    char strPath[USERLISTAPI_PATH_LEN_MAX];
    char strHeader[USERLISTAPI_HEADER_LEN_MAX];
    ...
    ds_snzprintf(strHeader, sizeof(strHeader), USERLISTAPI_LANG_HEADER, pRef->strLangCode);

    //send request
    if ((iWebRequestId = DirtyWebApiRequestEx(pRef->pWebApi, pRequest->uUserIndex,(pRequest->uMaskProcessed == USERLISTAPI_MASK_RICH_PRESENCE) ? USERLISTAPI_ACTIVITY_API_GROUP : USERLISTAPI_API_GROUP, SCE_NP_WEBAPI2_HTTP_METHOD_GET, strPath, NULL, 0, NULL, strHeader, &_UserListApiDirtyWebApiCallback, pRef)) < 0)
    {
        NetPrintf(("userlistapi5: [%p]  WebApi request failed\n", pRef));
        return((int32_t)iWebRequestId);
    }
    ...
    for (uint32_t iElement = 0; iElement < pRef->uAccountIdsNum; ++iElement)
    {
        ...
        switch (pRequest->uMaskProcessed)
        {
            ...
            case USERLISTAPI_MASK_PRESENCE:
            {
                ...
                pCurElement = JsonFind2(pJsonParse, pList, ".accountId", iElement);
                JsonGetString(pCurElement, strTmpString, sizeof(strTmpString), "");
                accountId = ds_strtoull(strTmpString, NULL, 10);
                DirtyUserFromNativeUser(&EventData.UserData.DirtyUser, &accountId);
        ...
        ds_strsubzcpy(strRawData, sizeof(strRawData), pList, pEntryEnd - pList);
...
static int32_t _UserListApiProcessRequest(UserListApiRefT *pRef)
{
    ...
    switch (pRef->pActiveRequest->eRequestState)
    {
        ...
        case USERLISTAPI_GETACCOUNTIDS_WAIT:
        {
            ...
            else if ((pRef->pActiveRequest->uTypeMask != 0) && (iResult != 1) && (pRef->uAccountIdsNum != 0))
            {
...
UserListApiRefT *UserListApiCreate(uint32_t uMaxResponseSize)
{
    ...
    ds_snzprintf(pState->strLangCode, sizeof(pState->strLangCode), "en-US");
...
/*!
    ...
    \Notes
        ...
        \verbatim
            'lang' - Set the a lanugage for the accept langauge header (must follow the format in https://p.siedev.net/resources/documents/SDK/1.000/PSN_WebAPI-Overview/0003.html?language=en)
        \endverbatim
...
int32_t UserListApiControl(UserListApiRefT *pRef, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    if (iControl == 'lang')
    {
        if ((iValue2 > USERLISTAPI_LANGCODE_LEN_MAX) || (pValue == NULL))
        {
            NetPrintf(("userlistapips5: UserListApiControl('lang') invaid parameter\n", iControl));
            return(-1);
        }

        ds_strnzcpy(pRef->strLangCode, pValue, sizeof(pRef->strLangCode));
        return(0);
    }
