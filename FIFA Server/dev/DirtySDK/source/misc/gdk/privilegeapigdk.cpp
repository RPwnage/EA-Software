...
/*!
    ...
    \Input ePrivilege           - the privilege id as defined by the PrivilegeApi
...
/*F********************************************************************************/
/*!
    \Function _PrivilegeApiDestroyInternal
    ...
    \Version 12/17/2020 (eesponda)
*/
/********************************************************************************F*/
static int32_t _PrivilegeApiDestroyInternal(void *pDestroyData)
{
    PrivilegeApiRequestT *pRequest = (PrivilegeApiRequestT *)pDestroyData;
    ...
    DirtyMemFree(pRequest, PRIVILEGEAPI_MEMID, pRequest->iMemGroup, pRequest->pMemGroupUserData);
    return(0);
}
...
int32_t PrivilegeApiDestroy(PrivilegeApiRefT *pRef)
{
    ...
    {
        if ((pRef->aUserContexts[iUserIndex] != NULL) && XAsyncGetStatus(&pRef->aUserContexts[iUserIndex]->AsyncBlock, false) == E_PENDING)
        {
            XAsyncCancel(&pRef->aUserContexts[iUserIndex]->AsyncBlock);
            NetConnControl('recu', 0, 0, (void *)_PrivilegeApiDestroyInternal, pRef->aUserContexts[iUserIndex]);
        }
        else
        {
            DirtyMemFree(pRef->aUserContexts[iUserIndex], PRIVILEGEAPI_MEMID, pRef->iMemGroup, pRef->pMemGroupUserData);
        }
    }
...
int32_t PrivilegeApiCheckPrivilegesAsyncWithUi(PrivilegeApiRefT *pRef, int32_t iUserIndex, const PrivilegeApiPrivE *pPrivileges, int32_t iPrivilegeCount, int32_t *pHint, const char *pUiMessage)
{
    ...
    {
        NetPrintf(("privilegeapigdk: [%p] invalid user index used for privilege check\n", pRef));
        return(-1);
    }

    /* we allocate the requests as needed, this allows us to keep around the memory for as long as we want. this is
       helpful we have to abort a running async operation. in that case we detach the pointer from the current slot
       so that further requests can be made for this user index. this allocation strategy also minimizes our
       allocations since we don't allocate per request */
    if (pRef->aUserContexts[iUserIndex] == NULL)
    {
        if ((pRequest = (PrivilegeApiRequestT *)DirtyMemAlloc(sizeof(*pRequest), PRIVILEGEAPI_MEMID, pRef->iMemGroup, pRef->pMemGroupUserData)) == NULL)
        {
            NetPrintf(("privilegeapigdk: [%p] failed to allocate memory for request state\n", pRef));
            return(-1);
        }
        ds_memclr(pRequest, sizeof(*pRequest));
        pRequest->iMemGroup = pRef->iMemGroup;
        pRequest->pMemGroupUserData = pRef->pMemGroupUserData;
        pRef->aUserContexts[iUserIndex] = pRequest;
    }
    else
    {
        pRequest = pRef->aUserContexts[iUserIndex];
    }
...
int32_t PrivilegeApiCheckResult(PrivilegeApiRefT *pRef, int32_t iRequestId)
{
    ...
    const int32_t iUserIndex = iRequestId - 1;

    // check request indexing
    if ((iUserIndex < 0) || (iUserIndex > NETCONN_MAXLOCALUSERS) || (pRef->aUserContexts[iUserIndex] == NULL))
    {
        NetPrintf(("privilegeapigdk: [%p] invalid request id %d specified\n", pRef, iRequestId));
        return(-1);
    }
    pRequest = pRef->aUserContexts[iUserIndex];
...
int32_t PrivilegeApiReleaseRequest(PrivilegeApiRefT *pRef, int32_t iRequestId)
{
    ...
    // pending requests cannot be cleaned up
    if (XAsyncGetStatus(&pRequest->AsyncBlock, false) == E_PENDING)
    {
        NetPrintf(("privilegeapigdk: [%p] request is still pending and cannot be cleaned up. use abort first if the request is no longer needed\n", pRef));
        return(-1);
    }
...
int32_t PrivilegeApiAbort(PrivilegeApiRefT *pRef, int32_t iRequestId)
{
    ...
    // if the request is still pending, cancel and detach it for external cleanup
    if (XAsyncGetStatus(&pRequest->AsyncBlock, false) == E_PENDING)
    {
        XAsyncCancel(&pRequest->AsyncBlock);
        NetConnControl('recu', 0, 0, (void *)_PrivilegeApiDestroyInternal, pRequest);
        pRef->aUserContexts[iUserIndex] = NULL;
    }
    else
    {
