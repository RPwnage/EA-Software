...
/*F********************************************************************************/
/*!
    \Function _CryptMont25519Multiply

    \Description
        Perform the curve multiplication based of the private and the input using the
        25519 module

    \Input *pState      - curve state
    \Input *pPoint      - point we are multiplying by the private key
    \Input *pResult     - [out] optional output of the result
    \Input *pCryptUsecs - [out] optional output of the timing information

    \Output
        int32_t         - always returns zero as required by the interface we are using

    \Version 07/13/2020 (eesponda)
*/
/********************************************************************************F*/
static int32_t _CryptMont25519Multiply(CryptMontT *pState, CryptBnT *pPoint, CryptEccPointT *pResult, uint32_t *pCryptUsecs)
{
    uint8_t aPoint[32], aResult[32], aA24[32];
    uint64_t uTickUsecs;
    FieldElementT X1, X2, X3, Z2, Z3, A24;

    // initialize the operation
    pState->iBitIndex = CryptBnBitLen(&pState->Prime) - 1;
    pState->uCryptUsecs = 0;

    // init x1 based on input
    ds_memclr(aPoint, sizeof(aPoint));
    CryptBnFinalLe(pPoint, aPoint, sizeof(aPoint));
    Crypt25519FieldFromBytes(&X1, aPoint);
    // init other fields
    Crypt25519FieldInit(&X2, 1);
    Crypt25519FieldInit(&Z2, 0);
    Crypt25519FieldCopy(&X3, &X1);
    Crypt25519FieldInit(&Z3, 1);
    // init a24 field from big number
    ds_memclr(aA24, sizeof(aA24));
    CryptBnFinalLe(&pState->A24, aA24, sizeof(aA24));
    Crypt25519FieldFromBytes(&A24, aA24);

    for (; pState->iBitIndex >= 0; pState->iBitIndex -= 1)
    {
        FieldElementT A, AA, B, BB, C, D, DA, CB, E;
        const uint8_t bBitSet = CryptBnBitTest(&pState->PrivateKey, pState->iBitIndex);
        uTickUsecs = NetTickUsec();

        pState->uSwap ^= bBitSet;

        // constant time conditional swap
        Crypt25519FieldConditionSwap(&X2, &X3, pState->uSwap);
        Crypt25519FieldConditionSwap(&Z2, &Z3, pState->uSwap);

        pState->uSwap = bBitSet;

        // A = x_2 + z_2
        Crypt25519FieldAdd(&A, &X2, &Z2);

        // AA = A ^ 2
        Crypt25519FieldSquare(&AA, &A);

        // B = x_2 - z_2
        Crypt25519FieldSubtract(&B, &X2, &Z2);

        // BB = B ^ 2
        Crypt25519FieldSquare(&BB, &B);

        // E = AA - BB
        Crypt25519FieldSubtract(&E, &AA, &BB);

        // C = x_3 + z_3
        Crypt25519FieldAdd(&C, &X3, &Z3);

        // D = x_3 - z_3
        Crypt25519FieldSubtract(&D, &X3, &Z3);

        // DA = D * A
        Crypt25519FieldMultiply(&DA, &D, &A);

        // CB = C * B
        Crypt25519FieldMultiply(&CB, &C, &B);

        // x_3 = (DA+CB)^2
        Crypt25519FieldAdd(&X3, &DA, &CB);
        Crypt25519FieldSquare(&X3, &X3);

        // z_3 = u * ((DA-CB)^2)
        Crypt25519FieldSubtract(&Z3, &DA, &CB);
        Crypt25519FieldSquare(&Z3, &Z3);
        Crypt25519FieldMultiply(&Z3, &Z3, &X1);

        // x_2 = (AA * BB)
        Crypt25519FieldMultiply(&X2, &AA, &BB);

        // z_2 = E * (AA + (a24 * E))
        Crypt25519FieldMultiply(&Z2, &A24, &E);
        Crypt25519FieldAdd(&Z2, &Z2, &AA);
        Crypt25519FieldMultiply(&Z2, &Z2, &E);
    ...
    // result = x_2 * (z_2 ^ (p - 2))
    Crypt25519FieldInvert(&Z2, &Z2);
    Crypt25519FieldMultiply(&X2, &X2, &Z2);
    Crypt25519FieldToBytes(aResult, &X2);
    CryptBnInitLeFrom(&pState->Result.X, aResult, sizeof(aResult));
    if (pResult != NULL)
    {
    ...
    // save timings to output if one was provided
    if (pCryptUsecs != NULL)
    {
...
/*F********************************************************************************/
/*!
    \Function _CryptMont448Multiply

    \Description
        Perform the curve multiplication based of the private and the input using the
        cryptbn module as we don't have an optimized version of 448 curve math
    ...
    \Output
        int32_t         - 0=complete, 1=in-progress

    \Version 04/11/2018 (eesponda)
*/
/********************************************************************************F*/
static int32_t _CryptMont448Multiply(CryptMontT *pState, CryptBnT *pPoint, CryptEccPointT *pResult, uint32_t *pCryptUsecs)
{
    int32_t iResult = 1;

    if ((pState->eState == CRYPTMONT_COMPUTE_POINT) && (_CryptMontPointCalculate(pState, pPoint)))
    {
...
/*F********************************************************************************/
/*!
    \Function CryptMontCleanup

    \Description
        Cleanup the state

    \Input *pState      - curve state we are cleaning up

    \Version 12/08/2020 (eesponda)
*/
/********************************************************************************F*/
void CryptMontCleanup(CryptMontT *pState)
{
...
int32_t CryptMontPublic(CryptMontT *pState, CryptEccPointT *pResult, uint32_t *pCryptUsecs)
{
    if (pState->iCurveType == CRYPTCURVE_X25519)
    {
        return(_CryptMont25519Multiply(pState, &pState->BasePoint, pResult, pCryptUsecs));
    }
    else if (pState->iCurveType == CRYPTCURVE_X448)
    {
        return(_CryptMont448Multiply(pState, &pState->BasePoint, pResult, pCryptUsecs));
    }
    return(-1);
}
...
int32_t CryptMontSecret(CryptMontT *pState, CryptEccPointT *pPublicKey, CryptEccPointT *pResult, uint32_t *pCryptUsecs)
{
    if (pState->iCurveType == CRYPTCURVE_X25519)
    {
        return(_CryptMont25519Multiply(pState, &pPublicKey->X, pResult, pCryptUsecs));
    }
    else if (pState->iCurveType == CRYPTCURVE_X448)
    {
        return(_CryptMont448Multiply(pState, &pPublicKey->X, pResult, pCryptUsecs));
    }
    return(-1);
}
