...
struct ProtoHttpServRefT
{
    ...
    ProtoHttpServCompleteCbT *pCompleteCb;
...
static void _ProtoHttpServDisconnect(ProtoHttpServRefT *pHttpServ, ProtoHttpServThreadT *pHttpThread, uint8_t bGraceful)
{
    ...
    // let higher-level module know that it is time to free up resources associated with abandoned request
    if (pHttpServ->pCompleteCb != NULL)
    {
        pHttpServ->pCompleteCb(&pHttpThread->RequestInfo, &pHttpThread->ResponseInfo, pHttpServ->pUserData);
    }
}
...
static void _ProtoHttpServUpdateThread(ProtoHttpServRefT *pHttpServ, ProtoHttpServThreadT *pHttpThread)
{
    ...
    if (((pHttpThread->ResponseInfo.iContentLength > 0) && (pHttpThread->RequestInfo.eRequestType != PROTOHTTP_REQUESTTYPE_HEAD)) || (pHttpThread->ResponseInfo.iChunkLength > 0))
    {
    ...
    else if (pHttpServ->pCompleteCb != NULL)
    {
        /* The complete callback is typically invoked from _ProtoHttpServDisconnect() when the underlying TCP
           connection is abandoned. It is also necessary to call it here to cover for cases where the http
           transaction (request/response) is ended without closing the underlying TCP connection which persistss
           and gets re-used for the next http transaction. */
        pHttpServ->pCompleteCb(&pHttpThread->RequestInfo, &pHttpThread->ResponseInfo, pHttpServ->pUserData);
    } 
...
{
    ProtoHttpServCallback2(pHttpServ, pRequestCb, pReceiveCb, pSendCb, pHeaderCb, NULL, pLogCb, pUserData);
}

/*F********************************************************************************/
/*!
    \Function ProtoHttpServCallback2
    ...
    \Input *pSendCb     - outbound data handler callback
    \Input *pHeaderCb   - inbound header callback
    \Input *pCompleteCb - complete callback (optional)
    \Input *pLogCb      - logging function to use (optional)
    \Input *pUserData   - user data for callbacks

    \Version 03/04/2021 (mclouatre)
*/
/********************************************************************************F*/
void ProtoHttpServCallback2(ProtoHttpServRefT *pHttpServ, ProtoHttpServRequestCbT *pRequestCb, ProtoHttpServReceiveCbT *pReceiveCb, ProtoHttpServSendCbT *pSendCb, ProtoHttpServHeaderCbT *pHeaderCb, ProtoHttpServCompleteCbT *pCompleteCb, ProtoHttpServLogCbT *pLogCb, void *pUserData)
{
    pHttpServ->pRequestCb = pRequestCb;
    pHttpServ->pReceiveCb = pReceiveCb;
    pHttpServ->pSendCb = pSendCb;
    pHttpServ->pHeaderCb = pHeaderCb;
    pHttpServ->pCompleteCb = pCompleteCb;
...
/*!
    ...
    \verbatim
            SELECTOR    DESCRIPTION
            'crpt'      Returns if crypto operation ongoing for any active thread
    \endverbatim

    \Version 03/09/2020 (jbrookes)
*/
/********************************************************************************F*/
int32_t ProtoHttpServStatus(ProtoHttpServRefT *pHttpServ, int32_t iSelect, void *pBuffer, int32_t iBufSize)
{
    if (iSelect == 'crpt')
    {
        ...
        {
            if ((pHttpThread->pProtoSSL != NULL) && (ProtoSSLStat(pHttpThread->pProtoSSL, 'crpt', NULL, 0) == TRUE))
            {
                return(TRUE);
            }
        }
