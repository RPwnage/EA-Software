...
static void _VoipNarrateStart(VoipNarrateRefT *pVoipNarrate, VoipNarrateRequestT *pRequest)
{
    ...
    pVoipNarrate->pVoiceDataCb(pVoipNarrate, pRequest->iUserIndex, NULL, VOIPNARRATE_STREAM_START, pRequest->uFlags, pVoipNarrate->pUserData);
            ...
            [pVoipNarrate, pRequest](IAsyncOperation<SpeechSynthesisStream ^> ^refAsyncOp, Windows::Foundation::AsyncStatus eStatus)
        {
            switch (eStatus)
            {
                case Windows::Foundation::AsyncStatus::Completed:
                {
                    try
                    {
                        // setup the reader first before assigning the result which ungates the use in the update
                        SpeechSynthesisStream ^refResult = refAsyncOp->GetResults();
                        pRequest->refReader = ref new DataReader(refResult);
                        pRequest->refReader->UnicodeEncoding = UnicodeEncoding::Utf8;
                        pRequest->refReader->ByteOrder = ByteOrder::LittleEndian;

                        // reader is setup, we can save the result
                        pRequest->refResult = refResult;
...
static void _VoipNarrateProcessResult(VoipNarrateRefT *pVoipNarrate, VoipNarrateRequestT *pRequest)
{
    ...
    if (pVoipNarrate->iDataLen > 0)
    {
        ...
        while (pVoipNarrate->iDataOff < pVoipNarrate->iDataLen)
        {
            ...
            if ((iRead = pVoipNarrate->pVoiceDataCb(pVoipNarrate, pRequest->iUserIndex, (const int16_t *)(pVoipNarrate->pData+pVoipNarrate->iDataOff), iLength, pRequest->uFlags, pVoipNarrate->pUserData)) > 0)
            {
...
static void _VoipNarrateRequestTransition(VoipNarrateRefT *pVoipNarrate, VoipNarrateRequestT *pRequest, bool bAnnounceEnd)
{
    ...
    if (bAnnounceEnd)
    {
        pVoipNarrate->pVoiceDataCb(pVoipNarrate, pRequest->iUserIndex, NULL, VOIPNARRATE_STREAM_END, pRequest->uFlags, pVoipNarrate->pUserData);
    }
...
/*!
    ...
    \Input uFlags           - VOIPNARRATE_CBFLAG_NONE
...
int32_t VoipNarrateInput(VoipNarrateRefT *pVoipNarrate, int32_t iUserIndex, VoipNarrateGenderE eGender, uint32_t uFlags, const char *pText)
{
    ...
    NetPrintfVerbose((pVoipNarrate->iVerbose, 1, "voipnarratexboxone: queueing request '%s'\n", pText));
