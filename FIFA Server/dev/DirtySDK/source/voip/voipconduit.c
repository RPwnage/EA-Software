...
/*!
    ...
    \Input bEncoded         - TRUE if data is encoded, FALSE if data is not encoded

    \Version 03/21/2004 (jbrookes)
*/
/********************************************************************************F*/
void VoipConduitReceiveVoiceData(VoipConduitRefT *pConduitRef, VoipUserT *pRemoteUser, const uint8_t *pData, int32_t iDataSize, uint8_t bEncoded)
{
        ...
        if (VOIP_SameUser(&pConduit->PacketUser, pRemoteUser))
        {
            ...
            for (iMixerIndex = 0; iMixerIndex < VOIP_MAXLOCALUSERS; ++iMixerIndex)
            {
                ...
                if (pConduitRef->apMixers[iMixerIndex] != NULL)
                {
                    ...
                    if (bPlayback)
                    {
                        if (VoipMixerAccumulate(pConduitRef->apMixers[iMixerIndex], (uint8_t *)pData, iDataSize, iConduitMask, iConduit, bEncoded) < 0)
                        {
