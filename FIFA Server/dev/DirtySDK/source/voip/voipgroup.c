...
typedef struct VoipGroupConnT
{
    ...
    uint32_t uRemoteLowLevelConnectivityId; //<! low level remote connectivity id associated with this high-level connection
    uint32_t uLocalLowLevelConnectivityId;  //<! low level local connectivity id associated with this high-level connection
...
static int32_t _VoipGroupManagerDisplayTranscribedTextCallback(int32_t iLowLevelConnId, int32_t iUserIndex, const char *pStrUtf8, void *pUserData)
{
    ...
    for (iGroup = 0, bDisplayed = FALSE; (iGroup < pManager->iMaxGroups) && !bDisplayed; iGroup++)
    {
        if ((pManager->aGroups[iGroup].bUsed) && (pManager->aGroups[iGroup].pDisplayTranscribedTextCb))
        {
            ...
            else // display transcribed text from local user
            {
                // forward to display callback; for remote users the source language prefix is skipped on the receiving side, but we do it here for local users
                bDisplayed = pManager->aGroups[iGroup].pDisplayTranscribedTextCb(-1, iUserIndex, pStrUtf8+5, pManager->aGroups[iGroup].pDisplayTranscribedTextUserData);
            }
        }
    }
    return(TRUE);
...
static void _VoipGroupPrintRefsToLowLevelConn(int32_t iLowLevelConnId)
{
    ...
    {
        char strVoipGroupReferencesToLowLevelConnId[128];
        char strReference[20];
        ds_memclr(strVoipGroupReferencesToLowLevelConnId, sizeof(strVoipGroupReferencesToLowLevelConnId));
        ds_memclr(strReference, sizeof(strReference));
        for (iRefIndex = 0; iRefIndex < MAX_REF_PER_LOW_LEVEL_CONNS; iRefIndex++)
        {
            ...
            {
                ds_snzprintf(strReference, sizeof(strReference), "%p  ", pManager->aLowLevelConnReferences[iLowLevelConnId][iRefIndex]);
                ds_strnzcat(strVoipGroupReferencesToLowLevelConnId, strReference, sizeof(strVoipGroupReferencesToLowLevelConnId));
            }
            else
            {
                break;
        ...
        NetPrintf(("voipgroup: references to low-level voip conn %d are:   %s\n", iLowLevelConnId, strVoipGroupReferencesToLowLevelConnId));
    }
    else
    {
...
/*F********************************************************************************/
/*!
    \Function   VoipGroupSetEventCallbackEx

    \Description
        Sets the extended event callback and userdata for the specified group

    \Input *pVoipGroup  - VoipGroup to use
    \Input *pCallbackEx - pointer to the callback to use
    \Input *pUserData   - pointer to the user specified data

    \Version 10/01/2019 (jbrookes)
*/
/********************************************************************************F*/
void VoipGroupSetEventCallbackEx(VoipGroupRefT *pVoipGroup, VoipCallbackExT *pCallbackEx, void *pUserData)
{
    VoipCommonSetEventCallbackEx((VoipCommonRefT *)VoipGetRef(), pCallbackEx, pUserData);
}
...
/*!
    ...
    \Input uRemoteLowLevelConnectivityId - low level remote client id
    \Input uLocalLowLevelConnectivityId - low level local client id
...
*/
/********************************************************************************F*/
int32_t VoipGroupConnect(VoipGroupRefT *pVoipGroup, int32_t iConnId, uint32_t uAddress, uint32_t uManglePort, uint32_t uGamePort, uint32_t uClientId, uint32_t uSessionId, uint8_t bIsConnectivityHosted, uint32_t uRemoteLowLevelConnectivityId, uint32_t uLocalLowLevelConnectivityId)
{
    ...
    // if the conn id is not valid, return an error.
    if ((iConnId < 0) || (iConnId >= MAX_CLIENTS_PER_GROUP))
    {
        NetPrintf(("voipgroup: [%p] critical error - invalid conn id\n", pVoipGroup));
        return(-1);
    }
    ...
    if ((iHighLevelConnIdInUse = _VoipGroupFindUsedHighLevelConn(pVoipGroup, uClientId)) != VOIP_CONNID_NONE)
    {
        ...
        if (iHighLevelConnIdInUse == iConnId)
        {
            NetPrintf(("voipgroup: [%p] current voipgroup already knows about client id 0x%08x (low level id 0x%08x) with high-level conn id %d\n", 
                pVoipGroup, uClientId, uRemoteLowLevelConnectivityId, iConnId));
        ...
        else
        {
            NetPrintf(("voipgroup: [%p] critical error - current voipgroup already knows about client id x%08x (low level id 0x%08x) but conn ids do not match (%d != %d)\n", 
                pVoipGroup, uClientId, uRemoteLowLevelConnectivityId, iHighLevelConnIdInUse, iConnId));
            return(-2);
        }
    }
    ...
    {
        if (pCollidingVoipGroup->bTunnel != pVoipGroup->bTunnel)
        {
            NetPrintf(("voipgroup: voip connection sharing does not support mixing voip groups with different tunnel configurations.\n"));
            return(-3);
        }
        ...
        // if the connections do not share an IP address, decide which to keep active and which to suspend
        if (pVoipCommon->Connectionlist.pConnections[pCollidingVoipGroup->aConnections[iCollidingConnId].iMappedLowLevelConnId].SendAddr.sin_addr.s_addr != SocketHtonl(uAddress))
        {
            /*
            Note:
            Voipgroups with bServer attribute set have precedence over voipgroups with bServer attribute not set,
            i.e. voip connectivity via voipserver is always preferred over p2p voip connectivity and over cc-hosted
            connectivity. Therefore, a newly created voip connection goes straight to "suspended state" if there already 
            exists a voip connection via the voipserver for the same client. Or, if the newly created voip connection 
            is to be established through a voipserver (dedicated or cc-hosted), then any existing p2p voip connection to the 
            same client is first moved to "suspended" state, and only then the voip connection through the voipserver is established.
            bServer will never be the true for connectivity hosted connection
            */
            if ((!pVoipGroup->bServer) && ((pCollidingVoipGroup->bServer) || (pCollidingVoipGroup->aConnections[iCollidingConnId].bIsConnectivityHosted)))
            {
                // no need to proceed with current connection establishment because the colliding voipgroup
                // already has voip connectivity via a voipserver (dedicated or cc-hosted), so it has precedence. 
                // just mark current connection as "SUSPENDED" such that it can be resumed later if needed.
                ...
                pVoipGroup->aConnections[iConnId].uRemoteLowLevelConnectivityId = uRemoteLowLevelConnectivityId;
                pVoipGroup->aConnections[iConnId].uLocalLowLevelConnectivityId = uLocalLowLevelConnectivityId;
                ...
                NetPrintf(("voipgroup: [%p] early suspension of high-level conn %d (sess id 0x%08x) - there already exists a low-level voip conn through the voipserver for client id 0x%08x (low level ids - remote: 0x%08x local: 0x%08x).\n",
                    pVoipGroup, iConnId, uSessionId, uClientId, uRemoteLowLevelConnectivityId, uLocalLowLevelConnectivityId));
                return(iConnId); // fake success
            } 
            else
            {
                // if the current voipgroup is a dedicated server
                // OR if the colliding voipgroup is not a dedicated server AND also not a cc-hosted connection
        ...
        else // otherwise, refcount
        {
            ...
            VoipCommonConnectionSharingAddSessionInfo(pVoipCommon, iMappedLowLevelConnId, uSessionId, iConnId, (void *)pVoipGroup);
        }
    }

    if (iMappedLowLevelConnId < 0)
    {
        // for connections that are not hosted through a voip server, we want iVoipServerConnId to be VOIP_CONNID_NONE
        int32_t iVoipServerConnId = VOIP_CONNID_NONE;
        ...
        {
            // update voip server conn id to the conn id
            iVoipServerConnId = iConnId;
        }
        ...
        //      (there is indeed a small transition time during which a connection may go to DISC state and a voip group still refers to it)
             ...
             (VoipStatus(VoipGetRef(), 'avlb', iConnId, NULL, 0) && !_VoipGroupLowLevelConnInUse(iConnId)) )
        {
            ...
            iMappedLowLevelConnId = VoipCommonConnect(pVoipCommon, iConnId, uAddress, uManglePort, uGamePort, uRemoteLowLevelConnectivityId, uSessionId, iVoipServerConnId, (void *)pVoipGroup);
        }
        else
        {
            // else, let the voip module select a low-level voip conn id for us.
            iMappedLowLevelConnId = VoipCommonConnect(pVoipCommon, VOIP_CONNID_NONE, uAddress, uManglePort, uGamePort, uRemoteLowLevelConnectivityId, uSessionId, iVoipServerConnId, (void *)pVoipGroup);
        }
    }
    ...
    NetPrintf(("voipgroup: [%p] high-level conn id %d (sess id 0x%08x) mapped to low-level conn id %d --> address %a, clientId=0x%08x (low level ids - remote: 0x%08x local: 0x%08x).\n", 
        pVoipGroup, iConnId, uSessionId, iMappedLowLevelConnId, uAddress, uClientId, uRemoteLowLevelConnectivityId, uLocalLowLevelConnectivityId));

    // set the local client id for the mapping
    VoipCommonSetLocalClientId(pVoipCommon, iMappedLowLevelConnId, uLocalLowLevelConnectivityId);
...
/*F********************************************************************************/
/*!
    \Function   VoipGroupListen

    \Description
        Listen for connections.  Connection notification is through the
        extended event callback handler.

    \Input *pVoipGroup  - voipgroup state
    \Input uListenPort  - port to listen on

    \Output
        int32_t         - negative=failed, else success

    \Version 10/01/2019 (jbrookes)
*/
/********************************************************************************F*/
int32_t VoipGroupListen(VoipGroupRefT *pVoipGroup, uint32_t uListenPort)
{
    VoipCommonRefT *pVoipCommon = (VoipCommonRefT *)VoipGetRef();
    return(VoipCommonListen(pVoipCommon, uListenPort));
}
...
void VoipGroupResume(VoipGroupRefT *pVoipGroup, int32_t iConnId, uint32_t uAddress, uint32_t uManglePort, uint32_t uGamePort, uint32_t uClientId, uint32_t uSessionId, uint8_t bIsConnectivityHosted)
{
    ...
    if (pVoipGroup->bServer == TRUE || bIsConnectivityHosted == TRUE)
    {
        // update voip server conn id to the conn id if server hosted
        iVoipServerConnId = iConnId;
    }
    ...
    {
        // conn id is available, use it!
        iMappedLowLevelConnId = VoipCommonConnect(pVoipCommon, iConnId, uAddress, uManglePort, uGamePort, pVoipGroup->aConnections[iConnId].uRemoteLowLevelConnectivityId, uSessionId, iVoipServerConnId, (void *)pVoipGroup);
    }
    else
    {
        // else, let Voip allocate one.
        iMappedLowLevelConnId = VoipCommonConnect(pVoipCommon, VOIP_CONNID_NONE, uAddress, uManglePort, uGamePort, pVoipGroup->aConnections[iConnId].uRemoteLowLevelConnectivityId, uSessionId, iVoipServerConnId, (void *)pVoipGroup);
    }
    ...
    NetPrintf(("voipgroup: [%p] high-level conn id %d (sess id 0x%08x) mapped to low-level conn id %d --> address %a, clientId=0x%08x (low level id 0x%08x).\n", 
        pVoipGroup, iConnId, uSessionId, iMappedLowLevelConnId, uAddress, uClientId, pVoipGroup->aConnections[iConnId].uRemoteLowLevelConnectivityId));

    // set the local client id for the mapping
    VoipCommonSetLocalClientId(pVoipCommon, iMappedLowLevelConnId, pVoipGroup->aConnections[iConnId].uLocalLowLevelConnectivityId);
...
void VoipGroupDisconnect(VoipGroupRefT *pVoipGroup, int32_t iConnId)
{
    ...
    if (pVoipGroup->aConnections[iConnId].eConnState != VOIPGROUP_CONNSTATE_SUSPENDED)
    {
        ...
        if (iMappedLowLevelConnId != VOIP_CONNID_NONE)
        {
            ...
            else
            {
                ...
                VoipCommonConnectionSharingDelSessionInfo(pVoipCommon, iMappedLowLevelConnId, pVoipGroup->aConnections[iConnId].uSessionId, (void *)pVoipGroup);
    ...
    pVoipGroup->aConnections[iConnId].uLocalLowLevelConnectivityId = 0;
    pVoipGroup->aConnections[iConnId].uRemoteLowLevelConnectivityId = 0;
...
/*!
    ...
    \Description
        Passes through to VoipCommonRemoteUserStatus2, afer adjusting connection ID
...
int32_t VoipGroupRemoteUserStatus(VoipGroupRefT *pVoipGroup, int32_t iConnId, int32_t iRemoteUserIndex)
{
    ...
    return(VoipCommonRemoteUserStatus2((VoipCommonRefT *)VoipGetRef(), iMappedConnId, iRemoteUserIndex, NULL));
}

/*F********************************************************************************/
/*!
    \Function   VoipGroupRemoteUserStatus2
    ...
    \Version 03/19/2020 (mgallant)

*/
/********************************************************************************F*/
int32_t VoipGroupRemoteUserStatus2(VoipGroupRefT *pVoipGroup, int32_t iConnId, int32_t iRemoteUserIndex, int32_t *pFirstPartyChatIndicator)
{
    int32_t iMappedConnId = _VoipGroupMatch(pVoipGroup, iConnId);
    ...
    return(VoipCommonRemoteUserStatus2((VoipCommonRefT *)VoipGetRef(), iMappedConnId, iRemoteUserIndex, pFirstPartyChatIndicator));
}
...
{
    return(VoipLocalUserStatus2(VoipGetRef(), iLocalUserIndex, NULL));
}

/*F********************************************************************************/
/*!
    \Function   VoipGroupLocalUserStatus2
    ...
    \Input *pVoipGroup                  - The VoipGroup
    \Input  iLocalUserIndex             - User index
    \Input *pFirstPartyChatIndicator    - Optional pointer to an int32_t to store chat indicator

    \Output
        int32_t                         - VOIP_LOCAL_* flags

    \Version 03/27/2020 (mgallant)
*/
/********************************************************************************F*/
int32_t VoipGroupLocalUserStatus2(VoipGroupRefT *pVoipGroup, int32_t iLocalUserIndex, int32_t *pFirstPartyChatIndicator)
{
    return(VoipLocalUserStatus2(VoipGetRef(), iLocalUserIndex, pFirstPartyChatIndicator));
}
