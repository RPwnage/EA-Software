...
typedef struct VoipCommonRefT
{
    ...
    VoipCallbackExT     *pCallbackEx;       //!< voip extended event callback (optional)
    ...
    uint8_t             bApplyMute;         //!< apply mute changes on the next voip thread pass
    ...
    uint32_t            uConnRecvMask;  //<! the per connection recv mask matching how +-pbk are set per player
    uint32_t            uConnSendMask;  //<! the per connection send mask matching how muting was apply in VoipCommpnApplyMute
...
// unset voip ref, used for external cleanup shutdown
...
// set event notification callback
void VoipCommonSetEventCallbackEx(VoipCommonRefT *pVoipCommon, VoipCallbackExT *pCallback, void *pUserData);
...
void VoipCommonApplyMute(VoipCommonRefT *pVoip);

void VoipCommonProcessChannelChange(VoipCommonRefT *pVoipCommon, int32_t iConnID);

// install root ca required to support given speech service provider
