...
#define VOIPNARRATE_READLEN                (VOIPNARRATE_FRAMESIZE)
#define VOIPNARRATE_BUFFER_LEN             (50000)
#define VOIPNARRATE_NUM_WCHARS             20                                                          
/*** Macros ***********************************************************************/
...
struct VoipNarrateRefT
{
    ...
    uint8_t aReadbuffer[VOIPNARRATE_READLEN];    //!< buffer of synthesized voice data read from the voice stream
    int32_t iAmountConsumed;                     //!< amount of synthesized voice data consumed by narrate cb in bytes
    ULONG uAmountRemain;                         //!< amount of synthesized voice data remaining (amount pull from the output stream - amount consumed by narrate cb)
...
{
    wchar_t strLang[VOIPNARRATE_NUM_WCHARS];
    wchar_t strGender[VOIPNARRATE_NUM_WCHARS];
    ISpObjectToken *pVoiceToken;
    IEnumSpObjectTokens *pVoiceTokenList;

    _snwprintf(strLang, VOIPNARRATE_NUM_WCHARS, L"Language=%d", pVoipNarrate->iLangCode);
    _snwprintf(strGender, VOIPNARRATE_NUM_WCHARS, L"Gender=%s", eGender == VOIPNARRATE_GENDER_FEMALE ? L"Female" : L"Male");
...
*/
/********************************************************************************F*/
static void _VoipNarrateStart(VoipNarrateRefT *pVoipNarrate, VoipNarrateGenderE eGender, uint32_t uFlags, const char *pText)
{
    HRESULT hResult;
    ...
    if (status.dwRunningState == SPRS_DONE)
    {
        if (pStream->GetDataSize() != 0)
        {
            ...
            if (pVoipNarrate->bFirstSynthOfPhrase)
            {
                ...
                NetPrintfVerbose((pVoipNarrate->iVerbose, 1, "voipnarratepc: sapi speak completed (size=%u)\n", pStream->GetDataSize()));
            }

            for (iResult = 1; iResult > 0;)
            {
                if (pVoipNarrate->uAmountRemain == 0)
                {
                    ds_memclr(pVoipNarrate->aReadbuffer, VOIPNARRATE_READLEN);
                    pStream->Read(pVoipNarrate->aReadbuffer, VOIPNARRATE_READLEN, &pVoipNarrate->uAmountRemain);
                    pVoipNarrate->Metrics.uDurationMsRecv += ((pVoipNarrate->uAmountRemain * 1000) / VOIPNARRATE_SAMPLERATE);
                    pVoipNarrate->iAmountConsumed = 0;
                }
                ...
                if (pVoipNarrate->bFirstSynthOfPhrase == TRUE)
                {
                    ...
                    pVoipNarrate->pVoiceDataCb(pVoipNarrate, 0, (const int16_t*)pVoipNarrate->aReadbuffer, VOIPNARRATE_STREAM_START, pVoipNarrate->uFlags, pVoipNarrate->pUserData);
                }

                iResult = pVoipNarrate->pVoiceDataCb(pVoipNarrate, 0, (const int16_t*)(pVoipNarrate->aReadbuffer + pVoipNarrate->iAmountConsumed), pVoipNarrate->uAmountRemain, pVoipNarrate->uFlags, pVoipNarrate->pUserData);
                pVoipNarrate->iAmountConsumed += iResult;
                pVoipNarrate->uAmountRemain -= iResult;
            }
        }
        else
        {
            ...
            // signal stream end
            pVoipNarrate->pVoiceDataCb(pVoipNarrate, 0, NULL, VOIPNARRATE_STREAM_END, pVoipNarrate->uFlags, pVoipNarrate->pUserData);
            pVoipNarrate->eVoiceState = VOIPNARRATE_VOICE_READY;
        }
    }
}
...
{
    NetPrintfVerbose((pVoipNarrate->iVerbose, 1, "voipnarratepc: queuing request '%s'\n", pText));
...
void VoipNarrateUpdate(VoipNarrateRefT *pVoipNarrate)
{
    ...
    if ((pVoipNarrate->pRequest != NULL) && (pVoipNarrate->eVoiceState == VOIPNARRATE_VOICE_READY))
    {
        ...
        _VoipNarrateStart(pVoipNarrate, Request.eGender, Request.uFlags, Request.strText);
    }
...
HRESULT STDMETHODCALLTYPE VoipNarrateVoiceStream::Read(void *pData, ULONG uDataLen, ULONG *pRead)
{
    ...
    //compact the buffer
    memmove(m_pBuffer, m_pBuffer + *pRead, m_uWritePos - *pRead);
    m_uWritePos -= *pRead;
