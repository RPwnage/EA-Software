...
struct VoipTranscribeRefT
{
    ...
    uint32_t uProfile;                  //!> transcription profile
    VoipSpeechProviderE eProvider;      //!< transcription provider
    ...
    char    strLang[8];                 //!< transcription language e.g. en-US; defaults to en-US
...
*/
/********************************************************************************F*/
static int32_t _VoipTranscribeCustomHeaderCb(ProtoHttp2RefT *pState, char *pHeader, uint32_t uHeaderSize, void *pUserRef)
{
...
static int32_t _VoipTranscribeTransportInit(VoipTranscribeRefT *pVoipTranscribe)
{
    ...
    if (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_AMAZON)
    {
...
{
    char strEventType[32], strHeader[512], strMessage[4096], strContentType[64];
    int32_t iMessageLen, iOffset, iReadResult, iResult;
    TransportT *pTransport = &pVoipTranscribe->Transport;

    // check for error response
    if ((iResult = pTransport->Status(pTransport->pState, pTransport->iStreamId, 'code', NULL, 0)) != PROTOHTTP_RESPONSE_OK)
    {
    ...
    // get content-type so we know how to parse the response
    ProtoHttpGetHeaderValue(NULL, strHeader, "content-type", strContentType, sizeof(strContentType), NULL);

    // parse event stream response
    if (!ds_stricmp(strContentType, "application/vnd.amazon.eventstream"))
    {
    ...
    else if (!ds_stricmp(strContentType, "application/x-amz-json-1.1"))
    {
        // parse json response
        return(_VoipTranscribeParseResponseAmazonJson(pVoipTranscribe, pResponse, pResult, iResultSize));
    }
    else
    {
        NetPrintf(("voiptranscribe: unhandled content-type '%s' in transcription response\n", strContentType));
        return(-1);
    }
...
{
    int32_t iResult = -1;
    if (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_IBMWATSON)
    {
        iResult = _VoipTranscribeParseResponseWatson(pVoipTranscribe, pResponse, pResult, iResultSize);
    }
    else if (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_MICROSOFT)
    {
        iResult = _VoipTranscribeParseResponseMicrosoft(pVoipTranscribe, pResponse, pResult, iResultSize);
    }
    else if (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_GOOGLE)
    {
    ...
    else if (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_AMAZON)
    {
...
/*!
    ...
    \Input *pVoipTranscribe - pointer to module state
    \Input *pUrl            - [out] buffer to hold url
    \Input iUrlLen          - url buffer length
...
*/
/********************************************************************************F*/
static char *_VoipTranscribeFormatHeaderWatson(VoipTranscribeRefT *pVoipTranscribe, char *pUrl, int32_t iUrlLen, char *pBuffer, int32_t iBufLen)
{
    TransportT *pTransport = &pVoipTranscribe->Transport;
    char strAuth[128];
    int32_t iOffset;
...
*/
/********************************************************************************F*/
static char *_VoipTranscribeFormatHeaderMicrosoft(VoipTranscribeRefT *pVoipTranscribe, char *pUrl, int32_t iUrlLen, char *pBuffer, int32_t iBufLen)
{
    TransportT *pTransport = &pVoipTranscribe->Transport;
    int32_t iOffset=0;
    ...
    // format transport-specific url
    ds_snzprintf(pUrl, iUrlLen, "%s?language=%s", pVoipTranscribe->strUrl, pVoipTranscribe->strLang);
    return(pUrl);
}
...
*/
/********************************************************************************F*/
static char *_VoipTranscribeFormatHeaderGoogle(VoipTranscribeRefT *pVoipTranscribe, char *pUrl, int32_t iUrlLen, char *pBuffer, int32_t iBufLen)
{
    TransportT *pTransport = &pVoipTranscribe->Transport;
    ...
    if (pTransport->eTransport == TRANSPORT_HTTP)
    {
        ...
        ds_snzprintf(pUrl, iUrlLen, "%s?key=%s", pVoipTranscribe->strUrl, pVoipTranscribe->strKey);
    }
    else
    {
...
*/
/********************************************************************************F*/
static char *_VoipTranscribeFormatHeaderAmazon(VoipTranscribeRefT *pVoipTranscribe, char *pUrl, int32_t iUrlLen, char *pBuffer, int32_t iBufLen)
{
    int32_t iOffset=0;
    ...
    iOffset += ds_snzprintf(pBuffer+iOffset, iBufLen-iOffset, "x-amzn-transcribe-language-code: %s\r\n", pVoipTranscribe->strLang);
...
*/
/********************************************************************************F*/
static char *_VoipTranscribeSetHeader(VoipTranscribeRefT *pVoipTranscribe, char *pUrl, int32_t iUrlLen, TransportT *pTransport)
{
    char strHeader[512] = "";
    // format header with provider-specific fields
    if (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_IBMWATSON)
    {
        pUrl = _VoipTranscribeFormatHeaderWatson(pVoipTranscribe, pUrl, iUrlLen, strHeader, sizeof(strHeader));
    }
    else if (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_MICROSOFT)
    {
        pUrl = _VoipTranscribeFormatHeaderMicrosoft(pVoipTranscribe, pUrl, iUrlLen, strHeader, sizeof(strHeader));
    }
    else if (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_GOOGLE)
    {
        pUrl = _VoipTranscribeFormatHeaderGoogle(pVoipTranscribe, pUrl, iUrlLen, strHeader, sizeof(strHeader));
    }
    else if (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_AMAZON)
    {
        pUrl = _VoipTranscribeFormatHeaderAmazon(pVoipTranscribe, pUrl, iUrlLen, strHeader, sizeof(strHeader));
    }
    // set the header
...
static int32_t _VoipTranscribeConnect(VoipTranscribeRefT *pVoipTranscribe)
{
    ...
    // set connect headers
    if ((pUrl = _VoipTranscribeSetHeader(pVoipTranscribe, strUrl, sizeof(strUrl), pTransport)) == NULL)
    {
        return(-1);
    }
...
{
    TransportT *pTransport = &pVoipTranscribe->Transport;
    char strRequest[128] = "", strUrl[256], *pRequest = strRequest;
    ...
    if ((pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_IBMWATSON) && (pTransport->eTransport == TRANSPORT_WEBSOCKETS))
    {
...
static int32_t _VoipTranscribeSendFinish(VoipTranscribeRefT *pVoipTranscribe)
{
    ...
    {
        iRequestLen =  (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_IBMWATSON) ? ds_snzprintf(strRequest, sizeof(strRequest), "{ \"action\": \"stop\" }") : 0;
...
{
    int32_t iResult = ds_snzprintf((char *)pVoipBuffer->pBuffer, pVoipBuffer->iBufLen, "{ \"config\": { \"encoding\": \"%s\", \"sampleRateHertz\": %d, \"languageCode\": \"%s\", \"profanity_filter\": \"true\"  }, \"audio\": { \"content\": \"",
        pVoipTranscribe->strAudioFormat, pVoipTranscribe->iAudioRate, pVoipTranscribe->strLang);
    return(iResult);
}
...
static int32_t _VoipTranscribeBase64Encode(VoipBufferT *pVoipBufferOut, VoipBufferT *pVoipBufferInp)
{
    ...
    iInpLen = DS_MIN(iInpLen, Base64DecodedSize(iOutLen-1));             // pick smallest of input and output length
...
static int32_t _VoipTranscribeProtobufStart(VoipTranscribeRefT *pVoipTranscribe, VoipBufferT *pVoipBuffer)
{
    ...
    if ((pEncoder = ProtobufWriteCreate(pVoipBuffer->pBuffer+1, pVoipBuffer->iBufLen-1, TRUE)) != NULL)
    {
        ...
        ProtobufWriteString(pEncoder, pVoipTranscribe->strLang, (signed)strlen(pVoipTranscribe->strLang), 3 /* language_code */);
...
static int32_t _VoipTranscribeAwsEncode(VoipTranscribeRefT *pVoipTranscribe, VoipBufferT *pVoipBufferOut, VoipBufferT *pVoipBufferInp)
{
    ...
    if (pVoipBufferInp != NULL)
    {
        ...
        // calculate how much input we have to write (and if we don't have any, don't write it)
        if ((iInpWrite = pVoipBufferInp->iBufOff-pVoipBufferInp->iBufInp) == 0)
        {
            return(0);
        }
    }
...
static VoipBufferT *_VoipTranscribeSendEncode(VoipTranscribeRefT *pVoipTranscribe, TransportT *pTransport, VoipBufferT *pVoipBufferSrc)
{
    ...
    if (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_GOOGLE)
    {
    ...
    {
        if ((pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_GOOGLE) && (pTransport->eTransport == TRANSPORT_HTTP))
        {
...
static int32_t _VoipTranscribeSend(VoipTranscribeRefT *pVoipTranscribe, VoipBufferT *pVoipBuffer)
{
    ...
    if ((pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_AMAZON) || (pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_GOOGLE))
    {
...
static uint32_t _VoipTranscribeConfig(VoipTranscribeRefT *pVoipTranscribe, uint32_t uProfile, const char *pUrl, const char *pCred)
{
    ...
    if (VOIPTRANSCRIBE_PROFILE_PROVIDER(uProfile) != VOIPSPEECH_PROVIDER_NONE)
    {
...
VoipTranscribeRefT *VoipTranscribeCreate(int32_t iBufSize)
{
    ...
    // allocate and initialize audio double buffer
    ...
    // allocate and initialize voip send buffer; this is used for google and amazon, which need data to be encoded
    ...
    ds_strnzcpy(pVoipTranscribe->strLang, "en-US", sizeof(pVoipTranscribe->strLang));
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'lang' - set transcription language (default=en); languages allowed are provider-specific
...
int32_t VoipTranscribeControl(VoipTranscribeRefT *pVoipTranscribe, int32_t iControl, int32_t iValue, int32_t iValue2, void *pValue)
{
    ...
    if (iControl == 'conf')
    {
        return(_VoipTranscribeConfig(pVoipTranscribe, _VoipTranscribe_Config.uProfile, _VoipTranscribe_Config.strUrl, _VoipTranscribe_Config.strKey));
    }
    // clear stt metrics
    if (iControl == 'cstm')
    {
    ...
    {
        if (strcmp(pVoipTranscribe->strLang, pValue))
        {
            ds_strnzcpy(pVoipTranscribe->strLang, pValue, sizeof(pVoipTranscribe->strLang));
            NetPrintf(("voiptranscribe: setting language to %s\n", pVoipTranscribe->strLang));
            // reset amazon sessionid string, required when changing languages
            pVoipTranscribe->strSessionId[0] = '\0';
        }
        return(0);
    }
    #if DIRTYCODE_LOGGING
...
{ 
    ...
    if ((pVoipTranscribe->eProvider == VOIPSPEECH_PROVIDER_IBMWATSON) && (pVoipTranscribe->eTransport == VOIPTRANSCRIBE_TRANSPORT_WEBSOCKETS) &&
