...
/*!
    ...
    \Input  bEncoded            - TRUE if data is encoded, FALSE if not encoded
...
*/
/********************************************************************************F*/
int32_t VoipMixerAccumulate(VoipMixerRefT *pMixerRef, uint8_t *pInputData, int32_t iDataSize, int32_t iMixMask, int32_t iChannel, uint8_t bEncoded)
{
    int32_t iNumSamples;
    ...
    // decode into mixbuffer if needed
    if (!bEncoded)
    {
        int16_t *pSamples = (int16_t *)pInputData;
        int32_t iOutputVolume = VoipCodecStatus(VOIP_CODEC_ACTIVE, 'ovol', 0, NULL, 0);
        iNumSamples = iDataSize / sizeof(*pSamples);

        for (int32_t iSample = 0; iSample < iNumSamples; iSample += 1)
        {
            pMixerRef->MixBuffers[iMixBuffer].pMixBuffer[iSample] += ((pSamples[iSample] * iOutputVolume) >> VOIP_CODEC_OUTPUT_FRACTIONAL);
        }
    }
    else
    {
