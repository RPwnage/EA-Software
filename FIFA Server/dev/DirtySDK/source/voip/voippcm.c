...
static int32_t _VoipPCMResample(VoipCodecRefT *pState, uint8_t *pOutBytes, uint32_t uOutBuffSize, const int16_t *pInput, int32_t iNumSamples);
...
/*F*************************************************************************************/
/*!
     \Function _VoipPCMResample

    \Description
        Resampling is not supported for PCM
...
static int32_t _VoipPCMStatus(VoipCodecRefT *pCodecState, int32_t iSelect, int32_t iValue, void *pBuffer, int32_t iBufSize)
{
    ...
    if (pModuleState != NULL)
    {
        if (iSelect == 'ovol')
        {
            return(pModuleState->iOutputVolume);
        }
