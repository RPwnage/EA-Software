...
static void _VoiceTRA(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp);
...
/*F*************************************************************************************/
/*!
    \Function _VoiceGetSpeechProvider

    \Description
        Get speech provider from string

    \Input *pName           - name of provider

    \Output
        VoipSpeechProviderE - speech provider

    \Version 11/04/2019 (jbrookes)
*/
/**************************************************************************************F*/
static VoipSpeechProviderE _VoiceGetProvider(const char *pName)
{
    VoipSpeechProviderE eProvider = VOIPSPEECH_PROVIDER_NONE;
    if (!ds_stricmp(pName, "watson")) eProvider = VOIPSPEECH_PROVIDER_IBMWATSON;
    if (!ds_stricmp(pName, "microsoft")) eProvider = VOIPSPEECH_PROVIDER_MICROSOFT;
    if (!ds_stricmp(pName, "google")) eProvider = VOIPSPEECH_PROVIDER_GOOGLE;
    if (!ds_stricmp(pName, "amazon")) eProvider = VOIPSPEECH_PROVIDER_AMAZON;
    return(eProvider);
}
...
static void _VoiceConnect(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    if (argc >= 3)
    {
        ...
        iClientIndex = (argc >= 5) ? strtol(argv[4], NULL, 10) : 0;
        ...
        pApp->iConnID = VoipGroupConnect(pApp->pVoipGroup, iClientIndex, uRemoteAddr, uManglePort, uRemotePort, pApp->uClientId, 0, FALSE, uRemoteClientId, pApp->uClientId);
    }
    else
    {
        #if !defined(DIRTYCODE_XBOXONE) || !defined(DIRTYCODE_GDK)
...
static void _VoiceTTS(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    if (!ds_stricmp(argv[2], "config"))
    {
        if (argc == 6)
        {
            VoipConfigNarration(VoipGetRef(), _VoiceGetProvider(argv[3]), argv[4], argv[5]);
        }
        else
        {
            ZPrintf("   usage: %s tts config [none|amazon|google|microsoft|watson] <url> <key>\n", argv[0]);
        }
    }
...
static void _VoiceSTT(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    {
        ZPrintf("   usage: %s stt config [none|amazon|google|microsoft|watson] [li16|wav|opus] [http|h2|ws] <url> <key>\n", argv[0]);
        return;
    }
    ...
    VoipConfigTranscription(VoipGetRef(), VOIPTRANSCRIBE_PROFILE_CONSTRUCT(_VoiceGetProvider(argv[3]), eFormat, eTransport), argv[6], argv[7]);
}
...
/*!
    ...
    \Description
        Voice subcommand - Translate functionality
...
static void _VoiceTRA(void *pCmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    if (bHelp)
    {
        ZPrintf("   usage: %s tra config [params] or [lang] \"text you wish to translate\"\n", argv[0]);
        return;
    }
    if (!ds_stricmp(argv[2], "config"))
    {
        if (argc == 6)
        {
            VoipConfigTranslation(VoipGetRef(), _VoiceGetProvider(argv[3]), argv[4], argv[5]);
        }
        else
        {
            ZPrintf("   usage: %s tra config [none|amazon|google|microsoft|watson] <url> <key>\n", argv[0]);
        }
    }
    else if (argv)
    {
        if (argc == 4)
        {
            int32_t iValue = (argv[2][0] << 8) | argv[2][1];
            VoipControl(VoipGetRef(), 'xlat', iValue, argv[3]);
        }
        else
        {
            ZPrintf("   usage: %s tra [lang] \"text you wish to translate\"\n", argv[0]);
        }
    }
}
...
static void _VoiceActivateLocalUser(void *CmdRef, int32_t argc, char *argv[], unsigned char bHelp)
{
    ...
    if (bHelp == TRUE || argc != 4)
    {
        ZPrintf("   usage: %s activate <local user index> <on|off> \n", argv[0]);
        return;
    }
    ...
    if (bHelp == TRUE || argc != 3)
    {
        ZPrintf("   usage: %s activate <on|off> \n", argv[0]);
        return;
    }
...
static int32_t _CmdVoiceCb(ZContext *argz, int32_t argc, char *argv[])
{
    ...
    if (pApp->pVoip != NULL)
    {
        static VoipSpeechToTextMetricsT _SttMetrics = { 0 }, SttMetrics;
        // get stt metrics
        VoipStatus(pApp->pVoip, 'sttm', 0, &SttMetrics, sizeof(SttMetrics));
        if (memcmp(&_SttMetrics, &SttMetrics, sizeof(SttMetrics)))
        {
            ZPrintf("voice: sttm change [EventCount: %3d DurationSent: %5dms CharsRecv: %4d EmptyRslt: %3d ErrorRslt: %3d Delay: %5dms]\n",
                SttMetrics.uEventCount, SttMetrics.uDurationMsSent, SttMetrics.uCharCountRecv,
                SttMetrics.uEmptyResultCount, SttMetrics.uErrorCount, SttMetrics.uDelay);
            ds_memcpy_s(&_SttMetrics, sizeof(_SttMetrics), &SttMetrics, sizeof(SttMetrics));
        }    
    }
