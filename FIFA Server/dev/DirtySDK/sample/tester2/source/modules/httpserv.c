...
#define HTTPSERV_CONTENTTYPE_BIN    (0)
#define HTTPSERV_CONTENTTYPE_TXT    (1)
...
//! map of filename extensions to content-types; https://www.iana.org/assignments/media-types/media-types.xhtml and https://www.sitepoint.com/mime-types-complete-list/
static const char *_ProtoHttpServ_strContentTypes[][2] =
{
    // first two types must be binary and text
    { ".bin",  "application/octet-stream"   },
    { ".txt",  "text/plain" },
    // further types sorted by mime name
...
/*F********************************************************************************/
/*!
    \Function _HttpServGetContentTypeFromData

    \Description
        Get content-type of text or binary by looking at up to the first 2k of
        file data.

    \Input iFileId      - file id

    \Output
        const char *    - content type

    \Version 11/07/2019 (jbrookes)
*/
/********************************************************************************F*/
static const char *_HttpServGetContentTypeFromData(ZFileT iFileId)
{
    const char *pContentType = _ProtoHttpServ_strContentTypes[HTTPSERV_CONTENTTYPE_BIN][1]; // default to binary
    char strFileData[2*1024];
    int32_t iResult;

    // read some data
    if ((iResult = ZFileRead(iFileId, strFileData, sizeof(strFileData)-1)) > 0)
    {
        // rewind after read
        ZFileSeek(iFileId, 0, SEEK_SET);
        // null-terminate in case it is text
        strFileData[sizeof(strFileData)-1] = '\0';
        // if looks like text set it to text
        if ((signed)strnlen(strFileData, iResult) == iResult)
        {
            pContentType = _ProtoHttpServ_strContentTypes[HTTPSERV_CONTENTTYPE_TXT][1];
        }
    }
    else
    {
        NetPrintf(("httpserv: error %d trying to determine content type of file\n", iResult));
    }
...
/*!
    ...
    \Description
        Get content-type based on target url extension
...
*/
/********************************************************************************F*/
static const char *_HttpServGetContentType(const char *strUrl, ZFileT iFileId)
{
    const char *pContentType;
    int32_t iType;

    // get mime type from extension
    for (iType = 0, pContentType = NULL; *_ProtoHttpServ_strContentTypes[iType][0] != '\0'; iType += 1)
    {
    ...
    // if we still don't have a content-type choose text or binary based on start of file
    if (pContentType == NULL)
    {
        pContentType = _HttpServGetContentTypeFromData(iFileId);
    }
...
static int32_t _HttpServProcessGet(HttpServT *pServerState, const ProtoHttpServRequestT *pRequest, ProtoHttpServResponseT *pResponse)
{
    ...
    // set content-type
    ds_strnzcpy(pResponse->strContentType, _HttpServGetContentType(pRequest->strUrl, iFileId), sizeof(pResponse->strContentType));
