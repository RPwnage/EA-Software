...
BOOL g_bLogging = FALSE;        //!< whether printf logging is enabled or not

static unsigned char _TransTo8BitASCIIa[256];

// master table
static Utf8TransTblT _TransTo8BitASCII[] = 
{
...
static void _T2HostDisplayOutput(const char *pBuf, int32_t iLen, int32_t iRefcon, void *pRefptr)
{
    ...
    char strTempText2[16*1024];

    // if logging is enabled, print it here
    if (g_bLogging)
    {
    ...
    *pTempText = '\0';
    ...
    // convert to ansi
    Utf8TranslateTo8Bit(strTempText2, sizeof(strTempText2), strTempText, '*', _TransTo8BitASCII);
    ...
    SendMessage(Ctrl, EM_REPLACESEL, FALSE, (LPARAM)strTempText2);
...
int32_t APIENTRY WinMain(HINSTANCE inst, HINSTANCE prev, char *cmdline, int32_t show)
{
    ...
    // init utf8->8bit ascii translation table
    {
        int32_t iEntry;
    ...
    // check for logging enable (use usual printf redirection semantics)
    if ((pBase = strstr(cmdline, "-logging ")) != NULL)
    {
        g_bLogging = TRUE;
        cmdline = pBase + strlen("-logging ");
    }
