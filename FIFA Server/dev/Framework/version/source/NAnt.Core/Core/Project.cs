...
namespace NAnt.Core
{
	public sealed class Project : IDisposable
	{
		...
		public const string NANT_PROPERTY_USE_VS_PACKAGE_REFERENCES = "nant.use-vs-package-references";
		...
		public /*readonly*/ IncludedFilesCollection IncludedFiles; // DAVE-TODO: should be readonly, but isn't for hackery in Frostbite's FBConfig
		...
		internal void PostPackageMapInit()
		{
			...
			if (PackageMap.Instance.BuildRootWasInitialized)
			{
			...
			// optionset used to filter out assemblies in visual studio that came from nuget references
			// when using <PackageReference>
			OptionSet nugetReferenceAssemblyOptions = new OptionSet();
			nugetReferenceAssemblyOptions.Options["nuget-reference-assembly"] = "true";
			NamedOptionSets.Add("nuget-reference-assembly", nugetReferenceAssemblyOptions);
