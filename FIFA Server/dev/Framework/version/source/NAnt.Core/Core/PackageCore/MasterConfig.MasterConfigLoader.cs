...
namespace NAnt.Core.PackageCore
{
	public sealed partial class MasterConfig
	{
		private sealed class MasterConfigLoader
		{
			...
			internal MasterConfig Load(Project project, string file)
			{
			...
			private void ProcessNugetVersions(Project project)
			{
				...
				foreach (KeyValuePair<string, NuGetPackage> autoConversion in m_masterVersionsToNugetAutoConversions)
				{
					...
					else if (project.Properties.GetBooleanPropertyOrDefault("nant.use-nuget-resolution", false))
					{
						string masterPackagesVersion = autoConversion.Value.Version;
						string nugetPackagesVersion = m_masterConfig.NuGetPackages[autoConversion.Key].Version;
						int comparisonResult = nugetPackagesVersion.StrCompareVersions(masterPackagesVersion);
						if (comparisonResult > 0)
						{
							m_log.Warning.WriteLine($"NuGet reference '{autoConversion.Key}' is set to version '{masterPackagesVersion}' in the 'masterversions' section of the masterconfig but '{nugetPackagesVersion}' in the 'nugetpackages' section. " +
								$"The 'nugetpackages' version '{nugetPackagesVersion}' will be used. The system will use the newer version in nugetpackages because the new nuget resolution system is enabled.");
						}
						else if (comparisonResult < 0)
						{
							throw new BuildException($"Error: NuGet reference '{autoConversion.Key}' is set to version '{masterPackagesVersion}' in the 'masterversions' section of the masterconfig but '{nugetPackagesVersion}' in the 'nugetpackages' section. " +
								$"Since the new nuget resolution system is enabled the nugetpackages version will be used but the version is now lower than before, please update the masterconfig so that the version in nugetpackages is the same or higher.");
						}
					}
				}
			...
			private void ParseConfig(XmlNode el)
			{
				...
				XmlUtil.ValidateAttributes(el, new string[] { "includes", "excludes", "package", "default", "extra-config-dir", "default-osx", "includes-osx", "excludes-osx", "default-unix", "includes-unix", "excludes-unix" });
				...
				(
					...
					extensions,
					// The followings are optional setup and most masterconfig.xml won't have the followings.
					// They will pass in null if not defined.
					XmlUtil.GetOptionalAttribute(el, "default-osx"),
					XmlUtil.GetOptionalAttribute(el, "includes-osx"),
					XmlUtil.GetOptionalAttribute(el, "excludes-osx"),
					XmlUtil.GetOptionalAttribute(el, "default-unix"),
					XmlUtil.GetOptionalAttribute(el, "includes-unix"),
					XmlUtil.GetOptionalAttribute(el, "excludes-unix")
				);
			}
