...
namespace NAnt.Core.PackageCore
{
	public class PackageMap
	{
		...
		public string BuildRoot
		{
			get
			{
				if (m_buildRoot is null)
				{
					if ((MasterConfig.BuildRoot?.AllowDefault ?? true) == false)
					{
					...
					{
						throw new BuildException("Package map was not initialized with a build root.");
					}
				}
				return m_buildRoot;
			}
		}
		public bool BuildRootWasInitialized => m_buildRoot != null;

		public IList<string> ConfigExcludes { get { return MasterConfig.Config.ActiveExcludes; } }

		public IList<string> ConfigIncludes { get { return MasterConfig.Config.ActiveIncludes; } }
		public string ConfigPackageName { get { return MasterConfig.Config.Package ?? DefaultConfigPackage; } }
		public string DefaultConfig { get { return MasterConfig.Config.ActiveDefault; } }
		...
		private PackageMap(Project project, MasterConfig masterConfig = null, string buildRootOverride = "", bool autoBuildAll = false, string autoBuildPackages = null, string autoBuildGroups = null, bool useV1 = false, string token = null)
		{
			...
			// set the build root, if we have an override, masterconfig buildroot or project with a base dir, otherwise is it left null and trying to access is error
			{
				if (!String.IsNullOrEmpty(buildRootOverride))
				{
					m_buildRoot = Path.GetFullPath(buildRootOverride);
				}
				else
				{
					string rootBase = null;
					if (!String.IsNullOrWhiteSpace(MasterConfig.MasterconfigFile))
					{
						// if we have a masterconfig path then buildroot is relative to that
						rootBase = Path.GetDirectoryName(Path.GetFullPath(MasterConfig.MasterconfigFile));
					}
					else if (!String.IsNullOrWhiteSpace(project.BaseDirectory))
					{
						// if not, but we have a build file file then root is relative to that
						rootBase = Path.GetFullPath(project.BaseDirectory);
					}

					if (MasterConfig.BuildRoot != null)
					{
						if (MasterConfig.BuildRoot.AllowDefault)
						{
							MasterConfig.BuildRoot.Path = MasterConfig.BuildRoot.Exceptions.ResolveException(project, MasterConfig.BuildRoot.Path);
							if (rootBase != null)
							{
								m_buildRoot = Path.GetFullPath(Path.Combine(rootBase, MasterConfig.BuildRoot.Path ?? "build"));
							}
						}
						else
						{
					...
					{
						m_buildRoot = Path.GetFullPath(Path.Combine(rootBase, "build"));
					}
				}
			}

			// init nuget manager (lazy, because we don't want to init until we have the buildroot set
			// or force the resolution of .net sdk dependent until it's needed)
			...
			(
				() =>
				{
					...
					// in order to resolve nuget runtimes we need a fallback graph - it's json file provided by sdk
					InternalQuickDependency.Dependent(project, "DotNetSdk");
					string runtimeGraphPath = project.Properties["package.DotNetSdk.runtimeIdentiferGraphFile"] ?? Path.Combine(project.Properties.GetPropertyOrFail("package.DotNetSDK.sdkversiondir"), "RuntimeIdentifierGraph.json");

					return new NugetManager
					(
		...
		// to setup packagemap these 2 things  need to happen:
		//		1 - PackageMap.Init - sets the masterconfig, required if we want to resolve versions
		//		2 - PackageMap.Instance.SetTopLevelPackage - needed for implicit resolution of command line package, needs to happen before we resolve version but ONLY in cases where we have a package 
		//			we consider to be 'top level'
		public static void Init(Project project, MasterConfig masterConfig = null, bool autobuildAll = false, string autobuildPackages = null, string autobuildGroups = null, string buildRootOverride = "", bool useV1 = false, string token = null)
		{
			if (s_instance != null)
			{
			...
			{
				s_instance = new PackageMap(project, masterConfig, buildRootOverride, autobuildAll, autobuildPackages, autobuildGroups, useV1, token);
		...
		private Release FindOrInstallReleaseInternal(Project project, MasterConfig.IPackage package, bool onDemandIsEnabled)
		{
			...
			else
			{
				string message = $"Could not find package '{package.Name}-{package.Version}' in package roots." + Environment.NewLine
								...
								+ "and on demand downloads from the package server (packages.ea.com) are " + (PackageServerOnDemand ? "on." : "off.") + Environment.NewLine
								+ "If you are expecting the package to be downloaded you may want to check your on demand package download settings (<ondemand>, <packageserverondemand> and <localondemand>) in your masterconfig or masterconfig fragments.";
		...
		private Release ThrowMissingPackageException(MasterConfig.IPackage packageSpec, string errorMessageHeader, Exception innerException = null, List<RejectedRelease> rejectedReleases = null, IPackageServer packageServer = null, bool listRemoteReleases = false)
		{
			...
			if (searchedPackageRoots)
			{
				...
				exceptionMessageBuilder.AppendLine("Package roots searched (in order):");
				
				int maxItemsToOutput = 3;
				int numItemsOutput = 0;
				...
				foreach (PackageRootList.Root root in PackageRoots)
				{
					...
					else
					{
						...
						if (numItemsOutput >= maxItemsToOutput)
						{
							if (!truncated)
							{
								truncated = true;
								numItemsOutput += (PackageRoots.HasOnDemandRoot? 1 : 0) +
												  (PackageRoots.HasLocalOnDemandRoot ? 1 : 0) +
												  (PackageRoots.HasDevelopmentRoot ? 1 : 0);
								exceptionMessageBuilder.AppendLine($"\t\t[...] {PackageRoots.Count() - maxItemsToOutput} roots were truncated.  To see the full list of package roots run nant with the -showmasterconfig parameter.");
							}
						}
						else
						{
				...
				exceptionMessageBuilder.AppendLine("Please see https://frostbite-ew.gitlab.ea.com/framework/fwdocs/user-guides/packagediscovery/ for additional details.");
			}
