...
namespace NAnt.Core.Tasks
{
	...
	[TaskName("exec", NestedElementsCheck = true)]
	public class ExecTask : ExternalProgramBase
	{
		...
		public ExecTask(Project project, string program, string commandline = null, string workingdir = null, 
			string message = null, bool stdout = true, bool silent = false, ArgumentSet args = null, OptionSet env = null)
			: base("exec")
		{
			Project = project;
			Program = program;
			...
			Stdout = stdout;
			Silent = silent;
			if (args != null) _argSet = args;
			if (env != null) EnvironmentVariables = env;
		}
		...
		public string CommandLineTemplate { get; set; } = CommandLineVariable + " " + ResponseFileVariable;
        ...
        [TaskAttribute("silent-err")]
        public bool SilentError
        {
            get { return base.QuietError; }
            set { base.QuietError = value; }
        }
		...
		private bool TaskNeedsRunning()
		{
			...
			if (Log.Level >= Logging.Log.LogLevel.Detailed && !Silent)
			{
		...
		void DisplayTaskBanner()
		{
			if (!Silent)
			{
				if (Message != null)
				{
					...
					Log.Minimal.WriteLine(LogPrefix + msg);
				}
				if (Message == null || Verbose || Log.Level >= Logging.Log.LogLevel.Diagnostic)
				{
					Log.Minimal.WriteLine(LogPrefix + "{0} {1}", ProgramFileName, GetCommandLine());
				}
			}
		}
		...
		protected override void PrepareProcess(ref System.Diagnostics.Process process)
		{
			...
			if (!ClearEnv)
			{
				...
				foreach (Property property in Project.Properties)
				{
					...
					{
						// Property names are case insensitive but environment variables on linux are case sensitive.
						// Property names are set to all lower case but usually environment variables are all upper case.
						// It is possible that they may be mixed case but we currently don't have a way to handle that.
						envMap[property.Suffix.ToUpperInvariant()] = property.Value;
					}
				}
			}
			else
