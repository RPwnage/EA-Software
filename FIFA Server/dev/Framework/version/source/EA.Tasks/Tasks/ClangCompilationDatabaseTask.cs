...
namespace EA.Eaconfig
{
	...
	public class ClangCompilationDatabase : Task
	{
		...
		[TaskAttribute("addheaders", Required = false)]
		...
		private bool _addheaders = false;
		...
		private void WriteOneFileItem(IMakeWriter compileCommandsWriter, IMakeWriter cppWriter, IMakeWriter cppClrWriter, CcCompiler cc, PathString directory, string scriptFile, FileItem fileItem, string intrinsicsIncludeFile)
		{
			var filecompiler = GetCompiler(cc, fileItem); // custom compiler options can be associated with individual file items
			...
			compilestep.file = fileItem.Path.Path.Replace('\\', '/');
			...
			if (isClr)
			{
				cppClrWriter.WriteLine(scriptFile + "," + fileItem.Path.Path);
			}
			else
			{
				...
				cppWriter.WriteLine(fileItem.Path.Path);
			}
		...
		private void WriteTool(IMakeWriter compileCommandsWriter, IMakeWriter cppWriter, IMakeWriter cppClrWriter, CcCompiler cc, PathString directory, string scriptFile)
		{
			...
			foreach (var fi in cc.SourceFiles.FileItems)
			{
				...
				WriteOneFileItem(compileCommandsWriter, cppWriter, cppClrWriter, cc, directory, scriptFile, fi, intrinsicsIncludeFile);
			}

			if (AddHeaders == true)
			{
				foreach (var fi in cc.HeaderFiles.FileItems)
				{
						WriteOneFileItem(compileCommandsWriter, cppWriter, cppClrWriter, cc, directory, scriptFile, fi, intrinsicsIncludeFile);
				}
			}
