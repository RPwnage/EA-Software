...
namespace EA.Eaconfig.Structured
{
	abstract public class ModuleBaseTask : ConditionalTaskContainer
	{
		...
		protected virtual void SetupBuildLayoutData()
		{
			...
			bool useTapEventsForTests = true;
			// Turn Off TAP events for mobile for now, otherwise it saturates the bandwidth between the device and the test-running PC.
			if (Project.Properties["config-system"] == "android" || Project.Properties["config-system"] == "iphone")
			{
				useTapEventsForTests = false;
			}
			buildLayoutTags.Options["UseTAPEventsForTests"] = Project.Properties.GetPropertyOrDefault("use-tap-events-for-tests", useTapEventsForTests.ToString().ToLower());
