...
namespace EA.Eaconfig.Build
{
	public class ModuleProcessor_ProcessDependencies : ModuleDependencyProcessorBase, IDisposable
	{
					...
					if (0 == Interlocked.CompareExchange(ref errors, 0, 0))
					{
						try
						{
							...
							else
							{
								...
								Dictionary<string, BuildGroups> declaredModules = GetPackageDeclaredModules(d.Dependent.PackageName, dependentPackage);
		...
		private Dictionary<string, BuildGroups> GetPackageDeclaredModules(string packageName, IPackage dependentPackage = null)
		{
			...
			// special case - Java modules don't require an initialize.xml definition. They can be accessed
			// directly via definition.
			if (dependentPackage != null)
			{
				foreach (IModule javaModule in dependentPackage.Modules.Where(module => module is Module_Java))
				{
					declaredModules[javaModule.Name] = javaModule.BuildGroup;
				}
			}
