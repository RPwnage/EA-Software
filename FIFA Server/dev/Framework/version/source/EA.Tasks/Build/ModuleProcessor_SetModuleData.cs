...
namespace EA.Eaconfig.Build
{
	internal class ModuleProcessor_SetModuleData : ModuleProcessorBase, IModuleProcessor, IDisposable
	{
		...
		public void Process(Module_Native module)
		{
					...
					case "cc":
						{
							...
							module.Cc.HeaderFiles.Append(headerfiles);
			...
			{
				foreach (string nugetReference in PropGroupValue("nugetreferences").ToArray())
				{
					module.NuGetReferences.Add(nugetReference);
				}
				foreach (string nugetReference in PropGroupValue("additional-nuget-references").ToArray())
				{
					module.NuGetReferences.Add(nugetReference);
				}
		...
		public void Process(Module_DotNet module)
		{
			...
			{
				module.NuGetReferences = new List<string>();
				foreach (string nugetReference in PropGroupValue("nugetreferences").ToArray())
				{
					module.NuGetReferences.Add(nugetReference);
				}
				...
				if (module.GetModuleBooleanPropertyValue(GetFrameworkPrefix(module) + ".unittest", false))
				{
					...
					{
						module.NuGetReferences.Add("Microsoft.NET.Test.Sdk");
					}
					module.NuGetReferences.Add("MSTest.TestFramework");
					module.NuGetReferences.Add("MSTest.TestAdapter");
				}
			}
			...
			foreach (string buildtask in StringUtil.ToArray(OptionSetUtil.GetOptionOrFail(BuildOptionSet, "build.tasks")))
			{
					...
					case "csc":
						...
						if (module.TargetFrameworkFamily == DotNetFrameworkFamilies.Framework)
						{
							...
							else
							{
								...
								InternalQuickDependency.Dependent(Project, "DotNet");
						...
						else
						{
							...
							string comp = Project.Properties["package.DotNetSdk.dotnet.exe"] ?? throw new BuildException("Property 'package.DotNetSdk.dotnet.exe' was not set - unable to initialize .NET Core compiler.");
							csc = new CscCompiler(PathString.MakeNormalized(comp, PathString.PathParam.NormalizeOnly),
															ConvertUtil.ToEnum<DotNetCompiler.Targets>(GetOptionString("csc.target", BuildOptionSet.Options)))
							{
							...
							{
								assemblyRefDirs.Add(PathString.MakeNormalized(Project.Properties["package.DotNetSdk.NETStandard.Library.ref.dir"]).Path);
							}
							else if (module.TargetFrameworkFamily == DotNetFrameworkFamilies.Core)
							{
								assemblyRefDirs.Add(PathString.MakeNormalized(Project.Properties["package.DotNetSdk.Microsoft.NETCore.App.ref.dir"]).Path);
								if (((uint) module.ProjectTypes & (uint) DotNetProjectTypes.WebApp) != 0)
								{
									assemblyRefDirs.Add(PathString.MakeNormalized(Project.Properties["package.DotNetSdk.Microsoft.AspNetCore.App.ref.dir"]).Path);
								}
								if (module.Configuration.System=="pc" || module.Configuration.System == "pc64")
								{
									...
									{
										assemblyRefDirs.Add(PathString.MakeNormalized(Project.Properties["package.DotNetSdk.Microsoft.WindowsDesktop.App.ref.dir"]).Path);
									}
								}
							}
				...
				if (nodefaultdefines == false)
				{
					...
					if (module.TargetFrameworkFamily == DotNetFrameworkFamilies.Core)
					{
						...
						string dotnetVersionDefineVS = Project.Properties["package.DotNetSdk.versionDefineVS.core"].TrimWhiteSpace();
						if (!String.IsNullOrEmpty(dotnetVersionDefineVS))
						{
							compiler.Defines.Add(dotnetVersionDefineVS);
						}
						string dotnetVersionDefineEA = Project.Properties["package.DotNetSdk.versionDefineEA.core"].TrimWhiteSpace();
					...
					{
						// trimwhitespace converts null to empty string
						string dotnetVersionDefineVS = Project.Properties["package.DotNetSdk.versionDefineVS.standard"].TrimWhiteSpace();
						if (!String.IsNullOrEmpty(dotnetVersionDefineVS))
						{
							compiler.Defines.Add(dotnetVersionDefineVS);
						}
						string dotnetVersionDefineEA = Project.Properties["package.DotNetSdk.versionDefineEA.standard"].TrimWhiteSpace();
						if (!String.IsNullOrEmpty(dotnetVersionDefineEA))
						{
							compiler.Defines.Add(dotnetVersionDefineEA);
						}
		...
		private void ResolveNugetReferences(Module module, List<string> nuGetReferences)
		{
			if (module.Package.Project.UseNugetResolution)
			{ 
				if (nuGetReferences != null)
				{
					...
					foreach (NugetPackage packageContents in flattenedContents)
					{
						...
						{
							foreach (FileInfo x in packageContents.CompileItems)
							{
								mod.Compiler.Assemblies.Include(new FileItem(new PathString(x.FullName, PathString.PathState.FullPath), optionSetName: "nuget-reference-assembly"));
							}
						...
						{
							foreach (var x in packageContents.CompileItems)
							{
								native.Cc.Assemblies.Include(new FileItem(new PathString(x.FullName, PathString.PathState.FullPath), optionSetName: "nuget-reference-assembly"));
							}
		...
		public void Process(Module_ExternalVisualStudio module)
		{
			...
			string projectOutput = PropGroupValue("project-output");
			if (string.IsNullOrEmpty(projectOutput))
			{
				var msg = String.Format("ProjectOutput parameter must be set on VisualStudioProject task");
				throw new BuildException(msg);
			}
			module.VisualStudioProjectOutput = PathString.MakeNormalized(projectOutput.TrimWhiteSpace());
			module.VisualStudioProjectDebugSymbols = PathString.MakeNormalized(PropGroupValue("project-symbols").TrimWhiteSpace());
		...
		private void ThrowIfBulkBuildPropertiesSet(Module_DotNet module)
		{
			...
			{
				throw new BuildException($"[SetupBulkBuild] BulkBuild features aren't available to C# modules, but '{module.GroupName}.bulkbuild.manual.sourcefiles' was specified.");
			}
			if (FileSetUtil.FileSetExists(Project, PropGroupName("bulkbuild.manual.sourcefiles." + module.Configuration.System)))
			{
				throw new BuildException($"[SetupBulkBuild] BulkBuild features aren't available to C# modules, but '{module.GroupName}.bulkbuild.manual.sourcefiles.{module.Configuration.System}' was specified.");
			}
			if (PropertyUtil.PropertyExists(Project, PropGroupName("bulkbuild.filesets")))
			{
				throw new BuildException($"[SetupBulkBuild] BulkBuild features aren't available to C# modules, but '{module.GroupName}.bulkbuild.filesets' was specified.");
			}
		}
		...
		{
			string prefix = null;
			if (compiler is CscCompiler)
			{
				prefix = "csc";
			}
			else
			{
				throw new NotImplementedException("SetGenerateDocs is being called on a dot net compiler type that it isn't designed to support");
			}
		...
		protected void SetLinkOutput(IModule module, out string outputname, out string outputextension, out PathString linkoutputdir, out string replacename)
		{
			...
			outputextension = MapDependentBuildOutputs.RemapExtension(module, binPath.Extension);
			replacename = binPath.OutputNameToken;
			var outputPath = Path.Combine(module.OutputDir.Path, binPath.OutputNameToken + outputextension);
			...
			module.Macros.Update("output", outputPath);
		}
