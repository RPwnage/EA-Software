...
{
	public abstract class VSProjectBase : ModuleGenerator
	{
		...
		internal VSProjectBase(VSSolutionBase solution, IEnumerable<IModule> modules, Guid projectTypeGuid) : base(solution, modules)
		{
		...
		internal readonly VSSolutionBase SolutionGenerator;
		...
		private readonly HashSet<string> m_uniqueMsBuildTargets = new HashSet<string>();
		...
		public void WriteUniqueTarget(string targetName, Action writeTarget)
		{
			if (!m_uniqueMsBuildTargets.Contains(targetName))
			{
				writeTarget();
				m_uniqueMsBuildTargets.Add(targetName);
			}
		}
		...
		internal IDictionary<PathString, FileEntry> GetAllFiles(Func<Tool, IEnumerable<Tuple<FileSet, uint, Tool>>> func)
		{
		...
		internal bool UpdateFileEntry(IModule module, IDictionary<PathString, FileEntry> files, PathString key, FileItem fileitem, string basedirectory, Configuration config, Tool tool = null, uint flags = 0)
		{
			FileEntry entry;
		...
		internal string GetFileEntryOptionOrDefault(FileEntry fe, string optionName, string defaultVal)
		{
		...
		internal OptionSet GetFileEntryOptionset(FileEntry fe)
		{
		...
		protected virtual void WriteCopyLocalFiles(IXmlWriter writer, bool isNewProjectSystem = false, bool isCsProj = false)
		{
			MSBuildConfigItemGroup copyLocalCustomFilesGroup = new MSBuildConfigItemGroup(AllConfigurations, "Copy Local Files");
			MSBuildConfigItemGroup fastUpToDateTriggers = null;
			MSBuildConfigItemGroup timestampUpdateForUpToDateCheck = null;

			Dictionary<string, Dictionary<string, HashSet<Configuration>>> newStyleUpToDateChecks = null;
			...
			foreach (IModule module in Modules)
			{
				...
				foreach (IGrouping<string, CopyLocalInfo> copyLocalInfoGroup in copyLocalInfosBySource)
				{
					...
					bool usingHardLink = !copyMetaData.TryGetValue("UseHardLink", out string useHardLink) || useHardLink.ToLowerInvariant() == "true";
					if (isCsProj && !isNewProjectSystem)
					{
						...
						// we are to use Visual Studio's fast up to date check mechanism using UpToDateCheckOutput and FrameworkTimestampUpdate
						// (needed for old style csproj).
					...
					if (isNewProjectSystem && isCsProj)
					{
						newStyleUpToDateChecks = newStyleUpToDateChecks ?? new Dictionary<string, Dictionary<string, HashSet<Configuration>>>();
						if (!newStyleUpToDateChecks.TryGetValue(absoluteSourcePath, out Dictionary<string, HashSet<Configuration>> perSourceCopies))
						{
							perSourceCopies = new Dictionary<string, HashSet<Configuration>>();
							newStyleUpToDateChecks.Add(absoluteSourcePath, perSourceCopies);
						}
						foreach (string absDestPath in absolutePreBuildDestPaths)
						{
							if (!perSourceCopies.TryGetValue(absDestPath, out HashSet<Configuration> perDestCopies))
							{
								perDestCopies = new HashSet<Configuration>();
								perSourceCopies.Add(absDestPath, perDestCopies);
							}
							perDestCopies.Add(module.Configuration);
						}
					}
					else
					{
						fastUpToDateTriggers = fastUpToDateTriggers ?? new MSBuildConfigItemGroup(AllConfigurations, "Copy Local Fast Up To Date Triggers");
						...
						if (!usingHardLink && isCsProj)
						{
							foreach (string absDestPath in absolutePreBuildDestPaths)
							{
								timestampUpdateForUpToDateCheck = timestampUpdateForUpToDateCheck ?? new MSBuildConfigItemGroup(AllConfigurations, "Timestamp Update For Up To Date Check");
			...
			if (newStyleUpToDateChecks != null)
			{
				writer.WriteStartElement("ItemGroup");
				writer.WriteNonEmptyAttributeString("Label", "Copy Local Fast Up To Date Triggers");
				foreach (KeyValuePair<string, Dictionary<string, HashSet<Configuration>>> sourceToDestConfigs in newStyleUpToDateChecks)
				{
					string sourceProjectPath = GetProjectPath(sourceToDestConfigs.Key);
					foreach (KeyValuePair<string, HashSet<Configuration>> destToConfigs in sourceToDestConfigs.Value)
					{
						// see https://github.com/dotnet/project-system/blob/main/docs/up-to-date-check.md#copied-files
						// the naming of terms is a bit counter intuitive, but after closely reading the docs
						// and some experimentation you Original the source of the copy and Include the destination

						writer.WriteStartElement("UpToDateCheckBuilt");
						writer.WriteAttributeString("Include", GetProjectPath(destToConfigs.Key));
						writer.WriteAttributeString("Original", sourceProjectPath);

						// wrap the whole item in conditions for configs if building against specific configs
						MSBuildConfigConditions.Condition condition = MSBuildConfigConditions.GetCondition(destToConfigs.Value, AllConfigurations);
						if (condition.ConditionType != MSBuildConfigConditions.Condition.Type.None)
						{
							writer.WriteAttributeString("Condition", MSBuildConfigConditions.FormatCondition("$(Configuration)|$(Platform)", condition, c => GetVSProjConfigurationName(c)));
						}
