		...
		public override string ProgramFileName 
		{
			get { 
				if (Compiler == null) 
				{
					...
					{
						return PathString.MakeNormalized(Project.Properties["package.DotNetSdk.dotnet.exe"]).Path;
					}
					else
					{
		...
		public override string ProgramArguments {
			get {
				if (TargetFrameworkFamily != null && (TargetFrameworkFamily.ToLower() == "core" || TargetFrameworkFamily.ToLower() == "standard"))
				{
					if (this is CscTask)
					{
						return PathString.MakeNormalized(Project.Properties["package.DotNetSdk.csc.dll"]) + " @\"" + _responseFileName + "\"";
					}
					else
					{
