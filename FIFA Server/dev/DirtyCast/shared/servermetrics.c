...
/*F********************************************************************************/
/*!
    \Function _ServerMetricsWriteMetric

    \Description
        Add a uint64_t or float metric to the specified buffer using datadog formatting.

    \Input *pWrite          - pointer to buffer to format data into
    \Input iSize            - size of buffer
    \Input **ppMetricValue  - [IN/OUT] pointer to metric value to be written, updated in function
    \Input eValuesType      - value type (uint64 or float)
...
*/
/********************************************************************************F*/
static int32_t _ServerMetricsWriteMetric(char *pWrite, int32_t iSize, void **ppMetricValue, ServerMetricsValuesE eValuesType)
{
    char strValue[32];
    int32_t iBytesWritten;
    int32_t iResult = SERVERMETRICS_ERR_ENTRY_TOO_LARGE; // default to 'entry too large' error

    switch (eValuesType)
    {
        case SERVERMETRICS_VALUES_UINT64:
            uint64_t **ppMetricValueIntCast = (uint64_t **)ppMetricValue;
            iBytesWritten = ds_snzprintf(&strValue[0], sizeof(strValue), "%llu,", **ppMetricValueIntCast);

            // make sure the value string fits in the provided buffer
            if (iBytesWritten <= iSize)
            {
                iResult = ds_snzprintf(pWrite, iSize, "%llu,", **ppMetricValueIntCast);
                *ppMetricValue = (void *)(++(*ppMetricValueIntCast));
            }
            break;

        case SERVERMETRICS_VALUES_FLOAT:
            float **ppMetricValueFloatCast = (float **)ppMetricValue;
            iBytesWritten = ds_snzprintf(&strValue[0], sizeof(strValue), "%.2f,", **ppMetricValueFloatCast);

            // make sure the value string fits in the provided buffer
            if (iBytesWritten <= iSize)
            {
                iResult = ds_snzprintf(pWrite, iSize, "%.2f,", **ppMetricValueFloatCast);
                *ppMetricValue = (void *)(++(*ppMetricValueFloatCast));
            }
            break;

        default:
            DirtyCastLogPrintf("servermetrics: %d is not a valid value type used with _ServerMetricsWriteMetric()\n", eValuesType);
            iResult = SERVERMETRICS_ERR_UNSUPPORTED_VALUETYPE;
            break;
    }
...
*/
/********************************************************************************F*/
int32_t ServerMetricsFormatEx(char *pBuffer, int32_t iBufSize, const char *pMetricName, ServerMetricsFormatE eFormat, void *pMetricValues, uint32_t uMetricValueCount, ServerMetricsValuesE eValuesType, ServerMetricsTypeE eMetricType, uint32_t uSampleRate, const char *pDimensionsStr)
{
    int32_t iResult = 0;
    ...
    switch (eFormat)
    {
        case SERVERMETRICS_FORMAT_DATADOG:
        {
            ...
            *pWrite++ = ':';
            ...
            for (uMetricValueIndex = 0; uMetricValueIndex < uMetricValueCount; uMetricValueIndex++)
            {
                iResult = _ServerMetricsWriteMetric(pWrite, pEnd - pWrite, &pMetricValues, eValuesType);
...
{
    return(ServerMetricsFormatEx(pBuffer, iBufSize, pMetricName, eFormat, &uMetricValue, 1, SERVERMETRICS_VALUES_UINT64, eMetricType, uSampleRate, pDimensionsStr));
}
...
{
    return(ServerMetricsFormatEx(pBuffer, iBufSize, pMetricName, eFormat, &fMetricValue, 1, SERVERMETRICS_VALUES_FLOAT, eMetricType, uSampleRate, pDimensionsStr));
}
