...
typedef struct GameServerOiMetricDataT
{
    ...
    uint32_t aRemotePacketLossCount[GAMESERVER_MAX_CLIENTS]; //!< remote packet loss count per client
    uint32_t aRemotePacketsTotalSent[GAMESERVER_MAX_CLIENTS]; //!< remote packets sent count per client
    uint8_t aRemotePacketLossMetricDataInitialized[GAMESERVER_MAX_CLIENTS]; // initialization tracker for both remote packet loss data
    uint32_t aLocalPacketLossCount[GAMESERVER_MAX_CLIENTS]; //!< local packet loss count per client
    uint32_t aLocalPacketsTotalSent[GAMESERVER_MAX_CLIENTS]; //!< local packets sent count per client
    uint8_t aLocalPacketLossMetricDataInitialized[GAMESERVER_MAX_CLIENTS]; // initialization tracker for both local packet loss data
...
typedef struct GameServerOiAccumulatorT
{
    union
    {
        uint64_t aUint64[GAMESERVER_OI_ACCUMULATOR_MAX_SIZE];
        float aFloat[GAMESERVER_OI_ACCUMULATOR_MAX_SIZE];
    } Values;
...
typedef struct GameServerOiAccumulatorsT
{
    ...
    GameServerOiAccumulatorT Latency;
    GameServerOiAccumulatorT LocalPacketLoss;
    GameServerOiAccumulatorT Odmpps;
    GameServerOiAccumulatorT RemotePacketLoss;
...
//! metric handler type
typedef int32_t(GameServerOiMetricsHandlerT)(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions);
...
/*** Function Prototypes ***************************************************************/
static int32_t _GameServerOiMetricsIdpps(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions);
static int32_t _GameServerOiMetricsOdmpps(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions);
static int32_t _GameServerOiMetricsInputDrop(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions);
static int32_t _GameServerOiMetricsLatency(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions);
static int32_t _GameServerOiMetricsLocalPacketLoss(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions);
static int32_t _GameServerOiMetricsRemotePacketLoss(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions);
...
static const GameServerOiMetricsHandlerEntryT _GameServerOiMetricsHandlers[] =
{
    ...
    { "gameserver.latency",     _GameServerOiMetricsLatency },
    { "gameserver.lpacketloss", _GameServerOiMetricsLocalPacketLoss },
    { "gameserver.odmpps",      _GameServerOiMetricsOdmpps },
    { "gameserver.rpacketloss", _GameServerOiMetricsRemotePacketLoss }
};
...
/*!
    ...
    \Input *pLink               - game server link module
...
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsInputDropWithoutAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    int32_t iClient;
    int32_t iOffset = 0;
...
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsInputDropWithAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    int32_t iClient;
    int32_t iOffset = 0;
        ...
        for (iClient = 0; (iClient < pMetricsRef->pConfig->uMaxClients) && (iOffset == 0); iClient++)
        {
            ...
            if ((iResult = GameServerDistStatus(pDist, 'drop', iClient, &uCurrentCount, sizeof(uCurrentCount))) >= 0)
            {
                ...
                if ((iResult == 0) && (pMetricsRef->MetricsData.aInputDropCountInitialized[iClient] == TRUE))
                {
                    ...
                    {
                        pMetricsRef->Accumulators.InputDrop.Values.aUint64[pMetricsRef->Accumulators.InputDrop.uCount++] = uRate;
                    }
                    else
                    {
    ...
    if (iOffset == 0)
    {
            ...
            (pMetricsRef->Accumulators.InputDrop.uCount != 0))
        {
            ...
            iResult = ServerMetricsFormatEx(pBuffer + iOffset, iBufSize - iOffset, pMetricName, eFormat,
                &pMetricsRef->Accumulators.InputDrop.Values.aUint64[0], pMetricsRef->Accumulators.InputDrop.uCount,
                SERVERMETRICS_VALUES_UINT64, SERVERMETRICS_TYPE_TIMER, 100, pCommonDimensions);
...
static int32_t _GameServerOiMetricsInputDrop(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    ...
    {
        return(_GameServerOiMetricsInputDropWithoutAccumulation(pMetricsRef, pDist, pLink, pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
    else
    {
        return(_GameServerOiMetricsInputDropWithAccumulation(pMetricsRef, pDist, pLink, pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
}
...
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsIdppsWithoutAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    int32_t iClient;
    int32_t iOffset = 0;
...
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsIdppsWithAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    int32_t iClient;
    int32_t iOffset = 0;
            ...
            #endif
            {
                ...
                if ((iResult == 0) && (pMetricsRef->MetricsData.aInputDistPacketCountInitialized[iClient] == TRUE))
                {
                    ...
                    if ((pMetricsRef->MetricsData.bIddpsGracePeriodDone[iClient] == TRUE) || (uRate >= 27))
                    {
                        ...
                        {
                            pMetricsRef->Accumulators.Idpps.Values.aUint64[pMetricsRef->Accumulators.Idpps.uCount++] = uRate;
                        }
                        else
                        {
                            iOffset = SERVERMETRICS_ERR_ACCUMULATION_OVERFLOW;
    ...
    if (iOffset == 0)
    {
            ...
            (pMetricsRef->Accumulators.Idpps.uCount != 0))
        {
            ...
            iResult = ServerMetricsFormatEx(pBuffer + iOffset, iBufSize - iOffset, pMetricName, eFormat,
                &(pMetricsRef->Accumulators.Idpps.Values.aUint64[0]), pMetricsRef->Accumulators.Idpps.uCount, SERVERMETRICS_VALUES_UINT64,
...
static int32_t _GameServerOiMetricsIdpps(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    if (pMetricsRef->iMaxAccumulationCycles == 0)
    {
        return(_GameServerOiMetricsIdppsWithoutAccumulation(pMetricsRef, pDist, pLink, pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
    else
    {
        return(_GameServerOiMetricsIdppsWithAccumulation(pMetricsRef, pDist, pLink, pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
}
...
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsOdmppsWithoutAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
...
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsOdmppsWithAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    uint32_t uCurrentCount=0;
    uint32_t uRate = 0;
    int32_t iOffset = 0;
    ...
    #endif
    {
        ...
        if ((iResult == 0) && (pMetricsRef->MetricsData.bOutputMultiDistPacketCountInitialized == TRUE))
        {
            ...
            if (pMetricsRef->Accumulators.Odmpps.uCount < GAMESERVER_OI_ACCUMULATOR_MAX_SIZE)
            {
                ...
                {
                    pMetricsRef->Accumulators.Odmpps.Values.aUint64[pMetricsRef->Accumulators.Odmpps.uCount++] = uRate;
                }
                ...
                if (pMetricsRef->Accumulators.iAccumulationCyclesCount == (pMetricsRef->iMaxAccumulationCycles - 1))
                {
                    iOffset = ServerMetricsFormatEx(pBuffer, iBufSize, pMetricName, eFormat,
                        &pMetricsRef->Accumulators.Odmpps.Values.aUint64[0], pMetricsRef->Accumulators.Odmpps.uCount,
                        SERVERMETRICS_VALUES_UINT64, SERVERMETRICS_TYPE_TIMER, 100, pCommonDimensions);
...
static int32_t _GameServerOiMetricsOdmpps(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    if (pMetricsRef->iMaxAccumulationCycles == 0)
    {
        return(_GameServerOiMetricsOdmppsWithoutAccumulation(pMetricsRef, pDist, pLink, pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
    else
    {
        return(_GameServerOiMetricsOdmppsWithAccumulation(pMetricsRef, pDist, pLink, pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
}
...
/*F********************************************************************************/
/*!
    \Function _GameServerOiMetricsLatencyWithoutAccumulation
    ...
    \Version 03/12/2021 (mgallant)
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsLatencyWithoutAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    int32_t iClient;
    int32_t iOffset = 0;
    ...
    for (iClient = 0; iClient < pMetricsRef->pConfig->uMaxClients; iClient++)
    {
        ...
        if ((iResult = GameServerLinkStatus(pLink, 'stat', iClient, &Stat, sizeof(NetGameLinkStatT))) == 0)
        {
            uint32_t uRate = Stat.late;
            iResult = ServerMetricsFormat(pBuffer + iOffset, iBufSize - iOffset, pMetricName, eFormat, uRate, SERVERMETRICS_TYPE_TIMER, 100, pCommonDimensions);
...
/*F********************************************************************************/
/*!
    \Function _GameServerOiMetricsLatencyWithAccumulation
...
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsLatencyWithAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    int32_t iClient;
    int32_t iOffset = 0;

    // avoid accumulating twice if we are being re-executed after back buffer expansion
    if (pMetricsRef->Accumulators.Latency.bPendingBackBufferExpansion == FALSE)
    {
        for (iClient = 0; (iClient < pMetricsRef->pConfig->uMaxClients) && (iOffset == 0); iClient++)
        {
            NetGameLinkStatT Stat;
            ...
            if ((iResult = GameServerLinkStatus(pLink, 'stat', iClient, &Stat, sizeof(NetGameLinkStatT))) == 0)
            {
                ...
                if (pMetricsRef->Accumulators.Latency.uCount < GAMESERVER_OI_ACCUMULATOR_MAX_SIZE)
                {
                    pMetricsRef->Accumulators.Latency.Values.aUint64[pMetricsRef->Accumulators.Latency.uCount++] = uRate;
                }
                else
                {
                    iOffset = SERVERMETRICS_ERR_ACCUMULATION_OVERFLOW;
    ...
    if (iOffset == 0)
    {
        // is it time to flush out the accumulated values?
        if ((pMetricsRef->Accumulators.iAccumulationCyclesCount == (pMetricsRef->iMaxAccumulationCycles - 1)) &&
            (pMetricsRef->Accumulators.Latency.uCount != 0))
        {
            int32_t iResult = 0;

            iResult = ServerMetricsFormatEx(pBuffer + iOffset, iBufSize - iOffset, pMetricName, eFormat,
                &(pMetricsRef->Accumulators.Latency.Values.aUint64[0]), pMetricsRef->Accumulators.Latency.uCount,
                SERVERMETRICS_VALUES_UINT64, SERVERMETRICS_TYPE_TIMER, 100, pCommonDimensions);
            ...
            if (iResult < 0)
            {
                ...
                {
                    // set flag used to detect that we are being re-executed after back buffer expansion
                    pMetricsRef->Accumulators.Latency.bPendingBackBufferExpansion = TRUE;
                }
...
/*F********************************************************************************/
/*!
    \Function _GameServerOiMetricsLatency
...
static int32_t _GameServerOiMetricsLatency(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    if (pMetricsRef->iMaxAccumulationCycles == 0)
    {
        return(_GameServerOiMetricsLatencyWithoutAccumulation(pMetricsRef, pDist, pLink,  pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
    else
    {
        return(_GameServerOiMetricsLatencyWithAccumulation(pMetricsRef, pDist, pLink, pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
}
...
/*F********************************************************************************/
/*!
    \Function _GameServerOiMetricsRemotePacketLossWithoutAccumulation
...
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsRemotePacketLossWithoutAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    int32_t iClient;
    int32_t iOffset = 0;
    ...
    for (iClient = 0; iClient < pMetricsRef->pConfig->uMaxClients; iClient++)
    {
        ...
        int32_t iResult;
        uint32_t uCurrentCount = 0;
        uint32_t uPacketsSentToRemote = 0;

        if ((iResult = GameServerLinkStatus(pLink, 'stat', iClient, &Stat, sizeof(NetGameLinkStatT))) == 0)
        {
            float fRate = 0;
            uCurrentCount = Stat.rpacklost;
            uPacketsSentToRemote = Stat.rpackrcvd + Stat.rpacklost;

            // calculate rate only if previous value has been initialized already
            if (pMetricsRef->MetricsData.aRemotePacketLossMetricDataInitialized[iClient] == TRUE)
            {
                // calculate as a percentage of total packets sent since last calculation
                fRate = ((uPacketsSentToRemote - pMetricsRef->MetricsData.aRemotePacketsTotalSent[iClient]) > 0) ? ((float)(uCurrentCount - pMetricsRef->MetricsData.aRemotePacketLossCount[iClient]) * 100.0f / (float)(uPacketsSentToRemote - pMetricsRef->MetricsData.aRemotePacketsTotalSent[iClient])) : 0;

                iResult = ServerMetricsFormatFloat(pBuffer + iOffset, iBufSize - iOffset, pMetricName, eFormat, fRate, SERVERMETRICS_TYPE_TIMER, 100, pCommonDimensions);
            ...
            // save down previous values
            pMetricsRef->MetricsData.aRemotePacketLossCount[iClient] = uCurrentCount;
            pMetricsRef->MetricsData.aRemotePacketsTotalSent[iClient] = uPacketsSentToRemote;
            pMetricsRef->MetricsData.aRemotePacketLossMetricDataInitialized[iClient] = TRUE;
        }
        else
        {
            // mark count as uninitialized to avoid next cycle diffing readings that are not back-to-back
            pMetricsRef->MetricsData.aRemotePacketLossCount[iClient] = 0;
            pMetricsRef->MetricsData.aRemotePacketsTotalSent[iClient] = 0;
            pMetricsRef->MetricsData.aRemotePacketLossMetricDataInitialized[iClient] = FALSE;
        }
    }
...
/*F********************************************************************************/
/*!
    \Function _GameServerOiMetricsRemotePacketLossWithAccumulation
...
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsRemotePacketLossWithAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    int32_t iClient;
    int32_t iOffset = 0;

    // avoid accumulating twice if we are being re-executed after back buffer expansion
    if (pMetricsRef->Accumulators.RemotePacketLoss.bPendingBackBufferExpansion == FALSE)
    {
        for (iClient = 0; (iClient < pMetricsRef->pConfig->uMaxClients) && (iOffset == 0); iClient++)
        {
            NetGameLinkStatT Stat;
            ...
            if ((iResult = GameServerLinkStatus(pLink, 'stat', iClient, &Stat, sizeof(NetGameLinkStatT))) == 0)
            {
                ...
                if (pMetricsRef->MetricsData.aRemotePacketLossMetricDataInitialized[iClient] == TRUE)
                {
                    // calculate as a percentage of total rpacksent
                    fRate = ((uPacketsSentToRemote - pMetricsRef->MetricsData.aRemotePacketsTotalSent[iClient]) > 0) ? ((float)(uCurrentCount - pMetricsRef->MetricsData.aRemotePacketLossCount[iClient]) * 100.0f / (float)(uPacketsSentToRemote - pMetricsRef->MetricsData.aRemotePacketsTotalSent[iClient])) : 0;
                    
                    if (pMetricsRef->Accumulators.RemotePacketLoss.uCount < GAMESERVER_OI_ACCUMULATOR_MAX_SIZE)
                    {
                        pMetricsRef->Accumulators.RemotePacketLoss.Values.aFloat[pMetricsRef->Accumulators.RemotePacketLoss.uCount++] = fRate;
                    }
                    else
                    {
                        iOffset = SERVERMETRICS_ERR_ACCUMULATION_OVERFLOW;
    ...
    if (iOffset == 0)
    {
        // is it time to flush out the accumulated values?
        if ((pMetricsRef->Accumulators.iAccumulationCyclesCount == (pMetricsRef->iMaxAccumulationCycles - 1)) &&
            (pMetricsRef->Accumulators.RemotePacketLoss.uCount != 0))
        {
            int32_t iResult = 0;

            iResult = ServerMetricsFormatEx(pBuffer + iOffset, iBufSize - iOffset, pMetricName, eFormat,
                &(pMetricsRef->Accumulators.RemotePacketLoss.Values.aFloat[0]), pMetricsRef->Accumulators.RemotePacketLoss.uCount,
                SERVERMETRICS_VALUES_FLOAT, SERVERMETRICS_TYPE_TIMER, 100, pCommonDimensions);
            ...
            if (iResult < 0)
            {
                ...
                {
                    // set flag used to detect that we are being re-executed after back buffer expansion
                    pMetricsRef->Accumulators.RemotePacketLoss.bPendingBackBufferExpansion = TRUE;
                }
...
/*F********************************************************************************/
/*!
    \Function _GameServerOiMetricsRemotePacketLoss
...
static int32_t _GameServerOiMetricsRemotePacketLoss(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    if (pMetricsRef->iMaxAccumulationCycles == 0)
    {
        return(_GameServerOiMetricsRemotePacketLossWithoutAccumulation(pMetricsRef, pDist, pLink, pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
    else
    {
        return(_GameServerOiMetricsRemotePacketLossWithAccumulation(pMetricsRef, pDist, pLink, pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
}
...
/*F********************************************************************************/
/*!
    \Function _GameServerOiMetricsLocalPacketLossWithoutAccumulation
...
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsLocalPacketLossWithoutAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    int32_t iClient;
    int32_t iOffset = 0;
    ...
    for (iClient = 0; iClient < pMetricsRef->pConfig->uMaxClients; iClient++)
    {
        ...
        int32_t iResult;
        uint32_t uCurrentCount = 0;
        uint32_t uPacketsSentToLocal = 0;

        if ((iResult = GameServerLinkStatus(pLink, 'stat', iClient, &Stat, sizeof(NetGameLinkStatT))) == 0)
        {

            float fRate = 0;
            uCurrentCount = Stat.lpacklost;
            uPacketsSentToLocal = Stat.lpackrcvd + Stat.lpacklost;

            // calculate rate only if previous value has been initialized already
            if (pMetricsRef->MetricsData.aLocalPacketLossMetricDataInitialized[iClient] == TRUE)
            {
                // calculate as a percentage of total lpacksent
                fRate = ((uPacketsSentToLocal - pMetricsRef->MetricsData.aLocalPacketsTotalSent[iClient]) > 0) ? ((float)(uCurrentCount - pMetricsRef->MetricsData.aLocalPacketLossCount[iClient]) * 100.0f / (float)(uPacketsSentToLocal - pMetricsRef->MetricsData.aLocalPacketsTotalSent[iClient])) : 0;
            ...
            // save down previous value
            pMetricsRef->MetricsData.aLocalPacketLossCount[iClient] = uCurrentCount;
            pMetricsRef->MetricsData.aLocalPacketsTotalSent[iClient] = uPacketsSentToLocal;
            pMetricsRef->MetricsData.aLocalPacketLossMetricDataInitialized[iClient] = TRUE;
        }
        else
        {
            // mark count as uninitialized to avoid next cycle diffing readings that are not back-to-back
            pMetricsRef->MetricsData.aLocalPacketLossCount[iClient] = 0;
            pMetricsRef->MetricsData.aLocalPacketsTotalSent[iClient] = 0;
            pMetricsRef->MetricsData.aLocalPacketLossMetricDataInitialized[iClient] = FALSE;
        }
    }
...
/*F********************************************************************************/
/*!
    \Function _GameServerOiMetricsLocalPacketLossWithAccumulation
...
*/
/********************************************************************************F*/
static int32_t _GameServerOiMetricsLocalPacketLossWithAccumulation(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    int32_t iClient;
    int32_t iOffset = 0;

    // avoid accumulating twice if we are being re-executed after back buffer expansion
    if (pMetricsRef->Accumulators.LocalPacketLoss.bPendingBackBufferExpansion == FALSE)
    {
        for (iClient = 0; (iClient < pMetricsRef->pConfig->uMaxClients) && (iOffset == 0); iClient++)
        {
            NetGameLinkStatT Stat;
            ...
            if ((iResult = GameServerLinkStatus(pLink, 'stat', iClient, &Stat, sizeof(NetGameLinkStatT))) == 0)
            {
                ...
                if (pMetricsRef->MetricsData.aLocalPacketLossMetricDataInitialized[iClient] == TRUE)
                {
                    ...
                    if (pMetricsRef->Accumulators.LocalPacketLoss.uCount < GAMESERVER_OI_ACCUMULATOR_MAX_SIZE)
                    {
                        pMetricsRef->Accumulators.LocalPacketLoss.Values.aFloat[pMetricsRef->Accumulators.LocalPacketLoss.uCount++] = fRate;
                    }
                    else
                    {
                        iOffset = SERVERMETRICS_ERR_ACCUMULATION_OVERFLOW;
    ...
    if (iOffset == 0)
    {
        // is it time to flush out the accumulated values?
        if ((pMetricsRef->Accumulators.iAccumulationCyclesCount == (pMetricsRef->iMaxAccumulationCycles - 1)) &&
            (pMetricsRef->Accumulators.LocalPacketLoss.uCount != 0))
        {
            int32_t iResult = 0;

            iResult = ServerMetricsFormatEx(pBuffer + iOffset, iBufSize - iOffset, pMetricName, eFormat,
                &(pMetricsRef->Accumulators.LocalPacketLoss.Values.aFloat[0]), pMetricsRef->Accumulators.LocalPacketLoss.uCount,
                SERVERMETRICS_VALUES_FLOAT, SERVERMETRICS_TYPE_TIMER, 100, pCommonDimensions);
            ...
            if (iResult < 0)
            {
                ...
                {
                    // set flag used to detect that we are being re-executed after back buffer expansion
                    pMetricsRef->Accumulators.LocalPacketLoss.bPendingBackBufferExpansion = TRUE;
                }
...
/*F********************************************************************************/
/*!
    \Function _GameServerOiMetricsLocalPacketLoss
...
static int32_t _GameServerOiMetricsLocalPacketLoss(GameServerOiMetricsRefT *pMetricsRef, GameServerDistT *pDist, GameServerLinkT *pLink, const char *pMetricName, ServerMetricsFormatE eFormat, char *pBuffer, int32_t iBufSize, const char *pCommonDimensions)
{
    if (pMetricsRef->iMaxAccumulationCycles == 0)
    {
        return(_GameServerOiMetricsLocalPacketLossWithoutAccumulation(pMetricsRef, pDist, pLink, pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
    else
    {
        return(_GameServerOiMetricsLocalPacketLossWithAccumulation(pMetricsRef, pDist, pLink, pMetricName, eFormat, pBuffer, iBufSize, pCommonDimensions));
    }
}
...
static int32_t _GameServerOiMetricsCallback(ServerMetricsT *pServerMetrics, char *pBuffer, int32_t *pSize, int32_t iMetricIndex, void *pUserData)
{
    ...
    GameServerLinkT* pLink = GameServerBlazeGetGameServerLink();
    ...
    if ((pDist != NULL) && GameServerStatus(pMetricsRef->pGameServer, 'gsta', 0, NULL, 0))
    {
        ...
        while (iMetricIndex < (signed)DIRTYCAST_CalculateArraySize(_GameServerOiMetricsHandlers))
        {
            ...
            if ((iResult = pEntry->pMetricHandler(pMetricsRef, pDist, pLink, pEntry->strMetricName, pMetricsRef->pConfig->eMetricsFormat, pBuffer + iOffset, iBufSize - iOffset, strCommonDimensions)) > 0)
            {
