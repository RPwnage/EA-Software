...
ClubsSlaveImpl::ClubsSlaveImpl() :
    ...
    mPerfCachedClubs(getMetricsCollection(), "cachedClubs", [this]() { return mClubDataCache.size(); }),
...
void ClubsSlaveImpl::purgeStaleObjectsFromDatabase()
{
    ...
    for (ClubDomainsById::const_iterator domainItr = mClubDomains.begin(); domainItr != mClubDomains.end(); ++domainItr)
    {
        ...
        {
            diff = purgeInactiveClubs(conn, domain->getClubDomainId(), domain->getMaxInactiveDaysPerClub());
            if(diff > 0)
...
void ClubsSlaveImpl::getStatusInfo(ComponentStatus& status) const
{
    ...
    blaze_snzprintf(buffer, sizeof(buffer), "%" PRIu64, mPerfCachedClubs.get());
    map["TotalCachedClubs"] = buffer;
            ...
            if (statusItr != userItr->second->end())
            {
                ...
                if (deleteOnlineStatusInClub)
                {
                    ...
                    if (userItr->second->empty())
                    {
                        ...
                        // onLocalUserSessionLogout should invalidate this member's cache, but just in case ...
                        invalidateCachedMemberInfo(blazeId, clubId);
                    }
                }
                ...
                if (clubItr != mOnlineStatusCountsForClub.end())
                {
                    ...
                    if ((*clubItr->second)[CLUBS_TOTAL_STATUS_COUNT_INDEX] == 0)
                    {
                        ...
                        // if any clubs are orphaned in this cache, we'll rely on the purge inactive clubs job to eventually remove them
                    }
                }
                else
                {
