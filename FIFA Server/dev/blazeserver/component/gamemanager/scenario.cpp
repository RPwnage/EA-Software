...
namespace GameManager
{
    ...
    BlazeRpcError MatchmakingScenarioManager::convertMatchmakingComponentErrorToGameManagerError(const BlazeRpcError &mmRpcError) const
    {
        switch(mmRpcError)
        {
        case ERR_OK:
    ...
    // This function compliments convertGameManagerErrorToGamePackerError on the GamePackerMaster
    BlazeRpcError MatchmakingScenarioManager::convertGamePackerErrorToGameManagerError(const BlazeRpcError &mmRpcError) const
    {
        switch (mmRpcError)
        {
        ...
        case GAMEPACKER_ERR_INVALID_MATCHMAKING_CRITERIA:        return GAMEMANAGER_ERR_INVALID_MATCHMAKING_CRITERIA;
        case GAMEPACKER_ERR_MISSING_REQUIRED_FILTER:             return GAMEMANAGER_ERR_INVALID_MATCHMAKING_CRITERIA;
        case GAMEPACKER_ERR_DUPLICATE_REQUIRED_FILTERS_PROVIDED: return GAMEMANAGER_ERR_INVALID_MATCHMAKING_CRITERIA;

        case GAMEPACKER_ERR_CROSSPLAY_DISABLED:          return GAMEMANAGER_ERR_CROSSPLAY_DISABLED;
        case GAMEPACKER_ERR_CROSSPLAY_DISABLED_USER:     return GAMEMANAGER_ERR_CROSSPLAY_DISABLED_USER;
        case GAMEPACKER_ERR_INVALID_TEMPLATE_NAME:       return GAMEMANAGER_ERR_INVALID_TEMPLATE_NAME;
        case GAMEPACKER_ERR_MISSING_TEMPLATE_ATTRIBUTE:  return GAMEMANAGER_ERR_MISSING_TEMPLATE_ATTRIBUTE;
        case GAMEPACKER_ERR_INVALID_TEMPLATE_ATTRIBUTE:  return GAMEMANAGER_ERR_INVALID_TEMPLATE_ATTRIBUTE;
...
//        else
        ...
        MatchmakingCriteriaError& critErr, const InetAddress& callerNetworkAddress, const OverrideUserSessionInfo *overrideUserSessionInfo /*= nullptr*/)
    {
        ...
        bool isPackerScenario = curScenarioConfig->getIsPackerScenario();
    ...
    bool MatchmakingScenarioManager::configure(const GameManagerServerConfig& configTdf)
    {
        ...
        // use reconfigure because we may be entering from configure OR reconfigure of the parent component, and the property manager's reconfigure clears old state
        if (!mPropertyManager.onReconfigure(configTdf.getProperties()))
        {
    ...
    EA::TDF::TdfHashValue MatchmakingScenarioManager::getScenarioRuleAttributesHash(const ScenarioRuleAttributes& scenarioRuleAttributes) const
    {
        ...
        for (; itr != end; ++itr)
        {
            ...
            for (; scenarioAttrMapItr != scenarioAttrMapEnd ; ++scenarioAttrMapItr)
            {
                ...
                EA::TDF::TdfHashValue scenarioAttrMapItrHash = EA::StdC::kCRC32InitialValue;
    ...
    EA::TDF::TdfHashValue MatchmakingScenarioManager::getMatchmakingFilterAttributesHash(const MatchmakingFilterDefinition& filterDefinition) const
    {
        EA::TDF::TdfHashValue hash = EA::StdC::kCRC32InitialValue;
        filterDefinition.computeHash(hash);

        EA::TDF::TdfHashValue requirementAttribHash = getTemplateAttributeTdfMappingHash(filterDefinition.getRequirement());
        hash = EA::StdC::CRC32(&requirementAttribHash, sizeof(requirementAttribHash), hash, false);
        return hash;

    }


    EA::TDF::TdfHashValue MatchmakingScenarioManager::getInputSanitizerAttributesHash(const SanitizerDefinition& sanitizerDefinition) const
    {
        EA::TDF::TdfHashValue hash = EA::StdC::kCRC32InitialValue;
        sanitizerDefinition.computeHash(hash);

        EA::TDF::TdfHashValue sanitizerAttribHash = getTemplateAttributeTdfMappingHash(sanitizerDefinition.getSanitizer());
        hash = EA::StdC::CRC32(&sanitizerAttribHash, sizeof(sanitizerAttribHash), hash, false);
        return hash;
    ...
    EA::TDF::TdfHashValue MatchmakingScenarioManager::getGameQualityFactorsHash(const PackerConfig& templatePackerConfig) const
    {
        EA::TDF::TdfHashValue hash = EA::StdC::kCRC32InitialValue;
        templatePackerConfig.computeHash(hash);

        for (const auto& gqfItr : templatePackerConfig.getQualityFactors())
        {
            const GameQualityFactorConfig& gqfDef = *gqfItr;
            EA::TDF::TdfHashValue definitionHash = EA::StdC::kCRC32InitialValue;
            gqfDef.computeHash(definitionHash);

            hash = EA::StdC::CRC32(&definitionHash, sizeof(definitionHash), hash, false);

            EA::TDF::TdfHashValue keysHash = EA::StdC::kCRC32InitialValue;
            gqfDef.getKeys().computeHash(keysHash);
            EA::TDF::TdfHashValue scoringMapHash = EA::StdC::kCRC32InitialValue;
            gqfDef.getScoringMap().computeHash(scoringMapHash);

            EA::TDF::TdfHashValue scoreShaperHash = getTemplateAttributeTdfMappingHash(gqfDef.getScoreShaper());
           
            hash = EA::StdC::CRC32(&keysHash, sizeof(keysHash), hash, false);
            hash = EA::StdC::CRC32(&scoringMapHash, sizeof(scoringMapHash), hash, false);
            hash = EA::StdC::CRC32(&scoreShaperHash, sizeof(scoreShaperHash), hash, false);

        }
        return hash;
    }

    EA::TDF::TdfHashValue MatchmakingScenarioManager::getTemplateAttributeTdfMappingHash(const TemplateAttributeTdfMapping& templateAttributeTdfMapping) const
    {
        EA::TDF::TdfHashValue hash = EA::StdC::kCRC32InitialValue;
        for (const auto& attributeItr : templateAttributeTdfMapping)
        {
            TemplateAttributeTdfName attributeTypeName = attributeItr.first;
            for (const auto& attrMapItr : *attributeItr.second)
            {
                const auto& attributeTdfMemberName = attrMapItr.first;
                EA::TDF::TdfHashValue attrMapItrHash = EA::StdC::kCRC32InitialValue;
                attrMapItr.second->computeHash(attrMapItrHash);

                hash = EA::StdC::CRC32(attributeTdfMemberName.c_str(), strlen(attributeTdfMemberName), hash, false);
                hash = EA::StdC::CRC32(&attrMapItrHash, sizeof(attrMapItrHash), hash, false);
            }
            hash = EA::StdC::CRC32(attributeTypeName.c_str(), strlen(attributeTypeName), hash, false);
        }
        return hash;
    }
    ...
    bool MatchmakingScenarioManager::hashScenarioConfiguration(const GameManagerServerConfig& config)
    {
        ...
        for (const auto& globalRuleMapItr : config.getScenariosConfig().getGlobalRules())
        {
            const auto& globalRuleName = globalRuleMapItr.first;
            ...
            globalRuleHash = EA::StdC::CRC32(globalRuleName.c_str(), strlen(globalRuleName), globalRuleHash, false);
        }

        EA::TDF::TdfHashValue globalFilterHash = EA::StdC::kCRC32InitialValue;
        for (const auto& globalFilterMapItr : config.getScenariosConfig().getGlobalFilters())
        {
            MatchmakingFilterName globalFilterName = globalFilterMapItr;
            EA::TDF::TdfHashValue filterAttrHash = EA::StdC::kCRC32InitialValue;;


            const auto& filterDefinitionItr = config.getFilters().find(globalFilterName.c_str());
            if (filterDefinitionItr != config.getFilters().end())
            {
                filterAttrHash = getMatchmakingFilterAttributesHash(*filterDefinitionItr->second);
                globalFilterHash = EA::StdC::CRC32(&filterAttrHash, sizeof(filterAttrHash), globalFilterHash, false);
                globalFilterHash = EA::StdC::CRC32(globalFilterName.c_str(), strlen(globalFilterName), globalFilterHash, false);
            }

            
        }

        EA::TDF::TdfHashValue globalInputSanitizerHash = EA::StdC::kCRC32InitialValue;
        for (const auto& globalSanitizerListItr : config.getScenariosConfig().getGlobalInputSanitizers())
        {
            const auto& globalSanitizerName = globalSanitizerListItr;
            EA::TDF::TdfHashValue sanitizerAttrHash = EA::StdC::kCRC32InitialValue;;
            
            const auto& sanitizerDefinitionItr = config.getInputSanitizers().find(globalSanitizerName.c_str());
            if (sanitizerDefinitionItr != config.getInputSanitizers().end())
            {
                sanitizerAttrHash = getInputSanitizerAttributesHash(*sanitizerDefinitionItr->second);
                globalInputSanitizerHash = EA::StdC::CRC32(&sanitizerAttrHash, sizeof(sanitizerAttrHash), globalInputSanitizerHash, false);
                globalInputSanitizerHash = EA::StdC::CRC32(globalSanitizerName.c_str(), strlen(globalSanitizerName), globalInputSanitizerHash, false);
            }
        ...
        const MatchmakingFilterMap& matchmakingFilterMap = config.getFilters();
        const SanitizerDefinitionMap& inputSanitizerMap = config.getInputSanitizers();
        const CreateGameTemplateMap& createGameTemplateMap = config.getCreateGameTemplatesConfig().getCreateGameTemplates();
        ...
        for (; itr != end; ++itr)
        {
            ...
            scenarioHash = EA::StdC::CRC32(&globalFilterHash, sizeof(globalFilterHash), scenarioHash, true);
            scenarioHash = EA::StdC::CRC32(&globalInputSanitizerHash, sizeof(globalInputSanitizerHash), scenarioHash, true);

            // sanitizers aren't per-subsession, unlike rules/filters, so just compute once here
            for (const auto& scenarioSanitizerListItr : scenarioConfig.getInputSanitizers())
            {
                auto const& scenarioSanitizerName = scenarioSanitizerListItr;
                EA::TDF::TdfHashValue sanitizerAttrHash = EA::StdC::kCRC32InitialValue;;

                const auto& sanitizerDefinitionItr = inputSanitizerMap.find(scenarioSanitizerName.c_str());
                if (sanitizerDefinitionItr != inputSanitizerMap.end())
                {
                    sanitizerAttrHash = getInputSanitizerAttributesHash(*sanitizerDefinitionItr->second);
                    scenarioHash = EA::StdC::CRC32(&sanitizerAttrHash, sizeof(sanitizerAttrHash), scenarioHash, false);
                    scenarioHash = EA::StdC::CRC32(scenarioSanitizerName.c_str(), strlen(scenarioSanitizerName), scenarioHash, false);
                }
            }
            ...
            if (scenarioConfig.getVariants().empty())
            {
                ...
                {
                    computeScenarioHash(subSessionConfigMapItr->first, subSessionConfigMapItr->second, scenarioRuleMap, matchmakingFilterMap, createGameTemplateMap, scenarioHash);
                }
            ...
            else
            {
                ...
                for (; curSession != endSession; ++curSession)
                {
                    ...
                    for (; curSubSession != endSubSession; ++curSubSession)
                    {
                        ...
                        {
                            computeScenarioHash(it->first, it->second, scenarioRuleMap, matchmakingFilterMap, createGameTemplateMap, scenarioHash);
                        }
                    }
    ...
    void MatchmakingScenarioManager::computeScenarioHash(const SubSessionName& subSessionName, const SubSessionConfig* subSessionConfig, 
        const ScenarioRuleMap& scenarioRuleMap, const MatchmakingFilterMap& matchmakingFilterMap, const CreateGameTemplateMap& cgTemplateMap, EA::TDF::TdfHashValue& scenarioHash)
    {
        ...
        for (const auto& subSessionRuleNameListItr : subSessionConfig->getRulesList())
        {
            // PACKER_TODO: include subsession filters, sanitizers and templates in the hash
            const ScenarioCustomRuleName& scenarioCustomRuleName = subSessionRuleNameListItr;
            const auto& it = scenarioRuleMap.find(scenarioCustomRuleName.c_str());
        ...
        for (const auto& scenarioFilterListItr : subSessionConfig->getFiltersList())
        {
            const MatchmakingFilterName& filterName = scenarioFilterListItr;
            EA::TDF::TdfHashValue filterAttrHash = EA::StdC::kCRC32InitialValue;


            const auto& filterDefinitionItr = matchmakingFilterMap.find(filterName.c_str());
            if (filterDefinitionItr != matchmakingFilterMap.end())
            {
                filterAttrHash = filterDefinitionItr->second->computeHash(filterAttrHash);

                subSessionHash = EA::StdC::CRC32(&filterAttrHash, sizeof(filterAttrHash), subSessionHash, false);
                subSessionHash = EA::StdC::CRC32(filterName.c_str(), strlen(filterName), subSessionHash, false);
            }

        }

        // if this is a packer session/subsession, we care about the underlying game template as well
        const TemplateName& cgTemplateName = subSessionConfig->getCreateGameTemplate();
        const auto& templateDefinitionItr = cgTemplateMap.find(cgTemplateName.c_str());
        if (templateDefinitionItr != cgTemplateMap.end())
        {
            const CreateGameTemplatePtr cgTemplate = templateDefinitionItr->second;
            if (!cgTemplate->getPackerConfig().getQualityFactors().empty())
            {
                EA::TDF::TdfHashValue packerConfigHash = getGameQualityFactorsHash(cgTemplate->getPackerConfig());
                subSessionHash = EA::StdC::CRC32(&packerConfigHash, sizeof(packerConfigHash), subSessionHash, false);
                subSessionHash = EA::StdC::CRC32(cgTemplateName.c_str(), strlen(cgTemplateName), subSessionHash, false);
            }
        }
    ...
    bool MatchmakingScenarioManager::reconfigure(const GameManagerServerConfig& configTdf)
    {
        ...
        // Cancel all existing legacy sessions (if the session is in the process of being created, this would wait for the session to finish before canceling it).
        cancelAllLegacyScenarios();
        clearScenarioConfigs();
    ...
    bool isPackerSubsessionCheck(const GameManager::SubSessionConfig& subSessionConfig, const CreateGameTemplatesConfig& cgTemplatesConfig)
    {
        // This function determines if the subsession uses a Packer CG template. 
        // The Scenarios config validation ensures that all subsession have the same values for this.
        if (subSessionConfig.getCreateGameTemplateAsTdfString().empty())
            return false;

        const auto& cgTemplateConfig = cgTemplatesConfig.getCreateGameTemplates();
        CreateGameTemplateMap::const_iterator templateIter = cgTemplateConfig.find(subSessionConfig.getCreateGameTemplate());
        if (templateIter == cgTemplateConfig.end())
        {
            return false;
        }

        CreateGameTemplate& templateConfig = (*templateIter->second);
        return !templateConfig.getPackerConfig().getQualityFactors().empty();
    }
    ...
    bool MatchmakingScenarioManager::validateConfig(const ScenariosConfig& config, const MatchmakingServerConfig& mmSettings, const GameManagerServerConfig* gameManagerConfigTdf, const GameManagerServerPreconfig& preConfig, ConfigureValidationErrors& validationErrors) const
    {
        ...
        for (; curScenario != endScenario; ++curScenario)
        {
            ...
            bool subsessionsUsePacker = false;
            ...
            for (; curSubSession != endSubSession; ++curSubSession)
            {
                ...
                {
                    // Packer Validation:
                    bool isPackerSubsession = isPackerSubsessionCheck(*curSubSession->second, gameManagerConfigTdf->getCreateGameTemplatesConfig());

                    // Validate that all subsessions have packer on or off (no mix-and-match)
                    if (curSubSession == scenarioConfig.getSubSessions().begin())
                    {
                        subsessionsUsePacker = isPackerSubsession;
                    }
                    else if (subsessionsUsePacker != isPackerSubsession)
                    {
                        StringBuilder strBuilder;
                        strBuilder << "[MatchmakingScenarioManager].validateConfig: Scenario: " << scenarioName.c_str() << ", SubSession: " << subSessionName.c_str() <<
                            " has packer enabled/disabled differently than an earlier subsession.  All subsessions must have packer on or off.";
                        validationErrors.getErrorMessages().push_back(strBuilder.get());
                    }
                    ...
                    PropertyNameList reqPropertyFilters = gameManagerConfigTdf->getProperties().getRequiredFilterPropertiesForPacker();
                    ...
                    for (auto& filterName : filterList)
                    {
                        ...
                        else
                        {
                            ...
                            // For Packer, check that all required property filters are set:  (here we remove elements from the temp set as we find filters that use them)
                            if (isPackerSubsession)
                            {
                                if (gameManagerConfigTdf->getProperties().getRequiredFilterPropertiesForPacker().findAsSet(filterItr->second->getGameProperty()))
                                {
                                    if (reqPropertyFilters.findAsSet(filterItr->second->getGameProperty()))
                                    {
                                        reqPropertyFilters.eraseAsSet(filterItr->second->getGameProperty());
                                    }
                                    else
                                    {
                                        // We don't allow you to have multiple filters mapped to the same (required) value:
                                        StringBuilder strBuilder;
                                        strBuilder << "[MatchmakingScenarioManager].validateConfig: Scenario: " << scenarioName.c_str() << ", SubSession: " << subSessionName.c_str() <<
                                            " is using Packer, but has Multiple required PropertyFilters mapped to GameProperty: " << filterItr->second->getGameProperty();
                                        validationErrors.getErrorMessages().push_back(strBuilder.get());
                                    }
                                }
                            }
                    ...
                    {
                        // Make sure that all of the required Filters exist for this subsession:
                        if (!reqPropertyFilters.empty())
                        {
                            StringBuilder strBuilder;
                            strBuilder << "[MatchmakingScenarioManager].validateConfig: Scenario: " << scenarioName.c_str() << ", SubSession: " << subSessionName.c_str() <<
                                " is using Packer, but does not have a required PropertyFilter mapped to GameProperty: ";
                            for (auto& missingPropertyFilter : reqPropertyFilters)
                                strBuilder << missingPropertyFilter << ", ";
                            validationErrors.getErrorMessages().push_back(strBuilder.get());
                        }

                        // Check that [FindGame] is not the current setting.   (The logic to default [] to [Find/Create] is in the subsessionconfg::configure())
                        if (curSubSession->second->getMatchmakingSettings().getSessionMode().getFindGame() && 
                            !curSubSession->second->getMatchmakingSettings().getSessionMode().getCreateGame())
                        {
                            StringBuilder strBuilder;
                            strBuilder << "[MatchmakingScenarioManager].validateConfig: Scenario: " << scenarioName.c_str() << ", SubSession: " << subSessionName.c_str() <<
    ...
    const char8_t* MatchmakingScenarioManager::getGlobalScenarioAttributeNameByTdfMemberName(const char8_t* memberName) const
{
    ...
    void MatchmakingScenarioManager::cancelAllLegacyScenarios()
    {
        // Make a temporary copy of the Scenario Map, so that we don't have to worry about potentially invalidated iterators:
        ScenarioByIdMap tempCopy(mScenarioByIdMap);
        ...
        {
            MatchmakingScenarioRef scenario = curIter->second;
            if (!scenario->getIsPackerScenario())
            {
                scenario->cancelSessions();
            }
        }
    ...
    ***************************************************************************************************/
    Blaze::BlazeRpcError MatchmakingScenarioManager::startMatchmaking(const StartMatchmakingRequest &request, StartMatchmakingInternalResponse &response, 
        ...
        const StartMatchmakingInternalRequest& subMasterRequest, bool hasBadReputation /*= false*/, PackerSessionInternalRequest* outPackerSessionReq)
    {
        auto overrideUserSessionInfo = scenarioSession.getOverrideUserSessionInfo();

        bool indirectMatchmakingAttempt =  (!ValidationUtils::isUserSessionInList(UserSession::getCurrentUserSessionId(), subMasterRequest.getUsersInfo()));

        // Invalid request.
        GameEntryType entryType = subMasterRequest.getRequest().getPlayerJoinData().getGameEntryType();
        ...
        if (!scenarioSession.getIsPackerSession())
        {
            ...
            errResult = ValidationUtils::validateRequestCrossplaySettings(isCrossplayEnabled, userJoinList, request.getSessionData().getSessionMode().getCreateGame(),
                ...
                platformRuleCriteria.getClientPlatformListOverride(), internalRequest.getOwnerUserSessionInfo().getUserInfo().getId(), platformRuleCriteria.getCrossplayMustMatch());
            if (errResult != ERR_OK)
            {
        ...
        {
            // Packer flow
            if (outPackerSessionReq != nullptr)
            {
                // Rather than sending the full request here, we add in the Subsession stuff and move on (no error)
                PackerSessionInternalRequest& packerSessionReq = *outPackerSessionReq;
                packerSessionReq.setPackerSessionId(sessionId);

                internalRequest.getRequest().getSessionData().copyInto(packerSessionReq.getMatchmakingSessionData());
                internalRequest.getRequest().getGameCreationData().copyInto(packerSessionReq.getGameCreationData());
                internalRequest.getGameExternalSessionData().copyInto(packerSessionReq.getGameExternalSessionData());
                internalRequest.getQosCriteria().copyInto(packerSessionReq.getQosCriteria());
                packerSessionReq.setSubSessionName(internalRequest.getScenarioInfo().getSubSessionName());
                packerSessionReq.setCreateGameTemplateName(internalRequest.getCreateGameTemplateName());
                internalRequest.getFindDedicatedServerRulesMap().copyInto(packerSessionReq.getFindDedicatedServerRulesMap());
                internalRequest.getMatchmakingFilters().copyInto(packerSessionReq.getMatchmakingFilters());            // Since this is just a list, we could pass it around better.
                internalRequest.getMatchmakingPropertyMap().copyInto(packerSessionReq.getMatchmakingPropertyMap());    // Properties are mostly global, but some are per-Subsession (CG template defined ones, mostly), so we set them here.
                internalRequest.getMatchmakingPropertyList().copyInto(packerSessionReq.getMatchmakingPropertyList());  // This defines the 'UNSET' stuff, based on various hackery
            }
            return ERR_OK;
        }
        else
    ...
    // This function sends off the request to the Master that includes all of the Scenario information without duplicates:
    BlazeRpcError MatchmakingScenarioManager::startPackerMatchmaking(const StartPackerScenarioRequest &packerReq, StartMatchmakingScenarioResponse& response, MatchmakingCriteriaError &error)
    {
        StartPackerSessionResponse packerResp;
        StartPackerSessionError packerErr;
        ...
        mComponent->incMMSessionReqForUser(currentUserSessionId);
        BlazeError err = gamepackerMaster->startPackerScenario(packerReq, packerResp, packerErr);
        if (err == ERR_OK)
        {
            // Similar code exists in MatchmakingScenarioSession::start(
            response.setEstimatedTimeToMatch(packerResp.getInternalResponse().getEstimatedTimeToMatch());
            response.setScid(packerResp.getInternalResponse().getStartMatchmakingResponse().getScid());
            response.setExternalSessionTemplateName(packerResp.getInternalResponse().getStartMatchmakingResponse().getExternalSessionTemplateName());
            response.setExternalSessionName(packerResp.getInternalResponse().getStartMatchmakingResponse().getExternalSessionName());
            response.setExternalSessionCorrelationId(packerResp.getInternalResponse().getStartMatchmakingResponse().getExternalSessionCorrelationId());
        }
        else
        {
            ERR_LOG("[MatchmakingScenarioManager].startMatchmaking: Failed to start packer Scenario for userSessionId(" << currentUserSessionId << "), err(" << err << ")");
            packerErr.getInternalError().copyInto(error);
        }
        mComponent->decMMSessionReqForUser(currentUserSessionId);
        ...
        if (err == Blaze::ERR_OK)
        {
            ...
            // Note:  We don't set the addMMSessionForUser for PackerSessions because that logic is only for QosValidation logic. 

            //for (auto& sessionData : packerReq.getSessionDataList())
            //{
            //    mComponent->addMMSessionForUser(currentUserSessionId, sessionData->getPackerSessionId());
            //}
        }


        return convertGamePackerErrorToGameManagerError(err.code);
    }
    ...
    ***************************************************************************************************/
            ...
            mScenarioData(BLAZE_NEW MatchmakingScenarioData())
    {
        ...
        mScenarioData->setMatchmakingSuccessGameId(INVALID_GAME_ID);
    ...
    {
        for (auto& curSession : mOwnedSessions)
        {
            delete curSession; // curSession is a pointer.
        }
    ...
    void MatchmakingScenario::sendNotifyRemoteMatchmakingEndedToGroup(GameManager::MatchmakingResult result)
    {
        ...
        sessionEndedNotification.setGameId(mScenarioData->getMatchmakingSuccessGameId());
    ...
    bool MatchmakingScenario::getIsPackerScenario() const
    {
        // Technically, we only need to check one (since we're config validating that they're all the same)
    ...
    BlazeRpcError MatchmakingScenario::setupAndStartSessions(StartMatchmakingScenarioResponse& response, MatchmakingCriteriaError& critErr, const InetAddress& callerNetworkAddress, UserIdentificationList& groupUserIds, CallerProperties& callerSourceProperties)
    {
        ...
        StartPackerScenarioRequest packerReq;

        // Fill in master request data:
        {
            // Common Data Types:
            mScenarioData->getScenarioRequest().getScenarioAttributes().copyInto(masterRequest.getScenarioInfo().getScenarioAttributes());
            ...
            // Scenario Info:
            masterRequest.getScenarioInfo().setOriginatingScenarioId(mScenarioData->getScenarioId());
            masterRequest.getScenarioInfo().setScenarioName(getConfigInfo().getScenarioName());
            masterRequest.getScenarioInfo().setScenarioVariant(getConfigInfo().mVariant);
            masterRequest.getScenarioInfo().setScenarioVersion(getConfigInfo().mVersion);
        ...
        if (errRet == ERR_OK)
        {
            ...
            mScenarioData->setHostBlazeId(masterRequest.getOwnerUserSessionInfo().getUserInfo().getId());
            ...
            for (; userJoinInfoIter != userJoinInfoEnd; ++userJoinInfoIter)
            {
                ...
                (*userJoinInfoIter)->getScenarioInfoPerUser().setScenarioName(getConfigInfo().getScenarioName());
                (*userJoinInfoIter)->getScenarioInfoPerUser().setScenarioVariant(getConfigInfo().mVariant);
                (*userJoinInfoIter)->getScenarioInfoPerUser().setScenarioVersion(getConfigInfo().mVersion);
                mScenarioData->getScenarioRequest().getScenarioAttributes().copyInto((*userJoinInfoIter)->getScenarioInfoPerUser().getScenarioAttributes());
            }
            ...
            for (uint32_t i = 0, numSessions = (uint32_t)mOwnedSessions.size(); i < numSessions; ++i)
            {
                ...
                PackerSessionInternalRequest* outPackerSessionReq = nullptr;
                ...
                { 
                    outPackerSessionReq = packerReq.getPackerSessionRequestList().pull_back();
                }
                curSession->setSessionId(sessionId);
                
                BlazeRpcError err = curSession->start(response, critErr, callerNetworkAddress, masterRequest, hasBadReputation, outPackerSessionReq, callerSourceProperties);    // Blocking
                if (err != ERR_OK)
                {
                    errRet = err;
                    break;
        ...
        if (getIsPackerScenario())
        {
            // if this was a PackerScenario, then we haven't actually sent a request off yet, and we need to use the masterRequest and send off the 
            packerReq.setPackerScenarioId(getScenarioId());

            GameManager::PackerScenarioInternalRequest& packerScenarioReq = packerReq.getPackerScenarioRequest();

            masterRequest.getRequest().getPlayerJoinData().copyInto(packerScenarioReq.getPlayerJoinData());
            masterRequest.getRequest().getCommonGameData().copyInto(packerScenarioReq.getCommonGameData());
            masterRequest.getUsersInfo().copyInto(packerScenarioReq.getUsersInfo());
            masterRequest.getOwnerUserSessionInfo().copyInto(packerScenarioReq.getOwnerUserSessionInfo());
            masterRequest.getUserSessionIpInformation().copyInto(packerScenarioReq.getUserSessionIpInformation());
            masterRequest.getXblAccountIdBlockList().copyInto(packerScenarioReq.getXblAccountIdBlockList());

            packerScenarioReq.setTrackingTag(masterRequest.getTrackingTag());
            packerScenarioReq.setTotalUsersOnline(masterRequest.getTotalUsersOnline());
            packerScenarioReq.setTotalUsersInGame(masterRequest.getTotalUsersInGame());
            packerScenarioReq.setTotalUsersInMatchmaking(masterRequest.getTotalUsersInMatchmaking());
            
            masterRequest.getScenarioInfo().copyInto(packerScenarioReq.getScenarioInfo());
            packerScenarioReq.setScenarioTimeoutDuration(masterRequest.getScenarioTimeoutDuration());

            const auto* scenariosConfig = mManager.getScenariosConfig();
            packerScenarioReq.setGlobalMatchEstimateFalloffWindow(scenariosConfig->getGlobalMatchEstimateFalloffWindow());
            auto scenariosConfigItr = scenariosConfig->getScenarios().find(getConfigInfo().getScenarioName());
            EA_ASSERT(scenariosConfigItr != scenariosConfig->getScenarios().end());
            packerScenarioReq.setScenarioMatchEstimateFalloffWindow(scenariosConfigItr->second->getMatchEstimateFalloffWindow());

            errRet = mManager.startPackerMatchmaking(packerReq, response, critErr);
        }
...
cancelSessions:
    ...
    {
        // Finished is ONLY used to track the last MatchmakingSuccessResult and GameId (for group users) 
        // Success is handed via NotifyMatchmakingSessionConnectionValidated, failure via onNotifyMatchmakingFailed.
        ...
        case SUCCESS_PSEUDO_FIND_GAME:
            ...
            mScenarioData->setMatchmakingSuccessGameId(notifyMatchmakingFinished.getMatchmakingGameId());
            break;
        default:
            break;
        }
    ...
    void MatchmakingScenario::onNotifyMatchmakingSessionConnectionValidated(const NotifyMatchmakingSessionConnectionValidated& data)
    {
        ...
        if (data.getDispatchSessionFinished())
        {
            ...
            mScenarioData->setMatchmakingSuccessGameId(mSuccessResults.getGameId());
    ...
    void MatchmakingScenario::onSessionFinished(bool success, MatchmakingSessionId sessionId)
    {
        ...
        {
            // This is the code block that hits when we have received one failure notification, but other sessions are still in progress.  (Not an error, no additional logging) 

            // Or if there's some random unknown session id:
            if (!foundSession)
            {
                WARN_LOG("[MatchmakingScenario].onSessionFinished: scenario(" << mScenarioData->getScenarioId() << ") recieved notification that unknown SessionId (" << sessionId << ") has finished.   Find out where this id came from!");
            }
        }
                                                    ...
                                                    StartMatchmakingInternalRequest& masterRequest, bool hasBadReputation, PackerSessionInternalRequest* outPackerSessionReq, CallerProperties& callerSourceProperties)
    {
        ...
        err = mScenarioOwner.mManager.startMatchmaking(request, startMMResponse, critErr, callerNetworkAddress, 
                                                       *this, masterRequest, hasBadReputation, outPackerSessionReq);    // Blocking
        if (err != ERR_OK)
        {
            ERR_LOG("[MatchmakingScenarioSession].start: Error ("<< critErr.getErrMessage() <<") occurred with matchmaking scenario.");
            mSessionData->setSessionState(MatchmakingScenarioSessionData::FINISHED);
    ...
    BlazeRpcError MatchmakingScenario::cancelSessions()
    {
        ...
        // Spawn a helper fiber because we can't sit around waiting for all the cancel commands to complete on the same fiber. 
    ...
    BlazeRpcError MatchmakingScenario::cancelSessionsFiber(MatchmakingScenarioRef scenarioRef)
    {
        ...
        if (getIsPackerScenario())
        {
            retErr = cancelPackerScenarioInternal();        // Packer does a single cancel command by Scenario.  (Note:  This does not change the Sessions' state to Cancelled.)
        }
        else
        {
            for (auto& curSession : mOwnedSessions)
            {
                ...
                BlazeRpcError err = curSession->cancel();
                if (err != ERR_OK)
                    retErr = err;
            }
        }
    ...
    BlazeRpcError MatchmakingScenario::cancelPackerScenarioInternal()
    {
        BlazeRpcError err = ERR_OK;
        if (!getIsPackerScenario())
        {
            ERR_LOG("[MatchmakingScenarioManager].cancelPackerScenarioInternal: Attempting to cancel PackerScenario for a non-Packer Scenario.");
            return ERR_SYSTEM;
        }

        auto* gamepackerMaster = getManager().getGamePackerMaster();
        if (gamepackerMaster == nullptr)
        {
            ERR_LOG("[MatchmakingScenarioManager].getGamePackerMaster: Unable to resolve gamepacker master.");
            return ERR_SYSTEM;
        ...
        CancelPackerScenarioRequest packerReq;
        packerReq.setPackerScenarioId(getScenarioId());

        err = gamepackerMaster->cancelPackerScenario(packerReq); 
        if (err == ERR_OK || err == GAMEPACKER_ERR_SESSION_NOT_FOUND || err == GAMEPACKER_ERR_SCENARIO_FINALIZING)
        {
            // Technically, we could be completing matchmaking at the time of the cancel (hence GAMEPACKER_ERR_SESSION_NOT_FOUND / GAMEPACKER_ERR_SCENARIO_FINALIZING)
            for (auto& curSession : mOwnedSessions)
                curSession->finish();
        }
        else
    ...
    {
        return mConfigInfo.getIsPackerSubsession();
    }
    ...
    bool MatchmakingScenarioConfigInfo::getIsPackerScenario() const
    {
        // Technically, we only need to check one (since we're config validating that they're all the same)
        for (auto& variant : mScenarioVariantsMap)
        {
            if (variant.second->getIsPackerVariant())
                return true;
        }
        return false;
    }
    ...
    bool MatchmakingScenarioVariantConfigInfo::getIsPackerVariant() const
    {
        // Technically, we only need to check one (since we're config validating that they're all the same)
        for (auto& variant : mScenarioSessionConfigList)
        {
            if (variant->getIsPackerSubsession())
                return true;
        }
        return false;
    }
    ...
    bool MatchmakingSubSessionConfigInfo::configure(const ScenariosConfig& config)
    {
        ...
        mIsPackerSubsession = isPackerSubsessionCheck(*mSubSessionConfig, mOwner->mScenarioManager->getComponent().getConfig().getCreateGameTemplatesConfig());
        if (mIsPackerSubsession)
        {
            // Previously in onValidateConfig, we set a default for the SessionMode if none is provided. 
            if (mRequest.getSessionData().getSessionMode().getBits() == 0)
            {
                mRequest.getSessionData().getSessionMode().setCreateGame();
                mRequest.getSessionData().getSessionMode().setFindGame();
            }
        }
    ...
    void MatchmakingSubSessionConfigInfo::getScenarioPropertiesUsed(PropertyNameList& outProperties) const
    {
        ...
        if (!mSubSessionConfig->getCreateGameTemplateAsTdfString().empty())
        {
            ...
            {
                ERR_LOG("[MatchmakingSubSessionConfigInfo].addGlobalAndScenarioRules: Unable to find cg template named " << mSubSessionConfig->getCreateGameTemplate()
                    << " for Scenario " << getScenarioName() << " subsession " << mSubSessionName.c_str() << ".");
...
//            }
        ...
        StartMatchmakingInternalRequest& masterRequest, MatchmakingCriteriaError &outErr) const
    {
        ...
        if (mSubSessionConfig && mOverallConfig)
        {
            ...
            // We update the subsession's name: 
            masterRequest.getScenarioInfo().setSubSessionName(mSubSessionName);
