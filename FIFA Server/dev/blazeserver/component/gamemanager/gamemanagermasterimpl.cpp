...
namespace GameManager
{
    ...
    ***************************************************************************************************/
    ...
    void GameManagerMasterImpl::onExportStorageRecord(StorageRecordId recordId)
    {
        ...
        // NOTE: No events are to be signaled during this removal, since it merely corresponds to the game
    ...
    ***************************************************************************************************/
        ...
        const UserSessionSetupReasonMap &setupReasons, GameSessionMasterPtr& dedicatedServer, JoinGameMasterResponse& joinResponse)
    {
        ...
        TeamIndex teamIndex = getTeamIndex(createGameRequest.getCreateRequest().getPlayerJoinData(), hostUserSessionInfo->getUserInfo());
        updateExternalSessionJoinParams(joinResponse.getExternalSessionParams(), *hostUserSessionInfo,
            (createGameRequest.getCreateRequest().getPlayerJoinData().getGameEntryType() == GAME_ENTRY_TYPE_MAKE_RESERVATION), 
            teamIndex, *availableGame);
    ...
    ***************************************************************************************************/
    ...
    // These are literally the same as the normal MM CG requests (currently), we just have them separated out for metrics purposes. 
    PackerCreateGameError::Error GameManagerMasterImpl::processPackerCreateGame(const Blaze::GameManager::PackerCreateGameRequest &request, Blaze::GameManager::CreateGameMasterResponse &response, const Message* message)
    {
        BlazeRpcError err = (BlazeRpcError)processMatchmakingCreateGame(request, response, message);
        return PackerCreateGameError::commandErrorFromBlazeError(err);
    }
    PackerCreateGameWithPrivilegedIdError::Error GameManagerMasterImpl::processPackerCreateGameWithPrivilegedId(const Blaze::GameManager::PackerCreateGameRequest &request, Blaze::GameManager::CreateGameMasterResponse &response, const Message* message)
    {
        BlazeRpcError err = (BlazeRpcError)processMatchmakingCreateGameWithPrivilegedId(request, response, message);
        return PackerCreateGameWithPrivilegedIdError::commandErrorFromBlazeError(err);
    }
        ...
        for (const auto& aItr : externalSessionMembers) //iterate externalIds in external session by platform
        {
            ...
            if (eItr == gameRosterMembers.end()) // no player set for this platform found
            {
                for (const auto& uItr : aItr.second) // take all the external Ids and add them for removal.
                    updateExternalSessionLeaveParams(unexpectedExternalSessionMembers, uItr, *game, aItr.first);
            }
            else
            {
