...
namespace GameManager
{
    ...
    bool PropertyManager::validateConfig(const PropertyConfig& config, ConfigureValidationErrors& validationErrors) const
    {
        ...
        TdfIdHashSet acceptableTdfs = { ReplicatedGameDataServer::TDF_ID,
        ...
        // Validate that the TDF members are mapped correctly here:
        ...
        // Validation function should be updated to ensure that the Properties are known, but outside of the PropertyManager we can't ensure that the Properties are all known yet, 
        // so we'd either have to change the activation order to ensure that the PropertyManager is created first, or lookup the properties directly from the config. 
        {
            StringBuilder strBuilder2;
            strBuilder2 << errorStringPrefix << " - getGameBrowserPackerProperties:";

            AttributeToTypeDescMap attrMap;
            TdfIdHashSet tdfIdSet = { PackerActiveGameData::TDF_ID };
            if (validateTemplateTdfAttributeMapping(config.getGameBrowserPackerProperties(), attrMap, validationErrors, strBuilder2.c_str(), tdfIdSet))
            {
                if (!attrMap.empty())
                {
                    StringBuilder strBuilder;
                    strBuilder << errorStringPrefix << "Attribute Names cannot be used in gameBrowserPackerProperties, only properties.";
                    validationErrors.getErrorMessages().push_back(strBuilder.get());
                }
                
                if (!config.getGameBrowserPackerProperties().empty())
                {
                    for (const auto& curAttrDesc : *config.getGameBrowserPackerProperties().begin()->second)
                    {
                        const auto& curPropName = curAttrDesc.second->getPropertyName();
                        if (mappedProperties.find(curPropName) == mappedProperties.end())
                        {
                            StringBuilder strBuilder;
        ...
        for (const auto& curPropName : config.getPackerOutputProperties())
        {
            ...
            if (gameCreationDataMapping != config.getTdfDataSources().end())
            {
                ...
                if (tdfMemberName != gameCreationDataMapping->second->end())
                {
                    ...
                    mGqfPropertyCgrReqMapping[curPropName.c_str()] = createGameReqValue.c_str();
                }
            }
        }

        // Cache the activeGame Properties in their own list.  (Rather than relying on a helper function to gather these each time.)
        if (!config.getGameBrowserPackerProperties().empty())
        {
            mGameBrowserPackerPropertiesList.reserve(config.getGameBrowserPackerProperties().size());
            for (const auto& curAttrDesc : *config.getGameBrowserPackerProperties().begin()->second)
            {
                mGameBrowserPackerPropertiesList.push_back(curAttrDesc.second->getPropertyName());
            }
        }
    ...
    bool PropertyManager::onReconfigure(const PropertyConfig& config)
    {
        // clean up old configs
        mCgRequestPropertyNameMapping.clear();
        mGqfPropertyCgrReqMapping.clear();
        mGqfPropertyCgrReqMapping.clear();

        // just run the standard config (currently always returns true. but we don't return the value in case that changes)
        onConfigure(config);
        return true;
    }
    ...
    bool PropertyManager::validateFilters(const PropertyConfig& config, const MatchmakingFilterMap& filters, ConfigureValidationErrors& validationErrors) const
    {
        ...
        TdfIdHashSet acceptableTdfs = { IntRangeFilter::TDF_ID,
                                        IntSetFilter::TDF_ID,
...
#undef ADD_DERIVED_FUNCTION
    ...
    bool PropertyManager::convertPropertiesToPackerActiveGameData(EA::TDF::TdfPtr& outActiveGameData, const PropertyNameMap& activeGameProperties) const
    {
        TemplateAttributes temp;
        const char8_t* missingAttr = "";
        BlazeRpcError blazeErr = applyTemplateTdfAttributes(outActiveGameData, mPropertyConfig.getGameBrowserPackerProperties(), temp, activeGameProperties, missingAttr, "PropertyManager::ActiveGameData");
        if (blazeErr != ERR_OK)
        {
            ERR_LOG("[PropertyManager].convertPropertiesToPackerActiveGameData: Properties unable to be applied to the PackerActiveGameData. Missing (" << missingAttr << ").");
            return false;
        }
    ...
    eastl::string PropertyManager::getMappedCgrAttribute(const char8_t* gameProperty)
    {
        eastl::string gamePropertyAttributes(eastl::string::CtorSprintf(), "%s%s", GameManager::GAME_PROPERTY_PREFIX, GameManager::PROPERTY_NAME_GAME_ATTRIBUTES);
        const auto itr = mGqfPropertyCgrReqMapping.find(gamePropertyAttributes.c_str());
        if (itr != mGqfPropertyCgrReqMapping.end())
        {
            eastl::string gqfGameProperty(gameProperty);
            auto pos = gqfGameProperty.find(gamePropertyAttributes.c_str());

            if (pos != eastl::string::npos)
            {
                gqfGameProperty.erase(0, gamePropertyAttributes.size());
                eastl::string cgrAttribute(eastl::string::CtorSprintf(), "%s%s", itr->second.c_str(), gqfGameProperty.c_str());
                return cgrAttribute;
            }
        }
        return eastl::string("");
