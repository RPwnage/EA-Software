...
void PsnMatchUtil::addMatchStatFormat(const char8_t* statKey, const char8_t* statFormat)
{
    PsnMatchUtil::MatchStatFormatMap::insert_return_type inserted = mMatchStatFormatMap.insert(statKey);
    if (inserted.second)
        inserted.first->second = statFormat;
}
    ...
    if (competitionType == PSNServices::Matches::PsnCompetitionTypeEnum::COMPETITIVE && groupingType == PSNServices::Matches::PsnGroupingTypeEnum::TEAM_MATCH)
    {
        ...
        for (const auto& team : mTeamList)
        {
            ...
            for (const auto& teamStat : team->statMap)
            {
                ...
                matchStat->setStatsValue(eastl::string(eastl::string::CtorSprintf(), mMatchStatFormatMap[teamStat.first].c_str(), teamStat.second).c_str());
            }
            ...
            for (const auto& teamMember : team->memberList)
            {
                ...
                for (const auto& teamMemberStat : teamMember->statMap)
                {
                    ...
                    matchStat->setStatsValue(eastl::string(eastl::string::CtorSprintf(), mMatchStatFormatMap[teamMemberStat.first].c_str(), teamMemberStat.second).c_str());
                }
            }
        }
    }
    else
    {
        ...
        for (const auto& player : mPlayerList)
        {
            ...
            for (const auto& playerStat : player->statMap)
            {
                ...
                matchStat->setStatsValue(eastl::string(eastl::string::CtorSprintf(), mMatchStatFormatMap[playerStat.first].c_str(), playerStat.second).c_str());
            }
        }
    }
