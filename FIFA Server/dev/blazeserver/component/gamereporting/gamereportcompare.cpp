...
void GameReportCompare::visit(EA::TDF::Tdf &rootTdf, EA::TDF::Tdf &parentTdf, uint32_t tag, EA::TDF::TdfVectorBase &value, const EA::TDF::TdfVectorBase &referenceValue)
{
    ...
    if (!mDifferenceFound)
    {
        ...
        else if (mStateStack.top().config->getEnableTdfVariableComparison() || (value.getValueType() != EA::TDF::TDF_ACTUAL_TYPE_TDF && value.getValueType() != EA::TDF::TDF_ACTUAL_TYPE_VARIABLE))
        {
