...
CoreFileWriter::CoreFileWriter(const char *name, uint32_t coreBufferSize)
    : BlazeThread(name, EXCEPTION_HANDLER, 4096*1024), mQueue("CoreFileWriter"), mLastContext(nullptr), mMainLogFd(-1)
{
    mShutdown = false;
...
void CoreFileWriter::outputLog(Logging::Level level, const StringBuilder& logMessage)
{
    // Here is some unfortunate but necessary evil because of how fork() works on Linux (https://man7.org/linux/man-pages/man2/fork.2.html)
    ...
    //      in the child refers to the same open file description (see open(2)) as the corresponding
    //      file descriptor in the parent.
    //
    // Because of the overall iffy'ness of forking a multithreaded program, and the fact that the Logger
    // actually runs on it's own dedicated thread, any attempt to use the good old BLAZE_INFO_LOG() type
    // macros will result in absolutely nothing being written to any log.
    // 
    // So instead, once we've forked, we need to write any new logs directly to the file-descriptors.  And we
    // need to do it all using fresh memory and stack allocated variables so reduce the risk of triggering another crash.
#ifdef GOOGLE_COREDUMPER_LIB
    uint32_t year, month, day, hour, min, sec, msec;
    TimeValue::getGmTimeComponents(TimeValue::getTimeOfDay(), &year, &month, &day, &hour, &min, &sec, &msec);
    eastl::string logEntry(eastl::string::CtorSprintf(),
        "%d/%02d/%02d-%02d:%02d:%02d.%03dZ %-6.6s <corewriter> system           [0000000000000000/0000000000000000/00000000000000000000] %s\n",
        year, month, day, hour, min, sec, msec, Logging::LevelToString(level), logMessage.c_str());

    if (Logger::isLogToStdOutEnabled())
    {
        write(1, logEntry.c_str(), logEntry.size());
    }
    if (Logger::isLogToFileEnabled())
    {
        write(mMainLogFd, logEntry.c_str(), logEntry.size());
    }
#endif
}
...
void CoreFileWriter::dumpCoreFile(const char8_t* fileNamePrefix, const char8_t* fileNameDetail, const char8_t* instanceName, uint64_t coreSizeLimit, bool alwaysSignalMonitor)
{
    ...
    mMainLogFd = Logger::getMainLogFd();
    ...
    if (coreFileHandle < 0)
    {
        outputLog(Logging::WARN, StringBuilder() << "[CoreFileWriter].dumpCoreFile() : Failed to retrieve core dump (errno=" << errno << ")");
    ...
    outputLog(Logging::INFO, StringBuilder() << "[CoreFileWriter].dumpCoreFile() : Writing core file " << coreFileName.c_str() << ", compression=" << compressor->compressor);
    ...
    if (coreFileHandle >= 0)
    {
        ...
        if (outfile < 0)
        {
            outputLog(Logging::WARN, StringBuilder() << "[CoreFileWriter].dumpCoreFile() : Failed to open file '" << coreFileName.c_str() << "' for core dump (errno=" << errno << ")");
        }
        else
        {
            ...
            {   
                ...
                {                
                    ...
                    {
                        outputLog(Logging::WARN, StringBuilder() << "[CoreFileWriter].dumpCoreFile() : Failed to write full core dump information (size limit reached " << coreSizeLimit);
                        break;
                    }
                    if (write(outfile, mCoreBuffer, coresize) < 0)
                    {
                        outputLog(Logging::WARN, StringBuilder() << "[CoreFileWriter].dumpCoreFile() : Failed to write core dump information");
                        break;
                    }
                ...
                if (coresize < 0)
                {
                    outputLog(Logging::WARN, StringBuilder() << "[CoreFileWriter].dumpCoreFile() : Failed to read core dump information");
                }
                else
                {
                    signalMonitor = true;
            ...
            close(outfile);
            outputLog(Logging::INFO, StringBuilder() << "[CoreFileWriter].dumpCoreFile() : Finished writing " << coreSizeTotal << " bytes of core dump information");
        }
        close(coreFileHandle);
    }
