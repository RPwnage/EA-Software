...
public:
    ...
    void processRequestOnFiber(const google::protobuf::Message* protobufMessage, bool serializeRequests);
    ...
    void logRpcPreamble(const EA::TDF::Tdf* request);
    void logRpcPostamble(const EA::TDF::Tdf* response, const grpc::Status& status);

    bool sendResponse(const google::protobuf::Message* response)
    {
        //logging handled via CommandInfo::sendGrpcResponse because response object here is NOT a tdf (and we need a tdf to log).
    ...
    bool isDoneTagReceived() const { return mDoneTagReceived; }
        ...
        if (asyncOpEnded(ASYNC_OP_TYPE_READ))
        {
            if (ok)
            {
                ...
                if (InboundRpcBase::mHandler != nullptr)
                {
                    ...
                    const EA::TDF::Tdf* requestTdf = getCommandInfo().requestProtobufToTdf ? getCommandInfo().requestProtobufToTdf(&mRequest) : nullptr;
                    logRpcPreamble(requestTdf);

                    InboundRpcBase::processRequestOnFiber(&mRequest, false);
                }
                else
                {
            ...
            else
            {
                ...
                InboundRpcBase::processRequestOnFiber(nullptr, false);
            }
        }
    }
