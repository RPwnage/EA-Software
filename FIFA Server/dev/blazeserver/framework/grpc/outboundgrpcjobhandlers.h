...
public:
    ...
    ***************************************************************************************************/
    ...
    inline void Release()
    {
        ...
        else if (mRefCount == 1 && mOwner != nullptr && mPendingAsyncOpCounter == 0)
        {
            // The mOwner holds a ref to the RPC, but doesn't know if the RPC is still running. 
            // This leads to a leak if the RPC doesn't complete successfully before the caller gives up on it, since we only remove the mOwner's RefPtr when the RPC completes (in asyncOpStarted(ASYNC_OP_TYPE_FINISH);).
            // The problem is, we have to keep the RPC alive as long as gRPC is using it (because it bound callbacks), but it's unclear WHEN gRPC will give up the data, and killing the callbacks 'early' results in a crash.
            // So we can't just kill the RPC as soon as the RPC is dead, we have to try to wait until all the async operations are done.
            shutdown();
        }
    }
