...
public:
    ...
    bool changeRequestHandlerSessionKey(const char8_t* newSessionKey, const char8_t* tempSessionKey);
    ...
    Blaze::AlertInfo& getAllowedIpAddressAlertInfo() { return mAllowedIpAddressAlertInfo; }

    bool isAllowedIpListEmpty() const
    {
        if (mAllowedIpNetworks == nullptr)
            return true;

        return (mAllowedIpNetworks->getCount() == 0);
    }

    // Note: returns true if the endpoint's allowedIpList config is empty.
    bool isAllowedIpAddress(uint32_t addr) const
    {
        if (mAllowedIpNetworks == nullptr)
            return true;
        
        return mAllowedIpNetworks->match(addr);
    }
    bool isAllowedIpAddress(const InetAddress& addr) const
    {
        return isAllowedIpAddress(addr.getIp());
    }

    bool isTrustedClientIpListEmpty() const
    {
        if (mTrustedClientIpNetworks == nullptr)
            return true;

        return (mTrustedClientIpNetworks->getCount() == 0);
    }

    // Note: returns false if the endpoint's trustedClientIpList config is empty.
    bool isTrustedClientIpAddress(uint32_t addr) const
    {
        if (mTrustedClientIpNetworks == nullptr)
            return false;

        return mTrustedClientIpNetworks->match(addr);
    }
    bool isTrustedClientIpAddress(const InetAddress& addr) const
    {
        return isTrustedClientIpAddress(addr.getIp());
    }

    // Note: returns false if the endpoint's hasInternalNicAccess config is empty.
    ...
    bool hasInternalNicAccess(const InetAddress& addr) const override
    {
    ...
    const char8_t* getEnvoyResourceName() const override { return mConfig.getEnvoyResourceName(); }
    bool getEnvoyAccessEnabled() const override;
...
private:
    ...
    eastl::unique_ptr<InetFilter> mAllowedIpNetworks;
    eastl::unique_ptr<InetFilter> mInternalNicAccessNetworks;
    Blaze::AlertInfo mAllowedIpAddressAlertInfo;
    ...
    eastl::unique_ptr<InetFilter> mTrustedClientIpNetworks;
