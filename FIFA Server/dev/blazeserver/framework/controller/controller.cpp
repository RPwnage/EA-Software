...
void Controller::doOnShutdownBroadcasted(ShutdownRequestPtr data)
{
    ...
    {
        InstanceIdList::const_iterator i =  data->getInstanceIds().begin();
        InstanceIdList::const_iterator e =  data->getInstanceIds().end();
        for (; i != e; ++i)
        {
            if (getInstanceId() == *i)
            {
...
const char8_t* Controller::resolveServiceName(const char8_t* serviceName) const
{
    if (serviceName != nullptr && serviceName[0] != '\0')
    {
        if (isServiceHosted(serviceName))
        {
            return serviceName;
        }
    }

    const char8_t* defaultServiceName = nullptr;

    if (isSharedCluster())
    {
        ClientPlatformType platform = getPlatformFromServiceNameString(serviceName); //Parse the platform from service name
        defaultServiceName = getDefaultServiceName(platform);    
        
        if (defaultServiceName != nullptr && defaultServiceName[0] != '\0')
        {
            BLAZE_TRACE_LOG(Log::CONTROLLER, "[Controller].resolveServiceName: Service name (" << serviceName << ") is not hosted by this instance. "
                "Falling back to default service name (" << defaultServiceName << ") for requested platfrom (" << ClientPlatformTypeToString(platform) << ").")
              
            return defaultServiceName;
        }
    }

    defaultServiceName = getDefaultServiceName(); //will never return nullptr, unless there is no default service name, which is a boot failure.
    
    if (defaultServiceName != nullptr && defaultServiceName[0] != '\0')
    {
        BLAZE_TRACE_LOG(Log::CONTROLLER, "[Controller].resolveServiceName: Service name (" << serviceName << ") is not hosted by this instance. "
            "Falling back to default service name (" << defaultServiceName << ").")
    }
...
{
    eastl::set<eastl::string> titleSet;
    eastl::set<eastl::string> versionSet;
    ...
    {
        eastl::string titles("");
        for (const auto& title : titleSet)
            titles += ", \"" + title + "\"";
        eastl::string message(eastl::string::CtorSprintf(), "[Controller].validateServiceNameStandard: Service Name <title> %s are inconsistent among hosted platforms. Same <title> should be used for all platforms.", titles.substr(2).c_str());
        validationErrors.getErrorMessages().push_back(message.c_str());
    }
    if (versionSet.size() > 1)
    {
        eastl::string versions("");
        for (const auto& version : versionSet)
            versions += ", \"" + version + "\"";
        eastl::string message(eastl::string::CtorSprintf(), "[Controller].validateServiceNameStandard: Service Name <version> %s are inconsistent among hosted platforms. Same <version> should be used for all platforms.", versions.substr(2).c_str());
        validationErrors.getErrorMessages().push_back(message.c_str());
    }
}

ClientPlatformType Controller::getPlatformFromServiceNameString(const char8_t* serviceName) const
{
    const eastl::string serviceNameString(serviceName);
    eastl::vector<eastl::string> serviceNameParts;
    blaze_stltokenize(serviceNameString, "-", serviceNameParts, true);  //Split service name by '-' with empty substrings allowed

    if (serviceNameParts.size() >= 3)
    {
        ClientPlatformType platform = INVALID;
        if (ParseClientPlatformType(serviceNameParts[2].c_str(), platform))
        {
            return platform;
        }
    }
