...
public:
    ...
    // In shared platform scenario, fall back to the default service(using client platform) if service name is not valid. 
    // In single platform scenario, fall back to the default service if service name is not valid.
    const char8_t* resolveServiceName(const char8_t* requestedServiceName) const;
...
private:
    ...
    ClientPlatformType getPlatformFromServiceNameString(const char8_t* serviceName) const;
