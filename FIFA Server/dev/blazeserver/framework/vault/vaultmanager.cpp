    ...
    switch (mVaultConfig.getAuthType())
    {
        ...
        case VAULT_AUTH_APPROLE:
        {
            if (params != nullptr)
            {
                ...
                const char* secretIdPath = approleParams.getSecretIdPath();
                char8_t diskSecret[256];
                if (strlen(secretIdPath) > 0)
                {
                    FILE* secretIdFile = fopen(secretIdPath, "r");
                    if (secretIdFile == nullptr)
                    {
                        BLAZE_ERR_LOG(Log::VAULT, "[VaultManager].login: Could not open secret id file " << secretIdPath);
                        error = ERR_SYSTEM;
                        break;
                    }

                    if (fgets(diskSecret, sizeof(diskSecret), secretIdFile) != diskSecret)
                    {
                        fclose(secretIdFile);
                        BLAZE_ERR_LOG(Log::VAULT, "[VaultManager].login: Could not read from secret id file " << secretIdPath);
                        error = ERR_SYSTEM;
                        break;
                    }

                    BLAZE_TRACE_LOG(Log::VAULT, "[VaultManager].login: Read in secret id from secret id file " << secretIdPath);
                    secretId = diskSecret;
