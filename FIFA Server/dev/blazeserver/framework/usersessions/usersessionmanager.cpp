...
{
    return 27; // Matches largest 00X_usersessions.sql version
}
...
bool UserSessionManager::configureCommon(const UserSessionsConfig& config, bool reconfigure)
{
    ...
    // Basically, if we're doing something that needs to hit each 'platform' we want to treat the Gen4/5 as the same:
    {
        eastl::map<eastl::string, ClientPlatformType> dbsByPlatform;
        for (auto& curDbName : config.getDbNamesByPlatform())
        {
            // Override the entry each time.   This means that the last entry will be the one used (so ps5 not ps4)
            if (gController->isPlatformHosted(curDbName.first))                 // Only use entries that are actually hosted.
                dbsByPlatform[curDbName.second.c_str()] = curDbName.first;
        }

        for (auto& curPlatform : dbsByPlatform)
        {
            mUserInfoDbPlatformSet.insert(curPlatform.second);
        }
    }
...
BlazeRpcError UserSessionManager::getRemoteUserExtendedData(UserSessionId remoteSessionId, UserSessionExtendedData& extendedData, UserSessionId subscriberSessionId)
{
    if (getMaster() != nullptr)
    {
        GetUserExtendedDataRequest req;
        req.setUserSessionId(remoteSessionId);
        req.setSubscriberSessionId(subscriberSessionId);
...
const ClientPlatformSet& UserSessionManager::getUserInfoDbPlatforms() const
{
    // Basically, if we're doing something that needs to hit each 'platform' we want to treat the Gen4/5 as the same:
    return mUserInfoDbPlatformSet;
}
...
void UserSessionManager::onUserSubscriptionsUpdated(const NotifyUpdateUserSubscriptions& data, UserSession*)
{
    ...
    {
    case ACTION_ADD:
        targetUserSession->addUsersAsync(data.getBlazeIds(), data.getIncludeSessionSubscription());
        break;
    case ACTION_REMOVE:
        targetUserSession->removeUsersAsync(data.getBlazeIds(), data.getIncludeSessionSubscription());
        break;
    }
}
...
BlazeRpcError UserSessionManager::lookupUserInfoByBlazeIds(const BlazeIdVector& blazeIds, BlazeIdToUserInfoMap& results, bool ignoreStatus, bool isRepLagAcceptable, ClientPlatformType clientPlatformType /*= INVALID*/)
{
    ...
    {
        BLAZE_ERR_LOG(Log::USER, "[UserSessionManager].lookupUserInfoByBlazeIds: lookup request size of " << blazeIds.size() << " exceeds max cache size of " << getConfig().getUserCacheMax() << ".  Increase max cache size.");
        return ERR_SYSTEM;
    }
    ...
    for (BlazeIdVector::const_iterator it = blazeIds.begin(), end = blazeIds.end(); it != end; ++it)
    {
        ...
        if (userInfo != nullptr)
        {
            // clientPlatformType is only used by the createNewUserSession call, so this shouldn't affect normal lookup operations:
            if (clientPlatformType != INVALID && clientPlatformType != userInfo->getPlatformInfo().getClientPlatform())
            {
                BLAZE_ERR_LOG(Log::USER, "[UserSessionManager].lookupUserInfoByBlazeIds: Resident user info includes data for user ("<< blazeId <<") on the wrong platform ("<< userInfo->getPlatformInfo().getClientPlatform() <<").  This prevents a new session from being created.");
                return ERR_SYSTEM;
            }
    ...
    if (!remainingIds.empty())
    {
        ...
        err = UserInfoDbCalls::lookupUsersByIds(remainingIds, userInfoList, metricsCountMap, !ignoreStatus, isRepLagAcceptable);
        if (err == ERR_OK)
        {
