...
public:
    ...
    bool isTrustedAddressesEmpty() const
    {
        if (mTrustedNetworks == nullptr)
            return true;

        return (mTrustedNetworks->getCount() == 0);
    }

    // Note: returns true if the endpoint's trust config is empty.
    ...
    const char8_t* getEnvoyResourceName() const override { return ""; }
