...
#include "arson/rpc/arsonslave/destroygame_stub.h"
...
namespace Arson
{
class DestroyGameCommand : public DestroyGameCommandStub
{
public:
    DestroyGameCommand(
        Message* message, Blaze::Arson::DestroyGameRequest* request, ArsonSlaveImpl* componentImpl)
        : DestroyGameCommandStub(message, request),
        mComponent(componentImpl)
    {
    }

    ~DestroyGameCommand() override { }

private:
    // Not owned memory
    ArsonSlaveImpl* mComponent;

    DestroyGameCommandStub::Errors execute() override
    {
        ...
        BlazeRpcError err = gameManagerComponent->destroyGame(mRequest.getDestroyGameRequest(), mResponse);
...
DEFINE_DESTROYGAME_CREATE()

DestroyGameCommandStub::Errors DestroyGameCommand::arsonErrorFromGameManagerError(BlazeRpcError error)
{
    ...
    switch (error)
    {
        ...
        default:
        {
            ...
            TRACE_LOG("[DestroyGameCommand].arsonErrorFromGameManagerError: unexpected error(" << SbFormats::HexLower(error) << "): return as ERR_SYSTEM");
            result = ERR_SYSTEM;
            break;
        }
    };
