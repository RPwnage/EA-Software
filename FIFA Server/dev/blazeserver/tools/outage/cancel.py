...
try:
    ...
    rdir.cancel_downtime(blaze.get_token(), options.instance, options.msgtype, start, dur, options.downtype.upper())
    
    sender = config.sender
    recp = config.mailinglist
    recp.append(options.admin)
    msgstr = 'User %s has cancelled an outage for %s at %s' % (options.admin, options.instance, options.start)
    msgsub = 'Cancelled Outage %s at %s' % (options.instance, options.start)
    msg = MIMEText(msgstr)
    msg['Subject'] = msgsub
    msg['From'] = sender
    msg['To'] = recp[0]
