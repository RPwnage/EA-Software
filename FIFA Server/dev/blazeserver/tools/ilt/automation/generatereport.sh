...
# This report is using 2 graphite instances:
# - We want Blaze to push metrics to the old graphite instance (due to ACL issue, we can't push to the new one anyway)
# - Torch and Redis pollers currently only send data to the new graphite instance
# The graphite parameter for calling this script probably doesn't make sense with this usage, but eventually, we want to use just one graphite instance (again) for this report.
GRAPHITE_SERVER_FOR_BLAZE="eadp.gos.graphite.ea.com"
...
function usage {
  echo "usage: $0 service_name environment platform target_psu reference_file run_duration_minutes target_directory [--graphite graphite_addr --graphite-prefix graphite_prefix_string --rpcrate rate_file1 rate_file2 interval] [--memcpu memcpu_file1 memcpu_file2] [options]"
  ...
  echo "  graphite: default is " $GRAPHITE_SERVER_FOR_BLAZE
...
mmscenarios="yes"
check_db="no" # db checks disabled by default becuase default security settings on DB hosts cause SSH to always fail
debug="no"
...
if [ $# -gt 6 ]; then
   for (( i=7;i<$#;i++ )) do
     ...
     if [ "$argval" = "--rpcrata" ]; then
       ...
       let i=$i+1
       GRAPHITE_SERVER_FOR_BLAZE=${args[$i]}
       continue
...
report_template=$(cat $relative_script_path/reporttemplate.html)
...
export report_cpumemgraphs=""
for instance in $instances 
do
  relfilename=$instance"_cpumem_"$report_compact_rundate".png"
  filename="$param_target_dir/$relfilename"
  url="http://$GRAPHITE_SERVER_FOR_BLAZE/render/?width=500&height=300&from="$graphite_time_from"&until="$graphite_time_until"&yMinRight=0&yMaxRight=110&title=Memory%2FCPU%20Usage%20"$instance"&uniqueLegend=true&hideLegend=false&yMinLeft=0&target=alias(color("$GRAPHITE_PREFIX"blaze."$param_env"."$param_service_name"."$instance"*.utilization.mem_resident_bytes%2C%22green%22)%2C%22Memory%22)&target=alias(color(secondYAxis("$GRAPHITE_PREFIX"blaze."$param_env"."$param_service_name"."$instance"*.utilization.cpu_pct)%2C%22blue%22)%2C%22CPU%20utilization%20%5B%25%5D%22)"
  curl ${curlOpts[*]} "$url" >"$filename" &
  buf="<img src=\"$relfilename\" />"$'\n'"<p><a href='$url'>$instance cpu/mem utilization</a></p>"
  report_cpumemgraphs="$report_cpumemgraphs"$'\n'"$buf"
done
...
export report_games_and_players_graph="games_and_players_"$report_compact_rundate".png"
export report_games_and_players_graph_url="http://$GRAPHITE_SERVER_FOR_BLAZE/render/?width=500&height=300&from="$graphite_time_from"&until="$graphite_time_until"&title=Active%20Game%20Sessions%20and%20Active%20Players&uniqueLegend=true&target=alias(color("$GRAPHITE_PREFIX"blaze."$param_env"."$param_service_name".coreMaster*.component.gamemanager_master.status.GMGauge_ACTIVE_GAMES%2C%22blue%22)%2C%22GMGauge_ACTIVE_GAMES%22)&target=alias(color(secondYAxis("$GRAPHITE_PREFIX"blaze."$param_env"."$param_service_name".coreMaster*.component.gamemanager_master.status.GMGauge_ACTIVE_PLAYERS)%2C%22green%22)%2C%22GMGauge_ACTIVE_PLAYERS%22)&hideLegend=false&yMinLeft=0&yMinRight=0"
curl ${curlOpts[*]} "$report_games_and_players_graph_url" >$param_target_dir/$report_games_and_players_graph

export report_find_game_gb_lists_graph="find_game_and_lists_"$report_compact_rundate".png"
export report_find_game_gb_lists_graph_url="http://$GRAPHITE_SERVER_FOR_BLAZE/render/?width=500&height=300&from="$graphite_time_from"&until="$graphite_time_until"&title=Active%20Find%20Game%20Session%20and%20Game%20List%20Counts&uniqueLegend=true&hideLegend=false&target=alias(color("$GRAPHITE_PREFIX"blaze."$param_env"."$param_service_name".searchSlave*.component.search.status.SSGaugeActiveFGSessions%2C%22blue%22)%2C%22SSGaugeActiveFGSessions%22)&target=alias(color(secondYAxis("$GRAPHITE_PREFIX"blaze."$param_env"."$param_service_name".searchSlave*.component.search.status.SSGaugeGameLists)%2C%22green%22)%2C%22SSGaugeGameLists%22)&yMinLeft=0&yMinRight=0"
curl ${curlOpts[*]} "$report_find_game_gb_lists_graph_url" >$param_target_dir/$report_find_game_gb_lists_graph
...
if [ -z "$runerrors" ]; then
  ...
  export report_errors="No errors were reported by automation script."
  exitcode=0
else
  export report_success="FAIL"
  export report_errors=$(echo "$runerrors" | awk '{ print "<li>"substr($0, 8)"</li>" }')
  exitcode=1
fi
