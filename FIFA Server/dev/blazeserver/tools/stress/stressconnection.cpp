...
bool StressConnection::connect(bool quiet)
{
    ...
    do {
        ...
        if (!mAddress.isResolved()) {
            ...
            {
                BLAZE_ERR_LOG(Log::CONNECTION, "[" << getType() << "Connection].connect: " << mResolver.getName() << " failed to resolve address: " << mAddress.toString(addressBuf, sizeof(addressBuf)));
                if (shouldReconnect()) {
                    continue;
                }
                return false;
...
bool StressConnection::flushSendQueue()
{
    while (!mSendQueue.empty() && !mMigrating)
    {
        ...
        if (frame->datasize() == 0)
            ...
            BLAZE_TRACE_LOG(Log::CONNECTION, "[" << getType() << "Connection].writeFrame: remaining size: " << frame->datasize());
    }
