...
{
    struct rlimit lim;
    #define LOG_RESOURCE_LIMIT(limit) if (getrlimit((limit), &lim) == 0) { EA::StdC::Fprintf(stderr, "  " #limit " : soft= %d, hard= %d\n", lim.rlim_cur, lim.rlim_max); }
    LOG_RESOURCE_LIMIT(RLIMIT_AS);
    LOG_RESOURCE_LIMIT(RLIMIT_CORE);
    LOG_RESOURCE_LIMIT(RLIMIT_CPU);
    LOG_RESOURCE_LIMIT(RLIMIT_DATA);
    LOG_RESOURCE_LIMIT(RLIMIT_FSIZE);
    LOG_RESOURCE_LIMIT(RLIMIT_LOCKS); 
    LOG_RESOURCE_LIMIT(RLIMIT_MEMLOCK);
    LOG_RESOURCE_LIMIT(RLIMIT_MSGQUEUE);
    LOG_RESOURCE_LIMIT(RLIMIT_NICE);
    LOG_RESOURCE_LIMIT(RLIMIT_NOFILE);
    LOG_RESOURCE_LIMIT(RLIMIT_NPROC);
    LOG_RESOURCE_LIMIT(RLIMIT_RSS);
    LOG_RESOURCE_LIMIT(RLIMIT_RTPRIO);
    LOG_RESOURCE_LIMIT(RLIMIT_RTTIME);
    LOG_RESOURCE_LIMIT(RLIMIT_SIGPENDING);
    LOG_RESOURCE_LIMIT(RLIMIT_STACK);
    #undef LOG_RESOURCE_LIMIT
}
#endif

static int32_t usage(const char* prg)
{
    printf("Usage: %s [-c config] [-s hostname:port] [--instance_name name] [--logdir dir] [--logname filename] [--testid uuid] [-num_conns count] [-block_size size] [-block_delay delay] [-instance_delay delay] [-start_index index] [-use_stress_login bool] [-login_by_jwt bool] [-cache_jwt_token bool]\n", prg);
    return 1;
}
...
extern "C" int main(int argc, char** argv)
{
    ...
    for(int32_t a = 1; a < argc; a++)
    {
        ...
        else if (strcmp(argv[a], "-login_by_jwt") == 0)
        {
            a++;
            if (a == argc)
                return usage(argv[0]);
            arguments.loginByJwt = argv[a];
        }
        else if (strcmp(argv[a], "-cache_jwt_token") == 0)
        {
            a++;
            if (a == argc)
                return usage(argv[0]);
            arguments.cacheJwtToken = argv[a];
        }
