...
/*!
    ...
    \Input *pVoipServer - voipserver module state
    \Input iConnSet     - index of the connection set we are querying
    \Input *pBuffer     - [out] buffer we write the information to
    \Input iBufSize     - size of the buffer
...
*/
/********************************************************************************F*/
static int32_t _VoipConciergeDiagnosticQueryConnSet(VoipServerRefT *pVoipServer, int32_t iConnSet, char *pBuffer, int32_t iBufSize)
{
    int32_t iOffset = 0;
    ...
    VoipTunnelStatus(pVoipTunnel, 'game', iConnSet, &Game, sizeof(Game));
