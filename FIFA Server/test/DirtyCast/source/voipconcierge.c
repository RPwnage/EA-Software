...
typedef struct VoipConciergeRefT
{
    ...
    uint32_t uLastCCSRegistrationAttempt;       //!< Last time CCS registration was attempted but resulted unsuccessful, 0 means "if unregistered, proceed with first attempt"
...
/*!
    ...
    \Description
        Remove client from the product map entry. Remove the product if it is no longer used.
...
static void _VoipConciergeRemoveClientFromProduct(VoipConciergeRefT *pVoipConcierge, const char *pProductId)
{
    ...
    for (iProductIndex = 0; iProductIndex < pVoipConcierge->iProductMapSize; iProductIndex += 1)
    {
        if (strncmp(pVoipConcierge->pProductMap[iProductIndex].strProductId, pProductId, sizeof(pVoipConcierge->pProductMap[iProductIndex].strProductId)) == 0)
        {
            ...
            // release the entry if the client count goes back to 0
            if (pVoipConcierge->pProductMap[iProductIndex].iClientCount == 0)
            {
                DirtyCastLogPrintf("voipconcierge: releasing product map entry --> %s\n", pVoipConcierge->pProductMap[iProductIndex].strProductId);
                ds_memclr(&pVoipConcierge->pProductMap[iProductIndex], sizeof(pVoipConcierge->pProductMap[iProductIndex]));
            }
            else if (pVoipConcierge->pProductMap[iProductIndex].iClientCount < 0)
            {
...
/*!
    ...
    \Input iConnSet        - index to add the client to (usually game specific)
    \Input iRequestId      - identifies the request for logging purposes
...
*/
/********************************************************************************F*/
static int32_t _VoipConciergeAddVoipClient(VoipTunnelRefT *pVoipTunnel, int32_t iConnSet, int32_t iRequestId, uint32_t uClientId, const GameConsoleT *pGameConsole, VoipTunnelClientT **ppNewClient)
{
    int32_t iResult;
...
/*!
    ...
    \Input *pVoipConcierge      - ref to the voipconcierge module
    \Input iConnSet             - the connection set we are checking/adding to
    \Input iRequestId           - identifies the request for logging purposes
...
*/
/********************************************************************************F*/
static int32_t _VoipConciergeAddConnection(VoipConciergeRefT *pVoipConcierge, int32_t iConnSet, int32_t iRequestId, const char *pClientInstanceId, const PostDataT *pPostData, AllocatedResourceT *pAllocatedResource)
{
    int32_t iConnIdx;
    ...
    for (iConnIdx = 0; iConnIdx < pPostData->iNumConnections; iConnIdx += 1)
    {
        ...
        if (((pClient1 = VoipTunnelClientListMatchId(pVoipTunnel, uClient1Id)) == NULL) || (pClient1->iGameIdx != iConnSet))
        {
            if (_VoipConciergeAddVoipClient(pVoipTunnel, iConnSet, iRequestId, uClient1Id, pGameConsole1, &pClient1) < 0)
            {
        ...
        if (((pClient2 = VoipTunnelClientListMatchId(pVoipTunnel, uClient2Id)) == NULL) || (pClient2->iGameIdx != iConnSet))
        {
            if (_VoipConciergeAddVoipClient(pVoipTunnel, iConnSet, iRequestId, uClient2Id, pGameConsole2, &pClient2) < 0)
            {
...
/*!
    ...
    \Input *pVoipConcierge      - ref to the VoipConcierge module
    \Input iConnSet             - the connection set we are checking/removing from
...
*/
/********************************************************************************F*/
static int32_t _VoipConciergeRemoveConnection(VoipConciergeRefT *pVoipConcierge, int32_t iConnSet, int32_t iRequestId, const DeleteDataT *pRequestData)
{
    ...
    /*  Either remove the clients from their active or suspended game:
        ...
        We will search for the suspended information using the iConnSet as a key.
    ...
    if (pClient1->iGameIdx == iConnSet && pClient2->iGameIdx == iConnSet)
    {
        int32_t iResult;
    ...
    else
    {
        ...
        if ((pSuspendInfo1 = _VoipConciergeSuspendInfoMatchId(pClient1, iConnSet)) == NULL)
        {
            DirtyCastLogPrintf("voipconcierge: client id 0x%08x does not belong to suspended connection set %04d (possible mismatch?) (req=%d)\n", pClient1->uClientId, iConnSet, iRequestId);
            return(VOIPCONCIERGE_ERR_INVALID_CONSOLE_INFO);
        }
        if ((pSuspendInfo2 = _VoipConciergeSuspendInfoMatchId(pClient2, iConnSet)) == NULL)
        {
            DirtyCastLogPrintf("voipconcierge: client id 0x%08x does not belong to suspended connection set %04d (possible mismatch?) (req=%d)\n", pClient2->uClientId, iConnSet, iRequestId);
            return(VOIPCONCIERGE_ERR_INVALID_CONSOLE_INFO);
        }
        ...
        {
            VoipTunnelClientListDel(pVoipTunnel, pClient1, iConnSet);
        }
        if (pSuspendInfo2->iNumClients == 0)
        {
            VoipTunnelClientListDel(pVoipTunnel, pClient2, iConnSet);
        }
    }
...
static void _VoipConciergeTokenCb(const char *pAccessToken, const TokenApiInfoDataT *pData, void *pUserData)
{
    ...
    {
        DirtyCastLogPrintf("voipconcierge: could not find the validating request by the access token, was it deleted?\n");
        return;
    }
...
static int32_t _VoipConciergeHandleDeleteRequest(VoipConciergeRefT *pVoipConcierge, const HostedConnectionRequestT *pRequest, HostedConnectionResponseT *pResponse)
{
    ...
    if (pRequest->iConnSet > 0)
    {
        ...
        if ((pDeleteData->uClientId1 == 0) && (pDeleteData->uClientId2 == 0))
        {
            ...
            VoipTunnelGameListDel(pVoipTunnel, iConnSet);
...
{
    // register with CCS if not yet registered
    if (ConciergeServiceStatus(pVoipConcierge->pConciergeService, 'regi', 0, NULL, 0) == FALSE)
    {
        const VoipServerConfigT *pConfig = VoipServerGetConfig(pVoipConcierge->pBase);
        uint32_t uCurrTick = NetTick();

        // only re-attempt every 10 sec
        if ((pVoipConcierge->uLastCCSRegistrationAttempt == 0) ||
            NetTickDiff(uCurrTick, pVoipConcierge->uLastCCSRegistrationAttempt) > 10000)
        {
            char strHostname[256], strHost[256], strVersions[64];
            VoipServerRefT *pVoipServer = VoipConciergeGetBase(pVoipConcierge);

            pVoipConcierge->uLastCCSRegistrationAttempt = uCurrTick;
    ...
    {
        // reset the registration attempt timer because it is only meaningful when we are not registered
        pVoipConcierge->uLastCCSRegistrationAttempt = 0;
    }
...
/*!
    ...
    \Notes
        ...
        \verbatim
            'actv'  - return number of active clients (per product, per tunnel vers)
            'done'  - return if the module has finished its time critical operations
            'fail'  - return TRUE if the server has entered the "max load" state  (can't accept more clients)
            'prod'  - return pointer to product map
