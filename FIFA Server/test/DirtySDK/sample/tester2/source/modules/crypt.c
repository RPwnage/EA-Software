...
//! type to encapsulate eddsa test data (assumes ed25519); $$todo$$ unify with other ecdsa tests when cryptcurve is refactored to follow the API needed for eddsa.
typedef struct CryptTestEddsaT
{
    const char strTestName[16];
    const uint8_t aPrivateKey[32];
    const uint8_t aPublicKey[32];
    const uint8_t aMessage[1024];
    const int32_t iMessageLen;
...
static const CryptTestEddsaT _Crypt_EddsaTests[] =
{
    {
        "test zero",
        {
...
static int32_t _CmdCryptTestMont(ZContext *argz, int32_t argc, char *argv[])
{
    ...
    CryptEccPointT AlicePublic, BobPublic, AliceShared, BobShared;
    uint8_t bResult25519 = TRUE, bResult448 = TRUE;
    ...
    while (CryptMontPublic(&Alice, &AlicePublic, NULL) > 0)
        ;
    #if DIRTYCODE_DEBUG
    bResult25519 &= memcmp(aAlicePublic25519, AlicePublic.X.aData, 32) == 0;
    #endif
    ZPrintf("crypt: alice public x25519 time=%u result=%s\n", Alice.uCryptUsecs, bResult25519 ? "SUCCESS" : "FAILURE");

    // generate bob public
    while (CryptMontPublic(&Bob, &BobPublic, NULL) > 0)
        ;
    #if DIRTYCODE_DEBUG
    bResult25519 &= memcmp(aBobPublic25519, BobPublic.X.aData, 32) == 0;
    #endif
    ZPrintf("crypt: bob public x25519 time=%u reuslt=%s\n", Bob.uCryptUsecs, bResult25519 ? "SUCCESS" : "FAILURE");
    ...
    while (CryptMontSecret(&Bob, &AlicePublic, &BobShared, NULL) > 0)
        ;
    #if DIRTYCODE_DEBUG
    bResult25519 &= memcmp(aSharedSecrete25519, AliceShared.X.aData, 32) == 0;
    bResult25519 &= memcmp(aSharedSecrete25519, BobShared.X.aData, 32) == 0;
    #else
    bResult25519 &= memcmp(AliceShared.X.aData, BobShared.X.aData, sizeof(AliceShared.X.aData)) == 0;
    #endif
    ...
    while (CryptMontPublic(&Alice, &AlicePublic, NULL) > 0)
        ;
    #if DIRTYCODE_DEBUG
    bResult448 &= memcmp(aAlicePublic448, AlicePublic.X.aData, 56) == 0;
    #endif
    ZPrintf("crypt: alice public x448 time=%u result=%s\n", Alice.uCryptUsecs, bResult448 ? "SUCCESS" : "FAILURE");

    // generate bob public
    while (CryptMontPublic(&Bob, &BobPublic, NULL) > 0)
        ;
    #if DIRTYCODE_DEBUG
    bResult448 &= memcmp(aBobPublic448, BobPublic.X.aData, 56) == 0;
    #endif
    ZPrintf("crypt: bob public x448 time=%u result=%s\n", Bob.uCryptUsecs, bResult448 ? "SUCCESS" : "FAILURE");
    ...
    while (CryptMontSecret(&Bob, &AlicePublic, &BobShared, NULL) > 0)
        ;
    #if DIRTYCODE_DEBUG
    bResult448 &= memcmp(aSharedSecret448, AliceShared.X.aData, 56) == 0;
    bResult448 &= memcmp(aSharedSecret448, BobShared.X.aData, 56) == 0;
    #else
    bResult448 &= memcmp(AliceShared.X.aData, BobShared.X.aData, sizeof(AliceShared.X.aData)) == 0;
    #endif

    ZPrintf("crypt: alice shared x448 time=%u result=%s\n", Alice.uCryptUsecs, bResult448 ? "SUCCESS" : "FAILURE");
    ZPrintf("crypt: bob shared x448 time=%u result=%s\n", Bob.uCryptUsecs, bResult448 ? "SUCCESS" : "FAILURE");

    return((bResult25519 && bResult448) ? 0 : 1);
}

/*F********************************************************************************/
/*!
    \Function _CmdCryptTestEd

    \Description
        Test the CryptEd module
    ...
    \Version 05/18/2020 (eesponda)
*/
/********************************************************************************F*/
static int32_t _CmdCryptTestEd(ZContext *argz, int32_t argc, char *argv[])
{
    CryptEdT Ed;
    CryptBnT PrivateKey;
    CryptEccPointT PublicKey;
    CryptEccPointT Signature;
    uint32_t uCryptUsecs, uSignResult, uVerifyResult, uIndex;
    uint8_t aSignature[64];
    int32_t iFailed = 0;

    for (uIndex = 0; uIndex < (sizeof(_Crypt_EddsaTests)/sizeof(_Crypt_EddsaTests[0])); uIndex += 1)
    {
        const CryptTestEddsaT *pTestCase = &_Crypt_EddsaTests[uIndex];

        // initialize our private/public key and module
        CryptBnInitFrom(&PrivateKey, -1, pTestCase->aPrivateKey, sizeof(pTestCase->aPrivateKey));
        CryptEdPointInitFrom(&PublicKey, pTestCase->aPublicKey, sizeof(pTestCase->aPublicKey));
        CryptEdInit(&Ed, CRYPTCURVE_X25519);

        // do sign operation and check result 
        CryptEdSign(&Ed, &PrivateKey, pTestCase->aMessage, pTestCase->iMessageLen, &Signature, &uCryptUsecs);
        CryptBnFinal(&Signature.X, aSignature, 32);
        CryptBnFinal(&Signature.Y, aSignature+32, 32);
        uSignResult = memcmp(aSignature, pTestCase->aSignature, sizeof(aSignature));
        ZPrintf("ed test=%s sign time=%u result=%u\n", pTestCase->strTestName, uCryptUsecs, uSignResult);

        // do verify operation and check result
        uVerifyResult = CryptEdVerify(&Ed, &PublicKey, pTestCase->aMessage, pTestCase->iMessageLen, &Signature, &uCryptUsecs);
        ZPrintf("ed test=%s verify time=%u result=%u\n", pTestCase->strTestName, uCryptUsecs, uVerifyResult);

        // increment failure count if needed
        if ((uSignResult != 0) || (uVerifyResult != 0))
        {
            iFailed += 1;
        }
    }
...
int32_t CmdCrypt(ZContext *argz, int32_t argc, char *argv[])
{
    ...
    {
        ZPrintf("%s: ed test %s\n", argv[0], _CmdCryptTestEd(argz, argc, argv) == 0 ? "succeeded" : "failed");
    }
