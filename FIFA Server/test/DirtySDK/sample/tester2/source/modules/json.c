    ...
    if (argc == 2)
    {
        ...
        if ((pFileData = ZFileLoad(argv[1], &iFileSize, FALSE)) != NULL)
        {
            if ((pJsonParseBuf = JsonParse2(pFileData, -1, 0, 0, NULL)) != NULL)
            {
                ...
                // {"__type":"AccessDeniedException","Message":"User: arn:aws:iam::997155550211:user/eadpgs-speech is not authorized to perform: translate:TranslateText"}

                if ((pCurrent = JsonFind2(pJsonParseBuf, NULL, "__type", 0)) != NULL)
                {
                    ...
                    //ds_snzprintf(pResult, iResultSize, "%s error (%s)", strExcept, strMessage);
                }
            }
            else
            {
