...
static int32_t _ProtoHttpGetHeaderFieldName(const char *pInpBuf, char *pOutBuf, int32_t iOutLen)
{
    ...
    for (; (iOutSize < iOutLen) && (pInpBuf[iOutSize] != ':') && (pInpBuf[iOutSize] != '\0') && (pInpBuf[iOutSize] != '\r'); iOutSize += 1)
    {
        pOutBuf[iOutSize] = pInpBuf[iOutSize];
    }
    if (iOutSize == iOutLen)
...
int32_t ProtoHttpGetNextHeader(void *pState, const char *pHdrBuf, char *pName, int32_t iNameSize, char *pValue, int32_t iValSize, const char **pHdrEnd)
{
    ...
    // if clrf is found, header is invalid and thus we write an empty value and return
    if ((pHdrBuf[iNameLen] == '\r') && (pHdrBuf[iNameLen + 1] == '\n'))
    {
        if (pHdrEnd != NULL)
        {
            *pHdrEnd = pHdrBuf + iNameLen;
        }
        if (pValue != NULL)
        {
            *pValue = '\0';
...
/*F********************************************************************************/
/*!
    \Function ProtoHttpFormatUserAgent

    \Description
        Format User-Agent header string

    \Input *pBuffer - [out] buffer to hold user-agent string
    \Input iBufSize - size of buffer
    \Input iClntVer - http client version

    \Output
        char *      - user-agent value
*/
/********************************************************************************F*/
char *ProtoHttpFormatUserAgent(char *pBuffer, int32_t iBufSize, int32_t iClntVer)
{
    ds_snzprintf(pBuffer, iBufSize, "ProtoHttp %d.%d/DS %d.%d.%d.%d.%d (" DIRTYCODE_PLATNAME ")", (iClntVer>>8)&0xff, iClntVer&0xff,
