...
static void _VoipStatusCb(int32_t iLocalUserIndex, uint32_t bStatus, VoipHeadsetStatusUpdateE eUpdate, void *pUserData)
{
    ...
    {
        NetPrintf(("voipxbox: warning, unexpected headset event for this platform!\n"));
    }

    if (bStatus)
    {
        NetPrintf(("voipxbox: [%d] headset active (audio in: on, audio out: on)\n", iLocalUserIndex));
    ...
    {
        NetPrintf(("voipxbox: [%d] headset inactive (audio in: off, audio out: on)\n", iLocalUserIndex));
        pRef->Common.uPortHeadsetStatus &= ~(1 << iLocalUserIndex);
        pRef->Common.Connectionlist.uLocalUserStatus[iLocalUserIndex] &= ~(VOIP_LOCAL_USER_HEADSETOK|VOIP_LOCAL_USER_INPUTDEVICEOK);
    }
}
...
static void _VoipRegisterLocalSharedTalker(VoipRefT *pVoip, uint32_t bRegister)
{
    if (bRegister)
    {
        ...
        if (VOIP_NullUser((VoipUserT *)(&pVoip->Common.Connectionlist.LocalUsers[VOIP_SHARED_USER_INDEX])))
        {
            ...
            NetPrintf(("voipxbox: local shared user added at index %d\n", VOIP_SHARED_USER_INDEX));
        }
    }
    else
    {
        ...
        if (!VOIP_NullUser((VoipUserT *)(&pVoip->Common.Connectionlist.LocalUsers[VOIP_SHARED_USER_INDEX])))
        {
            ...
            NetPrintf(("voipxbox: shared user removed at index %d\n", VOIP_SHARED_USER_INDEX));
        }
    }
}
...
static void _VoipThread(void *pParam)
{
    ...
    for (pVoip->iThreadActive = 1; pVoip->iThreadActive != 0; )
    {
        ...
        {
            if ((pVoip->Common.pHeadset != NULL) && (VoipHeadsetStatus(pVoip->Common.pHeadset, 'fpmu', iLocalUserIndex, NULL, 0)))
            {
                pVoip->Common.bApplyMute = TRUE;
            }
        }

        if (pVoip->Common.bApplyMute)
        {
            pVoip->Common.bApplyMute = FALSE;
            VoipCommonApplyMute((VoipCommonRefT *)pVoip);
        }

        // update transcriptions settings
        VoipCommonUpdateTranscription(&pVoip->Common);
                ...
                for (iLocalUserIndex = 0; iLocalUserIndex < VOIP_MAXLOCALUSERS; iLocalUserIndex++)
                {
                    if (!VOIP_NullUser(&pConnectionlist->LocalUsers[iLocalUserIndex]))
                    {
                        ...
                        for (iRemoteUserIndex = 0; iRemoteUserIndex < pConnectionlist->pConnections[iConnIndex].iMaxRemoteUsers; iRemoteUserIndex++)
                        {
                            ...
                            {
                                VoipCommonControl((VoipCommonRefT *)pVoip, ((pVoip->Common.uConnSendMask & (1 << iConnIndex)) && (pConnectionlist->aIsParticipating[iLocalUserIndex] == TRUE)) ? '+snd' : '-snd', iLocalUserIndex, pRemoteUser);
                                VoipCommonControl((VoipCommonRefT *)pVoip, ((pVoip->Common.uConnRecvMask & (1 << iConnIndex)) && (pConnectionlist->aIsParticipating[iLocalUserIndex] == TRUE)) ? '+rcv' : '-rcv', iLocalUserIndex, pRemoteUser);
                            }
                        }
                    }
                }
        ...
        Additionally, this protects access to the codec when in crossplay between the socket recv thread
        (decode) and the voip thread (reset/encode).
        */
        ...
        {
            NetPrintf(("voipxbox: [WARNING] Overall voip thread update took %d ms\n", NetTickDiff(uTime1, uTime0)/1000));
        }
    }
...
static int32_t _VoipShutdownInternal(void *pDestroyData)
{
    ...
    if (pVoip->Common.pHeadset != NULL)
    {
        if (VoipHeadsetDestroy(pVoip->Common.pHeadset) == VOIPHEADSET_DESTROY_RETRY)
        {
            NetCritLeave(&pVoip->Common.ThreadCrit);
            return(-1);
        }

        pVoip->Common.pHeadset = NULL;
    }
    NetCritLeave(&pVoip->Common.ThreadCrit);
...
VoipRefT *VoipStartup(int32_t iMaxPeers, int32_t iData)
{
    ...
    if (VoipGetRef() != NULL)
    {
        NetPrintf(("voipxbox: module startup called when already in a started state\n"));
        return(NULL);
    }
    ...
    {
        NetPrintf(("voipxbox: error creating voip thread (err=%d)\n", iResult));
        pVoip->iThreadActive = 0;
...
void VoipShutdown(VoipRefT *pVoip, uint32_t uShutdownFlags)
{
    ...
    if (VoipGetRef() == NULL)
    {
        NetPrintf(("voipxbox: module shutdown called when not in a started state\n"));
        return;
    }

    NetPrintf(("voipxbox: [%p] shutting down\n", pVoip));
...
void VoipSetLocalUser(VoipRefT *pVoip, int32_t iLocalUserIndex, uint32_t bRegister)
{
    if ((iLocalUserIndex < VOIP_MAXLOCALUSERS) && (iLocalUserIndex >= 0))
    {
        ...
        if (bRegister)
        {
            ...
            if (VOIP_NullUser((VoipUserT *)(&pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex])))
            {
                ...
                NetPrintf(("voipxbox: registering local user %lld at index %d\n", voipUser.AccountInfo.iPersonaId, iLocalUserIndex));
                _VoipRegisterLocalTalker(pVoip, iLocalUserIndex, TRUE, &voipUser);
            }
            else
            {
                NetPrintf(("voipxbox: warning -- ignoring attempt to register user at index %d because user %lld is already registered at that index\n",
        ...
        else
        {
            ...
            if (!VOIP_NullUser((VoipUserT *)(&pVoip->Common.Connectionlist.LocalUsers[iLocalUserIndex])))
            {
                ...
                NetPrintf(("voipxbox: disabling voice for user at index %d\n", iLocalUserIndex));
                _VoipRegisterLocalTalker(pVoip, iLocalUserIndex, FALSE, NULL);
            }
            else
            {
                NetPrintf(("voipxbox: warning -- ignoring attempt to un-register user at index %d because it is empty\n", iLocalUserIndex));
            }
        }
    ...
    {
        NetPrintf(("voipxbox: VoipSetLocalUser() invoked with an invalid user index %d\n", iLocalUserIndex));
    }
}
...
void VoipCommonActivateLocalUser(VoipCommonRefT *pVoipCommon, int32_t iLocalUserIndex, uint8_t bActivate)
{
    ...
    if ((pVoipCommon != NULL) && (iLocalUserIndex >= 0) && (iLocalUserIndex < NETCONN_MAXLOCALUSERS))
    {
        ...
        if (!VOIP_NullUser((VoipUserT *)(&pVoipCommon->Connectionlist.LocalUsers[iLocalUserIndex])))
        {
            if (NetConnStatus('xuid', iLocalUserIndex, &uXuid, sizeof(uXuid)) == 0)
            {
                ...
                {
                    NetPrintf(("voipxbox: cannot %s a null user %s participating state (local user index %d)\n",
            ...
            {
                NetPrintf(("voipxbox: local talker registration aborted - local user at index %d either doesn't exist or has an invalid xuid\n", iLocalUserIndex));
            }
        }
        else
        {
            NetPrintf(("voipxbox: VoipCommonActivateLocalUser() failed because no local user registered at index %d\n", iLocalUserIndex));
        }
    ...
    {
        NetPrintf(("voipxbox: VoipCommonActivateLocalUser() invoked with an invalid user index %d\n", iLocalUserIndex));
    }
}
...
int32_t VoipStatus(VoipRefT *pVoip, int32_t iSelect, int32_t iValue, void *pBuf, int32_t iBufSize)
{
    if (iSelect == 'pcac')
    {
        ...
        {
            NetPrintf(("voipxbox: error, exception thrown when calling get PartyChat::IsPartyChatActive (%S/0x%08x)\n", e->Message->Data(), e->HResult));
            return(-1);
        }
        #endif
    }
...
int32_t VoipControl(VoipRefT *pVoip, int32_t iControl, int32_t iValue, void *pValue)
{
    ...
    {
        #ifndef DIRTYCODE_GDK
        NetPrintf(("voipxbox: IsPartyChatSuppressed=%d", iValue));
        ...
        {
            NetPrintf(("voipxbox: error, exception thrown when calling set PartyChat::IsPartyChatSuppressed (%S/0x%08x)\n", e->Message->Data(), e->HResult));
            return(-1);
        }
        #endif
        return(iValue);
