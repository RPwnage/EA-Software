...
/*!
    ...
    \Notes
        $todo – CommUDP protocol updates to consider to enrich the CommUDP packets with info needed by the receiver
        for maintaining packLost and packSaved in a more accurate manner.

        - packLost is a concept that should apply regardless of whether traffic is reliable or not. Before the 
          reliability layer is exercised, all commudp packets (i.e. the outer shell conveying sub-packets with a single 
          UDP datagram) should be uniquely identifiable from a sequence number that is never re-used by the originator 
          (unless the counter wraps, which still guarantees packets to be unique within a proximity range). From the 
          receiver’s stand point, packet loss can then be determined like this:
             * if the seq number of the inbound packet is lower than last received seq number, ignore the packet
             * otherwise, if the seq number of the inbound packet is not the one that comes directly after the last 
               received seq number, add the sequence gap to packLost as those packets were lost
          This would simplify the tracking of lost packets, and also allow for tracking packets that are lost when they 
          are re-sent due to a NAK (since the outer shell would continue to increment). We don't currently have such a 
          seq number common to both reliable and unreliable traffic in commudp packets and would require a protocol 
          update to implement this.

        - packSaved is a concept that is only applicable to reliable traffic and was originally meant to count the 
          number of packets saved by redundancy. It is currently incremented any time a sub-packet is successfully 
          consumed; however, sub-packets in an incoming packet can be either
            * forward-consolidated packets = bundle of packets that piled up while waiting to be sent for the first
              time, or bundle of packets being re-sent as a result of the send index being moved back because of NAKing
            * redundant packets = bundle of packets already sent, waiting to be ACKed but appended to next send to
              achieve redundancy and mitigate packet loss early.
          Currently, packets are lacking info about which type of sub-packets they contain. Consequently, from the 
          receiver’s perspective, it is impossible to limit packSaved to what it was originally meant to be, i.e. 
          successfully consumed redundant packets only (excluding forward-consolidated packets). Additionally, if we 
          ever want to create a packSavedByNak metric, then we will need info in the packet to distinguish both sub-types 
          of forward-consolidated packets.
*/
/*************************************************************************************************H*/
...
struct CommUDPRef
{
    ...
    char *pRcvBuf;
    //! next reliable packet expected (sequence number)
    uint32_t uReliableExpectedSeq;
    //! last reliable packet received (sequence number, for tracking lost packets)
    // Note this packet may not have been processed if out of sequence
    uint32_t uReliableRcvSeq;
    //! next unreliable packet expected
    uint32_t uUnreliableExpectedSeq;
...
static void _CommUDPResetTransfer(CommUDPRef *pRef)
{
    ...
    pRef->uReliableExpectedSeq = COMMUDP_RAW_PACKET_DATA;
    pRef->uReliableRcvSeq = _CommUDPSeqnDelta(COMMUDP_RAW_PACKET_DATA, -1);
    pRef->uUnreliableExpectedSeq = COMMUDP_RAW_PACKET_UNREL;
...
static void _CommUDPProcessOutput(CommUDPRef *pRef, uint32_t uCurrTick)
{
    ...
    while (iTLimit > 0) {
        ...
        pRef->uRcvAck = pRef->uReliableExpectedSeq;
        Multi.body.uAck = _CommUDPSeqnDelta(pRef->uReliableExpectedSeq, -1);
...
/*!
    ...
    \Input uMainPacketSeq - sequence number of main packet
...
*/
/*************************************************************************************************F*/
static int32_t _CommUDPProcessInput(CommUDPRef *pRef, RawUDPPacketHeadT *pPacket, uint8_t *pData, uint32_t uCurrTick, uint32_t uMainPacketSeq)
{
    RawUDPPacketT *pBuffer;
    ...
    if (((pPacket->body.uSeq & COMMUDP_SEQ_MASK) >= COMMUDP_RAW_PACKET_UNREL) && ((pPacket->body.uSeq & COMMUDP_SEQ_MASK) < COMMUDP_RAW_PACKET_DATA))
    {
        ...
        int32_t delta = (pPacket->body.uSeq - pRef->uUnreliableExpectedSeq) & (COMMUDP_RAW_PACKET_UNREL - 1);

        // update lost packet count
        if ((pPacket->body.uSeq > pRef->uUnreliableExpectedSeq) || (delta < COMMUDP_RAW_PACKET_UNREL/4))
        {
            pRef->Common.packlost += delta;
        }

        // calculate new sequence number
        if ((pRef->uUnreliableExpectedSeq = (pPacket->body.uSeq + 1)) >= COMMUDP_RAW_PACKET_DATA)
        {
            pRef->uUnreliableExpectedSeq = COMMUDP_RAW_PACKET_UNREL + (pRef->uUnreliableExpectedSeq - COMMUDP_RAW_PACKET_DATA);
        }
    ...
    {
        // ignore old packets
        if (_CommUDPSeqnDiff(pPacket->body.uSeq, pRef->uReliableExpectedSeq) < 0)
        {
            return(0);
        }

        /* check for reliable packet loss and update trackers. 
           When processing a multi-packet, this will be called for each sub-packet that is not ignored in the
           statement above; however, because uMainPacketSeq is used for comparison, the updates will only occur 
           once per multi-packet as the statement will return false afterward. 
           This check also has a known limitation in that it will not catch the loss of packets which are lost 
           after they were re-sent due to a NAK being received */
        if (_CommUDPSeqnDiff(uMainPacketSeq, pRef->uReliableRcvSeq) > 0)
        {
            pRef->Common.packlost += (_CommUDPSeqnDiff(uMainPacketSeq, pRef->uReliableRcvSeq) - 1);
            pRef->uReliableRcvSeq = uMainPacketSeq;
        }
    ...
    if ((pPacket->body.uSeq & COMMUDP_SEQ_MASK) >= COMMUDP_RAW_PACKET_DATA)
    {
        pRef->uReliableExpectedSeq = _CommUDPSeqnDelta(pRef->uReliableExpectedSeq, 1);
...
static void _CommUDPProcessAlive(CommUDPRef *pRef, uint32_t uCurrTick)
{
    ...
    pRef->uRcvAck = pRef->uReliableExpectedSeq;
    Packet.body.uAck = _CommUDPSeqnDelta(pRef->uReliableExpectedSeq, -1);
    ...
    for (pRef = g_link; pRef != NULL; pRef = pRef->pLink)
    {
            ...
            (SockaddrCompare(&pRef->PeerAddr, &Sin) == 0) && (bConnIdentMatch))
        {
            ...
            else if (Packet.body.uSeq > COMMUDP_SEQ_MULTI_INC)
            {
                ...
                uint32_t uOldSeq = pRef->uReliableExpectedSeq;
                ...
                // remember we got something
                pRef->uRecvTick = Packet.head.uWhen;
                // save main packet sequence number for packet loss calculation
                uint32_t uMainPacketSeq = (Packet.body.uSeq & COMMUDP_SEQ_MASK);
                ...
                for (; iExtraSubPktCount >= 0; --iExtraSubPktCount)
                {
                    ...
                    if (_CommUDPProcessInput(pRef, &Multi, Packet.body.aData+ Packet.head.iLen, uCurrTick, uMainPacketSeq) < 0)
                    {
                        iExtraSubPktCount = 0;
                    }
                    // see if packets were saved
                    if ((iExtraSubPktCount > 0) && (uOldSeq != pRef->uReliableExpectedSeq))
                    {
                        pRef->Common.packsaved += 1;

                        #if COMM_PRINT
                        NetPrintf(("commudp: [%p] sub-packet %u was processed from main-packet %u\n", pRef, uOldSeq, uMainPacketSeq));
                        #endif
                        uOldSeq = pRef->uReliableExpectedSeq;
                    }
            ...
            else
            {
                ...
                _CommUDPProcessFlow(pRef, (RawUDPPacketHeadT *)&Packet, uCurrTick);
                // save the data
                _CommUDPProcessInput(pRef, (RawUDPPacketHeadT *)&Packet, Packet.body.aData, uCurrTick, Packet.body.uSeq);
            }
        ...
        if ((pRef->eState == OPEN) && (NetTickDiff(uCurrTick, pRef->uRecvTick) > pRef->iRecvTimeout) && (NetTickDiff(uCurrTick, pRef->uSendTick) < 2000))
        {
        ...
        if ((pRef->eState == OPEN) && (pRef->iSndNxt == pRef->iSndInp))
        {
            ...
            if (((reliabletimeout > BUSY_KEEPALIVE) && (pRef->uRcvAck != pRef->uReliableExpectedSeq)) ||
...
/*!
    ...
    \Notes
        ...
        \verbatim
            ...
            'time' - receive timeout (default = 120s)
...
int32_t CommUDPSend(CommUDPRef *pRef, const void *pBuffer, int32_t iLength, uint32_t uFlags)
{
    ...
    if (uFlags & COMM_FLAGS_UNRELIABLE)
    {
        ...
        pPacket->body.uAck = _CommUDPSeqnDelta(pRef->uReliableExpectedSeq, -1);
