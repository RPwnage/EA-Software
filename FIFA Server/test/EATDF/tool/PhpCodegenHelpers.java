...
public class PhpCodegenHelpers
{    
    ...
    // Writes out the constants from the map in php files organized by namespace.
    public static void generateConstantsFiles(String outputDirectory, HashMap<String, Vector<String>> constants)
    {
        ...
        while(it.hasNext()) {    
            ...
            try {
                ...
                constantsFile.mkdirs();
                outputFileName += "/Constants.php";
            ...
            try {
                fw.write("<?php " + NEWLINE + NEWLINE);
                
                fw.write("namespace " + constantEntry.getKey() + NEWLINE + "{" + NEWLINE + NEWLINE);

                fw.write("/**" + NEWLINE + " * Constants for the namespace " + constantEntry.getKey() + NEWLINE + " */" + NEWLINE);

                fw.write("class Constants" + NEWLINE + "{" + NEWLINE);
    ...
    /**
     * This main is called by the constants task defined in code_gen.xml
    ...
    public static void main(String[] args) throws Exception 
    {
        if(args.length < 2)
        {
            System.err.println("Missing arguments to PhpCodegenHelper main.");
            System.exit(-1);
        }
        else
        {
            ...
            while((filename = in.readLine()) != null) {
                PhpCodegenHelpers.consolidateConstants(filename, constantsMap);
            }

            PhpCodegenHelpers.generateConstantsFiles(outputDirectory, constantsMap);
        }
    }
}