...
#include "arson/rpc/arsonslave/bvgetrecord_stub.h"
...
namespace Arson
{
class BvGetRecordCommand : public BvGetRecordCommandStub
{
public:
    BvGetRecordCommand(
        Message* message, Blaze::ByteVault::GetRecordRequest* request, ArsonSlaveImpl* componentImpl)
        : BvGetRecordCommandStub(message, request),
        mComponent(componentImpl)
    {
    }

    ~BvGetRecordCommand() override { }

private:
    // Not owned memory
    ArsonSlaveImpl* mComponent;

    BvGetRecordCommandStub::Errors execute() override
    {
        ...
        {
            err = byteVault->getRecord(mRequest, mResponse);
        }
...
DEFINE_BVGETRECORD_CREATE()

BvGetRecordCommandStub::Errors BvGetRecordCommand::arsonErrorFromByteVaultError(BlazeRpcError error)
{
    ...
    switch (error)
    {
        ...
        default:
        {
            ...
            TRACE_LOG("[BvGetRecordCommand].arsonErrorFromByteVaultError: unexpected error(" << SbFormats::HexLower(error) << "): return as ERR_SYSTEM");
            result = ERR_SYSTEM;
            break;
        }
    };
