...
if [ $run_redis -eq 1 ]; then
  ...
  docker run -dit --net=host --name redis_$redisport redis:5.0 redis-server --port $redisport --cluster-enabled yes
...
if [ $authonly -ne 0 ] || [ ! -f $HOME/nant_credstore_keys.dat ]; then
  ...
  docker run $consoleopt --rm --net=host -u $devuid:$devgid -e HOME=$HOME -v $packageRoot:$packageRoot -v $HOME:$HOME $image mono $(ls -d $blazeRoot/Framework/*|head -n 1)/bin/eapm.exe credstore -user:$pkguser -password:"$pkgpasswd" -use-package-server
...
if [ $mount -ne 0 ] || [ -z "$libpath" ]; then
  ...
  echo "docker run $opts -v $packageRoot:$packageRoot -w $blazeRoot/blazeserver $image ./nant getpackageinfo"
  docker run $opts -v $packageRoot:$packageRoot -w $blazeRoot/blazeserver $image ./nant getpackageinfo
  result=$?
  if [ $mount -ne 0 ]; then
    if [ $result -ne 0 ]; then
...
if [ -z "$usercmd" ]; then
  echo "docker run $opts $useropts $workdir $image \"/bin/bash\""
  docker run $opts $useropts $workdir $image "/bin/bash"
else
  echo "docker run $opts $useropts $workdir $image $usercmd"
  docker run $opts $useropts $workdir $image $usercmd
fi
