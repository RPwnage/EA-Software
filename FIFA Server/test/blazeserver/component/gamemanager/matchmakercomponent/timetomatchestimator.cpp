...
namespace Matchmaker
{
        ...
        const GameManager::ScenariosConfig &scenariosConfig) const
    {
        ...
        if (scenarioIter != mScenarioTTMEstimates.end())
        {
            ...
            const auto ttmByPingSiteIter = ttmEstimate.mTimesToMatchByPingsiteGroup.find(pingSite);
            if (ttmByPingSiteIter != ttmEstimate.mTimesToMatchByPingsiteGroup.end())
            {
                const auto ttmByDGIter = ttmByPingSiteIter->second.find(delineationGroup);
                if (ttmByDGIter != ttmByPingSiteIter->second.end())
                {
                    ttmEstimateCopy = ttmByDGIter->second;
                    found = true;
                }
            }
    ...
    ***************************************************************************************************/
        ...
        const GameManager::ScenariosConfig &scenariosConfig, GameManager::MatchmakingResult result)
    {
        ...
        if (scenarioIter != mScenarioTTMEstimates.end())
        {
            ...
            updateTimeToMatchEstimate(scenarioTtmEstimate.mGlobalTimeToMatchEstimate, scenarioDuration, matchEstimateFalloffWindow, TimeValue::getTimeOfDay(), result);
            ...
            // ping-site update
            updateTimeToMatchEstimate(scenarioTtmEstimate.mTimesToMatchByPingsiteGroup[pingSite][delineationGroup], scenarioDuration, matchEstimateFalloffWindow, TimeValue::getTimeOfDay(), result);
        }
        else
        {
        ...
        updateNonScenarioTimeToMatch(scenarioDuration, pingSite, delineationGroup, scenariosConfig, result);
    ...
    void TimeToMatchEstimator::updateNonScenarioTimeToMatch(const TimeValue &matchmakingDuration, const PingSiteAlias &pingSite, const GameManager::DelineationGroupType& delineationGroup,
        const GameManager::ScenariosConfig &scenariosConfig, GameManager::MatchmakingResult result)
    {
        updateTimeToMatchEstimate(mGlobalTtmEstimate, matchmakingDuration, scenariosConfig.getGlobalMatchEstimateFalloffWindow(), TimeValue::getTimeOfDay(), result);
        updateTimeToMatchEstimate(mGlobalTTMEstimates[pingSite][""], matchmakingDuration, scenariosConfig.getGlobalMatchEstimateFalloffWindow(), TimeValue::getTimeOfDay(), result);
    }
    ...
    *************************************************************************************************/
    ...
    void TimeToMatchEstimator::initializeTtmEstimates(const GameManager::MatchmakingInstanceStatusResponse& matchmakingInstanceStatusResponse)
    {
        ...
        mGlobalTtmEstimate.mLastTimeToMatch = matchmakingInstanceStatusResponse.getGlobalTimeToMatchEstimateData().getGlobalEstimatedTimeToMatch();
        mGlobalTtmEstimate.mEstimatedTimeToMatch = matchmakingInstanceStatusResponse.getGlobalTimeToMatchEstimateData().getGlobalEstimatedTimeToMatch();
        mGlobalTtmEstimate.mLastDatapointTimestamp = matchmakingInstanceStatusResponse.getGlobalTimeToMatchEstimateData().getLastMatchTimestamp();

        // No good way to combine estimates when multiple groups are used:
        for (const auto& curPingSiteEstimate : matchmakingInstanceStatusResponse.getGlobalTimeToMatchEstimateData().getEstimatedTimeToMatchPerPingsiteGroup())
        {
        ...
        for (const auto& curScenario : matchmakingInstanceStatusResponse.getScenarioTimeToMatchEstimateData())
        {
