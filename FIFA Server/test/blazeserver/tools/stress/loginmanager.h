...
public:
    ...
    bool sendNucleusLoginRequest(const uint64_t id, const ClientPlatformType platformType, const bool connectToCommonServer, eastl::string& serverAuthCode);
    ...
    const uint16_t getNumberOfRetrys() const { return mNumberOfRetrys; }
...
private:
    ...
    bool readPidsFromFile(const char8_t* pidFilePath);

    bool requestAuthCode(const uint64_t id, const ClientPlatformType platformType, eastl::string& authCode);
    ...
    bool requestAccessToken(const uint64_t id, const ClientPlatformType platformType, const eastl::string& authCode, eastl::string& accessToken);
    bool requestOriginAccessToken(const uint64_t id, const ClientPlatformType platformType, eastl::string& accessToken);
...
private:
    ...
    char8_t mCommonServerAuthScope[256];
};
...
private:
    ...
    BlazeRpcError stressLogin();
    BlazeRpcError nucleusLogin();
};
