	...
	class TestRunner : IDisposable
	{
		...
		public int RunTests(string tests, string masterconfig)
		{
			...
			if (!String.IsNullOrEmpty(masterconfig))
			{
				...
				PackageMap.Init(project, m_globalMasterConfig);
				PackageMap.Instance.SetBuildRoot(project, m_buildRoot);
		...
		public void MergeWithGlobalMasterConfigFile(string localMasterconfigFilePath, string mergedMasterconfigPath, string testDir)
		{
			...
			if (!localMasterconfig.Config.Includes.IsNullOrEmpty()) includes = string.Join(" ", localMasterconfig.Config.Includes);
			if (!localMasterconfig.Config.Excludes.IsNullOrEmpty()) excludes = string.Join(" ", localMasterconfig.Config.Excludes);
			mergedMasterconfig.Config = new MasterConfig.ConfigInfo(
				localMasterconfig.Config.Package, 
				localMasterconfig.Config.Default,
				includes, excludes,
