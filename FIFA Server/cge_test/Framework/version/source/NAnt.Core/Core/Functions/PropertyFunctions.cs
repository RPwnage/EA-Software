...
namespace NAnt.Core.Functions
{
	...
	public class PropertyFunctions : FunctionClassBase
	{
		...
		/// <returns>True or False.</returns>
		[Function()]
		public static string PropertyEqualsAny(Project project, string propertyName, string comparands)
		{
			string property = project.Properties[propertyName];
			var values = comparands
				.Split(',')
				.Select(x => x.Trim(TRIM_CHARS));

			return (property != null && values.Contains(property)) ? bool.TrueString : bool.FalseString;
		}
		...
		{
			return project.Properties.Undefine(propertyName).ToString();
		}
