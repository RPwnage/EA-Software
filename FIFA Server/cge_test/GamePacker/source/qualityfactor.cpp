...
static const char8_t* PROPERTY_FIELD_VALUE_PARTICIPATION_TOO_LOW_TEXT = "Property field value participation is too low."; // no players define the value for the field being 
...
Score GameQualityFactor::activeEval(EvalContext& cxt, Game& game)
{
    ...
    const auto numParties = (uint32_t)game.getPartyCount(cxt.mGameState);
    ...
    for (uint32_t i = 0; i < numFields; ++i)
    {
        ...
        auto& fieldBlockItr = game.mFieldValueTable.mFieldValueColumnMap.at(fieldIndex);
        auto* fieldDescriptor = fieldBlockItr.second->mFieldRef.get();
        ...
        if (fieldBlockItr.second->mAddTableRefCount < numParties)
        {
        ...
        cxt.mCurFieldValueColumn = fieldBlockItr.second;
        ...
        auto score = INVALID_SCORE;

        if (fieldBlockItr.second->mSetValueBits.any())
        {
            // only evaluate game fields that have set values (value participation score is > 0%)
            score = mFactorSpec.mFactorTransform->activeEval(cxt, game);
        }
        else
        {
            // clamp field score to 0, because value particpation score is 0%
            score = MIN_SCORE;
...
Score GameQualityFactor::passiveEval(EvalContext& cxt, const Game& game) const
{
    ...
    for (uint32_t i = 0; i < numFields; ++i)
    {
        ...
        {
            // only evaluate game fields that have set values (value participation score is > 0%)
            score = mFactorSpec.mFactorTransform->passiveEval(cxt, game);
        }
        else
        {
            // clamp field score to 0, because value particpation score is 0%
        ...
        if (score != INVALID_SCORE)
        {
            ...
            {
                cxt.mViableFieldScoreList->push_back({ fieldDescriptor, score, fieldBlockItr.second->getValueParticipationScore(), (uint32_t)cxt.mViableFieldScoreList->size() });
            }
        }
    }
...
bool GameQualityFactor::computeSortedFieldScores(FieldScoreList& fieldScoreList, const Game& game, GameState state) const
{
