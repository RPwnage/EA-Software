...
/*F********************************************************************************/
/*!
    \Function _UserListApiDestroyInternal
...
*/
/********************************************************************************F*/
static int32_t _UserListApiDestroyInternal(void *pDestroyData)
{
    UserListApiRefT *pRef = (UserListApiRefT *)pDestroyData;
    uint32_t uUserIndex;

    // loop through the requests, return try again if any are still in progress
    for (uUserIndex = 0; uUserIndex < NETCONN_MAXLOCALUSERS; uUserIndex += 1)
    {
        UserListApiRequestT *pRequest = &pRef->aRequests[uUserIndex];

        // if the callback is null, the request entry does not represent a request
        if (pRequest->pNotifyCb == NULL)
        {
            continue;
        }

        // check the async status to see if any are pending
        ...
        // clean up the request if no longer pending
...
void UserListApiDestroy(UserListApiRefT *pRef)
{
    ...
    NetConnControl('recu', 0, 0, (void *)_UserListApiDestroyInternal, pRef);
}
...
int32_t UserListApiGetListAsync(UserListApiRefT *pRef, uint32_t uUserIndex, UserListApiTypeE eType, int32_t iLimit, int32_t iOffset, UserListApiFiltersT *pFilter, UserListApiCallbackT *pNotifyCb, uint32_t uTypeMask, void *pUserData)
{
    ...
    XblPresenceQueryFilters QueryFilters;
    UserListApiRequestT *pRequest = &pRef->aRequests[uUserIndex];
    ...
    // set the query filters based on the inputs
    ds_memclr(&QueryFilters, sizeof(QueryFilters));

    // if requesting rich presence we need to set the detail level to all
    if (uTypeMask & USERLISTAPI_MASK_RICH_PRESENCE)
    {
        QueryFilters.detailLevel = XblPresenceDetailLevel::All;
    }
    ...
    if (eType == USERLISTAPI_TYPE_FRIENDS)
    {
        ...
        {
            if (FAILED(hResult = XblPresenceGetPresenceForSocialGroupAsync(pRequest->pXblContext, pSocialGroup, NULL, &QueryFilters, &pRequest->PresenceBlock)))
            {
...
/*!
    ...
    \Notes
        You will still get callbacks for cancelled operations with an error as these
        cancellations happen asynchronously
...
void UserListApiCancelAll(UserListApiRefT *pRef)
{
    ...
    for (uUserIndex = 0; uUserIndex < NETCONN_MAXLOCALUSERS; uUserIndex += 1)
    {
        ...
        // clear the request if nothing was left pending
        if (!bCancelled)
        {
            _UserListApiCleanupRequestData(pRef, pRequest);
        }
