...
namespace GameManager
{
    ...
    public:
        ...
        {
            // Copy the new values into the old DEPRECATED locations:  (Remove this code when the deprecated values are killed.)
            if (request->getGameCreationData().getGameReportName()[0] != '\0')
            {
                request->setGameReportName(request->getGameCreationData().getGameReportName());
            }
