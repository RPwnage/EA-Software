...
// See ./framework/usersessions/db/mysql/usersessions.sql for details on the upsertUserInfo_v7() stored procedure.
#define CALL_UPSERT_USER_INFO \
    "CALL upsertUserInfo_v7(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
...
#define GET_USER_INFO_COMMON \
    ...
    "childaccount,originpersonaid,pidid,crossplatformopt,isprimarypersona from userinfo "
...
BlazeRpcError lookupUsersByIds(const BlazeIdVector& blazeIds, UserInfoPtrList& userInfoList, CountByPlatformMap& metricsCountMap, bool checkStatus, bool isRepLagAcceptable, ClientPlatformType platform)
{
    ...
    for (ClientPlatformSet::const_iterator itr = gController->getHostedPlatforms().begin(); itr != gController->getHostedPlatforms().end(); ++itr)
    {
        if ((platform != INVALID) && (platform != NATIVE) && (platform != *itr))
        {
            // If a specific (non INVALID/NATIVE) platform specified, ensure only that platform specific userInfo (e.g. w/last login time, etc) is returned.
            // E.g. the same XBL account/BlazeId can be used on xone or xbsx (at diff times), so login flow passes current client's platform here.
            continue;
        }
        metricsCountMap[*itr] += blazeIdSet.size();

        BlazeRpcError err = lookupUsersByIds(blazeIdSet, userInfoList, *itr, checkStatus, isRepLagAcceptable, &cachedPlatformInfo);
        if (blazeIdSet.empty())
...
BlazeRpcError lookupLatLonById(BlazeId blazeId, GeoLocationData& geoLocationData)
{
    ...
    for (ClientPlatformSet::const_iterator itr = gController->getHostedPlatforms().begin(); itr != gController->getHostedPlatforms().end(); ++itr)
    {
        BlazeRpcError err = lookupLatLonById(blazeId, *itr, geoLocationData);
        if (err == ERR_OK)
            return err;
...
Blaze::BlazeRpcError lookupCrossPlatformOptInById(BlazeId blazeId, ClientPlatformType platform, bool &crossPlatformOptIn)
{
    BlazeRpcError err = ERR_SYSTEM;
...
BlazeRpcError lookupUsersByPersonaNamePrefix(const char8_t* personaNamespace, const char8_t* personaName, uint32_t maxResultCount, UserInfoPtrList& userInfoList)
{
    ...
    for (ClientPlatformSet::const_iterator itr = gController->getHostedPlatforms().begin(); itr != gController->getHostedPlatforms().end(); ++itr)
    {
        if (blaze_strcmp(personaNamespace, gController->getNamespaceFromPlatform(*itr)) != 0)
            continue;
        ...
        BlazeRpcError err = lookupUsersByPersonaNamePrefix(personaNamespace, personaName, *itr, maxResultCount, userInfoList, &cachedPlatformInfo);
        if (err != ERR_OK)
            retError = err;
        else
...
BlazeRpcError lookupUsersByPersonaNamePrefixMultiNamespace(const char8_t* personaName, uint32_t maxResultCount, UserInfoPtrList& userInfoList)
{
    ...
    for (ClientPlatformSet::const_iterator itr = gController->getHostedPlatforms().begin(); itr != gController->getHostedPlatforms().end(); ++itr)
    {
        BlazeRpcError err = lookupUsersByPersonaNamePrefixMultiNamespace(personaName, *itr, maxResultCount, userInfoList, &cachedPlatformInfo);
        if ((uint32_t)userInfoList.size() >= maxResultCount)
            break;
    ...
    if ((uint32_t)userInfoList.size() >= maxResultCount && platformCount < gController->getHostedPlatforms().size())
    {
        BLAZE_TRACE_LOG(Log::USER, "[UserInfoDbCalls].lookupUsersByPersonaNamePrefixMultiNamespace: returned " << userInfoList.size() << " results from " << platformCount
            << " out of " << gController->getHostedPlatforms().size() << " hosted platforms.");
        return ERR_OK;
    }
    return retError;
}
...
BlazeRpcError lookupUsersByPersonaNames(const char8_t* personaNamespace, const PersonaNameVector& names, UserInfoPtrList& userInfoList, bool checkStatus)
{
    ...
    for (ClientPlatformSet::const_iterator itr = gController->getHostedPlatforms().begin(); itr != gController->getHostedPlatforms().end(); ++itr)
    {
        ...
        BlazeRpcError err = lookupUsersByPersonaNames(personaNamespace, names, *itr, userInfoList, checkStatus, &cachedPlatformInfo);
        if (err != ERR_OK)
            retError = err;
    }
    return retError;
...
BlazeRpcError lookupUsersByAccountIds(const AccountIdVector& accountIds, UserInfoPtrList& userInfoList)
{
    ...
    for (ClientPlatformSet::const_iterator itr = gController->getHostedPlatforms().begin(); itr != gController->getHostedPlatforms().end(); ++itr)
    {
        BlazeRpcError err = lookupUsersByAccountIds(accountIds, *itr, userInfoList, &cachedPlatformInfo);
        if (err != ERR_OK)
            retError = err;
    }
    return retError;
...
BlazeRpcError lookupUsersByOriginPersonaIds(const OriginPersonaIdVector &originPersonaIds, UserInfoPtrList& userInfoList, bool checkStatus)
{
    ...
    for (ClientPlatformSet::const_iterator itr = gController->getHostedPlatforms().begin(); itr != gController->getHostedPlatforms().end(); ++itr)
    {
        BlazeRpcError err = lookupUsersByOriginPersonaIds(originPersonaIds, *itr, userInfoList, checkStatus, &cachedPlatformInfo);
        if (err != ERR_OK)
            retError = err;
    }
    return retError;
...
BlazeRpcError updateUserInfoAttribute(const BlazeIdVector& blazeIds, UserInfoAttribute attribute, UserInfoAttributeMask mask)
{
    ...
    if (foundUnknownUser && totalRowsUpdated < blazeIds.size())
    {
        for (ClientPlatformSet::const_iterator itr = gController->getHostedPlatforms().begin(); itr != gController->getHostedPlatforms().end(); ++itr)
        {
            if (platformSet.find(*itr) == platformSet.end())
            {
                uint32_t rowsUpdated = 0;
                BlazeRpcError err = updateUserInfoAttribute(blazeIds, *itr, attribute, mask, rowsUpdated);
                if (err == ERR_OK)
...
BlazeRpcError updateOriginPersonaName(AccountId nucleusAccountId, const char8_t* originPersonaName, const char8_t* canonicalPersonaName)
{
    ...
    for (ClientPlatformSet::const_iterator itr = gController->getHostedPlatforms().begin(); itr != gController->getHostedPlatforms().end(); ++itr)
    {
        ClientPlatformType platform = *itr;
