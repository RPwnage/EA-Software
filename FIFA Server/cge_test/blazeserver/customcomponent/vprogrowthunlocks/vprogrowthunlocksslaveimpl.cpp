...
void VProGrowthUnlocksSlaveImpl::updatePlayerGrowth(BlazeId userId, Blaze::VProGrowthUnlocks::MatchEventsMap &matchEventsMap, BlazeRpcError &outError)
{
	...
	TRACE_LOG("[VProGrowthUnlocksSlaveImpl:" << this << "].updatePlayerGrowth: userId: " << userId
		<< ", matchEventsMap.size(): " << matchEventsMap.size() << ", xpReward: " << xpReward << ", xpEarned: " << xpEarned
...
void VProGrowthUnlocksSlaveImpl::grantMatchRatingXP(BlazeId userId, uint32_t matchCount, uint32_t matchRating, Blaze::VProGrowthUnlocks::AttributeTypeList attributeTypeList, BlazeRpcError &outError)
{
	TRACE_LOG("[VProGrowthUnlocksSlaveImpl:" << this << "].grantMatchRatingXP for user: " << userId);
	if (matchCount >= 1 && attributeTypeList.size() > 0)
	{
		Blaze::Stats::UpdateStatsHelper updateStatsHelper;
		Blaze::Stats::UpdateStatsRequestBuilder builder;
		Stats::ScopeNameValueMap playerKeyScopeMap;

		uint32_t baseRatingReward = 0;
		uint32_t actualRatingReward = 0;
		Blaze::VProGrowthUnlocks::MatchRatingXPRewardsMap::const_iterator ratingRewardIter = mMatchRatingXPRewardsMap.find(matchRating);
		if (ratingRewardIter != mMatchRatingXPRewardsMap.end())
		{
			baseRatingReward = ratingRewardIter->second;			

			for (uint32_t i = 0; i < attributeTypeList.size(); i++)
			{
				playerKeyScopeMap.clear();
				playerKeyScopeMap.insert(eastl::make_pair(KEYSCOPE_NAME_VPRO_ATTRIBUTE_TYPE, i));

				uint32_t typeIndex = 0;
				if (attributeTypeList[i] < Blaze::VProGrowthUnlocks::NUM_ATTRIBUTE_TYPE)
				{
					typeIndex = attributeTypeList[i];
				}
				actualRatingReward = baseRatingReward * mModifier[typeIndex] / 100 * matchCount;

				builder.startStatRow(VPRO_ATTRIBUTE_STATS_CATEGORY, userId, &playerKeyScopeMap);
				builder.incrementStat(VPRO_ATTRIBUTE_XPEARNED_COLUMN, actualRatingReward);
				builder.completeStatRow();
			}
