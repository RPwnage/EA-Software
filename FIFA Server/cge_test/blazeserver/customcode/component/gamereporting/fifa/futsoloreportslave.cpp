...
void FutSoloReportSlave::updateCustomNotificationReport()
{
	...
	char8_t hash[Fifa::HASH_MAX_LENGTH + 1] = ""; // size needs to match Blaze::HashUtil::SHA512_HEXSTRING_OUT
	FifaHMAC* hmac = FifaHMAC::Instance();
	if(hmac != NULL)
	{
		hmac->GenerateHMACFromGameReport(mProcessedReport->getGameReport(), hash);
	}

	gameCustomData->setMatchHash(hash);
