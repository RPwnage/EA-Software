	...
	> eapm post build/Config-0.8.0.zip -usev1";
		...
		internal override void Execute()
		{
			...
			if (useV1 == false)
			{
				...
				var tempDirectory = Path.Combine(Path.GetTempPath(), "eapmTemp");				
				var directory = Directory.CreateDirectory(Path.Combine(tempDirectory, packageName, packageVersion));
				directory.Attributes &= ~FileAttributes.ReadOnly;
				zipLib.UnzipSpecificFile(path, tempDirectory, $"{packageName}/{packageVersion}/Manifest.xml");
				var manifest = Path.Combine(new[] { tempDirectory, packageName, packageVersion , "Manifest.xml" });
				if (!File.Exists(manifest))
				{
					//zip lib will find it regardless of case but file exists requires specific casing
					manifest = Path.Combine(new[] { tempDirectory, packageName, packageVersion, "manifest.xml" });
					if (!File.Exists(manifest))
					{
						throw new ApplicationException(
							$"Package could not be posted, manifest does not exist at {manifest}");
					}
				}
