...
namespace Microsoft.Build.Tasks.Windows
{
    public sealed class ResourcesGenerator : Task
    {
        ...
        {
			// additional checking for known ea out-of-source errors
			IEnumerable<string> duplicates = ResourceFiles.Select(ti => ti.ItemSpec).GroupBy(s => s).Where(grp => grp.Count() > 1).Select(grp => grp.First());
			if (duplicates.Any())
			{
				Log.LogError("Duplicate resources as input: {0}!", String.Join(", ", duplicates));
				return false;
			}
