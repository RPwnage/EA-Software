			...
			out string outDependencyData)
		{
			...
			foreach (string line in inputStdout.Split(new char[] { '\n' }))
			{
				if (line.StartsWith("Note: including file:"))
				{
					...
					string formattedIncludeLine = NAnt.Core.Util.PathString.MakeNormalized(includeLine).Path.ToLower();
					if (!includes.Contains(includeLine))
					{
						includes.Add(includeLine);
						includeDependencyContent.Append(" \\" + Environment.NewLine + "  " + includeLine);
					}
					continue;
				}
