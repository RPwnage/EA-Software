...
{
	internal abstract partial class VSDotNetProject : VSProjectBase
	{
		...
		{
			return string.Join(";", ConfigPlatformMap.Keys.Select(x => x.Name).Distinct());
		}
		...
		protected override void WriteProject(IXmlWriter writer)
		{
			if (UseNewCSProjFormat())
			{
				...
				*/
				...
				foreach (IModule module in Modules)
				{
					...
					writer.WriteAttributeString("Condition", $" '$(Configuration)|$(Platform)' == '{GetVSProjConfigurationName(module.Configuration)}' ");
			...
			WriteCopyLocalFiles(writer, isNativeBuild: false);  // copy local files write must come after other file writes, other file writes can mark certain copy local operations
		...
		{
			// NUGET-TODO, ensure conistency across configs
			...
			// only enabling the use of useWPF when using .net core since that is what it is being tested with
			// and it is causing a bunch of errors when building frosted when .net core is not being used
			string sdk = (TargetFrameworkFamily != DotNetFrameworkFamilies.Framework && (useWPF || useWindowsForms)) ?
		...
		private void WriteNugetImportFile(IModule module, IEnumerable<string> importPaths, string fileExtension)
		{
			if (importPaths.Any())
			{
				...
				{
					string franeworkNugetImports = GetProjectPath(Path.Combine(module.GetVSTmpFolder().Path, $"{ProjectFileName}.framework.g.{fileExtension.TrimStart('.')}"));

					nugetPropsWriter.CacheFlushed += new CachedWriterEventHandler(OnProjectFileUpdate);
					nugetPropsWriter.FileName = franeworkNugetImports;
		...
		protected virtual IDictionary<string, string> GetProjectProperties()
		{
			...
			if (UseNewCSProjFormat())
			{
				...
				data.AddNonEmpty("TargetFramework", ConvertToNewTargetFramework(TargetFrameworkVersion));
		...
		class AssemblyReferenceContext
		{
			public string Condition;
			public bool IsCopyLocal;
			public string RefName;
			public string RefNamespace;
			public string RefHintPath;

			public AssemblyReferenceContext(string condition, bool isCopyLocal, string refHintPath, string refName, string refNamespace)
			{
				Condition = condition;
				IsCopyLocal = isCopyLocal;
				RefHintPath = refHintPath;
				RefName = refName;
				RefNamespace = refNamespace;
			}
		}

		static void AddAssemblyAttributes(IXmlWriter writer, OptionSet options, AssemblyReferenceContext context)
		{
			Dictionary<string, string> assemblyAttributes = options.GetOptionDictionary("visual-studio-assembly-attributes");

			if (assemblyAttributes == null)
			{
				assemblyAttributes = new Dictionary<string, string>();
			}

			if (!assemblyAttributes.ContainsKey("Condition"))
			{
				assemblyAttributes.AddNonEmpty("Condition", context.Condition);
			}

			if (!assemblyAttributes.ContainsKey("Include"))
			{
				assemblyAttributes.Add("Include", context.RefNamespace);
			}
		...
		static void AddAssemblyProperties(IXmlWriter writer, OptionSet options, AssemblyReferenceContext context)
		{
			...
			if (!assemblyProperties.ContainsKey("Name"))
			{
				assemblyProperties.Add("Name", context.RefName);
			}

			if (!assemblyProperties.ContainsKey("HintPath"))
			{
				assemblyProperties.Add("HintPath", context.RefHintPath);
			}
			...
			if (!assemblyProperties.ContainsKey("Private"))
			{
				...
				// -dvaliant 2016/04/04
				/* above value used to be  context.IsCopyLocal.ToString() rather than false - keeping this comment in case we find we 
				need to go back to this copy local mechanism. If this has been stable for a while, this comment can be removed */
			}
		...
		private void WriteReference(IXmlWriter writer, FileEntry reference, string conditionString)
		{
			...
			if (options == null)
			{
				writer.WriteNonEmptyAttributeString("Condition", conditionString);
				...
				if (IsTargetingNetCoreStandard())
				{
					writer.WriteElementString("Private", true); // set copylocal to true in order to get VS to add this to the .deps.json for .net core project
					// we stomp the target that actually trys to do the copy and replace it with our own
				}
				else
				{
					writer.WriteElementString("Private", false); 
					//isCopyLocal.ToString()); */ // TODO visual studio copy local
					// TODO visual studio copy local
					// -dvaliant 2016/04/04
					/* above value used to be isCopyLocal.ToString() rather than false - keeping this comment in case we find we 
					need to go back to this copy local mechanism. If this has been stable for a while, this comment can be removed */
				}
			}
			else
			{
				AssemblyReferenceContext context = new AssemblyReferenceContext(conditionString, isCopyLocal, refHintPath, refName, refNamespace);

				AddAssemblyAttributes(writer, options, context);
				AddAssemblyProperties(writer, options, context);
			}
		...
		{
			// This method is replaced with the nuget manager package flattening code (in ModuleProcessor_SetModuleData) but I haven't deleted it yet because I want Dave to check it :)
			return;

			/*if (StartupModule.NuGetReferences != null)
			{
				writer.WriteStartElement("ItemGroup");
				foreach (var reference in StartupModule.NuGetReferences.FileItems)
				{
					writer.WriteStartElement("PackageReference");
					writer.WriteAttributeString("Include", reference.FileName);
					writer.WriteAttributeString("Version", PackageMap.Instance.MasterConfig.GetNuGetPackageVersion(reference.FileName, moduleName: StartupModule.Package.Name + "/" + StartupModule.Name));
					writer.WriteEndElement(); //None
				}
				writer.WriteEndElement(); //ItemGroup
			}*/
				...
				if (group.Count() > 0)
				{
					...
					foreach (FileEntry fe in group)
					{
						...
						if (!firstFe.IsKindOf(FileEntry.ExcludedFromBuild))
						{
							...
							{
								bool addttService;
								WriteResourceFile(writer, fe, projectPath, relPathKey, resHash, srcHash, xomlHash, out addttService);
								if (addttService)
								{
		...
		protected virtual void WriteXamlFile(IXmlWriter writer, FileEntry fe, string projectPath)
		{
		...
		protected void WriteXomlFile(IXmlWriter writer, FileEntry fe, string projectPath)
		{
			string filename = fe.Path.Path;
			if (File.Exists(filename))
			{
		...
		protected void WriteResourceFile(IXmlWriter writer, FileEntry fe, string relPath, string relPathKey, IDictionary<string, string> resHash, IDictionary<string, string> srcHash, IDictionary<string, string> xomlHash, out bool addttService)
		{
			string filename = fe.Path.Path;
		...
		protected void WriteSourceFile(IXmlWriter writer, FileEntry fe, string projectPath, string relPathKey, IDictionary<string, string> resHash, IDictionary<string, string> srcHash, IDictionary<string, string> xomlHash, IDictionary<string, string> xamlHash, IDictionary<string, string> ttHash)
		{
			string filename = fe.Path.Path;
		...
		protected void ComputeCodeBehindCache(IEnumerable<FileEntry> fentries, out IDictionary<string, string> srcHash, out IDictionary<string, string> resHash, out IDictionary<string, string> xamlHash, out IDictionary<string, string> xomlHash, out IDictionary<string, string> ttHash)
		{
	...
	<BootstrapperPackage Include="Microsoft.Net.Framework.3.5">
	  ...
	  <Install>false</Install>
		...
		{
			if (Package.Project.Properties.GetBooleanPropertyOrDefault("eaconfig.sln.use-xaml-compile-override", true))
			{
		...
		protected IEnumerable<IDotNetVisualStudioExtension> GetExtensionTasks(ProcessableModule module)
		{
			...
			foreach (var extensionTaskName in extensionTaskNames)
			{
				...
				else if (task != null && (task as IMCPPVisualStudioExtension) == null)
				{
				...
				foreach (IModule module in Modules)
				{
					...
					if (mapEntry.Count() > 0 && ConfigurationMap.TryGetValue(mapEntry.First().Key, out Configuration dependentConfig))
					{
						...
						if (depModule != null)
						{
							...
							uint flags = 0;
							if ((depModule.IsKindOf(Module.DotNet) || depModule.IsKindOf(Module.Managed)) && moduleDependency.IsKindOf(DependencyTypes.Link))
							{
		...
		protected virtual void ParseTextTemplate(FileEntry fe, out string ext, out string action, out OptionSet options, out bool addService)
		{
