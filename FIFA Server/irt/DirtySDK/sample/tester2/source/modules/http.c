...
*/
/********************************************************************************F*/
static int32_t _HttpCustomHeaderCallback(ProtoHttpRefT *pProtoHttp, char *pHeader, uint32_t uHeaderSize, void *pUserRef)
{
    int32_t iOffset;

    NetPrintf(("http: custom header callback, size=%d\n", uHeaderSize));
...
int32_t CmdHttp(ZContext *argz, int32_t argc, char *argv[])
{
    ...
    else if ((argc == 3) && !strcmp(argv[1], "spas"))
    {
        ds_strnzcpy(pRef->strPass, argv[2], sizeof(pRef->strPass));
        return(iResult);
    }
            ...
            ((argc > 2) && (argc < 6)))
    {
        ...
        if (strlen(pRef->strPass) > 0)
        {
            ProtoHttpControl(pRef->http, 'spas', (signed)strlen(pRef->strPass), 0, pRef->strPass);
        }
