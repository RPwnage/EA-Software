        ...
        if (pStream->GetDataSize() != 0)
        {
            ...
            if (iReadLen != 0)
            {
                ...
                if (pVoipNarrate->bFirstSynthOfPhrase == TRUE)
                {
                    ...
                    pVoipNarrate->pVoiceDataCb(pVoipNarrate, 0, (const int16_t *)buffer, VOIPNARRATE_STREAM_START, pVoipNarrate->uFlags, pVoipNarrate->pUserData);
                }
                pVoipNarrate->pVoiceDataCb(pVoipNarrate, 0, (const int16_t *)buffer, iReadLen, pVoipNarrate->uFlags, pVoipNarrate->pUserData);
            }
        }
        else
        {
            ...
            // signal stream end
            pVoipNarrate->pVoiceDataCb(pVoipNarrate, 0, (const int16_t *)buffer, VOIPNARRATE_STREAM_END, pVoipNarrate->uFlags, pVoipNarrate->pUserData);
            pVoipNarrate->eVoiceState = VOIPNARRATE_VOICE_READY;
        }
    }
}
