...
/*!
    ...
    \Input iOutputLen   - size of output buffer; may be zero if pOutput is NULL
...
static int32_t _Base64Decode(const char *pInput, int32_t iInputLen, char *pOutput, int32_t iOutputLen, const signed char *pDecode)
{
    ...
    {
        // scan input
        for (iInputCnt = 0; (iInputCnt < 4) && (iInputOff < iInputLen) && (pInput[iInputOff] != '\0') && ((iOutputOff < iOutputLen) || (pOutput == NULL)); iInputOff += 1)
        {
            // ignore whitespace
...
/*!
    ...
    \Notes
        pOutput is NUL terminated, so iOutputLen must be large enough
        to hold Base64EncodedSize(iInputLen)+1 bytes.
