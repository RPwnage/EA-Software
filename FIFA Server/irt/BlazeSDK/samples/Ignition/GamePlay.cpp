...
#include <xsapi-c/services_c.h> //for XblMultiplayerSendInvitesResult, XAsyncBlock in actionSendSessionInvite
#include <XGame.h> //for XGameGetXboxTitleId() in actionSendSessionInvite()
#include "xboxgdk/GdkInvitesSample.h" //for getXblContext in actionSendSessionInvite
#endif
...
void GamePlay::initActionSendSessionInvite(Pyro::UiNodeAction &action)
{
    ...
    action.setText("Send Invite via XDK");
    action.setDescription("Opens the session invitation dialog.");
...
void GamePlay::actionSendSessionInvite(Pyro::UiNodeAction *action, Pyro::UiNodeParameterStructPtr parameters)
{
    if (mGame != nullptr)
    {
        ...
        if (!SUCCEEDED(hr))
        {
            NetPrintf(("actionSendSessionInvite: XGameGetXboxTitleId failed: %d\n", hr));
            return;
        }
        ...
        blaze_strnzcpy(sessionReference.Scid, mGame->getScid(), strlen(mGame->getScid()) + 1);
        blaze_strnzcpy(sessionReference.SessionName, mGame->getExternalSessionName(), strlen(mGame->getExternalSessionName()) + 1);
        blaze_strnzcpy(sessionReference.SessionTemplateName, mGame->getExternalSessionTemplateName(), strlen(mGame->getExternalSessionTemplateName()) + 1);

        // setup and send the invite
        auto asyncBlock = std::make_unique<XAsyncBlock>();
        asyncBlock->queue = nullptr; //this sample isn't using
        ...
        asyncBlock->callback = [](XAsyncBlock* asyncBlock)
        {
            ...
            if (!SUCCEEDED(hr2))
            {
                NetPrintf(("actionSendSessionInvite: XblMultiplayerSendInvitesResult failed: %d\n", hr2));
            }
            else
            {
                NetPrintf(("actionSendSessionInvite: XblMultiplayerSendInvitesResult: inviteHandle(%.*s)\n", sizeof(handles[0].Data), handles[0].Data));
            }
        };
        uint64_t xuids[1] = {};
        xuids[0] = extId;
        ...
        {
            NetPrintf(("actionSendSessionInvite: XblMultiplayerSendInvitesAsync failed: %d\n", hr));
        }
#else
        try
        {
