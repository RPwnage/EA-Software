...
private:
    ...
    BlazeRpcError doExpressLogin(ClientPlatformType clientPlatform, ClientType clientType, const char8_t* productName)
    {
        ...
        {
            INFO_LOG("[ExpressLoginCommand].execute : Email is not white-listed.");
        ...
        err = AuthenticationUtil::authFromOAuthError(mComponent->oAuthSlaveImpl()->getAccessTokenByPassword(mRequest.getEmail(), mRequest.getPassword(), emailTokenResponse, productName, inetAddr));
        if (err != ERR_OK)
        {
            WARN_LOG("[ExpressLoginCommand].doExpressLogin: getAccessTokenByUser failed with error[" << ErrorHelp::getErrorName(err) << "]");
        ...
        if (personaLogin)
        {
            ...
            err = AuthenticationUtil::authFromOAuthError(mComponent->oAuthSlaveImpl()->getAuthCodeForPersona(mRequest.getPersonaName(), namespaceName, emailTokenResponse.getAccessToken(), productName, resp));
            if (err == ERR_OK)
            {
                ...
                err = AuthenticationUtil::authFromOAuthError(mComponent->oAuthSlaveImpl()->getAccessTokenByAuthCode(authCode, personaTokenResponse, productName, inetAddr));
                if (err != ERR_OK)
                {
                    WARN_LOG("[ExpressLoginCommand].doExpressLogin: getAccessTokenByUser failed with error[" << ErrorHelp::getErrorName(err) << "]");
                    if (err == AUTH_ERR_INVALID_TOKEN)
        ...
        {
            ERR_LOG("[ExpressLoginCommand].doExpressLogin: nucleusidentity is nullptr");
            return ERR_SYSTEM;
        }
        ...
        if (mPersonaId == INVALID_BLAZE_ID)
        {
            if (personaLogin)
            {
                ...
                if (err != Blaze::ERR_OK || personaListResponse.getPersonas().getPersona().empty())
                {
    ...
    ExpressLoginCommandStub::Errors execute() override
    {
        ...
        if (err != Blaze::ERR_OK)
        {
            ...
            gUserSessionMaster->generateLoginEvent(err, getUserSessionId(), USER_SESSION_NORMAL, platformInfo, mDeviceId.c_str(), mDeviceLocality,
               ...
               addrStr, serviceName, geoIpData, mComponent->getProjectId(productName), LOCALE_BLANK, 0, clientVersion, false, mIsUnderage);
        }
        else
        {
