    ...
    namespace GameManager
    {
        ...
        ***************************************************************************************************/
            ...
            const XBLServices::ArenaTeamParamsList* teamParams, Blaze::ExternalSessionResult* result, bool commit)
        {
            ...
            ExternalSessionUpdateInfo::PresenceModeByPlatformMap::const_iterator presenceIt = params.getUpdateInfo().getPresenceModeByPlatform().find(mPlatform);
            PresenceMode presenceMode = (presenceIt == params.getUpdateInfo().getPresenceModeByPlatform().end()) ? PRESENCE_MODE_STANDARD : presenceIt->second;
            ...
            putRequest.getBody().getConstants().getSystem().setVisibility(toVisibility(presenceMode));
            ...
            TRACE_LOG("[ExternalSessionUtilXboxOne].create: " << (commit? "" : "simulate ") << "creating new MPS(" << ExternalSessionTypeToString(externalSessionType) << ", id=" << externalSessionId << ", sessionName: " << sessionName <<
                ", Visibility=" << toVisibility(presenceMode) << ", Closed=" << toClosed(params.getUpdateInfo()) << ", JoinRestriction=" << toJoinRestriction(params.getUpdateInfo()) << ", customData=" << custData.c_str() << ") with xuid " << xblId << ". Scid: " << mConfig.getScid() << ", SessionTemplateName: " << sessionTemplateName << ".");
        ...
        ***************************************************************************************************/
        ...
        Blaze::BlazeRpcError ExternalSessionUtilXboxOne::getExternalSession(const char8_t* sessionTemplateName, const char8_t* sessionName, XBLServices::MultiplayerSessionResponse& result)
        {
            Blaze::XBLServices::XBLClientSessionDirectorySlave * xblClientSessionDirectorySlave = (Blaze::XBLServices::XBLClientSessionDirectorySlave *)Blaze::gOutboundHttpService->getService(Blaze::XBLServices::XBLClientSessionDirectorySlave::COMPONENT_INFO.name);
            if (xblClientSessionDirectorySlave == nullptr)
            {
        ...
        ***************************************************************************************************/
        const char8_t* ExternalSessionUtilXboxOne::toVisibility(PresenceMode presenceMode) const
        {
            switch (presenceMode)
            {
        ...
        ***************************************************************************************************/
        ...
        {
            if (updateInfo.getSettings().getOpenToJoinByPlayer())
                return "none";
        ...
        Blaze::BlazeRpcError ExternalSessionUtilXboxOne::getProfiles(const ExternalIdList& externalIds, ExternalUserProfiles& buf, const char8_t* groupAuthToken)
        {
            ...
            for (ExternalIdList::const_iterator itr = externalIds.begin(); itr != externalIds.end(); ++itr)
            {
                if (EA_LIKELY(*itr != INVALID_EXTERNAL_ID))
                    request.getGetExternalUserProfilesRequestBody().getExternalUserIds().push_back(xuidBuf.sprintf("%" PRIu64, *itr).c_str());
            }
            ...
            if (err != ERR_OK)
            {
                ERR_LOG("[ExternalUserProfileUtilXboxOne].getProfiles: Failed to get external user profiles, error: " << ErrorHelp::getErrorName(err)
                    << ". Raw response: " << eastl::string(reinterpret_cast<const char8_t*>(rspRaw->data()), rspRaw->datasize()).c_str());
                return err;
            }
