...
@members
{
    ...
    void fixupTdfId(HashTree node, boolean isClient)
    {
        ...
        // All Tdfs on the server will at least have a hash to remove server-side restriction 
        // for setting only explicitly registered Tdfs on variable tdf members.
        if ((val == null && !isClient) || 
            (val != null && val.equalsIgnoreCase("hash")))
        {
            symbol.put("tdfid", fullClassName.hashCode());
        }
    }
...
bitfield_block
        :       ^(BITFIELD attributes[$BITFIELD]?  bitfield_member*) -> ^(BITFIELD bitfield_member*) 
        ;
...
enum_block
        : ^(ENUM attributes[$ENUM]? enum_member*) -> ^(ENUM enum_member*)
        ;
...
non_collection_type  returns [TdfSymbol symbol]
        ...
        |   BLAZE_OBJECT_TYPE
            { 
                TdfSymbol scope = $SymbolScope[0]::Scope.getOrAddSymbol("Blaze", TdfSymbol.SymbolType.TYPE, TdfSymbol.Category.NAMESPACE, null, null);
                $symbol = scope.getOrAddSymbol($text, TdfSymbol.SymbolType.TYPE, TdfSymbol.Category.BLAZE_OBJECT_TYPE, null, null); 
            }
        |   BLAZE_OBJECT_ID
            { 
                TdfSymbol scope = $SymbolScope[0]::Scope.getOrAddSymbol("Blaze", TdfSymbol.SymbolType.TYPE, TdfSymbol.Category.NAMESPACE, null, null);
                $symbol = scope.getOrAddSymbol($text, TdfSymbol.SymbolType.TYPE, TdfSymbol.Category.BLAZE_OBJECT_ID, null, null); 
            }
        |   BLAZE_OBJECT_PRIMITIVE 
            { 
                TdfSymbol scope = $SymbolScope[0]::Scope.getOrAddSymbol("Blaze", TdfSymbol.SymbolType.TYPE, TdfSymbol.Category.NAMESPACE, null, null);
                $symbol = scope.getOrAddSymbol($text, TdfSymbol.SymbolType.TYPE, TdfSymbol.Category.INT_PRIMITIVE, null, null); 
            }
        |   TIMEVALUE
            {
                TdfSymbol scope = $SymbolScope[0]::Scope.getOrAddSymbol("Blaze", TdfSymbol.SymbolType.TYPE, TdfSymbol.Category.NAMESPACE, null, null);
                $symbol = scope.getOrAddSymbol($text, TdfSymbol.SymbolType.TYPE, TdfSymbol.Category.TIMEVALUE, null, null);
            }
