...
var CONST_GET_SESSION_TEMPLATE_RESPONSE = {
    "fixed": { "constants": { "system": { "maxMembersCount": CONST_SESSIONTEMPLATE_MAXMEMBERSCOUNT } } },
...
function sessions_get_response(sessionName, response) {
    try {
...
function sessions_put_response(scid, templateName, sessionName, requestHeaders, requestBodyJSON, response, noCommit) {
    try {
        ...
        var mps = sessions_get_response(sessionName, response);
        ...
        if (!mpsExisted && !isLeaveRequest) {
            ...
            if (!addMembersToMultiplayerSession(scid, templateName, sessionName, requestHeaders, requestBodyJSON.members, mps, response)) {
                return "";//rsp status set
            }
            ...
            origSize = util.count(mps.members);
            var endCount = remMembersFromMultiplayerSession(sessionName, requestBodyJSON.members, mps);
        ...
        if (mpsExisted && hasReqMemberPropUpdates) {
            ...
            var result = doReqMemberPropertyUpdates(requestHeaders, requestBodyJSON.members, mps, response);
            if (result !== "") {
...
function addMembersToMultiplayerSession(scid, templateName, sessionName, requestHeaders, reqMembers, mps, response) {
    try {
        for (var reqMemberKey in reqMembers) {
            ...
            if (existingMember !== "") {
                // if not claiming a reservation, somehow Blaze is doing redundant call here. XBL would not actually return/log error, but to detect excess Blaze calls, log it
                if ((reqMemberKey.substr(0, "reserve_".length) !== "reserve_") || existingMember.properties.system.active === true) {
...
function remMembersFromMultiplayerSession(sessionName, reqMembers, mps) {
    try {
        for (var reqMemberKey in reqMembers) {
...
function getXuidForMemberInPutRequest(requestHeaders, reqMembers, reqMemberKey, response) {
    try {
        ...
        if (isMeKey && (util.isUnspecifiedNested(requestHeaders, 'authorization'))) {
        ...
        if ((isMeKey? requestHeaders.authorization : reqMembers[reqMemberKey].constants.system.xuid) === "0") {
            ERR("[MPSD]getXuidForMemberInPutRequest: xuid in request was INVALID");
            response.statusCode = 400;
            return "";
        }
        return (isMeKey? requestHeaders.authorization : reqMembers[reqMemberKey].constants.system.xuid);
    } catch (err) {
...
function handles_post_setactivity(request, requestBodyJSON, response) {
    try {
        ...
        var mps = sessions_get_response(sessRef.name, response);
...
function doReqMemberPropertyUpdates(reqHeaders, reqMembers, mps, response) {
    try {
        for (var k in reqMembers) {
