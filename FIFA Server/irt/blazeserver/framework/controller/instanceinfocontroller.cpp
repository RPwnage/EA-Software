...
bool InstanceInfoController::initializeRedirectorConnection()
{
    ...
    {        
        ...
        {
            char8_t addrStr[256];
            BLAZE_ERR_LOG(Log::CONTROLLER, "[InstanceInfoController].initializeOutboundConnectionManager: could not resolve "
    ...
    else if (mBootConfigTdf.getUseRedirectorForRemoteServerInstances())
    {
        ...
        {
            BLAZE_ERR_LOG(Log::CONTROLLER, "[InstanceInfoController].initializeOutboundConnectionManager: Both redirectorSlaveAddr and remoteServerInstances are not defined in boot config -- we must have at least one defined.");
            return false;
        }
        BLAZE_INFO_LOG(Log::CONTROLLER, "[InstanceInfoController].initializeOutboundConnectionManager: Starting server without redirectorSlaveAddr, using remoteServerInstances only.");
    }
...
{
    InstanceId newInstanceId = RedisInstanceIdAllocator::getOrRefreshInstanceId(gController->getInstanceName(), gController->getInstanceId(), mBootConfigTdf.getInstanceIdAllocatorConfig().getRedisTTL());
    if (newInstanceId == INVALID_INSTANCE_ID)
    {
        BLAZE_ERR_LOG(Log::CONTROLLER, "[InstanceInfoController].controllerBootstrap: Failed to allocate instance id from redis.");
    }
...
void InstanceInfoController::onInstanceIdReady()
{
    ...
    mInstanceEnvoyResourcesRedisKey.sprintf("eadp.blaze.instances.{%s}.envoy.eds.v2", instanceName);
...
bool InstanceInfoController::startRemoteInstanceSynchronization()
{
    ...
    if (!mBootConfigTdf.getRemoteServerInstances().empty())
    {
            ...
            itEnd = mBootConfigTdf.getRemoteServerInstances().end(); it != itEnd; ++it)
        {
            ...
            if (err != ERR_OK)
            {
                BLAZE_ERR_LOG(Log::CONTROLLER, "[InstanceInfoController].controllerBootstrap: " <<
...
void InstanceInfoController::onControllerConfigured()
{
    ...
    BLAZE_INFO_LOG(Log::CONTROLLER, "[InstanceInfoController].controllerBootstrap: instance is ready to publish full instance info");
}
...
void InstanceInfoController::onControllerReconfigured()
{
    ...
    BLAZE_INFO_LOG(Log::CONTROLLER, "[InstanceInfoController].internalReconfigure: framework config has been reloaded; publishing the instance info again.");
}
...
void InstanceInfoController::refreshRedisInstanceId()
{
    ...
    InstanceId newInstanceId = RedisInstanceIdAllocator::getOrRefreshInstanceId(gController->getInstanceName(), existingInstanceId, mBootConfigTdf.getInstanceIdAllocatorConfig().getRedisTTL());
    if (newInstanceId == INVALID_INSTANCE_ID)
    {
...
void InstanceInfoController::populateEnvoyEndpoint(EnvoyEndpointMap& envoyEndpoints, const Blaze::IEndpoint* blazeEndpoint)
{
    ...
    const char8_t* resourceName = blazeEndpoint->getName();
...
bool InstanceInfoController::publishInstanceInfoToRedis()
{
    ...
    if (cluster != nullptr)
    {
        ...
        // Publish the instance version info used by the Blaze Control Plane / Envoy
        {
            ...
            char8_t versionString[BUFFER_SIZE_64];
            cmd.add(TimeValue::getTimeOfDay().toString(versionString, BUFFER_SIZE_64));
...
BlazeRpcError InstanceInfoController::fetchRemoteInstanceInfo(RemoteInstanceInfo& remoteInstanceInfo)
{
    ...
    eastl::vector<eastl::string> instanceNames;
    RedisInstanceIdAllocator::getInstanceNamesLedger(instanceNames);
    ...
    remoteInstanceInfo.mAddrList.reserve(instanceNames.size());
    for (const eastl::string& instanceName : instanceNames)
    {
        RemoteInstanceInfoFieldMap::FieldValueMap fieldValueMap;
        ...
        if (redisErr == REDIS_ERR_OK)
        {
            ...
            else if (gController->isInService())
            {
                ...
                InstanceId instanceId = RedisInstanceIdAllocator::checkAndCleanupInstanceNameOnLedger(instanceName);
