...
bool DbScheduler::parseDbNameByPlatformTypeMap(const char8_t* componentName, const DbNameByPlatformTypeMap& dbNameByPlatformTypeMap, DbIdByPlatformTypeMap& dbIdByPlatformTypeMap)
{
    ...
    typedef eastl::hash_map<uint32_t, ClientPlatformType> PlatformByDbIdMap;
    PlatformByDbIdMap platformByDbId;
    for (DbNameByPlatformTypeMap::const_iterator itr = dbNameByPlatformTypeMap.begin(); itr != dbNameByPlatformTypeMap.end(); ++itr)
    {
        ...
        PlatformByDbIdMap::insert_return_type ret = platformByDbId.insert(dbId);
        if (!ret.second)
        {
            BLAZE_ERR_LOG(Log::DB, "[DbScheduler::parseDbNameByPlatformTypeMap] db config with name '" << itr->second.c_str() << "' configured by component '" << componentName << "' either does not exist or is not unique to platform '" << ClientPlatformTypeToString(itr->first) <<
                "' (also used on platform '" << ClientPlatformTypeToString(ret.first->second) << "')");
            return false;
        }
        ret.first->second = itr->first;
    }

    // Fail if one of the db names was invalid
    for (PlatformByDbIdMap::iterator itr = platformByDbId.begin(); itr != platformByDbId.end(); ++itr)
    {
        if (gDbScheduler->getConfig(itr->first) == nullptr)
        {
            //No config.  No choice but to bail.
            BLAZE_ERR_LOG(Log::DB, "[DbScheduler::parseDbNameByPlatformTypeMap] no valid configuration found for db config name '" << dbNameByPlatformTypeMap.find(itr->second)->second.c_str() << "' configured by component '" << componentName << "' for platform '" << ClientPlatformTypeToString(itr->second) << "'");
            return false;
        }
        dbIdByPlatformTypeMap[itr->second] = itr->first;
    }
    return true;
}
