/*************************************************************************************************/
/*!
    \file   updategamename_command.cpp
...
#include "arson/rpc/arsonslave/updategamename_stub.h"
...
namespace Arson
{
class UpdateGameNameCommand : public UpdateGameNameCommandStub
{
public:
    UpdateGameNameCommand(
        Message* message, Blaze::Arson::UpdateGameNameRequest* request, ArsonSlaveImpl* componentImpl)
        : UpdateGameNameCommandStub(message, request),
        mComponent(componentImpl)
    {
    }

    ~UpdateGameNameCommand() override { }

private:
    // Not owned memory
    ArsonSlaveImpl* mComponent;

    UpdateGameNameCommandStub::Errors execute() override
    {
        ...
        BlazeRpcError err = gameManagerComponent->updateGameName(mRequest.getUpdateGameNameRequest());
...
DEFINE_UPDATEGAMENAME_CREATE()

UpdateGameNameCommandStub::Errors UpdateGameNameCommand::arsonErrorFromGameManagerError(BlazeRpcError error)
{
    ...
    switch (error)
    {
        ...
        default:
        {
            ...
            TRACE_LOG("[UpdateGameNameCommand].arsonErrorFromGameManagerError: unexpected error(" << SbFormats::HexLower(error) << "): return as ERR_SYSTEM");
            result = ERR_SYSTEM;
            break;
        }
    };
